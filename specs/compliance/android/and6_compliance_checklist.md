...
|----------|-------------|--------------|-----------------|-------|
| ETSI EN 319 401 security target | Narrative describing security objectives/assumptions for Android SDK binaries. | `specs/compliance/android/eu/security_target.md` | Revalidate every GA + LTS release. | Must cite build provenance hashes for the release train. |
...
| Device-lab reservation log | Booking workflow, approvals, capacity snapshots, and escalation ladder used to keep StrongBox pools ≥80 % during freezes. | `specs/compliance/android/device_lab_reservation.md` | Update whenever reservations are created/changed. | Reference the `_android-device-lab` ticket IDs and weekly calendar export noted in the procedure. |
| Device-lab failover runbook & drill bundle | Quarterly rehearsal plan and artefact manifest demonstrating fallback lanes, Firebase burst queue, and external StrongBox retainer readiness. | `specs/compliance/android/device_lab_failover_runbook.md` + `artifacts/android/device_lab_contingency/<YYYYMMDD>-failover-drill/` | Quarterly (or after hardware roster changes). | Log drill IDs in the evidence log and attach the manifest hash + PagerDuty export noted in the runbook. |
...
- Keep the DPIA summary aligned with the latest telemetry redaction policy and
  attach the Norito diff excerpt referenced in `specs/sdk/android/telemetry_redaction.md`.
...
- Record the bilingual cover letter template that ships with submissions inside
  `specs/compliance/android/jp/README.md` so Support can reuse it.
...
- **Quarterly drills:** exercise the runbook in
  `specs/compliance/android/device_lab_failover_runbook.md`, attach the
...
`specs/compliance/android/device_lab_contingency.md` (include date,
...
- Set `ANDROID_LINT_KEEP_WORKDIR=1` when you need to retain the temporary
  ...
  `ANDROID_LINT_SUMMARY_OUT=specs/compliance/android/evidence/android_lint_jdeps.txt`
...
3. **File evidence.** Update
   `specs/compliance/android/evidence_log.csv` and the reservation ticket
