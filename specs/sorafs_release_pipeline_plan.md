...
- `.github/workflows/sorafs-cli-release.yml` runs the strict release gate,
  ...
  release manifest.
  `fixtures/documentation/sorafs_ci.md` remains an integration example, not the
...
- `scripts/check_sorafs_production_readiness.py` is the final aggregate
  ...
  carry its exact ordered `{gate, sha256}` rows in
  `readiness_summary_sha256`, and use the same final deployment ID and
  environment as every lane. The
...
### Foundational nine-to-17 hard cut

Each prerequisite evidence package must use plural `readiness_summaries`.
Every reference is opened and validated by its authoritative bundled lane
checker; a caller-supplied digest is not evidence. For packages covering more
than one lane, `evidence_generated_at_unix` is the maximum validated
`newest_generated_at_unix` of the referenced summaries. The builder copies the
validated ordered `{gate, sha256}` rows into the signed prerequisite row's
