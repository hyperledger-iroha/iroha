...
- **Fallback behaviour.** The spec will require relays to expose a QUIC/TLS 1.3 endpoint with the same
  ...
  downgrade event. This flow is documented in `specs/soranet_handshake_plan.md`.
...
- **SNNet-1a (Handshake Plan).** Ensure Section 4 references the detailed transcript binding and ticket
  policies captured in `specs/soranet_handshake_plan.md`. Updates to capability hashing or dual-KDF
...
- **SNNet-1b (Salt Rotation Plan).** Section 7 will cite the salt rotation procedures in
  `specs/soranet_salt_plan.md`. The specification will embed the final `SaltAnnouncementV1` schema,
...
- **SNNet-1c (Capabilities Plan).** Capability TLV table in Section 6 must be derived from
  `specs/soranet_capabilities_plan.md`. Any new TLV types or GREASE behaviours require updating the
...
- **Working group cadence.** Weekly sync between protocol authors and the SNNet subteams will review:
  ...
  Notes from these syncs become part of the spec changelog (`specs/soranet/spec.md#changelog`) so
