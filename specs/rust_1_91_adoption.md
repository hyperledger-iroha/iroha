...
| Libraries | `{integer}::strict_*` arithmetic | Evaluate IVM crypto and PoW difficulty math; switch to strict ops where panics should occur | DONE | Cursor batching, trigger contract refcounts, and executor bytecode builders now rely on `strict_add`/`strict_mul` so overflow panics instead of silently wrapping (`【crates/iroha_core/src/query/cursor.rs:96】【crates/iroha_core/src/smartcontracts/isi/triggers/set.rs:812】【crates/iroha_core/src/executor.rs:1891】【crates/iroha_core/src/state.rs:8056】`); relay uptime/bandwidth accumulators use the same guards (`【tools/soranet-relay/src/incentives.rs:20】【tools/soranet-relay/src/incentives.rs:260】`). First-release SoraNet admission uses one static difficulty, so no adaptive counter or difficulty-step arithmetic remains. |
...
| Libraries | `Duration::from_mins`/`::from_hours` | Config defaults migrated (this change); propagate to Torii/Sorafs tests | DONE | Torii connect/tests + SoraFS APIs now use the helpers end-to-end (`crates/iroha_torii/src/{connect.rs,lib.rs,sorafs/**,telemetry/peers/monitor.rs}`, `crates/iroha_torii/tests/zk_vk_post_integration.rs`), and the SoraFS orchestrator timing/parity suites (`crates/sorafs_orchestrator/{src/lib.rs,tests/*}`) mirror the same pattern so minute/hour constants no longer spell out `from_secs(60 * N)`. |
...
2. **Strict arithmetic rollout** (`tracking label: crypto strict math`): ✅ Completed —
   executor/trigger/cursor paths use `strict_*`. SoraNet's first-release static
   difficulty design removed the adaptive counters and difficulty-step path.
