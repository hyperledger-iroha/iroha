...
in `nexus.md` and the CBDC onboarding plan
(`cbdc_lane_playbook.md`) by providing hands-on procedures,
...
> audit trail that regulators rely on. The data-model, CLI, Torii, event,
> telemetry, fixture, and validation surfaces described below are implemented
> in this repository.
...
The deterministic manifest/profile scaffold lets operators and SDK automation
bootstrap UAID capability bundles without hand-editing fixtures. Use
`iroha app space-directory manifest scaffold` to emit a
...
- Torii now exposes `GET /v1/space-directory/uaids/{uaid}` for operators and
  ...
  payloads; I105 strings remain the default.【torii/portfolio_api.md】
...
reviews can reference the exact payload that Torii served and preserve
deterministic enforcement evidence across all dataspaces.
...
**Runtime automation.** The core node keeps the ledger map in sync:
...
Operators seed the manifest itself; Torii and telemetry see the fresh
UAID↔dataspace view as soon as manifests
...
- Scheduler expiry/revocation paths have regression coverage, and telemetry
