...
- **Location:** Zoom (`https://meet.sora.dev/docs-devrel-sync`) with shared
  notes in HackMD or `specs/docs_devrel/minutes/<yyyy-mm>.md`
...
| Owner | Task | Artefact |
|-------|------|----------|
| Scribe | Create the month’s notes file (`specs/docs_devrel/minutes/<yyyy-mm>.md`) using the template below. | Notes file |
| Localization PM | Refresh `specs/sdk/android/i18n_plan.md#translation-status` and the staffing log; pre-fill proposed decisions. | i18n plan |
...
| Docs tooling | Export `docs/i18n/manifest.json` digests + outstanding ticket list from `specs/sdk/android/i18n_requests/`. | Manifest & ticket summary |
...
2. **Localization staffing review (15 min)**
   - Review the staffing decision log in
     `specs/sdk/android/i18n_plan.md#staffing-decision-log`.
...
Copy this skeleton into `specs/docs_devrel/minutes/<yyyy-mm>.md`:
...
1. **Minutes committed:** within 24 hours (`specs/docs_devrel/minutes/`).
