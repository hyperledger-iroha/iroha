...
The self-cert handbook (`specs/sorafs_gateway_self_cert.md`) and deployment guide reference the refusal IDs when guiding operators through troubleshooting. The `sorafs_fetch --gateway-provider …` workflow can be used to reproduce individual scenarios for documentation or training.
