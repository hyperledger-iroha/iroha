...
matrix) must ship the same artefacts before a vote is scheduled.【specs/governance_playbook.md:1】

**Intake packet**

1. **Tracking template** – copy
   `fixtures/documentation/finance/repo_governance_packet_template.md` into your evidence
...
- Reference the artefact hashes inside the governance ticket or referendum and
  ...
  outlined in the governance playbook without chasing ad-hoc paths.【specs/governance_playbook.md:8】
...
1. Tar the evidence directory (config snippet, instruction payloads, proof logs,
   ...
   `specs/sorafs_governance_dag_plan.md`.
...
|----------|----------------|---------|
| Custodian acknowledgement (`custodian_ack_<custodian>.md`) | Link to the signed note referenced in `fixtures/documentation/finance/repo_governance_packet_template.md` (use `fixtures/documentation/finance/repo_custodian_ack_template.md` as the seed). | Shows the third party accepted the repo id, custody SLA, and settlement channel before assets move. |
...
directly from the referendum minutes.【specs/governance_playbook.md:1】
...
1. **Build the governance packet**
   ...
   - Add `gar.json` (council approval record), `referendum.md` (who approved
     ...
     §§4–5.【specs/finance/repo_runbook.md:1】
...
2. **Stage and hash every instruction.** Generate initiation/margin/unwind
   ...
   (store under `hashes/`) so `fixtures/documentation/finance/repo_governance_packet_template.md`
...
5. **Assemble the packet.** Copy the template from
   `fixtures/documentation/finance/repo_governance_packet_template.md`, fill the metadata,
...
- **ISO 20022 mapping:** When building ISO envelopes or reconciling external
  ...
  `specs/finance/settlement_iso_mapping.md` (`colr.012`, `sese.023`,
...
- Reference `specs/finance/settlement_iso_mapping.md` when coordinating
