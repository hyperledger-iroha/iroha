...
- `AppealPricingConfig::from_manifest_value` loads JSON fixtures such as
  `fixtures/documentation/ministry/appeal_pricing_config_baseline.json` for offline
...
- `AppealSettlementConfig::baseline_v1()` and
  ...
  fixture `fixtures/documentation/ministry/appeal_settlement_config_baseline.json`.
...
cargo run --locked -p sorafs_orchestrator --bin sorafs_cli -- \
  ...
  --panel-size=7 \
  --format=json \
  --config=fixtures/documentation/ministry/appeal_pricing_config_baseline.json
```
...
cargo run --locked -p sorafs_orchestrator --bin sorafs_cli -- \
  ...
  --panel-size=7 \
  --format=json \
  --config=fixtures/documentation/ministry/appeal_settlement_config_baseline.json
```
