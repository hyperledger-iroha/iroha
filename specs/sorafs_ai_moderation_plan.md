...
runnable fixture
(`fixtures/documentation/ai_moderation_perceptual_registry_202602.json`) is validation
...
- **Metrics:** Compute Brier score, Expected Calibration Error (ECE), and AUROC
  ...
  `specs/sorafs/reports/ai_moderation_calibration_<YYYYMM>.md`.
...
- Differential privacy is NOT applied here; gateways rely on quarantine + audit
  workflows instead. Redaction policies follow `specs/sorafs_gateway_compliance_plan.md`.

## 11. Calibration Publication (2026-02)
- **Manifest:** `fixtures/documentation/ai_moderation_calibration_manifest_202602.json`
...
- **Scoreboard:** `fixtures/documentation/ai_moderation_calibration_scorecard_202602.json`
  plus the human-readable report in
  `specs/sorafs/reports/ai_moderation_calibration_202602.md` capture Brier,
...
- Operators can invoke the shared validator via
  ...
  `fixtures/documentation/ai_moderation_calibration_manifest_202602.json` or the raw
...
- **Documentation lead** mirrors the JSON artefact under
  `fixtures/documentation/ai_moderation_perceptual_registry_<YYYYMM>.json` and registers
...
`specs/sorafs/reports/ai_moderation_calibration_<YYYYMM>.md` alongside the
...
| Publication | T±0 | Mirror the JSON + Norito artefacts to SoraFS, update `fixtures/documentation/` with the cohort file name, and circulate the CID + digest to gateway operators. |
| Post-publication check | T+1 day | Observability verifies that both regional gateways report the promoted catalog sequence through the governed compliance status API (see Section 14.4) and files a signed note in `specs/sorafs/reports/ai_moderation_corpus_<YYYYMM>.md`. |
...
3. Add the new cohort to `fixtures/documentation/ai_moderation_perceptual_registry_<YYYYMM>.json`,
