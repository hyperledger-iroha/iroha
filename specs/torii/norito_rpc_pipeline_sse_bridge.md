...
2. **Queue error envelope** – Queue failures are mapped to `ErrorEnvelope { code, message, details { queue, retry_after_seconds, reject_code, entrypoint_hash?, tx_hash? } }` via `Error::queue_error_envelope`. Indeterminate durability outcomes always identify the canonical entrypoint and include the signed-transaction hash only when one exists. Codes include `queue_full`, `per_user_queue_limit`, `already_enqueued`, `transaction_expired`, etc.【crates/iroha_torii/src/lib.rs:11076】【crates/iroha_torii/src/lib.rs:11090】 SDKs must parse the machine-readable `code` first and only retry when the envelope advertises `retry_after_seconds`.
