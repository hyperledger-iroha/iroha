...
Canonical specification: `specs/torii/nrpc_spec.md`
...
SoraNet handshake guide (`specs/soranet_handshake.md`).
...
| --- | --- | --- | --- | --- |
| Signed queries | `POST /v1/query` | `SignedQuery` via `NoritoVersioned`; its signed payload contains mandatory `NetworkId`, authority, creation time, nonzero TTL, 32-byte nonce, and exact query request | Norito or JSON query result depending on `Accept` | Torii validates exact genesis lineage and freshness, verifies the signature, then atomically consumes the nonce before authorization/execution. Bodies are one-shot: clients must not retry or follow redirects with the same signed bytes after dispatch. |
...
- **Configuration**: Operators control the rollout via `torii.transport.norito_rpc`
  ...
  `specs/config/client_api.md` for a detailed reference.
