...
Source of truth: This document + `specs/torii/norito_rpc.md`  
Related roadmap items: NRPC-1 (spec), NRPC-2 (rollout), NRPC-3 (SDK adoption)

### 0. Normative References
- [`specs/torii/norito_rpc.md`](norito_rpc.md) — background RFC and design rationale.  
- [`specs/torii/norito_rpc_rollout_plan.md`](norito_rpc_rollout_plan.md) — deployment/runbook guardrails.  
- [`specs/torii/norito_rpc_telemetry.md`](norito_rpc_telemetry.md) — observability contract.  
- [`specs/torii/norito_rpc_tracker.md`](norito_rpc_tracker.md) — action tracker.  
...
| Routes | No new endpoints. Every existing Torii handler accepts Norito payloads identical to the JSON DTOs enumerated in `specs/torii/app_api_parity_audit.md`. |
...
- The default global pre-auth cap is 1024 in-flight requests and is clamped to half of the available `RLIMIT_NOFILE` budget at runtime. The per-IP cap is 256, leaving room for a solo client's next heavy-query wave to wait behind the 64 execution slots. Capacity rejections neither spend rate tokens nor impose an IP cooldown. Global and per-scheme saturation return `503`; the per-IP concurrency quota returns `429`.
- Active connections per scheme are exported via `torii_active_connections_total{scheme="norito_rpc"}` and rejections are counted in `torii_pre_auth_reject_total{reason}` (`crates/iroha_core/src/telemetry.rs:3423`, `specs/torii/norito_rpc_telemetry.md:32`). Dashboards and the canary runbook already alert on these gauges (`specs/runbooks/torii_norito_rpc_canary.md:75`); clients must never open best-effort connections beyond the configured budget.
- Client expectations:
  ...
  2. Bound in-flight Norito calls. SDKs SHOULD gate concurrency locally and surface `429/503` responses that include `Retry-After` or `X-Iroha-Error-Code: preauth_rate_limited|preauth_scheme_capacity`.
  3. Honor `Retry-After` before retrying the rejected request; preserve reusable connections. Rate exhaustion recovers on token refill by default. Operators may set `torii.preauth_ban_duration_ms` for an additional cooldown; responses report its remaining duration rounded up to seconds. Capacity rejection and requests during an existing cooldown never extend that cooldown.
...
- **Admission path**: the pre-auth middleware tags a request as `ConnScheme::NoritoRpc` only when it declares exactly one canonical, parameter-free `Content-Type: application/x-norito` value (the legacy transaction/query paths remain protocol-classified) and runs the gate before routing. Look-alike, parameterized, malformed, and duplicate media values remain in the ordinary HTTP pool and are rejected by their route's media boundary. `evaluate_norito_rpc_gate` enforces the `enabled` flag, the mTLS requirement, and the stage logic, reads the allowlist via `HEADER_API_TOKEN`/`X-API-Token`, and returns `403` responses stamped with `X-Iroha-Error-Code: norito_rpc_disabled|norito_rpc_mtls_required|norito_rpc_canary_denied`. SDKs MUST treat those errors as downgrade signals.
...
- Breaking changes require simultaneous updates to `specs/torii/norito_rpc_rollout_plan.md` and `status.md`, plus a minimum of one release cycle notice.
