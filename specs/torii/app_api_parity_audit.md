...
- App API pagination/backpressure honours the flat `torii.app_api_default_list_limit`, `torii.app_api_max_list_limit`, `torii.app_api_max_fetch_size`, and `torii.app_api_rate_limit_cost_per_page` keys. Account assets/transactions and asset-holder listings bound `limit`/`fetch_size` and charge one configured unit per default-sized page, rounding partial pages up with a one-page minimum. Defaults charge one unit for up to 100 rows and five for 500 rows. The same endpoints accept an optional `asset_id` query parameter for pre-filtering.
