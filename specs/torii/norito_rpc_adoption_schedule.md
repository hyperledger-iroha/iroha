...
2. **Regeneration cadence.** Regenerate fixtures every Wednesday at 17:00 UTC (or within 48 h of any data-model change). Ownership rotates weekly between the SDK Program Lead and the Android Networking TL and is tracked in the NRPC-4 row of `specs/torii/norito_rpc_tracker.md`. During the governance call, run `cargo xtask norito-rpc-fixtures` to refresh `.norito` payloads, the manifest, and `schema_hashes.json`, then sync the manifest into each SDK repo via the existing helper scripts.
3. **CI hooks.**  
