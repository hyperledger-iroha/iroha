...
- The operator runbook now lives at [`specs/telemetry.md#norito-rpc-degraded-runbook`](../telemetry.md#norito-rpc-degraded-runbook) and covers detection signals, log review, proxy/MTU validation, SDK mock-harness correlation, and the brownout procedure (`torii.transport.norito_rpc.stage`). Keep that section updated whenever instrumentation or rollout knobs change.
