...
`specs/soranet/testnet_rollout_plan.md` into an actionable onboarding
...
3. **Telemetry**
   - Prometheus/OTLP exporters for `sorafs.fetch.*`, `soranet_privacy_*`, and
     ...
     `specs/soranet/testnet_rollout_plan.md`.
...
`fixtures/documentation/soranet_testnet_operator_kit/`). Edit only the operator input
sections:
...
1. **Metrics snapshot**
   - Export counters matching the schema used by
     `fixtures/documentation/soranet_testnet_operator_kit/07-metrics-sample.json`.
   - Run the validator:
     ```bash
...
- **Helpdesk:** Continue to route questions and promotion requests through the
  governance helpdesk noted in `specs/soranet/testnet_rollout_plan.md`.
- **Status cadence:** Once admitted, submit weekly deltas using
  `specs/status/soranet_testnet_weekly_digest.md` to keep the SNNet-10
