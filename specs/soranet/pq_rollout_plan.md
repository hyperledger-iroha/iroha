...
- Rollback hooks and references to the fire-drill runbook
  (`specs/soranet/pq_ratchet_runbook.md`).
...
See `fixtures/documentation/sorafs_rollout_stage_b.toml` and
`fixtures/documentation/sorafs_rollout_stage_c.toml` for ready-to-apply snippets.
...
- Reference the telemetry plan in the promotion ticket to prove
  `specs/soranet/snnet16_telemetry_plan.md` remains the canonical source
...
- Keep the vocabulary in sync with `specs/soranet/snnet16_telemetry_plan.md`
...
2. Set `rollout_phase = "canary"` (or override with `anonymity_policy anon-guard-pq`)
   ...
   `specs/soranet/pq_ratchet_runbook.md` to prove the downgrade pipeline.
...
- Reference `specs/ops/soranet_transport_rollback.md` whenever a demotion
