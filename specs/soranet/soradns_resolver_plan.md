...
are logged in `specs/sorafs_gateway_dns_design_runbook.md` as part of the
...
- **Evidence:** weekly `status.md` entry summarising QPS, privacy incidents, and
  proof updates. `specs/sorafs_gateway_dns_design_gar_telemetry.md` now
...
| Staging bake | Run recursive/authoritative nodes in staging, attach Grafana dashboard, and exercise EDNS/DNSSEC suites. | `specs/soranet/reports/soradns_staging_bake.md`. |
| GAR review | Submit ZRH proofs and privacy telemetry to GAR reviewers; attach NDJSON logs + dashboards. | `specs/soranet/gar_compliance_playbook.md` appendix. |
| Operator training | Deliver runbook briefing using `specs/sorafs_gateway_dns_design_runbook.md` plus this plan; capture attendance/quiz metrics. | `specs/sorafs_gateway_dns_design_attendance.md`. |
 | Production enablement | Flip traffic via the gateway CDN change tickets, enable alerts, and log the adoption date in `status.md`. | `specs/project_tracker/soradns_rollout/2026Q4.md`. |
