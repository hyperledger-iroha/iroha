...
A rendered copy lives in `fixtures/documentation/soranet_testnet_operator_kit/` (see the
...
`specs/soranet/snnet10_beta_onboarding.md` summarises prerequisites,
...
`specs/soranet/snnet10_stage_gate_template.md`, which also links to the
ready-to-copy Markdown form stored under
`fixtures/documentation/soranet_testnet_stage_gate/stage_gate_report_template.md`.
...
- ✅ `guard_directory` points at the latest `GuardDirectorySnapshotV2` artefact
  and `expected_snapshot_digest_hex` matches the exact domain-separated digest
  distributed through the independent governance channel (relay startup also
  enforces the snapshot and certificate validity windows).
...
- **Status digest:** publish weekly updates summarising relay count, PQ ratio,
  ...
  `specs/status/soranet_testnet_digest.md` once the cadence starts).
...
- `cargo xtask soranet-testnet-kit [--out <dir>]` materialises the onboarding kit
  ...
  `fixtures/documentation/soranet_testnet_operator_kit/`).
- `cargo xtask soranet-testnet-metrics --input <metrics.json> [--out <path|->]`
  ...
  `fixtures/documentation/soranet_testnet_metrics_sample.json`.
...
- The downgrade communication template for SDK/portal messaging resides in
  `specs/soranet/templates/downgrade_communication_template.md`.
- Weekly status digests should use
  `specs/status/soranet_testnet_weekly_digest.md` as the canonical form.
