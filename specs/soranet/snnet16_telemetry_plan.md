...
3. **Privacy dashboard cross-links.** The privacy board described in
   `specs/soranet/privacy_metrics_pipeline.md` now embeds the SN16
...
- `ProxyPolicyEventBuffer` (see `tools/soranet-relay/src/privacy.rs`) already
  ...
  The Sorafs orchestrator guide (`specs/sorafs_orchestrator_rollout.md`)
...
| Proxy toggle telemetry | Networking TL | Completed — the rollout guide (`specs/sorafs_orchestrator_rollout.md`) now documents the `/policy/proxy-toggle` workflow, downgrade_remediation block, and `sorafs_cli proxy set-mode` evidence flow on top of the existing gauge + schema updates. |
