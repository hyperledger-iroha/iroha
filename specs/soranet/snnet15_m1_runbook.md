...
- Command (per PoP):
  ...
  cargo xtask soranet-popctl \
    ...
    --image-tag <image_tag>
  ```
- Outputs to attach: `bundle_manifest.json`, `attestations/signoff.json`, FRR config, resolver template, EDNS/DS evidence (if not skipped), PXE/env/secrets templates, CI job stub, sigstore provenance stub. Use the bring-up checklist in `specs/soranet/popctl_bringup_checklist.md` to gate promotion.
...
- Command (per PoP):
  ...
  ```
- Outputs: `gateway_edge_h3.yaml`, `gateway_trustless_verifier.toml`, `gateway_waf_policy.yaml`, `gateway_m0_summary.json`. Update origin blocks and TLS/ECH vault refs before deployment. Reference shape in `specs/soranet_gateway_m0.md`.
