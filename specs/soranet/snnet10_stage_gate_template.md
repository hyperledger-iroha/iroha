...
- [`specs/soranet/testnet_rollout_plan.md`](testnet_rollout_plan.md) —
  authoritative scope, metrics, and verification checklist.
- [`specs/soranet/snnet10_beta_onboarding.md`](snnet10_beta_onboarding.md)
...
7. **Change management artefacts (T1 → T2 only)**
   ...
   - Signed rollback plan (DNS + guard cache invalidation + SDK comms) referring
     to `specs/soranet/templates/downgrade_communication_template.md`.

## Report structure

Mirror the following structure in your submission. A ready-to-copy Markdown file
lives in `fixtures/documentation/soranet_testnet_stage_gate/stage_gate_report_template.md`.
...
5. Once approved, archive the signed package in your CMDB and reference it in
   the next weekly digest (`specs/status/soranet_testnet_weekly_digest.md`).

## Example files

Copy `fixtures/documentation/soranet_testnet_stage_gate/stage_gate_report_template.md`
...
`fixtures/documentation/soranet_testnet_stage_gate/feed-sample.json`.
