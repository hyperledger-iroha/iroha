...
Permissioned and NPoS deployments use the same state machine; they differ only
in how the active validator set is sourced and how quorum is calculated.

Overview
- State machine: each height/view moves through Proposal, Prepare, Commit, and
  local finality. Validators prepare-vote once for a valid proposal satisfying
  the locked-QC rule, a PrepareQC updates the lock and authorizes Commit votes,
  and a CommitQC finalizes only when the block payload is locally available.
- Roles and rotation: validator ordering, leader selection, timeout cadence, and
  quorum policy derive only from chain state and configuration. Telemetry never
  changes signed consensus decisions.
- Fault tolerance and quorum: permissioned mode uses strict count
  supermajority `floor(2n / 3) + 1`; for `n <= 3` this naturally requires all
  validators. NPoS mode uses strict stake supermajority
  `signed_stake * 3 > total_stake * 2`; missing stake, arithmetic overflow, and
  zero total stake fail closed. Set A is always `min_votes_for_commit()`; adjust
  the validator roster via `trusted_peers` or the NPoS stake roster.
...
- Certificates: proposals carry height, view, epoch, validator-set id, parent
  block, block hash, payload hash, and highest QC. Prepare, Commit, and NewView
  certificates are Norito-encoded with `PROTO_VERSION = 1`. A timeout emits
  NewView; the next leader proposes from the highest QC certified by the
  new-view quorum.
- Reconfiguration: permissioned/NPoS changes are epoch-boundary validator-set
  changes. The old validator set finalizes the reconfiguration block, and the
  new validator set starts at the next height. Mid-round mode flips are not a
  protocol transition.
- DA/RBC: RBC is deterministic DA payload transport and repair, not the
  certified-block recovery gate. A CommitQC is the consensus proof; if it
  arrives before the payload, the node advances certified consensus state,
  directly requests the exact certified block from the QC validator set, and
  applies the block only after local validation confirms the certified roots.

### Safety and Liveness (Proof Sketch)

This is a proof sketch for Sumeragi V1 (not a formal proof). The pure state
machine is modeled in `crates/iroha_core/src/sumeragi/engine.rs`; networking,
workers, storage, and telemetry are adapters around the deterministic
transitions.
...
Safety (permissioned)
- Quorum intersection: permissioned quorums use `floor(2n / 3) + 1`; any two
  quorums intersect in more than one third of the validator set, so at least
  one honest validator is in the intersection when fewer than one third are
  Byzantine.
...
- Once the network is synchronous and an honest leader is selected for a view, proposals and
  commit votes propagate to collectors/topology; `floor(2n / 3) + 1` votes produce a commit QC
  and the block is committed when the payload is available.
...
- The pacemaker logic is the same, but commit QCs require >2/3 stake; liveness holds when
  more than two thirds of stake (by the active roster) is online and able to exchange votes.
...
[sumeragi.collectors]
k = 3
redundant_send_r = 3 # example strict supermajority for 4 validators
...
4. **Archive evidence** — every readiness rehearsal must attach the CLI summaries, JSON snapshots, and SSE capture referenced in the runbook plus the generated Markdown reports (`specs/generated/sumeragi_*_report.md`). Keep the artefact directory path recorded in `status.md` and `specs/project_tracker/npos_sumeragi_phase_a.md` so governance reviewers can re-download the same run.
...
- `sumeragi_da_gate_block_total{reason}` and `sumeragi_rbc_da_reschedule_total` — monitor DA availability warnings on missing local payloads; reschedule counters should remain zero because missing-local-payload handling now uses DA gate transitions and bounded recovery instead of deadline reschedules.
  ...
  - `sumeragi_rbc_deliver_defer_{ready,chunks}_total` and `/v1/sumeragi/telemetry.rbc_pending` + `rbc_backlog` expose READY quorum vs chunk-gate stalls, stash totals by reason, drops, and pending session counts so dashboards can alarm on stuck RBC sessions before DA/commit stalls.
  - `sumeragi_rbc_mismatch_total{peer,kind}` tracks per-peer chunk digest/payload hash/chunk root mismatches; mismatched payloads are dropped (no penalties) and logs are throttled per peer/kind to keep behavior deterministic.
  - `sumeragi_consensus_message_handling_total{kind,outcome,reason}` captures drop/deferral reasons for BlockCreated, BlockSyncUpdate, CertifiedBlockFetch, ProposalHint, Proposal, QcVote, Qc, VrfCommit, VrfReveal, ExecWitness, RbcInit, RbcChunk, RbcReady, RbcDeliver, FetchPendingBlock, and Evidence to triage missing-payload warnings and backpressure drops.
...
- Evidence helpers:
  - `GET /v1/sumeragi/evidence` and `GET /v1/sumeragi/evidence/count` carry the same read-only DTOs surfaced in the JS/Python SDKs. Evidence admission remains on the authenticated consensus peer path; Torii, the CLI, and SDKs expose no mutation API.
...
| CLI `iroha --output-format text ops sumeragi vrf-epoch`, `iroha --output-format text ops sumeragi vrf-penalties` | `finalized=true`, participant count equals roster size, penalty lists reflect drills only | Every epoch emits a seed and penalty set that matches the staking roster/governance intent | When counts drift or unexpected penalties appear, follow {doc}`sumeragi_randomness_evidence_runbook` to repair the validator's authenticated peer participation and capture artefacts for governance |
...
| HTTP `/v1/sumeragi/evidence/count` and CLI `iroha ops sumeragi evidence count` | Monotonic growth when faults occur; zero drift between peers | If counts diverge, compare paginated evidence lists and authenticated peer-ingress logs, then inspect Alertmanager snapshots |
...
- **Evidence observation stalls:** Diverging `iroha ops sumeragi evidence count` outputs or a flat `sumeragi_evidence_records_total` time series indicates peer admission, persistence, or audit projection is failing.
  1. Compare `evidence count` across at least two validators.
  2. Compare the corresponding paginated evidence records and authenticated consensus-ingress logs; do not inject evidence through Torii.
  3. If the divergence persists, capture the `count` and list outputs plus Alertmanager silence IDs, then roll back to the last known-good build.
...
- Validators must use BLS-Normal public keys and present a proof-of-possession (PoP). In NPoS, the consensus roster is filtered to the active public-lane validator set whenever stake records are available (including after commit-topology updates); if the commit topology omits active validators, the roster is widened with the active set before PoP filtering so the full validator set participates. When no active validators are present it falls back to `trusted_peers` after filtering out peers without BLS-Normal keys. If `trusted_peers_pop` is empty, the BLS trusted-peer set remains the validator roster. When `trusted_peers_pop` is present, it defines the validator subset among trusted BLS peers: peers with valid PoPs participate in consensus, while trusted peers without PoPs remain network-trusted observers and are excluded from consensus. Invalid PoP entries are rejected, non-BLS peers are always excluded, and the node `public_key`/`private_key` pair must be BLS-Normal for transport and consensus.
...
- Validators sign the proposed block header and send `CommitVote` to the deterministic collector set; votes fall back to the full commit topology only when collectors are disabled or local-only. Any collector that reaches quorum aggregates `floor(2n / 3) + 1` signatures (permissioned) or >2/3 total stake (NPoS) into a `CommitCertificate` and gossips it.
...
K / r Parameters
- Config keys: `sumeragi.collectors.k: usize` (collectors per height; default 1 in config) and `sumeragi.collectors.redundant_send_r: u8` (redundant send fanout; tooling may seed on-chain defaults from the validator count, and the config fallback is 3 for 4 validators when parameters are absent).
...
Adapter Model
- The Sumeragi main loop owns network queues, workers, telemetry, payload transport, and storage adapters. Shared consensus decisions belong to the pure V1 state machine: adapters feed inputs such as ticks, proposals, certificates, payload availability, validation results, and committed blocks, then execute the emitted outputs.

Backpressure & Telemetry
- Scheduler: with `sumeragi.advanced.worker.parallel_ingress = true`, per-queue worker threads feed a ticketed gate (one consensus step at a time, FIFO by ticket) instead of the single-thread priority mailbox; the priority mailbox (Votes → RBC chunks → blocks → block payloads → consensus control → lane relay → background) with starvation guards (`non_vote_starve_max`, `block_rx_starve_max`) is used only when `parallel_ingress = false`.
...
- See `specs/telemetry.md` and the README “Consensus metrics (Sumeragi)” section for metric names and example PromQL.
...
- RBC DELIVER carries the sender-observed READY signature bundle (sender index + signature). Receivers validate and record those signatures to recover READY quorum when READY messages were missed. An inbound DELIVER that still lacks READY quorum is deferred unless the node already has the block payload from a local authoritative source (pending, in-flight, or committed storage); a chunk-complete RBC session alone does not bypass the READY quorum gate. The `sumeragi.debug.rbc.force_deliver_quorum_one` test override only lowers local DELIVER emission; inbound DELIVER acceptance still uses the full commit-topology quorum, and a deferred inbound DELIVER does not trigger same-turn local DELIVER emission.
...
- DA availability: when `SumeragiParameter::DaEnabled = true`, availability evidence is tracked locally via RBC `READY` quorum (>= `min_votes_for_commit()`) and availability vote aggregation. Commit/finalize does not wait for availability evidence. A commit QC for a missing block now drives direct certified-block fetch; RBC `DELIVER` remains DA plumbing and an opportunistic payload source, but is not a prerequisite for following the commit QC.
- Certified-block recovery: `BlockMessage::CertifiedBlockFetch` carries exact commit-QC subjects. Requests are keyed by `(requester, height, view, block_hash)`, and responses carry `(height, view, block, commit_qc, validator_checkpoint, stake_snapshot)`. Receivers verify the height/view/hash and require the commit QC plus validator checkpoint to certify exactly the returned block before materializing it as pending. This can make certified/QC height temporarily exceed the locally applied block height; that is normal catch-up state, not a consensus failure. State application remains gated on local block validation and root checks against the certificate.
...
- Prepare-only fallback: if only a prepare certificate reaches quorum and no commit votes arrive by the quorum timeout, the adapter requeues the block’s transactions, rebroadcasts the block + block-sync update + prepare certificate, resets a highest certificate reference that pointed at the stalled block, and triggers a view change so the next round can propose a fresh block without stalling (applies to both DA-off permissioned runs and DA-off NPoS smoke tests).
...
- Kura persistence retry/backoff: when `kura.store_block` returns an error the block stays pending and retries with exponential backoff derived from `sumeragi.persistence.kura_retry_interval_ms`. After `sumeragi.persistence.kura_retry_max_attempts` the storage adapter aborts the payload and requeues its transactions so new proposals can make progress. Status counters (`status_snapshot().kura_store.*`) and telemetry (`sumeragi_kura_store_failures_total{outcome="retry|abort"}`) surface failures for alerting, and WSV remains untouched until persistence succeeds.
  - Safety-critical lane sidecars do not inherit Kura's batched fsync window. Lane payload ownership crosses the complete data/index/directory barrier before the canonical block marker can publish; recovered execution inputs and direct-execution preflights cross the same barrier before READY or a durable application receipt may depend on them. Rebuildable pipeline and diagnostic sidecars continue to follow the configured `kura.fsync_mode`.
...
- For chaos/performance validation, run the targeted stress scenarios via
  ...
  See `specs/sumeragi_chaos_performance_runbook.md` for the complete
...
- `/v1/sumeragi/telemetry` (or `iroha_cli --output-format text ops sumeragi telemetry`) — compact
  JSON snapshot with per-collector vote counts, certificate latency, RBC backlog/pending stash
  counters, and the latest VRF epoch summary. Authoritative reducer/QC identity remains on
  `/v1/sumeragi/status`.
...
- Import `specs/grafana_sumeragi_overview.json` for a ready-made Grafana
...
5. If both collectors are idle, inspect RBC backlog gauges to confirm payloads
  ...
  `iroha --output-format text ops sumeragi status` reports the authoritative v2
...
are surfaced via `/v1/sumeragi/telemetry.rbc_pending.{drops_cap_total,drops_cap_bytes_total,drops_ttl_total,drops_ttl_bytes_total,drops_bytes_total,evicted_total,max_*}`
and `iroha --output-format text ops sumeragi telemetry`; sustained movement should trigger alerts
and a manifest/collector audit. Stash counters (`/v1/sumeragi/telemetry.rbc_pending.stash_*`)
...
Enable `sumeragi.adaptive_observability` to let the adapter temporarily widen
...
- Trigger sources: certificate latency over `qc_latency_alert_ms` or bursts of missing
  ...
  advances by `da_reschedule_burst` between ticks.
- Mitigation: raise redundant collector fan-out to
  `collector_redundant_r.max(baseline)` and add `pacemaker_extra_ms` to the
  proposal interval; the adapter logs the decision with the measured metrics.
...
- Validators automatically broadcast authenticated `VrfCommit` and `VrfReveal` frames to the commit
  topology when the commit/reveal windows open and the node is part of the
  active validator set.
- Peer ingress authenticates the sender and preserves consensus provenance;
  Torii has no commit or reveal mutation endpoint.
- The main-loop adapter (`handle_vrf_commit`/`handle_vrf_reveal`) checks the local
...
- Repair a validator's peer connectivity or process health when it misses a
  commit or reveal. Operators cannot forge consensus provenance through Torii.
...
- Before the reveal deadline, watch `vrf.reveals_total` and `vrf_late_reveals_total`. If a reveal is missing, page the validator and restore its authenticated peer broadcast path before the deadline.
...
- `sumeragi_vrf_commits_emitted_total` /
  `sumeragi_vrf_reveals_emitted_total` — authenticated consensus frames
  broadcast by this validator.
...
- For `no_participation` increments, contact the affected validator and verify that its authenticated peer-transport logs show VRF broadcasts. If the validator was offline, stage a joint reconfiguration or jailing proposal before the next epoch to keep quorum guarantees intact.
- For `non_reveal` increments, collect the stored commitment from the epoch snapshot (`commitments[*].commitment_hex`), repair the validator, and have it rebroadcast the reveal through the authenticated peer protocol. Confirm `sumeragi_vrf_reveals_late_total` increments and `vrf_committed_no_reveal_total` drops back to zero after the late broadcast.
...
- `status.da_gate.last_satisfied` records `missing_data_recovered` when the local payload becomes available and `manifest_guard_recovered` when required manifest material arrives and passes the guard. Commit does not depend on local RBC delivery; RBC is transport/recovery and is tracked separately via the RBC endpoints and metrics.
...
| VRF penalties creeping up | Prometheus alerts on `increase(sumeragi_vrf_no_participation_total)` / `increase(sumeragi_vrf_non_reveal_penalties_total)` or CLI telemetry shows growing penalties. | Follow {doc}`sumeragi_randomness_evidence_runbook` to pull the per-epoch participation table, repair the validator's authenticated peer broadcast, and confirm the `prf.epoch_seed` remained stable. |
...
- **Filter** — refine the snapshot with `--kind DoublePrepare` / `DoubleCommit` / `InvalidQc` / `InvalidProposal` / `Censorship` / `SumeragiV2Equivocation` and paginate via `--limit` / `--offset` when auditing large incident windows.
...
- Record the current count via `iroha_cli ops sumeragi evidence count` before an authenticated consensus fault drill; the value should increase only after valid evidence is admitted.
- After admission, poll `iroha_cli --output-format text ops sumeragi evidence list` and confirm the new record's subject and consensus-admission heights match the committed proof.
...
- If peer ingress rejects evidence, inspect the structured consensus log and cross-check the underlying votes or proposal. No state change should occur; the count remains unchanged by design.
...
CI coverage keeps the evidence pipeline honest: `crates/iroha_core/src/sumeragi/evidence.rs`
round-trips every negative mutation through the Norito codec before feeding it
to the validator so encode/decode cannot "heal" malformed payloads. The
fuzz-style loop jitters signatures and block hashes to guard the deduplication
keys, and exact v2 tests cover authenticated admission, horizon enforcement,
ordering, proof validation, and committed-height binding.
