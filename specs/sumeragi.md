...
- Docker Compose: point `--config-dir` at the same localnet output and run `kagami docker --peers 4 --config-dir ./npos-local --image hyperledger/iroha:dev --out-file docker-compose.npos.yml --consensus-mode npos --no-banner --print` to emit a Compose file whose one-shot signer publishes the signed genesis and exact bound manifest into a shared volume before any validator starts. `GENESIS_CONSENSUS_MODE` overrides are applied by that signer (add the `GENESIS_NEXT_CONSENSUS_MODE`/`GENESIS_MODE_ACTIVATION_HEIGHT` pair only on Iroha2 staged networks).
