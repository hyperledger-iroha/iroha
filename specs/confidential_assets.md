...
1. Each attempt negotiates a fresh mandatory SoraNet NK2/NK3 session key inside TLS 1.3 or QUIC. The signed payload (`handshake_signature_payload`) uses an explicit V1 domain and binds the complete 256-bit domain-separated session hash, identity algorithm and public key, Norito-encoded advertised address, relay/consensus/confidential/crypto/trust capabilities, mandatory exact genesis-derived `NetworkId`, and mandatory listener-observed TLS/QUIC certificate fingerprint. The message is AEAD-encrypted before it leaves the node.
