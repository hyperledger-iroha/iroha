...
`specs/fastpq_plan.md` (targets/architecture) and
`specs/fastpq_migration_guide.md` (node-level upgrade steps) by focusing
...
| Wrapped benchmarks (`fastpq_metal_bench_*.json`, `fastpq_cuda_bench_*.json`) | Exact producer identity, complete six-lane operations, actual device parity/counters, host metadata and row usage. | Capture on each actual GPU host, wrap and sign using `scripts/fastpq/wrap_benchmark.py`; follow [the V1 benchmark contract](fastpq_benchmark_v1.md). Keep generic FFT/LDE and optional runtime scrape evidence under their own owners. |
...
1. **Capture GPU benchmarks.**
   ...
   - Wrap with `scripts/fastpq/wrap_benchmark.py --row-usage <decoded-witness> --require-lde-mean-ms 950 --sign-output`. Apply `--require-digest384-columns-mean-ms <reviewed-limit-ms>` only with a reviewed six-lane target. Missing or inconsistent six-lane evidence rejects the bundle.
...
- Run `dashboards/alerts/tests/fastpq_acceleration_rules.test.yml` (via
  ...
  compile.
- Validate the generated bundle with `ci/check_fastpq_rollout.sh` (or
  `make check-fastpq-rollout`) and pass `FASTPQ_ROLLOUT_BUNDLE=<path>` when you
  want to target a single rollout. No dedicated FastPQ rollout workflow or
  reference bundle is checked in; release automation must invoke the gate with
  the supplied path. The release pipeline can archive validated bundles
