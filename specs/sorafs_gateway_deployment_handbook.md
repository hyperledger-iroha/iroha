...
- **Out of scope:** SoraFS provider economics, PoR sampling design, or Torii core upgrade procedures. See `specs/sorafs_*.md` for complementary design notes.
...
CLI or point the script to `fixtures/documentation/sorafs_direct_mode_smoke.conf` (update the manifest hash
and stream-token entries before running):

```bash
./scripts/sorafs_direct_mode_smoke.sh \
  --config fixtures/documentation/sorafs_direct_mode_smoke.conf \
...
`fixtures/documentation/sorafs_direct_mode_policy.json`, records the fetch summary, and
...
- **Self-cert validation**: run `specs/sorafs_gateway_self_cert.md` workflows before onboarding operators.
