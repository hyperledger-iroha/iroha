...
`specs/sorafs_orchestrator_rollout.md` is live and that clients default to
...
- **Change window:** Align with the SNNet-10 stage-gate window for “Public Beta → Mainnet Default” (`specs/soranet/snnet10_stage_gate_template.md`).
...
- **Related tickets / RFCs:** SF-6c adoption checklist, SNNet-5 rollouts, SNNet-5a direct-mode fallback pack (`specs/sorafs/direct_mode_pack.md`), PQ ratchet drill log (`specs/soranet/pq_ratchet_runbook.md`), CI helper `ci/check_sorafs_orchestrator_adoption.sh`.
...
- **Telemetry redaction/compliance request:** Governance or compliance demands direct mode (for example, regulator prohibits anonymous relays for a jurisdiction). Detection: `specs/sorafs/direct_mode_pack.md` checklist, ticket prefix `SNNET-DIR-***`.
...
- **Communication drafts:** Pre-fill the operator + governance templates stored in `specs/sorafs/direct_mode_pack.md` (`Announce Direct Mode` section) and keep the status-page copy staged.
...
| 4 | Redeploy SDK/CLI defaults (JS/Swift/Python) with `transportPolicy: "direct-only"` or `policyOverride.transportPolicy = "direct-only"` while attaching the incident ticket to release notes. | SDK owners | Refer to `specs/sorafs/direct_mode_pack.md` guidance for each SDK. |
...
| Incident log | `specs/ops/archive/<incident-id>/` | SRE duty | Store `scoreboard.json`, `summary.json`, adoption report, guard directory hash, Alertmanager exports, and link to `specs/sorafs/direct_mode_pack.md` justification. |
