...
2. Run the generator:
   ...
   cargo xtask sns-scorecard \
     ...
     --output-json fixtures/documentation/sns/steward_scorecard_2026q1.json \
     --output-markdown specs/sns/reports/steward_scorecard_2026q1.md \
     --handoff-json fixtures/documentation/sns/steward_handoff_2026q1.json \
     --handoff-markdown specs/sns/reports/steward_handoff_2026q1.md \
     --handoff-dir fixtures/documentation/sns/handoffs/2026q1
   ```
3. Attach the JSON + Markdown artefacts to the governance docket (`fixtures/documentation/sns/` and `specs/sns/reports/`) and mirror the digest in `status.md`. The hand-off packet now also writes per-suffix council/DAO hand-offs under `fixtures/documentation/sns/handoffs/<quarter>/` (index + per-suffix JSON/Markdown) so tickets and DAO briefs can embed the exact deadlines/action owners.
4. Post the Markdown excerpt to the SNS council channel and submit it with the steward health review ticket. When the DAO needs visibility, share either the aggregate hand-off Markdown or the per-suffix Markdown from `fixtures/documentation/sns/handoffs/<quarter>/` so observers see the same deadlines and motion language the council uses.
...
Evidence: fixtures/documentation/sns/steward_scorecard_<quarter>.json
...
Body:
Council has opened an escalation window because the <metric> breach persists / guardian freeze remains open. Provide a signed corrective plan within 48 hours. Failure to do so will trigger the rotation workflow described in specs/sns/steward_replacement_playbook.md.
...
- For replacement events, include the signed Norito manifest that records the handover in `specs/sns/governance_addenda/<year>/<id>.md`.
- Update `specs/sns/governance_playbook.md` Appendix with the ticket ids so auditors can trace the full remediation chain.
...
4. **Regulatory updates:** File memo in `specs/sns/regulatory/<jurisdiction>/` when the steward operates in regulated regions.
