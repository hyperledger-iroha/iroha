...
The serialized-audit bridge experiment has been removed. It had no decoder,
manifest profile, candidate framing, or runtime acceptance path, and its
review-blocked graph changed the V4 parent verifier under a diagnostic feature.
The sole recursion backend is the fixed ABI-21/V4 Eq/Ep circuit, including in
memory-lab builds; enabling diagnostics does not change its proof parser,
function signature, public layout, or parent-presence constraints.
...
Both probe commands exit before publishing artifacts. The guard admits only
the three exact operations (the full measurement plus those two probes) and
