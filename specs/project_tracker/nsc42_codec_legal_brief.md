...
- Internal: `specs/norito_streaming.md` (§7 Encoder Profiles) and `specs/norito_streaming_legal.md` (once opinion delivered).
...
- **Operator guidance:** Captured in `specs/norito_streaming.md` (§19) and `specs/soranet/nsc-42-legal.md`; mirrors the jurisdictional matrix posted in `specs/norito_streaming_legal.md`.
...
- Release Engineering to add `codec-cabac`, `codec-trellis`, and `codec-simd-*` features to build matrix (`.github/workflows/codec-build-matrix.yml` placeholder) with documentation in `specs/references/build_flags.md` (draft).
...
- `specs/norito_streaming.md` — encoder profiles, CABAC/rANS integration notes (see §§7 & 19).
- `benchmarks/nsc/rans_compare.py` — harness for validating deterministic rANS tables.
- `specs/project_tracker/nsc55_rans_validation_plan.md` — cross-reference table generation/verification tasks.
- `specs/norito_streaming_legal.md` — counsel opinion + export posture.
...
## Deliverables
1. Written legal opinion filed in repo (`specs/norito_streaming_legal.md`).
