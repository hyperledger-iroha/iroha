...
1. **Deterministic baseline.** Every required run exercises the same simulator
   set to minimise non-deterministic failures; no physical device is a build,
   test, or release prerequisite.
2. **Optional hardware parity.** An explicitly requested lane may execute on
   Secure Enclave/StrongBox-capable hardware to validate key handling paths that
   simulators cannot emulate.
...
| `xcframework-smoke/strongbox` | `platform=iOS,name=iPhone 14 Pro,udid=<device-udid>` | Optional StrongBox/Secure Enclave validation on physical hardware. | Run only with `IOS6_SMOKE_ENABLE_HARDWARE=true`; absence never blocks baseline or release. |
...
export IOS6_SMOKE_ENABLE_HARDWARE=true # only for the optional hardware drill
...
- The pipeline annotates the build with the rendered dashboard summary and fails
  the step if a required simulator/fallback lane reports a failure. The hardware
  lane is recorded when explicitly requested, but is not a general release gate.
...
3. Queue the StrongBox lane only when `IOS6_SMOKE_ENABLE_HARDWARE=true`; if the
   device pool is busy, record the optional-lane incident and notify the hardware
   channel without blocking baseline evidence.
