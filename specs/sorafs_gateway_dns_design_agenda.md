...
`specs/sorafs_gateway_dns_design_minutes.md`; the current operator-facing
summary remains in `specs/sorafs_gateway_dns_design_pre_read.md`.
...
> roster is tracked in `specs/sorafs_gateway_dns_design_attendance.md`.

## 2. Pre-read Review (0–5 min)
- Quick acknowledgement that everyone read:
  - `specs/sorafs_gateway_dns_design_pre_read.md`
...
|------|-------|-----|-------|
| Confirm attendee availability | Networking TL | 2025-02-26 | Use template in `specs/sorafs_gateway_dns_design_invite.txt`. |
...
| Prepare diagrams (DNS flow, policy engine) | Tooling WG / Security | 2025-03-01 | Store under `specs/images/sorafs_gateway_kickoff/`. |
...
`specs/sorafs_gateway_dns_design_minutes.md` for corrections to decisions
