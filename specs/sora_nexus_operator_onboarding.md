...
2. Treat the files under `config/` as templates:
   ...
   - Mount the Kagami-produced public `genesis.expected_hash`, containing one canonical checked `hash:<64 uppercase hex>#<CRC16>` NetworkId literal, at `/run/iroha/genesis.expected_hash` for both the validator's `genesis.expected_hash_file` and the client's `network_id_file`; never render those values independently.
...
5. Attach the `authenticated_external_signer` authorization record,
   the verified signer-authority receipt, OIDC/cosign
