...
For future math upgrades (non-normative), see `specs/norito_streaming_math_notes.md`.
...
`fixtures/documentation/sorafs_direct_mode_policy.json`). Publishers can now turn that
...
scripts/norito_relay_selector.py \
  --scoreboard fixtures/documentation/norito_relay_scoreboard_sample.json \
  --metrics fixtures/documentation/norito_relay_metrics_sample.csv \
...
> `fixtures/documentation/norito_relay_scoreboard_sample.json` and
> `fixtures/documentation/norito_relay_metrics_sample.csv` documents the expected
...
breach should trigger the incident workflow described in `specs/telemetry.md`.
...
- JavaScript integration guidance already demonstrates the Torii streaming workflow in
  `specs/sdk/js/quickstart.md` (see “Torii Queries & Streaming”). Other SDKs should
