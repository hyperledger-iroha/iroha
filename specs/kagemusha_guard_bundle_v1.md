...
`GuardBundleV1` is the sole hardware-authority input to Kagemusha V1 state
...
`KagemushaHardwareProfileV1` (`HardwareProfileV1`) is an immutable
...
proof-bearing `KagemushaMintAuthorizationV1`. Before any payer debit or
...
`KagemushaMintCreditStatementV1` contains only a fresh randomized credential
...
operation kind
KagemushaHardwareProfileV1 and KagemushaHardwareCredentialV1 membership
...
request, proof-bearing KagemushaAcceptanceIntentAuthorizationV1,
its compact KagemushaAcceptanceIntentV1, exact-amount
KagemushaAcceptanceTicketV1, private request-ledger decision, and
...
once and carries balance and replay root unchanged. It may also install a named
successor verifier suite, but only when the recursive proof authenticates
separately governed exact from/to bridge evidence; otherwise it retains the
current verifier. Checked arithmetic and canonical empty values are enforced
inside the relation.
...
[`kagemusha_v1.md`](kagemusha_v1.md): canonical
`KagemushaEncryptedCreditEnvelopeV1`, X25519, HKDF-SHA256, and
...
plaintext is the fixed `KagemushaCreditOpeningV1`. The `k = 16` proof does not
...
3. Core derives proof authority from an `KagemushaAuthenticatedReleaseV1`,
...
4. Core constructs the hardware commit command only from that
   ...
   `KagemushaOutboxReservationV1`/terminal record, and emits one recoverable
   `KagemushaCommitCertificateV1`.
5. `KagemushaCommitWrapperProofV1` recursively verifies the candidate
...
Hardware first commits a self-free `KagemushaHardwareTerminalBodyV1` in this
...
hardware profile, policy epoch, `KagemushaCommitEvidenceV1`, and that hiding
...
`KagemushaAcceptanceIntentAuthorizationV1`. Before hardware persists the
...
`KagemushaAcceptanceIntentV1` or shape-valid proof bytes alone have no
...
or any physical profile have passed them; Kagemusha remains unqualified
