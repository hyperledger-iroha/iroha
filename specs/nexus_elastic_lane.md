...
- Wire PagerDuty/Alertmanager rules for the new lane before enabling admission. The `summary.json`
  next-steps array mirrors the checklist in `specs/nexus_operations.md`.
...
- Follow `specs/sora_nexus_operator_onboarding.md` for smoke tests (FindNetworkStatus, Torii
...
- `specs/nexus_operations.md` — operational checklist and telemetry requirements.
- `specs/sora_nexus_operator_onboarding.md` — detailed onboarding flow that references the
  new helper.
- `specs/nexus_lanes.md` — lane geometry, slugs, and storage layout used by the tool.
