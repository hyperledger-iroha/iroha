...
`specs/settlement_router.md` for the full specification and
`specs/cbdc_lane_playbook.md` for CBDC-specific policy knobs.
...
- Configuration lives under `[settlement]` in `config/config.toml`. Avoid custom
  ...
  `specs/settlement_router.md` for field descriptions.
...
4. **Reconciliation report** — optional but recommended: export
   `SettlementRecordV1` entries (see `specs/cbdc_lane_playbook.md`) and
...
- SDKs must:
  ...
  - Mirror the Norito payloads defined in `specs/settlement_router.md`
...
- Operators must monitor `governance.settlement_pause_total` and follow the
  incident template in `specs/nexus_operations.md`.
