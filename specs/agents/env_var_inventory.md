...
- prod: crates/irohad/src/soracloud_runtime.rs:15313 — `if let Some(android_sdk_root) = std::env::var_os("ANDROID_SDK_ROOT") {`

## CARGO (prod: 3, test: 3)

- test: crates/iroha_test_network/src/lib.rs:2630 — `let running_under_cargo = std::env::var_os("CARGO").is_some();`
...
- prod: mochi/mochi-core/src/supervisor.rs:1316 — `let cargo = env::var_os("CARGO")`
- prod: mochi/mochi-core/src/supervisor.rs:1365 — `let cargo = env::var_os("CARGO")`
- prod: mochi/mochi-core/src/supervisor.rs:1411 — `let cargo = env::var_os("CARGO")`
...
- prod: crates/iroha_cli/src/main_shared.rs:92 — `BuildLine::from_bin_name(env!("CARGO_BIN_NAME"))`
- prod: crates/iroha_cli/src/main_shared.rs:425 — `#[command(name = env!("CARGO_BIN_NAME"), version = env!("CARGO_PKG_VERSION"), author)]`
- prod: crates/iroha_kagami/src/genesis/generate.rs:1066 — `BuildLine::from_bin_name(env!("CARGO_BIN_NAME"))`
...
- test: crates/connect_norito_bridge/src/kagemusha_bridge_tests/tests/recipient_fixture_owner.rs:26 — `Path::new(env!("CARGO_MANIFEST_DIR"))`
- test: crates/connect_norito_bridge/src/kagemusha_bridge_tests/tests/recipient_fixture_owner.rs:34 — `let checked = Path::new(env!("CARGO_MANIFEST_DIR"))`
- test: crates/connect_norito_bridge/src/lib.rs:15016 — `env!("CARGO_MANIFEST_DIR"),`
...
- test: crates/iroha_cli/src/compute.rs:807 — `Path::new(env!("CARGO_MANIFEST_DIR"))`
- prod: crates/iroha_cli/src/main_shared.rs:1564 — `let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));`
...
- test: crates/iroha_config/tests/soranet_privacy_ingest_hard_cut.rs:9 — `let base_path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/base.toml");`
...
- test: crates/iroha_core/src/block.rs:24295 — `let src = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src");`
- test: crates/iroha_core/src/executor.rs:18229 — `std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../defaults/executor.to");`
- test: crates/iroha_core/src/executor_contract_dispatch_tests.rs:281 — `std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../defaults/executor.to");`
- test: crates/iroha_core/src/smartcontracts/isi/asset/core_numeric_mutation_tests.rs:5 — `let source_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src");`
...
- test: crates/iroha_core/src/smartcontracts/isi/soracloud.rs:24629 — `let fixture_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))`
...
- test: crates/iroha_data_model/src/nexus/manifest.rs:1247 — `let fixture_path = Path::new(env!("CARGO_MANIFEST_DIR"))`
- test: crates/iroha_data_model/src/nexus/manifest.rs:1386 — `let fixture_path = Path::new(env!("CARGO_MANIFEST_DIR"))`
- test: crates/iroha_data_model/src/nexus/manifest.rs:1396 — `let fixture_path = Path::new(env!("CARGO_MANIFEST_DIR"))`
- test: crates/iroha_data_model/src/qr_stream.rs:849 — `let mut path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));`
- test: crates/iroha_data_model/src/soranet/vpn.rs:3046 — `PathBuf::from(env!("CARGO_MANIFEST_DIR")).join(FIXTURE_PATH)`
...
- test: crates/iroha_genesis/src/genesis_tail_tests.rs:87 — `std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../defaults/genesis.json");`
- test: crates/iroha_genesis/src/genesis_tail_tests.rs:102 — `std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../defaults/genesis.json");`
- test: crates/iroha_genesis/src/lib.rs:2452 — `let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join(relative_path);`
- test: crates/iroha_genesis/src/lib.rs:4579 — `std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../defaults/genesis.json");`
- test: crates/iroha_genesis/src/lib.rs:4604 — `let repo_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");`
- test: crates/iroha_genesis/src/lib.rs:4659 — `let repo_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");`
...
- test: crates/iroha_kagami/src/genesis/sign.rs:1634 — `let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");`
- test: crates/iroha_kagami/src/genesis/sign.rs:1664 — `let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");`
- test: crates/iroha_kagami/src/genesis/sign.rs:1714 — `let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");`
- test: crates/iroha_kagami/src/genesis/sign.rs:1739 — `let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");`
- test: crates/iroha_kagami/src/genesis/sign.rs:1753 — `let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");`
- test: crates/iroha_kagami/src/genesis/sign.rs:1785 — `let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");`
- test: crates/iroha_kagami/src/genesis/sign.rs:2151 — `let workspace_root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");`
- test: crates/iroha_kagami/src/genesis/sign.rs:3467 — `PathBuf::from(env!("CARGO_MANIFEST_DIR"))`
- test: crates/iroha_kagami/src/genesis/sign.rs:3476 — `let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");`
- test: crates/iroha_kagami/src/genesis/sign.rs:3495 — `let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");`
- prod: crates/iroha_kagami/src/localnet.rs:587 — `env!("CARGO_MANIFEST_DIR"),`
- prod: crates/iroha_kagami/src/localnet.rs:4205 — `let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))`
- prod: crates/iroha_kagami/src/localnet.rs:4209 — `|| PathBuf::from(env!("CARGO_MANIFEST_DIR")),`
- test: crates/iroha_kagami/src/wizard.rs:1228 — `let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");`
...
- test: crates/iroha_test_network/src/bin/taira_privacy_release_runner.rs:4976 — `Path::new(env!("CARGO_MANIFEST_DIR")).join("../../fixtures/privacy/exact12_v1.tsv");`
...
- test: crates/iroha_test_network/src/lib.rs:769 — `PathBuf::from(env!("CARGO_MANIFEST_DIR"))`
...
- test: crates/iroha_torii/src/soracloud.rs:12045 — `Path::new(env!("CARGO_MANIFEST_DIR"))`
- test: crates/iroha_torii/src/sorafs/admission.rs:603 — `Path::new(env!("CARGO_MANIFEST_DIR"))`
- test: crates/iroha_torii/src/sorafs/api.rs:27229 — `let matrix_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))`
- test: crates/iroha_torii/src/sorafs/api.rs:39999 — `std::path::Path::new(env!("CARGO_MANIFEST_DIR"))`
- test: crates/iroha_torii/src/tests/lib_routed_reads/routed_read_source_bounds.rs:478 — `let mut pending = vec![std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src")];`
...
- test: crates/iroha_torii/tests/sorafs_discovery.rs:1535 — `PathBuf::from(env!("CARGO_MANIFEST_DIR"))`
...
- test: crates/norito/src/streaming/repo_fixture_test.rs:5 — `let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))`
...
- prod: crates/sorafs_chunker/src/bin/export_vectors.rs:463 — `let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));`
...
- test: crates/sorafs_manifest/src/reference.rs:6601 — `PathBuf::from(env!("CARGO_MANIFEST_DIR"))`
...
- test: crates/sorafs_orchestrator/src/lib.rs:6777 — `let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));`
- test: crates/sorafs_orchestrator/src/lib.rs:6905 — `let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));`
- test: crates/sorafs_orchestrator/src/lib.rs:8574 — `let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));`
...
- test: mochi/mochi-core/src/supervisor.rs:113 — `env!("CARGO_MANIFEST_DIR"),`
- prod: mochi/mochi-core/src/supervisor.rs:847 — `let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));`
- prod: mochi/mochi-core/src/supervisor.rs:1308 — `let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));`
- test: mochi/mochi-core/src/torii/tests/canonical_fixture_owner.rs:13 — `Path::new(env!("CARGO_MANIFEST_DIR"))`
- test: mochi/mochi-core/src/torii/tests/canonical_fixture_owner.rs:21 — `let checked = Path::new(env!("CARGO_MANIFEST_DIR"))`
- test: mochi/mochi-integration/src/mock_torii/tests/replay_fixture_owner.rs:13 — `Path::new(env!("CARGO_MANIFEST_DIR"))`
- test: mochi/mochi-integration/src/mock_torii/tests/replay_fixture_owner.rs:21 — `let checked = Path::new(env!("CARGO_MANIFEST_DIR"))`
...
- prod: mochi/mochi-ui-egui/src/gui.rs:140 — `env!("CARGO_MANIFEST_DIR"),`
...
- tool: xtask/src/main.rs:14223 — `Path::new(env!("CARGO_MANIFEST_DIR"))`
...
- test: integration_tests/tests/taira_public_localnet.rs:3051 — `let cargo_net_offline = std::env::var("CARGO_NET_OFFLINE")`
...
- test: crates/iroha_cli/src/main_shared_tests.rs:1361 — `&[("version", env!("CARGO_PKG_VERSION"))],`
...
- tool: tools/sora-vpn-helper/src/main.rs:77 — `const VERSION: &str = env!("CARGO_PKG_VERSION");`
...
- prod: crates/iroha_kagami/src/localnet.rs:4229 — `let target_dir = resolve_target_dir(&repo_root, env::var("CARGO_TARGET_DIR").ok().as_deref());`
- test: crates/iroha_test_network/src/lib.rs:1081 — `if let Ok(path) = std::env::var("CARGO_TARGET_DIR") {`
- test: crates/iroha_test_network/src/lib.rs:2038 — `if let Ok(path) = std::env::var("CARGO_TARGET_DIR") {`
...
- test: integration_tests/tests/taira_public_localnet.rs:3169 — `let target = std::env::var_os("CARGO_TARGET_DIR")`
- prod: mochi/mochi-core/src/supervisor.rs:888 — `let target_root = env::var_os("CARGO_TARGET_DIR")`
- prod: mochi/mochi-core/src/supervisor.rs:1342 — `let target_root = env::var_os("CARGO_TARGET_DIR")`
- prod: mochi/mochi-core/src/supervisor.rs:1388 — `let target_root = env::var_os("CARGO_TARGET_DIR")`
- prod: mochi/mochi-core/src/supervisor.rs:1434 — `let target_root = env::var_os("CARGO_TARGET_DIR")`
- tool: xtask/src/kagami_profiles.rs:1350 — `if let Ok(dir) = std::env::var("CARGO_TARGET_DIR") {`
...
- prod: crates/fastpq_prover/src/metal.rs:2483 — `option_env!("FASTPQ_METAL_LIB")`
...
- prod: crates/irohad/src/soracloud_runtime.rs:15317 — `if let Some(home) = std::env::var_os("HOME") {`
...
- test: crates/izanami/src/chaos.rs:7508 — `.or_else(|_| std::env::var("IROHA_ALLOW_NET"))`
...
- test: crates/connect_norito_bridge/src/kagemusha_bridge_tests/tests/recipient_fixture_owner.rs:37 — `let Some(raw_stage) = env::var_os("IROHA_CONNECT_RECIPIENT_FIXTURE_STAGE") else {`
...
- test: crates/iroha_genesis/src/genesis_manifest_tests.rs:702 — `let path = env::var("IROHA_DEBUG_GENESIS_PATH")`
- test: crates/iroha_genesis/src/genesis_manifest_tests.rs:762 — `let path = env::var("IROHA_DEBUG_GENESIS_PATH")`
- test: crates/iroha_genesis/src/genesis_manifest_tests.rs:793 — `let path = env::var("IROHA_DEBUG_GENESIS_PATH")`

## IROHA_DEBUG_SIGNED_GENESIS_PATH (test: 1)

- test: crates/iroha_genesis/src/genesis_manifest_tests.rs:730 — `let path = env::var("IROHA_DEBUG_SIGNED_GENESIS_PATH")`

## IROHA_DPN_VALIDATOR_RELEASE_COMMIT (test: 1)

- test: crates/iroha_telemetry/src/metrics.rs:5202 — `dpn_validator_release_commit: option_env!("IROHA_DPN_VALIDATOR_RELEASE_COMMIT")`

## IROHA_DUMP_MANIFEST_JSON (test: 1)

- test: crates/iroha_data_model/src/nexus/manifest.rs:1382 — `if std::env::var_os("IROHA_DUMP_MANIFEST_JSON").is_some() {`
...
- prod: crates/iroha_core/src/bin/pk2_bridge_finality_verify.rs:56 — `const BUILD_SOURCE_ID: Option<&str> = option_env!("IROHA_GIT_COMMIT_HASH");`
...
- test: crates/irohad/src/soracloud_runtime.rs:19150 — `if std::env::var("IROHA_INROU_LINUX_KVM").ok().as_deref() != Some("1") {`
- test: crates/irohad/src/soracloud_runtime/tests/runtime_tail.rs:197 — `|| std::env::var("IROHA_INROU_LINUX_KVM").ok().as_deref() != Some("1")`
- test: crates/irohad/src/soracloud_runtime/tests/runtime_tail.rs:360 — `|| std::env::var("IROHA_INROU_LINUX_KVM").ok().as_deref() != Some("1")`

## IROHA_INROU_LINUX_KVM_INITRD_IMAGE (test: 2)

- test: crates/irohad/src/soracloud_runtime/tests/runtime_tail.rs:208 — `let initrd_image = std::env::var("IROHA_INROU_LINUX_KVM_INITRD_IMAGE")`
- test: crates/irohad/src/soracloud_runtime/tests/runtime_tail.rs:371 — `let initrd_image = std::env::var("IROHA_INROU_LINUX_KVM_INITRD_IMAGE")`
...
- prod: crates/irohad/src/soracloud_runtime.rs:812 — `portable_vm_accel_from(std::env::var("IROHA_INROU_PORTABLE_ACCEL").ok().as_deref())`
...
- test: crates/irohad/src/soracloud_runtime/tests/runtime_tail.rs:16 — `let external_entrypoint = std::env::var("IROHA_INROU_PORTABLE_SMOKE_ENTRYPOINT")`

## IROHA_INROU_PORTABLE_SMOKE_HEALTHCHECK (test: 1)

- test: crates/irohad/src/soracloud_runtime/tests/runtime_tail.rs:18 — `let external_healthcheck = std::env::var("IROHA_INROU_PORTABLE_SMOKE_HEALTHCHECK")`
...
- test: crates/iroha_telemetry/src/metrics.rs:14132 — `std::env::var("IROHA_METRICS_PANIC_ON_DUPLICATE")`
...
- test: mochi/mochi-core/src/torii/tests/canonical_fixture_owner.rs:24 — `let Some(raw_stage) = env::var_os("IROHA_MOCHI_CANONICAL_FIXTURE_STAGE") else {`

## IROHA_MOCHI_REPLAY_FIXTURE_STAGE (test: 1)

- test: mochi/mochi-integration/src/mock_torii/tests/replay_fixture_owner.rs:25 — `let Some(raw_stage) = env::var_os("IROHA_MOCHI_REPLAY_FIXTURE_STAGE") else {`
...
- test: integration_tests/tests/sumeragi_localnet_smoke.rs:526 — `let Some(raw) = std::env::var("IROHA_REALISTIC_30TPS_LOAD_KIND")`

## IROHA_REALISTIC_30TPS_LOG_LEVEL (test: 1)

- test: integration_tests/tests/sumeragi_localnet_smoke.rs:2752 — `std::env::var("IROHA_REALISTIC_30TPS_LOG_LEVEL").unwrap_or_else(|_| "WARN".into());`

## IROHA_RELEASE_SOURCE_MANIFEST_SHA256 (test: 1)

- test: integration_tests/tests/taira_public_localnet.rs:3035 — `let manifest = std::env::var("IROHA_RELEASE_SOURCE_MANIFEST_SHA256")`
...
- test: crates/iroha_test_network/src/lib.rs:7545 — `if std::env::var_os("IROHA_SKIP_BIND_CHECKS").is_none() {`
...
- test: integration_tests/tests/taira_public_localnet.rs:3080 — `let raw = std::env::var("IROHA_TAIRA_EVIDENCE_PATH")`

## IROHA_TAIRA_KEEP_LOCALNET (test: 2)

- test: integration_tests/tests/taira_public_localnet.rs:3125 — `if std::env::var_os("IROHA_TAIRA_KEEP_LOCALNET").is_some() {`
- test: integration_tests/tests/taira_public_localnet.rs:3134 — `if std::env::var_os("IROHA_TAIRA_KEEP_LOCALNET").is_some() {`

## IROHA_TAIRA_SIM_SEED (test: 1)

- test: integration_tests/tests/taira_public_localnet.rs:699 — `let seed = std::env::var("IROHA_TAIRA_SIM_SEED")`
...
- test: integration_tests/tests/taira_public_localnet.rs:3047 — `let build_profile = std::env::var("IROHA_TEST_BUILD_PROFILE")`

## IROHA_TEST_CLIENT_TTL_MS (test: 5)

- test: integration_tests/tests/sumeragi_localnet_smoke.rs:2732 — `let previous_ttl = std::env::var_os("IROHA_TEST_CLIENT_TTL_MS");`
- test: integration_tests/tests/sumeragi_localnet_smoke.rs:3915 — `let previous_ttl = std::env::var_os("IROHA_TEST_CLIENT_TTL_MS");`
- test: integration_tests/tests/sumeragi_localnet_smoke.rs:4163 — `let previous_ttl = std::env::var_os("IROHA_TEST_CLIENT_TTL_MS");`
- test: integration_tests/tests/sumeragi_localnet_smoke.rs:4424 — `let previous_ttl = std::env::var_os("IROHA_TEST_CLIENT_TTL_MS");`
- test: integration_tests/tests/sumeragi_localnet_smoke.rs:5063 — `let previous_ttl = std::env::var_os("IROHA_TEST_CLIENT_TTL_MS");`

## IROHA_TEST_DUMP_GENESIS (test: 1)

- test: crates/iroha_test_network/src/lib.rs:14260 — `if let Ok(dump_path) = env::var("IROHA_TEST_DUMP_GENESIS") {`
...
- test: crates/iroha_core/src/executor.rs:18227 — `std::env::var_os("IROHA_TEST_USE_DEFAULT_EXECUTOR")?;`
- test: crates/iroha_core/src/executor_contract_dispatch_tests.rs:279 — `std::env::var_os("IROHA_TEST_USE_DEFAULT_EXECUTOR")?;`

## IROHA_THROUGHPUT_ARTIFACT_DIR (test: 3)

- test: integration_tests/tests/sumeragi_localnet_smoke.rs:3568 — `if let Some(artifact_root) = std::env::var_os("IROHA_THROUGHPUT_ARTIFACT_DIR") {`
- test: integration_tests/tests/sumeragi_localnet_smoke.rs:5014 — `if let Some(artifact_root) = std::env::var_os("IROHA_THROUGHPUT_ARTIFACT_DIR") {`
- test: integration_tests/tests/sumeragi_localnet_smoke.rs:5625 — `if let Some(artifact_root) = std::env::var_os("IROHA_THROUGHPUT_ARTIFACT_DIR") {`
...
- test: integration_tests/tests/sumeragi_localnet_smoke.rs:4430 — `std::env::var("IROHA_THROUGHPUT_RBC_ENCODING").unwrap_or_else(|_| "plain".to_owned());`

## IROHA_TORII_ALLOW_LIVE_ASSET_HOLDER_AGGREGATE (test: 1)

- test: crates/iroha_torii/src/routing.rs:67778 — `|| std::env::var("IROHA_TORII_ALLOW_LIVE_ASSET_HOLDER_AGGREGATE")`
...
- tool: xtask/src/main.rs:13888 — `if let Some(env_tokens) = std::env::var_os("IROHA_TORII_OPENAPI_TOKENS") {`
- tool: xtask/src/main.rs:13936 — `token_header = std::env::var("IROHA_TORII_OPENAPI_TOKENS")`

## IROHA_TORII_PUBLIC_DATASPACE_UPSTREAMS (prod: 1)

- prod: crates/iroha_torii/src/lib.rs:2194 — `let Ok(raw) = std::env::var("IROHA_TORII_PUBLIC_DATASPACE_UPSTREAMS") else {`
...
- test: crates/kotodama_lang/src/compiler.rs:17681 — `if cfg!(any(test, debug_assertions)) && std::env::var_os("IVM_COMPILER_DEBUG").is_some() {`
...
- test: crates/izanami/src/chaos.rs:7507 — `std::env::var("IZANAMI_ALLOW_NET")`
...
- test: crates/norito/src/core/sequence_plan_helper_tests.rs:66 — `if std::env::var_os("JSONSTAGE1_CUDA_REQUIRE").is_some() {`
- test: crates/norito/src/lib.rs:5939 — `if std::env::var_os("JSONSTAGE1_CUDA_REQUIRE").is_some() {`
...
- prod: mochi/mochi-core/src/supervisor.rs:5159 — `std::env::var_os("MOCHI_DATA_ROOT")`
...
- prod: mochi/mochi-core/src/supervisor.rs:4843 — `if std::env::var_os("MOCHI_TEST_USE_INTERNAL_GENESIS").is_some() {`
...
- test: crates/norito/src/lib.rs:354 — `match std::env::var_os("NORITO_DISABLE_PACKED_STRUCT") {`
...
- test: crates/norito/src/lib.rs:6199 — `std::env::var("NORITO_PAR_STAGE1_MIN")`
...
- test: crates/norito/src/lib.rs:6229 — `std::env::var("NORITO_STAGE1_GPU_MIN_BYTES")`

## NORITO_TRACE (test: 3)

- test: crates/norito/src/lib.rs:151 — `std::env::var_os("NORITO_TRACE").is_some()`
- test: crates/norito/src/lib.rs:156 — `*ENABLED.get_or_init(|| std::env::var_os("NORITO_TRACE").is_some())`
- test: crates/norito/src/lib.rs:172 — `let env_enabled = env::var_os("NORITO_TRACE").is_some();`
...
- prod: crates/ivm/src/ivm.rs:227 — `include!(concat!(env!("OUT_DIR"), "/syscall_signatures.rs"));`
...
- prod: crates/irohad/src/soracloud_runtime.rs:15250 — `let mut search_roots = std::env::var_os("PATH")`
- test: integration_tests/tests/kotodama_examples.rs:21 — `let path = env::var_os("PATH")?;`
- prod: mochi/mochi-core/src/supervisor.rs:1285 — `let path_var = env::var_os("PATH")?;`
- tool: tools/sora-vpn-backend/src/main.rs:1296 — `let Some(path) = env::var_os("PATH") else {`

## PATHEXT (prod: 1)

- prod: crates/irohad/src/soracloud_runtime.rs:15279 — `std::env::var("PATHEXT").unwrap_or_else(|_| ".COM;.EXE;.BAT;.CMD".to_owned());`

## PRINT_SORACLES_FIXTURES (test: 1)

- test: crates/iroha_data_model/src/oracle/mod.rs:3350 — `if std::env::var_os("PRINT_SORACLES_FIXTURES").is_some() {`
...
- test: crates/iroha_test_network/src/lib.rs:1753 — `if let Ok(profile) = std::env::var("PROFILE") {`
...
- test: integration_tests/tests/taira_public_localnet.rs:3050 — `std::env::var("PROFILE").wrap_err("PROFILE must be set by the release launcher")?;`
...
- test: crates/iroha_cli/tests/cli_smoke.rs:5168 — `match env::var("PYTHONPATH") {`
...
- test: crates/iroha_torii/tests/sorafs_discovery.rs:127 — `std::env::var("SORAFS_TORII_SKIP_INGEST_TESTS").map_or(true, |value| value != "1")`
...
- test: crates/iroha_test_network/src/lib.rs:2391 — `std::env::var("TEST_NETWORK_CARGO").unwrap_or_else(|_| "cargo".to_owned());`
...
- tool: xtask/src/main.rs:13883 — `if let Ok(single) = std::env::var("TORII_OPENAPI_TOKEN")`
- tool: xtask/src/main.rs:13932 — `let mut token_header = std::env::var("TORII_OPENAPI_TOKEN")`
...
- test: crates/iroha_telemetry/src/metrics.rs:5208 — `target_triple: option_env!("VERGEN_CARGO_TARGET_TRIPLE")`
...
- test: xtask/src/kagami_profiles.rs:2370 — `if std::env::var("XTASK_TEST_KAGAMI_BIN").is_err() {`
- test: xtask/src/kagami_profiles.rs:2373 — `let kagami_path = PathBuf::from(std::env::var("XTASK_TEST_KAGAMI_BIN").unwrap());`
