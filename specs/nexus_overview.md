...
| **Lane Catalog** | Configuration section (`[nexus]` in `config.toml`) that maps lane IDs to aliases, routing policies, and retention knobs. Operators can introspect the effective catalog via `irohad --sora --config … --trace-config`. | `specs/sora_nexus_operator_onboarding.md` |
