...
source anchors are not the complete 524-test `G-UNIT` receipt, and no
...
tests are mapped row evidence, not the complete 524-test `G-UNIT` receipt, so
...
`scripts/run_sumeragi_v2_release_gates.sh` contain exactly 524 unique required
...
The G-UNIT static inventory checks establish exact `524/524` source consistency and also
...
execution of all 524 required tests, so `G-UNIT` stays Open.
...
- `G-UNIT`, `G-SDK`, and `G-FORMAL` remain open even though the exact 524-row
  ...
  524-test execution, complete SDK harness, or formal-engine result was
