...
- After a successful WSV commit, `kura` writes a Norito checkpoint and commit manifest before publishing the Sumeragi-v2 finality artifact. Both sidecars have a first-release 64 KiB encoded limit enforced before publication and by identity-checked max-plus-one reads before decoding, so a corrupt local artifact cannot drive an unbounded startup allocation. The manifest binds the durable block, checkpoint hash, CommitQC execution roots, and a domain-separated digest of the complete cryptographically verified v2 artifact. Startup permits generic execution replay only through the contiguous full-body prefix where checkpoint, checkpoint-bound manifest, canonical block wire, and finality artifact all agree, and it preflights every body in the complete replay range before the first WSV mutation. At most one incomplete durable tip is a recoverable commit crash window: it is never pruned or generic-replayed, and network ingress remains closed while the exact frozen height context, checksummed body and validation marker, and cryptographically reverified safety-WAL Decision resume through the idempotent v2 Apply service. A missing or corrupt interior sidecar, a multi-height gap, an unavailable replay body, or an impossible sidecar publication order stops startup without partial replay. An explicitly configured audited hash-only snapshot is the sole exception to historical parent-CommitQC availability: its digest-pinned bootstrap record binds the exact anchor height, block hash, block timestamp, canonical WSV, first v2 context, and validator PoPs. Kura stays read-only until that record is authenticated. Because token finalization can complete deferred storage recovery, startup then rereads the exact fallible durable count, recomputes and reauthenticates the replay plan, and repeats the whole-range body preflight before geometry or WSV mutation. Later normally signed snapshots retain that original lineage and must match the exact hash vector and complete first full finality artifact; the self-digested Kura marker never authenticates it by itself. The first executable block derives time from the anchor plus committed cadence, and all later heights return to ordinary parent-CommitQC continuity.
...
- Read a recovery sidecar via: `GET /v1/pipeline/recovery/:height`
  - Requires a fresh, replay-resistant `OperatorSignature` over the exact
    genesis `NetworkId`, `GET`, substituted path, query, and empty body; token
    authentication is not a fallback
