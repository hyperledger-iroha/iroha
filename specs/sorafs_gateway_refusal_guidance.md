...
`specs/sorafs_gateway_capability_tests.md` and the self-certification kit
captured in `specs/sorafs_gateway_self_cert.md`.
...
`specs/sorafs_gateway_capability_tests.md` and reuse the `error` field as
...
| C3 | 412 | `admission_envelope_mismatch` | `governance_violation`, `alias_stale` | Validate the provider admission envelope and manifest council signatures with the current CLI tooling. | `specs/sorafs/pin_registry_validation_plan.md`. |
...
4. Confirm that the gateway cache refreshed within
   `alias_refresh_window` (see `specs/sorafs_alias_policy.md`). If the
...
2. Run `cargo run -p iroha_cli --bin iroha -- app sorafs toolkit validate admission --input <envelope.to>` to emit the SF-11 validation report and telemetry evidence.
3. Confirm the admission verifier used the current governed council key set and
   rotation epoch; do not trust a key carried only by the submitted envelope.
