...
complements the canonical envelope spec (`specs/taikai_segment_envelope.md`)
...
- **Ingest writer.** Calls `/v1/da/ingest` with deterministic metadata,
  ...
  `specs/da/replication_policy.md`.
...
| Tool | Purpose | Notes |
|------|---------|-------|
| `iroha app taikai bundle` / `sorafs_cli taikai bundle` | Offline bundling of CMAF fragments into CAR + TSE. | Available today; referenced by `specs/taikai_segment_envelope.md`. |
