...
- **What it does not cover:** production incident response (see
  `specs/sorafs_gateway_operator_playbook.md`) or post-kickoff roadmap
...
|------------|------------------|------------------|--------------------|
| Networking TL (DNS stack) | Maintain deterministic host plan, run RAD directory releases, own resolver telemetry inputs. | networking@sora | `artifacts/soradns_directory/<timestamp>/`, `specs/soradns/deterministic_hosts.md` diff, RAD release metadata. |
...
| Docs / DevRel | Capture minutes, update `specs/sorafs_gateway_dns_design_*` files, publish evidence summary in portal. | docs@sora / devrel@sora | Updated pre-read, runbook changelog, telemetry appendix, agenda action register. |

# 3. Inputs & Prerequisites

- Deterministic host spec (`specs/soradns/deterministic_hosts.md`) and the
  resolver attestation contract (`specs/soradns/resolver_attestation_directory.md`).
- Gateway artefacts: profile, deployment handbook, TLS automation, direct-mode
  guidance, and self-cert workflow (see `specs/sorafs_gateway_*` docs).
...
1. **Confirm attendees & agenda:** Update
   `specs/sorafs_gateway_dns_design_attendance.md` and share the current
   `specs/sorafs_gateway_dns_design_agenda.md`.
...
4. Record the printed directory ID, SHA-256, and output paths in
   `specs/sorafs_gateway_dns_design_gar_telemetry.md` plus the kickoff
   meeting notes.
...
3. Attach `sorafs_gateway_report.json` and `.to` envelopes to the evidence
   bundle; cross-link them from `specs/sorafs_gateway_dns_design_attendance.md`
...
| T‑2 h | Networking TL | Re-run the GAR telemetry script and append deltas to `specs/sorafs_gateway_dns_design_gar_telemetry.md`. |
...
Copy the following skeleton into `specs/sorafs_gateway_dns_design_minutes.md`
...
artefact bundle and link it from `specs/sorafs_gateway_dns_design_attendance.md`.
...
1. Update `specs/sorafs_gateway_dns_design_pre_read.md` and the agenda
...
4. If conformance failures occurred, open GitHub issues with the `sf-5a` label
   and list them in `specs/sorafs_gateway_dns_design_attendance.md`.
