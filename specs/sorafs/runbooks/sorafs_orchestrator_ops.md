...
3. **Dry-run with fixtures** — Exercise the same command against the public fixtures in `fixtures/documentation/sorafs_ci_sample/` to ensure the orchestrator binary matches the expected version before touching production payloads.
...
1. **Capture the raw log stream.** Mirror the `telemetry::sorafs.fetch.*` span for every fetch during the burn-in window (NDJSON or the plain-text format shown in `fixtures/documentation/soranet_pq_ratchet_fire_drill.log`). Store the rotated files under `artifacts/sorafs_orchestrator/<stamp>/telemetry/` so reviewers can replay them later.
...
- `dashboards/alerts/soranet_policy_rules.yml` contains the
  ...
  used in `specs/sorafs_orchestrator_rollout.md`.
...
- Drills should exercise the script weekly even when no alerts fire; log each
  run under `specs/sorafs/runbooks/sorafs_orchestrator_ops.md` §6.4 so the
