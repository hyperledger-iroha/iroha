...
escape the HSM boundary. This does not complete the blocker: deployment-owned
...
HSM/KMS identity and live owner/key/policy eligibility used to construct that
transaction.
...
panel-notification receipt, saturated notification capacity fails with
...
receipt compaction remains open.
...
The repository does not ship a supervised deployment-owned broker executable
or genuine backends for enrollment/wallet hybrid secrets, issuer HSM signing,
KMS wrapping, API authentication, registry submission/readback, private
...
The source also exports the exact eight-header inbound verifier for endpoint
scope, method, canonical absolute URL/query, selected public headers, body
length and BLAKE3 digest, freshness, nonce, and pinned Ed25519 signature. It is
not installed in deployment-owned Kubo/head ingress, and its low-level bounded
caller-owned replay cache remains process-local until the receiver supplies its
own sealed cross-replica adapter.
...
signers for Kubo/head operations, install the inbound verifier with durable
receiver-side shared replay state, and supervise two instances with successful
CAS/failover,
...
with no errors. The deterministic promotion/replay archive contains 20
top-level inputs: topology qualification, resilience qualification,
foundational envelope, and the 17 lane summaries. The nine prerequisite
package manifests are transitively committed by the foundational envelope;
they are not extra aggregate input slots.
