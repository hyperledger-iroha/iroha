...
This does not wire configuration, broker, Torii capture, a sealed ordered
gateway outbox, or the regional callback owner.
...
the stream-token owner allocates through sealed monotonic state and invokes the
durable journal callback through an ordered outbox; the complete SDK/native
validation matrix passes; the bounded exact retained-history
...
`specs/sorafs_pop_credentials_plan.md` under “Runtime Adapter Blocker
Runbook.”
...
not installed in deployment-owned Kubo/head ingress, and its bounded
caller-owned replay cache is process-local rather than sealed or cross-replica.
...
shared replay state, and supervise two instances with successful CAS/failover,
