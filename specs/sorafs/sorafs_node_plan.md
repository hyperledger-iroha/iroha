...
|----------|-----------|-------|
| `GET /v1/sorafs/pin`, `POST /v1/sorafs/pin/register`, `GET /v1/sorafs/pin/{digest_hex}` | Read finalized bounded pin pages, register paid public pins for authenticated accounts, and fetch one exact bounded native manifest record. | Registration accepts one exact-network signed transaction with one `RegisterPinManifest`; Core derives lifecycle epochs, charges deterministic quota/fees, and rejects retired client epoch fields. Lists return `PinManifestPageV1` at one finalized height/hash with an exclusive digest cursor, row/byte ceilings, and O(1) consensus-maintained charged usage. Detail reads return `PinManifestFinalizedRecordV1` and accept only the paired expected finalized height/hash precondition. |
| `POST /v1/sorafs/storage/fetch`, `POST /v1/sorafs/storage/token` | Run an operator-only legacy local range diagnostic and issue operator-authenticated storage access tokens. Storage ingest has no public route. | Authenticate the exact request before decoding, enforce token policy and scheduler/back-pressure limits, and never use the legacy JSON fetch as an unsigned remote-hydration fallback. Provider bytes arrive only through the durable finalized-ledger outbox. |
| `GET /v1/sorafs/storage/manifest/{manifest_id}`, `GET /v1/sorafs/storage/plan/{manifest_id}`, `GET /v1/sorafs/storage/car/{manifest_id}`, `GET /v1/sorafs/storage/chunk/{manifest_id}/{chunk_digest}` | Serve bounded manifest metadata and deterministic chunk plans; CAR and chunk bytes require a request-bound protocol stream capability. | Keep readback arrays bounded while preserving total counts, verify digest/path bindings, and validate stream-token identity, nonce, range, concurrency, request, and byte budgets before streaming bytes. |
| `GET /v1/sorafs/storage/peers`, `GET /v1/sorafs/storage/state`, `POST /v1/sorafs/proof/stream` | Report peer state, expose operator-only local storage state, and request authenticated, finalized-pin-bound PoR witnesses. The unauthenticated local PoR sampling route is retired; proof and verdict admission use the governed capacity lifecycle. | Reuse chunk-store sampling behind the authenticated proof stream, verify generated witnesses against the committed manifest root, update telemetry, and preserve governance-verdict replay state. |
