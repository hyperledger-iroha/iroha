...
| `POST /v1/sorafs/storage/token` | Issue operator-authenticated storage access tokens. Storage ingest and JSON range fetches have no public routes. | Authenticate the exact request before decoding and enforce token policy and scheduler/back-pressure limits. Provider bytes arrive only through the durable finalized-ledger outbox; clients read them through capability-bound CAR/chunk routes. |
