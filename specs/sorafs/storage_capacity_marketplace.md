...
- The trustless gateway profile enumerates the HTTP request/response matrix, proof formats, and
  ...
  `specs/sorafs_gateway_profile.md` for the normative specification.
...
- Capacity disputes are first-class in the capacity registry: `/v1/sorafs/capacity/state`
  ...
  requests and note the response’s `dispute_id_hex` for revocation and audit tracking.【crates/iroha_torii/src/sorafs/api.rs:520】【crates/iroha_torii/src/routing.rs:4889】【specs/sorafs/dispute_revocation_runbook.md:45】
...
- `/v1/sorafs/capacity/state` now emits a deterministic `fee_projection` payload alongside the live
  ...
  `specs/grafana_sorafs_metering.json` so billing teams can monitor GiB·hour accrual,
  projected nano-SORA fees, and SLA compliance in real time.【crates/iroha_torii/src/sorafs/api.rs:268】【crates/iroha_torii/src/routing.rs:5143】【specs/grafana_sorafs_metering.json:1】
...
- Provider onboarding & exit runbook covering artefact storage, role-based approvals,
  and Torii verification commands (`specs/sorafs/capacity_onboarding_runbook.md`).
...
3. Archive the nightly reconciliation digest alongside the governance packets in
   `fixtures/documentation/sorafs_capacity_marketplace_validation/` so treasury approvals can
   reference deterministic evidence. The validation checklist in
   `specs/sorafs/reports/capacity_marketplace_validation.md` links to the latest
...
3. Follow the escalation workflow in `specs/sorafs/dispute_revocation_runbook.md`—
   ...
   `specs/sorafs/reports/capacity_marketplace_validation.md` so reviewers can trace
...
2. Verify Torii behaviour by calling `POST /v1/sorafs/capacity/declare` and
   ...
   `specs/sorafs/runbooks/multi_source_rollout.md`.
3. Exercise the exit path once per release candidate:
   ...
   - Capture the operator checklist (access revocation, manifest cache cleanup, treasury
     sign-off) in `specs/sorafs/runbooks/sorafs_node_ops.md` and update the validation
     evidence bundle with the exit ticket ID.
4. Archive the signed artefact bundle described in
   `specs/sorafs/capacity_onboarding_runbook.md` so governance, treasury, and SRE
...
`specs/sorafs/reports/capacity_marketplace_validation.md` as rollout
evidence changes.
