...
reference (`specs/sorafs_proto_plan.md`) so operators, SDK authors, and
...
|--------|-----------|-------------------|--------------------|
| Chunking & proofs | CDC profile | Target 256 KiB (64 KiB min, 512 KiB max) with BLAKE3 digests, two-level (64 KiB / 4 KiB) PoR tree per chunk. | `node scripts/check_sf1_vectors.mjs` and `ci/check_sorafs_fixtures.sh` (see `specs/sorafs/chunker_conformance.md`). `sorafs_car::ChunkStore` emits the PoR tree (`sorafs_car/src/lib.rs`). |
...
- Dashboards to watch: `torii_sorafs_admission_total` (result labels), warning
  ratios from `specs/sorafs/provider_advert_rollout.md`, and the range
...
- HTTP clients MUST forward `Sora-PDP` and (when enabled) `Sora-PoTR-*` headers
  ...
  (`specs/soradns_gateway_content_binding.md`).
- Verification hooks:
...
- Issued orders expire if providers do not acknowledge (`precommit`) within
  5 minutes. Operators verify expiry from finalized, height/hash-bound
  replication-order queries and the GovernanceLog DAG.
- Completion requires:
  1. `precommit` ack from the provider,
  2. QA PoR `PASS` published to the governance DAG, and
  3. a finalized completion record committed within the same epoch.
- Artefacts: `GovernanceLogNodeV1` entries (see
  `specs/sorafs_governance_dag_plan.md`) reference the manifest digest,
...
- `sorafs_cli fetch` and SDKs default to `transport_policy=soranet-first` with
  ...
  (see `specs/sorafs_orchestrator_rollout.md`).
...
- Handshake: hybrid X25519 + Kyber (ML-KEM‑768) per `crates/iroha_crypto` and
  `crates/soranet_pq`. The PQ rollout plan in `specs/soranet/pq_rollout_plan.md`
...
- Telemetry: `torii_stream_transport_policy_total` and
  ...
  `specs/ops/soranet_transport_rollback.md`.
