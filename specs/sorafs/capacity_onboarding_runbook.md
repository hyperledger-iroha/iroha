...
(`specs/sorafs/reports/capacity_marketplace_validation.md`). Follow it for
...
|------|------------------|--------------------|
| Provider | Draft `CapacityDeclarationV1` spec, capture hardware inventory, supply attestation bundles if mandated by `specs/sorafs/provider_admission_policy.md`. | `specs/<provider>.json`, hardware affidavit, contact roll. |
...
1. Governance creates a ballot referencing the payload hash and logs the vote
   outcome in `fixtures/documentation/sorafs_capacity_marketplace_validation/<date>_onboarding_signoff.md`.
...
3. Support/SRE note the onboarding in `specs/sorafs/ops_log.md`, linking
...
4. **Archive artefacts.** Store the telemetry request/response, final state
   ...
   `fixtures/documentation/sorafs_capacity_marketplace_validation/<date>_exit_signoff.md`.
...
`specs/sorafs/storage_capacity_marketplace.md`.
...
fixtures/documentation/sorafs_capacity_marketplace_validation/
