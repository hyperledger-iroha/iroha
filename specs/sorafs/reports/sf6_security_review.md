...
|----|----------|------|---------|------------|
| SF6-SR-01 | High | Release signing | The retired CLI path derived an ephemeral key from unverified OIDC token bytes and trusted a public key carried in the same self-asserted bundle. | Removed that CLI surface and all production callers. The current release path requires `signing_provider=authenticated_external_signer`, a governed raw Ed25519 public key and reviewed fingerprint, plus a SHA256-pinned native `iroha app sorafs toolkit release-manifest` verifier. Production promotion also requires signer authorization and exact completed-operation evidence; its remaining integration is tracked in `specs/sorafs/v1_closure_ledger.md`. |
