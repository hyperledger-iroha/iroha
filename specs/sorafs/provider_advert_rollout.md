...
Grafana export: [`specs/grafana_sorafs_admission.json`](../grafana_sorafs_admission.json).
...
`specs/grafana_sorafs_admission.json`; copy it into your Grafana folder if
...
- Council-signed admission envelope from another genesis network → reject
  startup or registration against the authoritative local `State` network
  identity. A mismatched provider advert → reject with
  `reason="network_mismatch"`.
- Signature failure after changing network identity, timestamps, policy flags,
  algorithm, public key, or body → reject; all are bound by the domain-separated
  envelope signature.
...
- Durable replay checkpoint write/load failure → fail closed with
  ...
  disable discovery on startup corruption or a checkpoint from another network.
  Investigate filesystem integrity and
