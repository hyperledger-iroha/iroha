...
- **Registry alignment** — confirm chunker profiles and manifests reference the
  same `namespace.name@semver` tuple (`specs/sorafs/chunker_registry.md`).
- **Admission policy** — review the signed provider adverts and alias proofs
  needed for `manifest submit` (`specs/sorafs/provider_admission_policy.md`).
- **Pin registry runbook** — keep `specs/sorafs/runbooks/pin_registry_ops.md`
...
3. **Telemetry dashboards**
   - Import `fixtures/documentation/sorafs_proof_streaming_dashboard.json` into Grafana.
   - Layer additional panels for pin registry health
     (`specs/sorafs/runbooks/pin_registry_ops.md`) and chunk range stats.
4. **Multi-source enablement**
   - Follow the staged rollout steps in
     `specs/sorafs/runbooks/multi_source_rollout.md` when turning on the
     orchestrator, and archive the scoreboard/telemetry artefacts for audits.

## Incident handling

- Follow the escalation paths in `specs/sorafs/runbooks/`:
