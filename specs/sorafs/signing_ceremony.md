...
- Regenerate fixtures with `export_vectors --write` using an empty mode-0700
  external `--staging-root` and an absolute private `--signing-key-file`. The
  generator validates the complete signed stage before atomically publishing
  checked-in paths; `--check` leaves the repository read-only.
...
the vote back to the signed Norito artefact.【specs/governance_playbook.md:24】
...
1. Fetch the retention policy snapshot (`torii.da_ingest.replication_policy`) and
   ...
   `specs/da/replication_policy.md`). Store the CLI output or CI artefact.
2. Record the manifest digest, governance tag, blob class, and retention values
   inside `fixtures/documentation/da_manifest_review_template.md`. Attach the filled
...
3. Include moderation/compliance context when the request originated from a
   ...
   escalation record (`specs/sorafs_gateway_compliance_plan.md`,
