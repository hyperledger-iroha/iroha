...
`iroha_config` manifest (see `specs/references/configuration.md`) and reload
it through the config watcher described in `specs/android_runbook.md`.
...
> (`specs/android_runbook.md` §§1–2) captures the required log lines.
...
`specs/sorafs/developer/sdk/index.md` for gateway request details.
...
`specs/sdk/android/readiness/signal_inventory_worksheet.md`:
...
`specs/sdk/android/readiness/signal_inventory_worksheet.md`, including the hashed-authority
...
(`specs/torii/nrpc_spec.md`) so HTTP and Norito transports land on the shared dashboards:
...
| `android_sdk_submission_latency_seconds_bucket` | Histogram | End-to-end latency (device → Torii ack). Dashboards render it as `android_sdk_submission_latency_p95` with the thresholds captured in `specs/android_support_playbook.md`. |
...
| `android_sdk_offline_replay_errors_total` | Counter | Counts permanent queue failures (labels: `reason`, `transport`). AND7 chaos scenarios (see `specs/sdk/android/telemetry_chaos_checklist.md`) assert this stays zero during brownouts. |
...
`specs/sdk/android/readiness/schema_diffs/`. Run
...
- Follow `specs/android_runbook.md` for Torii/network timeout, StrongBox,
...
- Chaos exercises (`specs/sdk/android/readiness/labs/telemetry_lab_01.md`)
inject queue growth, exporter brownouts, and override tokens; archive the
resulting artefacts under `specs/sdk/android/readiness/archive/<date>/`.
...
contract documented in `specs/torii/norito_rpc_brief.md` and satisfies the
...
- `specs/android_runbook.md`
- `specs/sdk/android/telemetry_redaction.md`
- `specs/sdk/android/readiness/`
