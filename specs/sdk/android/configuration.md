...
operator runbooks (see `specs/android_runbook.md`) can rely on the same
...
- If manifest validation fails, emit the `android.telemetry.config.reload`
  ...
  telemetry disabled) per `specs/android_runbook.md` §1/§2.
...
- Document the queue path in the manifest so support engineers can pull artefacts
  during incidents (`specs/android_runbook.md` §3). The operator console
...
1. Drop the new manifest alongside the old file and signal the
   ...
   `specs/android_runbook.md`). The watcher compares digests, reloads via
...
2. If telemetry needs a temporary override, follow the override workflow in the
   ...
   `specs/sdk/android/telemetry_override_log.md` in sync with the active
...
1. **Schema diff:** Run
   ...
   `specs/sdk/android/telemetry_schema_diff.md`.
...
4. **Log verification:** Capture the manifest digest, telemetry exporter target,
   ...
   the release evidence bundle (`specs/sdk/android/readiness/...`).
