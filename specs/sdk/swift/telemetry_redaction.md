...
- `specs/sdk/swift/telemetry_chaos_checklist.md` documents the override/salt drift rehearsal
...
|-----------|---------|------------|-----------------------|-------|
| `swift.torii.http.request` | Span (`ToriiClient`) | `authority_hash`, `route`, `status_code`, `latency_ms`, `one_shot` | `authority_hash = blake2b_256(authority || salt)`; traces retained 30 days | Mirrors Rust `torii.http.request`; `route` excludes query/body data and signed pipeline submissions always set `one_shot=true`. |
| `swift.torii.http.ambiguous` | Event | `authority_hash`, `route`, `error_code`, `hash_prefix` | Same authority hash as above; transaction hash is truncated to 12 hex characters; retained 30 days | Records a one-shot signed submission whose transport outcome requires hash/status reconciliation. It never triggers replay. |
...
| `swift.offline.queue_depth` | Gauge | `queue_kind`, `authority_hash`, `device_profile_bucket` | Hash authority + bucket device profile; retained 30 days | Covers Connect session queues only; signed pipeline envelopes are never automatically queued or replayed. |
...
`specs/sdk/mobile_device_profile_alignment.md` so IOS7 telemetry lines up
...
4. **Runbook updates** — Update `specs/swift_parity_triage.md` with the
...
- **Decision logging:** outcomes recorded in `specs/sdk/swift/telemetry_redaction_minutes_20260212.md`
...
1. **Device-profile alignment (due 2026-03-01).** ✅ Completed — Swift now
   references the shared mapping in `specs/sdk/mobile_device_profile_alignment.md`
