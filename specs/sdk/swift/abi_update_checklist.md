...
| Digest/status updates | Swift Program PM / Docs | `status.md`, `specs/status/swift_weekly_digest.md`, exporter automation in `scripts/swift_status_export.py` |
...
3. **Keep policy helpers current.**
   ...
   - Document the change in `crates/ivm/docs/syscalls.md` and `crates/ivm/docs/pointer_abi.md`
     (these files feed `specs/ivm_syscalls.md`).
...
3. **Bridge enforcement.** If the ABI introduces new bridge calls or metadata,
   ...
   follow the instrumentation guide in `specs/sdk/swift/native_bridge_instrumentation_checklist.md`
...
5. **Docs.** Keep developer docs pinned to ABI v1 if APIs change
   (`specs/sdk/swift/connect_workshop.md`, `specs/sdk/swift/support_playbook.md`, etc.).
...
2. Update `status.md` (Latest Updates + Risks) and the weekly digest
   (`specs/status/swift_weekly_digest.md`) with:
