...
| **Recording** | Stored under `specs/sdk/swift/archive/meetings/2026-11-21/` (audio, chat export, and HackMD export) within 24 h |

## Objectives

1. Validate the Torii pipeline + Norito RPC spec (`specs/torii/nrpc_spec.md`)
   against the Swift parity blockers tracked in `specs/sdk/swift/connect_risk_tracker.md`
...
- `specs/torii/nrpc_spec.md` (current Norito RPC spec)
- `specs/torii/norito_rpc_rollout_plan.md` + tracker (`specs/torii/norito_rpc_tracker.md`)
- `specs/status/swift_weekly_digest.md` (latest gate/risk highlights)
...
1. **Spec freeze memo** — update `specs/torii/nrpc_spec.md` with annotated
   deltas and link the meeting notes; attach HackMD export under
   `specs/sdk/swift/archive/meetings/2026-11-21/`.
2. **Fixture/CI commitments** — add the agreed cadence/owner notes to
   `specs/android_fixture_changelog.md` (if impacted) and the Swift parity
   dashboard backlog.
3. **Telemetry checklist** — extend `specs/status/swift_weekly_digest.md`
   + `specs/status/swift_weekly_digest_template.md` so `/v1/pipeline`
...
- Store recordings/notes in
  `specs/sdk/swift/archive/meetings/2026-11-21/` for auditors; include the
...
- If Torii changes block the freeze, escalate via the SDK council and update the
  risk tracker (`specs/sdk/swift/connect_risk_tracker.md`) immediately.
