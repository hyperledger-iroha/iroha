...
- Produce owner-assigned actions that unblock the IOS3 risks tracked in
  `specs/sdk/swift/connect_risk_tracker.md`.
...
- `specs/sdk/swift/connect_risk_tracker.md` (risk context and week-over-week deltas).
- `specs/status/swift_weekly_digest.md` (open issues and alerts for Connect telemetry).
- `specs/sdk/swift/readiness/archive/2026-03/connect_fault.md` (prior fault-analysis baseline).
- `specs/sdk/swift/connect_replay_fixtures.md` (capture/replay recipe used to collect workshop fixtures).
...
- Pre-session checks (facilitator runs these 1–2 hours before start):
  ...
  - Keep a copy of the reference bundle at `specs/sdk/swift/readiness/archive/2026-05/connect/` handy for dry-runs; replace `session.json` with live Torii values when running the workshop.
...
|-----|----------------|-------|--------|-------|
| Heartbeat loss and reconnect | Replay trace with dropped heartbeats (`specs/sdk/swift/readiness/archive/2026-05/connect/heartbeat_loss.json`) → expect resumable session + OTLP emission | Swift Connect maintainers | Planned | Capture hashes + OTLP payload for `ConnectHeartbeatRecoveryTests` |
...
- Updates filed for `specs/status/swift_weekly_digest.md` and
...
| [IOS3-CONNECT-004](issues/IOS3-CONNECT-004.md) | WebSocket fixture pack (heartbeat, salt rotation, multi-observer) | SDK Program PM | In progress | Fixtures referenced in the coverage gap table; stored under `specs/sdk/swift/readiness/archive/2026-05/connect/`. |
