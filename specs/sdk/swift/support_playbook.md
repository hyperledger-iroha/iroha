...
(`specs/sdk/swift/reproducibility_checklist.md`), telemetry readiness plan,
...
4. After-action review (AAR) template: reuse
   `specs/status/swift_weekly_digest.md` sections and file within
   **5 business days**.
...
3. **Reproducibility:** Follow
   `specs/sdk/swift/reproducibility_checklist.md`, capturing
...
- Redaction policy lives in `specs/sdk/swift/telemetry_redaction.md`;
  overrides must be recorded via
  helper) and logged in the parity digest.
- Quarterly chaos drills follow
  `specs/sdk/swift/telemetry_chaos_checklist.md` (salt rotation,
