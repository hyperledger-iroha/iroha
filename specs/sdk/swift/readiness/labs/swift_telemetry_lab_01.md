...
| D — Offline Queue Replay | Confirm `swift.offline.queue_depth` parity + replay runbook. | Run NoritoDemoXcode offline flow or `swift test --filter OfflineQueueSmokeTests`, toggle airplane mode, replay once connectivity restored. | Pending queue snapshots, `specs/sdk/swift/readiness/screenshots/<date>/queue-replay.png`. |
