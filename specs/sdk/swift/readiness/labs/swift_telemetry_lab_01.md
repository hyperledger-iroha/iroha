...
This lab guide complements `specs/sdk/swift/telemetry_chaos_checklist.md`
...
| D — One-shot Ambiguity | Confirm a failed signed submission emits `swift.torii.http.ambiguous`, sends no second POST, and resolves by transaction hash. | Run the one-shot pipeline transport tests, induce an airplane-mode failure, then query pipeline status for the original envelope hash. | Request-count trace, ambiguity event, and hash/status reconciliation capture. |
...
4. Store logs under `specs/sdk/swift/readiness/archive/YYYY-MM/`.
5. Upload screenshots to `specs/sdk/swift/readiness/screenshots/<date>/`
...
Store completed templates in `specs/sdk/swift/readiness/archive/2026-03/`.
