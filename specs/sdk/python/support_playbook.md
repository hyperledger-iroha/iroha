...
4. **Authentication and provenance:** Bind the reviewed package checksums into
   ...
   Ed25519 `authenticated_external_signer` provider using software or optional
   hardware custody under the same reviewed public-key contract, and pass
   ...
   boundary; it cannot replace manifest authentication. Verification makes no
   claim about key origin or exportability.
...
3. **Evidence:** Store logs, fixture diffs, release manifests, and console
   output under `specs/sdk/python/readiness/incidents/<YYYY-MM-DD>/`.
...
- `specs/sdk/python/release_automation.md` — detailed release checklist.
- `specs/sdk/python/connect_end_to_end.md` — Connect walkthrough cited in
  partner enablement.
