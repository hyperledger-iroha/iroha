...
| 2026-01-15 | Governance council | Requested explicit `/v1/pipeline` adoption status, governance vote timing, and risk owners so the council can tie digest updates to rollout gates. | Added a “Governance Watchers” block to `specs/status/swift_weekly_digest_template.md` (and mirrored it in the dated digest) so owners + status/risk notes are recorded next to the metrics snapshot. | Closed |
...
| 2026-04-26 | Governance council | Asked the weekly digest to highlight `/v1/pipeline` adoption status plus the upcoming governance vote date so release gate owners can see blockers at a glance. | Added the “Governance Watchers” table to `specs/status/swift_weekly_digest.md`, tying `/v1/pipeline` and vote readiness rows to named owners with the CR-2 dependency called out. | Closed |
...
| 2026-05-05 | Torii Platform TL | Asked the digest to surface the Torii spec review schedule and link the agenda so `/v1/pipeline` freeze work stays aligned with IOS2 risks. | Added a “Torii spec review (IOS2)” row to the Governance Watchers block in `specs/status/swift_weekly_digest.md` pointing to `specs/sdk/swift/torii_spec_review.md` and the locked 2026-11-21 session. | Closed |
