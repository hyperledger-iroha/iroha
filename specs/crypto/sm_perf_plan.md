...
- Aggregated JSON files feed directly into the baseline updates and are attached to the performance review in `specs/crypto/sm_perf_baseline_comparison.md`.
...
- When hardware or toolchains change, refresh comparison tolerances/notes in `specs/crypto/sm_perf_baseline_comparison.md`, tighten `ci/check_sm_perf.sh` tolerances if the new medians stabilise, and align any dashboard/alert thresholds with the new baselines so ops alarms stay meaningful.
