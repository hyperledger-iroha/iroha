...
4. **Record keeping:** decision artifacts (vote roll call, KPI baseline, steward contract hash) are stored under `specs/governance/sns/<proposal-id>/`.
...
- Every suffix must publish quarterly transparency bundles (runbooks, KPI dashboards, financial statements) under `specs/sns/reports/<suffix>/<YYYY-MM>.md`.
- GAR, ZF (Zonefile), and DNS artifacts must be signed and pinned via the deterministic registry workflow (`specs/soradns/soradns_registry_rfc.md`).
...
4. **Activation & Evidence (Days 22-28).** Registry/DNS operators enforce the change (updated GAR templates, dispute queues, or name restrictions). Evidence bundles (legal memo hash, steward acknowledgement, configuration diff) are stored under `specs/sns/regulatory/<jurisdiction>/<YYYY-MM>.md` and referenced in guardian runbooks.
