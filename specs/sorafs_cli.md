...
Looking for a turnkey script? `fixtures/documentation/sorafs_cli_quickstart.sh` wraps the
...
README in `fixtures/documentation/sorafs_ci_sample/`. Those fixtures are not
...
cargo run -p sorafs_orchestrator --bin sorafs_cli -- \
  ...
  --manifest fixtures/documentation/ai_moderation_perceptual_registry_202602.json
```
...
`specs/sorafs_appeal_pricing_plan.md`:
...
`fixtures/documentation/ministry/appeal_pricing_config_baseline.json`, keeping the CLI in
...
manifest (baseline: `fixtures/documentation/ministry/appeal_settlement_config_baseline.json`)
