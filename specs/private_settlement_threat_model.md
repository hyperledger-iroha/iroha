...
| APS-T02 | A capsule, wrapped DEK, policy, route, bundle, leg, roster incarnation, or key epoch is substituted. | XChaCha20-Poly1305 authenticates padded plaintext; X25519/ML-KEM-768 wraps the random DEK independently to each governed auditor; canonical AAD binds every context field; approval signatures bind statement/proof/capsule/policy digests and expiry. | Cryptographic implementation and AAD completeness require independent review and adversarial vectors. |
...
| APS-T06 | A stale root, duplicate nullifier/output, old epoch, or replayed bundle is accepted. | Committee staging reserves pool/nullifier/output keys durably; global planning rechecks current WSV roots, epochs, governance, unique state keys, prior receipts, and abort markers; finalized writes advance each leg once. | Restart/finality reconciliation must release only authoritative terminal reservations; disk corruption must fail closed. |
| APS-T07 | A forged or malformed quorum bypasses a committee. | Authority records require exactly four validators and proofs of possession; certificates require exactly three distinct canonical signers over purpose-separated bodies; all nodes verify locally. | Safety assumes at most one Byzantine validator per committee. Real message-control tests must cover withheld, delayed, reordered, and healed votes. |
...
| APS-T09 | Key rotation makes retained capsules undecryptable or accepts stale approvals. | Policies bind key epochs and activation/retirement heights; stale or insufficient approvals fail Prepare; operators must retain retired decryption keys or rewrap capsules through the regulatory retention period. | Rewrapping operations and HSM/KMS retention are deployment-controlled and must be exercised in the release recovery runbook. |
...
| APS-T11 | Oversized proofs, capsules, participant lists, or carrier records exhaust CPU, memory, disk, or network. | Configuration and hard protocol bounds cap 2–255 legs, proof/capsule/carrier bytes, canonical padding classes, retention, file count, and total sidecar bytes; parsing and feature checks fail before expensive work where possible. | Internet-facing deployments still need connection/rate quotas. N=255 codec/wire-size tests and N=2/3/4/8/16 resource measurements are release gates. |
...
- Auditor signing-key compromise: retire the key epoch, raise or replace the
  policy threshold, reject stale approvals, and audit approvals issued during
  the exposure window.
