...
| APS-T04 | Dummy activity or unused fixed slots leak cardinality, prevent successive spends, or permit value creation. | Exactly two input and three output slots are encoded. Positive inputs require membership; zero inputs are virtual dummies with the same hash/path topology. Activity is fixed by the private value, and a bundle/leg/slot/domain-bound memo gives each virtual dummy a fresh commitment and nullifier. The relation enforces balance and forbids the directional public-balance bridge. | AIR, activity, nullifier, and three-output constraints require independent cryptographic review plus positive/negative proof tests. A wallet regression exercises two successive frontier appends without a second bootstrap. |
...
| APS-T08 | An auditor signs plaintext that misstates the proved input notes or violates local policy. | The AIR computes the required public `audit_input_commitment` from both exact ordered note openings, including value, authority digest, rho, blinding, and memo. The auditor decrypts locally, recomputes that binding and canonical activity from the capsule, checks all other commitments and policy/key validity, and signs a purpose-separated approval; every validator revalidates the approval threshold. Payer signatures and a self-consistent capsule alone cannot replace this input binding. | A policy-threshold compromise defeats content-policy assurance and capsule confidentiality for that policy. Separate organizations and independently qualified custody controls are recommended. |
...
| APS-T18 | Snapshot rollback, malformed schema, or corruption silently drops private-settlement replay or active Prepare-lock state. | Snapshots require the exact current 188-field `World` schema, including all eight canonical settlement maps; missing, reordered, renamed, additional, or retired fields are rejected. Populated roots, nullifiers, outputs, replicated staged locks, receipts, and terminal markers remain committed. The snapshot-skipped recipient index is reconstructed from canonical outputs and cross-checked before startup accepts state. | Operators must retain signed snapshot/Kura recovery evidence and deploy only snapshots emitted by the current V1 schema. |
