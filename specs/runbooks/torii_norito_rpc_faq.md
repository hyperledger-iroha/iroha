...
`specs/runbooks/torii_norito_rpc_canary.md`.
...
| `transport.norito_rpc.*` in `specs/config/client_api.md` | Client-facing overrides (CLI / SDK discovery). | Use `cargo xtask client-api-config diff` to inspect pending changes before pushing them to Torii. |
...
- **Canary / staging runs** — follow `specs/runbooks/torii_norito_rpc_canary.md`.
...
- **Production promotion** — execute the stage report template in
  `specs/torii/norito_rpc_stage_reports.md`. Record the config hash,
...
canary runbook (`specs/runbooks/torii_norito_rpc_canary.md`).
...
2. Link the release ticket to the latest stage report in
   `specs/torii/norito_rpc_stage_reports.md`.
