...
|----------|-------------|--------|---------|
| Transparency summary | Human-readable report with executive summary, highlights, risk items | Markdown → PDF | `specs/ministry/reports/<YYYY-Q>.md` → `artifacts/ministry/transparency/<YYYY-Q>/summary.pdf` |
...
| Source | Feed | Notes |
|--------|------|-------|
| Moderation ledger (`specs/sorafs_transparency_plan.md`) | Hourly `ModerationLedgerBlockV1` exports stored in CAR files | Already live for SFM-4c; reused for quarterly aggregation. |
| AI calibration + false-positive rates | `specs/sorafs_ai_moderation_plan.md` fixtures + calibration manifests (`fixtures/documentation/ai_moderation_calibration_*.json`) | Metrics aggregated per policy, region, and model profile. |
...
[`specs/ministry/emergency_canon_policy.md`](emergency_canon_policy.md), ensuring
...
- Publication details, metrics tables, and approvals are captured in `specs/ministry/reports/2026-Q3.md`, which now serves as the canonical reference for auditors reviewing the Q3 window.
...
- AI moderation accuracy: per-model FP/FN rate, drift vs calibration target, and alert thresholds tied to `specs/sorafs/reports/ai_moderation_calibration_*.md`.
...
- Volunteer briefs & panel diversity: submissions per language, conflict-of-interest disclosures, publication lag. Balanced brief fields are specified in `specs/ministry/volunteer_brief_template.md`, ensuring fact tables and moderation tags are machine readable.
...
| T + 10 d | Draft summary + data appendix | Docs/DevRel + Policy analyst | `specs/ministry/reports/<YYYY-Q>.md` |
...
- Red-team linkage: pass one or more `--red-team-report specs/ministry/reports/<YYYY-MM>-mod-red-team-<scenario>.md` files to the ingest step so the transparency snapshot and sanitized metrics carry drill IDs, scenario classes, evidence bundle paths, and dashboard SHAs alongside the ledger/appeal/denylist data. This keeps MINFO-9 drill cadence reflected in every transparency packet without manual edits.
- Volunteer submissions must follow `specs/ministry/volunteer_brief_template.md` (example: `fixtures/documentation/ministry/volunteer_brief_template.json`). The ingest step expects a JSON array of those objects, automatically filters `moderation.off_topic` entries, enforces disclosure attestations, and records fact-table coverage so dashboards can highlight missing citations.
...
| Create quarterly report template (`reports/<YYYY-Q>.md`) | 🈴 Completed | Template added at `specs/ministry/reports/2026-Q3-template.md`; copy/rename per quarter and replace the `{{...}}` tokens before publishing. |
