...
1. **Agenda proposals.** One or more files that follow
   [`specs/ministry/agenda_council_proposal.md`](agenda_council_proposal.md).
...
2. **Duplicate registry (optional).** A JSON file matching
   `fixtures/documentation/ministry/agenda_duplicate_registry.json`. Conflicts are
...
cargo xtask ministry-agenda impact \
  ...
  --policy-snapshot fixtures/documentation/ministry/policy_snapshot_example.json \
  ...
  "proposals": [
    {
      ...
      "total_targets": 2,
      "source_path": "fixtures/documentation/ministry/agenda_proposal_example.json",
      "hash_families": [
