...
- ZK-envelope ballots attach deterministic proof URIs (for example,
  `sorafs://proofs/pj-2026-02/juror-5`) so downstream
...
cargo xtask ministry-jury sortition \
  --roster fixtures/documentation/ministry/policy_jury_roster_example.json \
...
- `--roster` accepts a deterministic PoP roster (JSON example:
  `fixtures/documentation/ministry/policy_jury_roster_example.json`).  Each entry
...
`fixtures/documentation/ministry/policy_jury_sortition_example.json` so SDK teams can
