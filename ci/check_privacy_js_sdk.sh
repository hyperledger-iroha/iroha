...
"${NODE_BIN}" --test \
  test/activeVerifyingKeyIds.test.js \
  test/verifierBackendRegistry.test.js \
  test/toriiVerifyingKey.test.js
"${NODE_BIN}" scripts/bundle-size-check.mjs
"${NODE_BIN}" --test test/bundleSizeCheck.test.js
...
"${NODE_BIN}" --test test/package_dist.test.js
