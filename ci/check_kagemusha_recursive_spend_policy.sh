...
def check_rust_reserved_lineage_policy():
    ...
    if not re.search(
        r"KAGEMUSHA_RECURSIVE_AGGREGATION_PROOF_INSTANCE_COLUMNS\s*:\s*usize\s*=\s*55\s*;",
        core,
    ):
        fail("recursive Kagemusha proof public instance width must remain 55")
...
def check_docs_reserved_lineage_policy():
    ...
    for needle in (
        ...
        "Native bridge and SDK verifier wrappers return `valid = false` with a stable missing-record diagnostic",
