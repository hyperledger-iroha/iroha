...
ADVERSARIAL_COVERAGE = {
    "crates/iroha_data_model/src/offline/mod.rs": (
        ...
        "one_hop_record_backed_lineage_wrong_record",
    ...
    "crates/connect_norito_bridge/src/lib.rs": (
        ...
        "fn kagemusha_recursive_spend_redeem_bridge_rejects_witnessless_reserved_lineage_before_backend",
        "witnessless Reserved-lineage redeem must fail closed",
...
ACTIVE_KAGEMUSHA_TODO_SCAN_PATHS = (
    ...
    "crates/iroha_data_model/src/offline/mod.rs",
    "crates/iroha_core/src/zk.rs",
    "crates/iroha_core/src/bin/kagemusha_recursive_spend_v3_bundle.rs",
    "crates/iroha_core/src/zk/kagemusha_finality.rs",
    ...
    "crates/iroha_data_model/tests/kagemusha_v2_semantic_dag_golden.rs",
    ...
    "javascript/iroha_js/test/kagemushaRecursiveSpend.test.js",
    "javascript/iroha_js/test/toriiBrowserClient.test.js",
    "javascript/iroha_js/test/package_dist.test.js",
    "python/iroha_python/README.md",
    "python/iroha_python/iroha_python_rs/src/lib.rs",
    "python/iroha_python/src/iroha_python/kagemusha.py",
...
ACTIVE_KAGEMUSHA_TODO_CONTENT_SCAN_PATHS = (
    *REQUIRED_KAGEMUSHA_RUNNER_INPUT_TODO_CONTENT_SCAN_PATHS,
    ...
    "java/iroha_android/src/main/java/org/hyperledger/iroha/android/offline/OfflineOperationCodec.java",
    "java/iroha_android/src/main/java/org/hyperledger/iroha/android/offline/OfflineOperationStatus.java",
    ...
    "kotlin/core-jvm/src/main/java/org/hyperledger/iroha/sdk/offline/OfflineOperation.kt",
...
TORII_KAGEMUSHA_REDEEM_COVERAGE = {
    ...
    "crates/iroha_data_model/src/offline/mod.rs": (
        ...
        "pub lineage_witness: Option<KagemushaRecursiveSpendLineageWitnessV2>",
    ...
    "crates/iroha_torii/src/offline_commands.rs": (
        ...
        "canonical_operation_references_and_applied_redeem_status_preserve_operation_id",
        "terminal_finality_requires_nonzero_exact_identity_hash_height_and_time",
        "rejected_and_expired_pipeline_entries_are_terminal_without_retry",
        "kagemusha_v2_anchor_finality_binding_rejects_identity_hash_or_height_mismatch",
        "authorization_refresh_cannot_alias_a_second_anchor_or_bypass_exact_replay",
    ),
    "crates/iroha_torii/src/openapi.rs": (
        '"/v1/offline/redeem",',
        ...
        '"required": ["bundle", "recipient", "amount", "redeem_proof", "redemption", "lineage_verifier_record", "block_height", "operation_id", "authorization"]',
        '"OfflineLineageWitness",\n'
        "            norito::json!({\n"
        '                "type": "object",\n'
        '                "required": ["nodes", "final_bundle_digest"],',
        '"OfflineRedeemRequest".to_owned(),\n'
        "        norito::json!({\n"
        '            "type": "object",\n'
        '            "description": "Direct structured JSON representation of OfflineRedeemRequest. application/x-norito carries the corresponding typed value directly, not a wrapper containing encoded bytes. Unknown and duplicate members are rejected.",\n'
    ...
    "docs/source/offline_kagemusha.md": (
        "Torii redemption uses `POST /v1/offline/redeem`.",
        ...
        "missing or inconsistent queue, pipeline,",
    ...
    "crates/iroha_torii/tests/offline_redeem_contract.rs": (
        ...
        'TORII_SOURCE.contains("offline_api::OfflineRedeemRequest")',
        'TORII_SOURCE.contains("NoritoJson(request)")',
        'OFFLINE_API_SOURCE.contains("as OfflineRedeemRequest")',
        '"iroha.torii.v1.offline.redeem.request"',
        'issuer.contains("handle_redeem")',
...
POLICY_NEGATIVE_CONTROL_COMMANDS = (
    ...
    (
        "typed offline redeem route negative control",
        "ci/check_kagemusha_recursive_spend_policy.sh --negative-control-torii-kagemusha-route",
    ),
    (
        "typed offline redeem exact-field binding negative control",
        "ci/check_kagemusha_recursive_spend_policy.sh --negative-control-torii-kagemusha-exact-fields",
    ),
    (
        "typed offline redeem nested lineage shape negative control",
        "ci/check_kagemusha_recursive_spend_policy.sh --negative-control-torii-kagemusha-archive-field-shape",
    ),
    (
        "retired offline redeem field negative control",
        "ci/check_kagemusha_recursive_spend_policy.sh --negative-control-torii-kagemusha-retired-fields",
    ),
    (
        "retired offline redeem public alias negative control",
        "ci/check_kagemusha_recursive_spend_policy.sh --negative-control-torii-kagemusha-retired-redeem-aliases",
    ),
    (
        "typed offline redeem auxiliary-field rejection negative control",
        "ci/check_kagemusha_recursive_spend_policy.sh --negative-control-torii-kagemusha-auxiliary-fields",
    ),
    (
...
def check_rust_reserved_lineage_policy():
    ...
    if not re.search(
        r"KAGEMUSHA_RECURSIVE_SPEND_LINEAGE_TRANSITION_CIRCUIT_WIRED_V1\s*:\s*bool\s*=\s*false\s*;",
        rust,
    ):
        fail(
            "missing Rust transition-circuit fail-closed guard for witnessless Reserved-lineage"
        )
...
def check_docs_reserved_lineage_policy():
    forbidden = [
        ...
        re.compile(
            r"witnessless\s+Reserved-lineage[^.\n]*(?:enabled|available|admitted)",
            re.I,
        ),
        re.compile(r"canAppendWitnesslessLineage[^.\n]*returns\s+`?true`?", re.I),
    ]
    required = re.compile(
        r"((?:TRANSITION_CIRCUIT_WIRED_V1|TransitionCircuitWiredV1|"
        r"transitionCircuitWiredV1)[^.\n]*(?:false|`false`)|"
        r"witnessless\s+Reserved-lineage[^.\n]*(?:fail\s+closed|disabled|not\s+admitted))",
        re.I,
    )
    for relative in DOC_PATHS:
        text = read(relative)
        if required.search(text) is None:
            fail(f"{relative} does not document the fail-closed witnessless Reserved-lineage boundary")
        for pattern in forbidden:
            if pattern.search(text):
                fail(f"{relative} contains a stale enabled witnessless Reserved-lineage claim")
    ...
    for needle in (
        ...
        "Receiver-side Reserved-lineage preverification recognizes two strict public-instance layouts",
        ...
        "Chain admission nevertheless rejects both layouts while `KAGEMUSHA_RECURSIVE_SPEND_LINEAGE_TRANSITION_CIRCUIT_WIRED_V1` is false",
        "the receiver-side layouts do not authorize witnessless append or redemption",
        "Product append output remains on the semantic circuit until circuit-authenticated recursive verification is wired",
...
def check_torii_kagemusha_redeem_coverage():
    command_relative = "crates/iroha_torii/src/offline_commands.rs"
    command_source = read(command_relative).split("\n#[cfg(test)]", 1)[0]
    for retired in (
        ...
        if retired in command_source:
            fail(
                f"{command_relative} contains retired offline redemption wrapper/parser marker: "
                + retired
            )
    require_needles(
        TORII_KAGEMUSHA_REDEEM_COVERAGE,
...
if mode == "--negative-control-sdk-preferred-cap-edge":
    ...
    mutated = source.replace(
        "the semantic append circuit remains preferred while lineage transition verification is unavailable",
        "semantic append selection drift",
        1,
    )
    if mutated == source:
    ...
    try:
        ...
        message = str(error)
        if "the semantic append circuit remains preferred while lineage transition verification is unavailable" not in message:
            raise SystemExit("negative control failed: SDK preferred cap edge drift was not detected")
        print("negative control rejected SDK semantic append preference drift")
        print(message.splitlines()[0])
        raise SystemExit(0)
    raise SystemExit("negative control failed: SDK preferred cap edge drift was not detected")
...
if mode == "--negative-control-torii-kagemusha-route":
    run_first_release_negative_cases(
        (
            (
                "crates/iroha_torii_shared/src/route_catalog.rs",
                ...
                "is missing typed first-release Torii offline redeem ingress coverage: pub const REDEEM_PATH",
            ),
            (
                "crates/iroha_torii/src/offline_commands.rs",
                lambda source: source.replace(
                    "const PATH_OFFLINE_REDEEM: &str = iroha_torii_shared::uri::OFFLINE_REDEEM;",
                    'const PATH_OFFLINE_REDEEM: &str = "/v1/offline/v2/notes/redeem";',
                    1,
                ),
                "is missing typed first-release Torii offline redeem ingress coverage: const PATH_OFFLINE_REDEEM",
            ),
        ),
        "typed offline redeem route",
    )

if mode == "--negative-control-torii-kagemusha-exact-fields":
    run_first_release_negative_cases(
        (
            (
                "crates/iroha_torii/src/openapi.rs",
                lambda source: source.replace(
                    '"required": ["bundle", "recipient", "amount", "redeem_proof", "redemption", "lineage_verifier_record", "block_height", "operation_id", "authorization"]',
                    '"required": ["bundle", "recipient", "amount", "redeem_proof", "redemption", "lineage_verifier_record", "block_height", "operation_id"]',
                    1,
                ),
                "is missing typed first-release Torii offline redeem ingress coverage: \"required\": [\"bundle\"",
            ),
            (
                "crates/iroha_torii/src/offline_commands.rs",
                lambda source: source.replace(
                    "require_idempotency_key(headers, redeem_request.authorization.operation_id)?;",
                    "// skipped exact signed operation-id binding",
                    1,
                ),
                "is missing typed first-release Torii offline redeem ingress coverage: require_idempotency_key",
            ),
        ),
        "typed offline redeem exact-field binding",
    )

if mode == "--negative-control-torii-kagemusha-archive-field-shape":
    run_first_release_negative_cases(
        (
            (
                "crates/iroha_torii/src/openapi.rs",
                lambda source: source.replace(
                    '"required": ["nodes", "final_bundle_digest"]',
                    '"required": ["pallas_open_envelopes_archive"]',
                    1,
                ),
                "is missing typed first-release Torii offline redeem ingress coverage: \"OfflineLineageWitness\"",
            ),
        ),
        "typed offline redeem nested lineage shape",
    )

if mode == "--negative-control-torii-kagemusha-retired-fields":
    run_first_release_negative_cases(
        (
            (
                "crates/iroha_torii/src/offline_commands.rs",
                lambda source: source.replace(
                    "redeem_request: OfflineRedeemRequest,",
                    "redeem_request_norito_base64: String,",
                    1,
                ),
                "contains retired offline redemption wrapper/parser marker: redeem_request_norito_base64",
            ),
            (
                "crates/iroha_torii/src/offline_commands.rs",
                lambda source: source.replace(
                ...
                "contains retired offline redemption wrapper/parser marker: KagemushaRecursiveSpendRedeemRequestV1",
            ),
        ),
        "retired offline redeem field",
    )

if mode == "--negative-control-torii-kagemusha-retired-redeem-aliases":
    run_first_release_negative_cases(
        (
            (
                "crates/iroha_torii_shared/src/offline_api.rs",
                lambda source: source.replace(
                    "KagemushaRecursiveSpendRedeemRequestV2 as OfflineRedeemRequest",
                    "KagemushaRecursiveSpendRedeemRequestV1 as OfflineRedeemRequest",
                    1,
                ),
                "is missing typed first-release Torii offline redeem ingress coverage: KagemushaRecursiveSpendRedeemRequestV2 as OfflineRedeemRequest",
            ),
        ),
        "retired offline redeem public alias",
    )

if mode == "--negative-control-torii-kagemusha-auxiliary-fields":
    run_first_release_negative_cases(
        (
            (
                "crates/iroha_torii/src/openapi.rs",
                ...
                "is missing typed first-release Torii offline redeem ingress coverage: \"OfflineRedeemRequest\".to_owned()",
            ),
            (
                "crates/iroha_data_model/src/offline/mod.rs",
                lambda source: source.replace(
                ...
                'is missing typed first-release Torii offline redeem ingress coverage: #[norito(schema_name = "iroha.torii.v1.offline.redeem.request")]',
            ),
        ),
        "typed offline redeem auxiliary-field rejection",
    )
...
if mode == "--negative-control-torii-kagemusha-redeem":
    cases = (
        ...
        (
            ...
            "    request: &OfflineRedeemRequest,\n"
            ") -> Result<(), Error> {\n"
            "    // skipped fail-closed proof-backend gate",
        ),
    )
    first_message = None
    for target, before, after in cases:
        ...
        if mutated == source:
            raise SystemExit(
                "negative control failed: unable to mutate typed first-release Torii offline redeem ingress: "
                + target
                + ": "
                + before
            )
        ...
        try:
            ...
            message = str(error)
            expected = (
                f"{target} is missing typed first-release Torii offline redeem ingress coverage: "
                + before
            )
            if expected not in message:
                raise SystemExit(
                    "negative control failed: typed first-release Torii offline redeem drift was rejected for the wrong reason: "
                    + message.splitlines()[0]
                )
            if first_message is None:
                first_message = message
            ...
            text_overrides.pop(target, None)
        raise SystemExit(
            "negative control failed: typed first-release Torii offline redeem ingress drift was not detected for "
            + target
            + ": "
            + before
        )
    if first_message is None:
        raise SystemExit(
            "negative control failed: typed first-release Torii offline redeem ingress drift was not detected"
        )
    print("negative control rejected typed first-release Torii offline redeem ingress drift")
    print(first_message.splitlines()[0])
    raise SystemExit(0)
...
if mode == "--negative-control-torii-kagemusha-openapi":
    cases = (
        ...
        (
            "docs/portal/static/openapi/torii.json",
            '"$ref": "#/components/schemas/OfflineRedeemRequest"',
            '"$ref": "#/components/schemas/OfflineRedeemWrapper"',
        ),
        (
            "docs/portal/static/openapi/versions/current/torii.json",
            '"/v1/offline/redeem": {',
    ...
    for target, before, after in cases:
        ...
        if mutated == source:
            raise SystemExit(
                "negative control failed: unable to mutate typed first-release Torii offline redeem OpenAPI: "
                + target
                + ": "
                + before
            )
        ...
        try:
            ...
            if expected not in message:
                raise SystemExit(
                    "negative control failed: typed first-release Torii offline redeem OpenAPI drift was rejected for the wrong reason: "
                    + message.splitlines()[0]
                )
            if first_message is None:
                first_message = message
            ...
            text_overrides.pop(target, None)
        raise SystemExit(
            "negative control failed: typed first-release Torii offline redeem OpenAPI drift was not detected for "
            + target
            + ": "
            + before
        )
    if first_message is None:
        raise SystemExit(
            "negative control failed: typed first-release Torii offline redeem OpenAPI drift was not detected"
        )
    print("negative control rejected typed first-release Torii offline redeem OpenAPI drift")
    print(first_message.splitlines()[0])
    raise SystemExit(0)
...
if mode == "--negative-control-torii-kagemusha-smoke":
    ...
    cases = (
        ...
        (
            "offline_operation_polling_preserves_redeem_identity_and_finality_integrity",
            "offline_operation_polling_ignores_redeem_identity_and_finality_integrity",
        ),
    )
    first_message = None
    source = read(target)
    for before, after in cases:
        ...
        if mutated == source:
            raise SystemExit(
                "negative control failed: unable to mutate typed first-release Torii offline redeem smoke coverage: "
                + before
            )
        text_overrides[target] = mutated
        try:
            ...
            if expected not in message:
                raise SystemExit(
                    "negative control failed: typed first-release Torii offline redeem smoke drift was rejected for the wrong reason: "
                    + message.splitlines()[0]
                )
            if first_message is None:
                first_message = message
            ...
            text_overrides.pop(target, None)
        raise SystemExit(
            "negative control failed: typed first-release Torii offline redeem smoke drift was not detected for "
            + before
        )
    if first_message is None:
        raise SystemExit(
            "negative control failed: typed first-release Torii offline redeem smoke drift was not detected"
        )
...
if mode == "--negative-control-core-lineage-profile-split":
    cases = (
        ...
        (
            "crates/iroha_core/src/zk.rs",
            "one-hop lineage token must reject an append verifier record",
            "one-hop lineage token accepted an append verifier record",
            "one-hop lineage token must reject an append verifier record",
        ),
        (
            "crates/iroha_core/src/zk.rs",
            "append lineage projected token must reject a one-hop verifier record",
            "append lineage projected token accepted a one-hop verifier record",
            "append lineage projected token must reject a one-hop verifier record",
        ),
        (
            "crates/iroha_cli/src/zk.rs",
...
print("recursive Kagemusha witnessless Reserved-lineage policy is fail-closed")
PY
