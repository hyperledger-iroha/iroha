...
static_mutations = (
    *ATTESTATION_STATIC_MUTATIONS,
    ...
    (MODEL,
        ...
        'reject release proof-pair maximum drift',
        ('single exact first-release proof-pair profile',)),
    (MODEL_RELEASE_V4,
        'self.initialization_pair.len() != initialization_pair_bytes',
        'false',
        'reject qualification receipts with arbitrary initialization-pair lengths',
    ...
    (MODEL_RELEASE_V4,
        ...
        'false',
        'reject qualification receipts with arbitrary recursive-pair lengths',
        ('release manifest, qualification receipt, and promotion bound to one proof-pair profile',)),
    (MODEL_TAIL_TESTS,
    ...
    (MODEL_TAIL_TESTS,
        ...
        'fn unchecked_release_lifecycle_chronology()',
        'reject loss of strict lifecycle chronology coverage',
        ('active lifecycle transition and retained-policy regressions',)),
    (MODEL_TAIL_TESTS,
        'fn release_lifecycle_state_requires_nonzero_distinct_transaction_intents()',
        'fn unchecked_release_lifecycle_transaction_intents()',
        'reject loss of lifecycle transaction-intent coverage',
        ('active lifecycle transition and retained-policy regressions',)),
    ...
    (CORE, 'kagemusha_terminal_registry_v4::verifier_owner_manifest_sha256(record, role)?;', '[0; 32];', 'reject verifier qualification detached from release ownership', ('release owner/circuit-qualified verifier identity',)), (CORE, 'kagemusha_v4_release_verifier_id_has_exact_digest(id)\n        || kagemusha_v4_parity_for_circuit(circuit_id).is_some()', 'id.backend.as_str() == KAGEMUSHA_RECURSIVE_SPEND_PASTA_CYCLE_BACKEND_V4', 'reject widening native hydration to same-backend lookalikes', ('narrow V4 candidate',)), (CORE, 'ensure_exact_kagemusha_v4_native_verifier_storage_shape(record, identity.parity)?;', '// storage shape bypassed', 'reject malformed native verifier storage exclusion', ('atomic native Eq/Ep hydration',)),
    ...
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        ...
        'reject an unbound Applied wait outcome',
        D_SUBMIT_UNCERTAIN),
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        '    if outcome.terminal_kind != outcome.r#final.status.kind',
        '    if false',
        'reject inconsistent Applied wait summary and final status',
        D_STATUS_IDENTITY),
    (KAGEMUSHA_ROLLOUT_COMPONENT,
    ...
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        ...
        '                    .get_transaction_status_response_global(authorization.verified.canary_transaction().hash())\n                    .wrap_err(',
    ...
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        ...
        '        let pipeline_status = client\n            .get_transaction_status_response_global(expectations.activation_transaction_intent())',
    ...
    (PROMOTION_WORKFLOW,
        ...
        'name: Publish unauthenticated Kagemusha mobile builds',
        'reject a mislabeled Kagemusha mobile authorization workflow',
        ('name: Authorize Kagemusha V4 mobile production builds',)),
    (KAGAMI,
...
for (relative, before, after, failure, diagnostic) in (
    ...
    (IROHAD_STARTUP, 'pub fn install_runtime_effective_config_with_validator_seal_reader(',
        'pub fn install_runtime_effective_config_with_unchecked_reader(',
