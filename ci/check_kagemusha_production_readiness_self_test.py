...
if sys.platform == 'darwin':
    try:
        ...
        (descriptor, _) = pin_regular_metadata(acl_free_path, 'self-test ACL-free macOS input')
        try:
...
ATTESTATION_STATIC_MUTATIONS = (
    ...
    (ANDROID_AUTH,
        ...
        '    if true {\n        return Ok(());\n    }',
        'reject software-enforced Android authorization properties',
        ('strict Android application, boot, and authorization identity',)),
...
static_mutations = (
    *ATTESTATION_STATIC_MUTATIONS,
    ...
    (CORE_TX,
        ...
        'let allows_direct_kagemusha_lifecycle_authority = true',
        'reject a lifecycle admission exception without verified multisignatures',
...
for relative, test_name, diagnostic in (
    ...
    (IROHAD_STARTUP_TESTS, 'authenticated_snapshot_rejects_wrong_local_peer_without_installing',
        'authenticated snapshot startup install and fail-closed regressions'),
    (IROHAD_STARTUP_TESTS, 'authenticated_snapshot_rejects_projection_mismatch_without_installing',
        'authenticated snapshot startup install and fail-closed regressions'),
    (IROHAD_STARTUP_TESTS, 'authenticated_snapshot_without_configured_seal_does_not_install',
        'authenticated snapshot startup install and fail-closed regressions'),
    (CORE_LIFECYCLE, 'direct_ordinary_multisig_cancel_executes_exact_staged_transition',
        'direct signed ordinary Cancel and Deactivate transition regressions'),
    (CORE_LIFECYCLE, 'direct_ordinary_multisig_deactivate_executes_exact_enabled_transition',
        'direct signed ordinary Cancel and Deactivate transition regressions'),
    (CORE_SUMERAGI_APPLY_RUNTIME_GATE_TESTS,
        'production_proposal_validation_enforces_kagemusha_runtime_projection',
        'production proposal and Commit runtime-projection regressions'),
    (CORE_SUMERAGI_APPLY_RUNTIME_GATE_TESTS,
        'production_commit_apply_enforces_kagemusha_runtime_projection',
        'production proposal and Commit runtime-projection regressions'),
    ...
    (CORE_SUMERAGI_WORKER_RUNTIME_GATE_TESTS,
        'production_vote_worker_signs_prepare_and_commit_for_exact_kagemusha_projection',
        'production Prepare and Commit signing runtime-projection regressions'),
):
