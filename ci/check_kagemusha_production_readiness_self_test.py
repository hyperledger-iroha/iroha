...
expect_static_rejection({BUNDLE_SOURCE_SEAL_INPUTS: conflicted_build_inputs},
    ...
    'unresolved Git merge conflict marker')
D_RECORD_CANARY = ('one exact signed ordinary nonce TTL height-expiry Record canary',)
D_WIRE_BOUNDARY = ('complete signed canary wire bound at all transaction boundaries',)
D_CANARY_MARKER = ('activation-bound exact reservation and signed-wire marker',)
D_CANARY_EXEC = ('consensus-authorized exact-wire one-shot canary execution',)
D_BLOCK_PROOF = ('promotion-keyed full canary wire block proof evidence and issuer signature',)
D_ROLLOUT_PHASES = ('eight phase rollout with isolated create-only fallback credentials',)
D_FINAL_RECEIPT = ('deferred issuer signing and proof-verified no-replace final receipt',)
D_EXACT_PERMIT = ('fresh exact permit Record authorization creation and private no-replace publication',)
D_EXPECTATIONS = ('exact expectations provenance and canary anchor binding',)
D_POST_RECEIPT = ('exact post-receipt promotion network and exclusive proof-corridor binding',)
D_STRUCTURAL = ('structural expired-journal reconciliation and fresh precommit publication',)
D_CANARY_JOURNAL = ('private canary journal committed before minimal reservation disclosure and POST',)
D_VALIDATOR_TIPS = ('four signed validator identities with shared canary-rooted finality and exact tips',)
D_ROOT_TCB = ('root-TCB pinned controller, Kagami, policy, candidate, and snapshot launch',)
D_RESERVATION = ('minimal controller-signed non-disclosing exact-call reservation',)
D_ROSTER = ('permissioned four-unit runtime roster',)
D_KAGAMI_EXEC = ('authenticated Kagami verifier execution',)
D_PRECOMMIT = ('pre-commit controller-signed permit with consensus time authority and height bounds',)
D_PROOF_CHAIN = ('proof-anchored exact committed canary and contiguous post-receipt finality',)
D_ROOT_PUBLISH = ('root-owned no-replace fsync publication with commit uncertainty',)
D_SAFE_RESUME = ('retrospective refusal and matching-journal safe-resume decision',)
D_CATALOG_HISTORY = ('historical consumption plus separate current catalog validator boundary',)
static_mutations = (
    ...
    (CORE_RUNTIME_EFFECTIVE_CONFIG_COMPONENT,
        ...
        'reject a non-validator projection', D_ROSTER),
    (CORE_RUNTIME_EFFECTIVE_CONFIG_COMPONENT,
        'context.mode != ConsensusMode::Permissioned', 'false',
        'reject a non-permissioned projection', D_ROSTER),
    (CORE_RUNTIME_EFFECTIVE_CONFIG_COMPONENT,
        'context.roster.iter().any(|member| member.power != 1)', 'false',
        'reject a weighted projection roster', D_ROSTER),
    (CORE_RUNTIME_EFFECTIVE_CONFIG_COMPONENT,
