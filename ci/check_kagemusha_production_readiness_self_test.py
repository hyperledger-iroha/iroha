...
static_mutations = (
    ...
    (MODEL_CANARY_EVIDENCE_COMPONENT,
        ...
        'reject unsigned controller canary authorization',
    ...
    (MODEL_CANARY_EVIDENCE_COMPONENT,
        ...
        ('one exact signed ordinary nonce TTL height-expiry Log canary',)),
    (MODEL_CANARY_EVIDENCE_COMPONENT,
    ...
    (MODEL_CANARY_EVIDENCE_COMPONENT,
        ...
        ('one exact signed ordinary nonce TTL height-expiry Log canary',)),
    (MODEL_CANARY_EVIDENCE_COMPONENT,
        '        || body.network_id != binding.network_id', '        || false',
        'reject authorization detached from the activation network',
        ('exact post-receipt promotion/network authorization binding',)),
    (MODEL_CANARY_EVIDENCE_COMPONENT,
    ...
    (MODEL_CANARY_EVIDENCE_COMPONENT,
        ...
        ('proof-anchored later canary block and contiguous finality extension',)),
    (MODEL_CANARY_EVIDENCE_COMPONENT,
    ...
    (CORE,
        'let promotion_marker = plan_v4_promotion_id(self.promotion_id, state_transaction)?;',
        'let promotion_marker = unchecked_v4_promotion_id(self.promotion_id, state_transaction)?;',
    ...
    (OFFLINE_CLI, '#[command(name = "rollout-v4")]',
        ...
        ('rollout-v4 offline CLI wiring and credential separation',)),
    (KAGEMUSHA_ROLLOUT_COMPONENT, 'pub(crate) struct Args {',
        'pub(super) struct Args {',
        'reject a crate-public enum with a private rollout argument type',
    ...
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        ...
        ('fresh authorization, retrospective refusal, exact journal, and POST ordering',)),
    (KAGEMUSHA_ROLLOUT_COMPONENT,
    ...
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        ...
        ('exact controller authorization creation and no-replace publication',
         'preflighted no-replace authorization and commit-uncertain reporting')),
    (KAGEMUSHA_ROLLOUT_COMPONENT,
    ...
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        ...
        ('exact controller authorization creation and no-replace publication',)),
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        '    let verification_time_unix_ms = artifact.body.authorized_at_unix_ms;',
        '    let verification_time_unix_ms = current_unix_ms()?;',
        'reject structural authorization reconciliation after wall expiry',
        ('exact controller authorization loader and durable canary journal',)),
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        '    if bytes != authorization.exact_bytes {', '    if false {',
        'reject a digest-only canary submission journal',
        ('exact controller authorization loader and durable canary journal',)),
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        '        let pre_journal_status = if journal_observation == SubmissionJournalObservation::Absent {',
        '        let pre_journal_status = None;\n        if journal_observation == SubmissionJournalObservation::Absent {',
        'reject journal publication without an absent-journal pre-status proof',
        ('fresh authorization, retrospective refusal, exact journal, and POST ordering',)),
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        '        if pre_journal_status.is_some() {', '        if false {',
        'reject retrospective canary-state acceptance',
        ('fresh authorization, retrospective refusal, exact journal, and POST ordering',)),
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        '                let verification_time_unix_ms = current_unix_ms()?;\n                authorization',
    ...
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        ...
        '                        stage: "ordinary pre-POST status failure",',
        'reject ordinary errors after the canary journal commit point',
        ('post-journal pre-POST status reconciliation',
         'post-journal canary outcomes must remain commit-uncertain')),
    (KAGEMUSHA_ROLLOUT_COMPONENT,
    ...
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        ...
        '    // receipt terminal corridor check removed',
        'reject canary finality detached from the receipt terminal',
        ('proof-anchored later canonical canary block and finality extension',)),
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        '        || decode_framed_signed_block(&block_bytes),\n    )\n    .map_err(|error| eyre!("failed to decode canonical canary SignedBlock wire: {error}"))?;',
        '        || decode_unframed_block(&block_bytes),\n    )\n    .map_err(|error| eyre!("failed to decode canonical canary SignedBlock wire: {error}"))?;',
        'reject noncanonical canary SignedBlock wire',
        ('proof-anchored later canonical canary block and finality extension',)),
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        '    let proof_anchor = TrustedBlockProofAnchor::from_untrusted_finality_artifact(\n        &block,\n        finality_artifact,\n        committed.entrypoint_hash(),\n    )\n    .map_err(|error| eyre!("canary proof does not authenticate its carrier block: {error}"))?;',
        '    let proof_anchor = UntrustedBlockHint::from_untrusted_finality_artifact(\n        &block,\n        finality_artifact,\n        committed.entrypoint_hash(),\n    )\n    .map_err(|error| eyre!("canary proof does not authenticate its carrier block: {error}"))?;',
        'reject a canary carrier without a trusted block proof anchor',
        ('proof-anchored later canonical canary block and finality extension',)),
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        '            expectations.binding().promotion_id,\n            CANARY_EVIDENCE_FILE_NAME,',
        '            [0; 32],\n            CANARY_EVIDENCE_FILE_NAME,',
        'reject a canary evidence leaf detached from its promotion',
        ('promotion-keyed exact canary evidence signing and no-replace publication',)),
    (KAGEMUSHA_ROLLOUT_COMPONENT,
    ...
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        ...
        ') -> Result<FinalizedCanaryEvidence> {\n    let details = client\n        .get_successful_transaction_details(expectations.activation_transaction_intent().into())',
        'reject activation-details substitution for canary details',
        ('proof-anchored later canonical canary block and finality extension',)),
    (KAGEMUSHA_ROLLOUT_COMPONENT,
    ...
    (KAGEMUSHA_ROLLOUT_COMPONENT,
        ...
        ('commit-uncertain canary publication reporting',)),
    (KAGEMUSHA_PROMOTION_PUBLISHER_COMPONENT,
        '        "--kagami",\n        "--kagami-sha256",',
        '        "--kagami-sha256",\n        "--kagami",',
