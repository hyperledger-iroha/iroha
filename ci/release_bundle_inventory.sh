...
usage() {
    cat <<'EOF'
Usage: ci/release_bundle_inventory.sh --output <path> <bundle.tar.zst> [bundle.tar.zst ...]

Inspect canonical release bundles, verify required files/binaries, and emit a
JSON inventory summarising the metadata and validation checks. The resulting
file is attached to the release approval ticket alongside the manifests.
...
log() {
    printf '[release-inventory] %s\n' "$*" >&2
}

require_cmd() {
    if ! command -v "$1" >/dev/null 2>&1; then
        printf '[release-inventory] missing required command: %s\n' "$1" >&2
        exit 1
    fi
}
...
while (($#)); do
    case "$1" in
        ...
        --)
            ...
            ;;
        -*)
            printf '[release-inventory] unknown option: %s\n' "$1" >&2
            usage >&2
            exit 1
            ;;
        *)
...
if meta.get("profile") != "iroha3":
    raise SystemExit(f"bundle {bundle} has unsupported profile {meta.get('profile')!r}")
if meta.get("config") != "nexus":
    raise SystemExit(f"bundle {bundle} has unsupported config {meta.get('config')!r}")
...
required_configs = ["config/genesis.json", "config/client.toml", "config/config.toml"]
