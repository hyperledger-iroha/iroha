...
ABI24_ARTIFACT_CHECKER="${ROOT_DIR}/scripts/check_native_sdk_artifact.py"
...
if [[ "${PRIVACY_CSHARP_NATIVE_ARTIFACT}" != \
  ...
  "${PRIVACY_CSHARP_NATIVE_MANIFEST}" != \
    "${NATIVE_DIRECTORY}/native-sdk-abi24-csharp.json" ]]; then
  echo "error: C# privacy native paths are not canonical Linux ABI-24 paths" >&2
  exit 1
fi
...
"${PYTHON_BIN}" -I -B "${ABI24_ARTIFACT_CHECKER}" verify \
