//go:build purego || !amd64

// Copyright 2020-2026 Consensys Software Inc.
// Licensed under the Apache License, Version 2.0. See the LICENSE file for details.

// Code generated by consensys/gnark-crypto DO NOT EDIT

package extensions

import (
	fr "github.com/consensys/gnark-crypto/field/koalabear"
)

func mulAccByElement_avx512(alpha *E4, scale *fr.Element, res *E4, N uint64) {
	panic("mulAccByElement_avx512 is not implemented")
}

func vectorAdd_avx512(res, a, b *E4, N uint64) {
	panic("vectorAdd_avx512 is not implemented")
}

func vectorSub_avx512(res, a, b *E4, N uint64) {
	panic("vectorSub_avx512 is not implemented")
}

func vectorMul_avx512(res, a, b *E4, N uint64) {
	panic("vectorMul_avx512 is not implemented")
}

func vectorScalarMul_avx512(res, a, b *E4, N uint64) {
	panic("vectorScalarMul_avx512 is not implemented")
}

func vectorInnerProduct_avx512(res *[32]uint64, a, b *E4, N uint64) {
	panic("vectorInnerProduct_avx512 is not implemented")
}

func vectorSum_avx512(res *[4]uint64, a *E4, N uint64) {
	panic("vectorSum_avx512 is not implemented")
}

func vectorMulByElement_avx512(res, a *E4, b *fr.Element, N uint64) {
	panic("vectorMulByElement_avx512 is not implemented")
}

func vectorButterfly_avx512(a, b *E4, N uint64) {
	panic("vectorButterfly_avx512 is not implemented")
}

func vectorButterflyPair_avx512(a *E4, N uint64) {
	panic("vectorButterflyPair_avx512 is not implemented")
}

func vectorButterflyPair_E6_avx512(a *E6, N uint64) {
	panic("vectorButterflyPair_E6_avx512 is not implemented")
}

func vectorMulByElement_E6_avx512(res, a *E6, b *fr.Element, N uint64) {
	panic("vectorMulByElement_E6_avx512 is not implemented")
}

func vectorMulAccByElement_E6_avx512(dst, a *E6, b *fr.Element, N uint64) {
	panic("vectorMulAccByElement_E6_avx512 is not implemented")
}

func vectorScalarMulAccByElement_E6_avx512(vector *E6, a *fr.Element, sTiled *fr.Element, N uint64) {
	panic("vectorScalarMulAccByElement_E6_avx512 is not implemented")
}

func vectorScalarMul_E6_avx512(res, a *E6, Kcol *fr.Element, N uint64) {
	panic("vectorScalarMul_E6_avx512 is not implemented")
}

func vectorScalarMulAcc_E6_avx512(res, a *E6, Kcol *fr.Element, N uint64) {
	panic("vectorScalarMulAcc_E6_avx512 is not implemented")
}

func vectorInnerProductByElement_avx512(res, a *E4, b *fr.Element, N uint64) {
	panic("vectorInnerProductByElement_avx512 is not implemented")
}

func vectorDITWithTwiddles_E6_avx512(a0, a1 *E6, twiddles *fr.Element, N uint64) {
	panic("vectorDITWithTwiddles_E6_avx512 is not implemented")
}

func vectorDIFWithTwiddles_E6_avx512(a0, a1 *E6, twiddles *fr.Element, N uint64) {
	panic("vectorDIFWithTwiddles_E6_avx512 is not implemented")
}
