...
/**
 * @title SccpTronSourceBridge
 * @dev Owner-governed TRON/TVM source emitter for SCCP source-chain proofs.
 *
 * Successful calls to `submitSccpSourceEvent(uint32,uint32,bytes32)` are what
 * the Rust SCCP TRON source adapter proves under a signed java-tron transaction
 * Merkle root.
 * The emitted event keeps the same one-topic payload shape used by legacy
 * receipt diagnostics: topic0 is `SccpSourceEvent(bytes32)`, topic1 is the
 * indexed SCCP source event digest, and event data is empty.
 */
contract SccpTronSourceBridge is Ownable {
    ...
    bytes32 private constant SOURCE_BRIDGE_CONFIG_PREFIX =
        keccak256("iroha:sccp:tron-source-bridge-config:v1");
    ...
    constructor(
        ...
        require(
            configuredSourceDomain == SCCP_DOMAIN_TRON,
            "Source domain must be TRON"
        );
        require(
