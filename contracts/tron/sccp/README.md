...
`TairaXorSccpBridge.finalizeFromTaira(bytes,bytes32[6],bytes32,bytes)` accepts
the canonical SCCP transfer payload bytes recorded on TAIRA. The bridge parses
that payload, requires the `taira_tron_xor` route and `xor` asset, recomputes
the SCCP transfer message id from `sccp:transfer:v1 || canonical_payload`, and
mints only the decoded TRON recipient and amount. The proof public input
`payloadHash` remains the canonical SCCP commitment payload hash generated by
Iroha over those same canonical payload bytes. TRON-origin burns reject
non-`taira_tron_xor` route hashes, non-`xor` asset hashes, empty recipients,
and raw TAIRA recipient payloads above 256 bytes before burning tokens or
emitting the source event.
...
node scripts/sccp_tron_taira_xor_deploy.mjs compile-taira-contract
NODE_PATH=/tmp/iroha-sccp-smoke-node/node_modules \
...
node scripts/sccp_tron_taira_xor_deploy.mjs evidence \
  ...
  --source-bridge <SccpTronSourceBridge> \
  --verifier <SccpTronGroth16Bn254MessageVerifier>
# After live readback/canary evidence confirms the deployed views:
node scripts/sccp_tron_taira_xor_deploy.mjs route-manifest \
  --settlement-asset-definition-id <canonical-xor-asset-definition-id> \
  --verifier-code-hash <0x-verifier-runtime-code-hash> \
  --verifier artifacts/sccp-tron/production-verifier-key.json \
  --vk-backend <registered-ivm-vk-backend> \
  --vk-name <registered-ivm-vk-name> \
  --live-evidence artifacts/sccp-tron/live-evidence.json \
  --expected-destination-binding-hash <0x-live-destination-binding-hash> \
  --expected-destination-binding-key <canonical-destination-binding-key> \
...
without credentials, query strings, fragments, or
localhost/private-network hosts. Deployer secret files are written `0600`, and
`doctor`, `deploy`, `sign-transaction`, and funding checks reject deployer
secrets that are symlinked or not exactly `0600`. After each broadcast transaction, the helper waits
...
`doctor` is a non-broadcasting preflight that checks the local Node version,
TRON/TAIRA source files, live-evidence helper scripts, optional `solc` and
`ethers` package availability, deployer secret validity, production verifier
material, safe endpoint shape, and, with `--check-account true`, deployer
funding readiness through `/wallet/getaccount`. The JSON report intentionally
includes only deployer addresses and never prints the private key.
The helper also exposes `sign-transaction` and `broadcast` commands for
operator-reviewed deployment transactions. `sign-transaction` refuses generic
unsigned TRON JSON; it only signs a route-scoped `unsigned_artifact` emitted by
the dry-run deployment plan, or the dry-run plan itself when the operator passes
an explicit `--step <step-key>`. The signed output stores the route, asset,
purpose, step metadata, transaction id, recoverable secp256k1 signature, and
verifies that the signature recovers to the deployer before writing the
artifact. Broadcast accepts that route-scoped signed artifact and remains
deliberately unavailable without the explicit mainnet confirmation flag.
Both commands reject secret-bearing artifacts, including obvious secret fields,
recovery-phrase-shaped values, and PEM private-key blocks, before signing or
broadcasting.
The `route-manifest` command is offline/read-only. It validates deployment
evidence, including `taira_tron_xor` / `xor` hashes and the full required
post-deploy checklist, TAIRA burn-record contract bytecode, decoded artifact
size bounds, canonical settlement asset IDs, registered IVM verifying-key
references, verifier key material, saved live readback/canary evidence from
`scripts/sccp_tron_live_evidence.py`, and the computed TRON destination binding
key/hash before writing the wallet/Torii route manifest draft. In production
mode, `--live-evidence` must prove full TOML readiness, source-bridge ownership
by `TairaXorSccpBridge`, destination verifier code/key/binding hashes, and a
passed route-canary transaction with raw-data owner plus signature-owner
binding. It only writes `productionReady: true` when the operator also passes
`--live-readback-checked true` and `--confirm-mainnet taira_tron_xor`;
otherwise the manifest is explicitly disabled.
