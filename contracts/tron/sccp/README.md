...
Iroha over those same canonical payload bytes.
...
`estimate-budget` recommended minimum. TRON gateway overrides must be HTTPS
public endpoints without credentials, query strings, fragments, or
localhost/private-network hosts. After each broadcast transaction, the helper waits
...
evidence, TAIRA burn-record contract bytecode, canonical settlement asset IDs,
registered IVM verifying-key references, verifier key material, and the
computed TRON destination binding key/hash before writing the wallet/Torii route
manifest draft. It only writes `productionReady: true` when the operator also
passes `--live-readback-checked true` and
`--confirm-mainnet taira_tron_xor`; otherwise the manifest is explicitly
disabled.
