...
| TM-010 | Remote attacker / insider | Operator or KAGEMUSHA V1 redemption private key is stored where it can be exfiltrated (disk/config/CI artifacts) | Steal a private key and issue valid signed operator requests or KAGEMUSHA V1 redemption transactions | Full operator-plane compromise or unauthorized KAGEMUSHA V1 redemption submission with low detectability | Operator keys; KAGEMUSHA V1 redemption authority; runtime config; consensus liveness | Torii loads the KAGEMUSHA V1 redemption key from exactly one of `torii.kagemusha_v1_commands.redemption_private_key` or `torii.kagemusha_v1_commands.redemption_private_key_file` and derives the redemption authority from its public key. Evidence: `crates/iroha_config/src/parameters/user.rs` (`ToriiKagemushaV1Commands::parse`), `crates/iroha_torii/src/lib.rs`; `Dockerfile` runs as non-root | Key storage and rotation are deployment responsibilities; signature auth and KAGEMUSHA V1 redemption submission inherit this risk | Keep private keys in deployment-owned non-extractable custody; avoid embedding them in repositories or world-readable configuration; enforce strict file permissions and rotation; consider multi-signature or threshold authorization for privileged actions | Alert on operator actions and KAGEMUSHA V1 redemptions from new IPs/ASNs; maintain immutable audit logs; rotate keys on suspicion | Medium | High | **high** |
