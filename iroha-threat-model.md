...
| TM-010 | Remote attacker / insider | Operator or Offline Cash V1 command-submission private key is stored where it can be exfiltrated (disk/config/CI artifacts) | Steal a private key and issue valid signed operator requests or Offline Cash V1 transactions | Full operator-plane compromise or unauthorized Offline Cash V1 command submission with low detectability | Operator keys; Offline Cash V1 submission authority; runtime config; consensus liveness | Torii loads the Offline Cash V1 submission private key from `torii.offline_cash_v1_commands.private_key` or `TORII_OFFLINE_CASH_V1_COMMANDS_PRIVATE_KEY` and derives the submission authority from its public key. Evidence: `crates/iroha_config/src/parameters/user.rs` (`ToriiOfflineCashV1Commands::parse`), `crates/iroha_torii/src/lib.rs`; `Dockerfile` runs as non-root | Key storage and rotation are deployment responsibilities; signature auth and Offline Cash V1 command submission inherit this risk | Keep private keys in deployment-owned non-extractable custody; avoid embedding them in repositories or world-readable configuration; enforce strict file permissions and rotation; consider multi-signature or threshold authorization for privileged actions | Alert on operator actions and Offline Cash V1 submissions from new IPs/ASNs; maintain immutable audit logs; rotate keys on suspicion | Medium | High | **high** |
