...
- Operator routes (`/v1/configuration` and operator-gated telemetry/profiling when enabled) are intended to be publicly reachable and should authenticate via signature from an operator-controlled private key. `GET /v1/nexus/lifecycle` is read-only and exposes the exact catalog commitment under the normal Torii access policy; its deprecated `POST` method always returns `403 local_lane_lifecycle_disabled` without reading or mutating lifecycle state. Topology changes use signed `SetParameter(nexus_lane_lifecycle_v1)` transactions and require the on-chain `CanSetParameters` permission. Evidence: `crates/iroha_torii/src/lib.rs`, `crates/iroha_torii/src/routing.rs`, and `crates/iroha_core/src/smartcontracts/isi/world.rs`.
...
| `POST /v1/nexus/lifecycle` | Internet HTTP | Internet → normal Torii access policy → fixed rejection | Retired compatibility method; always `403 local_lane_lifecycle_disabled`; body is not decoded and state/queue are never mutated | `crates/iroha_torii/src/lib.rs` (`handler_post_nexus_lane_lifecycle`), `crates/iroha_torii/src/routing.rs` (`handle_post_nexus_lane_lifecycle`) |
