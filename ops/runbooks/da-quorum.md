...
summary: Operator procedure for responding to DA quorum degradation and attester outages referenced in roadmap item NX-18.
---
...
- **Objective:** Detect and remediate DA quorum drops before finality exceeds the 1 s SLA.
...
| Ratio <0.95 | `DA_QUORUM_PAGE` | Freeze risky lanes (set `allow_new=false`), notify attester leads, consider resharding or increasing redundancy. | Page DA TL, inform Core Consensus. |
...
1. Export the Grafana panels (image + JSON) showing the quorum ratio and finality lag.
...
2. Check `nexus_lane_finality_lag_slots` ≤1 and `iroha_slot_duration_ms_latest` ≤1000 ms.
3. Capture before/after dashboards and attach to the incident ticket.
