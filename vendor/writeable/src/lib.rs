...
pub trait Writeable {
    ...
    /// Returns a borrowed string representation when available.
    #[inline]
...
macro_rules! impl_display_with_writeable {
    ...
    ($type:ty) => {
        ...
        $crate::impl_display_with_writeable!(@impl_block, $type);
    };
    ($type:ty, #[$cfg:meta]) => {
        $crate::impl_display_with_writeable!(@display, $type);
        $crate::impl_display_with_writeable!(@impl_block, $type, #[$cfg]);
    };
    (@impl_block, $type:ty) => {
        impl $type {
