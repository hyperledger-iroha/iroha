...
impl Hash for Fp {
    fn hash<H: Hasher>(&self, state: &mut H) {
        state.write(self.to_repr().as_ref());
    }
}
...
impl Fp {
    ...
    pub const fn double(&self) -> Fp {
        let d0 = self.0[0].wrapping_shl(1);
        let carry0 = self.0[0] >> 63;
        let limb1 = self.0[1].wrapping_shl(1);
        let d1 = limb1 | carry0;
        let carry1 = self.0[1] >> 63;
        let limb2 = self.0[2].wrapping_shl(1);
        let d2 = limb2 | carry1;
        let carry2 = self.0[2] >> 63;
        let limb3 = self.0[3].wrapping_shl(1);
        let d3 = limb3 | carry2;
        let carry3 = self.0[3] >> 63;

        let doubled = Fp([d0, d1, d2, d3]);

        let needs_sub = (carry3 != 0)
            || (d3 > MODULUS.0[3])
            || (d3 == MODULUS.0[3] && d2 > MODULUS.0[2])
            || (d3 == MODULUS.0[3] && d2 == MODULUS.0[2] && d1 > MODULUS.0[1])
            || (d3 == MODULUS.0[3]
                && d2 == MODULUS.0[2]
                && d1 == MODULUS.0[1]
                && d0 >= MODULUS.0[0]);
...
#[test]
fn double_matches_add_for_selected_values() {
    let near_modulus = Fp::from_raw([
        MODULUS.0[0] - 1,
        MODULUS.0[1],
        MODULUS.0[2],
        MODULUS.0[3],
    ]);
    let mixed = Fp::from_raw([
        0x8f3a_d9b2_8c4d_5e71,
        0x1d2c_3b4a_5968_7786,
        0x0fed_cba9_8765_4321,
        0x1234_5678_9abc_def0,
    ]);
    let broad_carry = Fp::from_raw([
    ...
    for value in &values {
        assert_eq!(value.double(), value.add(value));
    }
}
