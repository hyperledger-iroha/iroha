     ...
     $curve_id:literal, $a_raw:expr, $b_raw:expr, $curve_type:ident) => {
        ...
        impl<'a, 'b> Mul<&'b $scalar> for &'a $name {
            ...
            fn mul(self, other: &'b $scalar) -> Self::Output {
                ...
                let mut table = [$name::identity(); TABLE_SIZE];
                table[1] = *self;
                for idx in 2..TABLE_SIZE {
                    table[idx] = (&table[idx - 1]) + self;
                }

                let select = |idx: usize| -> $name {
                    let mut selected = table[0];
                    for j in 1..TABLE_SIZE {
                        let mask = Choice::from((idx == j) as u8);
                        selected = $name::conditional_select(&selected, &table[j], mask);
                    }
                    selected
                };
                ...
                for &byte in other.to_repr().iter().rev() {
                    for idx in [(byte >> 4) as usize, (byte & 0x0f) as usize] {
                        for _ in 0..WINDOW {
                            acc = acc.double();
                        }
                        let selected = select(idx);
                        acc = (&acc) + &selected;
                    }
                }
        ...
        impl<'a, 'b> Mul<&'b $scalar> for &'a $name_affine {
            ...
            fn mul(self, other: &'b $scalar) -> Self::Output {
                ...
                let base = self.to_curve();
                let mut table = [$name::identity(); TABLE_SIZE];
                table[1] = base;
                for idx in 2..TABLE_SIZE {
                    table[idx] = (&table[idx - 1]) + &base;
                }
        ...
        impl GroupEncoding for $name_affine {
            ...
            fn to_bytes(&self) -> [u8; 32] {
                let sign_bit = self.y.is_odd().unwrap_u8() << 7;
                let mut encoded = self.x.to_repr();
                encoded[31] |= sign_bit;
                ...
                for byte in &mut encoded {
                    let keep = *byte;
                    let zero = 0u8;
                    *byte = u8::conditional_select(&keep, &zero, is_identity);
                }
                encoded
            }
        }
...
mod tests {
    ...
    #[test]
    fn ep_affine_to_bytes_roundtrip_and_identity() {
        let identity = EpAffine::identity();
        assert_eq!(identity.to_bytes(), [0u8; 32]);

        let generator = EpAffine::generator();
        let encoded = generator.to_bytes();
        let decoded = EpAffine::from_bytes(&encoded);
    ...
    #[test]
    fn eq_affine_to_bytes_roundtrip() {
        let generator = EqAffine::generator();
        let encoded = generator.to_bytes();
        let decoded = EqAffine::from_bytes(&encoded);
        assert!(bool::from(decoded.is_some()));
        let decoded = decoded.unwrap();
        assert_eq!(decoded, generator);
    }
    ...
    #[test]
    fn ep_scalar_mul_matches_naive_for_small_scalars() {
        let base = Ep::generator();
        let base_affine = EpAffine::generator();
        for k in 0u64..32 {
            let scalar = Fq::from_raw([k, 0, 0, 0]);
            let mut naive = Ep::identity();
            for _ in 0..k {
                naive = (&naive) + &base;
            }

            let proj = (&base) * &scalar;
            let aff = (&base_affine) * &scalar;
            assert_eq!(proj, naive);
            assert_eq!(aff, naive);
        }
    }

    #[test]
    fn ep_scalar_mul_respects_additivity() {
        let base = Ep::generator();
        let a = Fq::from_raw([0x243f6a8885a308d3, 0x13198a2e03707344, 0xa4093822299f31d0, 0x082efa98ec4e6c89]);
        let b = Fq::from_raw([0x452821e638d01377, 0xbe5466cf34e90c6c, 0xc0ac29b7c97c50dd, 0x3f84d5b5b5470917]);
        let lhs = {
            let p1 = (&base) * &a;
            let p2 = (&base) * &b;
            (&p1) + &p2
    ...
    #[test]
    fn eq_scalar_mul_matches_naive_for_small_scalars() {
        let base = Eq::generator();
        let base_affine = EqAffine::generator();
        for k in 0u64..32 {
            let scalar = Fp::from_raw([k, 0, 0, 0]);
            let mut naive = Eq::identity();
            for _ in 0..k {
                naive = (&naive) + &base;
            }
    ...
    #[test]
    fn eq_scalar_mul_respects_additivity() {
        let base = Eq::generator();
        let a = Fp::from_raw([0x0f1bbcdc9a198a13, 0xf8f8eb5f636f3c47, 0x3b329279b9f3f36c, 0x1a2b3c4d5e6f7788]);
        let b = Fp::from_raw([0x112210f4b16c1cf6, 0x21a1d3aa0b4c5d6e, 0x0c0d0e0f1a1b1c1d, 0x2d3e4f5061728394]);
        let lhs = {
            let p1 = (&base) * &a;
            let p2 = (&base) * &b;
            (&p1) + &p2
