...
impl<K: Message, V: Message> NSMutableDictionary<K, V> {
    ...
    pub fn insert(&mut self, key: Id<K>, value: Id<V>) -> Option<Id<V>> {
        ...
        let old_obj = self.get(&key).map(|old_obj| unsafe {
            util::mutable_collection_retain_removed_id(NonNull::from(old_obj))
        });
...
impl<K: Message, V: Message> NSDictionary<K, V> {
    ...
    where
        K: IsIdCloneable,
    {
        let enumerator = unsafe { this.keyEnumerator() };
        // SAFETY: The enumerator is produced from the consumed dictionary.
        IntoKeys(unsafe { iter::IntoIterWithBackingEnum::new_immutable(this, enumerator) })
    }
...
mod iter_helpers {
    ...
    __impl_iter! {
        impl<K: Message, V: IsIdCloneable> Iterator<Item = Id<V>> for IntoValues<K, V> { ... }
    }

    /// A consuming iterator over the keys of a `NSDictionary`.
    #[derive(Debug)]
    pub struct IntoKeys<K: Message, V: Message>(
        pub(super) iter::IntoIterWithBackingEnum<NSDictionary<K, V>, NSEnumerator<K>>,
    );

    __impl_iter! {
        impl<K: IsIdCloneable, V: Message> Iterator<Item = Id<K>> for IntoKeys<K, V> { ... }
    }
}
