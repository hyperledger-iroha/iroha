...
#[cfg(all(feature = "objective-c", feature = "Foundation_NSException"))]
fn is_range_exception(exception: &Foundation::NSException) -> bool {
    let name = exception.name();
    let expected = Foundation::NSRangeException as *const Foundation::NSExceptionName;
    let actual = Id::as_ptr(&name) as *const Foundation::NSExceptionName;
    actual == expected
}

#[cfg(all(feature = "objective-c", feature = "Foundation_NSException"))]
fn handle_array_exception<T>(exception: Id<objc2::exception::Exception>) -> Option<T> {
    match Foundation::NSException::from_exception(exception) {
        Ok(ns_exception) => {
            if is_range_exception(&ns_exception) {
                None
            } else {
                unsafe { objc2::exception::throw(ns_exception.into_exception()) }
            }
        }
        Err(exception) => unsafe { objc2::exception::throw(exception) },
    }
}

#[cfg(all(feature = "objective-c", feature = "Foundation_NSException"))]
fn catch_array_access<T>(operation: impl FnOnce() -> T) -> Option<T> {
    unsafe {
        match objc2::exception::catch(operation) {
            Ok(result) => Some(result),
            Err(exception) => handle_array_exception(exception),
        }
    }
}
...
impl<T: Message> NSMutableArray<T> {
    ...
    pub fn into_vec(array: Id<Self>) -> Vec<Id<T>> {
        ...
        array
            .to_vec()
            .into_iter()
            .map(|obj| unsafe { util::mutable_collection_retain_removed_id(NonNull::from(obj)) })
            .collect()
    }
}
...
extern_methods!(
    unsafe impl<T: Message> NSArray<T> {
        ...
        pub fn get(&self, index: usize) -> Option<&T> {
            #[cfg(all(feature = "objective-c", feature = "Foundation_NSException"))]
            {
                return catch_array_access(|| unsafe { self.get_unchecked(index) });
            }

            #[cfg(not(all(feature = "objective-c", feature = "Foundation_NSException")))]
            {
                if index < self.len() {
            ...
            T: IsMutable,
        {
            #[cfg(all(feature = "objective-c", feature = "Foundation_NSException"))]
            {
                return catch_array_access(|| unsafe { self.get_unchecked_mut(index) });
            }
...
impl<T: Message> NSMutableArray<T> {
    ...
    pub fn insert(&mut self, index: usize, obj: Id<T>) {
        ...
        {
            unsafe {
                if let Err(exception) = objc2::exception::catch(|| {
        ...
        #[cfg(not(all(feature = "objective-c", feature = "Foundation_NSException")))]
        {
            ...
            if index <= len {
                ...
                // index is checked to be in bounds (Objective-C allows
                // insertion at the end of the collection).
    ...
    pub fn replace(&mut self, index: usize, obj: Id<T>) -> Result<Id<T>, Id<T>> {
        if let Some(old_obj) = self.get(index) {
            ...
            let old_obj =
                unsafe { util::mutable_collection_retain_removed_id(NonNull::from(old_obj)) };
    ...
    pub fn remove(&mut self, index: usize) -> Option<Id<T>> {
        ...
        // SAFETY: We remove the object from the array below.
        let obj = unsafe { util::mutable_collection_retain_removed_id(NonNull::from(obj)) };
        // SAFETY: The index is checked to be in bounds.
