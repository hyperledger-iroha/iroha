...
/// Key codes reported by GameController.
///
/// Matches CoreFoundation's `CFIndex`; Apple documents the type as `NSInteger`
/// sized on Darwin platforms, while Windows builds (LLP64) use `long long`.
/// `common::CFIndex` captures those target-specific layouts.
pub type GCKeyCode = crate::common::CFIndex;
...
mod tests {
    ...
    #[test]
    fn gc_key_code_uses_cfindex_layout() {
        assert_eq!(size_of::<GCKeyCode>(), size_of::<crate::common::CFIndex>());
    }
}
