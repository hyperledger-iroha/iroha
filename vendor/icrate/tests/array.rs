...
use icrate::Foundation::NSMutableArray;
...
#[test]
#[cfg(all(feature = "Foundation_NSMutableArray", feature = "Foundation_NSNumber"))]
fn insert_at_len_appends() {
    let mut array = NSMutableArray::from_vec(vec![NSNumber::new_u8(1)]);
    let len = array.len();
    array.insert(len, NSNumber::new_u8(2));
    assert_eq!(array.len(), len + 1);
    let last = array.last().expect("last element");
    assert_eq!(last.unsignedCharValue(), 2);
}
...
fn insert_out_of_bounds_panics() {
    ...
    let result = catch_unwind(AssertUnwindSafe(|| {
        array.insert(array.len() + 2, NSNumber::new_u8(3));
    }));
...
#[test]
#[cfg(all(feature = "Foundation_NSMutableArray", feature = "Foundation_NSNumber"))]
fn remove_and_pop_return_original_ids() {
    let first = NSNumber::new_u8(7);
    let first_ptr = Id::as_ptr(&first);
    let second = NSNumber::new_u8(8);
    let second_ptr = Id::as_ptr(&second);
    let mut array = NSMutableArray::from_vec(vec![first, second]);

    let removed = array.remove(0).expect("element removed");
    assert_eq!(Id::as_ptr(&removed), first_ptr);

    let popped = array.pop().expect("element popped");
    assert_eq!(Id::as_ptr(&popped), second_ptr);
...
#[test]
#[cfg(all(feature = "Foundation_NSMutableArray", feature = "Foundation_NSNumber"))]
fn mutable_array_sort_by_orders_numbers() {
    let mut array = NSMutableArray::new();
    array.addObject(NSNumber::new_i32(3));
    array.addObject(NSNumber::new_i32(1));
    array.addObject(NSNumber::new_i32(2));
    array.addObject(NSNumber::new_i32(-5));

    array.sort_by(|lhs, rhs| lhs.as_i32().cmp(&rhs.as_i32()));

    let values: Vec<_> = array.iter().map(NSNumber::as_i32).collect();
    assert_eq!(values, vec![-5, 1, 2, 3]);
}

#[test]
#[cfg(all(feature = "Foundation_NSMutableArray", feature = "Foundation_NSNumber"))]
fn mutable_array_sort_by_propagates_panic() {
    ...
    let result = std::panic::catch_unwind(AssertUnwindSafe(|| {
        array.sort_by(|lhs, rhs| {
            if first_call {
                first_call = false;
                panic!("compare failure");
            }
            lhs.as_i32().cmp(&rhs.as_i32())
        });
    }));
    assert!(result.is_err());
