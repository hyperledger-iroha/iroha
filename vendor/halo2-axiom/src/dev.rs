...
impl<F: Field> MockProver<F> {
    ...
    fn namespaced_region_name(&self, region_name: String) -> String {
        if self.namespace_prefix.is_empty() {
            region_name
        } else if region_name.is_empty() {
            self.namespace_prefix.clone()
        } else {
            format!("{}::{}", self.namespace_prefix, region_name)
        }
    }
}
...
impl<F: Field> Assignment<F> for MockProver<F> {
        ...
        N: FnOnce() -> NR,
    {
        ...
        let region_name = name().into();
        self.current_region = Some(Region {
            name: self.namespaced_region_name(region_name),
        ...
        N: FnOnce() -> NR,
    {
        ...
        let prefix_len = self.namespace_prefix.len();
        if prefix_len != 0 {
    ...
    fn pop_namespace(&mut self, gadget_name: Option<String>) {
        ...
        let frame = self
            .namespace_stack
            .pop()
            .expect("pop_namespace called without matching push_namespace");
        self.namespace_prefix.truncate(frame.prefix_len);
    }
}
...
impl<F: FromUniformBytes<64> + Ord> MockProver<F> {
    ...
    pub fn run<ConcreteCircuit: Circuit<F>>(
        ...
        let phases: Vec<_> = prover.cs.phases().collect();
...
mod tests {
    ...
    #[test]
    fn region_names_include_namespace_prefix() {
        const K: u32 = 4;
        ...
        impl Circuit<Fp> for NsCircuit {
            type Config = NsConfig;
            type FloorPlanner = SimpleFloorPlanner;
            type Params = ();
            #[cfg(feature = "circuit-params")]
            fn params(&self) {}
            ...
            fn synthesize(
                ...
                let mut ns = layouter.namespace(|| "outer");
                ns.assign_region(
                    || "inner",
                    |mut region| {
                        region.assign_advice(
        ...
        let prover = MockProver::run(K, &NsCircuit, vec![]).unwrap();
        assert_eq!(prover.regions.len(), 1);
        assert_eq!(prover.regions[0].name, "outer::inner");
    }
