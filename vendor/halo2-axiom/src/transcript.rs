    ...
    C::Scalar: FromUniformBytes<64>,
{
    ...
    fn finalize(mut self) -> W {
        self.writer
            .flush()
            .expect("failed to flush transcript writer");
        self.writer
    }
}
...
mod tests {
    ...
    #[test]
    fn finalize_flushes_blake2b_writer() {
        let tracker = FlushTracker::default();
        let transcript = Blake2bWrite::<_, EpAffine, Challenge255<EpAffine>>::init(tracker.clone());
        let tracker = transcript.finalize();
        assert!(tracker.flushed);
    }

    #[test]
    fn finalize_flushes_keccak_writer() {
        let tracker = FlushTracker::default();
        let transcript =
            Keccak256Write::<_, EpAffine, Challenge255<EpAffine>>::init(tracker.clone());
        let tracker = transcript.finalize();
        assert!(tracker.flushed);
    }
}
