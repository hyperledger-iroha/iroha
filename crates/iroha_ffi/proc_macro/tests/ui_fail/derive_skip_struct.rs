...
// getset permits attaching `#[getset(skip)]` to a struct, but that attribute does not actually
// influence code generation. iroha_ffi rejects such usage to avoid surprising users, and this
// UI test ensures the diagnostic stays in place.
// #[getset(skip)]
