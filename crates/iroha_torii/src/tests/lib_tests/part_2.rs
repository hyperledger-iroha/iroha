...
#[tokio::test]
async fn fee_quote_endpoint_verifier_enforces_kagemusha_lifecycle_witness_floor_and_order() {
    let _guard = app_auth_test_guard(crate::app_auth::CanonicalRequestAuthConfig::default());
    let mut signers = [
        checked_torii_test_ed25519_keypair(0xa6, "derive lifecycle fee quote member A"),
        checked_torii_test_ed25519_keypair(0xa7, "derive lifecycle fee quote member B"),
    ];
    signers.sort_by(|left, right| left.public_key().cmp(right.public_key()));
    let policy = iroha_data_model::account::MultisigPolicy::new(
        2,
        vec![
            iroha_data_model::account::MultisigMember::new(signers[0].public_key().clone(), 2)
                .expect("member A"),
            iroha_data_model::account::MultisigMember::new(signers[1].public_key().clone(), 1)
                .expect("member B"),
        ],
    )
    ...
    let cancellation = iroha_data_model::isi::offline::CancelKagemushaRecursiveReleaseV4::new(
        iroha_data_model::offline::KagemushaV4ReleaseCancellationV1 {
            schema: iroha_data_model::offline::KAGEMUSHA_V4_RELEASE_CANCELLATION_SCHEMA_V1
                .to_owned(),
            version: iroha_data_model::offline::KAGEMUSHA_V4_RELEASE_LIFECYCLE_VERSION_V1,
            promotion_id: [0x11; 32],
            manifest_sha256: [0x22; 32],
            expected_predecessor_lifecycle:
                iroha_data_model::offline::KagemushaExactBytesDigestV1 {
                    byte_len: 1,
                    sha256: [0x33; 32],
                },
            transition_id: [0x44; 32],
            reason:
                iroha_data_model::offline::KagemushaV4ReleaseLifecycleReasonV1::GovernanceCancelled,
            evidence: None,
        },
    );
    let payload = TransactionBuilder::new(
    ...
    .into_payload()
    .expect("build direct lifecycle quote payload");
    let body = norito::json::to_vec(&FeeQuoteRequest { payload })
        .expect("encode direct lifecycle quote request");
    let method = axum::http::Method::POST;
    let query_uri: axum::http::Uri = "/v1/fees/quote?x=1".parse().expect("query quote URI");
    let exact_uri: axum::http::Uri = "/v1/fees/quote".parse().expect("exact quote URI");
    ...
    let nonce = "lifecycle-query-bypass-regression";
    let witness_for = |uri: &axum::http::Uri, members: &[&KeyPair]| {
        let mut witness = iroha_data_model::soracloud::CanonicalRequestWitnessV1 {
            ...
            subject_account: authority.clone(),
            timestamp_ms,
            ...
            .expect("hash exact fee quote request"),
            signatures: Vec::new(),
        };
        let message = canonical_request_witness_message(&witness).expect("witness message");
        witness.signatures = members
            .iter()
            .map(
                |member| iroha_data_model::soracloud::CanonicalRequestSignatureWitnessV1 {
                    signer: member.public_key().clone(),
                    signature: IrohaSignature::try_new(member.private_key(), &message)
                        .expect("sign witness"),
                },
            )
            .collect();
    ...
    let headers_for = |witness| {
        let mut headers = HeaderMap::new();
        headers.insert(
            HEADER_WITNESS,
            witness_header_value(witness)
                .expect("encode witness header")
                .parse()
    ...
    let one_weight_two = witness_for(&query_uri, &[&signers[0]]);
    let error = handler_fee_quote(
        State(app.clone()),
        method.clone(),
        query_uri,
        headers_for(&one_weight_two),
        axum::body::Bytes::from(body.clone()),
    )
    .await
    ...
    let verified = crate::app_auth::verify_fee_quote_canonical_request(
        &app.state,
        &headers_for(&canonical),
        &method,
        &exact_uri,
        &body,
    )
    .expect("query rejection must not burn the corrected request nonce")
    .expect("corrected lifecycle request is authenticated");
