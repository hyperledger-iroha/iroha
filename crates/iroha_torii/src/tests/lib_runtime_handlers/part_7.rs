...
async fn resolve_hosted_http_runtime_target_never_promotes_canary_when_baseline_is_unhealthy() {
    ...
    assert!(
        error
            .message
            .contains("refusing to promote a canary outside its traffic allocation")
    );
}
#[tokio::test]
...
#[tokio::test]
async fn resolve_hosted_http_runtime_target_rejects_local_snapshot_when_authoritative_runtime_state_lags()
 {
    let temp = tempfile::tempdir().expect("tempdir");
    let mut app = seed_public_hosted_http_rollout_app(
        &temp,
