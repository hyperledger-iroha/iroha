    ...
    async fn execute_torii_proxy_request_across_candidates_accepts_retryable_then_success() {
        let first_peer_id = PeerId::from(
            checked_torii_test_ed25519_keypair(
                0x95,
                "derive retryable first proxy peer fixture key",
            )
            .public_key()
            .clone(),
        );
        let second_peer_id = PeerId::from(
            checked_torii_test_ed25519_keypair(
                0x96,
                "derive retryable second proxy peer fixture key",
            )
            .public_key()
            .clone(),
        );
    ...
    async fn execute_torii_proxy_request_across_candidates_returns_last_retryable_response() {
        let peer_id = PeerId::from(
            checked_torii_test_ed25519_keypair(
                0x97,
                "derive last retryable proxy peer fixture key",
            )
            .public_key()
            .clone(),
        );
    ...
    async fn proxied_transaction_submission_preserves_public_accept_and_prefer_contracts() {
        ...
        for (format, minimal_response) in [
            ...
            let decode_public_receipt = |body: &[u8]| match format {
                ...
                ResponseFormat::Json => {
                    norito::json::from_slice::<TransactionSubmissionReceipt>(body)
    ...
    async fn queue_plan_synced_accepts_only_exact_durable_acceptance_evidence() {
        ...
        let expected_hash_literal =
            accepted_queue_hash_for_proxy_submit(&app, &request).to_string();
        ...
        let rewrite_certificate =
            |mut snapshot: ToriiProxyHttpResponseV1,
    ...
    async fn node_query_projection_checkpoint_plan_handler_returns_preview_payload() {
        ...
        let authority = checked_torii_test_ed25519_keypair(
            0x99,
            "derive projection plan authority fixture key",
        );
        let alice =
    ...
    async fn node_query_projection_checkpoint_publish_handler_persists_payload() {
        ...
        let authority = checked_torii_test_ed25519_keypair(
            0x9b,
            "derive projection publish authority fixture key",
        );
        let alice =
    ...
    fn make_signed_block(
        ...
        let keypair =
            checked_torii_test_ed25519_keypair(0x24, "derive Torii block-header fixture key");
    ...
    fn make_empty_signed_block(
        ...
        let keypair =
            checked_torii_test_ed25519_keypair(0x27, "derive Torii empty-header fixture key");
        let header = BlockHeader::new(
            NonZeroU64::new(height).expect("nonzero height"),
            prev_hash,
            None,
