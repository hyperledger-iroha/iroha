type CapturedLocalReadProxyFailures = Arc<
    std::sync::Mutex<
        Vec<(
    ...
    >,
>;
type CapturedLocalReadReconcileRequests = Arc<
    std::sync::Mutex<
        Vec<(
            SoracloudLocalReadRequest,
            iroha_core::soracloud_runtime::SoracloudRuntimeExecutionError,
...
struct LocalReadRuntimeCaptures {
    proxy_failures: CapturedLocalReadProxyFailures,
    reconcile_requests: CapturedLocalReadReconcileRequests,
}
...
fn install_unavailable_local_read_runtime(
    ...
    message: &'static str,
) -> LocalReadRuntimeCaptures {
    let captures = LocalReadRuntimeCaptures {
        proxy_failures: Arc::new(std::sync::Mutex::new(Vec::new())),
        reconcile_requests: Arc::new(std::sync::Mutex::new(Vec::new())),
    };
    Arc::get_mut(app)
        ...
        captured_proxy_failures: Arc::clone(&captures.proxy_failures),
        captured_reconcile_requests: Arc::clone(&captures.reconcile_requests),
    }));
    captures
}

fn assert_captured_proxy_failure(
    captured: &CapturedLocalReadProxyFailures,
    request: &SoracloudLocalReadRequest,
    primary_peer_id: &String,
    message_fragment: &str,
) {
    let failures = captured.lock().expect("proxy failure capture lock");
    assert_eq!(failures.len(), 1);
    assert_eq!(failures[0].0.service_name, request.service_name);
    assert_eq!(failures[0].1, *primary_peer_id);
    assert_eq!(
        failures[0].2.kind,
        SoracloudRuntimeExecutionErrorKind::Unavailable
    );
    assert!(failures[0].2.message.contains(message_fragment));
}

fn assert_captured_reconcile_request(
    captured: &CapturedLocalReadReconcileRequests,
    request: &SoracloudLocalReadRequest,
    message_fragment: &str,
) {
    let requests = captured.lock().expect("reconcile capture lock");
    assert_eq!(requests.len(), 1);
    assert_eq!(requests[0].0.service_name, request.service_name);
    assert_eq!(
        requests[0].1.kind,
        SoracloudRuntimeExecutionErrorKind::Unavailable
    );
    assert!(requests[0].1.message.contains(message_fragment));
}

fn assert_captured_versioned_reconcile_request(
    captured: &CapturedLocalReadReconcileRequests,
    request: &SoracloudLocalReadRequest,
    message_fragment: &str,
) {
    ...
    assert_eq!(requests.len(), 1);
    assert_eq!(requests[0].0.service_name, request.service_name);
    assert_eq!(requests[0].0.service_version, request.service_version);
    assert_eq!(
        requests[0].1.kind,
        SoracloudRuntimeExecutionErrorKind::Unavailable
    );
...
fn generated_hf_incoming_primary_peer_id() -> String {
    PeerId::from(
...
fn generated_hf_proxy_primary_peer_id() -> String {
    PeerId::from(
        checked_torii_test_ed25519_keypair(0x7a, "derive generated-HF proxy primary fixture key")
            .public_key()
            .clone(),
    )
    .to_string()
...
fn generated_hf_proxy_local_peer_id() -> String {
    PeerId::from(
...
struct OfflineManifestAuthorityFixture {
    app: SharedAppState,
    local_peer_id: PeerId,
    authoritative_validator: AccountId,
    authoritative_peer_id: PeerId,
}

#[cfg(any(feature = "p2p_ws", feature = "connect"))]
impl OfflineManifestAuthorityFixture {
    fn new() -> Self {
        let local_keypair =
    ...
    fn install_direct_manifest(&mut self) {
        let app = Arc::get_mut(&mut self.app).expect("unique app state");
        ...
        install_lane_manifest_registry_for_test(
            ...
            &[(
                LaneId::SINGLE,
                vec![(
                    self.authoritative_validator.clone(),
                    self.authoritative_peer_id.clone(),
                )],
            )],
        );
    }

    fn install_http_bridge_manifest(&mut self) {
        let app = Arc::get_mut(&mut self.app).expect("unique app state");
        let state = Arc::get_mut(&mut app.state).expect("unique state");
...
async fn torii_proxy_candidate_peers_fail_closed_when_manifest_authoritative_peers_are_offline() {
    let mut fixture = OfflineManifestAuthorityFixture::new();
    fixture.install_direct_manifest();
    let OfflineManifestAuthorityFixture {
        app,
        local_peer_id,
        authoritative_peer_id: _,
        authoritative_validator: _,
    } = fixture;
...
async fn torii_proxy_candidate_peers_bridge_to_offline_manifest_authority_when_torii_url_is_present()
 {
    let mut fixture = OfflineManifestAuthorityFixture::new();
    fixture.install_http_bridge_manifest();
    let OfflineManifestAuthorityFixture {
        app,
        local_peer_id,
        authoritative_peer_id,
...
async fn local_nexus_read_fanout_completes_without_recursive_self_proxying() {
    ...
    let local_peer_id = PeerId::from(
        checked_torii_test_ed25519_keypair(0x67, "derive local Nexus fanout peer fixture key")
            .public_key()
            .clone(),
    );
    Arc::get_mut(&mut app)
...
async fn validate_incoming_soracloud_proxy_request_authority_accepts_generated_hf_primary() {
    let primary_peer_id = generated_hf_incoming_primary_peer_id();
...
async fn incoming_proxy_authority_failure_requests_generated_hf_reconcile() {
    ...
    let mut app = mk_app_state_for_tests_with_world(world);
    let LocalReadRuntimeCaptures {
        reconcile_requests: captured_reconcile_requests,
        ..
    } = install_unavailable_local_read_runtime(
        &mut app,
        Some(local_peer_id),
    ...
    assert_captured_versioned_reconcile_request(
        &captured_reconcile_requests,
        &request,
        "not the authoritative warm primary",
    );
}
...
async fn incoming_proxy_forward_failure_reports_remote_primary_health_and_requests_reconcile_once()
{
    ...
    let mut app = mk_app_state_for_tests_with_world(world);
    let LocalReadRuntimeCaptures {
        proxy_failures: captured_proxy_failures,
        reconcile_requests: captured_reconcile_requests,
    } = install_unavailable_local_read_runtime(
        &mut app,
        Some(local_peer_id),
    ...
    assert_captured_proxy_failure(
        &captured_proxy_failures,
        &request,
        &primary_peer_id,
        "timed out",
    );
...
async fn unexpected_assigned_proxy_responder_requests_generated_hf_reconcile() {
    ...
    assert_captured_reconcile_request(
        &captured_reconcile_requests,
        &request,
        "unexpected proxy responder",
    );
}
...
async fn soracloud_proxy_failure_reports_remote_primary_health() {
    let primary_peer_id = generated_hf_proxy_primary_peer_id();
    let local_peer_id = generated_hf_proxy_local_peer_id();
    let (world, service_name, service_version) = seed_generated_hf_public_world(&primary_peer_id);
    let mut app = mk_app_state_for_tests_with_world(world);
    let LocalReadRuntimeCaptures {
        proxy_failures: captured_proxy_failures,
...
async fn proxy_execution_failure_reports_remote_primary_health_and_requests_reconcile() {
    ...
    assert_captured_reconcile_request(&captured_reconcile_requests, &request, "timed out");
}
