...
#[cfg(feature = "connect")]
#[test]
fn torii_proxy_v1_roundtrip_and_forwarding_preserve_transaction_admission_binding() {
    let keypair = checked_torii_test_ed25519_keypair(0xfd, "derive proxy V1 admission fixture key");
    let authority = AccountId::new(keypair.public_key().clone());
    ...
    let context = queue::QueuePlanAdmissionContextV1 {
        ...
        predecessor_block_hash: Some(HashOf::from_untyped_unchecked(Hash::new(
            b"proxy-v1-roundtrip-predecessor",
        ))),
        ...
        .digest(),
        route_incarnations: vec![queue::QueuePlanRouteIncarnationV1 {
            ...
            lane_incarnation: Hash::new(b"proxy-v1-roundtrip-incarnation"),
    ...
    let admission_binding = Some(
        ...
        .expect("build exact V1 admission binding"),
    );
    ...
    let encoded = norito::to_bytes(&request).expect("encode V1 proxy admission request");
    let decoded = norito::decode_from_bytes::<ToriiProxyRequestV1>(&encoded)
        .expect("decode V1 proxy admission request");
    assert_eq!(decoded, request);
    let forwarded = super::forwarded_torii_proxy_request(&request, &forwarding_peer);
    assert_eq!(forwarded.schema_version, TORII_PROXY_REQUEST_VERSION_V1);
...
fn queue_plan_synced_test_certificate_snapshot(
    request: &ToriiProxyRequestV1,
    mut receipts: Vec<QueuePlanAdmissionAttestationV1>,
) -> ToriiProxyHttpResponseV1 {
    ...
    ToriiProxyHttpResponseV1 {
        ...
        headers,
        body: norito::to_bytes(&QueuePlanAdmissionCertificateV1 {
...
fn queue_plan_admission_publication_targets_every_live_successor_except_self() {
    let (_app, mut request) =
        incoming_proxy_submit_fixture(0xc0, ToriiProxyTransactionAdmissionV1::QueuePlanSynced);
    let signers = (0_u8..4)
        .map(|offset| {
            checked_torii_test_ed25519_keypair(
...
fn queue_plan_admission_publication_validates_and_persists_idempotently() {
    ...
    let retained = app
        .kura
        .pending_queue_plan_admission_certificate(first_hash)
        .expect("read the exact durable publication")
...
async fn strict_proxy_finalization_keeps_w_through_actual_durable_admission() {
    let signers = (0_u8..4)
        .map(|offset| {
            checked_torii_test_keypair_from_seed_byte(
                0xc8_u8.wrapping_add(offset),
                Algorithm::BlsNormal,
                "derive owned-finalization fixture key",
            )
        })
        .collect::<Vec<_>>();
    let (app, request) = incoming_proxy_submit_fixture_with_validator_signers(
    ...
    let response =
        super::proxy_response_finalization::complete(intermediate, reservation, |response| async {
            ...
            assert_eq!(
                ...
                0,
                "W remains owned after body extraction, durable persistence and publication"
            );
            assert!(
                app.kura
                    .pending_queue_plan_admission_certificate(hash)
                    .expect("read durable certificate before returning a receipt")
                    .is_some()
            );
            response
        })
...
#[cfg(feature = "connect")]
#[test]
fn queue_plan_publication_ingest_requires_configured_certified_receiver() {
    let signers = (0_u8..4)
        .map(|offset| {
            checked_torii_test_keypair_from_seed_byte(
                0xc0_u8.wrapping_add(offset),
                Algorithm::BlsNormal,
                "derive certified receiver fixture key",
            )
        })
        .collect::<Vec<_>>();
    ...
    let original_receiver = app
        .local_peer_id
        .clone()
        .expect("configured certified receiver");
    let outsider = PeerId::new(
        checked_torii_test_ed25519_keypair(0xef, "derive outsider publication receiver")
            .public_key()
            .clone(),
    );
    for receiver in [None, Some(outsider)] {
        Arc::get_mut(&mut app)
            .expect("unique fixture app")
        ...
        assert!(
            app.kura
                .pending_queue_plan_admission_certificate(Hash::new(&publication.certificate))
                .expect("inspect rejected publication")
                .is_none()
        );
    }
    Arc::get_mut(&mut app)
        .expect("unique fixture app")
        .local_peer_id = Some(original_receiver);
    assert!(matches!(
        super::ingest_queue_plan_admission_publication(&app, &publication)
            .expect("certified receiver can persist the exact same publication"),
        QueuePlanAdmissionPublicationIngestOutcome::Durable { .. }
    ));
}
...
fn queue_plan_admission_publication_retains_future_until_catch_up() {
    ...
    assert_eq!(
        app.kura
            .pending_queue_plan_admission_certificate(Hash::new(&incomplete.certificate))
            .expect("inspect rejected incomplete publication"),
        None
    );
    let publication = QueuePlanAdmissionPublicationV1 {
        schema_version: QUEUE_PLAN_ADMISSION_PUBLICATION_VERSION_V1,
    ...
    assert_eq!(
        app.state
            .classify_pending_queue_plan_admission(&publication.certificate, future_height)
            .expect("classifiable future certificate")
            .1,
        PendingQueuePlanAdmissionDisposition::Future {
            authority_height: future_height,
            proposal_height: future_height + 1,
            state_height,
            carrier_height: future_height,
        }
    );
    let hash = Hash::new(&publication.certificate);
    assert!(
        matches!(super::ingest_queue_plan_admission_publication(&app, &publication)
    ...
    assert_eq!(
        ...
        0,
        "future publication must not acquire transaction ownership"
    );
    app.state
        .append_committed_block_header_for_tests(future_header);
    assert_eq!(
        app.state
            .classify_pending_queue_plan_admission(&publication.certificate, future_height + 1)
            .expect("publication is reclassifiable after the canonical frontier arrives")
            .1,
    ...
    assert_eq!(
        app.kura
            .pending_queue_plan_admission_certificate(hash)
            .expect("inspect retained publication"),
...
async fn queue_plan_synced_certificate_requires_canonical_distinct_authority_quorum() {
    let route = RoutingDecision::new(LaneId::SINGLE, DataSpaceId::UNIVERSAL);
    let (_app, mut request) =
        incoming_proxy_submit_fixture(0xd0, ToriiProxyTransactionAdmissionV1::QueuePlanSynced);
    let signers = (0_u8..4)
        .map(|offset| {
            checked_torii_test_ed25519_keypair(
    ...
    let quorum_certificate: QueuePlanAdmissionCertificateV1 =
    ...
    noncanonical_snapshot.body = norito::to_bytes(&QueuePlanAdmissionCertificateV1 {
        version: QUEUE_PLAN_ADMISSION_CERTIFICATE_VERSION_V1,
...
async fn queue_plan_synced_max_roster_reaches_honest_quorum_past_byzantine_prefix() {
    let route = RoutingDecision::new(LaneId::SINGLE, DataSpaceId::UNIVERSAL);
    let (_app, mut request) =
        incoming_proxy_submit_fixture(0xca, ToriiProxyTransactionAdmissionV1::QueuePlanSynced);
...
async fn queue_plan_synced_first_quorum_never_waits_for_pending_equivocation_evidence() {
    let route = RoutingDecision::new(LaneId::SINGLE, DataSpaceId::UNIVERSAL);
    let (_app, mut request) =
        incoming_proxy_submit_fixture(0xed, ToriiProxyTransactionAdmissionV1::QueuePlanSynced);
    let signers = (0_u8..4)
        .map(|offset| {
            checked_torii_test_ed25519_keypair(
...
fn queue_plan_synced_local_authority_stays_in_shared_quorum_aggregation() {
    let (app, request) =
        incoming_proxy_submit_fixture(0xe6, ToriiProxyTransactionAdmissionV1::QueuePlanSynced);
    let local_peer_id = app
        .local_peer_id
        .clone()
        .expect("strict fixture configures a local peer");
    ...
    *admission_binding = Some(
    ...
    assert!(
        std::fs::metadata(&journal_path)
            ...
            .len()
            > 0,
        "f+1 acceptance must include a locally fsynced V1 journal record"
    );
...
async fn queue_plan_synced_admission_context_rejects_height_plan_leg_incarnation_roster_and_threshold_drift()
 {
    let (app, request) =
        incoming_proxy_submit_fixture(0xd6, ToriiProxyTransactionAdmissionV1::QueuePlanSynced);
    let mutate_context =
        |mut request: ToriiProxyRequestV1,
         mutation: fn(&mut queue::QueuePlanAdmissionContextV1)| {
            let ToriiProxyRequestKindV1::SubmitTransaction {
                admission_binding, ..
            } = &mut request.request
            else {
    ...
    let error = super::queue_plan_synced_acceptance_expectation(&nondeterministic_request_id)
        .expect_err("semantic request identity is checked before live-state admission");
    assert!(error.contains("noncanonical semantic request identity"));
    let response =
...
async fn queue_plan_synced_response_bounds_reject_headers_body_encoding_and_decode_amplification() {
    let (app, request) =
        incoming_proxy_submit_fixture(0xd7, ToriiProxyTransactionAdmissionV1::QueuePlanSynced);
    let receipt =
    ...
    oversized_body.body =
        vec![0_u8; QUEUE_PLAN_SYNCED_CERTIFICATE_MAX_BODY_BYTES_V1.saturating_add(1)];
    assert!(
    ...
    too_many_headers.headers = (0..=QUEUE_PLAN_SYNCED_MAX_HEADERS_V1)
    ...
    oversized_name.headers = vec![iroha_core::torii_proxy::ToriiProxyHeaderV1 {
        name: "n".repeat(QUEUE_PLAN_SYNCED_MAX_HEADER_NAME_BYTES_V1.saturating_add(1)),
        value: Vec::new(),
    }];
    assert!(
    ...
    oversized_value.headers = vec![iroha_core::torii_proxy::ToriiProxyHeaderV1 {
        ...
        value: vec![b'v'; QUEUE_PLAN_SYNCED_MAX_HEADER_VALUE_BYTES_V1.saturating_add(1)],
    }];
    assert!(
...
fn queue_plan_synced_certificate_binds_exact_durable_journal_claim() {
    let (app, request) =
        incoming_proxy_submit_fixture(0xdb, ToriiProxyTransactionAdmissionV1::QueuePlanSynced);
    let expectation = super::queue_plan_synced_acceptance_expectation(&request)
    ...
    let snapshot_for = |mutation: fn(&mut QueuePlanAdmissionCertificateV1)| {
        ...
        snapshot.body = norito::to_bytes(&certificate).expect("encode mutated V1 certificate");
        snapshot
    };
...
async fn queue_plan_quorum_is_accepted_before_registry_application() {
    let signers = (0_u8..4)
        .map(|offset| {
            checked_torii_test_keypair_from_seed_byte(
                0xde_u8.wrapping_add(offset),
                Algorithm::BlsNormal,
                "derive unapplied durable-admission fixture key",
            )
        })
        .collect::<Vec<_>>();
    let (app, request) = incoming_proxy_submit_fixture_with_validator_signers(
    ...
    assert_eq!(
        expected.durability_threshold, 2,
        "four validators require f+1 durability votes"
    );
    let receipts = signers
        .iter()
        .take(expected.durability_threshold)
...
async fn incoming_queue_plan_synced_exact_retry_survives_height_advance() {
    ...
    let journal_path = journal_dir.path().join("queue_plan_journal.norito");
    let (app, request) =
        incoming_proxy_submit_fixture(0xd5, ToriiProxyTransactionAdmissionV1::QueuePlanSynced);
    app.queue
        .install_plan_journal(&journal_path, 1024 * 1024, true)
...
async fn incoming_submit_queue_plan_synced_fails_closed_when_local_peer_and_signer_diverge() {
    ...
    let journal_path = journal_dir.path().join("queue_plan_journal.norito");
    let (mut app, request) =
        incoming_proxy_submit_fixture(0xe4, ToriiProxyTransactionAdmissionV1::QueuePlanSynced);
    let expected = super::queue_plan_synced_acceptance_expectation(&request)
        .unwrap()
        .unwrap();
    let signing_peer = PeerId::from(app.torii_proxy_bridge_signer.public_key().clone());
    let mismatched_peer = expected
        .admission_binding
        .admission_context
        .route_incarnations[0]
        .validator_set
        .iter()
        .find(|peer| **peer != signing_peer)
        .expect("another bound authority reaches the signer-consistency check")
        .clone();
    assert_ne!(mismatched_peer, signing_peer);
    Arc::get_mut(&mut app)
...
#[cfg(feature = "connect")]
#[test]
fn queue_plan_synced_catch_up_hint_requires_bounded_unambiguous_response() {
    use iroha_core::torii_proxy::ToriiProxyHeaderV1;
    let valid = ToriiProxyHttpResponseV1 {
        status_code: 503,
        headers: vec![ToriiProxyHeaderV1 {
            name: "x-iroha-reject-code".to_owned(),
            value: b"queue_plan_admission_context_future".to_vec(),
        }],
        body: Vec::new(),
    ...
    for variant in 0..7 {
        ...
        match variant {
            ...
            1 => invalid.headers.clear(),
            2 => invalid.headers[0].value = b"route_unavailable".to_vec(),
            3 => invalid.headers.push(ToriiProxyHeaderV1 {
                name: "X-Iroha-Reject-Code".to_owned(),
                value: valid.headers[0].value.clone(),
            }),
            4 => invalid.body = vec![0; QUEUE_PLAN_SYNCED_CERTIFICATE_MAX_BODY_BYTES_V1 + 1],
            5 => invalid.headers.push(ToriiProxyHeaderV1 {
                name: "content-encoding".to_owned(),
                value: b"gzip".to_vec(),
            }),
            6 => invalid.headers.push(ToriiProxyHeaderV1 {
                name: "content-length".to_owned(),
                value: b"10".to_vec(),
            }),
            _ => unreachable!(),
...
#[cfg(feature = "connect")]
#[tokio::test]
async fn queue_plan_synced_persistent_future_preserves_partial_claim_at_deadline() {
    let (_app, mut request) =
        incoming_proxy_submit_fixture(0xd1, ToriiProxyTransactionAdmissionV1::QueuePlanSynced);
    let signers = (0_u8..4)
        .map(|index| {
            checked_torii_test_ed25519_keypair(0xd2 + index, "persistent catch-up authority")
        })
        .collect::<Vec<_>>();
    let authorities = bind_queue_plan_synced_test_authorities(&mut request, &signers);
    request.deadline_unix_ms = super::torii_proxy_now_unix_ms().unwrap() + 350;
    let receipt = queue_plan_synced_test_certificate_snapshot(
        &request,
        vec![exact_queue_plan_synced_test_receipt(
            &request,
            &signers[0],
    ...
    let expected_deadline = request.deadline_unix_ms;
    let attempts = Arc::new(Mutex::new([0_usize; 2]));
    let counts = attempts.clone();
    let selected = authorities[..2].to_vec();
    ...
    let response = tokio::time::timeout(
        ...
        super::execute_torii_proxy_request_across_candidates(
            ...
            move |candidate, request| {
                ...
                assert_eq!(request.deadline_unix_ms, expected_deadline);
                let index = selected
                    .iter()
                    .position(|peer| peer == candidate.peer_id())
                    .unwrap();
                ...
                async move { Ok::<_, ToriiProxyAttemptError>(response) }
            },
            move |request_id| {
    ...
    .await
    .expect("persistent catch-up must stay within the original request budget");
    assert_eq!(response.status(), StatusCode::SERVICE_UNAVAILABLE);
    assert_eq!(
    ...
    assert!(
        counts[1] >= 2 && counts[1] <= 8,
        "bounded retries: {counts:?}"
    );
...
#[cfg(feature = "connect")]
#[tokio::test]
async fn queue_plan_synced_deadline_cancels_only_its_owned_waiter() {
    let (app, mut request) =
        incoming_proxy_submit_fixture(0xe1, ToriiProxyTransactionAdmissionV1::QueuePlanSynced);
    let signers = (0_u8..4)
        .map(|index| {
            checked_torii_test_ed25519_keypair(0xe2 + index, "cancelled catch-up authority")
        })
        .collect::<Vec<_>>();
    let authorities = bind_queue_plan_synced_test_authorities(&mut request, &signers);
    request.deadline_unix_ms = super::torii_proxy_now_unix_ms().unwrap() + 100;
    let pending_key = (request.request_id, authorities[0].clone());
    ...
    let response = tokio::time::timeout(
        ...
        super::execute_torii_proxy_request_across_candidates(
            ...
            move |candidate, request| {
                ...
                let started = attempt_started.clone();
                async move {
                    let (tx, _rx) = tokio::sync::oneshot::channel();
                    let _waiter = super::register_torii_proxy_pending_waiter(
                        &app,
                        (request.request_id, candidate.peer_id().clone()),
                        tx,
                        1024,
                        true,
                    );
    ...
    .await
    .expect("deadline must cancel the indefinitely pending attempt");
    ...
    drop(live_waiter);
    assert!(app.torii_proxy_pending.lock().is_empty());
    assert!(matches!(
        live_rx.try_recv(),
...
#[cfg(feature = "connect")]
async fn exact_queue_plan_synced_quorum_snapshot(
    app: &SharedAppState,
    request: &ToriiProxyRequestV1,
    second_signer: &KeyPair,
) -> ToriiProxyHttpResponseV1 {
    let mut snapshot = exact_queue_plan_synced_acceptance_snapshot(app, request).await;
    let expected = super::queue_plan_synced_acceptance_expectation(request)
        .unwrap()
        .unwrap();
    let mut certificate: QueuePlanAdmissionCertificateV1 =
        norito::decode_from_bytes(&snapshot.body).unwrap();
    assert_eq!(
        certificate.attestations.len(),
        1,
        "one authority supplies one durable leaf"
    );
    assert_eq!(
        expected.durability_threshold, 2,
        "the four-validator fixture requires f+1"
    );
    let signer = PeerId::new(second_signer.public_key().clone());
    let index = certificate.binding.admission_context.route_incarnations[0]
        .validator_set
        .iter()
    ...
    certificate
        .attestations
        .push(sign_queue_plan_synced_test_receipt(
            &certificate.binding,
    ...
    certificate
        .attestations
        .sort_by_key(|attestation| attestation.validator_index);
    snapshot.body = norito::to_bytes(&certificate).unwrap();
    snapshot.headers = super::canonical_queue_plan_synced_certificate_headers(&expected);
    assert_eq!(
        super::validate_queue_plan_synced_acceptance(&snapshot, &expected)
            .unwrap()
            .len(),
...
#[cfg(feature = "connect")]
#[tokio::test]
async fn queue_plan_synced_attempt_window_is_parallel_bounded_and_released_at_quorum() {
    ...
    let (_app, mut request) =
        incoming_proxy_submit_fixture(0xa0, ToriiProxyTransactionAdmissionV1::QueuePlanSynced);
    let signers = (0_u8..7)
        .map(|index| {
            checked_torii_test_ed25519_keypair(0xa1 + index, "bounded-window authority fixture")
        })
        .collect::<Vec<_>>();
    let authorities = bind_queue_plan_synced_test_authorities(&mut request, &signers);
    let snapshots = (0..3)
    ...
    let count_active = active.clone();
    let count_peak = peak.clone();
    let response = tokio::time::timeout(
        Duration::from_secs(2),
        super::execute_torii_proxy_request_across_candidates(
            authorities
            ...
            move |candidate, _request| {
                let index = authorities
                    ...
                    .position(|peer| peer == candidate.peer_id())
                    .unwrap();
                let snapshot = snapshots.get(index).cloned();
                let active = count_active.clone();
                let peak = count_peak.clone();
                let barrier = first_wave.clone();
                async move {
                    let count = active.fetch_add(1, AtomicOrdering::SeqCst) + 1;
                    let _guard = ActiveAttempt(active);
                    peak.fetch_max(count, AtomicOrdering::SeqCst);
                    if index < 4 {
                        barrier.wait().await;
                    }
                    match snapshot {
                        Some(snapshot) => Ok::<_, ToriiProxyAttemptError>(snapshot),
    ...
    .await
    .expect("four bounded attempts must run together without waiting for silent peers");
    assert_eq!(response.status(), StatusCode::ACCEPTED);
    assert_eq!(
        peak.load(AtomicOrdering::SeqCst),
        4,
        "a seven-authority roster must not expand the response window"
    );
    assert_eq!(
        active.load(AtomicOrdering::SeqCst),
        0,
        "the first valid quorum cancels every remaining attempt"
    );
...
async fn strict_proxy_admission_retains_w_across_delayed_state_publication() {
    let signers = (0_u8..4)
        .map(|offset| {
            checked_torii_test_keypair_from_seed_byte(
                0xe4_u8.wrapping_add(offset),
                Algorithm::BlsNormal,
                "derive delayed-publication fixture key",
            )
        })
        .collect::<Vec<_>>();
    let (app, request) = incoming_proxy_submit_fixture_with_validator_signers(
    ...
    assert_eq!(app.state.committed_height(), 0);
    let expected = super::queue_plan_synced_acceptance_expectation(&request)
        .unwrap()
        .unwrap();
    let receipts = signers
    ...
    let work = tokio::spawn(async move {
        super::proxy_response_finalization::complete(
            super::torii_proxy_snapshot_to_response(snapshot),
            memory,
            |response| async move {
    ...
    // Acknowledge the real typed overlap before delaying publication. Merely
    // spawning a task and sleeping would not prove it reached persistence.
    tokio::time::timeout(Duration::from_secs(10), async {
        while observed_wait_height.load(std::sync::atomic::Ordering::Acquire) == 0 {
            assert!(
                !work.is_finished(),
                "admission returned before waiting for State"
            );
    ...
    .unwrap();
    assert_eq!(
        observed_wait_height.load(std::sync::atomic::Ordering::Acquire),
        1
    );
    // Deliberately exceed the synchronous frontier reconciliation interval.
    tokio::time::sleep(Duration::from_millis(750)).await;
    assert!(
        !work.is_finished(),
        "the original request must survive a publication gap over 250 ms"
    );
    assert_eq!(app.torii_proxy_memory_inflight.available_permits(), 0);
    assert_eq!(
        app.kura
    ...
    let response = tokio::time::timeout(Duration::from_secs(10), work)
        .await
        .unwrap()
        .unwrap();
    assert_eq!(response.status(), StatusCode::ACCEPTED);
...
#[cfg(feature = "connect")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn late_physical_admission_preserves_durability_and_reports_reconciliation() {
    let signers = (0_u8..4)
        .map(|offset| {
            checked_torii_test_keypair_from_seed_byte(
                0xec_u8.wrapping_add(offset),
                Algorithm::BlsNormal,
                "derive late-physical-admission fixture key",
            )
        })
        .collect::<Vec<_>>();
    let (app, request) = incoming_proxy_submit_fixture_with_validator_signers(
    ...
    let deadline = super::queue_plan_publication_wait::PersistenceDeadline::new(
        Instant::now() - (super::TORII_PROXY_EXECUTION_BUDGET - Duration::from_millis(500)),
        request.deadline_unix_ms,
    );
    ...
    let work = tokio::spawn(async move {
        super::proxy_response_finalization::complete(
            ...
            |_| async move {
                ...
                // This is the production post-persistence response boundary, after
                // the required durable work; elapsed time must not become a fresh 202.
                super::queue_plan_completed_admission_response(
                    snapshot,
                    &expected.admission_binding,
                    &deadline,
                )
    ...
    let response = tokio::time::timeout(Duration::from_secs(5), work)
        .await
        .unwrap()
        .unwrap();
    assert_eq!(response.status(), StatusCode::SERVICE_UNAVAILABLE);
    ...
    assert!(envelope.message().contains("reconcile"));
    let details = envelope
        .details
        .expect("reconciliation retains exact transaction identities");
    assert_eq!(details.entrypoint_hash, Some(expected_entrypoint));
    assert_eq!(details.tx_hash, expected_transaction);
    assert_eq!(
        details.reject_code.as_deref(),
        Some(super::QUEUE_PLAN_OUTCOME_UNKNOWN_REJECT_CODE)
    );
    assert!(
        details.queue.is_none(),
        "distributed ambiguity cannot fabricate local queue telemetry"
    );
    assert_eq!(
        app.kura
            .pending_queue_plan_admission_certificate(hash)
