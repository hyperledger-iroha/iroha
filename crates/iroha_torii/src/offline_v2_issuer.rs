...
fn load_kagemusha_v2_redeem_operation_receipt(
    ...
    // TODO: Persist the redemption transaction and its operation receipt atomically, then load and
    // validate that receipt here before enabling either first submission or idempotent replay.
...
mod tests {
    ...
    #[test]
    fn kagemusha_v2_redeem_receipts_remain_disabled_until_atomic_persistence_exists() {
        let key_pair = checked_seed_keypair(0x68);
        let authorization = signed_kagemusha_v2_authorization(&key_pair, NOW_MS, NOW_MS + 60_000);
        let asset_definition_id = asset_definition_for_parse_tests();
        let app = app_with_account_and_asset_scope(
            &authorization.authority,
            &asset_definition_id,
            AssetBalanceScope::Global,
        );

        let error = load_kagemusha_v2_redeem_operation_receipt(&app, &authorization)
            .expect_err("redeem receipt lookup must fail closed while persistence is unavailable");
        match error {
            Error::AppServiceUnavailable { code, message } => {
                assert_eq!(code, "OFFLINE_KAGEMUSHA_REDEEM_RECEIPT_UNAVAILABLE");
                assert!(message.contains(&hex::encode(authorization.operation_id)));
                assert!(message.contains("redemption remains disabled"));
            }
            other => panic!("unexpected disabled receipt error: {other}"),
        }
    }
