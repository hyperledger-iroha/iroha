...
impl OfflineV2IssuerRuntime {
    ...
    fn retained_operations(
        &self,
    ) -> Result<std::sync::RwLockWriteGuard<'_, BTreeMap<[u8; 32], OfflineOperationRecord>>, Error>
    {
    ...
    fn retained_operation(
        ...
        Ok(self.retained_operations()?.get(&operation_id).cloned())
    }
}
...
impl OfflineV2IssuerRuntime {
    fn claim_submission(
        ...
        let operations = self.retained_operations()?;
    ...
    fn record_admitted_operation(
        ...
        let operation_id = request.authorization().operation_id;
        let mut operations = self.retained_operations()?;
        if let Some(existing) = operations.get(&operation_id) {
            ensure_same_offline_request(&existing.request.as_ref(), &request)?;
...
fn find_admitted_offline_operation(
    ...
    let operations = issuer.retained_operations()?;
    let operation_id = request.authorization().operation_id;
...
pub(crate) fn handle_operation_status(
    ...
    let issuer = require_issuer(app)?;
    let record = issuer.retained_operation(operation_id)?;
...
mod tests {
    ...
    fn app_with_offline_history(
        ...
        inner.kura = kura;
        inner.offline_v2_issuer = Some(issuer);
        app
    }
    ...
    #[test]
    fn status_cache_lookup_prunes_expired_operations_without_new_submissions() {
        let issuer = submission_test_issuer();
        let mut request = submission_test_request(0x31);
        request.authorization.expires_at_ms = 0;
        let operation_id = request.authorization.operation_id;
        issuer.operations.write().expect("operations lock").insert(
            operation_id,
            OfflineOperationRecord {
                request: OfflineOperationRequest::TopUp(&request).into_owned(),
                transaction_hash: submission_test_hash(0x32),
                submitted_at_ms: request.authorization.issued_at_ms,
            },
        );
        ...
        assert!(
            issuer
                ...
                .expect("status cache lookup")
                .is_none(),
            "status-only traffic must not retain an expired operation forever"
        );
        assert!(
            issuer
                .operations
    ...
    fn transaction_signing_failure_does_not_expose_the_signer_error() {
        let error = offline_v2_transaction_signing_error(
