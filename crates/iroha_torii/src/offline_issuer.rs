...
use iroha_data_model::offline::{
    OFFLINE_BEARER_SIGNATURE_ALGORITHM_ECDSA_P256_SHA256,
    OFFLINE_BEARER_SIGNATURE_ALGORITHM_ED25519, OfflineBearerAssetSendLimitV2,
    OfflineBearerCertificateV2, OfflineBearerCreditReceiptV2, OfflineBearerDebitReceiptV2,
    OfflineBearerPolicyBundleV2, OfflineBearerSettlementBatchV2,
};
...
const PATH_BEARER_SETTLEMENTS: &str = "/v1/offline/bearer/settlements";
...
pub(crate) struct OfflineIssuerRuntime {
    ...
    bearer_settlement_replay: Arc<Mutex<BTreeMap<String, String>>>,
}
...
impl OfflineIssuerRuntime {
    pub(crate) fn from_config(config: actual::ToriiOfflineIssuer) -> Self {
        Self {
            ...
            bearer_settlement_replay: Arc::new(Mutex::new(BTreeMap::new())),
        }
    }
...
pub(crate) async fn handle_key_refill(
    ...
    let bearer_policy = build_bearer_policy_bundle(&issuer, now_ms)?;
    let bearer_certificate = build_bearer_certificate(
        &issuer,
        &parsed,
        &attestation,
        &bearer_policy,
        &(*app.chain_id).to_string(),
        now_ms,
    )?;
    ...
    json_ok(json_object(vec![
        ...
        (
            "bearer_policy_bundle",
            bearer_json_value(&bearer_policy, "offline_bearer_policy_bundle")?,
        ),
        (
            "bearer_policy_bundle_norito_base64",
            bearer_norito_base64(&bearer_policy, "offline_bearer_policy_bundle_norito")?,
        ),
        (
            "bearer_purse_certificate",
            bearer_json_value(&bearer_certificate, "offline_bearer_certificate")?,
        ),
        (
            "bearer_purse_certificate_norito_base64",
            bearer_norito_base64(&bearer_certificate, "offline_bearer_certificate_norito")?,
        ),
    ]))
}
...
pub(crate) async fn handle_bearer_settlement(
    ...
    let settlement_value = normalize_bearer_settlement_value(&value)?;
    let account_literal = required_string(&settlement_value, "account_id")
        .or_else(|_| {
            let debit = settlement_value
                .get("debit_receipts")
                .and_then(Value::as_array)
    ...
    let mut accepted = validate_bearer_settlement_batch_for_submission(
        app.as_ref(),
        &issuer,
        &settlement_value,
        now_ms(),
    ...
    let settlement_tx_hash =
        submit_bearer_settlement_transaction(app.as_ref(), &issuer, &accepted.accepted_settlements)
            .await?;
    for settlement in &accepted.accepted_settlements {
        record_bearer_settlement_acceptance(
            &issuer,
            &settlement.transfer_id,
            settlement.fingerprint.clone(),
        );
    }
    accepted.accepted_settlements.clear();
    json_ok(json_object(vec![
        ...
        (
            "settlement_tx_hash",
            settlement_tx_hash.map_or(Value::Null, string_value),
        ),
    ]))
}

fn normalize_bearer_settlement_value(value: &Value) -> Result<Value, Error> {
    let mut normalized = value.clone();
    let Value::Object(map) = &mut normalized else {
        return Ok(normalized);
    };

    if !map.contains_key("policy_bundle") {
        if let Some(raw) = optional_string(value, "policy_bundle_norito_base64") {
            let policy: OfflineBearerPolicyBundleV2 =
                decode_bearer_norito_base64(raw, "policy_bundle_norito_base64")?;
            map.insert(
                "policy_bundle".to_owned(),
                json::to_value(&policy).map_err(|source| Error::SerializationFailure {
                    context: "offline bearer policy JSON conversion",
                    source,
                })?,
            );
        }
    ...
    if (!map.contains_key("debit_receipts") || !map.contains_key("credit_receipts"))
        && let Some(raw) = optional_string(value, "settlement_batch_norito_base64")
    {
        let batch: OfflineBearerSettlementBatchV2 =
            decode_bearer_norito_base64(raw, "settlement_batch_norito_base64")?;
        let Value::Object(mut batch_map) =
            json::to_value(&batch).map_err(|source| Error::SerializationFailure {
                context: "offline bearer settlement batch JSON conversion",
                source,
            })?
        else {
            return Err(validation(
                "OFFLINE_BEARER_BATCH_INVALID",
                "Offline Bearer settlement batch did not decode as an object.",
            ));
        };
        for field in [
            "version",
            ...
            if let Some(field_value) = batch_map.remove(field) {
                map.insert(field.to_owned(), field_value);
            }
        }
    }
...
fn decode_bearer_norito_base64<T>(raw: &str, field: &'static str) -> Result<T, Error>
where
    for<'de> T: norito::NoritoDeserialize<'de>,
{
    let bytes = BASE64_STANDARD.decode(raw.trim()).map_err(|_| {
        validation_owned(
            "OFFLINE_BEARER_NORITO_BASE64_INVALID",
            format!("{field} must be base64-encoded Norito bytes."),
        )
    })?;
    norito::decode_from_bytes::<T>(&bytes).map_err(|err| {
        validation_owned(
            "OFFLINE_BEARER_NORITO_INVALID",
            format!("{field} is not a canonical Offline Bearer Norito payload: {err}"),
        )
    })
}
...
struct BearerSettlementValidation {
    ...
    rejected: Vec<Value>,
    accepted_settlements: Vec<BearerAcceptedSettlement>,
}

#[derive(Debug, Clone)]
struct BearerAcceptedSettlement {
    transfer_id: String,
    fingerprint: String,
    debit: OfflineBearerDebitReceiptV2,
    credit: OfflineBearerCreditReceiptV2,
}
...
fn validate_bearer_settlement_batch(
    ...
    validate_bearer_settlement_batch_with_replay(
        issuer,
        value,
        now_ms,
        |transfer_id, fingerprint| {
            Ok(classify_bearer_settlement_replay_in_memory(
                issuer,
                transfer_id,
                fingerprint,
            ))
...
fn validate_bearer_settlement_batch_for_submission(
    app: &AppState,
    issuer: &OfflineIssuerRuntime,
    value: &Value,
    now_ms: u64,
    ...
    validate_bearer_settlement_batch_with_replay(
        ...
        now_ms,
        |transfer_id, fingerprint| {
            classify_bearer_settlement_replay(app, issuer, transfer_id, fingerprint)
        },
        false,
    )
}

fn validate_bearer_settlement_batch_with_replay(
    issuer: &OfflineIssuerRuntime,
    value: &Value,
    now_ms: u64,
    mut classify_replay: impl FnMut(&str, &str) -> Result<BearerSettlementReplay, Error>,
    record_on_accept: bool,
) -> Result<BearerSettlementValidation, Error> {
    let policy_value = value.get("policy_bundle").ok_or_else(|| {
        validation(
            "OFFLINE_BEARER_POLICY_REQUIRED",
            "Offline Bearer settlement requires policy_bundle.",
        )
    })?;
    let policy: OfflineBearerPolicyBundleV2 =
        json::from_value(policy_value.clone()).map_err(|err| {
            validation_owned(
                "OFFLINE_BEARER_POLICY_INVALID",
                format!("Offline Bearer policy_bundle is not canonical: {err}"),
            )
        })?;
    verify_bearer_policy_signature(issuer, &policy).map_err(|reason| {
        validation_owned(
            "OFFLINE_BEARER_POLICY_SIGNATURE_INVALID",
            format!("Offline Bearer policy signature is invalid: {reason}"),
        )
    })?;
    require_bearer_policy_fresh(&policy, now_ms).map_err(|reason| {
        validation_owned(
            "OFFLINE_BEARER_POLICY_STALE",
            format!("Offline Bearer policy is not valid now: {reason}"),
        )
    })?;

    let mut batch_value = value.clone();
    let Value::Object(batch_map) = &mut batch_value else {
        return Err(validation(
            "OFFLINE_BEARER_INVALID_JSON",
            "Offline Bearer settlement body must be a JSON object.",
        ));
    };
    for field in [
        "account_id",
        ...
        "witness_base64",
        "policy_bundle",
        "policy_bundle_norito_base64",
        "settlement_batch_norito_base64",
    ] {
        batch_map.remove(field);
    }
    let batch: OfflineBearerSettlementBatchV2 = json::from_value(batch_value).map_err(|err| {
        validation_owned(
            "OFFLINE_BEARER_BATCH_INVALID",
            format!("Offline Bearer settlement batch is not canonical: {err}"),
        )
    })?;
    if batch.version != 2 {
        return Err(validation(
    ...
    let mut debit_by_transfer: std::collections::BTreeMap<&str, Vec<&OfflineBearerDebitReceiptV2>> =
        std::collections::BTreeMap::new();
    let mut credit_by_transfer: std::collections::BTreeMap<
        &str,
        Vec<&OfflineBearerCreditReceiptV2>,
    > = std::collections::BTreeMap::new();
    for receipt in &batch.debit_receipts {
        debit_by_transfer
            .entry(receipt.transfer_id.as_str())
            .or_default()
            .push(receipt);
    }
    for receipt in &batch.credit_receipts {
        credit_by_transfer
            .entry(receipt.transfer_id.as_str())
            .or_default()
            .push(receipt);
    }
    let mut transfer_ids = debit_by_transfer
        .keys()
        .chain(credit_by_transfer.keys())
        .copied()
    ...
    let mut accepted_settlements = Vec::new();

    for transfer_id in std::mem::take(&mut transfer_ids) {
        let debit_receipts = debit_by_transfer.remove(transfer_id).unwrap_or_default();
        let credit_receipts = credit_by_transfer.remove(transfer_id).unwrap_or_default();
        if debit_receipts.len() > 1 || credit_receipts.len() > 1 {
            duplicates.push(string_value(transfer_id));
            continue;
        }
        let debit = debit_receipts.first().copied();
        let credit = credit_receipts.first().copied();
        if credit.is_some() && debit.is_none() {
            rejected.push(rejected_transfer(transfer_id, "debit_receipt_required"));
            continue;
        }
        let fingerprint = match bearer_transfer_fingerprint(debit, credit) {
            Ok(fingerprint) => fingerprint,
            Err(reason) => {
                rejected.push(rejected_transfer(transfer_id, reason));
                continue;
            }
        };
        match classify_replay(transfer_id, &fingerprint)? {
            BearerSettlementReplay::New => {}
            BearerSettlementReplay::Duplicate => {
                duplicates.push(string_value(transfer_id));
                continue;
            }
            BearerSettlementReplay::TamperedVariant => {
                rejected.push(rejected_transfer(transfer_id, "tampered_replay"));
                continue;
            }
        }

        let result = match (debit, credit) {
            (Some(debit), Some(credit)) => validate_bearer_debit_receipt(
                issuer,
                &policy,
                &batch,
                debit,
                now_ms,
                BearerDebitPurseRole::Recipient,
            )
            .and_then(|()| {
                validate_bearer_credit_receipt(issuer, &policy, &batch, credit, debit, now_ms)
            }),
            (Some(_), None) => Err("credit_receipt_required"),
            (None, Some(_)) => unreachable!("credit without debit handled above"),
            (None, None) => unreachable!("transfer id came from receipt maps"),
        };
        match result {
            Ok(()) => {
                if record_on_accept {
                    record_bearer_settlement_acceptance(issuer, transfer_id, fingerprint.clone());
                }
                let Some(debit) = debit else {
                    unreachable!("accepted settlement has a debit receipt")
                };
                let Some(credit) = credit else {
                    unreachable!("accepted settlement has a credit receipt")
                };
                accepted_settlements.push(BearerAcceptedSettlement {
                    transfer_id: transfer_id.to_owned(),
                    fingerprint,
                    debit: debit.clone(),
                    credit: credit.clone(),
                });
                accepted.push(string_value(transfer_id));
            }
...
enum BearerDebitPurseRole {
    Sender,
    Recipient,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum BearerSettlementReplay {
    New,
    Duplicate,
    TamperedVariant,
}

fn classify_bearer_settlement_replay(
    app: &AppState,
    issuer: &OfflineIssuerRuntime,
    transfer_id: &str,
    fingerprint: &str,
) -> Result<BearerSettlementReplay, Error> {
    let durable = durable_bearer_settlement_fingerprint(app, issuer, transfer_id)?;
    Ok(match durable {
        Some(existing) if existing == fingerprint => BearerSettlementReplay::Duplicate,
        Some(_) => BearerSettlementReplay::TamperedVariant,
        None => classify_bearer_settlement_replay_in_memory(issuer, transfer_id, fingerprint),
    })
}

fn classify_bearer_settlement_replay_in_memory(
    issuer: &OfflineIssuerRuntime,
    transfer_id: &str,
    fingerprint: &str,
) -> BearerSettlementReplay {
    let replay = issuer
        .bearer_settlement_replay
        .lock()
        .expect("offline bearer settlement replay lock");
    match replay.get(transfer_id) {
        None => BearerSettlementReplay::New,
        Some(existing) if existing == fingerprint => BearerSettlementReplay::Duplicate,
        Some(_) => BearerSettlementReplay::TamperedVariant,
    }
}

fn record_bearer_settlement_acceptance(
    issuer: &OfflineIssuerRuntime,
    transfer_id: &str,
    fingerprint: String,
) {
    let mut replay = issuer
        .bearer_settlement_replay
        .lock()
        .expect("offline bearer settlement replay lock");
    replay.entry(transfer_id.to_owned()).or_insert(fingerprint);
}

fn durable_bearer_settlement_fingerprint(
    app: &AppState,
    issuer: &OfflineIssuerRuntime,
    transfer_id: &str,
) -> Result<Option<String>, Error> {
    let key = bearer_settlement_metadata_key(transfer_id);
    let world = app.state.world_view();
    let Ok(account) = world.account(&issuer.authority) else {
        return Ok(None);
    };
    let Some(raw) = account.metadata().get(&key) else {
        return Ok(None);
    };
    raw.try_into_any_norito::<String>()
        .map(Some)
        .map_err(|err| {
            validation_owned(
                "OFFLINE_BEARER_REPLAY_STATE_INVALID",
                format!("Offline Bearer replay marker is not a settlement fingerprint: {err}"),
            )
        })
}

async fn submit_bearer_settlement_transaction(
    app: &AppState,
    issuer: &OfflineIssuerRuntime,
    settlements: &[BearerAcceptedSettlement],
) -> Result<Option<String>, Error> {
    if settlements.is_empty() {
        return Ok(None);
    }
    let instructions = bearer_settlement_instructions(app, issuer, settlements)?;
    let tx = TransactionBuilder::new((*app.chain_id).clone(), issuer.authority.clone().into())
    ...
    routing::handle_transaction_with_metrics(
        ...
        tx,
        app.telemetry.clone(),
        PATH_BEARER_SETTLEMENTS,
    )
    .await?;
    Ok(Some(tx_hash))
}

fn bearer_settlement_instructions(
    app: &AppState,
    issuer: &OfflineIssuerRuntime,
    settlements: &[BearerAcceptedSettlement],
) -> Result<Vec<InstructionBox>, Error> {
    let mut instructions = Vec::with_capacity(settlements.len() * 2);
    for settlement in settlements {
        let sender = parse_bearer_settlement_account(
            app,
            &settlement.debit.sender_certificate.account_id,
            "sender",
        )?;
        let recipient = parse_bearer_settlement_account(
            app,
            &settlement.credit.recipient_certificate.account_id,
            "recipient",
        )?;
        let asset_definition_id =
            AssetDefinitionId::from_str(&settlement.debit.asset_definition_id).map_err(|err| {
                validation_owned(
                    "OFFLINE_BEARER_ASSET_INVALID",
                    format!("Offline Bearer settlement asset_definition_id is invalid: {err}"),
                )
            })?;
        let amount = parse_positive_amount(&settlement.debit.amount, "amount")?;
        let asset_id = AssetId::new(asset_definition_id, sender);
        instructions.push(InstructionBox::from(Transfer::asset_numeric(
            asset_id, amount, recipient,
        )));
        instructions.push(InstructionBox::from(SetKeyValue::account(
            issuer.authority.clone(),
            bearer_settlement_metadata_key(&settlement.transfer_id),
            Json::new(settlement.fingerprint.clone()),
        )));
    }
    Ok(instructions)
}

fn parse_bearer_settlement_account(
    app: &AppState,
    account_literal: &str,
    role: &'static str,
    ...
    .map_err(|err| {
        validation_owned(
            "OFFLINE_BEARER_ACCOUNT_INVALID",
            format!(
                "Offline Bearer {role} account_id is invalid: {}",
                err.reason()
            ),
        )
    })
...
fn bearer_settlement_metadata_key(transfer_id: &str) -> Name {
    let digest = sha256_hex(transfer_id.as_bytes());
    Name::from_str(&format!("offline_bearer_settlement_{digest}"))
        .expect("hex-encoded transfer hash produces a valid metadata key")
}

fn bearer_transfer_fingerprint(
    debit: Option<&OfflineBearerDebitReceiptV2>,
    credit: Option<&OfflineBearerCreditReceiptV2>,
) -> Result<String, &'static str> {
    let mut hasher = Sha256::new();
    hasher.update(b"iroha:offline-bearer-v2:settlement-transfer");
    match debit {
        Some(receipt) => hasher.update(
            receipt
                .receipt_hash()
                .map_err(|_| "debit_receipt_hash_invalid")?
                .as_ref(),
        ),
    ...
    match credit {
        Some(receipt) => hasher.update(
            receipt
                .receipt_hash()
                .map_err(|_| "credit_receipt_hash_invalid")?
                .as_ref(),
        ),
        None => hasher.update([0_u8; 32]),
    }
...
fn validate_bearer_debit_receipt(
    ...
    now_ms: u64,
    batch_purse_role: BearerDebitPurseRole,
) -> Result<(), &'static str> {
    if receipt.chain_id != batch.chain_id {
        return Err("chain_mismatch");
    }
    match batch_purse_role {
        BearerDebitPurseRole::Sender if receipt.sender_certificate.purse_id != batch.purse_id => {
            return Err("purse_mismatch");
        }
        BearerDebitPurseRole::Recipient
            if receipt.recipient_certificate.purse_id != batch.purse_id =>
        {
            return Err("purse_mismatch");
        }
        _ => {}
    ...
    if !receipt
        .policy_hash_hex
        .eq_ignore_ascii_case(&policy.policy_hash_hex)
    {
        return Err("policy_hash_mismatch");
    }
    if policy.revoked_transfer_ids.iter().any(|candidate| {
        candidate == &receipt.transfer_id || candidate == &receipt.payment_request_id
    }) {
        return Err("transfer_revoked");
    }
    if receipt.signature_algorithm != receipt.sender_certificate.signature_algorithm {
        return Err("signature_algorithm_mismatch");
    }
    if receipt.sender_certificate.asset_definition_id != receipt.asset_definition_id
        || receipt.recipient_certificate.asset_definition_id != receipt.asset_definition_id
    {
        return Err("asset_mismatch");
    }
    let amount = parse_positive_amount(&receipt.amount, "amount").map_err(|_| "amount_invalid")?;
    if amount > issuer.max_tx_value.clone() {
        return Err("amount_exceeds_policy");
    }
    if amount
        > max_bearer_transaction_amount_for_asset(policy, &receipt.asset_definition_id)
            .map_err(|_| "asset_amount_limit_invalid")?
    {
        return Err("amount_exceeds_asset_policy");
    }
    let pre_balance = parse_amount(&receipt.sender_pre_balance, "sender_pre_balance")
        .map_err(|_| "sender_pre_balance_invalid")?;
    let post_balance = parse_amount(&receipt.sender_post_balance, "sender_post_balance")
        .map_err(|_| "sender_post_balance_invalid")?;
    if pre_balance < Numeric::zero() || post_balance < Numeric::zero() {
        return Err("sender_balance_invalid");
    }
    if pre_balance
        .checked_sub(amount.clone())
        .filter(|expected| expected == &post_balance)
        .is_none()
    {
        return Err("sender_balance_transition_invalid");
    }
    if receipt.created_at_ms > now_ms {
        return Err("created_at_future");
    }
    if receipt.expires_at_ms <= receipt.created_at_ms {
        return Err("expiry_invalid");
    }
    if receipt.sender_sequence == 0 {
        return Err("sender_sequence_invalid");
    }
    if receipt.debit_signature.is_empty() || receipt.receive_challenge_signature.is_empty() {
        return Err("signature_required");
    }
    validate_bearer_certificate(
        issuer,
        policy,
        &receipt.sender_certificate,
        receipt.created_at_ms,
        Some(&receipt.policy_hash_hex),
        Some(&receipt.asset_definition_id),
    )?;
    validate_bearer_certificate(
        issuer,
        policy,
        &receipt.recipient_certificate,
        receipt.created_at_ms,
        Some(&receipt.policy_hash_hex),
        Some(&receipt.asset_definition_id),
    )?;
    verify_bearer_device_signature(
        &receipt.sender_certificate,
        &receipt.signature_algorithm,
        &receipt
            .signing_bytes()
            .map_err(|_| "debit_payload_invalid")?,
        &receipt.debit_signature,
    )?;
    Ok(())
}
...
fn validate_bearer_credit_receipt(
    ...
    if receipt.chain_id != batch.chain_id || receipt.chain_id != debit.chain_id {
        return Err("chain_mismatch");
    }
    if receipt.recipient_certificate.purse_id != batch.purse_id {
        return Err("purse_mismatch");
    }
    if receipt.transfer_id != debit.transfer_id {
        return Err("transfer_mismatch");
    }
    if receipt.amount != debit.amount {
        return Err("amount_mismatch");
    }
    if receipt.accepted_at_ms > debit.expires_at_ms {
        return Err("accepted_after_expiry");
    }
    if receipt.signature_algorithm != receipt.recipient_certificate.signature_algorithm {
        return Err("signature_algorithm_mismatch");
    }
    let amount = parse_positive_amount(&receipt.amount, "amount").map_err(|_| "amount_invalid")?;
    if amount > issuer.max_tx_value.clone() {
    ...
    if amount
        > max_bearer_transaction_amount_for_asset(
            policy,
            &receipt.recipient_certificate.asset_definition_id,
        )
        .map_err(|_| "asset_amount_limit_invalid")?
    {
        return Err("amount_exceeds_asset_policy");
    }
    let pre_balance = parse_amount(&receipt.recipient_pre_balance, "recipient_pre_balance")
        .map_err(|_| "recipient_pre_balance_invalid")?;
    let post_balance = parse_amount(&receipt.recipient_post_balance, "recipient_post_balance")
        .map_err(|_| "recipient_post_balance_invalid")?;
    if pre_balance < Numeric::zero() || post_balance < Numeric::zero() {
        return Err("recipient_balance_invalid");
    }
    let expected_post = pre_balance
        .checked_add(amount.clone())
        .ok_or("recipient_balance_transition_invalid")?;
    if expected_post != post_balance {
        return Err("recipient_balance_transition_invalid");
    }
    if post_balance > issuer.max_balance.clone() {
        return Err("recipient_balance_exceeds_policy");
    }
    if receipt.accepted_at_ms > now_ms {
        return Err("accepted_at_future");
    }
    if receipt.recipient_sequence == 0 {
        return Err("recipient_sequence_invalid");
    }
    if receipt.credit_signature.is_empty() {
        return Err("signature_required");
    }
    validate_bearer_certificate(
        issuer,
    ...
    verify_bearer_device_signature(
        ...
        &receipt
            .signing_bytes()
            .map_err(|_| "credit_payload_invalid")?,
        &receipt.credit_signature,
    )?;
    Ok(())
}
...
fn validate_bearer_certificate(
    ...
    expected_asset_definition_id: Option<&str>,
) -> Result<(), &'static str> {
    if expected_asset_definition_id
        .is_some_and(|expected| expected != certificate.asset_definition_id)
    {
        return Err("asset_mismatch");
    }
    if expected_policy_hash
        .is_some_and(|expected| !expected.eq_ignore_ascii_case(&certificate.policy_hash_hex))
    {
        return Err("policy_hash_mismatch");
    }
    if certificate.issuer_id != policy.issuer_id {
    ...
    if !policy
        .allowed_hardware_classes
        .iter()
        .any(|candidate| candidate == &certificate.hardware_class)
    {
        return Err("hardware_class_disallowed");
    }
    if certificate.public_key.is_empty() {
        return Err("public_key_required");
    }
    if certificate.issued_at_ms > event_time_ms {
        return Err("certificate_from_future");
    }
    if certificate.expires_at_ms <= event_time_ms {
        return Err("certificate_expired");
    }
    if event_time_ms.saturating_sub(certificate.issued_at_ms) > duration_ms(issuer.certificate_ttl)
        || event_time_ms.saturating_sub(certificate.issued_at_ms) > policy.max_certificate_age_ms
    {
        return Err("certificate_too_old");
    }
    if issuer
        .blacklisted_account_ids
        .iter()
        .any(|candidate| candidate == &certificate.account_id)
        || policy
            .blacklisted_account_ids
            .iter()
            .any(|candidate| candidate == &certificate.account_id)
    ...
    if issuer
        .revoked_verdict_ids
        .iter()
        .any(|candidate| candidate == &certificate.device_id || candidate == &certificate.key_id)
        || policy
            .blacklisted_device_ids
            .iter()
            .any(|candidate| candidate == &certificate.device_id)
        || policy
            .blacklisted_key_ids
            .iter()
            .any(|candidate| candidate == &certificate.key_id)
        || policy.revoked_certificate_ids.iter().any(|candidate| {
            candidate == &certificate.certificate_id || candidate == &certificate.key_id
        })
    {
        return Err("certificate_revoked");
    }
    verify_bearer_issuer_signature(
        issuer,
        &policy.signature_algorithm,
        &certificate
            .signing_bytes()
            .map_err(|_| "certificate_payload_invalid")?,
        &certificate.issuer_signature,
    )
    .map_err(|_| "certificate_signature_invalid")?;
    Ok(())
}

fn require_bearer_policy_fresh(
    policy: &OfflineBearerPolicyBundleV2,
    now_ms: u64,
) -> Result<(), &'static str> {
    if policy.issued_at_ms > now_ms {
        return Err("policy_from_future");
    }
    if policy.expires_at_ms <= now_ms {
        return Err("policy_expired");
    }
    if now_ms.saturating_sub(policy.issued_at_ms) > policy.max_policy_age_ms {
        return Err("policy_too_old");
    }
    Ok(())
}

fn max_bearer_transaction_amount_for_asset(
    policy: &OfflineBearerPolicyBundleV2,
    ...
    policy
        .asset_send_limits
        .iter()
        .find(|limit| limit.asset_definition_id == asset_definition_id)
        .map(|limit| parse_positive_amount(&limit.max_transaction_amount, "max_transaction_amount"))
        .transpose()?
        .map_or_else(
            || parse_positive_amount(&policy.max_transaction_amount, "max_transaction_amount"),
            Ok,
        )
}

fn verify_bearer_policy_signature(
    issuer: &OfflineIssuerRuntime,
    policy: &OfflineBearerPolicyBundleV2,
) -> Result<(), &'static str> {
    let signing_bytes = policy
        .signing_bytes()
        .map_err(|_| "policy_payload_invalid")?;
    verify_bearer_issuer_signature(
        issuer,
        &policy.signature_algorithm,
        &signing_bytes,
        &policy.issuer_signature,
    )
}

fn verify_bearer_issuer_signature(
    issuer: &OfflineIssuerRuntime,
    signature_algorithm: &str,
    payload: &[u8],
    signature: &[u8],
) -> Result<(), &'static str> {
    let (algorithm, public_key) = issuer.key_pair.public_key().to_bytes();
    match signature_algorithm {
        OFFLINE_BEARER_SIGNATURE_ALGORITHM_ED25519 if algorithm == Algorithm::Ed25519 => {
            verify_ed25519_signature(public_key, payload, signature)
        }
        OFFLINE_BEARER_SIGNATURE_ALGORITHM_ECDSA_P256_SHA256 => {
            verify_p256_signature(public_key, payload, signature)
        }
        _ => Err("unsupported_signature_algorithm"),
    }
}

fn verify_bearer_device_signature(
    certificate: &OfflineBearerCertificateV2,
    signature_algorithm: &str,
    payload: &[u8],
    signature: &[u8],
) -> Result<(), &'static str> {
    if signature_algorithm != certificate.signature_algorithm {
        return Err("signature_algorithm_mismatch");
    }
    match signature_algorithm {
        OFFLINE_BEARER_SIGNATURE_ALGORITHM_ED25519 => {
            verify_ed25519_signature(&certificate.public_key, payload, signature)
        }
        OFFLINE_BEARER_SIGNATURE_ALGORITHM_ECDSA_P256_SHA256 => {
            verify_p256_signature(&certificate.public_key, payload, signature)
        }
        _ => Err("unsupported_signature_algorithm"),
    }
}
...
fn build_bearer_policy_bundle(
    issuer: &OfflineIssuerRuntime,
    now_ms: u64,
) -> Result<OfflineBearerPolicyBundleV2, Error> {
    let signature_algorithm = issuer_bearer_signature_algorithm(issuer)?;
    let expires_at = now_ms.saturating_add(duration_ms(issuer.authorization_ttl));
    let max_token_age_ms = duration_ms(issuer.authorization_ttl);
    let policy_hash_hex = bearer_policy_hash_hex(issuer, now_ms, expires_at)?;
    let mut policy = OfflineBearerPolicyBundleV2 {
        policy_id: "offline-bearer-v2-default".to_owned(),
        policy_hash_hex,
        issuer_id: issuer.authority.to_string(),
        issued_at_ms: now_ms,
        expires_at_ms: expires_at,
        max_certificate_age_ms: duration_ms(issuer.certificate_ttl),
        max_policy_age_ms: duration_ms(issuer.authorization_refresh).max(60_000),
        max_token_age_ms,
        max_offline_balance: issuer.max_balance.to_string(),
        max_transaction_amount: issuer.max_tx_value.to_string(),
        allowed_hardware_classes: vec![
            "secure-element".to_owned(),
            "strongbox".to_owned(),
            "secure-enclave".to_owned(),
        ],
        blacklisted_account_ids: issuer.blacklisted_account_ids.clone(),
        blacklisted_device_ids: issuer.revoked_verdict_ids.clone(),
        blacklisted_key_ids: issuer.revoked_verdict_ids.clone(),
        signature_algorithm: signature_algorithm.to_owned(),
        issuer_signature: vec![0],
        policy_epoch: now_ms,
        policy_source: "torii-offline-issuer".to_owned(),
        revoked_certificate_ids: issuer.revoked_verdict_ids.clone(),
        revoked_transfer_ids: Vec::new(),
        asset_send_limits: issuer
            .asset_send_limits
            .iter()
            .map(|limit| OfflineBearerAssetSendLimitV2 {
                asset_definition_id: limit.asset_definition_id.clone(),
                max_transaction_amount: issuer.max_tx_value.to_string(),
                daily_send_limit: limit.daily_send_limit.to_string(),
                monthly_send_limit: limit.monthly_send_limit.to_string(),
            })
            .collect(),
    };
    let signing_bytes = policy
        .signing_bytes()
        .map_err(|source| Error::SerializationFailure {
            context: "offline_bearer_policy_payload",
            source: source.into(),
        })?;
    policy.issuer_signature = issuer.sign_bytes(&signing_bytes).payload().to_vec();
    Ok(policy)
}

fn build_bearer_certificate(
    issuer: &OfflineIssuerRuntime,
    request: &ParsedOfflineRequest,
    attestation: &VerifiedDeviceAttestation,
    policy: &OfflineBearerPolicyBundleV2,
    chain_id: &str,
    now_ms: u64,
) -> Result<OfflineBearerCertificateV2, Error> {
    let signature_algorithm = bearer_device_signature_algorithm(attestation)?;
    let public_key_encoding = bearer_device_public_key_encoding(signature_algorithm);
    let purse_id = offline_identifier(
        "purse",
        &format!(
            "{}:{}:{}:{}",
            chain_id, request.account_literal, request.device_id, request.offline_public_key
        ),
    );
    let mut certificate = OfflineBearerCertificateV2 {
        certificate_id: offline_identifier("bearer-cert", &format!("{purse_id}:{}", now_ms)),
        chain_id: chain_id.to_owned(),
        issuer_id: policy.issuer_id.clone(),
        purse_id,
        account_id: request.account_literal.clone(),
        asset_definition_id: request.asset_definition_literal.clone(),
        device_id: request.device_id.clone(),
        key_id: attestation.key_id.clone(),
        hardware_class: bearer_hardware_class(attestation),
        signature_algorithm: signature_algorithm.to_owned(),
        public_key_encoding: public_key_encoding.to_owned(),
        public_key: attestation.public_key.clone(),
        issued_at_ms: now_ms,
        expires_at_ms: now_ms.saturating_add(duration_ms(issuer.certificate_ttl)),
    ...
    let signing_bytes =
        certificate
            .signing_bytes()
            .map_err(|source| Error::SerializationFailure {
                context: "offline_bearer_certificate_payload",
                source: source.into(),
            })?;
    certificate.issuer_signature = issuer.sign_bytes(&signing_bytes).payload().to_vec();
    Ok(certificate)
}

fn bearer_policy_hash_hex(
    issuer: &OfflineIssuerRuntime,
    issued_at_ms: u64,
    expires_at_ms: u64,
) -> Result<String, Error> {
    let value = json_object(vec![
        ("policy_id", string_value("offline-bearer-v2-default")),
        ("issuer_id", string_value(issuer.authority.to_string())),
        ("issued_at_ms", number_value(issued_at_ms)),
        ("expires_at_ms", number_value(expires_at_ms)),
        (
            "max_certificate_age_ms",
            number_value(duration_ms(issuer.certificate_ttl)),
        ),
        (
            "max_policy_age_ms",
            number_value(duration_ms(issuer.authorization_refresh).max(60_000)),
        ),
        (
            "max_token_age_ms",
            number_value(duration_ms(issuer.authorization_ttl)),
        ),
        (
            "max_offline_balance",
            string_value(issuer.max_balance.to_string()),
        ),
        (
    ...
    sha256_json_hex(&value, "offline_bearer_policy_hash")
}

fn issuer_bearer_signature_algorithm(issuer: &OfflineIssuerRuntime) -> Result<&'static str, Error> {
    let (algorithm, _) = issuer.key_pair.public_key().to_bytes();
    match algorithm {
        Algorithm::Ed25519 => Ok(OFFLINE_BEARER_SIGNATURE_ALGORITHM_ED25519),
        _ => Err(validation(
            "OFFLINE_BEARER_ISSUER_ALGORITHM_UNSUPPORTED",
            "Offline Bearer issuer key algorithm is not supported by the current policy signer.",
        )),
    }
}

fn bearer_device_signature_algorithm(
    attestation: &VerifiedDeviceAttestation,
) -> Result<&'static str, Error> {
    match attestation.public_key.len() {
        32 => Ok(OFFLINE_BEARER_SIGNATURE_ALGORITHM_ED25519),
        65 => Ok(OFFLINE_BEARER_SIGNATURE_ALGORITHM_ECDSA_P256_SHA256),
        _ => Err(validation(
            "OFFLINE_BEARER_DEVICE_KEY_UNSUPPORTED",
            "Offline Bearer device public key length is unsupported.",
        )),
    }
}

fn bearer_device_public_key_encoding(signature_algorithm: &str) -> &'static str {
    match signature_algorithm {
        OFFLINE_BEARER_SIGNATURE_ALGORITHM_ECDSA_P256_SHA256 => "x963_uncompressed_p256",
        _ => "raw_ed25519",
    }
}

fn bearer_hardware_class(attestation: &VerifiedDeviceAttestation) -> String {
    let platform = attestation.platform.to_ascii_lowercase();
    let algorithm = attestation.assertion_key_algorithm.to_ascii_lowercase();
    if platform.contains("ios") || algorithm.contains("secure-enclave") {
        "secure-enclave".to_owned()
    } else if algorithm.contains("strongbox") || algorithm.contains("keymint") {
        "strongbox".to_owned()
    } else {
        "secure-element".to_owned()
    }
...
fn bearer_json_value<T: json::JsonSerialize>(
    value: &T,
    context: &'static str,
) -> Result<Value, Error> {
...
fn bearer_norito_base64<T: norito::NoritoSerialize>(
    value: &T,
    context: &'static str,
) -> Result<Value, Error> {
    let bytes = norito::to_bytes(value).map_err(|source| Error::SerializationFailure {
        context,
        source: json::Error::Message(source.to_string()),
    })?;
    Ok(string_value(BASE64_STANDARD.encode(bytes)))
}
...
mod tests {
    ...
    #[test]
    fn bearer_settlement_validation_accepts_structural_v2_batch() {
        let (issuer, _) = sample_issuer();
        let batch = sample_bearer_settlement_batch("4", "1");

        let result =
            validate_bearer_settlement_batch(&issuer, &batch, NOW_MS).expect("valid settlement");

        assert_eq!(result.accepted, vec![string_value("transfer-1")]);
        assert_eq!(result.accepted_settlements.len(), 1);
        assert_eq!(
            result.accepted_settlements[0].credit.transfer_id,
            result.accepted_settlements[0].debit.transfer_id
        );
    ...
    #[test]
    fn bearer_settlement_validation_rejects_debit_without_credit_receipt() {
        let (issuer, _) = sample_issuer();
        let mut batch = sample_bearer_settlement_batch("4", "1");
        let credit_receipts = batch
            .get_mut("credit_receipts")
            .and_then(Value::as_array_mut)
            .expect("credit receipts");
        credit_receipts.clear();

        let result = validate_bearer_settlement_batch(&issuer, &batch, NOW_MS)
            .expect("batch shape is valid");

        assert!(result.accepted.is_empty());
        let rejected = result.rejected.first().expect("rejected transfer");
        assert_eq!(
            rejected.get("reason").and_then(Value::as_str),
            Some("credit_receipt_required")
        );
    }

    #[test]
    fn bearer_settlement_validation_returns_duplicate_for_replayed_transfer() {
        let (issuer, _) = sample_issuer();
        let batch = sample_bearer_settlement_batch("4", "1");
        validate_bearer_settlement_batch(&issuer, &batch, NOW_MS).expect("first settlement");

        let result = validate_bearer_settlement_batch(&issuer, &batch, NOW_MS)
            .expect("replayed settlement shape is valid");

        assert!(result.accepted.is_empty());
        assert_eq!(result.duplicates, vec![string_value("transfer-1")]);
        assert!(result.rejected.is_empty());
    }

    #[test]
    fn bearer_settlement_validation_rejects_tampered_variant_after_acceptance() {
        let (issuer, _) = sample_issuer();
        let batch = sample_bearer_settlement_batch("4", "1");
        validate_bearer_settlement_batch(&issuer, &batch, NOW_MS).expect("first settlement");

        let mut tampered = batch;
        let debit_receipts = tampered
            .get_mut("debit_receipts")
            .and_then(Value::as_array_mut)
            .expect("debit receipts");
        let receipt = debit_receipts.first_mut().expect("debit receipt");
        insert_field(receipt, "sender_post_balance", string_value("2"));
        let result = validate_bearer_settlement_batch(&issuer, &tampered, NOW_MS)
            .expect("tampered settlement shape is valid");
    ...
    #[test]
    fn bearer_settlement_validation_rejects_bad_balance_transition() {
        let (issuer, _) = sample_issuer();
        let batch = sample_bearer_settlement_batch("4", "4");
        ...
        assert_eq!(
            rejected.get("reason").and_then(Value::as_str),
            Some("sender_balance_transition_invalid")
        );
    }

    #[test]
    fn bearer_settlement_validation_rejects_stale_certificate() {
        let (issuer, _) = sample_issuer();
        let stale_issued_at = NOW_MS - duration_ms(issuer.certificate_ttl) - 1;
        let batch = sample_bearer_settlement_batch_with_sender_issued_at(stale_issued_at);
        ...
        assert_eq!(
            rejected.get("reason").and_then(Value::as_str),
            Some("certificate_too_old")
        );
    }

    fn sample_bearer_settlement_batch(
        sender_pre_balance: &str,
        sender_post_balance: &str,
    ) -> Value {
        sample_bearer_settlement_batch_with_sender(
            sender_pre_balance,
            sender_post_balance,
            NOW_MS - 1_000,
        )
    ...
    fn sample_bearer_settlement_batch_with_sender_issued_at(sender_issued_at_ms: u64) -> Value {
        sample_bearer_settlement_batch_with_sender("4", "1", sender_issued_at_ms)
    }

    fn sample_bearer_settlement_batch_with_sender(
        sender_pre_balance: &str,
        sender_post_balance: &str,
        sender_issued_at_ms: u64,
    ) -> Value {
        let (issuer, _) = sample_issuer();
        let policy_hash = "00112233445566778899aabbccddeeff";
        let asset = "usd#offline";
        let policy = sample_bearer_policy(&issuer, policy_hash);
        let sender_key_pair = KeyPair::from_seed(vec![0x51; 32], Algorithm::Ed25519);
        let recipient_key_pair = KeyPair::from_seed(vec![0x52; 32], Algorithm::Ed25519);
        let sender = sample_bearer_certificate(
            &issuer,
            &sender_key_pair,
            "alice",
            "sender-purse",
        ...
        let recipient = sample_bearer_certificate(
            &issuer,
            &recipient_key_pair,
            "bob",
            "recipient-purse",
        ...
        let recipient_for_credit = recipient.clone();
        let mut debit = OfflineBearerDebitReceiptV2 {
            version: 2,
            transfer_id: "transfer-1".to_owned(),
            chain_id: "chain-1".to_owned(),
            payment_request_id: "request-1".to_owned(),
            sender_certificate: sender,
            recipient_certificate: recipient,
            asset_definition_id: asset.to_owned(),
            amount: "3".to_owned(),
            sender_pre_balance: sender_pre_balance.to_owned(),
            sender_post_balance: sender_post_balance.to_owned(),
            sender_sequence: 1,
            created_at_ms: NOW_MS,
            expires_at_ms: NOW_MS + 60_000,
            policy_hash_hex: policy_hash.to_owned(),
            receive_challenge_signature: vec![0xA5; 64],
            signature_algorithm: OFFLINE_BEARER_SIGNATURE_ALGORITHM_ED25519.to_owned(),
            debit_signature: vec![0],
        };
        debit.debit_signature = Signature::new(
            sender_key_pair.private_key(),
            &debit.signing_bytes().expect("debit payload"),
        )
        .payload()
        .to_vec();
        let mut credit = OfflineBearerCreditReceiptV2 {
            version: 2,
            transfer_id: "transfer-1".to_owned(),
            chain_id: "chain-1".to_owned(),
            recipient_certificate: recipient_for_credit,
            amount: "3".to_owned(),
            recipient_pre_balance: "0".to_owned(),
            recipient_post_balance: "3".to_owned(),
            recipient_sequence: 1,
            accepted_at_ms: NOW_MS,
            signature_algorithm: OFFLINE_BEARER_SIGNATURE_ALGORITHM_ED25519.to_owned(),
            credit_signature: vec![0],
        };
        credit.credit_signature = Signature::new(
            recipient_key_pair.private_key(),
            &credit.signing_bytes().expect("credit payload"),
        )
        .payload()
        .to_vec();
        sample_bearer_settlement_value(
            policy,
            OfflineBearerSettlementBatchV2 {
                version: 2,
                chain_id: "chain-1".to_owned(),
                purse_id: "recipient-purse".to_owned(),
                debit_receipts: vec![debit],
                credit_receipts: vec![credit],
            },
        )
    }

    fn sample_bearer_certificate(
        issuer: &OfflineIssuerRuntime,
        key_pair: &KeyPair,
        account_id: &str,
        purse_id: &str,
        ...
        let mut certificate = OfflineBearerCertificateV2 {
            certificate_id: format!("cert-{purse_id}"),
            chain_id: "chain-1".to_owned(),
            issuer_id: "offline-issuer".to_owned(),
            purse_id: purse_id.to_owned(),
            account_id: account_id.to_owned(),
            asset_definition_id: asset_definition_id.to_owned(),
            device_id: format!("device-{purse_id}"),
            key_id: format!("key-{purse_id}"),
            hardware_class: "secure-element".to_owned(),
            signature_algorithm: OFFLINE_BEARER_SIGNATURE_ALGORITHM_ED25519.to_owned(),
            public_key_encoding: "raw_ed25519".to_owned(),
        ...
        certificate.issuer_signature = Signature::new(
            issuer.key_pair.private_key(),
            &certificate.signing_bytes().expect("certificate payload"),
        )
        .payload()
        .to_vec();
        certificate
    ...
    fn sample_bearer_policy(
        ...
        let mut policy = OfflineBearerPolicyBundleV2 {
            ...
            expires_at_ms: NOW_MS + 60_000,
            max_certificate_age_ms: duration_ms(issuer.certificate_ttl),
            max_policy_age_ms: 60_000,
            max_token_age_ms: 60_000,
            max_offline_balance: issuer.max_balance.to_string(),
            max_transaction_amount: issuer.max_tx_value.to_string(),
            allowed_hardware_classes: vec!["secure-element".to_owned()],
            blacklisted_account_ids: Vec::new(),
            blacklisted_device_ids: Vec::new(),
            blacklisted_key_ids: Vec::new(),
            signature_algorithm: OFFLINE_BEARER_SIGNATURE_ALGORITHM_ED25519.to_owned(),
            issuer_signature: vec![0],
            policy_epoch: 1,
            policy_source: "test".to_owned(),
            revoked_certificate_ids: Vec::new(),
            revoked_transfer_ids: Vec::new(),
            asset_send_limits: Vec::new(),
        };
        policy.issuer_signature = Signature::new(
            issuer.key_pair.private_key(),
            &policy.signing_bytes().expect("policy payload"),
        )
        .payload()
        .to_vec();
        policy
    ...
    fn sample_bearer_settlement_value(
        policy: OfflineBearerPolicyBundleV2,
        batch: OfflineBearerSettlementBatchV2,
    ) -> Value {
        let mut value = json::to_value(&batch).expect("batch json");
        let Value::Object(map) = &mut value else {
            panic!("batch must encode to object");
        };
        map.insert(
            "policy_bundle".to_owned(),
            json::to_value(&policy).expect("policy json"),
        );
        value
    }
