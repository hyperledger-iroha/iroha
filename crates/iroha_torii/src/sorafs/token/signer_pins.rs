//! Exact independently configured public pins for stream-token issuance.
...
/// these pins, and possession of them establishes neither signer authorization nor current finality.
#[derive(Clone)]
pub struct StreamTokenSignerPinsV1 {
...
impl fmt::Debug for StreamTokenSignerPinsV1 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("StreamTokenSignerPinsV1 { public pins redacted }")
    }
}
impl StreamTokenSignerPinsV1 {
    ...
    pub fn from_config(
        ...
        let invalid = || StreamTokenIssuerError::InvalidSignerConfig;
        let token = &storage.stream_tokens;
        if !token.enabled {
            return if token.signer.is_none() {
                Ok(None)
            } else {
                Err(invalid())
            };
        }
        let signer_backend = token.signer.as_ref().ok_or_else(invalid)?;
        ...
        if chain_id.len() > 128
            || [
                ...
                &signer_backend.service_id,
                &signer_backend.administrator_id,
            ]
            .iter()
            .any(|value| value.len() > 128)
            || signer_backend.observer.runtime_handle.len() > 256
        {
            return Err(invalid());
        }
        ...
        let binding = SignerCustodyBindingV1 {
            ...
            network_id,
            runtime_handle: signer_backend.runtime_handle.clone(),
            key_handle: signer_backend.key_handle.clone(),
            service_id: signer_backend.service_id.clone(),
            administrator_id: signer_backend.administrator_id.clone(),
            ...
            public_key: strong_key(signer_backend.public_key)?,
            key_revision: signer_backend.key_revision,
            policy_revision: signer_backend.policy_revision,
            policy_digest: signer_backend.policy_digest,
        };
        ...
        let observer_config = &signer_backend.observer;
...
#[norito_schema(name = "iroha_torii::sorafs::token::signer_pins::PinsPreimageV1")]
#[derive(Encode)]
...
fn strong_key(bytes: [u8; 32]) -> Result<PublicKey, StreamTokenIssuerError> {
    ...
    if key.is_weak() {
        return Err(StreamTokenIssuerError::InvalidSignerConfig);
    }
...
fn authority(
    ...
    if !is_production_identity_v1(&value.service_id, 128)
        || !is_production_identity_v1(&value.administrator_id, 128)
