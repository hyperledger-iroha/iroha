...
use sorafs_manifest::{
    ...
    StakePointer, StreamBudgetV1, StreamTokenBodyV1, TradeEventV1, TransportHintV1,
    TransportProtocol,
...
use sorafs_orchestrator::appeals::{
    AppealClass, AppealClassConfig, AppealPricingConfig, AppealQuote, AppealQuoteInput,
    AppealUrgency,
};
...
fn appeal_pricing_status_json(config: &AppealPricingConfig) -> Value {
    json_object(vec![
        ...
        json_entry("deposit_api", Value::from("pending_runtime_escrow")),
        json_entry(
            "report_api",
            Value::from("pending_governance_dag_publisher"),
        ),
        json_entry(
...
mod advert_tests {
    ...
    use sorafs_manifest::{
        ...
        SETTLEMENT_RECEIPT_VERSION_V1, SignatureAlgorithm, StakePointer, build_reputation_snapshot,
    ...
    fn sorafs_app_state_with_governance_publish_index() -> (SharedAppState, TempDir, String) {
        ...
        index.insert("entry_count".into(), Value::from(2_u64));
        index.insert(
            "payload_kind_counts".into(),
            Value::Object(payload_kind_counts),
        );
    ...
    #[test]
    fn appeal_pricing_status_marks_mutating_flows_pending() {
        let config = baseline_appeal_pricing_config();
        ...
        assert_eq!(
            ...
            Some("pending_runtime_escrow")
        );
        assert_eq!(
            value.get("report_api").and_then(Value::as_str),
            Some("pending_governance_dag_publisher")
        );
        assert_eq!(
  ...
  "routes": [
    {
      ...
      "cache_ttl_ms": 2500
        ...
        let (plan, payload) = CarBuildPlan::from_files_with_profile(
            vec![FileEntry {
                path: vec!["app-api.json".to_owned()],
                data: app_api_manifest_bytes,
            }],
            sorafs_chunker::ChunkProfile::DEFAULT,
        )
        ...
        assert!(
            inner
                ...
                .manifest_metadata_by_digest(&manifest_digest)
                .is_err(),
            "test node should start without the requested app api CID cached locally"
        );
        let state = Arc::new(inner);

        let manifest_response = crate::app_api::handle_get_app_api_cid_manifest(
            State(state.clone()),
            Path(content_cid.clone()),
        )
        .await;
        ...
        let manifest_body = body::to_bytes(manifest_response.into_body(), usize::MAX)
            .await
            .expect("read app api manifest body");
        let manifest_value: Value =
            norito::json::from_slice(&manifest_body).expect("decode app api manifest response");
        assert_eq!(
            manifest_value.get("app_id").and_then(Value::as_str),
            Some("soraswap.trader")
        );
        assert_eq!(
            manifest_value.get("content_cid").and_then(Value::as_str),
            Some(content_cid.as_str())
        ...
        assert!(
            state
                ...
                .manifest_metadata_by_digest(&manifest_digest)
                .is_ok(),
            "app api CID miss should hydrate local storage from a remote provider",
        );

        let manifest_response =
            crate::app_api::handle_get_app_api_cid_manifest(State(state), Path(content_cid)).await;
        assert_eq!(manifest_response.status(), StatusCode::OK);
        assert_eq!(
            manifest_requests.load(Ordering::SeqCst),
            1,
