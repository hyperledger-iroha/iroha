...
pub fn report_torii_api_hit(telemetry: &crate::routing::MaybeTelemetry, endpoint: &str) {
    telemetry.with_metrics(|metrics| {
        metrics.inc_torii_api_token_hit(endpoint, "present");
    });
}
#[cfg(not(all(feature = "app_api", feature = "telemetry")))]
pub fn report_torii_api_hit(_telemetry: &crate::routing::MaybeTelemetry, _endpoint: &str) {}
