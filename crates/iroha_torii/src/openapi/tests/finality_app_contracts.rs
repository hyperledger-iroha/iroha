...
fn assert_item_ref(schema: &Map, expected: &str, context: &str) {
    assert_eq!(
        schema.get("items").and_then(Value::as_object).and_then(|items| items.get("$ref")).and_then(Value::as_str),
        Some(expected),
...
fn sumeragi_v2_da_schema_requires_reed_solomon16_without_plain_compatibility() {
    ...
    let allowed = contract_property(&schemas, "SumeragiV2PayloadEncoding", "encoding")
        .get("enum").and_then(Value::as_array).expect("payload encoding enum");
...
fn bridge_finality_v2_schemas_are_exact_closed_and_bounded() {
    ...
    assert_schema_shapes(&schemas, &[
        ...
        SchemaShape { name: "SumeragiV2HeightContext", required: "height.context.required", optional: Some("height.context.optional") },
        ...
        SchemaShape { name: "SumeragiV2BlockSubject", required: "block.subject.required", optional: Some("block.subject.optional") },
        SchemaShape { name: "SumeragiV2MergeCarrierCommitment", required: "merge.carrier.required", optional: None },
        SchemaShape { name: "SumeragiV2ExecutionCommitment", required: "execution.required", optional: Some("execution.optional") },
        ...
        SchemaShape { name: "BlockHeader", required: "block.header.required", optional: Some("block.header.optional") },
    ]);
    ...
    assert_property_refs(&schemas, &[
        PropertyRefContract { owner: "SumeragiV2ValidatorPower", property: "validator", expected: "#/components/schemas/SumeragiV2BlsValidatorId" },
        PropertyRefContract { owner: "SumeragiV2MergeCarrierCommitment", property: "entry_hash", expected: "#/components/schemas/Hash" },
        PropertyRefContract { owner: "SumeragiV2FinalityArtifact", property: "commit_qc", expected: "#/components/schemas/SumeragiV2CommitQuorumCertificate" },
        PropertyRefContract { owner: "SumeragiV2QuorumCertificate", property: "execution_commitment", expected: "#/components/schemas/SumeragiV2ExecutionCommitment" },
        PropertyRefContract { owner: "SumeragiV2CommitQuorumCertificate", property: "execution_commitment", expected: "#/components/schemas/SumeragiV2ExecutionCommitment" },
    ]);
...
fn bridge_finality_schema_matches_norito_json_and_decoder_rejects_v1_fields() {
    ...
    for absent in ["next_epoch_snapshot", "parent_commit_qc", "snapshot_bootstrap"] { assert!(!context.contains_key(absent)); }
    ...
    let removed = missing.as_object_mut().and_then(|proof| proof.get_mut("finality_artifact"))
        .and_then(Value::as_object_mut).and_then(|artifact| artifact.get_mut("commit_qc"))
        .and_then(Value::as_object_mut).expect("mutable CommitQC").remove("execution_commitment");
...
fn bridge_finality_operations_describe_durable_v2_evidence() {
    let document = generate_spec();
    for contract in [
        OperationResponseContract { path: "/v1/bridge/finality/{height}", method: "get", status: "200", schema_ref: "#/components/schemas/BridgeFinalityProof" },
        OperationResponseContract { path: "/v1/bridge/finality/attestation/{height}", method: "get", status: "200", schema_ref: "#/components/schemas/BridgeFinalityAttestationV1" },
        OperationResponseContract { path: "/v1/bridge/finality/bundle/{height}", method: "get", status: "200", schema_ref: "#/components/schemas/BridgeFinalityBundle" },
    ] {
        ...
        let norito = response_content(operation, "200", contract.path).get("application/x-norito")
            .and_then(|media| media.get("schema")).and_then(Value::as_object).expect("Norito response schema");
        assert_eq!(norito.get("type").and_then(Value::as_str), Some("string"));
        assert_eq!(norito.get("format").and_then(Value::as_str), Some("binary"));
    }
...
fn generated_spec_documents_exact_authoritative_sumeragi_v2_status() {
    ...
    for (path, expected, label) in [
        ("/v1/sumeragi/status", "#/components/schemas/SumeragiStatusResponse", "authoritative status"),
        ("/v1/sumeragi/diagnostics", "#/components/schemas/SumeragiDiagnosticsResponse", "operator diagnostics"),
    ] {
        let actual = document.get("paths").and_then(Value::as_object).and_then(|paths| paths.get(path))
            .and_then(Value::as_object).and_then(|item| item.get("get")).and_then(Value::as_object)
    ...
    for present in contract_strings("status.present") { assert!(properties.contains_key(present), "status missing {present}"); }
    for absent in contract_strings("status.absent") { assert!(!properties.contains_key(absent), "status retained {absent}"); }
    ...
    for field in ["restart_required", "height_context", "liveness"] { assert!(required.iter().any(|value| value.as_str() == Some(field))); }
    assert!(!required.iter().any(|value| value.as_str() == Some("last_commit_qc")));
    assert_property_refs(schemas, &[
        PropertyRefContract { owner: "SumeragiV2HeightContextStatus", property: "mode", expected: "#/components/schemas/SumeragiV2ConsensusMode" },
        PropertyRefContract { owner: "SumeragiV2HeightContextStatus", property: "quorum", expected: "#/components/schemas/SumeragiV2DualQuorum" },
        PropertyRefContract { owner: "SumeragiV2CommitQcStatus", property: "certificate", expected: "#/components/schemas/SumeragiV2QuorumCertificateRef" },
    ]);
    ...
    for field in ["height", "view", "phase", "leader", "locked_prepare_qc"] { assert!(!diagnostics.contains_key(field)); }
    ...
    assert_property_refs(schemas, &[
        PropertyRefContract { owner: "NativeAmxLegRecord", property: "participant_proposal", expected: "#/components/schemas/NativeAmxParticipantLaneBlockProposal" },
        PropertyRefContract { owner: "NativeAmxLegRecord", property: "participant_settlement", expected: "#/components/schemas/NativeAmxParticipantSettlementCommitment" },
    ...
    for (field, minimum, maximum, unique, item) in [
        ("validator_set", 1, 128, Some(true), "#/components/schemas/SumeragiV2BlsValidatorId"),
        ("validator_set_pops", 1, 128, None, "#/components/schemas/SumeragiV2BlsProof"),
    ] {
    ...
    for value in ["prepare", "commit"] { assert!(phase_values.iter().any(|entry| entry.as_str() == Some(value))); }
    assert_eq!(phase.get("detail").and_then(|detail| detail.get("type")).and_then(Value::as_str), Some("null"));
    for (name, tag, expected) in [
        ("LaneLiquidityProfile", "profile", &["Tier1", "Tier2", "Tier3"][..]),
        ("LaneVolatilityClass", "bucket", &["Stable", "Elevated", "Dislocated"]),
    ] {
        let values = contract_property(schemas, name, tag).get("enum").and_then(Value::as_array).expect("tag enum");
        for expected in expected { assert!(values.iter().any(|value| value.as_str() == Some(*expected))); }
    }
    assert_property_refs(schemas, &[
        PropertyRefContract { owner: "LaneSwapMetadata", property: "liquidity_profile", expected: "#/components/schemas/LaneLiquidityProfile" },
        PropertyRefContract { owner: "LaneSwapMetadata", property: "volatility_class", expected: "#/components/schemas/LaneVolatilityClass" },
    ]);
    for field in ["total_local_amount", "total_xor_due", "total_xor_after_haircut", "total_xor_variance"] {
    ...
    for retired in ["total_local_micro", "total_xor_due_micro", "total_xor_after_haircut_micro", "total_xor_variance_micro"] { assert!(!settlement_properties.contains_key(retired)); }
    ...
    for (owner, field) in [
        ("NexusFeeReceipt", "lane_id"), ("NativeAmxAttestationBody", "coordinator_lane_id"),
        ("NativeAmxAttestationBody", "participant_lane_id"), ("NativeAmxLegRecord", "lane_id"),
        ("NativeAmxReceipt", "lane_id"), ("LaneSettlementCommitment", "lane_id"), ("LaneRelayEnvelope", "lane_id"),
    ] {
...
fn generated_spec_documents_soracloud_private_uploaded_model_routes() {
    let document = generate_spec();
    let paths = document.get("paths").and_then(Value::as_object).expect("paths");
    for path in ["/v1/soracloud/status", "/v1/soracloud/hf/deploy", "/v1/soracloud/model/upload/private/execute", "/v1/soracloud/model/upload/private/receipts"] { assert!(paths.contains_key(path)); }
    let status = openapi_operation(&document, "/v1/soracloud/status", "get").get("description").and_then(Value::as_str).expect("Soracloud status description");
    for phrase in ["configured_lane_count", "declared_lane_count", "active lane ids/count", "autoscale-capacity lane ids/count"] { assert!(status.contains(phrase)); }
    let deploy = openapi_operation(&document, "/v1/soracloud/hf/deploy", "post");
    assert_eq!(deploy.get(SORACLOUD_HF_DEPLOY_CONTRACT_EXTENSION).and_then(Value::as_str), Some(SORACLOUD_HF_DEPLOY_CONTRACT_V1));
    assert_eq!(operation_response_schema_ref(deploy, "200", "/v1/soracloud/hf/deploy"), "#/components/schemas/SoracloudHfDeployDraftV1");
    ...
    let schemas = component_schemas(&document);
    assert_eq!(contract_schema(schemas, "SoracloudHfDeployDraftV1").get(SORACLOUD_HF_DEPLOY_CONTRACT_EXTENSION).and_then(Value::as_str), Some(SORACLOUD_HF_DEPLOY_CONTRACT_V1));
    assert_array_bounds(contract_property(schemas, "SoracloudHfDeployDraftV1", "tx_instructions"), 1, 3, None);
    assert_eq!(contract_property(schemas, "SoracloudTxInstruction", "payload_hex").get("pattern").and_then(Value::as_str), Some("^(?:[0-9a-f]{2})+$"));
    let properties = contract_schema(schemas, "PrivateUploadedModelReceiptListResponse").get("properties").and_then(Value::as_object).expect("receipt-list properties");
    for key in contract_strings("private.receipt.metadata") { assert!(properties.contains_key(key), "metadata missing {key}"); }
}
...
fn generated_spec_documents_app_query_page_metadata() {
    ...
    assert_operation_response_contracts(&document, &[
        ...
        OperationResponseContract { path: "/v1/accounts/query", method: "post", status: "200", schema_ref: "#/components/schemas/AccountQueryResponse" },
        OperationResponseContract { path: "/v1/domains", method: "get", status: "200", schema_ref: "#/components/schemas/DomainListResponse" },
        OperationResponseContract { path: "/v1/domains/query", method: "post", status: "200", schema_ref: "#/components/schemas/DomainQueryResponse" },
        OperationResponseContract { path: "/v1/accounts/{account_id}/assets", method: "get", status: "200", schema_ref: "#/components/schemas/AccountAssetListResponse" },
        OperationResponseContract { path: "/v1/accounts/{account_id}/assets/query", method: "post", status: "200", schema_ref: "#/components/schemas/AccountAssetQueryResponse" },
        OperationResponseContract { path: "/v1/assets/definitions", method: "get", status: "200", schema_ref: "#/components/schemas/AssetDefinitionListResponse" },
        OperationResponseContract { path: "/v1/assets/definitions/query", method: "post", status: "200", schema_ref: "#/components/schemas/AssetDefinitionQueryResponse" },
        OperationResponseContract { path: "/v1/assets/{definition_id}/holders", method: "get", status: "200", schema_ref: "#/components/schemas/AssetHolderListResponse" },
        OperationResponseContract { path: "/v1/assets/{definition_id}/holders/query", method: "post", status: "200", schema_ref: "#/components/schemas/AssetHolderQueryResponse" },
        ...
        OperationResponseContract { path: "/v1/repo/agreements", method: "get", status: "200", schema_ref: "#/components/schemas/RepoAgreementListResponse" },
        OperationResponseContract { path: "/v1/repo/agreements/query", method: "post", status: "200", schema_ref: "#/components/schemas/RepoAgreementListResponse" },
    ]);
    let schemas = component_schemas(&document);
    ...
    for field in contract_strings("app.page.properties") { assert!(properties.contains_key(field), "metadata missing {field}"); }
    ...
    for field in contract_strings("repo.query.fields") { assert!(query.contains_key(field)); }
}
...
fn alias_openapi_documents_optional_public_and_exact_restricted_auth() {
    ...
    for (path, reject, forbidden) in [
        ("/v1/retail/recipients/lookup", "recipient_lookup_signature_required", true),
        ("/v1/retail/recipients/route", "recipient_lookup_signature_required", true),
        ("/v1/fee-sponsor-programs/by-id", "fee_sponsor_program_signature_required", false),
...
fn protected_contract_identity_openapi_is_signed_and_exact() {
    ...
    assert_schema_shapes(schemas, &[
        ...
        SchemaShape { name: "ContractAliasBinding", required: "contract.alias.binding.required", optional: Some("contract.alias.binding.optional") },
...
fn multisig_read_auth_contract_is_path_specific() {
    ...
    for path in ["/v1/multisig/spec", "/v1/multisig/proposals/query", "/v1/multisig/proposals/resolve"] {
        ...
        for phrase in ["Both canonical `multisig_account_id`", "`multisig_account_alias` selectors require", "body fields never establish signer identity"] { assert!(description.contains(phrase)); }
    }
