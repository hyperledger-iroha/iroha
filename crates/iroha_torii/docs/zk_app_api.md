...
- Proving keys: for `halo2/ipa`, the IVM prove helper (`/v1/zk/ivm/prove`) loads proving key bytes from the same directory using `<backend>__<name>.pk` naming.
  The `.pk` file is a Norito archive containing the Halo2 `SerdeFormat::Processed` proving key plus the canonical circuit family and verifier-key commitment, and must be generated by `iroha app zk ivm derive-pk`.
