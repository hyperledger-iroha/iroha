...
#[tokio::test]
async fn contracts_redeploy_same_alias_reclaims_stale_inactive_binding() {
    if std::env::var("IROHA_RUN_IGNORED").ok().as_deref() != Some("1") {
        eprintln!(
            "Skipping: contracts deploy integration test gated. Set IROHA_RUN_IGNORED=1 to run."
        );
...

    {
        ...
        tx.world.clear_contract_alias(&first_address);
        tx.world
            .bind_contract_alias(&stale_address, alias.clone(), None, None, 10_000)
            .expect("bind alias to stale undeployed address");
        tx.apply();
        block.commit().expect("commit stale binding simulation");
    }
