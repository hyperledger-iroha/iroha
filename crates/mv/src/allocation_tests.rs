...
fn partition_retains_exact_original_pool_and_conserves_real_credits() {
    ...
    let mut part = without_allocations(|| whole.try_partition(24).unwrap());
    ...
    let error = without_allocations(|| whole.try_partition(41).unwrap_err());
    assert_eq!(
        error,
        InsufficientReservation {
            requested_bytes: 41,
    ...
    let wakes = Arc::new(WakeCount::default());
    assert!(poll(&mut wait, &wakes).is_pending());
    let zero = without_allocations(|| whole.try_partition(0).unwrap());
