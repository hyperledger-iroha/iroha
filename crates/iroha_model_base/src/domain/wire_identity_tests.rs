//! The original domain frames, schema, JSON and storage-key records at their base owner.
...
#[test]
fn domain_owner_matches_all_pre_extraction_envelopes_and_storage_keys() {
    ...
    let expected: Vec<_> = captured
        ...
        .filter(|record| {
            let case = record
                ...
                .expect("fixture case");
            case == "domain" || case.starts_with("domain/")
        })
        .collect();
    assert_eq!(expected.len(), 7, "every captured domain envelope");
