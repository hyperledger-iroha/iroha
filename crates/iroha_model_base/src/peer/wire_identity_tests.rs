//! Complete original peer identity frames, schemas, JSON and signing transcripts.
...
#[test]
fn peer_owner_matches_all_pre_extraction_envelopes() {
    ...
    let expected: Vec<_> = captured
        ...
        .filter(|record| {
            let case = record
                ...
                .expect("fixture case");
            case == "peer" || case.starts_with("peer/")
        })
        .collect();
    assert_eq!(expected.len(), 7, "every captured peer envelope");
