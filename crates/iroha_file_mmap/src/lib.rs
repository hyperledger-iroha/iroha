...
mod tests {
    ...
    #[cfg(windows)]
    #[test]
    fn windows_information_is_stable_and_tracks_hard_links() {
        ...
        let link = file.path().with_extension("hard-link");
        std::fs::hard_link(file.path(), &link).expect("create hard link");
        let linked = windows_file_information(file.as_file()).expect("sample linked file");
        assert_eq!(linked.number_of_links(), 2);
