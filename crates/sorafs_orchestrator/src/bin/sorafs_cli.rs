...
fn manifest_submit(raw_args: Vec<String>) -> Result<(), String> {
    ...
    let (
        ...
        chain_id_hint,
        failure_message,
    ) = if gas_asset_id.as_ref().map(|value| !value.trim().is_empty()).unwrap_or(false) {
    ...
    if let Some(asset_id) = gas_asset_id.as_ref().map(|value| value.trim()).filter(|value| !value.is_empty()) {
