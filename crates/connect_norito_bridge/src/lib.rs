...
unsafe fn read_kagemusha_archive_bytes(ptr: *const c_uchar, len: c_ulong) -> BridgeResult<Vec<u8>> {
    if ptr.is_null() {
        return Err(BridgeError::NullPtr);
    }
    let len: usize = len.try_into().map_err(|_| BridgeError::KagemushaProve)?;
    if kagemusha_archive_out_of_bounds(len) {
...
fn ensure_kagemusha_recursive_spend_pallas_archive(archive: &[u8]) -> BridgeResult<()> {
    if kagemusha_archive_out_of_bounds(archive.len()) {
        return Err(BridgeError::KagemushaProve);
    }
    Ok(())
}
...
const PRIVACY_FFI_ERROR_NULL_POINTER: u32 = 1;
...
fn privacy_result_for_request_archive(
    ...
    if privacy_request_archive_out_of_bounds(request_bytes.len()) {
        return privacy_failure_result(
    ...
    let request = privacy_decode_public_request_archive(request_bytes);
    match request {
...
fn privacy_decode_public_request_archive(
    ...
    if !privacy_archive_has_repeated_schema_byte(request_bytes, PRIVACY_REQUEST_SCHEMA_BYTE) {
        return Err(());
    }
    let mut normalized = request_bytes.to_vec();
...
unsafe fn read_privacy_request(
    ...
    let request_len =
        usize::try_from(request_len).map_err(|_| PRIVACY_FFI_ERROR_MALFORMED_NORITO)?;
    if privacy_request_archive_out_of_bounds(request_len) {
        return Err(PRIVACY_FFI_ERROR_MALFORMED_NORITO);
    }
    if request_len > isize::MAX as usize {
        return Err(PRIVACY_FFI_ERROR_MALFORMED_NORITO);
    }
    let bytes = unsafe { slice::from_raw_parts(request_ptr, request_len) };
    privacy_decode_public_request_archive(bytes).map_err(|_| PRIVACY_FFI_ERROR_MALFORMED_NORITO)
}
...
unsafe fn iroha_privacy_process_request_v1(
    ...
    let result = match unsafe { read_privacy_request(request_ptr, request_len) } {
        Ok(request) => privacy_result_for_request(request, operation),
        Err(PRIVACY_FFI_ERROR_NULL_POINTER) => privacy_failure_result(
            PRIVACY_FFI_ERROR_NULL_POINTER,
            "null privacy proof request pointer",
            None,
        ),
        Err(PRIVACY_FFI_ERROR_MALFORMED_NORITO) => privacy_failure_result(
            PRIVACY_FFI_ERROR_MALFORMED_NORITO,
            "malformed Norito V1 privacy proof request",
            None,
        ),
        Err(code) => privacy_failure_result(code, "privacy proof request failed", None),
    };
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn iroha_privacy_build_proof_v1(
    request_ptr: *const c_uchar,
    request_len: c_ulong,
    out_ptr: *mut *mut c_uchar,
    out_len: *mut c_ulong,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn iroha_privacy_verify_proof_v1(
    request_ptr: *const c_uchar,
    request_len: c_ulong,
    out_ptr: *mut *mut c_uchar,
    out_len: *mut c_ulong,
...
pub unsafe extern "C" fn connect_norito_kagemusha_prove_verified_compact_payment_token(
    ...
    let result = (|| {
        clear_bridge_output_or_null(out_compact_token_ptr, out_compact_token_len)?;
        let bytes = unsafe {
            read_kagemusha_archive_bytes(verified_bundle_norito_ptr, verified_bundle_norito_len)
        }?;
...
pub unsafe extern "C" fn connect_norito_kagemusha_prove_verified_compact_payment_token_with_records(
    ...
    let result = (|| {
        ...
        unsafe {
            write_kagemusha_archive_bridge(out_compact_token_ptr, out_compact_token_len, &archive)
        }
    })();
...
/// Pallas open-envelope archive. The output is Norito-archive bytes of
...
pub unsafe extern "C" fn connect_norito_kagemusha_prove_verified_recursive_aggregation_proof_bundle_with_records_and_pallas_open_envelopes(
    ...
    let result = (|| {
        clear_bridge_output_or_null(out_proof_bundle_ptr, out_proof_bundle_len)?;
        ...
        unsafe {
            write_kagemusha_archive_bridge(out_proof_bundle_ptr, out_proof_bundle_len, &archive)
        }
    })();
...
pub unsafe extern "C" fn connect_norito_kagemusha_prove_verified_recursive_compact_payment_token_with_records_and_pallas_open_envelopes(
    ...
    let result = (|| {
        clear_bridge_output_or_null(out_compact_token_ptr, out_compact_token_len)?;
        let record_bundle_archive = unsafe {
            read_kagemusha_archive_bytes(
                verified_record_bundle_norito_ptr,
                verified_record_bundle_norito_len,
            )
        }?;
        let pallas_open_envelopes_archive = unsafe {
            read_kagemusha_archive_bytes(
                pallas_open_envelopes_norito_ptr,
                pallas_open_envelopes_norito_len,
            )
...
fn verify_kagemusha_recursive_spend_compact_payment_token_projection_archives(
    ...
    let token: iroha_data_model::offline::KagemushaCompactPaymentToken =
        norito::decode_from_bytes(compact_token_archive)
            .map_err(|_| BridgeError::KagemushaProve)?;
    let record: iroha_data_model::proof::VerifyingKeyRecord =
        norito::decode_from_bytes(verifier_record_archive)
            .map_err(|_| BridgeError::KagemushaProve)?;
    match block_height {
    ...
    .map_err(|_| BridgeError::KagemushaProve)?;
    let verified = match block_height {
...
///
/// This verifier accepts only Reserved-lineage projected compact tokens. Malformed
/// archives, non-lineage circuit ids, stale verifier records, and binding drift
/// return [`ERR_KAGEMUSHA_PROVE`]. Shape-valid proofs that fail backend
/// verification return success with `*out_valid = 0`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn connect_norito_kagemusha_verify_recursive_spend_compact_payment_token_projection(
    compact_token_norito_ptr: *const c_uchar,
    compact_token_norito_len: c_ulong,
    verifier_record_norito_ptr: *const c_uchar,
    verifier_record_norito_len: c_ulong,
    out_valid: *mut c_uchar,
) -> c_int {
    let result = (|| {
        if !out_valid.is_null() {
        ...
        let verifier_record_archive = unsafe {
            read_kagemusha_archive_bytes(verifier_record_norito_ptr, verifier_record_norito_len)
        }?;
        if verify_kagemusha_recursive_spend_compact_payment_token_projection_archives(
...
/// Verify a projected recursive spend compact-token archive against a lineage verifier record at `block_height`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn connect_norito_kagemusha_verify_recursive_spend_compact_payment_token_projection_at_height(
    compact_token_norito_ptr: *const c_uchar,
    compact_token_norito_len: c_ulong,
    verifier_record_norito_ptr: *const c_uchar,
    verifier_record_norito_len: c_ulong,
...
///
/// Input is a Norito-encoded `KagemushaRecursiveSpendBundleV1`. Output is a
/// Norito-encoded `KagemushaCompactPaymentToken` that preserves the bundle's
/// recursive proof bytes while binding them to the folded compact public
/// inputs.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn connect_norito_kagemusha_recursive_spend_compact_payment_token_from_bundle(
    ...
    let result = (|| {
        clear_bridge_output_or_null(out_compact_token_ptr, out_compact_token_len)?;
        let bundle_archive =
            unsafe { read_kagemusha_archive_bytes(bundle_norito_ptr, bundle_norito_len) }?;
...
pub unsafe extern "C" fn connect_norito_kagemusha_recursive_spend_init(
    ...
    let result = (|| {
        clear_bridge_output_or_null(out_bundle_ptr, out_bundle_len)?;
        let bytes =
        ...
        unsafe { write_kagemusha_archive_bridge(out_bundle_ptr, out_bundle_len, &archive) }
    })();
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn connect_norito_kagemusha_recursive_spend_transition_profile_init(
    request_norito_ptr: *const c_uchar,
    request_norito_len: c_ulong,
    out_profile_ptr: *mut *mut c_uchar,
    out_profile_len: *mut c_ulong,
) -> c_int {
    let result = (|| {
        clear_bridge_output_or_null(out_profile_ptr, out_profile_len)?;
        let bytes =
            unsafe { read_kagemusha_archive_bytes(request_norito_ptr, request_norito_len) }?;
        let profile =
            kagemusha_recursive_spend_transition_profile_init_from_request_archive(&bytes)?;
        let archive = norito::to_bytes(&profile).map_err(|_| BridgeError::KagemushaProve)?;
        unsafe { write_kagemusha_archive_bridge(out_profile_ptr, out_profile_len, &archive) }
    })();

    bridge_result_to_code(result)
}

fn kagemusha_recursive_spend_transition_profile_init_from_request_archive(
    request_archive: &[u8],
) -> BridgeResult<iroha_data_model::offline::KagemushaRecursiveSpendTransitionProfileV1> {
    use iroha_core::zk::{
...
/// Build the canonical Reserved-lineage transition profile for an append request.
///
/// Input is Norito archive bytes of
/// `iroha_data_model::offline::KagemushaRecursiveSpendAppendRequestV1`.
/// Output is Norito archive bytes of
/// `KagemushaRecursiveSpendTransitionProfileV1`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn connect_norito_kagemusha_recursive_spend_transition_profile_append(
    request_norito_ptr: *const c_uchar,
    request_norito_len: c_ulong,
    out_profile_ptr: *mut *mut c_uchar,
    out_profile_len: *mut c_ulong,
    ...
    let result = (|| {
        ...
        let bytes =
            unsafe { read_kagemusha_archive_bytes(request_norito_ptr, request_norito_len) }?;
        let profile =
            kagemusha_recursive_spend_transition_profile_append_from_request_archive(&bytes)?;
        let archive = norito::to_bytes(&profile).map_err(|_| BridgeError::KagemushaProve)?;
        unsafe { write_kagemusha_archive_bridge(out_profile_ptr, out_profile_len, &archive) }
    })();
...
fn kagemusha_recursive_spend_transition_profile_append_from_request_archive(
    request_archive: &[u8],
) -> BridgeResult<iroha_data_model::offline::KagemushaRecursiveSpendTransitionProfileV1> {
    use iroha_core::zk::{
        ...
        .is_empty()
    {
        ...
        let hop = request
            ...
            .first()
            .ok_or(BridgeError::KagemushaProve)?;
        let current_hop_proof_hash = kagemusha_fold_step_proof_hash(&hop.attachment.proof)
            .map_err(|_| BridgeError::KagemushaProve)?;
...
///
/// Input is Norito archive bytes of
/// `KagemushaRecursiveSpendTransitionProfileV1`. Output is Norito archive bytes
/// of `KagemushaRecursiveSpendLineageAppendBoundaryV1`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn connect_norito_kagemusha_recursive_spend_lineage_append_boundary(
    profile_norito_ptr: *const c_uchar,
    profile_norito_len: c_ulong,
    out_boundary_ptr: *mut *mut c_uchar,
    out_boundary_len: *mut c_ulong,
) -> c_int {
    let result = (|| {
        clear_bridge_output_or_null(out_boundary_ptr, out_boundary_len)?;
        let bytes =
            unsafe { read_kagemusha_archive_bytes(profile_norito_ptr, profile_norito_len) }?;
        let boundary =
            kagemusha_recursive_spend_lineage_append_boundary_from_transition_profile_archive(
                &bytes,
            )?;
        let archive = norito::to_bytes(&boundary).map_err(|_| BridgeError::KagemushaProve)?;
        unsafe { write_kagemusha_archive_bridge(out_boundary_ptr, out_boundary_len, &archive) }
    })();

    bridge_result_to_code(result)
}

fn kagemusha_recursive_spend_lineage_append_boundary_from_transition_profile_archive(
    profile_archive: &[u8],
    ...
    let profile: KagemushaRecursiveSpendTransitionProfileV1 =
        norito::decode_from_bytes(profile_archive).map_err(|_| BridgeError::KagemushaProve)?;
    let boundary =
...
pub unsafe extern "C" fn connect_norito_kagemusha_recursive_spend_lineage_witness_from_init_result(
    ...
    let result = (|| {
        clear_bridge_output_or_null(out_witness_ptr, out_witness_len)?;
        let request_bytes =
            unsafe { read_kagemusha_archive_bytes(request_norito_ptr, request_norito_len) }?;
        let bundle_bytes =
            unsafe { read_kagemusha_archive_bytes(bundle_norito_ptr, bundle_norito_len) }?;
        ...
        unsafe { write_kagemusha_archive_bridge(out_witness_ptr, out_witness_len, &archive) }
    })();
...
pub unsafe extern "C" fn connect_norito_kagemusha_recursive_spend_lineage_witness_append_result(
    ...
    let result = (|| {
        ...
        let previous_witness_bytes = unsafe {
            read_kagemusha_archive_bytes(previous_witness_norito_ptr, previous_witness_norito_len)
        }?;
        let request_bytes =
            unsafe { read_kagemusha_archive_bytes(request_norito_ptr, request_norito_len) }?;
        let bundle_bytes =
...
fn kagemusha_recursive_spend_verify_from_request_archive(
    ...
    use iroha_core::zk::{
        kagemusha_recursive_spend_verify_result_with_lineage_record,
        kagemusha_recursive_spend_verify_result_with_lineage_record_at_height,
    };
    ...
    match request.block_height {
        ...
        None => kagemusha_recursive_spend_verify_result_with_lineage_record(
            &request.bundle,
            request.lineage_verifier_record.as_ref(),
        ),
    }
...
fn kagemusha_recursive_spend_redeem_from_request_archive(
    ...
    use iroha_core::zk::{
        ...
        verify_kagemusha_recursive_spend_bundle_with_record_at_height,
        verify_kagemusha_recursive_spend_lineage_witness_and_bundle_with_record_resolver,
    ...
    if let Some(lineage_witness) = &request.lineage_witness {
        match request.bundle.recursive_proof.verifier_key_id.name.as_str() {
            KAGEMUSHA_RECURSIVE_AGGREGATION_CIRCUIT_ID => {
                ...
                if let Some(record) = request.lineage_verifier_record.as_ref() {
                    ...
                    .map_err(|_| BridgeError::KagemushaProve)?;
                    if !verify_kagemusha_recursive_spend_bundle(&request.bundle, &vk_box) {
                        return Err(BridgeError::KagemushaProve);
                    }
                } else {
                    verify_kagemusha_recursive_spend_lineage_witness_and_bundle_with_vk_box(
        ...
        if iroha_data_model::offline::is_kagemusha_recursive_spend_lineage_proof_circuit_id(
            ...
            match request.block_height {
                ...
                None => {
                    preverify_kagemusha_recursive_spend_bundle_with_record(&request.bundle, record)
                }
            }
            .map_err(|_| BridgeError::KagemushaProve)?;
            let verified = match request.block_height {
                ...
                None => {
                    verify_kagemusha_recursive_spend_bundle_with_record(&request.bundle, record)
                }
            };
            if !verified {
                return Err(BridgeError::KagemushaProve);
...
mod offline_note_prover_tests {
    ...
    fn append_zk1_raw_instance_columns(buf: &mut Vec<u8>, columns: Vec<Vec<[u8; 32]>>) {
        ...
        let mut payload = Vec::with_capacity(8 + columns.len() * rows * Hash::LENGTH);
    ...
    fn sample_kagemusha_recursive_spend_bundle() -> KagemushaRecursiveSpendBundleV1 {
        ...
        let accumulator = KagemushaRecursiveSpendAccumulatorV1 {
            ...
            transition_profile_binding_digest: fixed_bytes(b"bridge-recursive-transition-binding"),
        ...
        FIXTURE
            .get_or_init(|| {
                ...
                let evidence =
                    ...
                    .expect("derive bridge recursive spend init evidence");
                let accumulator =
                    kagemusha_recursive_spend_accumulator_from_initial_evidence(
                        &evidence,
                        &current_note,
                    )
                    .expect("bridge recursive spend init accumulator");
                let public_inputs =
                    kagemusha_recursive_spend_public_inputs_from_accumulator(&accumulator)
                        .expect("bridge recursive spend init public inputs");
                let public_inputs_hash = public_inputs
                    .public_inputs_hash()
                    .expect("bridge recursive spend init public-input hash");
                let bundle = KagemushaRecursiveSpendBundleV1 {
                    accumulator,
                    recursive_proof: KagemushaRecursiveAggregationProof {
                        verifier_key_id: VerifyingKeyId::new(
                        ...
                        public_inputs,
                        public_inputs_hash,
                        proof: ProofBox::new(ZK_BACKEND_HALO2_IPA.to_owned(), vec![0xB5; 64]),
                    },
                };
                (
                    bundle,
    ...
    fn sample_recursive_spend_lineage_verifier_record() -> VerifyingKeyRecord {
    ...
    fn sample_fast_recursive_spend_lineage_witness_for_bundle(
        bundle: &KagemushaRecursiveSpendBundleV1,
    ) -> KagemushaRecursiveSpendLineageWitnessV1 {
        let vk_id = VerifyingKeyId::new(ZK_BACKEND_HALO2_IPA, "bridge-fast-lineage-hop");
        let verifier_key = VerifyingKeyBox::new(ZK_BACKEND_HALO2_IPA.to_owned(), vec![0xC9; 32]);
        let vk_commitment = hash_vk(&verifier_key);
        let proof_schema = b"bridge-fast-lineage-hop-public-inputs-v1".to_vec();
        ...
        let mut record = VerifyingKeyRecord::new_with_owner(
            ...
            Some("bridge-fast-lineage-hop".to_owned()),
            iroha_core::zk::KAGEMUSHA_VERIFIER_NAMESPACE,
            BackendTag::Halo2IpaPasta,
            "pallas",
            proof_schema_hash,
        ...
        let intermediate_root = fixed_bytes(b"bridge-fast-lineage-intermediate-root");
        let note0 = KagemushaSpendableNoteDescriptorV1 {
            note_commitment: fixed_bytes(b"bridge-fast-lineage-note-0"),
            spend_nullifier: fixed_bytes(b"bridge-fast-lineage-nullifier-0"),
            amount: bundle.accumulator.current_note.amount.clone(),
        };
        let step0 = KagemushaVerifiedFoldStep {
            root_before: bundle.accumulator.initial_root,
            input_nullifiers: bundle.accumulator.topup_anchor_nullifiers.clone(),
            output_commitments: vec![note0.note_commitment],
                ...
                let mut attachment = ProofAttachment::new_ref(
                    ...
                    ProofBox::new(
                        ...
                        norito::to_bytes(&proof_envelope)
                            .expect("encode first fast lineage hop proof"),
                    ),
                    vk_id.clone(),
                );
        ...
        let step1 = KagemushaVerifiedFoldStep {
            ...
            input_nullifiers: vec![note0.spend_nullifier],
            ...
            attachment: {
                ...
                let mut attachment = ProofAttachment::new_ref(
                    ...
                    ProofBox::new(
                        ZK_BACKEND_HALO2_IPA.to_owned(),
                        norito::to_bytes(&proof_envelope)
                            .expect("encode second fast lineage hop proof"),
                    ),
                    vk_id.clone(),
                );
                attachment.vk_commitment = Some(vk_commitment);
        ...
        let previous_accumulator = KagemushaRecursiveSpendAccumulatorV1 {
            domain: bundle.accumulator.domain.clone(),
            ...
            topup_anchor_nullifiers: bundle.accumulator.topup_anchor_nullifiers.clone(),
            hop_count: 1,
            lineage_digest: fixed_bytes(b"bridge-fast-lineage-digest-0"),
            aggregation_transcript_digest: fixed_bytes(b"bridge-fast-lineage-digest-0"),
            nullifier_digest: Hash::new(b"bridge-fast-lineage-nullifier-digest-0"),
            output_commitment_digest: Hash::new(b"bridge-fast-lineage-output-digest-0"),
            fold_digest: Hash::new(b"bridge-fast-lineage-fold-digest-0"),
            recursive_proof_chain_digest: fixed_bytes(b"bridge-fast-lineage-proof-chain-0"),
            transition_profile_binding_digest: fixed_bytes(
                b"bridge-fast-lineage-transition-binding-0",
            ),
            append_opening_preflight_digest: [0u8; 32],
            append_boundary_digest: [0u8; 32],
            verifier_params_fingerprint: bundle.accumulator.verifier_params_fingerprint,
            fixed_window_table_schedule_digest: bundle
                .accumulator
                .fixed_window_table_schedule_digest,
            fixed_window_shared_table_manifest_digest: bundle
                .accumulator
                .fixed_window_shared_table_manifest_digest,
            fixed_window_table_base_digest: bundle.accumulator.fixed_window_table_base_digest,
            verifier_witness_batch_digest: bundle.accumulator.verifier_witness_batch_digest,
            verifier_opening_len: bundle.accumulator.verifier_opening_len,
        ...
        let previous_public_inputs =
            kagemusha_recursive_spend_public_inputs_from_accumulator(&previous_accumulator)
                .expect("fast lineage previous proof public inputs");
        let previous_public_inputs_hash = previous_public_inputs
            .public_inputs_hash()
            .expect("fast lineage previous proof public-input hash");
        ...
        let pallas_open_envelopes_archive = sample_fast_pallas_open_envelopes_archive_for_lineage(
            2,
            vk_commitment,
            proof_schema_hash,
        );
        KagemushaRecursiveSpendLineageWitnessV1 {
            ...
            pallas_open_envelopes_archive,
            current_notes: vec![note0, bundle.accumulator.current_note.clone()],
    ...
    -> KagemushaRecursiveSpendRedeemRequestV1 {
        ...
        previous_proof
            ...
            recursive_spend_lineage_scalar_projection(0x73);
        previous_proof.public_inputs_hash = previous_proof
            .public_inputs
            .public_inputs_hash()
            .expect("reserved previous lineage public-input hash");
        request.lineage_witness = Some(witness);
        request
    }

    fn sample_fast_pallas_open_envelopes_archive_for_lineage(
        hop_count: usize,
        vk_commitment: [u8; Hash::LENGTH],
        public_inputs_schema_hash: [u8; Hash::LENGTH],
    ) -> Vec<u8> {
        let envelopes = (0..hop_count)
            .map(|hop_index| {
                ...
                iroha_zkp_halo2::OpenVerifyEnvelope {
                    ...
                    transcript_label: format!("bridge-fast-lineage-open-envelope-{hop_index}"),
                    vk_commitment: Some(vk_commitment),
                    public_inputs_schema_hash: Some(public_inputs_schema_hash),
                    domain_tag: Some([label.wrapping_add(14); Hash::LENGTH]),
                }
            })
            .collect::<Vec<_>>();
        norito::to_bytes(&envelopes).expect("encode fast lineage Pallas envelopes")
    }
    ...
    fn attach_reserved_lineage_envelope(
        ...
        request
            ...
            .public_inputs
            .recursive_verifier_scalar_projection_digest =
            recursive_spend_lineage_scalar_projection(0x5B);
    ...
    fn attach_reserved_lineage_envelope_without_lineage_slice(
        request: &mut KagemushaRecursiveSpendRedeemRequestV1,
    ) {
        attach_reserved_lineage_envelope(request, false);
    }
    ...
    fn sample_two_hop_kagemusha_verified_record_bundle() -> KagemushaVerifiedFoldRecordBundle {
        static BUNDLE: OnceLock<KagemushaVerifiedFoldRecordBundle> = OnceLock::new();
        BUNDLE
            .get_or_init(|| {
                ...
                let record = record_bundle
                    .verifier_records
                    .first()
                    .expect("one-hop fixture has verifier record")
                    .record
                    .clone();
                let verifier_key = record_bundle
                    .bundle
                    .steps
                    .first()
                    .expect("one-hop fixture has step")
                    .verifier_key
                    .clone();
                let first_spend_key = [0x11_u8; Hash::LENGTH];
                let first_input_rho = [0x21_u8; Hash::LENGTH];
                let first_input_diversifier =
                    confidential_v2::derive_confidential_diversifier_v2(b"kagemusha-bridge-input");
                let first_input_owner_tag =
                    confidential_v2::derive_confidential_owner_tag_v2_with_diversifier(
                        &first_spend_key,
                        first_input_diversifier,
                    )
                    .expect("first input owner tag");
                let first_input_commitment = confidential_v2::derive_confidential_note_v2(
                    &asset.to_string(),
                    7,
                    first_input_rho,
                    first_input_owner_tag,
                )
                .expect("first input commitment");
                let mut second_tree = vec![first_input_commitment];
                assert_eq!(
                    confidential_v2::compute_confidential_root_v2(&second_tree)
                        .expect("first root"),
                ...
                let second_spend_key = [0x41_u8; Hash::LENGTH];
                let second_input_diversifier =
                    confidential_v2::derive_confidential_diversifier_v2(b"kagemusha-bridge-output");
                let final_owner_tag =
                    confidential_v2::derive_confidential_owner_tag_v2_with_diversifier(
                        &[0x51_u8; Hash::LENGTH],
                        confidential_v2::derive_confidential_diversifier_v2(
                            b"kagemusha-bridge-second-output",
                        ),
                    )
                    .expect("second output owner tag");
                let second_root_before =
                    confidential_v2::compute_confidential_root_v2(&second_tree)
                        .expect("second root before");
                let second_proof = confidential_v2::build_confidential_transfer_proof_v2(
                    &chain_id,
                    &asset.to_string(),
                    &second_spend_key,
                    &second_tree,
                ...
                let mut final_tree = second_tree;
                final_tree.extend(second_proof.output_commitments.iter().copied());
                let second_root_after = confidential_v2::compute_confidential_root_v2(&final_tree)
    ...
    fn sample_recursive_compact_token_shape(
        record_bundle: &KagemushaVerifiedFoldRecordBundle,
    ) -> KagemushaCompactPaymentToken {
        let mut public_inputs =
            kagemusha_verified_folded_public_inputs_from_record_bundle(record_bundle)
                .expect("verified folded public inputs");
        public_inputs.aggregation_mode = KAGEMUSHA_AGGREGATION_MODE_RECURSIVE_IN_CIRCUIT_V1;
        let public_inputs_hash = public_inputs
            .public_inputs_hash()
            .expect("recursive compact public input hash");
        let envelope = OpenVerifyEnvelope {
            backend: BackendTag::Halo2IpaPasta,
            circuit_id: KAGEMUSHA_RECURSIVE_COMPACT_CIRCUIT_ID_V1.to_owned(),
            vk_hash: Hash::new(b"bridge-recursive-compact-vk").into(),
            public_inputs: KAGEMUSHA_RECURSIVE_AGGREGATION_PROOF_PUBLIC_INPUTS_SCHEMA.to_vec(),
            proof_bytes: vec![0xC7],
            aux: Vec::new(),
        };
        KagemushaCompactPaymentToken {
            public_inputs,
            folded_proof: KagemushaFoldedProof {
                ...
                proof: ProofBox::new(
                    ZK_BACKEND_HALO2_IPA.to_owned(),
                    norito::to_bytes(&envelope).expect("encode recursive compact envelope"),
                ),
            },
        }
    }
    ...
    fn sample_recursive_compact_shape_token(
        ...
        let verified_steps = record_bundle
            ...
            .map(|step| iroha_data_model::offline::KagemushaFoldStep {
                ...
                proof_hash: kagemusha_fold_step_proof_hash(&step.attachment.proof)
                    .expect("Kagemusha hop proof hash"),
                proof_public_inputs_digest: kagemusha_fold_step_public_inputs_digest(
                    &step.attachment.proof,
                )
                .expect("Kagemusha hop public-input digest"),
        ...
        let evidence = kagemusha_recursive_aggregation_evidence_from_steps(
            ...
            &verified_steps,
            4,
            fixed_bytes(b"bridge-recursive-compact-shape-verifier-params"),
            kagemusha_recursive_fixed_window_table_schedule_digest(4)
                .expect("recursive compact fixed-window schedule digest"),
            kagemusha_recursive_fixed_window_shared_table_manifest_digest(4)
                .expect("recursive compact shared-table manifest digest"),
            fixed_bytes(b"bridge-recursive-compact-shape-table-base"),
            fixed_bytes(b"bridge-recursive-compact-shape-witness-batch"),
        )
        .expect("recursive compact shape evidence");
        let public_inputs =
            iroha_data_model::offline::kagemusha_folded_public_inputs_from_aggregation_statement(
                &evidence.aggregation_statement,
            )
            .expect("recursive compact folded public inputs");
        let public_inputs_hash = public_inputs
            .public_inputs_hash()
            .expect("recursive compact public input hash");
        let mut recursive_public_inputs =
            kagemusha_recursive_aggregation_proof_public_inputs_from_evidence(&evidence)
                .expect("recursive compact proof public inputs");
        ...
        assert_eq!(
            recursive_public_inputs.folded_public_inputs_hash,
        ...
        if multi_row_instances {
            for (index, column) in public_instance_columns.iter_mut().enumerate() {
                let mut row = [0u8; Hash::LENGTH];
                row[..8].copy_from_slice(
                    &(u64::try_from(index).expect("test index fits u64") + 1).to_le_bytes(),
                );
                column.push(row);
            }
        }
        append_zk1_raw_instance_columns(&mut proof_bytes, public_instance_columns);
        let envelope = OpenVerifyEnvelope {
            backend: BackendTag::Halo2IpaPasta,
            circuit_id: KAGEMUSHA_RECURSIVE_COMPACT_CIRCUIT_ID_V1.to_owned(),
            vk_hash: hash_vk(&compact_vk),
            public_inputs: KAGEMUSHA_RECURSIVE_AGGREGATION_PROOF_PUBLIC_INPUTS_SCHEMA.to_vec(),
            proof_bytes,
            aux: Vec::new(),
        };
            ...
            folded_proof: KagemushaFoldedProof {
                ...
                proof: ProofBox::new(
                    ...
                    norito::to_bytes(&envelope)
                        .expect("encode shape-valid recursive compact envelope"),
                ),
            },
        }
    }
    ...
    fn mutate_kagemusha_compact_token_envelope(
        token: &mut KagemushaCompactPaymentToken,
        mutate: impl FnOnce(&mut iroha_data_model::zk::OpenVerifyEnvelope),
    ) {
        let mut envelope: iroha_data_model::zk::OpenVerifyEnvelope =
            norito::decode_from_bytes(&token.folded_proof.proof.bytes)
                .expect("Kagemusha compact token proof should be an OpenVerifyEnvelope");
        mutate(&mut envelope);
        token.folded_proof.proof.bytes =
            norito::to_bytes(&envelope).expect("encode mutated compact-token OpenVerifyEnvelope");
    }

    fn call_single_archive_ffi_with_stale_output(
    ...
    fn call_recursive_aggregation_ffi_with_stale_output(
        record_archive: &[u8],
    ...
    fn kagemusha_compact_ffi_rejects_oversized_lengths_without_output() {
        ...
        assert_eq!(
            out_len, 0,
            "oversized recursive compact record-bundle input must clear stale output lengths"
        );
        ...
        assert_eq!(
            out_len, 0,
            "oversized recursive compact Pallas envelope input must clear stale output lengths"
        );
    }

    #[test]
    fn kagemusha_recursive_compact_ffi_rejects_windowed_records_before_unavailable() {
        let record_bundle = sample_kagemusha_verified_record_bundle();
        let envelope_archive = sample_current_hop_pallas_open_envelopes_archive(
            &record_bundle,
            "bridge-recursive-compact-windowed-record-open-envelope",
        );
        let mut windowed_record_bundle = record_bundle;
        ...
        windowed_record_bundle
            ...
            .expect("one-hop fixture has verifier record")
            .record
            .withdraw_height = Some(4);
        let windowed_record_archive = norito::to_bytes(&windowed_record_bundle)
            .expect("encode height-windowed recursive compact record bundle");
    ...
    #[test]
    fn kagemusha_recursive_spend_compact_projection_verifier_ffi_rejects_malformed_inputs() {
        ...
        assert_eq!(
            valid, 0,
            "malformed projection verifier calls must clear stale valid flags"
        );
    ...
    fn kagemusha_recursive_compact_ffi_fails_closed_and_rejects_adversarial_inputs() {
        ...
        let record_bundle = sample_kagemusha_verified_record_bundle();
        let record_archive =
            norito::to_bytes(&record_bundle).expect("encode verified record bundle");
        let envelope_archive = sample_current_hop_pallas_open_envelopes_archive(
            &record_bundle,
            "bridge-recursive-compact-one-hop-open-envelope",
        );
        let _: iroha_data_model::offline::KagemushaVerifiedFoldRecordBundle =
            norito::decode_from_bytes(&record_archive)
                .expect("valid recursive compact record bundle fixture must decode");
        ...
        assert!(out_ptr.is_null());
        assert_eq!(out_len, 0);
        let detached_record_bundle = KagemushaVerifiedFoldRecordBundle {
            bundle: KagemushaVerifiedFoldBundle {
                chain_id: "kagemusha-bridge-detached-compact-record"
                    .parse()
                    .expect("detached chain id"),
                asset: record_bundle.bundle.asset.clone(),
                steps: Vec::new(),
            },
            verifier_records: Vec::new(),
        };
        let detached_record_archive = norito::to_bytes(&detached_record_bundle)
            .expect("encode detached recursive compact record bundle");
        let (status, out_ptr, out_len) = call_recursive_compact_ffi_with_stale_output(
        ...
        assert!(out_ptr.is_null());
        assert_eq!(out_len, 0);
        let mut windowed_record_bundle = record_bundle.clone();
        windowed_record_bundle
            .verifier_records
            .first_mut()
            .expect("one-hop fixture has verifier record")
        ...
        let mut extra_envelope_archive_items: Vec<iroha_zkp_halo2::OpenVerifyEnvelope> =
            norito::decode_from_bytes(&envelope_archive)
                .expect("decode valid recursive compact Pallas envelope fixture");
        extra_envelope_archive_items.push(
            extra_envelope_archive_items
                .first()
                .expect("one-hop envelope archive contains one item")
                .clone(),
        );
        ...
        let multi_hop_record_archive =
            norito::to_bytes(&multi_hop_record_bundle).expect("encode multi-hop record bundle");
        let multi_hop_envelope_archive = sample_pallas_open_envelopes_archive_for_record_bundle(
            &multi_hop_record_bundle,
            "bridge-recursive-compact-multi-hop-open-envelope",
        );
        let multi_hop_envelope_archive_items: Vec<iroha_zkp_halo2::OpenVerifyEnvelope> =
            norito::decode_from_bytes(&multi_hop_envelope_archive)
                .expect("valid multi-hop recursive compact Pallas fixture must decode");
        let mut missing_envelope_archive_items = multi_hop_envelope_archive_items.clone();
        missing_envelope_archive_items.pop();
        let missing_envelope_archive = norito::to_bytes(&missing_envelope_archive_items)
            .expect("encode missing recursive compact Pallas envelope archive");
        let (status, out_ptr, out_len) = call_recursive_compact_ffi_with_stale_output(
        ...
        assert!(out_ptr.is_null());
        assert_eq!(out_len, 0);
        let mut duplicated_envelope_archive_items = multi_hop_envelope_archive_items.clone();
        duplicated_envelope_archive_items.push(
            duplicated_envelope_archive_items
                .last()
                .expect("multi-hop Pallas archive contains envelopes")
                .clone(),
        );
        let duplicated_envelope_archive = norito::to_bytes(&duplicated_envelope_archive_items)
            .expect("encode duplicated recursive compact Pallas envelope archive");
        let (status, out_ptr, out_len) = call_recursive_compact_ffi_with_stale_output(
            &multi_hop_record_archive,
        ...
        let forged_metadata_domain =
            fixed_bytes(b"bridge-recursive-compact-forged-multi-hop-domain");
        let mut forged_metadata_envelope_archive_items = multi_hop_envelope_archive_items.clone();
        forged_metadata_envelope_archive_items
            .first_mut()
            .expect("multi-hop Pallas archive contains envelopes")
            .domain_tag = Some(forged_metadata_domain);
        let forged_metadata_envelope_archive =
            norito::to_bytes(&forged_metadata_envelope_archive_items)
                .expect("encode forged-metadata recursive compact Pallas envelope archive");
        let (status, out_ptr, out_len) = call_recursive_compact_ffi_with_stale_output(
            &multi_hop_record_archive,
        ...
        assert!(out_ptr.is_null());
        assert_eq!(out_len, 0);
        let mut reordered_envelope_archive_items = multi_hop_envelope_archive_items;
        reordered_envelope_archive_items.reverse();
        let reordered_envelope_archive = norito::to_bytes(&reordered_envelope_archive_items)
            .expect("encode reordered recursive compact Pallas envelope archive");
        let (status, out_ptr, out_len) = call_recursive_compact_ffi_with_stale_output(
            &multi_hop_record_archive,
        ...
        assert_eq!(
            token.public_inputs.aggregation_mode,
            iroha_data_model::offline::KAGEMUSHA_AGGREGATION_MODE_RECURSIVE_IN_CIRCUIT_V1
        );
        assert_eq!(
            token.folded_proof.verifier_key_id.name,
            iroha_data_model::offline::KAGEMUSHA_RECURSIVE_COMPACT_CIRCUIT_ID_V1
        );
        assert!(
            !verify_kagemusha_recursive_compact_payment_token(&token, &compact_vk),
            "core recursive compact verifier must remain fail-closed for well-formed ABI-7 token shapes"
        );
        assert!(
            !verify_kagemusha_compact_payment_token(&token, &compact_vk),
            "legacy checked-prefold compact verifier must not accept recursive mode-2 tokens"
        );
        let mut out_valid: c_uchar = 0;
        let status = unsafe {
        ...
        assert_eq!(
            status, ERR_KAGEMUSHA_PROVE,
            "ABI-7 compact verifier must reject dummy proof envelopes before returning a soft invalid result"
        );
        assert_eq!(
            out_valid, 0,
            "malformed dummy compact tokens must clear stale valid flags"
        );

        let shape_valid_token = sample_recursive_compact_shape_valid_token(&record_bundle);
        assert_eq!(
            shape_valid_token.public_inputs.aggregation_mode,
            iroha_data_model::offline::KAGEMUSHA_AGGREGATION_MODE_RECURSIVE_IN_CIRCUIT_V1
        );
        assert_eq!(
        ...
        let forged_vk_hash_archive =
            norito::to_bytes(&forged_vk_hash_token).expect("encode forged-vk-hash compact token");
        out_valid = 0xAA;
        let status = unsafe {
            connect_norito_kagemusha_verify_recursive_compact_payment_token(
        ...
        let multi_row_instance_token =
            sample_recursive_compact_multi_row_instance_token(&record_bundle);
        let multi_row_instance_archive = norito::to_bytes(&multi_row_instance_token)
            .expect("encode multi-row recursive compact token");
        out_valid = 0xAA;
        let status = unsafe {
            connect_norito_kagemusha_verify_recursive_compact_payment_token(
        ...
        let mut sentinel_spoofed_circuit_id = shape_valid_token.clone();
        sentinel_spoofed_circuit_id
            .folded_proof
            .verifier_key_id
            .name = format!(
            "forged::{}",
        ...
        let sentinel_spoofed_archive = norito::to_bytes(&sentinel_spoofed_circuit_id)
            .expect("encode sentinel-spoofed compact token");
        out_valid = 0xAA;
        let status = unsafe {
            connect_norito_kagemusha_verify_recursive_compact_payment_token(
        ...
        assert_eq!(
            status, ERR_KAGEMUSHA_PROVE,
            "malformed compact tokens must not spoof the unavailable sentinel through interpolated circuit ids"
        );
        assert_eq!(
            out_valid, 0,
            "sentinel-spoofed compact tokens must clear stale valid flags"
        );

        let mut stale_shape_binding = shape_valid_token.clone();
        stale_shape_binding.public_inputs.hop_count = stale_shape_binding
            .public_inputs
            .hop_count
            .checked_add(1)
            .expect("test hop count increment");
        stale_shape_binding.folded_proof.public_inputs_hash = stale_shape_binding
            .public_inputs
            .public_inputs_hash()
            .expect("stale shape public-input hash");
        let stale_shape_archive =
            norito::to_bytes(&stale_shape_binding).expect("encode stale shape compact token");
        out_valid = 0xAA;
        let status = unsafe {
            connect_norito_kagemusha_verify_recursive_compact_payment_token(
        ...
        assert_eq!(
            out_valid, 0,
            "stale shape bindings must clear stale valid flags"
        );

        let mut forged_token = token.clone();
        forged_token.public_inputs.hop_count = forged_token
            .public_inputs
            .hop_count
            .checked_add(1)
            .expect("test hop count increment");
        let forged_token_archive =
            norito::to_bytes(&forged_token).expect("encode forged compact token");
        out_valid = 0xAA;
        let status = unsafe {
            connect_norito_kagemusha_verify_recursive_compact_payment_token(
        ...
        assert_eq!(
            status, ERR_KAGEMUSHA_PROVE,
            "ABI-7 compact verifier must reject malformed public-input bindings before returning a soft invalid result"
        );
        assert_eq!(
            out_valid, 0,
            "malformed compact-token bindings must clear stale valid flags"
        );
        ...
        assert_eq!(
            out_valid, 0,
            "malformed verifier input must clear stale valid flags"
        );
        ...
        assert_eq!(
            out_valid, 0,
            "null verifier input must clear stale valid flags"
        );
        ...
        assert_eq!(
            out_valid, 0,
            "oversized verifier input must reject before slicing and clear stale valid flags"
        );
        ...
        let mut out_ptr = std::ptr::dangling_mut::<c_uchar>();
        let mut out_len = 99;
        let status = unsafe {
            connect_norito_kagemusha_prove_verified_recursive_compact_payment_token_with_records_and_pallas_open_envelopes(
                ptr::null(),
        ...
        assert!(
            out_ptr.is_null(),
            "null-input rejection must clear stale output pointers"
        );
        assert_eq!(
            out_len, 0,
            "null-input rejection must clear stale output lengths"
        );

        out_ptr = std::ptr::dangling_mut::<c_uchar>();
        out_len = 99;
        let status = unsafe {
            connect_norito_kagemusha_prove_verified_recursive_compact_payment_token_with_records_and_pallas_open_envelopes(
                malformed_archive.as_ptr(),
        ...
        assert!(
            out_ptr.is_null(),
            "null-envelope rejection must clear stale output pointers"
        );
        assert_eq!(
            out_len, 0,
            "null-envelope rejection must clear stale output lengths"
        );
    ...
    fn kagemusha_unanchored_compact_token_ffi_rejects_valid_bundle_without_records() {
        ...
        let (status, out_ptr, out_len) = call_single_archive_ffi_with_stale_output(
            connect_norito_kagemusha_prove_verified_compact_payment_token,
            &archive,
        );
    ...
    fn kagemusha_verified_record_recursive_aggregation_proof_bundle_ffi_rejects_adversarial_inputs()
    {
        ...
        let (status, out_ptr, out_len) = call_recursive_aggregation_ffi_with_stale_output(
            &record_archive,
            malformed_envelope_archive,
        );
        assert_eq!(status, ERR_KAGEMUSHA_PROVE);
        assert!(out_ptr.is_null());
        assert_eq!(out_len, 0);
        ...
        let (status, out_ptr, out_len) =
            call_recursive_aggregation_ffi_with_stale_output(&missing_archive, &envelope_archive);
        assert_eq!(status, ERR_KAGEMUSHA_PROVE);
        assert!(out_ptr.is_null());
        assert_eq!(out_len, 0);
    ...
    fn kagemusha_native_archive_writer_rejects_empty_and_oversized_outputs() {
        assert!(kagemusha_archive_out_of_bounds(0));
        assert!(!kagemusha_archive_out_of_bounds(1));
        ...
        let mut out_len: c_ulong = 99;
        let err = unsafe { write_kagemusha_archive_bridge(&mut out_ptr, &mut out_len, &[]) }
            .expect_err("empty Kagemusha native archive output must reject");
        assert!(matches!(err, BridgeError::KagemushaProve));
        assert!(out_ptr.is_null());
        assert_eq!(out_len, 0);
    }
    ...
    fn kagemusha_recursive_spend_ffi_rejects_invalid_archives_without_output() {
        ...
        for (entry_name, entry) in entries {
            ...
            let mut out_ptr: *mut c_uchar = ptr::dangling_mut::<c_uchar>();
            let mut out_len: c_ulong = 99;
            let status = unsafe { entry(b"x".as_ptr(), c_ulong::MAX, &mut out_ptr, &mut out_len) };
            assert_eq!(status, ERR_KAGEMUSHA_PROVE, "{entry_name} oversized");
            assert!(
                out_ptr.is_null(),
                "{entry_name} oversized must not return bytes"
            );
            assert_eq!(out_len, 0, "{entry_name} oversized must not set length");
        }
    }

    #[test]
    fn kagemusha_recursive_spend_compact_projection_ffi_returns_bound_token() {
        let bundle = sample_kagemusha_recursive_spend_bundle();
        let archive = norito::to_bytes(&bundle).expect("encode recursive spend bundle");
        let mut out_ptr: *mut c_uchar = ptr::null_mut();
        let mut out_len: c_ulong = 0;
        ...
        assert_eq!(status, 0);
        assert!(!out_ptr.is_null(), "projection output pointer must be set");
        assert!(out_len > 0, "projection output archive must be non-empty");
        let token = decode_kagemusha_compact_token(out_ptr, out_len);
        let expected = kagemusha_recursive_spend_compact_payment_token_from_bundle(&bundle)
            .expect("project recursive spend bundle");
        ...
        assert_eq!(
            ...
            expected.folded_proof.public_inputs_hash
        );
        assert_eq!(
            token.folded_proof.proof.bytes,
    ...
    fn kagemusha_recursive_spend_init_ffi_rejects_forged_current_hop_pallas_metadata() {
        for case in ["vk_commitment", "public_inputs_schema_hash"] {
            ...
            let mut envelopes: Vec<iroha_zkp_halo2::OpenVerifyEnvelope> =
                norito::decode_from_bytes(&witness.pallas_open_envelopes_archive)
                    .expect("decode bridge current-hop Pallas archive");
            ...
            match case {
                "vk_commitment" => {
                    envelope.vk_commitment =
                        Some(fixed_bytes(b"bridge-forged-current-hop-pallas-vk"));
                }
                "public_inputs_schema_hash" => {
                    envelope.public_inputs_schema_hash =
                        Some(fixed_bytes(b"bridge-forged-current-hop-pallas-schema"));
                }
            ...
            let request = KagemushaRecursiveSpendInitRequestV1 {
                ...
                pallas_open_envelopes_archive: norito::to_bytes(&envelopes)
                    .expect("encode forged current-hop Pallas archive"),
                current_note: bundle.accumulator.current_note.clone(),
                lineage_verifier_key: None,
                lineage_proving_key_archive: None,
                block_height: None,
            };
            let archive = norito::to_bytes(&request).expect("encode forged init request");
            let mut out_ptr: *mut c_uchar = ptr::null_mut();
            let mut out_len: c_ulong = 0;
            ...
            assert!(
                out_ptr.is_null(),
                "{case} current-hop Pallas metadata splice must not return bytes"
            );
    ...
    fn kagemusha_recursive_spend_init_ffi_rejects_forged_current_hop_proof_circuit_id() {
        let (bundle, witness) = sample_verifying_semantic_recursive_spend_lineage_fixture();
        let mut record_bundle = witness.record_bundle.clone();
        mutate_kagemusha_bundle_hop_envelope(&mut record_bundle.bundle, |envelope| {
            envelope.circuit_id = "bridge-forged-current-hop-proof-circuit-id".to_owned();
        });
        let request = KagemushaRecursiveSpendInitRequestV1 {
            record_bundle,
        ...
        let archive = norito::to_bytes(&request).expect("encode forged circuit-id init request");
        let mut out_ptr: *mut c_uchar = ptr::null_mut();
        let mut out_len: c_ulong = 0;
    ...
    fn kagemusha_recursive_spend_init_ffi_rejects_missing_lineage_key_artifacts() {
        ...
        let archive = norito::to_bytes(&request).expect("encode missing-artifacts init request");
        let mut out_ptr: *mut c_uchar = ptr::null_mut();
        let mut out_len: c_ulong = 0;
    ...
    fn assert_recursive_spend_single_archive_ffi_rejects_empty_nested_pallas(
        label: &str,
        entry: unsafe extern "C" fn(
            *const c_uchar,
            c_ulong,
        ...
        let (status, out_ptr, out_len) = call_single_archive_ffi_with_stale_output(entry, archive);
        assert_eq!(status, ERR_KAGEMUSHA_PROVE, "{label}");
        assert!(
            out_ptr.is_null(),
            "{label} empty nested Pallas archive must not return output bytes"
        );
        assert_eq!(out_len, 0, "{label}");
    }

    #[test]
    fn kagemusha_recursive_spend_ffi_rejects_empty_nested_pallas_archives_without_output() {
        let mut init_request = sample_recursive_spend_init_request_for_transition_profile();
        ...
        let zero_envelope_archive =
            norito::to_bytes(&Vec::<iroha_zkp_halo2::OpenVerifyEnvelope>::new())
                .expect("encode zero-envelope Pallas archive");
        let mut zero_envelope_init_request =
            sample_recursive_spend_init_request_for_transition_profile();
        zero_envelope_init_request.pallas_open_envelopes_archive = zero_envelope_archive.clone();
        let zero_envelope_init_request_archive = norito::to_bytes(&zero_envelope_init_request)
            .expect("encode zero-envelope init request");
        assert_recursive_spend_single_archive_ffi_rejects_empty_nested_pallas(
            "init zero-envelope",
            connect_norito_kagemusha_recursive_spend_init,
            &zero_envelope_init_request_archive,
        );
        assert_recursive_spend_single_archive_ffi_rejects_empty_nested_pallas(
        ...
        let mut zero_envelope_append_request =
            sample_recursive_spend_append_request(sample_kagemusha_recursive_spend_bundle(), None);
        zero_envelope_append_request.pallas_open_envelopes_archive = zero_envelope_archive;
        let zero_envelope_append_request_archive = norito::to_bytes(&zero_envelope_append_request)
            .expect("encode zero-envelope append request");
        assert_recursive_spend_single_archive_ffi_rejects_empty_nested_pallas(
            "append zero-envelope",
            connect_norito_kagemusha_recursive_spend_append,
            &zero_envelope_append_request_archive,
        );
        assert_recursive_spend_single_archive_ffi_rejects_empty_nested_pallas(
        ...
        let good_bundle_archive = norito::to_bytes(&sample_kagemusha_recursive_spend_bundle())
            .expect("encode recursive spend bundle");
        let mut out_ptr: *mut c_uchar = ptr::dangling_mut::<c_uchar>();
        let mut out_len: c_ulong = 99;
        let status = unsafe {
            connect_norito_kagemusha_recursive_spend_lineage_witness_from_init_result(
                init_request_archive.as_ptr(),
                init_request_archive.len() as c_ulong,
                good_bundle_archive.as_ptr(),
                good_bundle_archive.len() as c_ulong,
                &mut out_ptr,
                &mut out_len,
            )
        };
        ...
        assert!(
            out_ptr.is_null(),
            "init lineage witness helper must not return output for empty nested Pallas archives"
        );
        assert_eq!(out_len, 0);
        ...
        let status = unsafe {
            connect_norito_kagemusha_recursive_spend_lineage_witness_from_init_result(
                zero_envelope_init_request_archive.as_ptr(),
                zero_envelope_init_request_archive.len() as c_ulong,
                good_bundle_archive.as_ptr(),
                good_bundle_archive.len() as c_ulong,
                &mut out_ptr,
                &mut out_len,
        ...
        let (_, previous_witness) = sample_verifying_semantic_recursive_spend_lineage_fixture();
        let previous_witness_archive =
            norito::to_bytes(&previous_witness).expect("encode previous lineage witness");
        out_ptr = ptr::dangling_mut::<c_uchar>();
        out_len = 99;
        let status = unsafe {
            connect_norito_kagemusha_recursive_spend_lineage_witness_append_result(
                previous_witness_archive.as_ptr(),
                previous_witness_archive.len() as c_ulong,
                append_request_archive.as_ptr(),
                append_request_archive.len() as c_ulong,
                good_bundle_archive.as_ptr(),
                good_bundle_archive.len() as c_ulong,
                &mut out_ptr,
                &mut out_len,
        ...
        assert!(
            out_ptr.is_null(),
            "append lineage witness helper must not return output for empty nested Pallas archives"
        );
        assert_eq!(out_len, 0);
        ...
        let status = unsafe {
            connect_norito_kagemusha_recursive_spend_lineage_witness_append_result(
                previous_witness_archive.as_ptr(),
                previous_witness_archive.len() as c_ulong,
                zero_envelope_append_request_archive.as_ptr(),
                zero_envelope_append_request_archive.len() as c_ulong,
                good_bundle_archive.as_ptr(),
                good_bundle_archive.len() as c_ulong,
                &mut out_ptr,
                &mut out_len,
    ...
    fn attach_recursive_spend_previous_proof_open_verify_envelope(
        ...
        let envelope = OpenVerifyEnvelope {
            ...
            vk_hash: fixed_bytes(vk_hash_label),
            public_inputs:
                iroha_data_model::offline::KAGEMUSHA_RECURSIVE_AGGREGATION_PROOF_PUBLIC_INPUTS_SCHEMA
                    .to_vec(),
    ...
    fn sample_previous_recursive_proof_open_envelopes_archive(
        ...
        let metadata =
            ...
            .expect("canonical previous recursive proof opening metadata");
        let envelope = sample_pallas_open_envelope_with_metadata(4, label, metadata);
        norito::to_bytes(&vec![envelope]).expect("encode previous proof open-envelope archive")
    }

    fn sample_current_hop_pallas_open_envelopes_archive(
        record_bundle: &KagemushaVerifiedFoldRecordBundle,
        label: &str,
    ) -> Vec<u8> {
        assert_eq!(
            ...
            1,
            "current-hop Pallas archive helper requires exactly one hop"
        );
        sample_pallas_open_envelopes_archive_for_record_bundle(record_bundle, label)
    }

    fn sample_pallas_open_envelopes_archive_for_record_bundle(
        record_bundle: &KagemushaVerifiedFoldRecordBundle,
        label: &str,
    ) -> Vec<u8> {
    ...
    fn refresh_reserved_lineage_previous_bundle_public_inputs(
        previous_bundle: &mut KagemushaRecursiveSpendBundleV1,
    ) {
        let mut public_inputs =
            kagemusha_recursive_spend_public_inputs_from_accumulator(&previous_bundle.accumulator)
                .expect("mutated previous accumulator public inputs");
        public_inputs.recursive_verifier_scalar_projection_digest =
            recursive_spend_lineage_scalar_projection(0x6C);
        previous_bundle.recursive_proof.public_inputs = public_inputs;
        previous_bundle.recursive_proof.public_inputs_hash = previous_bundle
            .recursive_proof
            .public_inputs
            .public_inputs_hash()
            .expect("mutated previous public-input hash");
    }

    fn sample_recursive_spend_append_request(
        previous_bundle: KagemushaRecursiveSpendBundleV1,
        ...
        let pallas_open_envelopes_archive = sample_current_hop_pallas_open_envelopes_archive(
            &record_bundle,
        ...
        KagemushaRecursiveSpendAppendRequestV1 {
            ...
            output_proof_circuit_id:
                iroha_data_model::offline::KAGEMUSHA_RECURSIVE_AGGREGATION_PROOF_CIRCUIT_ID_V1
                    .to_owned(),
            previous_recursive_proof_open_envelopes_archive: Vec::new(),
            lineage_verifier_key: None,
            lineage_proving_key_archive: None,
            block_height: None,
        }
    ...
    -> KagemushaRecursiveSpendInitRequestV1 {
        ...
        let step = record_bundle
            ...
            .steps
            .first()
            .expect("sample init record bundle has one hop");
        KagemushaRecursiveSpendInitRequestV1 {
            pallas_open_envelopes_archive: sample_current_hop_pallas_open_envelopes_archive(
                &record_bundle,
                "bridge-transition-profile-init-current-hop-open",
            ),
            current_note: KagemushaSpendableNoteDescriptorV1 {
                note_commitment: step.output_commitments[0],
                spend_nullifier: fixed_bytes(b"bridge-transition-profile-init-nullifier"),
                amount: Numeric::new(7, 0),
            },
            record_bundle,
            lineage_verifier_key: None,
    ...
    -> KagemushaRecursiveSpendAppendRequestV1 {
        ...
        if previous_bundle.accumulator.initial_root == root_before {
            previous_bundle.accumulator.initial_root =
                fixed_bytes(b"bridge-lineage-append-distinct-initial-root");
        }
        previous_bundle.accumulator.topup_anchor_nullifiers =
            vec![fixed_bytes(b"bridge-lineage-append-distinct-topup-anchor")];
        ...
        previous_bundle.accumulator.current_note = KagemushaSpendableNoteDescriptorV1 {
            note_commitment: fixed_bytes(b"bridge-lineage-append-previous-note"),
        ...
        attach_recursive_spend_previous_proof_open_verify_envelope(
            &mut previous_bundle,
            b"bridge-lineage-append-missing-artifact-previous-vk",
        );

        let previous_recursive_proof_open_envelopes_archive =
            sample_previous_recursive_proof_open_envelopes_archive(
                &previous_bundle,
                "bridge-lineage-append-missing-artifact-previous-open",
            );
        let request = KagemushaRecursiveSpendAppendRequestV1 {
            previous_bundle,
        ...
        request
            .validate_public_binding()
            .expect("missing-key-artifact append request is otherwise well formed");
        request
    }

    fn call_recursive_spend_append_ffi(
        request: &KagemushaRecursiveSpendAppendRequestV1,
    ) -> (i32, *mut c_uchar, c_ulong) {
    ...
    #[test]
    fn kagemusha_recursive_spend_transition_profile_init_ffi_enforces_request_block_height() {
        let mut base = sample_recursive_spend_init_request_for_transition_profile();
        window_first_recursive_spend_hop_record(&mut base.record_bundle);

        let result = kagemusha_recursive_spend_transition_profile_init_from_request_archive(
            &norito::to_bytes(&base).expect("encode no-height init transition request"),
        );
        assert!(
            matches!(result, Err(BridgeError::KagemushaProve)),
            "height-unbound current-hop record must reject before profile generation"
        );
        ...
        assert!(
            matches!(result, Err(BridgeError::KagemushaProve)),
            "future current-hop record must reject before profile generation"
        );

        let mut in_window = base.clone();
        in_window.block_height = Some(2);
        let profile = kagemusha_recursive_spend_transition_profile_init_from_request_archive(
        ...
        assert!(
            matches!(result, Err(BridgeError::KagemushaProve)),
            "withdrawn current-hop record must reject before profile generation"
        );
    }

    #[test]
    fn kagemusha_recursive_spend_transition_profile_append_ffi_enforces_request_block_height() {
        let mut base = sample_reserved_lineage_append_request_missing_key_artifacts();
        base.output_proof_circuit_id =
            KAGEMUSHA_RECURSIVE_AGGREGATION_PROOF_CIRCUIT_ID_V1.to_owned();
        base.validate_public_binding()
            .expect("semantic append transition request is well formed");
        window_first_recursive_spend_hop_record(&mut base.record_bundle);

        let result = kagemusha_recursive_spend_transition_profile_append_from_request_archive(
            &norito::to_bytes(&base).expect("encode no-height append transition request"),
        );
        assert!(
            matches!(result, Err(BridgeError::KagemushaProve)),
            "height-unbound current-hop record must reject before append profile generation"
        );
        ...
        assert!(
            matches!(result, Err(BridgeError::KagemushaProve)),
            "future current-hop record must reject before append profile generation"
        );

        let mut in_window = base.clone();
        in_window.block_height = Some(2);
        let profile = kagemusha_recursive_spend_transition_profile_append_from_request_archive(
        ...
        assert!(
            matches!(result, Err(BridgeError::KagemushaProve)),
            "withdrawn current-hop record must reject before append profile generation"
        );
    }
    ...
    fn kagemusha_recursive_spend_transition_profile_append_ffi_rejects_stale_previous_public_input_hash()
     {
        ...
        request.previous_bundle.recursive_proof.public_inputs_hash =
            Hash::new(b"bridge-recursive-spend-transition-stale-previous-pi");
        let archive = norito::to_bytes(&request)
            .expect("encode stale-previous-hash transition profile append request");
        let mut out_ptr: *mut c_uchar = ptr::null_mut();
        let mut out_len: c_ulong = 0;
    ...
    fn sample_recursive_spend_append_transition_profile_with_opening_contract() -> (
        KagemushaRecursiveSpendAppendRequestV1,
        iroha_data_model::offline::KagemushaRecursiveSpendTransitionProfileV1,
    ) {
        let record_bundle = sample_kagemusha_verified_record_bundle();
        let step = record_bundle
            .bundle
        ...
        if previous_bundle.accumulator.initial_root == step.root_before {
            previous_bundle.accumulator.initial_root =
                fixed_bytes(b"bridge-transition-profile-distinct-initial-root");
        }
        previous_bundle.accumulator.topup_anchor_nullifiers =
            vec![fixed_bytes(b"bridge-transition-profile-distinct-topup")];
        previous_bundle.accumulator.final_root = step.root_before;
        previous_bundle.accumulator.current_note = KagemushaSpendableNoteDescriptorV1 {
            note_commitment: fixed_bytes(b"bridge-transition-profile-previous-note"),
        ...
        attach_recursive_spend_previous_proof_open_verify_envelope(
            &mut previous_bundle,
            b"bridge-transition-profile-previous-proof-envelope-vk",
        );

        let previous_proof_open_archive = sample_previous_recursive_proof_open_envelopes_archive(
            &previous_bundle,
            "bridge-transition-profile-previous-proof-open",
        );
        let pallas_open_envelopes_archive = sample_current_hop_pallas_open_envelopes_archive(
            &record_bundle,
            "bridge-transition-profile-current-hop-open",
        );
        let current_note = KagemushaSpendableNoteDescriptorV1 {
            note_commitment: step.output_commitments[0],
            spend_nullifier: fixed_bytes(b"bridge-transition-profile-current-nullifier"),
            amount: Numeric::new(7, 0),
        };
        let request = KagemushaRecursiveSpendAppendRequestV1 {
            previous_bundle,
        ...
        request
            .validate_public_binding()
            .expect("append request with previous proof openings is well formed");

        let profile = kagemusha_recursive_spend_transition_profile_append_from_request_archive(
            &norito::to_bytes(&request).expect("encode append transition-profile request"),
        )
    ...
    #[test]
    fn kagemusha_recursive_spend_lineage_append_boundary_ffi_rejects_semantic_profile_archives() {
        fn call_append_boundary_ffi(
            profile: &iroha_data_model::offline::KagemushaRecursiveSpendTransitionProfileV1,
        ) -> (i32, *mut c_uchar, c_ulong) {
            let archive = norito::to_bytes(profile).expect("encode transition profile archive");
            let mut out_ptr: *mut c_uchar = ptr::null_mut();
            let mut out_len: c_ulong = 0;
        ...
        let init_request = sample_recursive_spend_init_request_for_transition_profile();
        let init_profile = kagemusha_recursive_spend_transition_profile_init_from_request_archive(
            &norito::to_bytes(&init_request).expect("encode init transition-profile request"),
        )
        .expect("init transition profile should build");
        let (status, out_ptr, out_len) = call_append_boundary_ffi(&init_profile);
        assert_eq!(
            status, ERR_KAGEMUSHA_PROVE,
            "append-boundary FFI must reject Norito-valid initial transition profiles"
        );
        assert!(out_ptr.is_null());
        assert_eq!(out_len, 0);

        let (_, mut stale_append_profile) =
            sample_recursive_spend_append_transition_profile_with_opening_contract();
        stale_append_profile
            .append_opening_preflight
            .as_mut()
            .expect("sample append profile carries opening preflight")
            .current_hop_proof_hash = Hash::new(b"bridge-append-boundary-stale-current-proof");
        let (status, out_ptr, out_len) = call_append_boundary_ffi(&stale_append_profile);
        assert_eq!(
            status, ERR_KAGEMUSHA_PROVE,
            "append-boundary FFI must reject Norito-valid profiles with stale opening contracts"
        );
        assert!(out_ptr.is_null());
        assert_eq!(out_len, 0);

        let (_, mut duplicate_output_profile) =
            sample_recursive_spend_append_transition_profile_with_opening_contract();
        ...
        let (status, out_ptr, out_len) = call_append_boundary_ffi(&duplicate_output_profile);
        assert_eq!(
            status, ERR_KAGEMUSHA_PROVE,
    ...
    fn kagemusha_recursive_spend_transition_profile_append_ffi_binds_append_opening_preflight() {
        ...
        let append_opening_preflight_digest = profile
            ...
            .expect("bridge append profile binds append opening preflight digest");
        assert_ne!(
            append_opening_preflight_digest,
            [0u8; Hash::LENGTH],
            "bridge append opening preflight digest must be non-zero"
        );
        assert!(
            profile
                .previous_recursive_proof_open_envelopes_archive_digest
                .is_some(),
            "bridge append profile must retain the previous-proof opening archive digest"
        );
        let append_opening_preflight = profile
            .append_opening_preflight
            .as_ref()
            .expect("bridge append profile binds full append opening preflight contract");
        assert_eq!(
            append_opening_preflight.append_opening_preflight_digest,
            append_opening_preflight_digest,
            "bridge append profile contract digest must match the profile digest field"
        );
        assert_eq!(
            ...
            "bridge append profile contract must bind the previous opening archive digest"
        );
        assert_eq!(
            append_opening_preflight.current_hop_proof_hash,
            profile.current_hop_statement.proof_hash,
            "bridge append profile contract must bind the current-hop proof hash"
        );
        ...
        forged_current_envelopes[0].domain_tag = Some(fixed_bytes(
            b"bridge-transition-profile-forged-current-domain",
        ));
        forged_current_hop_opening.pallas_open_envelopes_archive =
            norito::to_bytes(&forged_current_envelopes)
                .expect("encode bridge forged current-hop Pallas archive");
        let err = kagemusha_recursive_spend_transition_profile_append_from_request_archive(
            &norito::to_bytes(&forged_current_hop_opening)
                .expect("encode bridge forged current-hop transition request"),
        )
        .expect_err("bridge append profile must reject forged current-hop opening metadata");
        assert!(
            matches!(err, BridgeError::KagemushaProve),
            "unexpected bridge forged current-hop opening error: {err:?}"
        );
        ...
        request
            ...
            .clear();
        let legacy_profile =
            kagemusha_recursive_spend_transition_profile_append_from_request_archive(
                &norito::to_bytes(&request).expect("encode legacy append transition request"),
            )
            .expect("legacy append transition profile without previous proof openings");
        assert_eq!(
        ...
        assert_eq!(
            ...
            "legacy append profiles must not bind absent previous proof opening bytes"
        );
        let profile_digest =
            iroha_data_model::offline::kagemusha_recursive_spend_transition_profile_digest(
                &profile,
            )
            .expect("append opening profile digest");
        let legacy_profile_digest =
            iroha_data_model::offline::kagemusha_recursive_spend_transition_profile_digest(
                &legacy_profile,
            )
            .expect("legacy append profile digest");
        assert_ne!(
            profile_digest, legacy_profile_digest,
            "binding append opening preflight bytes must change the bridge transition profile digest"
        );
    }

    #[test]
    fn kagemusha_recursive_spend_append_ffi_rejects_forged_previous_proof_opening_metadata() {
        for case in ["vk_commitment", "public_inputs_schema_hash", "domain_tag"] {
            let mut request = sample_recursive_spend_append_request(
                sample_reserved_lineage_previous_bundle(),
                Some(sample_recursive_spend_lineage_verifier_record()),
            );
            attach_recursive_spend_previous_proof_open_verify_envelope(
                &mut request.previous_bundle,
            ...
            let archive = sample_previous_recursive_proof_open_envelopes_archive(
                &request.previous_bundle,
                "bridge-recursive-spend-append-previous-proof-open",
            );
            let mut envelopes: Vec<iroha_zkp_halo2::OpenVerifyEnvelope> =
                norito::decode_from_bytes(&archive)
                    .expect("decode canonical previous proof open-envelope archive");
            let envelope = envelopes
                .first_mut()
                .expect("previous proof archive contains one envelope");
            match case {
                "vk_commitment" => {
                    envelope.vk_commitment = Some(fixed_bytes(
                        b"bridge-recursive-spend-append-forged-previous-proof-vk",
                    ));
                }
                "public_inputs_schema_hash" => {
                    envelope.public_inputs_schema_hash = Some(fixed_bytes(
                        b"bridge-recursive-spend-append-forged-previous-proof-schema",
                    ));
                }
                "domain_tag" => {
                    envelope.domain_tag = Some(fixed_bytes(
                        b"bridge-recursive-spend-append-forged-previous-proof-domain",
                    ));
                }
                _ => unreachable!("covered previous-proof metadata tamper case"),
            }
            request.previous_recursive_proof_open_envelopes_archive =
                norito::to_bytes(&envelopes).expect("encode forged previous proof open archive");
    ...
    fn kagemusha_recursive_spend_append_ffi_rejects_malformed_previous_proof_opening_archives() {
        ...
        let canonical_archive = sample_previous_recursive_proof_open_envelopes_archive(
            &previous_bundle,
            "bridge-recursive-spend-append-previous-proof-open",
        );
        ...
        let previous_open_envelope = previous_open_envelopes
            .first()
            .expect("canonical previous proof archive contains one envelope")
            .clone();
        ...
        for (case, previous_proof_open_archive) in [
            ...
            (
                ...
                norito::to_bytes::<Vec<iroha_zkp_halo2::OpenVerifyEnvelope>>(&Vec::new())
                    .expect("encode empty previous-proof opening archive"),
            ),
            (
                ...
                .expect("encode over-count previous-proof opening archive"),
            ),
        ] {
            let mut request = sample_recursive_spend_append_request(
    ...
    fn kagemusha_recursive_spend_append_ffi_rejects_forged_previous_proof_circuit_id() {
        ...
        previous_proof_envelope.circuit_id =
            "bridge-forged-previous-recursive-proof-circuit-id".to_owned();
        previous_bundle.recursive_proof.proof.bytes = norito::to_bytes(&previous_proof_envelope)
    ...
    #[test]
    fn kagemusha_recursive_spend_append_ffi_rejects_unsupported_output_circuit() {
        let mut request =
            sample_recursive_spend_append_request(sample_kagemusha_recursive_spend_bundle(), None);
        request.output_proof_circuit_id =
        ...
        assert!(
            out_ptr.is_null(),
            "append with an unsupported output circuit id must not return output bytes"
        );
        assert_eq!(out_len, 0);
    }

    #[test]
    fn kagemusha_recursive_spend_append_ffi_rejects_lineage_output_without_previous_openings() {
        let mut request =
            sample_recursive_spend_append_request(sample_kagemusha_recursive_spend_bundle(), None);
        request.output_proof_circuit_id =
            KAGEMUSHA_RECURSIVE_SPEND_LINEAGE_PROOF_CIRCUIT_ID_V1.to_owned();
        ...
        assert!(
            out_ptr.is_null(),
            "Reserved-lineage output append without previous-proof openings must not return output bytes"
        );
        assert_eq!(out_len, 0);
    }
    ...
    fn kagemusha_recursive_spend_append_ffi_rejects_missing_lineage_key_artifacts() {
        for (case, verifier_key, proving_key_archive) in [
            ...
            assert!(
                out_ptr.is_null(),
                "{case} must not return Reserved-lineage append output bytes"
            );
            assert_eq!(out_len, 0, "{case}");
        }
    }
    ...
    fn kagemusha_recursive_spend_append_ffi_rejects_semantic_previous_to_lineage_output() {
        ...
        attach_recursive_spend_previous_proof_open_verify_envelope(
            &mut previous_bundle,
            b"bridge-recursive-spend-semantic-previous-proof-envelope-vk",
        );
        let previous_proof_open_archive = sample_previous_recursive_proof_open_envelopes_archive(
            &previous_bundle,
            "bridge-recursive-spend-semantic-previous-proof-open",
        );
    ...
    fn kagemusha_recursive_spend_append_ffi_rejects_lineage_output_at_hop_cap() {
        ...
        attach_recursive_spend_previous_proof_open_verify_envelope(
            &mut previous_bundle,
            b"bridge-recursive-spend-capped-lineage-previous-proof-envelope-vk",
        );
        let previous_proof_open_archive = sample_previous_recursive_proof_open_envelopes_archive(
            &previous_bundle,
            "bridge-recursive-spend-capped-lineage-previous-proof-open",
        );
        let mut request = sample_recursive_spend_append_request(
            previous_bundle,
            Some(sample_recursive_spend_lineage_verifier_record()),
    ...
    #[test]
    fn kagemusha_recursive_spend_append_ffi_rejects_missing_previous_lineage_record() {
        let request =
            sample_recursive_spend_append_request(sample_reserved_lineage_previous_bundle(), None);
        let (status, out_ptr, out_len) = call_recursive_spend_append_ffi(&request);
    ...
    #[test]
    fn kagemusha_recursive_spend_append_ffi_rejects_non_lineage_previous_record() {
        let mut record = VerifyingKeyRecord::new_with_owner(
            1,
            KAGEMUSHA_RECURSIVE_AGGREGATION_PROOF_CIRCUIT_ID_V1,
            Some("bridge-recursive-spend-append".to_owned()),
            iroha_core::zk::KAGEMUSHA_VERIFIER_NAMESPACE,
            BackendTag::Halo2IpaPasta,
            "pallas",
            iroha_data_model::offline::kagemusha_recursive_aggregation_proof_public_inputs_schema_hash(
            ),
            fixed_bytes(b"bridge-recursive-spend-append-semantic-record"),
        );
        record.status = ConfidentialStatus::Active;
        record.max_proof_bytes = 64;

        let request = sample_recursive_spend_append_request(
            sample_reserved_lineage_previous_bundle(),
            Some(record),
        );
        let (status, out_ptr, out_len) = call_recursive_spend_append_ffi(&request);
        ...
        assert!(
            out_ptr.is_null(),
            "reserved-lineage append with a non-lineage verifier record must not return output bytes"
        );
        assert_eq!(out_len, 0);
    }

    #[test]
    fn kagemusha_recursive_spend_append_ffi_rejects_forged_previous_lineage_record_commitment() {
        let mut record = sample_recursive_spend_lineage_verifier_record();
        record.commitment = fixed_bytes(b"bridge-recursive-spend-append-forged-lineage-vk");
        ...
        assert!(
            out_ptr.is_null(),
            "reserved-lineage append with a forged verifier-record commitment must not return output bytes"
        );
        assert_eq!(out_len, 0);
    }
    ...
    #[test]
    fn kagemusha_recursive_spend_verify_ffi_rejects_reserved_lineage_without_matching_record() {
        fn assert_verify_request_rejected(
            request: &KagemushaRecursiveSpendVerifyRequestV1,
            label: &str,
        ) {
            let archive = norito::to_bytes(request).expect("encode recursive spend verify request");
            let err =
                kagemusha_recursive_spend_verify_from_request_archive(&archive).expect_err(label);
            assert!(
                matches!(err, BridgeError::KagemushaProve),
                "unexpected verify request error: {err:?}"
            );
        ...
        let missing_record = KagemushaRecursiveSpendVerifyRequestV1 {
            bundle: sample_reserved_lineage_previous_bundle(),
            lineage_verifier_record: None,
            block_height: None,
        };
        assert_verify_request_rejected(
            &missing_record,
            "reserved-lineage verify request without a lineage record must reject at decode boundary",
        );

        let mut semantic_record = VerifyingKeyRecord::new_with_owner(
            1,
            KAGEMUSHA_RECURSIVE_AGGREGATION_PROOF_CIRCUIT_ID_V1,
            Some("bridge-recursive-spend-verify".to_owned()),
            iroha_core::zk::KAGEMUSHA_VERIFIER_NAMESPACE,
            BackendTag::Halo2IpaPasta,
            "pallas",
            iroha_data_model::offline::kagemusha_recursive_aggregation_proof_public_inputs_schema_hash(
            ),
            fixed_bytes(b"bridge-recursive-spend-verify-semantic-record"),
        );
        semantic_record.status = ConfidentialStatus::Active;
        semantic_record.max_proof_bytes = 4096;
        let wrong_record = KagemushaRecursiveSpendVerifyRequestV1 {
            bundle: sample_reserved_lineage_previous_bundle(),
            lineage_verifier_record: Some(semantic_record),
            block_height: None,
        };
        assert_verify_request_rejected(
            &wrong_record,
            "reserved-lineage verify request with a semantic verifier record must reject at decode boundary",
        );

        let mut forged_record = sample_recursive_spend_lineage_verifier_record();
        forged_record.commitment = fixed_bytes(b"bridge-recursive-spend-verify-forged-lineage-vk");
        let forged_record = KagemushaRecursiveSpendVerifyRequestV1 {
            bundle: sample_reserved_lineage_previous_bundle(),
        ...
        assert_verify_request_rejected(
            &forged_record,
            "reserved-lineage verify request with forged verifier-record commitment must reject at decode boundary",
        );
    }

    #[test]
    fn kagemusha_recursive_spend_verify_ffi_enforces_request_block_height() {
        fn verify_result(
            request: &KagemushaRecursiveSpendVerifyRequestV1,
        ) -> KagemushaRecursiveSpendVerifyResultV1 {
            let archive = norito::to_bytes(request).expect("encode recursive spend verify request");
            kagemusha_recursive_spend_verify_from_request_archive(&archive)
    ...
    #[test]
    fn kagemusha_recursive_spend_redeem_bridge_requires_lineage_record_for_reserved_previous_proof()
    {
        ...
        let archive =
            norito::to_bytes(&request).expect("encode missing-record mixed lineage request");
        assert!(
            kagemusha_recursive_spend_redeem_from_request_archive(&archive).is_err(),
            "bridge must reject mixed lineage redeems without the active lineage verifier record"
        );
    ...
    fn kagemusha_recursive_spend_redeem_bridge_accepts_witnessless_reserved_lineage_public_binding()
    {
        ...
        attach_reserved_lineage_envelope_without_lineage_slice(&mut missing_lineage_slice);
        missing_lineage_slice.lineage_verifier_record =
            Some(sample_recursive_spend_lineage_verifier_record());
        let missing_lineage_slice_archive =
            norito::to_bytes(&missing_lineage_slice).expect("encode missing-slice request");
        assert!(
            kagemusha_recursive_spend_redeem_from_request_archive(&missing_lineage_slice_archive)
                .is_err(),
            "bridge must reject reserved-lineage redeems without verifier-slice columns"
        );
        ...
        let malformed_archive = norito::to_bytes(&malformed_envelope)
            .expect("encode malformed lineage envelope request");
        assert!(
            kagemusha_recursive_spend_redeem_from_request_archive(&malformed_archive).is_err(),
            "bridge must reject reserved lineage recursive redeem requests with malformed proof envelopes"
        );
    }

    #[test]
    #[ignore = "builds and verifies a real recursive spend proof fixture"]
    ...
    fn kagemusha_recursive_spend_redeem_bridge_rejects_adversarial_lineage_witnesses() {
        let bundle = sample_kagemusha_recursive_spend_bundle();
        let witness = sample_fast_recursive_spend_lineage_witness_for_bundle(&bundle);
        let base_request = {
            let mut request = sample_recursive_spend_redeem_request(42);
            request.bundle = bundle;
            request.lineage_witness = Some(witness.clone());
        ...
        fn assert_rejects(request: KagemushaRecursiveSpendRedeemRequestV1, label: &str) {
            let public_binding_err = request
                .validate_public_binding()
                .expect_err("fast adversarial lineage witness must reject before backend work");
            assert!(
                !public_binding_err.to_string().contains("backend"),
                "fast adversarial lineage witness `{label}` must reject before backend work: {public_binding_err}"
            );
            let archive = norito::to_bytes(&request)
        ...
        let mut current_note_count_mismatch = base_request.clone();
        current_note_count_mismatch
            .lineage_witness
            .as_mut()
            .expect("lineage witness")
            .current_notes
            .pop();
        assert_rejects(current_note_count_mismatch, "current note count mismatch");
        ...
        let sibling_output = fixed_bytes(b"bridge-lineage-final-note-sibling-output");
        {
            ...
            witness.record_bundle.bundle.steps[1]
                .output_commitments
                .push(sibling_output);
            witness.current_notes[1].spend_nullifier = sibling_output;
        }
        assert_rejects(
    ...
    #[test]
    #[ignore = "backend-shaped adversarial lineage witnesses reach real Halo2 verification"]
    fn kagemusha_recursive_spend_redeem_bridge_rejects_backend_deep_lineage_witnesses() {
        let bundle = sample_kagemusha_recursive_spend_bundle();
        let witness = sample_fast_recursive_spend_lineage_witness_for_bundle(&bundle);
        let base_request = {
            let mut request = sample_recursive_spend_redeem_request(42);
        ...
        fn assert_rejects(request: KagemushaRecursiveSpendRedeemRequestV1, label: &str) {
            ...
            assert!(
                kagemusha_recursive_spend_redeem_from_request_archive(&archive).is_err(),
                "bridge must reject backend-deep adversarial lineage witness: {label}"
            );
        }
        ...
        let mut reserved_lineage_with_record_witness = base_request;
...
fn java_kagemusha_recursive_spend_compact_payment_token_from_bundle(
    bundle_archive: &[u8],
) -> Result<Vec<u8>, String> {
    let bundle: iroha_data_model::offline::KagemushaRecursiveSpendBundleV1 =
        norito::decode_from_bytes(bundle_archive).map_err(|err| {
            format!("failed to decode Kagemusha recursive spend bundle archive: {err}")
        })?;
    let token =
        iroha_data_model::offline::kagemusha_recursive_spend_compact_payment_token_from_bundle(
            &bundle,
        ...
        .map_err(|err| {
            format!("failed to project Kagemusha recursive spend bundle into compact token: {err}")
        })?;
    norito::to_bytes(&token)
        .map_err(|err| format!("failed to encode Kagemusha recursive compact payment token: {err}"))
}
...
fn java_kagemusha_verify_recursive_spend_compact_payment_token_projection(
    compact_token_archive: &[u8],
    verifier_record_archive: &[u8],
    block_height: Option<u64>,
) -> Result<bool, String> {
    ...
    .map_err(|_| {
        "invalid Kagemusha recursive spend compact projection token or verifier record archive"
            .to_owned()
    })
}
...
fn java_native_kagemusha_prove_verified_recursive_compact_payment_token_with_records_and_pallas_open_envelopes(
    env: &mut jni::JNIEnv<'_>,
    record_bundle_archive: jni::objects::JByteArray<'_>,
    pallas_open_envelopes_archive: jni::objects::JByteArray<'_>,
) -> jni::sys::jbyteArray {
    let result = (|| -> Result<jni::sys::jbyteArray, String> {
        let record_bundle_bytes =
            read_java_byte_array(env, &record_bundle_archive, "recordBundleArchive")
                .ok_or_else(|| "invalid Kagemusha record bundle archive bytes".to_string())?;
        let pallas_open_envelope_bytes = read_java_byte_array(
            env,
            &pallas_open_envelopes_archive,
            "pallasOpenEnvelopesArchive",
...
fn java_native_kagemusha_verify_recursive_compact_payment_token(
    ...
    let result = (|| -> Result<jni::sys::jboolean, String> {
        let token_bytes = read_java_byte_array(env, &compact_token_archive, "compactTokenArchive")
            .ok_or_else(|| "invalid Kagemusha recursive compact payment token bytes".to_string())?;
        let token: iroha_data_model::offline::KagemushaCompactPaymentToken =
...
fn java_native_kagemusha_recursive_spend_compact_payment_token_from_bundle(
    env: &mut jni::JNIEnv<'_>,
    bundle_archive: jni::objects::JByteArray<'_>,
) -> jni::sys::jbyteArray {
    let result = (|| -> Result<jni::sys::jbyteArray, String> {
        let bundle_bytes = read_java_byte_array(env, &bundle_archive, "bundleArchive")
            .ok_or_else(|| "invalid Kagemusha recursive spend bundle bytes".to_string())?;
        let token_archive =
            java_kagemusha_recursive_spend_compact_payment_token_from_bundle(&bundle_bytes)?;
        let array = env
            .byte_array_from_slice(&token_archive)
            .map_err(|err| err.to_string())?;
        Ok(array.into_raw())
...
fn java_native_kagemusha_verify_recursive_spend_compact_payment_token_projection(
    env: &mut jni::JNIEnv<'_>,
    compact_token_archive: jni::objects::JByteArray<'_>,
    verifier_record_archive: jni::objects::JByteArray<'_>,
    block_height: Option<jni::sys::jlong>,
) -> jni::sys::jboolean {
    let result = (|| -> Result<jni::sys::jboolean, String> {
        let token_bytes = read_java_byte_array(env, &compact_token_archive, "compactTokenArchive")
            .ok_or_else(|| {
                "invalid Kagemusha recursive spend compact projection token bytes".to_owned()
            })?;
...
fn java_native_kagemusha_recursive_spend_lineage_append_boundary_archive(
    env: &mut jni::JNIEnv<'_>,
    profile_archive: jni::objects::JByteArray<'_>,
) -> jni::sys::jbyteArray {
    let result = (|| -> Result<jni::sys::jbyteArray, String> {
        let profile_bytes = read_java_byte_array(env, &profile_archive, "profileArchive")
            .ok_or_else(|| {
                "invalid Kagemusha recursive spend transition profile bytes".to_owned()
            })?;
        let boundary =
            kagemusha_recursive_spend_lineage_append_boundary_from_transition_profile_archive(
                &profile_bytes,
            ...
            .map_err(|_| {
                "invalid Kagemusha recursive spend lineage append boundary transition profile"
                    .to_owned()
            })?;
        let archive = norito::to_bytes(&boundary)
            .map_err(|err| format!("failed to encode lineage append boundary: {err}"))?;
        let array = env
...
fn java_privacy_result_archive(
    request_archive: &[u8],
    operation: PrivacyProofOperationV1,
) -> Result<Vec<u8>, String> {
    java_privacy_public_archive(
        &privacy_result_for_request_archive(request_archive, operation),
        privacy_result_schema_byte(operation),
...
fn java_native_privacy_result_archive(
    env: &mut jni::JNIEnv<'_>,
    request_archive: jni::objects::JByteArray<'_>,
    operation: PrivacyProofOperationV1,
    context: &str,
    ...
    let result = (|| -> Result<jni::sys::jbyteArray, String> {
        let mut request_bytes = read_java_byte_array(env, &request_archive, "requestArchive")
...
pub unsafe extern "system" fn Java_org_hyperledger_iroha_sdk_privacy_PrivacyNativeBridge_nativeBuildProof(
    ...
    java_native_privacy_result_archive(
        &mut env,
        request_archive,
        PrivacyProofOperationV1::Build,
        "build proof",
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_sdk_privacy_PrivacyNativeBridge_nativeVerifyProof(
    mut env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
    request_archive: jni::objects::JByteArray<'_>,
) -> jni::sys::jbyteArray {
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_android_privacy_PrivacyNativeBridge_nativeBuildProof(
    mut env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
    request_archive: jni::objects::JByteArray<'_>,
) -> jni::sys::jbyteArray {
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_android_privacy_PrivacyNativeBridge_nativeVerifyProof(
    mut env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
    request_archive: jni::objects::JByteArray<'_>,
) -> jni::sys::jbyteArray {
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_sdk_offline_KagemushaRecursiveCompactPaymentTokenProver_nativeBridgeAbiVersion(
    _env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
) -> jni::sys::jint {
    CONNECT_NORITO_BRIDGE_ABI_VERSION as jni::sys::jint
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_sdk_offline_KagemushaRecursiveCompactPaymentTokenProver_nativeProveVerifiedRecursiveCompactPaymentTokenWithRecordsAndPallasOpenEnvelopes(
    mut env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
    record_bundle_archive: jni::objects::JByteArray<'_>,
    pallas_open_envelopes_archive: jni::objects::JByteArray<'_>,
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_sdk_offline_KagemushaRecursiveCompactPaymentTokenProver_nativeVerifyRecursiveCompactPaymentToken(
    mut env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
    compact_token_archive: jni::objects::JByteArray<'_>,
) -> jni::sys::jboolean {
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_sdk_offline_KagemushaRecursiveCompactPaymentTokenProver_nativeRecursiveSpendCompactPaymentTokenFromBundle(
    mut env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
    bundle_archive: jni::objects::JByteArray<'_>,
) -> jni::sys::jbyteArray {
    java_native_kagemusha_recursive_spend_compact_payment_token_from_bundle(
        &mut env,
        bundle_archive,
    )
}
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_sdk_offline_KagemushaRecursiveCompactPaymentTokenProver_nativeVerifyRecursiveSpendCompactPaymentTokenProjection(
    mut env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
    compact_token_archive: jni::objects::JByteArray<'_>,
    verifier_record_archive: jni::objects::JByteArray<'_>,
) -> jni::sys::jboolean {
    java_native_kagemusha_verify_recursive_spend_compact_payment_token_projection(
        &mut env,
        compact_token_archive,
        verifier_record_archive,
        None,
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_sdk_offline_KagemushaRecursiveCompactPaymentTokenProver_nativeVerifyRecursiveSpendCompactPaymentTokenProjectionAtHeight(
    mut env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
    compact_token_archive: jni::objects::JByteArray<'_>,
    verifier_record_archive: jni::objects::JByteArray<'_>,
...
pub unsafe extern "system" fn Java_org_hyperledger_iroha_sdk_offline_KagemushaRecursiveSpendProver_nativeTransitionProfileInit(
    ...
    java_native_kagemusha_recursive_spend_archive(
        &mut env,
        request_archive,
        "transition profile init",
        |bytes| {
            let profile =
                kagemusha_recursive_spend_transition_profile_init_from_request_archive(bytes)
                    .map_err(|_| {
                        "invalid Kagemusha recursive spend transition profile init request"
                            .to_owned()
                    })?;
            norito::to_bytes(&profile)
                .map_err(|err| format!("failed to encode transition profile: {err}"))
        },
    )
}
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_sdk_offline_KagemushaRecursiveSpendProver_nativeTransitionProfileAppend(
    mut env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
    request_archive: jni::objects::JByteArray<'_>,
) -> jni::sys::jbyteArray {
    java_native_kagemusha_recursive_spend_archive(
        ...
        |bytes| {
            let profile =
                kagemusha_recursive_spend_transition_profile_append_from_request_archive(bytes)
                    .map_err(|_| {
                        "invalid Kagemusha recursive spend transition profile append request"
                            .to_owned()
                    })?;
            norito::to_bytes(&profile)
                .map_err(|err| format!("failed to encode transition profile: {err}"))
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_sdk_offline_KagemushaRecursiveSpendProver_nativeLineageAppendBoundary(
    mut env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
    profile_archive: jni::objects::JByteArray<'_>,
) -> jni::sys::jbyteArray {
    java_native_kagemusha_recursive_spend_lineage_append_boundary_archive(&mut env, profile_archive)
}
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_android_offline_KagemushaRecursiveCompactPaymentTokenProver_nativeBridgeAbiVersion(
    _env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
) -> jni::sys::jint {
    CONNECT_NORITO_BRIDGE_ABI_VERSION as jni::sys::jint
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_android_offline_KagemushaRecursiveCompactPaymentTokenProver_nativeProveVerifiedRecursiveCompactPaymentTokenWithRecordsAndPallasOpenEnvelopes(
    mut env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
    record_bundle_archive: jni::objects::JByteArray<'_>,
    pallas_open_envelopes_archive: jni::objects::JByteArray<'_>,
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_android_offline_KagemushaRecursiveCompactPaymentTokenProver_nativeVerifyRecursiveCompactPaymentToken(
    mut env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
    compact_token_archive: jni::objects::JByteArray<'_>,
) -> jni::sys::jboolean {
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_android_offline_KagemushaRecursiveCompactPaymentTokenProver_nativeRecursiveSpendCompactPaymentTokenFromBundle(
    mut env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
    bundle_archive: jni::objects::JByteArray<'_>,
) -> jni::sys::jbyteArray {
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_android_offline_KagemushaRecursiveCompactPaymentTokenProver_nativeVerifyRecursiveSpendCompactPaymentTokenProjection(
    mut env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
    compact_token_archive: jni::objects::JByteArray<'_>,
    verifier_record_archive: jni::objects::JByteArray<'_>,
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_android_offline_KagemushaRecursiveCompactPaymentTokenProver_nativeVerifyRecursiveSpendCompactPaymentTokenProjectionAtHeight(
    mut env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
    compact_token_archive: jni::objects::JByteArray<'_>,
    verifier_record_archive: jni::objects::JByteArray<'_>,
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_android_offline_KagemushaRecursiveSpendProver_nativeTransitionProfileInit(
    mut env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
    request_archive: jni::objects::JByteArray<'_>,
) -> jni::sys::jbyteArray {
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_android_offline_KagemushaRecursiveSpendProver_nativeTransitionProfileAppend(
    mut env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
    request_archive: jni::objects::JByteArray<'_>,
) -> jni::sys::jbyteArray {
...
#[allow(clippy::missing_safety_doc)]
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_org_hyperledger_iroha_android_offline_KagemushaRecursiveSpendProver_nativeLineageAppendBoundary(
    mut env: jni::JNIEnv<'_>,
    _class: jni::objects::JClass<'_>,
    profile_archive: jni::objects::JByteArray<'_>,
) -> jni::sys::jbyteArray {
...
mod tests {
    ...
    fn take_privacy_output<T>(out_ptr: *mut c_uchar, out_len: c_ulong) -> T
    where
        T: norito::NoritoSerialize,
        for<'de> T: norito::NoritoDeserialize<'de>,
    {
        let mut bytes = take_privacy_output_bytes(out_ptr, out_len);
        normalize_privacy_public_archive_for_decode::<T>(&mut bytes);
        norito::decode_from_bytes(&bytes).expect("decode privacy Norito V1 payload")
    }
    ...
    fn privacy_capabilities_are_norito_v1_and_fail_closed() {
        ...
        let rc = unsafe { iroha_privacy_capabilities_v1(&mut out_ptr, &mut out_len) };
        assert_eq!(rc, 0);
        let capabilities: PrivacyCapabilitiesV1 = take_privacy_output(out_ptr, out_len);

        assert!(privacy_capabilities_invariants_hold(&capabilities));
        assert_eq!(capabilities.version, PRIVACY_FFI_VERSION_V1);
        assert_eq!(capabilities.gate_version, PRIVACY_PRODUCTION_GATE_VERSION);
        assert_eq!(
            capabilities.algorithms.len(),
            PRIVACY_ALGORITHM_ENTRIES.len()
        );
        assert!(capabilities.algorithms.iter().any(|algorithm| {
    ...
    #[test]
    fn privacy_ffi_archives_use_public_schema_hashes() {
        let mut out_ptr: *mut c_uchar = ptr::null_mut();
        let mut out_len: c_ulong = 0;

        let capabilities_rc = unsafe { iroha_privacy_capabilities_v1(&mut out_ptr, &mut out_len) };
        assert_eq!(capabilities_rc, 0);
        let mut capabilities_archive = take_privacy_output_bytes(out_ptr, out_len);
        assert!(
        ...
        let build_request = privacy_request(
            "confidential-transfer-v2",
            "buildConfidentialTransferProofV2",
            Vec::new(),
        );
    ...
    #[test]
    fn privacy_jni_archives_use_public_schema_hashes() {
        let mut capabilities_archive =
            java_privacy_capabilities_archive().expect("encode JNI privacy capabilities");
        assert!(
            privacy_archive_has_repeated_schema_byte(
                &capabilities_archive,
                PRIVACY_CAPABILITIES_RESULT_SCHEMA_BYTE,
            ),
            "JNI capabilities output must use the public privacy capabilities schema"
        );
        normalize_privacy_public_archive_for_decode::<PrivacyCapabilitiesV1>(
            &mut capabilities_archive,
        );
        let capabilities: PrivacyCapabilitiesV1 =
            norito::decode_from_bytes(&capabilities_archive).expect("decode JNI capabilities");
        assert!(privacy_capabilities_invariants_hold(&capabilities));
        ...
        let mut build_result_archive =
            java_privacy_result_archive(&build_request_archive, PrivacyProofOperationV1::Build)
                .expect("encode JNI build result");
        assert!(
            privacy_archive_has_repeated_schema_byte(
                &build_result_archive,
                PRIVACY_BUILD_PROOF_RESULT_SCHEMA_BYTE,
            ),
            "JNI build output must use the public privacy build-result schema"
        );
        normalize_privacy_public_archive_for_decode::<PrivacyProofResultV1>(
            &mut build_result_archive,
        );
        ...
        let mut verify_result_archive =
            java_privacy_result_archive(&verify_request_archive, PrivacyProofOperationV1::Verify)
                .expect("encode JNI verify result");
        assert!(
            privacy_archive_has_repeated_schema_byte(
                &verify_result_archive,
                PRIVACY_VERIFY_PROOF_RESULT_SCHEMA_BYTE,
            ),
            "JNI verify output must use the public privacy verify-result schema"
        );
        normalize_privacy_public_archive_for_decode::<PrivacyProofResultV1>(
            &mut verify_result_archive,
        );
        ...
        let private_request_archive =
            norito::to_bytes(&build_request).expect("encode private Rust-schema request");
        let mut malformed_result_archive =
            java_privacy_result_archive(&private_request_archive, PrivacyProofOperationV1::Build)
                .expect("encode JNI malformed-result archive");
        assert!(
            privacy_archive_has_repeated_schema_byte(
                &malformed_result_archive,
                PRIVACY_BUILD_PROOF_RESULT_SCHEMA_BYTE,
            ),
            "JNI malformed build output must still use the public build-result schema"
        );
        normalize_privacy_public_archive_for_decode::<PrivacyProofResultV1>(
            &mut malformed_result_archive,
        );
        let malformed_result: PrivacyProofResultV1 =
            norito::decode_from_bytes(&malformed_result_archive)
                .expect("decode JNI malformed-result archive");
        assert_malformed_privacy_request_result(&malformed_result, "jni-private-rust-schema");
    }
    ...
    fn privacy_request_archives_reject_private_rust_schema_hashes() {
        ...
        let build_result =
            privacy_result_for_request_archive(&private_archive, PrivacyProofOperationV1::Build);
        assert_malformed_privacy_request_result(&build_result, "private-rust-schema-build");

        let mut out_ptr: *mut c_uchar = ptr::null_mut();
        let mut out_len: c_ulong = 0;
        let rc = unsafe {
            iroha_privacy_build_proof_v1(
                private_archive.as_ptr(),
                private_archive.len() as c_ulong,
                &mut out_ptr,
                &mut out_len,
            )
        };
        assert_eq!(rc, 0);
        let ffi_result: PrivacyProofResultV1 = take_privacy_output(out_ptr, out_len);
        assert_malformed_privacy_request_result(&ffi_result, "private-rust-schema-ffi-build");
    }
    ...
    fn privacy_capabilities_reject_missing_output_buffer() {
        ...
        let rc = unsafe { iroha_privacy_capabilities_v1(ptr::null_mut(), &mut out_len) };
        assert_eq!(rc, ERR_NULL_PTR);
        assert_eq!(out_len, 0);
    }

    #[test]
    fn privacy_capabilities_reject_missing_output_length_buffer() {
        let mut out_ptr = ptr::without_provenance_mut::<c_uchar>(0x01);
        let rc = unsafe { iroha_privacy_capabilities_v1(&mut out_ptr, ptr::null_mut()) };
        assert_eq!(rc, ERR_NULL_PTR);
        assert!(out_ptr.is_null());
    }

    #[test]
    fn privacy_proof_entrypoints_reject_missing_output_buffers_without_decoding_request() {
        let request = privacy_request(
            "orchard-halo2-actions-v1",
            "buildOrchardActionBundleProofV1",
            b"candidate-proof".to_vec(),
        );
        let request_bytes = public_privacy_request_archive(&request);
        let mut out_len: c_ulong = 911;

        let rc_missing_ptr = unsafe {
            iroha_privacy_build_proof_v1(
        ...
        assert_eq!(rc_missing_ptr, ERR_NULL_PTR);
        assert_eq!(out_len, 0);

        let mut out_ptr = ptr::without_provenance_mut::<c_uchar>(0x03);
        out_len = 912;
        let rc_missing_len = unsafe {
            iroha_privacy_verify_proof_v1(
                request_bytes.as_ptr(),
                request_bytes.len() as c_ulong,
                &mut out_ptr,
        ...
        assert_eq!(rc_missing_len, ERR_NULL_PTR);
        assert!(out_ptr.is_null());
        assert_eq!(out_len, 912);
    }

    #[test]
    fn privacy_proof_entrypoints_prioritize_missing_output_buffers_over_bad_requests() {
        let mut out_len: c_ulong = 913;
        let oversized_len = (isize::MAX as c_ulong).saturating_add(1);
        let rc_missing_ptr = unsafe {
            iroha_privacy_build_proof_v1(ptr::null(), oversized_len, ptr::null_mut(), &mut out_len)
        };
        assert_eq!(rc_missing_ptr, ERR_NULL_PTR);
        assert_eq!(out_len, 0);

        let mut out_ptr = ptr::without_provenance_mut::<c_uchar>(0x04);
        let rc_missing_len = unsafe {
            iroha_privacy_verify_proof_v1(ptr::null(), oversized_len, &mut out_ptr, ptr::null_mut())
        };
        assert_eq!(rc_missing_len, ERR_NULL_PTR);
        assert!(out_ptr.is_null());
    }
    ...
    #[test]
    fn privacy_allocated_buffers_zeroize_payload_and_header_before_free() {
        let payload = b"privacy-native-output-secret";
        let mut out_ptr: *mut c_uchar = ptr::null_mut();
        let mut out_len: c_ulong = 0;

        unsafe {
            write_privacy_bytes(&mut out_ptr, &mut out_len, payload)
                .expect("privacy output allocation succeeds");
        }
        assert!(!out_ptr.is_null());
        assert_eq!(out_len as usize, payload.len());

        let header = unsafe { privacy_buffer_header_from_payload(out_ptr) };
        unsafe {
            assert_eq!((*header).magic, PRIVACY_BUFFER_HEADER_MAGIC);
            assert_eq!((*header).len, payload.len());
        }

        let base = unsafe { clear_privacy_allocated_buffer(out_ptr) };
        let zeroed_payload = unsafe { slice::from_raw_parts(out_ptr, payload.len()) };
        assert!(zeroed_payload.iter().all(|byte| *byte == 0));
        let zeroed_header =
            unsafe { slice::from_raw_parts(header.cast::<u8>(), PRIVACY_BUFFER_HEADER_BYTES) };
    ...
    #[test]
    fn privacy_build_proof_rejects_null_request_with_norito_error() {
        let mut out_ptr: *mut c_uchar = ptr::null_mut();
        let mut out_len: c_ulong = 0;

        let rc =
            unsafe { iroha_privacy_build_proof_v1(ptr::null(), 0, &mut out_ptr, &mut out_len) };
        assert_eq!(rc, 0);
        let result: PrivacyProofResultV1 = take_privacy_output(out_ptr, out_len);

        assert_eq!(result.status, PRIVACY_FFI_STATUS_ERROR);
        assert_eq!(result.error_code, PRIVACY_FFI_ERROR_NULL_POINTER);
        assert!(!result.verified);
        assert!(result.proof.is_empty());
        assert!(result.algorithm_id.is_empty());
    }
    ...
    #[test]
    fn privacy_verify_proof_rejects_malformed_norito() {
        let malformed = b"not a Norito V1 privacy request";
        let mut out_ptr: *mut c_uchar = ptr::null_mut();
        let mut out_len: c_ulong = 0;
    ...
    #[test]
    fn privacy_verify_proof_rejects_oversized_request_length_before_slicing() {
        let malformed = b"oversized privacy request";
        let mut out_ptr: *mut c_uchar = ptr::null_mut();
        let mut out_len: c_ulong = 0;
        let oversized_len = (isize::MAX as c_ulong).saturating_add(1);
    ...
    fn privacy_build_proof_rejects_empty_algorithm_and_entrypoint() {
        let request = privacy_request("", "", Vec::new());
        let request_bytes = public_privacy_request_archive(&request);
        let mut out_ptr: *mut c_uchar = ptr::null_mut();
        let mut out_len: c_ulong = 0;
    ...
    fn privacy_build_proof_rejects_unknown_algorithm() {
        let request = privacy_request(
            "forged-shadow-algorithm",
            "buildForgedShadowProof",
            Vec::new(),
        );
        let request_bytes = public_privacy_request_archive(&request);
        let mut out_ptr: *mut c_uchar = ptr::null_mut();
        ...
        assert_eq!(result.algorithm_id, "forged-shadow-algorithm");
        assert_eq!(result.entrypoint, "buildForgedShadowProof");
    ...
    fn privacy_failure_results_never_serialize_witness_material() {
        let witness = b"native-ffi-witness-never-echo-5f6a4d2c";

        let mut unsupported = privacy_request(
            "forged-shadow-algorithm",
            "buildForgedShadowProof",
            Vec::new(),
        );
        unsupported.witness = witness.to_vec();
        let unsupported_result =
            privacy_result_for_request(unsupported, PrivacyProofOperationV1::Build);
        assert_eq!(
        ...
        let mut bad_entrypoint = privacy_request(
            "orchard-halo2-actions-v1",
            "buildForgedProductionProof",
            Vec::new(),
        );
        bad_entrypoint.witness = witness.to_vec();
        let bad_entrypoint_result =
            privacy_result_for_request(bad_entrypoint, PrivacyProofOperationV1::Build);
        assert_eq!(
        ...
        let mut missing_vk = privacy_request(
            "confidential-transfer-v2",
            "buildConfidentialTransferProofV2",
            Vec::new(),
        );
        missing_vk.vk_ref.clear();
        missing_vk.witness = witness.to_vec();
        let missing_vk_result =
            privacy_result_for_request(missing_vk, PrivacyProofOperationV1::Build);
        assert_eq!(
        ...
        let mut wrong_vk_backend = privacy_request(
            "confidential-transfer-v2",
            "buildConfidentialTransferProofV2",
            Vec::new(),
        );
        wrong_vk_backend.vk_ref = "groth16-bls12-377:confidential_transfer_v2".to_owned();
        wrong_vk_backend.witness = witness.to_vec();
        let wrong_vk_backend_result =
            privacy_result_for_request(wrong_vk_backend, PrivacyProofOperationV1::Build);
        assert_eq!(
        ...
        let mut wrong_vk_name = privacy_request(
            "confidential-transfer-v2",
            "buildConfidentialTransferProofV2",
            Vec::new(),
        );
        wrong_vk_name.vk_ref = "halo2-ipa-pasta:vk_test".to_owned();
        wrong_vk_name.witness = witness.to_vec();
        let wrong_vk_name_result =
            privacy_result_for_request(wrong_vk_name, PrivacyProofOperationV1::Build);
        assert_eq!(
        ...
        let mut empty_public_inputs = privacy_request(
            "confidential-transfer-v2",
            "buildConfidentialTransferProofV2",
            Vec::new(),
        );
        empty_public_inputs.public_inputs.clear();
        empty_public_inputs.witness = witness.to_vec();
        let empty_public_inputs_result =
            privacy_result_for_request(empty_public_inputs, PrivacyProofOperationV1::Build);
        assert_eq!(
        ...
        let mut disabled_build = privacy_request(
            "confidential-transfer-v2",
            "buildConfidentialTransferProofV2",
            Vec::new(),
        );
        ...
        let mut disabled_verify = privacy_request(
            "confidential-transfer-v2",
            "buildConfidentialTransferProofV2",
            b"candidate-proof".to_vec(),
        );
        ...
        let mut witness_shadow_verify = privacy_request(
            "confidential-transfer-v2",
            "buildConfidentialTransferProofV2",
            b"candidate-proof".to_vec(),
        );
        witness_shadow_verify.witness = witness.to_vec();
        let witness_shadow_verify_result =
            privacy_result_for_request(witness_shadow_verify, PrivacyProofOperationV1::Verify);
        assert_eq!(
    ...
    fn privacy_failure_results_preserve_error_invariants_without_proof_reflection() {
        let proof_marker = b"native-ffi-proof-never-echo-7db8";
    ...
    fn privacy_build_proof_rejects_unknown_entrypoint_for_known_algorithm() {
        ...
        assert_eq!(result.entrypoint, "buildForgedProductionProof");
    ...
    fn privacy_proof_ffi_rejects_wrong_backend_vk_ref_before_production_gate() {
        ...
        request.vk_ref = vk_ref.to_owned();
    ...
    fn privacy_build_proof_rejects_proof_shadow_before_production_gate() {
        let request = privacy_request(
            "confidential-transfer-v2",
            "buildConfidentialTransferProofV2",
            b"forged-build-proof-shadow".to_vec(),
        );
    ...
    fn privacy_verify_proof_rejects_witness_shadow_before_production_gate() {
        let mut request = privacy_request(
            "confidential-transfer-v2",
            "buildConfidentialTransferProofV2",
            b"secret proof".to_vec(),
        );
        request.witness = b"forged-verify-witness-shadow".to_vec();
    ...
    fn privacy_build_proof_rejects_supported_algorithm_until_production_gate_passes() {
        ...
        let zk_ace_request_bytes = public_privacy_request_archive(&zk_ace_request);
        let mut zk_ace_out_ptr: *mut c_uchar = ptr::null_mut();
        let mut zk_ace_out_len: c_ulong = 0;

        let zk_ace_rc = unsafe {
            iroha_privacy_build_proof_v1(
                zk_ace_request_bytes.as_ptr(),
                zk_ace_request_bytes.len() as c_ulong,
                &mut zk_ace_out_ptr,
                &mut zk_ace_out_len,
            )
        };
        assert_eq!(zk_ace_rc, 0);
        let zk_ace_result: PrivacyProofResultV1 =
            take_privacy_output(zk_ace_out_ptr, zk_ace_out_len);

        assert_eq!(zk_ace_result.status, PRIVACY_FFI_STATUS_ERROR);
        assert_eq!(
