...
#[test]
fn price_bounds_reject_unknown_risk_class_object_keys() {
    let table = r"
[price_bounds.Unknown]
max_cycles_delta_bps = 500
max_egress_delta_bps = 600
"
    .parse()
    ...
    let error = ConfigReader::new()
        .with_toml_source(TomlSource::inline(table))
        .read_and_complete::<user::ComputeEconomics>()
        .expect_err("unknown risk class keys must fail");
    assert!(format!("{error:?}").contains("price_bounds"));
}
#[test]
