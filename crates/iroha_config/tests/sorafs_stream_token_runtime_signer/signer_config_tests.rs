//! Adversarial configuration admission for one configured signer and independent trust.
use super::*;

#[test]
fn stream_token_runtime_binding_rejects_incomplete_disabled_and_non_production_forms() {
    let fields = signer_fields();
    ...
    rejects(
        &enabled_with_fields(&[]),
        "signer is required when issuance is enabled",
    );
    for (field, value) in &fields {
        ...
        rejects(
            ...
            &format!("{SIGNER_PREFIX}.{field} is required"),
        );
        let dormant = format!(
            "[sorafs.storage.stream_tokens]\nenabled = false\n{}",
            signer_tables(&[(*field, value.clone())])
        );
        rejects(
            &dormant,
            "signer runtime bindings are forbidden while issuance is disabled",
        );
    }
    for field in ["runtime_handle", "key_handle"] {
        for invalid in [
            "software://sorafs/stream-token/user:secret@host",
            "software://sorafs/stream-token/primary?secret=value",
            "signer:prod/../key",
            ...
            rejects(
                ...
                "canonical credential-free production signer handle",
            );
        }
    }
}
...
fn stream_token_runtime_binding_rejects_noncanonical_or_invalid_ed25519_keys() {
    for field in [
        ...
        for (public_key, expected) in [
            ...
            assert!(
                error.contains(&format!("{SIGNER_PREFIX}.{field}")) && error.contains(expected),
...
fn identities_and_runtime_handles_have_exact_public_bounds() {
    ...
    for field in ["runtime_handle", "key_handle"] {
        for scheme in ["hsm", "kms", "pkcs11", "software", "signer"] {
            let handle = format!("{scheme}:{}", "a".repeat(128 - scheme.len() - 1));
            parse_overlay(&replaced_field(field, quoted(&handle))).expect("128-byte signer handle");
            rejects(
    ...
    for invalid in [
        format!("{observer}a"),
        "software://sorafs/observer/user:secret@host".to_owned(),
...
#[test]
fn software_signer_and_observer_handles_use_the_same_authenticated_configuration() {
    for (runtime_handle, key_handle, observer_handle) in [
        (
            "software://sorafs/stream-token/primary",
            "software:production/stream-token/key-7",
            "software://sorafs/observer/primary",
        ),
        (
            "signer:production/stream-token/primary",
            "signer:production/stream-token/key-7",
            "finalized-source:prod/software/primary",
        ),
    ] {
        let mut fields = signer_fields();
        for (field, value) in &mut fields {
            *value = match *field {
                "runtime_handle" => quoted(runtime_handle),
                "key_handle" => quoted(key_handle),
                "observer.runtime_handle" => quoted(observer_handle),
                _ => continue,
            };
        }
        let actual = parse_overlay(&enabled_with_fields(&fields))
            .expect("software custody has the same public trust requirements");
        let signer = actual.torii.sorafs_storage.stream_tokens.signer.unwrap();
        assert_eq!(signer.runtime_handle, runtime_handle);
        assert_eq!(signer.key_handle, key_handle);
        assert_eq!(signer.observer.runtime_handle, observer_handle);
        assert_eq!(
            signer.public_key,
            hex::decode(public_key_hex(0x42)).unwrap().as_slice()
        );
        assert_eq!(signer.key_revision, 7);
        assert_ne!(signer.public_key, signer.attester.authority.public_key);
        assert_ne!(signer.public_key, signer.observer.authority.public_key);
    }
}
...
fn signer_key_revision_is_the_only_token_generation_and_never_truncates() {
    ...
    for revision in [0, u64::from(u32::MAX) + 1] {
        rejects(
            ...
            "signer.key_revision must be within 1..=4294967295",
        );
    }
    for field in [
        "policy_revision",
    ...
    for field in [
        ...
        for (value, diagnostic) in [
            ...
            rejects(
                ...
                &format!("{SIGNER_PREFIX}.{field} {diagnostic}"),
            );
        }
    }
}
    ...
    for (field, maximum) in [
        ...
        for value in [0, maximum + 1] {
            rejects(
                ...
                &format!("{SIGNER_PREFIX}.{field} must be within 1..={maximum}"),
            );
        }
    }
...
#[test]
fn disabled_defaults_do_not_construct_signer_or_observer_trust() {
    let actual = parse_overlay("").expect("default configuration");
    assert!(!actual.torii.sorafs_storage.stream_tokens.enabled);
    assert!(actual.torii.sorafs_storage.stream_tokens.signer.is_none());
    assert!(
    ...
    for source in [
        "[sorafs.storage.stream_tokens.signer]\n",
        "[sorafs.storage.stream_tokens.signer.attester]\n",
        "[sorafs.storage.stream_tokens.signer.observer]\n",
    ] {
        assert!(
...
#[test]
fn retired_hardware_configuration_table_has_no_alias() {
    let retired = enabled_overlay().replace(
        "sorafs.storage.stream_tokens.signer",
        "sorafs.storage.stream_tokens.hardware",
    );
    let error = parse_overlay(&retired).expect_err("only the signer table is supported");
    assert!(error.contains("unknown parameter"));
    assert!(error.contains("sorafs.storage.stream_tokens.hardware"));
}
