//! Strict configuration admission for independently administered stream-token signer trust.

use super::*;
use iroha_primitives::production_identity::is_production_identity_v1;

const PREFIX: &str = "sorafs.storage.stream_tokens.signer";
...
pub struct SorafsStreamTokenSignerConfig {
    ...
    /// Independently administered signer attester trust.
    #[config(nested)]
...
/// Required public signer-attestation authority and eligibility policy.
#[derive(Debug, Default, ReadConfig, Clone, norito::JsonDeserialize)]
...
pub struct SorafsStreamTokenObserverConfig {
    /// Credential-free production observer handle, distinct from the signer handle.
    pub runtime_handle: Option<String>,
...
impl SorafsStreamTokenSignerConfig {
    ...
    pub(super) fn parse(
    ...
    ) -> Option<actual::SorafsStreamTokenSignerConfig> {
        ...
        let runtime_handle = parser.text(
            ...
            &path("runtime_handle"),
            signer_handle,
            "must be a canonical credential-free production signer handle",
        );
...
impl SorafsStreamTokenObserverConfig {
    ...
    fn parse(&self, parser: &mut Parser<'_>) -> Option<actual::SorafsStreamTokenObserverConfig> {
        let runtime_handle = parser.text(
            self.runtime_handle.as_deref(),
            &path("observer.runtime_handle"),
            observer_handle,
            "must be a canonical credential-free production observer handle",
        );
...
impl Parser<'_> {
    ...
    fn identity(&mut self, value: Option<&str>, field: &str) -> Option<String> {
        self.text(
            value,
            field,
            |value| is_production_identity_v1(value, 128),
...
fn signer_handle(value: &str) -> bool {
