//! Offline Cash V1 command configuration tests.
...
impl TestKeyFile {
    fn unique_path() -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "iroha-config-offline-cash-v1-{}-{sequence}.key",
            std::process::id()
        ))
    }
    fn create(contents: &str) -> Self {
        ...
        #[cfg(unix)]
        {
            let mut file = fs::OpenOptions::new()
                ...
                .mode(0o600)
                .open(&path)
                .expect("create owner-only Offline Cash V1 key file");
            file.write_all(contents.as_bytes())
                .expect("write Offline Cash V1 key file");
        }
        #[cfg(not(unix))]
        fs::write(&path, contents).expect("write Offline Cash V1 key file");
        Self(path)
    }
    fn missing() -> Self {
        let path = Self::unique_path();
...
fn sample() -> ToriiOfflineCashV1Commands {
    let key_pair = KeyPair::from_seed(vec![0x41; 32], Algorithm::Ed25519);
    ToriiOfflineCashV1Commands {
...
fn parse_valid(config: ToriiOfflineCashV1Commands) -> actual::ToriiOfflineCashV1Commands {
    let mut emitter = Emitter::new();
    let parsed = config
        .parse(&mut emitter)
        .expect("valid Offline Cash V1 command configuration");
    emitter
        .into_result()
        .expect("valid Offline Cash V1 command configuration must not emit errors");
    parsed
}
fn assert_rejected(config: ToriiOfflineCashV1Commands) {
    let mut emitter = Emitter::new();
    assert!(config.parse(&mut emitter).is_none());
    assert!(
        emitter.into_result().is_err(),
        "invalid Offline Cash V1 command configuration must emit an error"
    );
}
#[test]
fn parses_minimal_offline_cash_v1_submission_authority() {
...
#[test]
fn parses_owner_held_offline_cash_v1_submission_key() {
...
#[test]
fn rejects_duplicate_offline_cash_v1_private_key_sources() {
...
#[test]
fn rejects_missing_offline_cash_v1_private_key_source() {
    let mut config = sample();
    config.private_key = None;
    assert_rejected(config);
}
#[test]
fn rejects_missing_or_malformed_offline_cash_v1_private_key_file() {
    for key_file in [
        TestKeyFile::missing(),
...
#[test]
fn rejects_unsupported_offline_cash_v1_private_key_algorithm() {
