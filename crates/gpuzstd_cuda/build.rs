...
fn main() {
    ...
    if env::var_os("GPUZSTD_CUDA_SKIP_BUILD").is_some() {
        println!("cargo:warning=GPUZSTD_CUDA_SKIP_BUILD set; skipping CUDA kernel compilation.");
        return;
    }
    ...
    build.compiler(nvcc);
    build.debug(false);
    build.file("cuda/gpuzstd_cuda.cu");
    build.flag("-std=c++17");
    build.flag("-O3");
    build.flag("-lineinfo");
    build.flag("-Xptxas=-O3");
