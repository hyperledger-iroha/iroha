...
mod tests {
    ...
    #[test]
    fn model_json_traits_use_the_protocol_owner_without_feature_gates() {
        let input: syn::ItemMod = parse_quote! {
            mod model {
                struct Value(u32);
            }
        };
        ...
        let output: syn::ItemMod =
            syn::parse2(impl_model(&mut emitter, &input)).expect("model expansion");
        let items = output.content.expect("inline model module").1;
        let Item::Use(import) = &items[0] else {
            panic!("the model injects its JSON traits from their owner");
        };
        assert!(
            import.attrs.is_empty(),
            "protocol imports must be unconditional"
        );
        assert!(import.leading_colon.is_some());
        assert_eq!(
            import.tree.to_token_stream().to_string(),
            quote!(norito::json::{JsonDeserialize, JsonSerialize}).to_string()
        );
    }
    #[test]
    fn struct_fields_documented_and_restricted() {
        let input: syn::DeriveInput = parse_quote! {
            pub struct Sample<'a> {
                pub visible: &'a str,
                hidden: bool,
                pub(crate) crate_visible: u16,
                pub(super) parent_visible: u8,
            }
        };
        ...
        for field in fields {
            ...
            let expected: Visibility = match name.as_str() {
                "visible" => expected_public_field_visibility(),
                "hidden" => Visibility::Inherited,
                "crate_visible" => parse_quote!(pub(crate)),
                "parent_visible" => parse_quote!(pub(super)),
                _ => panic!("unexpected field {name}"),
            };
            assert_eq!(field.vis, expected, "unexpected visibility for {name}");
        }
    }
    fn expected_public_field_visibility() -> Visibility {
        if cfg!(feature = "transparent_api") {
            parse_quote!(pub)
        } else {
    ...
    #[test]
    fn tuple_fields_documented_with_exact_visibility_policy() {
        let input: syn::DeriveInput = parse_quote! {
            pub struct Tuple(pub u32, bool, pub(crate) u16, pub(super) u8);
        };
        let output: ItemStruct = syn::parse2(process_pub_item(input)).expect("tuple output");
        assert!(
            doc_strings(&output.attrs)
                .iter()
                .any(|doc| doc.contains("Model struct `Tuple` exported via `#[model]`.")),
            "tuple doc missing"
        );
        let syn::Fields::Unnamed(fields) = output.fields else {
            panic!("expected tuple fields");
        };
        let expected = [
            expected_public_field_visibility(),
            Visibility::Inherited,
            parse_quote!(pub(crate)),
            parse_quote!(pub(super)),
        ];
        assert_eq!(fields.unnamed.len(), expected.len());
        for (index, (field, expected)) in fields.unnamed.iter().zip(expected).enumerate() {
            assert_eq!(
                field.vis, expected,
                "unexpected tuple field visibility at {index}"
            );
            assert!(
                doc_strings(&field.attrs)
                    .iter()
                    .any(|doc| doc.contains(&format!("Field #{index} for `Tuple`."))),
                "missing tuple field doc at {index}"
            );
        }
    }
    #[test]
    ...
    fn union_fields_documented_and_restricted() {
        ...
        assert_eq!(output.fields.named.len(), 4);
        for field in &output.fields.named {
            ...
            let expected: Visibility = match name.as_str() {
                "first" => expected_public_field_visibility(),
                "second" => Visibility::Inherited,
                "crate_visible" => parse_quote!(pub(crate)),
                "parent_visible" => parse_quote!(pub(super)),
                _ => panic!("unexpected union field {name}"),
            };
            assert_eq!(
                field.vis, expected,
