...
impl<T: Pload + message::ClassifyTopic, K: Kex, E: Enc> NetworkBase<T, K, E> {
    ...
    async fn run(mut self, shutdown_signal: ShutdownSignal) {
        ...
        loop {
            ...
            tokio::select! {
                // Safety traffic receives bounded priority bursts above. Use Tokio's
                // fair selection here so a continuously ready queue cannot starve
                // topology, repair, listener, timer, or shutdown work.
