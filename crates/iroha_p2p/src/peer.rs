    ...
    impl<E: Enc, M: Pload> MessageReader<E, M> {
        ...
        fn parse_next_encrypted_frame(&mut self) -> Result<bool, Error> {
            ...
            let parsed = (|| -> Result<ParsedFrame<M>, Error> {
                ...
                if decrypted_len == 0 {
                    return Ok(ParsedFrame::Malformed {
                ...
                while offset < decrypted_len {
                    ...
                    let decoded = match decoded {
                        ...
                        Err(err) => {
                            iroha_logger::warn!(
                                error = ?err,
                                payload_type = core::any::type_name::<M>(),
                                encrypted_frame_bytes = size,
                                decrypted_payload_bytes = decrypted_len,
                                decode_offset = offset,
                                remaining_bytes = remaining.len(),
                                inner_frame_bytes = frame_len,
                                decoded_messages,
                                "Failed to decode decrypted peer payload frame"
                            );
                            return Ok(ParsedFrame::Malformed {
                                context: MalformedPayloadFrameContext::new(
                ...
                if offset == decrypted_len {
                    Ok(ParsedFrame::Messages(frame_messages))
                } else {
                    Ok(ParsedFrame::Malformed {
                        context: MalformedPayloadFrameContext::new(
                            MalformedPayloadFrameReason::TrailingBytes,
                            size,
                            Some(decrypted_len),
