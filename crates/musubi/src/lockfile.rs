...
use iroha_data_model::{
    ChainId,
    musubi::{
        ArchiveId, MUSUBI_MAX_DEPENDENCIES_V1, MUSUBI_MAX_RESOLUTION_DEPTH_V1,
        MUSUBI_MAX_RESOLUTION_NODES_V1, MUSUBI_REGISTRY_VERSION_V1, MusubiAbiBindingV1,
        MusubiContentDigestV1, MusubiDependencyKindV1, MusubiExactDependencyEdgeV1,
        MusubiPackageIdV1, MusubiPackageScopeV1, MusubiPackageSelectorV1, MusubiRegistrySnapshotV1,
        MusubiReleaseDigestV1, MusubiReleaseIdV1, MusubiVerificationLockV1,
        MusubiVerificationNodeV1, MusubiVersionReqV1, MusubiVersionV1,
    },
    nexus::DataSpaceId,
};
...
/// One selected workspace root and its parent-local exact edges.
...
pub struct LockfileV1 {
    ...
    /// Sorted selected workspace roots.
...
impl LockfileV1 {
    ...
    pub fn validate(&self) -> Result<(), LockfileError> {
        ...
        if self.genesis_hash.iter().all(|byte| *byte == 0) {
            return Err(LockfileError::invalid("genesis hash must not be zero"));
        }
        self.snapshot
            .validate()
            .map_err(|error| LockfileError::invalid(error.reason()))?;
    ...
    ///
    /// Duplicate TOML keys are rejected by the parser and every unknown field
    /// is rejected here. A missing or retired schema returns [`LockfileError::Legacy`].
    pub fn parse(document: &str) -> Result<Self, LockfileError> {
        ...
        let genesis_hash = parse_digest(required_string(&table, "genesis-hash")?)?;
        ...
        let roots = parse_table_array(&table, "root")?
            .iter()
            .map(parse_root)
            .collect::<Result<Vec<_>, _>>()?;
        let nodes = parse_table_array(&table, "node")?
            .iter()
            .map(parse_node)
            .collect::<Result<Vec<_>, _>>()?;
        Self::new(chain_id, genesis_hash, snapshot, roots, nodes)
    }

    /// Load and strictly parse a lock document.
    pub fn read(path: &Path) -> Result<Self, LockfileError> {
        let document = fs::read_to_string(path).map_err(LockfileError::Io)?;
        Self::parse(&document)
    }
    ...
    pub fn render(&self) -> Result<String, LockfileError> {
        ...
        writeln!(output, "chain = {}", quote(lock.chain_id.as_str())).expect("write to string");
        writeln!(
            output,
            "genesis-hash = {}",
            quote(&hex_digest(lock.genesis_hash))
        )
        .expect("write to string");
        writeln!(
            output,
            "finalized-height = {}",
            quote(&lock.snapshot.finalized_height.to_string())
        )
        .expect("write to string");
        writeln!(
            output,
            "finalized-block-hash = {}",
            quote(&hex_digest(lock.snapshot.finalized_block_hash))
        )
        .expect("write to string");
        writeln!(
            output,
            "index-revision = {}",
            quote(&lock.snapshot.index_revision.to_string())
        )
        .expect("write to string");
...
fn render_edge(output: &mut String, edge: &MusubiExactDependencyEdgeV1) {
...
mod tests {
    ...
    fn lock() -> LockfileV1 {
        ...
        LockfileV1::new(
            "musubi-test".parse().expect("chain"),
            [9; 32],
