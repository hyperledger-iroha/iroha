...
async fn four_peer_committed_catalog_transition_preserves_history_and_replay() -> Result<()> {
    ...
    let startup_deadline = Instant::now() + FUNCTIONAL_FINALITY_TIMEOUT;
    let network = timeout_at(
        startup_deadline,
        tokio::task::spawn_blocking(|| {
            // The custom genesis owns its instruction list. Put this exact future-DS
            // role inside that list so native genesis grants it to ALICE before live work.
            multiroute::network_builder_with_genesis_transactions(vec![vec![
                resolution_delegation_role(),
            ]])
            ...
            .with_base_seed_if_unset(
                "four_peer_committed_catalog_transition_preserves_history_and_replay",
            )
            .build()
        }),
    )
    .await
    .wrap_err("catalog fixture genesis preparation exceeded its deadline")??;
    let result = async {
        let finality = FixtureFinality::from_network(&network)?;
        let genesis = network.genesis();
        let delegation_role = resolution_delegation_role();
        ensure!(
            genesis.0.has_results()
                && genesis
                    .0
                    ...
                    .filter(|instruction| *instruction == &delegation_role)
                    .count()
                    == 1,
        ...
        .await
        .wrap_err("catalog fixture startup exceeded its deadline")??;
        ensure!(
            network.peers().len() == 4,
            "fixture must start all four peers"
        );
        assert_resolution_delegation(&network).await?;
        let (before, previous_runtime) = lifecycle_and_runtime(&network.peers()[0]).await?;
        ensure!(
            before.lanes.len() == 3 && previous_runtime.is_none(),
            "fixture must start without runtime additions"
        );
        ...
        .await?;
        let (manifest, committee) = current_manifest(&network).await?;
        let lane = LaneConfig {
            id: ADDED_LANE,
            dataspace_id: ADDED_DATASPACE,
            alias: "runtime-committee".into(),
            visibility: LaneVisibility::Restricted,
            ..LaneConfig::default()
        };
        let mut manifest_hash = [0_u8; 32];
        manifest_hash[..8].copy_from_slice(&ADDED_DATASPACE.as_u64().to_le_bytes());
        ...
        let (after, runtime) = lifecycle_and_runtime(&network.peers()[0]).await?;
        let runtime =
            runtime.ok_or_else(|| eyre!("transition did not publish protected runtime catalog"))?;
        ensure!(
            after.lanes.len() == 4
                && after.lanes.contains(&lane)
                && runtime.dataspaces == vec![dataspace]
                && runtime.manifests == vec![manifest]
                && runtime.baseline_dataspaces_hash
                    == dataspace_catalog_hash(&baseline_dataspaces()?),
        ...
        let granted = submit_on_route(
            &network,
            &finality,
            vec![Grant::account_permission(resolution_permission(), BOB_ID.clone()).into()],
            ADDED_LANE,
            ADDED_DATASPACE,
            Some(&committee),
        )
        ...
        let mut history = vec![old, changed, granted];
        assert_catalog_and_history(
            &network, &finality, &before, &after, &runtime, &history, &committee, true,
        )
        .await?;
        let replay_height = history.last().unwrap().height;
        network.shutdown().await;
        let old_logs = network
            .peers()
            .iter()
            .map(NetworkPeer::latest_stdout_log_path)
            .collect::<Vec<_>>();
        // No snapshot writer has run. Keep the complete geometry journal and Kura history in
        // place: a durable snapshot would legitimately compact the configured-primary floor.
        for peer in network.peers() {
            ensure!(
                published_snapshot_height(peer)?.is_none(),
                "retained replay must precede any signed snapshot publication"
            );
        }
        ...
        timeout_at(
            ...
            try_join_all(network.peers().iter().map(|peer| async {
                // This is the only configuration change: enable native snapshot publication
                // for the later cold-restore phase, after startup replays the retained prefix.
                peer.start_checked(signed_snapshot_restart_layers(&network, peer), None)
                    .await?;
                loop {
                    if validator_admission_ready(peer, restart_deadline).await {
                        let status =
                            validator_status_until(peer.client().client(), restart_deadline)
                                .await?;
                        if status.blocks >= replay_height
                            && replayed_complete_history(peer, replay_height)?
                        {
        ...
        .await
        .wrap_err("four-peer retained Kura replay did not become ready")??;
        ensure!(
            network
                .peers()
                .iter()
                .zip(&old_logs)
                .all(|(peer, old)| peer.latest_stdout_log_path() != *old),
            "replay reused a previous daemon run"
        );
        assert_catalog_and_history(
            &network, &finality, &before, &after, &runtime, &history, &committee, true,
        )
        ...
        let revoked = submit_on_route(
            &network,
            &finality,
            vec![Revoke::account_permission(resolution_permission(), BOB_ID.clone()).into()],
            ADDED_LANE,
            ADDED_DATASPACE,
            Some(&committee),
        )
        .await?;
            ...
            revoked.height > replay_height,
            "retained replay did not resume new-lane finality"
        );
        history.push(revoked);
        assert_catalog_and_history(
            &network, &finality, &before, &after, &runtime, &history, &committee, false,
        )
        .await?;
        let snapshot_applied_height = history.last().unwrap().height;
        restart_validator_from_applied_snapshot(&network, snapshot_applied_height).await?;
        assert_catalog_and_history(
            &network, &finality, &before, &after, &runtime, &history, &committee, false,
        )
        .await?;
        ...
        .await?;
            ...
            resumed.height > snapshot_applied_height,
        ...
        .await?;
            ...
            old_resumed.height > history.last().unwrap().height,
            "old dataspace stopped progressing after expansion"
        );
        history.push(old_resumed);
        assert_catalog_and_history(
            &network, &finality, &before, &after, &runtime, &history, &committee, true,
        )
        .await?;
