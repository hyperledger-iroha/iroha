                ...
                while let Some(Ok(event)) = events.next().await {
                    if let EventBox::Pipeline(PipelineEventBox::Block(block)) = event {
                        if *block.status() == BlockStatus::Applied {
                            ...
                            let is_empty_block = block.header().transactions_hash().is_none();
                            let prev_height_non_empty = block_height_non_empty_tx.borrow().unwrap();
                            // TODO: this works only if `prev_height + 1 == height`
                            let height_non_empty = prev_height_non_empty + (if is_empty_block { 0 } else { 1 });
