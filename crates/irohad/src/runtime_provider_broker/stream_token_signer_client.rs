// Actual bounded signer and observer transports. Metadata confirms routing only; the issuer
// separately verifies independent challenged custody/completion evidence before releasing tokens.
use iroha_torii::sorafs::{StreamTokenSignerClientV1, StreamTokenStateObserverClientV1};
...
#[derive(Clone)]
struct StreamTokenSignerBrokerClient {
...
impl StreamTokenSignerBrokerClient {
    ...
    ) -> Result<ScrubbedBytes, StreamTokenSignerCallErrorV1> {
        let binding = self
            .binding
            .stream_token_signer_binding
            .as_ref()
            .ok_or(StreamTokenSignerCallErrorV1::Refused)?;
        let derived = SignerStreamTokenExpectedV1::new(body, binding.custody())
            .map_err(|_| StreamTokenSignerCallErrorV1::Refused)?;
        if &derived != expected {
            return Err(StreamTokenSignerCallErrorV1::Refused);
        }
...
impl StreamTokenSignerClientV1 for StreamTokenSignerBrokerClient {
    fn handle(&self) -> &str {
        &self.binding.handle
    }
    ...
    fn sign(
        ...
        let mut latch = self
            .latch
            .lock()
            .map_err(|_| StreamTokenSignerCallErrorV1::Unavailable)?;
        match *latch {
            StreamTokenSignLatchV1::Idle => {}
            StreamTokenSignLatchV1::Ambiguous(_) => {
                return Err(StreamTokenSignerCallErrorV1::AmbiguousCompletion);
            }
        ...
        StreamTokenSignerReceiptV1::new(receipt.to_vec()).map_err(|_| {
    ...
    fn recover(
        ...
        let receipt = session
            ...
            .map_err(stream_token_transport_error)?;
        let receipt = StreamTokenSignerReceiptV1::new(receipt.to_vec())?;
