//! Public metadata roundtrips and substitutions; these fixtures do not establish signer authority.
...
#[test]
fn observer_cannot_reuse_the_signer_client_route() {
    let exact = fixture();
    exact.validate().unwrap();
    assert!(
        StreamTokenSignerRuntimeBindingV1::new(
            exact.custody().clone(),
...
pub(in crate::runtime_provider_registry) fn fixture() -> StreamTokenSignerRuntimeBindingV1 {
    let key =
        iroha_crypto::KeyPair::try_from_seed(vec![0x71; 32], iroha_crypto::Algorithm::Ed25519)
            .unwrap();
    StreamTokenSignerRuntimeBindingV1::new(
        SignerCustodyBindingV1 {
            chain_id: "signer_backend-catalog-chain".to_owned(),
...
#[test]
fn complete_signer_catalog_survives_canonical_roundtrip_in_every_layout() {
    ...
    assert_eq!(binding.stream_token_signer_binding(), Some(&signer_backend));
...
#[test]
fn structural_metadata_rejects_credentials_missing_provider_and_unbounded_inputs() {
    let mutations: &[fn(&mut StreamTokenSignerRuntimeBindingV1)] = &[
        |value| {
            value.custody.runtime_handle = "software://sorafs/stream-token/primary?secret=x".into()
        },
        |value| value.custody.key_handle = "software://user:secret@signer/key".into(),
        ...
        |value| value.observer_handle = "software://sorafs/observer/primary?secret=x".into(),
        |value| value.observer_handle = "state://user:secret@observer/primary".into(),
...
#[test]
fn software_signer_and_observer_survive_public_catalog_roundtrip() {
    let mut custody = fixture().custody().clone();
    custody.runtime_handle = "software://sorafs/stream-token/primary".into();
    custody.key_handle = "software:production/stream-token/key-7".into();
    for observer_handle in [
        "software://sorafs/observer/primary",
        "state://sorafs/software/primary",
    ] {
        let metadata = StreamTokenSignerRuntimeBindingV1::new(
            custody.clone(),
            observer_handle.into(),
            [0x74; 32],
        )
        .expect("software providers use the same public binding contract");
        let catalog = IrohaRuntimeProviderBindingsV1 {
...
fn catalog_rejects_enclosing_network_or_header_drift() {
    ...
    catalog.chain_id = "signer_backend-catalog-chain".to_owned();
    ...
    let mutations: [fn(&mut IrohaRuntimeProviderBindingV1); 4] = [
        ...
        |value| value.stream_token_signer_binding = None,
    ];
    for mutate in mutations {
...
fn every_provider_or_trust_pin_change_alters_the_exported_catalog_identity() {
    ...
    let encode = |signer_backend: StreamTokenSignerRuntimeBindingV1| {
    ...
    let mutations: [fn(&mut StreamTokenSignerRuntimeBindingV1); 6] = [
        |value| {
            value.custody.purpose = SignerPurposeBindingV1::StreamToken {
                provider_id: [0x75; 32],
            }
