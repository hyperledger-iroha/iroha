...
impl StreamTokenSignerClientV1 for Route {
    fn handle(&self) -> &str {
        self.route()
    }
    fn sign(
...
fn fixture() -> (
    ...
    StreamTokenSignerRuntimeBindingV1,
) {
    let metadata = super::super::stream_token_signer_binding::tests::fixture();
...
fn approved(metadata: &StreamTokenSignerRuntimeBindingV1) -> StreamTokenApprovedCustodyAnchorV1 {
...
fn dependencies(metadata: &StreamTokenSignerRuntimeBindingV1) -> IrohaRuntimeDeps {
    IrohaRuntimeDeps::default()
        .with_sorafs_stream_token_signer_client(Route::exact(&metadata.custody().runtime_handle))
...
fn exact_dependency_handles_are_observed_twice_without_signing_or_qualification() {
    ...
    let signer_backend = Route::exact(&metadata.custody().runtime_handle);
    let observer = Route::exact(metadata.observer_handle());
    let deps = IrohaRuntimeDeps::default()
        .with_sorafs_stream_token_signer_client(signer_backend.clone())
    ...
    assert_eq!(signer_backend.calls.load(Ordering::SeqCst), 2);
    ...
    for mutate_hardware in [false, true] {
        for first_only in [false, true] {
            for (route, error) in [
                ...
                if mutate_hardware {
                    deps.sorafs_stream_token_signer_client = Some(changed);
                } else {
...
fn all_dependency_subsets_are_exactly_scoped_and_missing_clients_fail_closed() {
    ...
    for mask in 0_u8..8 {
        ...
        if mask & 1 != 0 {
            deps = deps.with_sorafs_stream_token_signer_client(Route::exact(
                &metadata.custody().runtime_handle,
            ));
        }
        if mask & 2 != 0 {
