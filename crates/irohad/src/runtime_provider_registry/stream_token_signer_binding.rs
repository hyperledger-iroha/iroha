//! Exact public signer and independent observer routing pins for one stream-token capability.
...
/// Canonical public catalog claim for one provider's configured signer and observer clients.
///
/// Decoding or validating this value establishes no custody, finality or completion
...
#[norito_schema(
    name = "irohad::runtime_provider_registry::stream_token_signer_binding::StreamTokenSignerRuntimeBindingV1"
)]
#[derive(Clone, Debug, PartialEq, Eq, norito::Decode, norito::Encode)]
pub struct StreamTokenSignerRuntimeBindingV1 {
    custody: SignerCustodyBindingV1,
    observer_handle: String,
    trust_pins_digest: [u8; 32],
}

impl StreamTokenSignerRuntimeBindingV1 {
    /// Construct public catalog metadata from independently configured custody and observer pins.
    ///
    /// # Errors
    /// Rejects wrong-role/provider custody, invalid keys/revisions/identities, unbounded or
    /// credential-bearing observer handles, and a missing full configuration commitment.
    pub fn new(
        custody: SignerCustodyBindingV1,
        observer_handle: String,
        trust_pins_digest: [u8; 32],
