...
//!
//! Handles and catalog metadata are claims. They cannot qualify signer possession, policy,
...
pub(super) fn validate_dependency_bindings(
    ...
    let signer_backend = dependencies
    ...
    let observer = dependencies
        ...
        .ok_or(Error::IncompleteResolution)?;
    let check_routes = || {
        let signer_handle = signer_backend.handle();
        let observer_handle = observer.handle();
        if !is_production_runtime_handle(signer_handle)
            || !is_production_runtime_handle(observer_handle)
        {
            return Err(Error::TestProviderRejected);
        }
        if signer_handle != metadata.custody().runtime_handle.as_str()
