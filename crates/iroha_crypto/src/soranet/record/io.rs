//! Tokio stream adapters for the SoraNet record protocol.
...
use super::{MAX_RECORD_PLAINTEXT_LEN, RECORD_HEADER_LEN, RecordOpener, RecordSealer};
...
impl<R: AsyncRead + Unpin> AsyncRead for RecordReader<R> {
    fn poll_read(
        ...
        loop {
            ...
            let opened =
                this.opener
                    .open_parts_into(&this.header, &this.ciphertext, &mut this.plaintext);
...
mod tests {
    ...
    use crate::{
        SessionKey,
        soranet::record::{RecordEndpoint, RecordLayer, RecordStreamContext, RecordStreamKind},
    };
    ...
    async fn adapters_reconstruct_plaintext_across_record_and_read_boundaries() {
        ...
        let context =
            RecordStreamContext::new(RecordEndpoint::Client, RecordStreamKind::Bidirectional, 9);
    ...
    async fn reader_rejects_tampered_transport_bytes() {
        ...
        let context =
            RecordStreamContext::new(RecordEndpoint::Client, RecordStreamKind::Unidirectional, 2);
    ...
    async fn reader_rejects_unframed_plaintext_without_exposing_it() {
        ...
        let context =
            RecordStreamContext::new(RecordEndpoint::Client, RecordStreamKind::Bidirectional, 4);
        let relay = RecordLayer::new(&key, RecordEndpoint::Relay)
            .expect("relay")
            .stream(context)
            .expect("relay stream");
