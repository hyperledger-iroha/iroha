//! Generate canonical, serialization-derived ABI-21 Kagemusha peer measurements.
//!
...
const REVIEWED_CARGO_LOCK_PATH: &str = "fixtures/kagemusha/cargo-lock.reviewed.v1";
...
const REVIEWED_CARGO_LOCK: &[u8] =
    include_bytes!("../../../../fixtures/kagemusha/cargo-lock.reviewed.v1");
...
fn render_fixture() -> Result<String, Box<dyn Error>> {
    ...
    output.push_str(&format!(
        "  \"cargo_lock\": \"{REVIEWED_CARGO_LOCK_PATH}\",\n"
    ));
    output.push_str(&format!(
        "  \"cargo_lock_sha256\": \"{}\",\n",
        hex::encode(Sha256::digest(REVIEWED_CARGO_LOCK))
    ));
