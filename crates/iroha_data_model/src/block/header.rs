...
impl From<wire::BlockHeaderWire> for BlockHeader {
    fn from(w: wire::BlockHeaderWire) -> Self {
        fn opt_hash_from_bytes<T>(b: Option<[u8; 32]>) -> Option<HashOf<T>> {
            b.map(|arr| HashOf::from_untyped_unchecked(Hash::prehashed(arr)))
        }
        fn digest_from_wire(
...
mod tests {
    ...
    fn block_header_json_requires_every_nullable_commitment_slot() {
        ...
        for field in nullable_fields {
            let mut value = norito::json::to_value(&header).expect("serialize block header");
            assert!(
                value
                    .as_object_mut()
            ...
            assert!(
                norito::json::from_value::<BlockHeader>(value).is_err(),
                "the first-release block-header JSON layout must require {field}"
            );
        }
    }

    #[test]
    fn block_header_wire_roundtrip_preserves_every_commitment() {
        fn typed_hash<T>(byte: u8) -> HashOf<T> {
        ...
        header.set_da_proof_policies_hash(Some(typed_hash::<DaProofPolicyBundle>(0x44)));
        header.set_da_commitments_hash(Some(typed_hash::<DaCommitmentBundle>(0x55)));
        header.set_da_pin_intents_hash(Some(typed_hash::<DaPinIntentBundle>(0x66)));
        header.set_prev_roster_evidence_hash(Some(typed_hash::<PreviousRosterEvidence>(0x77)));
        ...
        let decoded = BlockHeader::from(wire::BlockHeaderWire::from(header));
    ...
    fn header_hash_captures_execution_context_hash_and_preserves_no_context_hash() {
        ...
        assert_eq!(
            base,
            header.hash(),
            "a current header without execution context must retain its canonical hash"
        );
    }
    #[test]
    fn header_hash_captures_npos_effects_hash() {
        ...
        assert_ne!(
            base,
            header.hash(),
            "NPoS effects must influence the canonical V1 header projection"
        );
    }
