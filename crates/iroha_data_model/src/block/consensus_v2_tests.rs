...
fn native_amx_manifest_leaf_rejects_reordered_or_duplicate_members() {
    ...
    leaf.previous_native_settlement_hash = Some(HashOf::from_untyped_unchecked(Hash::prehashed(
        [0; Hash::LENGTH],
    )));
    assert_eq!(
        leaf.validate(),
...
fn rs16_fixture_chunks(context: &HeightContext, payload: &[u8]) -> Vec<Vec<u8>> {
    encode_payload_chunks(context.da_layout, payload).expect("fixture payload encoding succeeds")
}
