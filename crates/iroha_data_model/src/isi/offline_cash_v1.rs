//! Chain-facing instructions and operation records for Offline Cash V1.
//!
...
use crate::{
    ...
    offline::{
        ...
        OfflineCashHardwareProfileV1, OfflineCashMintAuthorizationContextV1,
        OfflineCashMintAuthorizationStatementV1, OfflineCashMintAuthorizationV1,
        OfflineCashMintCreditStatementV1, OfflineCashMintCreditV1,
        OfflineCashPastaStateCommitmentV1, OfflineCashRedemptionVoucherV1,
...
/// Sole chain-facing Offline Cash instruction and operation layout version.
pub const OFFLINE_CASH_CHAIN_VERSION_V1: u16 = 1;
/// Stable schema name for the canonical top-up request body.
pub const OFFLINE_CASH_TOP_UP_REQUEST_SCHEMA_NAME_V1: &str =
    "iroha.torii.v1.offline_cash.top_up.request";
/// Stable schema name for the canonical redemption request body.
pub const OFFLINE_CASH_REDEMPTION_REQUEST_SCHEMA_NAME_V1: &str =
    "iroha.torii.v1.offline_cash.redeem.request";
/// Exact number of siblings in a proof against the ordinary-write sparse tree.
pub const OFFLINE_CASH_RESERVE_RECEIPT_WITNESS_SIBLINGS_V1: usize = 256;
/// Reserved ordinary-write key tag for a finalized Offline Cash operation.
pub const OFFLINE_CASH_RESERVE_RECEIPT_WITNESS_KEY_TAG_V1: u8 = 0xD5;
/// Exact tagged key length: one tag byte followed by the operation identifier.
pub const OFFLINE_CASH_RESERVE_RECEIPT_WITNESS_KEY_BYTES_V1: usize = 33;

const TOP_UP_ISSUANCE_DOMAIN_V1: &[u8] = b"iroha:offline-cash:v1:top-up-issuance";
const TOP_UP_REQUEST_DOMAIN_V1: &[u8] = b"iroha:offline-cash:v1:top-up-request";
const REDEMPTION_REQUEST_DOMAIN_V1: &[u8] = b"iroha:offline-cash:v1:redemption-request";
const RESERVE_RECEIPT_DOMAIN_V1: &[u8] = b"iroha:offline-cash:v1:reserve-receipt";
const MINT_FINALITY_SEAL_MESSAGE_DOMAIN_V1: &[u8] =
    b"iroha:offline-cash:v1:mint-finality-seal-message";
const MINT_FINALITY_EPOCH_ROSTER_DOMAIN_V1: &[u8] =
    b"iroha:offline-cash:v1:mint-finality-epoch-roster";
const MINT_FINALITY_PEER_ID_DOMAIN_V1: &[u8] = b"iroha:offline-cash:v1:mint-finality-peer-id";
/// Domain for the marked SHA-256 bridge from paired Poseidon roots to `ExecutionCommitment`.
pub const OFFLINE_CASH_MINT_FINALITY_ROOT_DOMAIN_V1: &[u8] =
    b"iroha:offline-cash:v1:mint-finality-root";

/// Largest exact `2f + 1` mint-finality seal set admitted by the consensus roster bound.
pub const OFFLINE_CASH_MINT_FINALITY_MAX_SEALS_V1: usize = (MAX_VALIDATORS_PER_HEIGHT * 2) / 3 + 1;
/// Depth of the sparse block-local top-up commitment tree.
///
/// The 32-bit index space removes any special Offline Cash admission maximum; ordinary bounded
/// block bytes remain the practical throughput limit.
pub const OFFLINE_CASH_MINT_FINALITY_TREE_DEPTH_V1: usize = 32;
...
pub struct OfflineCashMintFinalityValidatorKeysV1 {
...
/// Network-independent epoch-scoped Offline Cash mint-finality authority.
///
...
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(deny_unknown_fields)]
pub struct OfflineCashMintFinalityEpochRosterTemplateV1 {
    /// Sole first-release layout version.
    pub version: u16,
    /// Exact Sumeragi election epoch.
    pub epoch: u64,
    /// Strictly ordered exact `3f + 1` consensus/Pasta key roster.
    pub validators: Vec<OfflineCashMintFinalityValidatorKeysV1>,
}

impl OfflineCashMintFinalityEpochRosterTemplateV1 {
    ...
    pub fn validate(&self) -> Result<(), OfflineCashIsiValidationErrorV1> {
    ...
    ) -> Result<OfflineCashMintFinalityEpochRosterV1, OfflineCashIsiValidationErrorV1> {
        self.validate()?;
        let roster = OfflineCashMintFinalityEpochRosterV1 {
...
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(deny_unknown_fields)]
pub struct OfflineCashMintFinalityGenesisParametersV1 {
    /// Mandatory epoch-zero authority template.
    pub epoch_roster: OfflineCashMintFinalityEpochRosterTemplateV1,
    /// Mandatory optionality for an epoch-one authority when height one is an
    /// epoch-zero boundary.
    #[norito(required)]
    pub next_epoch_roster: Option<OfflineCashMintFinalityEpochRosterTemplateV1>,
}

impl OfflineCashMintFinalityGenesisParametersV1 {
...
/// Exact epoch-scoped Offline Cash mint-finality authority.
#[derive(Debug, Clone, PartialEq, Eq, Decode, Encode, IntoSchema)]
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(deny_unknown_fields)]
pub struct OfflineCashMintFinalityEpochRosterV1 {
    /// Sole first-release layout version.
    pub version: u16,
...
impl OfflineCashMintFinalityEpochRosterV1 {
    /// Validate exact committee geometry, identity order, and key uniqueness.
    ///
    /// Canonical curve-point decoding is additionally performed by Core before
    /// the roster is admitted for signing or verification.
    ...
    /// encodable.
    pub fn finality_epoch_id(&self) -> Result<[u8; 32], OfflineCashIsiValidationErrorV1> {
        self.validate()?;
        ...
        for slot in 0..MAX_VALIDATORS_PER_HEIGHT {
            if let Some(validator) = self.validators.get(slot) {
                hasher.update([1]);
                hasher.update(offline_cash_mint_finality_peer_id_digest_v1(
                    &validator.validator,
                )?);
...
pub fn offline_cash_mint_finality_peer_id_digest_v1(
    peer_id: &PeerId,
) -> Result<[u8; 32], OfflineCashIsiValidationErrorV1> {
    let bytes = norito::encode_canonical(peer_id)
        .map_err(|error| OfflineCashIsiValidationErrorV1::Encoding(error.to_string()))?;
    let mut hasher = Sha256::new();
...
#[must_use]
pub fn offline_cash_mint_finality_root_v1(root: OfflineCashPastaStateCommitmentV1) -> Hash {
    let mut hasher = Sha256::new();
    hasher.update(OFFLINE_CASH_MINT_FINALITY_ROOT_DOMAIN_V1);
    hasher.update([0]);
...
/// Structural failure at the Offline Cash V1 chain boundary.
#[derive(Clone, Debug, PartialEq, Eq, thiserror::Error)]
pub enum OfflineCashIsiValidationErrorV1 {
    /// A value used a version other than the sole first-release layout.
    #[error("unsupported Offline Cash chain version {actual}")]
    UnsupportedVersion {
        /// Encountered version.
        actual: u16,
    },
    /// A required public field was zero, inconsistent, or non-canonical.
    #[error("invalid Offline Cash field `{field}`")]
    InvalidField {
        /// Stable field label.
        field: &'static str,
    },
    /// A canonical wire object failed its own validation.
    #[error("invalid Offline Cash wire object: {0}")]
    InvalidWire(String),
    /// Canonical Norito encoding needed for a commitment failed.
    #[error("failed to encode canonical Offline Cash data: {0}")]
    Encoding(String),
    /// Finality evidence or its reserve-receipt witness was invalid.
    #[error("invalid Offline Cash finality evidence: {0}")]
    InvalidFinality(String),
    /// Checked reserve subtraction failed.
    #[error("Offline Cash reserve totals underflow")]
    ReserveUnderflow,
    /// An operation status combined mutually exclusive fields.
    #[error("invalid Offline Cash operation status shape")]
    InvalidStatus,
    /// A terminal result was inspected without an externally pinned consensus context.
    #[error("Offline Cash terminal validation requires a caller-pinned finality context")]
    MissingTrustAnchor,
}
...
/// crash recovery can reproduce the exact terminal [`OfflineCashMintCreditV1`].
#[derive(Debug, Clone, PartialEq, Eq, Decode, Encode, IntoSchema)]
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(schema_name = "iroha.torii.v1.offline_cash.top_up.request")]
#[norito(deny_unknown_fields)]
pub struct OfflineCashTopUpRequestV1 {
    ...
    pub network_id: NetworkId,
    /// Asset debited online and represented by the resulting offline credit.
    ...
    pub payer: AccountId,
    /// Offline account that owns the resulting device-bound mint credit.
    ...
    #[norito(required)]
    pub mint_authorization: Option<OfflineCashMintAuthorizationV1>,
}

impl PartialOrd for OfflineCashTopUpRequestV1 {
    fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
        Some(self.cmp(other))
    }
}

impl Ord for OfflineCashTopUpRequestV1 {
...
#[norito(schema_name = "iroha.offline-cash.v1.top-up-issuance-preimage")]
struct OfflineCashTopUpIssuancePreimageV1 {
    version: u16,
    operation_id: [u8; 32],
    release_id: [u8; 32],
    suite_id: [u8; 32],
...
impl OfflineCashTopUpRequestV1 {
    /// Project the exact ID-independent context authorized by recipient hardware.
    #[must_use]
    pub fn mint_authorization_context(&self) -> OfflineCashMintAuthorizationContextV1 {
    ...
    fn issuance_preimage(
        &self,
    ) -> Result<OfflineCashTopUpIssuancePreimageV1, OfflineCashIsiValidationErrorV1> {
        Ok(OfflineCashTopUpIssuancePreimageV1 {
            version: self.version,
            operation_id: self.operation_id,
            release_id: self.release_id,
            suite_id: self.suite_id,
    ...
    ) -> Result<OfflineCashMintCreditStatementV1, OfflineCashIsiValidationErrorV1> {
        Ok(OfflineCashMintCreditStatementV1 {
            version: OFFLINE_CASH_WIRE_VERSION_V1,
            lifecycle: crate::offline::OfflineCashLifecycleBindingV1 {
                version: OFFLINE_CASH_WIRE_VERSION_V1,
                network_id: self.network_id,
                ...
                hardware_profile_id: self.hardware_credential.hardware_profile_id,
                policy_epoch: self.hardware_credential.policy_epoch,
                operation_kind: crate::offline::OfflineCashOperationKindV1::MintFold,
                request_id: [0; 32],
                acceptance_ticket_id: [0; 32],
                credit_id,
                ciphertext_digest: offline_cash_ciphertext_digest_v1(&self.encrypted_credit),
            },
            recipient_credential_commitment: self.recipient_credential_commitment,
            authorization_context_digest: self
                .mint_authorization_context()
    ...
    fn validate_identity_base(&self) -> Result<(), OfflineCashIsiValidationErrorV1> {
    ...
    fn validate_shape_base(&self) -> Result<(), OfflineCashIsiValidationErrorV1> {
    ...
    pub fn expected_credit_id(&self) -> Result<[u8; 32], OfflineCashIsiValidationErrorV1> {
        self.validate_identity_base()?;
    ...
    pub fn seal_pre_encryption_identifiers(
        mut self,
    ) -> Result<Self, OfflineCashIsiValidationErrorV1> {
        self.validate_identity_base()?;
    ...
    pub fn seal_identifiers(self) -> Result<Self, OfflineCashIsiValidationErrorV1> {
    ...
    pub fn mint_authorization_statement(
        &self,
    ) -> Result<OfflineCashMintAuthorizationStatementV1, OfflineCashIsiValidationErrorV1> {
        ...
        let statement = OfflineCashMintAuthorizationStatementV1 {
            version: OFFLINE_CASH_WIRE_VERSION_V1,
    ...
    pub fn validate_shape(&self) -> Result<(), OfflineCashIsiValidationErrorV1> {
    ...
    ) -> Result<(), OfflineCashIsiValidationErrorV1> {
        self.validate_shape()?;
        self.hardware_credential
    ...
    pub fn canonical_digest(&self) -> Result<[u8; 32], OfflineCashIsiValidationErrorV1> {
        self.validate_shape()?;
...
#[derive(Debug, Clone, PartialEq, Eq, Decode, Encode, IntoSchema)]
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(schema_name = "iroha.torii.v1.offline_cash.redeem.request")]
#[norito(deny_unknown_fields)]
pub struct OfflineCashRedemptionRequestV1 {
    /// Chain layout version.
    pub version: u16,
    /// Globally unique idempotency key chosen before submission.
    #[cfg_attr(feature = "json", norito(json = "crate::json_helpers::fixed_bytes"))]
    ...
    pub voucher: OfflineCashRedemptionVoucherV1,
}

impl PartialOrd for OfflineCashRedemptionRequestV1 {
    fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
        Some(self.cmp(other))
    }
}

impl Ord for OfflineCashRedemptionRequestV1 {
    fn cmp(&self, other: &Self) -> core::cmp::Ordering {
        self.encode().cmp(&other.encode())
    }
}

impl OfflineCashRedemptionRequestV1 {
    /// Return the complete released lifecycle binding authenticated by the
    /// terminal wrapper.
    #[must_use]
    pub const fn lifecycle(&self) -> &crate::offline::OfflineCashLifecycleBindingV1 {
...
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(deny_unknown_fields)]
pub struct OfflineCashReserveReceiptV1 {
    ...
    pub kind: OfflineCashOperationKindV1,
    ...
    ///
    /// This is non-zero only for [`OfflineCashOperationKindV1::TopUp`].  It is
...
impl OfflineCashReserveReceiptV1 {
    /// Return the exact reserve available after this operation.
    ///
    /// # Errors
    ///
    /// Returns an error if persisted redemptions exceed top-ups.
    pub fn available(&self) -> Result<u128, OfflineCashIsiValidationErrorV1> {
        self.total_topups
            .checked_sub(self.total_redemptions)
            .ok_or(OfflineCashIsiValidationErrorV1::ReserveUnderflow)
    }
    ...
    pub fn validate(&self) -> Result<(), OfflineCashIsiValidationErrorV1> {
        ...
        self.available()?;
        let is_canonical_first_top_up = self.kind == OfflineCashOperationKindV1::TopUp
    ...
    pub fn validate_against_previous_receipt(
        ...
        let (previous_total_topups, previous_total_redemptions) = match previous {
            None => {
                if self.kind != OfflineCashOperationKindV1::TopUp
...
/// Circuit-native leaf committed by a finalized block for one Offline Cash V1 top-up.
///
...
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(deny_unknown_fields)]
pub struct OfflineCashTopUpLeafV1 {
    /// Chain layout version.
    pub version: u16,
...
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(deny_unknown_fields)]
pub struct OfflineCashTopUpMembershipWitnessV1 {
    /// Exact leaf whose statement and amount become public helper instances.
    pub leaf: OfflineCashTopUpLeafV1,
    ...
    pub root: OfflineCashPastaStateCommitmentV1,
    /// Exactly 32 paired siblings from leaf to root.
    pub siblings: Vec<OfflineCashPastaStateCommitmentV1>,
}

impl OfflineCashTopUpMembershipWitnessV1 {
    ...
    pub fn validate(&self, top_up_count: u32) -> Result<(), OfflineCashIsiValidationErrorV1> {
        self.leaf.validate()?;
        ...
        if self.siblings.len() != OFFLINE_CASH_MINT_FINALITY_TREE_DEPTH_V1
            || top_up_count == 0
    ...
    pub fn validate_against(
        ...
        message.validate()?;
        self.validate(message.offline_cash_top_up_count)?;
        if offline_cash_mint_finality_root_v1(self.root) != message.offline_cash_top_up_root {
...
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(deny_unknown_fields)]
pub struct OfflineCashMintFinalitySealMessageV1 {
    /// Chain layout version.
    pub version: u16,
    ...
    pub offline_cash_top_up_root: Hash,
...
impl OfflineCashMintFinalitySealMessageV1 {
    ...
    pub fn validate(&self) -> Result<(), OfflineCashIsiValidationErrorV1> {
        ...
        if self
            ...
            || self.block_height == 0
            || (self.offline_cash_top_up_count == 0 && self.next_finality_epoch_id.is_none())
            || self.offline_cash_top_up_root == Hash::prehashed([0; Hash::LENGTH])
            || self
                .height_context_id
                .0
                .as_ref()
    ...
    pub fn signing_digest(&self) -> Result<[u8; 32], OfflineCashIsiValidationErrorV1> {
        self.validate()?;
        let mut hasher = Sha256::new();
        ...
        hasher.update(self.offline_cash_top_up_root.as_ref());
        hasher.update(self.offline_cash_top_up_count.to_le_bytes());
...
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(deny_unknown_fields)]
pub struct OfflineCashPastaSchnorrSignatureV1 {
...
impl OfflineCashPastaSchnorrSignatureV1 {
    fn validate(&self) -> Result<(), OfflineCashIsiValidationErrorV1> {
        require_nonzero(
...
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(deny_unknown_fields)]
pub struct OfflineCashMintFinalityValidatorSealV1 {
    ...
    pub eq_proof_signature: OfflineCashPastaSchnorrSignatureV1,
    /// Signature checked by the Ep/Fq mint helper using the roster's Vesta key.
    pub ep_proof_signature: OfflineCashPastaSchnorrSignatureV1,
}
...
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(deny_unknown_fields)]
pub struct OfflineCashMintFinalitySealShareV1 {
    /// Sole first-release layout version.
    pub version: u16,
    /// Common block-level statement signed by every validator.
    pub message: OfflineCashMintFinalitySealMessageV1,
    /// Paired seal belonging to the enclosing vote's signer index.
    pub seal: OfflineCashMintFinalityValidatorSealV1,
}

impl OfflineCashMintFinalitySealShareV1 {
...
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(deny_unknown_fields)]
pub struct OfflineCashMintFinalitySealBundleV1 {
    /// Common signed finality message.
    pub message: OfflineCashMintFinalitySealMessageV1,
    /// Strictly increasing exact-quorum signer records.
    pub seals: Vec<OfflineCashMintFinalityValidatorSealV1>,
}

impl OfflineCashMintFinalitySealBundleV1 {
    ...
    pub fn validate(&self) -> Result<(), OfflineCashIsiValidationErrorV1> {
        ...
        if self.seals.len() != expected
            || self.seals.len() > OFFLINE_CASH_MINT_FINALITY_MAX_SEALS_V1
            || self
                .seals
                .windows(2)
...
impl OfflineCashReserveReceiptWitnessV1 {
    ...
    pub fn reconstructed_root(&self) -> Result<Hash, OfflineCashIsiValidationErrorV1> {
        self.receipt.validate()?;
        if self.key != Self::expected_key(self.receipt.operation_id) {
            return Err(invalid("reserve_receipt_witness.key"));
        }
        if self.siblings.len() != OFFLINE_CASH_RESERVE_RECEIPT_WITNESS_SIBLINGS_V1 {
...
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(deny_unknown_fields)]
pub struct OfflineCashOperationFinalityV1 {
    ...
    pub reserve_receipt_witness: OfflineCashReserveReceiptWitnessV1,
    ...
    #[norito(required)]
    pub top_up_membership_witness: Option<OfflineCashTopUpMembershipWitnessV1>,
}
...
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(deny_unknown_fields)]
pub struct OfflineCashFinalityTrustAnchorV1 {
...
impl OfflineCashFinalityTrustAnchorV1 {
...
impl OfflineCashOperationFinalityV1 {
    ...
    pub fn validate_against(
        ...
        {
            return Err(OfflineCashIsiValidationErrorV1::InvalidFinality(
                "response finality does not match the caller-pinned network context".into(),
            ));
        }
        self.finality_artifact
            .verify()
            .map_err(|error| OfflineCashIsiValidationErrorV1::InvalidFinality(error.to_string()))?;
        ...
        match (receipt.kind, &self.top_up_membership_witness) {
            ...
            (OfflineCashOperationKindV1::Redemption, None) => {}
...
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(deny_unknown_fields)]
pub struct OfflineCashTopUpResultV1 {
    ...
    pub finality: OfflineCashOperationFinalityV1,
    /// Exact byte-reproducible device-bound mint credit.
    pub mint_credit: OfflineCashMintCreditV1,
}

impl OfflineCashTopUpResultV1 {
    ...
    /// [`OfflineCashTopUpRequestV1::validate_against_profile`] with the enabled
    ...
    pub fn validate_against(
        ...
        if receipt.operation_id != self.request.operation_id
            || receipt.kind != OfflineCashOperationKindV1::TopUp
...
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(deny_unknown_fields)]
pub struct OfflineCashRedemptionResultV1 {
    /// Result layout version.
    pub version: u16,
    /// Exact accepted terminal request.
    pub request: OfflineCashRedemptionRequestV1,
    /// Certified reserve debit and beneficiary-credit receipt.
    pub finality: OfflineCashOperationFinalityV1,
}

impl OfflineCashRedemptionResultV1 {
    ...
    pub fn validate_against(
        ...
        if receipt.operation_id != self.request.operation_id
            || receipt.kind != OfflineCashOperationKindV1::Redemption
            || receipt.request_digest != self.request.canonical_digest()?
...
pub enum OfflineCashOperationResultV1 {
    /// Finalized top-up and exact mint credit.
    #[codec(index = 0)]
    TopUp(OfflineCashTopUpResultV1),
    /// Finalized full or partial redemption.
    #[codec(index = 1)]
    Redemption(OfflineCashRedemptionResultV1),
}

impl OfflineCashOperationResultV1 {
...
pub enum OfflineCashOperationStateV1 {
...
pub enum OfflineCashOperationRejectionCodeV1 {
...
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(deny_unknown_fields)]
pub struct OfflineCashOperationRejectionV1 {
    /// Stable machine-readable failure class.
    pub code: OfflineCashOperationRejectionCodeV1,
...
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(deny_unknown_fields)]
pub struct OfflineCashOperationStatusV1 {
    ...
    pub state: OfflineCashOperationStateV1,
    /// Present exactly when `state` is `Applied`.
    #[norito(required)]
    pub result: Option<OfflineCashOperationResultV1>,
    /// Present exactly when `state` is `Rejected`.
    #[norito(required)]
    pub rejection: Option<OfflineCashOperationRejectionV1>,
}

impl OfflineCashOperationStatusV1 {
    ...
    pub fn validate(&self) -> Result<(), OfflineCashIsiValidationErrorV1> {
        ...
        match (&self.state, &self.result, &self.rejection) {
            (OfflineCashOperationStateV1::Pending, None, None) => Ok(()),
            (OfflineCashOperationStateV1::Applied, Some(_), None) => {
                Err(OfflineCashIsiValidationErrorV1::MissingTrustAnchor)
            }
            (OfflineCashOperationStateV1::Rejected, None, Some(rejection)) => require_nonzero(
                "operation_status.rejection.detail_digest",
                rejection.detail_digest,
            ),
            _ => Err(OfflineCashIsiValidationErrorV1::InvalidStatus),
        }
    }
    ...
    pub fn validate_against(
        ...
        match (&self.state, &self.result, &self.rejection) {
            (OfflineCashOperationStateV1::Pending, None, None) => Ok(()),
            (OfflineCashOperationStateV1::Applied, Some(result), None) => {
                result.validate_against(trust_anchor)?;
                if result.kind() != self.kind || result.operation_id() != self.operation_id {
                    return Err(OfflineCashIsiValidationErrorV1::InvalidStatus);
                }
                Ok(())
            }
            (OfflineCashOperationStateV1::Rejected, None, Some(rejection)) => require_nonzero(
...
/// Canonical operation lookup selector used by `/v1/offline/operations/{operation_id}`.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Decode, Encode, IntoSchema)]
#[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
#[norito(deny_unknown_fields)]
pub struct OfflineCashOperationLookupV1 {
...
impl OfflineCashOperationLookupV1 {
...
isi! {
    ...
    #[cfg_attr(feature = "json", derive(DeriveJsonSerialize, DeriveJsonDeserialize))]
    pub struct RedeemOfflineCashV1 {
        /// Complete terminal redemption request.
        pub request: OfflineCashRedemptionRequestV1,
    }
}

impl crate::seal::Instruction for TopUpOfflineCashV1 {}
impl crate::seal::Instruction for RedeemOfflineCashV1 {}

fn offline_cash_instruction_decode_flags() -> u8 {
    norito::core::effective_decode_flags().unwrap_or_else(norito::core::default_encode_flags)
}

macro_rules! impl_offline_cash_instruction_decode_from_slice {
    ($ty:ty, $request:ty) => {
        impl<'a> norito::core::DecodeFromSlice<'a> for $ty {
            fn decode_from_slice(bytes: &'a [u8]) -> Result<(Self, usize), norito::core::Error> {
                let flags = offline_cash_instruction_decode_flags();
...
impl_offline_cash_instruction_decode_from_slice!(TopUpOfflineCashV1, OfflineCashTopUpRequestV1);
impl_offline_cash_instruction_decode_from_slice!(
    RedeemOfflineCashV1,
    OfflineCashRedemptionRequestV1
);

impl TopUpOfflineCashV1 {
    /// Canonical first-release instruction wire identifier.
    pub const WIRE_ID: &'static str = "iroha.offline_cash.v1.top_up";
...
impl RedeemOfflineCashV1 {
    /// Canonical first-release instruction wire identifier.
    pub const WIRE_ID: &'static str = "iroha.offline_cash.v1.redeem";
...
fn require_chain_version(version: u16) -> Result<(), OfflineCashIsiValidationErrorV1> {
    if version != OFFLINE_CASH_CHAIN_VERSION_V1 {
        return Err(OfflineCashIsiValidationErrorV1::UnsupportedVersion { actual: version });
    }
    Ok(())
}
...
fn validate_mint_finality_roster_shape(
    version: u16,
    validators: &[OfflineCashMintFinalityValidatorKeysV1],
...
fn invalid(field: &'static str) -> OfflineCashIsiValidationErrorV1 {
    OfflineCashIsiValidationErrorV1::InvalidField { field }
}

fn wire_error(error: impl core::fmt::Display) -> OfflineCashIsiValidationErrorV1 {
    OfflineCashIsiValidationErrorV1::InvalidWire(error.to_string())
}
...
mod tests {
    ...
    use crate::{
        ...
        offline::{
            ...
            OFFLINE_CASH_XCHACHA20POLY1305_TAG_BYTES_V1, OfflineCashDevicePublicKeyV1,
            OfflineCashDeviceSignatureV1, OfflineCashHardwarePlatformClassV1,
            OfflineCashPairedProofV1, offline_cash_credit_opening_canonical_len_v1,
    ...
    fn network() -> NetworkId {
        NetworkId::from_genesis_hash(HashOf::<BlockHeader>::from_untyped_unchecked(Hash::new(
            b"offline-cash-v1-isi",
        )))
    }

    fn mint_finality_template(epoch: u64) -> OfflineCashMintFinalityEpochRosterTemplateV1 {
        let mut validators = (1_u8..=4)
            .map(|seed| OfflineCashMintFinalityValidatorKeysV1 {
    ...
    fn mint_finality_genesis_parameters_require_explicit_next_roster_option() {
        ...
        norito::json::value::from_value::<OfflineCashMintFinalityGenesisParametersV1>(value)
    ...
    fn attach_test_mint_authorization(
        mut request: OfflineCashTopUpRequestV1,
        ...
        request
            .attach_mint_authorization(OfflineCashMintAuthorizationV1 {
                version: OFFLINE_CASH_WIRE_VERSION_V1,
                statement,
    ...
    fn redemption_request() -> OfflineCashRedemptionRequestV1 {
        let network_id = network();
        let asset = asset();
        let asset_incarnation = asset_incarnation(1);
        let lifecycle = crate::offline::OfflineCashLifecycleBindingV1 {
            version: OFFLINE_CASH_WIRE_VERSION_V1,
            network_id,
            protocol_version: OFFLINE_CASH_WIRE_VERSION_V1,
            suite_id: [0x10; 32],
            ...
            policy_epoch: 1,
            operation_kind: crate::offline::OfflineCashOperationKindV1::RedeemSplit,
            request_id: [0; 32],
            acceptance_ticket_id: [0; 32],
            credit_id: [0; 32],
            ciphertext_digest: [0; 32],
        };
        let commit_evidence = crate::offline::OfflineCashCommitEvidenceV1::TrustedTime(
            crate::offline::OfflineCashTrustedCommitTimeV1 {
                time_evidence_commitment: [0x13; 32],
            },
        );
        let statement = crate::offline::OfflineCashRedemptionStatementV1 {
            version: OFFLINE_CASH_WIRE_VERSION_V1,
            lifecycle,
            amount: 12_000,
        ...
        let outbox_reservation = crate::offline::OfflineCashOutboxReservationV1 {
            reservation_id: [0x15; 32],
            operation_kind: crate::offline::OfflineCashOperationKindV1::RedeemSplit,
            reserved_outbox_bytes: crate::offline::OFFLINE_CASH_REDEMPTION_OUTBOX_MIN_BYTES_V1,
            issued_at_ms: 8_000,
            expires_at_ms: 10_000,
        };
        let unsealed_certificate = crate::offline::OfflineCashCommitCertificateV1 {
            version: OFFLINE_CASH_WIRE_VERSION_V1,
            certificate_id: [0; 32],
        ...
        let terminal_body = crate::offline::OfflineCashHardwareTerminalBodyV1 {
        ...
        let commit_certificate_digest = digest_encoded(
            b"iroha:offline-cash:v1:commit-certificate",
            &commit_certificate,
        )
        ...
        let voucher = OfflineCashRedemptionVoucherV1 {
            version: OFFLINE_CASH_WIRE_VERSION_V1,
            statement,
            proof: crate::offline::OfflineCashCommitWrapperProofV1 {
                version: OFFLINE_CASH_WIRE_VERSION_V1,
                ...
                eq_proof: vec![0xA1; 128],
                ep_proof: vec![0xB2; 128],
                eq_history: vec![0xC3; crate::offline::OFFLINE_CASH_HISTORY_ACCUMULATOR_BYTES_V1],
                ep_history: vec![0xD4; crate::offline::OFFLINE_CASH_HISTORY_ACCUMULATOR_BYTES_V1],
            },
            commit_certificate,
    ...
    fn reserve_receipt(kind: OfflineCashOperationKindV1) -> OfflineCashReserveReceiptV1 {
        let network_id = network();
        let asset = asset();
        let asset_incarnation = asset_incarnation(1);
        OfflineCashReserveReceiptV1 {
            version: OFFLINE_CASH_CHAIN_VERSION_V1,
            operation_id: [1; 32],
            kind,
            request_digest: [2; 32],
            mint_statement_digest: if kind == OfflineCashOperationKindV1::TopUp {
                [4; 32]
            } else {
                [0; 32]
            },
            ...
            total_topups: 100,
            total_redemptions: if kind == OfflineCashOperationKindV1::Redemption {
                50
            } else {
                0
            },
    ...
    fn mint_lifecycle_binds_randomized_credential_and_ciphertext() {
        ...
        assert_eq!(
            ...
            crate::offline::OfflineCashOperationKindV1::MintFold
        );
        ...
        assert_eq!(
            ...
            offline_cash_ciphertext_digest_v1(&request.encrypted_credit)
        );
        ...
        assert_eq!(
            ...
            offline_cash_ciphertext_digest_v1(&changed_ciphertext.encrypted_credit)
        );
        assert_ne!(
            changed_statement.lifecycle.ciphertext_digest,
            statement.lifecycle.ciphertext_digest
    ...
    fn top_up_and_reserve_pool_are_bound_to_asset_incarnation() {
        ...
        next_incarnation.liability_pool_id = offline_cash_liability_pool_id_v1(
        ...
        let mut mismatched_receipt = reserve_receipt(OfflineCashOperationKindV1::TopUp);
    ...
    fn redemption_exposes_only_lifecycle_and_terminal_conflict_key() {
        ...
        assert_eq!(
            ...
            crate::offline::OfflineCashOperationKindV1::RedeemSplit
        );
        assert_eq!(
    ...
    fn requests_and_receipts_roundtrip_canonically() {
        ...
        let decoded: OfflineCashTopUpRequestV1 =
    ...
    fn reserve_receipt_uses_checked_conservation() {
        let mut receipt = reserve_receipt(OfflineCashOperationKindV1::Redemption);
        receipt.total_redemptions = receipt.total_topups + 1;
        assert_eq!(
            receipt.validate(),
            Err(OfflineCashIsiValidationErrorV1::ReserveUnderflow)
        );

        let mut receipt = reserve_receipt(OfflineCashOperationKindV1::TopUp);
    ...
    fn reserve_receipt_chain_uses_an_opaque_cas_head_without_a_revision() {
        let mut first = reserve_receipt(OfflineCashOperationKindV1::TopUp);
        ...
        let mut next = reserve_receipt(OfflineCashOperationKindV1::TopUp);
        ...
        let mut unchained_redemption = reserve_receipt(OfflineCashOperationKindV1::Redemption);
    ...
    fn reserve_receipt_authority_rejects_forged_deltas_and_predecessors() {
        ...
        let mut top_up = reserve_receipt(OfflineCashOperationKindV1::TopUp);
        ...
        let mut redemption = reserve_receipt(OfflineCashOperationKindV1::Redemption);
    ...
    fn reserve_receipt_authority_uses_checked_addition() {
        let mut saturated = reserve_receipt(OfflineCashOperationKindV1::TopUp);
        ...
        let mut overflowing = reserve_receipt(OfflineCashOperationKindV1::TopUp);
    ...
    fn receipt_witness_requires_exact_key_and_256_levels() {
        ...
        let witness = OfflineCashReserveReceiptWitnessV1 {
            key: OfflineCashReserveReceiptWitnessV1::expected_key(receipt.operation_id),
            receipt,
            siblings: vec![Hash::new([]); OFFLINE_CASH_RESERVE_RECEIPT_WITNESS_SIBLINGS_V1],
        };
    ...
    fn operation_status_shape_is_exact() {
        let pending = OfflineCashOperationStatusV1 {
            version: OFFLINE_CASH_CHAIN_VERSION_V1,
            operation_id: [1; 32],
        ...
        let rejected = OfflineCashOperationStatusV1 {
            state: OfflineCashOperationStateV1::Rejected,
            rejection: Some(OfflineCashOperationRejectionV1 {
                code: OfflineCashOperationRejectionCodeV1::ReserveUnderflow,
                detail_digest: [2; 32],
            }),
            ..pending.clone()
        };
        ...
        let invalid = OfflineCashOperationStatusV1 {
            state: OfflineCashOperationStateV1::Pending,
        ...
        assert_eq!(
            invalid.validate(),
            Err(OfflineCashIsiValidationErrorV1::InvalidStatus)
        );
    }

    #[test]
    fn instruction_registry_uses_only_offline_cash_v1_ids() {
        let top_up = TopUpOfflineCashV1::new(top_up_request()).expect("top-up instruction");
        let registry = crate::isi::InstructionRegistry::new()
            .register_with_id_slice::<TopUpOfflineCashV1>(TopUpOfflineCashV1::WIRE_ID)
            .register_with_id_slice::<RedeemOfflineCashV1>(RedeemOfflineCashV1::WIRE_ID);
        let (payload, flags) = norito::codec::encode_with_header_flags(&top_up);
        let framed =
            norito::core::frame_bare_with_header_flags::<TopUpOfflineCashV1>(&payload, flags)
                .expect("frame top-up instruction");
        let decoded = crate::isi::InstructionRegistry::decode(
            &registry,
            TopUpOfflineCashV1::WIRE_ID,
            &framed,
        )
