...
impl RwaParentRef {
    /// Construct a provenance edge with a nominal non-negative quantity.
    pub fn new(rwa: RwaId, quantity: impl Into<Quantity>) -> Self {
        Self {
            rwa,
...
impl Rwa {
    ...
    pub fn available_quantity(&self) -> Option<Quantity> {
        self.quantity.checked_sub(&self.held_quantity).ok()
    }
}

impl NewRwa {
    /// Construct a new RWA registration payload with a nominal quantity.
    #[allow(clippy::too_many_arguments)]
    pub fn new(
        domain: DomainId,
        quantity: impl Into<Quantity>,
...
mod tests {
    ...
    #[test]
    fn negative_numeric_payloads_cannot_decode_as_durable_rwa_quantities() {
        let encoded = ForgedRwaParentRef {
            rwa: rwa_id(),
            quantity: Numeric::new(-1_i32, 0),
        }
        .encode();
        assert!(
            RwaParentRef::decode(&mut encoded.as_slice()).is_err(),
            "a negative parent contribution must not cross the nominal RWA boundary"
        );

        let encoded = ForgedRwaData {
            quantity: Numeric::new(-1_i32, 0),
            spec: NumericSpec::integer(),
            primary_reference: "forged".to_owned(),
            status: None,
        ...
        .encode();
        assert!(
            RwaData::decode(&mut encoded.as_slice()).is_err(),
            "a negative stored lot quantity must not cross the nominal RWA boundary"
        );
    }
}
