...
fn balance_key_is_independent_of_chain_display_and_ambient_layout() {
    ...
    for discriminant in [0, 369, 753, 65_535] {
        ...
        for flags in [0, 1, 2, 3, 4, 5, 6, 7, 0x1b, 0x3f] {
            ...
            // The guard masks unsupported bits before installing the ambient layout.
            let ambient_flags = norito::core::effective_decode_flags();
            assert_eq!(transfer_balance_key(&asset, &account).unwrap(), expected);
            assert_eq!(norito::core::effective_decode_flags(), ambient_flags);
        }
    }
    assert_eq!(
        displays.len(),
