//! Shared operator/native bootstrap codec, threshold, scope and replay checks.
...
#[test]
fn canonical_package_roundtrips_and_authenticates() {
    let (_, policy, package) = fixture();
    let encoded = archive(&package);
    let decoded = KagemushaMobileBootstrapPackageV1::decode_canonical_exact(&encoded).unwrap();
    assert_eq!(decoded, package);
    assert_ne!(decoded.authenticate(&pins(&policy)).unwrap(), [0; 32]);
    let checkpoint: KagemushaMobileBootstrapCheckpointV1 =
        norito::decode_canonical(&norito::encode_canonical(&package.checkpoint).unwrap()).unwrap();
    assert_eq!(checkpoint, package.checkpoint);
    ...
    approval.verify(&checkpoint, &policy).unwrap();
}

#[test]
fn partial_approval_does_not_authenticate_a_threshold_package() {
    let (_, policy, mut package) = fixture();
