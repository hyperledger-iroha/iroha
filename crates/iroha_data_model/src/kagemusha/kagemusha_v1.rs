...
pub const KAGEMUSHA_INBOX_STAGE_MIN_BYTES_V1: u32 =
    durable_budget_component_from_usize(KAGEMUSHA_PAYMENT_MAX_BYTES_V1)
        + KAGEMUSHA_INBOX_STAGING_METADATA_MAX_BYTES_V1
        + durable_budget_component_from_usize(KAGEMUSHA_ACKNOWLEDGEMENT_MAX_BYTES_V1);
...
pub const KAGEMUSHA_PREPARED_TRANSITION_PAIRED_PROOF_MAX_BYTES_V1: u32 =
    durable_budget_component_from_usize(KAGEMUSHA_PAIRED_PROOF_MAX_BYTES_V1);
...
const fn durable_budget_component_from_usize(value: usize) -> u32 {
    assert!(value <= u32::MAX as usize);
    // The assertion makes the four-byte projection exact on every supported
    // word size, including during evaluation of the published constants.
...
const fn checked_outbox_budget_v1(canonical_envelope_max_bytes: usize) -> u32 {
    let parts = [
        ...
        durable_budget_component_from_usize(KAGEMUSHA_COMMIT_CERTIFICATE_MAX_BYTES_V1),
        KAGEMUSHA_REDEMPTION_PROOF_MAX_BYTES_V1,
        durable_budget_component_from_usize(canonical_envelope_max_bytes),
...
#[norito(deny_unknown_fields)]
#[derive(norito::NoritoSchema)]
#[norito_schema(name = "iroha_data_model::kagemusha::kagemusha_v1::KagemushaOperationKindV1")]
...
#[derive(norito::NoritoSchema)]
#[norito_schema(name = "iroha_data_model::kagemusha::kagemusha_v1::KagemushaCommitEvidenceV1")]
...
pub fn kagemusha_mint_authorization_context_capacity_v1(
    ...
    let paired_payload = 1_358_usize
        ...
        .and_then(|length| length.checked_add(canonical_compact_length_width_v1(companion_payload)))
        .ok_or_else(|| invalid(field))?;
...
#[cfg(test)]
mod captured_kagemusha_v1_schema_tests;
