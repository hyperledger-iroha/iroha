...
fn saturated_delivered_panel_notifications_with_probe(
    ...
    let (awaiting, _) = awaiting_acceptance_snapshot(2, finalized_block_hash, governance.clone());
    ...
    open_test_orchestrator!(orchestrator = bounds.clone(); test_runtime_deps!(checkpoint_store.clone(); Arc::new(MockSubmitter::new(ModerationSubmissionLookupV1::Unknown)); reader.clone(); Arc::new(MockHandoffSink::default()); publication_sink.clone(); Arc::new(MockPanelNotificationSink::default()); archive_dependency); "orchestrator");
    orchestrator.reconcile().expect("queue assignments");
    let sink = MockPanelNotificationSink::default();
    let mut assignments = orchestrator
        .claim_panel_notifications([0xA1; 32], 1_000, 3)
        .expect("claim assignments");
    assert_eq!(assignments.len(), 3);
    if sort_assignments {
        assignments.sort_by_key(|claim| claim.notification.notification_id);
    }
...
fn audited_two_generation_panel_notification_archive(
    ...
    let first = compact_panel_receipts!(fixture.orchestrator, 2; "first archive compaction"; "first archive head");
    assert!(publish_panel_archive!(fixture.orchestrator; "publish first archive head"));
    let second = compact_panel_receipts!(fixture.orchestrator, 1; "second archive compaction"; "second archive head");
    assert!(publish_panel_archive!(fixture.orchestrator; "publish second archive head"));
    let trusted = audit_panel_archive!(fixture.orchestrator, MODERATION_PANEL_NOTIFICATION_ARCHIVE_AUDIT_PAGE_MAX_V1; "establish trusted archive progress");
    ...
    assert!(
        panel_archive_health!(fixture.orchestrator; "trusted archive health").archive_is_fresh()
    );
...
fn panel_notification_capacity_recovers_only_after_exact_signed_archive_readback() {
    ...
    unpack_saturated_fixture!(saturated_delivered_panel_notifications(&temp, "panel-capacity-archive.norito"); governance, reader, checkpoint_store, archive, orchestrator,);
    ...
    let head =
        compact_panel_receipts!(orchestrator, 2; "retry exact archived batch"; "archive head");
    ...
    assert!(publish_panel_archive!(orchestrator; "publish sealed archive head"));
    orchestrator
        .reconcile()
...
fn panel_notification_archive_publishes_audits_and_rotates_signers() {
    let temp = tempfile::tempdir().expect("tempdir");
    unpack_saturated_fixture!(saturated_delivered_panel_notifications_with_probe(&temp, "panel-archive-rotation.norito", [0x2B; 32], true, "terminal delivery receipt", None); mut bounds, reader, checkpoint_store, archive, publication_sink, orchestrator,);
    let runtime_deps = || test_runtime_deps!(checkpoint_store.clone(); Arc::new(MockSubmitter::new(ModerationSubmissionLookupV1::Unknown)); reader.clone(); Arc::new(MockHandoffSink::default()); publication_sink.clone(); Arc::new(MockPanelNotificationSink::default()); archive.clone());
    let first =
        compact_panel_receipts!(orchestrator, 2; "first archive compaction"; "first archive head");
    ...
    let unpublished = panel_archive_health!(orchestrator; "unpublished archive health");
    ...
    assert!(publish_panel_archive!(orchestrator; "publish first head"));
    assert!(!publish_panel_archive!(orchestrator; "idempotent empty publication replay"));
    assert_eq!(publication_sink.published_archive_head_count(), 1);
    let first_audit = audit_panel_archive!(orchestrator, MODERATION_PANEL_NOTIFICATION_ARCHIVE_AUDIT_PAGE_MAX_V1; "complete first audit sweep");
    assert_eq!(first_audit.verified_heads, 1);
    assert_eq!(first_audit.last_completed_generation, 1);
    assert!(first_audit.cycle_complete);
    assert!(panel_archive_health!(orchestrator; "fresh first archive health").archive_is_fresh());
    ...
    open_test_orchestrator!(rotated = bounds.clone(); runtime_deps(); "rotated signer");
    let epochs = rotated
        .panel_notification_archive_signer_epochs()
    ...
    let second = compact_panel_receipts!(rotated, 2; "post-rotation archive compaction"; "second archive head");
    ...
    assert!(publish_panel_archive!(rotated; "publish rotated head"));
    assert_eq!(publication_sink.published_archive_head_count(), 2);
    let second_audit = audit_panel_archive!(rotated, MODERATION_PANEL_NOTIFICATION_ARCHIVE_AUDIT_PAGE_MAX_V1; "audit both signer epochs");
    ...
    assert!(panel_archive_health!(rotated; "fresh rotated archive health").archive_is_fresh());
...
fn panel_notification_archive_full_history_audit_resumes_after_restart() {
    ...
    let incomplete_health =
        panel_archive_health!(fixture.orchestrator; "incomplete full-history health");
    ...
    unpack_saturated_fixture!(fixture; bounds, reader, checkpoint_store, archive, publication_sink, orchestrator,);
    drop(orchestrator);
    open_test_orchestrator!(restarted = bounds; test_runtime_deps!(checkpoint_store; Arc::new(MockSubmitter::new(ModerationSubmissionLookupV1::Unknown)); reader; Arc::new(MockHandoffSink::default()); publication_sink; Arc::new(MockPanelNotificationSink::default()); archive); "restart with the sealed full-history cursor");
    assert_eq!(
        panel_archive_health!(restarted; "restarted incomplete full-history health"),
        incomplete_health
    );
    let completed =
        audit_panel_archive!(restarted, 1; "resume and complete the sealed full-history audit");
    ...
    let completed_health = panel_archive_health!(restarted; "completed full-history health");
    assert_eq!(
...
fn panel_notification_archive_full_history_corrupt_predecessor_fails_closed_after_restart() {
    ...
    assert!(matches!(
        ModerationOrchestratorV1::open(
            bounds,
            test_runtime_deps!(checkpoint_store.clone(); Arc::new(MockSubmitter::new(ModerationSubmissionLookupV1::Unknown,)); reader; Arc::new(MockHandoffSink::default()); publication_sink; Arc::new(MockPanelNotificationSink::default()); archive.clone()),
        ),
        Err(ModerationOrchestratorError::PanelNotificationArchiveInvalid)
    ));
...
fn panel_notification_archive_full_history_invalid_bounds_preserve_trusted_progress() {
    ...
    let trusted_health = panel_archive_health!(fixture.orchestrator; "trusted archive health");
    ...
    for maximum_heads in [0, 17] {
        ...
        assert_eq!(
            panel_archive_health!(fixture.orchestrator; "trusted health after rejected bound"),
            trusted_health
        );
...
fn panel_notification_archive_callbacks_run_without_the_state_mutex() {
    ...
    unpack_saturated_fixture!(saturated_delivered_panel_notifications_with_probe(&temp, "panel-archive-reentrant.norito", [0x29; 32], false, "finalize assignment", Some(probe.clone())); orchestrator,);
    let orchestrator = Arc::new(orchestrator);
    probe.attach(&orchestrator);
    let head =
        compact_panel_receipts!(orchestrator, 1; "archive outside state mutex"; "archive head");
    assert!(publish_panel_archive!(orchestrator; "publication outside state mutex"));
    assert_eq!(
        orchestrator
            .panel_notification_archive_head()
...
fn panel_notification_archive_rejects_corrupt_signature_rollback_and_predecessor_substitution() {
    let temp = tempfile::tempdir().expect("tempdir");
    unpack_saturated_fixture!(saturated_delivered_panel_notifications(&temp, "panel-archive-adversarial.norito"); archive, orchestrator,);
    let first = compact_panel_receipts!(orchestrator, 1; "first compaction"; "first head");
    let first_bytes = archive.artifact(first.operation_id);
    publish_panel_archive!(orchestrator; "publish first archive head");
    let second = compact_panel_receipts!(orchestrator, 1; "second compaction"; "second head");
    publish_panel_archive!(orchestrator; "publish second archive head");
...
fn panel_notification_archive_crash_boundary_replays_exact_batch_after_restart_with_smaller_hint() {
    let temp = tempfile::tempdir().expect("tempdir");
    unpack_saturated_fixture!(saturated_delivered_panel_notifications(&temp, "panel-archive-crash.norito"); bounds, reader, checkpoint_store, archive, orchestrator,);
    ...
    drop(orchestrator);
    open_test_orchestrator!(restarted = bounds; test_runtime_deps!(checkpoint_store; Arc::new(MockSubmitter::new(ModerationSubmissionLookupV1::Unknown)); reader; Arc::new(MockHandoffSink::default()); Arc::new(MockHandoffSink::default()); Arc::new(MockPanelNotificationSink::default()); archive.clone()); "restart from pre-prune sealed checkpoint");
    let recovered = compact_panel_receipts!(restarted, 1; "replay exact archived batch despite a smaller page-size hint"; "recovered archive head");
...
fn panel_notification_archive_conflicting_replica_is_fenced_by_sealed_checkpoint_cas() {
    let temp = tempfile::tempdir().expect("tempdir");
    unpack_saturated_fixture!(saturated_delivered_panel_notifications(&temp, "panel-archive-replica-a.norito"); mut bounds, reader, checkpoint_store, archive, orchestrator: first,);
    bounds.checkpoint_path = temp
        .path()
        .canonicalize()
        .expect("canonical tempdir")
        .join("panel-archive-replica-b.norito");
    open_test_orchestrator!(second = bounds; test_runtime_deps!(checkpoint_store; Arc::new(MockSubmitter::new(ModerationSubmissionLookupV1::Unknown)); reader; Arc::new(MockHandoffSink::default()); Arc::new(MockHandoffSink::default()); Arc::new(MockPanelNotificationSink::default()); archive.clone()); "open second replica at the same sealed source checkpoint");
    let committed =
        compact_panel_receipts!(first, 1; "first replica compaction"; "first replica head");
    assert_eq!(
...
fn panel_notification_claims_recover_crashes_and_finalize_one_stable_receipt() {
    ...
    orchestrator_fixture!(orchestrator; reader = Arc::new(MockSnapshotReader::new(snapshot)); submitter = Arc::new(MockSubmitter::new(ModerationSubmissionLookupV1::Unknown)); checkpoint = config(&temp, "panel-crash-recovery.norito"); => checkpoint.clone(); deps(Arc::clone(&reader), Arc::clone(&submitter)); "orchestrator");
    ...
    drop(orchestrator);
    open_test_orchestrator!(restarted = checkpoint; deps(reader, submitter); "restart with durable claims");
    assert!(
        restarted
...
fn panel_notification_backoff_poison_and_retry_exhaustion_are_bounded() {
    ...
    let orchestrator = isolated_orchestrator!(config(&temp, "panel-retry.norito"); snapshot.clone(); MockSubmitter::new(ModerationSubmissionLookupV1::Unknown); "retry orchestrator");
    ...
    let poison = isolated_orchestrator!(config(&temp, "panel-poison.norito"); snapshot; MockSubmitter::new(ModerationSubmissionLookupV1::Unknown); "poison orchestrator");
...
fn panel_notification_claim_inputs_tokens_and_clock_are_fail_closed() {
    ...
    bounds.max_handoffs = 3;
    let orchestrator = isolated_orchestrator!(bounds; snapshot; MockSubmitter::new(ModerationSubmissionLookupV1::Unknown); "orchestrator");
    orchestrator.reconcile().expect("queue notifications");
    assert_eq!(
...
fn panel_notification_checkpoint_tampering_and_old_versions_fail_closed() {
    ...
    orchestrator_fixture!(orchestrator; reader = Arc::new(MockSnapshotReader::new(snapshot)); submitter = Arc::new(MockSubmitter::new(ModerationSubmissionLookupV1::Unknown)); bounds = config(&temp, "panel-tamper.norito"); => bounds.clone(); deps(Arc::clone(&reader), Arc::clone(&submitter)); "orchestrator");
    orchestrator.reconcile().expect("queue notifications");
    orchestrator
        .claim_panel_notifications([0xA1; 32], 1_000, 3)
...
fn panel_notification_source_provenance_mismatch_is_rejected() {
    ...
    let orchestrator = isolated_orchestrator!(config(&temp, "panel-provenance.norito"); snapshot; MockSubmitter::new(ModerationSubmissionLookupV1::Unknown); "orchestrator");
    assert!(matches!(
        orchestrator.reconcile(),
...
fn panel_notification_scan_rejects_cross_snapshot_event_gaps() {
    ...
    orchestrator_fixture!(orchestrator; reader = Arc::new(MockSnapshotReader::new(awaiting)); submitter = Arc::new(MockSubmitter::new(ModerationSubmissionLookupV1::Unknown)); => config(&temp, "panel-event-gap.norito"); deps(Arc::clone(&reader), submitter); "orchestrator");
