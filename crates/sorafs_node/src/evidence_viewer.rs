...
//! bearer grants, and evidence bytes outside durable state. The checkpoint
//! contains only finalized authorization anchors, one-way token/assertion
//! digests, and Ed25519-authenticated payload-free receipts. Runtime providers
//! are injected by the embedding daemon; there is no file, environment, or
//! in-process key fallback. Production audit/readback comes from the exact
//! signed receipt checkpoint and [`EvidenceViewerTransparencyProjectionV1`];
//! the older `NodeHandle` session/access registry is intentionally not fed by
//! this service.
...
use crate::{
    ...
    ModerationEvidenceViewerSessionRecord, ModerationQuarantineObjectRangePayload,
    ModerationQuarantineObjectRecord, NodeHandle, decode_local_checkpoint_canonical,
    moderation::{
...
impl Default for EvidenceViewerStateV1 {
    fn default() -> Self {
        Self {
...
impl EvidenceViewerServiceV1 {
    /// Open and verify the canonical durable service checkpoint.
    ///
    /// # Errors
    ///
    /// Fails closed when configuration/dependency identities do not match, the
    /// checkpoint cannot be read, or any receipt, chain link, bound, or
    ...
    pub fn open(
        ...
        {
            Some(bytes) => {
                let sequence_limit = config
                    .max_challenges
                    .saturating_add(config.max_sessions)
                    .saturating_add(config.max_receipts)
                    .saturating_add(config.max_idempotency_records)
                    .saturating_add(config.max_sessions.saturating_mul(4));
                let envelope = decode_local_checkpoint_canonical::<
                    EvidenceViewerCheckpointEnvelopeV1,
                >(
                    &bytes, config.checkpoint_max_bytes, sequence_limit
                )
                .map_err(|_| EvidenceViewerErrorV1::InvalidCheckpoint)?;
                let (checkpoint, checkpoint_anchor) =
                    verify_checkpoint_envelope(&config, envelope)?;
                (
                    state_from_checkpoint(&config, checkpoint, checkpoint_anchor)?,
                    false,
                )
            }
            None => (EvidenceViewerStateV1::default(), true),
        };
        let service = Self {
            config,
            deps,
    ...
    pub fn issue_challenge(
        ...
        let binding_digest = challenge_binding_digest(
            ...
            authorization.finalized_at_unix_ms,
        );
        let request_digest =
    ...
    pub fn create_session(
        ...
        let binding_digest = challenge_binding_digest(
            ...
            object.payload_digest,
            purpose_digest,
            challenge_record.policy_digest,
            challenge_record.finalized_height,
            challenge_record.finalized_block_hash,
            challenge_record.finalized_at_unix_ms,
        );
    ...
    fn reconcile_erasure_intents(&self) -> Result<(), EvidenceViewerErrorV1> {
        ...
        for intent in intents {
            let erasure_commit_digest = self
                .deps
                .erasure
                .erase(
                    intent.operation_id,
...
mod tests {
    ...
    impl EvidenceViewerErasureBoundaryV1 for MockErasureBoundary {
        fn erase(
            ...
            let mut commits = self
                ...
                .lock()
                .map_err(|_| EvidenceViewerExternalErrorV1::Unavailable)?;
            if let Some(commit_digest) = commits.get(&operation_id) {
    ...
    impl ModerationQuarantineKeyWrapper for TestQuarantineKeyWrapper {
        ...
        fn unwrap_dek(
            ...
            if key_id != self.key_id {
                return Err("unknown test key handle".to_owned());
            }
            SymmetricEncryptor::<ChaCha20Poly1305>::new_with_key(self.key)
                .map_err(|error| error.to_string())?
                .decrypt(
    ...
    impl EvidenceViewerFixture {
        ...
        fn open(&self) -> EvidenceViewerServiceV1 {
            EvidenceViewerServiceV1::open(self.config.clone(), self.deps.clone(), self.node.clone())
    ...
    fn missing_checkpoint_capacity_fails_closed_before_service_exposure() {
        ...
        assert_eq!(
            EvidenceViewerServiceV1::open(config, fixture.deps.clone(), fixture.node.clone())
    ...
    #[test]
    fn access_checkpoint_failure_rolls_back_receipt_grant_and_anchor() {
        let fixture = EvidenceViewerFixture::new();
        let service = fixture.open();
        let challenge = fixture.issue_challenge(
            &service,
        ...
        let before = service
            .transparency_projection(checkpoint_digest, None, 16)
            .expect("projection before checkpoint sabotage");
        let checkpoint_bytes =
            std::fs::read(&fixture.config.checkpoint_path).expect("read valid checkpoint");
        std::fs::remove_file(&fixture.config.checkpoint_path).expect("remove valid checkpoint");
        std::fs::create_dir(&fixture.config.checkpoint_path).expect("block checkpoint target");
        ...
        assert_eq!(
            service
                ...
                .expect_err("failed checkpoint must suppress access response"),
            EvidenceViewerErrorV1::CheckpointUnavailable
        );
        assert_eq!(
            service
        ...
        std::fs::remove_dir(&fixture.config.checkpoint_path).expect("remove checkpoint blocker");
        write_local_checkpoint_atomic_bounded(
            &fixture.config.checkpoint_path,
            &checkpoint_bytes,
            fixture.config.checkpoint_max_bytes,
        )
        .expect("restore signed checkpoint");
        drop(service);
        let restarted = fixture.open();
        assert_eq!(
            restarted
