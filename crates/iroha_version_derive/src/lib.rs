...
///
/// Links the item to a generated versioned container. For a JSON-only
/// container, use [`version_with_json()`].
///
/// ### Arguments
...
///
/// Adds support for both Norito binary and JSON serialization. To declare only
/// with JSON support, use [`declare_versioned_with_json`](`declare_versioned_with_json()`).
///
...
fn impl_declare_versioned(
    ...
    let norito_impl = if with_norito {
        impl_decode_versioned(enum_name, &args.generics)
    } else {
        quote!()
    };
    let norito_derives = if with_norito {
        quote!(norito::derive::Encode, norito::derive::Decode,)
    } else {
        quote!()
    };
    let norito_variant_attributes: Vec<_> = version_numbers
        .iter()
        .map(|version| {
            if with_norito {
