...
fn gov_vote_zk_against_mock() {
    ...
    assert_eq!(
        entry.get("salt_hex").and_then(norito::json::Value::as_str),
        Some(salt_hex.as_str())
    );
    assert!(
        entry.contains_key("payload_fingerprint_hex"),
        "payload fingerprint annotation missing"
...
fn gov_vote_zk_subcommand_emits_summary_and_json() {
    ...
    let summary_text = String::from_utf8_lossy(&summary.stdout);
    let expected_summary = format!(
        "vote-zk: election_id=ref-zk ok=true accepted=true instrs=1 fingerprint={fingerprint} owner={owner_str} amount={amount} duration_blocks={duration_blocks} direction={direction} salt_hex={salt_hex}"
    );
    assert_eq!(summary_text.trim_end(), expected_summary);
