...
* `-i`, `--id <ID>` — Domain name
* `-f`, `--from <FROM>` — Source account identifier (canonical Katakana i105 literal)
* `-t`, `--to <TO>` — Destination account identifier (canonical Katakana i105 literal)
...
* `-i`, `--id <ID>` — Account identifier (canonical Katakana i105 literal)
* `--limit <LIMIT>` — Maximum number of items to return (server-side limit)
* `--offset <OFFSET>` — Offset into the result set (server-side offset)
...
* `-i`, `--id <ID>` — Canonical domainless account identifier for registration (canonical Katakana i105 literal)
...
* `--account <ACCOUNT>` — Account identifier (canonical Katakana i105), required with `--definition-alias`
...
* `--definition <DEFINITION>` — Canonical asset definition id (unprefixed Base58 address) used with `--account`
* `--definition-alias <DEFINITION_ALIAS>` — Asset definition alias (`<name>#<domain>.<dataspace>` or `<name>#<dataspace>`) used with `--account`
* `--account <ACCOUNT>` — Source account identifier (canonical Katakana i105), required with `--definition-alias`
* `--scope <SCOPE>` — Optional balance scope (`global` or `dataspace:<id>`)
* `-t`, `--to <TO>` — Destination account identifier (canonical Katakana i105 literal)
...
* `--authority <AUTHORITY>` — Account executing the trigger (canonical Katakana i105 literal)
...
* `--data-account <DATA_ACCOUNT>` — Data filter preset: events for an account (canonical Katakana i105 literal)
...
* `-s`, `--signatories <SIGNATORIES>` — List of signatories for the multisig account (canonical Katakana i105 literal)
...
* `--approver <ACCOUNT>` — Optional validator account IDs (canonical Katakana i105 account literals) authorizing the deployment alongside the authority
...
* `--owner <OWNER>` — Owner account id for plain voting mode (canonical Katakana i105 account literal; must equal transaction authority)
...
* `--authority <AUTHORITY>` — Authority `AccountId` for signing (canonical Katakana i105 account literal)
...
* `--missing <MISSING>` — Account id of the member to replace (canonical Katakana i105 account literal)
...
* `--authority <AUTHORITY>` — Authority account identifier (canonical Katakana i105 account literal)
...
* `--asset <ASSET_ID>` — Canonical unprefixed Base58 `AssetDefinitionId`
* `--from <ACCOUNT_ID>` — Account identifier to debit (canonical Katakana i105 account literal)
...
* `--asset <ASSET_ID>` — Canonical unprefixed Base58 `AssetDefinitionId`
* `--to <ACCOUNT_ID>` — Recipient account identifier to credit (canonical Katakana i105 account literal)
...
* `--validator <ACCOUNT_ID>` — Validator account identifier (canonical Katakana i105 account literal)
...
* `--authority <ACCOUNT_ID>` — Authority account identifier (canonical Katakana i105 account literal)
* `--private-key <HEX>` — Hex-encoded private key for signing
...
* `--host <ACCOUNT-ID>` — Host account identifier responsible for the call (canonical Katakana i105 account literal)
...
* `--gas-rate-per-minute <U64>` — Gas rate charged per minute (defaults to 0)
  ...
  Default value: `0`
* `--billing-account <ACCOUNT-ID>` — Optional billing account that will cover usage (canonical Katakana i105 account literal)
...
* `--participant <ACCOUNT-ID>` — Participant account joining the call (canonical Katakana i105 account literal)
...
* `--domain <DOMAIN-ID>` — Domain identifier hosting the call
* `--call-name <NAME>` — Call name within the domain
* `--participant <ACCOUNT-ID>` — Participant account leaving the call (canonical Katakana i105 account literal)
...
* `--domain <DOMAIN-ID>` — Domain identifier hosting the call
* `--call-name <NAME>` — Call name within the domain
* `--relay <ACCOUNT-ID>` — Relay account identifier being reported (canonical Katakana i105 account literal)
...
* `--selector <SELECTOR>` — Selector literal (`label.suffix`) in canonical Katakana i105 form
...
* `address` — Account address helpers (canonical Katakana i105 conversions)
...
* `<ADDRESS>` — Address literal to parse (canonical Katakana i105 or public key)
