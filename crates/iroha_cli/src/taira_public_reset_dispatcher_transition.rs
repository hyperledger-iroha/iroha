...
impl DispatcherTransition {
    ...
    pub(in super::super) fn run<W: Write>(&self, output: &mut W) -> Result<()> {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            let result = norito::json!({"schema": ("iroha.taira.dispatcher-transition-result.v1"), "plan_sha256": (self.expected_plan_sha256), "operation_id": (plan.operation_id), "candidate_commit": (plan.candidate.commit), "action": (match self.action { Action::Check => "check", Action::Apply => "apply", Action::Rollback => "rollback" }), "guards": (guard_rows), "guard_state": (if self.action == Action::Check { "candidate_target" } else { "observed_result" }), "runtime_and_history_preserved": (true), "ledger_mutated": (false)});
