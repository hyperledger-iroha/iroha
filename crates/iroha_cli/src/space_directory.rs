...
use norito::json::{self, JsonDeserialize, JsonSerialize, Map as JsonMap, Value as JsonValue};
use reqwest::{
...
fn load_manifest_from_sources(
    ...
    match (manifest_path, manifest_json_path) {
        ...
        (None, Some(path)) => {
            ...
            match norito::json::from_slice(&bytes) {
                Ok(manifest) => Ok(manifest),
                Err(primary_err) => {
                    let value: JsonValue = norito::json::from_slice(&bytes)
                        .wrap_err("manifest JSON could not be parsed")?;
                    let normalized = normalize_manifest_json(value)?;
                    manifest_from_normalized_json(&normalized).wrap_err_with(|| {
                        format!(
                            "manifest JSON could not be parsed as AssetPermissionManifest (legacy shape: {primary_err})"
                        )
                    })
                }
            }
...
fn normalize_manifest_json(mut value: JsonValue) -> Result<JsonValue> {
    let entries = value
        .as_object_mut()
        .ok_or_else(|| eyre!("manifest JSON must be an object"))?
        .get_mut("entries")
        .ok_or_else(|| eyre!("manifest JSON missing `entries` array"))?;
    let entries_array = entries
        .as_array_mut()
        .ok_or_else(|| eyre!("manifest `entries` must be an array"))?;
    for entry in entries_array {
        normalize_manifest_entry(entry)?;
    }
    Ok(value)
}

fn normalize_manifest_entry(entry: &mut JsonValue) -> Result<()> {
    let map = entry
        .as_object_mut()
        .ok_or_else(|| eyre!("manifest entry must be a JSON object"))?;
    if let Some(scope) = map.get_mut("scope") {
        normalize_manifest_scope(scope)?;
    }
    if let Some(effect) = map.get_mut("effect") {
        normalize_manifest_effect(effect)?;
    } else {
        return Err(eyre!("manifest entry missing `effect`"));
    }
    Ok(())
}

fn normalize_manifest_scope(scope: &mut JsonValue) -> Result<()> {
    let Some(scope_map) = scope.as_object_mut() else {
        return Err(eyre!("manifest scope must be a JSON object"));
    };

    if let Some(role) = scope_map.get_mut("role") {
        normalize_role_tag(role)?;
    }
    Ok(())
}

fn normalize_role_tag(role: &mut JsonValue) -> Result<()> {
    if matches!(role, JsonValue::Object(map) if map.contains_key("role")) {
        return Ok(());
    }
    if let Some(label) = role.as_str() {
        let mut map = JsonMap::new();
        map.insert("role".into(), JsonValue::from(label.to_owned()));
        map.insert("details".into(), JsonValue::Null);
        *role = JsonValue::Object(map);
        return Ok(());
    }
    Err(eyre!("manifest scope role must include a `role` tag"))
}

fn normalize_manifest_effect(effect: &mut JsonValue) -> Result<()> {
    let Some(effect_map) = effect.as_object_mut() else {
        return Err(eyre!("manifest effect must be a JSON object"));
    };

    if let Some(decision_value) = effect_map.get("decision") {
        let decision = decision_value
            .as_str()
            .ok_or_else(|| eyre!("effect `decision` must be a string"))?;
        if decision.eq_ignore_ascii_case("allow")
            && let Some(details) = effect_map.get_mut("details")
        {
            normalize_allowance(details)?;
        }
        return Ok(());
    }

    if let Some(details) = effect_map.remove("Allow") {
        let mut details = details;
        normalize_allowance(&mut details)?;
        *effect = tagged_decision("Allow", details);
        return Ok(());
    }
    if let Some(details) = effect_map.remove("Deny") {
        *effect = tagged_decision("Deny", details);
        return Ok(());
    }

    Err(eyre!(
        "manifest effect must include a `decision` tag or legacy `Allow`/`Deny` keys"
    ))
}

fn normalize_allowance(details: &mut JsonValue) -> Result<()> {
    let Some(map) = details.as_object_mut() else {
        return Err(eyre!("allowance must be a JSON object"));
    };
    if let Some(window) = map.get_mut("window") {
        normalize_allowance_window(window)?;
    }
    Ok(())
}

fn normalize_allowance_window(window: &mut JsonValue) -> Result<()> {
    if matches!(window, JsonValue::Object(map) if map.contains_key("window")) {
        return Ok(());
    }
    if let Some(label) = window.as_str() {
        let mut map = JsonMap::new();
        map.insert("window".into(), JsonValue::from(label.to_owned()));
        map.insert("details".into(), JsonValue::Null);
        *window = JsonValue::Object(map);
        return Ok(());
    }
    Err(eyre!("allowance window must include a `window` tag"))
}

fn tagged_decision(decision: &str, details: JsonValue) -> JsonValue {
    let mut map = JsonMap::new();
    map.insert("decision".into(), JsonValue::from(decision.to_owned()));
    map.insert("details".into(), details);
    JsonValue::Object(map)
}

fn manifest_from_normalized_json(value: &JsonValue) -> Result<AssetPermissionManifest> {
    let root = value
        .as_object()
        .ok_or_else(|| eyre!("manifest JSON must be an object"))?;
    let version = match root.get("version") {
        Some(raw) => match parse_u64_value(raw, "version")? {
            1 => ManifestVersion::V1,
            other => return Err(eyre!("unsupported manifest version {other}")),
        },
    ...
    let uaid_text = root
        ...
        .ok_or_else(|| eyre!("manifest `uaid` must be a string"))?;
    let uaid_hex = uaid_text.strip_prefix("uaid:").unwrap_or(uaid_text);
    let uaid =
        UniversalAccountId::from_str(uaid_hex).wrap_err("manifest `uaid` could not be parsed")?;
    let dataspace = parse_dataspace_id(root, "dataspace")?;
    let issued_ms = parse_u64_field(root, "issued_ms")?;
    let activation_epoch = parse_u64_field(root, "activation_epoch")?;
    let expiry_epoch = root
        .get("expiry_epoch")
        .map(|value| parse_u64_value(value, "expiry_epoch"))
        .transpose()?;
    ...
    for entry in entries_value {
        entries.push(parse_manifest_entry(entry, dataspace)?);
    }
...
fn parse_manifest_entry(
    ...
    let scope_value = map
        .get("scope")
        .ok_or_else(|| eyre!("manifest entry missing `scope`"))?;
    let effect_value = map
        .get("effect")
        .ok_or_else(|| eyre!("manifest entry missing `effect`"))?;

    let scope = parse_manifest_scope(scope_value, manifest_dataspace)?;
    let effect = parse_manifest_effect(effect_value)?;
    let notes = map
        .get("notes")
        .and_then(JsonValue::as_str)
        .map(str::to_owned);
...
fn parse_manifest_scope(
    value: &JsonValue,
    manifest_dataspace: DataSpaceId,
) -> Result<CapabilityScope> {
    let scope_map = value
        .as_object()
        .ok_or_else(|| eyre!("manifest `scope` must be an object"))?;
    let dataspace = scope_map
        .get("dataspace")
        .map(|value| parse_u64_value(value, "scope.dataspace").map(DataSpaceId::from))
        .transpose()?;
    let program = scope_map
        .get("program")
        .and_then(JsonValue::as_str)
        .map(|text| {
            SmartContractId::from_str(text)
                .wrap_err("scope `program` is not a valid smart contract id")
        })
        .transpose()?;
    let method = scope_map
        .get("method")
        .and_then(JsonValue::as_str)
        .map(|text| Name::from_str(text).wrap_err("scope `method` is not a valid name"))
        .transpose()?;
    let asset = scope_map
        .get("asset")
        .and_then(JsonValue::as_str)
        .map(|text| {
            AssetDefinitionId::from_str(text).wrap_err("scope `asset` is not a valid asset id")
        })
        .transpose()?;
    let role = scope_map.get("role").map(parse_amx_role).transpose()?;
...
fn parse_amx_role(value: &JsonValue) -> Result<AmxRole> {
    let Some(label) = value
        ...
        return Err(eyre!("scope `role` must be a string"));
    };

    match label {
        text if text.eq_ignore_ascii_case("Initiator") => Ok(AmxRole::Initiator),
        text if text.eq_ignore_ascii_case("Participant") => Ok(AmxRole::Participant),
        other => Err(eyre!("unsupported scope role `{other}`")),
    }
}

fn parse_manifest_effect(value: &JsonValue) -> Result<ManifestEffect> {
    let effect_map = value
        .as_object()
        .ok_or_else(|| eyre!("manifest `effect` must be an object"))?;
    let decision = effect_map
        .get("decision")
        .and_then(JsonValue::as_str)
        .ok_or_else(|| eyre!("manifest `effect` missing `decision`"))?;
    let details = effect_map
        .get("details")
        .ok_or_else(|| eyre!("manifest `effect` missing `details`"))?;

    match decision {
        text if text.eq_ignore_ascii_case("Allow") => {
            let details_map = details
                .as_object()
                .ok_or_else(|| eyre!("effect `details` must be an object for Allow"))?;
            let window_value = details_map
                .get("window")
                .ok_or_else(|| eyre!("allowance missing `window` field"))?;
            let window = parse_manifest_allowance_window(window_value)?;
            let max_amount = details_map
                .get("max_amount")
                .map(|value| parse_numeric_value(value, "max_amount"))
                .transpose()?;
            Ok(ManifestEffect::Allow(Allowance { max_amount, window }))
        }
        text if text.eq_ignore_ascii_case("Deny") => {
            let reason = details
                .as_object()
                .ok_or_else(|| eyre!("effect `details` must be an object for Deny"))?
                .get("reason")
                .and_then(JsonValue::as_str)
                .map(str::to_owned);
            Ok(ManifestEffect::Deny(DenyDirective { reason }))
        }
        other => Err(eyre!("unsupported manifest decision `{other}`")),
    }
}

fn parse_manifest_allowance_window(value: &JsonValue) -> Result<AllowanceWindow> {
    let Some(label) = value
        .as_object()
        ...
        return Err(eyre!("allowance `window` must be a string"));
    };

    match label {
        text if text.eq_ignore_ascii_case("PerSlot") => Ok(AllowanceWindow::PerSlot),
        text if text.eq_ignore_ascii_case("PerMinute") => Ok(AllowanceWindow::PerMinute),
        text if text.eq_ignore_ascii_case("PerDay") => Ok(AllowanceWindow::PerDay),
        other => Err(eyre!("unsupported allowance window `{other}`")),
    }
}

fn parse_u64_field(map: &JsonMap, field: &str) -> Result<u64> {
    let value = map
        .get(field)
        .ok_or_else(|| eyre!("manifest missing `{field}` field"))?;
    parse_u64_value(value, field)
}

fn parse_dataspace_id(map: &JsonMap, field: &str) -> Result<DataSpaceId> {
    parse_u64_field(map, field).map(DataSpaceId::from)
}

fn parse_u64_value(value: &JsonValue, field: &str) -> Result<u64> {
    if let Some(value) = value.as_u64() {
        return Ok(value);
    }
    if let Some(text) = value.as_str() {
        return text.parse::<u64>().wrap_err_with(|| {
            format!("manifest `{field}` must be an unsigned integer (found `{text}`)")
        });
    }
    Err(eyre!("manifest `{field}` must be an unsigned integer"))
}

fn parse_numeric_value(value: &JsonValue, field: &str) -> Result<Numeric> {
    if let Some(text) = value.as_str() {
        return Numeric::from_str(text).wrap_err_with(|| {
            format!("manifest `{field}` is not a valid numeric literal (`{text}`)")
        });
    }
    if let Some(number) = value.as_u64() {
        return Numeric::from_str(&number.to_string()).wrap_err_with(|| {
            format!("manifest `{field}` is not a valid numeric literal (`{number}`)")
        });
    }
    Err(eyre!(
        "manifest `{field}` must be encoded as a string or number"
    ))
}
