...
impl ManifestScaffoldProfileArgs {
    ...
    fn build_profile_value(
        ...
        let governance_issuer = self.governance_issuer.clone().unwrap_or_else(|| {
            "soraゴヂアヌゼマャグンロリカリヱザキュアチュゥトヺュサビセジゲキョスクノョボリピドヒュヷペキキュザホフツコオコ".to_owned()
        });
        ...
        let da_attesters = if self.da_attesters.is_empty() {
            vec![
                "soraゴヂアニヤナサヰイユヶサヲワニュスゥァヨワコモペバプボチョナソヒョニュニョムベイゴエホタフナナハカウセミカ".to_owned(),
                "soraゴヂアニヤチョゴヂヴテヒャルヺリヒョマピスヒヮキュゲッチョクヌツヌラハチャヌゴキョククエェボォエボツテヒトウカ".to_owned(),
            ]
        } else {
