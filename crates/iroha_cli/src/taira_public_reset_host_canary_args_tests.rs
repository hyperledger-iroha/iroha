...
fn coordinator_write_canary_argv_passes_child_validation_for_all_core_actions() {
    ...
    for scope in [
            ...
            for kind in &["onboarding", "faucet", "write_canary"] {
                for action in [
                    ...
                    .unwrap_or_else(|error| {
                        panic!("{context}: actual child parser failed: {error}")
                    });
                    ...
                    let (current, predecessor) =
                        crate::taira::validate_parent_write_canary_for_test(&child).unwrap_or_else(
                            |error| {
                                panic!(
                                    "{context}: child action/policy validation failed: {error:#}"
                                )
                            },
                        );
                    assert_eq!(
        ...
        if scope == super::super::QualificationScopeV1::CoreTestnet {
            let mut denied = 0;
            for phase in ["restart-wave-2", "restart-wave-3", "restart-wave-4"] {
                for kind in ["onboarding", "faucet", "write_canary"] {
                    for action in [
                        WriteCanaryChildAction::Prepare,
                        WriteCanaryChildAction::Submit,
                        ...
                        let error = transport
                            .write_canary_base_args(phase, kind, &idempotency_key, 120, action)
                            .expect_err(
                                "an unselected core restart wave cannot produce child argv",
                            );
                        assert!(
                            error
                                .to_string()
                                .contains("outside the candidate/public plan"),
                            "{phase}/{kind}/{action:?}: {error:#}"
                        );
                        denied += 1;
                    }
                }
            ...
            assert_eq!(
                denied, 27,
                "all extra core phases are denied before child launch"
            );
        }
    }
}
