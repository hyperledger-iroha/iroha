...
fn borrowed_fields_accept_payload_only_values_and_preserve_layouts() {
    for value in ["", "signing payload", "界\0"] {
        assert_same_payload(&borrowed_norito::String(value), &value.to_owned());
    }
    for values in [vec![], vec![PayloadOnly(0), PayloadOnly(u64::MAX)]] {
        assert_same_payload(&borrowed_norito::Vec(&values), &values);
    }
    for value in [None, Some(PayloadOnly(42))] {
        assert_same_payload(&borrowed_norito::Option(&value), &value);
    }
}
