...
pub(super) fn without_receipt_field(
    ...
    for field in 0..9 {
        ...
        let (len, prefix) = norito::core::read_len_from_slice_with_flags(&payload[offset..], flags)
            .expect("current schema field");
        offset += prefix + len;
        if field != omitted {
            shortened.extend_from_slice(&payload[start..offset]);
        }
    }
    assert_eq!(offset, payload.len(), "exact nine-field current schema");
    norito::core::frame_bare_with_header_flags::<SignerStreamTokenReceiptV1>(&shortened, flags)
        .expect("current schema missing-field frame")
}
