...
fn add_mod(a: u64, b: u64) -> u64 {
    ...
    u64::try_from(sum % u128::from(GOLDILOCKS_MODULUS)).expect("mod reduction fits u64")
}
...
fuzz_target!(|input: LookupInput| {
    ...
    let result = compute_lookup_grand_product(&selectors, &witnesses, gamma)
        .expect("fuzz input constructs canonical equal-length lookup columns");
    let expected = selectors
        .iter()
        .zip(witnesses.iter())
        .filter(|(selector, _)| **selector != 0)
        .fold(1, |accumulator, (_, witness)| {
            mul_mod(accumulator, add_mod(*witness, gamma))
        });
