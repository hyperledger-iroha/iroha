...
impl BatchSeed {
    fn into_batch(self) -> TransitionBatch {
        ...
        // This target exercises valid opaque-effect traces. Arbitrary bytes
        // under the reserved transfer transcript key describe a different,
        // typed input grammar and would turn expected decode rejection into a
        // fuzzer panic before the trace layout is reached.
        metadata.remove("transfer_transcripts");
...
fn process_seed(seed: BatchSeed) {
    ...
    let trace = build_trace(&batch).expect("bounded MetaSet batch must produce a valid trace");
