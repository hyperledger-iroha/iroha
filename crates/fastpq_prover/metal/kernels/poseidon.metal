...
// Canonical V1 six-lane frame hashing. Parameters are generated by fastpq_isi;
// this kernel does not reuse the older scalar permutation's fixed constants.
struct Digest384FrameArgsV1 {
    uint frame_count;
    uint word_count;
};
inline void digest384_permute_v1(thread ulong state[3], uint lane,
                                  device const ulong* parameters) {
    for (uint round = 0; round < 65; ++round) {
        for (uint column = 0; column < 3; ++column) {
            state[column] = add_mod(state[column], parameters[18 + (lane * 65 + round) * 3 + column]);
        }
        ...
        for (uint row = 0; row < 3; ++row) {
            mixed[row] = 0;
            for (uint column = 0; column < 3; ++column) {
                mixed[row] = add_mod(mixed[row], mul_mod(parameters[1188 + row * 3 + column], state[column]));
            }
        }
        for (uint column = 0; column < 3; ++column) state[column] = mixed[column];
    }
}
kernel void digest384_hash_frames_v1(
    device const ulong* words [[buffer(0)]],
    device const ulong* descriptors [[buffer(1)]],
    device const ulong* parameters [[buffer(2)]],
    device ulong* output [[buffer(3)]],
    constant Digest384FrameArgsV1& args [[buffer(4)]],
    uint tid [[thread_position_in_grid]]) {
    uint frame = tid / 6;
    uint lane = tid % 6;
    if (frame >= args.frame_count) return;
    ulong offset = descriptors[frame * 3];
    ulong length = descriptors[frame * 3 + 1];
    ulong lane_word = descriptors[frame * 3 + 2];
    if (length == 0 || (length & 1) != 0 || offset > args.word_count
        || length > args.word_count - offset || lane_word >= length) {
        output[tid] = FIELD_MODULUS;
        return;
    }
    ulong state[3];
    for (uint column = 0; column < 3; ++column) state[column] = parameters[lane * 3 + column];
    for (ulong index = 0; index < length; index += 2) {
        state[0] = add_mod(state[0], index == lane_word ? ulong(lane) : words[offset + index]);
        state[1] = add_mod(state[1], index + 1 == lane_word ? ulong(lane) : words[offset + index + 1]);
