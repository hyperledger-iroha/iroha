...
fn complete_retained_artifact(is_axt: bool) {
    ...
    let frame =
        std::fs::read(directory.join(format!("compact-shake-{label}-two-delta-{bundle_hash}.bin")))
    ...
    let path = directory.join(format!("compact-shake-{label}-model-artifact-{hash}.bin"));
