//! Explicit complete six-lane compact V1 proof with real public transfer facts.
//!
...
fn complete_candidate_transfer_verifies_after_private_witnesses_are_dropped() {
    ...
    // TODO: Replace this retired SHAKE artifact hash only with an actual fresh
    // canonical six-lane proof capture; source migration is not fixture evidence.
    assert_eq!(
        sha,
    ...
    let path = directory.join(format!("compact-v1-full-transfer-{sha}.bin"));
