...
use crate::{
    backend::{
        ...
        compact_bundle::{self as bundle, BundleLimits, ShakeAxtBundleWire, ShakeBundleWire},
        compact_protocol::shared_openings::prove_shake_shared,
...
fn retain_public_frame(label: &str, bytes: &[u8]) -> String {
    ...
    let path = dir.join(format!("compact-shake-{label}-{sha}.bin"));
...
fn complete_candidate_axt_facade_drops_private_data_before_verification() {
    ...
    assert_eq!(
        norito::core::with_decode_limits_scope(exact, || verify_shake_axt_transfer(
            &prepared,
            &expected,
            context(&fixture),
            &bytes,
    ...
    assert!(
        norito::core::with_decode_limits_scope(low, || verify_shake_axt_transfer(
            &prepared,
            &expected,
            context(&fixture),
            &bytes,
...
fn complete_candidate_bundle(axt: bool) {
    ...
    let verify = |bytes: &[u8], limits: BundleLimits| {
        if axt {
            bundle::verify_shake_axt_transfer_bundle(
                &prepared,
                &expected,
                context(&fixture),
                bytes,
            ...
            bundle::verify_shake_transfer_bundle(
                &prepared,
                &expected,
                bytes,
                limits,
    ...
    let (roots, frames) = if axt {
        let w: ShakeAxtBundleWire = norito::decode_canonical(&bytes).unwrap();
        (w.intermediate_roots, w.segments)
    } else {
        let w: ShakeBundleWire = norito::decode_canonical(&bytes).unwrap();
        (w.intermediate_roots, w.segments)
    };
    for kind in 0..5 {
        ...
        let corrupt = if axt {
            norito::encode_canonical(&ShakeAxtBundleWire {
                version: 1,
                intermediate_roots: roots,
                segments: frames,
            })
            ...
            norito::encode_canonical(&ShakeBundleWire {
                version: 1,
                intermediate_roots: roots,
                segments: frames,
            })
    ...
    if axt {
        ...
        let retagged = bundle::encode_shake_axt_wire(
            &ShakeAxtBundleWire {
                version: 1,
                intermediate_roots: roots,
                segments: frames,
