...
fn notify_trace_merkle_mode_observer(mode: ExecutionMode) {
    if let Ok(guard) = trace_merkle_mode_observer_slot().read()
        && let Some(callback) = guard.clone()
    {
        callback(mode);
    }
...
{
    if let Ok(mut guard) = trace_merkle_mode_observer_slot().write() {
        *guard = Some(Arc::new(observer));
    }
}
#[cfg(test)]
...
/// canonical power-of-two length, its columns have inconsistent lengths, or
/// its padded length exceeds the selected parameter domain.
...
fn merkle_root_with_first_level_using(
    ...
    let mut current = match first_level {
        None | Some([]) => compute_level(leaves),
