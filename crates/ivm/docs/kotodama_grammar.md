...
- Iroha syscalls: `mint_asset(acc, asset, amount)`, `burn_asset(acc, asset, amount)`, `transfer_asset(from, to, asset, amount, dataspace)`, `register_asset(asset, symbol, quantity, mintable)`, `create_new_asset(asset, symbol, quantity, account, mintable)`, `nft_mint_asset(id, owner)`, `nft_transfer_asset(from, id, to)`, `nft_set_metadata(id, key, json)`, `nft_burn_asset(id)`.
- Trigger syscalls: `create_trigger(json)`, `register_trigger(json)` (alias), `remove_trigger(name)`/`unregister_trigger(name)`, `set_trigger_enabled(name, enabled)`.
 ...
 - Nested calls: `call_contract(String|Blob contract, String|Blob entrypoint, Json payload) -> bytes` invokes an active callee's public or view entrypoint (top-level transaction calls remain public-only). Pointer-shaped manifest returns preserve the complete pointer-ABI TLV inside the returned `NoritoBytes`, exactly matching `pointer_to_norito(expected)` for `tlv_eq`; `int`/`bool` remain canonical Norito scalar bytes and unit returns a null pointer. Public callers require `permission(...)`, and a protected callee checks the immediate caller contract subject. [Implemented]
