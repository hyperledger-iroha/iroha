...
- Path builder (host): `host::path_map_key(base: Name, key: int) -> Name` builds canonical `"<base>/<key>"`.
 ...
 - Typed constructors for pointer-ABI values: `account_id("6cmzPVPX...")`, `asset_definition("rose#wonderland")`, `asset_id("rose##6cmzPVPX...")`, `nft_id("n0$wonderland")`, `name("cursor")`, `json("{\"k\":\"v\"}")`, `blob("...")`, `norito_bytes("...")`, plus AXT/Nexus types `dataspace_id("0x…")`, `axt_descriptor("0x…")`, `asset_handle("0x…")`, and `proof_blob("0x…")`. String literals (plain or `0x`-hex) emit typed Norito TLVs into the data section; passing a `Blob`/`NoritoBytes` decodes the TLV at runtime via `pointer_from_norito`.
