...
- `DataSpaceId`, `AxtDescriptor`, `AssetHandle`, and `ProofBlob` underpin the AXT (atomic cross-transaction) flow. Every V1 `AssetHandle` carries one exact issuer-signed `AssetDefinitionId`; default and WSV hosts fully validate these pointers when servicing AXT syscalls, ensuring descriptor membership, exact handle/intent/proof asset equality, capability binding equality, and proof material are honoured.
