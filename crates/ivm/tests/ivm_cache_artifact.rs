...
fn artifact_predecode_uses_header_version() {
    ...
    assert_eq!(m2.version_major, 2);
    assert_eq!(d2.len(), 1);
    // Different header version should produce distinct cache entries (not pointer-equal)
