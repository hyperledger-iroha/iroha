...
fn global_capacity_eviction_and_runtime_resize() {
    ...
    // Insert s1, s2
    let _ = ivm_cache::global_get(&s1).expect("decode s1");
    let _ = ivm_cache::global_get(&s2).expect("decode s2");
    // Access s1 to make it MRU
    let _ = ivm_cache::global_get(&s1).expect("hit s1");
    // Insert s3: should evict s2 (LRU)
    let (_h1, _m1, e1) = ivm_cache::global_counters();
    let _ = ivm_cache::global_get(&s3).expect("decode s3");
    ...
    let _ = ivm_cache::global_get(&s3).expect("decode s3 after resize");
