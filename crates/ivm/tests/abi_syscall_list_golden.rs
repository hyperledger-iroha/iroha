//! Golden test for the ABI syscall list ordering and contents.
//! The canonical list below is owned by `gen_syscalls_doc`.

#[test]
fn abi_syscall_list_matches_golden() {
    use ivm::syscalls as S;

    // Keep the ordering sorted by canonical syscall number.
    // BEGIN GENERATED ABI V1 SYSCALL LIST
    let golden: &[u32] = &[
        S::SYSCALL_DEBUG_PRINT,
        S::SYSCALL_EXIT,
        S::SYSCALL_ABORT,
        S::SYSCALL_DEBUG_LOG,
        S::SYSCALL_CONTRACT_ABORT,
        S::SYSCALL_REGISTER_DOMAIN,
        S::SYSCALL_UNREGISTER_DOMAIN,
        S::SYSCALL_TRANSFER_DOMAIN,
        S::SYSCALL_REGISTER_ACCOUNT,
        S::SYSCALL_UNREGISTER_ACCOUNT,
        S::SYSCALL_REGISTER_PEER,
        S::SYSCALL_UNREGISTER_PEER,
        S::SYSCALL_ADD_SIGNATORY,
        S::SYSCALL_REMOVE_SIGNATORY,
        S::SYSCALL_SET_ACCOUNT_QUORUM,
        S::SYSCALL_SET_ACCOUNT_DETAIL,
        S::SYSCALL_REGISTER_ASSET,
        S::SYSCALL_UNREGISTER_ASSET,
        S::SYSCALL_MINT_ASSET,
        S::SYSCALL_BURN_ASSET,
        S::SYSCALL_TRANSFER_V1,
        S::SYSCALL_NFT_MINT_ASSET,
        S::SYSCALL_NFT_TRANSFER_ASSET,
        S::SYSCALL_NFT_SET_METADATA,
        S::SYSCALL_NFT_BURN_ASSET,
        S::SYSCALL_TRANSFER_V1_BATCH_BEGIN,
        S::SYSCALL_TRANSFER_V1_BATCH_END,
        S::SYSCALL_TRANSFER_V1_BATCH_APPLY,
        S::SYSCALL_TRANSFER_ASSET_SCOPED,
        S::SYSCALL_CREATE_ROLE,
        S::SYSCALL_DELETE_ROLE,
        S::SYSCALL_GRANT_ROLE,
        S::SYSCALL_REVOKE_ROLE,
        S::SYSCALL_GRANT_PERMISSION,
        S::SYSCALL_REVOKE_PERMISSION,
        S::SYSCALL_GRANT_CONTRACT_ENTRYPOINT,
        S::SYSCALL_REVOKE_CONTRACT_ENTRYPOINT,
        S::SYSCALL_CREATE_TRIGGER,
        S::SYSCALL_REMOVE_TRIGGER,
        S::SYSCALL_SET_TRIGGER_ENABLED,
        S::SYSCALL_DEACTIVATE_CONTRACT_INSTANCE,
        S::SYSCALL_REMOVE_SMART_CONTRACT_BYTES,
        S::SYSCALL_REGISTER_SMART_CONTRACT_CODE,
        S::SYSCALL_REGISTER_SMART_CONTRACT_BYTES,
        S::SYSCALL_ACTIVATE_CONTRACT_INSTANCE,
        S::SYSCALL_STATE_GET,
        S::SYSCALL_STATE_SET,
        S::SYSCALL_STATE_DEL,
        S::SYSCALL_DECODE_INT,
        S::SYSCALL_ENCODE_INT,
        S::SYSCALL_BUILD_PATH_KEY_NORITO,
        S::SYSCALL_JSON_ENCODE,
        S::SYSCALL_JSON_DECODE,
        S::SYSCALL_SCHEMA_ENCODE,
        S::SYSCALL_SCHEMA_DECODE,
        S::SYSCALL_SCHEMA_INFO,
        S::SYSCALL_NAME_DECODE,
        S::SYSCALL_POINTER_TO_NORITO,
        S::SYSCALL_POINTER_FROM_NORITO,
        S::SYSCALL_TLV_EQ,
        S::SYSCALL_ZK_VERIFY_TRANSFER,
        S::SYSCALL_ZK_VERIFY_UNSHIELD,
        S::SYSCALL_ZK_VOTE_VERIFY_BALLOT,
        S::SYSCALL_ZK_VOTE_VERIFY_TALLY,
        S::SYSCALL_ZK_ROOTS_GET,
        S::SYSCALL_ZK_VOTE_GET_TALLY,
        S::SYSCALL_VRF_VERIFY,
        S::SYSCALL_VRF_VERIFY_BATCH,
        S::SYSCALL_ZK_VERIFY_BATCH,
        S::SYSCALL_TLV_LEN,
        S::SYSCALL_JSON_GET_JSON,
        S::SYSCALL_JSON_GET_NAME,
        S::SYSCALL_JSON_GET_ACCOUNT_ID,
        S::SYSCALL_JSON_GET_NFT_ID,
        S::SYSCALL_JSON_GET_BLOB_HEX,
        S::SYSCALL_VRF_EPOCH_SEED,
        S::SYSCALL_JSON_GET_ASSET_DEFINITION_ID,
        S::SYSCALL_JSON_OBJECT,
        S::SYSCALL_JSON_SET_I64,
        S::SYSCALL_JSON_SET_ACCOUNT_ID,
        S::SYSCALL_JSON_GET_JSON_DIRECT,
        S::SYSCALL_JSON_GET_NAME_DIRECT,
        S::SYSCALL_JSON_GET_ACCOUNT_ID_DIRECT,
        S::SYSCALL_JSON_GET_NFT_ID_DIRECT,
        S::SYSCALL_JSON_GET_BLOB_HEX_DIRECT,
        S::SYSCALL_JSON_GET_ASSET_DEFINITION_ID_DIRECT,
        S::SYSCALL_JSON_SET_I64_DIRECT,
        S::SYSCALL_JSON_SET_ACCOUNT_ID_DIRECT,
        S::SYSCALL_BUILD_PATH_KEY_NORITO_DIRECT,
        S::SYSCALL_SCHEMA_INFO_DIRECT,
        S::SYSCALL_SM3_HASH,
        S::SYSCALL_SM2_VERIFY,
        S::SYSCALL_SM4_GCM_SEAL,
        S::SYSCALL_SM4_GCM_OPEN,
        S::SYSCALL_SM4_CCM_SEAL,
        S::SYSCALL_SM4_CCM_OPEN,
        S::SYSCALL_SHA256_HASH,
        S::SYSCALL_SHA3_HASH,
        S::SYSCALL_BLAKE2B256_HASH,
        S::SYSCALL_KECCAK256_HASH,
        S::SYSCALL_IROHA_HASH,
        S::SYSCALL_SMARTCONTRACT_EXECUTE_INSTRUCTION,
        S::SYSCALL_SMARTCONTRACT_EXECUTE_QUERY,
        S::SYSCALL_CREATE_NFTS_FOR_ALL_USERS,
        S::SYSCALL_SET_SMARTCONTRACT_EXECUTION_DEPTH,
        S::SYSCALL_GET_AUTHORITY,
        S::SYSCALL_SUBSCRIPTION_BILL,
        S::SYSCALL_SUBSCRIPTION_RECORD_USAGE,
        S::SYSCALL_RESOLVE_ACCOUNT_ALIAS,
        S::SYSCALL_CURRENT_TIME_MS,
        S::SYSCALL_CALL_CONTRACT,
        S::SYSCALL_ANONYMOUS_ESCROW_OPEN_OFFER,
        S::SYSCALL_ANONYMOUS_ESCROW_ACCEPT,
        S::SYSCALL_ANONYMOUS_ESCROW_MARK_PAYMENT_SENT,
        S::SYSCALL_ANONYMOUS_ESCROW_RELEASE,
        S::SYSCALL_ANONYMOUS_ESCROW_CANCEL,
        S::SYSCALL_ANONYMOUS_ESCROW_OPEN_DISPUTE,
        S::SYSCALL_AXT_BEGIN,
        S::SYSCALL_AXT_TOUCH,
        S::SYSCALL_AXT_COMMIT,
        S::SYSCALL_VERIFY_DS_PROOF,
        S::SYSCALL_USE_ASSET_HANDLE,
        S::SYSCALL_ESCROW_OPEN_OFFER,
        S::SYSCALL_ESCROW_ACCEPT,
        S::SYSCALL_ESCROW_MARK_PAYMENT_SENT,
        S::SYSCALL_ESCROW_RELEASE,
        S::SYSCALL_ESCROW_CANCEL,
        S::SYSCALL_ESCROW_OPEN_DISPUTE,
        S::SYSCALL_ESCROW_RESOLVE_DISPUTE,
        S::SYSCALL_ANONYMOUS_ESCROW_RESOLVE_DISPUTE,
        S::SYSCALL_SORACLOUD_READ_COMMITTED_STATE,
        S::SYSCALL_SORACLOUD_EMIT_STATE_MUTATION,
        S::SYSCALL_SORACLOUD_EMIT_MAILBOX_MESSAGE,
        S::SYSCALL_SORACLOUD_APPEND_JOURNAL,
        S::SYSCALL_SORACLOUD_PUBLISH_CHECKPOINT,
        S::SYSCALL_SORACLOUD_READ_SECRET,
        S::SYSCALL_SORACLOUD_READ_CREDENTIAL,
        S::SYSCALL_SORACLOUD_EGRESS_FETCH,
        S::SYSCALL_SORACLOUD_READ_CONFIG,
        S::SYSCALL_SORACLOUD_READ_SECRET_ENVELOPE,
        S::SYSCALL_SCHEMA_ENCODE_DIRECT,
        S::SYSCALL_SCHEMA_DECODE_DIRECT,
        S::SYSCALL_INPUT_PUBLISH_TLV,
        S::SYSCALL_ALLOC,
        S::SYSCALL_GET_PUBLIC_INPUT,
        S::SYSCALL_PROVE_EXECUTION,
        S::SYSCALL_GROW_HEAP,
        S::SYSCALL_VERIFY_PROOF,
        S::SYSCALL_GET_MERKLE_PATH,
        S::SYSCALL_PRIVATE_NUMERIC_VALCOM,
        S::SYSCALL_GET_ACCOUNT_BALANCE,
        S::SYSCALL_GET_MERKLE_COMPACT,
        S::SYSCALL_VERIFY_SIGNATURE,
        S::SYSCALL_GET_PRIVATE_INPUT,
        S::SYSCALL_COMMIT_OUTPUT,
        S::SYSCALL_GET_REGISTER_MERKLE_COMPACT,
        S::SYSCALL_QUERY_EXECUTE_NORITO,
        S::SYSCALL_CORE_QUERY_GET,
        S::SYSCALL_CORE_QUERY_PAGE,
        S::SYSCALL_QUERY_GET_PARAMETER,
        S::SYSCALL_QUERY_GET_CONTRACT_MANIFEST,
        S::SYSCALL_QUERY_GET_CONTRACT_INSTANCE,
        S::SYSCALL_SYSVAR_CHAIN_ID,
        S::SYSCALL_SYSVAR_BLOCK_HEIGHT,
        S::SYSCALL_SYSVAR_BLOCK_TIME_MS,
        S::SYSCALL_SYSVAR_AUTHORITY,
        S::SYSCALL_SYSVAR_CONTRACT_ADDRESS,
        S::SYSCALL_SYSVAR_ENTRYPOINT,
        S::SYSCALL_DECODE_ARGUMENT_RECORD,
        S::SYSCALL_SYSVAR_CONTRACT_SUBJECT,
        S::SYSCALL_NORMALIZE_NORITO_BYTES,
        S::SYSCALL_CALL_CONTRACT_QUANTITY2,
        S::SYSCALL_STATE_KEYS,
        S::SYSCALL_STATE_HAS,
        S::SYSCALL_STATE_LEN,
        S::SYSCALL_STATE_COUNT,
        S::SYSCALL_STATE_MAP_KEY_AT,
        S::SYSCALL_STATE_VALUE_ENCODE,
        S::SYSCALL_STATE_VALUE_DECODE,
        S::SYSCALL_JSON_BUILD,
        S::SYSCALL_INT_FROM_I64,
        S::SYSCALL_INT_FROM_U64,
        S::SYSCALL_INT_TRY_TO_I64,
        S::SYSCALL_INT_TRY_TO_U64,
        S::SYSCALL_INT_NEG,
        S::SYSCALL_INT_ADD,
        S::SYSCALL_INT_SUB,
        S::SYSCALL_INT_MUL,
        S::SYSCALL_INT_DIV,
        S::SYSCALL_INT_REM,
        S::SYSCALL_INT_EQ,
        S::SYSCALL_INT_NE,
        S::SYSCALL_INT_LT,
        S::SYSCALL_INT_LE,
        S::SYSCALL_INT_GT,
        S::SYSCALL_INT_GE,
        S::SYSCALL_INT_WRAP_NEG,
        S::SYSCALL_INT_WRAP_ADD,
        S::SYSCALL_INT_WRAP_SUB,
        S::SYSCALL_INT_WRAP_MUL,
        S::SYSCALL_DECIMAL_FROM_INT,
        S::SYSCALL_DECIMAL_NEG,
        S::SYSCALL_DECIMAL_ADD,
        S::SYSCALL_DECIMAL_SUB,
        S::SYSCALL_DECIMAL_MUL,
        S::SYSCALL_DECIMAL_DIV_EXACT,
        S::SYSCALL_DECIMAL_DIV_ROUND,
        S::SYSCALL_DECIMAL_EQ,
        S::SYSCALL_DECIMAL_NE,
        S::SYSCALL_DECIMAL_LT,
        S::SYSCALL_DECIMAL_LE,
        S::SYSCALL_DECIMAL_GT,
        S::SYSCALL_DECIMAL_GE,
        S::SYSCALL_DECIMAL_TRY_TO_INT_EXACT,
        S::SYSCALL_DECIMAL_TO_INT_TRUNC,
        S::SYSCALL_DECIMAL_TO_INT_ROUND,
        S::SYSCALL_QUANTITY_TRY_FROM_INT,
        S::SYSCALL_QUANTITY_TRY_FROM_DECIMAL,
        S::SYSCALL_QUANTITY_TO_DECIMAL,
        S::SYSCALL_QUANTITY_ADD,
        S::SYSCALL_QUANTITY_SUB,
        S::SYSCALL_QUANTITY_MUL_DECIMAL,
        S::SYSCALL_QUANTITY_DIV_DECIMAL_EXACT,
        S::SYSCALL_QUANTITY_DIV_DECIMAL_ROUND,
        S::SYSCALL_QUANTITY_RATIO_EXACT,
        S::SYSCALL_QUANTITY_RATIO_ROUND,
        S::SYSCALL_QUANTITY_EQ,
        S::SYSCALL_QUANTITY_NE,
        S::SYSCALL_QUANTITY_LT,
        S::SYSCALL_QUANTITY_LE,
        S::SYSCALL_QUANTITY_GT,
        S::SYSCALL_QUANTITY_GE,
        S::SYSCALL_JSON_GET_INT,
        S::SYSCALL_JSON_GET_DECIMAL,
        S::SYSCALL_JSON_GET_QUANTITY,
        S::SYSCALL_JSON_GET_INT_DIRECT,
        S::SYSCALL_JSON_GET_DECIMAL_DIRECT,
        S::SYSCALL_JSON_GET_QUANTITY_DIRECT,
        S::SYSCALL_SET_ASSET_TRANSFER_AVAILABILITY,
        S::SYSCALL_SET_ASSET_TRANSFER_DAILY_LIMIT,
        S::SYSCALL_SET_ASSET_HOLDING_LIMIT,
        S::SYSCALL_ACCOUNT_RECOVERY_PROPOSE,
        S::SYSCALL_ACCOUNT_RECOVERY_APPROVE,
        S::SYSCALL_ACCOUNT_RECOVERY_CANCEL,
        S::SYSCALL_ACCOUNT_RECOVERY_FINALIZE,
    ];
    // END GENERATED ABI V1 SYSCALL LIST

    let actual = ivm::syscalls::abi_syscall_list();
    assert_strictly_increasing(actual);
    assert_eq!(
        actual.len(),
        golden.len(),
        "ABI syscall list size changed ({} vs {}).",
        actual.len(),
        golden.len()
    );
    assert_eq!(
        actual, golden,
        "ABI syscall list changed; update the golden list if intentional."
    );
    // Sanity: the function is also referenced via constants to ensure names are in scope
    assert!(golden.contains(&S::SYSCALL_EXIT));
}

fn assert_strictly_increasing(values: &[u32]) {
    for window in values.windows(2) {
        if let [prev, next] = window {
            assert!(
                prev < next,
                "abi_syscall_list must be sorted and deduplicated; found {prev:#x} then {next:#x}"
            );
        }
    }
}
