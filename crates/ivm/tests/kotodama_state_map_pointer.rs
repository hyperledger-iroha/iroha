...
fn account_pointer_tlvs(id: &str) -> (Vec<u8>, Vec<u8>) {
    let account: AccountId = id.parse().expect("valid AccountId literal");
    let raw = encode_pointer_tlv(
        PointerType::AccountId,
        &<AccountId as norito::codec::Encode>::encode(&account),
    );
...
fn encode_int_norito(value: i64) -> Vec<u8> {
    ...
    encode_pointer_tlv(PointerType::NoritoBytes, ascii.as_bytes())
}
...
fn durable_map_account_id_path_hashes() {
    ...
    assert_eq!(stored.len(), 7 + "5".len() + IrohaHash::LENGTH);
}
...
fn durable_map_name_value_roundtrip() {
    ...
    host.insert_state_value(
        "aliases/42",
        encode_pointer_tlv(PointerType::Name, b"wonder"),
    );
    let stored = host.state_bytes("aliases/42").expect("name pointer stored");
    assert_eq!(stored.len(), 7 + "wonder".len() + IrohaHash::LENGTH);
}
