...
fn compact_bundle_norito_roundtrip_memory_and_registers() {
    ...
    vm.memory.commit();
    let b1 = ivm::merkle_utils::memory_compact_bundle(&mut vm.memory, addr, Some(8)).unwrap();
    ...
    let b2 = ivm::merkle_utils::registers_compact_bundle(&vm.registers, 7, Some(8)).unwrap();
