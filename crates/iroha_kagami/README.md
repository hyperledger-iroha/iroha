...
- Localnet defaults to a fast pipeline (1.0s total split ~1/3 block and ~2/3 commit), tunes the commit-inflight timeout to match, bumps redundant-send fanout for DA, raises `sumeragi.rbc_chunk_max_bytes` to 256 KiB, and disables Kura fsync (`kura.fsync_mode = "off"`) for local throughput. It also tunes DA availability timeouts for local runs (`sumeragi.da_quorum_timeout_multiplier = 3`, `sumeragi.da_availability_timeout_multiplier = 2`, `sumeragi.da_availability_timeout_floor_ms = 2000`). NPoS localnets now seed XOR stake for validators, raise `parameters.block.max_transactions` to 10,000, and disable empty-child fallback (`sumeragi.empty_child_fallback_enabled = false`) to keep idle networks quiet. Override the pipeline with `--block-time-ms`/`--commit-time-ms` and adjust the config fields when you need slower timings; if you set only one of `--block-time-ms` or `--commit-time-ms`, Kagami mirrors it to the other so the pipeline stays balanced.
