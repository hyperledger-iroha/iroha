...
Iroha3 requires NPoS and does not support staged cutovers; omit `--next-consensus-mode`/`--mode-activation-height` when targeting Iroha3.
...
- Bare metal (Iroha2 cutover): `kagami localnet --consensus-mode permissioned --next-consensus-mode npos --mode-activation-height <H> --peers <N> --out-dir <dir>` stages an NPoS cutover in genesis and renders peer configs/PoPs.
...
- Bare-metal localnet (Iroha2 cutover): `kagami localnet --consensus-mode permissioned --next-consensus-mode npos --mode-activation-height 5 --out-dir ./localnet-npos --peers 4` stages `next_mode = npos` at height 5 while keeping the permissioned fingerprint until activation.
