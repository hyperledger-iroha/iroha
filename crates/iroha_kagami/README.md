...
- Bare metal: `kagami localnet --consensus-mode npos --mode-activation-height <H> --peers <N> --out-dir <dir>` stages an NPoS cutover in genesis and renders peer configs/PoPs. Supply `--seed` to keep the validator roster and PoPs deterministic; edit `genesis.json` if you need to pin `sumeragi_npos_parameters.epoch_seed` for a fixed VRF seed.
- Localnet defaults to a fast pipeline (0.85s total split into block/commit), tunes the commit-inflight timeout to match, and bumps redundant-send fanout for DA. It also tunes DA availability timeouts for local runs (`sumeragi.da_quorum_timeout_multiplier = 1`, `sumeragi.da_availability_timeout_multiplier = 2`, `sumeragi.da_availability_timeout_floor_ms = 0`). Override the pipeline with `--block-time-ms`/`--commit-time-ms` and adjust the config fields when you need slower timings; if you set only one of `--block-time-ms` or `--commit-time-ms`, Kagami mirrors it to the other so the pipeline stays balanced.
- Localnet peer configs allow loopback CIDRs (`127.0.0.0/8`, `::1/128`) through the Torii pre-auth gate so demo clients don't hit temporary bans.
...
- Docker: reuse the same `genesis.json`/configs and run `kagami swarm --consensus-mode npos --mode-activation-height <H> --peers <N> --config-dir <dir> --image <img> --out-file <compose.yml>`. The Compose manifest forwards the cutover flags to the signing sidecar so `kagami genesis sign` stamps both `next_mode` and `mode_activation_height`.
...
- Bare-metal localnet: `kagami localnet --consensus-mode npos --mode-activation-height 5 --out-dir ./localnet-npos --peers 4` generates BLS keys/PoPs, stamps `next_mode = npos` with an activation height, and writes configs/genesis/scripts. Edit `genesis.json` to set a reproducible VRF seed via `parameters.custom.sumeragi_npos_parameters.epoch_seed` (32-byte array/hex) when you need deterministic epochs.
- Compose/Docker: generate an NPoS-ready `genesis.json` first (`kagami genesis generate --consensus-mode npos --mode-activation-height 5 --ivm-dir ./ivm_libs --genesis-public-key <PK>`), then sign it with your roster + PoPs: `kagami genesis sign genesis.json --consensus-mode npos --mode-activation-height 5 --topology "<peer ids>" --peer-pop "<pk=pop_hex>" --private-key <hex> --out-file genesis.signed.nrt`.
- Swarm cutover: `kagami swarm --consensus-mode npos --mode-activation-height 5 --config-dir ./cfg --image hyperledger/iroha:dev --out-file docker-compose.yml` now refuses to render when `genesis.json` is missing `sumeragi_npos_parameters`, so the compose run always carries the NPoS tuning baked into genesis.
...
- Ensure your `config.toml` lists validator BLS-Normal keys under `trusted_peers` **and** provides matching PoPs under `trusted_peers_pop`. The loader rejects legacy `trusted_peers_bls` mappings and any validator without a PoP; genesis-embedded `pop_hex` entries can satisfy the same requirement if you prefer manifest-only PoPs.
