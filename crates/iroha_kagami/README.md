...
- Normal mode omits `--seed`: Kagami parses every `peerN.toml`, verifies the
  exact signer/hash/validator/PoP binding, derives a content-addressed
  container-safe config projection, proves its Sumeragi, execution-policy, and
  Nexus/AMX fingerprints unchanged, and mounts the projection as a file-backed
  Compose secret. Validator keys are absent from Compose YAML and environment
  variables. The three public genesis artifact paths remain read-only; source
  configs and host-only Torii signer/client credentials are not mounted. It
  does not generate replacement validator identities.
...
storage and omits local account-onboarding and faucet services; the original
bare-metal configs are unchanged.
