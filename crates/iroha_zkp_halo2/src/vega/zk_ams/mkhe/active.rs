...
impl ZkAmsMkheActiveContributionV1 {
    ...
    pub fn digest(&self) -> Result<[u8; 32], ZkAmsMkheErrorV1> {
        ...
        let mut hash = Keccak256::new();
        hash.update(ACTIVE_CONTRIBUTION_DOMAIN_V1);
        hash.update(&statement);
        hash.update(&self.authentication.public_key);
        hash.update(&self.authentication.signature);
        Ok(hash.finalize())
    }
}
#[allow(clippy::too_many_arguments)]
...
fn active_contribution_statement_digest(
    ...
    hash.update(&contribution.roster_digest);
    hash.update(&contribution.epoch.to_be_bytes());
    hash.update(&contribution.transcript_digest);
    hash.update(&[contribution.round.tag()]);
    hash.update(&contribution.contribution_index.to_be_bytes());
    hash.update(&contribution.party.to_bytes());
...
fn derive_sparse_challenge(
    ...
    let mut terms: Vec<SparseChallengeTermV1> = Vec::new();
    terms
