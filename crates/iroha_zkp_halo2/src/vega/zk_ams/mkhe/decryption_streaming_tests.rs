...
fn compact_authority_source_surface_is_move_only_ordered_and_fail_closed() {
    ...
    let statement_constructor = streaming
        .split("pub fn from_verified_cpk_authority_v1<P>(")
        .nth(1)
        .expect("streaming statement constructor")
        .split("/// Exact governed roster retained by the compact statement.")
        .next()
        .expect("streaming statement constructor boundary");
    assert_eq!(
    ...
    let mut prior_end = 0;
    for marker in [
        "let mut parties = try_exact_capacity_vec_v1(",
        "parties.extend_from_slice(roster.parties());",
        "let parties = PartySet::new(parties)?;",
        "validate_streaming_ciphertext_live_v1(",
        "persistent_context.consume_streaming_authority_v1(",
        "let mut party_bindings = try_exact_capacity_vec_v1(",
        "for (party_index, persistent) in proof_bindings.into_iter().enumerate()",
        "party_bindings: party_bindings",
    ] {
        let next = statement_constructor[prior_end..]
            .find(marker)
            .unwrap_or_else(|| panic!("missing ordered statement-constructor marker: {marker}"));
        prior_end += next + marker.len();
    }
    assert_eq!(ZK_AMS_MKHE_RELEASE_ROSTER_SIZE_V1, 8);
    assert!(statement_constructor.contains("parties.extend_from_slice(roster.parties());"));
    assert!(statement_constructor.contains("let parties = PartySet::new(parties)?;"));
    assert!(!statement_constructor.contains(".to_vec()"));
    assert!(!statement_constructor.contains("Vec::new"));
    assert!(!statement_constructor.contains("try_reserve_exact"));
    let party_set_digest = mkhe
        .split("fn party_set_digest(")
        .nth(1)
        .expect("party-set digest")
        .split("struct AuthenticationSecret")
        .next()
        .expect("party-set digest boundary");
    let count = party_set_digest
        .find("let count = u8::try_from(parties.len())")
        .expect("fallible party count");
    let domain = party_set_digest
        .find("hash.update(b\"iroha.zk-ams.v1.mkhe.ordered-party-set\");")
        .expect("party-set domain");
    let count_absorption = party_set_digest
        .find("hash.update(&[count]);")
        .expect("party-set count absorption");
    let party_absorption = party_set_digest
        .find("hash.update(&party.0);")
        .expect("ordered party absorption");
    let finish = party_set_digest
        .find("Ok(hash.finalize())")
        .expect("party-set digest finalization");
    assert!(count < domain && domain < count_absorption);
    assert!(count_absorption < party_absorption && party_absorption < finish);
    assert!(!party_set_digest.contains("Vec"));
    assert!(!party_set_digest.contains("keccak256("));
...
#[test]
fn manifest_preflight_owner_is_exact_and_preserves_abort_order() {
    let streaming = include_str!("decryption_streaming.rs");
    let preflight = streaming
        .split("fn preflight_manifests(")
        .nth(1)
        .expect("manifest preflight")
        .split("fn validate_streaming_manifest_slot_v1(")
        .next()
        .expect("manifest preflight boundary");
    let mut prior_end = 0;
    for marker in [
        ...
        let next = preflight[prior_end..]
            .find(marker)
            .unwrap_or_else(|| panic!("missing ordered manifest-preflight marker: {marker}"));
        prior_end += next + marker.len();
    }
    assert_eq!(preflight.matches("try_exact_capacity_vec_v1(").count(), 1);
    let raw_new = ["Vec", "::new"].concat();
    let raw_reserve = ["try_reserve", "_exact"].concat();
    assert!(!preflight.contains(&raw_new));
    assert!(!preflight.contains(&raw_reserve));
    assert_eq!(ZK_AMS_MKHE_RELEASE_ROSTER_SIZE_V1, 8);
    assert_eq!(ZK_AMS_MKHE_DECRYPTION_SPLIT_MANIFEST_BYTES_V1, 498);
    assert_eq!(
        ZK_AMS_MKHE_RELEASE_ROSTER_SIZE_V1 * ZK_AMS_MKHE_DECRYPTION_SPLIT_MANIFEST_BYTES_V1,
        3_984
    );
}
#[test]
...
#[test]
fn staged_sparse_terms_owner_is_exact_and_preserves_scan_order() {
    let streaming = include_str!("decryption_streaming.rs");
    let terms = streaming
        .split("fn sparse_challenge_terms_v1(")
        .nth(1)
        .expect("sparse challenge terms")
        .split("fn sparse_fold_small_coefficient_v1(")
        .next()
        .expect("sparse challenge terms boundary");
    let mut prior_end = 0;
    for marker in [
        ...
        let next = terms[prior_end..]
            .find(marker)
            .unwrap_or_else(|| panic!("missing ordered sparse-terms marker: {marker}"));
        prior_end += next + marker.len();
    }
    assert_eq!(terms.matches("try_exact_capacity_vec_v1(").count(), 1);
    assert_eq!(
        terms
            .matches("ZkAmsMkheErrorV1::ResourceCeilingExceeded")
            .count(),
    ...
    assert!(!terms.contains("Vec::new"));
    assert!(!terms.contains("try_reserve_exact"));

    #[cfg(target_pointer_width = "64")]
    {
        let release_weight =
            wide_relation_challenge_weight(release_profile_v1().ring_degree).unwrap();
        assert_eq!(size_of::<usize>(), 8);
        assert_eq!(release_weight, 20);
        assert_eq!(size_of::<SparseChallengeTermV1>(), 16);
        assert_eq!(release_weight * size_of::<SparseChallengeTermV1>(), 320);
    }
}
#[test]
...
fn verifier_decode_and_streamed_limb_sources_use_only_zeroizing_owners() {
    ...
    let accepted_tail = combine
        .split("let (plaintext, maximum_residual_bits) =")
        .nth(1)
        .expect("accepted plaintext tail");
    for marker in [
        "decode_centered_plaintext(&profile, aggregate.as_rns()",
        "ordered_share_set_digest: set_hash.finalize()",
        "snapshot: snapshot.finish()",
    ] {
        assert!(accepted_tail.contains(marker));
    }
    for forbidden in ["Vec::", "collect::<", ".to_vec()", ".clone()", ".encode("] {
        assert!(!accepted_tail.contains(forbidden));
    }
}
#[test]
