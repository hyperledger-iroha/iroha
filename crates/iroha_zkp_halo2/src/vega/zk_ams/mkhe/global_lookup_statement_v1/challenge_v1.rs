...
const FIRST_SUMCHECK_ORDINAL_V1: u32 = 33;
const LAST_SUMCHECK_ORDINAL_V1: u32 = 266;
const FIRST_ENDPOINT_BATCH_ORDINAL_V1: u32 = 267;
const LAST_ENDPOINT_BATCH_ORDINAL_V1: u32 = 282;
const MASK_BATCH_ORDINAL_V1: u32 = 283;
const COMPLETE_CHALLENGE_ORDINAL_V1: u32 = 284;
...
const MASK_OPENING_LANGUAGE_V1: &[u8] = b"mask-scalars=702,padded=1024;segments:s0..13=(14*s,14),s14=(196,9),s15=(205,29);h=1/2;w_(s,t)=(h^(len-1-t)*(r^3-h),h^(len-1-t)*(r^2-h),h^(len-1-t)*(r-h));xi-batch=sum_s(xi^s*w_s);padding-weight=0;s0..14:<w_s,mask>+R_s-C_s=0;s15:<w15,mask>-Z*=0;R*+Z*-C15=0";
...
const ORDINAL_LANGUAGE_V1: &[u8] = b"outer-ordinals:z=0;rho[0..28]=1..29;alpha=30;lambda=31;mu=32;no-challenge-between-mu-and-gtilde_0;for-j=0..233:absorb-exact-gtilde_j-96B-then-r_j=33+j;absorb-exact-52-endpoint-commitments;nu[0..15]=267..282;xi=283;complete=284";
const PURPOSE_LANGUAGE_V1: &[u8] = b"purpose-payload=literal-label||statement_be_u16||coordinate_be_u16;no-coordinate=ffff;equations=(statement0..13,round0..13);group=(statement14,round0..8);lookup=(statement15,round0..28);challenge-fork=state||challenge-domain||ordinal_be_u32||attempt_u8||branch_u8||framed-purpose;accepted-append=framed-purpose,ordinal,attempt,scalar_le;rejected-forks-do-not-mutate;attempts=0..127";
...
const _: () = {
    ...
    // There is intentionally no pre-sumcheck batching challenge here.
    assert!(FIRST_SUMCHECK_ORDINAL_V1 == MU_ORDINAL_V1 + 1);
...
enum ChallengePredicateV1 {
    OutsideLookupTable = 1,
    Nonzero = 2,
}

#[derive(Clone, Copy)]
struct ChallengeCoordinateV1 {
    ordinal: u32,
    purpose: ChallengePurposeV1,
    predicate: ChallengePredicateV1,
}

impl ChallengePurposeV1 {
    const fn unscoped_v1(label: &'static [u8]) -> Self {
        Self {
            label,
            statement: NO_COORDINATE_V1,
            coordinate: NO_COORDINATE_V1,
        }
    }

    const fn scoped_v1(label: &'static [u8], statement: usize, coordinate: usize) -> Self {
        Self {
            label,
            statement: statement as u16,
            coordinate: coordinate as u16,
        }
    }
}
...
fn mask_constraint_residuals_v1(
    ...
    match statement {
        0..=14 => Ok([mask_evaluation + residual - public_claim, Scalar::zero()]),
        15 => Ok([
