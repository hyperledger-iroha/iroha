...
fn profile_pins_parent_kats_basis_layout_and_purpose_manifest() {
    ...
    assert_eq!(
        ...
        "e3730911785cb1e23332ee9a1361810c435f76b93becd54e3b0d189644b32d99"
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        "e077c21d002c377da1c6dcbbe2b1bcdfd58fe6316a8e697e1f37f454159645e3"
    );
    ...
    for frozen_scope in [
        b"exactly-19-committed-MLE-opening-proofs".as_slice(),
        b"parent-accounted-as-99-wire-bytes".as_slice(),
        b"3-separate-uninstantiated-proofs-with-unknown-wire-bytes".as_slice(),
        b"excluded-from-27276-byte-opening-subtotal".as_slice(),
    ] {
        assert!(
            COMMITTED_MLE_ACCOUNTING_SCOPE_LANGUAGE_V1
                .windows(frozen_scope.len())
                .any(|window| window == frozen_scope)
        );
    }
}
#[test]
...
fn endpoint_envelope_is_purpose_specific_exact_and_mutation_closed() {
    ...
    assert_eq!(
        hex::encode(envelope),
        "5a4745500100000510002000340313e077c21d002c377da1c6dcbbe2b1bcdfd58fe6316a8e697e1f37f454159645e3"
    );
    assert_eq!(
