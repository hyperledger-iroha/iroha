...
fn exact_carrier_recovery_verifies_direct_before_minting_combined_owner() {
    ...
    let handoff = source
        .split_once("pub(super) fn verify_rns_native_direct_global_membership_handoff_v1")
    ...
    let comparator_consume = handoff
        .find("let claimed_successor = comparator.into_previous_v1();")
        .expect("comparator consume");
    let exact_carrier = handoff
        .find("claimed_successor.into_parts_v1()")
        .expect("exact claimed carrier");
    let direct_verify = handoff
        .find("verify_rns_native_cross_field_rlwe_claimed_carrier_v1(")
        .expect("four-core direct verifier");
    let safe_core = handoff
        .find("source_packing_safe_core_v1(direct.safe_core_projection_v1())")
    ...
    assert!(
        membership_residual < membership_consume
            && membership_consume < comparator_consume
            && comparator_consume < exact_carrier
            && exact_carrier < direct_verify
            && direct_verify < safe_core
            && safe_core < outer
            && outer < combined
    );
...
fn safe_and_outer_mappings_preserve_the_audited_chronology() {
    ...
    let claimed = direct
        .split_once("pub(super) fn verify_rns_native_cross_field_rlwe_claimed_carrier_v1")
    ...
    let four_core = claimed
        .find("verify_kernel_for_suite_v1::<ZkAmsT256BulletproofSuiteV1, P>(")
