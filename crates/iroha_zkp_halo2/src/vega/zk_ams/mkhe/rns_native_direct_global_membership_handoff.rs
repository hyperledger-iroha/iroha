...
//! pointer-and-length identity for the sole successor borrow, and consumes the
//! claimed-root equality obligation. Only then does it mint one move-only
...
use super::{
    rns_native_cross_field_rlwe_direct::{
        RnsNativeCrossFieldAuthoritativeSourceV1, RnsNativeCrossFieldRlweDirectErrorV1,
        RnsNativeCrossFieldRlweSafeCoreProjectionV1,
        RnsNativeCrossFieldRlweTerminalBoundVerifiedV1,
        verify_rns_native_cross_field_rlwe_claimed_carrier_v1,
    },
...
///
/// The retained direct owner proves that claimed-root equality was discharged;
/// the retained inventory preserves the authenticated source/terminal lineage.
...
> {
    ...
    _inventory: super::rns_native_cross_field_inventory::RnsNativeCrossFieldInventoryPrerequisiteV1<
        'source,
        'proof,
        S,
    >,
...
/// carrier, verify the four direct cores, and discharge claimed-root equality.
///
/// `P` remains the private internal direct-source trait. There is no production
/// implementation or public generic entry point.
#[allow(
    dead_code,
    reason = "the generic source remains internal until the authenticated production adapter exists"
)]
pub(super) fn verify_rns_native_direct_global_membership_handoff_v1<'source, 'proof, S, P>(
    ...
    P: RnsNativeCrossFieldAuthoritativeSourceV1,
{
    ...
    let existing_radix_candidate_root = existing_radix.pre_z_candidate_root();
    let existing_radix_binding_digest = existing_radix.binding_digest();

    let q_mask = existing_radix.into_previous_v1();
    let q_mask_linear_relations_binding_digest = q_mask.binding_digest();

    let small_sign = q_mask.into_previous_v1();
    ...
    let (direct_parent, exact_claimed_successor) = claimed_successor.into_parts_v1();
    let (direct, inventory) = verify_rns_native_cross_field_rlwe_claimed_carrier_v1(
        direct_parent,
        exact_claimed_successor,
        existing_radix_candidate_root,
        source,
    )?;

    let safe_core = source_packing_safe_core_v1(direct.safe_core_projection_v1());
    ...
    let mut outer_bindings = RnsNativeSourcePackingCombinedOuterBindingsV1 {
        ...
        direct_binding_digest: direct.binding_digest(),
