//! Authenticated compact-source replay for the Phase-23 global lookup.
//!
//! This child consumes and retains the exact source-algebra prerequisite. It
//! authenticates every original plaintext and signed-witness source block,
//! then writes the three small signed witnesses as role-tagged, record-major
//! 16,384-byte `i8` planes into one unlinked confidential-spool leaf. No path,
//! key, plaintext, snapshot, callback, or detached authority escapes.

#![allow(
    dead_code,
    reason = "the parent prerequisite is production-uninhabited"
)]
...
/// Private sink authority. The production variant cannot be constructed; the
/// test path is consumed before writer creation and is never retained.
...
/// Move-only owner retaining the exact original prerequisite and authenticated
/// compact snapshot. It intentionally exposes no fields or getters.
#[must_use = "dropping this owner closes the compact source and original prerequisite"]
pub(in super::super) struct Phase23GlobalLookupSourceReplayV1<K, P> {
    prerequisite: Phase23SourceAlgebraPrerequisiteV2<K, P>,
