...
#[expect(clippy::too_many_arguments, reason = "explicit RKG equation inputs")]
...
pub(in super::super) fn direct_rkg_one_h1_limb_from_signed_v1(
    ...
    let mut output = ZeroizingU64VectorV1::with_capacity(common_a.len())?;
    for (product_coefficient, error_coefficient) in product.values().iter().zip(error) {
        output.push(super::super::mod_add(
            *product_coefficient,
            super::super::mod_mul(
                super::super::signed_mod(*error_coefficient, modulus),
                plaintext,
                modulus,
            ),
            modulus,
