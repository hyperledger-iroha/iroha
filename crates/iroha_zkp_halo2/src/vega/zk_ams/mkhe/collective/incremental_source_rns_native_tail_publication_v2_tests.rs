...
fn compiled_v1_tail_coordinator_pins_one_owner_one_publisher_and_exact_order() {
    ...
    for entrypoint in [
        "begin_from_cpk_ceremony_v2",
        "begin_from_finalized_cpk_v2",
        "begin_v2",
    ] {
        let signature = coordinator
            .split_once(&format!("fn {entrypoint}("))
            .and_then(|(_, suffix)| {
                suffix
                    .split_once(") -> Result")
                    .map(|(signature, _)| signature)
            })
            .unwrap_or_else(|| panic!("missing coordinator entry signature: {entrypoint}"));
        assert_eq!(
            signature.matches("ciphertext_publisher: P").count(),
            1,
            "{entrypoint} must accept exactly one ciphertext publisher",
        );
    }
...
fn callback_sink_failure_poison_path_skips_tail_and_later_output() {
    ...
    let result = invoke_v1_tail_callback_v2(
        ...
        RnsNativeV1TailCallbackInputsV2 {
            canonical_plaintext: &[],
            ephemeral: &[],
            error_zero: &[],
            error_one: &[],
            encryption_nonce: &[0x11; 32],
        },
        || {
...
fn coordinator_preserves_callback_error_origin_and_every_live_gate_remains_closed() {
    ...
    for required in [
        ...
        "(Err(encryption_error),None)=>Err(RnsNativeV1TailCoordinatorErrorV2::Encryption(encryption_error,",
        ...
        let compact_required = required.split_whitespace().collect::<String>();
        assert!(
