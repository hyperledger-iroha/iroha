...
//! predecessor, an opaque pre-direct inventory candidate projection,
//! and an opaque candidate-only radix axis, rehashes the same points, and derives the
...
//! linked final context. The production lease can be issued only by the sealed
//! canonical Envelope; every broader live-integration gate remains false.
//!
//! Live production activation remains unavailable. A private source-only
//! chronology moves the sole lineaged schedule through pre-auth claimed qPCS and final-context
//! inventory authentication, exact-preflights the nested existing-radix
//! allocation once, then retains its sealed candidate axis and validation
//! permit beside that same schedule and all terminal-root obligations. The
//! internal argument-free bind now constructs the direct schedule, while all
//! live/readiness/release gates remain false. Structural
...
use super::{
    ...
    rns_native_existing_radix_commitment_view::{
        RnsNativeExistingRadixCandidateAxisV1, RnsNativeExistingRadixDirectAliasV1,
...
const QUOTIENT_OPENING_SIGNS_V1: usize = 2;
const QUOTIENT_OPENING_OWNERS_V1: usize = EVALUATIONS_V1 * QUOTIENT_OPENING_SIGNS_V1;
const QUOTIENT_OPENING_SCALARS_PER_OWNER_V1: usize = BLOCK_COORDINATES_V1 + 1 + QUOTIENT_BITS_V1;
const QUOTIENT_OPENING_BYTES_PER_OWNER_V1: usize =
    QUOTIENT_OPENING_SCALARS_PER_OWNER_V1 * SCALAR_BYTES_V1;
const QUOTIENT_OPENING_STREAM_SCALARS_V1: usize =
    QUOTIENT_OPENING_OWNERS_V1 * QUOTIENT_OPENING_SCALARS_PER_OWNER_V1;
const QUOTIENT_OPENING_STREAM_BYTES_V1: usize =
    QUOTIENT_OPENING_STREAM_SCALARS_V1 * SCALAR_BYTES_V1;
...
// "Integrated" here means reachable from a live production entry. The private
// source-only chronology below is implemented, but live activation remains off.
...
const _: () = {
    ...
    assert!(QUOTIENT_OPENING_BYTES_PER_OWNER_V1 == 527_616);
    assert!(QUOTIENT_OPENING_STREAM_SCALARS_V1 == 6_595_200);
    assert!(QUOTIENT_OPENING_STREAM_BYTES_V1 == 211_046_400);
...
pub(super) struct RnsNativeCrossFieldRlweFixedAxesV1 {
    ...
    source_formula_digest: [u8; DIGEST_BYTES_V1],
    source_mapping_digest: [u8; DIGEST_BYTES_V1],
...
impl RnsNativePreDirectInventoryCandidateAxesV1 {
    fn from_projection_v1(
        ...
        if !projection.is_valid_for_direct_fixed_axes_v1(&[]) {
            return Err(RnsNativeCrossFieldRlweDirectErrorV1::InvalidContext);
        }
        Ok(Self {
            origin: RnsNativePreDirectInventoryCandidateAxesOriginV1::Projection(projection),
        })
    }

    fn is_valid_with_fixed_axes_v1(&self, other_axes: &[[u8; DIGEST_BYTES_V1]]) -> bool {
        match &self.origin {
            RnsNativePreDirectInventoryCandidateAxesOriginV1::Projection(projection) => {
                projection.is_valid_for_direct_fixed_axes_v1(other_axes)
            }
            #[cfg(test)]
            RnsNativePreDirectInventoryCandidateAxesOriginV1::RawFixture {
                context_digest,
                inventory_root,
            } => {
                *context_digest != [0; DIGEST_BYTES_V1]
                    && *inventory_root != [0; DIGEST_BYTES_V1]
                    && context_digest != inventory_root
                    && !other_axes.contains(context_digest)
                    && !other_axes.contains(inventory_root)
            }
        }
    }

    fn absorb_fixed_axes_v1(&self, hash: &mut Keccak256) {
        match &self.origin {
            RnsNativePreDirectInventoryCandidateAxesOriginV1::Projection(projection) => {
                projection.absorb_direct_fixed_axes_v1(hash);
            }
            #[cfg(test)]
            RnsNativePreDirectInventoryCandidateAxesOriginV1::RawFixture {
                context_digest,
    ...
    fn permits_full_direct_bind_v1(&self) -> bool {
        match &self.origin {
            RnsNativePreDirectInventoryCandidateAxesOriginV1::Projection(_) => true,
            #[cfg(test)]
            RnsNativePreDirectInventoryCandidateAxesOriginV1::RawFixture { .. } => true,
        }
    }
    ...
    pub(super) fn test_fixture_v1(
        ...
        let value = Self {
            origin: RnsNativePreDirectInventoryCandidateAxesOriginV1::RawFixture {
                context_digest,
                inventory_root,
            },
        };
    ...
    fn bind_pending_existing_radix_safe_core_projection_v1(
        &self,
        _terminal_predecessor_context_binding_digest: [u8; DIGEST_BYTES_V1],
        _direct_core_safe_digest: [u8; DIGEST_BYTES_V1],
    ) -> Result<RnsNativeCrossFieldRlweSafeCoreProjectionV1, RnsNativeCrossFieldRlweDirectErrorV1>
    {
        match &self.origin {
            RnsNativePreDirectInventoryCandidateAxesOriginV1::Projection(projection) => {
                Ok(projection.bind_pending_existing_radix_safe_core_v1(
                    _terminal_predecessor_context_binding_digest,
                    _direct_core_safe_digest,
                ))
            }
            #[cfg(test)]
            RnsNativePreDirectInventoryCandidateAxesOriginV1::RawFixture {
                context_digest,
                inventory_root,
            } => Ok(RnsNativeCrossFieldRlweSafeCoreProjectionV1 {
                terminal_predecessor_context_binding_digest:
                    _terminal_predecessor_context_binding_digest,
                candidate_pre_direct_inventory_context_digest: *context_digest,
                candidate_pre_direct_inventory_root: *inventory_root,
                existing_radix_candidate_root: [0; DIGEST_BYTES_V1],
                direct_core_safe_digest: _direct_core_safe_digest,
            }),
        }
    }
}
...
/// Opaque one-shot input proving that every claimed-qPCS component came from
/// the same authenticated owner in the qPCS module.
///
/// The sole constructor requires the leaf-private qPCS origin token. Fields
...
#[allow(
    missing_copy_implementations,
    dead_code,
    reason = "the authenticated claimed-qPCS components must reach the direct bind atomically"
)]
pub(super) struct RnsNativeCrossFieldRlweClaimedQpcsInputV2 {
    completed_qpcs: RnsNativeQpcsCompletedLineageV1,
...
/// Atomic direct-schedule owner produced only from the authenticated claimed
...
#[allow(
    dead_code,
    missing_copy_implementations,
    reason = "the future claimed-successor transition consumes this owner once"
)]
#[must_use = "claimed direct inventory remains non-authorizing until successor verification and all root discharges"]
pub(super) struct RnsNativeCrossFieldRlweClaimedInventoryNumericV2<
    'qpcs,
    'cross,
...
/// Non-borrowing, schedule-free numeric authority retained by the exact
/// claimed-direct parent.
///
/// The fixed array shape prevents a short public cache from reaching the
...
#[allow(
    dead_code,
    missing_copy_implementations,
    reason = "the atomic membership/direct chronology owns this sidecar exactly once"
)]
struct RnsNativeCrossFieldRlweNumericSidecarV2 {
...
impl RnsNativeCrossFieldRlweNumericSidecarV2 {
    fn new_v2(
        ...
        if authoritative_binding_digest == [0; DIGEST_BYTES_V1] || !origin.is_fresh_v2() {
            return Err(RnsNativeCrossFieldRlweDirectErrorV1::InvalidContext);
        }
        Ok(Self {
            origin,
    ...
    fn is_complete_v2(&self) -> bool {
        self.origin.is_complete_v2()
    }
}
...
impl RnsNativeCrossFieldNumericCursorV1 for RnsNativeCrossFieldRlweNumericSidecarV2 {
    ...
    fn take_numeric_evaluation_v1(
        ...
        *destination = RnsNativeCrossFieldNumericEvaluationV1::default();
        let public = self.origin.take_public_evaluation_v2(limb, repetition)?;
...
{
    /// Consume the sole claimed relation/inventory owner and the exact
    /// unmintable whole-carrier numeric origin into the claimed-successor
    /// chronology.
    ///
    /// A cursor that was ever touched is rejected before frame decoding.  The
    /// claimed-source digest remains an integrity input to the retained
    /// publication wrapper, while the numeric sidecar accepts only the
    /// authoritative terminal source binding used by the direct schedule.
    pub(super) fn into_claimed_successor_stage_v2(
        self,
        numeric_origin: RnsNativeClaimedDirectNumericOriginV2,
    ) -> Result<
    ...
    > {
        ...
        let authoritative_binding_digest = claimed_relation
            ...
            .source_binding_digest();
        let numeric_sidecar = RnsNativeCrossFieldRlweNumericSidecarV2::new_v2(
            numeric_origin,
            numeric_tails,
            authoritative_binding_digest,
        )?;
        preflight_rns_native_cross_field_rlwe_claimed_frame_with_numeric_v2(
            claimed_relation,
            inventory,
            numeric_sidecar,
            frame_preflight,
    ...
    RnsNativeCrossFieldRlweClaimedInventoryParentV1<'source, 'proof, S>
{
    ...
    pub(super) fn take_existing_radix_validation_permit_v1(
        ...
        self.existing_radix_validation_permit.take()
    }
}
    ...
    > {
        ...
        RnsNativeClaimedSuccessorV1::from_direct_claim_v1(
            RnsNativeCrossFieldRlweClaimedInventoryParentV1 {
                ...
                inventory,
                numeric_sidecar,
                existing_radix_validation_permit: Some(existing_radix_validation_permit),
            },
            successor_claim,
        )
    }
...
impl RelationScheduleV1 {
    ...
    ) -> Result<RnsNativeCrossFieldRlweClaimedRelationV1, RnsNativeCrossFieldRlweDirectErrorV1>
    {
        ...
        let terminal_chronology = self
            .bound
            .completed_qpcs
            .bind_provisional_terminal_chronology_v1(roots)
            .map_err(map_qpcs_complete_error_v1)?;
        let (equality_obligation, terminal_chronology) =
            terminal_chronology.into_cross_field_obligation_and_global_pending_v1();
...
/// Schedule-free cursor that moves one complete quotient opening into
/// caller-owned, already-zeroizing destinations.
///
...
fn bind_direct_q_mask_schedule_v1(
    ...
    if !axes.candidate_inventory_axes.permits_full_direct_bind_v1()
        || !axes
            .existing_radix_candidate_axis
            .permits_full_direct_bind_v1()
    {
        return Err(RnsNativeCrossFieldRlweDirectErrorV1::InvalidContext);
    }
...
>(
    ...
    claimed_qpcs_input: RnsNativeCrossFieldRlweClaimedQpcsInputV2,
) -> Result<
    RnsNativeCrossFieldRlweClaimedInventoryNumericV2<'qpcs, 'cross, S>,
    RnsNativeCrossFieldRlweDirectErrorV1,
> {
    let RnsNativeCrossFieldRlweBoundPreDirectInventoryV1 {
        inventory,
        candidate_inventory_axes,
        pre_direct_nested,
    } = bound_pre_direct_inventory;
    let RnsNativeCrossFieldRlweClaimedQpcsInputV2 {
        completed_qpcs,
        terminal_chronology,
        numeric_tails,
        source_binding_digest,
    ...
    let RnsNativeCrossFieldRlwePreDirectNestedOwnerV1 {
        frame,
        existing_radix:
            RnsNativeExistingRadixPreDirectSplitV1 {
                axis: existing_radix_candidate_axis,
    ...
    let axes = RnsNativeCrossFieldRlweFixedAxesV1 {
        profile_manifest_digest: final_transcript.profile_manifest_digest(),
        source_binding_digest: final_transcript.source_binding_digest(),
        source_formula_digest: linked.source().formula_digest(),
        source_mapping_digest: linked.source().mapping_digest(),
        terminal_predecessor_binding_digest: linked.terminal().binding_digest(),
        candidate_inventory_axes,
        existing_radix_candidate_axis,
        rns_aggregation_challenge_seed: final_transcript.rns_aggregation_challenge_seed(),
        qpcs_parameter_digest: linked.source().qpcs().parameter_digest(),
        qpcs_pre_relation_transcript_digest: final_transcript.qpcs_pre_relation_transcript_digest(),
    };
    let source = AuthenticatedInventoryQMaskSourceV1 {
        inventory: &inventory,
    };
    ...
    let mut schedule = derive_relation_schedule_v1(bound)?;
    let (cross_field_root_equality_obligation, terminal_chronology) =
        terminal_chronology.into_cross_field_obligation_and_global_pending_v1();
    schedule.cross_field_root_equality_obligation = Some(cross_field_root_equality_obligation);
    validate_relation_schedule_v1(&schedule)?;
    Ok(RnsNativeCrossFieldRlweClaimedInventoryNumericV2 {
        claimed_relation: RnsNativeCrossFieldRlweClaimedRelationV1 {
            schedule,
            terminal_chronology,
        },
    ...
    P: RnsNativeCrossFieldQuotientOpeningSourceV1,
{
    ...
    for local in 0..EVALUATIONS_PER_CORE_V1 {
        ...
        source.take_next_quotient_opening_v1(
            ordinal,
...
fn encode_wire_preflighted_v1(
    ...
    if successor_preflight.successor_len != successor.len() {
        return Err(RnsNativeCrossFieldRlweDirectErrorV1::InvalidIntegrity);
    }
    let total_len = OWNED_WIRE_BYTES_V1
        .checked_add(successor_preflight.successor_len)
        .ok_or(RnsNativeCrossFieldRlweDirectErrorV1::ArithmeticOverflow)?;
    let proof_refs = core::array::from_fn(|core| proofs[core].as_slice());
    ...
    wire.try_reserve_exact(total_len)
        .map_err(|_| RnsNativeCrossFieldRlweDirectErrorV1::ResourceExhausted)?;
    ...
    for (core, proof) in proofs.iter().enumerate() {
...
impl<'a> FramePreflightV1<'a> {
    fn decode_exact_v1(bytes: &'a [u8]) -> Result<Self, RnsNativeCrossFieldRlweDirectErrorV1> {
        ...
        for (core, proof) in core_proofs.iter_mut().enumerate() {
            ...
            *proof = decoder.take_v1(CORE_PROOF_BYTES_V1)?;
        }
...
const PRE_DIRECT_COMPARATOR_OFFSET_V1: usize = OWNED_WIRE_BYTES_V1;
const PRE_DIRECT_RANGE_OFFSET_V1: usize = PRE_DIRECT_COMPARATOR_OFFSET_V1
    + PRE_DIRECT_COMPARATOR_HEADER_BYTES_V1
    + PRE_DIRECT_COMPARATOR_RECORD_BYTES_V1;
const PRE_DIRECT_SMALL_OFFSET_V1: usize = PRE_DIRECT_RANGE_OFFSET_V1
    + PRE_DIRECT_RANGE_HEADER_BYTES_V1
    + PRE_DIRECT_RANGE_RECORD_BYTES_V1;
const PRE_DIRECT_Q_MASK_OFFSET_V1: usize = PRE_DIRECT_SMALL_OFFSET_V1
    + PRE_DIRECT_SMALL_HEADER_BYTES_V1
    + PRE_DIRECT_SMALL_RECORD_BYTES_V1;
const PRE_DIRECT_EXISTING_RADIX_OFFSET_V1: usize = PRE_DIRECT_Q_MASK_OFFSET_V1
    + PRE_DIRECT_Q_MASK_HEADER_BYTES_V1
    + PRE_DIRECT_Q_MASK_RECORD_BYTES_V1;
const PRE_DIRECT_EXISTING_RADIX_INVENTORY_OFFSET_V1: usize =
    PRE_DIRECT_EXISTING_RADIX_OFFSET_V1 + 414;
const PRE_DIRECT_EXISTING_RADIX_INVENTORY_END_V1: usize =
...
fn pre_direct_nested_residual_v1<'a>(
    ...
    record_bytes: usize,
    codec_domain: &[u8],
) -> Result<&'a [u8], RnsNativeCrossFieldRlweDirectErrorV1> {
    let minimum = header_bytes
        .checked_add(record_bytes)
        .and_then(|value| value.checked_add(1 + DIGEST_BYTES_V1))
        .ok_or(RnsNativeCrossFieldRlweDirectErrorV1::ArithmeticOverflow)?;
    if bytes.len() < minimum
        || bytes.get(..4) != Some(magic.as_slice())
        || bytes.get(4).copied() != Some(VERSION_V1)
        || bytes.get(5).copied() != Some(FLAGS_V1)
    {
        return Err(RnsNativeCrossFieldRlweDirectErrorV1::InvalidHeader);
    }
    let encoded_header = u16::from_be_bytes(
        bytes
            .get(6..8)
            .ok_or(RnsNativeCrossFieldRlweDirectErrorV1::InvalidHeader)?
            .try_into()
    ...
    let encoded_total = u32::from_be_bytes(
        bytes
            .get(8..12)
            .ok_or(RnsNativeCrossFieldRlweDirectErrorV1::InvalidHeader)?
            .try_into()
    ...
    let residual_length_offset = header_bytes
        .checked_sub(4)
        .ok_or(RnsNativeCrossFieldRlweDirectErrorV1::ArithmeticOverflow)?;
    let residual_len = usize::try_from(u32::from_be_bytes(
        bytes
            .get(residual_length_offset..header_bytes)
            .ok_or(RnsNativeCrossFieldRlweDirectErrorV1::InvalidHeader)?
            .try_into()
            .map_err(|_| RnsNativeCrossFieldRlweDirectErrorV1::InvalidHeader)?,
    ))
    .map_err(|_| RnsNativeCrossFieldRlweDirectErrorV1::ArithmeticOverflow)?;
    let residual_offset = header_bytes
        .checked_add(record_bytes)
        .ok_or(RnsNativeCrossFieldRlweDirectErrorV1::ArithmeticOverflow)?;
    let codec_offset = residual_offset
        .checked_add(residual_len)
        .ok_or(RnsNativeCrossFieldRlweDirectErrorV1::ArithmeticOverflow)?;
    let expected_total = codec_offset
        .checked_add(DIGEST_BYTES_V1)
        .ok_or(RnsNativeCrossFieldRlweDirectErrorV1::ArithmeticOverflow)?;
    if usize::from(encoded_header) != header_bytes
        || usize::try_from(encoded_total)
            .map_err(|_| RnsNativeCrossFieldRlweDirectErrorV1::ArithmeticOverflow)?
            != bytes.len()
        || residual_len == 0
    ...
    let expected_codec = hash.finalize();
    if bytes.get(codec_offset..) != Some(expected_codec.as_slice()) {
        return Err(RnsNativeCrossFieldRlweDirectErrorV1::InvalidIntegrity);
    }
    bytes
        .get(residual_offset..codec_offset)
        .ok_or(RnsNativeCrossFieldRlweDirectErrorV1::InvalidHeader)
}
...
fn relative_slice_offset_v1(base: &[u8], child: &[u8]) -> Option<usize> {
    ...
    (child_start >= base_start && child_end <= base_end)
        .then(|| child_start.checked_sub(base_start))?
}
...
#[allow(
    missing_copy_implementations,
    dead_code,
    reason = "the frame, axis, and permit must advance through one claimed chronology"
)]
struct RnsNativeCrossFieldRlwePreDirectNestedOwnerV1<'proof> {
    frame: FramePreflightV1<'proof>,
    existing_radix: RnsNativeExistingRadixPreDirectSplitV1,
}

/// One opaque move-only owner of the authenticated inventory and both
/// pre-direct projections derived from that exact inventory allocation.
///
/// Its fields are deliberately private: sibling modules can obtain this owner
...
#[allow(
    missing_copy_implementations,
    dead_code,
    reason = "the exact inventory and both pre-direct children must advance atomically"
)]
pub(super) struct RnsNativeCrossFieldRlweBoundPreDirectInventoryV1<
    'source,
    'proof,
    S: ZkAmsMkheRnsNativeSourceSnapshotV1,
> {
    inventory: RnsNativeCrossFieldInventoryPrerequisiteV1<'source, 'proof, S>,
    candidate_inventory_axes: RnsNativePreDirectInventoryCandidateAxesV1,
    pre_direct_nested: RnsNativeCrossFieldRlwePreDirectNestedOwnerV1<'proof>,
}
...
fn preflight_rns_native_cross_field_rlwe_nested_owner_v1<'source, 'proof, S>(
    ...
    RnsNativeCrossFieldRlwePreDirectNestedOwnerV1<'proof>,
    RnsNativeCrossFieldRlweDirectErrorV1,
>
where
    S: ZkAmsMkheRnsNativeSourceSnapshotV1,
{
    let base = inventory.continuation();
    let frame = FramePreflightV1::decode_exact_v1(base)?;
    let comparator = frame.successor;
    let range = pre_direct_nested_residual_v1(
        comparator,
        PRE_DIRECT_COMPARATOR_MAGIC_V1,
        PRE_DIRECT_COMPARATOR_HEADER_BYTES_V1,
        PRE_DIRECT_COMPARATOR_RECORD_BYTES_V1,
        PRE_DIRECT_COMPARATOR_CODEC_DOMAIN_V1,
    )?;
    let small = pre_direct_nested_residual_v1(
        range,
        PRE_DIRECT_RANGE_MAGIC_V1,
        PRE_DIRECT_RANGE_HEADER_BYTES_V1,
        PRE_DIRECT_RANGE_RECORD_BYTES_V1,
        PRE_DIRECT_RANGE_CODEC_DOMAIN_V1,
    )?;
    let q_mask = pre_direct_nested_residual_v1(
        small,
        PRE_DIRECT_SMALL_MAGIC_V1,
        PRE_DIRECT_SMALL_HEADER_BYTES_V1,
        PRE_DIRECT_SMALL_RECORD_BYTES_V1,
        PRE_DIRECT_SMALL_CODEC_DOMAIN_V1,
    )?;
    let existing_radix = pre_direct_nested_residual_v1(
        q_mask,
        PRE_DIRECT_Q_MASK_MAGIC_V1,
        PRE_DIRECT_Q_MASK_HEADER_BYTES_V1,
        PRE_DIRECT_Q_MASK_RECORD_BYTES_V1,
        PRE_DIRECT_Q_MASK_CODEC_DOMAIN_V1,
    )?;
    if relative_slice_offset_v1(base, comparator) != Some(PRE_DIRECT_COMPARATOR_OFFSET_V1)
        || relative_slice_offset_v1(base, range) != Some(PRE_DIRECT_RANGE_OFFSET_V1)
        || relative_slice_offset_v1(base, small) != Some(PRE_DIRECT_SMALL_OFFSET_V1)
        || relative_slice_offset_v1(base, q_mask) != Some(PRE_DIRECT_Q_MASK_OFFSET_V1)
        || relative_slice_offset_v1(base, existing_radix)
            != Some(PRE_DIRECT_EXISTING_RADIX_OFFSET_V1)
    {
        return Err(RnsNativeCrossFieldRlweDirectErrorV1::InvalidContext);
    }
    let existing_radix = preflight_rns_native_existing_radix_candidate_v1(base, existing_radix)
        .map_err(|_| RnsNativeCrossFieldRlweDirectErrorV1::InvalidIntegrity)?;
    Ok(RnsNativeCrossFieldRlwePreDirectNestedOwnerV1 {
        frame,
        existing_radix,
    })
}

/// Consume one authenticated inventory into the only production owner that
...
pub(super) fn bind_rns_native_cross_field_rlwe_pre_direct_inventory_v1<'source, 'proof, S>(
    ...
    RnsNativeCrossFieldRlweBoundPreDirectInventoryV1<'source, 'proof, S>,
    RnsNativeCrossFieldRlweDirectErrorV1,
>
where
    S: ZkAmsMkheRnsNativeSourceSnapshotV1,
{
    let candidate_projection = inventory
        .take_pre_direct_candidate_projection_v1()
        .map_err(|_| RnsNativeCrossFieldRlweDirectErrorV1::InvalidContext)?;
    let candidate_inventory_axes =
        RnsNativePreDirectInventoryCandidateAxesV1::from_projection_v1(candidate_projection)?;
    let pre_direct_nested = preflight_rns_native_cross_field_rlwe_nested_owner_v1(&inventory)?;
    Ok(RnsNativeCrossFieldRlweBoundPreDirectInventoryV1 {
        inventory,
        candidate_inventory_axes,
        pre_direct_nested,
    })
...
fn preflight_rns_native_cross_field_rlwe_claimed_frame_with_numeric_v2<'source, 'proof, S>(
    claimed_relation: RnsNativeCrossFieldRlweClaimedRelationV1,
    inventory: RnsNativeCrossFieldInventoryPrerequisiteV1<'source, 'proof, S>,
    numeric_sidecar: RnsNativeCrossFieldRlweNumericSidecarV2,
    preflight: FramePreflightV1<'proof>,
    ...
    S: ZkAmsMkheRnsNativeSourceSnapshotV1,
{
    ...
    if relative_slice_offset_v1(inventory.continuation(), preflight.successor)
        != Some(PRE_DIRECT_COMPARATOR_OFFSET_V1)
    {
        return Err(RnsNativeCrossFieldRlweDirectErrorV1::InvalidContext);
    }
...
impl<'a> RnsNativeCrossFieldRlweClaimEqualityPendingVerifiedV1<'a> {
    ...
    #[cfg(test)]
    const fn binding_digest(&self) -> [u8; DIGEST_BYTES_V1] {
        self.binding_digest
    }
}
    ...
    P: RnsNativeCrossFieldAuthoritativeSourceV1,
{
    ...
    let mut transcript_digests = [[0_u8; DIGEST_BYTES_V1]; CORES_V1];
    for (core, transcript_digest) in transcript_digests.iter_mut().enumerate() {
        let state = initial_core_transcript_state_v1(&inputs, core)?;
    ...
    let pending_safe_core_projection = inputs
        .schedule
        .bound
        .axes
        .candidate_inventory_axes
        .bind_pending_existing_radix_safe_core_projection_v1(
            inputs
                .schedule
                .bound
                .axes
    ...
    let safe_core_projection = inputs
        .schedule
        .bound
        .axes
        .existing_radix_candidate_axis
        .complete_verified_safe_core_projection_v1(pending_safe_core_projection)
        .map_err(|_| RnsNativeCrossFieldRlweDirectErrorV1::InvalidContext)?;
...
fn validate_claimed_handoff_fixed_axes_v1<S: ZkAmsMkheRnsNativeSourceSnapshotV1>(
    ...
    existing_radix: &RnsNativeExistingRadixDirectAliasV1<'_>,
) -> Result<(), RnsNativeCrossFieldRlweDirectErrorV1> {
    validate_claimed_inventory_transcript_v1(
        claimed_relation,
        inventory.terminal_transcript_digest_v1(),
    ...
    let final_transcript = claimed_relation
        .terminal_chronology
        .final_challenge_seeds_v1();
    let linked = inventory.linked();
    if axes.profile_manifest_digest != final_transcript.profile_manifest_digest()
        ...
        || !axes
            .existing_radix_candidate_axis
            .matches_authenticated_alias_v1(inventory.continuation(), existing_radix)
...
#[allow(
    dead_code,
    missing_copy_implementations,
    reason = "the final membership handoff consumes this owner exactly once"
)]
pub(super) struct RnsNativeCrossFieldRlweAtomicVerifiedV2<
    'source,
    'proof,
    S: ZkAmsMkheRnsNativeSourceSnapshotV1,
> {
...
#[allow(
    dead_code,
    missing_copy_implementations,
    reason = "the source-packing handoff retains this exact verified owner"
)]
pub(super) struct RnsNativeCrossFieldRlweAllRootsVerifiedV2<
    'source,
    'proof,
    S: ZkAmsMkheRnsNativeSourceSnapshotV1,
> {
...
{
    pub(super) fn discharge_terminal_roots_v2(
        self,
        verified_global_lookup_root: RnsNativeGlobalLookupVerifiedCoreRootV2,
    ) -> Result<
    ...
    S: ZkAmsMkheRnsNativeSourceSnapshotV1,
{
    ...
    if existing_radix_validation_permit.is_some() {
        return Err(RnsNativeCrossFieldRlweDirectErrorV1::InvalidContext);
    }
    validate_claimed_handoff_fixed_axes_v1(&claimed_relation, &inventory, &existing_radix)?;
    preflight.validate_schedule_v1(&claimed_relation.schedule)?;
    ...
    let equality_pending = {
        ...
        verify_preflighted_kernel_for_suite_v1::<ZkAmsT256BulletproofSuiteV1, _>(
            schedule, source, preflight,
        )?
    };
    if !numeric_sidecar.is_complete_v2() {
