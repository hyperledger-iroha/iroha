...
fn equations_endpoints_ipas_and_exclusive_gate_schedule_are_exact() {
    ...
    assert_eq!(
        ...
        HiddenEndpointRoleV1::Equation {
            ...
            endpoint: EquationEndpointV1::Residual,
        }
    );
    assert_eq!(
...
fn centering_boundary_recurrence_has_exact_sign_and_eighteen_borrows() {
    ...
    let formulas = [CoefficientEquationRoleV1::CenteringComparatorSubtraction.formula_v1(), CoefficientEquationRoleV1::ComparatorBorrowBooleanity.formula_v1(), CoefficientEquationRoleV1::CenteredLiftSelector.formula_v1()].concat();
    assert!(formulas.windows(b"borrow[0..17]".len()).any(|w| w == b"borrow[0..17]"));
    assert!(formulas.windows(b"for h=1..16".len()).any(|w| w == b"for h=1..16"));
    assert!(formulas.windows(b"K_17=0".len()).any(|w| w == b"K_17=0"));
    assert!(formulas.windows(b"mixed_top=b_D*borrow_16".len()).any(|w| w == b"mixed_top=b_D*borrow_16"));
...
fn conditional_accounting_soundness_and_fail_closed_gates_are_frozen() {
    assert_eq!(conditional_accounting_v1(), Ok((33_230_555, 323_877)));
...
fn opaque_owner_and_source_guards_are_static() {
    ...
    assert!(challenges.contains("FIRST_SUMCHECK_ORDINAL_V1 == MU_ORDINAL_V1 + 1"));
