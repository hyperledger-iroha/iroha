...
fn axes_v1() -> RnsNativeCrossFieldRlweFixedAxesV1 {
    RnsNativeCrossFieldRlweFixedAxesV1 {
        ...
        existing_radix_candidate_axis: RnsNativeExistingRadixCandidateAxisV1::test_fixture_v1(
            digest_v1(8),
        )
...
impl RnsNativeCrossFieldQuotientOpeningCursorV1 for TouchSourceV1<'_> {
    fn take_next_quotient_opening_v1(
        &mut self,
        _relation_ordinal: usize,
        _sign: RnsNativeCrossFieldQuotientOpeningSignV1,
        _values: &mut [Scalar],
...
fn exact_four_core_geometry_and_cap_are_settled() {
    ...
    assert_eq!(QUOTIENT_OPENING_BYTES_PER_OWNER_V1, 527_616);
    assert_eq!(QUOTIENT_OPENING_STREAM_SCALARS_V1, 6_595_200);
    assert_eq!(QUOTIENT_OPENING_STREAM_BYTES_V1, 211_046_400);
...
struct FullZeroSourceV1 {
    ...
    next_opening_owner: usize,
    opening_poisoned: bool,
...
impl FullZeroSourceV1 {
    fn new_v1(schedule: &RelationScheduleV1) -> Result<Self, RnsNativeCrossFieldRlweDirectErrorV1> {
        ...
        Ok(Self {
            ...
            next_opening_owner: 0,
            opening_poisoned: false,
            algebra_fault: None,
        })
    }
...
impl RnsNativeCrossFieldQuotientOpeningCursorV1 for FullZeroSourceV1 {
    fn take_next_quotient_opening_v1(
        &mut self,
        ...
        if self.next_opening_owner >= QUOTIENT_OPENING_OWNERS_V1
        ...
        *commitment_mask = match sign {
            RnsNativeCrossFieldQuotientOpeningSignV1::Positive => {
                self.positive_masks[relation_ordinal]
            }
            RnsNativeCrossFieldQuotientOpeningSignV1::Negative => {
                self.negative_masks[relation_ordinal]
            }
        };
        self.next_opening_owner += 1;
        self.opening_poisoned = false;
        Ok(())
    }
}
...
fn q_mask_root_uses_only_pre_qpcs_safe_axes_and_minimal_point_source() {
    ...
    changed_post_qpcs.existing_radix_candidate_axis =
        RnsNativeExistingRadixCandidateAxisV1::test_fixture_v1(digest_v1(44))
            .expect("changed existing-radix candidate axis");
    assert_eq!(
...
#[test]
fn pre_direct_candidate_axes_only_consume_the_sealed_projection_in_production() {
    ...
    let constructor = candidate_impl
        .split_once("fn from_projection_v1(")
        .expect("sealed projection constructor")
        .1
        .split_once(") -> Result<Self")
        .expect("sealed projection constructor signature")
        .0;
    assert!(constructor.contains("RnsNativePreDirectInventoryCandidateProjectionV1"));
    assert!(!constructor.contains("[u8;"));
    assert!(!candidate_impl.contains("pub(super) fn from_projection_v1("));
...
#[test]
fn bound_pre_direct_inventory_is_the_only_production_direct_bind_surface() {
    let source = include_str!("rns_native_cross_field_rlwe_direct.rs");
    let owner_declaration = source
        .find("pub(super) struct RnsNativeCrossFieldRlweBoundPreDirectInventoryV1")
        .expect("bound pre-direct inventory owner");
    let owner_prefix = &source[owner_declaration.saturating_sub(320)..owner_declaration];
    assert!(!owner_prefix.contains("derive(Clone"));
    assert!(!owner_prefix.contains("derive(Copy"));
    let owner = source[owner_declaration..]
        .split_once("/// Preflight the recursive successor allocation")
        .expect("bound owner boundary")
        .0;
    for field in [
        "inventory: RnsNativeCrossFieldInventoryPrerequisiteV1",
        "candidate_inventory_axes: RnsNativePreDirectInventoryCandidateAxesV1",
        "pre_direct_nested: RnsNativeCrossFieldRlwePreDirectNestedOwnerV1",
    ] {
        assert_eq!(owner.matches(field).count(), 1, "bound field: {field}");
    }
    assert!(!owner.contains("pub(super) inventory:"));
    assert!(!owner.contains("pub(super) candidate_inventory_axes:"));
    assert!(!owner.contains("pub(super) pre_direct_nested:"));
    assert!(!source.contains("Clone for RnsNativeCrossFieldRlweBoundPreDirectInventoryV1"));
    assert!(!source.contains("Copy for RnsNativeCrossFieldRlweBoundPreDirectInventoryV1"));

    let constructor = source
        .split_once("pub(super) fn bind_rns_native_cross_field_rlwe_pre_direct_inventory_v1")
        .expect("single-inventory pre-direct constructor")
        .1
        .split_once("fn validate_claimed_inventory_transcript_v1")
        .expect("single-inventory constructor boundary")
        .0;
    let signature = constructor
        .split_once(") -> Result<")
        .expect("single-inventory constructor signature")
        .0;
    assert!(signature.contains("inventory: RnsNativeCrossFieldInventoryPrerequisiteV1"));
    assert!(!signature.contains("candidate_inventory_axes:"));
    assert!(!signature.contains("pre_direct_nested:"));
    let take = constructor
        .find("take_pre_direct_candidate_projection_v1()")
        .expect("take exact candidate projection");
    let convert = constructor
        .find("RnsNativePreDirectInventoryCandidateAxesV1::from_projection_v1")
        .expect("convert exact candidate projection");
    let nested = constructor
        .find("preflight_rns_native_cross_field_rlwe_nested_owner_v1(&inventory)")
        .expect("preflight the same inventory");
    let mint = constructor
        .find("Ok(RnsNativeCrossFieldRlweBoundPreDirectInventoryV1")
        .expect("mint bound owner");
    assert!(take < convert && convert < nested && nested < mint);
    ...
    let direct_bind = source
        ...
        .1
        .split_once(") -> Result<")
        .expect("production direct bind signature")
        .0;
    assert!(direct_bind.contains("bound_pre_direct_inventory:"));
    assert!(direct_bind.contains("claimed_qpcs_input:"));
    assert!(!direct_bind.contains("inventory: RnsNativeCrossFieldInventoryPrerequisiteV1"));
    assert!(!direct_bind.contains("candidate_inventory_axes:"));
    assert!(!direct_bind.contains("pre_direct_nested:"));
    for detached_qpcs_input in [
        "completed_qpcs:",
        "terminal_chronology:",
        "numeric_tails:",
        ...
        assert!(
            !direct_bind.contains(detached_qpcs_input),
            "detached qPCS input leaked into direct bind: {detached_qpcs_input}"
        );
    }

    let claimed_qpcs_declaration = source
        .find("pub(super) struct RnsNativeCrossFieldRlweClaimedQpcsInputV2")
        .expect("opaque claimed-qPCS input");
    let claimed_qpcs_prefix =
        &source[claimed_qpcs_declaration.saturating_sub(320)..claimed_qpcs_declaration];
    assert!(!claimed_qpcs_prefix.contains("derive(Clone"));
    assert!(!claimed_qpcs_prefix.contains("derive(Copy"));
    let claimed_qpcs_owner = source[claimed_qpcs_declaration..]
        .split_once("impl RnsNativeCrossFieldRlweClaimedQpcsInputV2")
        .expect("opaque claimed-qPCS input boundary")
        .0;
    for field in [
        ...
        assert_eq!(
            claimed_qpcs_owner.matches(field).count(),
            1,
            "claimed-qPCS field: {field}"
        );
    }
    assert!(!claimed_qpcs_owner.contains("pub(super) completed_qpcs:"));
    assert!(!claimed_qpcs_owner.contains("pub(super) terminal_chronology:"));
    assert!(!claimed_qpcs_owner.contains("pub(super) numeric_tails:"));
    assert!(!claimed_qpcs_owner.contains("pub(super) source_binding_digest:"));
    assert!(!claimed_qpcs_owner.contains("pub(super) _origin:"));
    assert!(!source.contains("impl Clone for RnsNativeCrossFieldRlweClaimedQpcsInputV2"));
    assert!(!source.contains("impl Copy for RnsNativeCrossFieldRlweClaimedQpcsInputV2"));
}
...
fn claimed_relation_surface_is_atomic_move_only_and_source_ordered() {
    ...
    assert!(capability_forward.contains(".terminal_chronology"));
    ...
    let preflight = source
        .split_once("fn preflight_rns_native_cross_field_rlwe_claimed_frame_with_numeric_v2")
    ...
    assert!(preflight.contains("preflight: FramePreflightV1<'proof>"));
    assert!(!preflight.contains("FramePreflightV1::decode_exact_v1"));
    let successor_identity = preflight
        .find("relative_slice_offset_v1(inventory.continuation(), preflight.successor)")
        .expect("cached successor allocation identity");
    let schedule_match = preflight
        .find("preflight.validate_schedule_v1(&claimed_relation.schedule)")
        .expect("exact schedule match");
    let sidecar_move = preflight
        .rfind("numeric_sidecar,")
        .expect("numeric sidecar enters exact parent");
    let carrier = preflight
        .find(".into_claimed_successor_v1()")
        .expect("sole successor carrier");
    assert!(
        transcript_match < successor_identity
            && successor_identity < schedule_match
            && schedule_match < sidecar_move
...
fn source_and_transcript_privacy_invariants_are_source_settled() {
    ...
    assert_eq!(
        source
            .matches("RnsNativeMembershipBackedDirectSourceV1 {")
            .count(),
        1,
        "membership-backed source is constructed only by the atomic claimed verifier"
    );
    let atomic_claimed = source
        .split_once("pub(super) fn verify_rns_native_cross_field_rlwe_claimed_with_alias_v2")
        .expect("atomic claimed verifier")
        .1
        .split_once("#[cfg(test)]")
        .expect("atomic claimed verifier boundary")
        .0;
    assert!(atomic_claimed.contains("let source = RnsNativeMembershipBackedDirectSourceV1 {"));

    let opening_cursor = source
        .split_once("pub(super) trait RnsNativeCrossFieldQuotientOpeningCursorV1: Sized")
        .expect("split opening cursor")
        .1
        .split_once("/// Full prover source marker")
        .expect("opening cursor boundary")
        .0;
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "opening cursor leaks {forbidden}"
        );
    }
    assert!(!source.contains("fn take_positive_quotient_owner_v1("));
    assert!(!source.contains("fn take_negative_quotient_owner_v1("));
    ...
    let fixed_axes = source
        ...
        .expect("fixed axes")
        .1
        .split_once("/// Opaque successor-independent inventory axis")
    ...
    for candidate_axis in [
        ...
        assert!(
            ...
            "missing sealed pre-direct candidate axis: {candidate_axis}"
        );
    }
    ...
    let candidate_axes = source
        ...
        .0;
    let candidate_wrapper = candidate_axes
        .split_once("\n}")
        .expect("opaque candidate wrapper boundary")
        .0;
    assert!(candidate_wrapper.contains("origin: RnsNativePreDirectInventoryCandidateAxesOriginV1"));
    assert!(!candidate_wrapper.contains("context_digest:"));
    assert!(!candidate_wrapper.contains("inventory_root:"));
    assert!(
        candidate_axes.contains("Projection(RnsNativePreDirectInventoryCandidateProjectionV1)")
    );
