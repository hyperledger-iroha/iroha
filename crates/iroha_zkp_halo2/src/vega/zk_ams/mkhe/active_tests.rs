...
fn roster_pop_binds_full_roster_epoch_index_party_key_and_commitment() {
    ...
    for mutation in 0..7 {
        let mut changed = fixture.roster;
        match mutation {
            0 => changed.epoch += 1,
...
fn governed_roster_and_authentication_capacity_source_is_stack_only() {
    ...
    let contribution = active
        .split("impl ZkAmsMkheActiveContributionV1")
        .nth(1)
        .unwrap()
        .split("fn authenticate_active_contribution")
        .next()
        .unwrap();
    let contribution_statement = active
        .split("fn active_contribution_statement_digest(")
        .nth(1)
        .unwrap()
        .split("fn validate_active_contribution")
        .next()
        .unwrap();
    for corridor in [
        roster,
    ...
    for corridor in [contribution, contribution_statement] {
        assert!(corridor.contains("let mut hash = Keccak256::new()"));
        assert!(!corridor.contains("with_capacity"));
        assert!(!corridor.contains("keccak256("));
    }
