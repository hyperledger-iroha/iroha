...
//! * the collective streaming producer owns canonical packed chunks and native
//!   RLWE source publication in the exact 43-record family/chunk order; this
//!   kernel retains public geometry and the authenticated source prerequisite
//!   without a duplicate packed-accumulator or topology-only opening API;
...
// Compile the private source owner against the canonical crypto spool API.
...
fn rns_link_secret_inner_product_v1(
    ...
    let mut sum = ZeroizingT256ScalarCopyV1::new(Scalar::zero());
    for (left, right) in left.iter().zip(right) {
        sum.add_product_assign(left, right);
    }
    Ok(sum)
}
...
fn rns_link_secret_msm_with_extra_v1(
    ...
    let term_count = scalars
        .len()
        .checked_add(usize::from(extra.is_some()))
        .ok_or(ZkAmsMkheErrorV1::InvalidPhase23Fold)?;
    let mut terms = SecretMultiexpBuilder::<ZkAmsT256BulletproofSuiteV1>::new(term_count)
        .map_err(|_| ZkAmsMkheErrorV1::InvalidPhase23Fold)?;
    ...
    if let Some((scalar, point)) = extra {
        terms
            .push(scalar, point)
            .map_err(|_| ZkAmsMkheErrorV1::InvalidPhase23Fold)?;
    }
...
fn prove_rns_link_ipa_v1(
    ...
    while witness.len() > 1 {
        ...
        let left = rns_link_secret_msm_with_extra_v1(
            a_left,
            g_right,
            Some((c_left.as_ref(), &key.evaluation_generator)),
        )?;
        let right = rns_link_secret_msm_with_extra_v1(
            a_right,
            g_left,
...
fn prove_rns_link_bitness_sumcheck_v1(
    ...
    while bits.len() > 1 {
        ...
        for index in 0..half {
            let delta_a = ZeroizingT256ScalarCopyV1::new(a_one[index] - a_zero[index]);
            let delta_b = ZeroizingT256ScalarCopyV1::new(b_one[index] - b_zero[index]);
            ...
            for (ordinal, evaluation) in evaluations.iter_mut().enumerate() {
                ...
                *evaluation += (eq_zero[index] + point * delta_eq)
                    * (a_zero[index] + point * delta_a.get())
                    * (b_zero[index] + point * delta_b.get());
            }
        }
...
mod tests {
    ...
    const CONTEXT_AUTHORITY_SOURCE_KECCAK_V1: [u8; 32] = [
        0x48, 0x2f, 0x06, 0xe1, 0x05, 0xfc, 0x20, 0xbc, 0x79, 0xf0, 0x8b, 0x13, 0xde, 0xce, 0x34,
        0xb3, 0x7e, 0x7b, 0xa9, 0xce, 0x02, 0x2c, 0x32, 0x05, 0xf4, 0xc1, 0x2f, 0x2c, 0x05, 0x69,
        0xf7, 0x4a,
    ];
    const _: fn() = || {
    ...
    fn production_challenge_source_has_no_readiness_or_kat_reference() {
        ...
        assert!(production.contains("mut blinding: Scalar,"));
        assert!(
            production.contains("let blinding = ZeroizingT256ScalarCopyV1::take(&mut blinding);")
        );
        assert!(!production.contains("ZeroizingT256ScalarCopyV1::new(blinding)"));
        let bitness_prover = production
            .split_once("fn prove_rns_link_bitness_v1(")
            .expect("bitness prover definition")
            .1
            .split_once("\nfn verify_rns_link_bitness_v1(")
            .expect("bitness verifier follows prover")
            .0;
        let transfer = bitness_prover
            .find("let blinding = ZeroizingT256ScalarCopyV1::take(&mut blinding);")
            .expect("callee-owned blinding transfer");
        let first_fallible_validation = bitness_prover
            .find("if !is_nonzero_digest(relation_context_digest)")
            .expect("first bitness validation");
        assert!(
            transfer < first_fallible_validation,
            "the callee-owned Copy scalar must enter its erasing owner before any early return"
        );
        assert!(!bitness_prover.contains("bits.as_slice().iter().copied()"));
        assert!(bitness_prover.contains("for bit in bits.as_slice()"));
        assert!(bitness_prover.contains("ipa_witness.push(*bit);"));
        let bitness_sumcheck = production
            .split_once("fn prove_rns_link_bitness_sumcheck_v1(")
            .expect("bitness sumcheck prover")
            .1
            .split_once("\nfn verify_rns_link_bitness_sumcheck_v1(")
            .expect("bitness sumcheck verifier follows prover")
            .0;
        assert!(!bitness_sumcheck.contains("bits.as_slice().iter().copied()"));
        assert!(bitness_sumcheck.contains("for bit in bits.as_slice()"));
        assert!(bitness_sumcheck.contains("bit_minus_one.push(*bit - Scalar::one());"));
        assert!(!bitness_sumcheck.contains("let delta_a = a_one[index] - a_zero[index]"));
        assert!(!bitness_sumcheck.contains("let delta_b = b_one[index] - b_zero[index]"));
        assert!(bitness_sumcheck.contains("ZeroizingT256ScalarCopyV1::new(a_one[index]"));
        assert!(bitness_sumcheck.contains("ZeroizingT256ScalarCopyV1::new(b_one[index]"));
        let secret_inner_product = production
            .split_once("fn rns_link_secret_inner_product_v1(")
            .expect("secret inner-product helper")
            .1
            .split_once("\nfn rns_link_secret_msm_v1(")
            .expect("secret MSM follows inner product")
            .0;
        assert!(!secret_inner_product.contains(".iter().copied()"));
        assert!(!secret_inner_product.contains(".fold(Scalar::zero()"));
        assert!(secret_inner_product.contains("let mut sum = ZeroizingT256ScalarCopyV1::new"));
        assert!(secret_inner_product.contains("sum.add_product_assign(left, right);"));
        let secret_msm = production
            .split_once("fn rns_link_secret_msm_v1(")
            .expect("secret MSM helper")
            .1
            .split_once("\nfn rns_link_squeeze_nonzero_v1(")
            .expect("transcript squeeze follows secret MSM")
            .0;
        assert!(!secret_msm.contains(".iter().copied()"));
        ...
        assert!(production.contains("Some((c_right.as_ref(), &key.evaluation_generator))"));
    ...
    #[test]
    fn retired_bulk_accumulator_and_topology_only_opening_paths_are_absent() {
        let source = include_str!("phase23_rns_link.rs");
        ...
        for retired in [
            ...
            "verify_and_consume_phase23_native_bgv_opening_v1",
        ] {
            assert!(!production.contains(retired), "retired owner: {retired}");
        }
        let directory =
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src/vega/zk_ams/mkhe");
        for file in [
            "phase23_rns_link_state_owned.rs",
            "phase23_rns_link_q_relation_adapter.rs",
        ] {
            assert!(!directory.join(file).exists(), "retired source: {file}");
        }
        assert!(production.contains("mod external_source;"));
        assert!(production.contains("mod cross_field_v2;"));
    }
    #[test]
