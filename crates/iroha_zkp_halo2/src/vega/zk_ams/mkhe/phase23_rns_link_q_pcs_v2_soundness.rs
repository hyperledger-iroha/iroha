...
//!
//! This private child is deliberately non-authorizing. It parses a bounded
//! canonical envelope, derives all 190 relation points only after the masked
//! ten-row root, checks every `P~(r) = (r^N + 1)H~(r)` identity before binding
//! the opening-quotient root, and then derives all ten batching and FRI-fold
//! challenge schedules. It also enforces exact multiproof shapes and canonical
//! field encodings. A private child authenticates those borrowed multiproofs
//! and verifies the quotient, batching, and fold equations. The production
//! constructor remains uninhabited until authenticated source replay exists.

use core::{convert::Infallible, fmt};

use crate::vega::sponge::{Shake256Reader, keccak256};

use super::{RELEASE_MODULI_V1, mod_add_v1, mod_mul_v1, mod_pow_v1};
...
/// Shared verifier-literal post-C0 challenge state for the private prover.
pub(super) struct ProverPostRootPointsV2 {
    live: Option<ProverPostRootLiveV2>,
}

/// Exact evaluations have been checked and absorbed, but Cq is not bound.
pub(super) struct ProverEvaluationsBoundV2 {
    live: Option<ProverPostRootLiveV2>,
}

/// The opening-quotient root and its future batch schedule are frozen.
