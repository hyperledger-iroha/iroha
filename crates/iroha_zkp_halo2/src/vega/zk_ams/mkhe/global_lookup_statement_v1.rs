...
const CONDITIONAL_TOTAL_BYTES_V1: usize = 33_230_555;
const CONDITIONAL_CAP_BYTES_V1: usize = 32 * 1_048_576;
const CONDITIONAL_MARGIN_BYTES_V1: usize = 323_877;
...
const _: () = {
    ...
    assert!(CONDITIONAL_MARGIN_BYTES_V1 == CONDITIONAL_CAP_BYTES_V1 - CONDITIONAL_TOTAL_BYTES_V1);
...
impl CoefficientEquationRoleV1 {
    const fn formula_v1(self) -> &'static [u8] {
        match self {
            ...
            Self::BatchedTopBooleanAndExclusion => b"bD(bD-1)+a*bS(bS-1)+a^2*bD*bS=0",
            Self::CenteringComparatorSubtraction => b"B=2^15;tau=(pT-1)/2;K=tau+1;K_17=0;h=0:D_0-K_0=Delta_0-B*borrow_0;for h=1..16:D_h-K_h-borrow_{h-1}=Delta_h-B*borrow_h;mixed_top=b_D*borrow_16;c=borrow_17=borrow_16-mixed_top",
            Self::ComparatorBorrowBooleanity => b"exactly18borrows:borrow[0..17];borrow_h*(borrow_h-1)=0;mixed_top=b_D*borrow_16;c=borrow_17=borrow_16-mixed_top;c=1 iff D<K",
            Self::CenteredLiftSelector => b"sigma=1-borrow_17;M=D-sigma*pT",
            Self::SmallPositiveLinearDerivation => b"x_plus=x_signed+x_negative",
            Self::SmallSignDisjointness => b"x_plus*x_negative=0",
...
fn hidden_endpoint_role_v1(ordinal: usize) -> Result<HiddenEndpointRoleV1, GlobalLookupErrorV1> {
    if ordinal < COEFFICIENT_EQUATIONS_V1 * 3 {
        let endpoint = match ordinal % 3 {
            0 => EquationEndpointV1::Left,
            1 => EquationEndpointV1::Right,
            2 => EquationEndpointV1::Residual,
            _ => return Err(GlobalLookupErrorV1::Shape),
        };
...
fn endpoint_gate_coordinate_v1(
    ...
    Ok(EndpointGateCoordinateV1 {
        ...
        role: if ordinal % 2 == 0 {
            EndpointGateRoleV1::EndpointProduct
        } else {
            EndpointGateRoleV1::MaskedResidual
        },
    })
}
...
fn conditional_accounting_v1() -> Result<(usize, usize), GlobalLookupErrorV1> {
    let total = PRIOR_RADIX_QPCS_BYTES_V1
        .checked_add(CONDITIONAL_CROSS_FIELD_BYTES_V1)
        .and_then(|value| value.checked_add(GLOBAL_LOOKUP_DELTA_BYTES_V1))
        .ok_or(GlobalLookupErrorV1::Arithmetic)?;
    let margin = CONDITIONAL_CAP_BYTES_V1
        .checked_sub(total)
        .ok_or(GlobalLookupErrorV1::Arithmetic)?;
    (total == CONDITIONAL_TOTAL_BYTES_V1 && margin == CONDITIONAL_MARGIN_BYTES_V1)
        .then_some((total, margin))
