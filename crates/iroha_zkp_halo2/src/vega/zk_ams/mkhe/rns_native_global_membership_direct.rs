...
/// Sanitized, acyclic projection of the already-verified global lookup core.
...
struct RnsNativeGlobalLookupCleanCoreV2 {
    ...
    inverse_rho_challenge_digest: [u8; DIGEST_BYTES_V1],
    inverse_sumcheck_transcript_digest: [u8; DIGEST_BYTES_V1],
    inverse_endpoint_transcript_digest: [u8; DIGEST_BYTES_V1],
    u_sum: [u8; POINT_BYTES_V1],
    multiplicity: [u8; POINT_BYTES_V1],
    membership_transcript_digest: [u8; DIGEST_BYTES_V1],
    chronology_tag: ZkAmsMkheRnsNativeGlobalLookupChronologyTagV2,
}

/// Concrete membership-derived global-root evidence. Its digest and chronology
/// tag are private and can be consumed only by transcript equality discharge.
#[allow(
    dead_code,
    missing_copy_implementations,
    reason = "the global-root equality obligation consumes this evidence exactly once"
)]
    ...
    pub(in super::super::super) fn matches_claimed_global_lookup_root_v2(
        ...
        claimed_root != [0; DIGEST_BYTES_V1]
            ...
            && self.chronology_tag.matches_exact_chronology_v2(
                ...
                expected_pre_global_capability_digest,
            )
    }
}
...
fn verified_global_lookup_core_root_v2(
    ...
    let RnsNativeGlobalLookupCleanCoreV2 {
        pre_global_capability_digest,
        pre_z_binding_digest,
        z,
        post_z_transcript_digest,
        existing_inverse_root,
        added_inverse_root,
        alias_root,
        global_inverse_root,
        inverse_rho_challenge_digest,
        inverse_sumcheck_transcript_digest,
        inverse_endpoint_transcript_digest,
        u_sum,
        multiplicity,
    ...
    for (label, value) in [
        ...
        (
            b"post-z-transcript".as_slice(),
            post_z_transcript_digest.as_slice(),
        ),
        (
            b"existing-inverse-root".as_slice(),
            existing_inverse_root.as_slice(),
        ),
        (
            b"added-inverse-root".as_slice(),
            added_inverse_root.as_slice(),
        ),
        (b"alias-root".as_slice(), alias_root.as_slice()),
        (
            b"global-inverse-root".as_slice(),
            global_inverse_root.as_slice(),
        ),
        (
            b"inverse-rho".as_slice(),
            inverse_rho_challenge_digest.as_slice(),
        ),
        (
            b"inverse-sumcheck".as_slice(),
            inverse_sumcheck_transcript_digest.as_slice(),
        ),
        (
            b"inverse-endpoint".as_slice(),
            inverse_endpoint_transcript_digest.as_slice(),
        ),
        (b"u-sum".as_slice(), u_sum.as_slice()),
        (b"multiplicity".as_slice(), multiplicity.as_slice()),
        (
            b"membership-transcript".as_slice(),
...
/// Derive the acyclic global-lookup clean-core root by walking one already
/// verified membership owner. Every selected field is predecessor-core data;
/// no claimed global root, final transcript, residual, codec, successor, or
/// enclosing binding is admitted.
pub(in super::super::super) fn derive_rns_native_verified_global_lookup_core_root_v2<
    S: super::super::ZkAmsMkheRnsNativeSourceSnapshotV1,
>(
    membership: &RnsNativeGlobalMembershipPrerequisiteV1<'_, '_, S>,
) -> Result<RnsNativeGlobalLookupVerifiedCoreRootV2, RnsNativeGlobalMembershipDirectErrorV1> {
    let inverse = membership.previous();
    let post_z = inverse.previous();
    if membership.u_sum_commitment() != inverse.u_sum_commitment() {
        return Err(RnsNativeGlobalMembershipDirectErrorV1::InvalidContext);
    }
    let pre_global_capability = post_z.previous().pre_global_lookup_capability_v1();
    let pre_global_capability_digest = pre_global_capability
        .sole_z_binding_digest_v1()
        .map_err(|_| RnsNativeGlobalMembershipDirectErrorV1::InvalidContext)?;
    let chronology_tag = pre_global_capability
    ...
    let digests = [
        ...
        post_z.added_inverse_root(),
        post_z.alias_root(),
        post_z.global_inverse_root(),
        inverse.rho_challenge_digest(),
        inverse.sumcheck_transcript_digest(),
        inverse.endpoint_transcript_digest(),
    ...
    verified_global_lookup_core_root_v2(RnsNativeGlobalLookupCleanCoreV2 {
        pre_global_capability_digest,
        pre_z_binding_digest: post_z.pre_z_binding_digest(),
        z,
        post_z_transcript_digest: post_z.post_z_transcript_digest(),
        existing_inverse_root: post_z.existing_inverse_root(),
        added_inverse_root: post_z.added_inverse_root(),
        alias_root: post_z.alias_root(),
        global_inverse_root: post_z.global_inverse_root(),
        inverse_rho_challenge_digest: inverse.rho_challenge_digest(),
        ...
        u_sum,
        multiplicity,
        membership_transcript_digest: membership.transcript_digest(),
        chronology_tag,
    })
}
