...
type PointFoldPairV1<S> = (Vec<<S as ProofSuite>::Point>, Vec<<S as ProofSuite>::Point>);

fn try_collect_public_point_fold_pair_v1<S, G, H>(
    g_fold: G,
    h_fold: H,
) -> Result<PointFoldPairV1<S>, GeneralizedBulletproofErrorV1>
where
    S: ProofSuite,
...
pub(crate) fn try_exact_multiexp_v1<S: ProofSuite>(
...
impl<'a, S: ProofSuite> ArithmeticCircuitStatement<'a, S> {
        ...
        T: ProverTranscript<S>,
    {
        ...
        if l.len() != polynomial_count
            || l.capacity() != polynomial_count
            || l.as_ptr() != l_allocation
            || r.len() != polynomial_count
            || r.capacity() != polynomial_count
            || r.as_ptr() != r_allocation
        {
            return Err(GeneralizedBulletproofErrorV1::ResourceOverflow);
        }
