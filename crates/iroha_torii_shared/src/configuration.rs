//! Canonical configuration snapshots and operator update requests for Torii.
//!
//! These wire records have no dependency on node configuration or runtime services.
...
fn required<T>(value: Option<T>, field: &str) -> Result<T, NoritoError> {
    value.ok_or_else(|| NoritoError::Message(format!("missing {field}")))
}
/// P2P shaping profile selected by an operator.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum LaneProfile {
    /// Datacenter or validator shaping profile.
    Core,
    /// Constrained home connection shaping profile.
    Home,
}
impl LaneProfile {
    /// Parse the canonical profile label.
    #[must_use]
        ...
        while !w.peek_object_end()? {
            ...
            match kh {
                x if x == kh_lane_profile && w.last_key() == "lane_profile" => {
                    ...
                    lane_profile = Some(LaneProfile::parse_label(label).ok_or_else(|| {
                        NoritoError::Message("lane_profile must be exactly `core` or `home`".into())
                    })?);
                }
...
impl FastJsonWrite for ConfigurationUpdate {
    fn write_json(&self, out: &mut String) {
        out.push('{');
        // logger
        out.push_str("\"logger\":");
...
impl FastJsonWrite for Configuration {
    fn write_json(&self, out: &mut String) {
        out.push('{');
...
impl<'a> FastFromJson<'a> for NoritoRpcSummary {
    fn parse(w: &mut TapeWalker<'a>, arena: &mut Arena) -> Result<Self, NoritoError> {
        ...
        Ok(Self {
            ...
            require_mtls: required(require_mtls, "require_mtls")?,
            canary_allowlist_size: required(allow_size, "canary_allowlist_size")?,
        })
    }
}
...
fn finalize_network(
    ...
    Ok(Network {
        chain_discriminant: required(chain_discriminant, "chain_discriminant")?,
        block_gossip_size: required(block_gossip_size, "block_gossip_size")?,
        block_gossip_period_ms: required(block_gossip_period_ms, "block_gossip_period_ms")?,
        peer_gossip_period_ms: required(peer_gossip_period_ms, "peer_gossip_period_ms")?,
        trust_decay_half_life_ms: required(trust_decay_half_life_ms, "trust_decay_half_life_ms")?,
        trust_penalty_bad_gossip: required(trust_penalty_bad_gossip, "trust_penalty_bad_gossip")?,
        trust_penalty_unknown_peer: required(
            trust_penalty_unknown_peer,
            "trust_penalty_unknown_peer",
        )?,
        trust_min_score: required(trust_min_score, "trust_min_score")?,
        trust_gossip: required(trust_gossip, "trust_gossip")?,
        relay_ttl: required(relay_ttl, "relay_ttl")?,
        lane_profile: required(lane_profile, "lane_profile")?,
        require_sm_handshake_match: required(
            require_sm_handshake_match,
            "require_sm_handshake_match",
        )?,
        require_sm_openssl_preview_match: required(
            require_sm_openssl_preview_match,
            "require_sm_openssl_preview_match",
        )?,
        transaction_gossip_size: required(tx_gossip_size, "transaction_gossip_size")?,
        transaction_gossip_period_ms: required(
            tx_gossip_period_ms,
            "transaction_gossip_period_ms",
        )?,
        transaction_gossip_resend_ticks: required(
            tx_gossip_resend_ticks,
            "transaction_gossip_resend_ticks",
        )?,
        soranet_handshake: required(soranet_handshake, "soranet_handshake")?,
        soranet_privacy: required(soranet_privacy_summary, "soranet_privacy")?,
        soranet_vpn: required(soranet_vpn_summary, "soranet_vpn")?,
    })
}
impl<'a> FastFromJson<'a> for Network {
    fn parse(w: &mut TapeWalker<'a>, arena: &mut Arena) -> Result<Self, NoritoError> {
        ...
        let mut lane_profile: Option<LaneProfile> = None;
...
impl<'a> FastFromJson<'a> for SoranetPrivacySummary {
    fn parse(w: &mut TapeWalker<'a>, _arena: &mut Arena) -> Result<Self, NoritoError> {
        ...
        w.expect_object_end()?;
        Ok(Self {
            bucket_secs: required(bucket_secs, "bucket_secs")?,
            min_handshakes: required(min_handshakes, "min_handshakes")?,
            flush_delay_buckets: required(flush_delay_buckets, "flush_delay_buckets")?,
            force_flush_buckets: required(force_flush_buckets, "force_flush_buckets")?,
            max_completed_buckets: required(max_completed_buckets, "max_completed_buckets")?,
            max_share_lag_buckets: required(max_share_lag_buckets, "max_share_lag_buckets")?,
            expected_shares: required(expected_shares, "expected_shares")?,
            event_buffer_capacity: required(event_buffer_capacity, "event_buffer_capacity")?,
        })
    }
}
impl<'a> FastFromJson<'a> for SoranetVpnSummary {
    fn parse(w: &mut TapeWalker<'a>, arena: &mut Arena) -> Result<Self, NoritoError> {
        ...
        w.expect_object_end()?;
        let flow_label_bits = required(flow_label_bits, "flow_label_bits")?;
        ...
        let lease_secs = u32::try_from(required(lease_secs, "lease_secs")?)
            .map_err(|_| NoritoError::Message("lease_secs exceeds u32::MAX".into()))?;
        let exit_class = VpnExitClassV1::try_from_label(&required(exit_class, "exit_class")?)
            .map_err(|err| NoritoError::Message(err.to_string()))?
            .as_label()
            .to_string();
        Ok(SoranetVpnSummary {
            ...
            cell_size_bytes: required(cell_size_bytes, "cell_size_bytes")?,
            cover_to_data_per_mille: required(cover_to_data_per_mille, "cover_to_data_per_mille")?,
            max_cover_burst: required(max_cover_burst, "max_cover_burst")?,
            heartbeat_ms: required(heartbeat_ms, "heartbeat_ms")?,
            jitter_ms: required(jitter_ms, "jitter_ms")?,
            padding_budget_ms: required(padding_budget_ms, "padding_budget_ms")?,
            guard_refresh_secs: required(guard_refresh_secs, "guard_refresh_secs")?,
            dns_push_interval_secs: required(dns_push_interval_secs, "dns_push_interval_secs")?,
            meter_family: required(meter_family, "meter_family")?,
            operator_account_id: required(operator_account_id, "operator_account_id")?,
            lease_fee: required(lease_fee, "lease_fee")?,
            settlement_grace_secs: required(settlement_grace_secs, "settlement_grace_secs")?,
            route_pushes: required(route_pushes, "route_pushes")?,
            excluded_routes: required(excluded_routes, "excluded_routes")?,
            dns_servers: required(dns_servers, "dns_servers")?,
        })
    }
}
impl JsonDeserialize for SoranetVpnSummary {
    fn json_deserialize(parser: &mut json::Parser<'_>) -> Result<Self, json::Error> {
        parse_via_fast(parser)
    }
}
...
impl<'a> FastFromJson<'a> for ConfigurationUpdate {
    fn parse(w: &mut TapeWalker<'a>, arena: &mut Arena) -> Result<Self, NoritoError> {
        w.expect_object_start()?;
...
impl JsonDeserialize for ConfigurationUpdate {
    fn json_deserialize(parser: &mut json::Parser<'_>) -> Result<Self, json::Error> {
        parse_via_fast(parser)
    }
}
impl<'a> FastFromJson<'a> for Configuration {
    fn parse(w: &mut TapeWalker<'a>, arena: &mut Arena) -> Result<Self, NoritoError> {
        use iroha_crypto::PublicKey;
        w.expect_object_start()?;
...
mod test {
    ...
    fn config_update_with_handshake_roundtrip() {
        ...
        let parsed: ConfigurationUpdate =
            norito::json::from_json(&json).expect("deserialize update");
    ...
    fn snapshot_value() -> json::Value {
        json::from_json(include_str!("../../../fixtures/torii/configuration.json"))
            .expect("canonical configuration fixture")
    }
    #[test]
    fn network_snapshot_requires_every_field() {
        let network = snapshot_value().get("network").unwrap().clone();
        for field in network.as_object().unwrap().keys() {
            let payload = json_without_object_field(network.clone(), field);
            assert!(
                json::from_json::<Network>(&payload).is_err(),
                "missing {field} must fail"
            );
        }
    }
    #[test]
    fn vpn_snapshot_requires_every_field() {
        let vpn = snapshot_value()
            .get("network")
            .unwrap()
            .get("soranet_vpn")
            .unwrap()
            .clone();
        for field in vpn.as_object().unwrap().keys() {
            let payload = json_without_object_field(vpn.clone(), field);
            assert!(
                json::from_json::<SoranetVpnSummary>(&payload).is_err(),
                "missing {field} must fail"
            );
        }
    }
    #[test]
    fn nested_snapshot_summaries_require_every_field() {
        let snapshot = snapshot_value();
        for path in [
            vec!["network", "soranet_privacy"],
            vec!["transport", "norito_rpc"],
        ] {
            let mut summary = &snapshot;
            for key in &path {
                summary = summary.get(*key).expect("fixture summary");
            }
            for field in summary.as_object().unwrap().keys() {
                let mut incomplete = snapshot.clone();
                let mut cursor = &mut incomplete;
                for key in &path {
                    cursor = cursor.as_object_mut().unwrap().get_mut(*key).unwrap();
                }
                cursor.as_object_mut().unwrap().remove(field);
                let payload = json::to_json(&incomplete).unwrap();
                let error = json::from_json::<Configuration>(&payload)
                    .expect_err("incomplete nested snapshot must fail");
                assert!(
                    error.to_string().contains(field),
                    "missing {path:?}.{field}: {error}"
                );
            }
        }
    }
    #[test]
    fn complete_snapshot_roundtrips() {
        let fixture = include_str!("../../../fixtures/torii/configuration.json");
    ...
    fn logger_filters_escape_json_and_roundtrip_without_a_logging_runtime() {
        let logger = Logger {
            level: Level::INFO,
            filter: Some("module[span{field=\"quoted\"}]=trace\n".to_owned()),
        };
        let encoded = json::to_json(&logger).expect("encode logger");
        let decoded: Logger = json::from_json(&encoded).expect("decode logger");
        assert_eq!(decoded.filter, logger.filter);
    }
    #[test]
    fn lane_profiles_have_one_exact_label() {
        for profile in [LaneProfile::Core, LaneProfile::Home] {
            assert_eq!(LaneProfile::parse_label(profile.as_label()), Some(profile));
        }
        for label in ["CORE", "home ", "default", ""] {
            assert_eq!(LaneProfile::parse_label(label), None);
        }
    }
    #[test]
    ...
    fn config_update_rejects_confidential_gas_override() {
        let err = norito::json::from_json::<ConfigurationUpdate>(
            r#"{
    ...
    fn vpn_summary_parse_rejects_unknown_exit_class() {
        let payload = r#"{"flow_label_bits":24,"lease_secs":600,"exit_class":"ultra-fast"}"#;
    ...
    fn vpn_summary_parse_rejects_overflowing_lease() {
        ...
        let payload = format!(r#"{{"flow_label_bits":24,"lease_secs": {overflowing}}}"#);
        let mut parser = norito::json::Parser::new(&payload);
