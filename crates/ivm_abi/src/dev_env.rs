...
/// Return `true` when the provided environment flag is set **and** the build is running with debug
/// assertions (dev/tests). Release builds always return `false` so production nodes ignore debug
/// shims. Return true when the named debug flag is set in a debug/test build.
#[inline]
