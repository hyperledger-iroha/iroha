...
use crate::{
    ...
    torii_proxy::QueuePlanAdmissionBindingV1,
};
...
const QUEUE_PLAN_STARTUP_LIVE_ROOT_STEP_DOMAIN: &[u8] =
    b"iroha:queue-plan-startup-live-root:step:v1\0";
const QUEUE_PLAN_STARTUP_LIVE_ROOT_FINAL_DOMAIN: &[u8] =
    b"iroha:queue-plan-startup-live-root:final:v1\0";
const QUEUE_PLAN_STARTUP_PHASE_ROOT_EMPTY_DOMAIN: &[u8] =
    b"iroha:queue-plan-startup-reservation-phase-root:empty:v1\0";
const QUEUE_PLAN_STARTUP_PHASE_ROOT_STEP_DOMAIN: &[u8] =
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(super) struct QueuePlanStartupLiveClaimIdentityV1 {
    pub(super) entrypoint_hash: HashOf<TransactionEntrypoint>,
...
impl QueuePlanStartupReplayReceiptV1 {
    ...
    pub(super) fn with_drifted_live_record_root(mut self) -> Self {
        self.live_record_root = Hash::new_from_chunks(&[
            b"iroha:queue-plan-startup-live-root:test-drift:v1\0",
            self.live_record_root.as_ref(),
        ]);
        self
    }
}
fn queue_plan_startup_live_record_root(
    ...
    for claim in claims {
        if ordered.insert(claim.entrypoint_hash, claim).is_some() {
            return Err(invalid_data(
                "queue-plan startup live-record root contains a duplicate entrypoint",
            ));
        }
    }
    ...
    let count = u64::try_from(ordered.len())
        .map_err(|_| invalid_data("queue-plan startup live-record count exceeds u64"))?;
    Ok((
        ordered.len(),
        Hash::new_from_chunks(&[
            QUEUE_PLAN_STARTUP_LIVE_ROOT_FINAL_DOMAIN,
            rolling.as_ref(),
            &count.to_be_bytes(),
        ]),
    ))
...
fn queue_plan_startup_reservation_phase_root(
    ...
    let mut rolling = Hash::new(QUEUE_PLAN_STARTUP_PHASE_ROOT_EMPTY_DOMAIN);
    for phase in ordered.values() {
        let key_digest = phase.key.digest();
        let tags = [
            phase.reservation_phase.tag(),
            phase.queue_plan_phase.tag(),
            u8::from(phase.plan_tombstone_marked),
        ];
        rolling = Hash::new_from_chunks(&[
            QUEUE_PLAN_STARTUP_PHASE_ROOT_STEP_DOMAIN,
            rolling.as_ref(),
            key_digest.as_ref(),
            &tags,
        ]);
    ...
    let count = u64::try_from(ordered.len())
        .map_err(|_| invalid_data("queue-plan startup reservation phase count exceeds u64"))?;
    Ok((
        ordered.len(),
        Hash::new_from_chunks(&[
            QUEUE_PLAN_STARTUP_PHASE_ROOT_FINAL_DOMAIN,
            rolling.as_ref(),
            &count.to_be_bytes(),
        ]),
    ))
...
impl QueuePlanJournal {
    ...
    pub fn remove_exact_global_admission_binding_strict_durable(
        ...
        let mut requested_entrypoints = BTreeSet::new();
        requested_entrypoints.insert(entrypoint_hash.clone());
        let live_identity = {
            let mut replay =
                self.prepare_replay_with_removed_entrypoints(Some(&requested_entrypoints))?;
            replay.verify_snapshot_content()?;
            if let Some(live) = replay.live_positions.get(&entrypoint_hash) {
                live.validate_global_admission_for_reservation_commit(key)?;
                Some((live.plan_digest, live.claim_digest))
            } else if let Some(removed) = replay.removed_positions.get(&entrypoint_hash) {
                removed.validate_global_admission_for_reservation_commit(key)?;
                replay.verify_snapshot_content()?;
                return Ok(QueuePlanJournalExactRemoveResult::AlreadyAbsent);
            } else {
                return Err(invalid_data(
                    "queue plan journal reservation target is neither live nor exactly tombstoned",
                ));
            }
        };
...
mod tests {
    ...
    fn startup_replay_receipt_binds_exact_storage_and_complete_live_root() {
        ...
        let first_path = dir.path().join("startup-receipt-first.norito");
        let second_path = dir.path().join("startup-receipt-second.norito");
        let (record, binding) = globally_bound_record("startup-receipt-exact");
        let key = reservation_key_for_record(&record, binding.canonical_hash());
        let phase = commit_recovery_phase(key, QueuePlanReservationPhaseV1::Live, false);
        let claim = QueuePlanStartupLiveClaimIdentityV1 {
            entrypoint_hash: record.entrypoint_hash.clone(),
            routing_plan_digest: record.plan_digest(),
        ...
        first
            ...
            .expect("persist exact first claim");
        let receipt = first
            .observe_startup_replay_receipt(core::slice::from_ref(&phase))
            .expect("observe exact startup image");
        assert!(
            receipt
                .binds_live_claims(core::iter::once(claim))
                .expect("bind complete live root")
        );
        assert!(
            !receipt
                .clone()
                .with_drifted_live_record_root()
                .binds_live_claims(core::iter::once(claim))
        ...
        second
            ...
            .expect("persist byte-equivalent second claim");
        assert!(
            !second
                .revalidate_startup_replay_receipt(&receipt, core::slice::from_ref(&phase),)
        ...
        first
            ...
            .expect("append later live claim");
        assert!(
            !first
                .revalidate_startup_replay_receipt(&receipt, core::slice::from_ref(&phase),)
                .expect("compare changed exact image"),
