...
fn install_native_amx_evidence_fixture_heights_with_predecessor_drift(
    ...
    let configured_catalog_hash = kura
        ...
        .expect("read Native AMX fixture catalog baseline")
        .expect("Native AMX fixture has an authenticated catalog");
    kura.establish_or_verify_configured_primary_geometry_anchor(
        entry,
        lane_incarnation,
        configured_catalog_hash,
    )
    .expect("bind active Native AMX participant to durable primary geometry");
