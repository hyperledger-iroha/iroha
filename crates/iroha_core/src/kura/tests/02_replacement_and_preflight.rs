    ...
    fn partial_stage_discard_recovers_committed_replacement_before_returning() {
        ...
        let replacement_hash = {
            ...
            kura.replace_top_block(Arc::clone(&replacement)).expect(
                "published replacement must report committed success after cleanup recovery",
            );
            assert_eq!(
    ...
    fn coordinated_retained_and_finality_payload_hash_tamper_fails_crypto_and_restart() {
        ...
        let config = kura_config_for_dir(&temp_dir, nonzero!(1_usize));
        {
            ...
            let mut finality = KuraV2FinalityRecord::decode_all(&mut finality_input)
                .expect("decode finality record");
    ...
    fn retained_block_v2_is_readable_and_upgrades_before_body_eviction() {
        ...
        assert_eq!(
            prepared.merge_reference.as_ref(),
        ...
        assert_eq!(
            ...
            RETAINED_BLOCK_RECORD_VERSION_V2
        );
        assert!(decoded.merge_reference.is_none());
        let (_, _, _, _, live_reference) = kura
    ...
    fn startup_rejects_prune_intent_crossing_durable_v2_finality_before_mutation() {
        ...
        let _commit_receipt = kura
            ...
            .expect("finalize canonical block");
        let blocks_dir = kura.active_blocks_dir.lock().clone();
        kura.persist_prune_intent(&KuraPruneIntentV1 {
            version: 1,
            source_height: 1,
    ...
    fn authenticated_snapshot_lane_restore_rejects_primary_path_drift_atomically() {
        ...
        let configured_catalog =
            LaneCatalog::new(nonzero!(1_u32), vec![ModelLaneConfig::default()])
                .expect("configured catalog");
        let configured = RuntimeLaneConfig::from_catalog(&configured_catalog);
        let kura_cfg = KuraConfig {
            init_mode: InitMode::Strict,
