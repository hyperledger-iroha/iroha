    ...
    fn reopened_evicted_read_never_enters_memory_cache_or_survives_finality_loss() {
        ...
        let (canonical, finality_bytes) = {
            let (kura, _) =
                Kura::new(&small_config, &RuntimeLaneConfig::default()).expect("open small Kura");
            let blocks = store_dummy_block_arcs(&kura, 4);
            let canonical = Arc::clone(&blocks[1]);
            let height = nonzero!(2_usize);
            let (_, payload_len) = advertise_required_replicas(&kura, height);
        ...
        let (kura, _) =
            Kura::new(&large_config, &RuntimeLaneConfig::default()).expect("reopen large Kura");
        let height = nonzero!(2_usize);
        assert_eq!(
    ...
    fn fast_init_preserves_remote_only_metadata_after_restart() {
        ...
        let (kura, block_count) =
            Kura::new(&config, &RuntimeLaneConfig::default()).expect("fast reopen");
    ...
    fn strict_init_removes_malformed_sidecar_with_matching_length() {
        ...
        let (kura, block_count) =
            Kura::new(&config, &RuntimeLaneConfig::default()).expect("strict reopen");
        assert_eq!(
            block_count.0, 4,
    ...
    fn test_kura_with_default_lane_markers(
        ...
        let (kura, block_count) = Kura::new(config, lane_config).expect("init Kura test fixture");
    ...
    fn rebind_autonomous_lane_payload_for_kura(
        ...
        proposal.descriptor.subject_hash = Hash::new(
            format!("claim-subject:{}:{lane_block_height}", lane_id.as_u32()).into_bytes(),
        );
        proposal.descriptor.payload_ownership_hash = Hash::new(
            format!("claim-ownership:{}:{lane_block_height}", lane_id.as_u32()).into_bytes(),
        );
    ...
    fn repropose_autonomous_lane_payload_for_kura(
        ...
        proposal.descriptor.payload_ownership_hash = Hash::new(
            format!("kura-autonomous-reproposal-ownership:{proposal_height}").into_bytes(),
        );
        proposal.descriptor.rbc_instance_hash =
