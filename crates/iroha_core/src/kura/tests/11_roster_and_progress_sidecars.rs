...
fn progress_prepend_directory_failure_retries_without_corruption() {
    for (label, failure) in strict_progress_sidecar_failure_modes().into_iter().skip(2) {
        ...
        let height_two = norito::to_bytes(&DummySidecar { height: 2 }).expect("encode height two");
        let height_one = norito::to_bytes(&DummySidecar { height: 1 }).expect("encode height one");
        let namespace = kura
            .open_bound_progress_namespace(&data_path, &index_path)
        ...
        assert_eq!(
            fs::metadata(&data_path)
                ...
                .len(),
            u64::try_from(height_two.len() + height_one.len()).expect("fixture length fits u64"),
...
fn bound_progress_recovery_handles_crash_phases_without_path_escape() {
    ...
    let append_intent = |kura: &Kura,
        ...
        BoundProgressAppendIntentV1 {
            ...
            new_index_len: if index_write_offset == old_index_len {
                old_index_len + u64::try_from(new_index_bytes.len()).expect("index window length")
            } else {
                old_index_len
            },
            index_write_offset,
    ...
    for (label, inject) in journal_faults {
        ...
        let payload =
            norito::to_bytes(&DummySidecar { height: 1 }).expect("encode journal fault payload");
        let namespace = kura
            .open_bound_progress_namespace(&data_path, &index_path)
    ...
    for calls_before_failure in [0, 1] {
        ...
        for target_index in 0..directory_count {
            ...
            fail_bound_progress_intent_directory_sync_for_tests(calls_before_failure, target_index);
            assert!(
                !Kura::append_indexed_progress_sidecar(
                    &data_path,
                    &index_path,
    ...
    // appends the intended payload once.
    {
        let (_temp_dir, kura, data_path, index_path) = fixture();
        let first =
            norito::to_bytes(&DummySidecar { height: 1 }).expect("encode wrong-hash predecessor");
        let intended =
    ...
    for extension in [
        ...
        let index_after = fs::metadata(&index_path).expect("main index after recovery rejection");
        assert_eq!(
