...
fn native_amx_prune_special_files_reject_symlinks_and_hardlinks_on_both_paths() {
    for special in ["stable", "temporary"] {
        for unsafe_kind in ["symlink", "hardlink"] {
            ...
            let lane_config = RuntimeLaneConfig::default();
            let (kura, _) =
                Kura::new(&config, &lane_config).expect("initialize Native prune unsafe-file Kura");
            let entry = kura
                .lane_storage_entry(LaneId::SINGLE)
...
fn native_amx_prune_rejects_legacy_and_unexpected_special_names_without_downgrade() {
    for hostile_name in [
        ...
        let lane_config = RuntimeLaneConfig::default();
        let (kura, _) =
            Kura::new(&config, &lane_config).expect("initialize Native prune legacy-name Kura");
        let entry = kura
            .lane_storage_entry(LaneId::SINGLE)
...
fn native_amx_prune_rejects_legacy_name_before_consuming_valid_v2_intent() {
    ...
    let lane_config = RuntimeLaneConfig::default();
    let (kura, _) =
        Kura::new(&config, &lane_config).expect("initialize Native prune mixed legacy/V2 Kura");
    let entry = kura
        .lane_storage_entry(LaneId::SINGLE)
...
fn native_amx_prune_two_pair_partial_unlinks_recover_every_prefix_idempotently() {
    for deleted_prefix in 0..=4 {
        ...
        let lane_config = RuntimeLaneConfig::default();
        let (kura, _) =
            Kura::new(&config, &lane_config).expect("initialize Native prune partial-prefix Kura");
        let entry = kura
            .lane_storage_entry(LaneId::SINGLE)
        ...
        drop(kura);
        let (reopened, _) = Kura::new(&config, &lane_config).unwrap_or_else(|error| {
...
fn native_amx_prune_protected_checkpoint_or_commit_semantic_drift_fails_before_unlink() {
    for damage in ["checkpoint-state", "commit-checkpoint"] {
        ...
        let lane_config = RuntimeLaneConfig::default();
        let (kura, _) =
            Kura::new(&config, &lane_config).expect("initialize Native prune metadata-drift Kura");
        let entry = kura
            .lane_storage_entry(LaneId::SINGLE)
