...
mod tests {
    ...
    fn semantic_commit_decision_id_ignores_reproposal_round_and_qc_representation() {
        ...
        let mut different_round = baseline;
        different_round.round.view = different_round.round.view.saturating_add(1);
        different_round.proposal_round = different_round.round;
        assert!(baseline.same_commit_decision(different_round));
        assert_eq!(
            semantic_commit_decision_id(baseline).expect("baseline decision id"),
            semantic_commit_decision_id(different_round).expect("re-proposed decision id"),
        );
