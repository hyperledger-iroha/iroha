...
pub(crate) enum V2LaneWorkError {
    ...
    /// A durable lane, merge, or Native AMX signing guard failed closed.
    #[error("durable signing guard failed closed: {0}")]
    SigningGuard(String),
}
...
impl V2LaneWorkAdapter {
    ...
    /// Re-enqueue bounded lane artifacts, Native AMX requests, merge candidates,
    /// and merge shares with rotating class priority for reliable retransmission.
    pub(crate) fn schedule_retransmission(&mut self, active_view: wire::View) {
        ...
        let active_merge_view = self
            ...
            .filter(|view| *view == active_view)
            .filter(|_| self.merge_parent_frontier_is_exact());
        if active_merge_view.is_none() {
            self.purge_queued_merge_broadcasts();
        }
    ...
    fn authorize_local_merge_claim(
        ...
        let Some(parent_qc) = self.context.parent_commit_qc.as_ref() else {
            return Err(MergeSidecarError::SigningGuard(
                "merge signing requires a committed global parent".to_owned(),
            ));
        };
        let expected_parent = parent_qc.subject.block_hash;
    ...
    fn refresh_merge_candidates(&mut self, active_view: wire::View) {
        ...
        if let Err(error) =
            self.authorize_local_merge_claim(&candidate, active_view, local_index, key.digest)
        {
            match error {
...
mod tests {
    ...
    fn fixture_at_height_inner(
        ...
        fixture_with_height_limits_voting_and_parent(
            mode,
            height,
    ...
    fn fixture_with_height_limits_and_voting(
        ...
        fixture_with_height_limits_voting_and_parent(mode, height, limits, voting_enabled, false)
    }

    fn fixture_with_height_limits_voting_and_parent(
        mode: wire::ConsensusMode,
        height: u64,
        limits: V2LaneWorkLimits,
        voting_enabled: bool,
    ...
    fn same_round_merge_claims_survive_successful_kura_staging() {
        ...
        for (index, key_pair) in keys.iter().enumerate() {
            ...
            let signature = Signature::try_new(key_pair.private_key(), digest.as_ref())
                ...
                .to_vec();
            assert_eq!(
                adapter.accept_merge_signature(
            ...
            if adapter.durably_staged_merge_entries.contains(&key) {
                break;
            }
        }
        assert!(
            adapter.merge_entries.contains_key(&key)
                && adapter.durably_staged_merge_entries.contains(&key),
            "fixture shares must form quorum, publish the certified entry, and retain it for retransmission"
        );
    ...
    fn quorate_merge_persistence_retries_after_transient_kura_failure() {
        ...
        assert_eq!(
            adapter
                ...
                .expect("read exact retried merge entry"),
            Some(certified_entry),
            "retry must publish the exact State-validated certificate cached before failure"
        );
    }
