...
mod tests {
    ...
    fn penalty_staking_ids() -> (AssetDefinitionId, AccountId, AccountId) {
        let escrow = AccountId::new(
            KeyPair::try_from_seed(vec![0xC4; 32], Algorithm::Ed25519)
                .expect("deterministic penalty escrow")
                .public_key()
                .clone(),
        );
        let slash_sink = AccountId::new(
            KeyPair::try_from_seed(vec![0xC5; 32], Algorithm::Ed25519)
                .expect("deterministic penalty slash sink")
                .public_key()
                .clone(),
        );
        let asset_definition = AssetDefinitionId::derive_from_components(
            iroha_data_model::domain::DomainId::try_new("penalties", "universal")
                .expect("penalty staking domain"),
            "stake".parse().expect("penalty staking asset name"),
        );
    ...
    fn fresh_state() -> State {
        ...
        let (asset_definition, escrow, slash_sink) = penalty_staking_ids();
        {
            let mut accounts = state.world.accounts.block();
            for account_id in [&escrow, &slash_sink] {
        ...
        }
        {
            let definition = AssetDefinition::numeric(
                ...
                "penalty staking custody".to_owned(),
                AssetBalancePolicy::Global,
                None,
            )
            .build(&escrow);
            let mut definitions = state.world.asset_definitions.block();
            definitions.insert(asset_definition.clone(), definition);
            definitions.commit();
        }
        {
    ...
    fn add_validator_record_on_lane(state: &State, lane_id: LaneId, peer: &PeerId) -> AccountId {
        ...
        {
            let mut accounts = state.world.accounts.block();
            if accounts.get(&validator).is_none() {
                let (_, account) = Account::new(validator.clone())
                    .build(&validator)
                    .into_key_value();
                accounts.insert(validator.clone(), account);
            }
            accounts.commit();
        }
        ...
        let share = PublicLaneStakeShare {
            ...
            bonded: Quantity::from(10_000_u64),
            pending_unbonds: BTreeMap::new(),
            metadata: Metadata::default(),
        };
        let mut block = state.world.public_lane_stake_shares.block();
        block.insert((lane_id, validator.clone(), validator.clone()), share);
        block.commit();
        fund_penalty_escrow(state, &Quantity::from(10_000_u64));
        validator
    }
