...
// Serialize fail-stop-sensitive consensus side effects with the process-lifetime Kura recovery
// latch. This closes check-then-act races for independent background workers: either an admitted
// effect owns this gate before recovery begins, or recovery owns it first and the effect rejects.
static CONSENSUS_OUTPUT_GATE: OnceLock<Mutex<()>> = OnceLock::new();
#[cfg(not(test))]
...
#[cfg(not(test))]
static CONSENSUS_RECOVERY_REQUIRED: AtomicBool = AtomicBool::new(false);
#[cfg(test)]
std::thread_local! {
    ...
    // Fatal Kura recovery is process-global in production. Keep it thread-local in unit tests so
    // one isolated actor fixture cannot fail-stop unrelated fixtures running in parallel.
    static TEST_CONSENSUS_RECOVERY_REQUIRED: std::cell::Cell<bool> = const {
...
fn activate_consensus_safety_halt_snapshot_while_guarded(snapshot: ConsensusSafetyHaltSnapshot) {
    // Serialize halt publication with the final side-effect admission check. An effect that
    // already owns this gate is pre-halt work; after this acquisition completes, no new
    // fail-stop-sensitive consensus effect can begin.
    let output_gate = CONSENSUS_OUTPUT_GATE.get_or_init(|| Mutex::new(()));
    let _output_guard = match output_gate.lock() {
        Ok(guard) => guard,
        Err(poisoned) => {
            set_consensus_recovery_required();
            iroha_logger::error!(
                "consensus output gate was poisoned while activating the durable safety halt"
            );
            poisoned.into_inner()
        }
    };
...
fn set_consensus_recovery_required() {
    #[cfg(test)]
    {
        TEST_CONSENSUS_RECOVERY_REQUIRED.with(|required| required.set(true));
    }
    #[cfg(not(test))]
    {
        CONSENSUS_RECOVERY_REQUIRED.store(true, Ordering::Release);
    }
}

/// Activate the process-lifetime, restart-cleared Kura recovery fail-stop.
///
/// The output gate is held while publishing the flag so a background consensus side effect cannot
/// pass its final admission check concurrently with the fatal durability transition.
pub(super) fn activate_consensus_recovery_required() {
    let gate = CONSENSUS_OUTPUT_GATE.get_or_init(|| Mutex::new(()));
    let _output_guard = match gate.lock() {
        Ok(guard) => guard,
        Err(poisoned) => {
            iroha_logger::error!(
                "consensus output gate was poisoned while entering Kura recovery; retaining fail-stop"
            );
            poisoned.into_inner()
        }
    };
...
pub fn consensus_recovery_required_active() -> bool {
    #[cfg(test)]
    {
        return TEST_CONSENSUS_RECOVERY_REQUIRED.with(std::cell::Cell::get);
    }
    #[cfg(not(test))]
    CONSENSUS_RECOVERY_REQUIRED.load(Ordering::Acquire)
}

/// Guard authorizing one fail-stop-sensitive consensus side effect.
///
/// Holding this guard serializes the effect with activation of the restart-recovery latch.
/// Once recovery is active, no new guard can be acquired until the process restarts.
#[must_use = "the guard must be held until the consensus side effect is complete"]
pub struct ConsensusOutputGuard {
    _output_guard: MutexGuard<'static, ()>,
}

/// Acquire permission for one externally visible or durable background consensus side effect.
///
/// Returns `None` after either a durable safety halt or a fatal Kura recovery latch. A poisoned
/// output gate also fails closed and activates the restart-recovery flag.
#[must_use]
pub fn consensus_output_guard() -> Option<ConsensusOutputGuard> {
    let gate = CONSENSUS_OUTPUT_GATE.get_or_init(|| Mutex::new(()));
    let output_guard = match gate.lock() {
        Ok(guard) => guard,
        Err(poisoned) => {
            set_consensus_recovery_required();
            drop(poisoned.into_inner());
            iroha_logger::error!(
                "consensus output gate is poisoned; suppressing fail-stop-sensitive consensus effect"
            );
            return None;
        }
    };
    if consensus_safety_halt_active() || consensus_recovery_required_active() {
        return None;
    }
    Some(ConsensusOutputGuard {
...
/// Clear the restart-recovery fail-stop for isolated unit tests.
#[cfg(test)]
pub(crate) fn reset_consensus_recovery_required_for_tests() {
    TEST_CONSENSUS_RECOVERY_REQUIRED.with(|required| required.set(false));
}
...
pub fn reset_commit_certs_for_tests() {
    ...
    #[cfg(test)]
    {
        ...
        reset_consensus_recovery_required_for_tests();
    }
}
