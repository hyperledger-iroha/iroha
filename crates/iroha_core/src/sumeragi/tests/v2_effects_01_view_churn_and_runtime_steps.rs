...
fn pending_kura_fake_validate_fixture(
    ...
    let (_, evidence) = verify_pending_kura_apply_parts(
        &fixture.context,
        decision,
        &recovered,
    ...
    (fixture, executor, services, evidence)
}

#[test]
fn synthetic_runtime_cannot_mint_pending_tip_apply_successor() {
    let fixture = Fixture::new();
    let directory = TempDir::new().expect("recovery directory");
    let mut store = V2BodyStore::open_with_policy(
        directory.path(),
    ...
    assert!(matches!(
        ...
        Err(EffectExecutorError::PendingApplyRecoveryMismatch(reason))
            if reason.contains("runtime cannot commit a deferred pending-Kura validation marker")
    ));
    ...
    let restored = executor
        ...
        .as_ref()
        .expect("synthetic-runtime rejection retains recovery evidence");
    assert!(restored.deferred_validated_marker.is_some());
    assert!(restored.is_exact(&fixture.context));
    assert!(executor.status().fail_closed);
    assert!(executor.output_guard.restart_required());
    assert_eq!(services.closed.len(), 1);
    assert!(services.closed[0].contains("runtime cannot commit a deferred pending-Kura"));
...
#[test]
fn pending_kura_validate_capacity_retains_marker_before_runtime_commit_hook() {
    let (fixture, mut executor, mut services, evidence) =
        pending_kura_fake_validate_fixture(EffectQueueConfig::new(1, 2, 1_048_576, 1));
    executor
        .consume_effects(vec![timeout_sign(&fixture, 0)], &mut services)
        .expect("one durable timeout signature fills pending-work capacity");
    assert_eq!(executor.pending_signatures.len(), 1);
    let sign_tasks = services.sign_tasks.clone();
    executor.pending_tip_recovery = Some(evidence);
    for stage in 0..2 {
        assert!(matches!(
            executor
                .step_pending_tip_recovery(Instant::now(), &mut services)
                .unwrap_or_else(|error| panic!("advance local recovery stage {stage}: {error:?}")),
            EffectExecutorStep::Advanced { effects: 1 }
        ));
    }
    let evidence_before = executor
        .pending_tip_recovery
        .clone()
        .expect("recovery evidence before capacity-blocked Validate");
    assert_eq!(
        evidence_before.stage(),
        PendingKuraApplyRecoveryStage::DeterministicValidation
    );
    assert!(evidence_before.deferred_validated_marker.is_some());
    let next_work_id_before = executor.next_work_id;
    let pending_signature_ids = executor
        .pending_signatures
        .keys()
        .copied()
        .collect::<Vec<_>>();
    assert_eq!(
        executor
            .step_pending_tip_recovery(Instant::now(), &mut services)
            .expect("capacity retains the exact Validate head"),
        EffectExecutorStep::Advanced { effects: 0 }
    );
    assert_eq!(
        executor.pending_tip_recovery_last_result,
        Some(PendingTipRecoveryAttemptResult::Advanced)
    );
    let retained = executor
        .retained_effect_batch
        .as_ref()
        .expect("capacity-blocked Validate remains retained");
    assert_eq!(retained.effects.len(), 1);
    let retained_effect = retained.effects[0].effect.clone();
    let retained_ownership = retained.effects[0].ownership.clone();
    let retained_owner = retained.effects[0].ownership.owner().clone();
    let retained_oldest_at = retained.oldest_at;
    assert!(matches!(
        retained_effect,
        AdapterEffect::ValidateBody { tag: effect_tag, round, subject }
            if effect_tag == tag(0)
                && round == fixture.manifest.round
                && subject == fixture.manifest.subject
    ));
    assert_eq!(
        executor.pending_tip_recovery.as_ref(),
        Some(&evidence_before)
    );
    assert_eq!(executor.next_work_id, next_work_id_before);
    assert_eq!(
        executor
            .pending_signatures
            .keys()
            .copied()
            .collect::<Vec<_>>(),
        pending_signature_ids
    );
    assert_eq!(services.sign_tasks, sign_tasks);
    assert!(services.apply_tasks.is_empty());
    ...
    assert_eq!(
        executor
            .step_pending_tip_recovery(Instant::now(), &mut services)
            .expect("retained Validate waits while capacity remains full"),
    ...
    let retained = executor
        .retained_effect_batch
        .as_ref()
        .expect("waiting turn retains the same Validate head");
    assert_eq!(retained.effects.len(), 1);
    assert_eq!(retained.effects[0].effect, retained_effect);
    assert!(
        retained_ownership.exactly_matches_bound_effect_occurrence(
            &retained.effects[0].ownership,
            &retained_effect,
        )
    );
    assert_eq!(retained.effects[0].ownership.owner(), &retained_owner);
    assert_eq!(retained.oldest_at, retained_oldest_at);
    assert_eq!(
        executor.pending_tip_recovery.as_ref(),
        Some(&evidence_before)
    );
