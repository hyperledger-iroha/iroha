...
fn certified_serve_cross_relay_retry_replays_one_terminal_tombstone() {
    ...
    let mut route_fixture = NetworkReplyRouteTestFixture::with_source_capacity(source_a.clone(), 2);
...
fn certified_serve_delayed_lower_view_cross_relay_cannot_resurrect() {
    ...
    let (routes, ownership) = fair_ingress_route_owner(message, requester, source_b, route_delayed);
    assert!(matches!(
        command_tx
...
fn remote_auxiliary_flood_cannot_consume_consensus_or_control_reservations() {
    ...
    let subject = wire::BlockSubject {
        ...
        block_hash: HashOf::<BlockHeader>::from_untyped_unchecked(Hash::new(b"reserved I/O block")),
