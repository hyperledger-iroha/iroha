...
fn mismatches_and_duplicates_never_remove_or_overwrite() {
    ...
    let exact_owner = admitted_owner(&first, 2);
    let slot = super::super::PhysicalSlotId::for_capacity(super::super::CapacityClass::Effect, 0);
    let address = ConcreteWorkAddress::new(exact_owner, 2, slot).expect("valid address");
    let exact_lease = lease(exact_owner, 2, slot, digest);
    let mut registry = ConcreteLifecycleWorkRegistry::default();
    registry
    ...
    let wrong_ordinal_lease = lease(exact_owner, 3, slot, digest);
    assert!(matches!(
    ...
    assert!(matches!(
        registry.take_for_lease(&lease(exact_owner, 2, slot, wrong_digest), slot),
        Err(RegistryError::DigestMismatch)
    ));
    assert_eq!(registry.len(), 1);
    assert!(matches!(
