    ...
    fn durable_view_cut_drains_live_obsolete_carrier_despite_downstream_backpressure() {
        ...
        let next_view = round.view.checked_add(1).expect("fixture view has a successor");
        let next = super::serviced_candidate_store::LeaderWireRecoveryAuthority::from_replayed_adapter(
            round.context_id,
            round.height,
            [0xA6; 32],
        ...
        assert_eq!(replacement.status, super::FairV2IngressLeaderWireStatus::Ingress);
    ...
    fn certified_body_response_survives_view_and_decision_cuts_at_fair_ingress() {
        ...
        let wire::ConsensusMessageV2Payload::CertifiedBodyResponse(response) = &envelope.payload
        else {
