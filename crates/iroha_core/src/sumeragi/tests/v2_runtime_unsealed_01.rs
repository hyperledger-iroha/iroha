...
fn real_adapter_fence_completion_breaks_pre_and_post_timeout_retransmit_debt() {
    ...
    assert!(
        prepare_bypass
            ...
            .is_some_and(|predecessor| {
                let RuntimeSelectedCandidateOwnership::Exact(candidate) = &prepare_bypass.candidate
                else {
                    return false;
                };
    ...
    local_rank_mutation.projection_hash = runtime_scheduler_projection_hash(&local_rank_mutation);
    assert_eq!(
...
include!("v2_runtime_unsealed_01b_lifecycle_bounds.rs");
