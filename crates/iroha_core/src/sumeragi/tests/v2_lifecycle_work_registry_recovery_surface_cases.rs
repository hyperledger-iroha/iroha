...
fn recovered_decision_apply_terminal_settlement_is_exact_and_post_fsync_infallible() {
    ...
    for required in [
        ...
        assert!(
            ...
            "completion rejoin omitted {required}"
        );
    }
    ...
    for required in [
        ...
        assert!(
            ...
            "publication omitted {required}"
        );
    }
    let preflight = publication
        .find("if !exact_current || !exact_staged")
        .unwrap();
    ...
    for required in [
        ...
        assert!(
            adapter_preview.contains(required),
            "adapter preview omitted {required}"
        );
    }
    ...
    let peek = exact_request
        .find("let Some(next) = self.next_effect()")
        .unwrap();
    let preflight = exact_request
        .find(".can_retain_lane_work_effect(&next)")
        .unwrap();
...
fn recovered_next_wal_vote_completion_stays_closed_and_attests_its_ready_pair() {
    ...
    let closed = replay
        .split_once("struct RecoveredLifecycleNextWalVoteSignedBroadcastProjectionV1")
    ...
    for required in [
        ...
        assert!(
            ...
            "closed projection omitted {required}"
        );
    }
    let oracle = replay
    ...
    for required in [
        ...
        assert!(
            ...
            "signed oracle omitted {required}"
        );
    }
    ...
    for required in [
        ...
        assert!(
            pair.contains(required),
            "pair attestation omitted {required}"
        );
    }
}
