...
fn prepared_serve_carrier_is_atomically_superseded_by_decision() {
        ...
        [(false, false), (true, false), (false, true), (true, true)]
    {
        ...
        fs::remove_dir(&temporary_state).expect("unblock prepared-carrier Decision publication");
        assert!(
    ...
    for (dequeue_original, seal_before_retry, seal_after_decision) in [
        ...
        if expected_established_state == V2IoServeState::CompletionPending {
            let error = command_tx
                .serve_completion_delivery_ownership(admission.lifecycle_id, response.request_hash)
                .expect_err("delivery cannot overtake the selected Decision-losing retry carrier");
            assert!(
        ...
        let temporary_state = state_path.with_extension("norito.tmp");
        fs::create_dir(&temporary_state).expect("block atomic carrier-only retirement publication");
        let error = ingress
            .try_recv_if_checked(|_| {
                command_tx
                    .stage_selected_serve_rejection(
...
fn decision_serve_fence_rolls_back_failed_batch_and_converts_before_ordinals() {
        ...
        .expect("publish the terminal conversion and Decision atomically");
    {
        ...
        assert_eq!(
            tracked.state,
            V2IoServeState::Rejected(CertifiedServeNegativeOutcome::SupersededByDurableDecision(
                decided_subject
            ),)
        );
        assert!(tracked.terminal.is_none());
    ...
    );
    {
        ...
        assert!(
            tracked
                .reply_routes
                .as_ref()
                .is_some_and(|routes| routes.has_same_exact_history(&reply_routes_before_failure)),
...
fn completion_pending_serve_is_suppressed_after_decision_before_delivery() {
    ...
    let error = command_tx
        .serve_completion_delivery_ownership(serve_admission.lifecycle_id, response.request_hash)
...
fn production_restart_rejects_negative_tombstone_with_physical_retry_waiter() {
    ...
    let decided_subject = wire::BlockSubject {
        block_hash: HashOf::from_untyped_unchecked(Hash::new(b"negative waiter Decision subject")),
