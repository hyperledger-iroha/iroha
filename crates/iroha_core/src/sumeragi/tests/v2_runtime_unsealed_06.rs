...
fn commit_certificate_response_waits_for_embedded_qc_progress_capacity() {
    ...
    let response = |certificate| {
        wire::ConsensusMessageV2::new(wire::ConsensusMessageV2Payload::CommitCertificateResponse(
...
fn commit_certificate_response_coalesces_with_exact_busy_deferred_qc() {
    ...
    assert_eq!(
        runtime
            .enqueue_network_with_ingress_ownership(
                exact_message.clone(),
                fair_network_ownership(&exact_message, PeerId::new(keys[1].public_key().clone()),),
            )
    ...
    assert_eq!(
        runtime
            .enqueue_network_with_ingress_ownership(
                exact_message.clone(),
                fair_network_ownership(&exact_response, PeerId::new(keys[2].public_key().clone()),),
            )
