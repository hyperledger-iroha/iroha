...
fn durable_kura_replica_advert_rollover_claim_rejects_identity_and_recipient_drift() {
    ...
    let advert = sign_advert(KuraReplicaAdvertV1 {
        ...
        finality_artifact_hash: HashOf::from_untyped_unchecked(Hash::new(b"rollover-finality")),
        keeper_index: 0,
        keeper: keeper.clone(),
        signature: Vec::new(),
    });
    ...
    let (history, local_validator, initial_source) =
        selected_history.expect("history fixture has a deterministic selected CommitQC keeper");
    ...
    successor.set_exact_output_admission_hook(move |post, ticket| {
        ...
        if let Some(previous) = delivered.as_ref() {
            assert_eq!(
                previous, advert,
                "all targets receive the same exact advert"
            );
        } else {
