    ...
    v2_apply_test!(
        prospective_autoscale_retirement_queue_veto_rejects_exact_reserved_route,
        {
            ...
            let journal_dir =
                tempfile::tempdir().expect("retirement Queue-veto journal directory");
            queue
            ...
            let error = fixture
                .service
                .validate_autoscale_retirement_queue_binding(
                    reservation.lane_id,
            ...
            assert!(message.contains(&format!(
                "dataspace {}",
                reservation.dataspace_id.as_u64()
            )));
    ...
    v2_apply_test!(checked_apply_carrier_authorization_binds_exact_state_entry, {
        let fixture = ApplyFixture::new();
        let transaction = fixture
            .body
            .external_transactions()
    ...
    v2_apply_test!(
        startup_reconciliation_consumes_replayed_committed_merge_reservation,
        {
            ...
            let stale_journal_dir =
                tempfile::tempdir().expect("stale reservation journal directory");
            let stale_plan_path = stale_journal_dir.path().join("queue-plans.norito");
            let stale_reservation_path = stale_journal_dir
                .path()
                .join("lane-reservations.norito");
            let stale_first_queue =
                Queue::from_config(QueueConfig::default(), stale_events.clone());
            stale_first_queue
            ...
            assert_eq!(
                stale_reservation.lane_incarnation,
                reservation.lane_incarnation,
            ...
            drop(first_queue);
            let (old_incarnation, new_incarnation) = replace_recreatable_reservation_lane(
                fixture.state.as_ref(),
                &reservation_lane,
            );
            ...
            assert_ne!(
                fixture.state.lane_incarnation_at_height(
                    reservation.lane_id,
