...
include!("v2_adapter_05_direct_lifecycle_sign_publication_cases.rs");
#[test]
...
fn stale_validation_completion_uses_the_current_tag_when_decision_arrives_first() {
    ...
    let sealed = adapter
        ...
        .as_ref()
        .expect("the retained Decision WAL seal is required for exact binding checks");
    assert!(sealed.exactly_binds_pending_apply_decision(
        current_tag,
        decision.round,
