...
use super::{
    ...
    rbc_session_backlog_missing_chunks, rbc_session_has_complete_delivery,
    rbc_session_has_invalid_chunk_shape,
};
use crate::{
...
pub(super) fn apply_hydrated_payload(
    ...
    if expected_chunks == 0 {
        ...
        session.chunks = vec![None; capacity];
        outcome.updated = true;
    } else if chunk_count != expected_chunks {
        session.invalid = true;
        outcome.updated = true;
        outcome.layout_mismatch = true;
        return outcome;
    ...
    if session.recount_received_chunks() {
        outcome.updated = true;
    }
...
mod tests {
    ...
    #[test]
    fn rbc_ready_signature_valid_rejects_malformed_ed25519_signature_r() {
        let chain: ChainId = "rbc-ready-malformed-r-helper".parse().expect("chain id");
        let keypair = checked_sample_keypair(b"rbc-ready-malformed-r-helper".to_vec());
        let topology = crate::sumeragi::network_topology::Topology::new(vec![PeerId::new(
            keypair.public_key().clone(),
        )]);
        let mut ready = RbcReady {
            ...
            epoch: 0,
            roster_hash: Hash::new(&topology.as_ref().to_vec().encode()),
            chunk_root: Hash::prehashed([0x49; Hash::LENGTH]),
            sender: 0,
            signature: Vec::new(),
        };
    ...
    #[test]
    fn rbc_deliver_signature_valid_rejects_malformed_ed25519_signature_r() {
        let chain: ChainId = "rbc-deliver-malformed-r-helper".parse().expect("chain id");
        let keypair = checked_sample_keypair(b"rbc-deliver-malformed-r-helper".to_vec());
        let topology = crate::sumeragi::network_topology::Topology::new(vec![PeerId::new(
            keypair.public_key().clone(),
        )]);
        let mut deliver = RbcDeliver {
            ...
            epoch: 0,
            roster_hash: Hash::new(&topology.as_ref().to_vec().encode()),
            chunk_root: Hash::prehashed([0x4B; Hash::LENGTH]),
            sender: 0,
            signature: Vec::new(),
            ready_signatures: Vec::new(),
        };
...
pub(super) fn rbc_ready_signature_valid(
    ...
    let Some(signature) = rbc_signature_for_peer(&ready.signature, peer) else {
        return false;
    };
...
pub(super) fn rbc_deliver_signature_valid(
    ...
    let Some(signature) = rbc_signature_for_peer(&deliver.signature, peer) else {
        return false;
    };
    signature.verify(peer.public_key(), &preimage).is_ok()
}
...
pub(super) enum DeliverAcceptance {
    ...
    InvalidChunkShape { received: u32, total: u32 },
    InvalidChunkRoot,
}
...
pub(super) fn evaluate_deliver_acceptance_with_policy(
    ...
    if total == 0 || received > total {
        return DeliverAcceptance::InvalidChunkShape { received, total };
    }
...
impl Actor {
    pub(super) fn release_block_payload_dedup(&self, key: &BlockPayloadDedupKey) {
        let mut guard = super::super::lock_block_payload_dedup_cache(&self.block_payload_dedup);
        guard.remove(key);
    }
    ...
    pub(super) fn evaluate_rbc_deliver_outcome(
        ...
        let mut drop_kind: Option<super::status::ConsensusMessageReason> = None;
        if session.is_invalid() {
            ignored = true;
            warn!(
                height = key.1,
        ...
        if !session.is_invalid() {
            ...
            match acceptance {
                ...
                DeliverAcceptance::InvalidChunkShape { received, total } => {
                    session.invalid = true;
                    invalidate = true;
                    ignored = true;
                    drop_kind = Some(super::status::ConsensusMessageReason::InvalidPayload);
                    warn!(
                        height = key.1,
                        view = key.2,
                        block = %key.0,
                        received,
                        total,
                        "invalid RBC DELIVER session chunk shape; refusing delivery"
                    );
                }
                DeliverAcceptance::Accept => {
                    first_deliver =
    ...
    pub(super) fn handle_rbc_store_evictions(&mut self, removed: &[SessionKey]) {
        ...
        for key in removed {
            let existed = self.subsystems.da_rbc.rbc.sessions.contains_key(key);
