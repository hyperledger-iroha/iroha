...
impl Actor {
    ...
    fn authenticate_retained_snapshot_with_fixed_authority(
        ...
        if outcome.is_conflict()
            || !self.install_authenticated_commit_signer_locks(qc, &signers, &topology)
        {
            return None;
        }
        AuthenticatedCommitRoster::from_registered(
    ...
    fn rehydrate_pending_from_block_for_qc(
        ...
        if !self.enforce_pending_block_cap(qc.subject_block_hash, "qc_local_payload_rehydrate") {
            debug!(
                height = qc.height,
                view = qc.view,
                block = %qc.subject_block_hash,
                source,
                "deferring QC payload rehydration because retained requeue owners fill the pending-body cap"
            );
            return false;
        }
    ...
    fn qc_missing_payload_target(
        qc: &crate::sumeragi::consensus::Qc,
    ) -> (HashOf<BlockHeader>, u64, u64) {
        if matches!(qc.phase, crate::sumeragi::consensus::Phase::NewView)
            && let Some(highest_qc) = qc.highest_qc
            && highest_qc.subject_block_hash == qc.subject_block_hash
        {
            return (
    ...
    fn force_targeted_missing_payload_fetch_for_qc(
        ...
        let now = Instant::now();
        let (target_hash, target_height, target_view) = Self::qc_missing_payload_target(qc);
        let stall_snapshot =
            self.missing_payload_fetch_window_snapshot(target_height, target_hash, now);
        ...
        let mut stall_dwell_ms = 0_u128;
        let mut last_targeted_fetch_at_ms: Option<u128> = None;
        if let Some(stall) = stall_snapshot {
            stall_window_index = stall.window_index;
            stall_dwell_ms = now.saturating_duration_since(stall.entered_at).as_millis();
            last_targeted_fetch_at_ms = stall
                .last_targeted_fetch_at
                .map(|at| now.saturating_duration_since(at).as_millis());
            if self.missing_payload_fetch_window_already_emitted(
                stall.height,
                stall.block_hash,
                stall.window_index,
            ) {
                debug!(
                    ...
                    stall_dwell_ms = now.saturating_duration_since(stall.entered_at).as_millis(),
                    last_targeted_fetch_at_ms,
                    "suppressing repeated deferred-QC targeted fetch in same-height stall window"
                );
                return false;
            }
        ...
        if let Some(stall) = stall_snapshot {
            self.mark_missing_payload_fetch_window_emitted(
                stall.height,
                stall.block_hash,
                stall.window_index,
                now,
        ...
        info!(
            ...
            targets = targets.len(),
            same_height_stall_mode = stall_snapshot.is_some(),
            same_height_stall_window_index = stall_window_index,
            same_height_stall_all_peers = stall_all_peers,
            same_height_stall_dwell_ms = stall_dwell_ms,
            same_height_stall_last_targeted_fetch_at_ms = last_targeted_fetch_at_ms,
            reason,
    ...
    fn deferred_frontier_payload_fetch_due(
        &self,
        ...
        let qc = &entry.qc;
        let (_, target_height, _) = Self::qc_missing_payload_target(qc);
        matches!(
            qc.phase,
            crate::sumeragi::consensus::Phase::Commit | crate::sumeragi::consensus::Phase::NewView
        ) && target_height == committed_height.saturating_add(1)
            && target_height == self.active_consensus_round_height()
    ...
    fn force_deferred_frontier_payload_fetch(
        &mut self,
        qc: &crate::sumeragi::consensus::Qc,
        reason: &'static str,
        now: Instant,
        ...
        let (target_hash, target_height, target_view) = Self::qc_missing_payload_target(qc);
        if target_height != self.committed_height_snapshot().saturating_add(1)
            || target_height != self.active_consensus_round_height()
            || target_hash != qc.subject_block_hash
            || self.frontier_block_materialized_locally(target_hash)
        {
            return false;
        }

        if matches!(qc.phase, crate::sumeragi::consensus::Phase::Commit) {
            self.note_frontier_commit_qc_observed(target_hash, target_height, target_view, now);
        } else {
        ...
        let targeted_fetch_requested = self.force_targeted_missing_payload_fetch_for_qc(qc, reason);
        if exact_owner_routed || exact_body_requested || targeted_fetch_requested {
            info!(
                phase = ?qc.phase,
                height = qc.height,
                view = qc.view,
                ...
                exact_owner_routed,
                exact_body_requested,
                targeted_fetch_requested,
                reason,
                "prompted deferred frontier QC payload repair"
            );
        }
        exact_owner_routed || exact_body_requested || targeted_fetch_requested
    }
    ...
    pub(super) fn try_replay_deferred_qcs(&mut self) -> bool {
        ...
        for (key, qc) in deferred_entries {
            ...
            if let Some(stall_window) = stall_window
                && now.saturating_duration_since(state.last_attempt) < stall_window
            {
                continue;
            }
        ...
        for action in actions {
            ...
            match action {
                ...
                ReplayAction::Expire { key, qc, reason } => {
                    ...
                    let recovered = self.handle_roster_unavailable_recovery(
                        qc.height,
                        current_view,
                    ...
                    if let Some(state) = self.deferred_qc_roster_state.get_mut(&key) {
                        // Avoid hammering recovery every tick while the roster remains unavailable.
                        state.first_seen = now;
                        state.last_attempt = now;
                    }
                    if recovered {
    ...
    pub(super) fn try_replay_deferred_missing_payload_qcs(&mut self, now: Instant) -> bool {
        ...
        for (key, entry) in deferred_entries {
            ...
            if self.deferred_frontier_payload_fetch_due(&entry, committed_height, now) {
                actions.push(ReplayAction::PromptFrontierPayloadFetch {
                    key,
                    qc: entry.qc.clone(),
                    reason: entry.reason,
                });
            ...
            if let Some(stall_window) = stall_window
                && now.saturating_duration_since(entry.last_attempt) < stall_window
            {
                continue;
            }
        ...
        for action in actions {
            ...
            match action {
                            ...
                            )
                        {
                                ...
                                )
                            {
                                if !matches!(action, super::FrontierRecoveryAdvance::None) {
                                    ...
                                    self.deferred_missing_payload_qcs.get_mut(&key)
                                {
                                    entry.last_attempt = now;
                                }
                                branch_progress |=
                                    !matches!(action, super::FrontierRecoveryAdvance::None);
                ...
                ReplayAction::PromptFrontierPayloadFetch { key, qc, reason } => {
                    let fetched = self.force_deferred_frontier_payload_fetch(&qc, reason, now);
                    if let Some(entry) = self.deferred_missing_payload_qcs.get_mut(&key) {
                        entry.last_attempt = now;
                        if fetched {
                            entry.attempts = entry.attempts.saturating_add(1);
                ...
                ReplayAction::Expire { key, qc, reason } => {
                        ...
                        )
                    {
                            ...
                            )
                        {
                            ...
                            branch_progress = true;
                        } else if let Some(entry) = self.deferred_missing_payload_qcs.get_mut(&key)
                        {
                            entry.last_attempt = now;
                            branch_progress |= fetched;
                        }
    ...
    pub(super) fn requester_has_local_roster_proof(
        ...
        let has_committed_snapshot = self
            .state
        ...
        let has_precommit_signer_record =
            crate::sumeragi::status::precommit_signers_for_round(block_hash, height, view, epoch)
                .is_some();
        let highest_matches = self.highest_qc.is_some_and(|highest| {
            highest.phase == crate::sumeragi::consensus::Phase::Commit
                && highest.subject_block_hash == block_hash
                && highest.height == height
                && highest.view == view
        });

        has_committed_snapshot || has_commit_qc || has_precommit_signer_record || highest_matches
    }
    ...
    fn qc_signers_for_votes_with_stats(
        ...
        self.qc_signers_for_votes_with_stats_for_chain_order(
            phase,
            block_hash,
            height,
            view,
    ...
    #[allow(clippy::too_many_arguments)]
    fn qc_signers_for_votes_with_stats_for_chain_order(
        &self,
        phase: crate::sumeragi::consensus::Phase,
        block_hash: HashOf<BlockHeader>,
        height: u64,
    ...
    #[allow(clippy::too_many_arguments)]
    fn preferred_recorded_new_view_chain_order_binding(
        &self,
        block_hash: HashOf<BlockHeader>,
        height: u64,
        view: u64,
        ...
        for vote in self.stored_votes().filter(|vote| {
            ...
            candidates.insert((vote.chain_order_hash, vote.rechain_seq));
        }

        let voting_len = signature_topology.as_ref().len();
        let mut best_binding = current;
        let mut best_score = (0usize, 0usize, true);
        for binding in candidates {
            let (accepted_votes, stats) = self.qc_signers_for_votes_with_stats_for_chain_order(
                crate::sumeragi::consensus::Phase::NewView,
                block_hash,
                height,
                view,
            ...
            let signers = accepted_votes.keys().copied().collect::<BTreeSet<_>>();
            let voting_signers = voting_signer_count(&signers, voting_len);
            let score = (voting_signers, stats.raw_votes, binding == current);
            if score > best_score {
                best_score = score;
    ...
    pub(super) fn defer_qc_if_block_missing_with_quorum_hint(
        ...
        if self.should_suppress_lock_rejected_block_fetch(
            height,
            block_hash,
            "defer_qc_if_block_missing_with_quorum_hint",
        ) {
            self.clear_missing_block_view_change(&block_hash);
            debug!(
                phase = ?phase,
                ...
                view,
                block = %block_hash,
                "deferring QC aggregation without fetch: block hash is lock-rejected under the current lock anchor"
            );
            return true;
        }
        ...
        if deferred {
            ...
            } else if let Some((first_seen, last_requested, attempts)) = self
                .pending
                .missing_block_requests
                .get(&block_hash)
                .map(|stats| (stats.first_seen, stats.last_requested, stats.attempts))
            {
                if self
                    .phase_tracker
                    .current_view(height)
    ...
    pub(super) fn retry_missing_block_requests(
        ...
        for block_hash in pending_keys {
            ...
            let mut effective_retry_window =
                self.missing_block_retry_window_with_backoff(retry_window, stats_snapshot.attempts);
            if matches!(
                stats_snapshot.priority,
                super::MissingBlockPriority::Consensus
            ) && self
                .missing_qc_height_stall_snapshot(stats_snapshot.height, now)
                .is_some_and(|stall| stall.mode_active && stall.height == stats_snapshot.height)
            {
                effective_retry_window =
                    effective_retry_window.max(self.missing_qc_height_stall_window());
            }
    ...
    pub(super) fn try_form_qc_from_votes(
        ...
        let voting_len = signature_topology.as_ref().len();
        let (mut chain_order_hash, mut rechain_seq) = self
            .vnext_chain_order_binding_for_signature_topology(
                height,
                view,
                consensus_mode,
        ...
        if matches!(phase, crate::sumeragi::consensus::Phase::NewView) {
            (chain_order_hash, rechain_seq) = self.preferred_recorded_new_view_chain_order_binding(
                block_hash,
                height,
                view,
                epoch,
        ...
        let expected_chain_order_override =
            matches!(phase, crate::sumeragi::consensus::Phase::NewView)
                .then_some((chain_order_hash, rechain_seq));
    ...
    fn qc_signer_snapshot(
        ...
        let (consensus_mode, mode_tag, _) = self.consensus_context_for_height(height);
        let (valid_votes, stats) = self.qc_signers_for_votes_with_stats_for_chain_order(
            phase,
            block_hash,
            height,
            view,
            ...
            consensus_mode,
            mode_tag,
            chain_order_binding.0,
            chain_order_binding.1,
        );
    ...
    pub(super) fn qc_missing_block_defer(
        ...
        let target_hash = block_hash;
        let target_height = Self::missing_dependency_subject_height_for_phase(phase, height);
        ...
        if !block_known
            && self.should_suppress_lock_rejected_block_fetch(
                target_height,
                target_hash,
                "qc_missing_block_defer",
            )
        {
            self.clear_missing_block_view_change(&target_hash);
            debug!(
                phase = ?phase,
                height,
                view,
                ...
                target_view,
                target_block = %target_hash,
                "keeping QC deferred: missing block hash is lock-rejected under the current lock anchor"
            );
            return true;
        }
        ...
        if !block_known && quorum_met {
            ...
            let contiguous_frontier = target_height
            ...
            if contiguous_frontier {
                ...
                if resilience_generic_fallback {
                    ...
                    let (
                        ...
                        .get(&target_hash)
                        .and_then(|request| {
                            (request.height == target_height).then(|| {
                                let request_age =
                    ...
                    if !targets.is_empty() && !missing_fetch_fresh {
                        ...
                        let requester_roster_proof_known = self.requester_has_local_roster_proof(
                            target_hash,
                            target_height,
                            target_view,
                        );
        ...
        for peer in &signer_peers {
                ...
                && *existing != subject
            {
                super::status::activate_consensus_safety_halt(
                    "authenticated_commit_signer_lock_conflict",
                    qc.height,
                    qc.epoch,
                    Some(existing.block_hash),
                    Some(qc.subject_block_hash),
                );
                error!(
                    height = qc.height,
    ...
    pub(super) fn register_authenticated_commit_qc(
        ...
        match super::status::record_authenticated_commit_authority_while_guarded(
            ...
            | super::status::CommitQcRecordOutcome::Duplicate => self
                .install_authenticated_commit_signer_locks(
                    qc,
                    &authenticated_signers,
                    &authoritative_topology,
                ),
    ...
    fn handle_qc_with_aggregate_and_roster_and_stake(
        ...
        let (expected_chain_order_hash, expected_rechain_seq) = expected_chain_order_override
            .unwrap_or_else(|| self.vnext_chain_order_binding_for_qc(&qc));
        if qc.chain_order_hash != expected_chain_order_hash
            || qc.rechain_seq != expected_rechain_seq
        {
            warn!(
        ...
        if !block_known_locally {
            ...
            let committed_height = self.committed_height_snapshot();
            let subject_height = target_height;
            if self.missing_hash_is_non_actionable_dependency(
                subject_height,
                target_view,
                target_hash,
                ...
                self.deferred_missing_payload_qcs.remove(&qc_key);
                self.clear_missing_block_request(&target_hash, MissingBlockClearReason::Obsolete);
                self.clear_missing_block_view_change(&target_hash);
                debug!(
                    phase = ?qc.phase,
                    height = qc.height,
            ...
            if matches!(qc.phase, crate::sumeragi::consensus::Phase::Commit) {
                ...
                if let Some(stats) = self.pending.missing_block_requests.get_mut(&target_hash)
                    && retry_window > stats.retry_window
                {
                    stats.retry_window = retry_window;
                }
