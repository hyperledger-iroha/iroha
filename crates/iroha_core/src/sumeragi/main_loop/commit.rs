    ...
    let spawn_result =
        crate::sumeragi::sumeragi_thread_builder("sumeragi-commit").spawn(move || {
            'worker: while let Ok(work) = work_rx.recv() {
                ...
                let deliver_safety_halt =
                    matches!(&result.outcome, CommitOutcome::SafetyHalted { .. });
                loop {
                    if crate::sumeragi::consensus_recovery_required_active()
                        || (crate::sumeragi::consensus_safety_halt_active() && !deliver_safety_halt)
                    {
                        continue 'worker;
                    }
        ...
        return (
            CommitOutcome::Rejected {
                ...
                error: BlockValidationError::ExecutionContextInvalid(
                    "consensus fail-stop active".to_owned(),
                ),
                pipeline_events,
            },
            timings,
...
impl Actor {
    ...
    fn apply_commit_outcome(
        ...
        if super::status::consensus_recovery_required_active() {
            return false;
        }
