...
fn snapshot_rejects_nonregular_artifacts() {
    ...
    let (store, _) = ServicedCandidateStore::open(&wal, context.id(), context.height, OWNER_A, 1)
        .expect("derive snapshot path");
    fs::create_dir(store.path_for_test()).expect("place directory at snapshot path");
    assert!(ServicedCandidateStore::open(&wal, context.id(), context.height, OWNER_A, 1).is_err());
}
...
fn finalized_snapshot_retirement_leaves_successor_rollover_empty() {
    ...
    let (store, _) = ServicedCandidateStore::open(&wal, context.id(), context.height, OWNER_A, 2)
    ...
    assert!(
        store
            .persist_with_producer_continuations(&BTreeMap::new(), &producer_continuations, true,)
            .is_err(),
    ...
    assert!(
        ServicedCandidateStore::open(&wal, successor.id(), successor.height, OWNER_A, 2,).is_err(),
    ...
    let (_successor, restored) =
        ServicedCandidateStore::open(&successor_wal, successor.id(), successor.height, OWNER_A, 2)
