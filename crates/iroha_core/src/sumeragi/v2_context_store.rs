...
impl V2ContextStore {
    ...
    ///
    /// Candidate validation uses this path so untrusted evidence cannot create
    /// directories as a side effect. `None` means the node lacks the required
    /// deterministic history and must fail the admission closed.
    ///
    /// # Errors
    ///
    /// Returns an error when the existing path cannot be inspected or is not a
    /// directory.
    pub(crate) fn open_existing(
        ...
        match fs::metadata(&directory) {
            Ok(metadata) if metadata.is_dir() => Ok(Some(Self { directory })),
            Ok(_) => Err(io_error(
                &directory,
                io::Error::new(
                    ErrorKind::NotADirectory,
                    "v2 context store is not a directory",
                ),
            )),
...
mod tests {
    ...
    #[test]
    fn read_only_open_does_not_create_missing_context_directory() {
        let root = tempfile::tempdir().expect("tempdir");
        assert!(
            V2ContextStore::open_existing(root.path())
                .expect("read-only missing lookup")
                .is_none()
        );
        assert!(!root.path().join("contexts").exists());

        let store = V2ContextStore::open(root.path()).expect("create store");
        store.persist(&record()).expect("persist record");
        let existing = V2ContextStore::open_existing(root.path())
            .expect("read-only existing lookup")
            .expect("context directory exists");
        assert_eq!(
            existing.load(1).expect("load existing record"),
            Some(record())
        );
    }
