include!("v2_lane_work_terminal_ingress_tests.rs");
...
fn prepare_autonomous_test_lane(
    ...
    assert_eq!(dataspace_id, DataSpaceId::new(7));
    ...
    let lane = nexus
        ...
        .expect("autonomous lane geometry was installed before genesis");
    assert_eq!(lane.dataspace_id, dataspace_id);
    let manifests = adapter.state.lane_manifests.read();
    let status = manifests
        .status(lane_id)
        .expect("autonomous lane manifest was installed before genesis");
    ...
    let committee = adapter
        ...
        .expect("configured lane authority resolves at the frozen height")
        .into_validators();
    assert_eq!(committee.into_iter().collect::<BTreeSet<_>>(), expected);
    assert_eq!(
        super::super::v2_recovery::committed_nexus_amx_context_hash(adapter.state.as_ref())
            .expect("valid configured Nexus authority"),
        adapter.context.nexus_amx_context_hash
    );
    assert_eq!(
        super::super::v2_recovery::committed_execution_policy_hash(adapter.state.as_ref())
            .expect("valid configured execution policy"),
...
fn autonomous_nonmember_test_fixture(
    height: u64,
    lane_only_key_seed: u8,
    voting_enabled: bool,
) -> (V2LaneWorkAdapter, Vec<KeyPair>, Vec<KeyPair>) {
    let mut lane_keys = (1_u8..=4)
        .map(|seed| {
            KeyPair::try_from_seed(vec![seed; 32], Algorithm::BlsNormal)
                .expect("deterministic global fixture validator")
        })
        .collect::<Vec<_>>();
    lane_keys.sort_by(|left, right| left.public_key().cmp(right.public_key()));
    lane_keys.remove(3);
    lane_keys.push(
        KeyPair::try_from_seed(vec![lane_only_key_seed; 32], Algorithm::BlsNormal)
            .expect("deterministic lane-only validator"),
    );
    lane_keys.sort_by(|left, right| left.public_key().cmp(right.public_key()));
    let (adapter, global_keys) = fixture_at_height_inner_with_initial_lane(
    ...
    assert!(
        lane_keys
            .iter()
            .all(|key| key.public_key() != adapter.local_peer.public_key())
    );
    (adapter, global_keys, lane_keys)
}
...
fn autonomous_ready_crosses_payload_and_certificate_durability_before_commit_vote() {
    ...
    assert!(
        adapter
            ...
            .expect("read exact autonomous body availability")
    );
    ...
    assert!(
        !adapter
            ...
            .expect("read current autonomous transition"),
    ...
    assert!(
        adapter
            .missing_autonomous_artifact_became_terminal(&proposal, &restore_error)
    ...
    assert!(
        !adapter
            ...
            .expect("read lane progress eligibility"),
    ...
    competing.descriptor.lane_block_view = competing.descriptor.lane_block_view.saturating_add(1);
    ...
    validate_lane_block_proposal(&competing)
        .expect("the competing candidate is structurally valid");
    assert!(
        !adapter
            .lane_application_receipt_available(&competing)
            .expect("authenticate the occupied receipt independently of the candidate")
    );
    assert!(
        !adapter
            .proposal_can_progress(&competing)
            .expect("a valid applied slot rejects the competing candidate")
    );
    assert!(
        !adapter
            .proposal_can_be_transported(&competing)
            .expect("a valid applied slot rejects competing transport")
    );
    assert!(
        !adapter.output_guard.restart_required(),
        "a losing candidate cannot poison local storage"
    );
    let source = adapter
        .state
        ...
        .join("lane_artifacts");
    let receipt_path = source.join("application_receipts.norito");
    let index_path = source.join("application_receipts.index");
    std::fs::write(
        &receipt_path,
        b"damaged previously authenticated application receipt",
    )
    .expect("damage the occupied durable receipt after its successful read");
    let before = (
        std::fs::read(&receipt_path).unwrap(),
        std::fs::read(&index_path).unwrap(),
    );
    assert!(
        adapter.proposal_can_progress(&competing).is_err(),
        "a foreign candidate cannot hide corruption in the occupied local receipt"
    );
    assert!(adapter.output_guard.restart_required());
    assert!(
        adapter.lane_sessions.get(&session_key).is_some(),
        "failure retains the certified owner"
    );
    assert_eq!(
        (
            std::fs::read(receipt_path).unwrap(),
...
fn voting_validator_outside_lane_committee_skips_private_new_view_cursor_restore() {
    ...
    assert!(adapter.frozen_roster_contains(&adapter.local_peer));
    assert!(!adapter.local_can_own_autonomous_payload(&proposal));
    assert!(adapter.lane_sessions.contains_proposal(&proposal));
    assert!(
        adapter
            .proposal_body_available(&proposal)
            .expect("the exact protected autonomous carrier is available")
    );
    assert!(autonomous_artifact(&adapter, &proposal, adapter.context.epoch).is_none());

    // Read the same retained carrier as a real overlapping committee member.
    // Membership requires private cursor custody even when public replica bytes exist.
    let observer = adapter.local_peer.clone();
    let member = lane_keys
        .iter()
        .find(|key| {
            global_keys
                .iter()
        ...
        .expect("the fixture has a validator in both global and lane committees");
    adapter.local_peer = PeerId::new(member.public_key().clone());
    assert!(adapter.frozen_roster_contains(&adapter.local_peer));
    assert!(adapter.local_can_own_autonomous_payload(&proposal));
    assert!(matches!(
    ...
    assert_eq!(
        ...
        "a non-member verifies the exact protected carrier without private READY custody",
    );
    adapter
    ...
    assert!(
        autonomous_artifact(&adapter, &proposal, adapter.context.epoch).is_none(),
        "verification must not synthesize private committee custody",
    );
    assert!(!adapter.output_guard.restart_required());
}
#[test]
fn voting_validator_outside_lane_committee_retires_transport_only_loser_without_queue() {
    let (mut adapter, global_keys, lane_keys) = autonomous_nonmember_test_fixture(9, 0xF1, true);
    let lane_id = LaneId::new(1);
    let dataspace_id = DataSpaceId::new(7);
...
fn autonomous_producer_retries_after_predecessor_application_receipt_arrives() {
    ...
    let successor_context = successor_context_for_parent(&parent, &predecessor_block, &keys);
...
fn autonomous_fixture_binds_final_lane_context_before_opening_signing_guards() {
    for mode in [wire::ConsensusMode::Permissioned, wire::ConsensusMode::Npos] {
        for author in [false, true] {
            ...
            let context = adapter.context.clone();
            let snapshot = crate::snapshot::canonical_state_snapshot_hash(adapter.state.as_ref())
                .expect("capture initialized autonomous fixture");
            assert!(adapter.voting_enabled);
            ...
            assert_eq!(
                crate::snapshot::canonical_state_snapshot_hash(adapter.state.as_ref())
                    .expect("capture verified autonomous fixture"),
                snapshot,
                "preparation must preserve configured geometry and every retained State journal"
            );
            assert_eq!(
                adapter.context.id(),
                context.id(),
...
fn historical_autonomous_hydration_replaces_same_slot_conflict_at_capacity() {
    ...
    let successor_context = successor_context_for_parent(&adapter, carrier.as_ref(), &keys);
...
fn nonmember_canonical_replica_pre_qc_fixture() -> NonmemberCanonicalReplicaPreQcFixture {
    let (mut adapter, global_keys, lane_keys) = autonomous_nonmember_test_fixture(2, 0xF2, true);
    let lane_id = LaneId::new(1);
    let dataspace_id = DataSpaceId::new(7);
    prepare_autonomous_test_lane(&mut adapter, &lane_keys, lane_id, dataspace_id);
    ...
    let successor_context = successor_context_for_parent(&adapter, &carrier, &global_keys);
    let committed = ValidBlock::committed_from_replay_signed_block(carrier);
    commit_test_block_to_state(adapter.state.as_ref(), &committed, &adapter.context);
    assert_ne!(
        adapter
...
fn finalized_public_autonomous_certificate_is_served_across_disjoint_rosters() {
    ...
    let session = CommittedLaneBlockSession {
        ...
        prepare_qc: finalized_nonmember_prepare_qc(&public_proposal, &public_payload, &lane_keys),
    ...
    assert!(
        adapter
            .proposal_predecessor_is_ready_for_progress(&public_proposal)
            .expect("authenticate retained predecessor readiness")
    );
    assert_eq!(
...
fn restarted_nonmember_accepts_public_qcs_and_persists_only_canonical_replica() {
    ...
    assert!(
        recovered
            ...
            .expect("read pending historical recovery")
    );
    assert!(matches!(
        recovered
...
fn repeated_non_empty_retries_never_make_queue_plan_synced_work_ordinary_eligible() {
    ...
    assert_eq!(
        adapter
            ...
            .into_keys()
            .collect::<Vec<_>>(),
        vec![(lane_id, dataspace_id)],
        "single-route retry uses the authority established before its durable parent and journals",
    );
    adapter
        .retain_merge_sidecars_for_global_view(0, None, None)
...
fn autonomous_producer_outside_lane_committee_skips_storage_planning() {
    let (mut observer, _, lane_keys) = autonomous_nonmember_test_fixture(9, 0xE9, false);
    let lane_id = LaneId::new(1);
    let dataspace_id = DataSpaceId::new(7);
