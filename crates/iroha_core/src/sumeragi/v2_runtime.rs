...
impl RuntimeIngressOwnershipEvidence {
    ...
    fn merge_downstream(&mut self, candidate: Self) -> Result<(), RuntimeIngressMergeError> {
        ...
        // vote, and transport traffic. Aggregate QCs and TCs are idempotent
        // authenticated facts which can legitimately arrive from every voter,
        // so certificates retain a bounded set of disjoint source carriers in
        // one serialized runtime command.
