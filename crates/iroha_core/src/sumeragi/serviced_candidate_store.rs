...
/// capability lets it retire view-scoped records made obsolete by a certified
/// view advance or durable Decision without treating its own terminal
/// projection as authority. Historical certified-body responses are request-
/// bound recovery completions; the reducer can make Decision durable before
/// obtaining that exact body, so neither cut can obsolete them. The capability
/// is process-local and never enters either snapshot format.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
impl LeaderWireRecoveryAuthority {
    ...
    fn obsoletes_identity(self, identity: &FairV2IngressLeaderWireIdentity) -> bool {
        identity.phase != FairV2IngressLeaderWirePhase::CertifiedResponse
...
impl LeaderWireLifecycleStoreGate {
    ...
    fn load_and_reconcile(
        ...
        for mut record in decoded.records {
            ...
            // has durably advanced beyond this view, no view-scoped ingress
            // owner from the obsolete episode can be resurrected. A request-
            // bound certified-body response remains necessary historical
            // recovery data even after Decision: the reducer can decide before
            // obtaining its exact body. Retire rejected records while preserving
            // both file high-watermarks so subsequent admission cannot reuse an
            // ordinal.
