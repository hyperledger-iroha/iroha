...
// Temporary allowances cover scaffolding for the upcoming NPoS pipeline. Remove
// them once the new scheduling and telemetry hooks are fully wired.
#![allow(dead_code)]
...
#[cfg(test)]
use new_view::NewViewValidationError;
use new_view::{NewViewFreshnessError, check_new_view_freshness, validate_new_view_signature};
use pacing::{
    AdaptiveAction, AdaptiveObservabilityMetrics, AdaptiveObservabilityState, BackpressureGate,
    Pacemaker, PacemakerBackpressure, PacemakerBackpressureAction, ProposeAttemptMonitor,
    TickTimingMonitor,
};
use pending_block::{
...
fn missing_quorum_stale(
    ...
    commit_timeout != Duration::ZERO && vote_count < min_votes && pending_age >= commit_timeout
}
...
fn prevote_quorum_stale(
    ...
    matches!(qc_phase, Some(crate::sumeragi::consensus::Phase::Prevote))
...
fn qc_bls_preimage(
    ...
    if qc.phase == crate::sumeragi::consensus::Phase::Available {
        ...
        available_vote_preimage(chain_id, mode_tag, &vote)
    } else {
...
pub(crate) fn validate_block_sync_qc(
    ...
    if !qc_aggregate_consistent(qc, &signature_topology, chain_id, mode_tag) {
...
fn derive_block_sync_qc_from_signers(
    ...
    let roster_len_hint = if commit_topology_len > 0 {
        commit_topology_len
    } else {
        block_signers
            .iter()
    ...
    if roster_len_hint == 0 {
        return None;
    }
    let min_votes = if roster_len_hint > 3 {
        ((roster_len_hint.saturating_sub(1)) / 3) * 2 + 1
    } else {
        roster_len_hint
    };
    ...
    if commit_topology_len > 0
        && block_signers.iter().any(|idx| {
            usize::try_from(*idx)
                .ok()
                .is_some_and(|val| val >= commit_topology_len)
        })
    {
        warn!(
            incoming_hash = %block_hash,
    ...
    let mut signers_bitmap = vec![0u8; roster_len_hint.div_ceil(8)];
    for signer in block_signers {
        let Ok(idx) = usize::try_from(*signer) else {
            continue;
        };
        if idx >= roster_len_hint {
            continue;
        }
        let byte = idx / 8;
        let bit = idx % 8;
...
pub(super) fn vote_signature_check(
    ...
    let preimage = match vote.phase {
        crate::sumeragi::consensus::Phase::Available => {
            let available = crate::sumeragi::consensus::AvailableVote {
            ...
            available_vote_preimage(chain_id, mode_tag, &available)
        }
        _ => vote_preimage(chain_id, mode_tag, vote),
    };
    if vote.bls_sig.is_empty() {
...
fn fetch_pending_block_response(
    pending: Option<&SignedBlock>,
    committed: Option<&SignedBlock>,
    state: &State,
    kura: &Kura,
    mode_tag: &str,
    ...
    pending
        .map(|block| BlockMessage::BlockCreated(super::message::BlockCreated::from(block)))
        .or_else(|| {
            committed.map(|block| {
                BlockMessage::BlockSyncUpdate(block_sync_update_with_roster(
                    block, state, kura, mode_tag, trusted, me,
                ))
            })
        })
}
...
impl NewViewEntry {
    ...
    fn update_highest(&mut self, candidate: crate::sumeragi::consensus::QcHeaderRef) {
        ...
        let promotes_phase = incoming == existing
            && candidate.phase == crate::sumeragi::consensus::Phase::Precommit
            && self.highest_qc.phase != crate::sumeragi::consensus::Phase::Precommit;
...
fn should_emit_availability_vote(da_enabled: bool, vote_already_sent: bool) -> bool {
    da_enabled && !vote_already_sent
}

struct AvailabilityVoteInputs<'a> {
    chain_id: &'a ChainId,
    mode_tag: &'a str,
...
fn build_availability_vote(inputs: AvailabilityVoteInputs<'_>) -> AvailableVote {
    ...
    let preimage = available_vote_preimage(chain_id, mode_tag, &vote);
    let signature = Signature::new(private_key, &preimage);
...
pub(super) struct Actor {
    ...
    merge_committee: MergeCommitteeState,
    ...
    vote_log: BTreeMap<
        ...
        crate::sumeragi::consensus::Vote,
    >,
    commit_votes: BTreeMap<CommitVoteKey, crate::sumeragi::consensus::CommitVote>,
    exec_vote_log: BTreeMap<
        (u64, u64, u64, crate::sumeragi::consensus::ValidatorIndex),
        crate::sumeragi::consensus::ExecVote,
    >,
    ...
    locked_qc: Option<crate::sumeragi::consensus::QcHeaderRef>,
    vrf: VrfActor,
    broadcast_new_views: BTreeMap<(u64, u64), NewViewBroadcastState>,
    view_change_chain: super::view_change::ProofChain,
...
struct PendingBlockState {
    ...
    availability_rebroadcast_last_sent: BTreeMap<HashOf<BlockHeader>, Instant>,
...
impl Actor {
    ...
    pub(super) fn attach_commit_worker(&mut self) -> std::thread::JoinHandle<()> {
        ...
        self.commit.work_tx = Some(commit_handle.work_tx);
        self.commit.result_rx = Some(commit_handle.result_rx);
...
fn should_send_inline_background(request: &BackgroundRequest) -> bool {
    match request {
        BackgroundRequest::Broadcast { msg } | BackgroundRequest::Post { msg, .. } => matches!(
            msg,
        ...
        | BackgroundRequest::BroadcastControlFlow { .. } => false,
    }
}
...
fn reset_runtime_state_for_mode_flip(
    pacemaker: &mut Pacemaker,
    new_view_tracker: &mut NewViewTracker,
    view_change_chain: &mut super::view_change::ProofChain,
    broadcast_new_views: &mut BTreeMap<(u64, u64), NewViewBroadcastState>,
    phase_tracker: &mut PhaseTracker,
    ...
    *view_change_chain = super::view_change::ProofChain::default();
    broadcast_new_views.clear();
...
fn stake_quorum_reached_for_peers(
    ...
    for ((_lane_id, validator_id), record) in view.world.public_lane_validators().iter() {
        ...
        let entry = stake_map.entry(peer_id).or_insert_with(|| record.total_stake.clone());
...
fn block_sync_history_roster_for_block(
    ...
    let cert = super::status::commit_certificate_history()
        .into_iter()
        .filter(|cert| cert.block_hash == block_hash && cert.height <= block_height)
...
fn block_sync_update_with_roster(
    ...
    let prf_seed = if mode_tag == NPOS_TAG {
        let state_view = state.view();
        Some(super::npos_seed_for_height(&state_view, block_height))
    } else {
        None
    };
    let selection = persisted_roster_for_block(state, kura, consensus_mode, block_height, block_hash)
        .or_else(|| block_sync_history_roster_for_block(state, consensus_mode, block_hash, block_height))
        .or_else(|| {
            let view = state.view();
...
fn validate_commit_certificate_roster(
    ...
    for block_sig in &cert.signatures {
        let idx = usize::try_from(block_sig.index()).map_err(|_| {
            RosterValidationError::SignerIndexOverflow(
                block_sig.index().try_into().unwrap_or(u32::MAX),
            )
        })?;
        ...
        let validator =
            cert.validator_set
                .get(idx)
                .ok_or_else(|| RosterValidationError::SignerOutOfRange {
                    signer: idx_u32,
                    roster_len: roster_len.try_into().unwrap_or(u32::MAX),
                })?;
        block_sig
            .signature()
            .verify_hash(validator.public_key(), cert.block_hash)
            .map_err(|_| RosterValidationError::SignatureInvalid(idx_u32))?;
...
fn validate_checkpoint_roster(
    ...
    let mut seen = BTreeSet::new();
    for block_sig in &checkpoint.signatures {
        let idx = usize::try_from(block_sig.index()).map_err(|_| {
            RosterValidationError::SignerIndexOverflow(
                block_sig.index().try_into().unwrap_or(u32::MAX),
            )
        ...
        block_sig
            .signature()
            .verify_hash(validator.public_key(), checkpoint.block_hash)
            .map_err(|_| RosterValidationError::SignatureInvalid(idx_u32))?;
    }
...
impl Actor {
    fn synthesize_commit_certificate(
        ...
        let height = block.header().height().get();
        let view = u64::from(block.header().view_change_index());
        let topology = signature_topology_for_roster(roster, height, view, mode_tag, prf_seed);
        let state_view = state.view();
        if crate::block::ValidBlock::validate_signatures_subset(block, &topology, &state_view)
            .is_err()
        {
            return None;
        }
        let signers = validated_block_signers(block, &topology, &state_view, mode_tag, prf_seed)
            .ok()?;
    ...
    fn persist_roster_sidecar_for_commit(&self, block: &SignedBlock, roster: &[PeerId]) {
        let prf_seed = if matches!(self.consensus_mode, ConsensusMode::Npos) {
            ...
            Some(super::npos_seed_for_height(
                &view,
        ...
        if let Some(cert) = Self::synthesize_commit_certificate(
            ...
            self.mode_tag(),
            prf_seed,
            self.epoch_for_height(block.header().height().get()),
            self.consensus_mode,
        ) {
    ...
    fn record_rbc_session_roster(
        ...
        self.rbc.session_rosters.entry(key).or_insert(roster);
    }

    fn clear_rbc_session_roster(&mut self, key: &super::rbc_store::SessionKey) {
        self.rbc.session_rosters.remove(key);
    }
    ...
    fn genesis_qc_stub_for_header(
        ...
        let bitmap_len = topology_len.div_ceil(8);
        Some(crate::sumeragi::consensus::Qc {
            phase: qc.phase,
    ...
    fn parent_hash_for(
        ...
        if let Some(inflight) = self.commit.inflight.as_ref()
    ...
    pub(super) fn new(
        ...
        let (consensus_mode, npos_collectors, epoch_manager) = {
            let view = state.view();
            ...
            let manager = if matches!(mode, ConsensusMode::Npos) {
                ...
                let epoch_len = config.epoch_length_blocks.max(1);
                em.set_params(
                    epoch_len,
                    config.vrf_commit_deadline_offset,
                    config.vrf_reveal_deadline_offset,
                );
        ...
        let adaptive_state = AdaptiveObservabilityState::new(
            ...
            super::status::da_gate_missing_availability_total(),
        );
        ...
        let mut actor = Self {
            ...
            network,
            lane_relay,
            merge_committee: MergeCommitteeState::new(),
            da_spool_dir,
            peers_gossiper,
            genesis_network,
            block_count,
            ...
            pending: PendingBlockState {
                ...
                availability_rebroadcast_last_sent: BTreeMap::new(),
            ...
            locked_qc: None,
            vrf: VrfActor::new(),
            broadcast_new_views: BTreeMap::new(),
            view_change_chain: super::view_change::ProofChain::default(),
        ...
        super::status::set_tx_queue_backpressure(actor.propose.backpressure_gate.state());
        ...
        } else if actor.rbc.chunk_store.is_some() {
            actor.update_rbc_store_pressure(super::rbc_store::StorePressure::Normal {
                sessions: actor.rbc.sessions.len(),
                bytes: 0,
            });
        }
        Ok(actor)
    ...
    fn collector_plan_params(&self) -> (usize, u8, Option<[u8; 32]>) {
        match self.consensus_mode {
            ConsensusMode::Permissioned => (
            ...
            ConsensusMode::Npos => self.npos_collectors.map_or_else(
                || {
                    (
                        ...
                        self.epoch_manager.as_ref().map(EpochManager::seed),
                    )
                },
                |cfg| (cfg.k, cfg.redundant_send_r, Some(cfg.seed)),
            ),
        }
    }
    ...
    fn reset_collector_state(&mut self) {
        ...
        let (_, redundant_r, _) = self.collector_plan_params();
        self.propose.collector_redundant_limit = redundant_r.max(1);
        self.propose
            .adaptive_state
            .update_base_collector_limit(self.propose.collector_redundant_limit);
    ...
    fn init_collector_plan(
        ...
        self.propose.collector_plan_subject = Some((height, view));
        let (k, redundant_r, seed_opt) = self.collector_plan_params();
        if k == 0 {
            return;
        }

        let targets =
            deterministic_collectors(topology, self.consensus_mode, k, seed_opt, height, view);
        if targets.is_empty() {
            return;
        }

        self.propose.collector_plan_targets.clone_from(&targets);
        self.propose.collector_plan = Some(CollectorPlan::new(targets));
        self.propose.collector_redundant_limit = redundant_r.max(1);
        self.propose.collector_role_index = {
        ...
        if let Some(plan) = self.propose.collector_plan.as_mut() {
    ...
    fn ensure_collector_plan(
        ...
        if self.propose.collector_plan_subject == Some((height, view)) {
            return;
        }
    ...
    fn note_collector_contact(&mut self, peer: PeerId, redundant: bool) {
        let inserted = self.propose.collectors_contacted.insert(peer.clone());
        if inserted {
            let count = self.propose.collectors_contacted.len() as u64;
        ...
        if redundant && inserted {
            ...
            #[cfg(feature = "telemetry")]
            {
                ...
                if let Some(role_idx) = self.propose.collector_role_index {
    ...
    fn next_redundant_collector(&mut self) -> Option<PeerId> {
        let limit = usize::from(self.propose.collector_redundant_limit.max(1));
        if self.propose.collectors_contacted.len() >= limit {
            if self
                .propose
                .collector_plan
                .as_mut()
        ...
        self.propose.collector_plan.as_mut().and_then(|plan| {
            let next = plan.next();
    ...
    fn apply_adaptive_observability(&mut self, now: Instant) -> bool {
        ...
        match self.propose.adaptive_state.evaluate(
            self.propose.adaptive_cfg,
            metrics,
            &mut self.propose.pacemaker,
            &mut self.propose.collector_redundant_limit,
            now,
        ) {
            AdaptiveAction::Applied => {
                info!(
                    qc_latency_ms = metrics.max_qc_latency_ms,
                    missing_availability = metrics.missing_availability_total,
                    collector_limit = self.propose.collector_redundant_limit,
                    propose_interval_ms = self.propose.pacemaker.propose_interval.as_millis(),
    ...
    pub(super) fn tick(&mut self) -> bool {
        ...
        if queue_ready && !self.should_defer_proposal() && self.commit.inflight.is_none() {
        ...
        let state = self.propose.backpressure_gate.state();
        ...
        if should_attempt_proposal && self.commit.inflight.is_none() {
            let propose_start = Instant::now();
            if self.on_pacemaker_propose_ready(now) {
                progress = true;
            }
    ...
    #[cfg(feature = "telemetry")]
    fn note_new_view_received(&self) {
        self.telemetry.inc_new_view_recv();
    }
    ...
    pub(super) fn on_block_message(&mut self, msg: BlockMessage) -> Result<()> {
        ...
        match msg {
            ...
            BlockMessage::AvailabilityVote(vote) => {
                self.handle_available_vote(vote);
                Ok(())
            }
            ...
            BlockMessage::PrevoteVote(vote) => {
                self.handle_vote(vote.0);
                Ok(())
            }
            BlockMessage::PrevoteQC(qc) => self.handle_qc(qc.0),
            BlockMessage::PrecommitVote(vote) => {
                ...
                info!(
                    ...
                    "processing incoming precommit vote"
                );
                self.handle_vote(vote.0);
                Ok(())
            }
            BlockMessage::PrecommitQC(qc) => self.handle_qc(qc.0),
            BlockMessage::CommitVote(vote) => self.handle_commit_vote(vote),
            ...
            BlockMessage::WitnessAvailAck(_ack) => Ok(()),
            BlockMessage::WitnessAvailQC(qc) => self.handle_witness_qc(qc),
    ...
    fn handle_merge_entry_candidates(&mut self) -> Result<()> {
        ...
        for candidate in candidates {
            ...
            {
                let entry = self.merge_committee.pending.entry(key).or_insert_with(|| {
                    MergeCommitteeEntry {
    ...
    fn try_commit_merge_candidates(
        ...
        for key in ordered_keys {
            let (candidate, qc) = {
                let Some(entry) = self.merge_committee.pending.get(key) else {
                    continue;
                };
            ...
            match self.state.commit_merge_entry(candidate.into_entry(qc)) {
                Ok(_) => {
                    self.merge_committee.pending.remove(key);
                }
                Err(err) => {
    ...
    pub(super) fn on_consensus_control(&mut self, msg: ControlFlow) -> Result<()> {
        ...
        match msg {
            ControlFlow::NewView(frame) => self.handle_new_view(frame),
            ControlFlow::Evidence(ev) => self.handle_evidence(ev),
            ControlFlow::ViewChangeProof(proof) => self.handle_view_change_proof(proof),
        }
    }
    ...
    fn schedule_background(&mut self, request: BackgroundRequest) {
        if should_send_inline_background(&request) {
            self.send_background_immediate(request);
            return;
        }
        let dispatched = {
            #[cfg(feature = "telemetry")]
        ...
        if let Err(request) = dispatched {
            ...
            super::status::inc_bg_post_inline(kind);
            #[cfg(feature = "telemetry")]
            self.telemetry.inc_bg_post_inline(kind);
            self.send_background_immediate(request);
        }
    }
    ...
    fn block_message_kind(msg: &BlockMessage) -> &'static str {
        match msg {
            ...
            BlockMessage::AvailabilityVote(_) => "AvailabilityVote",
            BlockMessage::AvailabilityQC(_) => "AvailabilityQC",
            BlockMessage::PrevoteVote(_) => "PrevoteVote",
            BlockMessage::PrevoteQC(_) => "PrevoteQC",
            BlockMessage::PrecommitVote(_) => "PrecommitVote",
            BlockMessage::PrecommitQC(_) => "PrecommitQC",
            BlockMessage::CommitVote(_) => "CommitVote",
            ...
            BlockMessage::WitnessAvailAck(_) => "WitnessAvailAck",
            BlockMessage::WitnessAvailQC(_) => "WitnessAvailQC",
    ...
    fn consensus_control_kind(msg: &ControlFlow) -> &'static str {
        match msg {
            ControlFlow::NewView(_) => "NewView",
            ControlFlow::Evidence(_) => "Evidence",
            ControlFlow::ViewChangeProof(_) => "ViewChangeProof",
        }
    }
    ...
    fn build_rbc_ready(
        ...
        let prf_seed = if matches!(self.consensus_mode, ConsensusMode::Npos) {
            let view = self.state.view();
            Some(super::npos_seed_for_height(&view, key.1))
        } else {
            None
        };
        let signature_topology =
            topology_for_view(&topology, key.1, key.2, self.mode_tag(), prf_seed);
        ...
        let preimage = rbc_ready_preimage(&self.chain_id, self.mode_tag(), &ready);
    ...
    fn build_rbc_deliver(
        ...
        let preimage = rbc_deliver_preimage(&self.chain_id, self.mode_tag(), &deliver);
        let signature = Signature::new(self.common_config.key_pair.private_key(), &preimage);
    ...
    fn maybe_emit_rbc_ready(&mut self, key: super::rbc_store::SessionKey) -> Result<()> {
        ...
        if let Some(updated) = self.rbc.sessions.get(&key).cloned() {
    ...
    fn rebroadcast_rbc_ready_bundle(
        ...
        if let Some(last) = self.rbc.ready_rebroadcast_last_sent.get(&key) {
        ...
        self.rbc.ready_rebroadcast_last_sent.insert(key, now);
    }
    ...
    fn rebroadcast_rbc_payload_bundle(
        ...
        if let Some(last) = self.rbc.payload_rebroadcast_last_sent.get(&key) {
            if now.saturating_duration_since(*last) < cooldown {
                return;
            }
        }
        ...
        self.rbc.payload_rebroadcast_last_sent.insert(key, now);
    }
    ...
    fn force_view_change_if_idle(&mut self, now: Instant) -> bool {
        ...
        let proposal_seen = self.proposals_seen.contains(&(height, current_view));
        let timeout = idle_view_timeout(
            proposal_seen,
            self.commit_quorum_timeout(),
            self.propose.pacemaker.propose_interval,
        );
    ...
    fn empty_child_fallback(&self, now: Instant) -> Option<EmptyChildContext> {
        ...
        if lock.phase != crate::sumeragi::consensus::Phase::Precommit {
            return None;
        }
        ...
        if !empty_child_backoff_allows(
            self.propose.last_empty_child_attempt,
            parent_payload_hash,
            now,
            backoff,
        ) {
    ...
    fn record_empty_child_attempt(&mut self, parent_payload_hash: Hash, now: Instant) {
        self.propose.last_empty_child_attempt = Some((parent_payload_hash, now));
    }
    ...
    fn purge_rbc_state(
        ...
        self.rbc.sessions.remove(&key);
        self.clear_rbc_session_roster(&key);
        self.rbc.status_handle.remove(&key);
        self.rbc.pending.remove(&key);
        self.rbc.payload_rebroadcast_last_sent.remove(&key);
        self.rbc.ready_rebroadcast_last_sent.remove(&key);
        self.rbc.persisted_full_sessions.remove(&key);
    ...
    fn trigger_view_change_with_cause(&mut self, height: u64, view: u64, cause: ViewChangeCause) {
        ...
        self.propose.forced_view_after_timeout = Some((height, next_view));
        ...
        if let Some(mut highest_qc) = self.highest_qc.or(committed_qc) {
            if highest_qc.phase != crate::sumeragi::consensus::Phase::Precommit {
                if let Some(committed) = committed_qc {
                    ...
                    debug!(
                        ...
                        "skipping local NEW_VIEW record: highest QC is not precommit"
                    );
                }
            }
            let can_record = highest_qc.phase == crate::sumeragi::consensus::Phase::Precommit
                && height == highest_qc.height.saturating_add(1);
            if let Some(local_idx) = {
                let view_snapshot = self.state.view();
                self.local_validator_index(&view_snapshot)
            } {
                if can_record {
                    self.propose
                        .new_view_tracker
                        .record(height, next_view, local_idx, highest_qc);
                } else {
                    debug!(
                        height,
                        view = next_view,
                        ...
                        phase = ?highest_qc.phase,
                        "skipping local NEW_VIEW record: highest QC does not match target height"
                    );
                }
            }
            if can_record {
                self.maybe_broadcast_new_view(highest_qc, Some(height), Some(next_view));
            }
        }
        if let Err(err) = self.record_and_broadcast_view_change_proof(height, next_view) {
            warn!(
                ?err,
                height,
                view = next_view,
                "failed to broadcast view-change proof after quorum timeout"
            );
        }
...
impl PipelinePhase {
    ...
    fn telemetry_label(self) -> &'static str {
        match self {
            ...
            Self::CollectPrevote => "collect_prevote",
            Self::CollectPrecommit => "collect_precommit",
...
impl PhaseRecordFlags {
    ...
    const COLLECT_PREVOTE: u8 = 1 << 2;
    const COLLECT_PRECOMMIT: u8 = 1 << 3;
    ...
    fn mask(phase: PipelinePhase) -> u8 {
        match phase {
            ...
            PipelinePhase::CollectPrevote => Self::COLLECT_PREVOTE,
            PipelinePhase::CollectPrecommit => Self::COLLECT_PRECOMMIT,
...
impl PhaseEma {
    ...
    fn update(&mut self, phase: PipelinePhase, sample_ms: f64) -> f64 {
        match phase {
            ...
            PipelinePhase::CollectPrevote => self.collect_prevote.update(sample_ms),
            PipelinePhase::CollectPrecommit => self.collect_precommit.update(sample_ms),
    ...
    fn current(&self, phase: PipelinePhase) -> f64 {
        match phase {
            ...
            PipelinePhase::CollectPrevote => self.collect_prevote.current(),
            PipelinePhase::CollectPrecommit => self.collect_precommit.current(),
...
impl RbcSession {
    ...
    pub(super) fn from_persisted_unchecked(
        ...
        fn build_chunk(bytes: Vec<u8>, sender: Option<u32>) -> RbcChunkEntry {
        ...
        for chunk in chunks {
            ...
            data[idx] = Some(build_chunk(chunk.bytes, None));
        }
    ...
    fn note_chunk(&mut self, idx: u32, bytes: Vec<u8>, sender: Option<u32>) {
