...
include!("v2_lifecycle_launch_ready_proposal_sign_test_fixtures.rs");
...
fn live_terminal_height_authenticates_after_closed_drain_without_a_successor() {
    ...
    // An explicit shutdown request may stop earlier in the rollover branch.
    // The terminal-height path itself must authenticate before its own stop.
