...
impl SumeragiV2Adapter {
    ...
    fn release_unrecorded_producer(
        ...
        if self.ensure_canonical_reclaimed_producer_state_after_decision()? {
            // This token was reserved before the macro-step made its Decision
            // durable. Reclamation is now the authoritative release and its
            // canonical empty epoch must not be undone by stale process state.
            return Ok(());
        }
        self.pending_producer_handoffs.remove(&token.address);
        match token.change {
...
mod tests {
    ...
    fn durable_decision_release_does_not_restore_stale_process_only_predecessor() {
        ...
        let decision = wire::QuorumCertificate {
            ...
            signers: vec![0, 1, 2],
            aggregate_signature: vec![0x4C; 96],
        };
        adapter
