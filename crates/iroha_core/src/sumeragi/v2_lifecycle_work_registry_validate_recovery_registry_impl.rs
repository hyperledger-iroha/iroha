impl ConcreteLifecycleWorkRegistry {
    ...
    /// Consume one advanced Fetch/Store plus live Validate recovered Decision prefix.
    #[allow(clippy::result_large_err)]
    pub(super) fn install_recovered_wal_decision_validate<'registry>(
        &'registry mut self,
        verified: &VerifiedHeightContext,
        ledger_store: &super::ledger::LifecycleLedgerStoreV1,
        ledger: &super::ledger::LifecycleLedgerV1,
        ...
        RecoveredWalDecisionFetchInstallError,
    > {
        let invalid = |fetch, store, validate| RecoveredWalDecisionFetchInstallError {
            failure: RecoveredWalDecisionFetchInstallFailure::ValidateProjection {
                _fetch: fetch,
                _store: store,
                _validate: validate,
            },
        ...
        if !self.entries.is_empty()
            || !fetch.is_exact(verified)
            || !store.is_exact(verified)
            || !fetch.owns_decision_validate_projection(verified, &store, &validate)
            || !ledger_store.load().is_ok_and(|opened| opened == *ledger)
        {
            return Err(invalid(fetch, store, validate));
        }
        let Some(validate_record) = validate.live_validate_record(ledger) else {
            return Err(invalid(fetch, store, validate));
        };
        let owner = validate_record.owner();
        let validate_ordinal = validate_record.ordinal();
        let Some(store_record) = ledger.records().iter().find(|record| {
            record.owner() == owner
                && store.exactly_matches_advanced_validate_parent(record, owner, validate_ordinal)
        }) else {
            return Err(invalid(fetch, store, validate));
        };
        let Some(fetch_record) = ledger
            .records()
            .iter()
            .find(|record| fetch.names_record(record))
        else {
            return Err(invalid(fetch, store, validate));
        };
        if fetch_record.owner() != owner
            || !fetch.exactly_matches_advanced_apply_parent(fetch_record, store_record.ordinal())
            || !validate.exactly_matches_live_records(owner, store_record, validate_record)
        {
            return Err(invalid(fetch, store, validate));
        }
        let slot = PhysicalSlotId::for_capacity(CapacityClass::Effect, 0);
        let Some(address) = ConcreteWorkAddress::new(owner, validate_ordinal, slot) else {
            return Err(invalid(fetch, store, validate));
        };
        let Some(validate_seal) = validate.prepare_installed_seal(verified, address) else {
            return Err(invalid(fetch, store, validate));
        };
        ...
        let (
            ...
            replay_evidence,
            validate_seal,
        ) = validate.consume_for_registry(
            RecoveredDecisionValidateRegistryProjectionPermitV1::new(),
            validate_seal,
        );
        ...
        if !carrier.matches_recovered_decision_installed_seal(digest, &validate_seal)
            || carrier
                .project_candidate(verified)
                .ok()
                .as_ref()
                .is_none_or(|candidate| !validate_seal.matches_projected_candidate(candidate))
        {
            return Err(RecoveredWalDecisionFetchInstallError {
                failure: RecoveredWalDecisionFetchInstallFailure::ValidateCarrier {
                    _fetch: fetch,
                    _store: store,
                    _carrier: carrier,
                    _seal: validate_seal,
        ...
        Ok(InstalledRecoveredWalDecisionFetchRegistryCut {
            ...
            address,
            digest,
            validate_seal: Some(validate_seal),
        })
    }
    ...
    /// Preflight an ordinary body census beside the exact recovered-Decision
    /// Validate carrier installed from its sealed WAL/body projection.
    pub(super) fn preflights_recovered_body_pipeline_alongside_decision_validate(
        &self,
        validate_seal: &RecoveredDecisionValidateInstalledSealV1,
        works: &[(ConcreteWorkAddress, Option<LifecycleDigest>)],
    ) -> bool {
        let mut matching = self.entries.iter().filter_map(|(&address, work)| {
            let ConcreteLifecycleWorkKind::DurableValidateBody(validate) = &work.kind else {
                return None;
            };
            (work.validates_at(address)
                && validate.matches_recovered_decision_installed_seal(work.digest, validate_seal))
            .then_some(address)
        });
        let Some(validate_address) = matching.next() else {
            return false;
        };
        if matching.next().is_some() || self.entries.len() != 1 {
            return false;
        }
        let mut addresses = std::collections::BTreeSet::new();
        let mut owners = std::collections::BTreeSet::new();
        works.iter().all(|(address, digest)| {
            digest.is_some()
                && address.owner != validate_address.owner
                && !self.entries.contains_key(address)
                && addresses.insert(*address)
                && owners.insert(address.owner)
        })
...
impl ConcreteLifecycleWorkRegistry {
    ...
    pub(super) fn prepare_durable_store_execution(
        ...
        let (candidate, expected_payload, origin) = match &work.kind {
            ConcreteLifecycleWorkKind::DurableStoreBody(store) => {
            ...
            ConcreteLifecycleWorkKind::DurableRecoveredDecisionStore(store) => {
                ...
                let expected_payload =
                    durable_validate_body_payload(store.store.durable_body_receipt())
                        .ok_or(DurableStoreExecutionError::InvalidProjection)?;
                let origin = store
                    .project_validate_execution(verified)
                    .map(DurableStoreExecutionOriginV1::RecoveredDecision)
                    .ok_or(DurableStoreExecutionError::InvalidProjection)?;
                (candidate, expected_payload, origin)
            }
            _ => return Err(DurableStoreExecutionError::WrongWorkKind),
        };
...
impl super::concrete_admission::LifecycleWorkRegistryHolder {
    /// Persist and reinstall one genuine recovered-Decision Validate crash cut.
    pub(in crate::sumeragi) fn install_recovered_decision_validate_for_test(
        &mut self,
        verified: &VerifiedHeightContext,
        ...
        let Some(ledger) =
            fetch.exact_decision_validate_ledger_for_test(verified, &store, &validate)
        else {
            return false;
        };
        let Ok((ledger_store, empty)) =
            super::ledger::LifecycleLedgerStoreV1::open(ledger_root, ledger.context())
        else {
            return false;
        };
        if ledger_store
            .persist_exact_successor(&empty, &ledger)
            .is_err()
            || ledger
        ...
        let Ok(body_pipeline) =
            ledger.authenticate_durable_certified_body_pipeline_startup(verified, body_store)
        else {
            return false;
        };
        let Ok((body_pipeline, _adapter_startup)) =
            body_pipeline.replay_adapter_startup(adapter_startup)
        else {
            return false;
        };
        let registry = self.registry_for_test_mut();
        let Ok(mut installed) = registry.install_recovered_wal_decision_validate(
            verified,
            &ledger_store,
            &ledger,
            fetch,
        ...
        if installed.install_body_pipeline(body_pipeline).is_err() {
            return false;
        }
        let exact = installed
            .exact_decision_validate_work(&ledger_store)
            .is_some();
        drop(installed);
        exact
            && self
