impl ConcreteLifecycleWorkRegistry {
    ...
    /// Attest one exact Ready ordinary certified-Fetch completion.
    pub(super) fn attest_ready_certified_fetch(
        &self,
        coordinator: &LifecycleCoordinator,
        ordinal: u128,
    ) -> Result<ReadyCertifiedFetchAttestationV1, ReadyCertifiedFetchAttestationErrorV1> {
        if coordinator.fault.is_some() || coordinator.active_lease.is_some() {
            return Err(ReadyCertifiedFetchAttestationErrorV1::InvalidCoordinatorIndex);
        }
        let Some(record) = coordinator.records.get(&ordinal) else {
            return Err(ReadyCertifiedFetchAttestationErrorV1::InvalidCoordinatorIndex);
        };
        if record.ordinal != ordinal
            ...
            || coordinator
                .owner_index
                .values()
                .filter(|candidate| **candidate == record.owner)
                .count()
                != 1
        ...
        let address = ConcreteWorkAddress::new(record.owner, ordinal, slot)
            .ok_or(ReadyCertifiedFetchAttestationErrorV1::InvalidCoordinatorIndex)?;
        let work = self
            .entries
            .get(&address)
            .ok_or(ReadyCertifiedFetchAttestationErrorV1::InvalidRegistryCarrier)?;
        if work.digest != digest
            ...
            || self
                ...
                .keys()
                .filter(|candidate| {
                    candidate.owner == address.owner && candidate.ordinal == address.ordinal
                })
                .count()
                != 1
        {
            return Err(ReadyCertifiedFetchAttestationErrorV1::InvalidRegistryCarrier);
        }
        if !matches!(
            &work.kind,
            ConcreteLifecycleWorkKind::CertifiedFetchCompletion(_)
        ) {
            return Err(ReadyCertifiedFetchAttestationErrorV1::WrongWorkKind);
        }
        Ok(ReadyCertifiedFetchAttestationV1 { address, digest })
    }

    /// Attest one exact Ready ordinary durable Store carrier.
    pub(super) fn attest_ready_durable_store(
        &self,
        coordinator: &LifecycleCoordinator,
        ordinal: u128,
    ) -> Result<ReadyDurableStoreAttestationV1, ReadyDurableStoreAttestationErrorV1> {
        if coordinator.fault.is_some() || coordinator.active_lease.is_some() {
            return Err(ReadyDurableStoreAttestationErrorV1::InvalidCoordinatorIndex);
        }
        let Some(record) = coordinator.records.get(&ordinal) else {
            return Err(ReadyDurableStoreAttestationErrorV1::InvalidCoordinatorIndex);
        };
        if record.ordinal != ordinal
            ...
            || record.key.phase() != LifecyclePhase::Store
            || record.stage.kind() != LifecycleStageKind::StoreBody
            || record.stage.predecessor_scope() != PredecessorScope::Independent
            || !coordinator.ready_index.contains(&ordinal)
            || coordinator.key_index.get(&record.key) != Some(&ordinal)
            || coordinator.owner_index.get(&record.owner.causal_root()) != Some(&record.owner)
        ...
        let address = ConcreteWorkAddress::new(record.owner, ordinal, slot)
            .ok_or(ReadyDurableStoreAttestationErrorV1::InvalidCoordinatorIndex)?;
        let work = self
            .entries
            .get(&address)
            .ok_or(ReadyDurableStoreAttestationErrorV1::InvalidRegistryCarrier)?;
        if work.digest != digest
            || !work.validates_at(address)
            || self
                .entries
                ...
                != 1
        {
            return Err(ReadyDurableStoreAttestationErrorV1::InvalidRegistryCarrier);
        }
        if !matches!(&work.kind, ConcreteLifecycleWorkKind::DurableStoreBody(_)) {
            return Err(ReadyDurableStoreAttestationErrorV1::WrongWorkKind);
        }
        Ok(ReadyDurableStoreAttestationV1 { address, digest })
    }
        ...
        let work = ConcreteLifecycleWork {
            ...
            kind: ConcreteLifecycleWorkKind::DurableRecoveredDecisionApply(
                DurableRecoveredDecisionApplyWork {
                    carrier: DurableDecisionApplyRegistryCarrierV1::Recovered(carrier),
                    address,
                    dispatch_key: None,
                },
            ),
    ...
    /// Reclassify the exact claimed Validate address without minting a second
    /// scheduler attestation.
    pub(super) fn claimed_validate_requires_io_dispatch(
        &self,
        lease: &TurnLease,
    ) -> Result<bool, ReadyValidateCarrierError> {
        if lease.work_class() != LifecycleWorkClass::Validate || lease.physical_slots().len() != 1 {
            return Err(ReadyValidateCarrierError::InvalidCarrier);
        }
        ...
        let address = ConcreteWorkAddress::new(lease.owner(), lease.ordinal(), slot)
            .ok_or(ReadyValidateCarrierError::InvalidCarrier)?;
        self.classify_ready_validate_carrier(address, digest)
            .map(ReadyValidateCarrierSeal::requires_io_dispatch)
    }
...
impl<'a> PreparedCertifiedFetchExecution<'a> {
    ...
    /// for exact body-catalog equality and canonical reload.
