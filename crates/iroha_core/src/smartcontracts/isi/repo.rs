...
mod tests {
    ...
    #[test]
    #[allow(clippy::too_many_lines)]
    fn repo_substitution_matrix_cannot_change_fixed_settlement_terms() {
        ...
        let reverse = ReverseRepoIsi::new(agreement_id.clone());
        reverse
            .execute(&ALICE_ID, &mut stx)
            .expect("settlement derives the recorded collateral");
        let alice_alt_asset = AssetId::new(alt_collateral_id.clone(), ALICE_ID.clone());
        assert!(
            stx.world.assets.get(&alice_alt_asset).is_none(),
            "repo configuration cannot inject a substitute into an ID-only settlement"
        );
    }

    #[test]
    #[allow(clippy::too_many_lines)]
    fn repo_substitution_allowlist_cannot_enable_caller_selected_asset() {
        let (mut state, agreement_id, cash_def_id, collateral_def_id, alt_collateral_id) =
            setup_state_with_dual_collateral();
        ...
        let alice_alt_asset = AssetId::new(alt_collateral_id.clone(), ALICE_ID.clone());
        assert!(
            view.world.assets().get(&alice_alt_asset).is_none(),
            "even an allowlisted substitute is not representable at maturity"
        );
    }
