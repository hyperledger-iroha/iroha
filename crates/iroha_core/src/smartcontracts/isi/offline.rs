...
pub(crate) use kagemusha_release_lifecycle::{
    ...
    signed_lifecycle_entrypoint_context,
};
...
pub mod isi {
    ...
    fn authenticate_registered_kagemusha_v2_device_against_policy(
        ...
        if state.admission_policy_hash == release_policy_hash {
            ...
            let admission_policy = effective_offline_device_attestation_policy(state_transaction)?;
            validate_offline_attestation_policy(
                &admission_policy,
                state_transaction.block_unix_timestamp_ms(),
            )?;
            if state.admission_policy_hash
                != canonical_offline_device_attestation_policy_hash(&admission_policy)?
            {
                return Err(labeled_invariant(
                    "attestation_policy_changed",
