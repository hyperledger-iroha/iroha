...
pub mod isi {
    ...
    fn validate_p256_uncompressed_public_key(public_key: &[u8]) -> Result<(), Error> {
        ...
        reject_invalid_attestation!(
            p256_public_key_has_zero_coordinate_material(public_key),
    ...
    fn decode_cbor_value_exact(
        ...
        let value: ciborium::value::Value = ciborium::de::from_reader(&mut cursor)
            .map_err(|_| invalid_attestation(parse_message.to_owned()))?;
        reject_invalid_attestation!(cursor.position() != input.len() as u64, trailing_message);
        Ok(value)
    }
        ...
        let argument_bytes = match additional {
            ...
            _ => {
                return Err(invalid_attestation(format!(
                    "iOS App Attest {source} extensions must use definite valid CBOR"
                ))
                .into());
            }
        };
