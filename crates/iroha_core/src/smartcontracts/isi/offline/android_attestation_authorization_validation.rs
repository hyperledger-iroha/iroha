...
fn validate_android_root_of_trust(input: &[u8]) -> Result<AndroidRootOfTrust, Error> {
    ...
    if verified_boot_state != OFFLINE_ATTESTATION_ANDROID_VERIFIED_BOOT_STATE_VERIFIED {
        return Err(labeled_invariant(
            "invalid_attestation",
    ...
    let verified_boot_hash = verified_boot_hash
        .try_into()
        .expect("verified-boot hash length was checked");
