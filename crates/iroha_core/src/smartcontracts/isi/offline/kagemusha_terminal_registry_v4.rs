...
use crate::zk::{
    kagemusha_artifact_v4::{
        KagemushaValidatedArtifactPayloadV4, kagemusha_artifact_descriptor_v4,
...
pub(crate) struct ResolvedKagemushaTerminalVerifierV4 {
    ...
    step_eq_verifying_key: Vec<u8>,
    step_ep_verifying_key: Vec<u8>,
}
    ...
    fn activation_record(
        ...
        let (curve, authenticated_vk) = match parity {
            KagemushaPastaCycleParityV1::StepEq => (
                ...
                self.resolved.step_eq_verifying_key(),
            ),
            KagemushaPastaCycleParityV1::StepEp => (
                STEP_EP_VERIFIER_CURVE_V4,
                self.resolved.step_ep_verifying_key(),
            ),
        };
...
fn load_release_directory(
    ...
    let mut retained_step_eq_verifying_key = None;
    let mut retained_step_ep_verifying_key = None;
    let verifier = Arc::new(
        KagemushaPastaCycleOpaqueVerifierV4::from_authenticated_artifact_loader(
            &authenticated,
            |parity, kind| {
                let payload = read_release_role(directory, &authenticated, parity, kind)?;
                match kind {
                    KagemushaPastaCycleArtifactKindV4::ParamsIpa => {
                        let profile = manifest
                            ...
                            .ok_or_else(|| "Kagemusha V4 parameter profile is absent".to_owned())?;
                        let label = match parity {
                            KagemushaPastaCycleParityV1::StepEq => "Eq",
                            KagemushaPastaCycleParityV1::StepEp => "Ep",
                        };
                    ...
                    KagemushaPastaCycleArtifactKindV4::VerifyingKey => match parity {
                        KagemushaPastaCycleParityV1::StepEq => {
                            retained_step_eq_verifying_key = Some(payload.payload().to_vec());
                        }
                        KagemushaPastaCycleParityV1::StepEp => {
                            retained_step_ep_verifying_key = Some(payload.payload().to_vec());
                        }
                    },
                    KagemushaPastaCycleArtifactKindV4::ProvingKey
                    | KagemushaPastaCycleArtifactKindV4::BootstrapWitness => {}
    ...
    let retained_step_eq_verifying_key = retained_step_eq_verifying_key
        .ok_or_else(|| "Kagemusha V4 Eq verifier key was not loaded".to_owned())?;
    let retained_step_ep_verifying_key = retained_step_ep_verifying_key
        .ok_or_else(|| "Kagemusha V4 Ep verifier key was not loaded".to_owned())?;
    // `verifier` owns parsed parameters and verifier keys. Each raw role was
    // dropped before the next role was opened; only exact VK bytes required to
    // construct governed activation records remain resident.
    let resolved = ResolvedKagemushaTerminalVerifierV4 {
        release: authenticated,
        step_eq_verifying_key: retained_step_eq_verifying_key,
        step_ep_verifying_key: retained_step_ep_verifying_key,
    };
...
impl ResolvedKagemushaTerminalVerifierV4 {
    ...
    fn step_eq_verifying_key(&self) -> &[u8] {
        &self.step_eq_verifying_key
    }

    fn step_ep_verifying_key(&self) -> &[u8] {
