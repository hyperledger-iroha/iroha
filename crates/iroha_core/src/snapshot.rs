    ...
    F: Fn(&mut State) -> Result<(), TryReadError>,
{
    ...
    let input = std::str::from_utf8(bytes)
        .map_err(|_| TryReadError::Serialization(json::Error::InvalidUtf8))?;
    // The governed registry is checked before constructing any live State;
    // only this bounded field is materialized as a temporary JSON value.
...
mod tests {
    ...
    fn store_complete_snapshot_commit_evidence_for_blocks(
        ...
        let artifacts =
            signed_complete_wire_finality_for_snapshot_blocks(&state.network_id, blocks);
