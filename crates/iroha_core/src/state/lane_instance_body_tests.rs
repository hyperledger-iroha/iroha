...
state_test! { sync native_lane_instance_closed_worker_completion_restores_store_without_productive_ack
    ...
    assert!(matches!(owner.finish_body_job(completed,state,&closed).unwrap(),LaneBodyProgress::Retired));
    assert_eq!(owner.retirement_count(), 1);
    let retirement = owner.take_retirement().expect("real returned body stays owned");
    ...
    assert!(owner.take_retirement().is_none());
...
state_test! { sync native_lane_instance_rejects_signed_wrong_input_before_body_admission_and_keeps_timer
    ...
    let LaneBodyProgress::RejectedProposal=run_one_native_lane_body_job_for_test(&mut receiver,state,&observed) else { panic!("deterministic mismatch rejected before core body admission") };
    let retirement = receiver.take_retirement().expect("exact signed rejected proposal stays owned");
    assert_eq!(retirement.proposal(),Some(&proposal));
    assert!(retirement.rejection().is_some());
    assert!(receiver.take_retirement().is_none());
