...
use crate::{
    kura::{KuraPublicationLease, KuraWsvCheckpointReceipt},
    query::{
    ...
    DecisionBoundCarrierJournals<Admission, DetachedCarrierComponents, KuraWsvCheckpointReceipt>
{
    ...
    ///
    /// The caller joins the original target Kura and retains its publication
    /// lease across all durable phases. Each capture authenticates through that
    /// held boundary and probes its archive writer without waiting on readers.
    /// Refusal preserves the complete owner, partial immutable archive progress
    /// and all capture reservations; the caller releases the joint lease.
    /// Retrying invokes exact replay checks on any already completed capture.
    pub(crate) fn publish_archives(
        &mut self,
