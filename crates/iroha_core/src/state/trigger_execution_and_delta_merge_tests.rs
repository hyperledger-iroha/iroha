...
fn delta_merge_execute_trigger_by_call_executes_and_chains() {
    ...
    let added_count = events
        ...
        .filter(|e| {
            if let EventBox::Data(ev) = e
                && let data_pre::DataEvent::Asset(data_pre::AssetEvent::Added(ch)) = ev.as_ref()
            {
...
fn detached_execute_called_trigger_failure_rolls_back_state() {
    ...
    });
    {
        ...
        let fail_isi =
            Unregister::domain(DomainId::try_new("missing-trigger-domain", "universal").unwrap());
        let trigger = Trigger::new(
            trigger_id.clone(),
            Action::new(
                [
                    InstructionBox::from(create_asset),
                    InstructionBox::from(fail_isi),
                ],
                Repeats::Indefinitely,
                ALICE_ID.clone(),
                ExecuteTriggerEventFilter::new()
    ...
    let mut delta = DetachedStateTransactionDelta::default();
    delta.execute_trigger_by_call(ExecuteTriggerEvent {
        trigger_id,
        authority: ALICE_ID.clone(),
        args: Json::default(),
    });
    ...
    });
    {
        let mut state_block = state.block(block.as_ref().header());
        assert!(
            delta.merge_into(&mut state_block, &ALICE_ID).is_err(),
