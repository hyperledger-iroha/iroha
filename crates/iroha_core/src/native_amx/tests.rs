...
fn body(phase: NativeAmxPhase) -> NativeAmxAttestationBodyV2 {
    let mut body = NativeAmxAttestationBodyV2 {
        ...
        tx_entrypoint_hash: iroha_crypto::HashOf::<TransactionEntrypoint>::from_untyped_unchecked(
...
fn set_source(body: &mut NativeAmxAttestationBodyV2, byte: u8) {
    ...
    body.tx_entrypoint_hash =
        HashOf::<TransactionEntrypoint>::from_untyped_unchecked(Hash::prehashed(body.source_id));
}
#[cfg(unix)]
...
fn signing_guard_rejects_unsupported_pre_release_journals() {
        ...
        .enumerate()
    {
        ...
        let body = body(NativeAmxPhase::Prepare);
        let seed = 0x6F_u8.saturating_add(u8::try_from(index).expect("directory index fits u8"));
...
fn signing_guard_rejects_anchor_deletion_or_wrong_v5_anchor_version() {
    ...
    let version_anchor_bytes = fs::read(&version_anchor_path).expect("read canonical V5 anchor");
    ...
    fs::write(
        &version_anchor_path,
        norito::encode_canonical(&wrong_version_anchor).expect("encode wrong-version V5 anchor"),
    )
...
fn signing_guard_rejects_wrong_version_noncanonical_and_hardlinked_records() {
    ...
    fs::write(
        &path,
        norito::encode_canonical(&wrong_version_record).expect("encode wrong-version V5 record"),
    )
...
fn signing_guard_rejects_multiple_unpublished_tails() {
    ...
    let second = NativeAmxSigningRecordV2::from_body(2, first.record_hash, &second_body, &signer)
        .expect("second tail");
...
fn signing_guard_latches_poison_after_directory_or_lock_replacement() {
    for replace_directory in [false, true] {
        let root = tempfile::tempdir().expect("temp dir");
        let (_keypair, signer) = signing_guard_signer(if replace_directory { 0x8C } else { 0x8B });
        let body = body(NativeAmxPhase::Prepare);
        let guard = open_guard(root.path(), &body, signer, 8);
...
fn signing_guard_rejects_malformed_context_and_view_bodies() {
    ...
    entrypoint_drift.tx_entrypoint_hash = HashOf::<TransactionEntrypoint>::from_untyped_unchecked(
        Hash::new(b"conflicting-signing-guard-entrypoint"),
    );
    assert_eq!(
...
fn aligned_pops(validator_set: &[PeerId], keypairs: &[KeyPair]) -> Vec<Vec<u8>> {
    validator_set
        ...
        .map(|validator| {
            ...
            iroha_crypto::bls_normal_pop_prove(keypair.private_key()).expect("prove fixture PoP")
        })
        .collect()
}
fn full_plan_request(
    ...
    let coordinator = RoutingDecision::new(body.coordinator_lane_id, body.coordinator_dataspace_id);
    let participant = RoutingDecision::new(body.participant_lane_id, body.participant_dataspace_id);
...
fn full_plan_request_binds_canonical_routes_and_coordinator_proposal() {
    ...
    coordinator_participates.participant_settlement = coordinator_participates
        .body
        .computed_grouped_participant_settlement(None, &[coordinator_participates.body.source_id])
        .expect("single-source test fixture settlement is valid");
    coordinator_participates
        .body
