...
pub(crate) const PQ_MASP_ENGINE_DESCRIPTOR_V1: &[u8] = b"pq-masp-stark-v1:native-rust:first-release:inputs=1..2:outputs=1..2:values=u128-checked:membership=sha256-depth32-exact-ledger-domains:nullifier=stable-note-secret+rho+commitment+pool:ownership=statement-mldsa65-key-digest:authorization=outer-mldsa65-over-canonical-statement-digest+inner-proof-digest:producer=typed-redacted-witness+relation-and-key-preflight+rand0.9-trycrypto-fixed64-reservoir-zeroize-poison-error-or-unwind-policy-v1+block1-stark-replay+block2-independent-health-sha256-authorization-hedge+block3plus-stark+self-verify:encryption=mlkem768+xchacha20poly1305+internal-fixed64-health-sha256-seed:successor=validator-derived-only:legacy=unrepresentable";
