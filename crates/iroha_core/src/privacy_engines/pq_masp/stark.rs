...
pub(crate) const PQ_MASP_STARK_PROFILE_DESCRIPTOR_V1: &[u8] = b"pq-masp-stark-v0:relation=proof-managed-note:wire=PQA1-outer+PQS1-inner-v1:trace=2^14:base=556:profile-aux=1:profile-fixed=122:profile-constraints=1372:constraint-degree=4:max-inner-proof=9431915:sha256-wide-air:public-digest=verifier-fixed-terminal-bytes:tree-depth=32:authorization=ML-DSA-65:encryption=ML-KEM-768+XChaCha20Poly1305:value=u128-checked:fee=separate:legacy=unrepresentable:governance=typed-lifecycle";
...
pub(crate) const PQ_MASP_STARK_PROFILE_DIGEST_V1: [u8; 32] = [
    0x5d, 0xee, 0x5e, 0x9b, 0x46, 0x16, 0x08, 0x32, 0x95, 0x32, 0xec, 0x47, 0x51, 0x36, 0xd7, 0x36,
    0xde, 0x52, 0x8f, 0xf6, 0x42, 0xe1, 0x0b, 0x97, 0x7f, 0x8f, 0x0a, 0x52, 0x49, 0x96, 0x5c, 0xdb,
];

/// SHA-256 of the deterministic full-domain proof generated by the canonical
/// test fixture rebound to the deterministic
/// `pq-masp-full-facade-v1` ML-DSA key, with
/// `StdRng::from_seed([0xB7; 32])`.
pub(crate) const PQ_MASP_STARK_KAT_PROOF_SHA256_V1: [u8; 32] = [
    0x17, 0x36, 0x4d, 0x69, 0x92, 0x74, 0x2d, 0xcf, 0xcf, 0xf3, 0x0c, 0x9b, 0x22, 0x05, 0x4e, 0x83,
    0xc9, 0xb1, 0x34, 0x16, 0x0e, 0xa9, 0x6c, 0xff, 0x80, 0xb7, 0xad, 0x9d, 0x73, 0x93, 0x35, 0x0e,
];
...
/// Statement-only adapter used by both the prover and verifier.
pub(super) struct PqMaspStarkAdapterV1<'a> {
    statement: &'a PqMaspStarkStatementV1,
}

impl<'a> PqMaspStarkAdapterV1<'a> {
    pub(super) const fn new(statement: &'a PqMaspStarkStatementV1) -> Self {
        Self { statement }
    }
}
...
impl ProofManagedNoteStarkAdapterV1 for PqMaspStarkAdapterV1<'_> {
    ...
    fn public_input_digest_v1(&self) -> Result<[u8; 32], ProofManagedNoteStarkErrorV1> {
        ...
        sha256_frame_v1(b"pq-masp-stark-public-input-v1", &[&encoded])
...
pub(super) fn prove_pq_masp_stark_v1_with_rng<R: TryRngCore>(
    ...
    prove_proof_managed_note_stark_v1_with_rng(
        &PqMaspStarkAdapterV1::new(statement),
        &base_columns,
        rng,
    )
}
...
pub(super) fn prove_pq_masp_stark_v1(
    ...
    let base_columns = compile_pq_masp_prover_columns_v1(statement, witness)?;
    prove_proof_managed_note_stark_v1(&PqMaspStarkAdapterV1::new(statement), &base_columns)
}

/// Verify the exact PQ-MASP proof wire against the public statement.
pub(crate) fn verify_pq_masp_stark_v1(
    statement: &PqMaspStarkStatementV1,
    proof_bytes: &[u8],
) -> Result<(), ProofManagedNoteStarkErrorV1> {
    verify_proof_managed_note_stark_v1(&PqMaspStarkAdapterV1::new(statement), proof_bytes)
}
...
mod tests {
    ...
    use crate::privacy_engines::{
        pq_masp::{
            ...
            wire::{PqMaspWireErrorV1, decode_pq_masp_authorization_proof_v1},
        },
        proof_managed_note_stark::{
    ...
    fn prepared_columns(
        ...
        let adapter = PqMaspStarkAdapterV1::new(statement);
    ...
    fn compiled_profile_count_digest_and_outer_wire_budget_are_exact() {
        ...
        let expected_descriptor = format!(
            "pq-masp-stark-v0:relation=proof-managed-note:wire=PQA1-outer+PQS1-inner-v1:trace=2^{}:base={}:profile-aux={}:profile-fixed={}:profile-constraints={}:constraint-degree={}:max-inner-proof={}:sha256-wide-air:public-digest=verifier-fixed-terminal-bytes:tree-depth=32:authorization=ML-DSA-65:encryption=ML-KEM-768+XChaCha20Poly1305:value=u128-checked:fee=separate:legacy=unrepresentable:governance=typed-lifecycle",
            PQ_MASP_TRACE_LOG2_V1,
            PQ_MASP_BASE_WIDTH_V1,
            PQ_MASP_PROFILE_AUX_WIDTH_V1,
            PQ_MASP_PROFILE_FIXED_WIDTH_V1,
    ...
    fn full_domain_authorized_facade_roundtrip_and_adversarial_wires_fail_closed() {
        ...
        super::super::verify_pq_masp_v1(&statement, &authorized_proof)
        ...
        verify_pq_masp_stark_v1(&statement, &proof).expect("full-domain PQ-MASP verification");
        let proof_digest: [u8; 32] = Sha256::digest(&proof).into();
        assert_eq!(proof_digest, PQ_MASP_STARK_KAT_PROOF_SHA256_V1);

        assert!(
            verify_pq_masp_stark_v1(&statement, &vec![0; proof.len()]).is_err(),
        ...
        for wire in wires {
            assert!(
                verify_pq_masp_stark_v1(&statement, &wire).is_err(),
        ...
        assert!(
            verify_pq_masp_stark_v1(&changed_statement, &proof).is_err(),
        ...
        for offset in [0, 8, authorized_proof.len() / 2, authorized_proof.len() - 1] {
            ...
            tampered[offset] ^= 1;
            assert!(
                super::super::verify_pq_masp_v1(&statement, &tampered).is_err(),
        ...
        assert_eq!(
            super::super::verify_pq_masp_v1(&wrong_key_statement, &authorized_proof,),
            Err(super::super::PqMaspProofErrorV1::Authorization(
                PqMaspWireErrorV1::AuthorizationKeyMismatch,
            ))
        );
        let mut wrong_digest_statement = statement.clone();
        wrong_digest_statement.context.transaction_intent_digest =
            iroha_data_model::privacy::PrivacyTransactionIntentDigestV1::new([0xD2; 32]);
        assert_eq!(
            super::super::verify_pq_masp_v1(&wrong_digest_statement, &authorized_proof,),
            Err(super::super::PqMaspProofErrorV1::Authorization(
    ...
    fn hostile_prover_cannot_substitute_public_nullifier_after_rebuilding_aux() {
        ...
        let adapter = PqMaspStarkAdapterV1::new(&target);
