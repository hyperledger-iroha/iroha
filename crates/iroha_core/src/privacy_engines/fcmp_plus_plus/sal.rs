...
impl FcmpSalWitnessV1 {
    ...
    pub fn new(
        ...
        let x_bytes = Zeroizing::new(x);
        let y_bytes = Zeroizing::new(y);
        let r_i_bytes = Zeroizing::new(r_i);
        let r_r_i_bytes = Zeroizing::new(r_r_i);
        let x = Zeroizing::new(scalar_from_bytes(*x_bytes)?);
        let y = Zeroizing::new(scalar_from_bytes(*y_bytes)?);
        let r_i = Zeroizing::new(scalar_from_bytes(*r_i_bytes)?);
        let r_r_i = Zeroizing::new(scalar_from_bytes(*r_r_i_bytes)?);
        Ok(Self {
            x: *x,
            y: *y,
            r_i: *r_i,
