...
pub(crate) const IVM_PRIVATE_NOTE_STARK_PROFILE_DESCRIPTOR_V1: &[u8] = b"iroha-ivm-private-note-stark-v1:relation=proof-managed-note:wire=IPS1-v1:trace=2^14:base=556:profile-aux=1:profile-fixed=122:profile-constraints=1372:constraint-degree=4:max-proof=8388608:sha256-wide-air:public-digest=verifier-fixed-terminal-bytes:tree-depth=32:vm=16x8:ciphertext=IPNE-v1:fee=separate:legacy=unrepresentable:governance=typed-lifecycle";
...
pub(crate) const IVM_PRIVATE_NOTE_STARK_PROFILE_DIGEST_V1: [u8; 32] = [
    0x55, 0xdc, 0x27, 0x39, 0xd2, 0x64, 0xdf, 0x1a, 0x14, 0x1e, 0x50, 0xca, 0x84, 0x96, 0x67, 0x96,
    0x74, 0x6d, 0xb5, 0xc7, 0xb2, 0xaa, 0x32, 0x3e, 0xc9, 0xa1, 0x0e, 0xcb, 0x2a, 0x47, 0x31, 0x6d,
];
...
pub(crate) const IVM_PRIVATE_NOTE_STARK_KAT_PROOF_SHA256_V1: [u8; 32] = [
    0x16, 0x6e, 0x44, 0x63, 0xb8, 0xae, 0xd1, 0x10, 0x13, 0x5d, 0x3c, 0x08, 0x95, 0x60, 0xd2, 0xab,
    0xf1, 0x3e, 0x85, 0xec, 0xc3, 0xe9, 0xca, 0xc5, 0x40, 0x1e, 0xe6, 0x6a, 0x06, 0x29, 0xb7, 0x17,
];
...
impl<'a> PrivateNoteStarkAdapterV1<'a> {
    pub(super) const fn new(statement: &'a IrohaIvmPrivateNoteStarkStatementV1) -> Self {
        Self { statement }
    }
}
...
impl ProofManagedNoteStarkAdapterV1 for PrivateNoteStarkAdapterV1<'_> {
    ...
    fn public_input_digest_v1(&self) -> Result<[u8; 32], ProofManagedNoteStarkErrorV1> {
        ...
        sha256_frame_v1(b"ivm-private-note-stark-public-input-v1", &[&encoded])
...
pub(super) fn prove_private_note_stark_v1_with_rng<R: TryRngCore>(
    ...
    prove_proof_managed_note_stark_v1_with_rng(
        &PrivateNoteStarkAdapterV1::new(statement),
        &base_columns,
        rng,
    )
}
...
pub(super) fn prove_private_note_stark_v1(
    ...
    let base_columns = compile_private_note_prover_columns_v1(statement, witness)?;
    prove_proof_managed_note_stark_v1(&PrivateNoteStarkAdapterV1::new(statement), &base_columns)
}

/// Verify the exact private-note proof wire against the public statement.
pub(crate) fn verify_private_note_stark_v1(
    statement: &IrohaIvmPrivateNoteStarkStatementV1,
    proof_bytes: &[u8],
) -> Result<(), ProofManagedNoteStarkErrorV1> {
    verify_proof_managed_note_stark_v1(&PrivateNoteStarkAdapterV1::new(statement), proof_bytes)
}
...
mod tests {
    ...
    fn compiled_profile_count_and_digest_are_exact() {
        ...
        let expected_descriptor = format!(
            "iroha-ivm-private-note-stark-v1:relation=proof-managed-note:wire=IPS1-v1:trace=2^{}:base={}:profile-aux={}:profile-fixed={}:profile-constraints={}:constraint-degree={}:max-proof={}:sha256-wide-air:public-digest=verifier-fixed-terminal-bytes:tree-depth=32:vm=16x8:ciphertext=IPNE-v1:fee=separate:legacy=unrepresentable:governance=typed-lifecycle",
            PRIVATE_NOTE_TRACE_LOG2_V1,
            PRIVATE_NOTE_BASE_WIDTH_V1,
            PRIVATE_NOTE_PROFILE_AUX_WIDTH_V1,
            PRIVATE_NOTE_PROFILE_FIXED_WIDTH_V1,
    ...
    fn full_domain_stark_roundtrip_and_adversarial_wires_fail_closed() {
        ...
        super::super::verify_ivm_private_note_v1(&value.statement, &proof)
        ...
        assert!(
            verify_private_note_stark_v1(&value.statement, &rejected_draft_magic).is_err(),
        ...
        assert!(
            verify_private_note_stark_v1(&value.statement, &vec![0; proof.len()]).is_err(),
        ...
        for wire in wires {
            assert!(
                verify_private_note_stark_v1(&value.statement, &wire).is_err(),
        ...
        assert!(
            verify_private_note_stark_v1(&changed_statement, &proof).is_err(),
    ...
    fn native_rows_satisfy_every_extension_residue_on_the_trace_domain() {
        ...
        let adapter = PrivateNoteStarkAdapterV1::new(&value.statement);
    ...
    fn hostile_prover_cannot_substitute_a_statement_nullifier_after_rebuilding_aux() {
        ...
        let adapter = PrivateNoteStarkAdapterV1::new(&target_statement);
