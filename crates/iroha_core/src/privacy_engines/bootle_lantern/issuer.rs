...
pub const BOOTLE_LANTERN_ISSUER_PROFILE_DESCRIPTOR_V1: &[u8] = b"falcon512-ntru-r512-as-r64-rank8-interleaved|source:rust-fn-dsa-workspace-0.3-daf14859b5aa3f8d75c42966ba7de83e6eb59997-Unlicense|specialization:BLNS-specialization-no-main-construction-reduction|public-key:H_i[j]=h[8*j+i]|equation:s1+h*s2=t+A_tau*tau+credential-scope|keygen-candidates:4096|authorization-id-draws:4|authorization-lifetime-blocks<=4096|authorization-state:Fresh-Processing-Completed-or-Failed|issuance-store:bounded-strict-ILS1-file-store+canonical-process-lease+held-unix-exclusive-flock+atomic-sync-rename+explicit-height-pruning|preimage-attempts:64|tau:one-purpose-derived-64-byte-stream-eight-R64-MSB-first|preimage-rng:purpose-derived-56-byte-Falcon-ChaCha20-word-major|issuance-rng:one-health-checked-master64-per-holder-or-issuer-operation+closed-context-bound-SHAKE256-substreams|canonical-flow:keygen-authorize-request-ILQ1-with-ILB1-nonmutating-preflight-before-P1-verify-atomic-height-claim-issue-durable-commit-ILR1-finalize-ILN1|completed-retry-before-P1-after-process-reopen-and-independent-of-expiry|no-direct-issuance";
...
pub fn issuer_authorize_blind_issuance_with_rng_v1<R: CryptoRng + RngCore>(
    ...
    let credential_scope_digest = scope
        .digest()
        .map_err(|_| BootleLanternIssuanceErrorV1::CredentialScopeFailed)?;
    let mut checked = HealthCheckedCryptoRngV1::new(rng).map_err(map_randomness_error_v1)?;
...
/// Decode one canonical holder request, verify P1, atomically claim, and issue once.
///
/// This is the public issuer ingress. It rejects the complete `ILQ1` envelope
/// before policy checks, proof verification, store access, or issuer randomness.
///
/// # Errors
///
