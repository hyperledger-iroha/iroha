//! A compact, dual-field SHA-256 chip for paired-Pasta recursion.
//!
...
//! final digest therefore remain connected by permutation constraints. Each
//! 16-bit dense/spread relation is reduced to two exact lookups in one 256-row
//! byte table; no table row or constraint depends on hash history.
use ff::PrimeField;
use halo2_proofs::{
...
mod table8;
pub(crate) use table8::util;
#[allow(unused_imports)]
pub use table8::{Table8Chip, Table8Config};
/// Number of rows in the complete eight-bit dense/spread lookup table.
pub(crate) const TABLE8_SPREAD_TABLE_ROWS: usize = 1 << 8;
/// Audited upper estimate of rows occupied by one compression block.
///
/// This is a source-level layout estimate, not a synthesized resource
/// measurement. Integration must retain the existing exact capacity check.
pub(crate) const TABLE8_COMPRESSION_ROWS_ESTIMATE_UNMEASURED: usize = 2_304;
/// Estimated standalone one-lane Processed proving-key bytes at `k = 12`.
///
/// This uses 14 advice, 6 configured fixed, 22 selector, and 9 permutation
/// columns. It is source geometry only, not a measured artifact or RSS result.
pub(crate) const TABLE8_ONE_LANE_PROCESSED_PK_BYTES_K12_UNMEASURED: u64 = 10_094_062;
/// Estimated standalone five-lane Processed proving-key bytes at `k = 12`.
///
/// The estimate follows the repository's current Processed serialization
/// formula for 70 advice, 6 configured fixed, 110 selector, and 41 permutation
/// columns. It deliberately makes no claim about peak RSS or proof bytes; both
/// remain release-qualification measurements.
pub(crate) const TABLE8_FIVE_LANE_PROCESSED_PK_BYTES_K12_UNMEASURED: u64 = 41_556_142;
const _: () = {
    // Compile-time checks cover only the documented source estimates. Release
    // qualification must still measure Processed bytes, proof bytes, and RSS.
    assert!(TABLE8_COMPRESSION_ROWS_ESTIMATE_UNMEASURED < (1 << 12));
    assert!(TABLE8_ONE_LANE_PROCESSED_PK_BYTES_K12_UNMEASURED < 64 * 1024 * 1024);
    assert!(TABLE8_FIVE_LANE_PROCESSED_PK_BYTES_K12_UNMEASURED < 64 * 1024 * 1024);
};
...
///
/// Paired Pasta constructs these from the range-checked Base-circuit message
/// bytes. The Table8 packer copy-binds this cell into its raw region.
#[derive(Clone, Debug)]
...
impl<const LEN: usize> Bits<LEN> {
    fn spread<const SPREAD: usize>(&self) -> [bool; SPREAD] {
        table8::util::spread_bits(self.0)
    }
}
...
impl<const LEN: usize, F: PrimeField> From<&Bits<LEN>> for Assigned<F> {
    fn from(bits: &Bits<LEN>) -> Self {
        assert!(LEN <= 64);
        F::from(table8::util::lebs2ip(&bits.0)).into()
    }
}
impl From<&Bits<16>> for u16 {
    fn from(bits: &Bits<16>) -> Self {
        table8::util::lebs2ip(&bits.0) as u16
    }
}
impl From<u16> for Bits<16> {
    fn from(value: u16) -> Self {
        Self(table8::util::i2lebsp(value.into()))
    }
}
impl From<&Bits<32>> for u32 {
    fn from(bits: &Bits<32>) -> Self {
        table8::util::lebs2ip(&bits.0) as u32
    }
}
...
impl<const LEN: usize, F: PrimeField> AssignedBits<LEN, F> {
    /// Reconstitute an assigned bit string from a cell whose range proof lives in another chip.
    ///
    /// This is the same cross-region boundary used by [`AssignedByte`].  Callers must supply the
    /// exact physical cell behind the range-constrained source; subsequent Table8 decomposition
    /// copy-binds the cell again.
    pub(crate) fn from_range_checked_cell(value: Value<Bits<LEN>>, cell: Cell) -> Self {
        Self {
            value,
            cell,
            _marker: PhantomData,
...
/// Low-level instructions implemented by the Table8 chip.
///
/// `compress` accepts the same assigned 32-bit words used by the current Pasta
/// SHA queue, copy-binds all sixteen words into the schedule, copy-binds the
/// complete predecessor state into a fresh decomposition, and returns eight
/// range-constrained feed-forward words. Recursive callers select the standard
/// IV or the preceding block output; this leaf imposes no job-count or history
/// ceiling.
...
mod tests {
    ...
    impl<F: PrimeField> Circuit<F> for VectorCircuit {
        ...
        fn configure(meta: &mut ConstraintSystem<F>) -> Self::Config {
            ...
            TestConfig {
                sha: Table8Chip::<F>::configure(meta),
                input,
            }
        }
        fn synthesize(
            ...
            Table8Chip::<F>::load(config.sha.clone(), &mut layouter)?;
            let chip = Table8Chip::<F>::construct(config.sha);
    ...
    fn assert_fips_vectors<F: PrimeField + ff::FromUniformBytes<64> + Ord>() {
        ...
        MockProver::<F>::run(17, &circuit, vec![])
            .expect("Table8 synthesis")
            .assert_satisfied();
    }
    fn assert_single_compression_fits_k12<F: PrimeField + ff::FromUniformBytes<64> + Ord>() {
        let message = b"abc".to_vec();
        let expected: [u8; 32] = Sha256::digest(&message).into();
        let circuit = VectorCircuit {
            vectors: vec![(message, expected)],
            tamper_first_source_value: false,
        };
        MockProver::<F>::run(12, &circuit, vec![])
            .expect("one Table8 compression fits k=12")
            .assert_satisfied();
    }
    #[test]
    fn one_compression_fits_k12_in_both_pasta_parities() {
        assert_single_compression_fits_k12::<Fp>();
        assert_single_compression_fits_k12::<Fq>();
    }
    #[test]
    fn fips_vectors_fp() {
    ...
    fn source_value_is_copy_constrained() {
        ...
        let prover = MockProver::<Fp>::run(17, &circuit, vec![]).expect("Table8 synthesis");
