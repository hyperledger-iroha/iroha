...
use crate::zk::pasta_sha256_table8::{
    AssignedBits, IV,
    table8::{SpreadVar, SpreadWord, StateWord, Table8Assignment},
...
impl CompressionConfig {
    ...
    pub(super) fn assign_h_prime<F: PrimeField>(
        ...
        // Assign h_prime_lo, h_prime_hi, h_prime_carry
        {
            ...
            region.assign_advice(a_9, row + 1, h_prime_carry.map(F::from));
    ...
    pub(super) fn assign_state_addition<F: PrimeField>(
        ...
        region.assign_advice(a_9, row, st_new_carry.map(F::from));
        Ok(st_new_dense)
    }
    ...
    pub(super) fn assign_e_new<F: PrimeField>(
        ...
        region.assign_advice(a_9, row + 1, e_new_carry.map(F::from));
        // Rounds 0..62 are copy-bound into the next round's decomposition.
        // The last round has no successor, so range-check its raw E explicitly.
    ...
    pub(super) fn assign_a_new<F: PrimeField>(
        ...
        region.assign_advice(a_9, row, a_new_carry.map(F::from));
        // Rounds 0..62 are copy-bound into the next round's decomposition.
        // The last round has no successor, so range-check its raw A explicitly.
        if round_idx == 63.into() {
