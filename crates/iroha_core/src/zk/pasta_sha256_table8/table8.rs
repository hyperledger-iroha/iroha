...
/// Configuration for a [`Table8Chip`].
#[derive(Clone, Debug)]
...
#[derive(Clone, Debug)]
struct Table8SharedConfig {
    lookup: SpreadTable,
    constant: Column<Fixed>,
}
/// A chip that implements SHA-256 with one complete $2^8$ lookup table.
#[derive(Clone, Debug)]
pub struct Table8Chip<F: PrimeField> {
    config: Table8Config,
    _marker: PhantomData<F>,
}
impl<F: PrimeField> Chip<F> for Table8Chip<F> {
...
impl<F: PrimeField> Table8Chip<F> {
    fn assert_field_size() {
        assert!(
            F::NUM_BITS >= 65,
            "Table8 SHA-256 requires a field modulus larger than 2^64 - 1"
        );
    }
    ...
    fn configure_shared(meta: &mut ConstraintSystem<F>) -> Table8SharedConfig {
        let lookup = SpreadTableChip::<F>::configure_table(meta);
        let constant = meta.fixed_column();
        meta.enable_constant(constant);
        Table8SharedConfig { lookup, constant }
    }
    fn configure_lane(
        meta: &mut ConstraintSystem<F>,
        shared: &Table8SharedConfig,
        advice: [Column<Advice>; 10],
        lookup_aux: [Column<Advice>; 4],
        ...
        let [low_dense, low_spread, low_tag, high_nonzero] = lookup_aux;
    ...
    /// Configure independent Table8 lanes sharing one five-column byte table
    /// and one fixed constant column.
    pub(crate) fn configure_lanes<const LANES: usize>(
        ...
        std::array::from_fn(|_| {
            ...
            let lookup_aux = std::array::from_fn(|_| meta.advice_column());
            Self::configure_lane(meta, &shared, advice, lookup_aux)
        })
    }
    ...
    /// Construct an externally copy-bound SHA-256 chaining state.
    ///
    /// Each low/high half must come from a separately constrained 32-bit source word.  The next
    /// [`Sha256Instructions::compress`] call copy-decomposes and range-checks all halves before
    /// using them, so a shard continuation cannot smuggle a non-word field element into the
    /// compression state.
    pub(crate) fn state_from_dense_halves(
        halves: [[AssignedBits<16, F>; 2]; super::DIGEST_SIZE],
    ) -> <Self as Sha256Instructions<F>>::State {
        let [a, b, c, d, e, f, g, h] = halves.map(|[lo, hi]| RoundWordDense::from((lo, hi)));
        State::new(
            StateWord::A(RoundWordA::new_dense(a)),
            StateWord::B(RoundWord::new(b, None)),
            StateWord::C(RoundWord::new(c, None)),
            StateWord::D(d),
            StateWord::E(RoundWordE::new_dense(e)),
            StateWord::F(RoundWord::new(f, None)),
            StateWord::G(RoundWord::new(g, None)),
            StateWord::H(h),
        )
    }
    /// Loads the lookup table required by this chip into the circuit.
    pub fn load(config: Table8Config, layouter: &mut impl Layouter<F>) -> Result<(), Error> {
        SpreadTableChip::load(config.lookup, layouter)
    }
}
impl<F: PrimeField> Sha256Instructions<F> for Table8Chip<F> {
    type State = State<F>;
...
/// Common assignment patterns used by Table8 regions.
trait Table8Assignment<F: PrimeField> {
...
mod constraint_inventory_tests {
    use super::Table8Chip;
    use crate::zk::pasta_sha256_table8::{
        TABLE8_FIVE_LANE_PROCESSED_PK_BYTES_K12_UNMEASURED,
        TABLE8_ONE_LANE_PROCESSED_PK_BYTES_K12_UNMEASURED,
    };
    use ff::PrimeField;
    use halo2_proofs::{
    ...
    fn assert_inventory<F: PrimeField>() {
        let mut one = ConstraintSystem::<F>::default();
        let _ = Table8Chip::<F>::configure_lanes::<1>(&mut one);
        assert_eq!(one.num_advice_columns(), 14);
        assert_eq!(one.num_fixed_columns(), 6);
        assert_eq!(one.num_instance_columns(), 0);
        assert_eq!(one.num_selectors(), 22);
        assert_eq!(one.permutation().get_columns().len(), 9);
        assert_eq!(one.lookups().len(), 2);
        assert_eq!(one.degree(), 9);

        let mut five = ConstraintSystem::<F>::default();
        let _ = Table8Chip::<F>::configure_lanes::<5>(&mut five);
        assert_eq!(five.num_advice_columns(), 70);
        assert_eq!(five.num_fixed_columns(), 6);
        assert_eq!(five.num_instance_columns(), 0);
        assert_eq!(five.num_selectors(), 110);
        assert_eq!(five.permutation().get_columns().len(), 41);
        assert_eq!(five.lookups().len(), 10);
        assert_eq!(five.degree(), 9);
    }

    #[test]
    fn table8_geometry_is_identical_across_both_pasta_parities() {
        assert_inventory::<Fp>();
        assert_inventory::<Fq>();
    }

    #[test]
    fn source_level_k12_processed_key_estimate_is_below_64_mib() {
        // This mirrors Processed encoding geometry only. It is intentionally
        // not a claim about peak RSS, proof size, or measured key bytes.
        let domain_rows = 1_u64 << 12;
        let polynomial_bytes = domain_rows * 32 + 4;
        let estimate = |selectors: u64, permutation: u64| {
            let processed_fixed = 6_u64 + selectors;
            let verifying_key = (processed_fixed + permutation) * 32 + (1 + 4 + 1 + 4);
            (processed_fixed * 2 + permutation * 2 + 3) * polynomial_bytes + 4 * 4 + verifying_key
        };
        let one_lane = estimate(22, 9);
        let five_lane = estimate(110, 41);
        assert_eq!(one_lane, TABLE8_ONE_LANE_PROCESSED_PK_BYTES_K12_UNMEASURED);
        assert_eq!(
            five_lane,
            TABLE8_FIVE_LANE_PROCESSED_PK_BYTES_K12_UNMEASURED
        );
        assert!(one_lane < 64 * 1024 * 1024);
        assert!(five_lane < 64 * 1024 * 1024);
    }
}
