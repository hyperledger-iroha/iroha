    ...
    G: OfflineCashGuardBundleVerifierV1,
{
    ...
    pub fn preview_receive_fold(
        ...
        trusted_commit_time_ms: u64,
    ) -> Result<ReceiveFoldPreviewV1, OfflineCashStateErrorV1> {
        let staged = self
            .pending_credits
            .get(&credit_id)
        ...
        let incoming_proof_binding_digest =
            offline_cash_incoming_proof_binding_digest_v1(&sender_output, &staged.payment.proof)
        ...
        let transition = self.transition_preview(
            OfflineCashTransitionKindV1::ReceiveFold,
            successor,
            effect_digest,
            [0; 32],
            [0; 32],
    ...
    pub fn receive_fold_prepared(
        ...
        let staged = self.pending_credits.get(&preview.credit.credit_id).ok_or(
            OfflineCashStateErrorV1::CreditNotStaged(preview.credit.credit_id),
        )?;
        ...
        let expected_credit = ReceiveFoldCreditPreviewV1 {
            ...
            recipient_lane_id: self.state.lane.device_lane_id,
            incoming_proof_binding_digest: offline_cash_incoming_proof_binding_digest_v1(
                &sender_output,
