//! Exact sparse-Merkle index for consumed Offline Cash credit identities.
...
use super::{
    ...
    OFFLINE_CASH_CONSUMED_CREDIT_TREE_DEPTH_V1, OfflineCashStateErrorV1,
};
use crate::zk::offline_cash_v1_poseidon::{
    OFFLINE_CASH_REPLAY_EMPTY_DOMAIN_V1, OFFLINE_CASH_REPLAY_LEAF_DOMAIN_V1,
    OFFLINE_CASH_REPLAY_NODE_DOMAIN_V1, decode, digest_limbs, encode, hash,
};
...
pub(super) struct PreparedConsumedCreditBatchV1 {
    starting_root: OfflineCashPastaStateCommitmentV1,
    final_root: OfflineCashPastaStateCommitmentV1,
    inserts: Vec<PreparedConsumedCreditInsertV1>,
    final_overlay: BTreeMap<NodePosition, Option<OfflineCashPastaStateCommitmentV1>>,
}
...
struct PreparedConsumedCreditInsertV1 {
    ...
    successor_path:
        [OfflineCashPastaStateCommitmentV1; OFFLINE_CASH_CONSUMED_CREDIT_TREE_DEPTH_V1 + 1],
}
...
impl PreparedConsumedCreditBatchV1 {
    ...
    pub(super) fn starting_root(&self) -> OfflineCashPastaStateCommitmentV1 {
        self.starting_root
    }

    pub(super) fn final_root(&self) -> OfflineCashPastaStateCommitmentV1 {
        self.final_root
    }
...
pub(super) struct ExactConsumedCreditIndex {
    ...
    nodes: BTreeMap<NodePosition, OfflineCashPastaStateCommitmentV1>,
    empty_at_depth: [OfflineCashPastaStateCommitmentV1; 257],
}

impl ExactConsumedCreditIndex {
    pub(super) fn empty() -> Self {
        let mut empty_at_depth = [OfflineCashPastaStateCommitmentV1::ZERO; 257];
    ...
    pub(super) fn root(&self) -> OfflineCashPastaStateCommitmentV1 {
        self.nodes
            .get(&NodePosition {
                depth: 0,
                prefix: [0; 32],
    ...
    pub(super) fn preview_insert_witness(
        ...
        credit_id: CreditIdV1,
        envelope_digest: DigestV1,
    ) -> Result<ConsumedCreditInsertWitnessV1, OfflineCashStateErrorV1> {
    ...
    ) -> Result<PreparedConsumedCreditBatchV1, OfflineCashStateErrorV1> {
        if inserts.is_empty() {
            return Err(OfflineCashStateErrorV1::InvalidConsumedCreditInsertWitness);
        }
        ...
        for insert in &prepared.inserts {
                ...
                .or_else(|| batch_records.get(&insert.credit_id).copied())
            {
                return if existing == insert.envelope_digest {
                    Err(OfflineCashStateErrorV1::CreditAlreadyConsumed(
                        insert.credit_id,
                    ))
                } else {
                    Err(OfflineCashStateErrorV1::CreditConflict(insert.credit_id))
                };
            }
            ...
            if insert.credit_id.is_zero()
                ...
                || insert.successor_path[OFFLINE_CASH_CONSUMED_CREDIT_TREE_DEPTH_V1]
    ...
    fn siblings_root_to_leaf_with_overlay(
        &self,
        credit_id: CreditIdV1,
        overlay: &BTreeMap<NodePosition, Option<OfflineCashPastaStateCommitmentV1>>,
    ) -> [OfflineCashPastaStateCommitmentV1; OFFLINE_CASH_CONSUMED_CREDIT_TREE_DEPTH_V1] {
        core::array::from_fn(|parent_depth| {
            let child_depth = parent_depth + 1;
...
fn apply_successor_path_to_overlay(
    overlay: &mut BTreeMap<NodePosition, Option<OfflineCashPastaStateCommitmentV1>>,
    empty_at_depth: &[OfflineCashPastaStateCommitmentV1; 257],
    credit_id: CreditIdV1,
    successor_path: &[OfflineCashPastaStateCommitmentV1;
         OFFLINE_CASH_CONSUMED_CREDIT_TREE_DEPTH_V1 + 1],
) {
...
impl ConsumedCreditInsertWitnessV1 {
    ...
    #[must_use]
    pub fn canonical_empty_leaf_digest() -> OfflineCashPastaStateCommitmentV1 {
        hash_empty_leaf()
    }

    /// Return the canonical present-leaf digest bound to this credit and envelope.
    #[must_use]
    pub fn canonical_present_leaf_digest(&self) -> OfflineCashPastaStateCommitmentV1 {
    ...
    /// # Errors
    ///
    /// Returns [`OfflineCashStateErrorV1::InvalidConsumedCreditInsertWitness`] when any identity,
    /// path bit, sibling, leaf, or root relation is invalid.
    pub fn verify(&self) -> Result<(), OfflineCashStateErrorV1> {
    ...
    pub fn verify_binding(
        &self,
        predecessor_root: OfflineCashPastaStateCommitmentV1,
        credit_id: CreditIdV1,
        envelope_digest: DigestV1,
        successor_root: OfflineCashPastaStateCommitmentV1,
    ) -> Result<(), OfflineCashStateErrorV1> {
...
fn hash_empty_leaf() -> OfflineCashPastaStateCommitmentV1 {
    OfflineCashPastaStateCommitmentV1 {
        eq: encode(hash::<Fp>(OFFLINE_CASH_REPLAY_EMPTY_DOMAIN_V1, &[])),
        ep: encode(hash::<Fq>(OFFLINE_CASH_REPLAY_EMPTY_DOMAIN_V1, &[])),
    }
}
...
fn path_root_from_siblings(
    credit_id: CreditIdV1,
    leaf: OfflineCashPastaStateCommitmentV1,
    siblings_root_to_leaf: &[OfflineCashPastaStateCommitmentV1;
         OFFLINE_CASH_CONSUMED_CREDIT_TREE_DEPTH_V1],
) -> Result<OfflineCashPastaStateCommitmentV1, OfflineCashStateErrorV1> {
...
> {
    let key = credit_id.0;
    let mut path =
        [OfflineCashPastaStateCommitmentV1::ZERO; OFFLINE_CASH_CONSUMED_CREDIT_TREE_DEPTH_V1 + 1];
    path[OFFLINE_CASH_CONSUMED_CREDIT_TREE_DEPTH_V1] = leaf;
...
fn hash_node(
    ...
    let left_eq =
        decode::<Fp>(left.eq).ok_or(OfflineCashStateErrorV1::InvalidConsumedCreditInsertWitness)?;
    let right_eq = decode::<Fp>(right.eq)
        .ok_or(OfflineCashStateErrorV1::InvalidConsumedCreditInsertWitness)?;
    let left_ep =
        decode::<Fq>(left.ep).ok_or(OfflineCashStateErrorV1::InvalidConsumedCreditInsertWitness)?;
...
mod tests {
    ...
    fn prepared_batch_rejects_tampered_sibling_without_mutation() {
        ...
        assert_eq!(
            ...
            Err(OfflineCashStateErrorV1::InvalidConsumedCreditInsertWitness)
        );
