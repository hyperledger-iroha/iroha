...
fn fp_spec() -> &'static Spec<Fp> {
    ...
    SPEC.get_or_init(|| {
        Spec::new::<
            KAGEMUSHA_POSEIDON_FULL_ROUNDS_V1,
            KAGEMUSHA_POSEIDON_PARTIAL_ROUNDS_V1,
            KAGEMUSHA_POSEIDON_SECURE_MDS_V1,
        >()
    })
}
...
/// Circuit-side sponge using exactly the native Kagemusha V1 parameters.
///
...
/// own field constants.
pub(crate) struct KagemushaPoseidonChipV1<F: KagemushaPoseidonFieldV1> {
    hasher: PoseidonHasher<F, KAGEMUSHA_POSEIDON_WIDTH_V1, KAGEMUSHA_POSEIDON_RATE_V1>,
}

impl<F: KagemushaPoseidonFieldV1> KagemushaPoseidonChipV1<F> {
...
impl KagemushaPoseidonFieldV1 for Fp {
...
impl KagemushaPoseidonFieldV1 for Fq {
...
pub(crate) fn hash<F: KagemushaPoseidonFieldV1>(domain: u64, inputs: &[F]) -> F {
    let mut inline = [F::ZERO; KAGEMUSHA_POSEIDON_INLINE_ARITY_V1 + 2];
    let mut allocated = Vec::new();
    let preimage = if inputs.len() <= KAGEMUSHA_POSEIDON_INLINE_ARITY_V1 {
...
pub(crate) fn empty_replay_root<F: KagemushaPoseidonFieldV1>() -> F {
    ...
    for _ in 0..256 {
        root = hash(KAGEMUSHA_REPLAY_NODE_DOMAIN_V1, &[root, root]);
    }
    root
}
...
pub(crate) fn paired_commitment(eq: Fp, ep: Fq) -> (KagemushaPastaStateCommitmentV1, [u8; 32]) {
    ...
    let head = kagemusha_pasta_state_commitment_v1(components);
    (components, head)
}
...
mod tests {
    ...
    fn allocated_reference_hash<F: KagemushaPoseidonFieldV1>(domain: u64, inputs: &[F]) -> F {
    ...
    fn inline_native_preimage_matches_allocated_transcript() {
        for arity in [0_usize, 2, 4, 23, KAGEMUSHA_POSEIDON_INLINE_ARITY_V1 + 1] {
            ...
            let fq_inputs = (0..arity)
                ...
                .collect::<Vec<_>>();
            assert_eq!(
                hash(KAGEMUSHA_REPLAY_NODE_DOMAIN_V1, &fp_inputs),
                allocated_reference_hash(KAGEMUSHA_REPLAY_NODE_DOMAIN_V1, &fp_inputs),
            );
            assert_eq!(
                hash(KAGEMUSHA_REPLAY_NODE_DOMAIN_V1, &fq_inputs),
                allocated_reference_hash(KAGEMUSHA_REPLAY_NODE_DOMAIN_V1, &fq_inputs),
            );
        }
    }
    ...
    fn paired_wire_head_binds_both_components() {
        ...
        assert_eq!(head, kagemusha_pasta_state_commitment_v1(components));
        let mut substituted = components;
        substituted.ep = encode(Fq::from(6));
        assert_ne!(head, kagemusha_pasta_state_commitment_v1(substituted));
    }
