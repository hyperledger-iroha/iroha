...
pub struct KagemushaRecursiveIncomingEqGenerationWitnessV1<'a> {
    ...
    /// Eq post-commit payment proof or release-pinned inactive proof.
    pub proof: &'a [u8],
...
pub struct KagemushaRecursiveIncomingEpGenerationWitnessV1<'a> {
    ...
    /// Ep post-commit payment proof or release-pinned inactive proof.
    pub proof: &'a [u8],
...
///
/// Exactly one incoming proof slot is always present. `ReceiveFold` activates it; every other
/// operation carries the release-pinned valid padding proof and history, so proof shape does not
/// depend on operation kind.
#[cfg(feature = "zk-halo2-ipa")]
#[derive(Clone)]
pub struct KagemushaRecursiveStateGenerationWitnessV1<'a> {
    ...
    /// Eq post-commit `CommitWrapper` protocol compiled from the authenticated release key.
    pub eq_incoming_protocol: &'a PlonkProtocol<EqAffine>,
    /// Ep post-commit `CommitWrapper` protocol compiled from the authenticated release key.
    pub ep_incoming_protocol: &'a PlonkProtocol<EpAffine>,
    /// The single fixed Eq authorization proof/history slot.
    pub eq_incoming_slots: [KagemushaRecursiveIncomingEqGenerationWitnessV1<'a>; 1],
    /// The single fixed Ep authorization proof/history slot.
    pub ep_incoming_slots: [KagemushaRecursiveIncomingEpGenerationWitnessV1<'a>; 1],
...
impl<'a> KagemushaRecursiveStateGenerationWitnessV1<'a> {
    fn into_recursive<'b>(
        self,
        eq_incoming_slots: &'b [CompositeIncomingEqWitnessV1<'b>; 1],
        ep_incoming_slots: &'b [CompositeIncomingEpWitnessV1<'b>; 1],
