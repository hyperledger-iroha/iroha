//! Final zero-knowledge commit wrapper for Kagemusha V1.
//!
...
use iroha_data_model::kagemusha::{
    KAGEMUSHA_PAYMENT_OUTBOX_MIN_BYTES_V1, KAGEMUSHA_REDEMPTION_OUTBOX_MIN_BYTES_V1,
    KagemushaAcceptanceIntentAuthorizationStatementV1,
    KagemushaAcceptanceIntentAuthorizationV1, KagemushaAcceptanceIntentV1,
    KagemushaAcceptanceTicketV1, KagemushaCommitCertificateV1, KagemushaCommitEvidenceV1,
    KagemushaHardwareCredentialV1, KagemushaHardwareProfileV1, KagemushaLifecycleBindingV1,
    KagemushaNoCommitClosureStatementV1, KagemushaOperationKindV1,
    KagemushaOutboxReservationV1, KagemushaPaymentRequestV1,
    kagemusha_asset_identity_digest_v1,
};
use sha2::{Digest as _, Sha256};

use super::{DigestV1, KagemushaOperationV1};
...
const ACCEPTANCE_INTENT_DIGEST_DOMAIN_V1: &[u8] = b"iroha:kagemusha:v1:acceptance-intent";
const ACCEPTANCE_INTENT_AUTHORIZATION_STATEMENT_DIGEST_DOMAIN_V1: &[u8] =
    b"iroha:kagemusha:v1:acceptance-intent-authorization-statement";
const OUTBOX_RESERVATION_COMMITMENT_DOMAIN_V1: &[u8] = b"iroha:kagemusha:v1:outbox-reservation";
const PRECOMMIT_BINDING_DOMAIN_V1: &[u8] = b"iroha:kagemusha:v1:precommit-binding\0";
...
const SENDER_ONE_TIME_COMMITMENT_DOMAIN_V1: &[u8] =
    b"iroha:kagemusha:v1:sender-one-time-authorization\0";
const SENDER_TERMINAL_AUTHORIZATION_DOMAIN_V1: &[u8] =
    b"iroha:kagemusha:v1:sender-terminal-authorization\0";
...
const NO_COMMIT_CLOSURE_STATEMENT_DIGEST_DOMAIN_V1: &[u8] =
    b"iroha:kagemusha:v1:no-commit-closure-statement";
const NO_COMMIT_CANCELLATION_SUCCESSOR_DOMAIN_V1: &[u8] =
    b"iroha:kagemusha:v1:no-commit-cancellation-successor\0";
const PREDECESSOR_CONFLICT_NULLIFIER_DOMAIN_V1: &[u8] =
    b"iroha:kagemusha:v1:predecessor-conflict-nullifier\0";
const NO_COMMIT_RECOVERY_ID_DOMAIN_V1: &[u8] = b"iroha:kagemusha:v1:no-commit-recovery-id\0";
const NO_COMMIT_HARDWARE_BINDING_DOMAIN_V1: &[u8] =
    b"iroha:kagemusha:v1:no-commit-hardware-binding\0";
const NO_COMMIT_INCARNATION_DOMAIN_V1: &[u8] = b"iroha:kagemusha:v1:no-commit-incarnation\0";
...
pub(crate) struct KagemushaCommitWrapperPublicInputsV1 {
...
impl KagemushaCommitWrapperPublicInputsV1 {
    ...
    pub(crate) fn from_acceptance_intent_authorization(
        ...
        statement: &KagemushaAcceptanceIntentAuthorizationStatementV1,
    ...
    pub(crate) fn validate(&self) -> Result<(), String> {
        ...
        for (name, digest) in [
            ...
            if digest == [0; 32] {
                return Err(format!("Kagemusha commit wrapper {name} digest is zero"));
            }
        }
        ...
        if authorization {
            if self.operation != KagemushaOperationV1::SendSplit
...
/// Complete private values checked by the final wrapper.
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct KagemushaCommitWrapperPrivateTransitionV1 {
    ...
    pub(crate) request: Option<KagemushaPaymentRequestV1>,
    /// Sender one-use intent, present only for `SendSplit`.
    pub(crate) acceptance_intent: Option<KagemushaAcceptanceIntentV1>,
    /// Receiver-hardware ticket, present only for `SendSplit`.
    pub(crate) acceptance_ticket: Option<KagemushaAcceptanceTicketV1>,
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct KagemushaCommitWrapperIntentAuthorizationPrivateV1 {
    pub(crate) request: KagemushaPaymentRequestV1,
    pub(crate) statement: KagemushaAcceptanceIntentAuthorizationStatementV1,
}

/// Complete private opening for the sender-hardware no-commit closure branch.
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct KagemushaCommitWrapperNoCommitClosurePrivateV1 {
    pub(crate) statement: KagemushaNoCommitClosureStatementV1,
    /// Original proof-bearing sender authorization whose exact envelope digest is cancelled.
    pub(crate) intent_authorization: KagemushaAcceptanceIntentAuthorizationV1,
    /// Hardware-private nonce making the lane-scoped recovery identity unique.
    pub(crate) hardware_recovery_nonce: DigestV1,
}

impl KagemushaCommitWrapperIntentAuthorizationPrivateV1 {
    pub(crate) fn validate_against(
        ...
        self.statement
            ...
            .map_err(|error| error.to_string())?;
        let expected =
            KagemushaCommitWrapperPublicInputsV1::from_acceptance_intent_authorization(
                &self.request,
                &self.statement,
...
impl KagemushaCommitWrapperPrivateTransitionV1 {
    pub(crate) fn validate_against(
        &self,
        public: &KagemushaCommitWrapperPublicInputsV1,
    ) -> Result<(), String> {
        ...
        let is_send = public.operation == KagemushaOperationV1::SendSplit;
...
pub(super) fn canonical_no_commit_hardware_binding_v1(
    guard: &super::KagemushaNormalizedGuardStatementV1,
) -> DigestV1 {
    hash_fixed_v1(
...
fn canonical_intent_bytes_v1(intent: &KagemushaAcceptanceIntentV1) -> Vec<u8> {
...
pub(crate) fn canonical_terminal_commit_binding_digest_v1(
    public: &KagemushaCommitWrapperPublicInputsV1,
    private: &KagemushaCommitWrapperPrivateTransitionV1,
...
#[cfg(feature = "zk-halo2-ipa")]
const CANDIDATE_BINDING_DOMAIN_V1: &[u8] = b"iroha:kagemusha:v1:commit-wrapper-candidate\0";
...
pub(crate) struct KagemushaCommitWrapperWitnessV1<'a> {
    pub(crate) public: KagemushaCommitWrapperPublicInputsV1,
    pub(crate) private_transition: KagemushaCommitWrapperPrivateTransitionV1,
    /// Present only for the pre-ticket sender-authorization branch.
    pub(crate) intent_authorization: Option<KagemushaCommitWrapperIntentAuthorizationPrivateV1>,
    /// Present only for the sender-hardware no-commit closure branch.
    pub(crate) no_commit_closure: Option<KagemushaCommitWrapperNoCommitClosurePrivateV1>,
...
where
    C: CurveAffineExt,
{
    private_transition: &'a KagemushaCommitWrapperPrivateTransitionV1,
    intent_authorization: Option<&'a KagemushaCommitWrapperIntentAuthorizationPrivateV1>,
    no_commit_closure: Option<&'a KagemushaCommitWrapperNoCommitClosurePrivateV1>,
    ...
    ($circuit:ty, $field:ty, $opposite:ty, $label:literal) => {
        impl Circuit<$field> for $circuit {
            ...
            fn configure_with_params(
                ...
                KagemushaCommitWrapperCircuitConfigV1 {
                    base,
...
fn validate_intent_authorization_relation_v1(
    public: &KagemushaCommitWrapperPublicInputsV1,
    private: &KagemushaCommitWrapperPrivateTransitionV1,
    authorization: &KagemushaCommitWrapperIntentAuthorizationPrivateV1,
    ...
    if !enabled_profiles
        ...
        || guard.operation != KagemushaOperationV1::SendSplit
...
fn validate_no_commit_closure_relation_v1(
    public: &KagemushaCommitWrapperPublicInputsV1,
    private: &KagemushaCommitWrapperPrivateTransitionV1,
    closure: &KagemushaCommitWrapperNoCommitClosurePrivateV1,
    relation: &KagemushaGuardBundleRelationWitnessV1,
    enabled_profiles: &[DigestV1; COMMIT_WRAPPER_ENABLED_PROFILE_SLOTS_V1],
) -> Result<(), String> {
    closure
    ...
    let expected_public = KagemushaCommitWrapperPublicInputsV1::from_no_commit_closure(
        statement,
        public.candidate_envelope_digest,
        public.acceptance_ticket_digest,
        public.eq_deferred_audit,
...
fn validate_terminal_guard_relation_v1(
    ...
    let sender_authorization = if public.operation == KagemushaOperationV1::SendSplit {
...
#[cfg(feature = "zk-halo2-ipa")]
pub(crate) fn build_kagemusha_commit_wrapper_pair_v1(
    ...
    C::ScalarExt: KagemushaPoseidonFieldV1,
{
    ...
    let mut builder = BaseCircuitBuilder::new(false)
        .use_k(
            usize::try_from(super::KAGEMUSHA_RECURSION_IPA_K_V1)
                .expect("Kagemusha k fits usize"),
        )
        .use_lookup_bits(
...
fn constrain_terminal_commit_semantics_v1<F: KagemushaPoseidonFieldV1>(
    ...
    let payment_min =
        ctx.load_constant(F::from(u64::from(KAGEMUSHA_PAYMENT_OUTBOX_MIN_BYTES_V1)));
...
#[cfg(feature = "zk-halo2-ipa")]
fn constrain_intent_authorization_semantics_v1<F: KagemushaPoseidonFieldV1>(
    builder: &mut BaseCircuitBuilder<F>,
    jobs: &mut PastaSha256JobsV1<F>,
    public: &[AssignedValue<F>],
    private: &KagemushaCommitWrapperPrivateTransitionV1,
    authorization: Option<&KagemushaCommitWrapperIntentAuthorizationPrivateV1>,
    guard: &KagemushaAssignedGuardBundleV1<F>,
    enabled_profiles: &[DigestV1; COMMIT_WRAPPER_ENABLED_PROFILE_SLOTS_V1],
) -> Result<(), String> {
    if public.len() != COMMIT_WRAPPER_PUBLIC_PREFIX_COUNT_V1 {
...
fn constrain_no_commit_closure_semantics_v1<F: KagemushaPoseidonFieldV1>(
    builder: &mut BaseCircuitBuilder<F>,
    jobs: &mut PastaSha256JobsV1<F>,
    public: &[AssignedValue<F>],
    private: &KagemushaCommitWrapperPrivateTransitionV1,
    closure: Option<&KagemushaCommitWrapperNoCommitClosurePrivateV1>,
    guard: &KagemushaAssignedGuardBundleV1<F>,
    enabled_profiles: &[DigestV1; COMMIT_WRAPPER_ENABLED_PROFILE_SLOTS_V1],
) -> Result<(), String> {
    if public.len() != COMMIT_WRAPPER_PUBLIC_PREFIX_COUNT_V1 {
    ...
    C::ScalarExt: KagemushaPoseidonFieldV1,
{
    ...
    for (offset, expected) in [
        ...
        (
            state_relation::public_instance::ROTATE_VERIFIER_AUTHORIZATION_LO,
    ...
    C::ScalarExt: KagemushaPoseidonFieldV1,
{
    ...
    let expected_protocol = crate::zk::kagemusha_v1_poseidon::digest_limbs::<C::ScalarExt>(
        candidate_protocol_digest,
    );
