//! Actual original compiler identities for private recursion frames.
//!
//! Captured records are metadata only. Current frame roundtrips exercise shape and codec
//! boundaries using existing mock transcripts; they do not establish proof or hardware authority.
...
fn observed_identity<T: NoritoSchema>(identifier: &str) {
    let rows: Vec<Value> = norito::json::from_slice(include_bytes!(concat!(
        ...
        "/../../fixtures/core/recursion_frame_identity_observations.v1.json"
    )))
