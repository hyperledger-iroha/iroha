...
use iroha_data_model::{
    account::AccountId,
    kagemusha::{
        KAGEMUSHA_ACCEPTANCE_TICKET_MIN_RESERVED_INBOX_BYTES_V1, KAGEMUSHA_HALO2_K_V1,
        ...
        KagemushaTrustedCommitTimeV1, kagemusha_device_key_reference_v1,
...
use super::super::super::{
    ...
    KagemushaCommitWrapperEqGenerationWitnessV1, KagemushaCommitWrapperGenerationPublicV1,
    KagemushaCommitWrapperGenerationWitnessV1,
    KagemushaCommitWrapperPrivateGenerationWitnessV1, KagemushaEpAccumulatorV1,
    KagemushaEqAccumulatorV1, KagemushaGeneratedCommitWrapperArtifactsV1,
    KagemushaGeneratedCommitWrapperEnvelopeV1, KagemushaGuardBundleRelationWitnessV1,
    KagemushaLoadedEpCommitWrapperArtifactsV1, KagemushaLoadedEqCommitWrapperArtifactsV1,
    KagemushaMintFinalityArtifactsV1, KagemushaNoCommitClosureGenerationPublicV1,
    KagemushaNormalizedGuardStatementV1, KagemushaOperationV1, KagemushaPastaParityV1,
    KagemushaRecursionArtifactsV1, fold_kagemusha_ep_accumulators_v1,
    fold_kagemusha_eq_accumulators_v1, generate_kagemusha_commit_wrapper_artifacts_v1,
...
use super::super::{
    KagemushaAuthenticatedRecursiveVerifierV1, verify_ep_succinct_protocol,
    verify_eq_succinct_protocol,
};
use super::{sign, signing_key};
use crate::zk::{
    kagemusha_v1_poseidon::{KagemushaPoseidonFieldV1, digest_limbs, encode, from_u128},
    kagemusha_v1_state::{DigestV1, KagemushaStateV1},
};
...
fn eq_carrier_keys(parameters: &ParamsIPA<EqAffine>, count: usize) -> EqCarrierKeys {
    ...
    let protocol_digest =
        super::super::native_parent_protocol_digest_v1(&protocol, KagemushaPastaParityV1::Eq)
...
fn ep_carrier_keys(parameters: &ParamsIPA<EpAffine>, count: usize) -> EpCarrierKeys {
    ...
    let protocol_digest =
        super::super::native_parent_protocol_digest_v1(&protocol, KagemushaPastaParityV1::Ep)
...
fn prove_carrier(
    ...
    CarrierProof {
        ...
        eq_proof,
        ep_proof,
        eq_history: initial_kagemusha_eq_accumulator_v1(eq_parameters)
            .expect("Eq carrier history"),
        ep_history: initial_kagemusha_ep_accumulator_v1(ep_parameters)
...
fn set_digest<F: KagemushaPoseidonFieldV1>(instances: &mut [F], offset: usize, digest: DigestV1) {
    instances[offset..offset + 2].copy_from_slice(&digest_limbs::<F>(digest));
}

#[allow(clippy::too_many_arguments)]
fn candidate_instances<F: KagemushaPoseidonFieldV1>(
    ...
    history: &[u8; super::super::super::KAGEMUSHA_HISTORY_ACCUMULATOR_BYTES_V1],
) -> Vec<F> {
...
fn hardware_profile() -> KagemushaHardwareProfileV1 {
    ...
    KagemushaHardwareProfileV1 {
        ...
        allowed_suite_commitment: kagemusha_suite_commitment_v1(tagged(0x76)),
        policy_epoch: 1,
...
fn request(predecessor: &KagemushaStateV1) -> KagemushaPaymentRequestV1 {
    let key = signing_key();
...
fn guard_relation(
    ...
    credential: crate::zk::kagemusha_v1_recursion::KagemushaPlatformCredentialStatementV1,
...
fn private_transition(
    ...
    request: &KagemushaPaymentRequestV1,
    intent: KagemushaAcceptanceIntentV1,
    ticket: Option<KagemushaAcceptanceTicketV1>,
    one_use_hardware_authorization: DigestV1,
    sender_one_time_opening: DigestV1,
) -> KagemushaCommitWrapperPrivateGenerationWitnessV1 {
    ...
    let outbox_reservation = KagemushaOutboxReservationV1 {
        ...
        operation_kind: KagemushaOperationKindV1::SendSplit,
        reserved_outbox_bytes: KAGEMUSHA_PAYMENT_OUTBOX_MIN_BYTES_V1,
        issued_at_ms: 10,
        expires_at_ms: 90_000,
    };
    let commit_evidence =
...
struct WrapperFolds {
    eq_candidate: super::super::super::KagemushaEqFoldProofV1,
    ep_candidate: super::super::super::KagemushaEpFoldProofV1,
    eq_guard: super::super::super::KagemushaEqFoldProofV1,
    ep_guard: super::super::super::KagemushaEpFoldProofV1,
    eq_merge: super::super::super::KagemushaEqFoldProofV1,
    ep_merge: super::super::super::KagemushaEpFoldProofV1,
    eq_successor: KagemushaEqAccumulatorV1,
    ep_successor: KagemushaEpAccumulatorV1,
}
...
fn wrapper_folds(
    ...
    let eq_candidate = fold_kagemusha_eq_accumulators_v1(
        eq_parameters,
        &candidate.eq_current,
        &candidate.eq_history,
    )
    .expect("fold Eq candidate");
    let ep_candidate = fold_kagemusha_ep_accumulators_v1(
        ep_parameters,
    ...
    let eq_guard =
        fold_kagemusha_eq_accumulators_v1(eq_parameters, &guard.eq_current, &guard.eq_history)
            .expect("fold Eq Guard");
    let ep_guard =
        fold_kagemusha_ep_accumulators_v1(ep_parameters, &guard.ep_current, &guard.ep_history)
...
fn wrapper_witness<'a>(
    public: KagemushaCommitWrapperGenerationPublicV1,
    private_transition: KagemushaCommitWrapperPrivateGenerationWitnessV1,
    guard_relation: KagemushaGuardBundleRelationWitnessV1,
    enabled_hardware_profiles: [[u8; 32]; super::super::super::KAGEMUSHA_COMMIT_WRAPPER_ENABLED_PROFILE_SLOTS_V1],
    candidate_keys: &'a CarrierKeys,
    guard_keys: &'a CarrierKeys,
    candidate: &'a CarrierProof,
    guard: &'a CarrierProof,
    folds: &'a WrapperFolds,
) -> KagemushaCommitWrapperGenerationWitnessV1<'a> {
    KagemushaCommitWrapperGenerationWitnessV1 {
        public,
        private_transition,
...
fn load_wrapper_artifacts(
    ...
    (
        KagemushaLoadedEqCommitWrapperArtifactsV1 {
            parameters: ParamsIPA::new(KAGEMUSHA_HALO2_K_V1),
...
fn authenticated_verifier(
    ...
    let placeholder = |role, tag| KagemushaArtifactBindingV1 {
        role,
        sha256: tagged(tag),
        byte_len: 1,
    };
    let artifacts = KagemushaRecursionArtifactsV1 {
        release_id,
        profile_digest,
        ...
        mint_finality: KagemushaMintFinalityArtifactsV1 {
            proving_key_eq: placeholder(KagemushaArtifactRoleV1::MintCreditPkEq, 0xA1),
            verifying_key_eq: placeholder(KagemushaArtifactRoleV1::MintCreditVkEq, 0xA2),
            proving_key_ep: placeholder(KagemushaArtifactRoleV1::MintCreditPkEp, 0xA3),
            verifying_key_ep: placeholder(KagemushaArtifactRoleV1::MintCreditVkEp, 0xA4),
        },
        artifact_manifest_digest: manifest_digest,
...
fn real_no_commit_generation_branch_is_accepted_only_by_its_authenticated_release() {
    ...
    let authorization_statement = KagemushaAcceptanceIntentAuthorizationStatementV1 {
        version: KAGEMUSHA_WIRE_VERSION_V1,
        intent,
        release_id,
        suite_id: predecessor.suite_id,
    ...
    let eq_initial =
        initial_kagemusha_eq_accumulator_v1(&eq_parameters).expect("initial Eq history");
    let ep_initial =
        initial_kagemusha_ep_accumulator_v1(&ep_parameters).expect("initial Ep history");
    ...
    let mut enabled_profiles =
        [[0; 32]; super::super::super::KAGEMUSHA_COMMIT_WRAPPER_ENABLED_PROFILE_SLOTS_V1];
    enabled_profiles[0] = predecessor.hardware_profile_id;
    let authorization_public =
        KagemushaCommitWrapperGenerationPublicV1::AcceptanceIntentAuthorization(
            super::super::super::KagemushaAcceptanceIntentAuthorizationGenerationPublicV1 {
    ...
    let generated =
        generate_kagemusha_commit_wrapper_artifacts_v1(authorization_witness.clone())
    ...
    let generated_authorization =
        prove_kagemusha_commit_wrapper_v1(&eq_wrapper, &ep_wrapper, authorization_witness)
            .expect("prove real acceptance-authorization branch");
    let KagemushaGeneratedCommitWrapperEnvelopeV1::AcceptanceIntentAuthorization(
        authorization_proof,
    ...
    let closure_statement = KagemushaNoCommitClosureStatementV1 {
        version: KAGEMUSHA_WIRE_VERSION_V1,
        release_id,
        suite_id: predecessor.suite_id,
        vk_digest: predecessor.vk_digest,
    ...
    let closure_witness = wrapper_witness(
        KagemushaCommitWrapperGenerationPublicV1::NoCommitClosure(
            KagemushaNoCommitClosureGenerationPublicV1 {
    ...
    let generated_closure =
        prove_kagemusha_commit_wrapper_v1(&eq_wrapper, &ep_wrapper, closure_witness)
            .expect("prove real no-commit branch");
    let KagemushaGeneratedCommitWrapperEnvelopeV1::NoCommitClosure(closure) =
