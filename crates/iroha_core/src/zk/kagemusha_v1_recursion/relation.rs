//! Field-neutral fixed-shape arithmetic kernel for all six Kagemusha V1 operations.
//!
//! This is the executable balance/sequence/epoch portion of the final recursive state
//! circuit. It is intentionally not an [`super::KagemushaRecursiveVerifierV1`] backend: the
...
/// Private field-neutral witness for the six-operation arithmetic kernel.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct KagemushaOperationRelationWitnessV1 {
    ...
    pub replay_root_before: KagemushaPastaStateCommitmentV1,
    /// Produced sparse-Merkle replay root.
    pub replay_root_after: KagemushaPastaStateCommitmentV1,
}
...
pub struct KagemushaOperationRelationCircuitV1<F> {
    witness: Option<KagemushaOperationRelationWitnessV1>,
...
impl<F> KagemushaOperationRelationCircuitV1<F> {
    ...
    #[must_use]
    pub const fn new(witness: KagemushaOperationRelationWitnessV1) -> Self {
        Self {
...
{
    type Config = KagemushaOperationRelationConfigV1;
    ...
    fn configure(meta: &mut ConstraintSystem<F>) -> Self::Config {
        ...
        meta.create_gate("Kagemusha V1 six-operation relation", |meta| {
            ...
            constraints.extend([
                ...
                q.clone() * rotate.clone() * amount,
                q.clone() * rotate.clone() * (balance_after - balance_before),
                q.clone()
        ...
        meta.create_gate("Kagemusha V1 u128 bit", |meta| {
        ...
        meta.create_gate("Kagemusha V1 u128 accumulator", |meta| {
        ...
        KagemushaOperationRelationConfigV1 {
            main,
            range_bit,
            range_accumulator,
            operation_instance,
...
mod tests {
    ...
    fn root(value: u64) -> KagemushaPastaStateCommitmentV1 {
    ...
    fn witness(operation: KagemushaOperationV1) -> KagemushaOperationRelationWitnessV1 {
        let mut witness = KagemushaOperationRelationWitnessV1 {
            operation,
            balance_before: 9,
            balance_after: 9,
            amount: 0,
        ...
        match operation {
            KagemushaOperationV1::Bootstrap => {
                ...
                witness.replay_root_before = KagemushaPastaStateCommitmentV1::ZERO;
            }
    ...
    #[test]
    fn all_six_relations_are_executable() {
        for operation in [
            KagemushaOperationV1::Bootstrap,
    ...
    fn overflow_and_zero_monetary_amount_are_rejected() {
        let mut overflow = witness(KagemushaOperationV1::MintFold);
        overflow.balance_before = u128::MAX;
        overflow.balance_after = 4;
        let prover = MockProver::run(
            12,
            &KagemushaOperationRelationCircuitV1::<Fp>::new(overflow),
            vec![vec![Fp::from(1)]],
        )
        .expect("overflow witness synthesizes before constraint failure");
        assert!(prover.verify().is_err());

        let mut zero = witness(KagemushaOperationV1::SendSplit);
        zero.amount = 0;
        zero.balance_after = zero.balance_before;
        let prover = MockProver::run(
            12,
            &KagemushaOperationRelationCircuitV1::<Fp>::new(zero),
            vec![vec![Fp::from(2)]],
        )
