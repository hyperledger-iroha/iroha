...
/// Compare generated compact-key bytes directly, without another artifact-sized allocation.
struct GeneratedCompactKeyBytesV1<'a> {
    expected: &'a [u8],
    offset: usize,
}

impl std::io::Write for GeneratedCompactKeyBytesV1<'_> {
    fn write(&mut self, bytes: &[u8]) -> std::io::Result<usize> {
        let end = self.offset.checked_add(bytes.len()).ok_or_else(|| {
            std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                "compact key byte count overflow",
            )
        })?;
        if self.expected.get(self.offset..end) != Some(bytes) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                "compact key differs from exact generated bytes",
            ));
        }
        self.offset = end;
        Ok(bytes.len())
...
/// Check the generated compact frame without claiming threshold release authentication.
fn assert_generated_compact_proving_key_bytes_v1<C>(key: &ProvingKey<C>, expected: &[u8])
where
    C: CurveAffine + halo2_proofs::SerdeCurveAffine,
    C::Scalar: halo2_proofs::SerdePrimeField + ff::FromUniformBytes<64>,
{
    let mut canonical = GeneratedCompactKeyBytesV1 {
        expected,
        offset: 0,
    };
    key.write_compact_v1(&mut canonical)
        .expect("canonical compact PK equals exact generated bytes");
    assert_eq!(
        canonical.offset,
        expected.len(),
        "complete compact PK frame"
    );
}

#[test]
fn generated_compact_key_comparison_checks_chunks_content_and_exact_length() {
    use std::io::Write as _;

    let expected = b"exact compact key bytes";
    let mut exact = GeneratedCompactKeyBytesV1 {
        expected,
        offset: 0,
    };
    ...
    let mut wrong = GeneratedCompactKeyBytesV1 {
        expected,
        offset: 0,
    };
    assert_eq!(
    ...
    let mut short = GeneratedCompactKeyBytesV1 {
        expected,
        offset: 0,
    };
    ...
    let mut overflow = GeneratedCompactKeyBytesV1 {
        expected,
        offset: usize::MAX,
    };
    assert_eq!(
...
fn decode_state_keys(
    ...
    let eq_transport_proving_key =
        ProvingKey::read_compact_v1_checked::<_, KagemushaTransportDeciderEqCircuitV1>(
    ...
    assert_generated_compact_proving_key_bytes_v1(
    ...
    let ep_transport_proving_key =
        ProvingKey::read_compact_v1_checked::<_, KagemushaTransportDeciderEpCircuitV1>(
    ...
    let eq_proving_key =
        ProvingKey::read_compact_v1_checked::<_, KagemushaRecursiveStateEqCircuitV1>(
            &mut eq_pk_cursor,
            KAGEMUSHA_RECURSION_IPA_K_V1,
    ...
    let ep_proving_key =
        ProvingKey::read_compact_v1_checked::<_, KagemushaRecursiveStateEpCircuitV1>(
            &mut ep_pk_cursor,
            KAGEMUSHA_RECURSION_IPA_K_V1,
