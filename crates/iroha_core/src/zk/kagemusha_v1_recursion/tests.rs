...
use iroha_data_model::{
    ...
    kagemusha::{
        ...
        KagemushaCommitWrapperProofV1, KagemushaLifecycleBindingV1, KagemushaOperationKindV1,
        KagemushaPastaStateCommitmentV1, kagemusha_liability_pool_id_v1,
    },
};
use snark_verifier::{loader::native::NativeLoader, pcs::ipa::IpaAccumulator};

use super::*;
use crate::zk::kagemusha_v1_state::{ConsumedCreditInsertWitnessV1, CreditIdV1, DigestV1};
...
struct ExactFixtureVerifier {
    ...
    expected_proof: KagemushaCommitWrapperProofV1,
...
fn one_thousand_twenty_four_handoffs_keep_fixed_public_and_wire_shapes() {
    const STATE_PUBLIC_INSTANCE_COUNT: usize =
        kagemusha_state_public_instance_v1::ASSET_SCALE + 1;
...
fn wrapper_caps_and_incoming_binding_are_history_independent() {
    ...
    assert_eq!(KAGEMUSHA_CURRENT_PROOFS_MAX_BYTES_V1, 4_990);
    ...
    let binding =
        kagemusha_incoming_proof_binding_digest_v1(&output, &proof).expect("incoming binding");
    let mut different_terminal_output = output.clone();
    different_terminal_output.terminal_output_binding = digest(0xEF);
    assert_ne!(
        kagemusha_incoming_proof_binding_digest_v1(&different_terminal_output, &proof)
    ...
    assert_eq!(
        kagemusha_incoming_proof_binding_digest_v1(&output, &substituted),
        Err(KagemushaRecursionErrorV1::PublicBindingMismatch)
    );
    let mut different_history = proof;
    different_history.eq_history = eq_history(900).as_bytes().to_vec();
    assert_eq!(
        kagemusha_incoming_proof_binding_digest_v1(&output, &different_history)
...
fn real_k16_native_folds_decide_and_reject_substitution() {
    ...
    let eq_fold =
        fold_kagemusha_eq_accumulators_v1(&eq_params, &eq_current, &eq_predecessor).unwrap();
    ...
    let ep_fold =
        fold_kagemusha_ep_accumulators_v1(&ep_params, &ep_current, &ep_predecessor).unwrap();
