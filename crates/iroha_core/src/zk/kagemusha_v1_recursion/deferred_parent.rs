...
const KAGEMUSHA_PROTOCOL_IDENTITY_DOMAIN_V1: &[u8] =
    b"iroha:kagemusha:v1:compiled-protocol-identity";
const KAGEMUSHA_PROTOCOL_STRUCTURE_VERSION_V1: u32 = 1;
const KAGEMUSHA_PROTOCOL_IDENTITY_VERSION_V1: u32 = 1;
...
pub(super) struct KagemushaOrdinaryProofProfileV1 {
...
fn validate_zk_ipa_succinct_key_v1<C>(
    ...
    proof_kind: KagemushaIpaProofKindV1,
) -> Result<(), Error>
where
    C: CurveAffine,
{
    if !succinct_vk.zk() {
        let label = match proof_kind {
            KagemushaIpaProofKindV1::Ordinary => "ordinary proof",
            KagemushaIpaProofKindV1::Fold => "fold proof",
        };
        return Err(transcript_error(format!(
            "Kagemusha {label} requires a zero-knowledge IPA key"
        )));
    }
    Ok(())
}
...
fn ordinary_ipa_proof_profile_from_counts_v1(
    ...
    let transcript_items = witness_commitments
        ...
        .ok_or_else(|| "Kagemusha proof transcript-item count overflowed".to_owned())?;
    let byte_len = transcript_items
        .checked_mul(32)
        .ok_or_else(|| "Kagemusha proof byte length overflowed".to_owned())?;
    Ok(KagemushaOrdinaryProofProfileV1 {
        witness_commitments,
        quotient_commitments,
        evaluations,
        bgh19_rotation_sets,
...
pub(super) struct KagemushaLoadedParentProtocolV1<'chip, C>
where
    C: CurveAffineExt,
    ...
    C::ScalarExt: BigPrimeField + halo2_base::utils::ScalarField,
{
    if protocol.preprocessed.is_empty()
        ...
        || kagemusha_protocol_structure_digest_v1(protocol, parity).map_err(transcript_error)?
            != fixed_structure_digest
    {
    ...
    let transcript_state = loaded
        ...
        .as_ref()
        .ok_or_else(|| transcript_error("Kagemusha parent protocol has no transcript state"))?;
    ...
    Ok(KagemushaLoadedParentProtocolV1 { protocol: loaded })
}
    ...
    C::ScalarExt: BigPrimeField,
{
    ...
    let structure = kagemusha_protocol_structure_digest_v1(protocol, parity)?;
    let transcript_state = protocol
        .transcript_initial_state
        .ok_or_else(|| "Kagemusha compiled protocol has no transcript state".to_owned())?;
    let mut elements = pasta_poseidon_domain_elements_v1::<C::ScalarExt>(
        KAGEMUSHA_PROTOCOL_IDENTITY_DOMAIN_V1,
        KAGEMUSHA_PROTOCOL_IDENTITY_VERSION_V1,
    );
    ...
    elements.push(C::ScalarExt::from(
        u64::try_from(protocol.preprocessed.len())
            .map_err(|_| "Kagemusha preprocessed point count exceeds u64".to_owned())?,
    ));
    ...
    digest
        .as_ref()
        .try_into()
        .map_err(|_| "Kagemusha protocol digest is not 32 bytes".to_owned())
}

fn parity_tag(parity: KagemushaPastaParityV1) -> u32 {
    match parity {
...
fn append_len(output: &mut Vec<u8>, len: usize, label: &str) -> Result<(), String> {
    output.extend_from_slice(
        &u32::try_from(len)
            .map_err(|_| format!("Kagemusha {label} length exceeds u32"))?
            .to_le_bytes(),
    );
    Ok(())
}
...
fn append_index(output: &mut Vec<u8>, value: usize, label: &str) -> Result<(), String> {
    output.extend_from_slice(
        &u32::try_from(value)
            .map_err(|_| format!("Kagemusha {label} exceeds u32"))?
            .to_le_bytes(),
    );
    Ok(())
}
...
fn append_scalar<F: ff::PrimeField>(output: &mut Vec<u8>, value: F) -> Result<(), String> {
    ...
    if encoding.as_ref().len() != 32 {
        return Err("Kagemusha protocol scalar is not 32 bytes".to_owned());
    }
    ...
    match value {
        ...
        ciborium::value::Value::Map(mut fields) if fields.len() == 1 => {
            ...
            let rotation = i32::try_from(rotation)
                .map_err(|_| "Kagemusha Lagrange rotation exceeds i32".to_owned())?;
...
pub(super) fn kagemusha_protocol_structure_digest_v1<C>(
    protocol: &PlonkProtocol<C>,
    parity: KagemushaPastaParityV1,
) -> Result<[u8; 32], String>
where
    ...
    C::ScalarExt: ff::PrimeField,
{
    ...
    bytes.extend_from_slice(KAGEMUSHA_PROTOCOL_STRUCTURE_DOMAIN_V1);
    bytes.push(0);
    bytes.extend_from_slice(&KAGEMUSHA_PROTOCOL_STRUCTURE_VERSION_V1.to_le_bytes());
    ...
    C::ScalarExt: ff::PrimeField,
{
    if bool::from(point.is_identity()) {
        return Err("Kagemusha compiled protocol point is the identity".to_owned());
    }
    let encoding = point.to_bytes();
    if encoding.as_ref().len() != 32 {
        return Err("Kagemusha compiled protocol point is not 32 bytes".to_owned());
...
pub(super) struct KagemushaDeferredParentOutputV1<C>
where
    C: CurveAffineExt,
    C::Base: BigPrimeField,
    C::ScalarExt: BigPrimeField + halo2_base::utils::ScalarField,
...
where
    ...
    C::ScalarExt: BigPrimeField + halo2_base::utils::ScalarField,
{
    if accumulator.xi.len() != KAGEMUSHA_RECURSION_IPA_K_V1 as usize {
        return Err(Error::InvalidInstances);
    }
    Ok(IpaAccumulator::new(
        accumulator
...
pub(super) const fn accumulator_limb_count() -> usize {
    (KAGEMUSHA_RECURSION_IPA_K_V1 as usize + 1) * 2
}
...
mod tests {
    ...
    fn ordinary_ipa_profile_matches_measured_credential_transcript() {
        ...
        assert!(profile.byte_len > super::super::KAGEMUSHA_PARITY_PROOF_MAX_BYTES_V1);
    }
    ...
    fn ordinary_and_fold_admission_reject_non_zk_ipa_key() {
        ...
        let non_zk = IpaSuccinctVerifyingKey::new(
            Domain::new(
                ...
                root_of_unity::<Fp>(KAGEMUSHA_RECURSION_IPA_K_V1 as usize),
            ),
            EqAffine::identity(),
            EqAffine::identity(),
            None,
        ...
        for kind in [
            KagemushaIpaProofKindV1::Ordinary,
            KagemushaIpaProofKindV1::Fold,
        ] {
