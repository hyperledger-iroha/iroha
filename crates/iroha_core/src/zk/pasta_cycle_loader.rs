...
use super::pasta_dense_msm::{PastaDenseMsmJobsV1, PastaDenseMsmSourceV1};
...
pub(super) fn pasta_poseidon_domain_elements_v1<F: ff::PrimeField>(
    ...
    elements.push(F::from(
        u64::try_from(domain.len()).expect("fixed Pasta Poseidon domain length fits u64"),
    ));
...
pub(super) fn proper_uint_le_bytes<F: BigPrimeField>(
    ...
    let bytes = std::array::from_fn(|index| {
        PastaSha256ByteV1::range_checked(ctx, range, assigned_bytes[index])
    });
...
/// same field-native Poseidon commitment over injective compressed
...
#[derive(Clone, Debug)]
pub(super) struct AssignedDeferredSourceEncodingsV1<C>
where
    C: CurveAffineExt,
    Outer<C>: BigPrimeField,
{
    ...
    Inner<C>: BigPrimeField,
{
    ...
    /// Constrain the complete V1 selector-bound deferred audit for Poseidon.
    pub(super) fn assigned_equation_poseidon_elements_v1(
        &self,
    ...
    pub(super) fn assigned_point_bytes(
        ...
        ctx: &mut ScalarContext<C>,
        point: &DeferredScalarPoint<C>,
    ) -> Result<[PastaSha256ByteV1<Inner<C>>; 32], Error> {
        if bool::from(point.value.is_identity()) {
            return Err(Error::Transcript(
                std::io::ErrorKind::InvalidData,
    ...
    /// Constrain the degree-parameterized V1 accumulator representation.
    ///
    ...
    pub(super) fn assigned_accumulator_instance_limbs_v1(
        &self,
        ctx: &mut ScalarContext<C>,
    ...
    Inner<C>: BigPrimeField,
{
    ...
    /// be committed with the field-native V1 Poseidon construction; only then may
    /// [`Self::constrain_deferred_equation_batch_v1`] derive its unpredictable batching challenge
    /// from that digest.
    ...
    /// commits every source, selector, tag, source index, and coefficient. Rejecting zero gives the
    /// batching powers a non-degenerate Fiat-Shamir challenge without routing monetary authority
    /// through a second hash or a lossy cross-field conversion.
    fn deferred_batch_challenge_v1(
        &self,
        ctx: &mut SinglePhaseCoreManager<Outer<C>>,
        audit_digest: &Integer<C>,
    ) -> Result<Integer<C>, String> {
        let challenge: Integer<C> = self
            .scalar
            .field
            .enforce_less_than(ctx.main(), audit_digest.clone())
            .into();
        let is_zero = self.scalar.field.is_zero(ctx.main(), challenge.clone());
        self.base
            .gate()
            .assert_is_const(ctx.main(), &is_zero, &Outer::<C>::ZERO);
        Ok(challenge)
    }
    /// Accumulate all selector-gated equations by stable source index.
    fn deferred_equation_batch_coefficients_v1(
        &self,
        ctx: &mut SinglePhaseCoreManager<Outer<C>>,
    ...
    pub(super) fn constrain_deferred_equation_batch_v1(
        &mut self,
        ctx: &mut SinglePhaseCoreManager<Outer<C>>,
        audit: &AssignedDeferredPointAudit<C>,
        selectors: &[AssignedValue<Outer<C>>],
    ...
    {
        let by_source =
            self.deferred_equation_batch_coefficients_v1(ctx, audit, selectors, audit_digest)?;
    ...
    /// Constrain the complete reciprocal V1 audit Poseidon input and retain
    /// its source encodings for the V1 protocol-identity commitment.
    pub(super) fn assigned_equation_poseidon_elements_v1(
        &self,
        ctx: &mut SinglePhaseCoreManager<Outer<C>>,
        audit: &AssignedDeferredPointAudit<C>,
        gate_tags: &[u32],
        selectors: &[AssignedValue<Outer<C>>],
    ) -> Result<(Vec<Integer<C>>, AssignedDeferredSourceEncodingsV1<C>), String> {
        if audit.sources.is_empty()
            || audit.source_values.len() != audit.sources.len()
            || audit.equations.is_empty()
        ...
        for source in &audit.sources {
            let encoding = self.assigned_point_poseidon_elements_v1(ctx, source)?;
        ...
        Ok((
            elements,
            AssignedDeferredSourceEncodingsV1 {
...
pub(super) fn constrain_reciprocal_poseidon_v1<'chip, C>(
    ...
    base: &'chip FpChip<'chip, C::Base, C::Base>,
    scalar: &'chip FpChip<'chip, C::Base, C::ScalarExt>,
    elements: Vec<ProperCrtUint<C::Base>>,
) -> ProperCrtUint<C::Base>
where
    C: CurveAffineExt,
    ...
    C::ScalarExt: BigPrimeField,
{
    ...
    let loader = Halo2Loader::new(
        PastaCycleEccChip::<C>::new(base, scalar),
        std::mem::take(ctx),
    );
    ...
    let digest = {
        let mut poseidon =
            Poseidon::<C::ScalarExt, Scalar<C, PastaCycleEccChip<'chip, C>>, 3, 2>::new::<8, 57, 0>(
                &loader,
            );
...
mod tests {
    ...
    fn assigned_preimage_bytes<F: BigPrimeField>(bytes: &[PastaSha256ByteV1<F>]) -> Vec<u8> {
        bytes
            .iter()
            .copied()
        ...
        Inner<C>: BigPrimeField,
    {
        ...
        let (elements, _) = chip
            .assigned_equation_poseidon_elements_v1(&mut ctx, &audit, &gate_tags, &selectors)
            .expect("fixed reciprocal V1 audit");
        let poseidon = constrain_reciprocal_poseidon_v1::<C>(&mut ctx, &base, &scalar, elements);
        chip.constrain_deferred_equation_batch_generic_v1(&mut ctx, &audit, &selectors, &poseidon)
            .expect("fixed reciprocal V1 batch");
        *builder.pool(0) = ctx;
    ...
    fn reciprocal_source_encoding_fixture<C>(
        ...
        BaseCircuitBuilder<Outer<C>>,
        AssignedDeferredSourceEncodingsV1<C>,
    )
    where
        C: CurveAffineExt,
        Outer<C>: BigPrimeField,
        Inner<C>: BigPrimeField,
    {
        ...
        let (_, source_encodings) = chip
            .assigned_equation_poseidon_elements_v1(&mut ctx, &audit, &[0], &[selector])
            .expect("fixed reciprocal V1 source encodings");
        *builder.pool(0) = ctx;
        builder.calculate_params(Some(9));
    ...
    #[test]
    fn reciprocal_v1_source_map_reuses_native_chunks_in_both_pasta_parities() {
        fn check<C>()
        where
            C: CurveAffineExt,
            Outer<C>: BigPrimeField,
            Inner<C>: BigPrimeField,
        {
            ...
            let mapped = source_encodings
                .mapped_poseidon_elements_v1(&[generator, tripled], &[0, 2])
                .expect("strict V1 reciprocal source map");
                ...
                for (half, actual) in mapped_point.iter().enumerate() {
                    ...
                    assert_eq!(
                        ...
                        source_encodings.poseidon_elements[source_index][half]
                            .native()
                            .cell,
                        "V1 must retain the exact V1-assigned chunk cell"
                    );
                }
            }
            MockProver::run(builder.config_params.k as u32, &builder, vec![])
                .expect("V1 reciprocal source reuse mock prover")
                .assert_satisfied();
        }
        check::<EqAffine>();
        check::<EpAffine>();
    ...
    #[test]
    fn reciprocal_v1_source_map_rejects_ambiguous_or_mismatched_indices() {
        let generator = EqAffine::generator();
        let doubled = (generator.to_curve() + generator.to_curve()).to_affine();
        let tripled = (doubled.to_curve() + generator.to_curve()).to_affine();
        ...
        assert!(
            source_encodings
                .mapped_poseidon_elements_v1(&[tripled, generator], &[2, 0])
                .is_err(),
            "reordered source indices must fail"
        );
        assert!(
            source_encodings
                .mapped_poseidon_elements_v1(&[generator, generator], &[0, 0])
                .is_err(),
            "duplicate source indices must fail"
        );
        assert!(
            source_encodings
                .mapped_poseidon_elements_v1(&[generator, tripled], &[0, 3])
                .is_err(),
            "out-of-range source indices must fail"
        );
        assert!(
            source_encodings
                .mapped_poseidon_elements_v1(&[generator, doubled], &[0, 2])
                .is_err(),
            "host points must match their V1 audit sources"
        );
        assert!(
            source_encodings
                .mapped_poseidon_elements_v1(&[generator, tripled], &[0])
                .is_err(),
