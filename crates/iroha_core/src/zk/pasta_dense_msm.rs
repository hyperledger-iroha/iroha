...
pub(crate) struct PastaDenseMsmConfigV1 {
    lanes: [PastaDenseMsmLaneConfigV1; DENSE_LANES],
}
#[derive(Clone, Debug)]
...
impl PastaDenseMsmConfigV1 {
        ...
        Base<C>: BigPrimeField + WithSmallOrderMulGroup<3>,
    {
        ...
        meta.create_gate("Paired Pasta dense MSM machine", |meta| {
        ...
        PastaDenseMsmLaneConfigV1 { columns, q_enable }
    }
}
...
fn dense_lane_count(source_count: usize) -> Result<usize, String> {
    ...
    if lanes > DENSE_LANES {
        return Err(format!(
            "Paired Pasta dense MSM requires {lanes} physical lanes for {source_count} sources; only {DENSE_LANES} are configured"
        ));
    }
    Ok(lanes)
}
...
mod tests {
    ...
    fn configuration_stays_at_degree_five() {
        ...
        let _config = PastaDenseMsmConfigV1::configure::<EqAffine>(&mut meta);
    ...
    fn k16_lane_geometry_enforces_the_authenticated_capacity() {
        assert_eq!(K16_MAX_SOURCES_PER_LANE, 390);
        assert_eq!(dense_lane_count(1_170), Ok(3));
        let shard_rows = (0..3)
            .map(|lane| {
                let (start, end) = dense_shard_bounds(1_170, 3, lane);
        ...
        assert!(dense_lane_count(1_171).is_err());
        ...
        let jobs = PastaDenseMsmJobsV1 {
            jobs: vec![DenseMsmJob {
                ...
                source_count_tags: vec![
                    assigned(Fq::from(390)),
                    assigned(Fq::from(390)),
                    assigned(Fq::from(390)),
                ],
                sources: vec![unit_scalar_source(point); 1_170],
            }],
            use_unknown: false,
        };
        assert_eq!(jobs.capacity_profile(), Ok((1, 1_170, 65_523)));
    }
