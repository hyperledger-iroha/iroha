...
use iroha_data_model::offline::{
    ...
    OFFLINE_CASH_HARDWARE_REQUIRED_CAPABILITIES_V1, OFFLINE_CASH_WIRE_VERSION_V1,
    OfflineCashDevicePublicKeyV1, offline_cash_device_key_reference_v1,
};
use sha2::{Digest as _, Sha256};

use super::{DigestV1, OfflineCashNormalizedGuardStatementV1, OfflineCashOperationV1};
#[cfg(feature = "zk-halo2-ipa")]
use super::{
    OfflineCashEpAccumulatorV1, OfflineCashEpFoldProofV1, OfflineCashEqAccumulatorV1,
    OfflineCashEqFoldProofV1, OfflineCashPastaParityV1,
};
#[cfg(feature = "zk-halo2-ipa")]
use crate::zk::pasta_dense_msm::{PastaDenseMsmConfigV1, PastaDenseMsmJobsV1};
use crate::zk::{
    offline_cash_v1_poseidon::{OfflineCashPoseidonFieldV1, from_u128},
...
pub const OFFLINE_CASH_HARDWARE_POLICY_TREE_DEPTH_V1: usize = 16;

const MINIMUM_UNUSABLE_ROWS: usize = 9;
const DEVICE_KEY_REFERENCE_DOMAIN: &[u8] = b"iroha:offline-cash:v1:device-key-reference";
const DEVICE_AUTHORITY_DOMAIN: &[u8] = b"iroha:offline-cash:v1:device-proof-authority";
const PROVIDER_AUTHORITY_DOMAIN: &[u8] = b"iroha:offline-cash:v1:provider-proof-authority";
const POLICY_LEAF_DOMAIN: &[u8] = b"iroha:offline-cash:v1:hardware-policy-leaf";
const POLICY_NODE_DOMAIN: &[u8] = b"iroha:offline-cash:v1:hardware-policy-node";
const CREDENTIAL_STATEMENT_DOMAIN: &[u8] = b"iroha:offline-cash:v1:platform-credential-statement";

pub(super) fn normalized_guard_statement_digest_v1(
    statement: &super::OfflineCashNormalizedGuardStatementV1,
) -> DigestV1 {
...
const fn operation_tag(operation: super::OfflineCashOperationV1) -> u8 {
    match operation {
...
pub struct OfflineCashPlatformCredentialStatementV1 {
    /// Credential relation version.
    pub version: u16,
    /// Exact Offline Cash protocol version authorized by the profile.
...
impl OfflineCashPlatformCredentialStatementV1 {
    ...
    pub(super) fn validate(&self) -> Result<(), String> {
        self.device_public_key
            .validate()
            .map_err(|error| format!("invalid Offline Cash credential device key: {error}"))?;
        if self.version != OFFLINE_CASH_WIRE_VERSION_V1
            ...
            .contains(&[0; 32])
            || self.key_reference != offline_cash_device_key_reference_v1(&self.device_public_key)
        {
            return Err("invalid Offline Cash platform credential statement".to_owned());
        }
        Ok(())
    }
}
...
pub struct OfflineCashPlatformCredentialRelationWitnessV1 {
    /// Public semantic credential statement.
    pub statement: OfflineCashPlatformCredentialStatementV1,
    /// Provider-only proof-authority secret.
    pub provider_authority_secret: DigestV1,
    /// Bottom-up provider-profile policy path.
    pub policy_siblings: [DigestV1; OFFLINE_CASH_HARDWARE_POLICY_TREE_DEPTH_V1],
}

impl OfflineCashPlatformCredentialRelationWitnessV1 {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        {
            return Err("invalid Offline Cash provider credential authority".to_owned());
        }
        Ok(())
    }
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct OfflineCashGuardBundleRelationWitnessV1 {
    /// Canonical fixed-layout transition statement.
    pub statement: OfflineCashNormalizedGuardStatementV1,
    /// Release-pinned canonical empty inbox/outbox effect.
    pub canonical_empty_effect_digest: DigestV1,
    /// Credential authorizing consumption of the predecessor hardware epoch.
    pub predecessor_credential: OfflineCashPlatformCredentialStatementV1,
    /// Credential binding the successor hardware epoch.
    pub successor_credential: OfflineCashPlatformCredentialStatementV1,
...
impl OfflineCashGuardBundleRelationWitnessV1 {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        {
            return Err("invalid Offline Cash GuardBundle device authority".to_owned());
        }
        ...
        match self.statement.operation {
            OfflineCashOperationV1::Bootstrap => {
                ...
                {
                    return Err(
                        "Offline Cash bootstrap credential slots must be identical".to_owned()
                    );
                }
            }
            OfflineCashOperationV1::Rotate => {
                ...
                if self.statement.successor_hardware_epoch_generation
                    != self
                        ...
                        .predecessor_hardware_epoch_generation
                        .checked_add(1)
                        .ok_or_else(|| "Offline Cash hardware epoch overflow".to_owned())?
                ...
                {
                    return Err("invalid Offline Cash hardware rotation".to_owned());
                }
            }
            OfflineCashOperationV1::SuiteUpgrade => {
                ...
                if self.predecessor_device_authority_secret
                    ...
                    || self.predecessor_credential == self.successor_credential
                {
                    return Err("invalid Offline Cash suite-upgrade credential bridge".to_owned());
                }
            }
            _ => {
                validate_predecessor_credential_binding(
                ...
                if self.predecessor_credential != self.successor_credential
                    ...
                    || self.statement.predecessor_hardware_policy_id
                        != self.statement.successor_hardware_policy_id
                {
                    return Err("non-rotation changed the Offline Cash credential".to_owned());
                }
            }
        }
        Ok(())
...
fn validate_common_credential_binding(
    guard: &OfflineCashNormalizedGuardStatementV1,
    canonical_empty_effect_digest: DigestV1,
    credential: &OfflineCashPlatformCredentialStatementV1,
) -> Result<(), String> {
    ...
    {
        return Err("Offline Cash credential/GuardBundle lane binding mismatch".to_owned());
    }
    Ok(())
}
...
fn validate_predecessor_credential_binding(
    ...
    {
        return Err("Offline Cash predecessor credential binding mismatch".to_owned());
    }
    Ok(())
}
...
fn validate_successor_credential_binding(
    ...
    {
        return Err("Offline Cash successor credential binding mismatch".to_owned());
    }
    Ok(())
}
...
#[derive(Clone, Debug)]
pub struct OfflineCashPlatformCredentialRelationCircuitV1<F> {
    witness: Option<OfflineCashPlatformCredentialRelationWitnessV1>,
    marker: core::marker::PhantomData<F>,
}

impl<F> Default for OfflineCashPlatformCredentialRelationCircuitV1<F> {
    fn default() -> Self {
        Self {
            witness: None,
...
impl<F> OfflineCashPlatformCredentialRelationCircuitV1<F>
where
    F: OfflineCashPoseidonFieldV1,
{
    /// Construct a witnessed credential circuit after complete structural validation.
    pub fn new(witness: OfflineCashPlatformCredentialRelationWitnessV1) -> Result<Self, String> {
        witness.validate()?;
        Ok(Self {
    ...
    pub fn public_instances(&self) -> Result<Vec<F>, String> {
        let witness = self
            .witness
            .as_ref()
            .ok_or_else(|| "Offline Cash credential circuit has no witness".to_owned())?;
        witness.validate()?;
...
#[derive(Clone, Debug)]
pub struct OfflineCashPlatformCredentialCircuitConfigV1<F: OfflineCashPoseidonFieldV1> {
    base: BaseConfig<F>,
    sha: PastaSha256ConfigV1,
}

impl<F> Circuit<F> for OfflineCashPlatformCredentialRelationCircuitV1<F>
where
    F: OfflineCashPoseidonFieldV1,
{
    type Config = OfflineCashPlatformCredentialCircuitConfigV1<F>;
    ...
    fn params(&self) -> Self::Params {
        credential_builder::<F>(self.witness.as_ref())
            .expect("validated Offline Cash credential witness")
            .0
            .config_params
    }
    ...
    fn configure_with_params(meta: &mut ConstraintSystem<F>, params: Self::Params) -> Self::Config {
        ...
        OfflineCashPlatformCredentialCircuitConfigV1 {
            base,
    ...
    fn configure(_: &mut ConstraintSystem<F>) -> Self::Config {
        unreachable!("Offline Cash credential circuit uses authenticated Base parameters")
    }
    ...
    fn synthesize(
        ...
        <BaseCircuitBuilder<F> as Circuit<F>>::synthesize(
            &builder,
            config.base,
            layouter.namespace(|| "Offline Cash credential base relation"),
        )?;
        jobs.synthesize(
            &config.sha,
            &mut layouter,
            &builder.core().copy_manager,
            (1_usize << OFFLINE_CASH_HALO2_K_V1) - MINIMUM_UNUSABLE_ROWS,
        )
    }
}
    ...
    F: OfflineCashPoseidonFieldV1,
{
    ...
    let siblings = witness.map_or(
        [[0; 32]; OFFLINE_CASH_HARDWARE_POLICY_TREE_DEPTH_V1],
    ...
    jobs.validate_capacity((1_usize << OFFLINE_CASH_HALO2_K_V1) - MINIMUM_UNUSABLE_ROWS)?;
    Ok((builder, jobs))
}

/// Assigned semantic outputs consumed by the recursive GuardBundle wrapper.
pub(super) struct OfflineCashAssignedGuardBundleV1<F: OfflineCashPoseidonFieldV1> {
...
#[derive(Clone)]
pub(super) struct AssignedCredentialV1<F: OfflineCashPoseidonFieldV1> {
...
pub(super) fn constrain_guard_bundle_semantics_v1<F>(
    ...
    witness: &OfflineCashGuardBundleRelationWitnessV1,
) -> Result<OfflineCashAssignedGuardBundleV1<F>, String>
where
    F: OfflineCashPoseidonFieldV1,
{
    witness.validate()?;
    ...
    Ok(OfflineCashAssignedGuardBundleV1 {
        guard_digest,
...
pub(super) fn assign_credential_statement_v1<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    range: &RangeChip<F>,
    jobs: &mut PastaSha256JobsV1<F>,
    statement: &OfflineCashPlatformCredentialStatementV1,
...
#[cfg(feature = "zk-halo2-ipa")]
pub(crate) struct OfflineCashGuardBundleRecursiveWitnessV1<'a> {
    /// Complete normalized guard and device-authority witness.
    pub relation: OfflineCashGuardBundleRelationWitnessV1,
    ...
    pub eq_credential_fold_proof: &'a OfflineCashEqFoldProofV1,
    ...
    pub ep_credential_fold_proof: &'a OfflineCashEpFoldProofV1,
...
#[cfg(feature = "zk-halo2-ipa")]
#[derive(Clone, Debug)]
pub(crate) struct OfflineCashGuardBundleCircuitConfigV1<F: halo2_base::utils::ScalarField> {
    base: BaseConfig<F>,
    sha: PastaSha256ConfigV1,
...
#[cfg(feature = "zk-halo2-ipa")]
#[derive(Clone)]
pub(crate) struct OfflineCashGuardBundleEqCircuitV1 {
...
#[cfg(feature = "zk-halo2-ipa")]
#[derive(Clone)]
pub(crate) struct OfflineCashGuardBundleEpCircuitV1 {
...
macro_rules! impl_guard_bundle_circuit {
    ($circuit:ty, $field:ty, $opposite:ty, $label:literal) => {
        impl Circuit<$field> for $circuit {
            type Config = OfflineCashGuardBundleCircuitConfigV1<$field>;
            type FloorPlanner = V1;
            type Params = BaseCircuitParams;
            ...
            fn configure_with_params(
                ...
                let mut base = BaseConfig::configure(meta, params);
                base.set_usable_rows(usable_rows);
                OfflineCashGuardBundleCircuitConfigV1 {
                    base,
                    sha: PastaSha256ConfigV1::configure(meta),
...
impl_guard_bundle_circuit!(
    ...
    Fp,
    EpAffine,
    "Offline Cash Eq GuardBundle"
);
#[cfg(feature = "zk-halo2-ipa")]
impl_guard_bundle_circuit!(
    OfflineCashGuardBundleEpCircuitV1,
    Fq,
    EqAffine,
    "Offline Cash Ep GuardBundle"
);

/// Build the two mutually audited GuardBundle circuits.
#[cfg(feature = "zk-halo2-ipa")]
pub(crate) fn build_offline_cash_guard_bundle_pair_v1(
    eq_svk: &IpaSuccinctVerifyingKey<EqAffine>,
    ep_svk: &IpaSuccinctVerifyingKey<EpAffine>,
    witness: OfflineCashGuardBundleRecursiveWitnessV1<'_>,
) -> Result<
    (
        OfflineCashGuardBundleEqCircuitV1,
        OfflineCashGuardBundleEpCircuitV1,
...
> {
    ...
    if witness.eq_credential_audit == [0; 32]
        ...
        || crate::zk::offline_cash_v1_poseidon::decode::<Fp>(witness.eq_credential_audit).is_none()
        || crate::zk::offline_cash_v1_poseidon::decode::<Fq>(witness.ep_credential_audit).is_none()
    {
        return Err("Offline Cash GuardBundle credential audit is noncanonical".to_owned());
    }
...
fn build_guard_scalar_half_v1<C>(
    ...
    credential_history: &[u8; super::OFFLINE_CASH_HISTORY_ACCUMULATOR_BYTES_V1],
    eq_audit: DigestV1,
    ep_audit: DigestV1,
) -> Result<
    (
    ...
    C::ScalarExt: OfflineCashPoseidonFieldV1,
{
    ...
    if credential_history_instances.len() != accumulator_limb_count() {
        return Err("Offline Cash GuardBundle credential history is not fixed".to_owned());
    }
    ...
    for (instances, proof) in credential_instances.into_iter().zip(credential_proofs) {
        ...
        current_credentials.push(
            verify_ordinary_proof_v1(&loader, succinct_vk, &loaded_protocol, &[instances], proof)
                .map_err(|error| {
                format!("Offline Cash credential scalar verifier failed: {error:?}")
            })?,
        );
    }
    ...
    .map_err(|error| format!("Offline Cash credential fold failed: {error:?}"))?;
    bind_accumulator_limbs(&loader, &folded_credentials, &credential_history_instances)
        .map_err(|error| format!("Offline Cash credential history binding failed: {error:?}"))?;
    let output =
        finalize_tagged_deferred_audit_v1(&mut builder, loader, GUARD_CREDENTIAL_EQUATION_TAG_V1)
            .map_err(|error| format!("Offline Cash credential audit failed: {error:?}"))?;
    let expected_offset = match parity {
        OfflineCashPastaParityV1::Eq => GUARD_EQ_AUDIT_OFFSET_V1,
        OfflineCashPastaParityV1::Ep => GUARD_EP_AUDIT_OFFSET_V1,
    };
...
#[cfg(feature = "zk-halo2-ipa")]
fn guard_audit_cells<F: OfflineCashPoseidonFieldV1>(
    ...
    {
        return Err("Offline Cash GuardBundle public instance has wrong shape".to_owned());
    }
    builder.assigned_instances[0][offset..offset + 2]
        .try_into()
        .map_err(|_| "Offline Cash GuardBundle audit instance has wrong shape".to_owned())
}

#[derive(Clone)]
pub(super) struct AssignedUint<F: OfflineCashPoseidonFieldV1> {
...
pub(super) fn assign_bytes<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    range: &RangeChip<F>,
    bytes: &[u8],
...
pub(super) fn constant_bytes<F: OfflineCashPoseidonFieldV1>(
    bytes: &[u8],
) -> Vec<PastaSha256ByteV1<F>> {
    bytes
        .iter()
...
pub(super) fn hash<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    jobs: &mut PastaSha256JobsV1<F>,
...
pub(super) fn digest_limbs_assigned<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
...
pub(super) fn bind_equal_digest<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    _range: &RangeChip<F>,
...
fn select_digest<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    range: &RangeChip<F>,
...
pub(super) fn assert_digest_nonzero<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    range: &RangeChip<F>,
    digest: &[PastaSha256ByteV1<F>; 32],
) {
...
pub(super) fn assert_bytes_nonzero<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    range: &RangeChip<F>,
...
fn assert_equal_value<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
...
fn assert_if_equal_value<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    range: &RangeChip<F>,
    selector: AssignedValue<F>,
    left: AssignedValue<F>,
...
fn assert_if_zero<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    range: &RangeChip<F>,
    selector: AssignedValue<F>,
    value: AssignedValue<F>,
...
fn assert_if_increment<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    range: &RangeChip<F>,
    selector: AssignedValue<F>,
...
fn assert_if_digest_zero<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    range: &RangeChip<F>,
    selector: AssignedValue<F>,
    digest: &[PastaSha256ByteV1<F>; 32],
...
fn assert_if_digest_nonzero<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    range: &RangeChip<F>,
    selector: AssignedValue<F>,
    digest: &[PastaSha256ByteV1<F>; 32],
...
fn assert_if_digest_equal<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    range: &RangeChip<F>,
    selector: AssignedValue<F>,
    left: &[PastaSha256ByteV1<F>; 32],
...
fn policy_leaf(statement: &OfflineCashPlatformCredentialStatementV1) -> DigestV1 {
    let mut hasher = Sha256::new();
...
fn credential_statement_preimage(statement: &OfflineCashPlatformCredentialStatementV1) -> Vec<u8> {
...
fn blank_credential_statement() -> OfflineCashPlatformCredentialStatementV1 {
    ...
    let key = OfflineCashDevicePublicKeyV1::from_sec1_bytes(&[
...
mod tests {
    ...
    use super::*;
    use crate::zk::offline_cash_v1_poseidon::digest_limbs;

    fn credential_witness() -> OfflineCashPlatformCredentialRelationWitnessV1 {
        ...
        let mut statement = OfflineCashPlatformCredentialStatementV1 {
            ...
            key_reference: offline_cash_device_key_reference_v1(&device_public_key),
            device_public_key,
        ...
        let provisional = OfflineCashPlatformCredentialRelationWitnessV1 {
            statement,
            provider_authority_secret,
            policy_siblings,
        };
        statement.hardware_policy_id = provisional.policy_root();
        OfflineCashPlatformCredentialRelationWitnessV1 {
            statement,
            provider_authority_secret,
            policy_siblings,
        }
    ...
    fn guard_witness() -> OfflineCashGuardBundleRelationWitnessV1 {
    ...
    fn credential_public_digest_is_field_neutral() {
        ...
        assert_eq!(
            OfflineCashPlatformCredentialRelationCircuitV1::<Fp>::new(witness.clone())
        ...
        assert_eq!(
            OfflineCashPlatformCredentialRelationCircuitV1::<Fq>::new(witness)
    ...
    fn credential_relation_is_satisfied_in_both_pasta_fields() {
        let witness = credential_witness();
        let fp = OfflineCashPlatformCredentialRelationCircuitV1::<Fp>::new(witness.clone())
            .expect("Fp credential circuit");
        let fp_instances = fp.public_instances().expect("Fp public instances");
        MockProver::run(OFFLINE_CASH_HALO2_K_V1, &fp, vec![fp_instances])
            .expect("Fp credential prover")
            .assert_satisfied();

        let fq = OfflineCashPlatformCredentialRelationCircuitV1::<Fq>::new(witness)
            .expect("Fq credential circuit");
        let fq_instances = fq.public_instances().expect("Fq public instances");
        MockProver::run(OFFLINE_CASH_HALO2_K_V1, &fq, vec![fq_instances])
