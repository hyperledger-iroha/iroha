...
use iroha_data_model::{
    ...
    isi::offline_cash_v1::{
        ...
        OfflineCashPastaSchnorrSignatureV1, OfflineCashTopUpMembershipWitnessV1,
        offline_cash_mint_finality_peer_id_digest_v1, offline_cash_mint_finality_root_v1,
    },
    offline::OfflineCashMintCreditStatementV1,
};
use norito::codec::{Decode, Encode};
use sha2::{Digest as _, Sha256};

use super::{DigestV1, OfflineCashPastaParityV1};
use crate::zk::{
    offline_cash_v1_poseidon::{
        OfflineCashPoseidonChipV1, OfflineCashPoseidonFieldV1, decode, digest_limbs, from_u128,
    },
...
const MINT_ROOT_BRIDGE_DOMAIN_V1: &[u8] = b"iroha:offline-cash:v1:mint-finality-root";
const MINT_SEAL_MESSAGE_DOMAIN_V1: &[u8] = b"iroha:offline-cash:v1:mint-finality-seal-message";
const MINT_CHALLENGE_DOMAIN_V1: &[u8] = b"iroha:offline-cash:v1:mint-finality:challenge";
const MINT_FINALITY_EPOCH_ROSTER_DOMAIN_V1: &[u8] =
    b"iroha:offline-cash:v1:mint-finality-epoch-roster";
const MINT_CERTIFICATE_BINDING_DOMAIN_V1: &[u8] = b"iroha:offline-cash:v1:mint-certificate-binding";
...
#[repr(u64)]
pub enum OfflineCashMintAuthorityStepV1 {
...
pub(super) const OFFLINE_CASH_MINT_CERTIFICATE_PUBLIC_INSTANCE_COUNT_V1: usize = 5;

/// Private inputs consumed by the fixed-shape mint-certificate component.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct OfflineCashMintCertificateWitnessV1 {
    ...
    pub membership: OfflineCashTopUpMembershipWitnessV1,
    /// Exact current-epoch `2f + 1` paired Pasta seal bundle.
    pub seal_bundle: OfflineCashMintFinalitySealBundleV1,
    ...
    pub epoch_roster: OfflineCashMintFinalityEpochRosterV1,
}

impl OfflineCashMintCertificateWitnessV1 {
    ...
    pub fn validate_shape(&self) -> Result<(), String> {
        self.validate_for_step(OfflineCashMintAuthorityStepV1::FinalizedMint)
    }
    ...
    pub(super) fn validate_for_step(
        ...
        if self.membership.siblings.len() != OFFLINE_CASH_MINT_FINALITY_TREE_DEPTH_V1
            || offline_cash_mint_finality_root_v1(self.membership.root)
                != self.seal_bundle.message.offline_cash_top_up_root
        {
            return Err("mint-finality paired root or path shape differs from seal message".into());
        }
        if step != OfflineCashMintAuthorityStepV1::Bootstrap {
            self.seal_bundle
                .validate()
        ...
        if step == OfflineCashMintAuthorityStepV1::FinalizedMint {
            self.membership
                .validate_against(&self.seal_bundle.message)
                .map_err(|error| format!("invalid top-up membership witness: {error}"))?;
            super::mint_finality::verify_offline_cash_top_up_membership_v1(
                &self.membership,
                self.seal_bundle.message.offline_cash_top_up_count,
            )
            .map_err(|error| error.to_string())?;
        } else if step == OfflineCashMintAuthorityStepV1::Rotate
...
pub(super) struct OfflineCashAssignedMintCertificateV1<F: OfflineCashPoseidonFieldV1> {
...
pub(super) struct OfflineCashMintCertificateJobsV1<C>
where
    C: CurveAffineExt,
...
#[derive(Clone, Debug)]
pub(super) struct OfflineCashMintCertificateCircuitConfigV1<F: halo2_base::utils::ScalarField> {
    base: BaseConfig<F>,
...
#[derive(Clone)]
pub(super) struct OfflineCashMintCertificateEqCircuitV1 {
...
#[derive(Clone)]
pub(super) struct OfflineCashMintCertificateEpCircuitV1 {
...
macro_rules! impl_mint_certificate_circuit {
    ($circuit:ty, $field:ty, $signature_curve:ty, $label:literal) => {
        impl Circuit<$field> for $circuit {
            type Config = OfflineCashMintCertificateCircuitConfigV1<$field>;
            ...
            fn configure_with_params(
                ...
                OfflineCashMintCertificateCircuitConfigV1 {
                    base,
...
impl_mint_certificate_circuit!(
    OfflineCashMintCertificateEqCircuitV1,
    Fp,
    EpAffine,
    "Offline Cash Eq mint certificate"
);
impl_mint_certificate_circuit!(
    OfflineCashMintCertificateEpCircuitV1,
    Fq,
    EqAffine,
    "Offline Cash Ep mint certificate"
);

/// Build both fixed-shape certificate halves from the same exact finalized top-up evidence.
pub(super) fn build_offline_cash_mint_certificate_pair_v1(
    witness: OfflineCashMintCertificateWitnessV1,
) -> Result<
    (
        OfflineCashMintCertificateEqCircuitV1,
        OfflineCashMintCertificateEpCircuitV1,
...
> {
    ...
    let (eq_assigned, eq_jobs) = constrain_offline_cash_mint_certificate_v1::<EpAffine>(
        &mut eq_builder,
        &witness,
    ...
    let (ep_assigned, ep_jobs) = constrain_offline_cash_mint_certificate_v1::<EqAffine>(
        &mut ep_builder,
        &witness,
    ...
    if eq_builder.assigned_instances[0].len()
        != OFFLINE_CASH_MINT_CERTIFICATE_PUBLIC_INSTANCE_COUNT_V1
        || ep_builder.assigned_instances[0].len()
            != OFFLINE_CASH_MINT_CERTIFICATE_PUBLIC_INSTANCE_COUNT_V1
    {
        return Err("Offline Cash mint-certificate public shape drifted".to_owned());
    }
...
fn mint_certificate_builder<F: OfflineCashPoseidonFieldV1>() -> BaseCircuitBuilder<F> {
...
pub(super) fn constrain_offline_cash_mint_certificate_v1<C>(
    ...
    (
        OfflineCashAssignedMintCertificateV1<C::Base>,
        OfflineCashMintCertificateJobsV1<C>,
    ),
    String,
>
where
    C: CurveAffineExt,
    C::Base: OfflineCashPoseidonFieldV1 + WithSmallOrderMulGroup<3>,
    C::ScalarExt: BigPrimeField + WithSmallOrderMulGroup<3> + ReciprocalAffine,
{
    witness.validate_for_step(step)?;
    let expected_eq = matches!(parity, OfflineCashPastaParityV1::Eq);
    ...
    let top_up_count = assign_uint(
        ctx,
        &range,
        u128::from(message.offline_cash_top_up_count),
        32,
    );
    ...
    let signing_digest = sha_digest(
        ...
        [
            ...
            vec![PastaSha256ByteV1::constant(0)],
            constant_bytes(&OFFLINE_CASH_CHAIN_VERSION_V1.to_le_bytes()),
    ...
    for slot in 0..MAX_VALIDATORS_PER_HEIGHT {
        ...
        let (peer_digest, current_key_bytes, reciprocal_key_bytes) = if active_value {
            ...
            let peer_digest = offline_cash_mint_finality_peer_id_digest_v1(&keys.validator)
                .map_err(|error| error.to_string())?;
    ...
    Ok((
        OfflineCashAssignedMintCertificateV1 {
            step,
            bootstrap,
            rotate,
            finalized_mint,
        ...
        OfflineCashMintCertificateJobsV1 { sha, dense },
    ))
}
...
fn assign_digest_limbs<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    range: &RangeChip<F>,
    digest: DigestV1,
) -> [AssignedValue<F>; 2] {
...
fn assign_uint<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    range: &RangeChip<F>,
    value: u128,
    bits: usize,
...
fn uint_bytes_le<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    gate: &halo2_base::gates::GateChip<F>,
...
fn assign_bytes<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    range: &RangeChip<F>,
    bytes: &[u8],
) -> Vec<PastaSha256ByteV1<F>> {
...
fn mask_sha_bytes<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    range: &RangeChip<F>,
...
pub(super) fn sha_digest<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
...
pub(super) fn sha_digest_limbs<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    gate: &halo2_base::gates::GateChip<F>,
    digest: &[PastaSha256ByteV1<F>; 32],
) -> [AssignedValue<F>; 2] {
...
fn mark_iroha_hash<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    gate: &halo2_base::gates::GateChip<F>,
...
fn constrain_equal_if<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    gate: &halo2_base::gates::GateChip<F>,
...
fn assert_bytes_nonzero<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut Context<F>,
    gate: &halo2_base::gates::GateChip<F>,
    bytes: &[PastaSha256ByteV1<F>],
) {
...
fn dummy_signature<C>() -> OfflineCashPastaSchnorrSignatureV1
where
    C: CurveAffine,
    C::ScalarExt: PrimeField,
{
    OfflineCashPastaSchnorrSignatureV1 {
...
where
    C: CurveAffine,
    C::Base: OfflineCashPoseidonFieldV1,
{
    let mut signing = Vec::new();
    signing.extend_from_slice(MINT_SEAL_MESSAGE_DOMAIN_V1);
    signing.push(0);
    signing.extend_from_slice(&OFFLINE_CASH_CHAIN_VERSION_V1.to_le_bytes());
    ...
    signing.extend_from_slice(&message.offline_cash_top_up_count.to_le_bytes());
...
const _: () = {
    assert!(OFFLINE_CASH_MINT_FINALITY_TREE_DEPTH_V1 == 32);
