//! Real paired-Pasta recursive-history qualification for Offline Cash V1.
//!
...
use super::{
    ...
    OFFLINE_CASH_IPA_POSEIDON_WIDTH_V1, OFFLINE_CASH_RECURSION_IPA_K_V1,
    OfflineCashEpAccumulatorV1, OfflineCashEpFoldProofV1, OfflineCashEqAccumulatorV1,
    OfflineCashEqFoldProofV1, OfflineCashGeneratedRecursiveStateProofV1,
    OfflineCashGuardBundleRelationWitnessV1, OfflineCashLoadedEpRecursiveStateArtifactsV1,
    OfflineCashLoadedEqRecursiveStateArtifactsV1, OfflineCashNormalizedGuardStatementV1,
    OfflineCashOperationV1, OfflineCashPastaParityV1,
    OfflineCashPlatformCredentialRelationCircuitV1, OfflineCashPlatformCredentialRelationWitnessV1,
    OfflineCashPlatformCredentialStatementV1, OfflineCashRecursiveIncomingEpGenerationWitnessV1,
    OfflineCashRecursiveIncomingEqGenerationWitnessV1,
    OfflineCashRecursiveStateGenerationWitnessV1, OfflineCashStateRelationWitnessV1,
    OfflineCashSuiteUpgradeGenerationWitnessV1,
    commit_wrapper::COMMIT_WRAPPER_PUBLIC_INSTANCE_COUNT_V1,
    composite::{
        OfflineCashRecursiveStateEpCircuitV1, OfflineCashRecursiveStateEqCircuitV1, ep_succinct_vk,
        eq_succinct_vk,
    },
    deferred_parent::{
    ...
    guard_bundle::{
        ...
        OfflineCashGuardBundleEqCircuitV1, OfflineCashGuardBundleRecursiveWitnessV1,
        build_offline_cash_guard_bundle_pair_v1, device_authority_commitment_v1,
    },
    initial_offline_cash_ep_accumulator_v1, initial_offline_cash_eq_accumulator_v1,
    mint_authority::OFFLINE_CASH_MINT_AUTHORITY_PUBLIC_INSTANCE_COUNT_V1,
...
use crate::zk::{
    offline_cash_v1_poseidon::{
        OFFLINE_CASH_STATE_DOMAIN_V1, OfflineCashPoseidonFieldV1, decode as decode_pasta,
        digest_limbs, empty_replay_root, encode as encode_pasta, from_u128, hash as pasta_hash,
    },
    offline_cash_v1_state::{
        DevicePolicyBindingV1, DigestV1, HardwareEpochV1, OFFLINE_CASH_STATE_VERSION_V1,
        OfflineCashLaneIdV1, OfflineCashStateV1,
    },
};
...
const RECURSIVE_PUBLIC_INSTANCE_COUNT: usize =
    PUBLIC_INSTANCE_COUNT + OFFLINE_CASH_HISTORY_ACCUMULATOR_BYTES_V1 / 16;

fn digest(label: &[u8], index: u64) -> DigestV1 {
    let mut hasher = Sha256::new();
    hasher.update(b"iroha:offline-cash:v1:real-recursion-qualification");
    hasher.update([0]);
    hasher.update(label);
...
fn network() -> NetworkId {
    NetworkId::from_genesis_hash(HashOf::<BlockHeader>::from_untyped_unchecked(Hash::new(
        b"offline-cash-v1-real-recursion-qualification",
    )))
}
...
fn incarnation() -> AxtAssetIncarnationV1 {
    ...
    let registration = HashOf::<BlockHeader>::from_untyped_unchecked(Hash::new(
        b"offline-cash-v1-real-recursion-registration",
    ));
    AxtAssetIncarnationV1::derive(
        &network,
        &asset,
        &registration,
        &Hash::new(b"offline-cash-v1-real-recursion-execution"),
        1,
    )
}
...
pub(super) fn credential_witness(
...
) -> (OfflineCashPlatformCredentialRelationWitnessV1, DigestV1) {
    ...
    let mut statement = OfflineCashPlatformCredentialStatementV1 {
        ...
        asset_id: offline_cash_asset_identity_digest_v1(&lane.asset)
    ...
    let witness = OfflineCashPlatformCredentialRelationWitnessV1 {
        statement,
...
fn empty_replay_root_pair() -> OfflineCashPastaStateCommitmentV1 {
...
fn state_component<F: OfflineCashPoseidonFieldV1>(
    ...
    inputs.extend(digest_limbs::<F>(
        offline_cash_asset_identity_digest_v1(&state.lane.asset).expect("canonical state asset"),
    ));
    ...
    encode_pasta(pasta_hash(OFFLINE_CASH_STATE_DOMAIN_V1, &inputs))
}
...
pub(super) fn aggregate_state_with_balance(
    ...
    let mut state = OfflineCashStateV1 {
    ...
    state.state_commitment_components = OfflineCashPastaStateCommitmentV1 {
    ...
    state.state_commitment =
        offline_cash_pasta_state_commitment_v1(state.state_commitment_components);
...
pub(super) fn create_eq_proof<C: Circuit<Fp>>(
    ...
    let mut transcript =
        EqTranscript::new::<OFFLINE_CASH_IPA_POSEIDON_SECURE_MDS_V1>(Vec::<u8>::new());
    create_proof::<
...
pub(super) fn create_ep_proof<C: Circuit<Fq>>(
    ...
    let mut transcript =
        EpTranscript::new::<OFFLINE_CASH_IPA_POSEIDON_SECURE_MDS_V1>(Vec::<u8>::new());
    create_proof::<
...
fn dummy_ordinary_proof<C: CurveAffine>(protocol: &PlonkProtocol<C>, point: C) -> Vec<u8> {
    ...
    proof.extend_from_slice(point.as_ref());
    for _ in 0..(2 * OFFLINE_CASH_RECURSION_IPA_K_V1 as usize) {
        proof.extend_from_slice(point.as_ref());
    }
    proof.extend_from_slice(&scalar);
    proof.extend_from_slice(&scalar);
...
fn dummy_fold_bytes<C: CurveAffine>(point: C) -> Vec<u8> {
    let point = point.to_bytes();
    let scalar = [0_u8; 32];
    let mut proof = Vec::with_capacity(OFFLINE_CASH_IPA_FOLD_PROOF_BYTES_V1);
    proof.extend_from_slice(&scalar);
    proof.extend_from_slice(&scalar);
    proof.extend_from_slice(point.as_ref());
    proof.extend_from_slice(&scalar);
    ...
    proof.extend_from_slice(point.as_ref());
    proof.extend_from_slice(&scalar);
    assert_eq!(proof.len(), OFFLINE_CASH_IPA_FOLD_PROOF_BYTES_V1);
    proof
}

fn dummy_eq_fold() -> OfflineCashEqFoldProofV1 {
    OfflineCashEqFoldProofV1::try_from_bytes(&dummy_fold_bytes(EqAffine::generator()))
        .expect("fixed-shape Eq fold parser witness")
}

fn dummy_ep_fold() -> OfflineCashEpFoldProofV1 {
    OfflineCashEpFoldProofV1::try_from_bytes(&dummy_fold_bytes(EpAffine::generator()))
        .expect("fixed-shape Ep fold parser witness")
}

fn history_instances<F: OfflineCashPoseidonFieldV1>(
    prefix_len: usize,
...
struct CredentialProof {
    relation: OfflineCashPlatformCredentialRelationWitnessV1,
...
impl CredentialKeys {
    fn generate(
        ...
        witness: &OfflineCashPlatformCredentialRelationWitnessV1,
    ) -> Self {
        let eq_circuit = OfflineCashPlatformCredentialRelationCircuitV1::<Fp>::new(witness.clone())
            .expect("Eq credential circuit");
        let ep_circuit = OfflineCashPlatformCredentialRelationCircuitV1::<Fq>::new(witness.clone())
    ...
    fn prove(
        ...
        let eq_circuit =
            OfflineCashPlatformCredentialRelationCircuitV1::<Fp>::new(relation.clone())
                .expect("Eq credential circuit");
        let ep_circuit =
            OfflineCashPlatformCredentialRelationCircuitV1::<Fq>::new(relation.clone())
                .expect("Ep credential circuit");
...
fn bootstrap_guard_relation(
    ...
    let statement = OfflineCashNormalizedGuardStatementV1 {
        version: OFFLINE_CASH_WIRE_VERSION_V1,
        protocol_version: OFFLINE_CASH_WIRE_VERSION_V1,
        ...
        asset_id: offline_cash_asset_identity_digest_v1(&state.lane.asset)
...
fn rotation_guard_relation(
    ...
    let relation = OfflineCashGuardBundleRelationWitnessV1 {
        statement: OfflineCashNormalizedGuardStatementV1 {
            ...
            operation: OfflineCashOperationV1::Rotate,
            amount: 0,
            peer_credit_id: [0; 32],
            peer_recipient_lane_id: [0; 32],
            mint_finality_proof_binding_digest: [0; 32],
            ...
            asset_id: offline_cash_asset_identity_digest_v1(&predecessor.lane.asset)
...
pub(super) fn guard_public_instances<F: OfflineCashPoseidonFieldV1>(
...
fn guard_recursive_witness<'a>(
    ...
    eq_fold: &'a super::OfflineCashEqFoldOutputV1,
    ep_fold: &'a super::OfflineCashEpFoldOutputV1,
    eq_audit: DigestV1,
    ep_audit: DigestV1,
) -> OfflineCashGuardBundleRecursiveWitnessV1<'a> {
    OfflineCashGuardBundleRecursiveWitnessV1 {
        relation,
...
fn prove_guard(
    ...
    let eq_credential_fold = fold_offline_cash_eq_accumulators_v1(
        eq_params,
        &predecessor.eq_current,
        &successor.eq_current,
    )
    .expect("fold Eq credential proofs");
    let ep_credential_fold = fold_offline_cash_ep_accumulators_v1(
        ep_params,
        &predecessor.ep_current,
        &successor.ep_current,
    )
    .expect("fold Ep credential proofs");
    let (_, _, eq_audit, ep_audit) = build_offline_cash_guard_bundle_pair_v1(
    ...
    let (eq_circuit, ep_circuit, rebuilt_eq_audit, rebuilt_ep_audit) =
        build_offline_cash_guard_bundle_pair_v1(
            &eq_succinct_vk(eq_params),
            &ep_succinct_vk(ep_params),
            guard_recursive_witness(
                relation.clone(),
...
struct DisabledMint {
    ...
    eq_history: OfflineCashEqAccumulatorV1,
    ep_history: OfflineCashEpAccumulatorV1,
    eq_fold: OfflineCashEqFoldProofV1,
    ep_fold: OfflineCashEpFoldProofV1,
}
...
impl DisabledIncoming {
    fn new(
        ...
        let eq_history =
            initial_offline_cash_eq_accumulator_v1(eq_params).expect("Eq incoming seed history");
        let ep_history =
            initial_offline_cash_ep_accumulator_v1(ep_params).expect("Ep incoming seed history");
...
impl DisabledMint {
    fn new(
        ...
        let eq_history = initial_offline_cash_eq_accumulator_v1(eq_params)
            .expect("Eq disabled-mint seed history");
        let ep_history = initial_offline_cash_ep_accumulator_v1(ep_params)
            .expect("Ep disabled-mint seed history");
        let prefix = OFFLINE_CASH_MINT_AUTHORITY_PUBLIC_INSTANCE_COUNT_V1
            .checked_sub(accumulator_limb_count())
...
fn bootstrap_state_relation(
    ...
    let witness = OfflineCashStateRelationWitnessV1 {
        operation: OfflineCashOperationV1::Bootstrap,
        predecessor: None,
        successor,
        amount: 0,
...
struct ParentProof {
    ...
    eq_history: OfflineCashEqAccumulatorV1,
    ep_history: OfflineCashEpAccumulatorV1,
    eq_current: Option<OfflineCashEqAccumulatorV1>,
    ep_current: Option<OfflineCashEpAccumulatorV1>,
}
...
fn parent_from_generated(proof: OfflineCashGeneratedRecursiveStateProofV1) -> ParentProof {
    ParentProof {
...
fn state_generation_witness<'a>(
    ...
    eq_parent_fold: &'a OfflineCashEqFoldProofV1,
    ep_parent_fold: &'a OfflineCashEpFoldProofV1,
    eq_successor_history: &'a OfflineCashEqAccumulatorV1,
    ep_successor_history: &'a OfflineCashEpAccumulatorV1,
    eq_guard_history_fold: &'a OfflineCashEqFoldProofV1,
    ep_guard_history_fold: &'a OfflineCashEpFoldProofV1,
    eq_guard_merge_fold: &'a OfflineCashEqFoldProofV1,
    ep_guard_merge_fold: &'a OfflineCashEpFoldProofV1,
    incoming: &'a DisabledIncoming,
    mint: &'a DisabledMint,
    dummy_eq_fold: &'a OfflineCashEqFoldProofV1,
    dummy_ep_fold: &'a OfflineCashEpFoldProofV1,
) -> OfflineCashRecursiveStateGenerationWitnessV1<'a> {
    let eq_incoming_slot = OfflineCashRecursiveIncomingEqGenerationWitnessV1 {
    ...
    let ep_incoming_slot = OfflineCashRecursiveIncomingEpGenerationWitnessV1 {
    ...
    OfflineCashRecursiveStateGenerationWitnessV1 {
        state,
        guard_relation: guard.relation.clone(),
        suite_upgrade_bridge: OfflineCashSuiteUpgradeGenerationWitnessV1::ZERO,
        eq_parent_protocol,
        ep_parent_protocol,
...
struct StateHistoryFolds {
    eq_parent_fold: OfflineCashEqFoldProofV1,
    ep_parent_fold: OfflineCashEpFoldProofV1,
    eq_guard_history_fold: OfflineCashEqFoldProofV1,
    ep_guard_history_fold: OfflineCashEpFoldProofV1,
    eq_guard_merge_fold: OfflineCashEqFoldProofV1,
    ep_guard_merge_fold: OfflineCashEpFoldProofV1,
    eq_successor_history: OfflineCashEqAccumulatorV1,
    ep_successor_history: OfflineCashEpAccumulatorV1,
}
...
fn prepare_state_histories(
    ...
    let (eq_parent_fold, eq_base_history) = if let Some(current) = &parent.eq_current {
        let fold = fold_offline_cash_eq_accumulators_v1(eq_params, current, &parent.eq_history)
    ...
    let (ep_parent_fold, ep_base_history) = if let Some(current) = &parent.ep_current {
        let fold = fold_offline_cash_ep_accumulators_v1(ep_params, current, &parent.ep_history)
    ...
    let eq_guard_history =
        fold_offline_cash_eq_accumulators_v1(eq_params, &guard.eq_current, &guard.eq_history)
            .expect("complete Eq GuardBundle history");
    let ep_guard_history =
        fold_offline_cash_ep_accumulators_v1(ep_params, &guard.ep_current, &guard.ep_history)
            .expect("complete Ep GuardBundle history");
    let eq_guard_merge = fold_offline_cash_eq_accumulators_v1(
        eq_params,
        &eq_base_history,
        eq_guard_history.successor(),
    )
    .expect("merge Eq GuardBundle history");
    let ep_guard_merge = fold_offline_cash_ep_accumulators_v1(
        ep_params,
        &ep_base_history,
...
fn decode_state_keys(
    eq_params: &ParamsIPA<EqAffine>,
    ep_params: &ParamsIPA<EpAffine>,
    generated: super::OfflineCashGeneratedRecursiveStateArtifactsV1,
) -> StateKeys {
    let mut eq_vk_cursor = Cursor::new(generated.eq.verifying_key.as_ref());
    let eq_verifying_key = VerifyingKey::read::<_, OfflineCashRecursiveStateEqCircuitV1>(
        &mut eq_vk_cursor,
    ...
    let eq_proving_key = ProvingKey::read::<_, OfflineCashRecursiveStateEqCircuitV1>(
        &mut eq_pk_cursor,
        SerdeFormat::Processed,
        generated.eq_circuit_params.clone(),
    )
    ...
    let ep_verifying_key = VerifyingKey::read::<_, OfflineCashRecursiveStateEpCircuitV1>(
        &mut ep_vk_cursor,
        SerdeFormat::Processed,
    ...
    let ep_proving_key = ProvingKey::read::<_, OfflineCashRecursiveStateEpCircuitV1>(
        &mut ep_pk_cursor,
        SerdeFormat::Processed,
        generated.ep_circuit_params.clone(),
    )
...
fn generate_self_recursive_state_keys(
    ...
    let eq_initial =
        initial_offline_cash_eq_accumulator_v1(eq_params).expect("Eq recursive-state seed history");
    let ep_initial =
        initial_offline_cash_ep_accumulator_v1(ep_params).expect("Ep recursive-state seed history");
    ...
    for iteration in 0..8 {
        let eq_parent_digest =
            native_parent_protocol_digest_v1(&eq_parent_protocol, OfflineCashPastaParityV1::Eq)
                .expect("Eq candidate parent protocol digest");
        let ep_parent_digest =
            native_parent_protocol_digest_v1(&ep_parent_protocol, OfflineCashPastaParityV1::Ep)
        ...
        let keys = decode_state_keys(
            eq_params,
            ep_params,
            generate_offline_cash_recursive_state_artifacts_v1(witness)
                .expect("generate real recursive-state artifacts"),
        );
        let eq_before = offline_cash_protocol_structure_digest_v1(
            &eq_parent_protocol,
            OfflineCashPastaParityV1::Eq,
        )
        .expect("Eq parent structure");
        let ep_before = offline_cash_protocol_structure_digest_v1(
            &ep_parent_protocol,
            OfflineCashPastaParityV1::Ep,
        )
        .expect("Ep parent structure");
        let eq_after = offline_cash_protocol_structure_digest_v1(
            &keys.eq_protocol,
            OfflineCashPastaParityV1::Eq,
        )
        .expect("Eq generated structure");
        let ep_after = offline_cash_protocol_structure_digest_v1(
            &keys.ep_protocol,
            OfflineCashPastaParityV1::Ep,
        )
...
fn prove_state_step(
    state_relation: OfflineCashStateRelationWitnessV1,
    guard: &GuardProof,
    guard_keys: &GuardKeys,
    state_keys: &StateKeys,
    mint: &DisabledMint,
    parent: &ParentProof,
) -> OfflineCashGeneratedRecursiveStateProofV1 {
...
fn paired_state_proof(
    ...
    guard: &GuardProof,
    state_keys: &StateKeys,
    proof: &OfflineCashGeneratedRecursiveStateProofV1,
    ...
    let paired = OfflineCashPairedProofV1 {
        version: OFFLINE_CASH_WIRE_VERSION_V1,
        eq_protocol_digest: state_keys.eq_protocol_digest,
        ep_protocol_digest: state_keys.ep_protocol_digest,
...
fn terminally_verify_state_proof(
    ...
    let eq_terminal = fold_offline_cash_eq_accumulators_v1(
        &state_keys.eq.parameters,
    ...
    let ep_terminal = fold_offline_cash_ep_accumulators_v1(
        &state_keys.ep.parameters,
        &proof.ep_current_accumulator,
        &proof.ep_history,
    )
    .expect("terminal Ep state fold");
    verify_and_decide_offline_cash_ep_fold_v1(
        &state_keys.ep.parameters,
        &proof.ep_current_accumulator,
        &proof.ep_history,
        &ep_terminal,
...
fn run_real_recursive_handoffs(handoff_count: u64, checkpoints: &[u64]) {
    ...
    let eq_params = ParamsIPA::<EqAffine>::new(OFFLINE_CASH_HALO2_K_V1);
    let ep_params = ParamsIPA::<EpAffine>::new(OFFLINE_CASH_HALO2_K_V1);
    ...
    let eq_initial = initial_offline_cash_eq_accumulator_v1(&state_keys.eq.parameters)
        .expect("Eq state bootstrap history");
    let ep_initial = initial_offline_cash_ep_accumulator_v1(&state_keys.ep.parameters)
