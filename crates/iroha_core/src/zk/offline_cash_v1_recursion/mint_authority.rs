//! Stable recursive authority carrier for finalized Offline Cash mint credits.
//!
...
use iroha_data_model::offline::{OfflineCashMintCreditStatementV1, OfflineCashPairedProofV1};
...
use super::{
    DigestV1, OFFLINE_CASH_HISTORY_ACCUMULATOR_BYTES_V1, OfflineCashPastaParityV1,
    deferred_parent::{
    ...
    mint_helper::{
        OfflineCashMintAuthorityStepV1, OfflineCashMintCertificateJobsV1,
        OfflineCashMintCertificateWitnessV1, ReciprocalAffine,
        constrain_offline_cash_mint_certificate_v1,
    },
};
use crate::zk::{
    offline_cash_v1_poseidon::{OfflineCashPoseidonFieldV1, digest_limbs},
...
const MINT_PAIR_BINDING_DOMAIN_V1: &[u8] = b"iroha:offline-cash:v1:mint-authority-pair";
...
pub struct OfflineCashMintAuthorityCheckpointV1 {
...
impl OfflineCashMintAuthorityCheckpointV1 {
    /// Validate all non-cryptographic public bindings.
    pub fn validate_shape(&self) -> Result<(), String> {
        if !matches!(
            self.step,
            OfflineCashMintAuthorityStepV1::Bootstrap | OfflineCashMintAuthorityStepV1::Rotate
        ) {
        ...
        let eq_history: &[u8; OFFLINE_CASH_HISTORY_ACCUMULATOR_BYTES_V1] = self
            .proof
            .eq_history
            .as_slice()
            .try_into()
            .map_err(|_| "mint-authority Eq checkpoint history has wrong size".to_owned())?;
        let ep_history: &[u8; OFFLINE_CASH_HISTORY_ACCUMULATOR_BYTES_V1] = self
            .proof
            .ep_history
            .as_slice()
            .try_into()
            .map_err(|_| "mint-authority Ep checkpoint history has wrong size".to_owned())?;
        let expected = OfflineCashMintAuthorityPairBindingV1 {
            step: self.step,
            semantic_digest,
...
pub(super) mod public_instance {
    ...
    /// Low limb of the authenticated Offline Cash release identifier.
    pub const RELEASE_LO: usize = 8;
    /// High limb of the authenticated Offline Cash release identifier.
...
pub(super) const OFFLINE_CASH_MINT_AUTHORITY_PUBLIC_INSTANCE_COUNT_V1: usize =
...
pub struct OfflineCashMintAuthorityPairBindingV1<'a> {
    ...
    pub eq_history: &'a [u8; OFFLINE_CASH_HISTORY_ACCUMULATOR_BYTES_V1],
    /// Complete Ep helper history.
    pub ep_history: &'a [u8; OFFLINE_CASH_HISTORY_ACCUMULATOR_BYTES_V1],
}

impl OfflineCashMintAuthorityPairBindingV1<'_> {
...
pub(super) struct OfflineCashMintAuthorityParityWitnessV1<'a, C>
where
    C: CurveAffineExt,
{
    ...
    pub(super) successor_history: &'a [u8; OFFLINE_CASH_HISTORY_ACCUMULATOR_BYTES_V1],
}

/// Complete shared witness used to build the mutually audited helper pair.
pub(super) struct OfflineCashMintAuthorityPairWitnessV1<'a> {
    pub(super) step: OfflineCashMintAuthorityStepV1,
    ...
    pub(super) certificate: OfflineCashMintCertificateWitnessV1,
    pub(super) eq: OfflineCashMintAuthorityParityWitnessV1<'a, EqAffine>,
    pub(super) ep: OfflineCashMintAuthorityParityWitnessV1<'a, EpAffine>,
}

/// Base/Table16/dense-MSM configuration shared by both authority parities.
#[derive(Clone, Debug)]
pub(super) struct OfflineCashMintAuthorityCircuitConfigV1<F: halo2_base::utils::ScalarField> {
    base: BaseConfig<F>,
...
#[derive(Clone)]
pub(super) struct OfflineCashMintAuthorityEqCircuitV1 {
...
#[derive(Clone)]
pub(super) struct OfflineCashMintAuthorityEpCircuitV1 {
...
macro_rules! impl_mint_authority_circuit {
    ($circuit:ty, $field:ty, $opposite:ty, $label:literal) => {
        impl Circuit<$field> for $circuit {
            type Config = OfflineCashMintAuthorityCircuitConfigV1<$field>;
            ...
            fn configure_with_params(
                ...
                OfflineCashMintAuthorityCircuitConfigV1 {
                    base,
...
impl_mint_authority_circuit!(
    ...
    Fp,
    EpAffine,
    "Offline Cash Eq mint authority"
);
impl_mint_authority_circuit!(
    OfflineCashMintAuthorityEpCircuitV1,
    Fq,
    EqAffine,
    "Offline Cash Ep mint authority"
);

/// Build the stable mutually audited authority pair.
pub(super) fn build_offline_cash_mint_authority_pair_v1(
    eq_params: &ParamsIPA<EqAffine>,
    ep_params: &ParamsIPA<EpAffine>,
    witness: OfflineCashMintAuthorityPairWitnessV1<'_>,
) -> Result<
    (
        OfflineCashMintAuthorityEqCircuitV1,
        OfflineCashMintAuthorityEpCircuitV1,
...
fn build_scalar_half<C, S>(
    ...
    eq_successor_history: &[u8; OFFLINE_CASH_HISTORY_ACCUMULATOR_BYTES_V1],
    ep_successor_history: &[u8; OFFLINE_CASH_HISTORY_ACCUMULATOR_BYTES_V1],
    witness: OfflineCashMintAuthorityParityWitnessV1<'_, C>,
) -> Result<
    (
        BaseCircuitBuilder<C::ScalarExt>,
        OfflineCashMintCertificateJobsV1<S>,
...
where
    ...
    C::ScalarExt: OfflineCashPoseidonFieldV1 + ff::WithSmallOrderMulGroup<3>,
    ...
    S::ScalarExt: ReciprocalAffine,
{
    ...
    let (assigned, mut jobs) =
        constrain_offline_cash_mint_certificate_v1::<S>(&mut builder, certificate, parity, step)?;
    ...
    let history = match parity {
        OfflineCashPastaParityV1::Eq => &eq_history,
        OfflineCashPastaParityV1::Ep => &ep_history,
    };
    ...
    if builder.assigned_instances[0].len() != OFFLINE_CASH_MINT_AUTHORITY_PUBLIC_INSTANCE_COUNT_V1 {
    ...
    let expected_protocol = match parity {
        OfflineCashPastaParityV1::Eq => eq_protocol,
        OfflineCashPastaParityV1::Ep => ep_protocol,
    };
    let expected_audit = match parity {
        OfflineCashPastaParityV1::Eq => eq_audit,
        OfflineCashPastaParityV1::Ep => ep_audit,
    };
    ...
    if loaded.protocol.num_instance != [OFFLINE_CASH_MINT_AUTHORITY_PUBLIC_INSTANCE_COUNT_V1]
        || witness.parent_instances.len() != 1
        || witness.parent_instances[0].len() != OFFLINE_CASH_MINT_AUTHORITY_PUBLIC_INSTANCE_COUNT_V1
    {
    ...
    let output = finalize_deferred_audit_plan_v1(
        ...
        vec![step != OfflineCashMintAuthorityStepV1::Bootstrap; equation_count],
    )
...
{
    if parent.len() != OFFLINE_CASH_MINT_AUTHORITY_PUBLIC_INSTANCE_COUNT_V1 {
...
fn authority_builder<F: OfflineCashPoseidonFieldV1>() -> BaseCircuitBuilder<F> {
...
fn assigned_u128_bytes<F: OfflineCashPoseidonFieldV1>(
    ctx: &mut halo2_base::Context<F>,
...
fn public_digest_cells<F: OfflineCashPoseidonFieldV1>(
...
const _: () = {
    assert!(OFFLINE_CASH_MINT_AUTHORITY_PUBLIC_INSTANCE_COUNT_V1 == 56);
};
...
mod tests {
    ...
    fn binding<'a>(
        eq_history: &'a [u8; OFFLINE_CASH_HISTORY_ACCUMULATOR_BYTES_V1],
        ep_history: &'a [u8; OFFLINE_CASH_HISTORY_ACCUMULATOR_BYTES_V1],
    ) -> OfflineCashMintAuthorityPairBindingV1<'a> {
        OfflineCashMintAuthorityPairBindingV1 {
    ...
    fn pair_binding_rejects_cross_parity_history_and_audit_splicing() {
        let eq_history = [0x11; OFFLINE_CASH_HISTORY_ACCUMULATOR_BYTES_V1];
        let ep_history = [0x22; OFFLINE_CASH_HISTORY_ACCUMULATOR_BYTES_V1];
    ...
    fn carrier_public_shape_has_explicit_u128_cells() {
        ...
        assert_eq!(OFFLINE_CASH_MINT_AUTHORITY_PUBLIC_INSTANCE_COUNT_V1, 56);
    }
}
