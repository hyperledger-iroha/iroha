...
use crate::zk::pasta_sha256_table16::util::MASK_EVEN_32;
use ff::PrimeField;
...
impl<F: PrimeField> CompressionGate<F> {
    ...
    pub fn s_h_prime(
        ...
        // Choice's two terms are disjoint, so their sum is one 32-bit word.
        // H' therefore carries at most four. Constraining the carry directly
        // also binds it to the arithmetic equation; auxiliary host-derived
        // limbs would not provide that link.
        let carry_check = Gate::range_check(h_prime_carry, 0, 4);
        [
            ("h_prime equality check", h_prime_equ_check),
            ("h_prime carry check", carry_check),
        ]
        .into_iter()
        .map(move |(name, poly)| (name, s_h_prime.clone() * poly))
    }
    // s_add_halves to get new_state = old_state + {a,b,...,h}
    // Both terms are 32-bit, so the carry must be a bit.
    #[allow(clippy::too_many_arguments)]
    pub fn s_add_halves(
        ...
        let check = sum - (new_carry.clone() * F::from(1 << 32)) - new;
        let carry_check = Gate::range_check(new_carry, 0, 1);
        [("s_add_halves", check), ("s_add_halves carry", carry_check)]
            .into_iter()
            .map(move |(name, poly)| (name, s_add_halves.clone() * poly))
    }
