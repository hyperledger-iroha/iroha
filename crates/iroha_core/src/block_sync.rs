...
impl BlockSynchronizer {
    ...
    fn npos_cached_commit_qc_reusable(
        ...
        let signer_peers = Self::signer_peers_from_bitmap(cert).ok_or("invalid_signer_bitmap")?;
        ...
        pub(super) async fn handle_message(&self, block_sync: &mut BlockSynchronizer) {
            match self {
                ...
                Message::ShareBlocks(ShareBlocks {
                    ...
                    let now = Instant::now();
                    let solicited_by_gossip =
                        block_sync.request_tracker.allow_response(peer_id, now);
                    let solicited_by_recovery = !solicited_by_gossip
                        && block_sync
                            .sumeragi
    ...
    mod share_blocks_runtime_tests {
        ...
        fn share_blocks_enqueue_does_not_block_tokio_timer() {
            ...
            runtime.block_on(async {
                let (sumeragi, rbc_rx) = test_sumeragi_handle_with_rbc_cap(0, 0);
                let kura = Kura::blank_kura_for_testing();
                let state = Arc::new(State::new_for_testing(
                    World::new(),
                    Arc::clone(&kura),
        ...
        #[test]
        fn share_blocks_accepts_direct_recovery_response_permit() {
            let runtime = tokio::runtime::Builder::new_current_thread()
                .enable_time()
                .build()
                .expect("tokio runtime");
            ...
            runtime.block_on(async {
                let (sumeragi, rbc_rx) = test_sumeragi_handle_with_rbc_cap(1, 0);
                let kura = Kura::blank_kura_for_testing();
                let state = Arc::new(State::new_for_testing(
                    World::new(),
                    Arc::clone(&kura),
                ...
                block_sync
                    .sumeragi
                    .record_direct_block_sync_response_permit_for_tests(
