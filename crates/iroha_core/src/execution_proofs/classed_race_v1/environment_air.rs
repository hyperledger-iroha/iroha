//! Touring S1 environment compiled into the actual bounded integer arithmetic graph.

use super::super::integer_air::{IntegerAirV1, Value};
use super::{environment as env, rules::classed_track_length_v1};
use iroha_data_model::classed_race_v1::ClassedRaceTrackV1;

fn object(a: &mut IntegerAirV1, track: ClassedRaceTrackV1, progress: Value) -> [Value; 4] {
    let length = classed_track_length_v1(track);
...
#[cfg(test)]
mod tests {
    use super::super::super::integer_air::{Source, field};
    use super::*;
    ...
    fn every_track_object_edge_and_wet_control_matches_native_with_tamper_rejection() {
        for track in [
            ...
            let progress = a.input(0, classed_track_length_v1(track));
            let lateral = a.input(-15_300, 15_300);
            let velocity = a.input(-320, 320);
            let steer = a.input(-28, 28);
            let rain = a.input(0, 1);
            let distance = a.input(0, 3300);
            let speed = a.input(0, 3300);
            let [vx, oil] =
            ...
            for cell in 0..12 {
                for offset in [-9001, -9000, -1, 0, 9000, 9001] {
                    for lateral_offset in [-1401, -1400, 0, 1300, 1400, 1401] {
                        ...
                        for wet in [0, 1] {
                            let row =
                                a.witness(&[p, i64::from(x), -319, -28, wet, 3300, 3300], &[]);
                            ...
                            let expected = env::impact(track, p, p + 3300, x, 3300);
                            assert_eq!(
