...
pub mod tests {
    ...
    async fn governance_manifest_allows_multisig_propose_envelope_from_live_signer() {
        ...
        let signer = Account::new_in_domain(signer_id.clone(), domain_id.clone()).build(&signer_id);
        let cosigner =
            Account::new_in_domain(cosigner_id.clone(), domain_id.clone()).build(&cosigner_id);
        let validator =
            Account::new_in_domain(validator_id.clone(), domain_id.clone()).build(&validator_id);
        let multisig = Account::new_in_domain(multisig_id.clone(), domain_id)
