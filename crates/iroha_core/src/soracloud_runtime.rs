...
/// Return whether a validator account is bound to the exact peer on an active authoritative lane.
///
/// Host capability adverts do not own validator-to-peer identity. Callers that have a capability
/// peer must use this predicate instead of account-only validator eligibility so an on-chain peer
/// rotation invalidates the old advert and every placement derived from it immediately.
#[must_use]
pub fn soracloud_validator_peer_is_active(
    world: &impl WorldReadOnly,
    validator_account_id: &AccountId,
    peer_id: &str,
    lane_is_active_for_authority: impl Fn(LaneId) -> bool,
...
fn inrou_replica_assignment_has_active_capability(
    ...
    capability.validate().is_ok()
        ...
        && soracloud_validator_peer_is_active(
            world,
...
mod tests {
    ...
    fn validator_peer_rebind_invalidates_stale_hf_capability_and_assignment() {
        ...
        assert!(!soracloud_validator_peer_is_active(
            &world,
            &validator_account_id,
            &stale_peer_id,
            |lane_id| lane_id == LaneId::SINGLE,
