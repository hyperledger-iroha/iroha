...
/// (`CID1` + `IPAK` + `H2VK`) for the current IVM execution binding circuit,
...
mod zkparse {
    ...
    /// `CID1`/`IPAK`/`H2VK` envelope. Generator construction happens only
...
#[cfg(feature = "zk-halo2")]
// Retain the non-IPA verifier implementation for the legacy feature without
// exposing that backend through the production verifier dispatcher.
#[allow(dead_code)]
#[allow(clippy::too_many_lines)]
...
// Keep the fail-closed counterpart paired with the feature implementation.
#[allow(dead_code)]
