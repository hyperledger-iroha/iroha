...
fn ipa_poly_open_roundtrip_ok_and_fail() {
    use iroha_zkp_halo2::{
        OpenVerifyEnvelope, Params, PolyOpenTranscriptMetadata, Polynomial, PrimeField64,
        Transcript, backend::pallas::PallasBackend, norito_helpers as nh,
    };
    ...
    iroha_zkp_halo2::Polynomial::verify_open_with_metadata(
        &params, &mut tr_v, z, p_g, t, &proof, metadata,
    )
    ...
    let mut env_bad_domain = env.clone();
    env_bad_domain.domain_tag = Some([0x44; 32]);
    let bad_domain_bytes = norito::to_bytes(&env_bad_domain).expect("encode");
    let pb_bad_domain = ProofBox::new("halo2/ipa/poly-open".into(), bad_domain_bytes);
    assert!(!verify_backend("halo2/ipa/poly-open", &pb_bad_domain, None));
}
