...
fn json_encode_decode_roundtrip() {
    ...
    let json_body = norito::json::to_vec(&norito::json!({
        "a": 1u64,
        "b": [2u64, 3u64]
    }))
    ...
    let roundtrip: norito::json::Value =
        norito::json::from_slice(decoded_tlv.payload).expect("parse decoded json");
    let original: norito::json::Value = norito::json::from_slice(&json_body).unwrap();
...
fn schema_encode_decode_roundtrip() {
    ...
    let schema_body = schema_name.to_string().into_bytes();
    let schema_ptr = preload_input(&mut vm, 0, &make_tlv(PointerType::Name, &schema_body));

    let json_value = norito::json::to_vec(&norito::json!({
        "qty": 10i64,
        "side": "buy"
    }))
    .expect("serialize schema json");
    let json_ptr = preload_input(&mut vm, 256, &make_tlv(PointerType::Json, &json_value));
    ...
    let roundtrip: norito::json::Value =
        norito::json::from_slice(decoded_tlv.payload).expect("parse decoded schema json");
    let original: norito::json::Value = norito::json::from_slice(&json_value).unwrap();
    assert_eq!(roundtrip, original);
}
...
fn name_decode_from_norito_bytes() {
    ...
    let as_str = std::str::from_utf8(tlv.payload).expect("utf8 payload");
    assert_eq!(as_str, "cursor");
}
...
fn build_path_key_norito_appends_hash() {
    ...
    let base_bytes = base.to_string().into_bytes();
    ...
    let out_name =
        norito::codec::decode_adaptive::<Name>(out_tlv.payload).expect("decode output Name");
...
fn schema_info_returns_registry_snapshot() {
    ...
    let info: norito::json::Value =
        norito::json::from_slice(info_tlv.payload).expect("parse schema info json");
