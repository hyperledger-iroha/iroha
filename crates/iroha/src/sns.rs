...
use eyre::Result;
use iroha_torii_shared::sns::{
    SNS_REGISTRATION_NOT_FOUND_MAX_BYTES, SnsRegistrationNotFoundV1,
};
...
fn name_selector(namespace: SnsNamespacePath, literal: &str) -> Result<NameSelectorV1> {
    match namespace {
        ...
        SnsNamespacePath::Domain => {
            let domain = crate::data_model::domain::DomainId::parse_fully_qualified(literal.trim())?;
            Ok(NameSelectorV1::new(namespace.suffix_id(), domain.to_string())?)
        }
...
impl<'a> SnsApi<'a> {
    ...
    fn decode_optional_name_response(
        ...
        if response.status() == StatusCode::NOT_FOUND {
            ...
            eyre::ensure!(content_type.is_some_and(|value| value.trim().eq_ignore_ascii_case(APPLICATION_JSON)),
                "SNS absence requires the typed JSON registration response");
            eyre::ensure!(response.body().len() <= SNS_REGISTRATION_NOT_FOUND_MAX_BYTES,
...
mod tests {
    ...
    fn optional_name_http_absence_requires_exact_typed_json() {
        ...
        let body = norito::json::to_vec(&SnsRegistrationNotFoundV1::new(
            selector.suffix_id, selector.label.clone())).expect("absence");
        let response = Response::builder().status(StatusCode::NOT_FOUND)
            .header("Content-Type", APPLICATION_JSON).body(body.clone()).expect("response");
        assert!(SnsApi::decode_optional_name_response(&response, &selector).expect("typed absence").is_none());
        assert!(SnsApi::decode_name_response(&response).is_err(), "mandatory lookup must still fail");
        for (status, media, bytes) in [
            (StatusCode::NOT_FOUND, "text/plain", body.clone()),
            (StatusCode::NOT_FOUND, APPLICATION_JSON, b"registration `dpn` not found".to_vec()),
            ...
            (StatusCode::NOT_FOUND, APPLICATION_JSON, vec![b' '; SNS_REGISTRATION_NOT_FOUND_MAX_BYTES + 1]),
        ] {
            let response = Response::builder().status(status).header("Content-Type", media).body(bytes).expect("response");
            assert!(SnsApi::decode_optional_name_response(&response, &selector).is_err(), "{status} {media}");
        }
        for invalid in [
            ...
            let response = Response::builder().status(StatusCode::NOT_FOUND)
                .header("Content-Type", APPLICATION_JSON).body(invalid.as_bytes().to_vec()).expect("response");
            assert!(SnsApi::decode_optional_name_response(&response, &selector).is_err(), "{invalid}");
        }
    }
    #[test]
    fn optional_name_http_success_binds_canonical_namespace_and_record() {
        for (namespace, literal) in [
            ...
            let record = NameRecordV1::new(selector.clone(), iroha_test_samples::ALICE_ID.clone(),
                Vec::new(), 0, 0, 100, 200, 300, crate::data_model::metadata::Metadata::default());
            let response = Response::builder().status(StatusCode::OK)
                .header("Content-Type", APPLICATION_JSON)
                .body(norito::json::to_vec(&record).expect("record")).expect("response");
            assert_eq!(SnsApi::decode_optional_name_response(&response, &selector).expect("record"), Some(record));
            let changed = NameSelectorV1 { label: "other".to_owned(), ..selector };
