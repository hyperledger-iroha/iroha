...
- Run `make dev-workflow` (wrapper around `scripts/dev_workflow.sh`, documented in `specs/dev_workflow.md`). It wraps `cargo fmt --all`, `cargo clippy --workspace --all-targets --locked -- -D warnings`, `cargo build/test --workspace --locked` (tests can take several hours), and `swift test`.
...
- If you want to test your changes on a minimal network, the provided [`docker-compose.yml`](defaults/docker-compose.yml) creates a development network of 4 Iroha peers in docker containers that can be used to test consensus and asset propagation-related logic. Generate and export fresh runtime genesis custody first, as shown in the [repository quickstart](README.md#run-a-local-network); the manifest contains no signing key and fails closed without both key-file paths. We recommend interacting with that network using either [`iroha-python`](https://github.com/hyperledger-iroha/iroha-python), or the included Iroha client CLI.
