...
test("native OpenGameSession browser transaction signs local canonical bytes and verifies the wallet signature", () => {
  ...
  const wire = noritoEncodeInstructionBoxArchive(instruction, 753);
  ...
  assert.equal(browserSignedTransactionHashHex(finalized.signedTransaction, 753), finalized.hashHex);
...
test("large execution transaction builds, signs, validates and hashes without widening ordinary or mixed transactions", () => {
  ...
  assert.throws(() => browserTransactionPayloadHashHex(altered, 753), /ordinary transaction payload/);
});
