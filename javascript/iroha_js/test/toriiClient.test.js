...
function createNativeAmxReceipt(overrides = {}) {
  ...
  const sourceId = transactionHash.slice(5, 69);
  const commonBody = {
    round: {
...
test("getOfflineReadiness parses a full-range u64 without rounding", async () => {
  const activeTransferVerifier = JSON.stringify(offlineActiveTransferVerifier());
  const client = new ToriiClient(BASE_URL, {
    fetchImpl: async () => createResponse({
      status: 200,
      textBody: `{"asset_definition_id":"${OFFLINE_ASSET_DEFINITION_ID}",`
        + `"asset_scale":4,"evaluated_block_height":18446744073709551615,`
        + `"evaluated_block_hash":"${OFFLINE_EVALUATED_BLOCK_HASH}",`
...
test("getOfflineReadiness rejects invalid selectors and contradictory snapshots", async () => {
  ...
  const missingAssetScale = offlineReadinessPayload();
  delete missingAssetScale.asset_scale;
  const missingActiveTransferVerifier = offlineReadinessPayload();
  delete missingActiveTransferVerifier.active_transfer_verifier;
  for (const payload of [
    missingAssetScale,
    missingActiveTransferVerifier,
...
test("getOfflineReadiness rejects malformed or inactive transfer verifiers", async () => {
  const cases = [
    [
      "non-portable key name",
      offlineActiveTransferVerifier({
        id: { backend: "halo2/ipa", name: "offline:transfer" },
      }),
      /id\.name must not contain ':' characters/,
    ],
    [
      "oversized version",
      offlineActiveTransferVerifier({ version: 0x1_0000_0000 }),
      /version must be between 0 and 4294967295/,
    ],
    [
      "non-canonical commitment",
      offlineActiveTransferVerifier({ commitment: "AB".repeat(32) }),
      /commitment must be a lowercase 64-character hexadecimal string/,
    ],
    [
      "zero proof limit",
      offlineActiveTransferVerifier({ max_proof_bytes: 0 }),
      /max_proof_bytes must be between 1 and 4294967295/,
    ],
    [
      "invalid withdrawal window",
      offlineActiveTransferVerifier({ activation_height: 42, withdrawal_height: 42 }),
      /withdrawal_height must be greater than activation_height/,
    ],
    [
      "not active yet",
      offlineActiveTransferVerifier({ activation_height: 43 }),
      /must be active at evaluated_block_height/,
    ],
    [
      "already withdrawn",
      offlineActiveTransferVerifier({ withdrawal_height: 42 }),
      /must be active at evaluated_block_height/,
    ],
  ];

  for (const [label, verifier, expected] of cases) {
    const client = new ToriiClient(BASE_URL, {
      fetchImpl: async () => createResponse({
        status: 200,
        jsonData: offlineReadinessPayload({ active_transfer_verifier: verifier }),
        headers: { "content-type": "application/json" },
      }),
    });
    await assert.rejects(
      () => client.getOfflineReadiness(OFFLINE_ASSET_DEFINITION_ID),
      expected,
      label,
    );
  }
