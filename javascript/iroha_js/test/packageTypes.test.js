  ...
  try {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "type KagemushaPending = Extract<KagemushaOperationStatus, { state: 'pending' }>['value'];",
        "type KagemushaApplied = Extract<KagemushaOperationStatus, { state: 'applied' }>['value']['result'];",
        "type KagemushaTopUp = Extract<KagemushaApplied, { kind: 'top_up' }>['result'];",
        "type KagemushaRedeem = Extract<KagemushaApplied, { kind: 'redeem' }>['result'];",
        "type KagemushaReferenceU64IsLossless = Expect<bigint extends KagemushaOperationReference['identity']['issued_at_ms'] ? true : false>;",
        "type KagemushaPendingU64IsLossless = Expect<bigint extends KagemushaPending['identity']['expires_at_ms'] ? true : false>;",
        "type KagemushaTopUpHeightIsLossless = Expect<bigint extends KagemushaTopUp['finalized_block_height'] ? true : false>;",
        "type KagemushaAnchorHeightIsLossless = Expect<bigint extends KagemushaTopUp['anchor']['finalized_height'] ? true : false>;",
        "type KagemushaProofHeightIsLossless = Expect<bigint extends KagemushaTopUp['finality_proof']['commit_qc']['height_context']['height'] ? true : false>;",
        "type KagemushaRedeemHeightIsLossless = Expect<bigint extends KagemushaRedeem['finalized_block_height'] ? true : false>;",
        "type KagemushaAnchorCountStaysNumber = Expect<bigint extends KagemushaTopUp['finality_proof']['commit_qc']['certificate']['execution_commitment']['topup_anchor_count'] ? false : true>;",
        "type KagemushaLeafCountStaysNumber = Expect<bigint extends KagemushaTopUp['finality_proof']['anchor_path']['leaf_count'] ? false : true>;",
        "type KagemushaPortableApplied = Extract<KagemushaPortableOperationStatus, { state: 'applied' }>['value']['result'];",
        "type KagemushaPortableAppliedIsRedeem = Expect<KagemushaPortableApplied['kind'] extends 'redeem' ? true : false>;",
        "declare const kagemushaArchive: Uint8Array;",
        "declare const kagemushaReference: KagemushaOperationReference;",
        "declare const kagemushaIdentity: KagemushaOperationReference['identity'];",
        "declare const kagemushaPayload: Record<string, unknown>;",
        "declare const kagemushaBrowserClient: RootSdk.ToriiBrowserClient;",
        "const kagemushaRequest: KagemushaNoritoRequestV4 = { version: 4, norito: kagemushaArchive };",
        "const kagemushaBoundReference: KagemushaOperationReference = RootSdk.normalizeKagemushaOperationReference(kagemushaPayload, { expectedIdentity: kagemushaIdentity, location: '/v1/offline/operations/' + kagemushaIdentity.operation_id, retryAfter: '1' });",
        "// @ts-expect-error reference expectations must bind the complete signed request identity.",
        "RootSdk.normalizeKagemushaOperationReference(kagemushaPayload, { location: '/v1/offline/operations/' + kagemushaIdentity.operation_id, retryAfter: '1' });",
        "const kagemushaBrowserStatus: Promise<KagemushaPortableOperationStatus> = kagemushaBrowserClient.getKagemushaOperationStatus(kagemushaReference);",
        "const kagemushaPortableStatus: KagemushaPortableOperationStatus = RootSdk.normalizeKagemushaOperationStatus(kagemushaPayload, kagemushaReference);",
        "// @ts-expect-error Kagemusha operation ids are derived from the canonical request archive.",
        "const kagemushaRequestWithCallerId: KagemushaNoritoRequestV4 = { version: 4, operationId: '11'.repeat(32), norito: kagemushaArchive };",
        ...
        "// @ts-expect-error exact integer JSON getters accept canonical strings only.",
        "NumericV1.decodeIntJson(1);",
        "// @ts-expect-error exact decimal JSON getters reject legacy number tokens.",
        "NumericV1.decodeDecimalJson(1.25);",
        "// @ts-expect-error exact quantity JSON getters reject legacy number tokens.",
