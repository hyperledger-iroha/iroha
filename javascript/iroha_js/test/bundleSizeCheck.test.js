...
test("public browser aggregate bundles without Node inputs or global Buffer shims", async () => {
  ...
  assert.equal(result.outputFiles[0].contents.byteLength, 476_332);
  assert.ok(
...
test("remaining bundle targets retain exact pinned-esbuild baselines", async () => {
  ...
  const expected = new Map([
    ["toriiClient.js", { bytes: 989_667, modules: 62 }],
    ["transactionCodec.js (browser)", { bytes: 297_486, modules: 46 }],
    ["nexusApp.js (browser)", { bytes: 380_687, modules: 55 }],
