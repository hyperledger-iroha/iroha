...
function fakeSplitBundle(options, entryText = "") {
  ...
  const lazyChunks = isTorii
    ...
    : [
        ...
        {
          name: "active-vk",
          input: "dist/activeVerifyingKeyIds.js",
...
test("bundle-size check covers the browser transaction codec", () => {
  ...
  assert.ok(target.limitKb > 0 && target.limitKb <= 298);
});
...
test("bundle-size check proves the Nexus app export has a browser-only graph", () => {
  ...
  assert.ok(target.limitKb > 0 && target.limitKb <= 382);
});
...
test("bundle-size check gates the complete public browser aggregate", () => {
  ...
  assert.ok(target.limitKb > 0 && target.limitKb <= 485);
  assert.equal(target.reviewedEagerBytes, 496_464);
  assert.equal(target.reviewedCombinedBytes, 582_419);
  ...
  assert.match(
    target.lazyChunks[2].entryPoint,
    /dist[/\\]activeVerifyingKeyIds\.js$/u,
  );
});
...
test("browser graph audit derives every explicit browser-conditioned package export", async () => {
  ...
  assert.deepEqual(listExplicitBrowserExports(pkg), [
    ...
    {
      target: "./dist/verifierBackendRegistry.js",
      subpaths: ["./verifier-backend-registry"],
    },
    {
...
test("split graph audit permits only the reviewed literal lazy edges", () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      { specifier: "./activeVerifyingKeyIds.js", bytes: 0 },
    ],
  );
...
test("public browser aggregate audits eager, lazy, and unique combined closures", async () => {
  ...
  assert.equal(Object.keys(result.metafile.inputs).length, 85);
  assert.deepEqual(
    {
    ...
    },
    {
      eagerBytes: 496_464,
      lazyBytes: [
        { specifier: "./sumeragiTyped.js", bytes: 73_719 },
        { specifier: "./smartContractDeploymentSubmit.js", bytes: 9_204 },
        { specifier: "./activeVerifyingKeyIds.js", bytes: 3_032 },
      ],
      combinedBytes: 582_419,
      combinedLimitKb: 569,
    },
  );
  assert.equal(
    ...
    176,
    "public browser aggregate must retain the audited 176-byte eager headroom",
  );
  assert.ok(
    metrics.eagerBytes <= Math.floor(480_214 * 1.05),
    "public browser eager closure regressed more than 5% from the previous reviewed baseline",
  );
...
test("remaining bundle targets retain exact pinned-esbuild baselines", async () => {
  ...
  const maximumGrowth = new Map([
    ["toriiClient.js", (984 * 1024) / 945_975],
    ["transactionCodec.js (browser)", 1.05],
    ["nexusApp.js (browser)", (382 * 1024) / 371_403],
    ["canonicalRequest.js (browser)", 1.05],
  ]);
  const expected = new Map([
    ["toriiClient.js", { bytes: 1_007_565, modules: 99 }],
    ["transactionCodec.js (browser)", { bytes: 304_650, modules: 45 }],
    ["nexusApp.js (browser)", { bytes: 390_781, modules: 56 }],
    ["canonicalRequest.js (browser)", { bytes: 95_442, modules: 41 }],
  ]);
  const { build } = await import("esbuild");
  for (const target of BUNDLE_TARGETS.filter(({ label }) => expected.has(label))) {
    ...
    if (target.label === "toriiClient.js") {
      ...
      assert.equal(
        ...
        51,
        "Torii hard ceiling must retain the audited 51-byte eager headroom",
      );
      assert.deepEqual(
        splitMetrics.lazyChunks.map(({ specifier, bytes }) => ({ specifier, bytes })),
        [{ specifier: "./sumeragiTyped.js", bytes: 73_354 }],
      );
      assert.equal(splitMetrics.combinedBytes, 1_080_919);
      assert.equal(splitMetrics.combinedLimitKb, 1_056);
