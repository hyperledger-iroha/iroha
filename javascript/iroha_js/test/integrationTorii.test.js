...
test(
  ...
  async (t) => {
    ...
    if (!isPlainObject(CONNECT_PREVIEW_CONFIG)) {
      t.diagnostic(
        "set IROHA_TORII_INTEGRATION_CONNECT_PREVIEW to a JSON object (e.g. {\"network_id\":\"<64-lowercase-genesis-hex>\",\"node\":\"connect.devnet.example\"}) to exercise the preview bootstrapper",
      );
      return;
    }
