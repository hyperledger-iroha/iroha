...
test("multisig instruction vector embeds exact Rust archives without rebuilding inner frames", () => {
  ...
  const actual = api._encodeMultisigInstructions([instruction, second]);
  ...
  assert.throws(() => rejected._encodeMultisigInstructions([instruction]), (error) => error === rejection);
});
...
test("all four instruction APIs use the canonical owner even for formerly pure JS variants", () => {
  ...
  assert.deepEqual(api.noritoEncodeInstruction(instruction), frame);
  assert.deepEqual(api.noritoDecodeInstruction(frame), instruction);
  assert.deepEqual(api.noritoEncodeInstructionBoxArchive(instruction), archive);
  assert.deepEqual(api.noritoDecodeInstructionBoxArchive(archive), instruction);
...
test("owner rejection never falls back to a local encoder or archive decoder", () => {
  ...
  for (const operation of [
    ...
    () => api.noritoDecodeInstruction(Buffer.of(1)),
    () => api.noritoEncodeInstructionBoxArchive(instruction),
    () => api.noritoDecodeInstructionBoxArchive(Buffer.of(1)),
...
test("preencoded instruction frames are checked by Rust instead of passed through", () => {
  ...
  assert.throws(() => api.noritoEncodeInstruction(Buffer.of(1, 2, 3)), /not canonical/);
  assert.throws(() => api.noritoEncodeInstruction(Buffer.of(1, 2, 3).toString("base64")), /not canonical/);
  assert.deepEqual(api.noritoEncodeInstruction(Buffer.of(1, 2)), Buffer.of(1, 2));
});
...
test("frame decode retains raw JSON mode and owner receives only the requested byte view", () => {
  ...
  assert.equal(api.noritoDecodeInstruction(backing.subarray(1, 3), { parseJson: false }), json);
});
...
test("instruction JSON text reaches Rust without rounding unsafe u64 tokens", () => {
  ...
  assert.deepEqual(api.noritoEncodeInstruction(exact), Buffer.of(1));
  assert.equal(api.noritoDecodeInstruction(Buffer.of(1), { parseJson: false }), exact);
});
...
test("duplicate keys in JSON text remain visible to the canonical owner's rejection", () => {
  ...
  assert.throws(() => api.noritoEncodeInstruction(exact), (error) => error === rejection);
});
...
test("object inputs reject unsafe integers, non-finite numbers and unsupported bigint before native serialization", () => {
  ...
  for (const value of [Number.MAX_SAFE_INTEGER + 1, -(Number.MAX_SAFE_INTEGER + 1), NaN, Infinity, -Infinity, 1n]) {
    assert.throws(() => api.noritoEncodeInstruction({ Custom: { payload: { nested: [value] } } }), /exact JSON text/);
  }
  assert.equal(calls, 0);
  api.noritoEncodeInstruction({ Custom: { payload: { finiteFraction: 1.25, safe: Number.MAX_SAFE_INTEGER } } });
...
test("parsed owner output rejects rounded integers while preserving fractional JSON and exact raw mode", () => {
  for (const token of ["9007199254740993", "-9007199254740993", "18446744073709551615", "1e999"]) {
    ...
    assert.throws(() => api.noritoDecodeInstruction(Buffer.of(1)), /exact JSON text/);
    assert.throws(() => api.noritoDecodeInstructionBoxArchive(Buffer.of(1)), /exact JSON text/);
    assert.equal(api.noritoDecodeInstruction(Buffer.of(1), { parseJson: false }), exact);
  }
  ...
  assert.deepEqual(api.noritoDecodeInstruction(Buffer.of(1)), JSON.parse(exact));
  assert.deepEqual(api.noritoDecodeInstructionBoxArchive(Buffer.of(1)), JSON.parse(exact));
});
