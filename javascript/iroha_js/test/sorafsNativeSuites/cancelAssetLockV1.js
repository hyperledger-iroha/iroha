...
export function registerCancelAssetLockV1Tests(context) {
  const { test } = context;
  const { decodeCancelAssetLockV1, encodeCancelAssetLockV1 } = context.subject;
  const fixtureRoot = fileURLToPath(context.fixtureRoot);
