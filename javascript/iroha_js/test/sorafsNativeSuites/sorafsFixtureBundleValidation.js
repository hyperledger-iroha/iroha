// Source-owned assertion registration; never imports an SDK implementation.
import assert from "node:assert/strict";
import { readFileSync } from "node:fs";
import { createNativeTestHelper } from "../helpers/nativeRequirements.js";

/** Register the unchanged canonical suite against its fixed test-owned subject. */
export function registerSorafsFixtureBundleValidationTests(context) {
  const { test, nativeBinding, nativeBindingError } = context;
  const { SORAFS_FIXTURE_BUNDLE_MAX_PAYLOADS_V1, SORAFS_FIXTURE_BUNDLE_PAYLOAD_KINDS, validateFixtureBundle } = context.subject;
  const { makeNativeTest } = createNativeTestHelper(nativeBinding, nativeBindingError);
  const FIXTURE_ROOT = context.fixtureRoot;
