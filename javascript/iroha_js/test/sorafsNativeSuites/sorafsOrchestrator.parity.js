...
export function registerSorafsOrchestratorParityTests(context) {
  const { test: baseTest, nativeBinding, nativeBindingError, temporaryRoot } = context;
  const { getNativeBinding } = context.subject;
  const { makeNativeTest } = createNativeTestHelper(nativeBinding, nativeBindingError);
  const test = makeNativeTest(baseTest);
  const REPO_ROOT = fileURLToPath(context.repositoryRoot);
  const FIXTURE_ROOT = fileURLToPath(context.fixtureRoot);
  ...
  test("SoraFS orchestrator multi-fetch is deterministic across runs", () => {
    ...
    const tempDir = mkdtempSync(path.join(temporaryRoot, "sorafs-js-parity-"));
    try {
