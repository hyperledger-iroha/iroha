...
export function registerSorafsPdpValidationTests(context) {
  const { test } = context;
  const { SORAFS_PDP_PAYLOAD_KINDS, validatePdpBundle, validatePdpChallengeProof, validatePdpCommitmentChallenge, validatePdpPayload } = context.subject;
  ...
  function pdpNegativeOutcome(name) {
    return new URL(
      `pdp/negative/${name}_validation_outcome_v1.json`,
      context.fixtureRoot,
    );
  }
