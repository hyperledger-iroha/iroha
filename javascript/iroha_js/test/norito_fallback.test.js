...
test("JS-only norito fallback matches native bytes for a multisig-controlled account id", async () => {
  const nativeMod = await import("../src/norito.js?native-multisig-transfer");
  ...
  const instruction = {
    Transfer: {
      Asset: {
        source: nativeMod.noritoDecodeInstruction(loadInstructionBytes("mint_asset_numeric.json")).Mint
          .Asset.destination,
        object: "7",
  ...
  const nativeEncoded = nativeMod.noritoEncodeInstruction(instruction);
