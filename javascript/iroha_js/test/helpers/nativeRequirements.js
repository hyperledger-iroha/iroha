// Pure native capability gates for fixed source/installed test contexts.
/** Bind failure-reporting semantics to an already selected native load observation. */
export function createNativeTestHelper(binding, bindingError) {
  ...
  const nativeBindingError = bindingError;
  const hasNativeBinding = binding !== null;
  const noritoRequiredMethods = NORITO_REQUIRED_METHODS;
  const sm2RequiredMethods = SM2_REQUIRED_METHODS;
  ...
  function hasNoritoBinding(bindingOverride = nativeBinding) {
    return hasNativeMethods(bindingOverride, NORITO_REQUIRED_METHODS);
  }

  function hasSm2Binding(bindingOverride = nativeBinding) {
  ...
  function makeNativeTest(baseTest, options = {}) {
