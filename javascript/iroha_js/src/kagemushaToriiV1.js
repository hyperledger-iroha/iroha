...
const TEXT_KAGEMUSHA_V1_OPERATION = "KAGEMUSHA V1 operation ";
const TEXT_OPERATION_RESPONSE_MUST = " operation response must ";
...
export function normalizeKagemushaReadinessV1(value) {
  if (value === null || typeof value !== "object" || Array.isArray(value)) {
    throw new TypeError((TEXT_KAGEMUSHA_V1 + "readiness response must be an object"));
  }
  ...
  if (Object.keys(value).length !== fields.length || fields.some((field) => !Object.hasOwn(value, field))) {
    throw new TypeError((TEXT_KAGEMUSHA_V1 + "readiness response contains missing or unknown fields"));
  }
  if (value.kagemusha_handoff_capability !== "kagemusha_handoff_v1" || value.wire_version !== 1 || value.device_lifecycle_version !== 1 || typeof value.ready !== "boolean") {
    throw new TypeError((TEXT_KAGEMUSHA_V1 + "readiness response is incompatible"));
  }
...
export function kagemushaOperationIdHexV1(value) {
  if (typeof value === "string") {
    if (!/^[0-9a-f]{64}$/u.test(value) || /^0{64}$/u.test(value)) throw new TypeError((TEXT_KAGEMUSHA_V1_OPERATION + "ID must be 64 lowercase hexadecimal characters"));
    return value;
  }
  if (!ArrayBuffer.isView(value) && !(value instanceof ArrayBuffer)) throw new TypeError((TEXT_KAGEMUSHA_V1_OPERATION + "ID must be binary data or lowercase hexadecimal"));
  const bytes = value instanceof ArrayBuffer
    ? new Uint8Array(value)
    : new Uint8Array(value.buffer, value.byteOffset, value.byteLength);
  if (bytes.length !== 32 || bytes.every((byte) => byte === 0)) throw new TypeError((TEXT_KAGEMUSHA_V1_OPERATION + "ID must be one nonzero 32-byte value"));
...
export class UnverifiedKagemushaOperationStatusV1 {
  ...
  async verifyAgainst(trustAnchor, verifier) {
    if (trustAnchor === null || trustAnchor === undefined) {
      throw new TypeError((TEXT_KAGEMUSHA_V1 + "finality trust anchor is required"));
    }
    if (typeof verifier !== "function") {
      throw new TypeError((TEXT_KAGEMUSHA_V1 + "finality verifier must be a function"));
    }
...
export function normalizeUnverifiedKagemushaOperationStatusV1(value) {
  exactObject(value, ["version", "operation_id", "kind", "state", "result", "rejection"], (TEXT_KAGEMUSHA_V1_OPERATION + "status"));
  if (value.version !== 1) throw new TypeError((TEXT_KAGEMUSHA_V1_OPERATION + "status version must be 1"));
  const operationId = fixedBytes(value.operation_id, (TEXT_KAGEMUSHA_V1_OPERATION + "ID"));
  const kind = taggedUnit(value.kind, "kind", OPERATION_KINDS, (TEXT_KAGEMUSHA_V1_OPERATION + "kind"));
  const state = taggedUnit(value.state, "state", OPERATION_STATES, (TEXT_KAGEMUSHA_V1_OPERATION + "state"));
  let rejection = null;
  if (state === "pending") {
    if (value.result !== null || value.rejection !== null) throw new TypeError(("pending " + TEXT_KAGEMUSHA_V1 + "status must not contain a result or rejection"));
  } else if (state === "applied") {
    if (value.result === null || value.rejection !== null || typeof value.result !== "object" || Array.isArray(value.result)) {
      throw new TypeError(("applied " + TEXT_KAGEMUSHA_V1 + "status has an invalid terminal envelope"));
    }
  } else {
    if (value.result !== null) throw new TypeError(("rejected " + TEXT_KAGEMUSHA_V1 + "status must not contain a result"));
    exactObject(value.rejection, ["code", "detail_digest"], (TEXT_KAGEMUSHA_V1 + "rejection"));
    const code = taggedUnit(value.rejection.code, "code", REJECTION_CODES, (TEXT_KAGEMUSHA_V1 + "rejection code"));
    const detailDigest = fixedBytes(value.rejection.detail_digest, (TEXT_KAGEMUSHA_V1 + "rejection detail digest"));
...
export function requireKagemushaSubmissionResponseV1({
  ...
  if (statusCode === 202) {
    if (operationState !== "pending") {
      throw new TypeError(("KAGEMUSHA 202" + TEXT_OPERATION_RESPONSE_MUST + "be pending"));
    }
    if (
      ...
      throw new TypeError(("KAGEMUSHA 202" + TEXT_OPERATION_RESPONSE_MUST + "have a positive Retry-After"));
    }
    return;
  }
  if (statusCode === 200) {
    if (operationState !== "applied" && operationState !== "rejected") {
      throw new TypeError(("KAGEMUSHA 200" + TEXT_OPERATION_RESPONSE_MUST + "be applied or rejected"));
    }
    if (retryAfter !== null) {
      throw new TypeError(("KAGEMUSHA 200" + TEXT_OPERATION_RESPONSE_MUST + "not have Retry-After"));
    }
    return;
  }
