...
function assertAllowedFields(value, allowed, context) {
  assertPlainDataObject(value, context);
  for (const field of Object.keys(value)) {
    if (!allowed.has(field)) {
      fail("invalid_input", `${context}.${field} is not supported`);
    }
  }
}
...
function normalizeNetworkPrefix(value, context) {
  const normalized = normalizeUnsigned(value, UINT32_MAX, context);
  if (normalized > 0xffffn) {
    fail("bounds_exceeded", `${context} must fit in an unsigned 16-bit integer`);
  }
...
function normalizeMetadataValue(value, context, depth, state) {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value)) {
      fail("invalid_metadata", `${context} must not contain non-finite numbers`);
    }
    return value;
  }
  ...
  try {
    if (Array.isArray(value)) {
      return value.map((entry, index) =>
        normalizeMetadataValue(entry, `${context}[${index}]`, depth + 1, state),
      );
    }
    assertPlainDataObject(value, context);
    const output = {};
    for (const key of Object.keys(value).sort()) {
      validateMetadataKey(key, `${context}.${key}`);
...
function normalizeTransferInput(input, now) {
  assertAllowedFields(input, TRANSFER_INPUT_FIELDS, "transfer input");
...
function encodeTransferPayload(normalized) {
  ...
  const payload = struct([
    stringValue(normalized.chainId),
...
function bytes(value, context, { hex = false } = {}) {
  ...
  if (hex && typeof value === "string") {
    if (value.length > MAX_SIGNED_TRANSACTION_BYTES * 2 + 2) {
...
function validateStringArchive(payload, context) {
...
function validateMetadataArchive(payload, context) {
  ...
  for (let index = 0; index < Number(count); index += 1) {
    ...
    const key = validateStringArchive(entry.readField("key"), `${context}[${index}].key`);
    ...
    if (canonicalJsonStringify(parsed) !== jsonText) {
...
function validateTransactionPayload(payload, authorityLiteral) {
  ...
  const authority = accountInfo(authorityLiteral, "signable.authority");
  const reader = new Reader(payload, "transaction payload");
  const chainId = validateStringArchive(reader.readField("chain"), "transaction payload.chain");
  ...
  const encodedAuthority = accountArchive(authority);
  if (!authorityArchive.equals(encodedAuthority)) {
  ...
  if (!sourceOwner.equals(authority.publicKey)) {
...
function normalizeSignature(signature) {
  ...
  assertAllowedFields(signature, SIGNATURE_FIELDS, "signature");
...
export function buildBrowserTransferPayload(input) {
  return encodeTransferPayload(normalizeTransferInput(input, Date.now));
}
...
export function finalizeBrowserSignedTransaction(
  ...
  assertAllowedFields(signable, SIGNABLE_FIELDS, "signable");
  if (
  ...
  const publicKey = bytes(signingPublicKey, "signingPublicKey", { hex: true });
