...
export function inspectCanonicalTransactionPayloadBindings(
  ...
  const expectedAdmissionTag = expectedAdmissionIntent === "queue_plan_synced"
    ? TRANSACTION_ADMISSION_QUEUE_PLAN_SYNCED_TAG
    : expectedAdmissionIntent === "ordinary"
      ? TRANSACTION_ADMISSION_ORDINARY_TAG
      : null;
  if (expectedAdmissionTag === null) {
    fail(MALFORMED_PAYLOAD, "transaction payload requires an explicit admission intent policy");
  }
