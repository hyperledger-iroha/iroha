...
function checkIntRange(value) {
  if (value < INT_MIN || value > INT_MAX) {
    fail("mantissa_overflow", "numeric mantissa is outside the signed 4096-bit domain");
  }
  return value;
}
...
function normalizeScaled(mantissa, scale, quantity) {
  let normalizedMantissa = checkIntRange(checkedBigInt(mantissa, "mantissa"));
  if (!Number.isInteger(scale) || scale < 0 || scale > 28) {
    fail("invalid_scale", "numeric scale must be an integer in 0..28");
  }
...
function parseScaled(value, quantity) {
  ...
  if (fraction.length > 28) fail("invalid_scale", "numeric scale exceeds 28");
...
function decodeEnvelope(kind, input) {
  ...
  if (![0x0010, 0x0011, 0x0012, 0x0013].includes(pointerType)) {
    fail("unknown_type", "numeric envelope has an unknown pointer type");
  }
  if (pointerType === 0x0010) fail("type_not_allowed", "retired Amount pointer type is forbidden");
  ...
  if (!equalBytes(blake2b256(frame), suppliedHash)) {
