...
| --- | --- | --- | --- |
| A1 | Foundational, privacy and service model boundaries | `iroha_data_model`, model crates, Norito/schema | Independent cohesive units with acyclic dependencies; preserve mandatory aggregate protocol JSON for admission and consensus parameters while qualifying supported feature graphs. Qualify the declared-identity codec cutover across the remaining feature/runtime matrix, preserve exact named/frame goldens and default-stack state coverage, and then extract physical model crates. |
