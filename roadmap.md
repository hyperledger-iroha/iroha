...
Latest sync (offline petal transport): `offline petal` now defaults to a new `sora-temple` renderer (katakana tile field + `天` logo silhouette + data-derived 3-ring banding), exposes `render_style` + `estimated_payload_bytes_per_second` in encode manifests, and adds `offline petal eval-capture` to simulate distant/moving capture perturbations with a configurable reliability gate (default `--min-success-ratio 0.95`).
...
34. **OFFLINE-QR-STORM-FOLLOWUP — Validate sakura-storm transport reliability across scanners** (CLI/SDK/Docs, Line: Shared, Owner: Offline WG, Priority: High, Status: 🟨 In Progress, target TBD)
- [ ] Add scanner-loop validation for `--style sakura-storm` at the 3 KB/s preset (`chunk_size=336`, `parity_group=4`, `fps=12`) across iOS/Android/browser capture paths.
- [ ] Mirror sakura-storm playback skin parameters into Swift/Android/JS offline QR helpers so preview appearance stays consistent across SDKs.
- [ ] Publish a short operator runbook with recommended `ecc`/dimension/fps combinations for noisy camera environments.
