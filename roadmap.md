...
4. **LOCALNET-10K-TPS — 7-peer localnet throughput + stall resilience** (Consensus/Performance/Tooling, Line: Shared, Owner: Consensus WG, Priority: High, Status: 🈺 In Progress, target TBD)
 ...
 - [ ] Add a Kagami/localnet regression that asserts validator count + commit QC validator-set length match the peer count on startup.
 - [ ] Trace missing-payload view-change warnings under load: instrument drop/deferral reasons for BlockCreated/BlockSyncUpdate/ExecWitness/RBC DELIVER and verify missing-block fetch retries; fix remaining payload-drop paths under backpressure.
 - [ ] Scale bulk consensus-ingress caps for 1s block times, emit per-topic drop/penalty metrics for payload traffic, and update localnet defaults (no legacy compatibility toggles).
...
- [ ] Add an ignored 7-peer localnet throughput regression (target 10k tps, 1s block time) that asserts block height advances, commit time stays near target, and no consensus stalls occur; capture metrics in `status.md`.
- [ ] Harden NPoS stake-quorum misses: surface stake-quorum-missing counters in `/v1/sumeragi/status` and trigger a view-change/reschedule when stake quorum is not reachable within the commit timeout; add unit coverage.
...
16. **SUMERAGI-RBC-MISMATCH-COVERAGE — Fill remaining JSON coverage gaps** (Consensus/Sumeragi, Line: Shared, Owner: Consensus WG, Priority: Low, Status: 🈯 Planned, target TBD)
 - [ ] Add a JSON roundtrip assertion for `rbc_mismatch` in `sumeragi_status_wire_roundtrip_to_json_preserves_fields` (client JSON payload).
 - [ ] Add a consensus routing JSON test for `rbc_mismatch` in `crates/iroha_torii/src/routing/consensus.rs`.
 - [ ] Run targeted tests for the new assertions (client + torii routing).
...
42. **FASTPQ-PUBLIC-INPUTS — Finalize public input commitments** (Core/FASTPQ, Line: Iroha 3, Owner: FASTPQ WG, Priority: Medium, Status: 🈺 In Progress, target TBD)
 - [ ] Replace entry-hash-derived `tx_set_hash` with scheduler-provided transaction set commitments.
 - [ ] Populate `perm_root` from the permission table commitment once WSV wiring lands.
 - [ ] Emit dataspace-specific DSID bytes for non-global lanes and update docs/tests accordingly.
