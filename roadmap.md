...
Latest sync (consensus ingress/P2P): critical QC/VRF/RBC INIT/READY/DELIVER + block payload recovery use higher ingress caps without penalty cooldowns (still rate/byte limited and RBC-session tracked), READY/DELIVER route via the payload topic/cap, and the consensus frame cap aligns with the global limit.
