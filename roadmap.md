...
- [x] Add a consensus routing JSON test for `rbc_mismatch` in `crates/iroha_torii/src/routing/consensus.rs`.
 - [ ] Run targeted torii routing test `cargo test -p iroha_torii status_snapshot_json_includes_rbc_mismatch -- --nocapture` (client JSON test passed; see `status.md`).
...
- [ ] Run targeted unit tests for the mismatch attribution gap fixes and record results in `status.md`.
