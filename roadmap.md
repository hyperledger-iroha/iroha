...
work appear here.
Latest sync: Governance ZK ballot public-input handling now normalizes Torii `public` aliases and canonicalizes root/nullifier hex (including zk-v1), alongside SDK/CLI alias normalization and docs calling out partial-hint rejection; DA gate now accepts local payload hashes; asset integration tests re-run (decimal/quantity/exchange) passed; integration-test revalidation still pending (see `status.md` for details).
