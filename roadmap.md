...
| --- | --- | --- | --- |
| A1 | Foundational, privacy and service model boundaries | `iroha_data_model`, model crates, Norito/schema | Complete physical owner extraction beyond the implemented ChainId/DomainId/Name/StatePath/Metadata/topology/PeerId base boundary, with acyclic dependencies and mandatory aggregate protocol JSON for admission/consensus. Qualify the remaining declared-identity feature/runtime matrix, exact named/frame goldens and default-stack state coverage on the final candidate. |
| A2 | SDK and service runtime separation | Rust SDK, storage client, Musubi service | Remove telemetry/CAR/orchestrator runtime edges; one service owner for journals, clocks and publication; one typed storage adapter; finish the remaining standalone CLI retirement and canonical CLI release packaging; dependency-boundary gate passes for shipping normal/build graphs. |
| A3 | Immutable asynchronous Rust client | Rust SDK and all callers | Complete authority-owned operation mapping, remaining asynchronous capabilities and the structured error family. Carry canonical status/subscription/operator-configuration and account event/block streams into complete consumer qualification. Qualify Mochi's generation-bound genesis account reader and global-reader permissions on four validators; preserve isolated caches, cancellation, bounds, concurrent blocking waits and acceptance/finality distinctions. |
...
| A5 | Enforced architecture and compiler memory | Build tooling and component owners | Dependency checks and 5,000/3,000 source-file limits pass without expanded exceptions; substantive duplication removed. Requalify Core/Torii test and daemon memory baselines after the compilation repair; run the complete memory comparison on one pinned runner/candidate, preserving per-unit bounds, 25% model reduction and the 13-GiB release ceiling. Qualify lower-memory Mac and Linux cgroup hosts. |
| A6 | CI and generated-artifact ownership | CI router, generators, SDK consumers | Execute foundation-only and full/mixed selection on CI, build the distinct message-control artifact, and qualify selected network/corridor jobs with accurate required aggregation and owned provenance. One create-only Norito RPC fixture producer and verifier; generated manifests and SDK operation/signature inventory are complete and drift-checked. |
...
| N10 | Current governance and recovery fixtures | Core governance tests, integration owners | Migrate removed ballot/lock/referendum fixtures through canonical constructors and carry the repaired workspace compilation into qualification of the complete production/test feature matrix without reintroducing old APIs. |
...
| N12 | Production multilane completion and scaling | Core/Queue/Kura, maintained SDKs and release tooling | Close all six [implementation milestones](specs/sumeragi_v2_multilane_completion_goals.md) on the same candidate: authenticated atomic recovery/lifecycle and strict SDK evidence, current source-bound formal/runtime checks, four-peer tests, ten corridor seeds and two-hour soak. Qualify five pinned one/four-lane pairs with complete fixed-schedule transaction traces, exact routing, bounded whole-owner resource accounting and unchanged workload/limits; finish workspace and SDK checks. |
...
| S3 | C# immutable API and release package | C# SDK/native bridge | Canonical prepared-operation bindings and exact faucet claim metadata, immutable results and parse boundary, allocation benchmarks, reviewed API surface and public-member docs; pinned .NET formatting/build/sample/package checks plus native privacy/Hijiri/SoraFS tests and real Windows packaging execution. |
...
| ID | Outstanding outcome | Component owner | Completion criteria |
| --- | --- | --- | --- |
| C1 | Canonical Norito archive and derive APIs | Norito, derive family, primitives and MV | Finish external explicit-context callers and map/set/generated-record cleanup; admit scalar backends and final MV tree allocations through their actual owners. One fallible aligned-storage helper with allocation accounting; owning/borrowed archive context replaces global marker state; retire consumerless experimental adapters. Share generated-emitter implementation; production reachability, compile-time/token-size budgets and strict UI tests. |
...
| C4 | FASTPQ proof architecture and soundness | FASTPQ prover/verifier and independent reviewers | Complete actual full-domain proofs and fresh artifacts for the compiled six-lane compact V1 owner; replace the production replay representation with bounded-opening admission that meets the unchanged byte/work limits. Complete witness-privacy, AIR/FRI and independent qualification. Validate protocol adversary/qROM multi-artifact reduction and independent permutation implementation; bind reviewed parameters to regenerated final proof artifacts before activation. |
...
| ID | Outstanding outcome | Component owner | Completion criteria |
| --- | --- | --- | --- |
| R1 | Audited clean candidate and reproducible artifacts | Release engineering and every component owner | Complete the Core Norito identity migration and rerun the corrected [SoraCloud regression suite](docs/soracloud_validation.md), including default-stack lease execution and snapshot replay. Resolve named audit dispositions and merged failures; full locked workspace build/tests, strict applicable feature/Clippy matrix, formatting, canonical ABI/wire fixtures and fresh native/SDK checks. Complete source/artifact/compiler identity and independent signed envelopes; blocked/timeouts never count as passes. See [audit matrix](docs/audit_closeout_matrix.md) and [release runbook](specs/release_runbook.md). |
