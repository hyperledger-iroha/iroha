...
- [ ] Reproduce 4-peer DEBUG 1s localnet subscriber-queue drops with bulk ping load after relay worker-queue/backpressure gating, unresolved-RBC backlog deferrals, queue hash compaction + queued-len gating, and the localnet NPoS bootstrap stake/gas defaults + empty-child fallback toggle; capture per-topic drop counters and Torii tx-rate-limit behavior to isolate any remaining flood sources (run2 with `--count 3000 --parallel 16` showed no queue-full logs; run20 2h soak with 2 lanes/peer `--count 15000 --parallel 48` saw 0 queue drops/view-changes/429s; per-topic drop counters still pending).
- [ ] Re-run a 7-peer localnet load after the BlockSyncUpdate blocking enqueue change to confirm block height advances under block-queue pressure and updates are no longer dropped.
