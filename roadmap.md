...
Latest sync (IvmProved execution verification): `Executable::IvmProved` admission now verifies `halo2/ipa:ivm-execution-v1` envelopes end-to-end: (1) public-input binding to `(code_hash, overlay_hash, events_commitment, gas_policy_commitment)`, (2) backend proof verification, and (3) deterministic IVM replay that must reproduce the submitted overlay and commitments. Gas usage is committed (not exposed) via `gas_policy_commitment`; `ivm-execution-v1` now uses a canonical public-input schema descriptor/hash for VK binding (instances are checked directly from proof bytes). The legacy `halo2/ipa:ivm-overlay-bind-v1` binding-only circuit remains explicitly rejected, and Torii `v1/zk/ivm/prove` jobs now include in-memory TTL/cap GC.
