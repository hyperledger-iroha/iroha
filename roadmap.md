...
- [ ] Re-run remaining `tmp_failures.txt` cases (proof events/queries, extra_functional suites, sumeragi_da) after seeding the genesis commit roster on commit to confirm consensus recovery. `extra_functional::unregister_peer` now enforces peer sync after the block-sync RBC delivery fix; rerun pending once disk space is available.
