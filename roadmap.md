...
- [ ] Re-run the previously failing suites (connected_peers, unstable_network, sumeragi_da, time_trigger, npos_pacemaker) with the now more conservative default test-network parallelism (peers-per-network divisor 64) to confirm the resource-contention fix holds. Status: connected_peers ok; unstable_network 5/8/9-2/9-3 ok; time_trigger ok; sumeragi_da individual reruns ok (full suite still timing out); npos_pacemaker ok.
...
- [ ] Re-run Izanami 1 TPS (300s/200 blocks) and confirm `commit pipeline defers validation` no longer appears and block height reaches target.
 ...
 - [ ] Re-run `cargo test -p integration_tests --test sumeragi_localnet_smoke permissioned_localnet_soak_thousands -- --ignored --nocapture` to confirm the thousands-block soak stays stable (2026-01-29 run timed out after 20m with min_non_empty ~146 and queue_size ~205–225; network dir `/var/folders/7l/w31n0ppj4zg874c4szhllss00000gn/T/irohad_test_network_eFE2vf`).
...
24. **DOCS-I18N-STUBS — Replace auto-generated translation stubs** (Docs, Line: Shared, Owner: Docs WG, Priority: Low, Status: 🈺 In Progress, target TBD)
 ...
 - [ ] Replace stub translation files across repo root, `docs/source`, and `docs/portal` (preserve front matter, keep `source_hash`/`translation_last_reviewed` aligned).
   ...
   - [ ] Remaining portal/reference/versioned stubs (other areas still pending).
   - [ ] Portal i18n stub copies under `docs/portal/i18n/*/docusaurus-plugin-content-docs` that mirror the above reference content.
   - [ ] Repo root + `docs/source` stub translations (README/portal/devportal/SDK stubs).
