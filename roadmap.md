...
| --- | --- | --- | --- |
| A1 | Foundational, privacy and service model boundaries | `iroha_data_model`, model crates, Norito/schema | Independent cohesive units with acyclic dependencies; preserve mandatory aggregate protocol JSON for admission and consensus parameters while qualifying supported feature graphs. Complete the active declared-identity codec cutover across model and workspace consumers, preserve exact named/frame goldens, rerun default-stack state tests and then extract physical model crates. |
