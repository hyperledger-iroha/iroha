...
- **NPOS-PENALTY-ADJUDICATION — Move VRF/evidence penalty enforcement on-chain so validator rosters remain deterministic** (Consensus/NPoS, Line: Iroha 3, Owner: Sumeragi Core, Priority: High, Status: 🈺 In Progress, target TBD)
...
0. **KOTODAMA-LANG-PARITY — Close remaining Kotodama gaps** (IVM/Kotodama, Line: Shared, Owner: IVM WG, Priority: Medium, Status: 🈺 In Progress, target TBD)
 ...
 - [ ] Wire trigger DSL descriptors into deploy/runtime scheduling (beyond manifest-only metadata).
 - [ ] Implement `kotoba { ... }` localization blocks beyond the current no-op stub.
 ...
 - [ ] Expand access hints for opaque host-driven reads (define safe fallback keys and coverage rules).
...
3. **INTEGRATION-TEST-REVALIDATION — Re-run integration tests after Sumeragi drain-order fix** (QA/Consensus, Line: Shared, Owner: QA WG, Priority: High, Status: 🈺 In Progress, target TBD)
 ...
 - [x] Re-run `cargo test -p integration_tests --test asset client_add_asset_with_decimal_should_increase_asset_amount -- --nocapture` to confirm DA delivery gating no longer stalls decimal mint flows.
 - [x] Re-run `cargo test -p integration_tests --test asset client_add_asset_quantity_to_existing_asset_should_increase_asset_amount -- --nocapture` and `cargo test -p integration_tests --test asset find_rate_and_make_exchange_isi_should_succeed -- --nocapture` after the sync retry budget + asset test sync updates to confirm asset flows no longer time out on tx confirmation.
...
- [ ] Resolve build-directory lock timeouts so full-workspace test runs complete reliably (standardize `CARGO_TARGET_DIR` or serialize builds).
...
18. **SORAFS-OPS-READINESS — Durable repair state, self-healing, and GC** (SoraFS/Core/Storage/Ops, Line: Iroha 3, Owner: SoraFS WG, Priority: High, Status: 🈺 In Progress, target TBD)
 - [ ] First-release reset: define the canonical durable repair/GC schemas and remove the in-memory-only repair ticket storage (no compatibility shims).【crates/sorafs_node/src/repair.rs】
 ...
 - [ ] Config: add `sorafs.repair` + `sorafs.gc` settings (enabled flags, DB DSN/pool, claim/heartbeat TTLs, max attempts, worker concurrency/backoff, GC cadence/batch limits, retention grace) in `iroha_config` defaults/user/actual and thread into the node wiring (no env-only toggles).【crates/iroha_config/src/parameters/{defaults,actual,user}.rs】【crates/sorafs_node/src/config.rs】
 - [ ] Config keys: enumerate and document explicit settings for `sorafs.repair.*` (db pool, claim TTL, heartbeat interval, max attempts, concurrency, backoff) and `sorafs.gc.*` (interval, grace window, max deletions, pre-admission sweep) with deterministic defaults.
 - [ ] Defaults/samples: update default config templates with the new `sorafs.repair`/`sorafs.gc` sections and sane first-release values.
 - [ ] Torii surface: extend repair status endpoints with stable sorting + filters (status/provider) and add a global list endpoint for ops dashboards.
 - [ ] Torii worker APIs: add claim/heartbeat/complete/fail endpoints for repair workers with idempotency keys and lease validation (reject stale/out-of-order updates).
 - [ ] Authz: extend the SoraFS RBAC token model to cover repair worker endpoints with provider-owned keys/delegations (no admin-only bypass; update guardrail lint + docs).
 - [ ] Worker identity: define how providers register/rotate repair worker keys (governance or provider-owned config) and enforce signatures on worker actions.
 - [ ] Decentralized invariants: prohibit admin-only repair/GC overrides in production paths; all actions must be provable by provider/worker identities or governance approvals.
 - [ ] CLI tooling: add `iroha_cli sorafs repair` subcommands (list/claim/complete/fail/escalate) and GC inspection/dry-run reporting only; no privileged manual GC in production.
 ...
 - [ ] GC scheduling: add a background sweeper with configurable cadence/limits and a pre-admission GC attempt when capacity is exhausted before rejecting new pins (automatic on every node; no central admin trigger).
