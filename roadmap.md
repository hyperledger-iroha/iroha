...
work appear here.
Latest sync: WSV mock unregister permission cleanup; merge QC view lane-tip wiring, VRF epoch snapshot persistence for NPoS scheduling/telemetry, and censorship evidence attribution; no new open items added, but integration-test re-runs remain open (see `status.md`).
