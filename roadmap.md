...
0. **IVM-TRIGGER-CACHE-VERIFY — Re-run Izanami 1 TPS after trigger IVM cache** (Consensus/Perf, Line: Shared, Owner: Core WG, Priority: Medium, Status: 🈳 Not Started, target TBD)
 - [ ] Re-run Izanami 1 TPS with `RUST_LOG=iroha_core::sumeragi::main_loop=debug` and confirm `block validation timings` execution spikes drop and `commit pipeline defers validation` lines disappear or shrink (2026-01-28 run: committed height ~54, execution_ms p90 ~1529ms p95 ~1599ms p99 ~3019ms, commit pipeline defers validation still present 35–43 times/peer; logs in `/var/folders/7l/w31n0ppj4zg874c4szhllss00000gn/T/irohad_test_network_NQSMVs`).
...
- [ ] Telemetry + tests: add pacing-factor fields to `/v1/sumeragi/status` and cover governor scale up/down decisions (effective timing fields + Norito/JSON round-trip coverage landed).
