...
Latest sync (IvmProved execution verification): `Executable::IvmProved` admission now verifies `halo2/ipa:ivm-execution-v1` envelopes end-to-end: (1) public-input binding to `(code_hash, overlay_hash)`, (2) backend proof verification, and (3) deterministic IVM replay that must reproduce the submitted overlay. The legacy `halo2/ipa:ivm-overlay-bind-v1` binding-only circuit remains explicitly rejected.
