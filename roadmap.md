...
20. **IVM-ABI-SYSCALL-COMPLETION — Implement remaining ABI v1 syscalls in production CoreHost** (IVM/Core, Line: Shared, Owner: IVM WG, Priority: High, Status: 🈳 Not Started, target TBD)
 - [ ] Lifecycle/debug syscalls: define behavior + gas for `SYSCALL_EXIT`, `SYSCALL_ABORT`, `SYSCALL_DEBUG_LOG`, `SYSCALL_DEBUG_PRINT`; update VM runtime/error semantics as needed and align `crates/ivm/docs/syscalls.md` + generated tables.
 - [ ] Implement lifecycle/debug syscall handling in `crates/iroha_core/src/smartcontracts/ivm/host.rs` with deterministic behavior (no env gating); mirror any shared helpers in `crates/ivm/src/host.rs` and update `crates/ivm/tests/syscalls.rs`.
 - [ ] Account/peer/signatory/quorum ISIs: implement pointer-ABI decode + ISI queueing for `SYSCALL_REGISTER_ACCOUNT`, `SYSCALL_UNREGISTER_ACCOUNT`, `SYSCALL_REGISTER_ASSET`, `SYSCALL_UNREGISTER_ASSET`, `SYSCALL_REGISTER_PEER`, `SYSCALL_UNREGISTER_PEER`, `SYSCALL_ADD_SIGNATORY`, `SYSCALL_REMOVE_SIGNATORY`, `SYSCALL_SET_ACCOUNT_QUORUM`; add unit tests for TLV decoding + queued instructions.
 - [ ] Roles/permissions ISIs: implement `SYSCALL_CREATE_ROLE`, `SYSCALL_DELETE_ROLE`, `SYSCALL_GRANT_ROLE`, `SYSCALL_REVOKE_ROLE`, `SYSCALL_GRANT_PERMISSION`, `SYSCALL_REVOKE_PERMISSION` mapping + tests.
 ...
 - [ ] ZK/public input/VRF helpers: implement `SYSCALL_GET_PUBLIC_INPUT`, `SYSCALL_GET_ACCOUNT_BALANCE`, `SYSCALL_USE_NULLIFIER`, `SYSCALL_ZK_VERIFY_BATCH`, `SYSCALL_VRF_VERIFY`, `SYSCALL_VRF_VERIFY_BATCH` with WSV-backed storage where required; add tests and update ZK docs.
 - [ ] Merkle helpers: wire `SYSCALL_GET_MERKLE_COMPACT` and `SYSCALL_GET_REGISTER_MERKLE_COMPACT` to the VM merkle helpers with deterministic roots; add regression coverage.
 - [ ] Documentation + ABI sync: update syscall docs/golden tests and ensure ABI hashing remains unchanged or is intentionally versioned; keep behavior deterministic across hardware and peers.
