...
20. **IVM-ABI-SYSCALL-COMPLETION — Implement remaining ABI v1 syscalls in production CoreHost** (IVM/Core, Line: Shared, Owner: IVM WG, Priority: High, Status: 🈺 In Progress, target TBD)
 ...
 - [ ] Signatory/quorum ISIs: implement `SYSCALL_ADD_SIGNATORY`, `SYSCALL_REMOVE_SIGNATORY`, `SYSCALL_SET_ACCOUNT_QUORUM` once the v3 data model exposes the required ISIs; add unit tests for TLV decoding + queued instructions.
