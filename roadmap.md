...
- The focused SCCP prover corridor is green for the current production-hardening
  ...
  EVM/TRON Groth16 contract smoke coverage for post-generation payload/finality
  public-signal drift.
- EVM route-canary evidence now uses a v2 transcript aligned with the TRON
  ...
  evidence launch-ready. The full SCCP production corridor passes end to end
  with Rust SCCP verification, operator evidence scripts, JS/Python/Swift/
  Kotlin/Java Android SDK prover surfaces, EVM/TRON contract smoke, and core
...
- The focused SCCP production corridor is now captured by
  ...
  recompute every attachment hash and catch extra manifest artifacts that are
  not referenced by the readiness report, unknown corridor evidence phase keys,
  ...
  unmanifested or omitted required/phase artifacts, release notes that omit the
  manifest handoff, embedded
  ...
  from the copied evidence artifacts, non-canonical readiness-report artifact
  paths, missing or unknown manifest/readiness-report top-level fields,
  manifest readiness header drift from the report and summary, unknown embedded
  or standalone
  ...
  hash drift, route-canary route/destination hash drift from sibling lane
  evidence, cryptographic evidence row domain/chain drift from embedded lane
  rows, unknown manifest
  ...
  cryptographic evidence table from the embedded lane evidence so a
  release note cannot drift from the governed source, destination, route, or
  canary hashes that passed all-lanes preflight, and it revalidates each copied
...
- Keep SCCP bridge submission permissionless while requiring outbound message
  ...
  material record hash or source-adapter deployment record hash. The
...
live-frontier idle missing-QC suppression helper gate
(`live-frontier-idle-missing-qc`),
missing-QC reacquire admission helper gate
(`missing-qc-reacquire-admission`),
missing-QC reacquire action orchestration helper gate
(`missing-qc-reacquire-action`),
missing commit-QC actionable dependency helper gate
