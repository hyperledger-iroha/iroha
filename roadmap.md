...
Latest sync (config): NPoS config now derives optional timeouts/VRF windows from `block_time_ms`/`epoch_length_blocks`, with docs and unit coverage refreshed; minimal config fixtures now include PoP coverage and the config snapshot matches current crypto defaults.
...
- [x] Re-run Izanami 4-peer DA runs to confirm no `FrameTooLarge` disconnects and consensus reaches target blocks (2026-01-23 run: `IROHA_TEST_NETWORK_PERMIT_DIR=$(mktemp -d) target/debug/izanami --allow-net --nexus --peers 4 --faulty 0 --duration 300s --target-blocks 20 --progress-interval 10s --progress-timeout 180s --tps 5 --max-inflight 8 --workload-profile stable` completed with 110 successes/1 failure; no `FrameTooLarge` observed; duplicate metric registration warnings persisted).
