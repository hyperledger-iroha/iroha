...
- [ ] Re-run `cargo test -p integration_tests --test sumeragi_localnet_smoke permissioned_localnet_soak_thousands -- --ignored --nocapture` after the precommit-signer block-sync QC fallback and stricter uncertified roster acceptance to confirm the thousands-block soak stays stable.
...
- [ ] Re-run `cargo test --workspace` after the Sumeragi gap fixes; latest attempt (`CARGO_TARGET_DIR=target/codex-trigger`) timed out after ~20m during compilation.
...
25. **SUBSCRIPTION-TRIGGER-API — Trigger-based subscriptions for arrears + fixed billing** (Core/DataModel/Contracts/Torii, Line: Shared, Owner: App Platform WG, Priority: Medium, Status: 🈺 In Progress, target TBD)
 ...
 - [ ] Add Torii endpoints for plans/subscriptions (create/list/get/pause/resume/cancel/charge-now) and expose subscription listing queries.
 - [ ] Add CLI/SDK helpers for plan publishing, subscribe/cancel, usage reporting, and subscription management.
 - [x] Tests: unit coverage for calendar math + subscription billing/usage syscalls.
 - [ ] Tests: integration coverage with 4 peers for arrears usage billing, fixed-price advance billing, and retry/grace failure paths.
 - [ ] Telemetry/events: counters for billing attempts/outcomes and TriggerCompleted filters for subscription billing; update docs/runbooks.
