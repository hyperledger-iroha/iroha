...
Latest sync (config): NPoS config now derives optional timeouts/VRF windows from on-chain `SumeragiParameters.block_time_ms`/`epoch_length_blocks` (fallback to `sumeragi.npos.block_time_ms` pre-genesis) with warnings on deprecated `sumeragi_npos_parameters.block_time_ms`; docs/templates refreshed and minimal config fixtures now include PoP coverage and the config snapshot matches current crypto defaults.
...
0. **KOTODAMA-LANG-PARITY — Close remaining Kotodama gaps** (IVM/Kotodama, Line: Shared, Owner: IVM WG, Priority: Medium, Status: 🈺 In Progress, target TBD)
 ...
 - [x] Wire trigger DSL descriptors into deploy/runtime scheduling (activation registers/removes manifest triggers).
 ...
 - [x] Expand access hints for dynamic map keys (emit safe map-level conflict keys).
...
0. **IVM-TRIGGER-CACHE-VERIFY — Re-run Izanami 1 TPS after trigger IVM cache** (Consensus/Perf, Line: Shared, Owner: Core WG, Priority: Medium, Status: 🈳 Not Started, target TBD)
 - [ ] Re-run Izanami 1 TPS with `RUST_LOG=iroha_core::sumeragi::main_loop=debug` and confirm `block validation timings` execution spikes drop and `commit pipeline defers validation` lines disappear or shrink.
...
- [ ] Re-run `cargo test -p integration_tests --test sumeragi_localnet_smoke -- --nocapture` after the commit-topology fallback in `apply_without_execution` plus vote/QC deferral, roster-caching, deferred replay, commit-finalize deferral until tip extends, stalled-RBC flush/roster-derivation fixes, and NEW_VIEW future-view gating relaxation to confirm empty-roster stalls are gone (timed out after 300s; `permissioned_localnet_produces_blocks_within_bound` passed; `permissioned_localnet_reaches_100_blocks` still running; latest rerun failed: `permissioned_localnet_reaches_100_blocks` after ~374s, bound test passed; targeted rerun with `IROHA_TEST_NETWORK_KEEP_DIRS=1` failed after ~328s with height stalled at 2/101, logs in `/var/folders/7l/w31n0ppj4zg874c4szhllss00000gn/T/irohad_test_network_rQaNqe`; 2026-01-18 rerun `CARGO_TARGET_DIR=/tmp/iroha-codex-roadmap-target IROHA_TEST_NETWORK_PARALLELISM=1 IROHA_TEST_NETWORK_PERMIT_DIR=$(mktemp -d) cargo test -p integration_tests --test sumeragi_localnet_smoke -- --nocapture --test-threads=1` failed with `permissioned_localnet_produces_blocks_within_bound` and `permissioned_localnet_reaches_100_blocks` stalled at height 2/101 within 230s, no KEEP_DIRS; 2026-01-18 rerun with `IROHA_TEST_NETWORK_KEEP_DIRS=1` stalled at height 5/3/target 101 after 230s, logs in `/var/folders/7l/w31n0ppj4zg874c4szhllss00000gn/T/irohad_test_network_bpZ07y`; commit-QC history fallback for missing cached QCs landed; 2026-01-18 rerun with `IROHA_TEST_NETWORK_KEEP_DIRS=1` stalled at height 2/101 within 230s, logs in `/var/folders/7l/w31n0ppj4zg874c4szhllss00000gn/T/irohad_test_network_IFXgRd`).
- [ ] Re-run `cargo test -p integration_tests --test sumeragi_localnet_smoke -- --nocapture` after raising the RBC chunk fanout baseline to commit quorum, blocking relay enqueue for RBC READY/DELIVER, routing RBC READY/DELIVER over the high-priority consensus topic, retaining stale-view RBC/missing-block state in DA mode until payloads arrive, emitting local READY after READY quorum from peers, allowing derived roster rebroadcasts plus missing-block fetches on pending-stash drops, and keeping locally derived RBC rosters non-authoritative until INIT to reduce READY quorum stalls (latest run stalled at height 15; logs in `/var/folders/7l/w31n0ppj4zg874c4szhllss00000gn/T/irohad_test_network_KKBHHS`; 2026-01-18 rerun with the same command stalled at height 2/101 within 230s; 2026-01-18 rerun with `IROHA_TEST_NETWORK_KEEP_DIRS=1` stalled at height 5/3/target 101 after 230s, logs in `/var/folders/7l/w31n0ppj4zg874c4szhllss00000gn/T/irohad_test_network_bpZ07y`).
- [ ] Re-run `cargo test -p integration_tests --test sumeragi_localnet_smoke permissioned_localnet_soak_thousands -- --ignored --nocapture` after the per-entry StateView block-sync validation, async block-sync enqueue (spawn_blocking to avoid holding StateView while waiting on Sumeragi backpressure), soak backpressure tuning, deterministic f+1 RBC payload/READY rebroadcast senders, and P2P subscriber-cap + non-blocking relay ingress updates to confirm the thousands-block soak stays stable (queue-drain waits track progress; `IROHA_SOAK_TARGET_BLOCKS`, `IROHA_SOAK_SUBMIT_BATCH`, `IROHA_SOAK_QUEUE_SOFT_LIMIT` support shorter local runs; latest attempt with `IROHA_SOAK_TARGET_BLOCKS=200` failed to compile `iroha_torii` (Handler trait errors in `crates/iroha_torii/src/lib.rs`); previous attempt timed out after 900s at min_non_empty=42/201; current attempt with `IROHA_TEST_NETWORK_PARALLELISM=5` failed after ~18m at min_non_empty=87 with queue-drain timeout and status poll stalls; debug rerun with `IROHA_SOAK_TARGET_BLOCKS=100` hit a sumeragi stack overflow from recursive RBC persist polling, fix applied and rerun pending; 2026-01-18 rerun `IROHA_SOAK_TARGET_BLOCKS=200 IROHA_TEST_NETWORK_KEEP_DIRS=1 CARGO_TARGET_DIR=/tmp/iroha-codex-roadmap-target IROHA_TEST_NETWORK_PARALLELISM=1 IROHA_TEST_NETWORK_PERMIT_DIR=$(mktemp -d) cargo test -p integration_tests --test sumeragi_localnet_smoke permissioned_localnet_soak_thousands -- --ignored --nocapture --test-threads=1` stalled at min_non_empty=3/target 201 after 40s, logs in `/var/folders/7l/w31n0ppj4zg874c4szhllss00000gn/T/irohad_test_network_QIrHw7`).
...
- [ ] Re-run `cargo test -p integration_tests --test mod -- --nocapture` with `API_ADDRESS`/`PUBLIC_KEY` env overrides set to confirm test-network peers ignore host env config overrides (attempted 2026-01-21 with `IROHA_TEST_NETWORK_PARALLELISM=1` and `=2`/`--test-threads=2`; both ran ~1h and timed out, no explicit failures before timeout; 2026-01-21 rerun with `IROHA_TEST_NETWORK_PARALLELISM=1` + `--test-threads=1` timed out after 2h, last observed running `triggers::by_call_trigger::call_execute_trigger`, which passes when run alone with env overrides).
...
5. **SUMERAGI-PARAM-STREAMLINE — Reduce operator-facing consensus knobs** (Consensus, Line: Shared, Owner: Core WG, Priority: Medium, Status: 🈳 Not Started, target TBD)
- [x] Spec: `docs/source/sumeragi_parameter_streamlining.md`.
- [ ] Parameter surface: keep only the minimal on-chain/operator knobs (block/commit/min_finality/clock_drift, collectors, DA enable) and treat all other timing knobs as derived-by-default.
- [ ] Config shape: introduce `sumeragi.advanced` (or equivalent) for override-only knobs (pacemaker backoff/jitter caps, DA multipliers/floors, RBC TTL/storage, worker budgets); ensure deterministic fallbacks.
- [ ] NPoS overrides: move timeout overrides to the advanced section (or remove them) and derive from `block_time_ms` when unset; update validation to allow 0/None as "derive".
- [ ] Observability: extend `/v1/sumeragi/status` with the remaining effective derived fields not covered by the pacing task and document the new surface.
- [ ] Docs/templates: refresh config templates and docs to match the streamlined surface (no backward-compat required for first release).
...
- [x] Re-run the Izanami 1 TPS profile with auto-scaled `pipeline.workers` and `sumeragi.worker.validation_*` (2026-01-28 run stalled with committed height 26; vote_drain_ms mean 266ms p95 1156 max 3378; logs in `/var/folders/n2/xxntlr312qbfdnp0j1xp52hw0000gn/T/irohad_test_network_Wk33iK`).
- [ ] Investigate per-peer commit persistence stalls (Izanami `irohad_test_network_o6jN6V`: swaying_deer never logged `stored committed block to kura` for height 40 while other peers did); add commit→Kura instrumentation and correlate with state-lock contention.
...
- [ ] Split `execution_tx_apply_ms` into apply sub-stages (detached exec vs merge vs sequential fallback vs trigger execution vs finalization) to pinpoint the dominant apply hot spot before optimizing.
...
18. **SORAFS-OPS-READINESS — Durable repair state, self-healing, and GC** (SoraFS/Core/Storage/Ops, Line: Iroha 3, Owner: SoraFS WG, Priority: High, Status: 🈺 In Progress, target TBD)
 - [x] First-release reset: define the canonical durable repair/GC schemas and remove the in-memory-only repair ticket storage (no compatibility shims).【crates/sorafs_node/src/repair.rs】
 - [x] Repair persistence: add file-backed storage for repair tickets + PoR failure history, load on startup, and make state transitions atomic while preserving SLA deadlines.【crates/sorafs_node/src/repair.rs】【crates/sorafs_node/src/lib.rs】
 - [x] Repair store abstraction: define a `RepairStore` interface (file-backed) with compare-and-set updates, stable Norito payload storage, and deterministic task listing for status queries.
 - [x] On-disk schema: versioned Norito snapshot (`repair_state.to`) storing tickets, evidence, state transitions, and PoR history; document retention rules in ops notes.
 ...
 - [x] Config: add `sorafs.repair` + `sorafs.gc` settings (enabled flags, state dir, claim/heartbeat TTLs, max attempts, worker concurrency/backoff, GC cadence/batch limits, retention grace) in `iroha_config` defaults/user/actual and thread into the node wiring (no env-only toggles).【crates/iroha_config/src/parameters/{defaults,actual,user}.rs】【crates/sorafs_node/src/config.rs】
 - [x] Config keys: enumerate and document explicit settings for `sorafs.repair.*` (state dir, claim TTL, heartbeat interval, max attempts, concurrency, backoff) and `sorafs.gc.*` (interval, grace window, max deletions, pre-admission sweep) with deterministic defaults.
 ...
 - [x] Torii surface: extend repair status endpoints with stable sorting + filters (status/provider) and add a global list endpoint for ops dashboards.
- [x] Torii worker APIs: add claim/heartbeat/complete/fail endpoints for repair workers with idempotency keys and lease validation (reject stale/out-of-order updates).
- [x] Authz: extend the SoraFS RBAC token model to cover repair worker endpoints with provider-owned keys/delegations (no admin-only bypass; update guardrail lint + docs).
- [x] Worker identity: define how providers register/rotate repair worker keys (governance or provider-owned config) and enforce signatures on worker actions.
- [x] Decentralized invariants: prohibit admin-only repair/GC overrides in production paths; all actions must be provable by provider/worker identities or governance approvals.
- [x] CLI tooling: add GC inspection/dry-run reporting only; no privileged manual GC in production.
...
- [x] Repair prioritisation: sort by SLA deadline, failure severity, and provider impact; keep ordering deterministic to avoid oscillations under load.
 ...
 - [x] Telemetry (repair): add gauges/counters for repair backlog age and lease expiries; surface in Prometheus + OpenTelemetry.
   ...
   - [x] Add OpenTelemetry histograms for backlog age/queue depth and counters for lease expiries.
   - [x] Add unit tests that validate new metrics emit deterministic labels/values.
 - [x] Telemetry parity: ensure governance-published audit events carry the same IDs/labels as telemetry so cross-system correlation is deterministic.
   - [x] Add manifest/provider IDs to repair audit events and include ticket_id/manifest/provider/status/outcome in governance JSON metadata.
 ...
 - [x] Docs/runbooks: update SoraFS operator guides with decentralized repair persistence, worker controls, GC policy/config/telemetry, and “no admin after launch” invariants.
 - [x] Docs/runbooks: include provider worker key rotation + evidence signing guidance and governance dispute timelines.
...
20. **IVM-ABI-SYSCALL-COMPLETION — Implement remaining ABI v1 syscalls in production CoreHost** (IVM/Core, Line: Shared, Owner: IVM WG, Priority: High, Status: 🈺 In Progress, target TBD)
 - [x] Lifecycle/debug syscalls: define behavior + gas for `SYSCALL_EXIT`, `SYSCALL_ABORT`, `SYSCALL_DEBUG_LOG`, `SYSCALL_DEBUG_PRINT`; update VM runtime/error semantics as needed and align `crates/ivm/docs/syscalls.md` + generated tables.
 - [x] Implement lifecycle/debug syscall handling in `crates/iroha_core/src/smartcontracts/ivm/host.rs` with deterministic behavior (no env gating); mirror any shared helpers in `crates/ivm/src/host.rs` and update `crates/ivm/tests/syscalls.rs`.
 - [x] Account/peer/asset ISIs: implement pointer-ABI decode + ISI queueing for `SYSCALL_REGISTER_ACCOUNT`, `SYSCALL_UNREGISTER_ACCOUNT`, `SYSCALL_REGISTER_ASSET`, `SYSCALL_UNREGISTER_ASSET`, `SYSCALL_REGISTER_PEER`, `SYSCALL_UNREGISTER_PEER`; add unit tests for TLV decoding + queued instructions.
- [x] Signatory/quorum ISIs: implement `SYSCALL_ADD_SIGNATORY`, `SYSCALL_REMOVE_SIGNATORY`, `SYSCALL_SET_ACCOUNT_QUORUM` via multisig spec metadata + role updates; add unit tests for TLV decoding + queued instructions.
 - [x] Roles/permissions ISIs: implement `SYSCALL_CREATE_ROLE`, `SYSCALL_DELETE_ROLE`, `SYSCALL_GRANT_ROLE`, `SYSCALL_REVOKE_ROLE`, `SYSCALL_GRANT_PERMISSION`, `SYSCALL_REVOKE_PERMISSION` mapping + tests.
 - [x] Trigger ISIs: implement `SYSCALL_CREATE_TRIGGER`, `SYSCALL_REMOVE_TRIGGER`, `SYSCALL_SET_TRIGGER_ENABLED` (Norito request payloads + TLV decoding), add tests for trigger lifecycle, and update Kotodama hints/docs if required.
 - [x] ZK/public input/VRF helpers: implement `SYSCALL_GET_PUBLIC_INPUT`, `SYSCALL_GET_ACCOUNT_BALANCE`, `SYSCALL_USE_NULLIFIER`, `SYSCALL_ZK_VERIFY_BATCH`, `SYSCALL_VRF_VERIFY`, `SYSCALL_VRF_VERIFY_BATCH`; add tests and update ZK docs.
 - [x] Merkle helpers: wire `SYSCALL_GET_MERKLE_COMPACT` and `SYSCALL_GET_REGISTER_MERKLE_COMPACT` to the VM merkle helpers with deterministic roots; add regression coverage.
 - [x] Documentation + ABI sync: update syscall docs/golden tests and ensure ABI hashing remains unchanged or is intentionally versioned; keep behavior deterministic across hardware and peers.
...
42. **FASTPQ-PUBLIC-INPUTS — Finalize public input commitments** (Core/FASTPQ, Line: Iroha 3, Owner: FASTPQ WG, Priority: Medium, Status: 🈺 In Progress, target TBD)
 - [x] Replace entry-hash-derived `tx_set_hash` with scheduler-provided transaction set commitments.
 - [x] Populate `perm_root` from the permission table commitment once WSV wiring lands.
