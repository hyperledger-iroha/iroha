...
work appear here.
Latest sync: ignore non-extending pending blocks for proposal/view-change gating (localnet hang fix); reject zero-chunk RBC INITs and add empty-payload hydration guardrails; add periodic RBC payload/READY rebroadcasts for stalled sessions; WSV mock unregister permission cleanup; merge QC view lane-tip wiring, VRF epoch snapshot persistence for NPoS scheduling/telemetry, and censorship evidence attribution; parallelize localnet status polling to avoid serial Torii stalls; SoraFS proof token decode invariants hardened (empty entries/expiry ordering); integration-test re-runs remain open (see `status.md`); harden block sync peer sampling and invalidate bridge MMR cache on top-block rewrites; regression coverage added (see `status.md` for details).
