...
- [ ] Re-run `cargo test -p integration_tests --test sumeragi_localnet_smoke -- --nocapture` after the commit-topology fallback in `apply_without_execution` plus vote/QC deferral, roster-caching, deferred replay, and stalled-RBC flush/roster-derivation fixes to confirm empty-roster stalls are gone (timed out after 300s; `permissioned_localnet_produces_blocks_within_bound` passed; `permissioned_localnet_reaches_100_blocks` still running).
