...
| N11 | Atomic private settlement qualification | Core/private settlement, Nexus, Torii, native proof and evidence owners | [Canonical protocol](specs/private_settlement.md): QC-bound historical authority and one immutable merge view; unchanged-source adversarial/native/PQ full proofs; ten fresh 16-process N=3 successes, then N=2,3,4,8,16 faults at ten seeds each. Paired leakage, five warmups/thirty measured bundles and transparent controls; complete formal/independent audit, signed reproducible builds, SBOM, custody and validated release evidence. Remain disabled until all gates pass. |
...
[FASTPQ plan](specs/fastpq_plan.md) and the detailed historical
...
| C4 | FASTPQ proof architecture and soundness | FASTPQ prover/verifier and independent reviewers | Choose one justified verifier path: bounded-opening quotient/degree argument or explicit full replay. Validate protocol adversary/qROM multi-artifact reduction and independent permutation implementation; bind reviewed parameters to regenerated final proof artifacts before activation. |
| C5 | Privacy proof authority and degree bounds | ZK-ACE, native-STARK, AXT and IVM admission | Bind AXT exact finalized source state and issuer intent/amount; independent ≥128-bit ZK-ACE commitment binding and regenerated AIR/FRI certificate; explicit terminal degree/geometry checks; unsupported public-padding proof paths stay disabled. |
...
| C8 | Kaigi private-session model | Kaigi model, crypto, Core and SDKs | Roster proof binds signed participant authority and replay context; exact regenerated key/schema and cross-authority negatives. Shared nine-instruction SDK fixtures, suite-tagged HPKE, bounded long-session rolling usage accumulator, authenticated relay recovery/archive policy and measured signal-index startup. |
...
| P2 | SoraFS production promotion | SoraFS, operators and evidence owners | [V1 closure ledger](specs/sorafs/v1_closure_ledger.md) finding-free; live four-voter/multi-provider/dual-gateway L1 with resilience/load/24-hour soak and exactly 17 fresh summaries; ordered L2 promotion legs and trusted foundational envelope. Synthetic aggregates cannot authorize promotion. |
...
| P7 | Audited SCCP production corridors | SCCP circuits/contracts, runtime/native and release operators | Taira↔Ethereum/BSC/TRON/TON mainnet only. Regenerate reviewed circuits/keys/witness/runtime artifacts; real TRON TVM/TRE and exact pinned TON contracts/StateInit; authenticated deployment readback, positive/negative real-value canaries and independently reproduced signed release bundle. No fixtures, synthetic proofs or proof-controlled rosters as authority. |
...
| P9 | Persistent Taira joining and disposable networks | CLI/daemon and authorized network operators | One signed-bundle `iroha taira join` path initializes an owner-private permissionless observer, validates expiry/network/genesis and supports staking activation. Disposable four-validator `taira_devnet.py up` proves typed Applied, advancing convergence, MCP and guest workload; canonical source/artifact identity, no startup-only success or unmanaged cleanup. |
