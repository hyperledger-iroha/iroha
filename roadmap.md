...
- [ ] Reproduce 4-peer DEBUG 1s localnet subscriber-queue drops with bulk ping load after the relay worker-queue/backpressure-gating and unresolved-RBC backlog deferrals, capturing per-topic drop counters and Torii rate-limit behavior to isolate any remaining flood sources (run2 with `--count 3000 --parallel 16` showed no queue-full logs; try heavier load or adjust Torii high-load thresholds).
