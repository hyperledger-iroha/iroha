...
Latest sync (worker loop): tick scheduling now uses the minimum gap even when queues are backlogged so consensus progress is not throttled by pending drains; unit coverage added.
Latest sync (fixtures): SDK transaction payload fixtures now store wire-only instruction entries (no legacy kind/arguments), exporter strips legacy instruction fields on regen, legacy instruction-schema/double-payment JSON fixtures were removed across Android/Swift/Python, and fixture regen scripts no longer sync legacy schema files.
...
### Code / Runtime TODOs
- IVM CoreHost parity harness: add `host_roundtrip` (ivm) + `ivm_host_shadow_execute` (iroha_core) coverage so CoreHost results match native `Execute` paths; update STATUS once shadow-ready. (Source: `docs/source/ivm_isi_kotodama_alignment.ja.md`)
- IVM gas schedule enforcement: finish wiring canonical opcode gas tables into core limits/config checks and keep schedule validation deterministic across hosts. (Source: `docs/source/ivm_isi_kotodama_alignment.ja.md`)
