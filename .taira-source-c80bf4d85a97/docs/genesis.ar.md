...
1. توليد قالب (template):
   ...
   - عند اختيار `npos`، يزرع Kagami حمولة `sumeragi_npos_parameters` التي تضبط معدل fan-out للمجمعين، وسياسة الانتخاب، ونوافذ إعادة التهيئة؛ وتحولها عملية التطبيع/التوقيع إلى تعليمات `SetParameter` داخل الكتلة الموقعة.
...
- **المعلمات (Parameters)**: ضبط القيم الابتدائية لـ Sumeragi (أوقات
  ...
  يزرع Kagami `Sumeragi::NextMode` وpayload `sumeragi_npos_parameters` (الانتخاب، إعادة التهيئة)
