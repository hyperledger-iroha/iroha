...
<!-- الترجمة العربية لـ docs/connect_client_examples.md (Iroha Connect Client Examples) -->

## أمثلة عميل Iroha Connect (TypeScript و Kotlin)

يعرض هذا المستند مقاطع كود (snippets) بسيطة على جانب العميل تطبّق قواعد
الإصدار v0:
- بيانات الـ AAD في AEAD تربط الترويسة الخارجية (version, `sid`, `dir`,
  `seq`, `kind=Ciphertext`).
- يُشتق الـ nonce من `seq` (nonce وفق مواصفة IETF بطول 12 بايت:
  `0x00000000 || seq_le`).
- تُرسل إطارات التحكم بعد الموافقة (Post‑Approve) — مثل Close/Reject —
  ضمن بيانات مشفّرة.

هذه الأمثلة توضيحية فقط؛ تمّ إغفال عمليات التدقيق والتقوية (hardening)
المطلوبة في بيئة الإنتاج.

### TypeScript (libsodium + WebCrypto)

الاعتمادات: `libsodium-wrappers` (من أجل X25519 وBLAKE2b وChaCha20‑Poly1305)،
و WebCrypto (من أجل HKDF‑SHA‑256).
...
(async () => { await sodium.ready;
  ...
  // إنشاء جلسة: استدعاء POST /v1/connect/session مع sid محسوب على جانب العميل
...
fun main() {
  ...
  val shared = KeyAgreement.getInstance("X25519").apply {
    init(appKp.private)
    doPhase(appKp.public, true) // заменить на публичный ключ кошелька
  }.generateSecret()
