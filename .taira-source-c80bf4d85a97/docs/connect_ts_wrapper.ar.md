...
<!-- الترجمة العربية لـ docs/connect_ts_wrapper.md (TypeScript Connect Wrapper) -->

## غلاف TypeScript لـ Connect (انضمام WS + tokens)
...
// // بعد اشتقاق المفاتيح وبناء ConnectFrameV1 (مشفّر باستخدام Norito)، أرسل:
// ws.send(noritoEncodedBinaryFrame);
```

```ts
// تشفير SignResultOk (الحمولة فقط؛ تغليف Norito غير معروض هنا):
