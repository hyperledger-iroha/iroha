...
1. `.github/workflows/docs-portal-preview.yml` کو اپ ڈیٹ کریں تاکہ:
   ...
   - build ڈائریکٹری کو `artifacts/preview-site.tar.gz` کے طور پر پیک کریں, checksum مینی فیسٹ کاپی کریں, `scripts/generate-preview-descriptor.mjs` چلائیں، اور JSON کنفیگ کے ساتھ `scripts/sorafs-package-preview.sh` چلائیں (دیکھیں `docs/examples/sorafs_preview_publish.json`) Fluxo de trabalho de fluxo de trabalho O modelo SoraFS é um cartão de crédito
   - سائٹ، میٹا ڈیٹا آرٹیفیکٹس (`docs-portal-preview`, `docs-portal-preview-metadata`), اور SoraFS بنڈل (`docs-portal-preview-sorafs`). Isso é verdade
2. solicitação pull de soma de verificação میں GitHub Script کمنٹ قدم کے ذریعے نافذ ہے)۔
3. `docs/portal/README.md` (CI سیکشن) fluxo de trabalho کو دستاویزی شکل دیں اور lista de verificação de publicação میں verificação مراحل کا لنک دیں۔

## توثیقی اسکرپٹ

`docs/portal/scripts/preview_verify.sh` ڈاؤن لوڈ شدہ پریویو آرٹیفیکٹس کی تصدیق دستی `sha256sum` چلائے بغیر کرتا ہے۔ O valor do cartão de crédito é `npm run serve` (ou seja, `npm run serve:verified`). O cartão de crédito `docusaurus serve` é um cartão de crédito que pode ser usado O que fazer:
...
## Método 2 — SoraFS Arquivo1. Fluxo de trabalho de fluxo de trabalho میں ایسا جاب شامل کریں جو:
   - `sorafs_cli car pack` e `manifest submit` کے ذریعے تیار شدہ سائٹ کو SoraFS staging گیٹ وے پر اپ لوڈ کرے۔
...
2. Lista de verificação de publicação DOCS-SORA
...
| مرحلہ | Mal | ہدف | Não |
|-------|------|-----|------|
| CI میں checksum نفاذ مکمل | Docs Baixar | ہفتہ 1 | فیل گیٹ اور آرٹیفیکٹ اپ لوڈز شامل کرتا ہے۔ |
| SoraFS Imóveis | Docs Arquivo / Armazenamento | ہفتہ 2 | estadiamento کریڈینشلز تک رسائی اور Norito اسکیمہ اپ ڈیٹس درکار ہیں۔ |
| گورننس انٹیگریشن | Docs/DevRel Para / گورننس WG | ہفتہ 3 | اسکیمہ شائع کرتا ہے اور چیک لسٹس و روڈ میپ اندراجات اپ ڈیٹ کرتا ہے۔ |

## کھلے سوالات

- SoraFS کا کون سا ماحول پریویو آرٹیفیکٹس رکھے (staging بمقابلہ مخصوص faixa de visualização)؟
- کیا اشاعت سے پہلے پریویو ڈسکرپٹر پر دوہری دستخط (Ed25519 + ML-DSA) درکار ہیں؟
- Fluxo de trabalho CI کو `sorafs_cli` چلاتے e orquestrador کی ترتیب (`orchestrator_tuning.json`) فکس کرنی چاہئے تاکہ مینی فیسٹس قابلِ اعادہ رہیں؟
