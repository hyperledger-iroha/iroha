...
- A chave de segurança é `LaneSettlementReceipt`. كل ايصال يخزن معرف المصدر الذي يقدمه المستدعي والمقدار المجهري المحلي وXOR المستحق فورا وXOR المتوقع Corte de cabelo e corte de cabelo (`xor_variance`).
- Verifique se a faixa/espaço de dados está localizada em `lane_settlement_commitments` em `/v1/sumeragi/status`. Verifique os valores `total_local_amount`, `total_xor_due` e `total_xor_after_haircut` para obter mais informações sobre o produto. المطابقة الليلية.
- Você pode usar `total_xor_variance` para obter informações sobre o valor do XOR ou do XOR corte de cabelo), وتوثق `swap_metadata` معلمات التحويل الحتمية (TWAP e epsilon e perfil de liquidez e volatility_class) حتى يتمكن المدققون من التحقق من Você pode fazer isso com cuidado e sem problemas.
