...
- ہر ٹرانزیکشن جو گیس کی ادائیگی کرتا ہے وہ `LaneSettlementReceipt` کو رجسٹر کرتا ہے۔ ہر رسید کالر ، مقامی مائیکرو اماؤنٹ ، فوری طور پر XOR کی وجہ سے فراہم کردہ ماخذ شناخت کنندہ کو محفوظ کرتی ہے ، بال کٹوانے کے بعد متوقع XOR ، احساس شدہ تغیر (`xor_variance`) ، اور ملی سیکنڈ میں بلاک کا ٹائم اسٹیمپ۔
- بلاک پر عمل درآمد فی لین/ڈیٹا اسپیس کی رسیدیں شامل کرتا ہے اور `lane_settlement_commitments` کے ذریعے `/v1/sumeragi/status` میں شائع کرتا ہے۔ کل میں `total_local_amount` ، `total_xor_due` ، اور `total_xor_after_haircut` نے رات کے مفاہمت کی برآمدات کے لئے بلاک میں شامل کیا۔
- ایک نیا کاؤنٹر `total_xor_variance` ٹریک کرتا ہے کہ کتنا حفاظت کا مارجن کھایا گیا تھا (وجہ کے درمیان فرق
