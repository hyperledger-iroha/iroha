...
اس رن بک میں بتایا گیا ہے کہ SF-2C صلاحیت مارکیٹ پلیس تخروپن کٹ کو کیسے چلائیں اور اس کے نتیجے میں میٹرکس کا تصور کیا جائے۔ یہ کوٹہ مذاکرات ، فیل اوور مینجمنٹ اور `docs/examples/sorafs_capacity_simulation/` میں ڈٹرمینسٹک فکسچر کا استعمال کرتے ہوئے اختتام سے آخر تک ختم کرنے کی توثیق کرتا ہے۔ صلاحیت کے پے لوڈ ہمیشہ `sorafs_manifest_builder capacity` استعمال کرتے ہیں۔ ظاہر/کار پیکیجنگ اسٹریمز کے لئے `iroha app sorafs toolkit pack` استعمال کریں۔
...
`run_cli.sh` `sorafs_manifest_builder capacity` کو Norito پے لوڈز ، بیس 64 بلبس ، Torii درخواست باڈیوں ، اور JSON ڈائجسٹس کے لئے۔
