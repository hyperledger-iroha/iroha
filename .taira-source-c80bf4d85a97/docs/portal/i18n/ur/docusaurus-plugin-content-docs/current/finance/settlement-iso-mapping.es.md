...
ان ذمہ داریوں کو جو Norito ↔ ISO 20022 پل کو پیغامات جاری کرنے سے پہلے نافذ کرنا چاہئے۔
...
- ** حصے (`DlvrgSttlmPties` ، `RcvgSttlmPties`) **
...
| ** cusip ** | ریجیکس `^[A-Z0-9]{9}$` اور ماڈیول -10 2 وزن کے ساتھ (ہندسوں کے لئے حروف کا نقشہ) | صرف اس وقت جب آئین دستیاب نہیں ہے۔ انا/cusip کراس واک کے ذریعے نقشہ ایک بار کھایا جاتا ہے۔ [^cusip] |
...
- آڈٹ کے لئے برج میٹا ڈیٹا میں ڈیٹا پروویژن (ٹائم اسٹیمپ + ماخذ) ریکارڈ کریں۔ برقرار رکھیں
  جاری کردہ ہدایات کے ساتھ ساتھ اسنیپ شاٹ شناخت کنندہ۔
...
- سی ایل آئی بستی کے مددگار ایک ہی گارڈ ریلوں کا وارث ہیں: پاس
  ...
  `sese.023` XML اسنیپ شاٹ جاری کرنے سے پہلے پیش نظارہ آلہ کی شناخت کی توثیق کریں۔
...
| `payment_leg.quantity` | `CashLeg/Amt` | اعشاریہ تار ؛ عددی قیاس کے ذریعہ گول |
...
`sese.025` جاری کرنا جاری رکھے ہوئے ہے ، لیکن اس کی تصدیق سیکیورٹیز کے قانون کی عکاسی کرتی ہے
...
| ہم منصب شناخت کنندہ | `AddtlInf` json | موجودہ پل میٹا ڈیٹا میں مکمل اکاؤنٹڈ/بِک ٹوپلس کو سیریلائز کرتا ہے
...
| متبادل کولیٹرل کرنسی | `Substitution/SubstituteCcy` | کرنسی کوڈ |
...
* BICS 8 یا 11 بڑے حروف حروف حروف حروف (ISO9362) ہونا چاہئے۔ مسترد
  Norito میٹا ڈیٹا جو ادائیگی یا تصفیہ جاری کرنے سے پہلے اس چیک کو ناکام کرتا ہے
  تصدیق
...
* CLI آبادکاری کے مددگار (`iroha app settlement ... --atomicity ...`) اب جاری کریں
