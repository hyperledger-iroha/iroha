...
#### پیغام فیملی کے ذریعہ ترجیح کی نشاندہی کریں
...
- حصے ** بِک ** کو بطور ڈیفالٹ استعمال کرتے ہیں۔ ** لی ** قابل قبول ہے جہاں اسکیما اس کو بے نقاب کرتا ہے۔ [^سوئفٹ_بک]
...
- آئی ایس او کراس واک اسنیپ شاٹس کا استعمال کرتے ہوئے `iroha_core::iso_bridge::reference_data` کے ذریعہ کھایا گیا ہے
  ...
  `iso_reference_refresh_interval_secs` انتباہ کرنے کے لئے رن ٹائم صحت کو بے نقاب کرتا ہے۔ Torii
...
| `delivery_leg.asset_definition_id` (سیکیورٹی) | `SctiesLeg/FinInstrmId` | کیننیکل شناخت کنندہ (ISIN ، CUSIP ، وغیرہ) |
...
| ہدایات کی شناخت / لائف سائیکل لنکنگ | `CdtTrfTxInf/PmtId/InstrId` ، `CdtTrfTxInf/RmtInf` | Norito `settlement_id` سے میل کھاتا ہے تاکہ نقد ٹانگ سیکیورٹیز سائیڈ کے ساتھ صلح کرے۔ |
...
| کولیٹرل متبادل TX شناخت | `TxId` | متبادل کے ذریعہ تیار کردہ |
...
| Iroha سیاق و سباق | آئی ایس او 20022 پیغام | مقام نقشہ سازی |
| -------------------------------------- | ------------------------------------------------ |
...
- جاوا اسکرپٹ SDK اب `buildPacs008Message` / کو بے نقاب کرتا ہے
  `buildPacs009Message` (`javascript/iroha_js/src/isoBridge.js` دیکھیں) N/A کسٹمر
...
- مارکیٹ کیلنڈر/کٹ آفس کے ذریعہ ہولڈ/انوائنڈ ٹائمنگ کی تشکیل ؛ `sese.030`/`camt.056` کو منسوخ کرنے کی آخری تاریخ سے پہلے جاری کریں ، جب ضروری ہو تو واپسی پر فال بیک۔
