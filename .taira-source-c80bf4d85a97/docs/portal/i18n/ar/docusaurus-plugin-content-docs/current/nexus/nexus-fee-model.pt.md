...
- كل تحويل يتم تسجيله بالغاز هو `LaneSettlementReceipt`. كل ما يستلمه مخزن أو معرّف الأصل الناتج عن شارة الاتصال، أو القيمة الصغيرة المحلية، أو XOR يتم توزيعه على الفور، أو XOR منتظر بعد قصة الشعر، أو التباين المحقق (`xor_variance`)، أو الطابع الزمني للكتلة على ملايين المئات.
- تنفيذ الكتل التي يتم جمعها من خلال المسار/مساحة البيانات ونشرها عبر `lane_settlement_commitments` في `/v1/sumeragi/status`. كل ما في الأمر هو `total_local_amount`، `total_xor_due` و`total_xor_after_haircut`، لا يوجد كتلة للتصدير في أيام التوفيق.
- حساب جديد `total_xor_variance` يحدد مقدار هامش أمان المستهلك (الفرق بين قسم XOR وتوقعات ما بعد قص الشعر)، و`swap_metadata` يوثق معلمات المحادثة الحتمية (TWAP، epsilon، ملف تعريف السيولة e) volatility_class) حتى يتمكن المدققون من التحقق من إدخالات المنتج بشكل مستقل عن التكوين في وقت التشغيل.
