...
- كل معاملة يتم تسجيل الغاز فيها `LaneSettlementReceipt`. كل ما يتم استلامه هو المعرف الأصلي المقدم من المتصل، والميكرو مونتو المحلي، وXOR المطلوب على الفور، وXOR المتوقع بعد قص الشعر، والتباين الذي تم تحقيقه (`xor_variance`)، وعلامة وقت الكتلة بالملايين.
- تنفيذ الكتل المجمعة على المسار/مساحة البيانات والنشر عبر `lane_settlement_commitments` و`/v1/sumeragi/status`. توضح الإجماليات `total_local_amount`، و`total_xor_due`، و`total_xor_after_haircut`، إجمالي الكتلة لصادرات التوفيق الليلية.
- حاسب جديد `total_xor_variance` له هامش أمان للاستهلاك (الفرق بين XOR المستحق والتوقعات بعد قص الشعر)، و`swap_metadata` يوثق معلمات تحديد التحويل (TWAP، epsilon، ملف تعريف السيولة، وvolatility_class) لذلك يمكن للمراجعين التحقق من إدخالات التنظيف بشكل مستقل عن التكوين في وقت التشغيل.
