...
- `ABI V1` يثبت العقد على نسخة ABI 1 (النسخة المدعومة فقط وقت الكتابة).
- `--max-cycles 1000000` يطلب مجموعة غير محددة؛ ضع رقما موجبا لذلك حشوة المدربين لاجل اثباتات المعرفة الصفرية.
...
- إذا رفض `koto build` نسخة ABI، تحقق من ان المترجم والعقدة يستهدفان ABI v1 (شغّل `koto build --help` بدون مقترحات لتقديم الدعم).
