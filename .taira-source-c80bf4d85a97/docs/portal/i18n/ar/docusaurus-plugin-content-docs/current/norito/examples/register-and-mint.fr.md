...
- تأكد من وجود حساب الوجهة (على سبيل المثال `<i105-account-id>`)، بما يعكس مرحلة الإعداد في كل Quickstart SDK.
- قم باستدعاء نقطة الإدخال `register_and_mint` لإنشاء تعريف نشط ROSE وقم بإعداد 250 وحدة لـ Alice في معاملة واحدة.
- تحقق من المبيعات عبر `client.request(FindAccountAssets)` أو `iroha_cli ledger assets list --account <i105-account-id>` لتأكيد عملية الطحن.
