...
| `torii_sorafs_replication_backlog_total` | — | Հարմարավետ չափիչ հայելային `pending` պատվերներ: |
| `torii_sorafs_replication_sla_total` | `outcome` (`met` \| `missed` \| `pending`) | SLA հաշվառում. `met`-ը հաշվում է ավարտված պատվերները վերջնաժամկետում, `missed` ագրեգատները ուշ ավարտված են + ժամկետանց, `pending`-ը արտացոլում է չմարված պատվերները: |
...
3. **Պատվերների հերթ ըստ կարգավիճակի** – `torii_sorafs_registry_orders_total` (խմբավորված ըստ `status`-ի):
4. **Հետագում ընդդեմ ժամկետանց պատվերների** – համատեղում է `torii_sorafs_replication_backlog_total` և `torii_sorafs_registry_orders_total{status="expired"}` մակերեսի հագեցվածությունը:
...
6. **Հապաղում ընդդեմ վերջնաժամկետի թուլության** – ծածկույթ `torii_sorafs_replication_completion_latency_epochs{stat="p95"}` և `torii_sorafs_replication_deadline_slack_epochs{stat="avg"}`: Օգտագործեք Grafana փոխակերպումները՝ `min_over_time` դիտումներ ավելացնելու համար, երբ ձեզ անհրաժեշտ է բացարձակ անփույթ հատակ, օրինակ՝
...
- **SLA հաջողություն < 0,95 15 րոպեի համար**
  - Շեմը՝ `sum(torii_sorafs_replication_sla_total{outcome="met"}) / clamp_min(sum(torii_sorafs_replication_sla_total{outcome=~"met|missed"}), 1) < 0.95`
  - Գործողություն. Page SRE; սկսել replication backlog triage.
- ** 10-ից բարձր սպասվող կուտակումներ **
  - Շեմը. `torii_sorafs_replication_backlog_total > 10` պահպանվում է 10 րոպե
  - Գործողություն. Ստուգեք մատակարարի առկայությունը և Torii հզորության ժամանակացույցը:
- **Ժամկետանց պատվերներ > 0**
...
3. **Վերահանձնարարել կրկնօրինակում**
   - Նոր պատվերներ թողարկեք `sorafs_manifest_builder capacity replication-order`-ի միջոցով, երբ հետաձգված թուլությունը (`stat="avg"`) իջնում է 5 դարաշրջանից ցածր (մանիֆեստ/Ավտոմեքենայի փաթեթավորումն օգտագործում է `iroha app sorafs toolkit pack`):
...
Արտադրության մեջ կեղծանունների քեշի քաղաքականությունը միացնելիս կամ խստացնելիս հետևեք այս փուլային ընթացակարգին.

1. **Պատրաստել կոնֆիգուրացիան **
   - Թարմացրեք `torii.sorafs_alias_cache`-ը `iroha_config`-ում (օգտագործող → փաստացի) համաձայնեցված TTL-ներով և շնորհակալ պատուհաններով՝ `positive_ttl`, `refresh_window`, I18NI0000008NI2001, I18NI0000008NI2001 `revocation_ttl`, `rotation_max_age`, `successor_grace` և `governance_grace`: Կանխադրվածները համապատասխանում են `docs/source/sorafs_alias_policy.md` քաղաքականությանը:
...
2. **Չոր վազք բեմականացման մեջ**
   ...
   - Գործարկեք `cargo xtask sorafs-pin-fixtures`-ը՝ հաստատելու, որ կանոնական կեղծանունների սարքերը դեռ վերծանվում են և գնում են: Ցանկացած անհամապատասխանություն ենթադրում է վերին հոսքի բացահայտ շեղում, որը պետք է նախ լուծվի:
...
5. **Հետադարձ **
   - Եթե կեղծանունների թողարկումը հետ է մնում, և թարմացման պատուհանը հաճախակի է գործարկվում, ժամանակավորապես թուլացրեք քաղաքականությունը՝ ավելացնելով `refresh_window` և `positive_ttl` կազմաձևում, այնուհետև վերաբաշխեք: Պահպանեք `hard_expiry`-ը անձեռնմխելի, որպեսզի իսկապես հնացած ապացույցները դեռ մերժվեն:
   - Վերադարձեք նախկին կազմաձևին՝ վերականգնելով նախորդ `iroha_config` նկարը, եթե հեռաչափությունը շարունակում է ցույց տալ `error` բարձրացված թվերը, այնուհետև բացեք միջադեպ՝ կեղծանունների ստեղծման ուշացումները հետևելու համար:

## Հարակից նյութեր

- `docs/source/sorafs/pin_registry_plan.md` — իրականացման ճանապարհային քարտեզ և կառավարման համատեքստ:
- `docs/source/sorafs/runbooks/sorafs_node_ops.md` — պահեստավորման աշխատողի գործողություններ, լրացնում է այս ռեեստրի խաղային գիրքը: