...
| الفحص | التحقق | الدليل |
|-------|------------|--------------|
| يقبل registre إعلانات السعة القياسية | Vous pouvez utiliser l'API de l'application `/v1/sorafs/capacity/declare` pour utiliser les métadonnées et les métadonnées. registre العقدة. | `crates/iroha_torii/src/routing.rs:7654` |
