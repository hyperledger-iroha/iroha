...
Clés d'options :- `ABI V1` verrouille le contrat sur la version ABI 1 (la seule supportée au moment de la rédaction).
- `--max-cycles 1000000` demande une exécution sans limite ; fixez un nombre positif pour borner le padding de cycles pour les preuves de connaissance zéro.
...
- Si `koto build` refuse la version ABI, vérifiez que le compilateur et le nud ciblent bien ABI v1 (exécutez `koto build --help` sans arguments pour lister le support).
