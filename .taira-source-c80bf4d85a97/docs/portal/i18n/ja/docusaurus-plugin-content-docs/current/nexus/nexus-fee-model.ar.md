...
- `LaneSettlementReceipt` を参照してください。 كل ايصال يخزن معرف المصدر المذي يقدمه المستدعي والمقدار المجهري المحلي وXOR المستحق فورا وXORヘアカット والتباين المحقق (`xor_variance`) وطابع زمن الكتلة بالمللي ثانية。
- レーン/データスペース `lane_settlement_commitments` في `/v1/sumeragi/status`。 `total_local_amount` و`total_xor_due` و`total_xor_after_haircut` على مستوى الكتلة من اجل صادراتありがとうございます。
- عداد جديد `total_xor_variance` يتتبع مقدار هامش الامان المستهلك (الفرق بين XOR المستحق والتوقع بعدヘアカット) (ヘアカット) `swap_metadata` معلمات التحويل الحتمية (TWAP وepsilon وliquidity profile وvolatility_class) حتى يتمكن المدققون من التحقق من重要な情報を確認してください。
