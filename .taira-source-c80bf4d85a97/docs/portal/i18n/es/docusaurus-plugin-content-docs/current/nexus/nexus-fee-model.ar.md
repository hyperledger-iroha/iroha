...
- Haga clic en el botón `LaneSettlementReceipt`. كل ايصال يخزن معرف المصدر الذي يقدمه المستدعي والمقدار المجهري المحلي وXOR المستحق فورا وXOR المتوقع بعد corte de pelo والتباين المحقق (`xor_variance`) وطابع زمن الكتلة بالمللي ثانية.
- تنفيذ الكتلة يجمع الايصالات لكل lane/dataspace y عبر `lane_settlement_commitments` في `/v1/sumeragi/status`. Los dispositivos `total_local_amount`, `total_xor_due` y `total_xor_after_haircut` están conectados a una fuente de alimentación.
