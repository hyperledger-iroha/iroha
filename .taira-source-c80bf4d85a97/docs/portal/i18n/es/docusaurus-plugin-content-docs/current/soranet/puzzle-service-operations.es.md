...
descripción: Operación del demonio `soranet-puzzle-service` para tickets de admisión Argon2/ML-DSA.
---

:::nota Fuente canónica
Refleja `docs/source/soranet/puzzle_service_operations.md`. Mantengan ambas versiones sincronizadas hasta que los documentos heredados se retiren.
:::

# Guía de operaciones del servicio de rompecabezas

El demonio `soranet-puzzle-service` (`tools/soranet-puzzle-service/`) emite
...
los throttles del relé durante escenarios de DoS volumetrico.【tools/soranet-relay/tests/adaptive_and_puzzle.rs:337】

## Configurar la emisión de tokens

Configura los campos JSON del relé bajo `pow.token.*` (ver
`tools/soranet-relay/deploy/config/relay.entry.json` como ejemplo) para habilitar
los tokens ML-DSA. Como mínimo, proporciona la clave pública del emisor y una lista
opcional de revocación:
...
Norito JSON de revocaciones en tiempo de ejecución. Usa el CLI `soranet-admission-token`
(`cargo run -p soranet-relay --bin soranet_admission_token`) para acuñar e
tokens offline, anexar entradas `token_id_hex` al archivo de
revocaciones y auditar credenciales existentes antes de publicar actualizaciones a
producción.

Pasa la clave secreta del emisor al servicio de rompecabezas vía flags CLI:
...
los tickets PoW crudos de 74 bytes siguen siendo válidos cuando el verificador de
...
si el secreto del firmante no está configurado.

## Playbook de rotación de claves1. **Recoleccion del nuevo descriptor commit.** Gobernanza publica el relevo
   descriptor commit en el paquete del directorio. Copia la cadena hex en
   `handshake.descriptor_commit_hex` dentro del JSON de configuración del relé
   compartido con el servicio de rompecabezas.
2. **Revisión de límites de política del rompecabezas.** Confirma que los valores
   `pow.puzzle.{memory_kib,time_cost,lanes}` actualizados se alinean con el plan
   de liberación. Los operadores deben mantener la configuración determinista Argon2
...
## Monitoreo y alertas- **Latencia SLO:** Rastrea `soranet_handshake_latency_seconds` y mantén el P95
  por debajo de 300 ms. Los offsets del Soak Test proporcionan datos de calibración.
  para aceleradores de guardia.【docs/source/soranet/reports/pow_resilience.md:1】
- **Presión de cuota:** Usa `soranet_guard_capacity_report.py` con métricas de relé
...
es `docs/source/soranet/relay_audit_pipeline.md` ingeste estos registros para que los
Los cambios de política del rompecabezas siguen siendo auditables. Cuando la puerta de rompecabezas
este habilitado, archiva muestras de boletos emitidos y el snapshot de configuración
Norito con el ticket de rollout para futuros auditorios. Los tokens de admisión
