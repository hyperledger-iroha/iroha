...
2. معاون بائنریز کو بنائیں یا ڈاؤن لوڈ کریں:
   - `koto build` - Kotodama Código de bytes y código de bytes IVM/Norito
...
اہم فلیگز:- `ABI V1` کنٹریکٹ کو ABI ورژن 1 پر لاک کرتا ہے (تحریر کے وقت واحد سپورٹڈ ورژن).
- `--max-cycles 1000000` لامحدود اجرا کی درخواست کرتا ہے؛ pruebas de conocimiento cero کے لئے ciclo de relleno کو محدود کرنے کے لئے مثبت عدد دیں۔
