...
título: Registro de perfiles de fragmentador de SoraFS
sidebar_label: Registro de fragmentador
descripción: ID de perfil, parámetros y plan de negociación para el registro de fragmentador de SoraFS.
---
...
## Registro de perfiles de fragmentador de SoraFS (SF-2a)

El stack de SoraFS negocia el comportamiento de fragmentación mediante un registro pequeño con espacios de nombres.
...
los accesorios de puesta en escena y producción.

### Perfiles| Espacio de nombres | Nombre | SemVer | ID de perfil | Mín. (bytes) | Destino (bytes) | Máx. (bytes) | Máscara de corte | Multihash | Alias ​​| Notas |
|-----------|--------|--------|-------------|-------------|----------------|-------------|-----------------|-----------|-------|-------|
| `sorafs` | `sf1` | `1.0.0` | `1` | 65536 | 262144 | 524288 | `0x0000ffff` | `0x1f` (BLAKE3-256) | `["sorafs.sf1@1.0.0", "sorafs.sf1@1.0.0"]` | Perfil canónico usado en luminarias SF-1 |
...
* `multihash_code` – el multihash usado al producir digests de chunk (`0x1f`
  para el valor predeterminado de SoraFS).El manifiesto serializa perfiles mediante `ChunkingProfileV1`. La estructura registra
...
busque en el registro por `profile_id` y recurra a los parámetros inline cuando
aparecerán identificaciones desconocidas; la lista de alias garantiza que los clientes HTTP puedan
Seguir enviando manijas heredadas en `Accept-Chunker` sin adivinar. Las reglas de la
...
Para inspeccionar el registro desde herramientas, ejecuta el asistente CLI:
...
Todas las banderas del CLI que escriben JSON (`--json-out`, `--por-json-out`, `--por-proof-out`,
`--por-sample-out`) aceptan `-` como ruta, lo que transmite el payload a stdout en lugar de
crear un archivo. Esto facilita canalizar los datos a herramientas mientras se mantiene el
...
componentes principales. "Puente" se refiere al carril CARv1 + SHA-256
que requiere negociación limpia del cliente (`Accept-Chunker` + `Accept-Digest`).| Componente | Estado | Notas |
|-----------|--------|-------|
...
| `sorafs_manifest_builder` | ⚠️Retirado | Constructor de manifiesto fuera de soporte; usa `iroha app sorafs toolkit pack` para empaquetado CAR/manifest y mantén `--plan=-` para revalidación determinista. |
...
(documentada en la propuesta) antes de marcar `sorafs.sf1@1.0.0` como obsoleto en el registro y eliminar el
...
persiste la prueba a discoteca:
...
Puedes seleccionar un perfil por id numérico (`--profile-id=1`) o por identificador de registro
...
```El informe principal (y el archivo de prueba opcional) incluye el resumen de raíz, los bytes de hoja muestreados
(codificados en hexadecimal) y los digests hermanos de segmento/chunk para que los verificadores puedan repetir
las capas de 64 KiB/4 KiB contra el valor `por_root_hex`.

Para validar una prueba existente contra una carga útil, pasa la ruta vía
`--por-proof-verify` (el CLI añade `"por_proof_verified": true` cuando el testigo
coinciden con la raíz calculada):
...
Para muestreo en lote, usa `--por-sample=<count>` y opcionalmente proporciona una ruta de semilla/salida.
...
```
$ ejecución de carga -p sorafs_manifest --bin sorafs_manifest_builder --list-chunker-profiles
[
  {
    "id_perfil": 1,
...
y refleja la decisión vía el encabezado de respuesta `Content-Chunker`. Los manifiestos
embeben el perfil elegido para que los nodos downstream puedan validar el diseño de fragmentos
...
* **Ruta primaria** – CARv2, resumen de carga útil BLAKE3 (`0x1f` multihash),
  `MultihashIndexSorted`, perfil de trozo registrado como arriba.
...
* El perfil `sorafs.sf1@1.0.0` se asigna a los aparatos públicos en
  `fixtures/sorafs_chunker` y a los corpus registrados en
  `fuzz/sorafs_chunker`. La paridad de extremo a extremo se ejerce en Rust, Go y Node
...
* Los manifiestos producidos por `iroha app sorafs toolkit pack` y `sorafs_manifest_builder` incluyen los metadatos del registro.