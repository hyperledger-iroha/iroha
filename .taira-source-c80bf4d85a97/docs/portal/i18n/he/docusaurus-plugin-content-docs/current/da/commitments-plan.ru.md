...
проверках управления. Все payloadы Norito-кодированы; ללא retired codec ו-JSON אד-הוק.
...
- Экспортировать Torii запросы (`/v1/da/commitments/*`) והוכחות, позволяющие
...
|-------|--------|--------|-------|
| `/v1/da/commitments` | `POST` | `DaCommitmentQuery` (טווח-фильтр по מסלול/תקופה/רצף, עימוד) | Возвращает `DaCommitmentPage` с ספירה כוללת, התחייבויות и hash блока. |
| `/v1/da/commitments/prove` | `POST` | `DaCommitmentProofRequest` (נתיב + מניפסט hash или кортеж `(epoch, sequence)`). | Отвечает `DaCommitmentProof` (רשומה + נתיב מרקל + חשיש בלוקא). |
| `/v1/da/commitments/verify` | `POST` | `DaCommitmentProof` | עוזר חסר מדינה, הכללה пересчитывающий hash блока и проверяющий; זמין עבור SDKs ללא תשלום עבור `iroha_crypto`. |
