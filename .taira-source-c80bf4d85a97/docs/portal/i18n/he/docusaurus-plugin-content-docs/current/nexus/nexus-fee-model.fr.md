...
- עסקת צ'אקה qui paie du gasregistre un `LaneSettlementReceipt`. Chaque recu stocke l'identifiant source fourni par l'appelant, le micro-montant local, le XOR a regler immediatement, le XOR attendu apres le haircut, la variance realisee (`xor_variance`), et l'horodatage du bloc in millisecondes.
- L'execution de blocs agrege les recus par lane/dataspace et les publie via `lane_settlement_commitments` dans `/v1/sumeragi/status`. Les totalexposent `total_local_amount`, `total_xor_due`, et `total_xor_after_haircut` additionnes sur le bloc pour les exports nocturnes de reconciliation.
- Un nouveau compteur `total_xor_variance` suit la marge de securite consommee (הבדל בין XOR du et l'attendu לאחר תספורת), et `swap_metadata` מתעד את הפרמטרים של המרה קביעת המרה (TWAP, epsilon, פרופיל נזילות, מחלקה לביקורת) מנות ראשונות דה לה cotation independamment de la configuration d'execution.
