...
| M1 | الأسابيع 7–12 | يفرض CI fixtures حتمية؛ أدلة alias متاحة في staging؛ tooling يعرض flags توقع صريحة. | מסמכים, אחסון, ממשל | التأكد من بقاء fixtures موقعة، تسجيل aliases في سجل staging، تحديث قوائم الإصدار بإنفاذ `--car-digest/--root-cid`. | ⏳ معلّق |
| M2 | الأسابيع 13–20 | يصبح pinning المعتمد على registry هو المسار الأساسي؛ تتحول artefacts القديمة إلى قراءة فقط؛ تفضل البوابات أدلة registry. | אחסון, פעולות, ממשל | تمرير pinning عبر registry، تجميد hosts القديمة، نشر إشعارات ترحيل للمشغلين. | ⏳ معلّق |
| M3 | الأسبوع 21+ | فرض وصول قائم على alias فقط؛ تنبه المراقبة إلى تكافؤ registry؛ إزالة CDN القديم. | אופציות, רשתות, ערכות SDK | إزالة DNS القديم، تدوير عناوين URLs المخزنة مؤقتا، مراقبة لوحات التكافؤ، تحديث defaults الخاصة بالـ SDK. | ⏳ معلّق |
| R0–R3 | 2025-03-31 → 2025-07-01 | مراحل إنفاذ provider advert: R0 مراقبة، R1 تحذير، R2 فرض handles/capabilities القياسية، R3 تنقية payloads القديمة. | צפייה, אופציות, SDKs, DevRel | استيراد `grafana_sorafs_admission.json`، اتباع قائمة المشغل في `provider_advert_rollout.md`، جدولة تجديدات advert قبل بوابة R2 بـ 30+ יומיה. | ⏳ معلّق |
