...
- `POST /v1/puzzle/mint` - emite um ticket Argon2; um body JSON אופציונלי
...
- `GET /v1/token/config` - quando `pow.token.enabled = true`, חזור על מדיניות
...
`/v1/token/config` אטואליזדו; coordene עדכונים com o comando
...
פרסם ML-DSA-44 ארה"ב עבור אימות כרטיסים ל-PoW Assinados; `/v1/puzzle/config`
...
`POST /v1/puzzle/mint` aceita `"signed": true` (אופציונלי `"transcript_hash_hex"`) para
...
4. **Validar.** Emita um כרטיס דרך `POST /v1/puzzle/mint` e confirme que
   ...
   `/v1/token/config` para garantir que o מנפיק טביעת אצבע הודעה
...
- **יישור פאזל:** `soranet_handshake_pow_difficulty` deve corresponder a
  dificuldade retornada por `/v1/puzzle/config`. תצורת ממסר Divergencia indica
  מעופש או הפעל מחדש את Falho.
- **מוכנות אסימון:** Alerta se `/v1/token/config` cair para `enabled = false`
...
- **בריאות השירות:** גישוש `/healthz` ב-cadencia usual de liveness e alertar
  se `/v1/puzzle/mint` retornar HTTP 500 (אינדיקציה אי התאמה של parametros Argon2 ou
