...
Для этого необходимо создать реестр контактов SoraFS. (SLA) Установите `iroha_torii` и установите Prometheus, чтобы установить `torii_sorafs_*`. Введите Torii в реестр реестра на 30-е число дней после завершения регистрации. Он был создан в 2007 году в рамках проекта `/v1/sorafs/pin/*`. Установите флажок (`docs/source/grafana_sorafs_pin_registry.json`) для установки Grafana. Это было сделано для того, чтобы сделать это.
...
1. **Первый день**
   ...
   - اذا زاد التراكم مع اخفاقات مستقرة, افحص القبول (`/v1/sorafs/pin/*`) لتاكيد проявляется Да, это правда.
...
2. **Пробный прогон и постановка**
   ...
   - Конечные точки `/v1/sorafs/pin/{digest}` и `/v1/sorafs/aliases` могут быть заменены новыми, обновленными окнами, истекшими и жесткими истекшими сроками действия. Для HTTP-заголовков (`Sora-Proof-Status`, `Retry-After`, `Warning`) и JSON-файла.
