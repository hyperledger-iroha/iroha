...
В качестве примера можно привести SF-2c, который можно использовать в качестве оружия. چلائیں اور حاصل شدہ میٹرکس کیسے دیکھیں۔ `docs/examples/sorafs_capacity_simulation/` позволяет использовать детерминированные устройства, использовать квоту и аварийное переключение, а также сквозное исправление среза. ویلیڈیٹ کرتی ہے۔ Полезные нагрузки могут быть установлены `sorafs_manifest_builder capacity`. Манифест/CAR پیکجنگ کے لیے `iroha app sorafs toolkit pack` استعمال کریں۔
...
`run_cli.sh`, `sorafs_manifest_builder capacity`, а также полезные данные Norito, большие двоичные объекты base64, тела запросов Torii и сводки JSON. ہے برائے:
