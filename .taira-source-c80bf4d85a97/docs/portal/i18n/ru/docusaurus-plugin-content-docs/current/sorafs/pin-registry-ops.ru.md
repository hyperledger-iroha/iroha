...
3. **Перераспределить репликацию**
   - Выпустите новые заказы через `sorafs_manifest_builder capacity replication-order`, когда резерв резерва (`stat="avg"`) опустится ниже 5 этапов (упаковка манифеста/CAR использует `iroha app sorafs toolkit pack`).
