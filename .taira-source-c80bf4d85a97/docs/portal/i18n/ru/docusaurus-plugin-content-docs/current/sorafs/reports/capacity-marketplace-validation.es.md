...
|-------|------------|----------|
| Реестр принимает канонические объявления о возможностях | Тест интеграции с кодом `/v1/sorafs/capacity/declare` через API приложения, проверка управления фирмой, захват метаданных и передача данных в реестр узла. | `crates/iroha_torii/src/routing.rs:7654` |
...
Операторы должны регенерировать полезные данные запроса на подключение/спора с `sorafs_manifest_builder capacity {declaration,dispute}` и архивировать байты JSON/Norito, полученные вместе с билетом губернатора.
