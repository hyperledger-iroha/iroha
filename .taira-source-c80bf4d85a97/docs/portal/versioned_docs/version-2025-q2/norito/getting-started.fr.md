...
2. Construisez ou telechargez les binaires de support :
   - `koto build` - compilateur Kotodama qui emet du bytecode IVM/Norito
   - `ivm_run` et `ivm_tool` - utilitaires d'execution locale et d'inspection
   - `iroha` - utilise pour le deploiement de contrats via Torii
...
3. Assurez-vous qu'un nud Iroha est en cours d'execution lorsque vous atteignez l'etape de deploiement. Les exemples ci-dessous supposent que Torii est accessible a l'URL configuree dans votre profil `iroha` (`~/.config/iroha/cli.toml`).
...
- `ABI V1` verrouille le contrat sur la version ABI 1 (la seule supportee au moment de la redaction).
- `--max-cycles 1000000` demande une execution sans limite ; fixez un nombre positif pour borner le padding de cycles pour les preuves de connaissance zero.
...
Avec le bytecode enregistre, vous pouvez l'invoquer en soumettant une instruction qui reference le code stocke (p. ex., via `iroha contract call --contract-address <contract-address> --entrypoint main --wait` ou votre client applicatif). Assurez-vous que les permissions du compte autorisent les syscalls souhaites (`set_account_detail`, `transfer_asset`, etc.).
...
- Si `koto build` refuse la version ABI, verifiez que le compilateur et le nud ciblent bien ABI v1 (executez `koto build --help` sans arguments pour lister le support).
- Le CLI accepte des cles de signature en hex ou Base64. Pour les tests, vous pouvez utiliser les cles emises par `kagami keys --json`.
