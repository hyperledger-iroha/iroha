...
slug: /sorafs/pin-registry-ops-zh-hans
---

:::注意规范来源
镜子 `docs/source/sorafs/runbooks/pin_registry_ops.md`。保持两个版本在各个版本之间保持一致。
:::
...
| `torii_sorafs_replication_sla_total` | `outcome` (`met` \| `missed` \| `pending`) | SLA 核算：`met` 计算截止日期内已完成的订单，`missed` 汇总延迟完成+到期的订单，`pending` 镜像未完成的订单。 |
...
在生产中启用或收紧别名缓存策略时，请遵循此分阶段过程：1. **准备配置**
...
5. **后备**
   ...
   - 如果遥测继续显示 `error` 计数升高，请通过恢复先前的 `iroha_config` 快照来恢复到先前的配置，然后打开事件以跟踪别名生成延迟。
