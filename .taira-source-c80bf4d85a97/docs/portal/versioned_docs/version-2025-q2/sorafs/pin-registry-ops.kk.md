...
3. **Репликацияны қайта тағайындау**
   - Артта қалу (`stat="avg"`) 5 дәуірден төмен түскен кезде `sorafs_manifest_builder capacity replication-order` арқылы жаңа тапсырыстарды беріңіз (манифест/CAR қаптамасы `iroha app sorafs toolkit pack` пайдаланады).
