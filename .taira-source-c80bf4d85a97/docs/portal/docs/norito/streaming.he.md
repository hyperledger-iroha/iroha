...
- **נתיבי SoraNet.** `streaming.soranet.*` שולט בתעבורה אנונימית: `exit_multiaddr` (ברירת מחדל `/dns/torii/udp/9443/quic`), `padding_budget_ms` (ברירת מחדל 25 ms), `access_kind` (`authenticated` מול `read-only`), `channel_salt` אופציונלי, `provision_spool_dir` (ברירת מחדל `./storage/streaming/soranet_routes`), `provision_spool_max_bytes` (ברירת מחדל 0, ללא הגבלה), `provision_window_segments` (ברירת מחדל 4), ו-`provision_queue_capacity` (ברירת מחדל 256).
