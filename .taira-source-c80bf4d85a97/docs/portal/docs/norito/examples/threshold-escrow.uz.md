...
- `FindAssetById` / `iroha ledger asset list all --verbose` bilan balanslarni tekshiring va `GET /v1/contracts/state?paths=payer_account,recipient_account,escrow_account_id,escrow_asset_definition,target_amount_value,funded_amount_value,is_open,is_released,is_refunded&decode=json` bilan shartnoma holatini tekshiring.
