...
- **Feature bits.** capability negotiation `streaming.feature_bits` (ڈیفالٹ `0b11` = baseline feedback + privacy route provider) کا اعلان کرتی ہے تاکہ relays اور clients peers without matching capabilities کو deterministically reject کر سکیں۔
...
- **SoraNet routes.** `streaming.soranet.*` anonymous transport کو کنٹرول کرتا ہے: `exit_multiaddr` (ڈیفالٹ `/dns/torii/udp/9443/quic`)، `padding_budget_ms` (ڈیفالٹ 25 ms)، `access_kind` (`authenticated` بمقابلہ `read-only`)، optional `channel_salt`، `provision_spool_dir` (ڈیفالٹ `./storage/streaming/soranet_routes`)، `provision_spool_max_bytes` (ڈیفالٹ 0، unlimited)، `provision_window_segments` (ڈیفالٹ 4)، اور `provision_queue_capacity` (ڈیفالٹ 256).
