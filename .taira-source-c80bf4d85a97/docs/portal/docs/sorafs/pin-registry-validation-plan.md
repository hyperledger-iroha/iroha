...
- `/v1/sorafs/pin/register` requires the exact canonical Norito `ManifestV1`
  in `manifest_payload`, validates it under bounded decode limits, derives every
  digest/chunker/content/policy field from those bytes, and returns stable
  `sorafs_pin_*` application-validation labels. Retired parallel summaries are
  rejected.
