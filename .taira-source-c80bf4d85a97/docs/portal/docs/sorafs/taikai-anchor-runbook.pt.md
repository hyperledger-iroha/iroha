...
| `taikai_trm_alias_rotations_total{alias_namespace,alias_name}` | Incrementa sempre que `/v1/da/ingest` aceita um novo TRM para um alias; use `rate()` para validar a cadencia de rotacao. |
