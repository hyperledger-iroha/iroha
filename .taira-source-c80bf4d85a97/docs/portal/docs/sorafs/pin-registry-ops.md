...
2. **Dry-run in staging**
   ...
   - Exercise `/v1/sorafs/pin/{digest_hex}` with no cursor, a matching paired `expected_finalized_height`/`expected_finalized_block_hash_hex`, and a stale pair. Require exact `PinManifestFinalizedRecordV1` JSON, `no-store`, and HTTP 409 for the stale cursor.
   - Exercise `/v1/sorafs/aliases` separately with synthetic proofs covering fresh, refresh-window, expired, and hard-expired cases. Validate its HTTP status codes, `Sora-Proof-Status`, `Retry-After`, `Warning`, and JSON body fields against this runbook.
