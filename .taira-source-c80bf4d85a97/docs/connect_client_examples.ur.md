...
status: complete
translator: manual
source_hash: 0b1e5b574d966cbaeb344af83ef78b853e1cc57b8011ac3be69435ae1fff0d93
source_last_modified: "2025-11-02T04:40:28.812827+00:00"
translation_last_reviewed: 2025-11-14
---

<div dir="rtl">

<!-- docs/connect_client_examples.md (Iroha Connect Client Examples) کا اردو ترجمہ -->

## Iroha Connect کلائنٹ کے مثالیں (TypeScript اور Kotlin)

یہ دستاویز v0 رولز کو implement کرنے والے minimal کلائنٹ‑سائیڈ snippets
دکھاتی ہے:
- AEAD کا AAD outer header سے bind ہوتا ہے (version، `sid`, `dir`, `seq`,
  `kind=Ciphertext`)۔
- nonce کو `seq` سے derive کیا جاتا ہے (12‑بائٹ IETF nonce:
  `0x00000000 || seq_le`)۔
- Post‑Approve control frames (Close/Reject) خفیہ (encrypted) صورت میں
  بھیجے جاتے ہیں۔

یہ illustrative مثالیں ہیں؛ production‑grade hardening اور checks یہاں شامل
نہیں۔

### TypeScript (libsodium + WebCrypto)

Dependencies: `libsodium-wrappers` (X25519, BLAKE2b, ChaCha20‑Poly1305)،
...
(async () => { await sodium.ready;
  ...
  // سیشن بنائیں: POST /v1/connect/session، client‑calculated sid کے ساتھ
...
fun main() {
  ...
  val shared = KeyAgreement.getInstance("X25519").apply {
    init(appKp.private)
    doPhase(appKp.public, true) // یہاں wallet کی public key فراہم کریں
  }.generateSecret()
