...
- הגדרת `connect.enabled=false` משביתה את כל מסלולי Connect (`/v1/connect/*`).
- השארת הערך `true` (ברירת המחדל) מפעילה את נקודות הקצה של WS ואת `/v1/connect/status`.
...
- כאשר Connect מושבת בזמן הריצה (`connect.enabled=false`), מסלולי ה-WS והסטטוס אינם נרשמים; בקשות ל-`/v1/connect/ws` ו-`/v1/connect/status` יחזירו 404.
...
- השרת דורש מזהה `sid` מסופק על ידי הלקוח עבור `/v1/connect/session` (base64url או הקס, 32 בתים). יצירת `sid` חלופי כבר אינה מתבצעת.
