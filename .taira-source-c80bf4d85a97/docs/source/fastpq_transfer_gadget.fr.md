...
| Kotodama | Réduit l'assistant `transfer_batch((from,to,asset,amount), …)` en `transfer_v1_batch_begin`, les appels séquentiels `transfer_asset` et `transfer_v1_batch_end`. Chaque argument de tuple doit suivre la forme `(AccountId, AccountId, AssetDefinitionId, quantity)` ; les transferts uniques conservent le constructeur existant. |
