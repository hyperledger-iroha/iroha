...
- Бекіту фреймдеріне таңдалған тіркелгі/контроллер, рұқсат жаңғырығы, дәлелдеу жинағы (ZK дәлелі немесе аттестация) және кез келген саясат қосқыштары (мысалы, `deferred_queue_enabled`) кіреді.
...
- Қолданба `deferred_queue_enabled=false` ауыстырғанда, SDK дереу ағып кетеді және
