...
- Codegen (`kotodama::compiler`) emite um subconjunto de operações IVM e usa `SCALL` para operações de ativos:
  - `MintAsset` → definir x10=conta, x11=ativo, x12=&QuantityValueV1; `SCALL SYSCALL_MINT_ASSET`.
  - `BurnAsset`/`TransferAsset` semelhante (quantidade passada como ponteiro QuantityValueV1).
...
- SYSCALL_MINT_ASSET(conta: ptr AccountId, ativo: ptr AssetDefinitionId, valor: ptr QuantityValueV1) → ISI Mint
- SYSCALL_BURN_ASSET(conta: ptr AccountId, ativo: ptr AssetDefinitionId, quantidade: ptr QuantityValueV1) → ISI Burn
- SYSCALL_TRANSFER_ASSET(de: ptr AccountId, para: ptr AccountId, ativo: ptr AssetDefinitionId, valor: ptr QuantityValueV1) → Transferência ISI
