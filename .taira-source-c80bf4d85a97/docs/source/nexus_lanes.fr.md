...
- La télémétrie d'éviction DA ajoute `storage_da_cache_total{component=...,result=hit|miss}` et `storage_da_churn_bytes_total{component=...,direction=evicted|rehydrated}` pour suivre l'activité du cache et les octets déplacés pour `kura` et `wsv_cold`.
