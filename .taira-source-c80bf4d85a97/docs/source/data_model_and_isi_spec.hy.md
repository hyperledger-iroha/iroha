...
- Ակտիվ (Թվային) անանուխ/այրում. կարգավորում է մնացորդները և սահմանման `total_quantity`:
  - Նախապայմաններ. `Quantity` արժեքը պետք է բավարարի `AssetDefinition.spec()`; անանուխը թույլատրված է `mintable`-ով:
...
---## Գործնական օրինակներ
- հատում և փոխանցում.
  - `Mint::asset_quantity(10, asset_id)` → ավելացնում է 10, եթե դա թույլատրվում է սպեցիֆիկացիաներով/հատուկ հնարավորություններով; իրադարձություններ՝ `AssetEvent::Added`:
  - `Transfer::asset_quantity(asset_id, 5, to_account)` → շարժվում է 5; միջոցառումներ հեռացման/ավելացման համար:
...
- Լուծել alias-ը կանոնական Base58 id:
  - `POST /v1/assets/aliases/resolve` JSON `{ "alias": "pkr#bankb.paynet" }`-ով
