...
| **Giriş nəzarəti və autentifikasiyası** | SDK Torii TLS siyasətlərini və `/v1/pipeline` imzalanmış sorğuları qəbul edir; operator iş axınları arayışı eskalasiya üçün Dəstək Playbook §4–5 və imzalanmış Norito artefaktları vasitəsilə qapını ləğv edin. | `docs/source/android_support_playbook.md`; `docs/source/sdk/android/telemetry_redaction.md` (iş axını ləğv edin). | ✅ Həyata keçirilən |
