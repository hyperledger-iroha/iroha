...
- `AccountId` — канонические адреса создаются через `AccountAddress` как I105, а Torii нормализует входные данные через `AccountAddress::parse_encoded`. Строгий анализ времени выполнения принимает только канонический I105. Псевдонимы учетных записей в сети используют `name@domain.dataspace` или `name@dataspace` и преобразуются в канонические значения `AccountId`; они не принимаются строгими парсерами `AccountId`. Аккаунт: `{ id, metadata }`. Код: `crates/iroha_data_model/src/account.rs`.- Политика допуска учетных записей — домены контролируют неявное создание учетных записей, сохраняя Norito-JSON `AccountAdmissionPolicy` под ключом метаданных `iroha:account_admission_policy`. Если ключ отсутствует, пользовательский параметр уровня цепочки `iroha:default_account_admission_policy` предоставляет значение по умолчанию; если он также отсутствует, жесткое значение по умолчанию — `ImplicitReceive` (первая версия). Теги политики `mode` (`ExplicitOnly` или `ImplicitReceive`), а также дополнительные ограничения на каждую транзакцию (по умолчанию `16`) и ограничения на создание каждого блока, дополнительный `implicit_creation_fee` (сжигание или слив учетной записи), `min_initial_amounts` для каждого определения актива и необязательный `default_role_on_create` (предоставляется после `AccountCreated`, отклоняется с `DefaultRoleError`, если отсутствует). Genesis не может принять участие; отключенные/недействительные политики отклоняют инструкции в виде квитанций для неизвестных учетных записей с `InstructionExecutionError::AccountAdmission`. Неявные учетные записи отмечают метаданные `iroha:created_via="implicit"` до `AccountCreated`; роли по умолчанию выдают последующий `AccountRoleGranted`, а базовые правила владельца-исполнителя позволяют новой учетной записи расходовать свои собственные активы/NFT без дополнительных ролей. Код: `crates/iroha_data_model/src/account/admission.rs`, `crates/iroha_core/src/smartcontracts/isi/account_admission.rs`.- `AssetDefinitionId` — канонический адрес Base58 без префикса поверх канонических байтов определения актива. Это идентификатор общедоступного актива. Определение: `{ id, name, description?, alias?, spec: NumericSpec, mintable: Mintable, logo, metadata, owned_by, total_quantity }`. Литералы `alias` должны быть `<name>#<domain>.<dataspace>` или `<name>#<dataspace>`, где `<name>` соответствует имени определения актива, и они разрешаются только в канонический идентификатор актива Base58. Код: `crates/iroha_data_model/src/asset/definition.rs`.
  — Метаданные аренды псевдонима сохраняются отдельно от сохраненной строки определения актива. Core/Torii материализует `alias` из записи привязки при чтении определений.
  - Ответы определения актива Torii предоставляют `alias_binding { alias, status, lease_expiry_ms, grace_until_ms, bound_at_ms }`, где `status` — это `permanent`, `leased_active`, `leased_grace` или `expired_pending_cleanup`.
  - Селекторы псевдонимов учитывают время создания последнего зафиксированного блока. После `grace_until_ms` селекторы псевдонимов перестают разрешаться, даже если фоновая очистка еще не удалила устаревшую привязку; Чтение прямого определения может по-прежнему сообщать об устаревшей привязке как `expired_pending_cleanup`.
- `AssetId`: общедоступный идентификатор актива в канонической простой форме Base58. Псевдонимы активов, такие как `name#dataspace` или `name#domain.dataspace`, преобразуются в `AssetId`. При необходимости внутренние активы реестра могут дополнительно отображать разделенные поля `asset + account + optional dataspace`, но эта составная форма не является общедоступной `AssetId`.
- `NftId` — `nft$domain`. НФТ: `{ id, content: Metadata, owned_by }`. Код: `crates/iroha_data_model/src/nft.rs`.- `RoleId` — `name`. Роль: `{ id, permissions: BTreeSet<Permission> }` со строителем `NewRole { inner: Role, grant_to }`. Код: `crates/iroha_data_model/src/role.rs`.
...
- Регистрация AssetDefinition: строится из конструктора; устанавливает `owned_by = authority`.
  - Предпосылки: отсутствие определения; домен существует; `name` обязателен, должен быть непустым после обрезки и не должен содержать `#`/`@`.
...
- Ошибки: `FindError::Trigger`, если не зарегистрирован; `InvariantViolation`, если вызывается неавторизатором. Код: `core/.../isi/triggers/mod.rs` (и тесты в `core/.../smartcontracts/isi/mod.rs`).
...
- Триггерный ключ метаданных `__enabled` (bool) для выполнения вентилей; отсутствуют значения по умолчанию для включенных, а отключенные триггеры пропускаются по путям данных/времени/по вызову.
...
- Преобразовать псевдоним в канонический идентификатор Base58:
  - `POST /v1/assets/aliases/resolve` с JSON `{ "alias": "pkr#bankb.paynet" }`
...
- В средствах выбора общедоступных активов используются канонические идентификаторы определения активов Base58 плюс поля разделенного владения (`account`, необязательно `scope`). Литералы `AssetId` с необработанным кодированием остаются внутренними вспомогательными средствами и не являются частью поверхности селектора Torii/CLI.
- Фильтры и сортировки списков/запросов определения активов дополнительно принимают `alias_binding.status`, `alias_binding.lease_expiry_ms`, `alias_binding.grace_until_ms` и `alias_binding.bound_at_ms`.
...
Если вы хотите, чтобы эта спецификация была расширена до визуализированной таблицы API/поведения или привязана к каждому конкретному событию/ошибке, скажите слово, и я расширим ее.