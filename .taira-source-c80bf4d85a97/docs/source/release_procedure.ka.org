...
- [ ] Run =scripts/select_release_profile.py= for each target network (attach
  =artifacts/network_profiles.json= to the checklist) so Sora Nexus pulls the
  Iroha 3 bundle while other networks default to Iroha 2.
- [ ] Archive =artifacts/release_manifest.json= generated by
  =scripts/generate_release_manifest.py= as the authoritative hash list for
  tarballs/AppImages prior to signing or uploading artifacts.
