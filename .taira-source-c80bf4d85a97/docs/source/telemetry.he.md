...
- `/v1/sumeragi/telemetry` (JSON): טלמטריית קונסנזוס מצטברת. תצפיות על collectors נמצאות ב־`availability.collectors`, סיכומי chunks חסרים ב־`rbc_backlog`, ותורי pre-session וגבולותיהם ב־`rbc_pending`. הממשק אינו חושף מצב RBC לכל session, דגימות chunks, probes של delivery או תוכנית collectors דטרמיניסטית.
