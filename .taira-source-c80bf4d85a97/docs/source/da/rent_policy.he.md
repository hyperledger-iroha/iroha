...
|-------|----------------|--------|
| `base_rate_per_gib_month` | XOR מחויב לכל GiB לחודש שמירה. | `"0.25"` XOR |
...
| `egress_credit_per_gib` | אשראי ששולם כאשר ספק מגיש 1GiB של נתוני DA. | `"0.0015"` XOR |
...
השדה האחרון `egress_credit_per_gib` מאפשר לוחות מחוונים ותשלום
...
צרף את ה-Norito/JSON המקודד `DaRentPolicyV1` להצעות ממשל או להשכרה
