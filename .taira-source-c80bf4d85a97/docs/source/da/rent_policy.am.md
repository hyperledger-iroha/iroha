...
| መስክ | መግለጫ | ነባሪ |
|-------|-------------|--------|
| `base_rate_per_gib_month` | XOR በየወሩ በማቆየት በጂቢ ይከፍላል። | `"0.25"` XOR |
...
| `egress_credit_per_gib` | ክሬዲት የሚከፈለው አቅራቢው 1ጂቢ የDA ውሂብ ሲያገለግል ነው። | `"0.0015"` XOR |
...
የመጨረሻው `egress_credit_per_gib` መስክ ዳሽቦርድ እና ክፍያ ይፈቅዳል
...
የNorito/JSON ኮድ `DaRentPolicyV1` ከአስተዳደር ሀሳቦች ወይም ኪራይ ጋር ያያይዙ
