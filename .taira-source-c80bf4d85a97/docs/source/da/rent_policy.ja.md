...
|フィールド |説明 |デフォルト |
|----------|---------------|----------|
| `base_rate_per_gib_month` | XOR は、保持期間ごとに GiB ごとに課金されます。 | `"0.25"` XOR |
...
| `egress_credit_per_gib` |プロバイダーが 1 GiB の DA データを提供するときに支払われるクレジット。 | `"0.0015"` XOR |
...
`iroha app da rent-ledger --quote <path> --payer-account <id> --treasury-account <id> --protocol-reserve-account <id> --provider-account <id> --pdp-bonus-account <id> --potr-bonus-account <id> --asset-definition 61CtjvNd9T3THAR65GsMVHr82Bjc` を実行します
...
最後の `egress_credit_per_gib` フィールドにより、ダッシュボードと支払いが可能になります
スケジューラは、下り料金の払い戻しを、
...
Norito/JSON エンコードされた `DaRentPolicyV1` をガバナンス提案に添付するか、レンタルします
