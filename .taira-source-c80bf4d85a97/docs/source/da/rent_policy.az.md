...
`/v1/da/ingest`-ə təqdim olunur, üstəgəl PDP/PoTR icrasını mükafatlandıran bonuslar və
...
| Sahə | Təsvir | Defolt |
|-------|-------------|---------|
| `base_rate_per_gib_month` | XOR saxlama ayı üçün GiB başına tutulur. | `"0.25"` XOR |
...
| `egress_credit_per_gib` | Provayder 1GiB DA datasına xidmət etdikdə ödənilən kredit. | `"0.0015"` XOR |
...
Son `egress_credit_per_gib` sahəsi tablosuna və ödəməyə imkan verir
...
idarəetmə təkliflərinə və ya icarəyə Norito/JSON kodlu `DaRentPolicyV1` əlavə edin
...
1. **İngest:** `/v1/da/ingest` aktiv `DaRentPolicyV1` yükləyir, kotirovkalar icarəyə verilir
...
- `torii_da_rent_gib_months_total` — GiB-aylar `/v1/da/ingest` tərəfindən sitat gətirilib.
...
- ✅ `/v1/da/ingest` qəbzləri indi `rent_quote`-i yerləşdirir və CLI/SDK səthləri sitat gətirilənləri göstərir
