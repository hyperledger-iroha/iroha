...
ڈیزائن کے جائزے میں مدد کے لیے یہ تصریح `iroha_data_model` اور `iroha_core` پر موجودہ نفاذ سے ریورس انجنیئر ہے۔ بیک ٹِکس میں راستے مستند کوڈ کی طرف اشارہ کرتے ہیں۔
...
- `AccountId` — کینونیکل پتے `AccountAddress` کے ذریعے I105 کے طور پر تیار کیے جاتے ہیں اور Torii `AccountAddress::parse_encoded` کے ذریعے ان پٹس کو معمول بناتا ہے۔ سخت رن ٹائم تجزیہ صرف کیننیکل I105 کو قبول کرتا ہے۔ آن چین اکاؤنٹ عرف `name@domain.dataspace` یا `name@dataspace` استعمال کرتے ہیں اور کیننیکل `AccountId` اقدار کو حل کرتے ہیں۔ وہ سخت `AccountId` تجزیہ کاروں کے ذریعہ قبول نہیں کیے جاتے ہیں۔ اکاؤنٹ: `{ id, metadata }`۔ کوڈ: `crates/iroha_data_model/src/account.rs`۔- اکاؤنٹ میں داخلے کی پالیسی — ڈومینز Norito-JSON `AccountAdmissionPolicy` کو میٹا ڈیٹا کلید `iroha:account_admission_policy` کے تحت اسٹور کرکے مضمر اکاؤنٹ کی تخلیق کو کنٹرول کرتے ہیں۔ جب کلید غائب ہو تو، چین کی سطح کا کسٹم پیرامیٹر `iroha:default_account_admission_policy` ڈیفالٹ فراہم کرتا ہے۔ جب وہ بھی غائب ہو تو، ہارڈ ڈیفالٹ `ImplicitReceive` (پہلی ریلیز) ہے۔ پالیسی ٹیگز `mode` (`ExplicitOnly` یا `ImplicitReceive`) کے علاوہ اختیاری فی ٹرانزیکشن (پہلے سے طے شدہ `16`) اور فی بلاک تخلیق کیپس، ایک اختیاری SoraFS اکاؤنٹ، یا `min_initial_amounts` فی اثاثہ کی تعریف، اور ایک اختیاری `default_role_on_create` (`AccountCreated` کے بعد دیا جاتا ہے، اگر غائب ہو تو `DefaultRoleError` کے ساتھ مسترد کرتا ہے)۔ پیدائش آپٹ ان نہیں کر سکتی۔ غیر فعال/غلط پالیسیاں `InstructionExecutionError::AccountAdmission` والے نامعلوم اکاؤنٹس کے لیے رسید طرز کی ہدایات کو مسترد کرتی ہیں۔ `AccountCreated` سے پہلے مضمر اکاؤنٹس سٹیمپ میٹا ڈیٹا `iroha:created_via="implicit"`؛ پہلے سے طے شدہ رولز فالو اپ `AccountRoleGranted` کا اخراج کرتے ہیں، اور ایگزیکیوٹر کے مالک کے بنیادی اصول نئے اکاؤنٹ کو اضافی کرداروں کے بغیر اپنے اثاثے/NFTs خرچ کرنے دیتے ہیں۔ کوڈ: `crates/iroha_data_model/src/account/admission.rs`, `crates/iroha_core/src/smartcontracts/isi/account_admission.rs`۔- `AssetDefinitionId` — کینونیکل اثاثہ کی تعریف بائٹس پر کینونیکل غیر سابقہ ​​بیس 58 ایڈریس۔ یہ عوامی اثاثہ ID ہے۔ تعریف: `{ id, name, description?, alias?, spec: NumericSpec, mintable: Mintable, logo, metadata, owned_by, total_quantity }`۔ `alias` لٹریلز `<name>#<domain>.<dataspace>` یا `<name>#<dataspace>` ہونے چاہئیں، جس میں `<name>` اثاثہ کی تعریف کے نام کے برابر ہے، اور وہ صرف کینونیکل Base58 اثاثہ ID کو حل کرتے ہیں۔ کوڈ: `crates/iroha_data_model/src/asset/definition.rs`۔
  - عرفی لیز میٹا ڈیٹا ذخیرہ شدہ اثاثہ کی تعریف کی قطار سے الگ سے برقرار ہے۔ جب تعریفیں پڑھی جاتی ہیں تو Core/Torii بائنڈنگ ریکارڈ سے `alias` کو عملی شکل دیتا ہے۔
  - Torii اثاثہ کی تعریف کے جوابات `alias_binding { alias, status, lease_expiry_ms, grace_until_ms, bound_at_ms }` کو بے نقاب کرتے ہیں، جہاں `status` ہے `permanent`، `leased_active`، `leased_grace`، یا I00073X.
  - عرفی سلیکٹرز بلاک بنانے کے تازہ ترین وقت کے خلاف حل کرتے ہیں۔ `grace_until_ms` کے بعد، عرفی سلیکٹرز حل کرنا بند کر دیتے ہیں یہاں تک کہ اگر بیک گراؤنڈ سویپ نے ابھی تک باسی بائنڈنگ کو نہیں ہٹایا ہے۔ ڈائریکٹ ڈیفینیشن ریڈز اب بھی باسی بائنڈنگ کو `expired_pending_cleanup` کے بطور رپورٹ کر سکتا ہے۔
...
- `NftId` — `nft$domain`۔ NFT: `{ id, content: Metadata, owned_by }`۔ کوڈ: `crates/iroha_data_model/src/nft.rs`۔- `RoleId` — `name`۔ کردار: بلڈر `NewRole { inner: Role, grant_to }` کے ساتھ `{ id, permissions: BTreeSet<Permission> }`۔ کوڈ: `crates/iroha_data_model/src/role.rs`۔
...
- **Cryptographic Primitives** (خصوصیت `sm`):
  - SM2 کے لیے `Sm2PublicKey` / `Sm2Signature` کیننیکل SEC1 پوائنٹ + فکسڈ چوڑائی `r∥s` انکوڈنگ کا عکس۔ کنسٹرکٹرز منحنی رکنیت اور امتیازی ID کے الفاظ (`DEFAULT_DISTID`) کو نافذ کرتے ہیں، جب کہ توثیق خراب شکل والے یا ہائی رینج اسکیلرز کو مسترد کرتی ہے۔ کوڈ: `crates/iroha_crypto/src/sm.rs` اور `crates/iroha_data_model/src/crypto/mod.rs`۔
...
- ڈیٹا اسپیس سے حاصل کردہ ریلیشن اسٹورز (`space_directory_manifests`، `uaid_dataspaces`، `axt_policies`، `axt_replay_ledger`، لین-ریلے ایمرجنسی اوور رائیڈ رجسٹری) اور ڈیٹا اسپیس ٹارگٹ اکاؤنٹ کی اجازت (001001/001001 میں اکاؤنٹ اسٹورز) کو `State::set_nexus(...)` پر کاٹ دیا جاتا ہے جب ڈیٹا اسپیس فعال `dataspace_catalog` سے غائب ہوجاتا ہے، رن ٹائم کیٹلاگ اپ ڈیٹس کے بعد باسی ڈیٹا اسپیس حوالہ جات کو روکتا ہے۔ لین کے دائرہ کار والے DA/relay caches (`lane_relays`, `da_commitments`, `da_confidential_compute`, `da_pin_intents`) کو بھی اس وقت کاٹ دیا جاتا ہے جب لین ریٹائر ہو جاتی ہے یا کسی مختلف ڈیٹا اسپیس کو دوبارہ تفویض کی جاتی ہے لہذا ڈیٹا اسپیس میں لین-اسپیس کو مائل نہیں کیا جا سکتا۔ اسپیس ڈائرکٹری ISIs (`PublishSpaceDirectoryManifest`, `RevokeSpaceDirectoryManifest`, `ExpireSpaceDirectoryManifest`) بھی فعال کیٹلاگ کے خلاف `dataspace` کی توثیق کرتی ہے اور `InvalidParameter` کے ساتھ نامعلوم IDs کو مسترد کرتی ہے۔
...
- بنیادی خصوصیت: `Instruction: Send + Sync + 'static` کے ساتھ `dyn_encode()`، `as_any()`، مستحکم `id()` (ٹھوس قسم کے نام سے پہلے سے طے شدہ)۔
- `InstructionBox`: `Box<dyn Instruction>` ریپر۔ Clone/Eq/Ord `(type_id, encoded_bytes)` پر کام کرتا ہے لہذا مساوات قدر کے لحاظ سے ہے۔
- Norito serde `InstructionBox` کے لیے `(String wire_id, Vec<u8> payload)` کے طور پر سیریلائز کرتا ہے (اگر وائر ID نہ ہو تو `type_name` پر واپس آتا ہے)۔ ڈی سیریلائزیشن کنسٹرکٹرز کے لیے عالمی `InstructionRegistry` میپنگ شناخت کاروں کا استعمال کرتی ہے۔ ڈیفالٹ رجسٹری میں تمام بلٹ ان ISI شامل ہے۔ کوڈ: `crates/iroha_data_model/src/isi/{mod.rs,registry.rs}`۔

---

## ISI: اقسام، سیمنٹکس، غلطیاں
عمل درآمد کو `Execute for <Instruction>` کے ذریعے `iroha_core::smartcontracts::isi` میں لاگو کیا گیا ہے۔ ذیل میں عوامی اثرات، پیشگی شرائط، خارج ہونے والے واقعات، اور غلطیوں کی فہرست دی گئی ہے۔
...
- اثاثہ (عددی) ٹکسال/برن: بیلنس اور تعریف کے `total_quantity` کو ایڈجسٹ کرتا ہے۔
  ...
  - ریاست میں تبدیلیاں: ٹکسال پر غائب ہونے پر اثاثہ بناتا ہے۔ اگر بیلنس جلنے پر صفر ہو جاتا ہے تو اثاثہ کے اندراج کو ہٹا دیتا ہے۔
  ...
  - ایونٹس: `TriggerEvent::Extended`/`TriggerEvent::Shortened`۔
...
اقسام: `Grant<O, D>` اور `Revoke<O, D>`، `Permission`/`Role` کے لیے/سے `Account`، اور `Account` تک، اور `AccountId` کے لیے باکسڈ انومس کے ساتھ `Role`۔- اکاؤنٹ کو اجازت دیں: `Permission` شامل کرتا ہے جب تک کہ پہلے سے موروثی نہ ہو۔ واقعات: `AccountEvent::PermissionAdded`۔ خرابیاں: `Repetition(Grant, Permission)` اگر ڈپلیکیٹ ہو۔ کوڈ: `core/.../isi/account.rs`۔
...
- اکاؤنٹ سے رول منسوخ کریں: اگر موجود ہو تو میپنگ کو ہٹا دیتا ہے۔ واقعات: `AccountEvent::RoleRevoked`۔ خرابیاں: `FindError::Role` اگر غیر حاضر ہوں۔ کوڈ: `core/.../isi/account.rs`۔
...
- کردار سے اجازت منسوخ کریں: اس اجازت کے بغیر کردار کو دوبارہ بناتا ہے۔ واقعات: `RoleEvent::PermissionRemoved`۔ خرابیاں: `FindError::Permission` اگر غیر حاضر ہوں۔ کوڈ: `core/.../isi/world.rs`۔### محرکات: عمل کریں۔
قسم: `ExecuteTrigger { trigger: TriggerId, args: Json }`۔
- برتاؤ: ٹرگر سب سسٹم کے لیے `ExecuteTriggerEvent { trigger_id, authority, args }` کی قطار لگاتا ہے۔ دستی عمل درآمد کی اجازت صرف بذریعہ کال ٹرگرز (`ExecuteTrigger` فلٹر) کے لیے ہے۔ فلٹر کا مماثل ہونا چاہیے اور کال کرنے والا ٹرگر ایکشن اتھارٹی ہونا چاہیے یا اس اتھارٹی کے لیے `CanExecuteTrigger` ہولڈ ہونا چاہیے۔ جب صارف کی طرف سے فراہم کردہ ایگزیکیوٹر فعال ہوتا ہے، تو رن ٹائم ایگزیکیوٹر کے ذریعے ٹرگر ایگزیکیوشن کی توثیق کی جاتی ہے اور ٹرانزیکشن کے ایگزیکیوٹر فیول بجٹ (بیس `executor.fuel` کے علاوہ اختیاری میٹا ڈیٹا `additional_fuel`) استعمال کرتا ہے۔
...
### ایرر ماڈل
عام لفافہ: `InstructionExecutionError` تشخیص کی غلطیوں، استفسار میں ناکامی، تبادلوں، ہستی نہیں ملی، تکرار، مائنٹ ایبلٹی، ریاضی، غلط پیرامیٹر، اور غیر متغیر خلاف ورزی کے لیے متغیرات کے ساتھ۔ گنتی اور مددگار `crates/iroha_data_model/src/isi/mod.rs` میں `pub mod error` کے تحت ہیں۔
...
- `TransactionBuilder`/`SignedTransaction`: میٹا ڈیٹا، `chain_id`، `authority`، `authority`، اختیاری `creation_time_ms`، اختیاری I109، اور 0018 کے ساتھ ایک قابل عمل تعمیر، نشانیاں اور پیکجز `nonce`۔ کوڈ: `crates/iroha_data_model/src/transaction/`۔
...
- رن ٹائم ایگزیکیوٹر کی توثیق کا بجٹ (صارف کی طرف سے فراہم کردہ ایگزیکیوٹر): بنیادی `executor.fuel` پیرامیٹرز کے علاوہ اختیاری ٹرانزیکشن میٹا ڈیٹا `additional_fuel` (`u64`)، تمام ہدایات/ٹرگر کی توثیق کے اندر اشتراک کردہ۔
...
- عددی اثاثوں کو منٹ/ٹرانسفر/برن پر اپنے `NumericSpec` کو پورا کرنا ضروری ہے۔ spec کی مماثلت `TypeError::AssetNumericSpec` حاصل کرتی ہے۔
...
- ٹرگر لائف سائیکل:
  ...
  - ٹرگرز کو میٹا ڈیٹا کلید `__enabled` سے `false` پر سیٹ کر کے غیر فعال کیا جا سکتا ہے (فعال میں ڈیفالٹس غائب ہیں)؛ `SetKeyValue::trigger` یا IVM `set_trigger_enabled` syscall کے ذریعے ٹوگل کریں۔
  ...
  - اگر عمل درآمد کے وقت ایک ٹرگر کا IVM بائیک کوڈ غائب ہے، تو ٹرگر کو ہٹا دیا جاتا ہے اور ایگزیکیوشن کو ناکامی کے نتیجے کے ساتھ غیر آپشن سمجھا جاتا ہے۔
...
- عرف کو کیننیکل بیس 58 آئی ڈی میں حل کریں:
  - JSON `{ "alias": "pkr#bankb.paynet" }` کے ساتھ `POST /v1/assets/aliases/resolve`
...
- عوامی اثاثہ کے انتخاب کنندگان کینونیکل Base58 اثاثہ کی تعریف کے ids کے علاوہ تقسیم ملکیت والے فیلڈز (`account`، اختیاری `scope`) استعمال کرتے ہیں۔ خام انکوڈ شدہ `AssetId` لٹریلز اندرونی مددگار رہتے ہیں اور Torii/CLI سلیکٹر سطح کا حصہ نہیں ہیں۔
- اثاثہ کی تعریف کی فہرست/ استفسار کے فلٹرز اور قسمیں اضافی طور پر `alias_binding.status`، `alias_binding.lease_expiry_ms`، `alias_binding.grace_until_ms`، اور `alias_binding.bound_at_ms` کو قبول کرتی ہیں۔
