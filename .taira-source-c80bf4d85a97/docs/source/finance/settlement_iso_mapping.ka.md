...
##### `colr.010` / `.011` / `.012` (გირაოს მართვა)
...
##### `pacs.009` / `camt.054` (PvP დაფინანსება და განცხადებები)- ** აგენტები (`InstgAgt`, `InstdAgt`, მოვალე/კრედიტორი აგენტები)** → **BIC** სურვილისამებრ
...
- **Torii გადაყლაპვა** → გაგზავნეთ PvP დაფინანსების შედეგები `POST /v1/iso20022/pacs009`-ის მეშვეობით; ხიდი
...
| **BIC** | Regex `^[A-Z]{4}[A-Z]{2}[A-Z0-9]{2}([A-Z0-9]{3})?$` | სურვილისამებრ ფილიალის კოდი (ბოლო სამი სიმბოლო). დაადასტურეთ აქტიური სტატუსი RA ფაილებში.[^swift_bic] |
...
#### გადასასვლელი და მონაცემთა შენახვის ვალდებულებები- შეინახეთ **ISIN ↔ Norito აქტივის ID** და **CUSIP ↔ ISIN** გადასასვლელები. განაახლეთ ყოველ ღამე დან
...
- ISO გადასასვლელის კადრები მიიღება `iroha_core::iso_bridge::reference_data`-ის გამოყენებით
  ...
  `iso_reference_refresh_interval_secs` აჩვენებს გაშვების სიჯანსაღეს გაფრთხილებისთვის. Torii
...
- CLI დასახლების დამხმარეები მემკვიდრეობით იღებენ იგივე დამცავ რელსებს: უღელტეხილზე
  ...
  წინასწარი გადახედვა დაადასტურეთ ინსტრუმენტის ID-ები `sese.023` XML სნეპშოტის გამოშვებამდე.【crates/iroha_cli/src/main.rs#L3752】
- `cargo xtask iso-bridge-lint` (და CI შეფუთვა `ci/check_iso_reference_data.sh`) ლინტი
  ...
  გაშვებისას `--fixtures` მონიშნულია და უბრუნდება მონაცემთა ნიმუშებს `fixtures/iso_bridge/`-ში
  არგუმენტების გარეშე.【xtask/src/main.rs#L146】【ci/check_iso_reference_data.sh#L1】
- IVM დამხმარე ახლა შთანთქავს რეალურ ISO 20022 XML კონვერტებს (head.001 + `DataPDU` + `Document`)
  ...
  ბლოკები რჩება განზრახ გამოტოვებული. რეგრესიული ტესტები მოიხმარს ნიმუშებს და ახალსსათაურის კონვერტის მოწყობილობა რუკების დასაცავად.【crates/ivm/src/iso20022.rs:265】【crates/ivm/src/iso20022.rs:3301】【crates/ivm/src/iso20022.rs:3

#### მარეგულირებელი და ბაზრის სტრუქტურის მოსაზრებები

- **T+1 ანგარიშსწორება**: აშშ/კანადის აქციების ბაზრები გადავიდა T+1-ზე 2024 წელს; დაარეგულირეთ Norito
...
### მიწოდება-გადახდის წინააღმდეგ → `sese.023`| DvP ველი | ISO 20022 გზა | შენიშვნები |
|---------------------------------------------------|
...
> **ანგარიშსწორების კვალიფიკატორები** – ხიდი ასახავს საბაზრო პრაქტიკას ანგარიშსწორების პირობების კოდების (`SttlmTxCond`), ნაწილობრივი ანგარიშსწორების ინდიკატორების (`PrtlSttlmInd`) და სხვა არჩევითი კვალიფიკატორების კოპირებით Norito მეტამონაცემებიდან I101700X-ში, როდესაც არის I101700X. აღასრულეთ ISO გარე კოდების სიებში გამოქვეყნებული აღრიცხვები, რათა დანიშნულების CSD აღიაროს მნიშვნელობები.
...
ისინი აფინანსებენ ფასიანი ქაღალდების ანგარიშსწორებას.| PvP დაფინანსების სფერო | ISO 20022 გზა | შენიშვნები |
|--------------------------------------------------------------------------------------------|
...
მაგალითი (CLI ISO წინასწარი გადახედვა ბმულებით, შეჩერებით და ბაზრის MIC):
...
### რეპო გირაოს ჩანაცვლება → `colr.012`| რეპო ველი / კონტექსტი | ISO 20022 გზა | შენიშვნები |
|---------------------------------------------------------------------------------|-------|
...
| თმის შეჭრის ზომა | `Substitution/Haircut` | რიცხვითი; ასახავს ჩანაცვლების დროს გამოყენებული თმის შეჭრას |
| ორიგინალი/შემცვლელი ინსტრუმენტის ID | `Substitution/OriginalFinInstrmId`, `Substitution/SubstituteFinInstrmId` | არჩევითი ISIN/CUSIP თითოეული ფეხისთვის |
...
* `SupplementaryData` უნდა იყოს კანონიკური JSON (UTF‑8, დახარისხებული გასაღებები, JSON-მშობლიური
...
- `recipes/iso_bridge_builder.mjs` გვიჩვენებს, თუ როგორ უნდა დააკავშიროთ ეს დამხმარეები
...
- LuxCSD / Clearstream ISO 20022 ანგარიშსწორების მაგალითები, რომლებიც აჩვენებს `SttlmTpAndAddtlParams/SctiesMvmntTp` (`DELI`/`RECE`) და `Pmt` (`APMT`/`FREE`).[1](https://www.luxcsd.com/resource/blob/3434074/6f8add4708407a4701055be4dd04846b/c23005-eis-examples-cbf-data.pdf)
- Clearstream DCP სპეციფიკაციები, რომლებიც მოიცავს ანგარიშსწორების კვალიფიკატორებს (`SttlmTxCond`, `PrtlSttlmInd`).[2](https://www.clearstream.com/clearstream-en/res-library/market-coverage/instruction-specifications-swift-iso-20022-dcp-mode-ceu-spain-2357008)
- SWIFT PMPG სახელმძღვანელო, რომელიც გირჩევთ `pacs.009`-ს `CtgyPurp/Cd = SECU`-თან ერთად ფასიანი ქაღალდებთან დაკავშირებული PvP დაფინანსებისთვის.[3](https://www.swift.com/swift-resource/251897/download)
- ISO 20022 შეტყობინების განსაზღვრის ანგარიშები იდენტიფიკატორის სიგრძის შეზღუდვებისთვის (BIC, Max35Text).[4](https://www.iso20022.org/sites/default/files/2020-12/ISO20022_MDRPart2_ChangeOrVerifyAccountIdentification_2020_2021_v1_ForSEGReview.pdf)
- ANNA DSB ინსტრუქცია ISIN ფორმატისა და საკონტროლო ჯამის წესების შესახებ.[5](https://www.anna-dsb.com/isin/)

### გამოყენების რჩევები- ყოველთვის ჩასვით შესაბამისი Norito ფრაგმენტი ან CLI ბრძანება, რათა LLM-მა შეძლოს შემოწმება
...
- `SttlmParams/HldInd` შენარჩუნდება კრიტერიუმების დაკმაყოფილებამდე; გამოშვება `sese.030`-ით (`sese.031` სტატუსი).  
...
#### ცნობები
- SWIFT გირაოს მართვა MDR (`colr.010/011/012`).  
...
**დრო:** FX PvP ფანჯრები (CLS/ორმხრივი) და CSD-ის გამორთვა; შეინახეთ ფასიანი ქაღალდების ფეხები შეჩერებული ნაღდი ფულის დადასტურების მოლოდინში.#### შეტყობინების ქორეოგრაფია
...
2. `sese.023` DvP ინსტრუქცია(ებ)ი `HldInd=true`-ით, ასე რომ ფასიანი ქაღალდების ფეხი ელოდება ნაღდი ფულის დადასტურებას.  
...
5. თუ FX ფანჯარა დაირღვა → გააუქმეთ/გაუქმეთ ნაღდი ფული (`camt.056/029` ან `pacs.004`) და გააუქმეთ ფასიანი ქაღალდები (`sese.020` + `sese.027`, ან `camt.056/029` ან Norito).
...
- `PrtlSttlmInd` გაუთვალისწინებელი ცდომილების თავიდან ასაცილებლად.  
...
- დაბრუნების მოწყობილობები ახლა ნორმალიზდება `ChrgBr` (`DEBT`/`CRED`/`SHAR`/`SLEV`) და დაბრუნების საკუთრების მიზეზები, რომლებიც გამოვლენილია როგორც Norito, როგორც Norito, მომხმარებლის ხელახალი feeridge-ის გამოვლენა, როგორც Norito. კოდები XML კონვერტის ხელახალი ანალიზის გარეშე.
...
- აღასრულეთ ზემოთ მოცემული ქორეოგრაფია (გირაო: `colr.010/011/012 → sese.023/024/025`; FX დარღვევა: `pacs.009 (+pacs.002) → sese.023 held → release/cancel`).  
...
```წარადგინეთ დაკავშირებული ინსტრუქცია `SUBST-2025-04-001-B` (FoP მიღება შემცვლელი გირაოს) `SctiesMvmntTp=RECE`, `Pmt=FREE` და `WITH` კავშირით, რომელიც მიუთითებს `SUBST-2025-04-001-A`-ზე. გაათავისუფლეთ ორივე ფეხი შესაბამისი `sese.030`-ით, როგორც კი ჩანაცვლება დამტკიცდება.
...
გამოუშვით, როგორც კი `pacs.009` ორივე ფეხი მიაღწევს `ACSC`-ს:
...
`sese.031` ადასტურებს შეფერხების გათავისუფლებას, რასაც მოჰყვება `sese.025` ფასიანი ქაღალდების ნაწილის დაჯავშნის შემდეგ.
...
`pacs.002` აკონტროლებს გადახდის სტატუსს (`ACSC` = დადასტურებულია, `RJCT` = უარყოფა). თუ ფანჯარა დაირღვა, გაიხსენეთ `camt.056`/`camt.029` ან გაგზავნეთ `pacs.004` გადახდილი თანხების დასაბრუნებლად.