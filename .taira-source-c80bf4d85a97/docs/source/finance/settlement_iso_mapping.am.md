...
- **ፓርቲዎች / ወኪሎች ** → `DlvrgSttlmPties/Pty` እና `RcvgSttlmPties/Pty` ለ `sese.*` ፣
  ወይም የወኪሉ አወቃቀሮች በ `pacs.009`.
- ** መለያዎች ** → `…/Acct` ንጥረ ነገሮች ለመቆጠብ/ገንዘብ ሂሳቦች; በራሪ ወረቀቱን ያንጸባርቁ
  `AccountId` በ `SupplementaryData`።
...
- ** መሳሪያ (`FinInstrmId`)**
  ...
  - ውድቀት;
    - ** CUSIP ** ወይም ሌላ NSIN በ `…/OthrId/Id` ከ `Tp/Cd` ጋር ከ ISO ውጫዊ ስብስብ ጋር
      የኮድ ዝርዝር (ለምሳሌ `CUSP`); በ `Issr` ውስጥ ሰጪውን በተሰጠበት ጊዜ ያካትቱ።[^iso_mdr]
    - **Norito የንብረት መታወቂያ** እንደ ባለቤትነት፡ `…/OthrId/Id`፣ `Tp/Prtry="NORITO_ASSET_ID"`፣ እና
      በ `SupplementaryData` ውስጥ ተመሳሳይ እሴት ይመዝግቡ።
  - አማራጭ ገላጭ መግለጫዎች፡- **CFI** (`ClssfctnTp`) እና **FISN** ለመቅለል የሚደገፉበት
...
##### `pacs.009` / `camt.054` (PvP የገንዘብ ድጋፍ እና መግለጫዎች)- ** ወኪሎች (`InstgAgt`፣ `InstdAgt`፣ ተበዳሪ/አበዳሪ ወኪሎች)** → **BIC** ከአማራጭ ጋር
  LEI ከተፈቀደ።[^swift_bic]
- ** መለያዎች ***
...
- **Torii ማስመጣት** → የPvP የገንዘብ ድጋፍ እግሮችን በ`POST /v1/iso20022/pacs009` ያስገቡ። ድልድዩ
...
| ** CUSIP *** | Regex `^[A-Z0-9]{9}$` እና ሞዱል-10 ባለ 2 ክብደት (የቁምፊዎች ካርታ ወደ አሃዝ) | ISIN በማይገኝበት ጊዜ ብቻ; ካርታ በ ANNA/CUSIP የእግረኛ መንገድ አንዴ ከተገኘ።[^cusip] |
...
| **MIC** | ከ ISO 10383 RA ፋይል ያቆዩ; ቦታዎች ንቁ መሆናቸውን ያረጋግጡ (ምንም `!` ማቋረጫ ባንዲራ የለም) | ከመልቀቁ በፊት ኤምአይሲዎችን ጠቁም።[^iso_mic] |
| **ኢባን** | አገር-ተኮር ርዝመት፣ አቢይ ሆሄያት ፊደል፣ ሞድ-97 = 1 | በ SWIFT የተያዘ መዝገብ ይጠቀሙ; መዋቅራዊ ተቀባይነት የሌላቸውን IBANs ውድቅ ያድርጉ።[^swift_iban] |
| ** የባለቤትነት መለያ/የፓርቲ መታወቂያዎች** | `Max35Text` (UTF-8፣ ≤35 ቁምፊዎች) ከተስተካከለ ነጭ ቦታ ጋር | ለ`GenericAccountIdentification1.Id` እና `PartyIdentification135.Othr/Id` መስኮች ተፈጻሚ ይሆናል። የድልድይ ጭነት ከISO ንድፎች ጋር እንዲስማማ ከ35 ቁምፊዎች በላይ የሆኑ ግቤቶችን ውድቅ ያድርጉ። |
| ** የተኪ መለያ መለያዎች *** | ባዶ ያልሆነ `Max2048Text` በ `…/Prxy/Id` ከአማራጭ ዓይነት ኮድ ጋር በ`…/Prxy/Tp/{Cd,Prtry}` | ከዋናው IBAN ጎን ለጎን ተከማችቷል; የPvP ሀዲዶችን ለማንፀባረቅ ተኪ መያዣዎችን (ከአማራጭ ዓይነት ኮዶች ጋር) ሲቀበሉ ማረጋገጥ አሁንም IBANs ያስፈልገዋል። |
...
#### የእግረኛ መንገድ እና የመረጃ ጥገና ግዴታዎች- **ISIN ↔ Norito የንብረት መታወቂያ ** እና ** CUSIP ↔ ISIN** የእግረኛ መንገዶችን ማቆየት። በየምሽቱ አዘምን ከ
...
- የእያንዳንዱን የተጫነ የውሂብ ስብስብ ቅጂ ለማቆየት `iso_bridge.reference_data.cache_dir` አዋቅር
...
- IBAN እና ISO 4217 ማሰሪያዎች በተመሳሳይ ንብርብር ተፈጻሚ ይሆናሉ፡ pacs.008/pacs.009 አሁን ይፈስሳል።
  ...
  የመቋቋሚያ ምንዛሬ ከ`currency_assets` ጠፍቷል፣ ይህም የተበላሸ ድልድይ ይከላከላል
...
- የ CLI ሰፈራ ረዳቶች ተመሳሳይ የጥበቃ ሀዲዶችን ይወርሳሉ፡ ማለፍ
  ...
  የ`sese.023` XML ቅጽበተ ፎቶን ከማውጣቱ በፊት የተረጋገጠ የመሳሪያ መታወቂያዎችን አስቀድመው ይመልከቱ።【crates/iroha_cli/src/main.rs#L3752】
- `cargo xtask iso-bridge-lint` (እና የ CI መጠቅለያው `ci/check_iso_reference_data.sh`) lint
  የእግረኛ መሻገሪያ ቅጽበተ-ፎቶዎች እና የቤት ዕቃዎች። ትዕዛዙ `--isin`፣ `--bic-lei`፣ `--mic` እና
...
- የ IVM አጋዥ አሁን እውነተኛ ISO 20022 XML ኤንቨሎፕ (head.001 + `DataPDU` + `Document`) ያስገባል።
  ...
  ብሎኮች ሆን ተብሎ እንደተዘለሉ ይቆያሉ። የመመለሻ ሙከራዎች ናሙናዎችን እና አዲሱን ይበላሉየካርታ ስራዎችን ለመጠበቅ የራስጌ ኤንቨሎፕ እቃ።【crates/ivm/src/iso20022.rs:265】【crates/ivm/src/iso20022.rs:3301】【crates/ivm/src/iso20022.rs:3703】

#### የቁጥጥር እና የገበያ-መዋቅር ግምት

- **T+1 ሰፈራ**፡ የአሜሪካ/ካናዳ የፍትሃዊነት ገበያዎች በ2024 ወደ T+1 ተዛወሩ። አስተካክል Norito
...
- **የማያዣ ግዢዎች/መያዣዎች**: በግዢ ጊዜ እና አማራጭ መያዣዎች ላይ የ ESMA ዝመናዎችን ይከታተሉ
  ስለዚህ ሁኔታዊ ማድረስ (`HldInd`) ከቅርብ ጊዜ መመሪያ ጋር ይስማማል።[^csdr]
...
### መላኪያ-በክፍያ → `sese.023`| DvP መስክ | ISO 20022 መንገድ | ማስታወሻ |
|------------------------------------|------------|
...
| `delivery_leg.metadata`, `payment_leg.metadata` | `SctiesLeg/Metadata`, `CashLeg/Metadata` | አማራጭ Norito JSON በ UTF-8 ኮድ የተቀመጠ |

> ** የመቋቋሚያ ብቃቶች *** - ድልድዩ መስተዋቶች የገበያ ሁኔታን በመቅዳት የመቋቋሚያ ሁኔታ ኮዶችን (`SttlmTxCond`) ፣ ከፊል የሰፈራ አመልካቾችን (`PrtlSttlmInd`) እና ሌሎች ከ Norito ሜታዳታ ወደ Norito ሲቀርብ። መድረሻው ሲኤስዲ እሴቶቹን እንዲያውቅ በ ISO ውጫዊ ኮድ ዝርዝሮች ውስጥ የታተሙትን ቁጥሮች ያስፈጽሙ።
...
የዋስትና ክፍያን ይደግፋሉ።| PvP የገንዘብ ድጋፍ መስክ | ISO 20022 መንገድ | ማስታወሻ |
---------------------------------------------------------------------------|
...
| መመሪያ መለያ / የህይወት ዑደት ማገናኘት | `CdtTrfTxInf/PmtId/InstrId`, `CdtTrfTxInf/RmtInf` | ከ Norito `settlement_id` ጋር ይዛመዳል ስለዚህ የጥሬ ገንዘብ እግር ከደህንነቶች ጎን ጋር ይታረቃል። |
...
ምሳሌ (የ CLI ISO ቅድመ እይታ ከማገናኛ፣ ከመያዣ እና ከገበያ MIC ጋር)፡
...
### Repo የዋስትና ምትክ → `colr.012`| Repo መስክ / አውድ | ISO 20022 መንገድ | ማስታወሻ |
|-----------------------------------------------------------|
...
| የፀጉር መቆራረጥ መጠን | `Substitution/Haircut` | ቁጥር; በምትተካበት ወቅት የተተገበረውን የፀጉር አቆራረጥ ካርታ |
| ኦሪጅናል/የተተኩ የመሳሪያ መታወቂያዎች | `Substitution/OriginalFinInstrmId`, `Substitution/SubstituteFinInstrmId` | ለእያንዳንዱ እግር አማራጭ ISIN/CUSIP |
...
- `recipes/iso_bridge_builder.mjs` እነዚያን ረዳቶች እንዴት ሽቦ ማድረግ እንደሚቻል ያሳያል
  ...
  ኤክስኤምኤልን ፈጠረ፣ እና እንደ አማራጭ ለTorii (`ISO_SUBMIT=1`) ያቀርባል፣ እንደገና ጥቅም ላይ ይውላል።
  ልክ እንደ ISO ድልድይ የምግብ አዘገጃጀት መመሪያ ተመሳሳይ የጥበቃ ጊዜ።


### ዋቢዎች

- LuxCSD/ Clearstream ISO 20022 የሰፈራ ምሳሌዎች `SttlmTpAndAddtlParams/SctiesMvmntTp` (`DELI`/`RECE`) እና `Pmt` (`APMT`/`FREE`)[1](https://www.luxcsd.com/resource/blob/3434074/6f8add4708407a4701055be4dd04846b/c23005-eis-examples-cbf-data.pdf) 
- የሰፈራ ብቃቶችን (`SttlmTxCond`፣ `PrtlSttlmInd`) የሚሸፍኑ የDCP ዝርዝሮችን ያጽዱ።[2](https://www.clearstream.com/clearstream-en/res-library/market-coverage/instruction-specifications-swift-iso-20022-dcp-mode-ceu-spain-2357008)
- የስዊፍት PMPG መመሪያ `pacs.009` ከ `CtgyPurp/Cd = SECU` ጋር ከደህንነት ጋር ለተያያዘ PvP የገንዘብ ድጋፍ።[3](https://www.swift.com/swift-resource/251897/download)
- ISO 20022 የመልእክት ፍቺ ሪፖርቶች ለዪ ርዝመት ገደቦች (BIC፣ Max35Text)።[4](https://www.iso20022.org/sites/default/files/2020-12/ISO20022_MDRPart2_ChangeOrVerifyAccountIdentification_2020_2021_v1_ForSEGReview.pdf)
- ANNA DSB መመሪያ በISIN ቅርጸት እና በቼክሰም ደንቦች ላይ።[5](https://www.anna-dsb.com/isin/) 

### የአጠቃቀም ምክሮችLLM መመርመር እንዲችል ሁል ጊዜ የሚመለከተውን Norito ቅንጣቢ ወይም CLI ትእዛዝ ይለጥፉ
...
#### መልእክት ኮሪዮግራፊ
1. `colr.010` የማስያዣ ምትክ ጥያቄ → መያዣ ሰጪ/ተቀባይ ወይም ወኪል።  
2. `colr.011` የዋስትና ምትክ ምላሽ → መቀበል/ አለመቀበል (አማራጭ ውድቅ ምክንያት)።  
...
5. `sese.024` የሁኔታ ምክሮች (ተቀባይነት ያለው፣ የተዛመደ፣ በመጠባበቅ ላይ፣ ያልተሳካ፣ ውድቅ የተደረገ)።  
6. `sese.025` ማረጋገጫዎች አንዴ ከተያዙ።  
7. አማራጭ የገንዘብ ዴልታ (ክፍያ/የጸጉር መቁረጥ) → `pacs.009` FI-ወደ-FI ክሬዲት ማስተላለፍ ከ `CtgyPurp/Cd = SECU`; ሁኔታ በ `pacs.002` ፣ በ `pacs.004` በኩል ይመለሳል።
...
#### ሁኔታዊ/የማሳለፍ መስኮች
- `SctiesSttlmTxInstr/Lnkgs` (`WITH`/`BEFO`/`AFTE`) ሁለቱን መመሪያዎች በሰንሰለት ለማስያዝ።  
- `SttlmParams/HldInd` መመዘኛዎች እስኪሟሉ ድረስ መያዝ; በ`sese.030` (`sese.031` ሁኔታ) መልቀቅ።  
- `SttlmParams/PrtlSttlmInd` ከፊል ሰፈራ ለመቆጣጠር (`NPAR`, `PART`, `PARC`, `PARQ`).  
...
** ጊዜ: ** FX PvP መስኮቶች (CLS / ሁለትዮሽ) እና የሲኤስዲ መቁረጫዎች; የገንዘብ ማረጋገጫ በመጠባበቅ ላይ ያሉ የዋስትናዎች እግሮች እንዲቆዩ ያድርጉ።#### መልእክት ኮሪዮግራፊ
...
2. `sese.023` DvP መመሪያ(ዎች) ከ`HldInd=true` ጋር ስለዚህ የዋስትናዎች እግር የገንዘብ ማረጋገጫ ይጠብቃል።  
3. የህይወት ዑደት `sese.024` ማሳሰቢያዎች (ተቀባይነት ያለው / የተዛመደ / በመጠባበቅ ላይ).  
4. ሁለቱም `pacs.009` እግሮች መስኮቱ ከማለፉ በፊት `ACSC` ከደረሱ → በ `sese.030` → `sese.031` (mod status) → `sese.025` (ማረጋገጫ).  
5. የ FX መስኮት ከተጣሰ → ጥሬ ገንዘብን ይሰርዙ/ያስታውሱ (`camt.056/029` ወይም `pacs.004`) እና ዋስትናዎችን ይሰርዙ (`sese.020` + `sese.027` ፣ ወይም Norito ቀድሞ የተረጋገጠ ከሆነ)።
...
#### ሁኔታዊ/የማሳለፍ መስኮች
- `SttlmParams/HldInd` + `sese.030` መልቀቅ/በስኬት/ውድቀት ላይ መሰረዝ።  
- `Lnkgs` የዋስትና መመሪያዎችን ከገንዘብ እግር ጋር ለማያያዝ።  
ሁኔታዊ መላኪያ የሚጠቀሙ ከሆነ - T2S CoSD ደንብ።  
- `PrtlSttlmInd` ያልተፈለጉ ክፍሎችን ለመከላከል.  
- በ`pacs.009`፣ `CtgyPurp/Cd = SECU` ከደህንነት ጋር የተያያዘ የገንዘብ ድጋፍን ያሳያል።
...
- የመመለሻ ዕቃዎች አሁን `ChrgBr` መደበኛ አደረጉ (`DEBT`/`CRED`/`SHAR`/`SLEV`) እና የባለቤትነት መመለሻ ምክንያቶች በ Norito የኤክስኤምኤል ፖስታውን እንደገና መተንተን።
- በ `DataPDU` ፖስታዎች ውስጥ ያሉ የ AppHdr ፊርማ እገዳዎች ወደ ውስጥ ሲገቡ ችላ ይባላሉ። ኦዲቶች ከተከተቱ የXMLDSIG መስኮች ይልቅ በሰርጥ ፕሮቬንሽን ላይ መታመን አለባቸው።
...
```የተገናኘውን መመሪያ `SUBST-2025-04-001-B` (የተለዋጭ መያዣን ተቀባይ) ከ`SctiesMvmntTp=RECE`፣ `Pmt=FREE`፣ እና የ`WITH` ትስስር ወደ `SUBST-2025-04-001-A` ያቅርቡ። መተኪያው ከተፈቀደ በኋላ ሁለቱንም እግሮች በተዛማጅ `sese.030` ይልቀቁ።
...
`sese.031` የመያዣ ልቀቱን ያረጋግጣል፣ ከዚያም `sese.025` አንዴ የሴኩሪቲስ እግር ከተያዘ።
...
`pacs.002` የክፍያ ሁኔታን ይከታተላል (`ACSC` = የተረጋገጠ፣ `RJCT` = ውድቅ)። መስኮቱ ከተጣሰ በ`camt.056`/`camt.029` በኩል ያስታውሱ ወይም የተያዙ ገንዘቦችን ለመመለስ `pacs.004` ይላኩ።