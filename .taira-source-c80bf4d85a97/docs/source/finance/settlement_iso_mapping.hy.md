...
- **Հաշիվներ** → `…/Acct` տարրեր պահպանման/կանխիկի հաշիվների համար; հայելային մատյան
...
- **Գործիք (`FinInstrmId`)**
  ...
  - Հետադարձ կապ.
    - **CUSIP** կամ այլ NSIN `…/OthrId/Id`-ի ներքո՝ `Tp/Cd`-ով սահմանված ISO արտաքինից
      կոդերի ցանկ (օրինակ՝ `CUSP`); ներառել թողարկողին `Issr`-ում, երբ լիազորված է:[^iso_mdr]
...
- **Կուսակցություններ (`DlvrgSttlmPties`, `RcvgSttlmPties`)**
  ...
  - Հետադարձ. **LEI** որտեղ հաղորդագրության տարբերակը բացահայտում է հատուկ LEI դաշտ; եթե
    բացակայում է, կրում են սեփական ID-ներ՝ հստակ `Prtry` պիտակներով և ներառում BIC-ը մետատվյալներում:[^iso_cr]
- **Բնակավայրի / վայրի** → **MIC** վայրի համար և **BIC** CSD-ի համար:[^iso_mic]

##### `colr.010` / `.011` / `.012` (գրավի կառավարում)
...
##### `pacs.009` / `camt.054` (PvP ֆինանսավորում և քաղվածքներ)- **Գործակալներ (`InstgAgt`, `InstdAgt`, պարտապանի/պարտատիրոջ գործակալներ)** → **BIC** ընտրովի
...
- **Torii կլանում** → Ներկայացրեք PvP ֆինանսավորման ոտքերը `POST /v1/iso20022/pacs009`-ի միջոցով; կամուրջը
...
| **LEI** | Regex `^[A-Z0-9]{18}[0-9]{2}$` և mod-97 ստուգիչ թվանշան (ISO 17442) | Վավերացրե՛ք GLEIF-ի ամենօրյա դելտա ֆայլերի հետ մինչև ընդունումը։[^gleif] |
...
| **Վստահված անձի հաշվի նույնացուցիչներ** | Ոչ դատարկ `Max2048Text` տակ `…/Prxy/Id` կամընտիր տիպի կոդերով `…/Prxy/Tp/{Cd,Prtry}` | Պահպանվում է առաջնային IBAN-ի կողքին; վավերացումը դեռևս պահանջում է IBAN-ներ՝ միաժամանակ ընդունելով վստահված անձի բռնակներ (ընտրովի տիպի կոդերով)՝ PvP ռելսերը արտացոլելու համար: |
...
#### Հետիոտնային անցում և տվյալների պահպանման պարտավորություններ- Պահպանեք **ISIN ↔ Norito ակտիվի ID** և **CUSIP ↔ ISIN** հետիոտնային անցումներ: Թարմացրեք ամեն գիշեր սկսած
...
- Կարգավորեք `iso_bridge.reference_data.cache_dir`-ը, որպեսզի պահպանի յուրաքանչյուր բեռնված տվյալների կրկնօրինակը
...
- ISO-ի հետիոտնային խաչմերուկի նկարները ներծծվում են `iroha_core::iso_bridge::reference_data`-ի կողմից՝ օգտագործելով
  ...
  `iso_reference_refresh_interval_secs` ցուցադրում է աշխատաժամանակի առողջությունը ահազանգելու համար: Torii
...
- IBAN-ի և ISO 4217-ի կապակցումները կիրառվում են նույն շերտում. pacs.008/pacs.009-ը հոսում է հիմա
  արձակում են `InvalidIdentifier` սխալներ, երբ պարտապանի/պարտատիրոջ IBAN-ները չունեն կազմաձևված անուն-ազգանուններ կամ երբ
...
- CLI-ի կարգավորման օգնականները ժառանգում են նույն պահակային ռելսերը՝ անցում
  ...
  նախադիտեք հաստատեք գործիքների ID-ները՝ նախքան `sese.023` XML ակնարկը հրապարակելը:【crates/iroha_cli/src/main.rs#L3752】
...
- IVM օգնականն այժմ ընդունում է իրական ISO 20022 XML ծրարներ (գլուխ.001 + `DataPDU` + `Document`)
  ...
  բլոկները մնում են միտումնավոր բաց թողնված: Ռեգրեսիայի թեստերը սպառում են նմուշները և նորըվերնագրի ծրարի սարք՝ քարտեզագրումները պաշտպանելու համար:【crates/ivm/src/iso20022.rs:265】【crates/ivm/src/iso20022.rs:3301】【crates/ivm/src/iso20022.rs:3

#### Կարգավորող և շուկայական կառուցվածքի նկատառումներ

- **T+1 հաշվարկ**. ԱՄՆ/Կանադայի բաժնետոմսերի շուկաները 2024 թվականին տեղափոխվեցին T+1; հարմարեցնել Norito
...
### Առաքում՝ ընդդեմ վճարման → `sese.023`| DvP դաշտ | ISO 20022 ուղի | Ծանոթագրություններ |
|---------------------------------------------------- |
...
| **Հաղորդագրության նպատակը** | `SttlmTpAndAddtlParams/SctiesMvmntTp` | `DELI` (առաքում) կամ `RECE` (ստանալ); հայելիներ, որոնց ոտքը կատարում է ներկայացնող կողմը: |
...
նրանք ֆինանսավորում են արժեթղթերի մարումը:| PvP ֆինանսավորման դաշտ | ISO 20022 ուղի | Ծանոթագրություններ |
|---------------------------------------------------------------------------------------------------|
...
Օրինակ (CLI ISO-ի նախադիտում կապերով, պահում և շուկայական MIC).
...
### Repo գրավի փոխարինում → `colr.012`| Ռեպո դաշտ / համատեքստ | ISO 20022 ուղի | Ծանոթագրություններ |
|------------------------------------------------------------------------------------|----------|
...
| Սանրվածքի չափը | `Substitution/Haircut` | Թվային; քարտեզագրում է փոխարինման ժամանակ կիրառված սանրվածքը |
| Օրիգինալ/փոխարինող գործիքի ID-ներ | `Substitution/OriginalFinInstrmId`, `Substitution/SubstituteFinInstrmId` | Լրացուցիչ ISIN/CUSIP յուրաքանչյուր ոտքի համար |
...
* `SupplementaryData`-ը պետք է լինի կանոնական JSON (UTF‑8, տեսակավորված ստեղներ, JSON-հայրենի
...
* CLI կարգավորման օգնականները (`iroha app settlement ... --atomicity ...`) այժմ արտանետում են
  Norito հրահանգներ, որոնց կատարման պլանները քարտեզագրված են 1:1-ից մինչև `Plan/ExecutionOrder` և
...
- `recipes/iso_bridge_builder.mjs`-ը ցույց է տալիս, թե ինչպես միացնել այդ օգնականները
  ...
  ստեղծվել է XML, և կամայականորեն այն ներկայացնում է Torii (`ISO_SUBMIT=1`)՝ նորից օգտագործելու համար
  նույն սպասման արագությունը, ինչ ISO կամուրջի բաղադրատոմսը:


### Հղումներ

- LuxCSD / Clearstream ISO 20022 հաշվարկման օրինակներ, որոնք ցույց են տալիս `SttlmTpAndAddtlParams/SctiesMvmntTp` (`DELI`/`RECE`) և `Pmt` (`APMT`/`FREE`).[1](https://www.luxcsd.com/resource/blob/3434074/6f8add4708407a4701055be4dd04846b/c23005-eis-examples-cbf-data.pdf)
- Clearstream DCP բնութագրերը, որոնք ընդգրկում են հաշվարկային որակավորումները (`SttlmTxCond`, `PrtlSttlmInd`):[2](https://www.clearstream.com/clearstream-en/res-library/market-coverage/instruction-specifications-swift-iso-20022-dcp-mode-ceu-spain-2357008)
- SWIFT PMPG ուղեցույց, որն առաջարկում է `pacs.009` `CtgyPurp/Cd = SECU`-ով արժեթղթերի հետ կապված PvP ֆինանսավորման համար:[3](https://www.swift.com/swift-resource/251897/download)
- ISO 20022 հաղորդագրության սահմանման հաշվետվություններ նույնացուցիչի երկարության սահմանափակումների համար (BIC, Max35Text):[4](https://www.iso20022.org/sites/default/files/2020-12/ISO20022_MDRPart2_ChangeOrVerifyAccountIdentification_2020_2021_v1_ForSEGReview.pdf)
- ANNA DSB ուղեցույց ISIN ձևաչափի և ստուգիչ գումարի կանոնների վերաբերյալ:[5](https://www.anna-dsb.com/isin/)

### Օգտագործման խորհուրդներ- Միշտ տեղադրեք համապատասխան Norito հատվածը կամ CLI հրամանը, որպեսզի LLM-ն կարողանա ստուգել
...
6. `sese.025` հաստատումներ մեկ անգամ ամրագրված:  
...
- Տրանսպորտային մակարդակ. դարպասները կարող են արտանետել `admi.007` կամ մերժվել մինչև բիզնեսի մշակումը:  
- Հաշվարկների կյանքի ցիկլը՝ `sese.024` (մշակման կարգավիճակ + պատճառի ծածկագրեր), `sese.025` (վերջնական):  
...
**Ժամկետ.** FX PvP պատուհանների (CLS/երկկողմանի) և CSD-ի անջատումներ; պահեք արժեթղթերի ոտքերը՝ սպասելով կանխիկի հաստատմանը:#### Հաղորդագրություն խորեոգրաֆիա
...
5. Եթե արտարժույթի պատուհանը խախտված է, → չեղարկեք/հետ կանչեք կանխիկ դրամը (`camt.056/029` կամ `pacs.004`) և չեղարկեք արժեթղթերը (`sese.020` + `sese.027`, կամ `camt.056/029` կամ `pacs.004`), կամ `sese.027`, կամ Norito, եթե արդեն հաստատված է շուկայական կանոնը:
...
#### Պայմանականություն / արձակել դաշտերը
- `SttlmParams/HldInd` + `sese.030` թողարկում/չեղարկում հաջողության/անհաջողության դեպքում:  
...
- վերադարձի հարմարանքները այժմ նորմալացվում են `ChrgBr` (`DEBT`/`CRED`/`SHAR`/`SLEV`) և սեփականության վերադարձի պատճառները, որոնք բացահայտված են որպես Norito, որպես սպառողի վերադարձի ֆիրմային պատճառներ, որոնք բացահայտված են որպես Norito, որպես սպառողի վերադարձի պատճառներ կոդերը՝ առանց XML ծրարը նորից վերլուծելու:
...
- Վերաբերվեք `sese.024`/`sese.025` կարգավիճակներին և `pacs.002` արդյունքներին որպես մուտքի ազդանշաններ; `ACSC`-ը հրահրում է արձակումը, `RJCT` ուժերը արձակում են:  
...
#### Գրավի փոխարինման զույգ (`sese.023`) հրահանգների կապով
...
```Ներկայացրե՛ք կապակցված `SUBST-2025-04-001-B` հրահանգը (FoP ստանալ փոխարինող գրավ)՝ `SctiesMvmntTp=RECE`, `Pmt=FREE` և `WITH` կապով, որը ցույց է տալիս դեպի `SUBST-2025-04-001-A`: Փոխարինումը հաստատվելուց հետո բաց թողեք երկու ոտքերը համապատասխան `sese.030`-ով:

#### Արժեթղթերը պահվում են՝ սպասվող արտարժույթի հաստատման (`sese.023` + `sese.030`)
...
Թողարկեք, երբ `pacs.009` երկու ոտքերը հասնեն `ACSC`:
...
#### PvP ֆինանսավորման ոլորտ (`pacs.009` արժեթղթերի նպատակով)
