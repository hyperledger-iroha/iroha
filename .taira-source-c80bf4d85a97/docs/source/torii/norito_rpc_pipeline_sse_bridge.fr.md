...
source_last_modified: "2026-01-18T05:31:56.992506+00:00"
...
5. **Fallback order** – Norito RPC callers SHOULD attempt binary first and downgrade to JSON only when Torii returns `415 unsupported_layout`, `NRPC_DISABLED`, or a TLS failure. The downgrade must be logged alongside the queue envelope so operators know that `/v1/pipeline` carried the request.
...
4. Poll `/v1/pipeline/transactions/status?hash=<tx_hash>` using exponential backoff. Abort when:
