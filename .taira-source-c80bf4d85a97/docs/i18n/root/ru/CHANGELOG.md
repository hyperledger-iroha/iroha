...
— Замените использование `retired_codec` на `norito::codec` в ящиках.
- Начать миграцию инструментов на собственную сериализацию Norito.
— Удалите оставшуюся зависимость `retired codec` из рабочей области в пользу собственной сериализации Norito.
— Замените остаточные производные признаков retired codec на собственные реализации Norito и переименуйте модуль кодека с версией.
...
- использовать мультихеш-формат для закрытых ключей (#4541)
 - переименуйте `legacy_codec_decoder` в `norito_cli`
...
- Перейдите в новый контейнер, чтобы предотвратить конфликты с текущими пользователями.
 - Hyperledger#2158 Обновление `retired_codec` и другие зависимости. (кодек Norito)
- Исправлена сборка.
