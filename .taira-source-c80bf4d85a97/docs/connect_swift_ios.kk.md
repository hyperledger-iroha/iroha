...
## Ұсынылатын SDK ағыны (ConnectClient + Norito көпірі)

Толық Xcode интеграциясы (SPM/CocoaPods, XCFramework сымдары, ChaChaPoly көмекшілері) керек пе?
Үшін ұшына орау нұсқаулығын `docs/connect_swift_integration.md` қараңыз.

Swift SDK Norito қолдауы бар Connect стекін жеткізеді:

- `ConnectClient` жоғарғы жағында WebSocket (`/v1/connect/ws?...`) тасымалдауын қолдайды.
  `URLSessionWebSocketTask`.
- `ConnectSession` өмірлік циклді реттейді (ашу → мақұлдау/қабылдамау → қол қою → жабу) және
  бағыт пернелері орнатылғаннан кейін шифрленген мәтін жақтауларының шифрын шешеді.
- `ConnectCrypto` X25519 кілт генерациясын және Norito-үйлесімді бағыт кілтін көрсетеді
  туынды, сондықтан қолданбаларға ешқашан HKDF/HMAC сантехникасын қолмен енгізу қажет емес.
- `ConnectEnvelope`/`ConnectControl` шығарылған Norito терілген кадрларды білдіреді
  Тот көпірі (`connect_norito_bridge`); арқылы шифрленген мәтін конверттерінің шифры ашылады
  Android/Rust жүйесінде қолданылатын бірдей FFI көмекшілері теңдікке кепілдік береді.

Сеансты бастамас бұрын:
1. 32 байт сеанс идентификаторын (`sid`) басқаларымен бірдей BLAKE2b рецептісін пайдаланып шығарыңыз
   SDK (`"iroha-connect|sid|" || chain_id || app_pk || nonce16`).
2. `ConnectCrypto.generateKeyPair()` арқылы Connect пернелер жұбын жасаңыз немесе сақталған пернелерді қайта пайдаланыңыз.
   жеке кілт (ашық кілттерді `ConnectCrypto.publicKey(fromPrivateKey:)` көмегімен қайта есептеуге болады).
3. WebSocket клиентін жасаңыз және оны асинхронды контекст ішінде бастаңыз.
...
`ConnectSession`, егер шифрлық мәтін жақтаулары болса, `ConnectSessionError.missingDecryptionKeys` шығарады
бағыт пернелері орнатылмай тұрып келу; өңдеуден кейін оларды дереу алыңыз
`Approve` басқару (әмиянның ашық кілті пайдалы жүктемеге кіреді). Шифрлық мәтінді тексеру үшін
кадрларды қолмен бағыттау үшін `ConnectEnvelope.decrypt(frame:symmetricKey:)` нөміріне қоңырау шалыңыз
жақтау бағытына сәйкес келетін перне.

> **Кеңес:** Norito көпірі болмаған кезде (мысалы, Swift Package Manager қолданбасын құрастырады.
> XCFramework), SDK автоматты түрде JSON белгішесіне қайта түседі. Шифрлау көмекшілері
> (`ConnectCrypto.*`) көпірді қажет етеді, сондықтан өндіріс қолданбаларында XCFramework байланыстырыңыз.
...
- Connect немесе bridge біріктіру өзгерістерін жасамас бұрын, келесі әрекеттерді орындаңыз:

  ```bash
  make swift-ci
  ```

  Пәрмен Swift құрылғыларын тексереді, бақылау тақтасының арналарын тексереді және CLI көрсетеді
  қорытындылар. CI жұмыс процесі Buildkite метадеректеріне сүйенеді
  Нәтижелерді қайта симуляторға салыстыру үшін (`ci/xcframework-smoke:<lane>:device_tag`) немесе
  StrongBox жолақтары — конвейерлерді немесе агент тегтерін өзгерткеннен кейін метадеректерді әлі де растаңыз
  журналдарда пайда болады.
- Егер іске қосу сәтсіз болса, `docs/source/swift_parity_triage.md` орындаңыз және тексеріңіз
  Қай жолақты қалпына келтіру немесе одан әрі оқиға қажет екенін анықтау үшін `mobile_ci` шығысы
  өңдеу.