...
#[test]
fn test_non_utf8_surrogate_pairs_are_not_empty() {
    // U+1F600 (grinning face) requires surrogate pair in UTF-16, ensuring
    // `lengthOfBytesUsingEncoding:` has to transcode.
    let expected = "😀";
    let s = NSString::from_str(expected);
    assert_eq!(s.len_utf16(), expected.encode_utf16().count());
    assert!(!s.is_empty(), "grinning face string should not be treated as empty");
}
...
fn test_compare_with_str() {
    let ns = NSString::from_str("crab");
    assert_eq!(&*ns, "crab");
    assert!("crab" == &*ns);
    assert!(&*ns < "crustacean");
    let crustacean = NSString::from_str("crustacean");
    assert!("crab" < &*crustacean);
    assert!(&*ns > "");
    assert!("cr" < &*ns);

    #[cfg(feature = "Foundation_NSMutableString")]
    {
        let mut_ns = NSMutableString::from_str("hello");
        assert_eq!(&*mut_ns, "hello");
        assert!("hello" == &*mut_ns);
        assert!(&*mut_ns < "world");
        assert!("helln" < &*mut_ns);
    }
...
fn test_macro() {
    macro_rules! test {
        ($($s:expr,)+) => {$({
            ...
            assert_eq!(s1_string, s2_string);

            let expected = if $s.starts_with('\u{feff}') {
            ...
            assert_eq!(s1_string.as_str(), expected);
            assert_eq!(s2_string.as_str(), expected);
        })+};
    }
