...
/// # Safety
///
/// The pointer must reference an object that has just been removed from the
/// mutable collection, or the collection must be consumed right after this.
#[inline]
pub(crate) unsafe fn mutable_collection_retain_removed_id<T>(obj: NonNull<T>) -> Id<T>
where
    T: Message,
{
    ...
    // convert it to `Id<T>`.
    unsafe { Id::retain(obj.as_ptr()).unwrap_unchecked() }
}
