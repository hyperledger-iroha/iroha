...
impl Scalar {
    ...
    pub fn double(&self) -> Scalar {
        ...
        let d1 = (self.0[1] << 1) | carry0;

        let carry2 = self.0[2] >> 63;
        let d2 = (self.0[2] << 1) | carry1;

        let d3 = (self.0[3] << 1) | carry2;

        // Subtract the modulus if the doubled value overflowed the field.
...
mod tests {
    ...
    #[test]
    fn double_matches_addition_for_samples() {
        let samples = [
            Scalar::zero(),
            Scalar::one(),
            R2,
            GENERATOR,
            -Scalar::one(),
            Scalar::from_raw([42, 0, 0, 0]),
            Scalar::from_raw([0, 1, 0, 0]),
            Scalar::from_raw([0, 0, 1, 0]),
            Scalar::from_raw([0, 0, 0, 1 << 10]),
        ];

        for &sample in &samples {
            let doubled = sample.double();
            let summed = sample.add(&sample);
            assert_eq!(doubled, summed, "doubling mismatch for {sample:?}");
        }
    }
