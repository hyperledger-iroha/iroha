...
pub mod isi {
    ...
    impl Execute for Unregister<Account> {
        ...
        fn execute(
            ...
            if let Some((capability_id, _)) = state_transaction
                .world
                .asset_transfer_capabilities()
                .iter()
                .find(|(_, capability)| {
                    capability.status
                        == iroha_data_model::asset_transfer_capability::AssetTransferCapabilityStatusV1::Active
                        && (capability.grantor == account_id
                            || capability.delegate == account_id
                            || capability.destination == account_id
                            || capability.source.account() == &account_id)
                })
            {
                return Err(InstructionExecutionError::InvariantViolation(
                    format!(
                        "cannot unregister account {account_id}: it is referenced by active asset transfer capability {:?}; revoke or consume the capability first",
        ...
        fn execute(
            ...
            if let Some((capability_id, _)) = state_transaction
                ...
                .find(|(_, capability)| {
                    capability.status
                        == iroha_data_model::asset_transfer_capability::AssetTransferCapabilityStatusV1::Active
                        && capability.source.definition() == &asset_definition_id
                })
            {
                return Err(InstructionExecutionError::InvariantViolation(
                    format!(
                        "cannot unregister asset definition {asset_definition_id}: it is referenced by active asset transfer capability {:?}; revoke or consume the capability first",
                        capability_id.as_hash()
                    )
                    .into(),
                )
...
mod tests {
    ...
    fn seed_account_alias_lease_record(
            ...
            .expect("fixture alias domain")
        {
            ...
            let domain_selector =
                iroha_data_model::account::AccountDomainSelector::from_domain(&domain_id)
                    .expect("fixture domain selector");
            tx.world
                .domain_selectors
    ...
    #[test]
    fn set_asset_definition_balance_policy_rejects_missing_migration_dataspace() {
        let mut state = test_state();
        let authority = (*ALICE_ID).clone();
        let domain_id: DomainId =
        ...
        .execute(&authority, &mut tx)
        .expect_err("migration dataspace is required for global to restricted");
        ...
        assert!(
            message.contains("requires migrate_global_balances_to_dataspace"),
            "unexpected error: {message}"
        );
    }

    #[test]
    fn set_asset_definition_balance_policy_rejects_noop_with_migration_dataspace() {
        let mut state = test_state();
        let authority = (*ALICE_ID).clone();
        let domain_id: DomainId =
        ...
        .execute(&authority, &mut tx)
        .expect_err("migration dataspace is invalid for a policy no-op");
