...
- `metal-stage1`: Enable experimental Metal Stage‑1 JSON structural indexer (macOS/aarch64 only). Norito falls back to scalar if the helper reports unavailable or fails validation.
...
- `cuda-stage1`: Enable experimental CUDA Stage‑1 JSON structural indexer (attempts to load on Windows/macOS/Linux). Norito falls back to scalar if the helper reports unavailable or fails validation.
...
- Stage‑1 GPU threshold: Stage‑1 builders (Metal/CUDA) are gated by a 192 KiB default cutoff to amortize launch overhead; override with `NORITO_STAGE1_GPU_MIN_BYTES`. The Metal and CUDA Stage‑1 helpers classify quotes, backslashes, and structural characters on GPU, finalize the tape with quote-parity handling on the host, report the required tape length on no-space, and report unavailable when their GPU backend cannot run. Scalar/CPU SIMD paths remain the Norito-owned fallback and produce identical tapes.
...
Determinism: CRC64 and JSON Stage-1 accelerated paths produce bit-for-bit identical results to their portable counterparts. GPU zstd helpers emit single standard zstd frames that decode to identical payload bytes; compressed frame bytes may differ from the CPU zstd encoder because match finding is backend-specific. Do not use public Norito compressed bytes as consensus-canonical hash/signature input unless the callsite explicitly fixes its own compression implementation, as the query projection DA path does with `zstd::bulk::compress`. Hardware differences only affect performance and compressed representation, never decoded content.
