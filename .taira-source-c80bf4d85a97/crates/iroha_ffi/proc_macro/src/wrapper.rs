...
pub fn wrap_as_opaque(emitter: &mut Emitter, mut input: FfiTypeInput) -> TokenStream {
    ...
    // make PhantomData fields public so that the generated tuple structs can be
    // constructed outside of this module if needed (e.g. in tests or by
    // consumers of the macro). Lack of `pub` here causes the fields of
    // generated tuple structs to be private which in turn prevents `ffi_import`
    // from constructing them when wrapping FFI calls.
    let phantom_data_type_defs: Vec<_> = input
        .generics
        .type_params()
        .map(|param| quote! {, pub core::marker::PhantomData<#param>})
        .collect();
    ...
    // Preserve doc comments from the original type so generated wrappers
    // retain the user-facing documentation.
    ...
    quote! {
        #(#attrs)*
        #[repr(transparent)]
        // Expose the inner pointer so that the surrounding code (in particular
        // the `ffi_import` macro) can access it. Previously this field was
        // private which made the generated structures unusable outside of this
        // module and resulted in compilation errors when importing opaque
        // structures.
        #vis struct #name #ty_generics(pub *mut iroha_ffi::Extern #(#phantom_data_type_defs)*) #handle_bounded_where_clause;

        #(#attrs)*
        #[derive(Clone, Copy)]
        #[repr(transparent)]
        // Same visibility adjustment for reference wrappers used when passing
        // references across the FFI boundary.
        #vis struct #ref_name #ref_ty_generics (pub *const iroha_ffi::Extern, pub core::marker::PhantomData<&#lifetime ()> #(#phantom_data_type_defs)*) #handle_bounded_where_clause;

        #(#attrs)*
        #[repr(transparent)]
        #vis struct #ref_mut_name #ref_ty_generics(pub *mut iroha_ffi::Extern, pub core::marker::PhantomData<&#lifetime mut ()> #(#phantom_data_type_defs)*) #handle_bounded_where_clause;
...
fn wrap_method_with_signature_and_body(
    ...
    let method_doc: Vec<_> = fn_descriptor.doc_attrs().collect();
...
fn gen_ffi_fn_call_stmt(fn_descriptor: &FnDescriptor, ffi_fn_name: &Ident) -> TokenStream {
    ...
    let execution_fail_arm = fn_descriptor.output_arg.as_ref().map_or_else(
        || quote! {},
        |output| {
            if let Some((_, err_ty)) = unwrap_result_type(output.src_type()) {
                let err_ty = err_ty.clone();
                quote! {
                    iroha_ffi::FfiReturn::ExecutionFail => {
                        return Err(<#err_ty as iroha_ffi::FromFfiReturn>::from_ffi_return(__ffi_return));
                    }
                }
            } else {
                quote! {}
