...
PLAN_SCHEMA = "sorafs.reserve_rent.rollout_evidence_collection_plan.v1"
...
PLAN_REQUIRED_THRESHOLD_FIELDS = frozenset(
    {
        "max_ledger_age_secs",
        "max_lifecycle_lag_secs",
        "max_route_latency_ms",
        "max_bake_age_secs",
    }
)
PLAN_POSITIVE_THRESHOLD_FIELDS = frozenset({"max_route_latency_ms", "now_unix"})
...
def validate_plan_json(
    ...
    """Validate the reserve/rent collection-plan envelope before use."""

    return validate_runner_evidence_plan(
        rendered,
        plan,
        diagnostic_prefix="reserve/rent rollout runner plan",
