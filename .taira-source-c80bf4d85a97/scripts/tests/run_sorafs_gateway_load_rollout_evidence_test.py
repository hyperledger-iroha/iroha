"""Tests for scripts/run_sorafs_gateway_load_rollout_evidence.py."""
...
SPEC = importlib.util.spec_from_file_location(
    "run_sorafs_gateway_load_rollout_evidence",
    MODULE_PATH,
)
...
def complete_args(tmp_path: Path) -> list[str]:
    ...
    return [
        ...
        str(write_payload(payload_dir / "local-conformance.json")),
        "--staging-load-evidence",
        str(write_payload(payload_dir / "staging-load.json")),
        "--telemetry-slo-evidence",
        str(write_payload(payload_dir / "telemetry-slo.json")),
        "--transport-scope-evidence",
        str(write_payload(payload_dir / "transport-scope.json")),
...
def test_dry_run_prints_complete_gateway_load_rollout_plan(
    tmp_path: Path,
    capsys,
) -> None:
    ...
    assert plan["schema"] == "sorafs.gateway_load.rollout_evidence_collection_plan.v1"
    assert plan["verifier_summary_schema"] == "sorafs.gateway_load.rollout_evidence_gate.v1"
    ...
    assert plan["external_evidence"]["local_conformance"] == [
        str(tmp_path / "payloads" / "local-conformance.json")
    ]
    assert plan["evidence_contract"]["local_conformance"]["schema"] == (
        "sorafs.gateway_load.local_conformance.v1"
    )
    assert (
        "suite_report_digest_hex"
        in plan["evidence_contract"]["local_conformance"]["required_payload_fields"]
    )
    assert (
    ...
    assert (
        ...
        in plan["evidence_contract"]["transport_scope"]["required_payload_fields"]
    )
    assert (
        "iroha_config_bound"
    ...
    assert "check_sorafs_gateway_load_rollout_evidence.py" in verifier[1]
    assert "--evidence" in verifier
    assert str(tmp_path / "payloads" / "staging-load.json") in verifier
    assert verifier.count("--local-conformance-evidence") == 0
...
def test_response_file_dry_run_prints_complete_gateway_load_plan(
    ...
    args_file = write_args_file(
        tmp_path / "gateway-load-rollout.args",
    ...
    assert plan["external_evidence"]["local_conformance"]
    assert "telemetry_slo" in plan["evidence_contract"]


def test_split_response_file_dry_run_prints_complete_gateway_load_plan(
    tmp_path: Path,
    capsys,
) -> None:
    args_file = write_split_args_file(
        tmp_path / "split-gateway-load-rollout.args",
        complete_args(tmp_path),
    )
    ...
    assert plan["schema"] == "sorafs.gateway_load.rollout_evidence_collection_plan.v1"
    assert plan["steps"][0]["label"] == "rollout_evidence_gate"
    assert "transport_scope" in plan["evidence_contract"]


def test_missing_required_kind_evidence_fails_before_plan(
    tmp_path: Path,
    capsys,
) -> None:
    args = complete_args(tmp_path)
    evidence_index = args.index("--local-conformance-evidence")
...
def test_missing_payload_file_fails_before_plan(tmp_path: Path, capsys) -> None:
    args = complete_args(tmp_path)
    missing = tmp_path / "missing-staging-load.json"
    evidence_index = args.index("--staging-load-evidence") + 1
    ...
    assert "--staging-load-evidence" not in captured.err
...
def test_subset_gate_requires_only_selected_kind(tmp_path: Path, capsys) -> None:
    ...
    assert exit_code == 0
    plan = json.loads(capsys.readouterr().out)
    assert plan["required_kinds"] == ["local_conformance"]
    assert list(plan["evidence_contract"]) == ["local_conformance"]
    verifier = plan["steps"][0]["command"]
    assert verifier.count("--require-kind") == 1
    assert "local_conformance" in verifier
