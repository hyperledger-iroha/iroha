...
class OfflineCashCodecTest {
    ...
    @Test
    fun cashMutationRequestsRejectNonCanonicalSignedFields() {
        val binding = deviceBinding()
        val proof = deviceProof()
        val redeemProof = redeemRequestProof()
        ...
        assertEquals("1.2300", load.amount)
        assertEquals("1.2300", load.toJsonMap()["amount"])
        ...
        assertEquals("10", redeem.amount)
        assertEquals("10", redeem.toJsonMap()["amount"])

        val invalidCases = listOf<Pair<String, () -> Unit>>(
            "account_id must be an exact non-empty string" to {
                OfflineCashSetupRequest(
                    accountId = " acct",
                    assetDefinitionId = "asset",
                    deviceBinding = binding,
                    deviceProof = proof,
            ...
            "asset_definition_id must be an exact non-empty string" to {
                OfflineCashSetupRequest(
                    accountId = "acct",
                    assetDefinitionId = "",
                    deviceBinding = binding,
            ...
            "operation_id must be an exact non-empty string" to {
                OfflineCashLoadRequest(
                    operationId = " op-load",
                    lineageId = null,
                    accountId = "acct",
                    assetDefinitionId = "asset",
            ...
            "lineage_id must be an exact non-empty string" to {
                OfflineCashLoadRequest(
                    operationId = "op-load",
                    lineageId = "lineage-1\n",
                    accountId = "acct",
            ...
            "amount must be an exact amount string" to {
                OfflineCashLoadRequest(
                    operationId = "op-load",
                    lineageId = null,
                    accountId = "acct",
            ...
            "amount must be a non-negative amount" to {
                OfflineCashLoadRequest(
                    operationId = "op-load",
                    lineageId = null,
                    accountId = "acct",
        ...
        for ((expectedMessage, build) in invalidCases) {
            assertEquals(
                expectedMessage,
                assertFailsWith<IllegalArgumentException> { build() }.message,
            )
        }
    }
    ...
    private fun redeemRequestProof(): OfflineRedeemRequestProof =
