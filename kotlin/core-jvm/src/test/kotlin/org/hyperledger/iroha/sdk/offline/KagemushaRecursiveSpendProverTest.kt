...
class KagemushaRecursiveSpendProverTest {
    ...
    fun exposesStableModesAndCircuitIds() {
        ...
        assertEquals(
            ...
            KagemushaRecursiveSpendProver.PASTA_CYCLE_V3_TRANSCRIPT_PROFILE,
        )
        assertEquals(4_096, KagemushaRecursiveSpendProver.PASTA_CYCLE_V3_MAX_PROOF_BYTES)
        assertEquals(
        ...
        assertFalse(KagemushaRecursiveSpendProver.RECURSIVE_SPEND_LINEAGE_TRANSITION_CIRCUIT_WIRED_V1)
        assertEquals(
            1,
        ...
        for ((circuitId, hopCount) in listOf(
            "kagemusha-recursive-spend-lineage-v1" to -1,
            "kagemusha-recursive-spend-lineage-v1" to Int.MIN_VALUE,
            "kagemusha-recursive-spend-lineage-v1" to Int.MAX_VALUE,
        ...
        val supportedLineageCircuitIds = listOf(
            KagemushaRecursiveSpendProver.RECURSIVE_SPEND_LINEAGE_ONE_HOP_PROOF_CIRCUIT_ID_V1,
            KagemushaRecursiveSpendProver.RECURSIVE_SPEND_LINEAGE_APPEND_PROOF_CIRCUIT_ID_V1,
        )
        for (circuitId in supportedLineageCircuitIds) {
            for (hopCount in listOf(1, 2, 63, 64)) {
                assertFalse(KagemushaRecursiveSpendProver.canRedeemWitnessless(circuitId, hopCount))
                assertTrue(
                    KagemushaRecursiveSpendProver.requiresLineageWitnessForRedeem(circuitId, hopCount),
                    "$circuitId hop $hopCount must require a record-backed lineage witness",
                )
            }
        }
        for (hopCount in listOf(Int.MIN_VALUE, -1, 0, 1, 2, 63, 64, Int.MAX_VALUE)) {
            assertFalse(KagemushaRecursiveSpendProver.canAppendWitnesslessLineage(hopCount))
        }
        ...
        assertFalse(KagemushaRecursiveSpendProver.isSupportedAppendOutputCircuitId(null))
        assertFalse(KagemushaRecursiveSpendProver.isSupportedAppendOutputCircuitId(""))
        assertTrue(
        ...
        assertFalse(KagemushaRecursiveSpendProver.canProveAppendOutputCircuitId(null, 1))
        assertTrue(
            KagemushaRecursiveSpendProver.canProveAppendOutputCircuitId(
                KagemushaRecursiveSpendProver.RECURSIVE_AGGREGATION_PROOF_CIRCUIT_ID_V1,
    ...
    fun lineageKeyArtifactPackagesValidateReleaseProfiles() {
        ...
        val circuitIdBytes =
            KagemushaRecursiveSpendProver.RECURSIVE_SPEND_LINEAGE_ONE_HOP_PROOF_CIRCUIT_ID_V1
                .toByteArray(Charsets.UTF_8)
        val overlongVersionLengthArchive = kagemushaNoritoFrameFromSchemaHash(
            ...
            kagemushaOverlongCompactLength(2) +
                ...
                kagemushaNoritoField(kagemushaNoritoByteVec(ByteArray(64) { 0xad.toByte() })),
        ...
        val overlongCircuitStringArchive = kagemushaNoritoFrameFromSchemaHash(
            ...
            kagemushaNoritoField(byteArrayOf(1, 0)) +
                kagemushaNoritoField(kagemushaOverlongCompactLength(circuitIdBytes.size) + circuitIdBytes) +
                kagemushaNoritoField(verifierKeyCommitment(initVerifierKey)) +
                kagemushaNoritoField(kagemushaNoritoByteVec(ByteArray(64) { 0xae.toByte() })),
            TEST_NORITO_COMPACT_LEN_FLAG,
        )
        assertEquals(
            "lineage_proving_key_archive",
        ...
        val invalidUtf8CircuitArchive = kagemushaNoritoFrameFromSchemaHash(
            lineageProvingKeyArchiveSchemaHash,
            kagemushaNoritoField(byteArrayOf(1, 0)) +
                kagemushaNoritoField(kagemushaNoritoLength(1) + byteArrayOf(0xff.toByte())) +
                kagemushaNoritoField(verifierKeyCommitment(initVerifierKey)) +
                kagemushaNoritoField(
                    kagemushaNoritoByteVec(circuitIdBytes + ByteArray(64) { 0xaf.toByte() }),
                ),
            TEST_NORITO_COMPACT_LEN_FLAG,
        )
        assertEquals(
    ...
    fun sharedRecursiveSpendAbi6FixtureMatchesSdkSurface() {
        ...
        assertContains(archives, "\"norito_default\": false")
        assertFalse(archives.contains("\"norito_default\": true"))
        ...
        assertFalse(KagemushaRecursiveSpendProver.canAppendWitnesslessLineage(63))
    ...
    private fun kagemushaOverlongCompactLength(value: Int): ByteArray {
        require(value in 0 until 0x80) {
            "test helper only encodes small overlong lengths"
        }
