...
class OkHttpTransportExecutorTest {
    ...
    fun executeRunsOnSuppliedAsyncExecutor() {
        ...
        try {
            ServerSocket(0).use { server ->
                ...
                val executor = OkHttpTransportExecutor.create(
                    connectTimeout = null,
                    readTimeout = null,
    ...
    private fun assertRedirectIsNotFollowed(status: Int) {
        ServerSocket(0).use { server ->
            ...
            val response = OkHttpTransportExecutor.create().execute(request).get()
    ...
    fun boundedReaderRejectsActualOverflowWithUnderstatedContentLength() {
        val failure = assertFailsWith<IOException> {
            BoundedResponseBodyReader.readBoundedBody(
    ...
    fun boundedReaderRejectsHostileInputStreamReadCounts() {
        ...
        assertFailsWith<IOException> {
            BoundedResponseBodyReader.readBoundedBody(noProgress, 8, null)
        }
        ...
        assertFailsWith<IOException> {
            BoundedResponseBodyReader.readBoundedBody(overReported, 8, null)
        }
    }
    ...
    fun encodedContentLengthIsNotComparedToDecodedBufferedBytes() {
        ...
        val bufferedLength = BoundedResponseBodyReader.contentLengthForBufferedBody(headers, 1L)
        assertNull(bufferedLength)
        val exact = BoundedResponseBodyReader.readBoundedBody(
        ...
        assertEquals(
            1L,
            BoundedResponseBodyReader.contentLengthForBufferedBody(
    ...
    fun contentLengthMustBeCanonicalUnsignedDecimal() {
        ...
        assertEquals(
            Long.MAX_VALUE,
            BoundedResponseBodyReader.parseCanonicalContentLength(Long.MAX_VALUE.toString()),
        )
    ...
    fun bufferedResponseLimitMustFitInByteArray() {
        assertFailsWith<IllegalArgumentException> { OkHttpTransportExecutor.create(maximumResponseBytes = 0) }
        assertFailsWith<IllegalArgumentException> {
            OkHttpTransportExecutor.create(maximumResponseBytes = Int.MAX_VALUE.toLong() + 1L)
        }
        assertEquals(
            64L * 1024L * 1024L,
            OkHttpTransportExecutor.DEFAULT_MAXIMUM_RESPONSE_BYTES,
        )
        assertFailsWith<IllegalArgumentException> {
        ...
        ServerSocket(0).use { server ->
            ...
            try {
                val request = TransportRequest.builder()
                    ...
                    .build()
                OkHttpTransportExecutor.create(maximumResponseBytes = maximumResponseBytes).execute(request).get()
            } finally {
                serverThread.join(2000)
            }
        }
