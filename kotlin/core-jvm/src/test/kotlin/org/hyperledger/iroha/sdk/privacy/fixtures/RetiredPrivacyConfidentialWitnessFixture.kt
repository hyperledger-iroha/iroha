...
package org.hyperledger.iroha.sdk.privacy.fixtures
...
class RetiredPrivacyConfidentialNoteWitnessV1(
    amount: String,
    rho: ByteArray,
...
class RetiredPrivacyConfidentialTransferOutputWitnessV1(
    amount: String,
    rho: ByteArray,
    ownerTag: ByteArray,
) {
...
class RetiredPrivacyConfidentialUnshieldChangeWitnessV1(
    amount: String,
    rho: ByteArray,
) {
    private val rhoBytes = privacyFixed32(rho, "rho")
...
/** Historical witness used only to exercise rejection of the removed bridge archive. */
class RetiredPrivacyConfidentialWitnessV1(
    @JvmField val networkId: NetworkId,
    assetDefinitionId: String,
    spendKey: ByteArray,
    treeCommitments: List<ByteArray>,
    inputs: List<RetiredPrivacyConfidentialNoteWitnessV1>,
    transferOutputs: List<RetiredPrivacyConfidentialTransferOutputWitnessV1>,
    unshieldChange: List<RetiredPrivacyConfidentialUnshieldChangeWitnessV1>,
    publicAmount: String,
    rootHint: ByteArray,
) {
    ...
    init {
        ...
        require(treeCommitmentBytes.size <= RetiredPrivacyConfidentialWitnessCodecs.CONFIDENTIAL_TREE_CAPACITY_V2) {
            "treeCommitments must not exceed ${RetiredPrivacyConfidentialWitnessCodecs.CONFIDENTIAL_TREE_CAPACITY_V2}"
        }
        require(inputValues.size in 1..RetiredPrivacyConfidentialWitnessCodecs.CONFIDENTIAL_MAX_INPUTS_V2) {
            "inputs must contain one or two notes"
        }
        require(transferOutputValues.size <= RetiredPrivacyConfidentialWitnessCodecs.CONFIDENTIAL_MAX_TRANSFER_OUTPUTS_V2) {
            "transferOutputs must contain at most two notes"
        }
        require(unshieldChangeValues.size <= RetiredPrivacyConfidentialWitnessCodecs.CONFIDENTIAL_MAX_UNSHIELD_CHANGE_OUTPUTS_V3) {
    ...
    val inputs: List<RetiredPrivacyConfidentialNoteWitnessV1> get() = Collections.unmodifiableList(inputValues.toList())
    val transferOutputs: List<RetiredPrivacyConfidentialTransferOutputWitnessV1>
        get() = Collections.unmodifiableList(transferOutputValues.toList())
    val unshieldChange: List<RetiredPrivacyConfidentialUnshieldChangeWitnessV1>
...
/** Historical archive encoders owned solely by negative test fixtures. */
object RetiredPrivacyConfidentialWitnessCodecs {
    ...
    @JvmStatic
    fun encodeWitness(witness: RetiredPrivacyConfidentialWitnessV1): ByteArray =
        addNoritoHeaderPadding(
            NoritoCodec.encode(
                witness,
                SCHEMA_PRIVACY_CONFIDENTIAL_WITNESS_V1,
                RetiredPrivacyConfidentialWitnessAdapter,
                REQUEST_FLAGS,
            ),
    ...
    @JvmStatic
    fun encodeTransferWitness(witness: RetiredPrivacyConfidentialWitnessV1): ByteArray {
        validateTransferWitness(witness)
        return encodeWitness(witness)
    }

    @JvmStatic
    fun encodeUnshieldWitness(witness: RetiredPrivacyConfidentialWitnessV1): ByteArray {
        validateUnshieldWitness(witness)
        return encodeWitness(witness)
    }

    private fun validateTransferWitness(witness: RetiredPrivacyConfidentialWitnessV1) {
    ...
    private fun validateUnshieldWitness(witness: RetiredPrivacyConfidentialWitnessV1) {
...
private object RetiredPrivacyConfidentialWitnessAdapter : TypeAdapter<RetiredPrivacyConfidentialWitnessV1> {
    override fun encode(encoder: NoritoEncoder, value: RetiredPrivacyConfidentialWitnessV1) {
    ...
    override fun decode(decoder: NoritoDecoder): RetiredPrivacyConfidentialWitnessV1 =
        throw UnsupportedOperationException("privacy confidential witnesses are encode-only")
}

private object NoteWitnessAdapter : TypeAdapter<RetiredPrivacyConfidentialNoteWitnessV1> {
    override fun encode(encoder: NoritoEncoder, value: RetiredPrivacyConfidentialNoteWitnessV1) {
    ...
    override fun decode(decoder: NoritoDecoder): RetiredPrivacyConfidentialNoteWitnessV1 =
        throw UnsupportedOperationException("privacy confidential note witnesses are encode-only")
}

private object TransferOutputAdapter : TypeAdapter<RetiredPrivacyConfidentialTransferOutputWitnessV1> {
    override fun encode(encoder: NoritoEncoder, value: RetiredPrivacyConfidentialTransferOutputWitnessV1) {
        writeField(encoder) { writeU128(it, value.amount) }
        writeField(encoder) { writeBytesVec(it, value.rho) }
        writeField(encoder) { writeBytesVec(it, value.ownerTag) }
    }

    override fun decode(decoder: NoritoDecoder): RetiredPrivacyConfidentialTransferOutputWitnessV1 =
        throw UnsupportedOperationException("privacy confidential transfer outputs are encode-only")
}

private object UnshieldChangeAdapter : TypeAdapter<RetiredPrivacyConfidentialUnshieldChangeWitnessV1> {
    override fun encode(encoder: NoritoEncoder, value: RetiredPrivacyConfidentialUnshieldChangeWitnessV1) {
        writeField(encoder) { writeU128(it, value.amount) }
        writeField(encoder) { writeBytesVec(it, value.rho) }
    }

    override fun decode(decoder: NoritoDecoder): RetiredPrivacyConfidentialUnshieldChangeWitnessV1 =
