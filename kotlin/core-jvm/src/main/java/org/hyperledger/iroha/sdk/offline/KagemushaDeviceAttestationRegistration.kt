 ...
 *
 * This mirrors `KagemushaDeviceAttestationRegistration` exactly and exposes only the canonical
 * Kagemusha V1 model. Native attestation acquisition uses bridge ABI 23;
 * the on-chain registration format marker remains version 1.
 */
class KagemushaDeviceAttestationRegistration(
    val version: Int,
    val platform: String,
    val keyId: String,
    val deviceId: String,
    ...
    val publicKey: KagemushaDevicePublicKeyV1,
    ...
    init {
        ...
        val expectedChallengeHash = KagemushaDeviceAttestationCodec.canonicalChallengeHash(this)
        challengeHash?.let {
    ...
    fun noritoEncoded(): ByteArray = KagemushaDeviceAttestationCodec.encodeRegistration(this)

    /** Deterministic challenge hash that the platform report must bind. */
    fun canonicalChallengeHash(): ByteArray =
        KagemushaDeviceAttestationCodec.canonicalChallengeHash(this)
    ...
    private fun requireCore() {
        ...
        KagemushaDeviceAttestationCodec.validateAccountId(accountId)
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other !is KagemushaDeviceAttestationRegistration) return false
    ...
    companion object {
        ...
        const val DEVICE_ATTESTATION_CHALLENGE_DOMAIN: String =
            "iroha:kagemusha:v1:device-attestation-challenge"
        const val DEVICE_ATTESTATION_EVIDENCE_PREFIX: String =
            "kagemusha-device-attestation-evidence-v1"
        ...
        fun decodeCanonical(
            ...
            KagemushaDeviceAttestationCodec.decodeRegistrationCanonical(
                archive,
                chainDiscriminant,
            )
        ...
        fun androidPreKeyGenerationChallengeHash(
            ...
            androidSigningCertificateSha256: ByteArray,
            publicKey: KagemushaDevicePublicKeyV1,
            recentBlockHeight: Long,
            recentBlockHash: ByteArray,
            expiresAtMs: Long,
        ): ByteArray = KagemushaDeviceAttestationCodec.androidPreKeyGenerationChallengeHash(
            version,
            deviceId,
            accountId,
            assetDefinitionId,
