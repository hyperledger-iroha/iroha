...
class OfflineNoteV2IssuerDeviceBinding(
    val deviceId: String,
...
interface OfflineNoteV2IssuerDeviceBindingProvider {
    fun currentDeviceBinding(
        chainId: String,
        accountId: String,
        assetDefinitionId: String,
    ): OfflineNoteV2IssuerDeviceBinding
}

/** Torii-backed issuer client for Offline Note V2 wallet loads. */
class ToriiOfflineNoteV2IssuerClient @JvmOverloads constructor(
    private val canonicalAuth: ToriiCanonicalRequestAuth,
    private val deviceBindingProvider: OfflineNoteV2IssuerDeviceBindingProvider,
    ...
    private val nonceGenerator: OfflineNoteV2IdGenerator = UuidOfflineNoteV2IdGenerator(),
    ...
    override fun issueNote(request: OfflineNoteV2IssueRequest): CompletableFuture<OfflineNoteV2IssueResponse> {
        val pending = synchronized(this) { pendingLoads[request.loadContext.operationId] }
            ?: return failedFuture(OfflineToriiException("Missing Offline Note V2 load context for operation ${request.loadContext.operationId}."))
    ...
    private fun <T> executePost(
        ...
        executor.execute(request).whenComplete { response, throwable ->
            ...
            try {
                ...
                val error = OfflineToriiException("Failed to parse Offline Note V2 issuer response for ${request.uri.path}.", ex, response.statusCode, rejectCode, bodyPreview)
                notifyFailure(request, error)
                future.completeExceptionally(error)
            }
        }
    ...
    private data class PendingLoad(
        ...
        fun context(): OfflineNoteV2LoadContext =
...
private fun parseKeyCertificate(value: Map<String, Any?>): OfflineNoteV2.KeyCertificateV2 =
