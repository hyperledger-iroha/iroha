...
/**
 * Canonical NIST P-256 boundary for the sole Kagemusha device-authority profile.
 *
...
object KagemushaP256Codec {
    ...
    fun requireUncompressedPublicKey(sec1Bytes: ByteArray): ByteArray {
        ...
        val rhs = x.modPow(THREE, FIELD_PRIME)
            ...
            .add(CURVE_B)
            .mod(FIELD_PRIME)
        require(lhs == rhs) { "Kagemusha device public key is not a P-256 point" }
        return value
    }
    ...
    fun requireRawLowSSignature(rawBytes: ByteArray): ByteArray {
        ...
        requireScalar(r, "r")
        requireScalar(s, "s")
        require(s <= HALF_ORDER) { "Kagemusha device signature must use low-S form" }
        return value
    }
    ...
    fun rawLowSFromStrictDer(derBytes: ByteArray): ByteArray {
        ...
        require(der.size in 8..72 && unsigned(der[0]) == 0x30) {
            "Kagemusha ECDSA signature is not strict DER"
        }
        ...
        require(cursor == der.size) { "Kagemusha ECDSA signature has trailing DER bytes" }
...
class KagemushaDevicePublicKeyV2(sec1Bytes: ByteArray) {
    ...
    override fun equals(other: Any?): Boolean =
        other is KagemushaDevicePublicKeyV2 && value.contentEquals(other.value)
...
class KagemushaDeviceSignatureV2(rawBytes: ByteArray) {
    ...
    override fun equals(other: Any?): Boolean =
        other is KagemushaDeviceSignatureV2 && value.contentEquals(other.value)

    override fun hashCode(): Int = value.contentHashCode()

    companion object {
        @JvmStatic
        fun fromStrictDer(derBytes: ByteArray): KagemushaDeviceSignatureV2 =
            KagemushaDeviceSignatureV2(KagemushaP256Codec.rawLowSFromStrictDer(derBytes))
    }
}
