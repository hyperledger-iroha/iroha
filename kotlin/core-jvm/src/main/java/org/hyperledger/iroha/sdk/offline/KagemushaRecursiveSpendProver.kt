...
class KagemushaRecursiveSpendProver private constructor() {
    companion object {
        ...
        private data class ParsedBranchProjection(
            val projection: BranchProjection,
            val nextIndex: Int,
        )

        private fun exactBranchProjection(
            fields: Array<ByteArray>,
            offset: Int,
            label: String,
        ): ParsedBranchProjection {
            check(offset >= 0 && fields.size >= offset + 10) {
                "native Kagemusha $label projection omitted exact branch fields"
            }
            val claimCount = integer(fields[offset + 9], "$label branchClaimCount")
            check(
                claimCount in 1..MAXIMUM_INPUTS_PER_TRANSITION &&
                    fields.size >= offset + 10 + claimCount,
            ) { "native Kagemusha $label projection returned invalid branch claims" }
            return ParsedBranchProjection(
                BranchProjection(
                    Bundle(fields[offset]),
                    NoteMembershipWitness(fields[offset + 1]),
                    fields[offset + 2],
                    fields[offset + 3],
                    ...
                    fields[offset + 8],
                    (0 until claimCount).map { BranchClaim(fields[offset + 10 + it]) },
                ),
        ...
        fun buildAppendRequest(
            ...
            require(inputs.size in 1..MAXIMUM_LOCAL_APPEND_BUILDER_INPUTS) {
                "append requires exactly one or two inputs"
            }
            val inputList = inputs.toList()
            for (left in inputList.indices) {
                for (right in left + 1 until inputList.size) {
                    require(
                        !inputList[left].bundle.noritoEncoded().contentEquals(
                            inputList[right].bundle.noritoEncoded(),
                        ),
                    ) { "append inputs must have distinct bundle identities" }
                }
            }
            requireArtifactBridge()
            return AppendRequest(
                nativeBuildAppendRequestV2(
                    inputList.map { it.bundle.noritoEncoded() }.toTypedArray(),
                    inputList.map { it.opening.noritoEncoded() }.toTypedArray(),
                    inputList.map { it.membershipWitness.noritoEncoded() }.toTypedArray(),
        ...
        fun projectSplitResult(result: SplitResult): SplitProjection {
            ...
            check(fields.size >= 15) { "native Kagemusha split result projection returned invalid fields" }
            val recipient = exactBranchProjection(fields, 4, "recipient")
            check(recipient.nextIndex < fields.size) {
                "native Kagemusha split result projection omitted change presence"
            }
            val changePresent = bool(fields[recipient.nextIndex], "changePresent")
            val change = if (changePresent) {
                exactBranchProjection(fields, recipient.nextIndex + 1, "change")
            } else {
                null
            }
            val finalIndex = change?.nextIndex ?: recipient.nextIndex + 1
            check(finalIndex == fields.size) {
                "native Kagemusha split result projection returned trailing fields"
            }
            return SplitProjection(
                PeerPayment(fields[0]), recipient.projection, change?.projection,
        ...
        fun projectPeerPayment(payment: PeerPayment): PeerPaymentProjection {
            ...
            check(fields.size >= 12) { "native Kagemusha peer payment projection returned invalid fields" }
            val branch = exactBranchProjection(fields, 2, "peer payment")
            check(branch.nextIndex == fields.size) {
                "native Kagemusha peer payment projection returned trailing fields"
            }
            return PeerPaymentProjection(branch.projection, fields[0], fields[1])
        }
        ...
        fun projectVerifyResult(result: VerifyResult): VerifyProjection {
            ...
            check(fields.size >= 24) { "native Kagemusha verify result projection returned invalid fields" }
            val claimCount = integer(fields[22], "branchClaimCount")
            check(claimCount in 1..MAXIMUM_INPUTS_PER_TRANSITION && fields.size == 23 + claimCount) {
            ...
            return VerifyProjection(
                ...
                canonicalText(fields[11], "assetDefinitionId"), ArtifactBinding(fields[12]),
                fields[13], fields[14],
                canonicalText(fields[15], "verifierBackend"),
                canonicalText(fields[16], "verifierName"),
                canonicalText(fields[17], "verifierCircuitId"),
                fields[18].takeIf { it.isNotEmpty() }?.let { longInteger(it, "verifierActivationHeight") },
                fields[19].takeIf { it.isNotEmpty() }?.let { longInteger(it, "verifierWithdrawalHeight") },
                longInteger(fields[20], "verifiedAtBlockHeight"),
                longInteger(fields[21], "verifiedAtMilliseconds"),
                (0 until claimCount).map { BranchClaim(fields[23 + it]) },
            )
        }
        ...
        fun projectRedeemBuildResult(result: RedeemBuildResult): RedeemBuildProjection {
            ...
            check(fields.size >= 4) { "native Kagemusha redeem result projection returned invalid fields" }
            val changePresent = bool(fields[3], "changePresent")
            val change = if (changePresent) exactBranchProjection(fields, 4, "redemption change") else null
            val finalIndex = change?.nextIndex ?: 4
            check(finalIndex == fields.size) {
                "native Kagemusha redeem result projection returned trailing fields"
            }
            return RedeemBuildProjection(fields[0], fields[1], change?.projection, fields[2])
        }
    ...
    open class BranchProjection internal constructor(
        ...
        private val branchClaimsValue: List<BranchClaim> = branchClaims.toList()

        init {
            check(hopCount in 0..MAXIMUM_PEER_HOPS) { "native Kagemusha hop count is invalid" }
            check(proofStepCount > 0) { "native Kagemusha proof step count is invalid" }
            check(branchClaimsValue.size in 1..MAXIMUM_INPUTS_PER_TRANSITION) {
                "native Kagemusha branch claim vector is invalid"
            }
        }
        ...
        fun branchClaims(): List<BranchClaim> = branchClaimsValue.toList()

        fun conflictsWith(other: BranchProjection): Boolean = branchClaimsValue.any { left ->
            other.branchClaimsValue.any { right -> left.conflictsWith(right) }
        }
    }
    ...
    class VerifyProjection internal constructor(
        ...
        init {
            check(hopCount in 0..MAXIMUM_PEER_HOPS && proofStepCount > 0) {
