...
public final class OfflineNoteV2Halo2Prover {
  ...
  public static boolean verifyAudit(final OfflineNoteV2.AuditBundleV2 audit) {
    audit.validateProofBinding();
    final OfflineNoteV2.InstanceValues values = OfflineNoteV2.InstanceBuilder.auditInstanceValues(audit);
    if (!Arrays.equals(audit.getRecursiveProof().publicInputsHash(), publicInputsHash(values.publicValues()))) {
      return false;
    }
    return verifyZk1Payload(
        proofPayloadFromOpenVerifyEnvelope(audit.getRecursiveProof().getProof().bytes()),
        values.publicValues());
  }

  public static boolean verifyRedeem(final OfflineNoteV2.RedeemV2 redemption) {
    redemption.validateProofBinding();
    final OfflineNoteV2.InstanceValues values = OfflineNoteV2.InstanceBuilder.redeemInstanceValues(redemption);
    if (!Arrays.equals(redemption.getRecursiveProof().publicInputsHash(), publicInputsHash(values.publicValues()))) {
      return false;
    }
    return verifyZk1Payload(
        proofPayloadFromOpenVerifyEnvelope(redemption.getRecursiveProof().getProof().bytes()),
        values.publicValues());
  }
  ...
  private static byte[] proofPayloadFromOpenVerifyEnvelope(final byte[] envelope) {
    return NoritoCodec.decode(
        ...
        new TypeAdapter<byte[]>() {
          ...
          @Override
          public byte[] decode(final org.hyperledger.iroha.sdk.norito.NoritoDecoder decoder) {
  ...
  private interface FieldReader<T> {
    T read(org.hyperledger.iroha.sdk.norito.NoritoDecoder decoder);
  }

  private static <T> T readField(
      final org.hyperledger.iroha.sdk.norito.NoritoDecoder parent,
    ...
    final org.hyperledger.iroha.sdk.norito.NoritoDecoder child =
        new org.hyperledger.iroha.sdk.norito.NoritoDecoder(
  ...
  private static String readString(final org.hyperledger.iroha.sdk.norito.NoritoDecoder decoder) {
  ...
  private static byte[] readBytesVec(final org.hyperledger.iroha.sdk.norito.NoritoDecoder decoder) {
