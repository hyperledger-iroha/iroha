...
internal object KagemushaDeviceAttestationCodec {
    ...
    const val INSTRUCTION_WIRE_ID: String = "iroha.kagemusha.device_attestation.register"
    ...
    fun encodeRegistration(registration: KagemushaDeviceAttestationRegistration): ByteArray =
        NoritoCodec.encode(
            registration,
            REGISTRATION_SCHEMA,
            registrationAdapter,
    ...
    fun encodeInstructionPayload(registration: KagemushaDeviceAttestationRegistration): ByteArray =
        NoritoCodec.encode(
            registration,
            INSTRUCTION_SCHEMA,
            instructionAdapter,
    ...
    fun instruction(registration: KagemushaDeviceAttestationRegistration): InstructionBox =
        InstructionBox.fromWirePayload(INSTRUCTION_WIRE_ID, encodeInstructionPayload(registration))

    fun canonicalChallengeHash(value: KagemushaDeviceAttestationRegistration): ByteArray {
        if (value.platform == KagemushaDeviceAttestationRegistration.ANDROID_KEYMINT_PLATFORM) {
    ...
    fun androidPreKeyGenerationChallengeHash(
        ...
        require(version == KagemushaDeviceAttestationRegistration.REGISTRATION_VERSION) {
    ...
    private class RegistrationAdapter : TypeAdapter<KagemushaDeviceAttestationRegistration> {
        override fun encode(encoder: NoritoEncoder, value: KagemushaDeviceAttestationRegistration) {
        ...
        override fun decode(decoder: NoritoDecoder): KagemushaDeviceAttestationRegistration =
            KagemushaDeviceAttestationRegistration(
                ...
                publicKey = KagemushaDevicePublicKeyV1(readField(decoder, ::readP256PublicKey)),
    ...
    private class InstructionAdapter : TypeAdapter<KagemushaDeviceAttestationRegistration> {
        override fun encode(encoder: NoritoEncoder, value: KagemushaDeviceAttestationRegistration) {
    ...
    private class ChallengeAdapter : TypeAdapter<Challenge> {
        override fun encode(encoder: NoritoEncoder, value: Challenge) {
            ...
            field(encoder) { string(it, KagemushaDeviceAttestationRegistration.DEVICE_ATTESTATION_CHALLENGE_DOMAIN) }
    ...
    private class AndroidChallengeAdapter : TypeAdapter<AndroidChallenge> {
        override fun encode(encoder: NoritoEncoder, value: AndroidChallenge) {
            field(encoder) { string(it, KagemushaDeviceAttestationRegistration.DEVICE_ATTESTATION_CHALLENGE_DOMAIN) }
            field(encoder) { it.writeUInt(KagemushaDeviceAttestationRegistration.REGISTRATION_VERSION.toLong(), 16) }
            field(encoder) { string(it, KagemushaDeviceAttestationRegistration.ANDROID_KEYMINT_PLATFORM) }
            field(encoder) { string(it, value.deviceId) }
            field(encoder) { it.writeBytes(TransferWirePayloadEncoder.encodeAccountIdPayload(value.accountId)) }
            field(encoder) { optionAssetDefinitionId(it, value.assetDefinitionId) }
            ...
            field(encoder) { string(it, KagemushaDeviceAttestationRegistration.ANDROID_KEYMINT_ASSERTION_SCHEME) }
            field(encoder) { string(it, KagemushaDeviceAttestationRegistration.ANDROID_KEYMINT_ASSERTION_KEY_ALGORITHM) }
    ...
    private class Challenge(val registration: KagemushaDeviceAttestationRegistration)
    ...
    private fun readHash(decoder: NoritoDecoder, field: String): ByteArray =
        decoder.readBytes(32).also { KagemushaDeviceAttestationRegistration.requireHash(it, field) }
