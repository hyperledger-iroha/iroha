...
/** Pure JVM Halo2/IPA prover for Offline Note. */
public final class OfflineNoteHalo2Prover {
  public static final String CIRCUIT_ID = OfflineNote.RECURSIVE_VERIFIER_NAME;
  ...
  private OfflineNoteHalo2Prover() {}

  public static void prewarm() {
    CONTEXT.requireReady();
  }

  public static OfflineNote.RecursiveProof proveRedeem(
      final OfflineNote.Redeem redemption) {
    return prove(OfflineNote.InstanceBuilder.redeemInstanceValues(redemption));
  }

  public static OfflineNote.RecursiveProof proveAudit(
      final OfflineNote.AuditBundle audit) {
    return prove(OfflineNote.InstanceBuilder.auditInstanceValues(audit));
  }

  public static OfflineNote.RecursiveProof prove(final OfflineNote.InstanceValues values) {
    ...
    if (envelope.length > MAX_ENVELOPE_BYTES) {
      throw new IllegalArgumentException("Offline proof envelope exceeds QR budget: " + envelope.length);
    }
  ...
  public static byte[] proveZk1Payload(final OfflineNote.InstanceValues values) {
    ...
    if (publicScalars.length != PUBLIC_VALUE_COUNT || inputScalars.length != 4 || outputScalars.length != 2) {
      throw new IllegalArgumentException("invalid Offline instance values");
    }
    ...
    if (!hEval.equals(expected)) {
      throw new IllegalArgumentException("invalid Offline quotient evaluation");
    }
  ...
  public static boolean verifyZk1Payload(final byte[] payload, final long[] publicValues) {
    if (publicValues.length != PUBLIC_VALUE_COUNT) {
      throw new IllegalArgumentException("invalid Offline public value count");
    }
  ...
  public static boolean verifyAudit(final OfflineNote.AuditBundle audit) {
    ...
    final OfflineNote.InstanceValues values = OfflineNote.InstanceBuilder.auditInstanceValues(audit);
  ...
  public static boolean verifyRedeem(final OfflineNote.Redeem redemption) {
    redemption.validateProofBinding();
    final OfflineNote.InstanceValues values = OfflineNote.InstanceBuilder.redeemInstanceValues(redemption);
  ...
  private static F[] evaluateQuotientCoefficients(
    ...
    for (int i = 0; i < constraints.size(); i++) {
      if (!evaluatePolynomial(constraints.get(i), F.one(FP)).isZero()) {
        throw new IllegalArgumentException("Offline gate constraint does not vanish: " + i);
      }
    }
    F[] numerator = new F[0];
    ...
    for (int row = 0; row < domain.n; row++) {
      if (!evaluatePolynomial(numerator, root).isZero()) {
        throw new IllegalArgumentException("Offline quotient numerator does not vanish at row " + row);
      }
  ...
  private static byte[] zk1ProofPayload(final byte[] proofTranscript, final long[] publicValues) {
    ...
    for (final long value : publicValues) {
      write(instances, OfflineNote.instanceScalarBytes(value));
    }
