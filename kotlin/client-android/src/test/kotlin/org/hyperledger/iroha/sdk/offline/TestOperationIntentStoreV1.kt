...
class TestOperationIntentStoreV1 : KagemushaOperationIntentStoreV1 {
    ...
    var failSave = false
    var failAcceptedOperation: Int? = null
    ...
    override fun save(intent: KagemushaOperationIntentV1) {
        ...
        if (failReconciliation && intent.reconciliationEvidence() != null) {
            failReconciliation = false
            error("interrupted snapshot evidence sync")
        }
        if (failAcknowledgement && intent.acknowledged) {
            failAcknowledgement = false
            error("interrupted acknowledgement sync")
        }
        if (intent.operation == failAcceptedOperation && intent.canonicalReply() != null) {
            failAcceptedOperation = null
            error("interrupted sync after native Core acceptance")
        }
