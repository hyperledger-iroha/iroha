...
async fn permissioned_idle_chain_advances_only_for_external_or_internal_work() -> Result<()> {
    ...
    const BLOCK_CADENCE: Duration = Duration::from_millis(500);
    ...
    let builder = NetworkBuilder::new()
        ...
        .with_auto_populated_trusted_peers()
        .with_real_genesis_keypair()
        .with_block_cadence(BLOCK_CADENCE)
        .with_permissioned_consensus();
    ...
    let result: Result<()> = async {
        ...
        let cadence_ms = u64::try_from(BLOCK_CADENCE.as_millis())
        ...
        let idle_observation = BLOCK_CADENCE
            .saturating_mul(10)
        ...
        eprintln!(
            "EX-297 idle-chain evidence: cadence={BLOCK_CADENCE:?}, retransmit_interval_ms={retransmit_interval_ms}, retransmit_window={retransmit_observation:?}, commit_quorum_window={commit_quorum_observation:?}, baseline_settle_window={baseline_settle_window:?}, idle_window={idle_observation:?}, baseline_height={baseline_height}, external_height={external_height}, trigger_registration_height={registration_height}, internal_trigger_height={internal_height}, final_tip_hash={expected_internal_hash}"
        );
        Ok(())
    }
    .await;
