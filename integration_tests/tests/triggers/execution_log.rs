...
async fn client_verifies_transaction_entrypoint_and_result_proofs() -> Result<()> {
    ...
    let tx_hash: HashOf<SignedTransaction> = test_client.submit(user_instruction)?;
    ...
    let committed_tx: CommittedTransaction = test_client
        ...
        .first()
        .cloned()
        .expect("expected at least one matching transaction");
    ...
    assert!(!bad_proof.verify(&root, 9));
