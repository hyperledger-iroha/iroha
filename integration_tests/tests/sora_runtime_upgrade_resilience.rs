...
fn compute_runtime_upgrade_proposal_id(manifest: &RuntimeUpgradeManifest) -> [u8; 32] {
    use iroha_crypto::blake2::{Blake2b512, Digest as _};
...
async fn wait_for_runtime_upgrade_status(
    client: &Client,
...
async fn sora_runtime_upgrade_resilience_peer_restart_during_multibody_sortition() -> Result<()> {
    let Some(mut fixture) = setup_runtime_governance_fixture(stringify!(
    ...
    let outcome =
        enact_runtime_upgrade_round(&mut fixture, "restart-round", Some(restart_idx)).await?;

    for (peer_idx, peer) in fixture.network.peers().iter().enumerate() {
        let mut peer_client = peer.client();
        tune_client_timeouts(&mut peer_client);
        let runtime_status = wait_for_runtime_upgrade_status(
            &peer_client,
...
async fn sora_runtime_upgrade_resilience_chained_rounds_soak() -> Result<()> {
    ...
    let first = enact_runtime_upgrade_round(&mut fixture, "chain-round-1", None).await?;
    let second = enact_runtime_upgrade_round(&mut fixture, "chain-round-2", None).await?;
...
type AccountId = iroha::data_model::account::AccountId;

struct RuntimeGovernanceFixture {
    network: sandbox::SerializedNetwork,
    http: reqwest::Client,
    ready_peer_idx: usize,
    alice: Client,
    citizens: Vec<(AccountId, KeyPair)>,
    council_members: Vec<AccountId>,
    asset_def_id: AssetDefinitionId,
    alice_asset_id: AssetId,
    next_runtime_window_start: u64,
}

struct RuntimeRoundOutcome {
    runtime_upgrade_id_hex: String,
    activated_height: u64,
}

fn governance_builder_for_runtime_resilience() -> NetworkBuilder {
...
async fn setup_runtime_governance_fixture(
...
async fn enact_runtime_upgrade_round(
    ...
    let runtime_start_height = baseline_start_height.max(fixture.next_runtime_window_start);
    ...
    fixture.next_runtime_window_start = runtime_end_height.saturating_add(1);
