...
#[test]
fn transfer_nft() {
    let Some((network, _rt)) = start_network(stringify!(transfer_nft)) else {
        return;
    };
...
fn client_register_nft_second_time_should_fail() -> eyre::Result<()> {
    let Some((network, _rt)) = sandbox::start_network_blocking_or_skip(
        NetworkBuilder::new(),
        stringify!(client_register_nft_second_time_should_fail),
    )?
    else {
        return Ok(());
    };
...
fn unregister_nft_should_remove_nft_from_account() -> eyre::Result<()> {
    let Some((network, _rt)) = sandbox::start_network_blocking_or_skip(
        NetworkBuilder::new(),
        stringify!(unregister_nft_should_remove_nft_from_account),
    )?
    else {
        return Ok(());
    };
...
fn nft_owner_cant_modify_nft() -> Result<()> {
    let Some((network, _rt)) = sandbox::start_network_blocking_or_skip(
        NetworkBuilder::new(),
        stringify!(nft_owner_cant_modify_nft),
    )?
    else {
        return Ok(());
    };
...
fn nft_owner_can_transfer_nft() -> Result<()> {
    let Some((network, _rt)) = sandbox::start_network_blocking_or_skip(
        NetworkBuilder::new(),
        stringify!(nft_owner_can_transfer_nft),
    )?
    else {
        return Ok(());
    };
