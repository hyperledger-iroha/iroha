...
sys.path[:0] = [str(ROOT / 'scripts'), str(ROOT / 'scripts/nexus'),
               str(ROOT.parents[1] / 'python-venv/lib/python3.12/site-packages')]
...
class Fixture:
    def __init__(self, root):
        ...
        self.paths = m.ReleaseRuntimePaths(self.source, self.pathlist, digest(self.pathlist.read_bytes()),
            ...
            python_manifest_path, digest(python_raw), 'scripts/scaling_runtime_admission_test.py',
    ...
    def admit(self, **changes):
        kwargs = dict(paths=self.paths, identity=self.identity, runtime=self.runtime,
                      workers=self.workers, plan=self.plan)
...
class PrimitiveTests(unittest.TestCase):
    ...
    def test_no_public_or_subclass_constructor(self):
        ...
        with self.assertRaises(m.RuntimeAdmissionError): Foreign.admit(None, None, None, None, None)
...
if __name__ == '__main__':
    source_before = {str(path.relative_to(ROOT)): digest(path.read_bytes()) for path in sorted((ROOT/'scripts').rglob('*.py'))}
    audit_attempts = []
    ...
    source_after = {str(path.relative_to(ROOT)): digest(path.read_bytes()) for path in sorted((ROOT/'scripts').rglob('*.py'))}
    record = dict(tests_run=result.testsRun,failures=len(result.failures),errors=len(result.errors),
        skipped=len(result.skipped),elapsed_ns=time.monotonic_ns()-started,
        forbidden_process_attempts=audit_attempts,source_before=source_before,source_after=source_after,
        sources_unchanged=source_before==source_after,
        isolated=bool(sys.flags.isolated),no_site=bool(sys.flags.no_site),no_bytecode=bool(sys.flags.dont_write_bytecode),
        composition_seams=['host kernel observations','archived framework verifier positive path',
                           'current interpreter prefix and executable'],
        native_execution=False,performance_qualification=False)
    (ROOT.parent/'test-result.json').write_text(json.dumps(record,sort_keys=True,indent=2)+'\n')
    raise SystemExit(0 if result.wasSuccessful() and source_before==source_after and not audit_attempts else 1)
