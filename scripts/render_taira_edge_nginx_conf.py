...
PUBLIC_TORII_CORS_HEADERS = (
    "accept, authorization, content-type, x-iroha-account, x-iroha-signature, "
...
def render_edge_nginx_conf(
    ...
    *,
    ...
    public_torii_cors_origins = list(
        dict.fromkeys([f"https://{explorer_host}", *PUBLIC_TORII_CORS_ORIGINS])
    )
    lines: list[str] = [
        ...
        "",
        "# Certbot's TLS defaults contain only inheritable ssl_* directives. Keep the",
        "# include at HTTP scope so static virtual hosts cannot import routing directives.",
