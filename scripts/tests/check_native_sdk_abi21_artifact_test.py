...
def test_repository_wires_exact_abi21_release_contract() -> None:
    ...
    assert "REQUIRED_NATIVE_SIGNER_CONTRACT_REVISION: Int = 3" in kotlin_signer
    assert "REQUIRED_NATIVE_SIGNER_CONTRACT_REVISION = 3" in java_signer
    roadmap = read("roadmap.md")
    assert "`NativeSignerBridge` JNI contract revision 3" in roadmap
    assert "`NativeSignerBridge` JNI contract revision 1" not in roadmap
    assert "`NativeSignerBridge` JNI contract revision 2" not in roadmap
