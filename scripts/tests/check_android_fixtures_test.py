...
def test_errors_propagate_into_summary(tmp_path: Path) -> None:
    ...
    chain = "00000003"
    authority = "soraゴヂアニラショリャヒャャサピテヶベチュヲボヹヂギタクアニョロホドチャヘヱヤジヶハシャウンベニョャルフハケネキカ"
...
def test_creation_time_mismatch_triggers_error(tmp_path: Path) -> None:
    ...
    chain = "00000004"
    authority = "soraゴヂアニダベェユヌサヨニャノヲョネイッリニャネガヨペバヒョブルノホイキャヸムケチャピファノマオニツミチオウ"
    payloads_path = _write_payloads(
        tmp_path / "transaction_payloads.json",
        [
            {
        ...
        [
            {
                ...
                "chain": "00000004",
                "authority": "soraゴヂアヌゼマャグンロリカリヱザキュアチュゥトヺュサビセジゲキョスクノョボリピドヒュヷペキキュザホフツコオコ",
                "time_to_live_ms": None,
                "nonce": None,
            }
        ],
    ...
    manifest_path = _write_manifest(
        ...
        [
            _fixture_entry(
                ...
                creation_time_ms=1_735_000_000_777,
                chain="00000008",
                authority="soraゴヂアニヤチョゴヂヴテヒャルヺリヒョマピスヒヮキュゲッチョクヌツヌラハチャヌゴキョククエェボォエボツテヒトウカ",
                time_to_live_ms=None,
                nonce=None,
            )
        ],
        ...
        [
            {
                ...
                "creation_time_ms": 1_735_000_000_999,
                "chain": "00000010",
                "authority": "soraゴヂアニラキャショベヤパピセゥリグテゲレエムベカチテェムシァョニャアカラザハダゼヰペシュベドイケツニホセ",
                "time_to_live_ms": None,
                "nonce": 9,
            }
        ],
