...
class InstructionBoxMacroSourceTest(unittest.TestCase):
    """Keep direct conversions unique and require the sole Offline Cash V1 ISIs."""
    ...
    def test_offline_cash_v1_instructions_are_directly_boxable(self) -> None:
        source = SOURCE.read_text(encoding="utf-8")
        offline_calls = {
            call for call in DIRECT_CALL.findall(source)
            if "offline_cash" in call
        }
        self.assertEqual(
            offline_calls,
            {
                "crate::isi::offline_cash_v1::TopUpOfflineCashV1",
                "crate::isi::offline_cash_v1::RedeemOfflineCashV1",
            },
        )
