...
def test_dry_run_prints_complete_pdp_rollout_plan(tmp_path: Path, capsys) -> None:
    ...
    assert plan["evidence_contract"]["provider_transport"]["schema"] == (
        "sorafs.pdp.provider_transport_canary.v1"
    )
    assert (
        "proof_stream_pdp_enabled"
        in plan["evidence_contract"]["provider_transport"]["required_payload_fields"]
    )
    assert (
    ...
    assert (
        ...
        in plan["evidence_contract"]["governance_repair"]["required_payload_fields"]
    )
    assert (
...
def test_plan_json_shape_is_validated(tmp_path: Path) -> None:
    ...
    assert MODULE.validate_plan_json(["step"], plan, args) == [
        "PDP rollout runner plan must be an object"
    ]

    rendered["schema"] = "sorafs.pdp.rollout_evidence_collection_plan.v0"
    ...
    assert "PDP rollout runner plan schema must match the contract" in diagnostics
    assert "PDP rollout runner plan required_kinds must match args" in diagnostics
    assert "PDP rollout runner plan thresholds must match args" in diagnostics
    assert "PDP rollout runner plan external_evidence must match args" in diagnostics
    assert (
        "PDP rollout runner plan evidence_contract must match checker fields"
        in diagnostics
    )
    assert "runner plan steps must match command plan" in diagnostics
    assert "proof-generation.json" not in diagnostics
...
def test_plan_json_nested_shapes_are_validated(tmp_path: Path) -> None:
    ...
    rendered["external_evidence"] = {
        ...
        "proof_generation": "proof-generation.json",
        "bad\nkind": ["provider-transport.json"],
        "observability": ["bad\npath"],
    }
    rendered["evidence_contract"] = {
        "provider_transport": {
        ...
        "unknown_kind": {
            "schema": "sorafs.pdp.unknown.v1",
            "required_payload_fields": [],
        },
        "proof_generation": "contract-shaped-entry",
        "bad\nkind": {
            "schema": MODULE.KIND_BY_NAME["provider_transport"].schema,
    ...
    assert "PDP rollout runner plan fields must be canonical strings" in diagnostics
    assert "PDP rollout runner plan schema must be canonical" in diagnostics
    assert "PDP rollout runner plan verifier schema must be canonical" in diagnostics
    assert (
        "PDP rollout runner plan required_kinds must contain canonical strings"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan required_kinds must not contain duplicate kinds"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan required_kinds must use known kind names"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan thresholds keys must be canonical strings"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan thresholds must contain only configured threshold fields"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan thresholds.max_evidence_age_secs must be a non-negative integer"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan thresholds.max_route_latency_ms must be a positive integer"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan thresholds.max_proof_latency_ms must be a positive integer"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan thresholds.min_providers must be a positive integer"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan thresholds.min_challenges must be a positive integer"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan thresholds.min_proofs must be a positive integer"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan thresholds.now_unix must be a positive integer"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan external_evidence keys must be canonical kind names"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan external_evidence keys must use known kind names"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan external_evidence must map each kind to non-empty path lists"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan external_evidence paths must be canonical strings"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan evidence_contract keys must be canonical kind names"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan evidence_contract keys must use known kind names"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan evidence_contract must map each kind to a contract object"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan evidence_contract fields must be canonical strings"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan evidence_contract fields must be schema and required_payload_fields"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan evidence_contract schemas must match evidence kind"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan evidence_contract required_payload_fields must be non-empty lists"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan evidence_contract required_payload_fields must contain canonical strings"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan evidence_contract required_payload_fields must not contain duplicate fields"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan evidence_contract required_payload_fields must match checker fields"
        in diagnostics
    )
...
def test_plan_json_rejects_unrequired_external_evidence_and_contracts(
    ...
    plan = MODULE.build_command_plan(args)
    rendered = MODULE.plan_json(plan, args)
    rendered["external_evidence"]["proof_generation"] = [
        str(tmp_path / "proof-generation.json")
    ]
    rendered["evidence_contract"]["proof_generation"] = {
        "schema": MODULE.KIND_BY_NAME["proof_generation"].schema,
        "required_payload_fields": list(
            MODULE.EVIDENCE_REQUIRED_FIELDS["proof_generation"]
        ),
    }
    ...
    assert (
        "PDP rollout runner plan external_evidence must contain only required kinds"
        in diagnostics
    )
    assert (
        "PDP rollout runner plan evidence_contract must contain only required kinds"
        in diagnostics
    )
    assert "proof_generation" not in diagnostics
...
def test_execution_rejects_plan_validation_drift_before_running(
    ...
    def fake_validate_plan_json(rendered, plan, args):
        return ["PDP rollout runner plan schema must match the contract"]
    ...
    assert not ran_plan
    assert (
        "PDP rollout runner plan schema must match the contract"
...
def test_response_file_dry_run_prints_complete_pdp_plan(tmp_path: Path, capsys) -> None:
    ...
    assert plan["external_evidence"]["provider_transport"]
    assert "proof_generation" in plan["evidence_contract"]
...
def test_subset_gate_requires_only_selected_kind(tmp_path: Path, capsys) -> None:
    ...
    assert list(plan["evidence_contract"]) == ["provider_transport"]
    verifier = plan["steps"][0]["command"]
...
def test_subset_gate_rejects_evidence_for_unrequired_kind(
    ...
    required_payload = write_payload(tmp_path / "provider-transport.json")
    extra_payload = write_payload(tmp_path / "proof-generation.json")
    ...
    assert exit_code == 2
    captured = capsys.readouterr()
    assert "rollout evidence supplied for unrequired kind" in captured.err
    assert "proof_generation" not in captured.err
