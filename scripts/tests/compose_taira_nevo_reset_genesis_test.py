...
def test_composition_is_deterministic_unsigned_and_secret_free(tmp_path: Path) -> None:
    ...
    assert grants == ["CanRegisterAccount", "CanEnrollFeeSponsorProgram"]
...
def test_public_input_schema_rejects_noncanonical_or_reused_accounts(tmp_path: Path) -> None:
    payload = _public_payload()
    payload["api_signer_account_id"] = payload["onboarding_authority_account_id"]
    path = _write_public_inputs(tmp_path, payload)
    with pytest.raises(composer.CompositionError, match="must be distinct"):
        composer.load_public_inputs(path)
