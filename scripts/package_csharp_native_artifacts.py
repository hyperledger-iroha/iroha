#!/usr/bin/env python3
"""Assemble and verify the exact ABI-24 native inventory for the C# NuGet SDK.
...
EVIDENCE_MANIFEST_NAME = "native-sdk-abi24.json"
...
def _validate_evidence(
    *,
    ...
    if (
        ...
        fail(f"{asset.target}: native artifact bytes do not match their ABI-24 evidence")
    return manifest
...
def validate_package_manifest(value: object) -> dict[str, object]:
    ...
    if manifest["bridge_abi_version"] != artifact_checker.REQUIRED_BRIDGE_ABI_VERSION:
        fail("C# native package manifest does not require exact bridge ABI 24")
        ...
        with zipfile.ZipFile(package_path, "r") as archive:
            ...
            for asset in NATIVE_ASSETS:
                ...
                if (
                    ...
                    fail(f"{asset.package_path}: NuGet native bytes do not match ABI-24 evidence")
