...
def _normalize_solana_rpc_url(rpc_url: str) -> str:
    ...
    if host is None:
        raise ValueError("--rpc-url must be an http(s) URL")
    if parsed.scheme == "http" and not _rpc_host_is_loopback(host):
        raise ValueError("--rpc-url must use HTTPS unless it is loopback HTTP")
    if parsed.scheme == "https" and _rpc_host_is_non_public_dns(host):
...
def _rpc_host_is_non_public_dns(host: str) -> bool:
    ...
    return (
        _rpc_host_is_loopback(normalized)
...
def _json_rpc(
    ...
    *,
    ...
    rpc_url = _normalize_solana_rpc_url(rpc_url)
