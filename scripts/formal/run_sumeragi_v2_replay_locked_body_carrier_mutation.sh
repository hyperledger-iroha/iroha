#!/usr/bin/env bash
# Run the bounded replay locked-body non-authority carrier mutation pair.
...
readonly SANY_SUCCESS_MARKER="Semantic processing of module SumeragiV2ReplayLockedBodyCarrierMutation"
...
run_dir="$(mktemp -d "${TMPDIR:-/tmp}/sumeragi-replay-locked-body-carrier.XXXXXX")"
...
echo "[sany] replay locked-body carrier mutation parsed with pinned TLA2Tools v${TLA2TOOLS_VERSION}"
...
run_case fixed \
  ...
  "29 states generated, 19 distinct states found, 0 states left on queue." \
...
run_case dropped-locked-fetch \
  ...
  "State 4: <FinishResponsiveReplay" \
  '/\ phase = "Recovered"' \
  '/\ finished = TRUE' \
  "4 states generated, 4 distinct states found, 0 states left on queue." \
  "The depth of the complete state graph search is 4."

echo "[tlc] fixed replay retained a non-authority locked-body carrier through Finish"
echo "[tlc] dropped-Fetch replay reached Finish and orphaned the historical locked body"
