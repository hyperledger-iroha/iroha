...
AUTHORITY_RECOVERY_BINDINGS = (('SumeragiV2NativeApplicationEvidence',
 ...
 ('SumeragiV2NativeApplicationEvidence',
  ...
  ('recovery.effect_count()',
   ...
   'canonical_recovery_is_request_effect(&effect)?',
 ...
 ('SumeragiV2NativeApplicationEvidence',
  ...
  ('self.lane_geometry_lock.lock()',
   ...
   'self.lock_consensus_sidecar_read()?;',
 ...
 ('SumeragiV2NativeApplicationEvidence',
  ...
  ('lane_geometry_lock.lock()',
   ...
   'BoundProgressPair::Absent',
   'BoundProgressPair::Present',
   'read_active_certified_lane_block_artifact_from_bound_locked',
   'bound_progress_sidecar_unchanged'),
  ('lane_geometry_lock.lock()', 'self.lock_consensus_sidecar_read()?;',
   'ensure_prune_recovery_not_required',
   'ensure_bound_progress_pair_has_no_recovery_artifacts_locked',
   'read_active_certified_lane_block_artifact_from_bound_locked',
   'bound_progress_sidecar_unchanged')),
 ('SumeragiV2NativeApplicationEvidence',
  'crates/iroha_core/src/kura/receipt_namespace_durability.rs',
  'method',
  'Kura::lock_consensus_sidecar_read',
 ...
 ('SumeragiV2NativeApplicationEvidence',
  'crates/iroha_core/src/kura/consensus_storage_reads.rs',
  'method',
  'Kura::recover_exact_canonical_lane_artifact',
...
def validate_authority_recovery_item(item: str, binding: tuple, errors: list[str]) -> None:
    ...
    if symbol == "read_native_amx_participant_application_history_with_lease":
        ...
        probe = "self.lock_consensus_sidecar_read()"
        owned = f"lease.is_none().then(|| {probe}).transpose()?"
        if (code.count(_code(probe)) != 2 or code.count(_code(owned)) != 2
                or "self.sidecar_lock.lock()" in code):
            errors.append("authority/recovery item history delegate must lease-guard every sidecar_lock probe")
