#!/usr/bin/env bash
# Run the bounded PersistInstallTC generation-overflow mutation matrix.
...
readonly MODEL="SumeragiV2PersistInstallGenerationMutation.tla"
readonly SANY_SUCCESS_MARKER="Semantic processing of module SumeragiV2PersistInstallGenerationMutation"
...
run_dir="$(mktemp -d "${TMPDIR:-/tmp}/sumeragi-persist-generation.XXXXXX")"
...
echo "[sany] PersistInstallTC generation-overflow model parsed"
...
echo "[tlc] repaired overflow rejection retained the complete snapshot and pending request"
echo "[tlc] saturating partial-commit mutant failed its transactional-state invariant"
