...
""",
    ...
    apply_barrier_projection = _require_qualified_rust_item(
        paths["driver"], sources["driver"], "LaunchedProductionLifecycleV1",
        "producer_claim_projection", errors, "owner-derived Apply scheduling projection",
    )
    require_order(
        "driver",
        apply_barrier_projection,
        "typed Apply scheduling reads retained owners",
        (
            "self.executor.lifecycle_decision_apply_is_complete()",
            "return Ok(Claim::ApplyTerminalSettled)",
            "self.pending_lifecycle_completion.as_ref()",
            "PendingLifecycleCompletionV1::LifecycleDecisionApplyDeferred(_)",
            "Claim::AwaitingApplyCompletion",
            "self.owner.coordinator.active_lease.as_ref()",
            "LifecycleWorkClass::Apply => return Ok(Claim::AwaitingApplyCompletion)",
            "self.executor.live_lifecycle_decision_apply_key()",
            "prepare_ready_live_decision_apply_reconciliation(",
            "self.executor.exactly_owns_live_lifecycle_decision_apply(&authority)",
...
""",
    ...
    require_order(
        ...
        "executor borrows current owners and receiver cut before freezing timeout authority",
        (
            "self.ensure_open()?",
            ...
            "runtime.set_ingress_physical_cut(physical_cut)",
            "runtime.freeze_pre_timeout_locked_prepare_qc_cut(now, &external)",
        ),
    )
    ...
    executor_pre_timeout_entry = qualified_item(
        "effects",
        "step_pre_timeout_locked_prepare_qc_once",
        effect_executor_context,
        "effect executor special pre-timeout entry",
    )
    require_order(
        "effects",
        executor_pre_timeout_entry,
        "special executor entry drains existing debt before admitting a runtime turn",
        (
            "self.ensure_open()?",
            "self.retained_effect_batch.is_some()",
            "self.parked_effect_batch.is_some()",
    ...
    executor_pre_timeout_step = qualified_item(
        "effects",
        "step_pre_timeout_locked_prepare_qc_after_debt",
        effect_executor_context,
    ...
    require_order(
        ...
        "special executor turn borrows current owners and owns WAL, scheduler evidence, reconciliation, and effects",
        (
            "self.runtime_and_external_lifecycle_census()?",
    ...
    apply_barrier_settlement = item(
        "lifecycle_run_inner", "settle_apply_barrier_runner_decision_handoff"
    )
    require_order(
        "lifecycle_run_inner",
        apply_barrier_settlement,
