...
QUEUE_PLAN_AUTONOMOUS_ONLY_TEST_BINDINGS = (
    ...
    (
        ...
        "globally_bound_absent_registry_skips_unadmitted_claim_without_blocking_ready_work",
        (
            "vec![follower_hash]",
...
QUEUE_PLAN_STARTUP_REPLAY_BINDINGS = (
    ...
    (
        ...
        (
            ...
            ".durable_plan_claims",
            "current != *expected",
            "self.lane_reservations.lock().owns_entrypoint(hash)",
            "current.local_custody == QueuePlanLocalCustody::Autonomous",
    ...
    (
        "crates/iroha_core/src/queue.rs",
        "method",
        "LaneQueueReservationStore::owns_entrypoint",
        ("self.durable_owned_hashes().any(|owned| owned == hash)", ".plan_tombstoned"),
    ),
    (
        "crates/iroha_core/src/queue.rs",
        "method",
        ...
        (
            ...
            "reject_exact_queue_plan_admission_claim(&binding)?",
        ),
    ),
    (
        "crates/iroha_core/src/queue.rs",
...
def validate_queue_plan_replay_terminal_custody(items: dict, errors: list[str]) -> None:
    ...
    ordered("Queue::reject_exact_queue_plan_admission_claim",
            "binding.validate_structure()",
            "let queue_guard = self.push_remove_lock.lock();",
            "if self.transaction_selection_durability_faulted()",
            "self.wait_for_durability_transitions(&[hash]);",
            ...
            "!= binding",
            "self.reject_unreserved_terminal_plan_claim(&claim)")
    ordered("Queue::reject_unreserved_terminal_plan_claim",
            "let queue_guard = self.push_remove_lock.lock();",
            "if self.transaction_selection_durability_faulted()",
            "self.wait_for_durability_transitions(&[hash]);",
            "if current != *expected",
