#!/usr/bin/env bash
# Run the bounded Apply current-Decision-authority mutation pair.
...
readonly SANY_SUCCESS_MARKER="Semantic processing of module SumeragiV2ApplyAuthorityMutation"
...
run_dir="$(mktemp -d "${TMPDIR:-/tmp}/sumeragi-apply-authority.XXXXXX")"
...
echo "[sany] Apply current-Decision-authority model parsed"

common=(
  "$JAVA_BIN" -XX:+UseParallelGC -cp "$TLA2TOOLS_JAR" tlc2.TLC
  -cleanup -workers 1 -fp 81 -seed 913765123
)
...
echo "[tlc] repaired Apply transferred the historical target to a current application"
echo "[tlc] missing-authority mutant exposed exact timed-owner loss"
