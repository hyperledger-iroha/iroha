...
STATE_MERGE_BINDINGS = (('SumeragiV2AutoscaleLifecycle',
 ...
 ('SumeragiV2AutonomousReservationCarrier',
  ...
  'fn',
  'preexecute_merge_execution_sources',
  ('self.try_merge_preexecution_block(application_block_header)?',
 ...
 'None => &*state_block,',
 'token.native_amx_authority(&*state_block)',
 ...
 'token.native_amx_authority(&authority)',
 'validate_native_amx_receipt_against_plan',
...
# Local resource refusal is distinct from deterministic source non-fit at every
# wrapper and prefix trial; the original authority checks remain on the selector.
STATE_MERGE_BINDINGS += (('SumeragiV2AutonomousReservationCarrier',
  'crates/iroha_core/src/state.rs',
  'fn',
 ...
 ('SumeragiV2AutonomousReservationCarrier',
  ...
  ('Result<Option<MergeExecutionBatch>, StateAdmissionError>',
   ...
   'build_batch(midpoint)?.and_then(|batch|',
...
STATE_MERGE_ORDERED_CHECKS = {'pending_autoscale_lane_drain_body_with_frontier': ('if !autoscale_lane_drain_state_matches_context(',
                                                     ...
                                                     'validate_lane_drain_certificate_body(&body)'),
 'select_merge_execution_candidate_for_consensus': ('let mut sources = Vec::new();',
                                                   'validate_merge_execution_predecessor_against_frontier',
                                                   'if descriptor.validator_set != authoritative',
