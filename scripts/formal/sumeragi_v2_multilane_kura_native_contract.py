...
BINDINGS = (('crates/iroha_core/src/kura.rs',
 ...
 ('crates/iroha_core/src/kura.rs',
  ...
  ('NativeAmxEvidencePruneIntentV2::VERSION',
   ...
   'intent.removed_settlements',
   'preimage_heights != removal_heights',
 ...
 ('crates/iroha_core/src/kura.rs',
  ...
  ('complete_native_amx_evidence_prune_intent_locked',
   ...
   'publish_bound_noclobber_file_locked',
   'inventory_native_amx_evidence_files_locked',
   'validate_native_amx_evidence_prune_intent_locked',
   'resources: &mut TotalDiskUsageMutation',
   'batch.finish()')),
 ('crates/iroha_core/src/kura.rs',
  'fn',
  'validate_native_amx_retained_history_continuity',
 ...
 ('crates/iroha_data_model/src/block/consensus.rs',
  ...
  ('Self::try_new(', 'wire.previous_native_settlement_hash', 'wire.source_ids')),
 ('crates/iroha_data_model/src/block/consensus.rs',
  'method',
 ...
 ('crates/iroha_core/src/kura.rs',
  ...
  ('Self::validate_native_amx_retained_history_continuity(manifests, receipts, false)',
   'complete.iter().rev()',
   'kept_pair_bytes',
   'complete.difference(&kept_complete)',
   'NativeAmxEvidencePruneIntentV2::VERSION',
   'protected_latest',
   'retention.get().checked_add(1)',
   'pair_len > stable_byte_limit',
   'kept_complete.len() < retention.get()',
   'kept_complete.contains(&protected_height)',
   'collect_native_amx_prune_settlement_preimages',
   'bytes.len() > journal_byte_limit')),
 ('crates/iroha_core/src/kura.rs',
  'fn',
  'collect_native_amx_prune_settlement_preimages',
  ('retained_bytes',
...
COMPLETED_CAPACITY_BRANCH = ('let existing = self .native_amx_publication_capacity_reservations .lock() .get(&carrier) '
 ...
 '&plan)? { return Ok(false); } else if target_indices.len() != evidence.artifacts.len() { // A '
 'partial repair must not reconstruct a subset of an indexed // carrier and silently forget its '
 'other publication obligations. return Err(Error::PruneIntentConflict( "Native AMX targeted '
