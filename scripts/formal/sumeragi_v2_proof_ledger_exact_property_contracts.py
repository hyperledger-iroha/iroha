...
EXACT_FIXED_PROOF_PROPERTY_OPERATOR_BODIES = {
    ...
    (
        ...
        " /\\ (specification => \\A carrier: IsFiniteSet(carrier) => []( Cardinalit"
        "y( AsyncCandidateServiceTombstonesInIdentityCarrier( carrier)) <= Cardin"
        "ality(carrier))) /\\ (specification => [][(\\A candidate \\in AsyncCandidat"
        "eSet: /\\ AsyncCandidateServiceActiveTombstone(candidate) /\\ [AsyncNext]_"
        "AsyncAllVars /\\ ~AsyncCandidateServiceExitThisStep(candidate) => AsyncCa"
        "ndidateServiceActiveTombstone(candidate)')]_AsyncAllVars) /\\ (specificat"
        "ion => [](\\A left, right \\in AsyncCandidateSet: /\\ left.node = right.nod"
        "e /\\ left.consumerContext = right.consumerContext /\\ left.height = right"
        ".height /\\ left.view = right.view /\\ left.subject = right.subject /\\ lef"
        "t.kind = right.kind /\\ left.class = right.class /\\ left.item # NoAsyncIt"
        "em /\\ right.item # NoAsyncItem /\\ left.item.kind = \"CertifiedResponse\" /"
        "\\ right.item = [left.item EXCEPT !.source = right.item.source] /\\ AsyncR"
        "outeNeutralCandidateEvidence(left.evidence) = AsyncRouteNeutralCandidate"
        "Evidence(right.evidence) /\\ left.bodyIdentity = right.bodyIdentity /\\ le"
        "ft.manifestIdentity = right.manifestIdentity /\\ left.commitmentIdentity "
        "= right.commitmentIdentity => AsyncCandidateServiceIdentity(left) = Asyn"
        "cCandidateServiceIdentity(right))) /\\ (specification => [][(\\A identity "
        "\\in AsyncCandidateAdmissionIdentitySet: /\\ AsyncCandidateAdmissionIdenti"
        "tyObsolete(identity) /\\ identity \\notin AsyncScheduledCandidateAdmission"
        "Identities /\\ gst /\\ [AsyncNext]_AsyncAllVars => /\\ AsyncCandidateAdmiss"
        "ionIdentityObsolete(identity)' /\\ identity \\notin AsyncScheduledCandidat"
        "eAdmissionIdentities')]_AsyncAllVars) /\\ (specification => [][(\\A identi"
        "ty \\in AsyncCandidateAdmissionIdentitySet: /\\ AsyncCandidateAdmissionIde"
        "ntityTerminallyCovered(identity) /\\ identity \\notin AsyncScheduledCandid"
        "ateAdmissionIdentities /\\ gst /\\ [AsyncNext]_AsyncAllVars => /\\ AsyncCan"
        "didateAdmissionIdentityTerminallyCovered( identity)' /\\ identity \\notin "
        "AsyncScheduledCandidateAdmissionIdentities')]_AsyncAllVars) /\\ (specific"
        "ation => [][(\\A candidate \\in AsyncCandidateSet: /\\ AsyncLogicalCandidat"
        "eOwnershipInvariant /\\ AsyncProgressOwnershipInvariant /\\ AsyncCandidate"
        "ServiceLifecycleInvariant /\\ gst /\\ AsyncNext /\\ CandidateScheduled(cand"
        "idate) /\\ ~CandidateScheduledAfter(candidate) => \\/ AsyncCandidateIgnore"
        "dWithoutApplicationThisStep( candidate) \\/ AsyncCandidateServiceTombston"
        "ed(candidate)' \\/ AsyncCandidateSameOriginPhysicalOrDurableOwnerAfter( c"
        "andidate) \\/ AsyncCandidateMonotoneSemanticCoverageAfterIn( asyncControl"
        "ServiceState', candidate) \\/ AsyncCandidateTerminalTombstoned(candidate)"
        "')]_AsyncAllVars)"
    ),
    (
        "SumeragiV2AdequateLeaderServiceClosureProofs",
    ...
    (
        ...
        "/\\ AdequateLeaderCandidateItemWithinFrozenView( candidate.item, leaderVi"
        "ew) /\\ AdequateLeaderCandidateEvidenceWithinFrozenView( candidate.eviden"
        "ce, leaderView) /\\ candidate.proposalRound.context = candidate.causalOri"
        "gin.context /\\ candidate.proposalRound.height = candidate.height /\\ cand"
        "idate.proposalRound.view \\in 0..leaderView"
    ),
    (
        "SumeragiV2AdequateLeaderServiceClosureProofs",
    ...
    (
        ...
        "[context |-> candidate.causalOrigin.context, round |-> AdequateLeaderFro"
        "zenCandidateRound( AsyncCandidateRound( candidate.causalOrigin.context, "
        "candidate.height, candidate.view), leaderView), proposalRound |-> Adequa"
        "teLeaderFrozenCandidateRound( candidate.proposalRound, leaderView), subj"
        "ect |-> candidate.subject, phase |-> candidate.semanticPhase, executionC"
        "ommitment |-> candidate.commitmentIdentity]"
    ),
    (
        "SumeragiV2AdequateLeaderServiceClosureProofs",
    ...
    (
        ...
        "IF /\\ target \\in ValidatorIds /\\ leaderContext \\in ContextRecords /\\ lea"
        "der \\in ValidatorIds /\\ leaderView \\in Nat /\\ subject \\in Subjects THEN "
        "[context: {leaderContext}, round: [context: {leaderContext}, height: {le"
        "aderContext.height}, view: 0..leaderView], proposalRound: [context: {lea"
        "derContext}, height: {leaderContext.height}, view: 0..leaderView], subje"
        "ct: {subject}, phase: AsyncCandidateSemanticPhases, executionCommitment:"
        " SubjectOrNone] ELSE {}"
    ),
    (
        "SumeragiV2AdequateLeaderServiceClosureProofs",
    ...
    (
        ...
        "/\\ rank \\in AdequateLeaderTargetSemanticRankCarrier /\\ subject \\in Subje"
        "cts /\\ AdequateLeaderFrozenTargetCorridor( target, leaderContext, leader"
        ", leaderView) /\\ AdequateLeaderTargetRankFrontier( target, leaderContext"
        ", leader, leaderView, subject, rank) /\\ IsFiniteSet( AdequateLeaderTarge"
        "tRankOwnerSet( target, leaderContext, leader, leaderView, subject, rank)"
        ") /\\ IsFiniteSet( AdequateLeaderTargetRankOwnerSet( target, leaderContex"
        "t, leader, leaderView, subject, rank)') /\\ AsyncNext /\\ ~NodeHasDecision"
        "(target)' /\\ ~AdequateLeaderTargetStrictOccurrenceDescentGoal( target, l"
        "eaderContext, leader, leaderView, subject, <<rank, AdequateLeaderTargetR"
        "ankOwnerCount( target, leaderContext, leader, leaderView, subject, rank)"
        ">>)' /\\ AdequateLeaderTargetRankOwnerCount( target, leaderContext, leade"
        "r, leaderView, subject, rank)' = AdequateLeaderTargetRankOwnerCount( tar"
        "get, leaderContext, leader, leaderView, subject, rank) /\\ AdequateLeader"
        "TargetRankOwnerIdentitySet( target, leaderContext, leader, leaderView, s"
        "ubject, rank)' # AdequateLeaderTargetRankOwnerIdentitySet( target, leade"
        "rContext, leader, leaderView, subject, rank)"
    ),
    (
        "SumeragiV2AdequateLeaderServiceClosureProofs",
        "AdequateLeaderTargetCountIncreasingReplenishmentAction",
        ...
        ">>)' /\\ AdequateLeaderTargetRankOwnerCount( target, leaderContext, leade"
        "r, leaderView, subject, rank)' > AdequateLeaderTargetRankOwnerCount( tar"
        "get, leaderContext, leader, leaderView, subject, rank)"
    ),
    (
        "SumeragiV2AdequateLeaderServiceClosureProofs",
        "AdequateLeaderTargetStrictOccurrenceDescentGoal",
    ): (
        "\\/ NodeHasDecision(target) \\/ \\E lowerOccurrenceRank \\in SetLessThan( oc"
        "currenceRank, AdequateLeaderTargetOccurrenceRankOrdering, AdequateLeader"
        "TargetOccurrenceRankCarrier): \\/ AdequateLeaderTargetOccurrenceRankFront"
        "ier( target, leaderContext, leader, leaderView, subject, lowerOccurrence"
        "Rank) \\/ AdequateLeaderTargetZeroOwnerProducerCell( target, leaderContex"
        "t, leader, leaderView, subject, lowerOccurrenceRank)"
    ),
    (
        "SumeragiV2AdequateLeaderServiceClosureProofs",
    ...
    (
        ...
        "UNION {IF /\\ AdequateLeaderFrozenTargetCandidateIdentity( candidate, ran"
        "k, target, leaderContext, leader, leaderView, subject) /\\ AdequateLeader"
        "CandidateProducerContinuationRetirementMemory( candidate) THEN {Adequate"
        "LeaderFrozenCandidateOwnerIdentity( candidate, rank, target, leaderConte"
        "xt, leader, leaderView, subject)} ELSE {}: candidate \\in AsyncCandidateS"
        "et, rank \\in AdequateLeaderTargetSemanticRankCarrier}"
    ),
    (
        "SumeragiV2AdequateLeaderServiceClosureProofs",
    ...
    (
        ...
        "/\\ AdequateLeaderCandidateFrozenIdentityBudgetBridgeProperty( specificat"
        "ion) /\\ (specification => \\A target \\in ValidatorIds, leaderContext \\in "
        "ContextRecords, leader \\in ValidatorIds, leaderView \\in Views, subject \\"
        "in Subjects, occurrenceRank \\in AdequateLeaderTargetOccurrenceRankCarrie"
        "r: /\\ [][(\\A identity \\in AdequateLeaderFrozenCandidateOwnerUniverse( ta"
        "rget, leaderContext, leader, leaderView, subject): AdequateLeaderTargetC"
        "andidateServicedRetirementAction( target, leaderContext, leader, leaderV"
        "iew, subject, occurrenceRank, identity) => /\\ AdequateLeaderServicedCand"
        "idateMemory( target, leaderContext, leader, leaderView, subject, identit"
        "y)' /\\ AdequateLeaderServicedCandidateClosure( target, leaderContext, le"
        "ader, leaderView, subject, occurrenceRank, identity)')]_AsyncAllVars /\\ "
        "[](\\A identity \\in AdequateLeaderFrozenCandidateOwnerUniverse( target, l"
        "eaderContext, leader, leaderView, subject): /\\ gst /\\ AdequateLeaderServ"
        "icedCandidateMemory( target, leaderContext, leader, leaderView, subject,"
        " identity) /\\ AdequateLeaderServicedCandidateClosure( target, leaderCont"
        "ext, leader, leaderView, subject, occurrenceRank, identity) => [](Adequa"
        "teLeaderServicedCandidateMemory( target, leaderContext, leader, leaderVi"
        "ew, subject, identity) /\\ AdequateLeaderServicedCandidateClosure( target"
    ...
    (
        ...
        "specification => \\A target \\in ValidatorIds, leaderContext \\in ContextRe"
        "cords, leader \\in ValidatorIds, leaderView \\in Views, subject \\in Subjec"
        "ts, occurrenceRank \\in AdequateLeaderTargetOccurrenceRankCarrier: [][(\\A"
        " identity \\in AdequateLeaderFrozenCandidateOwnerUniverse( target, leader"
        "Context, leader, leaderView, subject): AdequateLeaderTargetCandidateSucc"
        "essfulServiceRetirementAction( target, leaderContext, leader, leaderView"
        ", subject, occurrenceRank, identity) => AdequateLeaderServicedCandidateM"
        "emory( target, leaderContext, leader, leaderView, subject, identity)')]_"
        "AsyncAllVars"
    ),
    (
        "SumeragiV2AdequateLeaderServiceClosureProofs",
        ...
        "r: /\\ [][(\\A identity \\in AdequateLeaderFrozenCandidateOwnerUniverse( ta"
        "rget, leaderContext, leader, leaderView, subject): AdequateLeaderTargetC"
        "andidateTerminalDiscardRetirementAction( target, leaderContext, leader, "
        "leaderView, subject, occurrenceRank, identity) => /\\ AdequateLeaderServi"
        "cedCandidateMemory( target, leaderContext, leader, leaderView, subject, "
        "identity)' /\\ AdequateLeaderServicedCandidateClosure( target, leaderCont"
        "ext, leader, leaderView, subject, occurrenceRank, identity)')]_AsyncAllV"
        "ars /\\ [](\\A identity \\in AdequateLeaderFrozenCandidateOwnerUniverse( ta"
        "rget, leaderContext, leader, leaderView, subject): /\\ gst /\\ AdequateLea"
        "derServicedCandidateMemory( target, leaderContext, leader, leaderView, s"
        "ubject, identity) /\\ AdequateLeaderServicedCandidateClosure( target, lea"
        "derContext, leader, leaderView, subject, occurrenceRank, identity) => []"
        "(AdequateLeaderServicedCandidateMemory( target, leaderContext, leader, l"
        "eaderView, subject, identity) /\\ AdequateLeaderServicedCandidateClosure("
        " target, leaderContext, leader, leaderView, subject, occurrenceRank, ide"
        "ntity))))"
    ),
    (
        "SumeragiV2AdequateLeaderServiceClosureProofs",
    ...
    (
        ...
        "/\\ (specification => []AsyncCandidateServiceTombstoneLifecycleInvariant)"
        " /\\ (specification => \\A carrier: IsFiniteSet(carrier) => []( Cardinalit"
        "y( HistoricalTemporalCandidateServiceTombstonesInIdentityCarrier( carrie"
        "r)) <= Cardinality(carrier))) /\\ (specification => [][(\\A candidate \\in "
        "AsyncCandidateSet: /\\ AsyncCandidateServiceActiveTombstone(candidate) /\\"
        " [AsyncNext]_AsyncAllVars /\\ ~AsyncCandidateServiceExitThisStep(candidat"
        "e) => AsyncCandidateServiceActiveTombstone(candidate)')]_AsyncAllVars) /"
        "\\ (specification => [](\\A left, right \\in AsyncCandidateSet: /\\ left.nod"
        "e = right.node /\\ left.consumerContext = right.consumerContext /\\ left.h"
        "eight = right.height /\\ left.view = right.view /\\ left.subject = right.s"
        "ubject /\\ left.kind = right.kind /\\ left.class = right.class /\\ left.ite"
        "m # NoAsyncItem /\\ right.item # NoAsyncItem /\\ left.item.kind = \"Certifi"
        "edResponse\" /\\ right.item = [left.item EXCEPT !.source = right.item.sour"
        "ce] /\\ AsyncRouteNeutralCandidateEvidence(left.evidence) = AsyncRouteNeu"
        "tralCandidateEvidence(right.evidence) /\\ left.causalOrigin = right.causa"
        "lOrigin /\\ left.bodyIdentity = right.bodyIdentity /\\ left.manifestIdenti"
        "ty = right.manifestIdentity /\\ left.commitmentIdentity = right.commitmen"
        "tIdentity => AsyncCandidateServiceIdentity(left) = AsyncCandidateService"
        "Identity(right))) /\\ (specification => [][(\\A identity \\in AsyncCandidat"
        "eAdmissionIdentitySet: /\\ AsyncCandidateAdmissionIdentityObsolete(identi"
        "ty) /\\ identity \\notin AsyncScheduledCandidateAdmissionIdentities /\\ gst"
        " /\\ [AsyncNext]_AsyncAllVars => /\\ AsyncCandidateAdmissionIdentityObsole"
        "te(identity)' /\\ identity \\notin AsyncScheduledCandidateAdmissionIdentit"
        "ies')]_AsyncAllVars) /\\ (specification => [][(\\A identity \\in AsyncCandi"
        "dateAdmissionIdentitySet: /\\ identity.service.phase = \"DeliverChunk\" /\\ "
        "AsyncCandidateAdmissionIdentityTerminallyCovered(identity) /\\ identity \\"
        "notin AsyncScheduledCandidateAdmissionIdentities /\\ gst /\\ [AsyncNext]_A"
        "syncAllVars => /\\ AsyncCandidateAdmissionIdentityTerminallyCovered( iden"
        "tity)' /\\ identity \\notin AsyncScheduledCandidateAdmissionIdentities')]_"
        "AsyncAllVars) /\\ (specification => [][(\\A identity \\in AsyncCandidateAdm"
        "issionIdentitySet: /\\ identity.service.phase = \"DeliverChunk\" /\\ identit"
        "y \\in AsyncScheduledCandidateAdmissionIdentities /\\ gst /\\ [AsyncNext]_A"
        "syncAllVars /\\ identity \\notin AsyncScheduledCandidateAdmissionIdentitie"
        "s' => AsyncCandidateAdmissionIdentityLifecycleCovered( identity)')]_Asyn"
        "cAllVars)"
    ),
    (
        "SumeragiV2AsyncHistoricalRecoveryTemporalSupportProofs",
    ...
    (
        ...
        "/\\ (specification => []AsyncServeLifecycleTypeInvariant) /\\ (specificati"
        "on => [](/\\ IsFiniteSet(asyncServeReservations) /\\ IsFiniteSet(asyncServ"
        "eTombstones) /\\ Cardinality(asyncServeTombstones) <= Cardinality(AsyncSe"
        "rveLifecycleFamilies))) /\\ (specification => \\A carrier: IsFiniteSet(car"
        "rier) => [](/\\ IsFiniteSet( HistoricalTemporalServeReservationsInIdentit"
        "yCarrier( carrier)) /\\ IsFiniteSet( HistoricalTemporalServeTombstonesInI"
        "dentityCarrier( carrier)) /\\ IsFiniteSet( HistoricalTemporalServeRollbac"
        "kTombstonesInIdentityCarrier( carrier)) /\\ IsFiniteSet( HistoricalTempor"
        "alServeRetiredRecordsInIdentityCarrier( carrier)) /\\ Cardinality( Histor"
        "icalTemporalServeTombstonesInIdentityCarrier( carrier)) <= Cardinality(c"
        "arrier))) /\\ (specification => [](\\A node \\in ValidatorIds, identity \\in"
        " AsyncServeLogicalRequestIdentities: AsyncServeLiveReservationOwned(node"
        ", identity) => /\\ Cardinality( AsyncServeReservationRecords(node, identi"
        "ty)) = 1 /\\ AsyncServeAdmissionOrdinal(node, identity) < asyncNextServeA"
        "dmissionOrdinal[node])) /\\ (specification => [](\\A node \\in ValidatorIds"
        ", family \\in AsyncServeLifecycleFamilies: AsyncServeLifecycleFamilyOwned"
        "(node, family) => /\\ Cardinality( AsyncServeFamilyAdmissionRecords(node,"
        " family) \\cup AsyncServeFamilyTombstoneRecords(node, family)) = 1 /\\ Asy"
        "ncServeFamilyOwnerIdentity(node, family) \\in AsyncServeLogicalRequestIde"
        "ntities /\\ AsyncServeFamilyHighWatermark(node, family) \\in Views)) /\\ (s"
        "pecification => [][(\\A node \\in ValidatorIds, identity \\in AsyncServeLog"
        "icalRequestIdentities: /\\ AsyncServeJobQueued(node, identity) /\\ gst /\\ "
        "[AsyncNext]_AsyncAllVars /\\ ~AsyncServeJobQueued(node, identity)' => Asy"
        "ncServeLifecycleTombstone(node, identity)')]_AsyncAllVars) /\\ (specifica"
        "tion => [][(\\A node \\in ValidatorIds, identity \\in AsyncServeLogicalRequ"
        "estIdentities: /\\ AsyncServeLogicalIdentityRetiredOrSuperseded( node, id"
        "entity) /\\ gst /\\ [AsyncNext]_AsyncAllVars => /\\ AsyncServeLogicalIdenti"
        "tyRetiredOrSuperseded( node, identity)' /\\ ~AsyncServeJobQueued(node, id"
        "entity)')]_AsyncAllVars) /\\ (specification => [][(\\A node, candidate: Hi"
        "storicalTemporalServeExactRetryCoalescingAction( node, candidate) => UNC"
        "HANGED asyncNextServeAdmissionOrdinal)]_AsyncAllVars) /\\ (specification "
        "=> [](\\A node \\in ValidatorIds, left, right \\in AsyncCertifiedRequestIte"
        "ms \\cup AsyncCommitCertificateRequestItems: AsyncServeLogicalRequestIden"
        "tity(node, left) = AsyncServeLogicalRequestIdentity(node, right) => LET "
        "identity == AsyncServeLogicalRequestIdentity(node, left) IN /\\ AsyncServ"
        "eReservationRecords(node, identity) = AsyncServeReservationRecords( node"
        ", AsyncServeLogicalRequestIdentity(node, right)) /\\ AsyncServeTombstoneR"
        "ecords(node, identity) = AsyncServeTombstoneRecords( node, AsyncServeLog"
        "icalRequestIdentity(node, right)) /\\ (AsyncServeLifecycleOwned(node, ide"
        "ntity) => AsyncServeAdmissionOrdinal(node, identity) = AsyncServeAdmissi"
        "onOrdinal( node, AsyncServeLogicalRequestIdentity( node, right)))))"
    ),
    (
        "SumeragiV2AsyncHistoricalRecoveryTemporalSupportProofs",
