#!/usr/bin/env bash
# Run the bounded CertificateRef historical-recovery mutation pair.
...
readonly SANY_SUCCESS_MARKER="Semantic processing of module SumeragiV2CertificateRefRecoveryMutation"
...
run_dir="$(mktemp -d "${TMPDIR:-/tmp}/sumeragi-certificate-ref-recovery.XXXXXX")"
...
echo "[sany] CertificateRef historical-recovery mutation parsed with pinned TLA2Tools v${TLA2TOOLS_VERSION}"
...
run_case exact-qc-equality-mutant \
  ...
  "State 2: <BeginLockUsingEquivalentQcB" \
  '/\ stage = "PendingLockCommitB"' \
  '/\ candidateScheduled = FALSE' \
  '/\ executedFrom = "PrepareQcA"' \
  '/\ pendingOwner = "PrepareQcB"' \
  'signers |-> {"validator-0", "validator-1", "validator-3"}' \
  "2 states generated, 2 distinct states found, 0 states left on queue." \
  "The depth of the complete state graph search is 2."

echo "[tlc] full CertificateRef ownership survived the valid PrepareQcA-to-PrepareQcB handoff"
echo "[tlc] whole-QC equality orphaned source-A recovery ownership after the exact handoff"
