#!/usr/bin/env bash
# Run the bounded certified-response identity-separation mutation pair.
...
run_dir="$(mktemp -d "${TMPDIR:-/tmp}/sumeragi-certified-response-identities.XXXXXX")"
...
echo "[sany] certified-response identity-separation mutation parsed with pinned TLA2Tools v${TLA2TOOLS_VERSION}"

common=(
  "$JAVA_BIN" -XX:+UseParallelGC -cp "$TLA2TOOLS_JAR" tlc2.TLC
  -cleanup -workers 1 -fp 97 -seed 139154308881391968
)
