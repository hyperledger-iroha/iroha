...
ABI24 intentionally has exactly six privacy C exports. Its no-argument
...
def _require_exact_abi24(root: Path) -> None:
    rust = _rust_exports(_rust_bridge_source(root))
    header = _header_exports(_read(root, C_HEADER))
    if rust != APPROVED_PRIVACY_EXPORTS:
        raise AuditError(
            "Rust ABI24 privacy exports differ from the exact approved six: "
            f"found {sorted(rust)}"
        )
    if header != APPROVED_PRIVACY_EXPORTS:
        raise AuditError(
            "C ABI24 privacy declarations differ from the exact approved six: "
...
def _require_authority_boundary(root: Path) -> None:
    ...
    if any(symbol in combined for symbol in forbidden):
        raise AuditError("ABI24 added a capability authority getter or retired alias")
    if "compiled_privacy_profile_catalog_v1" not in bridge:
        raise AuditError("ABI24 local catalog is no longer derived from native Rust profiles")
    if "contains no committed height" not in combined.lower():
        raise AuditError("ABI24 local catalog lost its explicit non-authority contract")
...
def _swift_cutover_gates(root: Path) -> dict[str, bool]:
    ...
    native_backed_validation = all(
        (
            ...
            "requiredBridgeABIVersion: UInt32 = 24" in bridge,
...
def audit(root: Path) -> dict[str, object]:
    ...
    return {
        ...
        "abi24_privacy_exports": sorted(APPROVED_PRIVACY_EXPORTS),
...
def _format_human(report: dict[str, object]) -> str:
    lines = [
        ...
        "ABI24 privacy exports: exact six",
