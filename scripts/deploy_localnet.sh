...
#   IROHAD_BIN       Path to the `iroha3d` binary (default: <target-dir>/<profile>/iroha3d).
...
waits for readiness, and verifies the built-in Kagemusha asset alias.
...
if [[ "$SKIP_ASSET_CHECK" != true ]]; then
  echo ""
  echo "Verifying built-in Kagemusha alias $ASSET_ALIAS..."
  ...
  asset_alias_response="$(
    ...
    echo "Failed to resolve built-in Kagemusha alias $ASSET_ALIAS." >&2
    exit 1
  }
...
echo "Built-in Kagemusha asset: $ASSET_NAME ($ASSET_ID) alias $ASSET_ALIAS"
