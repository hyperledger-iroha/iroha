...
# not belong to the canonical 850-test liveness inventory above. The later
...
# Collection is source-bound as 4,933 ledger/checker cases (including the
# lexically executed case components), 28 pinned-Verus evidence cases,
# 15 TLC-normalizer cases, and seven selected layout cases.
set +e
PYTHONDONTWRITEBYTECODE=1 PYTHONHASHSEED=0 python3 -m pytest -q -p no:cacheprovider \
...
proof_fidelity_pass_summary="$(
  grep -Ec '^4983 passed in [0-9]+([.][0-9]+)?s( \([0-9]+:[0-5][0-9]:[0-5][0-9]\))?$' "$proof_fidelity_contract_log" || true
)"
if ((proof_fidelity_pipeline_status[0] != 0 || proof_fidelity_pipeline_status[1] != 0)) \
  || [[ "$proof_fidelity_pass_summary" != 1 ]]; then
  echo "Sumeragi v2 proof-fidelity preflight did not run exactly 4983 passing tests (pytest=${proof_fidelity_pipeline_status[0]}, tee=${proof_fidelity_pipeline_status[1]})" >&2
  exit 1
fi
record_corridor_log \
  preflight-proof-fidelity pytest 4983 \
