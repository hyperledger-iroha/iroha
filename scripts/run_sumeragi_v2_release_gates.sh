...
PY
  ...
  if ((sealed_status == 0)); then
    if [[ "$("$release_python_bin" -I -S \
      ...
      echo "Sumeragi v2 production release gates passed, including exact 518/518 G-UNIT, strict 10/10 G-12P, the two-hour G-12P fault soak, sealed G-SCALE evidence, and 100,000 heights; receipt=${release_bootstrap_evidence_dir}/RELEASE_COMPLETED.json" >&2
    fi
  fi
  exit "$sealed_status"
fi
...
required_production_liveness_tests=(
  ...
  sumeragi::v2::tests::saturated_normal_lane_suppresses_exact_serviced_local_proposal_retry
  ...
  sumeragi::v2_lifecycle_coordinator::ledger::tests::durable_ready_fetch_recovery::fresh_certified_serve_postledger_failure_retains_tail_and_requires_restart
  sumeragi::v2_lifecycle_coordinator::ledger::tests::durable_ready_fetch_recovery::fresh_certified_serve_postrename_sync_failure_requires_restart
  ...
  sumeragi::v2_lifecycle_coordinator::ledger::tests::durable_ready_fetch_recovery::negative_and_cancelled_certified_serve_tombstones_stutter_exactly
...
readonly expected_production_liveness_test_count=863
...
# not belong to the canonical 863-test liveness inventory above. The later
...
required_multilane_core_focus_tests=(
  ...
  torii_proxy::tests::torii_transaction_admission_wire_indexes_are_contiguous
  torii_proxy::tests::queue_plan_synced_request_identity_is_semantic_and_exact_network_bound
  ...
  torii_proxy::tests::torii_proxy_v1_envelope_roundtrips_exact_deadline_bound_request
  ...
  kura::tests::historical_atomic_temp_rejects_obsolete_residue_before_promoting_valid_temp
...
required_multilane_queue_journal_focus_tests=(
  queue::journal::tests::queue_plan_journal_claim_digest_binds_exact_v1_record_bytes_and_context
  queue::journal::tests::v1_put_rejects_every_noncanonical_admission_context_before_append
  queue::journal::tests::v1_journal_replays_puts_and_exact_removes
  ...
  queue::journal::tests::every_recognizable_terminal_v1_prefix_is_repaired_after_valid_frame
  ...
  queue::journal::tests::compaction_preserves_live_fifo_order_and_uses_v1_frames
  ...
  queue::reservation_journal::tests::v1_envelope_rejects_unsupported_record_versions_without_rewrite
  queue::reservation_journal::tests::v1_release_batch_replay_is_atomic_idempotent_and_exact
  ...
  queue::tests::v1_replay_rejects_same_lane_id_after_incarnation_recreation
  queue::tests::v1_replay_revalidates_original_generation_across_height_advance
  queue::tests::v1_replay_preserves_immutable_admitted_plan_across_router_policy_drift
...
required_multilane_torii_focus_tests=(
  tests_runtime_handlers::torii_proxy_v1_roundtrip_and_forwarding_preserve_transaction_admission_binding
  ...
  tests_runtime_handlers::incoming_torii_proxy_rejects_every_non_v1_schema_before_dispatch
  ...
  tests_runtime_handlers::incoming_torii_proxy_rejects_malformed_v1_hop_chain_before_dispatch
...
# unambiguous one-test Cargo transcript per entry. The canonical 518-row TSV is
...
if ((corridor_enabled)); then
  ...
  if [[ "$(wc -l <"$corridor_g_unit_inventory" | tr -d '[:space:]')" != 519 ]]; then
    echo "G-UNIT inventory must contain one header and exactly 518 focused tests" >&2
    exit 1
  fi
fi
