#!/usr/bin/env python3
"""Verify the closed Offline Cash V1 release-evidence filesystem projection.
...
MANIFEST_SCHEMA = "iroha.offline_cash_v1.release_evidence_manifest"
PROJECTION_SCHEMA = "iroha.offline_cash_v1.authority_review_projection"
SCHEMA_VERSION = 1
WIRE_VERSION = 1
...
OBSERVER_POLICY_SCHEMA = "iroha.offline_cash_v1.trusted_observer_policy"
OBSERVATION_SCHEMA = "iroha.offline_cash_v1.verification_observation"
OBSERVER_AUTHORITY_ID_DOMAIN = b"iroha:offline-cash:v1:observer-authority\0"
OBSERVATION_APPROVAL_DOMAIN = b"iroha:offline-cash:v1:observation-approval\0"
CANDIDATE_CONTEXT_DIGEST_DOMAIN = b"iroha:offline-cash:v1:release-candidate-context"
ARTIFACT_SET_DIGEST_DOMAIN = b"iroha:offline-cash:v1:artifact-set"
VK_SET_DIGEST_DOMAIN = b"iroha:offline-cash:v1:vk-set"
PROFILE_QUALIFICATION_DIGEST_DOMAIN = b"iroha:offline-cash:v1:profile-qualification"
HARDWARE_POLICY_DIGEST_DOMAIN = b"iroha:offline-cash:v1:hardware-policy"
HARDWARE_PROFILE_DIGEST_DOMAIN = b"iroha:offline-cash:v1:hardware-profile"
SUITE_COMMITMENT_DOMAIN = b"iroha:offline-cash:v1:suite-commitment"
RELEASE_PROFILE_DIGEST_DOMAIN = b"iroha:offline-cash:v1:release-profile"
VERIFICATION_RECORDS_DIGEST_DOMAIN = b"iroha:offline-cash:v1:verification-records"
CANDIDATE_CONTEXT_SCHEMA = "iroha.offline_cash_v1.release_candidate_context"

ARTIFACT_SET_SCHEMA = "iroha.offline-cash.v1.release-artifact-set-digest-subject"
VK_SET_SCHEMA = "iroha.offline-cash.v1.release-vk-set-digest-subject"
PROFILE_QUALIFICATION_SCHEMA = (
    "iroha.offline-cash.v1.release-profile-qualification-digest-subject"
)
HARDWARE_POLICY_SCHEMA = "iroha.offline-cash.v1.release-hardware-policy-digest-subject"
HARDWARE_PROFILE_SCHEMA = "iroha.offline-cash.v1.hardware-profile-id-preimage"
RELEASE_PROFILE_SCHEMA = "iroha.offline-cash.v1.release-profile-digest-subject"
...
class OfflineCashEvidenceError(RuntimeError):
    """Raised when the Offline Cash evidence closure is invalid."""


def _fail(message: str) -> NoReturn:
    raise OfflineCashEvidenceError(message)
...
def _scan_evidence_tree(root: Path) -> list[str]:
    ...
    try:
        ...
        raise OfflineCashEvidenceError(f"failed to scan bounded evidence tree: {error}") from error
    finally:
        if root_fd >= 0:
            os.close(root_fd)
        if reopened_fd >= 0:
...
class EvidenceVerifier:
    ...
    def _report(
        ...
        try:
            ...
            raise OfflineCashEvidenceError(
...
def verify_evidence(
    *,
    ...
    manifest = load_json_object(payload, "offline-cash evidence manifest")
    try:
        canonical = canonical_json_bytes(dict(manifest))
    except (TypeError, ValueError) as error:
        raise OfflineCashEvidenceError("evidence manifest is not canonical JSON") from error
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Verify an immutable Offline Cash V1 release-evidence closure."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except (OfflineCashEvidenceError, ReleaseArtifactError, OSError, ValueError) as error:
        print(f"offline cash release evidence rejected: {error}", file=sys.stderr)
        return 1
