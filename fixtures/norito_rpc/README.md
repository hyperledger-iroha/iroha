...
payloads. Use `cargo run --locked -p xtask --features dev-tools --bin xtask --
norito-rpc-fixtures` to regenerate the canonical set from this directory's
`transaction_payloads.json`, and run `cargo run --locked -p xtask --features dev-tools --bin xtask
...
The same `norito-rpc-fixtures` and `norito-rpc-verify` commands above regenerate
and validate this file against the compiled data model.
