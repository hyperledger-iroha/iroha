...
final class ToriiClientTests: XCTestCase {
    ...
    private func assertBfvOperationKeyComponentVectors(_ operationVectors: [String: Any]) throws {
        ...
        XCTAssertEqual(try int(bootstrap, "max_refresh_rounds"), try int(evaluationKey, "bootstrap_max_refresh_rounds"))
        ...
        for (index, refresh) in roundRefreshes.enumerated() {
            ...
            if index == 0 {
                ...
                assertBfvComponentDigest("bootstrap round \(index) c0", try string(components, "c0_sha256"), seen: &componentDigests)
                assertBfvComponentDigest("bootstrap round \(index) c1", try string(components, "c1_sha256"), seen: &componentDigests)
            }
        }
        ...
        for vector in bootstrapRefreshVectors {
            ...
            let components = try object(vector, "output_components")
            XCTAssertEqual(try int(components, "coefficient_count"), publicDegree)
            assertBfvComponentDigest("bootstrap refresh vector \(name) c0", try string(components, "c0_sha256"), seen: &componentDigests)
            assertBfvComponentDigest("bootstrap refresh vector \(name) c1", try string(components, "c1_sha256"), seen: &componentDigests)
        }
        ...
        assertBfvComponentDigest("packed RotateLeft c0", try string(packedRotateComponents, "c0_sha256"), seen: &componentDigests)
        assertBfvComponentDigest("packed RotateLeft c1", try string(packedRotateComponents, "c1_sha256"), seen: &componentDigests)
        ...
        assertBfvComponentDigest("packed RotateLeft schedule c0", try string(packedRotateScheduleComponents, "c0_sha256"), seen: &componentDigests)
        assertBfvComponentDigest("packed RotateLeft schedule c1", try string(packedRotateScheduleComponents, "c1_sha256"), seen: &componentDigests)
    }
