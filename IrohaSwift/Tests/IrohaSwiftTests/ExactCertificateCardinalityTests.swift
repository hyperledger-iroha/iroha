...
final class ExactCertificateCardinalityTests: XCTestCase {
    ...
    private func payload(signerCount: Int, laneManifest: Any? = NSNull()) throws -> Data {
        let round: [String: Any] = [
            "context_id": [certificateHash], "height": 1, "view": 0,
        ]
        let subject: [String: Any] = [
            "parent_block_hash": NSNull(), "block_hash": certificateHash,
            "payload_hash": certificateHash,
        ]
        var commitment: [String: Any] = [
            "parent_state_root": certificateHash,
            "post_state_root": certificateHash,
            "ordinary_writes_root": certificateHash,
            ...
            "executed_block_wire_hash": certificateHash,
        ]
    ...
    func testCommitQCRejectsSignerSuperset() throws {
        ...
        let lane = ["root": certificateHash, "leaf_count": 1] as [String: Any]
        let parsedLane = try decode(lane).certificate.executionCommitment.laneFinalityManifest
        XCTAssertEqual(parsedLane?.root, certificateHash)
        XCTAssertEqual(parsedLane?.leafCount, 1)
        for invalid: Any? in [nil, ["leaf_count": 1],
                              ["root": certificateHash, "leaf_count": 0],
                              ["root": certificateHash, "leaf_count": 1025]] {
