...
public final class NoritoNativeBridge: @unchecked Sendable {
    ...
    private func hasKagemushaV2Symbol(_ symbol: String) -> Bool {
        #if canImport(Darwin)
        guard let bridgeHandle else { return false }
        return dlsym(bridgeHandle, symbol) != nil
        #else
        _ = symbol
        return false
        #endif
    ...
    public var isKagemushaRecursiveSpendTopUpAvailable: Bool {
        ...
        return isKagemushaRecursiveSpendAvailable
            && loadedBridgeAbiVersion == KagemushaRecursiveSpend.requiredNativeBridgeAbiVersion
