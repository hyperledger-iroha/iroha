...
public enum OfflineNoteWalletNoteJsonCodecError: Error, LocalizedError, Equatable {
    case invalidJson
    ...
    public var errorDescription: String? {
        switch self {
        case .invalidJson:
            return "Offline Note wallet note JSON must be an object."
        case let .invalidField(field):
            return "Offline Note wallet note field \(field) is invalid."
        }
    }
}

public enum OfflineNoteWalletNoteJsonCodec {
    ...
    public static func decode(_ payload: Data) throws -> OfflineNoteWalletNote {
        guard let object = try JSONSerialization.jsonObject(with: payload) as? [String: Any] else {
            throw OfflineNoteWalletNoteJsonCodecError.invalidJson
        }
        guard try uint(object["version"], field: "version") == version else {
            throw OfflineNoteWalletNoteJsonCodecError.invalidField("version")
        }
        guard let keyCertificateBytes = Data(base64Encoded: try string(
            ...
            throw OfflineNoteWalletNoteJsonCodecError.invalidField("key_certificate_norito_base64")
        }
        guard let noteCommitment = Data(hexString: try string(
            object["note_commitment_hex"],
            field: "note_commitment_hex"
        )) else {
            throw OfflineNoteWalletNoteJsonCodecError.invalidField("note_commitment_hex")
        }
    ...
    private static func encodeOrigin(_ origin: OfflineNoteCommitmentOrigin) throws -> [String: Any] {
        switch origin {
    ...
    private static func decodeOrigin(_ payload: [String: Any]) throws -> OfflineNoteCommitmentOrigin {
        switch try string(payload["type"], field: "origin.type") {
        case "issuer_load":
            return try .issuerLoad(OfflineNoteIssuerLoadOrigin(
        ...
        case "p2p_output":
            ...
            guard outputIndex <= UInt64(UInt32.max) else {
                throw OfflineNoteWalletNoteJsonCodecError.invalidField("origin.output_index")
            }
            return try .p2pOutput(OfflineNoteP2pOutputOrigin(
                paymentRequestId: string(payload["payment_request_id"], field: "origin.payment_request_id"),
                outputIndex: UInt32(outputIndex)
            ))
        default:
            throw OfflineNoteWalletNoteJsonCodecError.invalidField("origin.type")
        }
    }

    private static func decodeAuditTrail(_ value: Any?) throws -> [OfflineNoteAuditBundle] {
        guard let value else {
            return []
        }
        guard let values = value as? [Any] else {
            throw OfflineNoteWalletNoteJsonCodecError.invalidField("bearer_audit_trail_norito_base64")
        }
        return try values.enumerated().map { index, item in
            guard let encoded = item as? String,
                  let bytes = Data(base64Encoded: encoded) else {
                throw OfflineNoteWalletNoteJsonCodecError.invalidField(
                    "bearer_audit_trail_norito_base64[\(index)]"
                )
            }
            return try OfflineNoteDecoding.decodeAudit(bytes)
        }
    }
...
public enum OfflineNoteKeychainStoreError: Error, LocalizedError, Equatable {
    ...
    public var errorDescription: String? {
        ...
        case let .invalidLabel(label):
            return "Offline Note keychain store label is invalid: \(label)"
        case let .corrupt(reason):
            return "Offline Note keychain store is corrupted: \(reason)"
        case let .keychainFailure(status):
            return "Offline Note keychain operation failed with status \(status)"
        }
    }
}

public final class OfflineNoteKeychainStore: OfflineNoteStore {
    ...
    private let backing: any OfflineNoteKeychainBackingStore
    ...
    public init(label rawLabel: String = "default", configuration: Configuration = .default) throws {
        ...
        backing = OfflineNoteKeychainBacking(configuration: configuration)
    }

    init(label rawLabel: String, backing: any OfflineNoteKeychainBackingStore) throws {
        label = try Self.sanitize(label: rawLabel)
    ...
    public func findNote(noteCommitment: Data) throws -> OfflineNoteWalletNote? {
        try lock.withLock {
            try loadNotes()[noteCommitment.hexLowercased()]
        }
    }

    public func upsert(_ note: OfflineNoteWalletNote) throws {
    ...
    private func loadNotes() throws -> [String: OfflineNoteWalletNote] {
        if let metadata = try loadMetadata() {
            guard let data = try backing.load(label: collectionLabel(revision: metadata.revision)) else {
                throw OfflineNoteKeychainStoreError.corrupt("collection revision is missing")
            }
    ...
    private func decodeCollection(_ data: Data) throws -> [String: OfflineNoteWalletNote] {
        do {
            let collection = try JSONDecoder().decode(StoredCollection.self, from: data)
            guard collection.version == 1 else {
                throw OfflineNoteKeychainStoreError.corrupt("unsupported collection version")
            }
            var notes: [String: OfflineNoteWalletNote] = [:]
            for stored in collection.notes {
                guard let payload = Data(base64Encoded: stored.payloadBase64) else {
                    throw OfflineNoteKeychainStoreError.corrupt("note payload is not base64")
                }
                let note = try OfflineNoteWalletNoteJsonCodec.decode(payload)
                guard note.noteCommitmentHex == stored.commitmentHex else {
                    throw OfflineNoteKeychainStoreError.corrupt("note commitment index mismatch")
                }
                guard notes[stored.commitmentHex] == nil else {
                    throw OfflineNoteKeychainStoreError.corrupt("duplicate note commitment")
                }
                notes[stored.commitmentHex] = note
            }
            return notes
        } catch let storeError as OfflineNoteKeychainStoreError {
            throw storeError
        } catch {
            throw OfflineNoteKeychainStoreError.corrupt("failed to decode collection: \(error)")
        }
    }

    private func saveNotes(_ notes: [String: OfflineNoteWalletNote]) throws {
    ...
    private func loadMetadata() throws -> StoredMetadata? {
        ...
        do {
            ...
            guard metadata.version == 1, metadata.revision > 0 else {
                throw OfflineNoteKeychainStoreError.corrupt("unsupported metadata")
            }
            return metadata
        } catch let storeError as OfflineNoteKeychainStoreError {
            throw storeError
        } catch {
            throw OfflineNoteKeychainStoreError.corrupt("failed to decode metadata: \(error)")
        }
    }
    ...
    private static func sanitize(label: String) throws -> String {
        ...
        guard !trimmed.isEmpty else {
            throw OfflineNoteKeychainStoreError.invalidLabel(label)
        }
...
protocol OfflineNoteKeychainBackingStore {
    func load(label: String) throws -> Data?
    func save(label: String, data: Data) throws
    func delete(label: String) throws
}

private struct OfflineNoteKeychainBacking: OfflineNoteKeychainBackingStore {
    private let service = "org.hyperledger.iroha.offline-note-store"
    private let accessGroup: String?
    private let accessControl: SecAccessControl?

    init(configuration: OfflineNoteKeychainStore.Configuration) {
    ...
    func load(label: String) throws -> Data? {
        ...
        case errSecSuccess:
            guard let data = item as? Data else {
                throw OfflineNoteKeychainStoreError.keychainFailure(status)
            }
            return data
    ...
    func save(label: String, data: Data) throws {
        ...
        if status == errSecDuplicateItem {
            ...
            guard updateStatus == errSecSuccess else {
                throw OfflineNoteKeychainStoreError.keychainFailure(updateStatus)
            }
            return
        }
