...
canonical three-message exchange—an exact-amount request, its receiver-bound
proof-bearing payment, and the durable acknowledgement—plus independently
framed mint authorization, mint credit, and redemption voucher values through
bounded Norito archives or `kgm1:` text. The payment carries the terminal
commit certificate and paired proof; its proof-independent body digest binds
the output and actual encrypted credit. QR, NFC, and Nearby all use the same
canonical bytes from `../fixtures/offline/kagemusha_v1.json`; no transport has
a second codec.
