...
|----|--------|-----------------|--------------------|----------|
| C1 | WebSocket 中断和重新连接 |将 `/v1/connect/ws` 包裹在代理后面（例如 `kubectl -n demo port-forward svc/torii 18080:8080` + `toxiproxy-cli toxic add ... timeout`）或暂时阻止服务（`kubectl scale deploy/torii --replicas=0` ≤ 60 秒）。强制钱包继续发送帧，以便填满离线队列。 | `connect.reconnects_total` 递增，`connect.resume_latency_ms` 尖峰但保持 - 中断窗口的仪表板注释。- 包含重新连接 + 耗尽消息的示例日志摘录。 |
