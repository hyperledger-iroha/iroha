...
|----|-------|-----------------|------------------|----------|
| C1 | WebSocket uzilishi va qayta ulanish | `/v2/connect/ws` ni proksi-server orqasiga oʻrang (masalan, `kubectl -n demo port-forward svc/torii 18080:8080` + `toxiproxy-cli toxic add ... timeout`) yoki xizmatni vaqtincha bloklang (≤60s uchun `kubectl scale deploy/torii --replicas=0`). Oflayn navbatlar toʻldirilishi uchun hamyonni kadrlar yuborishda davom eting. | `connect.reconnects_total` oshadi, `connect.resume_latency_ms` ko'tariladi, lekin - O'chirish oynasi uchun asboblar panelidagi izoh.- Qayta ulanish + drenaj xabarlari bilan jurnaldan ko'chirma namunasi. |
