...
### Root views before authenticated Native control composition (September 19, 2026)

Original `status.md` paragraph:

All active Sumeragi edits and validation use `/Users/takemiyamakoto/dev/iroha` on `optimizations`. Combined build27 passes Core/Torii/test-network/Kagami/daemon compilation on 7,221 unchanged Rust/configuration/formal inputs. Its exact executables pass 346 distinct selected runtime controls: 240 composition/output/publication, 59 Native source/economic/recovery/recorder, 17 original-review and 30 Kura storage controls. Native source and execution entries now refuse recorder-owning callers before State access, including suppressed scratch. A local recorder conflict is distinct from an invalid input; State writers precede capture. The contention regression checks nine entries while another thread retains State. Actual signed single/atomic PipelineGas inputs produce one coordinator settlement and complete witness, with real Space Directory roots and exact monetary effects; missing-root metadata refusal discards all economic effects and releases the recorder. Build26 passed 344/346 controls; only the two new missing-manifest fixtures failed because startup correctly removed their hand-seeded derived binding. Build27 derives the index from actual manifest data and checks absent-root refusal. Production and formal sources are byte-identical between those builds. All 118 scoped source/inventory/mutation controls, the two original-review ledger/production joins and three isolated-index closure checks pass. Formatting of 69 changed Rust files and the codec guard pass. These are component checks; the recorded owner grants no ValidBlock, resource, publication or Apply authority.

Original `status.md` paragraph:

All 17 original-review controls pass on build27: complete sealed inputs, actual 800 KiB admissions batched inside a 2 MiB carrier with deferred custody, signed 160 KiB certified gossip within 256 KiB and authenticated completed-repair temporary recovery/tamper checks. Prior compiler, fixture-bound and default-stack failures remain recorded in the liveness specification; no stack override is used. Build14 governance remains 25/26, with authenticated genesis/complete publication still required for the signed-ballot fixture; five earlier lifecycle/merge ownership failures remain. Mandatory Native controls, authenticated suffix-context opening, sponsor/lease relay coverage, geometry/retirement and aggregate resource admission, original Validate-to-Apply custody, the consuming publisher, production cutover and unchanged four/seven-validator qualification remain open. Native Windows namespace durability remains unqualified. Three source-inventory live-index cases remain deselected because the release gate requires the new providers to be tracked; equivalent reviewed closure passes with a separate index, without changing the main index or weakening the rule. No L1–L6 goal is complete.

Original `roadmap.md` paragraph:

The next Sumeragi cutover must carry exact prepared State/resource ownership through validation, cached/recovered markers, voting and Apply; returning only the execution hash drops that owner. Acquire bounded descriptor/capacity resources before voting and preserve typed local deferral with a reachable retry. Prototype530 was withdrawn for post-finality descriptor growth. Retain historical Native authority, stable canonical storage and immutable incarnation directories. Replace mandatory per-route application files with one authenticated carrier proof bundle and bounded complete history references; preserve exact merge-source/finality/WSV joins before source release, and charge whole bundles while any route retains them. Defer physical GC with release, snapshot/recovery pins and an instance-local deletion fence. Preserve cross-route closure and drain/signing fences. Remove the post-finality local Queue veto only with the complete consuming path. Continue all edits and validation in `/Users/takemiyamakoto/dev/iroha` on `optimizations`. Complete and test the aggregate consuming State publisher using the retained journals and prepared MV publications; the current checkout passes 103 MV controls and strict MV library/test Clippy. Complete World and all four runtime journals have scoped publication evidence, and the corrected native opening/driver controls pass. Consume the now-retained ordinary source-prefix owner and exact lease-bound checkpoint receipt with the actual journals. Join the actual canonical validator and original Validate-to-Apply owner to the now-qualified recorded Native replay, retaining the original source groups, one witness and distinct output/metadata cuts. Next extract the actual pristine control owner from `ValidBlock::state_block_for_execution` into the recorded Native constructor and carry the original authenticated applying context through suffix finalization; complete the common AXT/DA/SCCP checks before capture. The Native recorder boundary and genuine single/atomic PipelineGas relay branch now pass runtime qualification; retain their source and lock-order invariants while completing sponsor/lease relay coverage. This Native boundary is not a universal audit of State APIs. Preserve bounded resources and complete publication authority before opening the live Native header gate. Separate retirement observation from maintenance only while preserving original route-directory custody and admitting aggregate evidence/writer resources. Finish bounded capture, file-handle and installation resources before exposing the sole State publication operation. The passing source-custody, exact retry, joint Kura/State and decision-binding controls establish component joins; they do not authorize publication or replace production handoff. Qualify native Windows namespace durability before claiming that storage target. Finish the original Validate-to-Apply custody and capacity policy, authenticated receipt retry and Direct/Merge pre-vote admission. Preserve independent beacon and key-lifecycle behavior during integration. Connect the process-lived shared lane reducer to transport, exact Decision groups and candidate application while retiring the old fresh-signing authority at one complete cutover. Resolve the retained State ownership failures, carrier proof custody and historical work authority, remove unused instance journals, and qualify one unchanged four/seven-validator fault/restart/final-transaction candidate before closing any liveness goal.

### Root views before source-owned Native carrier preparation (September 19, 2026)

Original `status.md` paragraph:

All active Sumeragi edits and validation use `/Users/takemiyamakoto/dev/iroha` on `optimizations`. Combined build29 passes Core/Torii/test-network/Kagami/daemon compilation on 7,037 unchanged Rust/configuration inputs. Its exact executables pass 359 distinct selected runtime controls: 244 composition/output/publication, 68 Native source/control/economic/recovery/recorder, 17 original-review and 30 Kura storage controls. Native recorded execution now consumes the actual pristine QueuePlan/NPoS control owner, requires an authenticated height context joined to the original State, generation, parent and committed policies, and opens pending suffix contexts only through that authority. Shared AXT/DA/SCCP postchecks precede the one witness capture; no-DA cursor validation avoids cache hydration under retained State writers. Proposed and recovered sources retain one complete carrier, including controls, and strict scratch refuses controls it cannot execute. All 133 scoped source/inventory/mutation and original-review binding checks pass on 8,113 unchanged recorded inputs, including all 22 live-index inventory controls with no deselection or index alteration. Formatting of all 15 changed Rust files and the codec guard pass. These are component checks; the recorded owner grants no ValidBlock, resource, publication or Apply authority.

Original `status.md` paragraph:

All 17 original-review controls pass on build29: complete sealed inputs, actual 800 KiB admissions batched inside a 2 MiB carrier with deferred custody, signed 160 KiB certified gossip within 256 KiB and authenticated completed-repair temporary recovery/tamper checks. Prior compiler, fixture-bound and default-stack failures remain recorded in the liveness specification; no stack override is used. Build14 governance remains 25/26, with authenticated genesis/complete publication still required for the signed-ballot fixture; five earlier lifecycle/merge ownership failures remain. Native DA/pin/SCCP work, broader NPoS evidence/penalty and sponsor/lease coverage, geometry/retirement and aggregate resource admission, original Validate-to-Apply custody, the consuming publisher, production cutover and unchanged four/seven-validator qualification remain open. Native Windows namespace durability and full workspace/release qualification remain open. Workspace-wide formatting still reports unrelated existing differences; the changed Rust files pass. No L1–L6 goal is complete.
...
## 2026-09-19 — before the terminal carrier publication checkpoint

All active Sumeragi edits and validation use `/Users/takemiyamakoto/dev/iroha` on `optimizations`. Combined build31 passes Core/Torii/test-network/Kagami/daemon compilation on 7,038 unchanged Rust/configuration inputs. Its exact executables pass 419 distinct selected runtime controls: 244 composition/output/publication, 128 Native source/preparation/signature/economic/recovery/recorder, 17 original-review and 30 Kura storage controls. Native source-owned preparation now runs shared global origin-signature, canonical time, policy, parent, payload and snapshot checks before actual recorded execution. It moves the original verified source groups, applying context, immutable stage, output seal, inventory and witness through the common metadata tail and detached journals. Preflight retains the original State generation: actual finalized height advancement returns observation refresh before classifying obsolete proposal errors. All 149 scoped source/inventory/mutation and original-review binding checks pass on 8,114 unchanged recorded inputs, including all 22 live-index inventory controls without deselection or index alteration. Formatting of all 24 Rust files in this work span and the codec guard pass. These are component checks; live Native admission, aggregate resource admission, publication and Apply remain inactive or incomplete.

All 17 original-review controls pass on build31: complete sealed inputs, actual 800 KiB admissions batched inside a 2 MiB carrier with deferred custody, signed 160 KiB certified gossip within 256 KiB and authenticated completed-repair temporary recovery/tamper checks. Build30 passed compilation and 124/127 Native/signature controls; three new success controls overflowed during fixture setup, before production preparation. Build31 separates fixture construction from assertion frames and passes all eight new controls on the default stack. Its formal bindings also reject the Native-branch mutation missed by build30. Earlier compiler, fixture and governance evidence remains recorded in the liveness specification; no stack override is used. Build14 governance remains 25/26, with authenticated genesis/complete publication still required for the signed-ballot fixture; five earlier lifecycle/merge ownership failures remain. Native DA/pin/SCCP work, broader NPoS evidence/penalty and sponsor/lease coverage, geometry/retirement and aggregate resource admission, original Validate-to-Apply custody, the consuming publisher, production cutover and unchanged four/seven-validator qualification remain open. Native Windows namespace durability and full workspace/release qualification remain open. Workspace-wide formatting still reports unrelated existing differences; the changed Rust files pass. No L1–L6 goal is complete.

The next Sumeragi cutover must carry exact prepared State/resource ownership through validation, cached/recovered markers, voting and Apply; returning only the execution hash drops that owner. Acquire bounded descriptor/capacity resources before voting and preserve typed local deferral with a reachable retry. Prototype530 was withdrawn for post-finality descriptor growth. Retain historical Native authority, stable canonical storage and immutable incarnation directories. Replace mandatory per-route application files with one authenticated carrier proof bundle and bounded complete history references; preserve exact merge-source/finality/WSV joins before source release, and charge whole bundles while any route retains them. Defer physical GC with release, snapshot/recovery pins and an instance-local deletion fence. Preserve cross-route closure and drain/signing fences. Remove the post-finality local Queue veto only with the complete consuming path. Continue all edits and validation in `/Users/takemiyamakoto/dev/iroha` on `optimizations`. Complete and test the aggregate consuming State publisher using the retained journals and prepared MV publications; the current checkout passes 103 MV controls and strict MV library/test Clippy. Complete World and all four runtime journals have scoped publication evidence, and the corrected native opening/driver controls pass. Consume the now-retained ordinary source-prefix owner and exact lease-bound checkpoint receipt with the actual journals. Native preparation now joins the shared global checks to the original source groups, whole carrier, applying context, State generation, actual output seal, one witness, metadata tail and detached journals. Consume that retained authority through production Validate-to-Apply only with complete resource and publication owners; cached views and restart markers must preserve one original execution per exact subject. Keep obsolete State observations as typed refreshes, including a real finalized height advance. The actual pristine QueuePlan/NPoS control owner and common AXT/DA/SCCP postchecks retain their scoped evidence. Complete Native DA/pin/SCCP owners, broader NPoS evidence/penalty and sponsor/lease relay coverage. Preserve the qualified recorder order and genuine single/atomic PipelineGas effects; this Native boundary is not a universal audit of State APIs. Preserve bounded resources and complete publication authority before opening the live Native header gate. Separate retirement observation from maintenance only while preserving original route-directory custody and admitting aggregate evidence/writer resources. Finish bounded capture, file-handle and installation resources before exposing the sole State publication operation. The passing source-custody, exact retry, joint Kura/State and decision-binding controls establish component joins; they do not authorize publication or replace production handoff. Qualify native Windows namespace durability before claiming that storage target. Finish the original Validate-to-Apply custody and capacity policy, authenticated receipt retry and Direct/Merge pre-vote admission. Preserve independent beacon and key-lifecycle behavior during integration. Connect the process-lived shared lane reducer to transport, exact Decision groups and candidate application while retiring the old fresh-signing authority at one complete cutover. Resolve the retained State ownership failures, carrier proof custody and historical work authority, remove unused instance journals, and qualify one unchanged four/seven-validator fault/restart/final-transaction candidate before closing any liveness goal.
