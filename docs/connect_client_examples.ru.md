...
<!-- Русский перевод docs/connect_client_examples.md (Iroha Connect Client Examples) -->

## Примеры клиента Iroha Connect (TypeScript и Kotlin)

В этом документе приведены минимальные клиентские сниппеты, реализующие
правила версии v0:
- AAD для AEAD привязывает внешний заголовок (version, `sid`, `dir`, `seq`,
  `kind=Ciphertext`).
- Nonce выводится из `seq` (12‑байтовый IETF‑nonce:
  `0x00000000 || seq_le`).
- Управляющие фреймы Post‑Approve (Close/Reject) отправляются в зашифрованном
  виде.

Это демонстрационные примеры; проверки и защитные меры уровня production
опущены.

### TypeScript (libsodium + WebCrypto)

Зависимости: `libsodium-wrappers` (X25519, BLAKE2b, ChaCha20‑Poly1305),
...
(async () => { await sodium.ready;
  ...
  // Создание сессии: POST /v2/connect/session с sid, вычисленным на клиенте
...
fun main() {
  ...
  val shared = KeyAgreement.getInstance("X25519").apply {
    init(appKp.private)
    doPhase(appKp.public, true) // заменить на публичный ключ wallet’а
  }.generateSecret()
