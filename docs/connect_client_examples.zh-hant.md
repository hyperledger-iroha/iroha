...
注意事項：
- 客戶端計算 `sid`（32 字節；base64url/hex）並將其 POST 到 `/v2/connect/session` 以獲取一次性令牌；服務器回顯 `sid`。使用 `Authorization: Bearer <token>` 或 `Sec-WebSocket-Protocol: iroha-connect.token.v1.<base64url(token)>` 加入 WS。
