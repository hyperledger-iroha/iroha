...
## TypeScript Connect Wrapper (WS შეერთება + ჟეტონები)
...
```
// დალუქვა SignResultOk (მხოლოდ დატვირთვა; Norito ჩარჩო არ არის ნაჩვენები):
// const signResult = new TextEncoder().encode(JSON.stringify({ SignResultOk: { signature: {ალგორითმი: 'ed25519', signature_hex: 'deadbeef' } } ));
