...
ყოველი კანონიკური დატვირთვა განლაგებულია როგორც `header · domain selector · controller`. The
...
#### 2.2 დომენის ამორჩევის კოდირება (ADDR-1a)

დომენის ამომრჩეველი დაუყოვნებლივ მიჰყვება სათაურს და არის მონიშნული კავშირი:

| ტეგი | მნიშვნელობა | ტვირთამწეობა | შენიშვნები |
|-----|---------|---------|-------|
| `0x00` | იმპლიციტური ნაგულისხმევი დომენი | არცერთი | შეესაბამება კონფიგურირებულ `default_domain_name()`-ს. |
| `0x01` | ლოკალური დომენის დაიჯესტი | 12 ბაიტი | დაიჯესტი = `blake2s_mac(key = "SORA-LOCAL-K:v1", canonical_label)[0..12]`. |
| `0x02` | გლობალური რეესტრის ჩანაწერი | 4 ბაიტი | ბიგ-ენდიანი `registry_id`; დაცულია გლობალური რეესტრის გაგზავნამდე. |

დომენის ლეიბლები კანონიზირებულია (UTS-46 + STD3 + NFC) ჰეშირებამდე. უცნობი ტეგები ზრდის `AccountAddressError::UnknownDomainTag`. დომენის მიმართ მისამართის დადასტურებისას, შეუსაბამო სელექტორები ზრდიან `AccountAddressError::DomainMismatch`-ს.
...
სელექტორი დაუყოვნებლივ არის კონტროლერის დატვირთვის გვერდით, ასე რომ დეკოდერს შეუძლია სიარული
მავთულის ფორმატი თანმიმდევრობით: წაიკითხეთ ტეგის ბაიტი, წაიკითხეთ ტეგის სპეციფიკური დატვირთვა და შემდეგ გადადით
კონტროლერის ბაიტებამდე.

** სელექტორის მაგალითები **

- *იმპლიციტური ნაგულისხმევი* (`tag = 0x00`). დატვირთვის გარეშე. კანონიკური ჰექსის მაგალითი ნაგულისხმევისთვის
  დომენი განმსაზღვრელი ტესტის გასაღების გამოყენებით:
  `0x02000001203b6a27bcceb6a42d62a3a8d02a6f0d73653215771de243a63ac048a18b59da29`.
- *ლოკალური დაიჯესტი* (`tag = 0x01`). Payload არის 12 ბაიტიანი დაიჯესტი. მაგალითი (`treasury` თესლი
  `0x01`): `0x0201b18fe9c1abbac45b3e38fc5d0001208a88e3dd7409f195fd52db2d3cba5d72ca6709bf1d94121bf3748801b40f6f5c`.
- *გლობალური რეესტრი* (`tag = 0x02`). Payload არის დიდი ენდიანი `registry_id:u32`. ბაიტები
  რაც მოჰყვება დატვირთვას, იდენტურია იმპლიციტური ნაგულისხმევი შემთხვევისა; სელექტორი უბრალოდ
  ცვლის დომენის ნორმალიზებულ სტრიქონს რეესტრის მაჩვენებლით. მაგალითის გამოყენებით
  `registry_id = 0x0000_002A` (ათწილადი42) და დეტერმინისტული ნაგულისხმევი კონტროლერი:
  `0x02020000002a000120641297079357229f295938a4b5a333de35069bf47b9d0704e45805713d13c201`.  
  დაშლა: `0x02` სათაური, `0x02` ამომრჩევი თეგი, `00 00 00 2A` რეესტრის ID, `0x00`
  კონტროლერის ტეგი, `0x01` მრუდის ID, `0x20` გასაღების სიგრძე, 32-ბაიტი Ed25519 გასაღების დატვირთვა.
...
- Domainless IH58 (სასურველი)/sora (მეორე საუკეთესო) ლიტერალები წყვეტს ჩაშენებულ სელექტორს დომენის სელექტორის გადამწყვეტის მეშვეობით; თუ არცერთი არ არის დაინსტალირებული (ან სელექტორი ვერ წყდება), ანალიზი ვერ ხერხდება `ERR_DOMAIN_SELECTOR_UNRESOLVED`-ით. იმპლიციტური ნაგულისხმევი სელექტორი გადადის კონფიგურირებულ ნაგულისხმევ დომენის ლეიბლზე, გადამწყვეტის მოთხოვნის გარეშე.

#### 2.5 ნორმატიული ორობითი ვექტორები

- ** ნაგულისხმევი ნაგულისხმევი დომენი (`default`, დაწყებული ბაიტი `0x00`)**  
  კანონიკური ექვსკუთხედი: `0x02000001203b6a27bcceb6a42d62a3a8d02a6f0d73653215771de243a63ac048a18b59da29`.  
...
|-------------|-------------------------------------------- --------------------------------------------|------------|
| ნაგულისხმევი | `0x00` | `0x02000001203b6a27bcceb6a42d62a3a8d02a6f0d73653215771de243a63ac048a18b59da29` | `sora2QGﾈkﾀﾍrNﾒBﾎwﾍwﾙwﾗXHwﾜCﾘﾂY8ryGUﾈﾎyQｲHyヰD8ｲﾁYVY9VF8` |
...
|------------------ |
| `default` დომენი (იმპლიციტური სელექტორი, თესლი `0x00`) | `RnuaJGGDL8HNkN8bwHwBTU32fTWQmbRoM3QZBJintx5RqTU7GgPJmNiA` | `sora2QGﾈkﾀﾍrNﾒBﾎwﾍwﾙwﾗXHwﾜCﾘﾂY8ryGUﾈﾎyQｲHyヰD8ｲﾁYVY9VF8` (არასავალდებულო `@default` სუფიქსი მარშრუტიზაციის მკაფიო მინიშნებების მიწოდებისას) |
...
- **`addr-single-default-ed25519` (Sora Nexus, პრეფიქსი `0x02F1`).**  
  IH58 `RnuaJGGDL8HNkN8bwHwBTU32fTWQmbRoM3QZBJintx5RqTU7GgPJmNiA`, შეკუმშული (`sora`)
...
2. **გამოიყვანეთ კანონიკური დიჯესტები.** გაუშვით
   ...
   `--append-domain` იმეორებს კონვერტირებულ დაშიფვრას, როგორც `<address>@<domain>`
   ...
   `iroha tools address normalize --input <file> --only-local` ლოკალური მასობრივი კონვერტაციისთვის
...
- **CLI tooling:** უზრუნველყოს დეტერმინისტული ოპერატორის სამუშაო ნაკადი `iroha tools address convert`-ის საშუალებით
  ...
  გაანალიზება, ჩაწერს მოწოდებულ დომენს (`input_domain` JSON-ში) და `--append-domain` დროშას
...
- **Torii ინტეგრაცია:** ქეში Nexus გამოხატავს TTL-ს, ემისიას
  ...
  გაამჟღავნეთ `POST /v1/accounts/resolve` `alias@domain`-ის კანონიკალიზაციისთვის,
  `public_key@domain`, `uaid:`/`opaque:` ლიტერალები, ან დაშიფრული მისამართები
  IH58 გადაჭრილი დომენისა და წყაროს დაბრუნებისას.
