...
- შეჯამებული, ადამიანისკენ მიმართული **Iroha Base58 მისამართი (IH58)** მიერ წარმოებული
  `AccountAddress::to_ih58`, რომელიც აკავშირებს ჯაჭვის დისკრიმინანტს ანგარიშთან
...
საფულეები და ჯაჭვის გარეშე ხელსაწყოები ეყრდნობა დღეს `alias@domain` მარშრუტიზაციის ნედლეულ მეტსახელებს. ეს
...
- აღწერეთ IH58 Base58 კონვერტი დანერგილი მონაცემთა მოდელში და
...
(`AccountAddress`), რომელიც შეიძლება გამოიცეს რამდენიმე ადამიანის წინაშე მდგარ ფორმატში. IH58 არის
...
- **IH58 (Iroha Base58)** – Base58 კონვერტი, რომელიც ათავსებს ჯაჭვს
...
- **Sora-შეკუმშული ხედი** – მხოლოდ Sora ანბანი **105 სიმბოლოსგან** აშენებული
  ...
  IH58 კომპლექტი. სტრიქონები იწყება სენტინელით `sora`, ჩაშენებულია Bech32m-დან მიღებული
...
`AccountAddress::parse_any` ავტომატურად ამოიცნობს IH58 (სასურველია), შეკუმშული (`sora`, მეორე საუკეთესო) ან კანონიკურ ექვსკუთხედს
(მხოლოდ `0x...`; შიშველი ექვსკუთხედი უარყოფილია) შეაქვს და აბრუნებს როგორც დეკოდირებულ დატვირთვას, ასევე აღმოჩენილს
`AccountAddressFormat`. Torii ახლა უწოდებს `parse_any`-ს ISO 20022-ის დამატებით
...
- ორობითი კონტროლერის დატვირთვა (`ControllerPayload::Multisig`) კოდირებს
  ...
  `AccountAddress::canonical_bytes()` წერს IH58 (სასურველია)/sora (მეორე საუკეთესო) იტვირთება.
- ჰეშინგი (`MultisigPolicy::digest_blake2b256()`) იყენებს Blake2b-256-ს
  ...
  დეტერმინისტული პოლიტიკის ID, რომელიც ემთხვევა IH58-ში ჩაშენებულ კონტროლერის ბაიტებს.
- მოწყობილობების დაფარვა ცხოვრობს `fixtures/account/address_vectors.json`-ში (ქეისები
  `addr-multisig-*`). საფულეები და SDK-ები უნდა ამტკიცებდნენ კანონიკურ IH58 სტრიქონებს
  ქვემოთ, რათა დაადასტურონ, რომ მათი შიფრები ემთხვევა Rust-ის განხორციელებას.

| საქმის ID | ბარიერი / წევრები | IH58 ლიტერალი (პრეფიქსი `0x02F1`) | სორა შეკუმშული (`sora`) ლიტერალური | შენიშვნები |
|---------|--------------------------------------------------|-----------------------|
...
- Torii და `address@domain` მეტსახელების Torii და SDK ანალიზები ახლა ასხივებენ იგივე `ERR_*` კოდებს, როდესაც IH58 (სასურველია)/სორა (მეორე საუკეთესო) შეყვანები ვერ მოხერხდება, სანამ მეტსახელის შეიძლება ხელახლა გამოაცხადოს დომენის შეცდომის მიზეზები (მაგ. პროზაული სიმებიდან გამოცნობის გარეშე.
...
- **ლოკალური დომენის დაიჯესტი (`treasury`, დაწყებული ბაიტი `0x01`)**  
  ...
  დაყოფა: `0x02` სათაური, ამომრჩევი ტეგი `0x01` პლუს დაიჯესტი `b1 8f e9 c1 ab ba c4 5b 3e 38 fc 5d`, რასაც მოჰყვება ერთი გასაღების დატვირთვა (`0x00` ტეგი, I18NI000002861, სიგრძე I2080X 32-ბაიტი Ed25519 გასაღები).ერთეულის ტესტები (`account::address::tests::parse_any_accepts_all_formats`) ამტკიცებს V1 ვექტორებს ქვემოთ `AccountAddress::parse_any`-ის მეშვეობით, რაც გარანტიას იძლევა, რომ ინსტრუმენტები შეიძლება დაეყრდნოს კანონიკურ დატვირთვას ექვსკუთხა, IH58 (სასურველი) და შეკუმშული (`sora`, მეორე საუკეთესო) ფორმებით. განაახლეთ გაფართოებული მოწყობილობების ნაკრები `cargo run -p iroha_data_model --example address_vectors`-ით.
...
ამიტომ `AccountAddress::to_ih58` და `to_compressed_sora` დამხმარეები ასხივებენ
...
| ანგარიში / სელექტორი | IH58 ლიტერალი (პრეფიქსი `0x02F1`) | სორა შეკუმშული (`sora`) ლიტერალური |
|------------------ |
...
პასუხები (`address_format=ih58|compressed`) და SDK დამხმარეები, ამიტომ UX დააკოპირეთ/ჩასვით
ნაკადებს შეუძლიათ სიტყვასიტყვით დაეყრდნონ მათ. დაუმატეთ `<address>@<domain>` მხოლოდ მაშინ, როდესაც გჭირდებათ მკაფიო მარშრუტიზაციის მინიშნება; სუფიქსი არ არის კანონიკური გამომავალი ნაწილი.
...
- **ჯაჭვის ალიასის სტილი:** `ih:<chain-alias>:<alias@domain>` მორებისა და ადამიანებისთვის
  ...
  შეუსაბამობები.
- **CAIP-10 ფორმა:** `iroha:<caip-2-id>:<ih58-addr>` ჯაჭვის აგნოსტიკისთვის
...
- **მანქანის დამხმარეები:** გამოაქვეყნეთ კოდეკები Rust, TypeScript/JavaScript, Python,
  და კოტლინი, რომელიც მოიცავს IH58 და შეკუმშულ ფორმატებს (`AccountAddress::to_ih58`,
  `AccountAddress::parse_any` და მათი SDK ეკვივალენტები). CAIP-10 დამხმარეები არიან
  მომავალი სამუშაო.

#### 2.7 დეტერმინისტული IH58 მეტსახელი

- **პრეფიქსის რუქა:** ხელახლა გამოიყენეთ `chain_discriminant`, როგორც IH58 ქსელის პრეფიქსი.
  `encode_ih58_prefix()` (იხ. `crates/iroha_data_model/src/account/address.rs`)
...
- **ანგარიშის მასალა:** IH58 შიფრავს კანონიკურ დატვირთვას, რომელიც აშენებულია
  `AccountAddress::canonical_bytes()` — სათაურის ბაიტი, დომენის ამომრჩევი და
  კონტროლერის დატვირთვა. არ არსებობს დამატებითი ჰეშირების ნაბიჯი; IH58 ათავსებს
...
- ** დაშიფვრა:** `encode_ih58()` აერთიანებს პრეფიქსის ბაიტებს კანონიკურს
  payload და აერთებს 16-ბიტიან საკონტროლო ჯამს, რომელიც მიღებულია Blake2b-512-დან ფიქსირებული
  პრეფიქსი `IH58PRE` (`b"IH58PRE" || prefix || payload`). შედეგი არის Base58-ში კოდირებული `bs58`-ის მეშვეობით.
  CLI/SDK დამხმარეები ავლენენ იმავე პროცედურას და `AccountAddress::parse_any`
  აბრუნებს მას `decode_ih58`-ის საშუალებით.

#### 2.8 ნორმატიული ტექსტური ტესტის ვექტორები

`fixtures/account/address_vectors.json` შეიცავს სრულ IH58 (სასურველია) და შეკუმშული (`sora`, მეორე საუკეთესო)
ლიტერალები ყოველი კანონიკური დატვირთვისთვის. მაჩვენებლები:

- **`addr-single-default-ed25519` (Sora Nexus, პრეფიქსი `0x02F1`).**  
  IH58 `6cmzPVPX5jDQFNfiz6KgmVfm1fhoAqjPhoPFn4nx9mBWaFMyUCwq4cw`, შეკუმშული (`sora`)
  `sora2QG…U4N5E5`. Torii ასხივებს ზუსტად ამ სტრიქონებს `AccountId`-დან
  `Display` განხორციელება (კანონიკური IH58) და `AccountAddress::to_compressed_sora`.
- **`addr-global-registry-002a` (რეესტრის სელექტორი → ხაზინა).**  
  IH58 `3oE9sLeRGP49Cu7mQ1nF4wtKAm29BG4TGLiRsaXe7mhbMP5WZ113nNW1N6RbqF`, შეკუმშული (`sora`)
...
- **შეცდომის შემთხვევა (`ih58-prefix-mismatch`).**  
  IH58 ლიტერალის ანალიზი, რომელიც კოდირებულია პრეფიქსით `NETWORK_PREFIX + 1` კვანძზე
  ველოდები ნაგულისხმევი პრეფიქსის გამომუშავებას
  `AccountAddressError::UnexpectedNetworkPrefix { expected: 753, found: 754 }`
  დომენის მარშრუტიზაციის მცდელობამდე. `ih58-checksum-mismatch` მოწყობილობა
...
სცენარები კანონიკურ ექვსკუთხედში, IH58 (სასურველია), შეკუმშული (`sora`, ნახევარი/სრული სიგანე), იმპლიციტური
...
|------|---------|-----------------|
| `global_domain` | აცხადებს, რომ დომენი რეგისტრირებულია გლობალურად და უნდა განისაზღვროს ჯაჭვის დისკრიმინატორთან და IH58 პრეფიქსით. | `{ "domain": "<label>", "chain": "sora:nexus:global", "ih58_prefix": 753, "selector": "global" }` |
...
`{domain, chain, discriminant/ih58_prefix}`. `tombstone` ჩანაწერები **აუცილებელი ** ციტირება
...
2. **გამოიყვანეთ კანონიკური დიჯესტები.** გაუშვით
   ...
   CLI იღებს IH58, `sora…` და კანონიკურ `0x…` ლიტერალებს; დაურთოს
   ...
   `legacy  suffix` იმეორებს კონვერტირებულ დაშიფვრას, როგორც `<address>@<domain>`
   ...
   ამომრჩეველები კანონიკურ IH58 (სასურველი), შეკუმშული (`sora`, მეორე საუკეთესო), თექვსმეტობითი ან JSON ფორმებად გამოტოვებისას
...
4. **გადაამოწმეთ და გამოაქვეყნეთ.** მიჰყევით runbook-ის საკონტროლო სიას (ჰეშები, Sigstore,
   ...
   ახდენს IH58 (სასურველი)/სორას (მეორე-საუკეთესო) ლიტერალების კანონიკიზაციას შეკვრის დაშვებისთანავე.
...
- ** ნაგულისხმევი ჩვენება: ** საფულეები აჩვენებს IH58 მისამართს (მოკლე, შეჯამებული)
  პლუს გადაჭრილი დომენი, როგორც რეესტრიდან მოტანილი ლეიბლი. დომენები არის
  ნათლად აღინიშნება, როგორც აღწერილობითი მეტამონაცემები, რომლებიც შეიძლება შეიცვალოს, ხოლო IH58 არის
  სტაბილური მისამართი.
- **შეყვანის კანონიკიზაცია:** Torii და SDK-ები მიიღებენ IH58 (სასურველია)/sora (მეორე საუკეთესო)/0x
  მისამართები პლუს `alias@domain`, `public_key@domain`, `uaid:…` და
  `opaque:…` ფორმირდება, შემდეგ ხდება კანონიკიზაცია IH58-ზე გამოსასვლელად. არ არსებობს
...
- **შეცდომის პრევენცია:** საფულეები აანალიზებენ IH58 პრეფიქსებს და აიძულებენ ჯაჭვის დისკრიმინაციას
...
- **კოდეკების ბიბლიოთეკები:** ოფიციალური Rust, TypeScript/JavaScript, Python და Kotlin
  ბიბლიოთეკები უზრუნველყოფენ IH58 კოდირების/გაშიფვრის პლუს შეკუმშული (`sora`) მხარდაჭერას
...
- **ეკრანის წამკითხველის მხარდაჭერა:** უზრუნველყოს ვიზუალურად დამალული ლეიბლები (`aria-label`/`aria-describedby`), რომლებიც აღწერს Base58 პრეფიქსის მთავარ ციფრებს და ანაწილებს IH58 დატვირთვას 4 ან 8-სიმბოლოიან ჯგუფებად, ასე რომ, ჯგუფური დამხმარე ტექნოლოგია იკითხება. გამოაცხადეთ ასლის/გაზიარების წარმატება თავაზიანი ცოცხალი რეგიონების მეშვეობით და დარწმუნდით, რომ QR წინასწარი გადახედვები შეიცავს აღწერით ალტერნატიულ ტექსტს („IH58 მისამართი <alias> ჯაჭვზე 0x02F1“).
- **მხოლოდ Sora-ზე შეკუმშული გამოყენება:** ყოველთვის მონიშნეთ `sora…` შეკუმშული ხედი, როგორც „მხოლოდ Sora“ და დააკოპირეთ ის აშკარა დადასტურების უკან. SDK-ებმა და საფულეებმა უარი უნდა თქვან შეკუმშული გამოსავლის ჩვენებაზე, როდესაც ჯაჭვის დისკრიმინანტი არ არის Sora Nexus მნიშვნელობა და უნდა მიმართონ მომხმარებლებს უკან IH58-ზე ქსელში გადარიცხვისთვის, რათა თავიდან იქნას აცილებული თანხების არასწორი მარშრუტი.

## განხორციელების ჩამონათვალი

- **IH58 კონვერტი:** პრეფიქსი კოდირებს `chain_discriminant`-ს კომპაქტურის გამოყენებით
  6-/14-ბიტიანი სქემა `encode_ih58_prefix()`-დან, სხეული არის კანონიკური ბაიტი
  (`AccountAddress::canonical_bytes()`) და საკონტროლო ჯამი არის პირველი ორი ბაიტი
  Blake2b-512-ის (`b"IH58PRE"` || პრეფიქსი || სხეული). სრული დატვირთვა არის Base58-
  კოდირებულია `bs58`-ით.
- ** რეესტრის კონტრაქტი: ** ხელმოწერილი JSON (და სურვილისამებრ Merkle root) გამოცემა
  `{discriminant, ih58_prefix, chain_alias, endpoints}` 24 სთ TTL და
  ბრუნვის გასაღებები.
- **დომენის პოლიტიკა:** ASCII `Name` დღეს; თუ ჩართულია i18n, გამოიყენეთ UTS-46 ამისთვის
  ...
  მთლიანი (255) სიგრძე.
- **ტექსტური დამხმარეები:** გემი IH58 ↔ შეკუმშული (`sora…`) კოდეკები Rust-ში,
...
- **CLI tooling:** უზრუნველყოს დეტერმინისტული ოპერატორის სამუშაო ნაკადი `iroha tools address convert`-ის საშუალებით
  (იხ. `crates/iroha_cli/src/address.rs`), რომელიც იღებს IH58/`sora…`/`0x…` ლიტერალებს და
  არასავალდებულო `<address>@<domain>` ეტიკეტები, ნაგულისხმევი IH58 გამომავალი Sora Nexus პრეფიქსის გამოყენებით (`753`),
  და ასხივებს მხოლოდ Sora-ზე შეკუმშულ ანბანს, როდესაც ოპერატორები პირდაპირ ითხოვენ მას
  `--format compressed` ან JSON შემაჯამებელი რეჟიმი. ბრძანება ახორციელებს პრეფიქსის მოლოდინებს
  გაანალიზება, ჩაწერს მოწოდებულ დომენს (`input_domain` JSON-ში) და `legacy  suffix` დროშას
  იმეორებს კონვერტირებულ დაშიფვრას, როგორც `<address>@<domain>`, რათა მანიფესტაციური განსხვავებები დარჩეს ერგონომიული.
- **Wallet/explorer UX:** მიჰყევით [მისამართების ჩვენების მითითებებს] (source/sns/address_display_guidelines.md)
  გაიგზავნება ADDR-6-ით — გთავაზობთ ორმაგი ასლის ღილაკებს, შეინახეთ IH58 QR დატვირთვად და გააფრთხილეთ
  მომხმარებლები, რომ შეკუმშული `sora…` ფორმა არის მხოლოდ Sora და მგრძნობიარეა IME გადაწერებისთვის.
...
- `GET /v1/accounts` იღებს არასავალდებულო `address_format` მოთხოვნის პარამეტრს და
  `POST /v1/accounts/query` იღებს იმავე ველს JSON კონვერტში.
  მხარდაჭერილი მნიშვნელობებია:
  - `ih58` (ნაგულისხმევი) — პასუხები ასხივებენ კანონიკურ IH58 Base58 დატვირთვას (მაგ.
    `6cmzPVPX5jDQFNfiz6KgmVfm1fhoAqjPhoPFn4nx9mBWaFMyUCwq4cw`).
  - `compressed` — პასუხები გამოსცემს მხოლოდ Sora-ს `sora…` შეკუმშულ ხედს, სანამ
...
- არასწორი მნიშვნელობები ბრუნდება `400` (`QueryExecutionFail::Conversion`). ეს საშუალებას იძლევა
  ...
  IH58-ის შენახვა თავსებადი ნაგულისხმევად.
- აქტივების მფლობელთა ჩამონათვალი (`GET /v1/assets/{definition_id}/holders`) და მათი JSON
  კონვერტის კოლეგა (`POST …/holders/query`) ასევე პატივს სცემს `address_format`.
  `items[*].account_id` ველი ასხივებს შეკუმშულ ლიტერალებს, როდესაც
  პარამეტრი/კონვერტის ველი დაყენებულია `compressed`, რომელიც ასახავს ანგარიშებს
...
| `ERR_INVALID_IH58_PREFIX` | მოთხოვნილი IH58 ქსელის პრეფიქსის კოდირება შეუძლებელია. | აირჩიეთ პრეფიქსი ჯაჭვის რეესტრში გამოქვეყნებულ `0..=16383` დიაპაზონში. |
...
| კოდი | წარუმატებლობა | რეკომენდირებული გამოსწორება |
|------|---------|------------------------|
| `ERR_INVALID_IH58_ENCODING` | IH58 სტრიქონი შეიცავს სიმბოლოებს ანბანის მიღმა. | დარწმუნდით, რომ მისამართი იყენებს გამოქვეყნებულ IH58 ანბანს და არ არის შეკვეცილი კოპირების/პასტის დროს. |
...
| `ERR_CHECKSUM_MISMATCH` | IH58 (სასურველი) ან შეკუმშული (`sora`, მეორე საუკეთესო) საკონტროლო ჯამის ვალიდაცია ვერ მოხერხდა. | მისამართის რეგენერაცია სანდო წყაროდან; ეს ჩვეულებრივ მიუთითებს ასლის/პასტის შეცდომაზე. |
| `ERR_INVALID_IH58_PREFIX_ENCODING` | IH58 პრეფიქსი ბაიტები არასწორია. | მისამართის ხელახლა დაშიფვრა შესაბამისი ენკოდერით; არ შეცვალოთ წამყვანი Base58 ბაიტი ხელით. |
...
| `ERR_UNSUPPORTED_ADDRESS_FORMAT` | ავტომატური ამოცნობა ვერ ცნობს შეყვანის ფორმატს. | პარსერების გამოძახებისას მიუთითეთ IH58 (სასურველი), შეკუმშული (`sora`) ან კანონიკური `0x` თექვსმეტობითი სტრიქონები. |
...
| `ERR_UNEXPECTED_NETWORK_PREFIX` | დეკოდირებული IH58 ქსელის პრეფიქსი განსხვავდება კონფიგურირებული მნიშვნელობისაგან. | გადაერთეთ მისამართზე სამიზნე ჯაჭვიდან ან შეცვალეთ მოსალოდნელი დისკრიმინანტი/პრეფიქსი. |
...
- **Pure Base58Check (ბიტკოინის სტილი).** უფრო მარტივი საკონტროლო ჯამი, მაგრამ უფრო სუსტი შეცდომის გამოვლენა
  ვიდრე Blake2b-დან მიღებული IH58 საკონტროლო ჯამი (`encode_ih58` წყვეტს 512-ბიტიან ჰეშს)
...
- **Bech32m კონვერტი.** QR-მეგობრული და გთავაზობთ ადამიანისათვის წასაკითხ პრეფიქსს, მაგრამ
  განსხვავდებოდა გადაზიდვის IH58 განხორციელებისგან (`AccountAddress::to_ih58`)
  და მოითხოვს ყველა მოწყობილობის/SDK-ის ხელახლა შექმნას. მიმდინარე საგზაო რუკა ინახავს IH58 +
...
- მიუთითეთ, როგორ აფორმებს Kotodama/IVM კონტრაქტებს წვდომა IH58 დამხმარეებზე (`to_address()`,
  `parse_address()`) და უნდა გამოამჟღავნოს თუ არა CAIP-10-ის ჯაჭვზე შენახვა
  რუკები (დღეს IH58 კანონიკურია).
- გამოიკვლიეთ Iroha ჯაჭვების რეგისტრაცია გარე რეესტრებში (მაგ., IH58 რეესტრი,
  CAIP სახელთა სივრცის დირექტორია) ეკოსისტემის უფრო ფართო გასწორებისთვის.

## შემდეგი ნაბიჯები

1. IH58 კოდირება დაეშვა `iroha_data_model`-ში (`AccountAddress::to_ih58`,
   `parse_any`); განაგრძეთ მოწყობილობების/ტესტების პორტირება ყველა SDK-ზე და გაასუფთავეთ ნებისმიერი
