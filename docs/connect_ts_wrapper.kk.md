...
## TypeScript Connect Wrapper (WS қосылу + таңбалауыштар)
...
```
// Sealing SignResultOk (тек пайдалы жүктеме; Norito жақтауы көрсетілмеген):
// const signResult = new TextEncoder().encode(JSON.stringify({ SignResultOk: { қолтаңба: { алгоритм: 'ed25519', signature_hex: 'deadbeef' } } }));
// const { aead } = sealEnvelope күту(kWallet, sidBytes, 'W2A', 1n, signResult);