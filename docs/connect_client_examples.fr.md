...
<!-- Traduction française de docs/connect_client_examples.md (Iroha Connect Client Examples) -->

## Exemples de Client Iroha Connect (TypeScript et Kotlin)

Ce document présente des snippets minimalistes côté client implémentant les
règles v0 :
- L’AAD de l’AEAD lie l’en‑tête externe (version, `sid`, `dir`, `seq`,
  `kind=Ciphertext`).
- Le nonce est dérivé de `seq` (nonce IETF 12 octets :
  `0x00000000 || seq_le`).
- Les frames de contrôle Post‑Approve (Close/Reject) sont envoyées chiffrées.

Ces exemples sont illustratifs ; les vérifications et durcissements de
production sont volontairement omis.

### TypeScript (libsodium + WebCrypto)

Dépendances : `libsodium-wrappers` (X25519, BLAKE2b, ChaCha20‑Poly1305),
...
fun main() {
  ...
  val shared = KeyAgreement.getInstance("X25519").apply {
    init(appKp.private)
    doPhase(appKp.public, true) // remplacer par la clé publique du wallet
  }.generateSecret()
