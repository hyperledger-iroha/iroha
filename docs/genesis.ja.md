...
- **パラメータ:** Sumeragi（block/commit 時間、許容ドリフト）、Block（最大トランザクション数）、Transaction（最大命令数・バイトコードサイズ）、Executor／スマートコントラクトの制限（燃料、メモリ、深さ）などを初期化。`Sumeragi::NextMode` と `sumeragi_npos_parameters`（選出、再構成）は `parameters` ブロックでシードされ、署名時に `SetParameter` 命令としてジェネシスブロックに反映されます。
