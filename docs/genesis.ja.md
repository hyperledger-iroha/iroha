...
- `consensus_mode` – マニフェストに記載するコンセンサスモード。必須です。Sora Nexus の public dataspace では `"Npos"` を使用し、他の Iroha3 dataspaces では `"Permissioned"` または `"Npos"` を使います。Iroha2 は `"Permissioned"` が既定です。
...
1. テンプレートを生成:
  ...
  ```
   `--consensus-mode` は Kagami が `parameters` ブロックにシードするコンセンサスパラメータを制御します。Sora Nexus の public dataspace では `npos` が必須で段階的な切り替えはサポートされません。他の Iroha3 dataspaces では `permissioned` または `npos` を選べます。Iroha2 は `permissioned` が既定で、必要に応じて `--next-consensus-mode`/`--mode-activation-height` で `npos` を段階導入できます。`npos` を指定すると `sumeragi_npos_parameters` がシードされ、NPoS pacemaker のタイミング、collector の fan-out、選出ポリシー、再構成ウィンドウを制御します。正規化/署名時に `SetParameter` 命令としてブロックへ反映されます。
...
- **パラメータ:** Sumeragi（block/commit 時間、許容ドリフト）、Block（最大トランザクション数）、Transaction（最大命令数・バイトコードサイズ）、Executor／スマートコントラクトの制限（燃料、メモリ、深さ）などを初期化。`Sumeragi::NextMode` と `sumeragi_npos_parameters`（NPoS タイミング、選出、再構成）は `parameters` ブロックでシードされ、署名時に `SetParameter` 命令としてジェネシスブロックに反映されます。
...
- Kagami でテンプレートを作成:
  - 組み込み ISI のみ:  
    `kagami genesis generate --ivm-dir <dir> --genesis-public-key <PK> --consensus-mode npos > genesis.json`（Sora Nexus public dataspace。Iroha2 または private Iroha3 は `--consensus-mode permissioned`）
