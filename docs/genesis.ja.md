...
1. テンプレートを生成:
   ...
   ```
   `--consensus-mode` は Kagami が `parameters` ブロックにシードするコンセンサスパラメータを制御します。従来どおりのパーミッションドネットワークには `permissioned`（既定値）、NPoS のパラメータを初期投入したい場合は `npos` を指定してください。`Sumeragi::NextMode` と `sumeragi_npos_parameters` は `parameters` に格納され、正規化/署名時に `SetParameter` 命令としてブロックへ反映されます。
...
- **トポロジ:** 任意のトランザクション（通常は最初か最後）で `topology` 配列に PeerId リストを指定し、初期ピア集合を提供できます。
