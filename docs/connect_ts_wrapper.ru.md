...
<!-- Русский перевод docs/connect_ts_wrapper.md (TypeScript Connect Wrapper) -->

## TypeScript‑обёртка для Connect (WS join + токены)
...
// // После вывода ключей и построения ConnectFrameV1 (закодированного в Norito) отправьте:
// ws.send(noritoEncodedBinaryFrame);
```

```ts
// Шифрование SignResultOk (только payload; Norito‑фрейминг не показан):
