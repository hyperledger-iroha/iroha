...
## Тип Скрипт тоташтырыу урау (WS ҡушыла + токендар)
...
```
// Нөктәле SignResultOk (тик түләү генә; I18NT000000001X рамка күрһәтелмәгән):
// const sightResult = яңы TextEncoder().
// const { aead } = герметизацияны көтөп (кУоллет, sidBytes, 'W2A', 1n, imageResult);