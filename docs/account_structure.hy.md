...
Յուրաքանչյուր կանոնական ծանրաբեռնվածություն դրված է որպես `header · domain selector · controller`: Այն
...
#### 2.2 Դոմենի ընտրիչի կոդավորումներ (ADDR-1a)

Դոմենի ընտրիչը անմիջապես հետևում է վերնագրին և պիտակավորված միություն է.

| Պիտակ | Իմաստը | Օգտակար բեռ | Ծանոթագրություններ |
|-----|---------|---------|-------|
| `0x00` | Լռելյայն լռելյայն տիրույթ | ոչ մեկը | Համապատասխանում է կազմաձևված `default_domain_name()`-ին: |
| `0x01` | Տեղական տիրույթի ամփոփում | 12 բայթ | Digest = `blake2s_mac(key = "SORA-LOCAL-K:v1", canonical_label)[0..12]`: |
| `0x02` | Համաշխարհային ռեգիստրի մուտքագրում | 4 բայթ | Big-Endian `registry_id`; վերապահված է մինչև համաշխարհային ռեգիստրի առաքումը: |

Դոմենի պիտակները կանոնականացվում են (UTS-46 + STD3 + NFC) նախքան հեշավորումը: Անհայտ պիտակները բարձրացնում են `AccountAddressError::UnknownDomainTag`: Հասցեն տիրույթի հետ վավերացնելիս անհամապատասխան ընտրիչները բարձրացնում են `AccountAddressError::DomainMismatch`:
...
Ընտրիչը անմիջապես հարում է կարգավորիչի օգտակար բեռին, այնպես որ ապակոդավորիչը կարող է քայլել
հաղորդալարի ձևաչափը ըստ հերթականության. կարդալ պիտակի բայթը, կարդալ պիտակի համար նախատեսված օգտակար բեռը, այնուհետև առաջ շարժվել
վերահսկիչի բայթերին:

**Ընտրողի օրինակներ**

- *Լռելյայն լռելյայն* (`tag = 0x00`): Առանց ծանրաբեռնվածության: Կանոնական վեցանկյունի օրինակ լռելյայն
  տիրույթ՝ օգտագործելով դետերմինիստական թեստային բանալին.
  `0x02000001203b6a27bcceb6a42d62a3a8d02a6f0d73653215771de243a63ac048a18b59da29`.
- *Տեղական մարսողություն* (`tag = 0x01`): Օգտակար բեռը 12 բայթանոց ամփոփումն է: Օրինակ (`treasury` սերմ
  `0x01`): `0x0201b18fe9c1abbac45b3e38fc5d0001208a88e3dd7409f195fd52db2d3cba5d72ca6709bf1d94121bf3748801b40f6f5c`:
- *Գլոբալ ռեգիստր* (`tag = 0x02`): Payload-ը մեծ էնդիական `registry_id:u32` է: Բայթերը
  որոնք հետևում են ծանրաբեռնվածությանը, նույնական են անուղղակի լռելյայն գործին. ընտրիչը պարզապես
  փոխարինում է նորմալացված տիրույթի տողը ռեեստրի ցուցիչով: Օրինակ օգտագործելով
  `registry_id = 0x0000_002A` (տասնորդական42) և դետերմինիստական լռելյայն վերահսկիչ.
  `0x02020000002a000120641297079357229f295938a4b5a333de35069bf47b9d0704e45805713d13c201`.  
  Բաշխում. `0x02` վերնագիր, `0x02` ընտրիչ պիտակ, `00 00 00 2A` ռեեստրի ID, `0x00`
  կարգավորիչի պիտակ, `0x01` կորի id, `0x20` ստեղնի երկարություն, 32 բայթ Ed25519 ստեղնաշարի օգտակար բեռ:
...
- Առանց տիրույթի IH58 (նախընտրելի)/sora (երկրորդ լավագույն) բառացիները լուծում են ներկառուցված ընտրիչը տիրույթի ընտրիչ լուծիչի միջոցով; եթե ոչ մեկը տեղադրված չէ (կամ ընտրիչը չի կարող լուծվել), ապա վերլուծությունը ձախողվում է `ERR_DOMAIN_SELECTOR_UNRESOLVED`-ով: Անուղղակի լռելյայն ընտրիչն անցնում է կազմաձևված լռելյայն տիրույթի պիտակին՝ առանց լուծիչ պահանջելու:

#### 2.5 Նորմատիվ երկուական վեկտորներ

- **Նախադրված լռելյայն տիրույթ (`default`, սկզբնական բայթ `0x00`)**  
  Կանոնական վեցանկյուն՝ `0x02000001203b6a27bcceb6a42d62a3a8d02a6f0d73653215771de243a63ac048a18b59da29`:  
...
|------------------------------------------------------------ -----------------------------------------------------------|
| լռելյայն | `0x00` | `0x02000001203b6a27bcceb6a42d62a3a8d02a6f0d73653215771de243a63ac048a18b59da29` | `sora2QGﾈkﾀﾍrNﾒBﾎwﾍwﾙwﾗXHwﾜCﾘﾂY8ryGUﾈﾎyQｲHyヰD8ｲﾁYVY9VF8` |
...
|--------------------------------------------------------------------------------------|
| `default` տիրույթ (իմպլիցիտ ընտրիչ, սերմ `0x00`) | `RnuaJGGDL8HNkN8bwHwBTU32fTWQmbRoM3QZBJintx5RqTU7GgPJmNiA` | `sora2QGﾈkﾀﾍrNﾒBﾎwﾍwﾙwﾗXHwﾜCﾘﾂY8ryGUﾈﾎyQｲHyヰD8ｲﾁYVY9VF8` (ըստ ցանկության `@default` վերջածանց, երբ հստակ երթուղային ակնարկներ է տրվում) |
...
- **`addr-single-default-ed25519` (Sora Nexus, նախածանց `0x02F1`):**  
  IH58 `RnuaJGGDL8HNkN8bwHwBTU32fTWQmbRoM3QZBJintx5RqTU7GgPJmNiA`, սեղմված (`sora`)
...
2. **Ստեղծիր կանոնական մարսողություններ։** Վազիր
   ...
   `--append-domain`-ը վերարտադրում է փոխարկված կոդավորումը որպես `<address>@<domain>`
   manifest diffs (այս վերջածանցը մետատվյալ է, այլ ոչ թե կանոնական հաշվի ID):
   Նոր գծի վրա ուղղված արտահանման համար օգտագործեք
   `iroha tools address normalize --input <file> --only-local` տեղական զանգվածային փոխակերպման համար
...
- **CLI գործիքավորում. ** Տրամադրեք օպերատորի դետերմինիստական աշխատանքային հոսք `iroha tools address convert`-ի միջոցով
  ...
  վերլուծել, գրանցել տրամադրված տիրույթը (`input_domain` JSON-ում) և `--append-domain` դրոշակը
...
- **Torii ինտեգրում.** Cache Nexus դրսևորվում է TTL-ի նկատմամբ, արտանետում
  ...
  բացահայտել `POST /v1/accounts/resolve`՝ `alias@domain` կանոնականացնելու համար,
  `public_key@domain`, `uaid:`/`opaque:` բառացի կամ կոդավորված հասցեներ
  IH58 լուծված տիրույթը և աղբյուրը վերադարձնելիս:
