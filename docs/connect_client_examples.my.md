...
မှတ်စုများ-
- Client သည် `sid` (32 bytes; base64url/hex) ကိုတွက်ချက်ပြီး `/v2/connect/session` သို့ တစ်ကြိမ်တည်းတိုကင်များရရှိရန်၊ ဆာဗာသည် `sid` ကို ပဲ့တင်ထပ်သည်။ `Authorization: Bearer <token>` သို့မဟုတ် `Sec-WebSocket-Protocol: iroha-connect.token.v1.<base64url(token)>` ဖြင့် WS ချိတ်ဆက်ပါ။
