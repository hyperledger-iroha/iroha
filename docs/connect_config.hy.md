...
- Սահմանեք `connect.enabled=false`՝ անջատելու բոլոր Connect երթուղիները (`/v2/connect/*`):
- Թողեք այն `true` (կանխադրված)՝ WS նստաշրջանի վերջնակետերը և `/v2/connect/status`-ը միացնելու համար:
...
- Երբ գործարկման ժամանակ անջատված է (`connect.enabled=false`), Միացեք WS-ը և կարգավիճակի երթուղիները չեն
  գրանցված; հարցումները դեպի `/v2/connect/ws` և `/v2/connect/status` վերադարձնել 404:
- Սերվերը պահանջում է հաճախորդի կողմից տրամադրված `/v2/connect/session`-ի համար նախատեսված `sid` (base64url կամ hex, 32 բայթ):
