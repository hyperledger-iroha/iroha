...
- `AccountId` - kanonik manzillar `AccountAddress` (I105 / `i105` siqilgan / hex) orqali ishlab chiqariladi va Torii `AccountAddress::parse_encoded` orqali kirishlarni normallashtiradi. I105 - afzal qilingan hisob formati; `i105` shakli Sora-faqat UX uchun ikkinchi o'rinda turadi. Tanish `alias` (rejected legacy form) qatori faqat marshrutlash taxallus sifatida saqlanadi. Hisob: `{ id, metadata }`. Kod: `crates/iroha_data_model/src/account.rs`.
...
Turlari: `Grant<O, D>` va `Revoke<O, D>`, `Permission`/`Role` dan `Account` gacha va `alias` (rejected legacy form)02020202020 uchun qutilarga o'rnatilgan raqamlar bilan.
