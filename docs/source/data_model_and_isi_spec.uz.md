...
- `AssetId`: kanonik kodlangan literal `<base58-asset-id>#<katakana-i105-account-id>` (eski matn shakllari birinchi versiyada qo'llab-quvvatlanmaydi).- `NftId` — `nft$domain`. NFT: `{ id, content: Metadata, owned_by }`. Kod: `crates/iroha_data_model/src/nft.rs`.
...
- Yalpish/yonish/o'tkazish chegaralaridagi aktiv identifikatorlari `<base58-asset-id>#<katakana-i105-account-id>` kanonik bo'lib qoladi; `iroha tools encode asset-id` bilan `--definition <base58-asset-definition-id>` yoki `--alias ...` plus `--account` dan foydalaning.
