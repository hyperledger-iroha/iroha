...
- `AssetId` — `asset#domain#account` или `asset##account`, если домены совпадают, где `account` — каноническая строка `AccountId` (предпочтительно IH58). Актив: `{ id, value: Numeric }`. Код: `crates/iroha_data_model/src/asset/{id.rs,value.rs}`.
