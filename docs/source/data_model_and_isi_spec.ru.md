...
- `AccountId` — канонические адреса создаются через `AccountAddress` (I105 / `i105` сжатый/шестнадцатеричный), а Torii нормализует входные данные через `AccountAddress::parse_encoded`. I105 — предпочтительный формат учетной записи; Форма `i105` является второй лучшей для UX только для Sora. Знакомая строка `alias` (rejected legacy form) сохраняется только как псевдоним маршрутизации. Аккаунт: `{ id, metadata }`. Код: `crates/iroha_data_model/src/account.rs`.
