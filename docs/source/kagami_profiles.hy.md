...
- Նմուշային փաթեթներ. նախապես ստեղծված փաթեթները գործում են `defaults/kagami/iroha3-{dev,taira,nexus}/`-ի ներքո (genesis.json, config.toml, docker-compose.yml, verify.txt, README): Վերականգնել `cargo xtask kagami-profiles [--profile <name>|all] [--out <dir>] [--kagami <bin>]`-ով:
