...
اور pending withdrawals کو `GET /v2/nexus/public_lanes/{lane}/stake` کے ذریعے ایکسپوز کرتا ہے تاکہ wallets
...
- **Torii/CLI quickstart:**
  ...
  - Lifecycle ISIs standard transaction path استعمال کرتے ہیں (Torii `/v2/transactions`
...
- ✅ Torii `/v2/nexus/public_lanes/{lane}/{validators,stake,rewards/pending}` Norito JSON کے ساتھ ایکسپوز کرتا ہے تاکہ
