...
- دفعات gossip للمعاملات تحمل tag للplane (public vs restricted) مشتقة من catalog للanes؛ الدفعات المقيدة ترسل unicast الى commit topology الحالية (وفق `transaction_gossip_restricted_target_cap`) بينما الدفعات العامة تستخدم `transaction_gossip_public_target_cap` (اضبط `null` للبث). يتم اعادة خلط اختيار الاهداف حسب `transaction_gossip_public_target_reshuffle_ms` و`transaction_gossip_restricted_target_reshuffle_ms` (افتراضي: `transaction_gossip_period_ms`). عندما تكون commit topology فارغة يمكن للمشغلين رفض او تمرير payloads المقيدة الى overlay العام عبر `transaction_gossip_restricted_public_payload` (افتراضي `refuse`). تكشف التليمترية محاولات fallback وعدادات forward/drop والسياسة المختارة مع اختيار الاهداف حسب dataspace.
...
- التحقق في جهة الاستقبال يسقط الدفعات التي لا تطابق catalog المحلي او التي لا يتطابق tag plane فيها مع رؤية dataspace المشتقة.
...
- `GET /v1/space-directory/uaids/{uaid}` — حل bindings الحالية للـ dataspace لUAID (عناوين موحدة، ids، program bindings). اضف `address_format=compressed` لاخراج Sora Name Service.
