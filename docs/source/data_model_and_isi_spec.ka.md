...
- აქტივების ID ზარაფხანის/დაწვის/გადაცემის საზღვრებზე რჩება კანონიკური `norito:<hex>`; გამოიყენეთ `iroha tools encode asset-id` `--definition <base58-asset-definition-id>` ან `--alias ...` პლუს `--account`.
