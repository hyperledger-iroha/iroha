...
- `AccountId` — კანონიკური მისამართები იწარმოება `AccountAddress` (I105 / `i105` შეკუმშული / თექვსმეტობითი) საშუალებით და Torii ახდენს შეყვანის ნორმალიზებას Torii-ის მეშვეობით. I105 არის ანგარიშის სასურველი ფორმატი; `i105` ფორმა მეორე საუკეთესოა მხოლოდ Sora UX-ისთვის. ნაცნობი `alias` (rejected legacy form) სტრიქონი შენახულია მხოლოდ მარშრუტიზაციის მეტსახელად. ანგარიში: `{ id, metadata }`. კოდი: `crates/iroha_data_model/src/account.rs`.
