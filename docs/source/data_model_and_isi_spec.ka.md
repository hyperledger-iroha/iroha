...
- `AssetId` — `asset#domain#account` ან `asset##account`, თუ ​​დომენები ემთხვევა, სადაც `account` არის კანონიკური `AccountId` სტრიქონი (სასურველია IH58). აქტივი: `{ id, value: Numeric }`. კოდი: `crates/iroha_data_model/src/asset/{id.rs,value.rs}`.
