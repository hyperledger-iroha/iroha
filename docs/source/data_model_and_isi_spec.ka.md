...
- `AssetId`: კანონიკური კოდირებული ლიტერალი `<asset-definition-id>#<account-id>` (მემკვიდრეობითი ტექსტური ფორმები არ არის მხარდაჭერილი პირველ გამოშვებაში).- `NftId` — `nft$domain`. NFT: `{ id, content: Metadata, owned_by }`. კოდი: `crates/iroha_data_model/src/nft.rs`.
...
- აქტივების ID ზარაფხანის/დაწვის/გადაცემის საზღვრებზე რჩება კანონიკური `<asset-definition-id>#<account-id>`; გამოიყენეთ `iroha tools encode asset-id` `--definition <base58-asset-definition-id>` ან `--alias ...` პლუს `--account`.
