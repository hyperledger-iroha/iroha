...
- `AccountId` — კანონიკური მისამართები იწარმოება `AccountAddress` (IH58 / `sora…` შეკუმშული / თექვსმეტობითი) საშუალებით და Torii ახდენს შეყვანის ნორმალიზებას Torii-ის მეშვეობით. IH58 არის ანგარიშის სასურველი ფორმატი; `sora…` ფორმა მეორე საუკეთესოა მხოლოდ Sora UX-ისთვის. ნაცნობი `alias` (rejected legacy form) სტრიქონი შენახულია მხოლოდ მარშრუტიზაციის მეტსახელად. ანგარიში: `{ id, metadata }`. კოდი: `crates/iroha_data_model/src/account.rs`.
