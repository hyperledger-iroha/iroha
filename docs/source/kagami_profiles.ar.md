...
- ملفات التعريف: `iroha3-dev` (السلسلة `iroha3-dev.local`، المجمعات k=1 r=1، بذور VRF المشتقة من معرف السلسلة عند تحديد NPoS)، `iroha3-testus` (السلسلة `iroha3-testus`، المجمعات k=3 r=3، يتطلب `--vrf-seed-hex` عندما يكون NPoS محدد)، `iroha3-nexus` (السلسلة `iroha3-nexus`، المجمعات k=5 r=3، تتطلب `--vrf-seed-hex` عند تحديد NPoS).
...
- نماذج الحزم: الحزم التي تم إنشاؤها مسبقًا موجودة ضمن `defaults/kagami/iroha3-{dev,testus,nexus}/` (genesis.json، config.toml، docker-compose.yml، Vere.txt، README). التجديد باستخدام `cargo xtask kagami-profiles [--profile <name>|all] [--out <dir>] [--kagami <bin>]`.
