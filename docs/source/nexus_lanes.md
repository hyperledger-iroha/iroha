...
- When neither `nexus.storage.local_budget_bytes` nor the legacy `nexus.storage.max_disk_usage_bytes` alias is set, `irohad` derives an implicit budget at startup from the filesystem backing `kura.store_dir`: `80%` of the currently available bytes. Explicit operator caps always win over that runtime-derived default.
