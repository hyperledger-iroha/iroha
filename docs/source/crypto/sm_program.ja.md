...
| SM4 | GB/T32907 | GM/T0002 | 128 ビットのブロック暗号。 Iroha は GCM/CCM ヘルパーを提供し、実装全体でビッグエンディアンのパリティを保証します。 |- **機能マニフェスト:** Torii `/v2/node/capabilities` エンドポイントは次の JSON 形式をアドバタイズするため、オペレーターとツールはプログラムで SM マニフェストを使用できます。
