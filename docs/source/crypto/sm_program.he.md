...
| SM4 | GB/T32907 | GM/T0002 | צופן בלוק 128 סיביות; Iroha מספק עוזרי GCM/CCM ומבטיח זוגיות גדולה בין יישומים. |- **מניפסט יכולת:** נקודת הקצה Torii `/v2/node/capabilities` מפרסמת את צורת ה-JSON הבאה, כך שהאופרטורים והכלים יכולים לצרוך את המניפסט SM באופן תכנותי:
