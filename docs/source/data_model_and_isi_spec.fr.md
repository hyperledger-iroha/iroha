...
- `AccountId` — les adresses canoniques sont produites via `AccountAddress` (I105 / `i105` compressé / hexadécimal) et Torii normalise les entrées via `AccountAddress::parse_encoded`. I105 est le format de compte préféré ; le formulaire `i105` est le deuxième meilleur pour l'UX Sora uniquement. La chaîne familière `alias` (rejected legacy form) est conservée uniquement comme alias de routage. Compte : `{ id, metadata }`. Code : `crates/iroha_data_model/src/account.rs`.
