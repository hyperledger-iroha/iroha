...
- `AssetId`：規範編碼文字 `<base58-asset-id>#<katakana-i105-account-id>`（第一個版本不支援舊文字形式）。- `NftId` — `nft$domain`。 NFT：`{ id, content: Metadata, owned_by }`。代碼：`crates/iroha_data_model/src/nft.rs`。
...
- 鑄造/銷毀/轉移邊界的資產 ID 維持規範 `<base58-asset-id>#<katakana-i105-account-id>`；將 `iroha tools encode asset-id` 與 `--definition <base58-asset-definition-id>` 或 `--alias ...` 加 `--account` 結合使用。
