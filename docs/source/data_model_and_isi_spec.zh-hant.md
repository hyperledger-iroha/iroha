...
- `AccountId` — 規範地址通過 `AccountAddress`（IH58 / `sora…` 壓縮/十六進制）生成，Torii 通過 `AccountAddress::parse_any` 標準化輸入。 IH58是首選賬戶格式； `sora…` 形式對於僅 Sora 的 UX 來說是第二好的。熟悉的 `alias@domain` 字符串僅保留作為路由別名。帳號：`{ id, metadata }`。代碼：`crates/iroha_data_model/src/account.rs`。
...
- `AssetId` — `asset#domain#account` 或 `asset##account`（如果域匹配），其中 `account` 是規範的 `AccountId` 字符串（首選 IH58）。資產：`{ id, value: Numeric }`。代碼：`crates/iroha_data_model/src/asset/{id.rs,value.rs}`。
