...
- `AccountId` — canonical addresses are produced via `AccountAddress` (IH58 / `snx1…` compressed / hex) and Torii normalises inputs through `AccountAddress::parse_any`. The familiar `alias@domain` string is retained as a routing alias only. Account: `{ id, metadata }`. Code: `crates/iroha_data_model/src/account.rs`.
...
- `AssetId` — `asset#domain#account@domain` or `asset##account@domain` if domains match. Asset: `{ id, value: Numeric }`. Code: `crates/iroha_data_model/src/asset/{id.rs,value.rs}`.
...
- Unregister Peer/Domain/Account/AssetDefinition/NFT/Role/Trigger: removes the target; emits deletion events. Additional cascading removals:
  - Unregister Domain: removes all accounts in domain, their roles, permissions, assets; removes all asset definitions and NFTs in domain; removes triggers whose authority domain matches. Events: `DomainEvent::Deleted`, plus per‑item deletion events. Errors: `FindError::Domain` if missing. Code: `core/.../isi/world.rs`.
  - Unregister Account: removes account’s permissions and roles; deletes assets owned by account; removes triggers whose authority is that account. Events: `AccountEvent::Deleted`. Errors: `FindError::Account` if missing. Code: `core/.../isi/domain.rs`.
  - Unregister AssetDefinition: deletes all assets of that definition. Events: `AssetDefinitionEvent::Deleted` and `AssetEvent::Deleted` per asset. Errors: `FindError::AssetDefinition`. Code: `core/.../isi/domain.rs`.
