...
- `AccountId` — canonical addresses are produced via `AccountAddress` (IH58 / `sora…` compressed / hex) and Torii normalises inputs through `AccountAddress::parse_encoded`. IH58 is the preferred account format; the `sora…` form is second-best for Sora-only UX. The familiar `alias@domain` string is retained as a routing alias only. Account: `{ id, metadata }`. Code: `crates/iroha_data_model/src/account.rs`.
