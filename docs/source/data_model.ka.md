...
- `AccountId`: კანონიკური იდენტიფიკატორი დაშიფრულია `AccountAddress`-ის მეშვეობით, რომელიც ავლენს IH58-ს, Sora შეკუმშულს (`sora…`) და კანონიკურ თექვსმეტობით კოდეკებს (Kotodama, Kotodama, `canonical_hex`, `parse_encoded`). IH58 არის ანგარიშის სასურველი ფორმატი; `sora…` ფორმა მეორე საუკეთესოა მხოლოდ Sora UX-ისთვის. ადამიანისთვის შესაფერისი მარშრუტიზაციის მეტსახელი `alias` (rejected legacy form) შენახულია UX-ისთვის, მაგრამ აღარ განიხილება, როგორც ავტორიტეტული იდენტიფიკატორი. Torii ახდენს შემომავალი სტრიქონების ნორმალიზებას `AccountAddress::parse_encoded`-ის მეშვეობით. ანგარიშის ID-ები მხარს უჭერენ როგორც ერთ გასაღების, ასევე მრავალსიგნ კონტროლერებს.
