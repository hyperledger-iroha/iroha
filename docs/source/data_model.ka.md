...
- `Account { id, metadata, label?, uaid?, opaque_ids[] }` — `label` არის არასავალდებულო პირველადი `AccountAlias`, რომელიც გამოიყენება ხელახალი ჩანაწერების მიერ, `uaid` ახორციელებს არასავალდებულო Nexus ფართოდ [უნივერსალური ანგარიშის ID]300(I) `opaque_ids` აკონტროლებს დამალულ იდენტიფიკატორებს, რომლებიც დაკავშირებულია ამ UAID-თან. შენახული ანგარიშის მდგომარეობა აღარ შეიცავს რაიმე დაკავშირებული დომენის ველს.
- მშენებლები:
  - `NewAccount` `Account::new(id)`-ის მეშვეობით რეგისტრირებს კანონიკურ დომენის გარეშე ანგარიშის თემას.
- Alias მოდელი:
...
- `AssetDefinition { id, name, description?, alias?, spec: NumericSpec, mintable: Mintable, logo: Option<SorafsUri>, metadata, owned_by: AccountId, total_quantity: Numeric }`.
  ...
  - Torii აქტივის განსაზღვრის პასუხები შეიძლება შეიცავდეს `alias_binding { alias, status, lease_expiry_ms, grace_until_ms, bound_at_ms }`, სადაც `status` არის ერთ-ერთი `permanent`, `leased_active`, `status`, Kotodama,
  - Alias-ის გარჩევადობა იყენებს უახლეს ჩადენილი ბლოკის დროის ნიშანს, ვიდრე კვანძის კედლის საათს. `grace_until_ms`-ის გავლის შემდეგ, ალიასის სელექტორები დაუყოვნებლივ წყვეტენ გადაჭრას, მაშინაც კი, თუ წმენდის გასუფთავებამ ჯერ არ ამოიღო ძველი შეკვრა; პირდაპირი განმარტებით წაკითხულებმა შეიძლება კვლავ შეატყობინოს გრძელვადიანი სავალდებულო, როგორც `expired_pending_cleanup`.
...
- `Sm4Key`: 128-ბიტიანი სიმეტრიული კლავიში გაზიარებული მასპინძელ სისტემასა და მონაცემთა მოდელის მოწყობილობებს შორის.
...
- `EventBox`: ჯამის ტიპი მილსადენისთვის, მილსადენის ჯგუფისთვის, მონაცემები, დრო, execute-trigger და trigger-დასრულებული მოვლენები; `EventFilterBox` ასახავს გამოწერებს და ააქტიურებს ფილტრებს.
...
## ISI (Iroha სპეციალური ინსტრუქციები)- ძირითადი თვისება: `Instruction` `dyn_encode`, `as_any` და სტაბილური თითო ტიპის იდენტიფიკატორით `id()` (ნაგულისხმევია კონკრეტული ტიპის სახელით). ყველა ინსტრუქცია არის `Send + Sync + 'static`.
...
- `TransactionBuilder`: აყალიბებს ტრანზაქციის სასარგებლო დატვირთვას `chain`, `authority`, `creation_time_ms`, სურვილისამებრ `time_to_live_ms` და `transfer`, `Executor { bytecode: IvmBytecode }`, `transfer`, `Executor { bytecode: IvmBytecode }`, Kotodama. `Executable`.
  - დამხმარეები: `with_instructions`, `with_bytecode`, `with_executable`, `with_metadata`, `set_nonce`, `set_ttl`, I1824NI00000
...
- კომუნალური საშუალებები: `presigned`, `set_transaction_results(...)`, `set_transaction_results_with_transcripts(...)`, `header()`, `signatures()`, `hash()`, I182NI0000X, I182NI000000
...
- `ExecWitness` შეტყობინებები (გადაცემა Torii-ის საშუალებით და კონსენსუსის ჭორებით დაფუძნებული ყულაბა) ახლა მოიცავს როგორც `fastpq_transcripts`, ასევე პროვერდისთვის მზად `fastpq_batches: Vec<FastpqTransitionBatch>`-ს ჩაშენებული X0, roots60,18NI70 perm_root, tx_set_hash), ასე რომ, გარე პროვერებს შეუძლიათ მიიღონ კანონიკური FASTPQ რიგები ტრანსკრიპტების ხელახალი კოდირების გარეშე.
...
- რეესტრი და კოდეკები:
  - `query_registry!{ ... }` აშენებს გლობალურ რეესტრს, რომელიც ასახავს კონკრეტული მოთხოვნის ტიპებს კონსტრუქტორებს ტიპის სახელების მიხედვით დინამიური დეკოდირებისთვის.
  ...
  - `QueryOutputBatchBox` არის ჯამის ტიპი ჰომოგენურ ვექტორებზე (მაგ., `Vec<Account>`, `Vec<Name>`, `Vec<AssetDefinition>`, `Vec<BlockHeader>`), პლუს ეფექტური pag და გაფართოების დამხმარეები.
- DSL: დანერგილია `query::dsl`-ში პროექციის ნიშნებით (`HasProjection<PredicateMarker>` / `SelectorMarker`) კომპილაციის დროში შემოწმებული პრედიკატებისა და სელექტორებისთვის. საჭიროების შემთხვევაში, `fast_dsl` ფუნქცია გამოავლენს უფრო მსუბუქ ვარიანტს.
...
- Norito: დაშიფვრავს/გაშიფრავს `norito::codec::{Encode, Decode}`-ით სქემით; `Json` დატვირთვა არის სერიული დეტერმინისტულად. ორმხრივი მგზავრობა სტაბილურია მანამ, სანამ ინსტრუქციის რეესტრში შედის `CustomInstruction` (ის ნაგულისხმევი რეესტრის ნაწილია).
...
- აღჭურვილია არასავალდებულო API-ების კონტროლით (`std`, `json`, `transparent_api`, `ffi_export`, `ffi_import`, `transparent_api`, `transparent_api`, `transparent_api`, I01 `fault_injection`).
...
- საჯარო აქტივების მოძიება მიმართავს საკუთრებაში არსებულ ნაშთებს `asset + account + optional scope`-ით; ნედლი კოდირებული `AssetId` ლიტერალები არის შიდა წარმოდგენა და არ არის Torii/CLI ამომრჩევის ზედაპირის ნაწილი.
- `POST /v1/assets/definitions/query` და `GET /v1/assets/definitions` იღებენ აქტივების განსაზღვრის ფილტრებს/დახარისხებას `alias_binding.status`, `alias_binding.lease_expiry_ms`, `alias_binding.grace_until_ms` და `alias_binding.grace_until_ms`, და `asset + account + optional scope`-ზე და `asset + account + optional scope`-ზე და Norito-ზე. `name`, `alias` და `metadata.*`.
