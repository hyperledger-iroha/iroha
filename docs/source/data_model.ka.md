...
ეს დოკუმენტი განმარტავს სტრუქტურებს, იდენტიფიკატორებს, მახასიათებლებს და პროტოკოლებს, რომლებიც ქმნიან Iroha v2 მონაცემთა მოდელს, როგორც დანერგილია `iroha_data_model` კრატში და გამოიყენება სამუშაო სივრცეში. ეს არის ზუსტი მითითება, რომელსაც შეგიძლიათ გადახედოთ და შესთავაზოთ განახლებები.
...
- IVM შენიშვნა: გარკვეული დესერიალიზაციის დროის ვალიდაცია გამორთულია Iroha ვირტუალური აპარატის (IVM) გამიზნებისას, ვინაიდან ჰოსტი ახორციელებს ვალიდაციას კონტრაქტების გამოძახებამდე (იხილეთ კრატის დოკუმენტები I100390X-ში).
- FFI კარიბჭე: ზოგიერთი ტიპი პირობითად არის ანოტირებული FFI-სთვის `iroha_ffi`-ის მეშვეობით `ffi_export`/`ffi_import`-ის უკან, რათა თავიდან იქნას აცილებული ზედნადები, როდესაც FFI არ არის საჭირო.

## ძირითადი თვისებები და დამხმარეები- `Identifiable`: ერთეულებს აქვთ სტაბილური `Id` და `fn id(&self) -> &Self::Id`. უნდა იყოს მიღებული `IdEqOrdHash`-ით რუკის/კომპლექტის კეთილგანწყობისთვის.
...
## სახელები და იდენტიფიკატორები- `Name`: სწორი ტექსტური იდენტიფიკატორი. არ იძლევა უფსკრული და დაჯავშნილი სიმბოლოები `@`, `#`, `$` (გამოიყენება კომპოზიტურ ID-ებში). კონსტრუირებადია `FromStr`-ის მეშვეობით ვალიდაციასთან ერთად. სახელები ნორმალიზებულია Unicode NFC-ზე ანალიზზე (კანონიკურად ექვივალენტური მართლწერები განიხილება, როგორც იდენტური და ინახება შედგენილი). სპეციალური სახელწოდება `genesis` დაცულია (შემოწმებულია ასოების გარეშე).
- `IdBox`: ჯამის ტიპის კონვერტი ნებისმიერი მხარდაჭერილი ID-სთვის (`DomainId`, `AccountId`, `AssetDefinitionId`, `AssetId`, `AssetId`, Kotodama `TriggerId`, `RoleId`, `Permission`, `CustomParameterId`). სასარგებლოა ზოგადი ნაკადებისთვის და Norito კოდირებისთვის, როგორც ერთი ტიპის.
...
- `AccountId`: ანგარიშის კანონიკური დომენის იდენტიფიკატორი, რომელიც კოდირებულია მხოლოდ `AccountAddress`-ით, როგორც I105. მკაცრი პარსერის შეყვანები უნდა იყოს კანონიკური I105; დომენის სუფიქსები (`@domain`), ანგარიშების მეტსახელის ლიტერალები, კანონიკური თექვსმეტობითი პარსერის შეყვანა, ძველი `norito:` დატვირთვა და `uaid:`/`opaque:` ანგარიშის პარსერის ფორმები უარყოფილია. ჯაჭვური ანგარიშის მეტსახელები იყენებენ `name@domain.dataspace` ან `name@dataspace` და წყვეტენ კანონიკურ `AccountId` მნიშვნელობებს.
- `AssetDefinitionId`: კანონიკური უპრეფიქსის Base58 მისამართი კანონიკური აქტივების განსაზღვრის ბაიტებზე. ეს არის საჯარო აქტივის ID. ჯაჭვური აქტივების მეტსახელები იყენებენ `name#domain.dataspace` ან `name#dataspace` და გადაიჭრება მხოლოდ ამ კანონიკური Base58 აქტივის ID-ით.
- `AssetId`: საჯარო აქტივის იდენტიფიკატორი კანონიკური შიშველი Base58 ფორმით. აქტივების მეტსახელები, როგორიცაა `name#dataspace` ან `name#domain.dataspace`, გადაწყვეტს `AssetId`-ს. შიდა ბუღალტერიის საქონელმა შეიძლება დამატებით გამოავლინოს გაყოფილი `asset + account + optional dataspace` ველები, სადაც საჭიროა, მაგრამ ეს კომპოზიციური ფორმა არ არის საჯარო `AssetId`.
...
- მშენებელი: `NewDomain` `with_logo`, `with_metadata`, შემდეგ `Registrable::build(authority)` კომპლექტი `owned_by`.

### ანგარიში
- `AccountId` არის კანონიკური ანგარიშის დომენის გარეშე, ჩასმული კონტროლერის მიერ და დაშიფრულია როგორც კანონიკური I105.
- `ScopedAccountId { account: AccountId, domain: DomainId }` ახორციელებს აშკარა დომენის კონტექსტს მხოლოდ იქ, სადაც საჭიროა სკოპური ხედი.
- `Account { id, metadata, label?, uaid?, linked_domains? }` — `label` არის არასავალდებულო სტაბილური მეტსახელი, რომელსაც იყენებენ ხელახალი ჩანაწერები, `uaid` ატარებს არასავალდებულო Nexus-ს ფართოდ [უნივერსალური ანგარიშის ID](`uaid`) (`uaid`). მიღებული ინდექსის მდგომარეობა და არა კანონიკური იდენტობის ნაწილი.
- მშენებლები:
  - `NewAccount` `Account::new(scoped_id)`-ის მეშვეობით ახდენს დომენთან დაკავშირებულ აშკარა რეგისტრაციას და, შესაბამისად, მოითხოვს `ScopedAccountId`.
  - `NewAccount` `Account::new_domainless(id)`-ის მეშვეობით რეგისტრირებს მხოლოდ უნივერსალური ანგარიშის საგანს დაკავშირებული დომენის გარეშე.
- Alias მოდელი:
  - კანონიკური ანგარიშის იდენტურობა არასოდეს მოიცავს დომენს ან მონაცემთა სივრცის სეგმენტს.
  - ანგარიშის მეტსახელები არის ცალკეული SNS/ანგარიშის ლეიბლის საკინძები, რომლებიც განლაგებულია `AccountId`-ის თავზე.
  - დომენით კვალიფიცირებული მეტსახელები, როგორიცაა `merchant@hbl.sbp`, ატარებენ როგორც დომენს, ასევე მონაცემთა სივრცეს ალიასის სავალდებულოში.
  - Dataspace-root მეტსახელები, როგორიცაა `merchant@sbp`, შეიცავს მხოლოდ მონაცემთა სივრცეს და, შესაბამისად, ბუნებრივად წყვილდება `Account::new_domainless(...)`-თან.
  - ტესტები და მოწყობილობები ჯერ უნივერსალური `AccountId` უნდა ჩათესონ, შემდეგ ცალ-ცალკე დაამატონ დომენის ბმულები, მეტსახელის იჯარა და ალიასის ნებართვები, დომენის დაშვებების კოდირების ნაცვლად, თავად ანგარიშის იდენტურობაში.

### აქტივების განმარტებები და აქტივები
- `AssetDefinitionId { aid_bytes: [u8; 16] }` ტექსტურად არის გამოსახული, როგორც არაპრეფიქსის Base58 მისამართი ვერსიითა და საკონტროლო ჯამით.
- `AssetDefinition { id, name, description?, alias?, spec: NumericSpec, mintable: Mintable, logo: Option<SorafsUri>, metadata, owned_by: AccountId, total_quantity: Numeric }`.
  ...
  - მეტსახელის საიჯარო მდგომარეობა ავტორიტეტულად ინახება მუდმივ ფსევდონიმად სავალდებულო ჩანაწერში; Inline `alias` ველი მიიღება, როდესაც განმარტებები იკითხება core/Torii API-ებით.
  - Torii აქტივის განსაზღვრის პასუხები შეიძლება შეიცავდეს `alias_binding { alias, status, lease_expiry_ms, grace_until_ms, bound_at_ms }`, სადაც `status` არის ერთ-ერთი `permanent`, `leased_active`, `alias_binding { alias, status, lease_expiry_ms, grace_until_ms, bound_at_ms }`, `alias_binding { alias, status, lease_expiry_ms, grace_until_ms, bound_at_ms }`, `alias_binding { alias, status, lease_expiry_ms, grace_until_ms, bound_at_ms }`.
  - Alias-ის გარჩევადობა იყენებს უახლეს ჩადენილი ბლოკის დროის ნიშანს, ვიდრე კვანძის კედლის საათს. მას შემდეგ, რაც `grace_until_ms` გაივლის, მეტსახელის სელექტორები დაუყოვნებლივ წყვეტენ გადაჭრას, მაშინაც კი, თუ წმენდის გასუფთავებამ ჯერ არ ამოიღო ძველი შეკვრა; პირდაპირი განმარტებით წაკითხულებმა შეიძლება კვლავ შეატყობინოს ხანგრძლივ შეკვრას, როგორც `expired_pending_cleanup`.
...
- `Asset { id, value: Numeric }` შესანახად მოსახერხებელი `AssetEntry`/`AssetValue`.- `AssetBalanceScope`: `Global` შეუზღუდავი ნაშთებისთვის და `Dataspace(DataSpaceId)` მონაცემთა სივრცით შეზღუდული ნაშთებისთვის.
- `AssetTotalQuantityMap = BTreeMap<AssetDefinitionId, Numeric>` გამოფენილია შემაჯამებელი API-ებისთვის.
...
ეს ტიპები დგანან არსებულ Ed25519/BLS/ML-DSA პრიმიტივებთან ერთად და გახდებიან საჯარო სქემის ნაწილი, როგორც კი სამუშაო სივრცე აშენდება `--features sm`-ით.

### ტრიგერები და მოვლენები
...
- `EventBox`: ჯამის ტიპი მილსადენის, მილსადენის პარტიისთვის, მონაცემები, დრო, execute-trigger და trigger-დასრულებული მოვლენები; `EventFilterBox` ასახავს გამოწერებს და ააქტიურებს ფილტრებს.
...
- `InstructionBox`: საკუთრებაში არსებული `Box<dyn Instruction>` შეფუთვა კლონით/eq/ordთ, რომელიც განხორციელებულია ID ტიპის + კოდირებული ბაიტით.
...
- `TransactionBuilder`: აყალიბებს ტრანზაქციის სასარგებლო დატვირთვას `chain`, `authority`, `creation_time_ms`, სურვილისამებრ `time_to_live_ms` და `metadata`, `metadata`, `metadata`, Kotodama. `Executable`.
  - დამხმარეები: `with_instructions`, `with_bytecode`, `with_executable`, `with_metadata`, `set_nonce`, `set_ttl`, I1828NI00000245X, I1828NI00000, `with_metadata`.
...
## ბლოკები- `SignedBlock` (ვერსიით) აერთიანებს:
  ...
  - `result: BlockResult` (შესრულების მეორადი მდგომარეობა), რომელიც შეიცავს `time_triggers`, შესვლის/შედეგის Merkle ხეებს, `transaction_results` და `fastpq_transcripts: BTreeMap<Hash, Vec<TransferTranscript>>`.
- კომუნალური საშუალებები: `presigned`, `set_transaction_results(...)`, `set_transaction_results_with_transcripts(...)`, `header()`, `signatures()`, `hash()`, I182NI0000, I182NI000000
...
- ბლოკის ჩართვის მტკიცებულებები (`BlockProofs`) ავლენს როგორც შესვლის/შედეგის Merkle-ის მტკიცებულებებს, ასევე `fastpq_transcripts` რუკას, რათა ჯაჭვის მიღმა პროვაიდერებმა შეძლონ ტრანსაქციის ჰეშთან დაკავშირებული გადაცემის დელტას მოძიება.
- `ExecWitness` შეტყობინებები (გადაცემული Torii-ის საშუალებით და კონსენსუსის ჭორებით დაფუძნებული ყულაბა) ახლა მოიცავს როგორც `fastpq_transcripts`-ს, ასევე პროვერდისთვის მზა `fastpq_batches: Vec<FastpqTransitionBatch>`-ს ჩაშენებული X0 root-ებით (Kotodama,70000000276X) perm_root, tx_set_hash), ასე რომ, გარე პროვერებს შეუძლიათ მიიღონ კანონიკური FASTPQ რიგები ტრანსკრიპტების ხელახალი კოდირების გარეშე.
...
- რეესტრი და კოდეკები:
  - `query_registry!{ ... }` აშენებს გლობალურ რეესტრს, რომელიც ასახავს კონკრეტული შეკითხვის ტიპებს კონსტრუქტორებისთვის ტიპის სახელის მიხედვით დინამიური დეკოდირებისთვის.
...
- DSL: დანერგილია `query::dsl`-ში პროექციის მახასიათებლებით (`HasProjection<PredicateMarker>` / `SelectorMarker`) კომპილაციის დროში შემოწმებული პრედიკატებისა და სელექტორებისთვის. საჭიროების შემთხვევაში, `fast_dsl` ფუნქცია გამოავლენს უფრო მსუბუქ ვარიანტს.
...
- პერსონალიზაციის ნიმუშები არსებობს `data_model/samples/executor_custom_data_model`-ში, რომელიც აჩვენებს:
  ...
  - მორგებული პარამეტრი განსაზღვრულია, როგორც ტიპი, რომელიც კონვერტირებადია `CustomParameter`-ში,
...
- Norito: დაშიფვრავს/გაშიფრავს `norito::codec::{Encode, Decode}`-ით სქემით; `Json` ტვირთამწეობა სერიულად არის განსაზღვრული. ორმხრივი მგზავრობა სტაბილურია მანამ, სანამ ინსტრუქციის რეესტრში შედის `CustomInstruction` (ის არის ნაგულისხმევი რეესტრის ნაწილი).
...
## მეტამონაცემები- `Metadata(BTreeMap<Name, Json>)`: გასაღები/ღირებულების შენახვა, რომელიც მიმაგრებულია მრავალ ერთეულზე (`Domain`, `Account`, `AssetDefinition`, `Nft`, ტრიგერები და ტრანზაქციები).
- API: `contains`, `iter`, `get`, `insert` და (`transparent_api`-ით) `remove`.

## თვისებები და დეტერმინიზმი

- აღჭურვილია არასავალდებულო API-ების კონტროლით (`std`, `json`, `transparent_api`, `ffi_export`, `ffi_import`, `fast_dsl`, `fast_dsl`, `json`, I01 `fault_injection`).
...
გამოიყენეთ IVM ჭკვიანი კონტრაქტის ბაიტიკოდი:
...
აქტივის განსაზღვრის ID / მეტსახელის სწრაფი მითითება (CLI + Torii):
...
- საჯარო აქტივების მოძიება მიმართავს საკუთრებაში არსებულ ნაშთებს `asset + account + optional scope`-ით; ნედლი კოდირებული `AssetId` ლიტერალები არის შიდა გამოსახულება და არ არის Torii/CLI ამომრჩევის ზედაპირის ნაწილი.
- `POST /v1/assets/definitions/query` და `GET /v1/assets/definitions` იღებენ აქტივების განსაზღვრის ფილტრებს/დალაგებენ `alias_binding.status`, `alias_binding.lease_expiry_ms`, `alias_binding.grace_until_ms`, და `alias_binding.lease_expiry_ms`, `alias_binding.grace_until_ms` და `alias_binding.bound_at_ms`-ზე და `alias_binding.bound_at_ms`-ზე. `name`, `alias` და `metadata.*`.
