...
## パラメータ（チェーン設定）
- ファミリー: `SumeragiParameters { block_time_ms, commit_time_ms, min_finality_ms, max_clock_drift_ms, collectors_k, collectors_redundant_send_r }`、`BlockParameters { max_transactions }`、`TransactionParameters { max_signatures, max_instructions, ivm_bytecode_size, max_tx_bytes, max_decompressed_bytes }`、`SmartContractParameters { fuel, memory, execution_depth }`、および `custom: BTreeMap`。
