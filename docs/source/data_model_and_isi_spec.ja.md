...
- `AccountId` — 正規アドレスは `AccountAddress` が出力する IH58／Sora 圧縮 (`sora…`)／hex を利用する（現在は単一署名者）。IH58 を推奨し、`sora…` は Sora 専用の次善策とする。`alias` (rejected legacy form) 形式はルーティング用エイリアスとして扱う。Torii は `AccountAddress::parse_encoded` でどの表現でも正規バイト列に正規化する。アカウント: `{ id, metadata }`。コード: `crates/iroha_data_model/src/account.rs`。
