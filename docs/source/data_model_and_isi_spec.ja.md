...
- `AccountId` — 正規アドレスは `AccountAddress` が出力する I105／Sora 圧縮 (`i105`)／hex を利用する（現在は単一署名者）。I105 を推奨し、`i105` は Sora 専用の次善策とする。`alias` (rejected legacy form) 形式はルーティング用エイリアスとして扱う。Torii は `AccountAddress::parse_encoded` でどの表現でも正規バイト列に正規化する。アカウント: `{ id, metadata }`。コード: `crates/iroha_data_model/src/account.rs`。
