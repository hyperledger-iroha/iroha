...
- `AssetId`: 正規エンコードされたリテラル `<base58-asset-id>#<katakana-i105-account-id>` (従来のテキスト形式は最初のリリースではサポートされていません)。- `NftId` — `nft$domain`。 NFT: `{ id, content: Metadata, owned_by }`。コード: `crates/iroha_data_model/src/nft.rs`。
...
- ミント/バーン/トランスファーの境界におけるアセット ID は正規の `<base58-asset-id>#<katakana-i105-account-id>` のままです。 `iroha tools encode asset-id` と `--definition <base58-asset-definition-id>`、または `--alias ...` と `--account` を使用します。
