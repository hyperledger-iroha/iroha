...
2. **Signed manifest** — Run
   ...
   per-operation limits; only add the legacy `--require-rows`/`--max-operation-ms` overrides when
...
| Row-usage snapshot | Fetch an ExecWitness (`iroha_cli audit witness --binary --out exec.witness`) and decode it with `iroha_cli audit witness --decode exec.witness --fastpq-parameter fastpq-lane-balanced` (FASTPQ batches are emitted automatically; pass `--no-fastpq-batches` only if you want to omit them) and archive the `row_usage` section for each FASTPQ batch. | The decoded batches now emit `row_usage.{total_rows,transfer_rows,non_transfer_rows,transfer_ratio,…}` so dashboards can chart transfer-gadget row savings versus the legacy path without reprocessing transcripts. Keep the snippet alongside the Metal JSON/log bundle and compare it with the previous baseline via `scripts/fastpq/check_row_usage.py` to fail CI whenever transfer ratios or total rows regress.【crates/iroha_cli/src/audit.rs:209】 |
