...
- Валидация на приемной стороне отбрасывает батчи, где lanes/dataspaces не совпадают с локальным каталогом или тег плоскости не соответствует вычисленной видимости dataspace.
...
- `GET /v1/space-directory/uaids/{uaid}` — разрешить текущие привязки dataspace
  ...
  `address_format=compressed` для вывода Sora Name Service.
...
- **Admin endpoint:** `POST /v1/nexus/lifecycle` (Torii) принимает Norito/JSON тело с `additions` (полные объекты `LaneMetadata`) и `retire` (lane ids) для добавления или удаления lanes без рестарта. Запросы gated на `nexus.enabled=true` и используют ту же Nexus configuration/state view, что и очередь.
