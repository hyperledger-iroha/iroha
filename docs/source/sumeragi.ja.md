...
- **K コレクターモード**: 各高さ/ビューに対し、PRF でリーダーを除外した K 個のコレクターを決定論的に選びます。PRF シードがない場合は `proxy_tail_index()`（含む）から始まるラップアラウンド選択にフォールバックし、リーダーを除外してコミットクォーラム以上を埋めます（非リーダー数が上限）。K=1 は単一の PRF コレクターを選びますが、PRF がない場合もフォールバックはプロキシテール起点でコミットクォーラム以上を埋め、コレクターファンアウトがクォーラム未満のときはコミットトポロジ全体にフォールバックします。
...
- フォールバック: `k` でコレクタが選べない場合、投票はコミットトポロジ全体にフォールバックします。`redundant_send_r` は少なくともコミットクォーラム相当にクランプされます（利用可能なコレクター数が上限）。
...
- `deterministic_collectors(topology, mode, k, seed, height, view)` は `(height, view)` と PRF シードに基づく決定論的なコレクター集合を返します。PRF シードがない場合は `proxy_tail_index()` 起点のラップアラウンド選択（リーダー除外）でコミットクォーラム以上を埋めるフォールバックを使い、K=1 でもプロキシテール起点でクォーラムを満たします。
- `Topology::role_at(index)` で任意ピアの役割を取得し、`is_validator(idx)` でその高さにおけるバリデータかどうかを判定します。

### API サーフェス
- `iroha_cli --output-format text ops sumeragi status`（`GET /v2/sumeragi/status/summary`）がトポロジ情報・役割分布・commit certificate 状態に加え、RBC の利用状況やエポック調整値（`epoch_len`/`epoch_commit`/`epoch_reveal`）を表示します。
- `iroha_cli --output-format text ops sumeragi params`（`GET /v2/sumeragi/params`）で `collectors_k`、`collectors_redundant_send_r`、Sumeragi パラメータ（タイムアウト、モード等）を確認できます。
- `iroha_cli ops sumeragi status`（`GET /v2/sumeragi/status`）はノードの役割履歴、ビュー、ロック中 commit certificate、未コミットブロックなど完全な Norito レコードを返します。
...
### エビデンス & スラッシング
- ダブル投票や無効な commit certificate/提案は `Evidence` レコードとして保存され、`/v2/sumeragi/evidence` エンドポイントから取得できます。
...
### CLI / API チートシート
- `iroha_cli --output-format text ops sumeragi status` / `GET /v2/sumeragi/status/summary`
- `iroha_cli ops sumeragi status` / `GET /v2/sumeragi/status`
- `iroha_cli --output-format text ops sumeragi params` / `GET /v2/sumeragi/params`
...
- `/v2/sumeragi/telemetry`（または `iroha_cli --output-format text ops sumeragi status`） — コレクター別の投票数、commit certificate レイテンシ、RBC backlog、最新 Highest/Locked QC ハッシュを含む軽量スナップショット。
- `/v2/sumeragi/status/sse` — 1 秒周期程度の SSE ストリームでライブ観測。
- `/v2/sumeragi/phases` / `iroha_cli --output-format text ops sumeragi phases` — `{propose_ms, collect_da_ms, collect_prevote_ms, collect_precommit_ms, collect_aggregator_ms, commit_ms, pipeline_total_ms}` と `ema_ms` を併記したフェーズ別タイムライン。
...
2. `/v2/sumeragi/telemetry` を見て `votes_ingested` が伸びていないコレクター index を特定。単一コレクターだけ停滞しているなら一時的に `sumeragi.collectors.redundant_send_r` を増やし、票を次順位へ回す。
3. `sumeragi_bg_post_queue_depth` と `p2p_*_throttled_total` でキューやトランスポートの逼迫を診断。
4. FASTPQ プローバ遅延が疑われる場合は `/v2/torii/zk/prover/reports` と Torii ログでプローバの失敗を調べ、必要に応じて再起動。
5. 両コレクターが停止しているなら RBC backlog を確認。`sumeragi_rbc_store_evictions_total` と `sumeragi_rbc_persist_drops_total` のスパイク、または `/v2/sumeragi/status` の `rbc_store.recent_evictions` を手掛かりにボトルネックとなるペイロードやエポックを特定。
...
4. **監視**: `sumeragi_da_summary::*` と `/v2/sumeragi/rbc/sessions` を定期取得し、遅延や失敗を早期検知します。
...
- オペレーター（または自動化）が Torii に Norito JSON を POST:
  - `POST /v2/sumeragi/vrf/commit` `{ "epoch": <u64>, "signer": <u32>, "commitment_hex": "0x…" }`
  - `POST /v2/sumeragi/vrf/reveal` `{ "epoch": <u64>, "signer": <u32>, "reveal_hex": "0x…" }`
...
更新されたシードは `deterministic_collectors` でコレクター選定に再利用され、`/v2/sumeragi/collectors` で `(height, view)` とともに公開されます。
...
- 手動投稿が必要な場合は Torii の POST エンドポイントを利用（自動化が通常は処理）。例:
  ...
  `/v2/sumeragi/collectors` でコレクター集合 (`collectors[*].peer_id`) を、`/v2/sumeragi/status` で `prf_epoch_seed`, `prf_height`, `vrf_late_reveals_total` 等を確認できます。
...
- コミットウィンドウでは全バリデータがコミットを出したか `iroha_cli --output-format text ops sumeragi telemetry` (`commitments_total`) と `/v2/sumeragi/telemetry` の `vrf.commitments` で確認。
- リビール締め切り前に `vrf.reveals_total` と `vrf_late_reveals_total` を監視。リビール漏れがあれば速やかにバリデータへ通知し、`POST /v2/sumeragi/vrf/reveal` で代理投稿できるよう準備。
...
- エポック rollover 後にペナルティが解消 (`vrf_committed_no_reveal_total` が対象サイナーで 0 へ) されていることを確かめ、`/v2/sumeragi/vrf/epoch/{n}` が `finalized: true` を示すか検証。
...
- `sumeragi_prf_epoch_seed`（`/v2/sumeragi/status`）— 現行シード（16 進文字列から復元して可視化可能）。
- `sumeragi_prf_context_height` / `_view` — コレクター／リーダー抽出に使用された `(height, view)`。
- `/v2/sumeragi/vrf/epoch/{epoch}` — `seed_hex`、参加状況、ペナルティ集計を含むスナップショット。CLI 版は `iroha_cli ops sumeragi vrf-epoch`。
- `/v2/sumeragi/telemetry` の `vrf` セクション — `{found, epoch, finalized, seed_hex, roster_len, participants_total, commitments_total, reveals_total, late_reveals_total, committed_no_reveal[], no_participation[], late_reveals[]}` を返し、リアルタイム参加率を可視化。
- `/v2/sumeragi/status` は `vrf_penalty_epoch`, `vrf_committed_no_reveal_total`, `vrf_no_participation_total`, `vrf_late_reveals_total` を公開。遅延リビールは `sumeragi_vrf_reveals_late_total` と `world.vrf_epochs[*].late_reveals` に記録され、シードに影響しないことが保証されます。
