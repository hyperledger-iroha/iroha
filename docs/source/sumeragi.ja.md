...
- `sumeragi.da.enabled` を有効にすると、ローカル payload 可用性は DA gate に記録され、可用性証跡（`availability evidence`）や RBC `READY` クォーラムは監査・テレメトリ・決定論的リカバリのために追跡されます。これらは別個のコミットクォーラムではありません。`missing_local_data` が有効な間、ローカル finalize は待機し、BlockCreated または RBC/ブロック同期で payload が揃うと続行します。`da_reschedule_total` はレガシーのため通常 0 のままです。
...
- DA/RBC スモークシナリオは `LARGE_PAYLOAD_BYTES = 1024` とプロトコル READY クォーラムを使います。RBC デリバリー時間、コミット時間、スループット、キュー深さをテレメトリで監視し、ハーネス予算違反をアラートします。
...
- `sumeragi.da.enabled` を有効にすると、`availability evidence` と RBC `READY` クォーラムは監査・テレメトリ・リカバリ証跡として追跡されます。ローカル payload が不足する場合は DA gate が finalize を待機させ、`RbcDeliver` またはブロック同期で満たされると続行します。
