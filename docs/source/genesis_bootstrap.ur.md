...
- ** مطلوبہ بہاؤ: ** جب اسٹوریج خالی ہو اور `genesis.file` غیر سیٹ ہو (اور
  `genesis.bootstrap_enabled=true`) ، `genesis.expected_hash` کے لیے درست دستخط شدہ جینیسس بلاک
  کو لازماً پن کرنا ضروری ہے۔ فائل اور ہیش دونوں کی عدم موجودگی کسی بھی درخواست کے بھیجے جانے سے پہلے
  اسٹارٹ اپ کنفیگریشن کی خرابی ہے۔ پھر نوڈ پے لوڈ لاتا ہے اور `validate_genesis_block` کے ذریعے دستخطوں کی توثیق کرتا ہے،
...
- ** آپریٹر اقدامات: ** یقینی بنائیں کہ کم از کم ایک قابل اعتماد ہم مرتبہ ایک درست جینیسس ، تشکیل کے ساتھ قابل رسائ ہے
  ...
  ریموٹ بوٹسٹریپ فعال کرنے سے پہلے درست بلاک کو `expected_hash` میں پن کریں۔ مقامی دستخط شدہ
  `genesis.file` خود ایک واضح آرٹیفیکٹ ہے اور اضافی ہیش کا تقاضا نہیں کرتا۔ مستقل پے لوڈ ہوسکتا ہے
