...
- `AssetId`: каноник кодлогдсон literal `<asset-definition-id>#<i105-account-id>` (эхний хувилбар дээр хуучин текст хэлбэрийг дэмждэггүй).- `NftId` — `nft$domain`. NFT: `{ id, content: Metadata, owned_by }`. Код: `crates/iroha_data_model/src/nft.rs`.
...
- Үнэлэх/шатаах/шилжүүлэх хил хязгаар дахь хөрөнгийн ID-ууд `<asset-definition-id>#<i105-account-id>` стандартад нийцдэг; `iroha tools encode asset-id`-г `--definition <base58-asset-definition-id>` эсвэл `--alias ...` дээр нэмэх нь `--account` ашиглана уу.
