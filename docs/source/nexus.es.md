...
- La validacion del lado receptor descarta lotes cuyas lanes/dataspaces no concuerdan con el catalogo local o cuya etiqueta de plano no coincide con la visibilidad del dataspace derivada.
...
#### Flujo de trabajo CLI (`iroha space-directory manifest ...`)
...
3. **Expirar** manifiestos que llegaron al fin de vida programado o **revocar**
   UAIDs bajo demanda. Ambos comandos aceptan `--uaid uaid:<hex>` y el id numerico
   de dataspace:
...
- `GET /v1/space-directory/uaids/{uaid}` - resolver las vinculaciones actuales de dataspace
  ...
  `address_format=compressed` para salida de Sora Name Service.
...
- **Endpoint admin:** `POST /v1/nexus/lifecycle` (Torii) acepta un cuerpo Norito/JSON con `additions` (objetos completos `LaneMetadata`) y `retire` (ids de lane) para agregar o retirar lanes sin reinicio. Las solicitudes se protegen con `nexus.enabled=true` y reutilizan la misma vista de configuracion/estado Nexus que la cola.
...
- **TODO:** Rebalanceo por lane de scheduler/DA/RBC, propagacion de sets de validadores y limpieza de almacenamiento estan pendientes; mantener solicitudes de ciclo de vida infrecuentes hasta que esos hooks lleguen.

Ruta de migracion (Iroha 2 -> Iroha 3)
1) Introducir IDs calificados por dataspace y composicion de bloque nexus/estado global en el data model; agregar feature flags para mantener modos compatibles con Iroha 2 durante la transicion.
...
- **Panel Grafana:** `dashboards/grafana/nexus_lanes.json` ahora exporta el dashboard "Nexus Lane Finality & Oracles" solicitado por NX-18. Los paneles cubren `histogram_quantile()` sobre `iroha_slot_duration_ms`, `iroha_da_quorum_ratio`, advertencias de disponibilidad DA (`sumeragi_da_gate_block_total{reason="missing_availability_qc"}`), gauges de precio/estancamiento/TWAP/haircut de oraculos, y el panel vivo de buffer `iroha_settlement_buffer_xor` para que los operadores prueben el slot de 1 s, DA y SLOs de tesoreria sin queries bespoke.
