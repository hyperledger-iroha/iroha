...
- `AssetId`: канондық кодталған литерал `<asset-definition-id>#<account-id>` (бұрынғы мәтіндік пішіндерге бірінші шығарылымда қолдау көрсетілмейді).- `NftId` — `nft$domain`. NFT: `{ id, content: Metadata, owned_by }`. Код: `crates/iroha_data_model/src/nft.rs`.
...
- Жалға беру/жаю/тасымалдау шекараларындағы актив идентификаторлары канондық `<asset-definition-id>#<account-id>` болып қалады; `iroha tools encode asset-id` `--definition <base58-asset-definition-id>` немесе `--alias ...` плюс `--account` арқылы пайдаланыңыз.
