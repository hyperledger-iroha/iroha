...
- `AssetId` — `asset#domain#account` немесе домендер сәйкес келсе `asset##account`, мұнда `account` канондық `AccountId` жолы (IH58 қолайлы). Актив: `{ id, value: Numeric }`. Код: `crates/iroha_data_model/src/asset/{id.rs,value.rs}`.
