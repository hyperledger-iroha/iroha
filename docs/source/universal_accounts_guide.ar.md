...
تعد معرفات UAIDs الآن بمثابة مرساة لطبقة الهوية الثانية:

- يحدد `IdentifierPolicyId` (`<kind>#<business_rule>`) العالمي
...
- تظل الدقة هي التدفق `resolve -> transfer`. Torii يحل مشكلة التعتيم
  ...
  الحساب الأساسي، وليس `uaid:` أو `opaque:` الحرفي مباشرة.
...
- `ram_lfe` هو تجريد الوظيفة المخفية الخارجية. ويغطي السياسة
...
لا يؤدي طرح الحساب العام إلى تغيير نموذج هوية الحساب الأساسي:

- يظل `AccountId` هو موضوع الحساب الأساسي بدون مجال.
- `ScopedAccountId { account, domain }` هو سياق المجال الصريح لطرق العرض أو
  التسجيلات التي تحقق رابط المجال. انها ليست الكنسي الثاني
  الهوية.
- الأسماء المستعارة لـ SNS/الحساب هي روابط منفصلة فوق هذا الموضوع. أ
...
- `linked_domains` في سجلات الحساب المخزن مشتقة من الحالة
  فهارس مجال الحساب. وهو يصف الروابط المتحققة حاليًا لذلك
  الموضوع؛ فهو ليس جزءًا من المعرف الأساسي.

قاعدة التنفيذ للمشغلين وحزم SDK والاختبارات: ابدأ من الأساسي
`AccountId`، ثم قم بإضافة عقود إيجار الاسم المستعار وأذونات مساحة البيانات/المجال والتصريحات الصريحة
روابط المجال بشكل منفصل. لا تقم بتجميع نطاق أساسي مزيف
الحساب لمجرد أن الاسم المستعار أو المسار يحمل مقطع مجال.

مسارات Torii الحالية:
...
- `normalizeIdentifierInput(value, normalization)` يطابق الصدأ
...
- يسرد `ToriiClient.listIdentifierPolicies()` بيانات تعريف السياسة، بما في ذلك BFV
  ...
  تكشف السياسات المبرمجة أيضًا عن `ram_fhe_profile` الذي تم فك تشفيره. هذا المجال
...
ما هي الارتباطات التي كانت نشطة في عصر معين.

## 4. القدرة على النشر تتجلى بالأدلة
...
- [ ] إيصال CLI/Torii لمكالمة النشر (stdout أو `--json-out` artifact).
- [ ] `SpaceDirectoryEvent` تفعيل الحمولة النافعة.
- [ ] تدقيق دليل الحزمة مع ملف تعريف مساحة البيانات، والخطافات، ونسخة البيان.
- [ ] الارتباطات + لقطات البيان التي تم جلبها من Torii بعد التنشيط.يعكس هذا المتطلبات الواردة في `docs/space-directory.md` §3.2 أثناء منح SDK
...
| `fixtures/space_directory/capability/jp_regulator_supervision.manifest.json` | حارة الإشراف JFSA. | إضافة بدل `cbdc.supervision.issue_stop_order` محدد (نافذة PerDay + `max_amount`) ورفض صريح على `force_liquidation` لفرض عناصر التحكم المزدوجة. |
