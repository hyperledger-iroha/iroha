...
- `AssetId` : littéral codé canonique `<asset-definition-id>#<account-id>` (les formes textuelles héritées ne sont pas prises en charge dans la première version).
...
- `AssetDefinitionId { aid_bytes: [u8; 16] }` exposé textuellement sous la forme `unprefixed Base58 address`.
...
asset-definition id / référence rapide alias (CLI + Torii) :
...
- Les ID d'actifs pour la création/gravure/transfert restent canoniques `<asset-definition-id>#<account-id>` ; construisez-les avec :
