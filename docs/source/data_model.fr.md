...
- `AccountId` : identifiant canonique codé via `AccountAddress`, qui expose IH58, Sora compressé (`sora…`) et les codecs hexadécimaux canoniques (`AccountAddress::to_ih58`, `to_compressed_sora`, `canonical_hex`, `parse_encoded`). IH58 est le format de compte préféré ; le formulaire `sora…` est le deuxième meilleur pour l'UX Sora uniquement. L'alias de routage convivial `alias@domain` est conservé pour l'UX mais n'est plus traité comme l'identifiant faisant autorité. Torii normalise les chaînes entrantes via `AccountAddress::parse_encoded`. Les identifiants de compte prennent en charge les contrôleurs à clé unique et multisig.
