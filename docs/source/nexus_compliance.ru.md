...
Статус: 🈯 Планируется — этот документ задает спецификацию и guardrails раскатки,
на которые ссылается пункт roadmap **NX-12 — Движок комплаенса lane и политика белого списка**.
Он объясняет модель данных, потоки governance, телеметрию и стратегию раскатки,
которые будут реализованы в `crates/iroha_core/src/compliance` и выставлены
через поверхности Torii/SDK, чтобы каждая lane и dataspace могла быть привязана к
...
- `GET /v1/lane-compliance/policies/{lane_id}` — получить актуальную policy reference.
- `POST /v1/lane-compliance/policies` — endpoint только для governance, отражающий ISI proposal helpers.
- `GET /v1/lane-compliance/decisions` — пагинированный аудит-лог с фильтрами по
...
1. **Admission (Torii)**
   ...
   - Каждая транзакция, входящая в очередь `/v1/pipeline`, помечается `LaneComplianceContext`
...
2. **Execution (iroha_core)**
   - При построении блока `iroha_core::compliance::Engine` повторяет то же решение, используя policy id,
     зафиксированный в заголовке блока. Это предотвращает расхождения даже при устаревших cache Torii.
   - Инструкции, которые меняют lane manifests или политики, должны передавать policy references,
     на которые они опираются, чтобы эти значения были зафиксированы on-chain.
3. **Async hooks**
