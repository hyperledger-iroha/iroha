...
## ٹرانسپورٹ کا معاہدہ- پرائمری ٹرانسپورٹ: ویب ساکٹ (`/v2/connect/ws?sid=<session_id>`)۔
...
- `sid` ایک 32 بائٹ شناخت کنندہ ہے جو `BLAKE2b-256("iroha-connect|sid|" || chain_id || app_ephemeral_pk || nonce16)` سے اخذ کیا گیا ہے۔  
  `/v2/connect/session` کو فون کرنے سے پہلے اس کی گنتی کریں۔ بٹوے اسے `approve` فریموں میں گونجتے ہیں تاکہ دونوں فریق کلیدی جرائد اور ٹیلی میٹری کو مستقل طور پر اہم کرسکیں۔
