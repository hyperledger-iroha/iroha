...
- `Account { id, metadata, label?, uaid?, opaque_ids[] }` — `label` yenidən açar qeydlər tərəfindən istifadə edilən isteğe bağlı əsas `AccountAlias`-dir, `uaid` genişmiqyaslı isteğe bağlı Nexus [Universal Hesab ID000] və [Universal Hesab ID000]00 (I18) daşıyır `opaque_ids` həmin UAID-ə bağlı gizli identifikatorları izləyir. Saxlanmış hesab vəziyyəti artıq heç bir əlaqəli domen sahəsi daşımır.
- İnşaatçılar:
  - `NewAccount` `Account::new(id)` vasitəsilə kanonik domensiz hesab mövzusunu qeyd edir.
- Alias modeli:
...
- `AssetDefinition { id, name, description?, alias?, spec: NumericSpec, mintable: Mintable, logo: Option<SorafsUri>, metadata, owned_by: AccountId, total_quantity: Quantity }`.
  ...
  - Torii aktiv tərifi cavablarına `alias_binding { alias, status, lease_expiry_ms, grace_until_ms, bound_at_ms }` daxil ola bilər, burada `status` `permanent`, `leased_active`, Norito və ya Norito-dən biridir.
  - Ləqəb həlli qovşaq divar saatından daha çox ən son bağlanmış blok vaxt damğasından istifadə edir. `grace_until_ms` keçdikdən sonra, ləqəb seçiciləri, hətta süpürgə təmizlənməsi köhnə bağlayıcını hələ aradan qaldırmasa belə, dərhal həll etməyi dayandırır; birbaşa tərif oxunuşları hələ də `expired_pending_cleanup` olaraq qalıcı bağlama haqqında məlumat verə bilər.
...
- Saxlama üçün əlverişli `AssetEntry`/`AssetValue` ilə `Asset { id, value: Quantity }`.- `AssetBalanceScope`: məhdudiyyətsiz balanslar üçün `Global` və məlumat məkanı ilə məhdudlaşdırılmış qalıqlar üçün `Dataspace(DataSpaceId)`.
- `AssetTotalQuantityMap = BTreeMap<AssetDefinitionId, Quantity>` xülasə API-ləri üçün açıqdır.

### NFTs
- `NftId { domain: DomainId, name: Name }`.
- `Nft { id, content: Metadata, owned_by: AccountId }` (məzmun ixtiyari açar/dəyər metadatasıdır).
- İnşaatçı: `Nft::new(id, content)` vasitəsilə `NewNft`.
...
- `Sm3Hash`: GM/T 0004 həzmini təmsil edən `[u8; 32]` yeni tip, manifestlər, telemetriya və sistem zəngi cavablarında istifadə olunur.
...
- `InstructionBox`: Klon/eq/ord ilə sahib `Box<dyn Instruction>` sarğı ID növü + kodlanmış bayt vasitəsilə həyata keçirilir.
...
- Təlimat reyestri: `instruction_registry!{ ... }` makrosu növün adı ilə əsaslanan iş vaxtı deşifrə reyestrini qurur. Dinamik (de)seriyaya nail olmaq üçün `InstructionBox` klonu və Norito serde tərəfindən istifadə olunur. `set_instruction_registry(...)` vasitəsilə heç bir reyestr açıq şəkildə qurulmayıbsa, ikili faylları möhkəm saxlamaq üçün bütün əsas ISI ilə daxili standart reyestr ilk istifadədə tənbəlliklə quraşdırılır.
...
## Bloklar- `SignedBlock` (versiyalaşdırılmış) kapsullaşdırır:
...
- Blok daxiletmə sübutları (`BlockProofs`) həm giriş/nəticə Merkle sübutlarını, həm də `fastpq_transcripts` xəritəsini ifşa edir ki, zəncirdən kənar ekspertlər tranzaksiya hash ilə əlaqəli transfer deltalarını əldə edə bilsinlər.
- `ExecWitness` mesajları (Torii vasitəsilə yayımlanır və konsensus dedi-qoduları ilə dəstəklənir) indi həm `fastpq_transcripts`, həm də daxil edilmiş kökləri olan `fastpq_batches: Vec<FastpqTransitionBatch>` (Kotodama, slotları, slotları ilə proverə hazırdır) daxildir. perm_root, tx_set_hash), beləliklə, xarici provers transkriptləri yenidən kodlaşdırmadan kanonik FASTPQ sıralarını qəbul edə bilər.
...
- DSL: Kompilyasiya zamanı yoxlanılan predikatlar və seçicilər üçün proyeksiya əlamətləri ilə (`HasProjection<PredicateMarker>` / `SelectorMarker`) `query::dsl`-də həyata keçirilir. `fast_dsl` xüsusiyyəti lazım olduqda daha yüngül variantı ortaya qoyur.
...
## Metadata- `Metadata(BTreeMap<Name, Json>)`: birdən çox quruma əlavə edilmiş açar/dəyər anbarı (`Domain`, `Account`, `AssetDefinition`, `Nft`, tetikler və əməliyyatlar).
- API: `contains`, `iter`, `get`, `insert` və (`transparent_api` ilə) `remove`.

## Xüsusiyyətlər və Determinizm

- Xüsusiyyətlər əlavə API-lərə nəzarət edir (`std`, `json`, `transparent_api`, `ffi_export`, `ffi_import`, `transparent_api`, `transparent_api`, I1030X, I1030X `fault_injection`).
...
- İctimai aktivlərin axtarışları `asset + account + optional scope` ilə məxsus qalıqlara ünvanlanır; xam kodlu `AssetId` literalları daxili təmsildir və Torii/CLI seçici səthinin bir hissəsi deyil.
- `POST /v1/assets/definitions/query` və `GET /v1/assets/definitions`, `alias_binding.status`, `alias_binding.lease_expiry_ms`, `alias_binding.grace_until_ms` və Kotodama-ə əlavə olaraq `alias_binding.status` və Kotodama-dən çox aktiv təyinetmə filtrlərini/çeşidlərini qəbul edir. `name`, `alias` və `metadata.*`.

## Versiyalaşdırma

- `SignedTransaction`, `SignedBlock` və `SignedQuery` kanonik Norito kodlu strukturlardır. Hər biri `EncodeVersioned` vasitəsilə kodlaşdırıldıqda cari ABI versiyası (hazırda `1`) ilə öz faydalı yükünü prefiks etmək üçün `iroha_version::Version` tətbiq edir.
