...
- `AccountId`: I105, Sora sıxılmış (`i105`) və kanonik hex kodekləri (`AccountAddress::to_i105`, I100080X, I1008030) ifşa edən `AccountAddress` vasitəsilə kodlanmış kanonik identifikator `canonical_hex`, `parse_encoded`). I105 üstünlük verilən hesab formatıdır; I105 forması yalnız Sora üçün UX üçün ikinci ən yaxşısıdır. İnsanlara uyğun marşrutlaşdırma ləqəbi `alias` (rejected legacy form) UX üçün qorunur, lakin artıq səlahiyyətli identifikator kimi qəbul edilmir. Torii `AccountAddress::parse_encoded` vasitəsilə daxil olan sətirləri normallaşdırır. Hesab identifikatorları həm tək açarlı, həm də multisig nəzarətçiləri dəstəkləyir.
