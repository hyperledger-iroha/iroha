...
- **מצב K-Collectors** – לכל גובה/View נבחרים דטרמיניסטית K אספנים באמצעות PRF מתוך הרוסטר הממויין, ללא הלידר. כאשר אין PRF seed יש fallback לפרוסה שעוטפת סביב שמתחילה ב-`proxy_tail_index()` (כולל), מדלגת על הלידר וממלאת לפחות את גודל קוורום הקומיט (מוגבל במספר העמיתים שאינם לידר). K=1 בוחר אספן יחיד לפי PRF; כאשר PRF אינו זמין הפולבק עדיין ממלא לפחות את קוורום הקומיט (מתחיל בפרוקסי‑טייל), והצבעות הקומיט עדיין נופלות לטופולוגיה המלאה אם פיזור האספנים נמוך מהקוורום.
...
- פולבק: אם `k` לא מחזיר קולקטורים, ההצבעות נופלות לטופולוגיית ה-commit המלאה; `redundant_send_r` נחתך לפחות לגודל קוורום הקומיט (בכפוף למספר האספנים הזמין).
...
- `deterministic_collectors(topology, mode, k, seed, height, view)` מחזירה סט אספנים דטרמיניסטי לפי `(height, view)` ו‑PRF seed. כאשר אין PRF seed יש fallback לבחירה שעוטפת סביב שמתחילה ב‑`proxy_tail_index()` (מדלגת על הלידר) וממלאת לפחות את קוורום הקומיט; גם ב‑K=1 הפולבק מתחיל בפרוקסי‑טייל וממלא קוורום.
...
- `sumeragi.da.enabled = true` רושם זמינות payload מקומית ב־DA gate ועוקב אחרי עדות זמינות (`availability evidence`) וקוורום RBC `READY` לצורכי ביקורת, טלמטריה וריקברי דטרמיניסטי. הם אינם קוורום קומיט נפרד. כל עוד `missing_local_data` פעיל, finalize מקומי ממתין; הוא ממשיך אחרי ש־BlockCreated או RBC/סנכרון בלוקים מביאים את ה-payload. `da_reschedule_total` הוא legacy ולכן בדרך כלל נשאר 0.
...
- `sumeragi.advanced.rbc.rebroadcast_sessions_per_tick` מגביל את מספר סשני ה־RBC שנשלחים מחדש בכל tick כדי למנוע סופות שידור כשיש backlog. להאצת התאוששות מעלים את הערך, ובמקרה של עומס תורים מורידים.
- תרחישי ה-smoke של DA/RBC משתמשים ב־`LARGE_PAYLOAD_BYTES = 1024` ובקוורום READY של הפרוטוקול, ומודדים זמני RBC, קומיט, סף קצב, עומק תורים וזרימות P2P. חריגה מתקציבי ההארנס נכשלת במבחן ומפעילה התרעה.
...
- כאשר `sumeragi.da.enabled` פעיל, `availability evidence` וקוורום RBC `READY` נעקבים כראיות ביקורת, טלמטריה וריקברי. אם payload מקומי חסר, DA gate משהה finalize מקומי עד ש־RBC `DELIVER` או סנכרון בלוקים משלימים אותו.
...
**ספי התרעה**
- זמני availability evidence: אם `sumeragi_qc_last_latency_ms{kind="availability"}` חוצה `0.6 * commit_time_ms` פעמיים או שה-P95 עובר `0.7 * commit_time_ms` (permissioned: `CommitTimeMs`, ‏NPoS: `effective_npos_timeouts.commit_ms`).
- קיפאון הצבעות: `sum(rate(sumeragi_da_votes_ingested_total[2m])) == 0` בזמן ש-`sumeragi_rbc_backlog_sessions_pending > 0`.
- קומיט: `commit_ms` או P95 של `sumeragi_phase_latency_ms{phase="commit"}` עולים על `0.75 * commit_time_ms` (permissioned: `CommitTimeMs`, ‏NPoS: `effective_npos_timeouts.commit_ms`), או נותרים אפס ≥3 סבבים למרות בלוקים חדשים.
- פיזור אספנים: `collect_aggregator_ms` > `0.5 * sumeragi.advanced.npos.timeouts.aggregator_ms` בשלושה סבבים, ‎`sumeragi_redundant_sends_total` > `sumeragi.collectors.redundant_send_r`, ‎`rate(sumeragi_gossip_fallback_total[5m]) > 0`, או ‎`increase(block_created_*[5m]) > 0`/`increase(pacemaker_backpressure_deferrals_total[5m]) > 0`.
