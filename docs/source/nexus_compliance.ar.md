...
الحالة: 🈯 مخطط — يوفر هذا المستند المواصفات وحواجز الاطلاق المشار اليها في بند خارطة الطريق **NX-12 — محرك امتثال المسارات وسياسة القائمة البيضاء**.
يشرح نموذج البيانات ومسارات الحوكمة والقياس عن بعد واستراتيجية الاطلاق التي ستنفذ داخل `crates/iroha_core/src/compliance` وتعرض عبر واجهات Torii/SDK، حتى يمكن ربط كل lane وكل dataspace بسياسات قضائية حتمية.
...
2. **التنفيذ (iroha_core)**
   - اثناء بناء الكتلة، يعيد `iroha_core::compliance::Engine` تشغيل القرار نفسه باستخدام policy id المثبت في راس الكتلة.
     يمنع ذلك التباعد حتى لو كانت كاشات Torii قديمة.
   - يجب على التعليمات التي تغير manifests الخاصة بالـ lane او السياسات توفير policy references التي تعتمد عليها
     لكي يتم التقاط تلك القيم على السلسلة.
