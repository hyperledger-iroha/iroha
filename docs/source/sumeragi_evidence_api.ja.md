...
- GET `/v1/sumeragi/evidence`
  ...
  - クエリパラメータ: `limit`（既定 50、最大 1000）、`offset`（既定 0）、`kind`（任意、`DoublePrepare|DoubleCommit|InvalidCommitCertificate|InvalidProposal` のいずれか）。
