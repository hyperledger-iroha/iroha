...
- אימות בצד קבלה מפיל כניסות שה-lanes/dataspaces שלהן לא תואמים לקטלוג המקומי, שתג ה-plane לא תואם ל-visibilty הנגזר של dataspace, או שה-route המוצהרת לא תואמת להחלטת הניתוב שמחושבת מחדש מקומית.
...
- `GET /v1/space-directory/uaids/{uaid}` — לפתור binding נוכחי של dataspace ל-UAID (כתובות מנורמלות, ids של dataspace, program bindings). הוסיפו `canonical Katakana i105 output` לפלט Sora Name Service (I105 מועדף; I105 הוא אפשרות שנייה ל‑Sora בלבד).
