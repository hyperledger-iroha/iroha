...
2. `mochi-ui`: დესკტოპის პროგრამა, რომელიც აგებულია `egui`/`eframe`-ზე, რომელიც ახდენს მომხმარებლის ინტერფეისს და დელეგირებს ყველა ორკესტრაციას `mochi-core` API-ის მეშვეობით.
...
- Torii კლიენტის თხელი ფენა ახვევს HTTP და WebSocket ზარებს, ეყრდნობა Iroha Rust კლიენტის ყუთებს, სადაც ეს შესაძლებელია, რათა თავიდან იქნას აცილებული retired codec კოდირების/დაშიფვრის ხელახალი განხორციელება.
...
- **დაბლოკეთ და ივენთების ნაკადები**: გამოიწერეთ `/v1/blocks/stream` და `/v1/events/ws`, შეინახეთ მეხსიერებაში მოძრავი ბუფერი UI პანელებისთვის.
- **State Explorer**: გაუშვით Norito მხარდაჭერილი `/v1/query` ზარები, რათა ჩამოთვალოთ დომენები, ანგარიშები, აქტივები და აქტივების განმარტებები პაგინაციის დამხმარეებით და მეტამონაცემების შეჯამებით.
- **ტრანზაქციის კომპოზიტორი**: დადგმა ზარაფხანის/გადაცემის ინსტრუქციის პროექტები, დაალაგეთ ისინი ხელმოწერილ ტრანზაქციებში, გადახედეთ Norito დატვირთვას, გაგზავნეთ `/v1/pipeline/transactions`-ით და დააკვირდით შედეგად მოვლენებს; სარდაფის ხელმოწერის კაკვები რჩება მომავალ გამეორებად.
