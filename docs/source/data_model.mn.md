...
- `IdBox`: Ямар ч дэмжигдсэн ID-д зориулсан нийлбэр төрлийн дугтуй (`DomainId`, `AccountId`, `AssetDefinitionId`, `AssetId`, `AssetId`, Kotodama, Norito `TriggerId`, `RoleId`, `Permission`, `CustomParameterId`). Ерөнхий урсгал болон Norito кодчилолд нэг төрлийн хувьд хэрэгтэй.
...
- `AccountId`: `AccountAddress`-ээр зөвхөн I105 гэж кодлогдсон домэйнгүй каноник данс танигч. Парсерын хатуу оролтууд нь каноник I105 байх ёстой; домэйны дагавар (`@domain`), дансны нэрийн литерал, каноник зургаан талт задлан шинжлэлийн оролт, хуучин `norito:` ачааллыг болон `uaid:`/`opaque:` данс задлагч маягтаас татгалзсан. Сүлжээний дансны хоч нэр нь `name@domain.dataspace` эсвэл `name@dataspace` ашигладаг ба `AccountId` стандартын утгуудаар шийдэгддэг.
...
- `Account { id, metadata, label?, uaid?, opaque_ids[] }` — `label` нь дахин түлхүүр бичлэгт ашиглагддаг нэмэлт `AccountAlias`, `uaid` нь нэмэлт Nexus өргөн [Universal Account ID]000, болон (Norito) `opaque_ids` нь тухайн UAID-д холбогдсон далд танигчдыг хянадаг. Хадгалсан бүртгэлийн төлөв нь ямар ч холбоотой домайн талбарыг агуулахаа больсон.
- Барилгачид:
  - `NewAccount` `Account::new(id)`-ээр дамжуулан каноник домэйнгүй дансны сэдвийг бүртгэдэг.
- Алс загвар:
...
- `AssetDefinition { id, name, description?, alias?, spec: NumericSpec, mintable: Mintable, logo: Option<SorafsUri>, metadata, owned_by: AccountId, total_quantity: Quantity }`.
  - `name` нь хүн рүү чиглэсэн дэлгэцийн текст шаардлагатай бөгөөд `#`/`@` агуулаагүй байх ёстой.
  ...
  - Torii хөрөнгийн тодорхойлолтын хариултууд нь `alias_binding { alias, status, lease_expiry_ms, grace_until_ms, bound_at_ms }`, Kotodama нь `permanent`, `leased_active`, Norito, эсвэл Norito, Norito-н нэгийг агуулж болно.
  - Alias ​​resolution нь зангилааны ханын цаг гэхээсээ илүү сүүлийн үеийн блокийн цагийн тэмдгийг ашигладаг. `grace_until_ms`-ийг давсны дараа шүүрдэх цэвэрлэгээ нь хуучирсан холбоосыг арилгаагүй байсан ч нэр сонгогчид шууд шийдвэрлэхээ зогсооно; Шууд тодорхойлолт уншилт нь `expired_pending_cleanup` гэж сунжирсан холболтыг мэдээлсэн хэвээр байж болно.
...
- `Asset { id, value: Quantity }`, хадгалахад тохиромжтой `AssetEntry`/`AssetValue`.- `AssetBalanceScope`: Хязгааргүй үлдэгдлийн хувьд `Global`, өгөгдлийн орон зайгаар хязгаарлагдсан үлдэгдлийн хувьд `Dataspace(DataSpaceId)`.
- `AssetTotalQuantityMap = BTreeMap<AssetDefinitionId, Quantity>` хураангуй API-д нээлттэй.

### NFT
- `NftId { domain: DomainId, name: Name }`.
- `Nft { id, content: Metadata, owned_by: AccountId }` (агуулга нь дурын түлхүүр/утга мета өгөгдөл).
- Барилгачин: `NewNft` `Nft::new(id, content)`-ээр дамжуулан.

### Үүрэг ба зөвшөөрөл
- `RoleId { name: Name }`.
- `Role { id, permissions: BTreeSet<Permission> }` барилгачин `NewRole { inner: Role, grant_to: AccountId }`.
- `Permission { name: Ident, payload: Json }` – `name` болон ачааллын схем нь идэвхтэй `ExecutorDataModel`-тай таарч байх ёстой (доороос харна уу).

### Үе тэнгийнхэн
- `PeerId { public_key: PublicKey }`.
- `Peer { address: SocketAddr, id: PeerId }` болон задлах `public_key@address` мөр хэлбэр.
...
- Зааврын бүртгэл: `instruction_registry!{ ... }` макро нь төрлийн нэрээр түлхүүрлэгдсэн ажиллах цагийн код тайлах бүртгэлийг бүтээдэг. `InstructionBox` клон болон Norito серде нь динамик (де) цуврал болгоход ашигладаг. Хэрэв `set_instruction_registry(...)`-ээр ямар ч бүртгэлийг тодорхой тохируулаагүй бол хоёртын файлыг найдвартай байлгахын тулд бүх үндсэн ISI бүхий анхдагч бүртгэлийг анх удаа ашиглахдаа залхуугаар суулгадаг.
...
- `TransactionBuilder`: `chain`, `authority`, `creation_time_ms`, нэмэлт `time_to_live_ms`, `nonce`, `nonce`, I018, `chain`, `creation_time_ms` ашиглан гүйлгээний ачааллыг бүтээдэг. `Executable`.
...
- Хэрэгслүүд: `presigned`, `set_transaction_results(...)`, `set_transaction_results_with_transcripts(...)`, `header()`, `signatures()`, `hash()`, `hash()`, Kotodama, Kotodama.
...
- Блок оруулах нотлох баримтууд (`BlockProofs`) оролт/үр дүнгийн Merkle нотолгоо болон `fastpq_transcripts` газрын зургийг хоёуланг нь ил гаргадаг тул сүлжээнээс гадуур шинжээчид гүйлгээний хэштэй холбоотой шилжүүлгийн дельтануудыг татаж авах боломжтой.
...
- DSL: `query::dsl`-д эмхэтгэх хугацаанд шалгагдсан предикатууд болон сонгогчид проекцын шинж чанартай (`HasProjection<PredicateMarker>` / `SelectorMarker`) хэрэгжсэн. `fast_dsl` функц нь шаардлагатай бол илүү хөнгөн хувилбарыг гаргадаг.
...
- Тохируулах дээжүүд `data_model/samples/executor_custom_data_model`-ийн дагуу байгаа бөгөөд үүнийг харуулж байна:
  ...
  - Захиалгат зааврыг гүйцэтгэхийн тулд `CustomInstruction` руу цувуулсан.
...
- Norito: схемийг агуулсан `norito::codec::{Encode, Decode}`-ээр кодчилдог/тайлдаг; `Json` ачааллыг тодорхойлон цуваа болгосон. Зааврын бүртгэлд `CustomInstruction` (энэ нь анхдагч бүртгэлийн нэг хэсэг) багтсан тохиолдолд хоёр талын аялал тогтвортой байна.
...
- API: `contains`, `iter`, `get`, `insert`, (`transparent_api`-тай) `remove`.
...
- Механик: `iroha_data_model_derive::model` макро нь нийтийн талбар бүрийг `#[cfg(feature = "transparent_api")] pub`-ээр дахин бичиж, анхдагч бүтээцэд зориулж хувийн хуулбарыг хадгалдаг. Энэ функцийг идэвхжүүлснээр тэдгээр cfg-г эргүүлэх тул `Account`, `Domain`, `Asset` гэх мэтийг устгах нь тэдгээрийн тодорхойлох модулиас гадуур хууль ёсны болно.
...
- Идэвхжүүлж байна: `Cargo.toml`-ийн хамааралд `features = ["transparent_api"]`-г нэмнэ. JSON проекц (жишээ нь, `iroha_torii`) шаардлагатай ажлын талбарын хайрцагнууд тугийг автоматаар дамжуулдаг боловч гуравдагч талын хэрэглэгчид байршуулалтыг хянаж, илүү өргөн API гадаргууг хүлээн зөвшөөрөхгүй бол үүнийг унтрааж байх ёстой.
...
- `POST /v1/assets/definitions/query` болон `GET /v1/assets/definitions`, `alias_binding.status`, `alias_binding.lease_expiry_ms`, `alias_binding.grace_until_ms`, `alias_binding.bound_at_ms`0, Kotodama-аас дээш хөрөнгийн тодорхойлолтыг хүлээн зөвшөөрдөг `name`, `alias`, `metadata.*`.

## Хувилбар

- `SignedTransaction`, `SignedBlock`, `SignedQuery` нь Norito кодлогдсон каноник бүтэц юм. Тус бүр нь `iroha_version::Version`-ийг `EncodeVersioned`-ээр кодолсон үед одоогийн ABI хувилбарт (одоогоор `1`) угтвар болгохын тулд `iroha_version::Version`-ийг хэрэгжүүлдэг.
