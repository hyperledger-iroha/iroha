...
- `LanePrivacyRegistry` (`crates/iroha_core/src/interlane/mod.rs`) manifest loader کی
  ...
  commitment maps محفوظ کرتا ہے۔ Transaction queue اس registry کو ہر manifest reload کے ساتھ
  install کرتی ہے (`Queue::install_lane_manifests`)، تاکہ admission path کے پاس ہمیشہ canonical
  commitments تک رسائی ہو۔
...
- Admission governance manifest handle (`crates/iroha_core/src/queue.rs`) کے ساتھ ایک
  `Arc<LanePrivacyRegistry>` رکھتا ہے، جو یقینی بناتا ہے کہ programmable-money modules اور مستقبل
  کے interlane hosts manifests rotate ہونے کے باوجود privacy descriptors کا consistent view پڑھیں۔

## Runtime Enforcement

Lane privacy proofs اب `ProofAttachment.lane_privacy` کے ذریعے transaction attachments کے ساتھ
جاتی ہیں۔ Queue ہر attachment کو routed lane کے registry کے خلاف `LanePrivacyRegistry::verify` سے
verify کرتی ہے، ثابت شدہ commitment ids ریکارڈ کرتی ہے، اور انہیں compliance engine میں feed کرتی
ہے۔ کوئی بھی rule جو `privacy_commitments_any_of` specify کرے اب `false` evaluate ہوتی ہے جب تک
کوئی matching attachment کامیابی سے verify نہ ہو جائے، لہذا programmable-money lanes required
witness کے بغیر queue نہیں ہو سکتیں۔ Unit coverage `interlane::tests` اور lane compliance tests
میں ہے تاکہ attachment path اور policy guardrails stable رہیں۔
