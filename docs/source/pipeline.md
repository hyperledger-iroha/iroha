...
- `ExecWitness` now also carries the per-transaction `fastpq_transcripts` bundles (entry hash plus transcripts) **and** a `fastpq_batches: Vec<FastpqTransitionBatch>` field so FASTPQ provers can pull both raw deltas and prover-ready rows directly from the witness stream instead of waiting for the block result bundle.
- Operators can fetch those batches via `iroha_cli audit witness --decode` (FASTPQ batches now emit by default; use `--no-fastpq-batches` to suppress them); the CLI streams the canonical `fastpq-lane-balanced` batches when present and falls back to on-the-fly conversion (using the same helper) so prover tooling always receives annotated entry-hash metadata.
- Every validator now runs the FASTPQ prover lane (`iroha_core::fastpq::lane`). The lane consumes each emitted witness, converts transcripts (or pre-baked DTO batches) into canonical `TransitionBatch`es, and invokes `fastpq_prover::Prover::canonical` in the background respecting `zk.fastpq.execution_mode` / `--fastpq-execution-mode`. Successes and failures are logged per entry hash without blocking consensus.
