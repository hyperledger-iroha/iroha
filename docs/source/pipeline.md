...
- After a successful WSV commit, `kura` writes a Norito checkpoint and commit manifest before publishing the Sumeragi-v2 finality artifact. The manifest binds the durable block, checkpoint hash, CommitQC execution roots, and a domain-separated digest of the complete cryptographically verified v2 artifact. Startup permits generic execution replay only through the contiguous full-body prefix where checkpoint, checkpoint-bound manifest, canonical block wire, and finality artifact all agree. At most one incomplete durable tip is a recoverable commit crash window: it is never pruned or generic-replayed, and network ingress remains closed while the exact frozen height context, checksummed body and validation marker, and cryptographically reverified safety-WAL Decision resume through the idempotent v2 Apply service. A missing or corrupt interior sidecar, a multi-height gap, or an impossible sidecar publication order stops startup. An explicitly configured audited hash-only snapshot is the sole exception to historical parent-CommitQC availability: its digest-pinned bootstrap record binds the exact anchor height, block hash, block timestamp, canonical WSV, first v2 context, and validator PoPs. Kura stays read-only until that record is authenticated; the first executable block derives time from the anchor plus committed cadence, and all later heights return to ordinary parent-CommitQC continuity.
