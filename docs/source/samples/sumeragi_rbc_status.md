...
- When `sumeragi.da.enabled = true`, availability evidence (an `availability evidence` or an RBC `READY` quorum) is tracked, but commit does not wait on it or on local RBC `DELIVER`. RBC remains transport/recovery and its delivery latency is still useful for debugging.
