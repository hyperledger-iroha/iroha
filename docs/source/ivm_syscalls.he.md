...
## ניהול גרסאות
- אוסף הקריאות הנתמכות תלוי בשדה `abi_version` שבכותרת הבייטקוד. מספרים שאינם מוכרים עבור `abi_version` הפעיל יגרמו ל־trap דטרמיניסטי עם `E_SCALL_UNKNOWN`.
...
- ‎`0x20..=0x5F`: גשר ISI של Iroha Core (יציב בכל גרסאות `abi_version` הנתמכות).
- ‎`0x60..=0x7F`: הרחבות ISI התלויות בפיצ'רים פרוטוקוליים.
- ‎`0x80..=0xFF`: שטח ניסיוני/ספקים (דורש `abi_version` תואם ודגל פיצ'ר).
