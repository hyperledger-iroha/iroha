...
- `Asset { id, value: Quantity }` 與儲存友善的 `AssetEntry`/`AssetValue`。- `AssetBalanceScope`：`Global` 用於無限制餘額，`Dataspace(DataSpaceId)` 用於資料空間限制餘額。
- `AssetTotalQuantityMap = BTreeMap<AssetDefinitionId, Quantity>` 公開用於摘要 API。
