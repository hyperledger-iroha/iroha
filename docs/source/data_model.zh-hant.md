...
- 生成器：`NewDomain` 與 `with_logo`、`with_metadata`，然後 `Registrable::build(authority)` 設定 `owned_by`。
...
### 資產定義與資產
- `AssetDefinitionId { aid_bytes: [u8; 16] }` 以文字形式公開為帶有版本控制和校驗和的無前綴 Base58 位址。
...
- `Asset { id, value: Numeric }` 與儲存友善 `AssetEntry`/`AssetValue`。- `AssetBalanceScope`：`Global` 用於無限制餘額，`Dataspace(DataSpaceId)` 用於資料空間受限餘額。
...
這些類型與現有的 Ed25519/BLS/ML-DSA 原語並存，一旦使用 `--features sm` 建置工作區，它們就會成為公共模式的一部分。
...
- `TransactionBuilder`：使用 `chain`、`authority`、`creation_time_ms`、選購 `time_to_live_ms` 和 `nonce`、I1800002383000024X200237X、I1000032020202020202320202020200202020202020202020202000202027X200020200202002002027X。
  - 幫助程式：`with_instructions`、`with_bytecode`、`with_executable`、`with_metadata`、`set_nonce`、`set_ttl`I18080402020402020202020202020202020200202020202020200202020202020202020202002020202020202020202020020202020200202020202020202002020202020202020020202020202002020202020020202020202020020202020202020202020202。
...
- 實用程式：`presigned`、`set_transaction_results(...)`、`set_transaction_results_with_transcripts(...)`、`header()`、`signatures()`、`hash()`I10807020070020200202021000202100702020202021070020202107020210702020210700202021070020210700202120700702。
- Merkle 根：交易入口點和結果透過 Merkle 樹提交；結果 Merkle root 被放入區塊頭中。
- 區塊包含證明 (`BlockProofs`) 公開條目/結果 Merkle 證明和 `fastpq_transcripts` 映射，以便鏈外證明者可以獲得與交易哈希相關的傳輸增量。
- `ExecWitness` 訊息（透過 Torii 串流並搭載共識八卦）現在包括 `fastpq_transcripts` 和帶有嵌入式 `public_inputs`（dsid、slot、roots、perm_root、 tx_0000277X（dsid、slot、roots、perm_root、 tx_set000277X（dsid、slot、roots、perm_root、 tx_set_QPAST行，而無需重新編碼轉錄本。
...
- 註冊表和編解碼器：
  ...
  - `QueryOutputBatchBox` 是同質向量的求和類型（例如，`Vec<Account>`、`Vec<Name>`、`Vec<AssetDefinition>`、`Vec<BlockHeader>`），以及NI00000295X、`Vec<BlockHeader>`），以及用於擴充高分頁的元組和擴展助手。
...
- 功能控制選用 API（`std`、`json`、`transparent_api`、`ffi_export`、`ffi_import`、`fast_dsl`、I303010354X `fault_injection`）。
- 確定性：所有序列化都使用 Norito 編碼，以便跨硬體移植。 IVM 字節碼是不透明的位元組 blob；執行不得引入非確定性縮減。主機驗證事務並向 IVM 確定性地提供輸入。

#

## 透明 API (`transparent_api`)- 目的：公開對 `#[model]` 結構/枚舉的完整、可變訪問，以用於內部組件（例如 Torii、執行器和集成測試）。如果沒有它，這些專案就會故意不透明，因此外部 SDK 只能看到安全的建構函式和編碼的有效負載。
...
- 除了 `id` 之外，`POST /v1/assets/definitions/query` 和 `GET /v1/assets/definitions` 還接受 `alias_binding.status`、`alias_binding.lease_expiry_ms`、`alias_binding.grace_until_ms` 和 I01835742356X、`alias_binding.grace_until_ms` 和過濾器上。 `name`、`alias` 和 `metadata.*`。
...
- 指令系列：展開公共文檔，列出 `mint_burn`、`register`、`transfer` 公開的內建 ISI 變體。
