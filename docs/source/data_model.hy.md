...
- Շինարարներ.
  - `NewAccount`-ը `Account::new_in_domain(id, domain)`-ի միջոցով իրականացնում է տիրույթի հետ կապված հստակ գրանցում և հետևաբար պահանջում է `ScopedAccountId`:
  - `NewAccount`-ը `Account::new(id)`-ի միջոցով գրանցում է միայն համընդհանուր հաշվի առարկան՝ առանց կապված տիրույթի:
- Alias մոդելը:
