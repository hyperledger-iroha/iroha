...
- Құрылысшылар:
  - `NewAccount` `Account::new_in_domain(id, domain)` арқылы анық доменге байланысты тіркеуді жүзеге асырады, сондықтан `ScopedAccountId` қажет.
  - `NewAccount` `Account::new(id)` арқылы байланысқан домені жоқ әмбебап тіркелгі тақырыбын ғана тіркейді.
- Бүркеншік ат үлгісі:
