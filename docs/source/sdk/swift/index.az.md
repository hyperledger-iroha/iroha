...
- Policy: the bridge is required by default; set `IROHASWIFT_USE_BRIDGE=optional` to compile
  ...
  CI. When disabled, SDK helpers return `bridgeUnavailable`/`nativeBridgeUnavailable` with
  the env hint and expected location.
...
> **Roadmap ADDR-5a:** Account-aware helpers (`getAssets`, `getTransactions`, and the matching `IrohaSDK` wrappers) accept I105/canonical literals and percent-encode `/v2/accounts/{account_id}/…` paths automatically so wallets can forward whatever selector they display without manual escaping.
