...
| Envelope handoff | Display IH58 QR + copy buttons and expose the mock hand-off inbox/endpoint so OA12 drills can cite the exported values | `scripts/android_sample_env.sh --handoff` seeds the inbox/endpoint; audit events flow through `PosAuditLogger` |
