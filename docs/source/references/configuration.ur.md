...
- `[sorafs.repair]`: مرمت شیڈولر کی ترتیبات۔ کلیدیں: `enabled` (ڈیفالٹ `false`)، اختیاری `db_dsn`، `db_pool_max_connections` (ڈیفالٹ `8`)، `claim_ttl_secs` (ڈیفالٹ `900`)، `heartbeat_interval_secs` (ڈیفالٹ `60`)، `max_attempts` (ڈیفالٹ `3`)، `worker_concurrency` (ڈیفالٹ `4`)، `backoff_initial_secs` (ڈیفالٹ `5`)، `backoff_max_secs` (ڈیفالٹ `60`)، اور `default_slash_penalty_nano` (ڈیفالٹ `1000000000`)۔
