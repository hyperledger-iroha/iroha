...
- `[ivm.banner]` (ڈیفالٹس: `show = true`, `beep = true`): Norito/IVM اسٹارٹ اپ بینر اور اختیاری جِنگل کنٹرول کرتا ہے۔ `beep` صرف تب اثر انداز ہوتا ہے جب بائنری `beep` فیچر کے ساتھ بنی ہو۔ یہ legacy `IROHA_BEEP` env shim کو بدلتا ہے—بینر یا بیپ کو env overrides کے بجائے کنفیگ سے بند کریں۔
...
- `[sumeragi]`: کنسینسس رول (validator/observer) اور collector سیٹنگز (K, redundant‑send r) کے ساتھ timing slack۔ DA availability `da_enabled` availability evidence (AvailabilityQC یا RBC `READY` quorum) کو ٹریک کرتا ہے مگر commit کو مؤخر نہیں کرتا؛ commit لوکل RBC deliveries کا انتظار نہیں کرتا اور legacy `da_reschedule_total` عام طور پر صفر رہتا ہے۔ per‑queue backpressure کو `msg_channel_cap_votes`, `msg_channel_cap_block_payload`, `msg_channel_cap_rbc_chunks`, `msg_channel_cap_blocks`, `control_msg_channel_cap` سے کنفیگر کیا جاتا ہے؛ legacy `msg_channel_cap` صرف تب fallback ہے جب per‑queue caps نہ ہوں۔ `mode_flip_enabled` (ڈیفالٹ `true`) live permissioned↔NPoS cutovers کے لیے kill switch ہے: `false` ہونے پر نوڈ staged mode کو status/telemetry میں ظاہر کرتا ہے مگر flip نہیں کرتا جب تک فلیگ بحال نہ ہو۔ persistence failures `kura_store_retry_interval_ms`/`kura_store_retry_max_attempts` کے مطابق backoff ہوتے ہیں تاکہ بلاکس pending رہیں؛ `commit_inflight_timeout_ms` stuck commit jobs کو abort کرتا ہے تاکہ proposals نہ رکیں۔ missing‑block fetch `missing_block_signer_fallback_attempts` (ڈیفالٹ 1؛ 0 پر ترجیح ختم) تک QC signers کو ترجیح دیتا ہے، پھر full commit topology پر آتا ہے۔ رکنیت کے ہیش میں انحراف `membership_mismatch_alert_threshold` (الرٹ سے پہلے مسلسل عدم مطابقت) اور `membership_mismatch_fail_closed` (ابھی تک عدم مطابقت رکھنے والے پئیرز کے کنسینس پیغامات ڈراپ کریں) کے ذریعے ظاہر کیا جاتا ہے۔ pacemaker proposal interval `npos.block_time_ms`, `npos.timeouts.propose`, `pacemaker_rtt_floor_multiplier` سے اخذ ہو کر `pacemaker_max_backoff_ms` سے capped ہوتا ہے؛ high‑latency لنکس پر RTT floor یا block time بڑھائیں۔ RBC READY/DELIVER quorum ہمیشہ commit topology (`Topology::min_votes_for_commit()`) کے مطابق ہے۔
- `[torii]`: پبلک API سرور سیٹنگز جیسے listen address، request size caps، اور query store parameters۔
- `torii.data_dir` (ڈیفالٹ: `./storage/torii`): Torii persistence کی بنیاد (attachments, webhook registry, DA caches)۔ legacy `IROHA_TORII_DATA_DIR` env shim کی جگہ لیتا ہے؛ tests عارضی override کے لیے `data_dir::OverrideGuard` استعمال کر سکتے ہیں۔
...
- `torii.strict_addresses` (ڈیفالٹ: `true`): enabled ہونے پر Torii ہر app‑facing endpoint پر legacy `alias@domain`/raw public‑key literals رد کرتا ہے اور `ERR_STRICT_ADDRESS_REQUIRED` لوٹاتا ہے۔ صرف dev/test میں رجعت تشخیص کے لیے `false` کریں اور canonical adoption کی تصدیق کے بعد `true` واپس کریں۔
...
- `[kura]`: بلاک اسٹوریج پاتھ اور میموری ونڈو؛ snapshots/replay کی initialization policy۔ `block_sync_roster_retention` (ڈیفالٹ 512) commit‑roster snapshots کے ذخیرہ کو محدود کرتا ہے، `roster_sidecar_retention` (ڈیفالٹ 512) `pipeline/roster_sidecars.*` میں sidecars کی تعداد محدود کرتا ہے۔
...
- `[governance]`: verifying keys، conviction parameters، approval thresholds، turnout floor، اور council sizing/eligibility۔
  - `debug_tie_break` / `debug_tx_eval` (ڈیفالٹ: `false`) debugging کے لیے overlay evaluation traces فعال کرتے ہیں؛ legacy `IROHA_DEBUG_TIE_BREAK`/`IROHA_DEBUG_TX_EVAL` env shims کی جگہ لیتے ہیں؛ پروڈکشن میں بند رکھیں۔
- `[norito]`: Norito serialization tuning (compression sizes/levels اور adaptive layout toggles)۔ design کے لحاظ سے deterministic۔
