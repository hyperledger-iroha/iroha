...
- جب `nexus.enabled = false` (Iroha 2 موڈ) ہو تو کنفیگز سنگل‑لین رہتے ہیں: `/status` اور `/v2/sumeragi/status` میں لین/ڈیٹاسپیس حصے چھپ جاتے ہیں، `/metrics` لین/ڈیٹاسپیس لیبلز بالکل چھوڑ دیتا ہے، اور لین‑اسکوپڈ اسٹیٹس ٹیلز `StatusSegmentNotFound` لوٹاتے ہیں؛ لہٰذا SDKs کو لین پیرامیٹرز بھیجنے سے پہلے فلیگ پر برانچ کرنا چاہیے۔ اس موڈ میں لین/ڈیٹاسپیس/روٹنگ overrides مسترد ہوتے ہیں—`nexus.*` سیکشنز ہٹا دیں جب تک `nexus.enabled=true` یا `--sora` سے Nexus/Sora فعال نہ ہو۔
- `[common]`: چین شناخت (`chain`)، نوڈ کی key pair، trusted peers، اکاؤنٹ ڈومین کا ضمنی لیبل (`default_account_domain_label`)، اور I105 chain discriminant (`chain_discriminant`)۔ اگر آپ کا genesis ڈومین یا نیٹ ورک پری فکس ڈیفالٹس سے مختلف ہو تو یہاں override کریں تاکہ I105/کمپریسڈ ایڈریسز canonical رہیں۔
...
- `[sumeragi]`: کنسینسس رول (validator/observer) اور collector سیٹنگز (K, redundant‑send r) کے ساتھ timing slack۔ DA tracking `sumeragi.da.enabled` سے فعال ہوتا ہے؛ availability evidence (availability evidence یا RBC `READY` quorum) advisory ہے اور commit کو بلاک نہیں کرتا۔ لوکل payload نہ ہو تو RBC deliveries یا BlockCreated/بلاک sync سے حاصل کیا جاتا ہے۔ per‑queue backpressure کو `sumeragi.advanced.queues.votes`, `sumeragi.advanced.queues.block_payload`, `sumeragi.advanced.queues.rbc_chunks`, `sumeragi.advanced.queues.blocks`, `sumeragi.advanced.queues.control` سے کنفیگر کیا جاتا ہے۔ `sumeragi.mode_flip.enabled` (ڈیفالٹ `true`) live permissioned↔NPoS cutovers کے لیے kill switch ہے: `false` ہونے پر نوڈ staged mode کو status/telemetry میں ظاہر کرتا ہے مگر flip نہیں کرتا جب تک فلیگ بحال نہ ہو۔ persistence failures `sumeragi.persistence.kura_retry_interval_ms`/`sumeragi.persistence.kura_retry_max_attempts` کے مطابق backoff ہوتے ہیں تاکہ بلاکس pending رہیں؛ `sumeragi.persistence.commit_inflight_timeout_ms` stuck commit jobs کو abort کرتا ہے تاکہ proposals نہ رکیں۔ missing‑block fetch `sumeragi.recovery.missing_block_signer_fallback_attempts` (ڈیفالٹ 1؛ 0 پر ترجیح ختم) تک commit certificate signers کو ترجیح دیتا ہے، پھر full commit topology پر آتا ہے۔ رکنیت کے ہیش میں انحراف `sumeragi.gating.membership_mismatch_alert_threshold` (الرٹ سے پہلے مسلسل عدم مطابقت) اور `sumeragi.gating.membership_mismatch_fail_closed` (ابھی تک عدم مطابقت رکھنے والے پئیرز کے کنسینس پیغامات ڈراپ کریں) کے ذریعے ظاہر کیا جاتا ہے۔ pacemaker proposal interval `SumeragiParameters.block_time_ms (scaled by pacing_factor_bps)`, `sumeragi.advanced.npos.timeouts.propose_ms`, `sumeragi.advanced.pacemaker.rtt_floor_multiplier` سے اخذ ہو کر `sumeragi.advanced.pacemaker.max_backoff_ms` سے capped ہوتا ہے؛ high‑latency لنکس پر RTT floor یا block time بڑھائیں۔ `sumeragi.advanced.npos.timeouts.*_ms` اور `sumeragi.npos.vrf.*` کے غیر متعین فیلڈز `SumeragiParameters.block_time_ms (scaled by pacing_factor_bps)` اور `sumeragi.npos.epoch_length_blocks` سے ڈیفالٹ نسبتوں کے مطابق اخذ ہوتے ہیں۔ INIT سے پہلے RBC stashز کو سیشنز کے درمیان `sumeragi.advanced.rbc.pending_session_limit` کے ذریعے محدود کیا جاتا ہے۔ RBC READY/DELIVER quorum ہمیشہ commit topology (`Topology::min_votes_for_commit()`) کے مطابق ہے۔
- `[torii]`: پبلک API سرور سیٹنگز جیسے listen address، request size caps، اور query store parameters۔
- `torii.events_buffer_capacity` (ڈیفالٹ: `10000`): `/v2/events/sse` اور webhook enqueue کے لیے broadcast channel سائز۔ سبسکرائبرز لیٹ ہوں تو میموری/بیک‑پریشر محدود کرنے کے لیے کم کریں۔
...
- `[sorafs.storage.pin]`: `/v2/sorafs/storage/pin` کے لیے auth/rate limits۔ `require_token=true` bearer token لگاتا ہے (`Authorization: Bearer ...` یا `X-SoraFS-Pin-Token`)، `tokens` allow‑list دیتا ہے، `allow_cidrs` clients محدود کرتا ہے، اور `rate_limit` token‑bucket (اختیاری ban کے ساتھ) لگاتا ہے۔ guards fail ہوں تو Torii pin سے پہلے ہی 401/403/429 واپس کرتا ہے، throttled responses میں `Retry-After` شامل ہوتا ہے۔
...
- `[norito]`: Norito serialization tuning (compression sizes/levels اور adaptive layout toggles)۔ design کے لحاظ سے deterministic۔ `max_archive_len` ڈیفالٹ 512 MiB ہے اور اسے کم از کم `sumeragi.advanced.rbc.store_max_bytes` اور `network.max_frame_bytes` تک بڑھایا جاتا ہے تاکہ consensus payloads decodable رہیں۔
