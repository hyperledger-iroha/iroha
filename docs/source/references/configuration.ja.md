...
- `[sorafs.repair]`: 修復スケジューラ設定。キー: `enabled`（既定 `false`）、任意の `db_dsn`、`db_pool_max_connections`（既定 `8`）、`claim_ttl_secs`（既定 `900`）、`heartbeat_interval_secs`（既定 `60`）、`max_attempts`（既定 `3`）、`worker_concurrency`（既定 `4`）、`backoff_initial_secs`（既定 `5`）、`backoff_max_secs`（既定 `60`）、`default_slash_penalty_nano`（既定 `1000000000`）。
