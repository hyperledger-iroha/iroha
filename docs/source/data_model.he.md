...
- בונים:
  - `NewAccount` דרך `Account::new_in_domain(id, domain)` מממש רישום מפורש המקושר לדומיין ולכן דורש `ScopedAccountId`.
  - `NewAccount` דרך `Account::new(id)` רושם רק את נושא החשבון האוניברסלי ללא דומיין מקושר.
- דגם כינוי:
