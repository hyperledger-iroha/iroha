...
**ייצוגים טקסטואליים:**
- `DomainId`: `wonderland`
- `AccountId`: הכתובת הקנונית מנוהלת דרך `AccountAddress` עם קודקים ל‑I105, תצוגת סורה הדחוסה (`i105`) והקסדצימלית (`canonical_hex`). I105 הוא הפורמט המועדף, ו־`i105` הוא אפשרות שנייה ייעודית ל‑Sora. מחרוזות `alias` (rejected legacy form) נשמרות ככינויי ניתוב בלבד. Torii מנרמל קלטים דרך `AccountAddress::parse_encoded`. Supports single-key and multisig controllers.
