...
- `IdBox`: מעטפה מסוג סכום עבור כל מזהה נתמך (`DomainId`, `AccountId`, `AssetDefinitionId`, `AssetId`, Kotodama0000720, Kotodama0,000720, `TriggerId`, `RoleId`, `Permission`, `CustomParameterId`). שימושי עבור זרימות גנריות וקידוד Norito כסוג יחיד.
...
- Builder: `NewDomain` עם `with_logo`, `with_metadata`, ואז `Registrable::build(authority)` ערכות `owned_by`.

### חשבון
- `AccountId` היא זהות החשבון הקנונית ללא דומיין הממוקמת על ידי הבקר ומקודדת כ-I105 הקנונית.
- `ScopedAccountId { account: AccountId, domain: DomainId }` נושא קונטקסט מפורש של תחום רק כאשר נדרשת תצוגה בהיקף.
- `Account { id, metadata, label?, uaid?, linked_domains? }` — `label` הוא כינוי יציב אופציונלי המשמש על ידי רשומות מפתח מחדש, `uaid` נושא את האופציונלי Nexus רחב [מזהה חשבון אוניברסלי](Kotodama) ו-I0184NI, ו-I0184NI מצב אינדקס נגזר ולא חלק מהזהות הקנונית.
- בונים:
  - `NewAccount` דרך `Account::new(scoped_id)` מממש רישום מפורש המקושר לדומיין ולכן דורש `ScopedAccountId`.
  - `NewAccount` דרך `Account::new_domainless(id)` רושם רק את נושא החשבון האוניברסלי ללא דומיין מקושר.
- דגם כינוי:
  - זהות חשבון קנונית לעולם אינה כוללת תחום או פלח מרחב נתונים.
  - כינויים של חשבון הם כריכות נפרדות של SNS/תווית חשבון הממוקמות על גבי `AccountId`.
  - כינויים מוסמכים לדומיין כגון `merchant@hbl.sbp` נושאים גם דומיין וגם מרחב נתונים בקישור הכינוי.
  - כינויים של בסיס נתונים של מרחב נתונים כגון `merchant@sbp` נושאים רק את מרחב הנתונים ולכן מתאימים באופן טבעי ל-`Account::new_domainless(...)`.
  - בדיקות ותקנים צריכים לראות תחילה את ה-`AccountId` האוניברסלי, ולאחר מכן להוסיף קישורי דומיין, חכירות כינוי והרשאות כינוי בנפרד במקום לקודד הנחות תחום לזהות החשבון עצמו.
...
- `AssetDefinition { id, name, description?, alias?, spec: NumericSpec, mintable: Mintable, logo: Option<SorafsUri>, metadata, owned_by: AccountId, total_quantity: Numeric }`.
  ...
  - Torii תגובות להגדרת נכס עשויות לכלול `alias_binding { alias, status, lease_expiry_ms, grace_until_ms, bound_at_ms }`, כאשר `status` הוא אחד מ-`permanent`, `leased_active`, Nexus, או.
  - רזולוציית כינוי משתמשת בחותמת הזמן העדכנית ביותר של חסימה מחויבת במקום בשעון קיר צומת. לאחר שה-`grace_until_ms` חלף, בוררי הכינוי מפסיקים להיפתר באופן מיידי, גם אם ניקוי הניקוי עדיין לא הסיר את הכריכה המעופשת; קריאות הגדרה ישירה עדיין עשויות לדווח על הכריכה המתמשכת כ-`expired_pending_cleanup`.
...
טיפוסים אלה יושבים לצד הפרימיטיבים הקיימים של Ed25519/BLS/ML-DSA והופכים לחלק מהסכמה הציבורית ברגע שמרחב העבודה נבנה עם `--features sm`.
...
- `EventBox`: סוג סכום עבור אירועי צינור, אצווה של צינור, נתונים, זמן, הפעלה-טריגר ואירועי טריגר שהושלמו; `EventFilterBox` משקף את זה עבור מנויים ומסנני טריגר.
...
- `InstructionBox`: עטיפה של `Box<dyn Instruction>` בבעלות עם שיבוט/eq/ord מיושם באמצעות מזהה סוג + בתים מקודדים.
...
- שגיאות: מודל שגיאה עשיר תחת `isi::error` (שגיאות בסוג הערכה, מצא שגיאות, יכולת טביעה, מתמטיקה, פרמטרים לא חוקיים, חזרה, אינוריאנטים).
- רישום הוראות: המאקרו `instruction_registry!{ ... }` בונה רישום פענוח בזמן ריצה המבוסס על שם סוג. בשימוש על ידי שיבוט `InstructionBox` ו-Norito כדי להשיג (דה) סדרה דינמית. אם שום רישום לא הוגדר במפורש דרך `set_instruction_registry(...)`, רישום ברירת מחדל מובנה עם כל ליבת ISI מותקן בעצלתיים בשימוש הראשון כדי לשמור על יציבות בינאריות.
...
- `TransactionBuilder`: בונה מטען עסקאות עם `chain`, `authority`, `creation_time_ms`, אופציונלי `time_to_live_ms` ו-Norito, I0100028NI8X, I01000230X, an `Executable`.
  - עוזרים: `with_instructions`, `with_bytecode`, `with_executable`, `with_metadata`, `set_nonce`, `set_ttl`, Kotodama.
...
- כלי עזר: `presigned`, `set_transaction_results(...)`, `set_transaction_results_with_transcripts(...)`, `header()`, `signatures()`, `hash()`, `header()`, `signatures()`.
- שורשי מרקל: נקודות כניסה ותוצאות של עסקאות מבוצעות באמצעות עצי מרקל; תוצאה שורש Merkle ממוקם בכותרת הבלוק.
- הוכחות הכללה בלוק (`BlockProofs`) חושפות הן הוכחות כניסה/תוצאה של Merkle והן את מפת `fastpq_transcripts` כך שמוכיחים מחוץ לשרשרת יכולים להביא את דלתות ההעברה הקשורות ל-hash של עסקאות.
- הודעות `ExecWitness` (מוזרמות דרך Torii ומבוססות על רכילות קונצנזוס) כוללות כעת הן `fastpq_transcripts` והן `fastpq_batches: Vec<FastpqTransitionBatch>` המוכנות להוכחה עם `fastpq_batches: Vec<FastpqTransitionBatch>` מוטבעים עם `fastpq_batches: Vec<FastpqTransitionBatch>` עם משובצים Kotodama perm_root, tx_set_hash), כך שמוכיחים חיצוניים יכולים להטמיע שורות FASTPQ קנוניות ללא קידוד מחדש של תמלילים.

## שאילתות- שני טעמים:
  - יחיד: יישם `SingularQuery<Output>` (לדוגמה, `FindParameters`, `FindExecutorDataModel`).
  - ניתן לחזרה: יישם `Query<Item>` (למשל, `FindAccounts`, `FindAssets`, `FindDomains` וכו').
- טפסים שנמחקו מהסוג:
...
- רישום וקודקים:
  ...
  - `QueryOutputBatchBox` הוא סוג סכום מעל וקטורים הומוגניים (לדוגמה, `Vec<Account>`, `Vec<Name>`, `Vec<AssetDefinition>`, `Vec<BlockHeader>`), בתוספת עוזרי עימוד ועימוד יעילים.
...
- כולל שליטה בממשקי API אופציונליים (`std`, `json`, `transparent_api`, `ffi_export`, `ffi_import`, `fast_dsl`, I103NI30X, I103NI3000 `fault_injection`).
...
- הפעלה: הוסף את `features = ["transparent_api"]` לתלות ב-`Cargo.toml`. ארגזים של סביבת עבודה הזקוקים להקרנת JSON (למשל, `iroha_torii`) מעבירים את הדגל באופן אוטומטי, אך צרכנים של צד שלישי צריכים לשמור אותו כבוי אלא אם כן הם שולטים בפריסה ומקבלים את משטח ה-API הרחב יותר.
...
- `POST /v1/assets/definitions/query` ו-`GET /v1/assets/definitions` מקבלים מסננים/מיונים של הגדרת נכסים מעל `alias_binding.status`, `alias_binding.lease_expiry_ms`, `alias_binding.grace_until_ms`, ו-`iroha_data_model` בנוסף ל-`iroha_data_model`, `name`, `alias` ו-`metadata.*`.

## גירסאות

- `SignedTransaction`, `SignedBlock` ו-`SignedQuery` הם מבנים קנוניים מקודדים Norito. כל אחד מיישם את `iroha_version::Version` כדי להגדיר את המטען שלו עם גרסת ה-ABI הנוכחית (כיום `1`) כאשר מקודדים באמצעות `EncodeVersioned`.
