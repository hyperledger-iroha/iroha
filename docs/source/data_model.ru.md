...
- Строители:
  - `NewAccount` через `Account::new_in_domain(id, domain)` материализует явную регистрацию, связанную с доменом, и поэтому требует `ScopedAccountId`.
  - `NewAccount` через `Account::new(id)` регистрирует только субъект универсальной учетной записи без связанного домена.
- Модель псевдонима:
