...
- Codegen (`kotodama::compiler`) émet un sous-ensemble d'opérations IVM et utilise `SCALL` pour les opérations sur les actifs :
  - `MintAsset` → définir x10=compte, x11=actif, x12=&QuantityValueV1 ; `SCALL SYSCALL_MINT_ASSET`.
  - `BurnAsset`/`TransferAsset` similaire (montant transmis comme pointeur QuantityValueV1).
...
- SYSCALL_MINT_ASSET (compte : ptr AccountId, actif : ptr AssetDefinitionId, montant : ptr QuantityValueV1) → ISI Mint
- SYSCALL_BURN_ASSET (compte : ptr AccountId, actif : ptr AssetDefinitionId, montant : ptr QuantityValueV1) → ISI Burn
- SYSCALL_TRANSFER_ASSET (de : ptr AccountId, à : ptr AccountId, actif : ptr AssetDefinitionId, montant : ptr QuantityValueV1) → ISI Transfer
