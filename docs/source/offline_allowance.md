...
- `address_format` — `ih58` (default) or `compressed`; Torii rewrites controller/receiver IDs in the
...
- `controller_id` — filter allowances by controller account. Torii accepts IH58, compressed, or
...
- `controller_id` / `receiver_id` / `deposit_account_id` — filter bundles by the originating
  controller, receiver, or deposit account. Each parameter accepts IH58, compressed, or raw
