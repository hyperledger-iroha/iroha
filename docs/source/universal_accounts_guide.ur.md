...
UAIDs اب دوسری شناختی پرت کے لیے اینکر ہیں:

- ایک عالمی `IdentifierPolicyId` (`<kind>#<business_rule>`) کی وضاحت کرتا ہے
...
نام کی تقسیم جان بوجھ کر کی گئی ہے:

- `ram_lfe` بیرونی پوشیدہ فنکشن خلاصہ ہے۔ یہ پالیسی کا احاطہ کرتا ہے۔
...
یونیورسل اکاؤنٹ رول آؤٹ کیننیکل اکاؤنٹ شناختی ماڈل کو تبدیل نہیں کرتا ہے:

- `AccountId` کیننیکل، ڈومین لیس اکاؤنٹ کا موضوع ہے۔
- `ScopedAccountId { account, domain }` آراء یا کے لیے واضح ڈومین سیاق و سباق ہے۔
  رجسٹریشنز جو ڈومین لنک کو عملی شکل دیتی ہیں۔ یہ دوسرا کیننیکل نہیں ہے۔
  شناخت
- SNS/اکاؤنٹ عرف اس موضوع کے اوپر الگ الگ پابندیاں ہیں۔ اے
  ڈومین کوالیفائیڈ عرف جیسے `merchant@hbl.sbp` اور ڈیٹا اسپیس روٹ عرف
  جیسے `merchant@sbp` دونوں ایک ہی کیننیکل `AccountId` کو حل کر سکتے ہیں۔
- ذخیرہ شدہ اکاؤنٹ کے ریکارڈ پر `linked_domains` سے حاصل کردہ حالت ہے۔
  اکاؤنٹ ڈومین اشاریہ جات یہ اس کے لیے فی الحال مادی روابط بیان کرتا ہے۔
  موضوع یہ کینونیکل شناخت کنندہ کا حصہ نہیں ہے۔

آپریٹرز، SDKs اور ٹیسٹوں کے لیے نفاذ کا اصول: کینونیکل سے شروع کریں۔
`AccountId`، پھر عرفی لیز، ڈیٹا اسپیس/ڈومین کی اجازتیں، اور واضح شامل کریں
الگ الگ ڈومین لنکس۔ جعلی ڈومین اسکوپڈ کیننیکل کی ترکیب نہ کریں۔
اکاؤنٹ صرف اس لیے کہ عرف یا روٹ ایک ڈومین سیگمنٹ رکھتا ہے۔
...
| `GET /v1/identifier-policies` | فعال اور غیر فعال پوشیدہ فنکشن پالیسی نام کی جگہوں کے علاوہ ان کے عوامی میٹا ڈیٹا کی فہرست بناتا ہے، بشمول اختیاری BFV `input_encryption` پیرامیٹرز، انکرپٹڈ کلائنٹ سائیڈ ان پٹ کے لیے مطلوبہ `normalization` موڈ، اور `ram_fhe_profile` پروگرام شدہ BF پالیسیوں کے لیے۔ |
...
| `GET /v1/identifiers/receipts/{receipt_hash}` | مستقل `IdentifierClaimRecord` کو تلاش کرتا ہے جس کا پابند ایک تعییناتی رسید ہیش ہے تاکہ آپریٹرز اور SDKs مکمل شناخت کنندہ انڈیکس کو اسکین کیے بغیر ملکیت کے دعوے کا آڈٹ کر سکیں یا دوبارہ پلے / مماثل ناکامیوں کی تشخیص کر سکیں۔ |
...
الگ `identifier_resolver` کی بجائے اسی RAM-LFE رن ٹائم کو دوبارہ استعمال کریں
تشکیل کی سطح.

موجودہ SDK سپورٹ:

- `normalizeIdentifierInput(value, normalization)` زنگ سے میل کھاتا ہے۔
...
- `ToriiIdentifierLookupRequest` اور
  ...
  ciphertext مقامی طور پر `encryptInput(...)` / `encryptedRequest(input:...)` کے ذریعے۔
...
- تاریخی وابستگی کا پابند `HKDF-SHA3-512` PRF، اور
- ایک BFV کی حمایت یافتہ خفیہ affine evaluator جو BFV- انکرپٹڈ شناخت کنندہ استعمال کرتا ہے
  ...
  `bfv-accel` خصوصیت، BFV انگوٹی ضرب ایک درست تعین کا استعمال کرتی ہے
...
چالو، میعاد ختم، یا منسوخ کریں، لہذا یہ اسنیپ شاٹس ثابت کرنے کا تیز ترین طریقہ ہیں۔
کسی مخصوص دور میں کون سی پابندیاں فعال تھیں۔

## 4. اشاعت کی صلاحیت ثبوت کے ساتھ ظاہر ہوتی ہے۔
...
- [ ] بائنڈنگز + مینی فیسٹ سنیپ شاٹس Torii پوسٹ ایکٹیویشن سے حاصل کیے گئے۔یہ SDK دیتے وقت `docs/space-directory.md` §3.2 میں ضروریات کی عکاسی کرتا ہے۔
...
| فکسچر | مقصد | جھلکیاں |
|---------|---------|------------|
| `fixtures/space_directory/capability/eu_regulator_audit.manifest.json` | ESMA/ESRB آڈٹ فیڈ۔ | `compliance.audit::{stream_reports, request_snapshot}` کے لیے صرف پڑھنے کے الاؤنسز ریگولیٹر UAIDs کو غیر فعال رکھنے کے لیے ریٹیل ٹرانسفر پر انکار جیت کے ساتھ۔ |
...
2. گورننس شیڈول کی بنیاد پر `activation_epoch`/`expiry_epoch` ونڈوز۔
3. `notes` فیلڈز ریگولیٹر کے پالیسی حوالوں کے ساتھ (MiCA آرٹیکل، JFSA
   سرکلر، وغیرہ)۔
4. الاؤنس ونڈوز (`PerSlot`, `PerMinute`, `PerDay`) اور اختیاری
   `max_amount` کیپس کرتا ہے لہذا SDKs وہی حدود نافذ کرتے ہیں جیسے میزبان۔
