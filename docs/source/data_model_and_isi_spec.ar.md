...
- `AccountId` - يتم إنتاج العناوين الأساسية عبر `AccountAddress` (I105 / `i105` مضغوط / سداسي عشري) ويقوم Torii بتسوية المدخلات من خلال `AccountAddress::parse_encoded`. I105 هو تنسيق الحساب المفضل؛ يعد نموذج `i105` هو ثاني أفضل نموذج لـ Sora-only UX. يتم الاحتفاظ بسلسلة `alias` (rejected legacy form) المألوفة كاسم مستعار للتوجيه فقط. الحساب: `{ id, metadata }`. الكود: `crates/iroha_data_model/src/account.rs`.
