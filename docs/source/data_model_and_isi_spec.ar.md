...
- `AssetId`: `<asset-definition-id>#<account-id>` الحرفي المشفر بشكل أساسي (النماذج النصية القديمة غير مدعومة في الإصدار الأول).- `NftId` — `nft$domain`. إن إف تي: `{ id, content: Metadata, owned_by }`. الكود: `crates/iroha_data_model/src/nft.rs`.
...
- تظل معرفات الأصول عند حدود النعناع/الحرق/النقل أساسية `<asset-definition-id>#<account-id>`؛ استخدم `iroha tools encode asset-id` مع `--definition <base58-asset-definition-id>` أو `--alias ...` بالإضافة إلى `--account`.
