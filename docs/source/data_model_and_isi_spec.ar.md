...
- `AccountId` - يتم إنتاج العناوين الأساسية عبر `AccountAddress` (IH58 / `sora…` مضغوط / سداسي عشري) ويقوم Torii بتسوية المدخلات من خلال `AccountAddress::parse_encoded`. IH58 هو تنسيق الحساب المفضل؛ يعد نموذج `sora…` هو ثاني أفضل نموذج لـ Sora-only UX. يتم الاحتفاظ بسلسلة `alias` (rejected legacy form) المألوفة كاسم مستعار للتوجيه فقط. الحساب: `{ id, metadata }`. الكود: `crates/iroha_data_model/src/account.rs`.
