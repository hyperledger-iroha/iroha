...
Este documento especifica un mecanismo determinista, controlado por gobernanza, para introducir
nuevas capacidades de IVM/host (p. ej., nuevos syscalls y tipos pointer-ABI) sin detener la red ni
...
- Coexistencia de multiples versiones ABI; nunca romper binarios existentes.
...
Definiciones
- Version ABI: entero pequeno declarado en `ProgramMetadata.abi_version` que selecciona un `SyscallPolicy` y una allowlist de tipos de puntero.
...
- Campos de `RuntimeUpgradeManifest`:
  ...
  - `abi_version: u16` - version ABI objetivo a activar.
...
<!-- END RUNTIME UPGRADE TYPES -->
  - Invariantes: `end_height > start_height`; `abi_version` es estrictamente mayor que cualquier version activa; `abi_hash` debe ser igual a `ivm::syscalls::compute_abi_hash(policy_for(abi_version))`; `added_*` debe listar exactamente el delta aditivo entre la nueva politica ABI y la previamente activa; los numeros/IDs existentes NO deben eliminarse ni renumerarse.
...
- ActivateRuntimeUpgrade { id: RuntimeUpgradeId }
  ...
  - Efectos: cambia el registro a `ActivatedAt(current_height)`; agrega `abi_version` al conjunto ABI activo.
...
- Cambio de host: cada bloque recomputa el conjunto ABI activo; una vez que una transaccion de activacion se confirma, las transacciones posteriores en el mismo bloque observan la nueva politica (validado por `runtime_upgrade_admission::activation_allows_new_abi_in_same_block`).
...
- Las versiones ABI existentes permanecen activas indefinidamente; las nuevas versiones solo extienden el conjunto activo.
...
1) Desplegar un binario de nodo que soporte la nueva version ABI (`v+1`) pero que no la active.
2) Observar la capacidad de la flota via telemetria (porcentaje de nodos anunciando soporte para `v+1`).
3) Enviar `ProposeRuntimeUpgrade` con una ventana suficientemente adelantada (p. ej., `H+N`).
4) En `start_height`, `ActivateRuntimeUpgrade` se ejecuta automaticamente como parte del bloque incluido y cambia el conjunto activo del host; los nodos que no se actualizaron seguiran funcionando para contratos antiguos pero rechazaran admision/ejecucion de programas `v+1`.
5) Despues de la activacion, recompilar/desplegar contratos que apunten a `v+1`.
...
Cambios de codigo requeridos (por crate)
- iroha_data_model
...
- ivm
  - Definir `ABI_V2` (ejemplo) con politica: extender `abi_syscall_list()`; mapping `is_syscall_allowed(policy, number)`; extension de politica de tipos de puntero.
  - Recomputar y fijar pruebas golden: `abi_syscall_list_golden.rs`, `abi_hash_versions.rs`, `pointer_type_ids_golden.rs`.
...
- Kotodama compiler
  - Permitir targeting `abi_version = v+1`; incrustar `abi_hash` correcto para la version seleccionada en manifests `.to`.
...
- Antes de la activacion, los nodos rechazan deterministicamente codigo con `abi_version = v+1`.
- Despues de la activacion en `start_height`, los nodos aceptan y ejecutan `v+1`; los programas antiguos siguen funcionando sin cambios.
