...
- ビルダー: `NewDomain` と `with_logo`、`with_metadata`、その後 `Registrable::build(authority)` は `owned_by` を設定します。
...
- `AssetDefinition { id, name, description?, alias?, spec: NumericSpec, mintable: Mintable, logo: Option<SorafsUri>, metadata, owned_by: AccountId, total_quantity: Numeric }`。
  ...
  - エイリアスのリース状態は、永続化されたエイリアス バインディング レコードに正式に保存されます。インライン `alias` フィールドは、コア/Torii API を通じて定義が読み取られるときに導出されます。
  - Torii 資産定義応答には `alias_binding { alias, status, lease_expiry_ms, grace_until_ms, bound_at_ms }` が含まれる場合があります。`status` は、`permanent`、`leased_active`、`leased_grace`、または `expired_pending_cleanup` のいずれかです。
...
これらのタイプは既存の Ed25519/BLS/ML-DSA プリミティブと並んで配置され、ワー​​クスペースが `--features sm` で構築されるとパブリック スキーマの一部になります。
...
- `EventBox`: パイプライン、パイプライン バッチ、データ、時間、実行トリガー、およびトリガー完了イベントの合計タイプ。 `EventFilterBox` は、サブスクリプションとトリガー フィルターのそれをミラーリングします。
...
- `InstructionBox`: タイプ ID + エンコードされたバイトを介して実装された clone/eq/ord を含む所有の `Box<dyn Instruction>` ラッパー。
- 組み込み命令ファミリーは次のように編成されています。
  - `mint_burn`、`transfer`、`register`、および `transparent` ヘルパーのバンドル。
  - メタ フローの列挙型を入力します: `InstructionType`、`SetKeyValueBox` のようなボックス化された合計 (domain/account/asset_def/nft/trigger)。
- エラー: `isi::error` に基づく豊富なエラー モデル (評価タイプのエラー、エラーの検索、ミント可能性、数学、無効なパラメーター、繰り返し、不変式)。
- 命令レジストリ: `instruction_registry!{ ... }` マクロは、型名をキーとしたランタイム デコード レジストリを構築します。動的な (逆) シリアル化を実現するために、`InstructionBox` クローンおよび Norito serde によって使用されます。 `set_instruction_registry(...)` 経由でレジストリが明示的に設定されていない場合は、バイナリの堅牢性を保つために、すべてのコア ISI を含む組み込みのデフォルト レジストリが最初の使用時に遅延インストールされます。
...
- IVM: Kotodama は IVM バイトコード (`.to`) にコンパイルされ、アプリケーション ロジックの推奨パスです。 `CustomInstruction` は、Kotodama でまだ表現できないエグゼキューター レベルの拡張にのみ使用してください。決定性とピア全体で同一の実行バイナリを確保します。
...
- 有効化: `features = ["transparent_api"]` を `Cargo.toml` の依存関係に追加します。 JSON プロジェクションを必要とするワークスペース クレート (`iroha_torii` など) はフラグを自動的に転送しますが、サードパーティのコンシューマは、デプロイメントを制御し、より広範な API サーフェスを受け入れない限り、フラグをオフにしておく必要があります。
...
- 公開資産の検索では、`asset + account + optional scope` で所有残高に対処します。 raw エンコードされた `AssetId` リテラルは内部表現であり、Torii/CLI セレクター サーフェスの一部ではありません。
...
- `SignedTransaction`、`SignedBlock`、および `SignedQuery` は、正規の Norito でエンコードされた構造体です。それぞれが `iroha_version::Version` を実装し、`EncodeVersioned` 経由でエンコードされるときにペイロードの先頭に現在の ABI バージョン (現在は `1`) を付加します。
