...
- `AssetId`: 正規エンコードされたリテラル `<base58-asset-id>#<katakana-i105-account-id>` (従来のテキスト形式は最初のリリースではサポートされていません)。
...
- ミント/バーン/転送のアセット ID は正規の `<base58-asset-id>#<katakana-i105-account-id>` のままです。以下を使用してそれらを構築します。
