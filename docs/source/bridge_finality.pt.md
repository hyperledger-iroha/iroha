...
`BridgeFinalityProof` (Norito ou Norito JSON) contém exatamente:
...
- `finality_artifact` é o `V2FinalityArtifact` exato e imutável persistido pelo
  caminho de aplicação do Sumeragi v2;
- `validator_set_pops` contém um PoP BLS-normal por entrada, na ordem de
...
hash do bloco, CommitQC e, apenas no fim de uma época, o snapshot autenticado da
próxima época. O contexto congela chain id, limites de época, modo, CommitQC
pai, roster ordenado de `ValidatorPower`, `DualQuorum`, compromisso Nexus/AMX,
layout de DA e semente do líder. O sujeito vincula `parent_block_hash`,
`block_hash` e `payload_hash`. A prova não aceita cópias duplicadas de altura,
...
associação, obtém os PoPs do estado confirmado e executa o verificador
canônico. Ele nunca reconstrói evidência a partir de estado mutável nem usa uma
...
5. um PoP BLS-normal válido para cada membro do roster;
...
transição v2. Fora do limite de época, época, roster, quorum e semente permanecem
congelados; no limite devem corresponder ao `next_epoch_snapshot` autenticado.
...
`BridgeFinalityBundle` contém a prova exata, um compromisso
`{ chain_id, height_context_id, block_height, block_hash, mmr_root?,
mmr_leaf_index?, mmr_peaks? }` e uma lista separada de assinaturas históricas,
atualmente vazia. O MMR opcional auxilia um checkpoint de raiz; não substitui
finalidade nem fornece caminho de inclusão. SCCP usa seu ramo Merkle tipado e
sua âncora governada.
