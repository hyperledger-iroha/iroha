...
Fractional transfers, bounded two-input joins, and independently redeemable
sender change use the canonical first-release
[recursive spend V2 contract](offline_kagemusha_v2_contract.md). The proof
...
> a host-computed proof hash or a private certificate to that opening still
> proves only the opening. `KAGEMUSHA_RECURSIVE_SPEND_V2_PROOF_BACKEND_AVAILABLE`
> must remain `false` until the outer circuit verifies the real inner Halo2
> transcript, fixed verifying key, public instances, and transition statement
> (or verifies a formally sound reduction from them). No artifacts generated by
> the prototype are production lineage artifacts.

The [recursive-verifier adapter audit](offline_kagemusha_recursion_adapter.md)
records the rejected KZG measurements, the exact Axiom 0.5 IPA wire mismatch,
...
`KAGEMUSHA_RECURSIVE_SPEND_V2_PROOF_BACKEND_AVAILABLE` is false, direct
transaction submission rejects before replay markers, escrow reservation,
balance transfer, confidential-tree updates, or anchor persistence, so callers
cannot strand funds by bypassing Torii readiness admission.

## Historical implementation notes (non-normative)

The remainder of this page records pre-release V1, ABI-6, and ABI-7 experiments
and their validation history. It is not a compatibility contract and must not
be used to construct first-release payloads, select production artifacts, or
infer backend availability. The V2 contract linked above is the sole normative
release design; conflicting statements below are retained only as engineering
history and belong in `status.md` as this page is shortened.
...
verifier-slice profiles, but witnessless chain admission is disabled until the
transition verifier is circuit-authenticated. The generic
...
Historical V1 Reserved-lineage verifier-slice circuits also have a semantic public-input
...
zero, while the intended Reserved-lineage design would constrain the field to
the embedded IPA verifier's field-friendly scalar projection. The first offline
hop uses the one-hop verifier-slice profile. Later offline hops use the append
profile. The existing fixed-coefficient opening prototype does **not** prove
the previous recursive proof or the next confidential-transfer hop; it remains
disabled while a circuit-authenticated verifier replaces it.
...
bridge produces a Reserved-lineage bundle for receiver-side verification, but
the current release does not admit it witnesslessly on chain. When both
...
bundles remain bounded by
`KAGEMUSHA_RECURSIVE_SPEND_LINEAGE_WITNESSLESS_MAX_HOPS_V1 = 64`, but that
constant is only the protocol bound. Because
`KAGEMUSHA_RECURSIVE_SPEND_LINEAGE_TRANSITION_CIRCUIT_WIRED_V1 = false`, every
online redemption carries a record-backed lineage witness; an active final
verifier record alone does not enable witnessless redemption.
...
Torii redemption uses `POST /v1/offline/redeem`. The request body is the direct
typed `OfflineRedeemRequest`: its structured representation for
`application/json`, or the canonical typed value with schema name
...
An admitted redemption returns `202 Accepted` and the same operation-resource
contract as top-up. Clients follow `Location` to
`GET /v1/offline/operations/{operation_id}` and handle the tagged
...
every in-cap profile while the transition verifier is not wired. The generic
...
envelope verifier-key hash must match the verifier-record commitment.
Receiver-side Reserved-lineage preverification recognizes two strict
public-instance layouts: one-hop init
...
transition profile. Chain admission nevertheless rejects both layouts while
`KAGEMUSHA_RECURSIVE_SPEND_LINEAGE_TRANSITION_CIRCUIT_WIRED_V1` is false; the
receiver-side layouts do not authorize witnessless append or redemption. Any
lineage bundle outside
...
accumulator binding. Witnessless semantic and Reserved-lineage redeem requests
return no instruction bytes. The authoritative readiness gate is
`KAGEMUSHA_RECURSIVE_SPEND_LINEAGE_TRANSITION_CIRCUIT_WIRED_V1 = false`, so
every circuit id and hop count requires a record-backed lineage witness in
addition to the verifier-record, proof-envelope, chain/asset, final commitment,
...
Ordinary fold and append construction rejects confidential-unshield-v3
envelopes even when their root, nullifier, and change commitment are otherwise
well formed. Unshield-v3 remains exclusive to final redemption, where Core
also binds the exact public amount and optional change output before mutation.
...
the output verifier key from the requested output circuit id. New wallets must
use the preferred append selector, which selects semantic recursive aggregation
for every input while transition verification is unavailable. Callers that use semantic output must
...
Reserved-lineage proofs use the recursive verifier-slice profile for
receiver-side verification, but not witnessless chain admission.
Verify
...
strict verifier-slice columns remain profile-checked, but the inactive
transition circuit gate prevents witnessless redemption. Record-backed lineage validation also keeps the final
...
`KAGEMUSHA_RECURSIVE_SPEND_LINEAGE_WITNESSLESS_MAX_HOPS_V1 = 64` as a protocol
bound and the transition readiness flag as `false`; wallets use the helper
predicates, which require a record-backed witness for every circuit and hop.
The SDK helpers `canRedeem...Witnessless` and
`requires...LineageWitnessForRedeem` expose that fail-closed decision so app
code does not duplicate it: the former is always false and the latter is always
...
the explicit `previous_recursive_proof_open_envelopes_archive` slot reserved
for the future circuit-authenticated Reserved-lineage append path. Explicit semantic
...
`canAppend...WitnesslessLineage(previousHopCount)` returns false for every
input while the transition verifier is unavailable, and
...
recommended selector for new append requests; in this release it always returns
the semantic recursive aggregation circuit.
`canProve...AppendOutput...CircuitId(outputCircuitId, previousHopCount)`;
it returns true for semantic recursive append outputs when the previous hop is
`1..63` and false for every Reserved-lineage append output while the transition
...
proofs may structurally target semantic or Reserved-lineage output, but the
current proving-capability check rejects Reserved-lineage output for every hop.
Request
...
first-hop init and one-hop verifier records. Product append output remains on
the semantic circuit until circuit-authenticated recursive verification is wired.
...
chain-admission checks. Those checks reject witnessless Reserved-lineage
bundles before backend verification because the transition circuit readiness
flag is false. Record-backed redemption then verifies the active lineage
verifier records, recursive proof envelopes, chain/asset binding, final
commitment binding, root freshness, and nullifier set. A tampered final spendable-note binding
