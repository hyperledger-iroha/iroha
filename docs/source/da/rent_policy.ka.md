...
გაგზავნილია `/v2/da/ingest`-ზე, პლუს ბონუსები, რომლებიც აჯილდოვებენ PDP/PoTR შესრულებას და
...
1. **ჩაღება:** `/v2/da/ingest` ატვირთავს აქტიურ `DaRentPolicyV1`-ს, ქირავდება ციტირებით
...
- `torii_da_rent_gib_months_total` — გიბ-თვეები ციტირებული `/v2/da/ingest`-ის მიერ.
...
- ✅ `/v2/da/ingest` ქვითრები ახლა ჩაშენებულია `rent_quote` და CLI/SDK ზედაპირებზე ნაჩვენებია ციტირებული
