...
ወረደ። የTorii's `/v2/da/commitments/verify` የመጨረሻ ነጥብ አንድ አይነት ጠባቂ ያንጸባርቃል፣
...
| `/v2/da/commitments/prove` | `POST` | `DaCommitmentProofRequest` (ሌይን + ግልጽ ሃሽ ወይም `(epoch, sequence)` tuple)። | በ`DaCommitmentProof` (መዝገብ + Merkle መንገድ + እገዳ ሃሽ) ምላሽ ይሰጣል። |
