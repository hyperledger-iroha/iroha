...
| `/v1/da/commitments/prove` | `POST` | `DaCommitmentProofRequest` (zolaq + manifest hash və ya `(epoch, sequence)` dəsti). | `DaCommitmentProof` (rekord + Merkle yolu + blok hash) ilə cavab verir. |
