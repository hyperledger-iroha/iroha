...
`/v2/da/ingest` тапшырылған, плюс бонустар, улар бүләк PDP/PoTR башҡарыу һәм
...
1. **Ингест:** `/v2/da/ingest` әүҙем `DaRentPolicyV1` әүҙем йөкләй, цитаталар аренда
...
- ✅ `/v2/da/ingest` квитанциялары хәҙер `rent_quote` һәм CLI/SDK өҫтө цитаталар күрһәтелә.
