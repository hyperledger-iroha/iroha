...
- `iroha app da get` 为已经支持的多源协调器添加了一个以 DA 为中心的别名
  ...
  使用票证路径 CLI 从 `/v2/da/manifests/<ticket>` 中提取清单，保留捆绑包
...
- `/v2/da/manifests` 响应现在显示 `manifest_hash` 以及 CLI + SDK 帮助程序（`iroha app da get`、
...
- Pin 注册表奇偶校验现在扩展到 SDK：`ToriiClient.registerSorafsPinManifest(...)`
  ...
  `/v2/sorafs/pin/register`。这可以防止 CI 机器人和自动化在以下情况下向 CLI 发起攻击：
...
- **PDP/PoTR 挂钩** — PDP 承诺直接源自块存储并被持久化
  ...
  `Sora-PDP-Commitment` 接头现在随 `/v2/da/ingest` 和 `/v2/da/manifests/{ticket}` 一起提供
...
## 实施注意事项- Torii 的 `/v2/da/ingest` 端点现在标准化有效负载压缩，强制重播缓存，
