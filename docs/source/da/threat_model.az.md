...
# Sora Nexus Məlumat Əlçatımlılığı Təhdid Modeli
...
1. **Təqdimat:** Müştərilər blobları Torii DA qəbul API vasitəsilə təqdim edir. Düyün
...
6. **İdarəetmə:** Parlament və DA nəzarət komitəsi operatorları təsdiq edir,
   ...
   prosesin şəffaflığını təmin etmək üçün eyni DA yolu ilə. Kirayə parametrləri
   DA-7 altında izlənənlər `docs/source/da/rent_policy.md`-də qeydə alınır, beləliklə, auditlər aparılır
   və icra baxışları hər blob üçün tətbiq edilən dəqiq XOR məbləğlərinə istinad edə bilər.
...
**Aşağı** məhdud təsir.| Aktiv | Təsvir | Dürüstlük | Mövcudluq | Məxfilik | Sahibi |
| --- | --- | --- | --- | --- | --- |
...
### Yol Hücumlarını qəbul edin**Ssenari:** Zərərli müştəri səhv formalaşdırılmış Norito faydalı yükləri və ya böyük ölçülü yüklər təqdim edir
...
- Norito DA sxemlərində indi invariantların kodlaşdırılması/şifrəsinin açılması üçün xüsusi qeyri-səlis qoşqu (`fuzz/da_ingest_schema.rs`) var; əhatə dairəsi panelləri hədəfin geriləməsi barədə xəbərdarlıq etməlidir.
...
- Rüblük giriş baxışının avtomatlaşdırılması (`cargo xtask da-privilege-audit`) gəzintiləri
...
## Tələb olunan təqiblər1. DA qəbulu Norito sxemlərini və nümunə vektorlarını dərc edin (DA-2-də aparılır).
...
3. **Tamamlandı (2026-02-05):** PDP/PoTR simulyasiya qoşqu indi QoS geriləmə modelləşdirməsi ilə sövdələşmə + bölmə ssenarilərini həyata keçirir; Aşağıda çəkilmiş icra və deterministik xülasələr üçün [`integration_tests/src/da/pdp_potr.rs`](/integration_tests/src/da/pdp_potr.rs) (`integration_tests/tests/da/pdp_potr_simulation.rs` altında testlərlə) baxın.
4. **Tamamlandı (29-05-2026):** `cargo xtask da-commitment-reconcile` daxilolma qəbzlərini DA öhdəlikləri (SignedBlockWire/`.norito`/JSON) ilə müqayisə edir, `artifacts/da/commitment_reconciliation.json` yayır və ATV-yə bağlanır. buraxılma/taxtalanma xəbərdarlığı (`xtask/src/da.rs`).
5. **Tamamlandı (29-05-2026):** `cargo xtask da-privilege-audit` manifest/replay spool (üstəlik operator tərəfindən təmin edilən yollar) üzərində gəzir, çatışmayan/kataloq olmayan/dünyada yazıla bilən girişləri qeyd edir və idarə panelləri üçün imzalanmış JSON paketi yaradır. (`artifacts/da/privilege_audit.json`), giriş-nəzərdən avtomatlaşdırma boşluğunu bağlayır.
...
- DA replay keşi və kursorun davamlılığı DA-2-yə düşdü. baxın
  ...
  `/v2/da/ingest` vasitəsilə barmaq izi yoxlanılır.
...
aşağıdakı göstəricilər:<!-- BEGIN_DA_SIM_TABLE -->
...
Defolt olaraq Norito JSON-u `artifacts/da/threat_model_report.json`-a yayır. Gecə
