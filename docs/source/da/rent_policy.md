...
|-------|-------------|---------|
| `base_rate_per_gib_month` | XOR charged per GiB per month of retention. | `"0.25"` XOR |
...
| `egress_credit_per_gib` | Credit paid when a provider serves 1 GiB of DA data. | `"0.0015"` XOR |
...
Every amount is a canonical decimal string. This preserves sub-micro precision
and values wider than fixed-width JSON integers while rejecting JSON numbers,
negative values, and noncanonical spellings. The final
`egress_credit_per_gib` field lets dashboards and payout schedulers align
egress reimbursements with the rent policy that produced the quote without
recomputing the policy math in scripting glue.
...
attach the Norito/JSON encoded `DaRentPolicyV1` to governance proposals or rent
