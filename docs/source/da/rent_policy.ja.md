...
`/v2/da/ingest` に送信され、PDP/PoTR の実行に報酬を与えるボーナスと、
...
1. **取り込み:** `/v2/da/ingest` はアクティブな `DaRentPolicyV1` をロードし、家賃を見積もります
...
- `torii_da_rent_gib_months_total` — `/v2/da/ingest` によって引用される GiB 月。
...
- ✅ `/v2/da/ingest` レシートには `rent_quote` が埋め込まれ、CLI/SDK サーフェスには引用符が表示されます。
