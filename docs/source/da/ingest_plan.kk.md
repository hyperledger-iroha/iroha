...
- `iroha app da get` әлдеқашан қуат беретін көп көзді оркестр үшін DA-ға бағытталған бүркеншік атын қосады
  ...
  билет жолы пайдаланылады CLI манифестті `/v2/da/manifests/<ticket>` ішінен шығарады, топтаманы сақтайды
...
- `iroha app da get-blob` канондық манифесттерді тікелей Torii-тен `GET /v2/da/manifests/{storage_ticket}` арқылы шығарады.
...
- `/v2/da/manifests` жауаптары енді `manifest_hash` және CLI + SDK көмекшілері (`iroha app da get`,
...
- PIN тізілімінің паритеті енді SDK-ға таралады: `ToriiClient.registerSorafsPinManifest(...)`
  ...
  `/v2/sorafs/pin/register`. Бұл CI боттары мен автоматтандыруды CLI-ге жіберуден сақтайды
...
- **PDP/PoTR ілмектері** — PDP міндеттемелері тікелей бөлшектер қоймасынан алынады және сақталады
  ...
  `Sora-PDP-Commitment` тақырыбы енді `/v2/da/ingest` және `/v2/da/manifests/{ticket}` екеуімен бірге жеткізіледі
...
## Іске асыру туралы ескертпелер- Torii `/v2/da/ingest` соңғы нүктесі енді пайдалы жүктің қысылуын қалыпқа келтіреді, қайта ойнату кэшін күшейтеді,
...
- Torii сонымен қатар `GET /v2/da/manifests/{storage_ticket}` ашады, осылайша SDK және операторлар манифесттерді ала алады
...
`POST /v2/da/ingest/chunk/start` қоңырау шалу арқылы ағындық сеанс. Torii a деп жауап береді
...
Кезеңдік артефактты тұтыну арқылы `POST /v2/da/ingest`. Сәтсіз сеанстар анық түрде тоқтатылуы мүмкін немесе
