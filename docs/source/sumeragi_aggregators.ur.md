...
- Permissioned mode میں collectors کا انتخاب PRF کے ذریعے کیا جاتا ہے جس کی seed epoch کی PRF/VRF state سے لی جاتی ہے۔ helper canonical topology سے `(height, view)` کیلئے deterministic ordering نکالتا ہے اور leader کو exclude کرتا ہے۔ جب PRF seed دستیاب نہ ہو تو determinism برقرار رکھنے کیلئے contiguous tail segment پر fallback ہوتا ہے۔
...
1. **Reproducibility:** ایک ہی validator topology، consensus mode، PRF seed اور `(height, view)` tuple کو ہر peer پر ایک جیسے primary/secondary collectors پیدا کرنے چاہئیں۔ helper topology کی پیچیدگیاں (proxy tail، Set B validators) چھپاتا ہے تاکہ ordering مختلف components اور tests میں portable رہے۔
2. **Rotation:** PRF-based selection دونوں modes میں heights اور views کے ساتھ primary collector rotate کرتا ہے، جس سے کوئی ایک Set B validator مستقل aggregation ذمہ داری نہیں رکھتا۔ contiguous tail fallback صرف تب استعمال ہوتا ہے جب PRF seed دستیاب نہ ہو۔
...
- Unit اور integration tests PRF determinism، fallback selection اور backoff state transitions کو validate کرتے ہیں۔
