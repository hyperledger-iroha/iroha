...
- Supported backends currently include `halo2/ipa` and other `halo2/…` variants built into the node. `stark/fri/*` is supported when built with feature `zk-stark` and enabled via config (`zk.stark.enabled=true`). `groth16/…` remains unsupported.
...
- POST `/v1/gov/ballots/zk-v1/ballot-proof` — accepts `BallotProof` JSON directly.
