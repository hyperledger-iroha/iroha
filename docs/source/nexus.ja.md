...
- `GET /v1/space-directory/uaids/{uaid}` — UAID の現在の dataspace bindings を解決（正規化アドレス、dataspace ids、program bindings）。Sora Name Service 形式には `address_format=compressed` を付与。
