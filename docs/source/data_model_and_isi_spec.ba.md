...
- `AccountId` — канон адрестары `AccountAddress` (IH58 / `sora…` ҡыҫылған / алты) һәм `AccountAddress::parse_encoded` аша Torii нормализацияһы аша етештерелә. IH58 - өҫтөнлөклө иҫәп форматында; `sora…` формаһы Сора-тик UX өсөн икенсе-иң яҡшы. Таныш `alias` (rejected legacy form) ептәре тик маршрутлаштырыу псевдонимы булараҡ ғына һаҡлана. Иҫәп: `{ id, metadata }`. Код: `crates/iroha_data_model/src/account.rs`.
