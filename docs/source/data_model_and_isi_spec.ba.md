...
- `AccountId` — канон адрестары `AccountAddress` (I105 / `i105` ҡыҫылған / алты) һәм `AccountAddress::parse_encoded` аша Torii нормализацияһы аша етештерелә. I105 - өҫтөнлөклө иҫәп форматында; `i105` формаһы Сора-тик UX өсөн икенсе-иң яҡшы. Таныш `alias` (rejected legacy form) ептәре тик маршрутлаштырыу псевдонимы булараҡ ғына һаҡлана. Иҫәп: `{ id, metadata }`. Код: `crates/iroha_data_model/src/account.rs`.
