...
- `AccountId` — kanonik ünvanlar `AccountAddress` (IH58 / `sora…` sıxılmış / hex) vasitəsilə istehsal olunur və Torii `AccountAddress::parse_any` vasitəsilə girişləri normallaşdırır. IH58 üstünlük verilən hesab formatıdır; `sora…` forması yalnız Sora UX üçün ikinci ən yaxşısıdır. Tanış `alias@domain` sətri yalnız marşrut ləqəbi kimi saxlanılır. Hesab: `{ id, metadata }`. Kod: `crates/iroha_data_model/src/account.rs`.
...
- `AssetId` — `asset#domain#account` və ya domenlər uyğun gəlirsə `asset##account`, burada `account` kanonik `AccountId` sətiridir (IH58 üstünlük verilir). Aktiv: `{ id, value: Numeric }`. Kod: `crates/iroha_data_model/src/asset/{id.rs,value.rs}`.
