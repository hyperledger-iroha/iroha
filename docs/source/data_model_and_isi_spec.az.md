...
- `AccountId` — kanonik ünvanlar `AccountAddress` (I105 / `i105` sıxılmış / hex) vasitəsilə istehsal olunur və Torii `AccountAddress::parse_encoded` vasitəsilə girişləri normallaşdırır. I105 üstünlük verilən hesab formatıdır; I105 forması yalnız Sora UX üçün ikinci ən yaxşısıdır. Tanış `alias` (rejected legacy form) sətri yalnız marşrut ləqəbi kimi saxlanılır. Hesab: `{ id, metadata }`. Kod: `crates/iroha_data_model/src/account.rs`.
