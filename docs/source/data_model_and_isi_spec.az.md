...
- `AssetId`: kanonik kodlaşdırılmış literal `<asset-definition-id>#<account-id>` (ilk buraxılışda köhnə mətn formaları dəstəklənmir).- `NftId` — `nft$domain`. NFT: `{ id, content: Metadata, owned_by }`. Kod: `crates/iroha_data_model/src/nft.rs`.
...
- `Metadata` — `BTreeMap<Name, Json>` yoxlanılmış daxiletmə/çıxarma ilə. Kodu: `crates/iroha_data_model/src/metadata.rs`.
- Abunə nümunəsi (tətbiq səviyyəsi): planlar `subscription_plan` metadatası olan `AssetDefinition` girişləridir; abunələr `subscription` metadatası olan `Nft` qeydləridir; faturalandırma, abunə NFT-lərinə istinad edən zaman tetikleyicileri tərəfindən həyata keçirilir. Bax `docs/source/subscriptions_api.md` və `crates/iroha_data_model/src/subscription.rs`.
...
- Dataspace-dən əldə edilən əlaqə anbarları (`space_directory_manifests`, `uaid_dataspaces`, `axt_policies`, `axt_replay_ledger`, zolaqlı relay fövqəladə halların ləğvi reyestrində) və verilənlər məkanı-hədəf icazələri (permissions permissions of dataspace in 81NI1ro1000/10I000 mağazalar) məlumat məkanları aktiv `dataspace_catalog`-dən itdikdə `State::set_nexus(...)`-də kəsilir, iş vaxtı kataloqu yeniləmələrindən sonra köhnə məlumat məkanı istinadlarının qarşısını alır. Zolaqlı DA/rele keşləri (`lane_relays`, `da_commitments`, `da_confidential_compute`, `da_pin_intents`) həm də zolaqdan çıxarıldıqda və ya data-space-a dəyişdirilə bilmədiyi üçün başqa bir məlumat məkanına təyin olunduqda budanır. Kosmik Kataloq ISI-ləri (`PublishSpaceDirectoryManifest`, `RevokeSpaceDirectoryManifest`, `ExpireSpaceDirectoryManifest`) həmçinin `dataspace`-i aktiv kataloqa qarşı doğrulayır və `InvalidParameter` ilə naməlum identifikatorları rədd edir.
...
Növlər: `Register<T: Registered>` və `Unregister<T: Identifiable>`, konkret hədəfləri əhatə edən `RegisterBox`/`UnregisterBox` cəmi növləri ilə.- Register Peer: dünya həmyaşıdları dəstinə əlavələr.
...
- Domain Qeydiyyatı: `owned_by = authority` ilə `NewDomain`-dən qurulur. İcazə verilməyib: `genesis` domeni.
  ...
  - Səhvlər: `Repetition(Register, DomainId)`, `InvariantViolation("Not allowed to register genesis domain")`. Kodu: `core/.../isi/world.rs`.

- Qeydiyyat Hesabı: `genesis` domenində icazə verilməyən `NewAccount`-dən qurulur; `genesis` hesabı qeydə alına bilməz.
...
- Register Trigger: triggeri süzgəc növü ilə müəyyən edilmiş müvafiq tətikdə saxlayır.
  ...
  - Səhvlər: `Repetition(Register, TriggerId)`, `InvalidParameterError::SmartContract(..)` konversiya/təsdiqləmə uğursuzluqları. Kod: `core/.../isi/triggers/mod.rs`.- Peer/Domain/Account/AssetDefinition/NFT/Role/Triggerin qeydiyyatdan çıxarılması: hədəfi silir; silinmə hadisələrini yayır. Əlavə kaskad çıxarılması:- Domeni qeydiyyatdan çıxarın: domen obyektini və onun seçici/təsdiqləmə siyasəti vəziyyətini silir; domendəki aktiv təriflərini (və həmin təriflərlə əsaslanan məxfi `zk_assets` yan vəziyyəti), həmin təriflərin aktivlərini (və hər bir aktiv metadatasını), domendəki NFT-ləri və domen əhatəli hesab etiketi/ləqəb proqnozlarını silir. O, həmçinin silinmiş domendən sağ qalan hesabları ayırır və silinmiş domenə və ya onunla birlikdə silinmiş resurslara istinad edən hesab/rol miqyaslı icazə daxiletmələrini kəsir (domen icazələri, silinmiş təriflər üçün aktiv tərifi/aktiv icazələri və silinmiş NFT ID-ləri üçün NFT icazələri). Domenin silinməsi qlobal `AccountId`, onun tx ardıcıllığı/UAID vəziyyətini, xarici aktiv və ya NFT sahibliyini, tetikleyici səlahiyyəti və ya sağ qalan hesaba işarə edən digər xarici audit/konfiqurasiya istinadlarını silmir. Qoruyucu relslər: domendəki hər hansı aktiv tərifinə hələ də repo-müqavilə, hesablaşma kitabçası, ictimai zolaqlı mükafat/iddia, oflayn müavinət/köçürmə, hesablaşma repo defoltları (`settlement.repo.eligible_collateral`, `settlement.repo.collateral_substitution_matrix`), idarəetmə-konfiqurasiya edilmiş səsvermə/viricilik-repo-müqavilə ilə istinad edildikdə rədd edir. aktiv tərifi arayışları, oracle-economics konfiqurasiya edilmiş mükafat/slash/dispute-bond aktiv tərifi arayışları və ya Nexus haqqı/staking aktivi tərifi arayışları (`nexus.fees.fee_asset_id`, `nexus.staking.stake_asset_id`). Hadisələr: `DomainEvent::Deleted`, üstəgəl hər bir maddənin silinməsisilinən domen əhatəli resurslar üçün hadisələr haqqında. Səhvlər: itkin olduqda `FindError::Domain`; `InvariantViolation` saxlanılan aktiv-tərif arayış ziddiyyətləri. Kodu: `core/.../isi/world.rs`.- Hesabı Qeydiyyatdan Çıxar: hesabın icazələrini, rollarını, tx ardıcıllığı sayğacını, hesab etiketinin xəritələşdirilməsini və UAID bağlamalarını silir; hesaba məxsus aktivləri (və hər bir aktiv metadatasını) silir; hesaba məxsus NFT-ləri silir; səlahiyyəti həmin hesab olan tetikleyicileri aradan qaldırır; silinmiş hesaba istinad edən hesab/rol miqyaslı icazə daxiletmələrini, silinmiş məxsus NFT ID-ləri üçün hesab/rol miqyaslı NFT hədəf icazələrini və silinmiş triggerlər üçün hesab/rol miqyaslı trigger-hədəf icazələrini kəsir. Mühafizə relsləri: hesabın hələ də domeni, aktiv tərifi, SoraFS provayder bağlaması, aktiv vətəndaşlıq qeydi, ictimai zolağın staking/mükafat vəziyyəti (hesabın iddiaçı və ya mükafat-aktiv sahibi kimi göründüyü mükafat-iddia açarları daxil olmaqla), aktiv oracle vəziyyətini (o cümlədən, twitter-buraxılış təminatçıları, oracle-provayderlər-provayderləri) rədd edir. və ya oracle-economics konfiqurasiya edilmiş mükafat/slash hesab istinadları), aktiv Nexus haqqı/staking hesabı arayışları (`nexus.fees.fee_sink_account_id`, `nexus.staking.stake_escrow_account_id`, `nexus.staking.slash_sink_account_id`; kanonik kimi təhlil edilir), aktivləşdirici domensiz hesaba yenidən baxıldı və uğursuzluqla silindi repo-müqavilə vəziyyəti, aktiv hesablaşma-mühasibat kitabı vəziyyəti, aktiv oflayn müavinət/köçürmə və ya oflayn hökm-ləğv vəziyyəti, aktiv aktiv tərifləri üçün aktiv oflayn depozit hesabı konfiqurasiya istinadları (`settlement.offline.escrow_accounts`), aktiv idarəetmə vəziyyəti (təklif/mərhələ təsdiqi)als/locks/slashes/şura/parlament siyahıları, təklif parlamenti snapshotları, icra müddəti təkmilləşdirmə təklifi qeydləri, idarəçilik tərəfindən konfiqurasiya edilmiş escrow/slash-receiver/viral-pool hesab istinadları, idarəetmə SoraFS telemetriya göndərişi vasitəsilə istinadlar I10X02 `gov.sorafs_telemetry.per_provider_submitters`, və ya `gov.sorafs_provider_owners` vasitəsilə idarəçilik tərəfindən konfiqurasiya edilmiş SoraFS provayder-sahibi arayışları, konfiqurasiya edilmiş məzmunun dərc edilməsinə icazə siyahısı hesab arayışları (`content.publish_allow_accounts`), aktiv sosial escrow göndəricisi, aktiv status-paylayıcı statusu, aktiv status-paylayıcı statusu aktiv zolaqlı relay fövqəladə validatoru ləğvetmə vəziyyəti və ya aktiv SoraFS pin reyestrinin emitent/bağlayıcı qeydləri (pin manifestləri, manifest ləqəbləri, replikasiya sifarişləri). Hadisələr: `AccountEvent::Deleted`, üstəgəl silinmiş NFT üçün `NftEvent::Deleted`. Səhvlər: itkin olduqda `FindError::Account`; `InvariantViolation` sahibsiz uşaqlar. Kodu: `core/.../isi/domain.rs`.- AssetDefinition'ı qeydiyyatdan çıxarın: həmin tərifin bütün aktivlərini və onların hər bir aktiv metadatasını silir və bu təriflə əsaslanmış məxfi `zk_assets` yan vəziyyətini silir; həmçinin silinmiş aktiv tərifinə və ya onun aktiv nümunələrinə istinad edən uyğun `settlement.offline.escrow_accounts` girişini və hesab/rol miqyaslı icazə daxiletmələrini kəsir. Mühafizə relsləri: tərifə hələ də repo-müqavilə, hesablaşma kitabçası, ictimai zolaqlı mükafat/iddia, oflayn müavinət/köçürmə vəziyyəti, hesablaşma repo defoltları (`settlement.repo.eligible_collateral`, `settlement.repo.collateral_substitution_matrix`), idarəetmə tərəfindən konfiqurasiya edilmiş səsvermə/vətəndaşlıq hüququ ilə istinad edildikdə rədd edir aktiv tərifi arayışları, oracle-economics konfiqurasiya edilmiş mükafat/slash/dispute-bond aktiv tərifi arayışları və ya Nexus haqqı/staking aktivi tərifi arayışları (`nexus.fees.fee_asset_id`, `nexus.staking.stake_asset_id`). Hadisələr: hər aktiv üçün `AssetDefinitionEvent::Deleted` və `AssetEvent::Deleted`. Səhvlər: istinad ziddiyyətlərində `FindError::AssetDefinition`, `InvariantViolation`. Kod: `core/.../isi/domain.rs`.
  - NFT-ni qeydiyyatdan çıxarın: NFT-ni silir və silinmiş NFT-yə istinad edən hesab/rol miqyaslı icazə daxiletmələrini kəsir. Hadisələr: `NftEvent::Deleted`. Səhvlər: `FindError::Nft`. Kod: `core/.../isi/nft.rs`.
  - Rolu Qeydiyyatdan Çıxar: əvvəlcə bütün hesablardan rolu ləğv edir; sonra rolu silir. Hadisələr: `RoleEvent::Deleted`. Səhvlər: `FindError::Role`. Kodu: `core/.../isi/world.rs`.- Tətiyi Qeydiyyatdan Çıxar: əgər varsa, tətiyi aradan qaldırır və silinmiş tətiyə istinad edən hesab/rol miqyaslı icazə daxiletmələrini kəsir; dublikatın qeydiyyatdan çıxarılması `Repetition(Unregister, TriggerId)` verir. Hadisələr: `TriggerEvent::Deleted`. Kod: `core/.../isi/triggers/mod.rs`.

### Nanə / Yandırmaq
Növlər: `Mint<O, D: Identifiable>` və `Burn<O, D: Identifiable>`, qutuda `MintBox`/`BurnBox`.

- Aktiv (Rəqəm) nanə/yandır: qalıqları və tərifin `total_quantity`-ni tənzimləyir.
  ...
  - Səhvlər: `TypeError::AssetNumericSpec(Mismatch)`, `MathError::Overflow`/`NotEnoughQuantity`. Kodu: `core/.../isi/asset.rs`.- Tətik təkrarları nanə/yandır: dəyişikliklər `action.repeats` bir tətik üçün sayılır.
...
- Aktiv (Rəqəm): mənbədən `AssetId` çıxarın, təyinat `AssetId` əlavə edin (eyni tərif, fərqli hesab). Sıfırlanmış mənbə aktivini silin.
...
- AssetDefinition mülkiyyəti: `AssetDefinition.owned_by` təyinat hesabına dəyişir.
  ...
  - Səhvlər: mənbə NFT-yə sahib deyilsə, `FindError::Account/Nft`, `InvariantViolation`. Kod: `core/.../isi/nft.rs`.
...
Növlər: `Grant<O, D>` və `Revoke<O, D>`, `Permission`/`Role`-dən `Account`-ə və `TriggerEvent::Created(TriggerId)`-ə qədər və Norito.- Hesaba İcazə verin: artıq xas olmadığı halda `Permission` əlavə edir. Hadisələr: `AccountEvent::PermissionAdded`. Səhvlər: dublikat varsa `Repetition(Grant, Permission)`. Kodu: `core/.../isi/account.rs`.
- Hesabdan İcazəni Ləğv et: varsa, silinir. Hadisələr: `AccountEvent::PermissionRemoved`. Səhvlər: `FindError::Permission` yoxdursa. Kod: `core/.../isi/account.rs`.
...
- Hesabdan Rolu Ləğv et: varsa, xəritələşdirməni silir. Hadisələr: `AccountEvent::RoleRevoked`. Səhvlər: `FindError::Role` yoxdursa. Kodu: `core/.../isi/account.rs`.
...
- Roldan İcazəni Ləğv et: bu icazə olmadan rolu yenidən qurur. Hadisələr: `RoleEvent::PermissionRemoved`. Səhvlər: `FindError::Permission` yoxdursa. Kodu: `core/.../isi/world.rs`.### Tətiklər: İcra edin
...
### Xəta Modeli
Ümumi zərf: `InstructionExecutionError` qiymətləndirmə xətaları, sorğu uğursuzluqları, çevrilmələr, tapılmayan obyekt, təkrarlama, zərb alətləri, riyaziyyat, etibarsız parametr və dəyişməz pozuntu variantları ilə. Sadalamalar və köməkçilər `pub mod error` altında `crates/iroha_data_model/src/isi/mod.rs` daxilindədir.
...
- `TransactionBuilder`/`SignedTransaction`: metadata, `chain_id`, `authority`, `creation_time_ms`, isteğe bağlı I00303 və metadata ilə icra edilə biləni qurur, işarələyir və paketləyir. `nonce`. Kod: `crates/iroha_data_model/src/transaction/`.
...
- Yaradılış qorumaları: `genesis` domenini və ya `genesis` domenində hesabları qeydiyyatdan keçirə bilməz; `genesis` hesabı qeydə alına bilməz. Kod/testlər: `core/.../isi/world.rs`, `core/.../smartcontracts/isi/mod.rs`.
...
- Trigger həyat dövrü:
  ...
  - `__enabled` metadata açarını `false`-ə təyin etməklə triggerləri söndürmək olar (aktiv etmək üçün defolt parametrlər yoxdur); `SetKeyValue::trigger` və ya IVM `set_trigger_enabled` sistem zəngi ilə keçid.
...
- Nanə/yandırma/köçürmə sərhədlərində aktiv identifikatorları kanonik olaraq qalır `<asset-definition-id>#<account-id>`; `iroha tools encode asset-id` ilə `--definition <base58-asset-definition-id>` və ya `--alias ...` plus `--account` istifadə edin.
