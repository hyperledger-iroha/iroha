...
| **Контроль доступа и аутентификация** | SDK учитывает политики TLS Torii и подписанные запросы `/v2/pipeline`; Справочник по рабочим процессам оператора. Руководство по поддержке §4–5 для эскалации и отмены шлюзования с помощью подписанных артефактов Norito. | `docs/source/android_support_playbook.md`; `docs/source/sdk/android/telemetry_redaction.md` (переопределить рабочий процесс). | ✅ Реализовано |
