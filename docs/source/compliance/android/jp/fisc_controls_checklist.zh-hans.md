...
| **访问控制和身份验证** | SDK 遵循 Torii TLS 策略和 `/v2/pipeline` 签名请求；操作员工作流程参考支持手册 §4–5，通过签名的 Norito 工件进行升级和覆盖门控。 | `docs/source/android_support_playbook.md`； `docs/source/sdk/android/telemetry_redaction.md`（覆盖工作流程）。 | ✅ 已实施 |
