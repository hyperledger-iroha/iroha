...
| **Contrôle d'accès et authentification** | Le SDK respecte les politiques TLS Torii et les requêtes signées `/v2/pipeline` ; Référence des flux de travail de l'opérateur Prise en charge du Playbook §4–5 pour l'escalade et le remplacement du déclenchement via des artefacts Norito signés. | `docs/source/android_support_playbook.md` ; `docs/source/sdk/android/telemetry_redaction.md` (flux de travail de remplacement). | ✅ Mis en œuvre |
