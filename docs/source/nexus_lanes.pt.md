...
- Snapshots de estado em camadas refletem o mesmo ciclo; cada lane escreve em `cold_store_root/lanes/lane_{id:03}_{slug}` e as aposentadorias migram a arvore para `cold_store_root/retired/lanes/`.
...
- `LaneCatalog`, `LaneMetadata` e `DataSpaceCatalog` vivem em `iroha_data_model::nexus` e fornecem estruturas compativeis com Norito para manifests e SDKs.
