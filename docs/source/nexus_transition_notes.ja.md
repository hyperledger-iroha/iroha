...
- オペレーション手順に `iroha_cli app nexus lane-report --summary --only-missing --fail-on-sealed` を追加し、`/v2/sumeragi/status` の `lane_governance_sealed_total` / `lane_governance_sealed_aliases` と組み合わせてマニフェスト未適用のレーンを早期検知できるようにします。
