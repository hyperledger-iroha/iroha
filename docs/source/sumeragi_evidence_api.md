...
- GET `/v1/sumeragi/evidence`
  ...
  - Query params: `limit` (default 50, max 1000), `offset` (default 0), `kind` (optional; one of `DoublePrevote|DoublePrecommit|InvalidQC|InvalidProposal`).
...
- POST `/v1/sumeragi/evidence`
  ...
  - Validation covers signer/height/view/epoch equality for double-vote payloads, requires non-empty single-signer payloads, and rejects `InvalidProposal` records that fail to advance height/view or whose parent hash disagrees with the embedded QC.
