...
2. `mochi-ui`：基於 `egui`/`eframe` 構建的桌面應用程序，用於呈現用戶界面並通過 `mochi-core` API 委託所有編排。
...
- **塊和事件流**：訂閱 `/v1/blocks/stream` 和 `/v1/events/ws`，為 UI 面板存儲內存滾動緩衝區。
- **狀態資源管理器**：運行 Norito 支持的 `/v1/query` 調用以列出域、帳戶、資產和資產定義以及分頁幫助程序和元數據摘要。
- **交易編輯器**：階段鑄造/轉移指令草稿，將其批處理為簽名交易，預覽 Norito 有效負載，通過 `/v1/pipeline/transactions` 提交，並監控生成的事件；保險庫簽名掛鉤仍然是未來的迭代。
