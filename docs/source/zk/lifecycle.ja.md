...
2. **Torii を介したアドミッション** – オペレーターは署名済みのガバナンス命令を Torii (`/v2/zk/vk/*`) に送信する。受理されたトランザクションは VK レジストリを登録または更新し、オンチェーンに保存されてピア間で複製される。
3. **コントラクト／ランタイムでの利用** – トランザクションやスマートコントラクトは `(backend, name)` で VK を参照するか、インライン VK ペイロードを埋め込む。実行時に証明検証の過程で VK コミットメントが解決される。
4. **証明検証** – クライアントは `/v2/zk/verify` または `/v2/zk/submit-proof` で証明を送信する。検証はトランザクション実行中の `iroha_core::zk` 内で行われ、成功すると Torii (`/v2/zk/proofs*`) からクエリできる `ProofRecord` が生成される。
...
- `POST /v2/zk/vk/register` – 署名済みの `RegisterVerifyingKey` 命令を送信する。
- `POST /v2/zk/vk/update` – より高いバージョンの `UpdateVerifyingKey` を送信する。
- `POST /v2/zk/vk/deprecate` – 既存の VK を廃止状態にマークする。
- `GET  /v2/zk/vk` – `backend`、`status`、`name_contains` などのフィルター付きで VK を一覧取得する。
- `GET  /v2/zk/vk/{backend}/{name}` – 単一の VK レコードを取得する。

### CLI ヘルパー

`iroha_cli app zk` コマンドは、Torii が期待する JSON DTO を送信する薄いラッパーを提供する。
...
- 証明エンベロープは `/v2/zk/verify`（同期）または `/v2/zk/submit-proof`（後で検査する用途）で受け付ける。どちらのエンドポイントも Norito でエンコードしたエンベロープまたは JSON DTO を受理する。
...
`/v2/zk/proofs` と `/v2/zk/proofs/count` は台帳上のレコードを公開する。
...
- `/v2/zk/proof/{backend}/{hash}` も直接参照用に利用できる。

### CLI カバレッジ

CLI には `iroha_cli app zk proofs` 以下に新しいサブコマンドが追加されている。
...
- TTL を過ぎたレポートは各スキャンのたびにガーベジコレクションされる。手動削除は `/v2/zk/prover/reports` からも継続して行える。
