...
3. **Contract/runtime usage** – Generic proof verification references VKs by `(backend, name)` through `vk_ref`. Specialized proof ISIs may accept inline VK payloads only where their instruction explicitly owns that policy. Execution resolves VK commitments during proof verification.
