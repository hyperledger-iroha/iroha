...
`iroha zk` サブコマンドが添付／レポート操作をラップします。

添付ファイル:
- アップロード: `iroha zk attach --file myproof.zk1 --content-type application/x-norito`
- 一覧: `iroha zk list-attachments --limit 10`
- ダウンロード: `iroha zk download-attachment --id <id> --output proof.zk1`
- 削除: `iroha zk delete-attachment --id <id>`

レポート:
- 一覧: `iroha zk list-reports --failed-only`
- 削除: `iroha zk delete-reports --ids <id1> <id2>`

プローバー操作:
- 起動: `iroha zk prover start`（バックグラウンドスレッドを生成）
- 停止: `iroha zk prover stop`
- ステータス: `iroha zk prover status`

検証スタブ:
- `iroha zk verify --json <PATH>` / `--norito <PATH>`
- `iroha zk submit-proof --json <PATH>` / `--norito <PATH>`
...
- `GET  /v1/zk/vk` — 一覧（フィルタ: `backend`, `status=Active|Deprecated`, `name_contains`, `limit`, `offset`, `order`, `ids_only`）

POST DTO 共通フィールド: `authority` (AccountId), `private_key` (ExposedPrivateKey), `backend` (string), `name` (string), `version` (u32), `gas_schedule_id` (string) に加え、必要に応じて `status` (`"Active"`/`"Deprecated"`), `max_proof_bytes` (u32), `metadata_uri_cid`, `vk_bytes_cid`, `activation_height` / `deprecation_height` / `withdraw_height` を指定できます。以下のいずれかを必須で含めます:
...
CLI:
- 登録: `iroha zk vk register --json ./vk_register.json`
- 更新: `iroha zk vk update --json ./vk_update.json`
- 廃止: `iroha zk vk deprecate --json ./vk_deprecate.json`
- 取得: `iroha zk vk get --backend <backend> --name <name>`

注意:
- コミットメントは `backend || bytes` のドメイン分離ハッシュ。送信時に検証。
- 廃止: ステータスを `Deprecated` に変更し、スペース節約のため鍵本体をクリア。バックエンド毎の保持上限は `zk.vk_deprecated_cap_per_backend`（0 は無制限）。超過時は最古を削除。コミットメントとバージョンは残る。
...
イベントは `VerifyingKeyEvent::{Registered{ id, record }, Updated{ id, record }, Deprecated{ id }}`。
