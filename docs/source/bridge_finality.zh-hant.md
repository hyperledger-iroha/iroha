...
無狀態驗證器嚴格核對版本、鏈、高度、header 雜湊、上下文、subject 和 CommitQC，並驗證
製品中的全部 PoP。簽名者索引必須嚴格遞增且在範圍內；CommitQC 必須同時滿足驗證器數量
...
mmr_leaf_index?, mmr_peaks? }`。可選 MMR 欄位僅是承諾，不是最終性或包含證明。
