...
| Kotodama | Իջեցնում է `transfer_batch((from,to,asset,amount), …)` օգնականը դեպի `transfer_v1_batch_begin`, հաջորդական `transfer_asset` զանգեր և `transfer_v1_batch_end`: Յուրաքանչյուր բազմակի արգումենտ պետք է հետևի `(AccountId, AccountId, AssetDefinitionId, quantity)` ձևին; միայնակ փոխանցումները պահպանում են առկա շինարարին. |
