...
# Solicitações de coordenação LLM

## Objetivo

Esses modelos de prompt ajudam os engenheiros a obter rapidamente esclarecimentos de @mtakemiya
quando os itens do roteiro deixam questões em aberto. Copie uma das seções abaixo no
Thread LLM, substitua os espaços reservados entre colchetes e inclua o arquivo relevante ou
referências de linha para que o contexto permaneça ancorado.

## Decisões de arquitetura ou design
...
## Configuração ou orientação do operador
...
## Criptografia ou primitivos de protocolo
...
## Engenharia ou processo de liberação
