...
1. Generate a stream token via the built-in CLI (`iroha app sorafs storage token issue`) and run `iroha app sorafs toolkit pack ./payload.bin --manifest-out manifest.to --json-out manifest_report.json`. The versioned report embeds `chunk_fetch_specs` together with its required whole-payload digest.
2. Obtain the gateway's authenticated 32-byte Ed25519 token-verification key, then run `iroha app sorafs fetch --manifest manifest.to --manifest-report manifest_report.json --manifest-id <manifest_digest_hex> --gateway-provider "name=primary,provider-id=<hex32>,gateway-key=<ed25519-public-key-hex>,base-url=https://gateway.example/,stream-token=<base64>" --max-peers 4 --retry-budget 3`. Use `--plan chunk_fetch_plan.json` only with the separately emitted strict `sorafs.chunk_fetch_plan.v1` object; a bare array or the whole report is not a plan.
