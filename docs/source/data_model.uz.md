...
- `AssetId`: kanonik kodlangan literal `norito:<hex>` (eski matn shakllari birinchi versiyada qo'llab-quvvatlanmaydi).
...
- `AssetDefinitionId { aid_bytes: [u8; 16] }` matnda `aid:<32-hex-no-dash>` sifatida ochilgan.
...
`aid` / taxallusning tezkor ma'lumotnomasi (CLI + Torii):
...
- Yalpiz/yoqish/o'tkazish uchun aktiv identifikatorlari `norito:<hex>` kanonik bo'lib qoladi; ularni yarating:
