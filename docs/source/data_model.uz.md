...
## Asosiy xususiyatlar va yordamchilar- `Identifiable`: ob'ektlar barqaror `Id` va `fn id(&self) -> &Self::Id`. Xarita/to'plam qulayligi uchun `IdEqOrdHash` bilan olinishi kerak.
...
- `Owned<T>`/`Ref<'world, K, V>`: Keraksiz nusxalarning oldini olish uchun omborlarda va so'rov filtrlarida ishlatiladigan engil o'ramlar.
...
- `IdBox`: Har qanday qoʻllab-quvvatlanadigan identifikator (`DomainId`, `AccountId`, `AssetDefinitionId`, `AssetId`, `AssetId`, Kotodama, Kotodama, Norito, `DomainId`) uchun yigʻma turdagi konvert. `TriggerId`, `RoleId`, `Permission`, `CustomParameterId`). Umumiy oqimlar va yagona turdagi Norito kodlash uchun foydalidir.
- `ChainId`: Tranzaktsiyalarda takroriy himoya qilish uchun foydalaniladigan noaniq zanjir identifikatori.Identifikatorlarning qator shakllari (`Display`/`FromStr` bilan ikki marta o'tish mumkin):
- `DomainId`: `name` (masalan, `wonderland`).
- `AccountId`: kanonik domensiz hisob identifikatori `AccountAddress` orqali faqat I105 sifatida kodlangan. Qattiq tahlil qiluvchi kirishlar kanonik bo'lishi kerak I105; domen qo'shimchalari (`@domain`), hisob taxalluslari literallari, kanonik olti burchakli tahliliy kiritish, eski `norito:` foydali yuklari va `uaid:`/`opaque:` hisob tahlil qilish shakllari rad etiladi. Zanjirdagi hisob taxalluslari `name@domain.dataspace` yoki `name@dataspace` dan foydalanadi va kanonik `AccountId` qiymatlariga mos keladi.
- `AssetDefinitionId`: kanonik aktivni aniqlash baytlari ustidagi kanonik prefikssiz Base58 manzili. Bu ommaviy aktiv identifikatori. Zanjirdagi obyekt taxalluslari `name#domain.dataspace` yoki `name#dataspace` dan foydalanadi va faqat ushbu kanonik Base58 obyekt identifikatoriga o‘rnatiladi.
- `AssetId`: kanonik yalang'och Base58 shaklida davlat aktivlari identifikatori. `name#dataspace` yoki `name#domain.dataspace` kabi obyekt taxalluslari `AssetId` ga mos keladi. Ichki buxgalteriya xoldinglari qo'shimcha ravishda kerak bo'lganda `asset + account + optional dataspace` bo'lingan maydonlarini ochishi mumkin, ammo bu kompozit shakl ommaviy `AssetId` emas.
...
- `AssetDefinition { id, name, description?, alias?, spec: NumericSpec, mintable: Mintable, logo: Option<SorafsUri>, metadata, owned_by: AccountId, total_quantity: Numeric }`.
  ...
  - taxallusni ijaraga olish holati doimiy taxallusni bog'lovchi yozuvda ishonchli tarzda saqlanadi; Inline `alias` maydoni ta'riflar yadro/Torii API orqali qayta o'qilganda olinadi.
  - Torii aktivlarni aniqlash boʻyicha javoblar `alias_binding { alias, status, lease_expiry_ms, grace_until_ms, bound_at_ms }`ni oʻz ichiga olishi mumkin, bunda `status` `permanent`, `leased_active`, Kotodama, yoki Kotodama,.
  - Taxallus o'lchamlari tugun devor soati o'rniga so'nggi tasdiqlangan blok vaqt tamg'asidan foydalanadi. `grace_until_ms` o'tgandan so'ng, taxallusni tanlovchilar, agar tozalash eskirgan bog'lashni olib tashlamagan bo'lsa ham, darhol hal qilishni to'xtatadi; to'g'ridan-to'g'ri aniqlangan o'qishlar hali ham `expired_pending_cleanup` sifatida uzluksiz bog'lanish haqida xabar berishi mumkin.
...
- Quruvchi: `NewNft` `Nft::new(id, content)` orqali.
...
- O'rnatilgan o'quv oilalari quyidagilar bo'yicha tashkil etiladi:
  ...
  - Meta oqimlari uchun raqamlarni kiriting: `InstructionType`, `SetKeyValueBox` (domain/account/asset_def/nft/trigger) kabi qutidagi summalar.
...
- Ko'rsatmalar reestri: `instruction_registry!{ ... }` makros turi nomi bilan kalitlangan ish vaqti dekodlash registrini yaratadi. `InstructionBox` klon va Norito serde tomonidan dinamik (de)seriyalashtirishga erishish uchun foydalaniladi. Agar `set_instruction_registry(...)` orqali hech qanday reestr aniq o'rnatilmagan bo'lsa, ikkilik fayllarni mustahkam saqlash uchun birinchi foydalanishda barcha asosiy ISI bilan o'rnatilgan standart registr dangasalik bilan o'rnatiladi.
...
- `TransactionBuilder`: `chain`, `authority`, `creation_time_ms`, ixtiyoriy `time_to_live_ms` va `nonce`, `nonce`, I018 va I018 bilan tranzaksiya yukini tuzadi `Executable`.
...
- Utilitalar: `presigned`, `set_transaction_results(...)`, `set_transaction_results_with_transcripts(...)`, `header()`, `signatures()`, `hash()`, `hash()`, I128NI00X00, Kotodama00, Kotodama00.
...
- `ExecWitness` xabarlari (Torii orqali uzatiladi va konsensus g'iybatiga asoslangan) endi ikkala `fastpq_transcripts` va proverga tayyor `fastpq_batches: Vec<FastpqTransitionBatch>` o'rnatilgan ildiz Kotodama, slotlar, perm_root, tx_set_hash), shuning uchun tashqi proverlar transkriptlarni qayta kodlamasdan kanonik FASTPQ qatorlarini qabul qilishlari mumkin.
...
- turi o'chirilgan shakllar:
  - `QueryBox<T>` - bu global registr tomonidan qo'llab-quvvatlangan, Norito serdeli, o'chirilgan, o'chirilgan `Query<Item = T>`.
  - `QueryWithFilter<T> { query, predicate, selector }` so'rovni DSL predikati/selektori bilan bog'laydi; `From` orqali o'chirilgan takrorlanadigan so'rovga aylanadi.
- Ro'yxatga olish kitobi va kodeklar:
  ...
  - `QueryOutputBatchBox` bir hil vektorlar (masalan, `Vec<Account>`, `Vec<Name>`, `Vec<AssetDefinition>`, `Vec<BlockHeader>`) ustidagi yigʻindi turi, plyus kortej va paginatsiyalash yordamchilari uchun.
...
- Xususiyatlar ixtiyoriy API-larni boshqaradi (`std`, `json`, `transparent_api`, `ffi_export`, `ffi_import`, `json`, `json`, I1805 `fault_injection`).
...
Obyektni aniqlash identifikatori / taxallusning tezkor ma'lumotnomasi (CLI + Torii):
...
- Davlat aktivlarini qidirish `asset + account + optional scope` bilan tegishli balanslarga murojaat qiladi; xom kodlangan `AssetId` literallari ichki ko'rinishdir va Torii/CLI selektor yuzasining bir qismi emas.
- `POST /v1/assets/definitions/query` va `GET /v1/assets/definitions`, `alias_binding.status`, `alias_binding.lease_expiry_ms`, `alias_binding.grace_until_ms` va Kotodama, Kotodama, `alias_binding.bound_at_ms` va Kotodama dan ortiq turlarni qabul qiladi. `name`, `alias` va `metadata.*`.

## Versiyalash

- `SignedTransaction`, `SignedBlock` va `SignedQuery` kanonik Norito kodlangan tuzilmalardir. Ularning har biri `EncodeVersioned` orqali kodlanganda foydali yukini joriy ABI versiyasi (hozirda `1`) bilan prefikslash uchun `iroha_version::Version` ni amalga oshiradi.
