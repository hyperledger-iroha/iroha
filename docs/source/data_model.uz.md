...
## Qo'llanish doirasi va asoslari
...
- `IdBox`: Har qanday qoʻllab-quvvatlanadigan identifikator uchun yigʻma turdagi konvert (`DomainId`, `AccountId`, `AssetDefinitionId`, `AssetId`, `AssetId`, Kotodama, Norito `TriggerId`, `RoleId`, `Permission`, `CustomParameterId`). Umumiy oqimlar va yagona turdagi Norito kodlash uchun foydalidir.
...
- `Account { id, metadata, label?, uaid?, opaque_ids[] }` — `label` ixtiyoriy birlamchi `AccountAlias`, qayta kalit yozuvlar tomonidan qoʻllaniladi, `uaid` ixtiyoriy Nexus boʻylab [Universal Hisob ID004] va (Norito) tashiydi. `opaque_ids` ushbu UAIDga bog'langan yashirin identifikatorlarni kuzatib boradi. Saqlangan hisob holati endi bog'langan domen maydoniga ega emas.
- Quruvchilar:
  - `NewAccount` `Account::new(id)` orqali kanonik domensiz hisob mavzusini ro'yxatdan o'tkazadi.
- Taxallus modeli:
...
- `AssetDefinition { id, name, description?, alias?, spec: NumericSpec, mintable: Mintable, logo: Option<SorafsUri>, metadata, owned_by: AccountId, total_quantity: Numeric }`.
  ...
  - Torii aktivlarni aniqlash boʻyicha javoblar `alias_binding { alias, status, lease_expiry_ms, grace_until_ms, bound_at_ms }`ni oʻz ichiga olishi mumkin, bunda `status` `permanent`, `leased_active`, Kotodama, yoki Norito.
  - Taxallus o'lchamlari tugun devor soati o'rniga so'nggi tasdiqlangan blok vaqt tamg'asidan foydalanadi. `grace_until_ms` o'tgandan so'ng, taxallusni tanlovchilar, agar tozalash eskirgan bog'lanishni olib tashlamagan bo'lsa ham, darhol hal qilishni to'xtatadi; to'g'ridan-to'g'ri aniqlangan o'qishlar hali ham `expired_pending_cleanup` sifatida uzluksiz bog'lanish haqida xabar berishi mumkin.
...
- `action::Action { executable: Executable, repeats: Repeats, authority: AccountId, filter: EventFilterBox, metadata }`.
  ...
  - Xavfsizlik: `TriggerCompleted` amal filtri sifatida ishlatilmaydi (seriyalashtirish vaqtida tasdiqlangan).
- `EventBox`: quvur liniyasi, quvur liniyasi-to'plami, ma'lumotlar, vaqt, bajarish-trigger va trigger-tugallangan hodisalar uchun yig'indi turi; `EventFilterBox` obunalar va trigger filtrlarini aks ettiradi.
...
- O'rnatilgan o'quv oilalari quyidagilar bo'yicha tashkil etiladi:
  ...
  - Meta oqimlari uchun raqamlarni yozing: `InstructionType`, `SetKeyValueBox` (domain/account/asset_def/nft/trigger) kabi qutiga solingan summalar.
...
- `TransactionBuilder`: tranzaksiya yukini `chain`, `authority`, `creation_time_ms`, ixtiyoriy `time_to_live_ms` va `nonce`, `nonce`, I018 va I018 bilan tuzadi `Executable`.
...
- Utilitalar: `presigned`, `set_transaction_results(...)`, `set_transaction_results_with_transcripts(...)`, `header()`, `signatures()`, `hash()`, `hash()`, Kotodama, Norito00, Norito00
...
- Blokni qo'shish dalillari (`BlockProofs`) kirish/natija Merkle dalillari va `fastpq_transcripts` xaritasini ochib beradi, shuning uchun zanjirdan tashqari provayderlar tranzaksiya xesh bilan bog'liq transfer deltalarini olishlari mumkin.
- `ExecWitness` xabarlari (Torii orqali uzatiladi va konsensus g'iybatiga asoslangan) endi ikkala `fastpq_transcripts` va proverga tayyor `fastpq_batches: Vec<FastpqTransitionBatch>`, o'rnatilgan Kotodama, slotlari o'z ichiga oladi. perm_root, tx_set_hash), shuning uchun tashqi proverlar transkriptlarni qayta kodlamasdan kanonik FASTPQ qatorlarini qabul qilishlari mumkin.
...
- DSL: `query::dsl` da kompilyatsiya vaqtida tekshiriladigan predikatlar va selektorlar uchun proyeksiya belgilari (`HasProjection<PredicateMarker>` / `SelectorMarker`) bilan amalga oshirilgan. Agar kerak bo'lsa, `fast_dsl` xususiyati engilroq variantni ochib beradi.
...
### Custom Instruction (ijrochi tomonidan belgilangan ISI)- Turi: `isi::CustomInstruction { payload: Json }` barqaror simli identifikator `"iroha.custom"`.
...
- Xususiyatlar ixtiyoriy API-larni boshqaradi (`std`, `json`, `transparent_api`, `ffi_export`, `ffi_import`, `transparent_api`, `transparent_api`, I1804 `fault_injection`).
...
- `POST /v1/assets/definitions/query` va `GET /v1/assets/definitions`, `alias_binding.status`, `alias_binding.lease_expiry_ms`, `alias_binding.grace_until_ms` va Kotodama, Kotodama, I1800003570, `name`, `alias` va `metadata.*`.
