...
- `PendingActivation(epoch)` — ガバナンス指定の activation epoch を待機。タプル payload は
  `current_epoch + 1` で計算される最短 activation epoch を保持する
...
- ガバナンスが承認/却下する。承認まで status は `PendingActivation` で、ターゲット epoch
  (`current_epoch + 1` 登録時) 到達後の次の境界で runtime が
  `Active` に昇格させる。
...
- **Activation/exit operations:** 登録は `PendingActivation` に入り、`current_epoch + 1`
