...
- **Torii/CLI quickstart:**
  ...
  - `iroha_cli app nexus public-lane validators --lane <id> [--summary]`
    ...
    `iroha_cli app nexus public-lane stake --lane <id> [--validator soraカタカナ...] [--summary]` は
  ...
  - Torii snapshots (dashboards/SDKs 向け):
    - `GET /v1/nexus/public_lanes/{lane}/validators` – metadata, status
      ...
      `canonical Katakana i105 literal rendering` で literal 表示を制御（canonical Katakana i105 のみ）。
    - `GET /v1/nexus/public_lanes/{lane}/stake` – stake shares (`validator`,
      `staker`, bonded amount) と pending unbond timers。`?validator=soraカタカナ...` は
      特定バリデータ向けにフィルタし、`canonical Katakana i105 rendering` は全 literal に適用。
