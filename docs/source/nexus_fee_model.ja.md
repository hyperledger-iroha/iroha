...
- IVM トランザクションは手数料の露出を抑えるために `gas_limit` (`u64`, > 0) メタデータを含める必要がある。
  `/v2/contracts/call` エンドポイントは `gas_limit` を明示的に要求し、無効な値は拒否される。
...
- ブロック実行は lane/dataspace ごとに receipts を集計し、`/v2/sumeragi/status` の
