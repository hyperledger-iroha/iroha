...
- Fixture: `fixtures/account/address_vectors.json` (IH58 (preferred) + compressed (`sora`, second-best) + multisig مثبت/منفی کیسز).
...
| سطح | نفاذ | نوٹس |
|---------|-------------|-------|
| Rust data-model | `crates/iroha_data_model/tests/account_address_vectors.rs` | JSON parse کرتا ہے، canonical payloads دوبارہ بناتا ہے، اور IH58 (preferred)/sora (second-best)/canonical conversions + structured errors کو check کرتا ہے. |
| Torii | `crates/iroha_torii/tests/account_address_vectors.rs` | server-side codecs validate کرتا ہے تاکہ Torii خراب IH58 (preferred)/sora (second-best) payloads کو deterministically reject کرے. |
| JavaScript SDK | `javascript/iroha_js/test/address.test.js` | V1 fixtures (IH58 (preferred)/sora (second-best)/fullwidth) کو mirror کرتا ہے اور ہر منفی کیس کیلئے Norito طرز error codes کی تصدیق کرتا ہے. |
| Swift SDK | `IrohaSwift/Tests/IrohaSwiftTests/AccountAddressTests.swift` | IH58 (preferred)/sora (second-best) decoding، multisig payloads، اور Apple پلیٹ فارمز پر error surfacing کو exercise کرتا ہے. |
