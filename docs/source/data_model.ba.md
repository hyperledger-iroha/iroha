...
- `AssetId`: канонический кодированный литераль `<asset-definition-id>#<account-id>` (мираҫ текст формалары беренсе релизда ярҙам итмәй).
...
- Мине/яндырыу/күсереү өсөн активтар идентификаторҙары канон `<asset-definition-id>#<account-id>` ҡала; уларҙы төҙөү:
  - 18НИ00000326Х
