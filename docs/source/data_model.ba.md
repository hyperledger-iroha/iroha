...
- `AssetId`: канонический кодированный литераль `<canonical-base58-asset-definition-id>` (мираҫ текст формалары беренсе релизда ярҙам итмәй).
...
- Мине/яндырыу/күсереү өсөн активтар идентификаторҙары канон `<canonical-base58-asset-definition-id>` ҡала; уларҙы төҙөү:
  - 18НИ00000326Х
