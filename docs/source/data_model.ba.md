...
- `AssetId`: канонический кодированный литераль `<asset-definition-id>#<i105-account-id>` (мираҫ текст формалары беренсе релизда ярҙам итмәй).
...
- Мине/яндырыу/күсереү өсөн активтар идентификаторҙары канон `<asset-definition-id>#<i105-account-id>` ҡала; уларҙы төҙөү:
  - 18НИ00000326Х
