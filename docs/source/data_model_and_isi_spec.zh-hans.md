...
- `AssetDefinitionId` — 规范 `aid:<32-lower-hex-no-dash>`（UUID-v4 字节）。定义：`{ id, name, description?, alias?, spec: NumericSpec, mintable: Mintable, logo, metadata, owned_by, total_quantity }`。 `alias` 文字必须是 `<name>#<domain>@<dataspace>` 或 `<name>#<dataspace>`，其中 `<name>` 等于资产定义名称。代码：`crates/iroha_data_model/src/asset/definition.rs`。
- `AssetId`：规范编码文字 `norito:<hex>`（第一个版本不支持旧文本形式）。- `NftId` — `nft$domain`。 NFT：`{ id, content: Metadata, owned_by }`。代码：`crates/iroha_data_model/src/nft.rs`。
...
- 参数更新：
  - `SetParameter(SumeragiParameter::BlockTimeMs(2500).into())` 更新并发出 `ConfigurationEvent::Changed`。CLI / Torii `aid` + 别名示例：
- 使用规范辅助+显式名称+长别名进行注册：
...
- 将别名解析为规范援助：
  - `POST /v2/assets/aliases/resolve` 与 JSON `{ "alias": "pkr#ubl@sbp" }`

迁移注意事项：
- `name#domain` 在第一个版本中故意不支持文本资产定义 ID。
- 铸造/销毁/转移边界的资产 ID 保持规范 `norito:<hex>`；将 `iroha tools encode asset-id` 与 `--definition aid:...` 或 `--alias ...` 加 `--account` 结合使用。
