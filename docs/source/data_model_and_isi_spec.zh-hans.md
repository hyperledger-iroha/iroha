...
- `AssetId`：规范编码文字 `<base58-asset-id>#<katakana-i105-account-id>`（第一个版本不支持旧文本形式）。- `NftId` — `nft$domain`。 NFT：`{ id, content: Metadata, owned_by }`。代码：`crates/iroha_data_model/src/nft.rs`。
...
- 铸造/销毁/转移边界的资产 ID 保持规范 `<base58-asset-id>#<katakana-i105-account-id>`；将 `iroha tools encode asset-id` 与 `--definition <base58-asset-definition-id>` 或 `--alias ...` 加 `--account` 结合使用。
