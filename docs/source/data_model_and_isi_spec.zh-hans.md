...
- `AccountId` — 规范地址通过 `AccountAddress`（IH58 / `sora…` 压缩/十六进制）生成，Torii 通过 `AccountAddress::parse_encoded` 标准化输入。 IH58是首选账户格式； `sora…` 形式对于仅 Sora 的 UX 来说是第二好的。熟悉的 `alias` (rejected legacy form) 字符串仅保留作为路由别名。帐号：`{ id, metadata }`。代码：`crates/iroha_data_model/src/account.rs`。
