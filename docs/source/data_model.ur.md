...
## نام اور شناخت کنندگان- `Name`: درست متنی شناخت کنندہ۔ وہائٹ ​​اسپیس اور محفوظ کردہ حروف `@`, `#`, `$` (مکمل IDs میں استعمال ہونے) کی اجازت نہیں دیتا ہے۔ توثیق کے ساتھ `FromStr` کے ذریعے قابل تعمیر۔ ناموں کو پارس کرنے پر یونیکوڈ NFC میں معمول بنایا جاتا ہے (مثبت طور پر مساوی املا کو ایک جیسی اور ذخیرہ شدہ کمپوزڈ سمجھا جاتا ہے)۔ خصوصی نام `genesis` محفوظ ہے (کیس کی غیر حساسیت سے جانچ پڑتال کی گئی ہے)۔
...
- بلڈر: `NewDomain` `with_logo` کے ساتھ، `with_metadata`، پھر `Registrable::build(authority)` `owned_by` سیٹ کرتا ہے۔
...
- `Account { id, metadata, label?, uaid?, linked_domains? }` — `label` ایک اختیاری مستحکم عرف ہے جسے ریکی ریکارڈز کے ذریعے استعمال کیا جاتا ہے، `uaid` اختیاری Nexus وسیع [یونیورسل اکاؤنٹ ID](Norito)، اور Norito ہے کینونیکل شناخت کے حصے کے بجائے اخذ کردہ اشاریہ ریاست۔
- معمار:
  - `NewAccount` بذریعہ `Account::new(scoped_id)` ایک واضح ڈومین سے منسلک رجسٹریشن کو عملی شکل دیتا ہے اور اس لیے `ScopedAccountId` کی ضرورت ہوتی ہے۔
  - `NewAccount` بذریعہ `Account::new_domainless(id)` صرف یونیورسل اکاؤنٹ کا مضمون رجسٹر کرتا ہے جس کا کوئی لنکڈ ڈومین نہیں ہے۔
- عرفی ماڈل:
  - کیننیکل اکاؤنٹ کی شناخت میں کبھی بھی ڈومین یا ڈیٹا اسپیس سیگمنٹ شامل نہیں ہوتا ہے۔
  - اکاؤنٹ کے عرفی نام `AccountId` کے اوپر الگ الگ SNS/اکاؤنٹ لیبل بائنڈنگز ہیں۔
  - ڈومین کوالیفائیڈ عرفی نام جیسے `merchant@hbl.sbp` عرف بائنڈنگ میں ڈومین اور ڈیٹا اسپیس دونوں رکھتے ہیں۔
  - ڈیٹا اسپیس روٹ عرفی نام جیسے کہ `merchant@sbp` صرف ڈیٹا اسپیس رکھتا ہے اور اس لیے قدرتی طور پر `Account::new_domainless(...)` کے ساتھ جوڑتا ہے۔
  - ٹیسٹ اور فکسچر کو پہلے عالمگیر `AccountId` کو سیڈ کرنا چاہیے، پھر اکاؤنٹ کی شناخت میں ڈومین کے مفروضوں کو انکوڈنگ کرنے کے بجائے الگ الگ ڈومین لنکس، عرفی لیز، اور عرفی اجازتیں شامل کریں۔

### اثاثہ کی تعریفیں اور اثاثے۔
- `AssetDefinitionId { aid_bytes: [u8; 16] }` متنی طور پر ایک غیر سابقہ بیس 58 ایڈریس کے طور پر ورژننگ اور چیکسم کے ساتھ سامنے آیا۔
- `AssetDefinition { id, name, description?, alias?, spec: NumericSpec, mintable: Mintable, logo: Option<SorafsUri>, metadata, owned_by: AccountId, total_quantity: Numeric }`۔
  ...
  - Torii اثاثہ کی تعریف کے جوابات میں `alias_binding { alias, status, lease_expiry_ms, grace_until_ms, bound_at_ms }` شامل ہو سکتا ہے، جہاں `status` `permanent`، `leased_active`، `leased_grace`, Norito, `leased_grace`, یا 05014X میں سے ایک ہے۔
  - عرف کی ریزولیوشن نوڈ وال کلاک کے بجائے جدید ترین کمٹڈ بلاک ٹائم اسٹیمپ کا استعمال کرتی ہے۔ ایک بار جب `grace_until_ms` گزر جاتا ہے، عرف سلیکٹرز فوری طور پر حل کرنا بند کر دیتے ہیں یہاں تک کہ اگر سویپ کلین اپ نے ابھی تک باسی بائنڈنگ کو نہیں ہٹایا ہے۔ ڈائریکٹ ڈیفینیشن ریڈز اب بھی لنگرنگ بائنڈنگ کو `expired_pending_cleanup` کے بطور رپورٹ کر سکتے ہیں۔
...
## کرپٹوگرافک پرائمیٹوز (خصوصیت `sm`)
- `Sm2PublicKey` اور `Sm2Signature`: SEC1 کے مطابق پوائنٹس اور SM2 کے لیے مقررہ چوڑائی `r∥s` دستخط۔ تعمیر کنندگان وکر کی رکنیت اور امتیازی IDs کی توثیق کرتے ہیں۔ Norito انکوڈنگ `iroha_crypto` کے ذریعہ استعمال کی جانے والی کینونیکل نمائندگی کا آئینہ دار ہے۔
...
یہ اقسام موجودہ Ed25519/BLS/ML-DSA پرائمیٹوز کے ساتھ بیٹھتی ہیں اور `--features sm` کے ساتھ ورک اسپیس بننے کے بعد عوامی اسکیما کا حصہ بن جاتی ہیں۔
...
- `action::Action { executable: Executable, repeats: Repeats, authority: AccountId, filter: EventFilterBox, metadata }`۔
  ...
  - حفاظت: `TriggerCompleted` کو ایکشن کے فلٹر کے طور پر استعمال نہیں کیا جا سکتا (سیریلائزیشن کے دوران توثیق شدہ)۔
...
- حسب ضرورت پیرامیٹرز: ایگزیکیوٹر کی وضاحت کردہ، `Json` کے طور پر لی جاتی ہے، جس کی شناخت `CustomParameterId` (a `Name`) سے ہوتی ہے۔

## ISI (Iroha خصوصی ہدایات)- بنیادی خصوصیت: `Instruction` کے ساتھ `dyn_encode`، `as_any`، اور ایک مستحکم فی قسم شناخت کنندہ `id()` (کنکریٹ قسم کے نام سے پہلے سے طے شدہ)۔ تمام ہدایات `Send + Sync + 'static` ہیں۔
...
- انسٹرکشن رجسٹری: `instruction_registry!{ ... }` میکرو ایک رن ٹائم ڈی کوڈ رجسٹری بناتا ہے جس کی قسم کے نام سے کی جاتی ہے۔ متحرک (de) سیریلائزیشن حاصل کرنے کے لیے `InstructionBox` کلون اور Norito serde کے ذریعے استعمال کیا جاتا ہے۔ اگر `set_instruction_registry(...)` کے ذریعے کوئی رجسٹری واضح طور پر سیٹ نہیں کی گئی ہے تو، تمام بنیادی ISI کے ساتھ ایک بلٹ ان ڈیفالٹ رجسٹری بائنریز کو مضبوط رکھنے کے لیے پہلے استعمال میں سستی سے انسٹال کی جاتی ہے۔
...
- `TransactionBuilder`: `chain`، `authority`، `creation_time_ms`، اختیاری `time_to_live_ms` اور `nonce`، اور `nonce`، اور `chain`، کے ساتھ ایک ٹرانزیکشن پے لوڈ بناتا ہے `Executable`۔
...
## بلاکس- `SignedBlock` (ورژنڈ) انکیپسیلیٹس:
  ...
  - `result: BlockResult` (ثانوی عمل درآمد کی حالت) جس میں `time_triggers`، اندراج/نتائج مرکل ٹریز، `transaction_results`، اور `fastpq_transcripts: BTreeMap<Hash, Vec<TransferTranscript>>` شامل ہیں۔
- یوٹیلیٹیز: `presigned`, `set_transaction_results(...)`, `set_transaction_results_with_transcripts(...)`, `header()`, `signatures()`, `hash()`, `hash()`, Kotodama, Kotodama,00018
...
- `ExecWitness` پیغامات (Torii کے ذریعے سٹریم کیے گئے اور متفقہ گپ شپ پر piggy-backed) میں اب `fastpq_transcripts` اور prover-ready `fastpq_batches: Vec<FastpqTransitionBatch>` شامل ہیں perm_root، tx_set_hash)، اس لیے بیرونی پروررز ٹرانسکرپٹس کو دوبارہ انکوڈنگ کیے بغیر کیننیکل FASTPQ قطاریں نگل سکتے ہیں۔

## سوالات- دو ذائقے:
  - واحد: `SingularQuery<Output>` (جیسے، `FindParameters`، `FindExecutorDataModel`) کو لاگو کریں۔
  - قابل تکرار: `Query<Item>` کو لاگو کریں (جیسے، `FindAccounts`، `FindAssets`، `FindDomains`، وغیرہ)۔
- قسم کے مٹائے گئے فارم:
  - `QueryBox<T>` ایک باکسڈ، مٹا ہوا `Query<Item = T>` Norito serde کے ساتھ ہے جسے عالمی رجسٹری کی حمایت حاصل ہے۔
  - `QueryWithFilter<T> { query, predicate, selector }` ڈی ایس ایل پریڈیکیٹ/سلیکٹر کے ساتھ ایک سوال جوڑتا ہے۔ `From` کے توسط سے مٹائے ہوئے دوبارہ قابل استفسار میں تبدیل ہوتا ہے۔
- رجسٹری اور کوڈیکس:
  ...
  - `QueryOutputBatchBox` یکساں ویکٹرز (مثال کے طور پر، `Vec<Account>`، `Vec<Name>`، `Vec<AssetDefinition>`، `Vec<AssetDefinition>`، `Vec<BlockHeader>`) پر ایک مجموعہ کی قسم ہے، پلس پیگنیشن ہیلپرز اور ایگزینشن ٹپلس۔
- DSL: `query::dsl` میں پروجیکشن ٹریٹس (`HasProjection<PredicateMarker>` / `SelectorMarker`) کے ساتھ مرتب وقت کی جانچ شدہ پیشین گوئیوں اور سلیکٹرز کے لیے لاگو کیا گیا ہے۔ ایک `fast_dsl` خصوصیت اگر ضرورت ہو تو ہلکے ویرینٹ کو ظاہر کرتی ہے۔

## ایگزیکیوٹر اور توسیع پذیری۔- `Executor { bytecode: IvmBytecode }`: توثیق کنندہ کے ذریعے انجام دیا گیا کوڈ بنڈل۔
...
- ڈیفالٹ ایگزیکیوٹر کا رویہ: `iroha_core` میں بلٹ ان ایگزیکیوٹر `CustomInstruction` پر عمل نہیں کرتا ہے اور اگر سامنا ہوا تو گھبرا جائے گا۔ ایک کسٹم ایگزیکیوٹر کو `InstructionBox` کو `CustomInstruction` پر کم کرنا چاہیے اور تمام توثیق کاروں پر پے لوڈ کی تعییناتی طور پر تشریح کرنی چاہیے۔
...
## میٹا ڈیٹا- `Metadata(BTreeMap<Name, Json>)`: متعدد اداروں سے منسلک کلید/ویلیو اسٹور (`Domain`, `Account`, `AssetDefinition`, `Nft`، ٹرگرز، اور ٹرانزیکشنز)۔
- API: `contains`, `iter`, `get`, `insert`، اور (`transparent_api` کے ساتھ) `remove`۔

## خصوصیات اور عزم

- فیچرز کنٹرول اختیاری APIs (`std`, `json`, `transparent_api`, `ffi_export`, `ffi_import`, `fast_dsl`, `fast_dsl`, I103NI, I1035 `fault_injection`)۔
...
## شفاف API (`transparent_api`)- مقصد: اندرونی اجزاء جیسے Torii، ایگزیکیوٹرز، اور انٹیگریشن ٹیسٹس کے لیے `#[model]` سٹرکٹس/enums تک مکمل، تغیر پذیر رسائی کو ظاہر کرتا ہے۔ اس کے بغیر، وہ آئٹمز جان بوجھ کر مبہم ہیں لہذا بیرونی SDKs صرف محفوظ کنسٹرکٹرز اور انکوڈ شدہ پے لوڈز کو دیکھتے ہیں۔
- میکانکس: `iroha_data_model_derive::model` میکرو ہر عوامی فیلڈ کو `#[cfg(feature = "transparent_api")] pub` کے ساتھ دوبارہ لکھتا ہے اور پہلے سے طے شدہ تعمیر کے لیے ایک نجی کاپی رکھتا ہے۔ خصوصیت کو فعال کرنے سے وہ cfgs پلٹ جاتے ہیں، لہذا `Account`، `Domain`، `Asset`، وغیرہ کو تباہ کرنا ان کے متعین ماڈیولز سے باہر قانونی ہو جاتا ہے۔
...
- فعال کرنا: `Cargo.toml` میں انحصار میں `features = ["transparent_api"]` شامل کریں۔ ورک اسپیس کریٹس جن کو JSON پروجیکشن کی ضرورت ہوتی ہے (مثال کے طور پر، `iroha_torii`) جھنڈا خود بخود آگے بڑھاتا ہے، لیکن فریق ثالث کے صارفین کو اسے بند رکھنا چاہیے جب تک کہ وہ تعیناتی کو کنٹرول نہ کریں اور وسیع تر API کی سطح کو قبول نہ کریں۔
...
- `POST /v1/assets/definitions/query` اور `GET /v1/assets/definitions` `alias_binding.status`، `alias_binding.lease_expiry_ms`، `alias_binding.grace_until_ms`، اور `alias_binding.grace_until_ms`، اور Norito کے علاوہ Norito سے زیادہ `GET /v1/assets/definitions`، `GET /v1/assets/definitions` پر اثاثہ کی تعریف کے فلٹرز قبول کرتا ہے `name`، `alias`، اور `metadata.*`۔
