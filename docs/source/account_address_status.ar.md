...
- Fixture: `fixtures/account/address_vectors.json` (IH58 + compressed + multisig حالات ايجابية/سلبية).
...
|---------|-------------|-------|
| Rust data-model | `crates/iroha_data_model/tests/account_address_vectors.rs` | يفك JSON، يعيد بناء payloads canonical، ويتحقق من تحويلات IH58/compressed/canonical + اخطاء منظمة. |
| Torii | `crates/iroha_torii/tests/account_address_vectors.rs` | يتحقق من codecs جهة الخادم لكي يرفض Torii payloads IH58/compressed غير الصحيحة بشكل حتمي. |
| JavaScript SDK | `javascript/iroha_js/test/address.test.js` | يعكس fixtures V1 (IH58/compressed/fullwidth) ويتحقق من رموز الاخطاء بنمط Norito لكل حالة سلبية. |
| Swift SDK | `IrohaSwift/Tests/IrohaSwiftTests/AccountAddressTests.swift` | يمتحن فك ترميز IH58/compressed و multisig payloads واظهار الاخطاء على منصات Apple. |
