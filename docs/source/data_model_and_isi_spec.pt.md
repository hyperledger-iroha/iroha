...
- `AssetId` — `asset#domain#account` ou `asset##account` se os domínios corresponderem, onde `account` é a string canônica `AccountId` (preferencialmente IH58). Ativo: `{ id, value: Numeric }`. Código: `crates/iroha_data_model/src/asset/{id.rs,value.rs}`.
