...
- **Primitivas criptográficas** (recurso `sm`):
  ...
  Esses tipos ficam junto com as primitivas Ed25519/BLS/ML-DSA existentes e estão disponíveis para consumidores de modelo de dados (Torii, SDKs, ferramentas genesis) assim que o recurso `sm` for habilitado.
...
- Cadastrar Conta: builds a partir de `NewAccount`, não permitidas no domínio `genesis`; A conta `genesis` não pode ser registrada.
...
- Propriedade de AssetDefinition: altera `AssetDefinition.owned_by` para a conta de destino.
  - Pré-requisitos: ambas as contas existem; existe definição; a fonte deve atualmente possuí-lo; a autoridade deve ser conta de origem, proprietário do domínio de origem ou proprietário do domínio de definição de ativo.
...
- Atualização de parâmetros:
  - `SetParameter(SumeragiParameter::BlockTimeMs(2500).into())` atualiza e emite `ConfigurationEvent::Changed`.CLI / Torii `aid` + exemplos de alias:
- Registre-se com ajuda canônica + nome explícito + alias longo:
  -`iroha ledger asset definition register --id aid:2f17c72466f84a4bb8a8e24884fdcd2f --name pkr --alias pkr#ubl@sbp`
- Registre-se com ajuda canônica + nome explícito + apelido curto:
...
- Resolver alias para ajuda canônica:
  -`POST /v2/assets/aliases/resolve` com JSON `{ "alias": "pkr#ubl@sbp" }`

Nota de migração:
- Os IDs de definição de ativos textuais `name#domain` não são intencionalmente suportados na primeira versão.
- Os IDs de ativos nos limites de criação/gravação/transferência permanecem canônicos `norito:<hex>`; use `iroha tools encode asset-id` com `--definition aid:...` ou `--alias ...` mais `--account`.
