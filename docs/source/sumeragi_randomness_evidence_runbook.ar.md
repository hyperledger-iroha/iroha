...
1. شغل `iroha --output-format text ops sumeragi params` لاثبات ان الباينري حمل
...
راقب `/v2/events/sse` مع stream مفلتر لاثبات ان SDKs ترى نفس البيانات: استخدم
...
- **Mode selection mismatch** — اذا اظهر `iroha --output-format text ops sumeragi params`
...
- **Evidence ingestion stalls** — عندما يتسطح `sumeragi_evidence_records_total` بينما
  تبعث اختبارات chaos اخطاء، شغل `iroha ops sumeragi evidence count` على عدة مدققين وتحقق
  من ان `/v2/sumeragi/evidence/count` يطابق مخرجات CLI. اي اختلاف يعني ان مستهلكي
