...
source_hash: 8598dbc8f90e793d84813a68d4602ce9188eae7fd3c969752be9e9dfdb5613df
source_last_modified: "2026-02-02T11:48:11.880618+00:00"
...
- Validators must use BLS-Normal public keys and present a proof-of-possession (PoP). In NPoS, the consensus roster is filtered to the active public-lane validator set whenever stake records are available (including after commit-topology updates); if the commit topology omits active validators, the roster is widened with the active set before PoP filtering so the full validator set participates. When no active validators are present it falls back to `trusted_peers` after filtering out peers without BLS-Normal keys (the legacy `trusted_peers_bls` mapping no longer exists). If `trusted_peers_pop` is empty, the BLS trusted-peer set remains the validator roster. When `trusted_peers_pop` is present, it defines the validator subset among trusted BLS peers: peers with valid PoPs participate in consensus, while trusted peers without PoPs remain network-trusted observers and are excluded from consensus. Invalid PoP entries are rejected, non-BLS peers are always excluded, and the node `public_key`/`private_key` pair must be BLS-Normal for transport and consensus.
...
- Dedup caches: vote + block-payload/RBC chunk deduplication uses bounded LRU+TTL caches partitioned by message kind; `/v1/sumeragi/status.dedup_evictions` exposes capacity vs TTL evictions for votes, proposals, block-created, block-sync updates, RBC READY/DELIVER payloads, and RBC chunks.
...
- Transaction queue backpressure: `sumeragi_tx_queue_depth`/`sumeragi_tx_queue_capacity` gauge the mempool count, `sumeragi_tx_queue_retained_bytes`/`sumeragi_tx_queue_max_retained_bytes` gauge retained queue memory, and `sumeragi_tx_queue_saturated_by_count`, `sumeragi_tx_queue_saturated_by_bytes`, `sumeragi_tx_queue_saturated_by_age`, plus `sumeragi_tx_queue_oldest_queued_age_ms` identify the pressure cause. `sumeragi_tx_queue_saturated` flips to 1 when any count, byte, or age pressure is active. When count or retained-byte saturation is active, validators skip redundant collector fan-out (behaving as `redundant_send_r = 1`) to shed load and expedite commit of queued transactions.
...
- DA-enabled runs derive the quorum timeout as `3 * (block_time + 3 * commit_time)` to leave headroom for RBC/availability-evidence propagation on slower hosts.
...
- Availability timeouts use `2 * max(quorum_timeout, 2s)` in DA mode to tolerate payload hydration before logging/rebroadcast; when `MissingLocalData` is recorded, quorum reschedules are deferred until the availability timeout so payload recovery can complete. Once the availability timeout elapses, quorum reschedules and idle view changes are allowed even if RBC backlog persists to avoid deadlocks.
...
- `/v1/sumeragi/telemetry` (or `iroha_cli --output-format text ops sumeragi status`) — compact
...
- `/v1/sumeragi/phases` (or `iroha_cli --output-format text ops sumeragi phases`) — latest
...
3. Inspect `sumeragi_bg_post_queue_depth` and `p2p_*_throttled_total` metrics to
   ...
   payload propagation.
4. For FASTPQ/prover stalls, check `/v1/torii/zk/prover/reports` (when the
...
and a manifest/collector audit. Stash counters (`/v1/sumeragi/status.pending_rbc.stash_*`)
...
- During the commit window validate that each validator submitted a commitment: `iroha_cli --output-format text ops sumeragi telemetry` exposes `commitments_total`, while `/v1/sumeragi/telemetry` lists the per-signer map under `vrf.commitments`.
...
- For `non_reveal` increments, collect the stored commitment from the epoch snapshot (`commitments[*].commitment_hex`) and have the validator resend the reveal via `POST /v1/sumeragi/vrf/reveal` (see the curl example above). Confirm `sumeragi_vrf_reveals_late_total` increments and `vrf_committed_no_reveal_total` drops back to zero after the late submission.
...
1. **Collect evidence** via `/v1/sumeragi/evidence` or `iroha_cli ops sumeragi evidence list` while the
...
5. **Verify the schedule** with `iroha_cli --output-format text ops sumeragi params` (or `/v1/sumeragi/params`) to
...
- **List** — `iroha_cli --output-format text ops sumeragi evidence list` (JSON via `/v1/sumeragi/evidence`) surfaces the total count and the most recent records; drop `--output-format text` for the full Norito payload.
...
- **Count** — `iroha_cli ops sumeragi evidence count` (or `GET /v1/sumeragi/evidence/count`) reports the deduplicated total so operators can confirm that rejected payloads did not persist.
