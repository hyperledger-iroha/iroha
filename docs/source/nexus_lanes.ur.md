...
- Tiered-state snapshots اسی lifecycle کو follow کرتے ہیں؛ ہر lane `<cold_root>/lanes/lane_{id:03}_{slug}` میں لکھتی ہے، جہاں `<cold_root>` `cold_store_root` (یا `da_store_root` جب `cold_store_root` سیٹ نہ ہو) ہے، اور retirements directory tree کو `<cold_root>/retired/lanes/` میں منتقل کر دیتی ہیں۔
...
- جب مجموعی budget exceed ہو تو eviction deterministic ہوتی ہے: پہلے SoraNet provision spools کو relative path کے lexicographic order میں trim کیا جاتا ہے، پھر SoraVPN spools، پھر tiered-state کے cold snapshots (قدیم سے جدید)، اور آخر میں Kura کے retired segments۔ Active block bodies اس وقت تک evict نہیں ہوتے جب تک DA rehydration دستیاب نہ ہو۔
...
- `LaneCatalog`, `LaneConfig`, اور `DataSpaceCatalog` `iroha_data_model::nexus` میں رہتے ہیں اور manifests اور SDKs کے لئے Norito format structures فراہم کرتے ہیں۔
