...
| Kotodama | Reduz o auxiliar `transfer_batch((from,to,asset,amount), …)` em `transfer_v1_batch_begin`, chamadas `transfer_asset` sequenciais e `transfer_v1_batch_end`. Cada argumento da tupla deve seguir o formato `(AccountId, AccountId, AssetDefinitionId, quantity)`; transferências únicas mantêm o construtor existente. |
