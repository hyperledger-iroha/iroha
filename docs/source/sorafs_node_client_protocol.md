...
MUST expose the canonical chunker handle (`sorafs.sf1@1.0.0`) and any legacy
aliases via `profile_aliases`, enabling back-compatible discovery while clients
transition. Unknown capability TLVs should be ignored when the advert sets
...
profile. The discovery tests ingest both the modern and legacy fixtures to
...
The `allow_unknown_capabilities` flag enables GREASE-style compatibility probes.
...
2. **Selection.** Gateway chooses the highest-preference profile it can serve.
   ...
   includes a `Content-Chunker: sorafs.sf1@1.0.0` hint to steer legacy clients.
3. **Response.** Successful responses MUST include `Content-Chunker` with the
   ...
   `;legacy=true` so callers can detect the downgrade.
...
use it to decide whether to emit the canonical CARv2 digest (`b3`) or a legacy
...
Provider adverts expose the canonical handle and any legacy aliases via
`profile_aliases`; clients SHOULD mirror that union (including canonical) when
constructing `Accept-Chunker` so gateways can service both modern and legacy
names deterministically.

### CAR Compatibility

Manifests commit to CARv2 archives that use `dag-cbor` (`0x71`) roots and
BLAKE3-256 (`0x1f`) payload digests. For interoperability with legacy tooling,
gateways MAY expose a bridge lane that emits CARv1 + SHA-256 (`0x12`) outputs
when (and only when) the client explicitly requests both of the following:

- `Accept-Chunker` lists a profile flagged as bridge-compatible (currently
  `sorafs.sf1@1.0.0` only).
- `Accept-Digest` advertises `sha2-256` with a weight ≥ the canonical `b3`.

Bridge responses MUST include `Content-Chunker: sorafs.sf1@1.0.0;legacy=true`
and `Content-Digest: sha2-256=<digest>` to make the downgrade explicit.
Gateways ignore `Accept-Digest: sha2-256` when the corresponding chunker profile
does not permit CARv1 export.

Regardless of negotiation outcome, the canonical CARv2 BLAKE3 digest remains the
attested value in `manifest_blake3.json` and `manifest_signatures.json`. Bridge
lanes exist solely for transitional clients and MUST NOT alter the manifest or
registry metadata.

Gateways MUST pin the published topics in the discovery layer for at least
`expires_at - issued_at` seconds. Failure to do so effectively withdraws the
provider from circulation.
