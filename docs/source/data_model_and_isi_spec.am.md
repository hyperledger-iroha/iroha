...
- `AccountId` — ቀኖናዊ አድራሻዎች በ `AccountAddress` (I105 / hex) እና Torii በ `AccountAddress::parse_encoded` በኩል ግብዓቶችን መደበኛ ያደርጋል። I105 ተመራጭ መለያ ቅርጸት ነው; የ `i105` ቅጽ ለሶራ-ብቻ UX ሁለተኛ-ምርጥ ነው። የሚታወቀው `alias` (rejected legacy form) ሕብረቁምፊ እንደ ማዞሪያ ተለዋጭ ስም ብቻ ነው የሚቆየው። መለያ: `{ id, metadata }`. ኮድ: `crates/iroha_data_model/src/account.rs`.
