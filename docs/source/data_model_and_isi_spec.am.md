...
- በአዝሙድ/በማቃጠል/በማስተላለፊያ ድንበሮች ላይ የንብረት መታወቂያዎች ቀኖናዊ `norito:<hex>` ይቆያሉ; `iroha tools encode asset-id` በ `--definition <base58-asset-definition-id>` ወይም `--alias ...` እና `--account` ይጠቀሙ።
