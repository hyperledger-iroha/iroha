...
- `AccountId` — ቀኖናዊ አድራሻዎች በ `AccountAddress` (IH58 / `sora…` compressed / hex) እና Torii በ `AccountAddress::parse_encoded` በኩል ግብዓቶችን መደበኛ ያደርጋል። IH58 ተመራጭ መለያ ቅርጸት ነው; የ `sora…` ቅጽ ለሶራ-ብቻ UX ሁለተኛ-ምርጥ ነው። የሚታወቀው `alias@domain` ሕብረቁምፊ እንደ ማዞሪያ ተለዋጭ ስም ብቻ ነው የሚቆየው። መለያ: `{ id, metadata }`. ኮድ: `crates/iroha_data_model/src/account.rs`.
