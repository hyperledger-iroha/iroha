...
- `AssetId`: ቀኖናዊ ኮድ በጥሬው `<base58-asset-id>#<katakana-i105-account-id>` (የቆዩ የጽሑፍ ቅጾች በመጀመሪያው መለቀቅ አይደገፉም)።- `NftId` - `nft$domain`. NFT: `{ id, content: Metadata, owned_by }`. ኮድ: `crates/iroha_data_model/src/nft.rs`.
...
- በአዝሙድ/በማቃጠል/በማስተላለፊያ ድንበሮች ላይ የንብረት መታወቂያዎች ቀኖናዊ `<base58-asset-id>#<katakana-i105-account-id>` ይቆያሉ; `iroha tools encode asset-id` በ `--definition <base58-asset-definition-id>` ወይም `--alias ...` እና `--account` ይጠቀሙ።
