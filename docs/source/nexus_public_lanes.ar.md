...
- **Torii/CLI quickstart:**
  ...
  - لقطات Torii للوحات المتابعة و SDKs:
    - `GET /v1/nexus/public_lanes/{lane}/validators` – metadata, status
      ...
      `canonical Katakana i105 literal rendering` يتحكم بعرض literals (I105 هو المفضل؛ I105 هو الخيار الثاني لسورا فقط).
    - `GET /v1/nexus/public_lanes/{lane}/stake` – stake shares (`validator`,
      `staker`, bonded amount) مع pending unbond timers. `?validator=soraカタカナ...`
      يرشح الاستجابة للوحة تركز على مدقق واحد؛ `canonical Katakana i105 rendering` يطبق على جميع literals.
