...
- `PendingActivation(epoch)` — انتظار عصر التفعيل المحدد من الحوكمة؛ حمولة الترتيب تخزن
  ابكر عصر تفعيل مشتق من `epoch_length_blocks`.
...
- **عمليات التفعيل/الخروج:** تدخل التسجيلات `PendingActivation` وتترقى تلقائيا في اول كتلة
...
- **Torii/CLI quickstart:**
  - `iroha nexus lane-report --summary` يعرض مدخلات كتالوج lanes، readiness للـ manifest،
  ...
  - لقطات Torii للوحات المتابعة و SDKs:
    - `GET /v1/nexus/public_lanes/{lane}/validators` – metadata, status
      ...
      `address_format=ih58|compressed` يتحكم بعرض literals (IH58 هو المفضل؛ compressed (`snx1`) هو الخيار الثاني لسورا فقط).
    - `GET /v1/nexus/public_lanes/{lane}/stake` – stake shares (`validator`,
      `staker`, bonded amount) مع pending unbond timers. `?validator=account@domain`
      يرشح الاستجابة للوحة تركز على مدقق واحد؛ `address_format` يطبق على جميع literals.
