...
- Les snapshots d etat en couches suivent le meme cycle; chaque lane ecrit dans `cold_store_root/lanes/lane_{id:03}_{slug}` et les retraits migrent l arbre de repertoires vers `cold_store_root/retired/lanes/`.
...
- `LaneCatalog`, `LaneMetadata` et `DataSpaceCatalog` vivent dans `iroha_data_model::nexus` et fournissent des structures compatibles Norito pour manifests et SDKs.
