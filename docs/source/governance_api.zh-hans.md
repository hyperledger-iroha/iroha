...
- 后 `/v2/gov/ballots/plain`
  ...
  - 响应：{“ok”：true，“accepted”：true，“tx_instructions”：[{…}]}
  - 注意：重新投票只能延长——新的投票不能减少现有锁定的数量或到期时间。 `owner`必须等于交易权限。最短持续时间为 `conviction_step_blocks`。- 后 `/v2/gov/finalize`
...
便利端点
- POST `/v2/gov/protected-namespaces` — 将 `gov_protected_namespaces` 直接应用于节点。
  - 请求：{“命名空间”：[“应用程序”，“系统”]}
...
5. 通过 `/v2/sumeragi/evidence` 和 `/v2/sumeragi/status` 进行**审计结果**，以确保证据反驳取得进展并由治理部门实施删除。
...
- 运行时和 CLI 通过 `/v2/sumeragi/params` 和 `iroha sumeragi params --summary` 公开分阶段参数，因此操作员可以确认激活高度和 i105...名册。
- 治理自动化应始终：
