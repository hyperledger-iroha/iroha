...
| `lane_relay_emergency_override_total{outcome="applied"}` | Should stay at 0 outside emergency drills. | Non-zero indicates admin override of lane relay validators. |
...
2. Prepare an admin multisig transaction using `SetLaneRelayEmergencyValidators`:
...
3. Submit the multisig as an authority with `CanManagePeers`.
4. Once the lane validator pool recovers, clear the override by re-submitting the instruction
