...
6. **انٹری sanity۔**
   ...
   - `local_alias` انٹری لازماً Norm v1 سے بنے 12 بائٹ digest کو شامل کرے
     (تصدیق کے لیے `iroha address convert <address-or-account_id> --format json --expect-prefix 753` استعمال کریں؛ JSON خلاصہ `input_domain` کے ذریعے domain دکھاتا ہے اور `--append-domain` `<ih58>@<domain>` کے طور پر دوبارہ انکوڈ دکھاتا ہے)۔
