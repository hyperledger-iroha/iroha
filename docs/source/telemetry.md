...
- **Dashboards.**
  ...
  - `dashboards/grafana/sorafs_appeal_finance.json` tracks appeal-finance report and weekly-rollup publication freshness, failures, payload bytes, rollup lag, and Governance DAG backlog via the same publication metrics.
...
- **Alerting.** Import `dashboards/alerts/sorafs_fetch_rules.yml`, `dashboards/alerts/sorafs_gateway_rules.yml`, `dashboards/alerts/sorafs_capacity_rules.yml`, `dashboards/alerts/sorafs_governance_dag_rules.yml`, and `dashboards/alerts/sorafs_appeal_finance_rules.yml` so ingestion, gateway, capacity saturation, local governance publication, and appeal-finance publication signals stay covered. Validate each pack with its fixture (`dashboards/alerts/tests/sorafs_fetch_rules.test.yml`, `dashboards/alerts/tests/sorafs_capacity_rules.test.yml`, `dashboards/alerts/tests/sorafs_governance_dag_rules.test.yml`, `dashboards/alerts/tests/sorafs_appeal_finance_rules.test.yml`) via `promtool test rules …` (or `scripts/check_prometheus_rules.sh dashboards/alerts/sorafs_*`) before publishing changes.
