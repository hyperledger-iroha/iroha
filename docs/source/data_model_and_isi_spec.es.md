...
- `AccountId`: las direcciones canónicas se producen a través de `AccountAddress` como I105 y Torii normaliza las entradas a través de `AccountAddress::parse_encoded`. El análisis estricto en tiempo de ejecución acepta solo I105 canónico. Los alias de cuentas en cadena utilizan `name@domain.dataspace` o `name@dataspace` y se resuelven en valores canónicos `AccountId`; no son aceptados por analizadores estrictos `AccountId`. Cuenta: `{ id, metadata }`. Código: `crates/iroha_data_model/src/account.rs`.- Política de admisión de cuentas: los dominios controlan la creación implícita de cuentas almacenando un Norito-JSON `AccountAdmissionPolicy` bajo la clave de metadatos `iroha:account_admission_policy`. Cuando la clave está ausente, el parámetro personalizado a nivel de cadena `iroha:default_account_admission_policy` proporciona el valor predeterminado; cuando eso también está ausente, el valor predeterminado estricto es `ImplicitReceive` (primera versión). Las etiquetas de política `mode` (`ExplicitOnly` o `ImplicitReceive`) más límites de creación opcionales por transacción (predeterminado `16`) y por bloque, un `implicit_creation_fee` opcional (cuenta de grabación o hundimiento), `min_initial_amounts` por definición de activo y un `default_role_on_create` opcional (otorgado después de `AccountCreated`, rechaza con `DefaultRoleError` si falta). Génesis no puede optar por participar; Las políticas deshabilitadas/no válidas rechazan instrucciones de estilo recibo para cuentas desconocidas con `InstructionExecutionError::AccountAdmission`. Las cuentas implícitas sellan los metadatos `iroha:created_via="implicit"` antes de `AccountCreated`; Los roles predeterminados emiten un seguimiento `AccountRoleGranted`, y las reglas de referencia del propietario del ejecutor permiten que la nueva cuenta gaste sus propios activos/NFT sin roles adicionales. Código: `crates/iroha_data_model/src/account/admission.rs`, `crates/iroha_core/src/smartcontracts/isi/account_admission.rs`.- `AssetDefinitionId`: dirección Base58 canónica sin prefijo sobre los bytes canónicos de definición de activos. Este es el ID del activo público. Definición: `{ id, name, description?, alias?, spec: NumericSpec, mintable: Mintable, logo, metadata, owned_by, total_quantity }`. Los literales `alias` deben ser `<name>#<domain>.<dataspace>` o `<name>#<dataspace>`, con `<name>` igual al nombre de definición de activo y se resuelven solo en el ID de activo canónico Base58. Código: `crates/iroha_data_model/src/asset/definition.rs`.
  - Los metadatos de arrendamiento de alias se conservan por separado de la fila de definición de activos almacenada. Core/Torii materializa `alias` del registro vinculante cuando se leen las definiciones.
  - Las respuestas de definición de activos Torii exponen `alias_binding { alias, status, lease_expiry_ms, grace_until_ms, bound_at_ms }`, donde `status` es `permanent`, `leased_active`, `leased_grace` o `expired_pending_cleanup`.
  - Los selectores de alias se resuelven según la última hora de creación del bloque comprometido. Después de `grace_until_ms`, los selectores de alias dejan de resolverse incluso si el barrido en segundo plano aún no ha eliminado el enlace obsoleto; Las lecturas de definición directa aún pueden informar que el enlace obsoleto es `expired_pending_cleanup`.
...
- `NftId` — `nft$domain`. NFT: `{ id, content: Metadata, owned_by }`. Código: `crates/iroha_data_model/src/nft.rs`.- `RoleId` — `name`. Rol: `{ id, permissions: BTreeSet<Permission> }` con el constructor `NewRole { inner: Role, grant_to }`. Código: `crates/iroha_data_model/src/role.rs`.
...
- `TriggerId` — `name`. Disparador: `{ id, action }`. Acción: `{ executable, repeats, authority, filter, metadata }`. Código: `crates/iroha_data_model/src/trigger/`.
...
- **Primitivas criptográficas** (característica `sm`):
  - `Sm2PublicKey` / `Sm2Signature` reflejan el punto SEC1 canónico + codificación `r∥s` de ancho fijo para SM2. Los constructores imponen la pertenencia a la curva y la semántica de identificación distintiva (`DEFAULT_DISTID`), mientras que la verificación rechaza escalares mal formados o de alto rango. Código: `crates/iroha_crypto/src/sm.rs` y `crates/iroha_data_model/src/crypto/mod.rs`.
  - `Sm3Hash` expone el resumen GM/T 0004 como un nuevo tipo `[u8; 32]` serializable Norito que se utiliza siempre que aparecen hashes en manifiestos o telemetría. Código: `crates/iroha_data_model/src/crypto/hash.rs`.- `Sm4Key` representa claves SM4 de 128 bits y se comparte entre llamadas al sistema de host y dispositivos de modelo de datos. Código: `crates/iroha_data_model/src/crypto/symmetric.rs`.
  Estos tipos se ubican junto a las primitivas Ed25519/BLS/ML-DSA existentes y están disponibles para los consumidores de modelos de datos (Torii, SDK, herramientas de génesis) una vez que la función `sm` está habilitada.
- Los almacenes de relaciones derivados del espacio de datos (`space_directory_manifests`, `uaid_dataspaces`, `axt_policies`, `axt_replay_ledger`, registro de anulación de emergencia de retransmisión de carril) y los permisos de destino del espacio de datos (`CanPublishSpaceDirectoryManifest{dataspace: ...}` en almacenes de permisos de cuenta/rol) se eliminan `State::set_nexus(...)` cuando los espacios de datos desaparecen del `dataspace_catalog` activo, lo que evita referencias a espacios de datos obsoletos después de las actualizaciones del catálogo de tiempo de ejecución. Las cachés de retransmisión/DA con ámbito de carril (`lane_relays`, `da_commitments`, `da_confidential_compute`, `da_pin_intents`) también se eliminan cuando un carril se retira o se reasigna a un espacio de datos diferente para que el estado local del carril no pueda filtrarse a través de las migraciones del espacio de datos. Los ISI del directorio espacial (`PublishSpaceDirectoryManifest`, `RevokeSpaceDirectoryManifest`, `ExpireSpaceDirectoryManifest`) también validan `dataspace` con el catálogo activo y rechazan ID desconocidos con `InvalidParameter`.
...
Tipos: `Register<T: Registered>` e `Unregister<T: Identifiable>`, con tipos de suma `RegisterBox`/`UnregisterBox` que cubren objetivos concretos.- Registrar pares: se inserta en el conjunto de pares mundiales.
...
- Registrar dominio: compila desde `NewDomain` con `owned_by = authority`. No permitido: dominio `genesis`.
  ...
  - Errores: `Repetition(Register, DomainId)`, `InvariantViolation("Not allowed to register genesis domain")`. Código: `core/.../isi/world.rs`.

- Registrar cuenta: compilaciones desde `NewAccount`, no permitidas en el dominio `genesis`; La cuenta `genesis` no se puede registrar.
...
- Registrar AssetDefinition: compila desde el constructor; establece `owned_by = authority`.
  - Condiciones previas: inexistencia de definición; el dominio existe; Se requiere `name`, no debe estar vacío después del recorte y no debe contener `#`/`@`.
...
- Registrar NFT: compilaciones del constructor; establece `owned_by = authority`.
  ...
  - Errores: `Repetition(Register, NftId)`. Código: `core/.../isi/nft.rs`.- Registrar función: se compila a partir de `NewRole { inner, grant_to }` (primer propietario registrado mediante asignación de funciones de cuenta), almacena `inner: Role`.
...
- Registrar disparador: almacena el disparador en el disparador apropiado establecido por tipo de filtro.
  ...
  - Errores: `Repetition(Register, TriggerId)`, `InvalidParameterError::SmartContract(..)` en fallas de conversión/validación. Código: `core/.../isi/triggers/mod.rs`.- Anular el registro de pares/dominio/cuenta/definición de activos/NFT/rol/disparador: elimina el objetivo; emite eventos de eliminación. Eliminaciones en cascada adicionales:- Anular registro de dominio: elimina la entidad de dominio más su estado de política de selección/respaldo; elimina las definiciones de activos en el dominio (y el estado lateral `zk_assets` confidencial codificado por esas definiciones), los activos de esas definiciones (y los metadatos por activo), los NFT en el dominio y las proyecciones de alias de cuenta arraigadas en el dominio eliminado. También elimina las entradas de permisos con alcance de cuenta/rol que hacen referencia al dominio eliminado o a los recursos eliminados con él (permisos de dominio, permisos de definición de activos/activos para definiciones eliminadas y permisos NFT para ID de NFT eliminados). La eliminación del dominio no elimina ni reescribe el `AccountId` global, su secuencia de transmisión/estado UAID, propiedad extranjera de activos o NFT, autoridad de activación u otras referencias de auditoría/configuración externas que apunten a la cuenta sobreviviente. Barandillas: se rechaza cuando cualquier definición de activo en el dominio todavía está referenciada por acuerdo de repo, libro mayor de liquidación, recompensa/reclamo de vía pública, asignación/transferencia fuera de línea, valores predeterminados de repo de liquidación (`settlement.repo.eligible_collateral`, `settlement.repo.collateral_substitution_matrix`), referencias de definición de activos de votación/ciudadanía/elegibilidad para el parlamento/recompensa viral configuradas por gobernanza, economía de oráculo configurada referencias de definición de activos de recompensa/barra/bono de disputa, o referencias de definición de activos de tarifa/participación Nexus (`nexus.fees.fee_asset_id`, `nexus.staking.stake_asset_id`). Eventos: `DomainEvent::Deleted`, más eventos de eliminación por elemento para el recurso de dominio eliminadoces. Errores: `FindError::Domain` si falta; `InvariantViolation` sobre conflictos de referencias de definición de activos retenidos. Código: `core/.../isi/world.rs`.- Anular registro de cuenta: elimina los permisos, las funciones, el contador de secuencia de transmisión, la asignación de etiquetas de cuenta y los enlaces UAID de la cuenta; elimina los activos propiedad de la cuenta (y los metadatos por activo); elimina los NFT propiedad de la cuenta; elimina los activadores cuya autoridad es esa cuenta; elimina las entradas de permisos con ámbito de cuenta/rol que hacen referencia a la cuenta eliminada, los permisos de destino NFT con ámbito de cuenta/rol para ID de NFT de propiedad eliminada y los permisos de destino de activador con ámbito de cuenta/rol para activadores eliminados. Barandillas: se rechaza si la cuenta todavía posee un dominio, definición de activo, enlace de proveedor SoraFS, registro de ciudadanía activo, estado de recompensa/representación de carril público (incluidas las claves de reclamo de recompensa donde la cuenta aparece como reclamante o propietario de activo de recompensa), estado activo de Oracle (incluidas las entradas del proveedor del historial de alimentación de Oracle, registros de proveedores vinculados a Twitter o referencias de cuentas de recompensa/barra configuradas en Oracle-economics), activo Referencias de cuenta de cuota/participación Nexus (`nexus.fees.fee_sink_account_id`, `nexus.staking.stake_escrow_account_id`, `nexus.staking.slash_sink_account_id`; analizadas como identificadores canónicos de cuenta sin dominio y rechazadas y cerradas por error en literales no válidos), estado de acuerdo de repositorio activo, estado de libro mayor de liquidación activo, asignación/transferencia fuera de línea activa o revocación de veredicto fuera de línea estado, referencias de configuración de cuenta de depósito en garantía activa fuera de línea para definiciones de activos activos (`settlement.offline.escrow_accounts`), estado de gobernanza activa (propuesta/aprobación de etapa)als/locks/slashes/council/parliament rosters, instantáneas del parlamento de propuestas, registros de proponentes de actualización en tiempo de ejecución, referencias de cuentas de depósito en garantía/slash-receiver/viral-pool configuradas para la gobernanza, referencias de remitentes de telemetría de gobernanza SoraFS a través de `gov.sorafs_telemetry.submitters` / `gov.sorafs_telemetry.per_provider_submitters`, o configuradas para la gobernanza Referencias de proveedor-propietario SoraFS a través de `gov.sorafs_provider_owners`), referencias de cuenta de lista permitida de publicación de contenido configurado (`content.publish_allow_accounts`), estado de remitente de depósito de garantía social activo, estado de creador de paquete de contenido activo, estado de propietario de intención de PIN de DA activo, estado de anulación del validador de emergencia de relé de carril activo o registro de pin SoraFS activo registros de emisor/encuadernador (manifiestos de pin, alias de manifiesto, órdenes de replicación). Eventos: `AccountEvent::Deleted`, más `NftEvent::Deleted` por NFT eliminado. Errores: `FindError::Account` si falta; `InvariantViolation` sobre huérfanos de propiedad. Código: `core/.../isi/domain.rs`.- Anular el registro de AssetDefinition: elimina todos los activos de esa definición y sus metadatos por activo, y elimina el estado lateral `zk_assets` confidencial codificado por esa definición; También elimina la entrada `settlement.offline.escrow_accounts` coincidente y las entradas de permisos con alcance de cuenta/rol que hacen referencia a la definición de activo eliminada o sus instancias de activo. Barandillas: se rechaza cuando todavía se hace referencia a la definición mediante acuerdo de repo, libro mayor de liquidación, recompensa/reclamo de vía pública, estado de asignación/transferencia fuera de línea, valores predeterminados de repo de liquidación (`settlement.repo.eligible_collateral`, `settlement.repo.collateral_substitution_matrix`), referencias de definición de activos de votación/ciudadanía/elegibilidad-parlamentaria/recompensa viral configuradas por gobernanza, economía de oráculo configurada referencias de definición de activos de recompensa/barra/bono de disputa, o referencias de definición de activos de tarifa/participación Nexus (`nexus.fees.fee_asset_id`, `nexus.staking.stake_asset_id`). Eventos: `AssetDefinitionEvent::Deleted` e `AssetEvent::Deleted` por activo. Errores: `FindError::AssetDefinition`, `InvariantViolation` en conflictos de referencia. Código: `core/.../isi/domain.rs`.
  - Anular el registro de NFT: elimina NFT y elimina las entradas de permisos con alcance de cuenta/rol que hacen referencia al NFT eliminado. Eventos: `NftEvent::Deleted`. Errores: `FindError::Nft`. Código: `core/.../isi/nft.rs`.
  - Anular registro de rol: primero revoca el rol de todas las cuentas; luego elimina el rol. Eventos: `RoleEvent::Deleted`. Errores: `FindError::Role`. Código: `core/.../isi/world.rs`.- Anular registro del activador: elimina el activador si está presente y elimina las entradas de permisos con alcance de cuenta/rol que hacen referencia al activador eliminado; La cancelación del registro duplicada produce `Repetition(Unregister, TriggerId)`. Eventos: `TriggerEvent::Deleted`. Código: `core/.../isi/triggers/mod.rs`.

### Menta / Quemar
Tipos: `Mint<O, D: Identifiable>` e `Burn<O, D: Identifiable>`, en cajas como `MintBox`/`BurnBox`.

- Activo (Numérico) mint/burn: ajusta saldos y definiciones `total_quantity`.
  ...
  - Errores: `TypeError::AssetNumericSpec(Mismatch)`, `MathError::Overflow`/`NotEnoughQuantity`. Código: `core/.../isi/asset.rs`.- Repeticiones del disparador mint/burn: cambia el recuento `action.repeats` para un disparador.
...
- Propiedad de AssetDefinition: cambia `AssetDefinition.owned_by` a la cuenta de destino.
  - Condiciones previas: ambas cuentas existen; la definición existe; la fuente debe ser propietaria actualmente; la autoridad debe ser la cuenta de origen, el propietario del dominio de origen o el propietario del dominio de definición de activos.
  - Eventos: `AssetDefinitionEvent::OwnerChanged`.
  - Errores: `FindError::Account/AssetDefinition`. Código: `core/.../isi/account.rs`.- Propiedad de NFT: cambia `Nft.owned_by` a la cuenta de destino.
  - Condiciones previas: ambas cuentas existen; NFT existe; la fuente debe ser propietaria actualmente; la autoridad debe ser la cuenta de origen, el propietario del dominio de origen, el propietario del dominio NFT o tener `CanTransferNft` para ese NFT.
...
- Errores: `FindError::<Target>` si el objetivo no existe; `FindError::MetadataKey` sobre la clave faltante para su extracción. Código: `crates/iroha_data_model/src/isi/transparent.rs` e implicaciones de ejecutor por objetivo.

### Permisos y Roles: Conceder/Revocar
Tipos: `Grant<O, D>` e `Revoke<O, D>`, con enumeraciones en cuadros para `Permission`/`Role` hacia/desde `Account`, y `Permission` hacia/desde `Role`.- Otorgar permiso a la cuenta: agrega `Permission` a menos que ya sea inherente. Eventos: `AccountEvent::PermissionAdded`. Errores: `Repetition(Grant, Permission)` si está duplicado. Código: `core/.../isi/account.rs`.
...
- `TransactionBuilder`/`SignedTransaction`: construye, firma y empaqueta un ejecutable con metadatos, `chain_id`, `authority`, `creation_time_ms`, opcional `ttl_ms` e `nonce`. Código: `crates/iroha_data_model/src/transaction/`.
...
- Los selectores de activos públicos utilizan identificadores de definición de activos Base58 canónicos más campos de propiedad dividida (`account`, `scope` opcional). Los literales `AssetId` codificados sin formato siguen siendo ayudantes internos y no forman parte de la superficie del selector Torii/CLI.
- Los filtros y clasificaciones de listas/consultas de definición de activos también aceptan `alias_binding.status`, `alias_binding.lease_expiry_ms`, `alias_binding.grace_until_ms` e `alias_binding.bound_at_ms`.
