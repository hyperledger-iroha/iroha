...
- `AccountId`: las direcciones canónicas se producen a través de `AccountAddress` (IH58 / `sora…` comprimido/hexadecimal) y Torii normaliza las entradas a través de `AccountAddress::parse_encoded`. IH58 es el formato de cuenta preferido; el formulario `sora…` es el segundo mejor para UX exclusivo de Sora. La conocida cadena `alias` (rejected legacy form) se conserva únicamente como alias de enrutamiento. Cuenta: `{ id, metadata }`. Código: `crates/iroha_data_model/src/account.rs`.
