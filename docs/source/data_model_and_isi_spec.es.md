...
- `AssetId`: literal codificado canónico `<base58-asset-id>#<katakana-i105-account-id>` (las formas textuales heredadas no se admiten en la primera versión).- `NftId` — `nft$domain`. NFT: `{ id, content: Metadata, owned_by }`. Código: `crates/iroha_data_model/src/nft.rs`.
...
- Activo (Numérico) mint/burn: ajusta saldos y definiciones `total_quantity`.
  - Condiciones previas: el valor `Numeric` debe satisfacer `AssetDefinition.spec()`; perfecto permitido por `mintable`:
    ...
    - `Limited(n)`: permite `n` operaciones adicionales de mint. Cada menta exitosa disminuye el contador; cuando llega a cero, la definición cambia a `Not` y emite los mismos eventos `MintabilityChanged` que los anteriores.
...
- Activo (Numérico): restar del origen `AssetId`, agregar al destino `AssetId` (misma definición, cuenta diferente). Eliminar activo de origen puesto a cero.
  ...
  - Eventos: `AssetEvent::Removed` (fuente), `AssetEvent::Added` (destino).
...
- Actualización de parámetros:
  - `SetParameter(SumeragiParameter::BlockTimeMs(2500).into())` actualiza y emite `ConfigurationEvent::Changed`.CLI/Torii asset-definition id + ejemplos de alias:
- Registrarse con ayuda canónica + nombre explícito + alias largo:
  -`iroha ledger asset definition register --id 66owaQmAQMuHxPzxUN3bqZ6FJfDa --name pkr --alias pkr#ubl.sbp`
- Registrarse con ayuda canónica + nombre explícito + alias corto:
...
- Las identificaciones de activos en los límites de acuñación/quema/transferencia siguen siendo canónicas `<base58-asset-id>#<katakana-i105-account-id>`; utilice `iroha tools encode asset-id` con `--definition <base58-asset-definition-id>` o `--alias ...` más `--account`.
