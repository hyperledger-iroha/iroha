...
>
> **Status (Jan 25, 2026):** `AccountId`/`AssetId` JSON now includes an explicit canonical encoded literals (e.g., `i105`, `<canonical-base58-asset-definition-id>`) so decoding does not rely on a domain-selector resolver. Alias inputs such as `label@domain` are rejected by runtime parsers and should be used only in explicit negative tests.
