...
1. Запустите `iroha --output-format text ops sumeragi params`, чтобы подтвердить, что бинарь
...
- **Mode selection mismatch** — Если `iroha --output-format text ops sumeragi params` показывает
