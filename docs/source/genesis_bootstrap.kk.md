...
- **Сұраныс ағыны:** жад бос және `genesis.file` орнатылмаған кезде (және
  `genesis.bootstrap_enabled=true`), `genesis.expected_hash` қол қойылған генезис блогын дәл
  әрі міндетті түрде бекітуі керек. Файл да, хэш те болмаса, ешбір сұрау жіберілмей тұрып
  іске қосу конфигурациясының қатесі шығады. Содан кейін түйін пайдалы жүктемені алады және `validate_genesis_block` арқылы қолтаңбаларды тексереді,
...
- **Оператор қадамдары:** жарамды генезисі бар кем дегенде бір сенімді теңдестің қол жетімді екеніне көз жеткізіңіз, конфигурациялаңыз
  ...
  қашықтан bootstrap-ты қоспас бұрын дәл блокты `expected_hash` ішінде бекітіңіз. Жергілікті қол қойылған
  `genesis.file` өзі айқын артефакт және қосымша хэшті қажет етпейді. Тұрақты пайдалы жүктемелер болуы мүмкін
