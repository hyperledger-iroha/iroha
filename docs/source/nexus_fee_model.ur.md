...
- IVM لین دین کو فیس ایکسپوژر محدود کرنے کے لئے `gas_limit` (`u64`, > 0) میٹاڈیٹا شامل کرنا لازم ہے۔
  `/v2/contracts/call` اینڈپوائنٹ واضح طور پر `gas_limit` مانگتا ہے، اور غلط اقدار مسترد ہوتی ہیں۔
...
- block execution lane/dataspace کے مطابق receipts کو aggregate کر کے `/v2/sumeragi/status` میں
