...
#### 通過 Torii 監控轉換錢包和審計員輪詢 `GET /v2/confidential/assets/{definition_id}/transitions` 進行檢查
...
- CLI 添加：`confidential create-keys`、`confidential send`、`confidential export-view-key`、用於解密備忘錄的審計工具，以及用於離線生成/檢查 Norito 備忘錄信封的 `iroha app zk envelope` 幫助程序。 Torii 通過 `POST /v2/confidential/derive-keyset` 公開相同的派生流程，返回十六進制和 base64 形式，以便錢包可以以編程方式獲取密鑰層次結構。
