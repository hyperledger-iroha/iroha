...
סטטוס: 🈯 מתוכנן — מסמך זה מספק מפרט ומסגרות השקה שמופיעות בפריט הרודמפ **NX-12 — מנוע תאימות Lane ומדיניות רשימת היתרים**.
הוא מסביר את מודל הנתונים, זרימות הממשל, הטלמטריה ואסטרטגיית ההשקה שיוטמעו בתוך `crates/iroha_core/src/compliance`
וייחשפו דרך משטחי Torii/SDK, כך שכל lane וכל dataspace יוכלו להיות קשורים למדיניות שיפוטית דטרמיניסטית.
...
2. **ביצוע (iroha_core)**
   - במהלך בניית בלוק, `iroha_core::compliance::Engine` משחזר את אותה החלטה באמצעות policy id שנרשם בכותרת הבלוק.
     כך נמנעת סטייה גם אם הקאשים של Torii מיושנים.
   - הוראות שמשנות lane manifests או מדיניות חייבות לספק policy references שעליהן הן נשענות,
     כדי שהערכים הללו ייתפסו on-chain.
3. **Hooks אסינכרוניים**
