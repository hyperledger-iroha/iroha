...
- Block-sync updates: peers gossip `BlockSyncUpdate` payloads to a capped fanout (`block_gossip_size`) after commits and for vote/backfill so stragglers and observers can sync without relying on full broadcast. Updates are only broadcast when they carry verifiable roster metadata (commit QC); otherwise the sender falls back to `BlockCreated` payload broadcasts to avoid dropped updates. Receivers apply backpressure when the block queue is full instead of dropping updates so commit certificate evidence is not lost. Attached availability evidence, prepare/new-view certificates, and commit certificates are only applied when their height and block hash match the payload; mismatches are ignored.
