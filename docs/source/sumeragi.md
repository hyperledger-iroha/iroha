...
- Block-sync updates: peers gossip `BlockSyncUpdate` payloads to a capped fanout (`block_gossip_size`) after commits and for vote/backfill so stragglers and observers can sync without relying on full broadcast. Attached Availability/Precommit QCs are only applied when their height and block hash match the payload; mismatches are ignored.
