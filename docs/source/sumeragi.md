...
- Leader: when a block is expected (transactions queued or a locked parent needs a child block) and the deadline elapses, the leader sends `BlockCreated` to the commit topology (validators); when there is no queued work and empty-child recovery is disabled (`sumeragi.empty_child_fallback_enabled = false`) or not needed, the pacemaker stays idle (no heartbeat proposals).
...
- Drops: relay ingress uses non-blocking enqueues for block/control messages, so full queues drop and increment `sumeragi_dropped_block_messages_total`/`sumeragi_dropped_control_messages_total`; `BlockSyncUpdate` bypasses the non-blocking path and blocks until space is available to preserve commit/QC evidence. Blocking backpressure remains for explicit `incoming_*` calls (internal producers). When relay drops spike, consensus rebroadcasts (votes/block sync/RBC payloads) and idle view-change triggers pause for one rebroadcast cooldown to avoid message storms. Unresolved RBC payloads also pause idle view changes and proposal assembly until the backlog clears.
...
- Proposal assembly scans a bounded snapshot of the queue (derived from the queued length at assembly start) so sustained admission traffic cannot stall block creation; newly arrived transactions remain queued for the next proposal.
...
- RBC chunk broadcasts pick a deterministic, per-session target set sized to at least the commit quorum (local + capped peers) so enough peers can reconstruct the payload without flooding the network.
