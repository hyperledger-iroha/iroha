...
- Iroha3 requires NPoS and disallows staged cutovers; omit `--next-consensus-mode`/`--mode-activation-height` and keep `--consensus-mode npos`.
...
- Docker Compose: point `--config-dir` at the same localnet output and run `kagami swarm --peers 4 --config-dir ./npos-local --image hyperledger/iroha:dev --out-file docker-compose.npos.yml --consensus-mode permissioned --next-consensus-mode npos --mode-activation-height 5 --no-banner --print` to emit a Compose file that re-signs genesis in-container with `GENESIS_CONSENSUS_MODE`/`GENESIS_NEXT_CONSENSUS_MODE`/`GENESIS_MODE_ACTIVATION_HEIGHT` overrides.
