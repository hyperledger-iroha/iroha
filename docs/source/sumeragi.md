...
- Leader: when a block is expected (transactions queued or a locked parent needs a child block) and the deadline elapses, the leader sends `BlockCreated` to the commit topology (validators); when there is no queued work and no empty-child recovery to perform, the pacemaker stays idle (no heartbeat proposals).
