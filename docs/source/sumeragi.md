...
- Proposal assembly defers when relay backpressure is active, when unresolved RBC sessions exist for the active tip, or when pending blocks extend the tip; once the view exceeds the commit quorum timeout the liveness override allows proposals and view changes even if backlog persists.
