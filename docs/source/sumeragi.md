...
- Commit-conflict recovery: if a valid commit QC arrives for a different hash at an already-committed height, the node validates the QC, selects a deterministic winner (higher view, then lower hash), and takes the winner’s chain. If the local block loses, it prunes Kura back to `height - 1`, stores the winning block, and pauses consensus until restart (TODO: in-process state replay). If the local block wins, it rebroadcasts the local commit QC to help peers converge.
