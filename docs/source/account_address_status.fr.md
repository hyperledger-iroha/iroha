...
- Fixture: `fixtures/account/address_vectors.json` (IH58 + compressed + multisig cas positifs/negatifs).
...
- `--verify` - compare le fichier actuel au contenu fraichement genere (echoue vite en cas de drift; incompatible avec `--stdout`).

### 3. Matrice des artefacts

| Surface | Application | Notes |
|---------|-------------|-------|
| Rust data-model | `crates/iroha_data_model/tests/account_address_vectors.rs` | Analyse le JSON, reconstruit les payloads canoniques et verifie les conversions IH58/compressed/canonical + les erreurs structurees. |
| Torii | `crates/iroha_torii/tests/account_address_vectors.rs` | Valide les codecs cote serveur pour que Torii refuse deterministiquement les payloads IH58/compressed malformes. |
| JavaScript SDK | `javascript/iroha_js/test/address.test.js` | Replique les fixtures V1 (IH58/compressed/fullwidth) et confirme les codes d'erreur style Norito pour chaque cas negatif. |
| Swift SDK | `IrohaSwift/Tests/IrohaSwiftTests/AccountAddressTests.swift` | Exerce le decodage IH58/compressed, les payloads multisig et la remontee des erreurs sur les plateformes Apple. |
