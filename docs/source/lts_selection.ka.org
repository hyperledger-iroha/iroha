...
4. The release must be supported by all SDKs.
   ...
   two other major SDKs, namely Kotlin, and TypeScript must support
   the LTS candidate. If the SDK cannot be made to support the
   candidate, the candidate release is disqualified.
...
come as separate packages that are marked as not installable alongside the
stable version.
