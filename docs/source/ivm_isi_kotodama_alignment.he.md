...
2) **ממשק syscall לעומת שמות/כיסוי ב-ISI ובמודל הנתונים**  
   - NFTs: שמות ה-syscall עברו לפורמט הקנוני `SYSCALL_NFT_*`, כך שהם תואמים למודל הנתונים. שמות ההיסטוריים (`*_ASSET_INSTANCE`) נשארים זמנית כאליאסים מסומנים כ-deprecated.
...
- ✔ שמות ה-syscall של NFT הותאמו והאליאסים המסומנים כ-`#[deprecated]` מתועדים בקוד ובמסמכים.
