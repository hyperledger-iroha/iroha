...
| Registry-first pinning | M2 | `sorafs pin propose` and `sorafs pin approve` wrap manifest submissions; CLI defaults to `--require-registry`. Torii submissions always carry the exact canonical `ManifestV1` in `manifest_payload`; no caller-supplied summary can override it. | Governance Ops | Registry CLI audit log, telemetry for failed proposals. |
