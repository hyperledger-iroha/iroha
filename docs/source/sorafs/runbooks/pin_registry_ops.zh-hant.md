...
lang: zh-hant
direction: ltr
source: docs/source/sorafs/runbooks/pin_registry_ops.md
status: complete
generator: scripts/sync_docs_i18n.py
source_hash: 6579c0615c79c2ec5233e127630693122e9e350dbc63e9661d610f9b88dd5efc
source_last_modified: "2026-01-22T14:35:37.735423+00:00"
...
the `/v2/sorafs/pin/*` endpoints. Import the curated dashboard
(`docs/source/grafana_sorafs_pin_registry.json`) for a ready-to-use Grafana layout that maps
...
1. **Identify cause**
   ...
   - If backlog grows with stable misses, inspect admission (`/v2/sorafs/pin/*`) to confirm manifests awaiting council approval.
...
1. **Prepare configuration**
   - Update `torii.sorafs_alias_cache` in `iroha_config` (user → actual) with the agreed TTLs and
     ...
     defaults match the policy in `docs/source/sorafs_alias_policy.md`.
...
2. **Dry-run in staging**
   ...
   - Exercise the `/v2/sorafs/pin/{digest}` and `/v2/sorafs/aliases` endpoints with synthetic proofs
...
3. **Enable in production**
   ...
   - Import `docs/source/grafana_sorafs_pin_registry.json` into Grafana (or update existing dashboards)
