...
title: SoraFS Orchestrator Rollout & Tuning
summary: Practical defaults, tuning guidance, and audit checkpoints for taking the multi-source orchestrator to GA.
---

# Orchestrator Rollout & Tuning Guide

This guide builds on the [configuration reference](orchestrator.md) and the
[multi-source rollout runbook](../runbooks/multi_source_rollout.md). It explains
...
6. **Capture manifest evidence.** `scoreboard.json.metadata.gateway_manifest_provided`
   and `summary.json.gateway_manifest_provided` stay `false` unless the run
   supplied a signed manifest envelope via `--gateway-manifest-envelope`
   (paired with `--gateway-manifest-id`/`--gateway-manifest-cid`). The adoption
   gate now rejects fixtures that report `gateway_provider_count>0` while this
   flag remains unset, and it also fails when the flag is `true` but no gateway
   providers participated. Whenever you exercise gateway-only or mixed captures
   persist the actual envelope alongside the artefacts so auditors can replay
   the manifest signature chain referenced in `scoreboard.json`. Direct-only
   captures must omit the envelope entirely so the CLI and SDKs continue to
   mark `gateway_manifest_provided=false`, giving governance a deterministic
   signal about which transports serviced each fetch.
...
`sorafs_cli fetch` (see `crates/sorafs_orchestrator/src/bin/sorafs_cli.rs`) and the
...
- {{#include ../snippets/multi_source_flag_notes.txt}}
