...
پرانے `namespace-name` envelopes اب بھی compatibility کے لیے verify ہوتے ہیں۔ Downstream automation
...
handle کے ساتھ `profile_aliases` دکھاتے ہیں تاکہ SDKs legacy `namespace-name` فارم سے migrate کرتے وقت compatibility
برقرار رکھ سکیں۔
...
policy کے مطابق رہتی ہے اور ایسے legacy providers کے غلط استعمال سے بچاؤ ہوتا ہے جو ranged chunk requests پوری نہیں کر سکتے۔
