...
head-chain inclusion is authenticated; the latest-only snapshot-id behavior is
either made genuinely historical or removed; identity/rotation/outage/restart/
fork/retry negatives pass; and the reviewed four-validator deployment proves
one committed outcome and recovery. A live current-head view, local snapshot
...
receipts only after finality. Native drawdowns compare the exact committed
remaining amount observed by the worker before moving custody, preventing
concurrent peers from applying the same stale partition twice. Refund follow-up
is durable before a completed drawdown is removed, and bounded completion
capacity never evicts an exactly-once tombstone. Standard `irohad` forwards
only an explicitly injected runtime registry and never manufactures a signer
from the node key.
