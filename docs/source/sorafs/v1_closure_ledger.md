...
| 3 | SF-2 | PDP provider protocol and proof validation. | Canonical PDP schemas plus the durable provider protocol and authenticated Torii challenge/next-work/proof/status/terminal-export family; malformed proof and inactive-admission failures converge on a durable terminal record and authoritative repair handoff. | Authenticated deployed provider transport, multi-provider proof replay, repair convergence, and observability. | local-complete; evidence-pending |
| 4 | SF-2c | PoR/PoTR challenge, receipt, and repair convergence. | PoR/PoTR schemas, coordinators, reference validation, final dual-signed PoTR receipt persistence, exactly-once latency-repair identity, and PoR latency/VRF/seed metrics. | Live provider/auditor runs, governed provider-key rotation, dual-signed durable receipts, exactly-once repair, and archive evidence. | local-complete; evidence-pending |
...
| `gateway_compliance` | 10 | `docs/source/sorafs_gateway_compliance_plan.md`; catalog/control foundations and compliance evidence gate. | Feed, policy, catalog, SSRF/payload-safety, binding, runner, and aggregate negatives. | Always-on controller, durable catalog/acknowledgement/override history, two-gateway promotion, rollback, and probes. | open |
...
| `por` | 6 | `docs/source/sorafs_por_plan.md`; PoR scheduler/randomness/governance foundations and emitted latency/VRF/seed metrics. | VRF/seed replay, proof/reference, metric-label bounds, checker/runner, and aggregate tests. | Run live randomness with provider/auditor scheduling and replay, archive the reporting output, and obtain governed approval. | local-complete; evidence-pending |
| `potr` | 6 | `docs/source/sorafs_potr_plan.md`; `crates/sorafs_node/src/potr.rs` and proof-stream/governance schemas. | Final receipt/reference, gateway Ed25519 and governed provider ML-DSA-65 signer negatives, persistence/restart, repair idempotency, PQ roster, reputation binding, checker/runner, and aggregate tests. | Exercise dual-signed receipts, key rotation, crash recovery, and exactly-once latency repair in the reviewed deployment. | local-complete; evidence-pending |
...
| Source-marker audit | `scripts/tests/check_sorafs_rollout_gate_contract_test.py`. | Zero active SoraFS deferred-work markers in production source at the release commit. | local-complete |
