...
source_hash: 4c894c24dc21810a4c258e02cda4f5d4fd38753b78d05fbe02df559f9a6ab942
source_last_modified: "2026-01-05T09:28:12.077374+00:00"
...
| Expectation flags | M1 | Pipelines call `sorafs_manifest_builder` with explicit expectations to pin outputs: | Docs CI | Updated scripts referencing expectation flags (see command block below). |
| Registry-first pinning | M2 | `sorafs pin propose` and `sorafs pin approve` wrap manifest submissions. Torii requires canonical padded-base64 `ManifestV1` in `manifest_payload`, derives digest, chunker, content length, pin policy, and fee inputs only from that decoded canonical manifest, and rejects retired duplicate summaries. Optional alias and nonzero predecessor fields remain. | Governance Ops | Registry CLI audit log, telemetry for failed proposals. |
