...
- `PendingActivation(epoch)` — ожидание governance-определенного epoch активации; кортеж хранит самый ранний
  epoch активации, полученный из `epoch_length_blocks`.
...
- **Activation/exit operations:** регистрации попадают в `PendingActivation` и авто-продвигаются в первом
  блоке, чей epoch достигает границы (`epoch_length_blocks`). Операторы также могут вызвать
...
- **Torii/CLI quickstart:**
  - `iroha nexus lane-report --summary` показывает записи каталога lanes, readiness manifests и режимы
  ...
  - Torii snapshots для dashboards и SDKs:
    - `GET /v1/nexus/public_lanes/{lane}/validators` – metadata, status
      ...
      `address_format=ih58|compressed` управляет отображением literals (IH58 предпочтительно; compressed (`snx1`) — второй по предпочтению, только для Sora).
    - `GET /v1/nexus/public_lanes/{lane}/stake` – stake shares (`validator`,
      `staker`, bonded amount) плюс pending unbond timers. `?validator=account@domain`
      фильтрует ответ для dashboards, сфокусированных на одном валидаторе; `address_format`
