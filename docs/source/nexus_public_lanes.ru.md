...
- **Torii/CLI quickstart:**
  ...
  - Torii snapshots для dashboards и SDKs:
    - `GET /v1/nexus/public_lanes/{lane}/validators` – metadata, status
      ...
      `address_format=ih58|compressed` управляет отображением literals (IH58 предпочтительно; compressed (`sora`) — второй по предпочтению, только для Sora).
    - `GET /v1/nexus/public_lanes/{lane}/stake` – stake shares (`validator`,
      `staker`, bonded amount) плюс pending unbond timers. `?validator=ih58...`
