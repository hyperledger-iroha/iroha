...
публикует активные shares и pending withdrawals через `GET /v2/nexus/public_lanes/{lane}/stake`, чтобы
...
- **Torii/CLI quickstart:**
  ...
  - Lifecycle ISIs используют стандартный транзакционный путь (Torii
    `/v2/transactions` или CLI instruction pipeline). Примеры Norito JSON payloads:
...
- ✅ Torii публикует `/v2/nexus/public_lanes/{lane}/{validators,stake,rewards/pending}` в Norito JSON,
