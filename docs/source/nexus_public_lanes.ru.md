...
- **Torii/CLI quickstart:**
  ...
  - Torii snapshots для dashboards и SDKs:
    - `GET /v1/nexus/public_lanes/{lane}/validators` – metadata, status
      ...
      `canonical Katakana i105 literal rendering` управляет отображением literals (I105 предпочтительно; I105 — второй по предпочтению, только для Sora).
    - `GET /v1/nexus/public_lanes/{lane}/stake` – stake shares (`validator`,
      ...
      фильтрует ответ для dashboards, сфокусированных на одном валидаторе; `canonical Katakana i105 rendering`
