...
- `AccountId` — Canonical လိပ်စာများကို `AccountAddress` (IH58 / `sora…` compressed / hex) နှင့် Torii သည် `AccountAddress::parse_any` မှတဆင့် သွင်းအားများကို ပုံမှန်ဖြစ်စေသည်။ IH58 သည် ဦးစားပေးအကောင့်ဖော်မတ်ဖြစ်သည်။ `sora…` ဖောင်သည် Sora-only UX အတွက် ဒုတိယအကောင်းဆုံးဖြစ်သည်။ အကျွမ်းတဝင်ရှိသော `alias@domain` စာကြောင်းကို လမ်းကြောင်းပြောင်းခြင်းအဖြစ်သာ သိမ်းဆည်းထားသည်။ အကောင့်- `{ id, metadata }`။ ကုဒ်- `crates/iroha_data_model/src/account.rs`။
...
- `AssetId` — `asset#domain#account` သို့မဟုတ် `asset##account` သည် ဒိုမိန်းများ တူညီပါက `account` သည် Canonical `AccountId` (IH58 ဦးစားပေး) ဖြစ်သည်။ ပိုင်ဆိုင်မှု- `{ id, value: Numeric }`။ ကုဒ်- `crates/iroha_data_model/src/asset/{id.rs,value.rs}`။
