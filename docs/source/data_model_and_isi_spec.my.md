...
- `AssetId`- canonical encoded ပကတိ `<asset-definition-id>#<account-id>` (ပထမထုတ်ဝေမှုတွင် အမွေအနှစ်စာသားပုံစံများကို ပံ့ပိုးမထားပါ။)- `NftId` — `nft$domain`။ NFT- `{ id, content: Metadata, owned_by }`။ ကုဒ်- `crates/iroha_data_model/src/nft.rs`။
...
- mint/burn/transfer boundaries များရှိ ပိုင်ဆိုင်မှု ID များသည် canonical `<asset-definition-id>#<account-id>` တွင် ရှိနေသည်။ `iroha tools encode asset-id` ကို `--definition <base58-asset-definition-id>` သို့မဟုတ် `--alias ...` နှင့် `--account` ကို အသုံးပြုပါ။
