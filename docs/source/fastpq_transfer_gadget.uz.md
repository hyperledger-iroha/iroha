...
| Kotodama | `transfer_batch((from,to,asset,amount), …)` yordamchisini `transfer_v1_batch_begin`, ketma-ket `transfer_asset` va `transfer_v1_batch_end` ga tushiradi. Har bir kortej argumenti `(AccountId, AccountId, AssetDefinitionId, quantity)` shakliga mos kelishi kerak; yagona transferlar mavjud quruvchini saqlab qoladi. |
