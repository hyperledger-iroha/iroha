...
`publicInputs`, `proofBytes`, `proofContext`, `statementHash`,
...
rejected before request hashing and before submission packaging. The JavaScript
...
the user-linked prover. Swift, Kotlin, and Java Android mobile proof-request and
submission constructors now apply the same source-domain, target-domain,
backend, and `stark-fri-v1` proof-family checks when the app passes a derived
binding object instead of a raw hash. The EVM-family and TRON result wrappers
reject empty, all-zero, or non-384-byte Groth16 proof bytes across JavaScript,
Python, Swift, Kotlin, and Java Android, so placeholder or non-canonical ABI proof output
cannot be packaged by SDK callers before Torii's deployment-bound preflight.
...
source-domain word is not SORA.
...
accepted-proof path. The TRON wrapper smoke also covers its public-input
preflight guards, cleartext source-domain overflow rejection, wrong-statement
rejection, and rejection of a valid proof replayed against a different TRON
deployment binding. The smoke script pins
...
ABI. Placeholder structural fixtures may still use the typed EVM-family
...
next committee becomes eligible.
...
runtime bytecode; callers may pass the hash directly or provide runtime
bytecode for the helper to derive it. When runtime bytecode is supplied, TOML
also carries `sccp_evm_source_bridge_runtime_bytecode_hex` so the all-lanes
preflight can replay the Keccak-256 hash instead of trusting the standalone
code-hash comment. `adapter_verifier_vk_hash` must still match the canonical
...
block hash, and an exact positive integer receipt block number. Boolean or
truthy placeholders for the block number are rejected before receipt metadata
can be marked ready.

```bash
python3 scripts/sccp_eth_source_bridge_evidence.py \
  --bridge-address <eth-source-bridge-address> \
  --source-bridge-emitter-code-hash <runtime-bytecode-hash> \
...
contract address. Production TOML rendering requires an explicit
`--expected-source-bridge-code-hash` pin plus expected source material and
source-adapter deployment record hashes, so the observed live bytecode cannot
self-authorize governance evidence.
...
before decoding, and rejects duplicate JSON object keys instead of accepting
...
number. The all-lanes preflight requires those comments for ETH/BSC source
material, decodes the bytecode, recomputes Keccak-256, and checks that both the
live metadata and replayed hash match the governed
...
hash. Route canary evidence must bind that route allowlist hash and destination
...
fails closed. Admission also enforces the all-lanes launch policy
against configured material: once a submitted non-SORA source lane is locally
ready, every advertised remote SCCP domain must have exactly matching
production-ready source material, source-adapter deployment, destination
rollout, and route allowlist records before the bridge proof is accepted. A
partial governed rollout cannot open one inbound lane while another advertised
lane is still missing production evidence. The default
...
preimages. Use the corresponding `--*-runtime-bytecode-file` inputs for
formatted hex artifacts.
...
proof version, proof source domain, and a live
`usedMessageProofs(messageId) = true` assertion; if a
`--route-canary-evidence-hash` is supplied, it must match that v2
transaction-derived transcript. JSON summaries stay in diagnostic mode with
`toml_ready = false`
...
`MessageProofAccepted` log at `--route-canary-log-index`, the submitted
`submitSccpMessageProof(bytes,bytes32[6],bytes32)` calldata, the 384-byte
proof tuple header, and `usedMessageProofs(messageId)`. The canonical EVM
route-canary transcript now commits the exact submitted calldata SHA-256,
decoded payload hash, ETH/BSC target-domain word, finality height, finality
block hash, proof ABI version `1`, SORA proof source-domain word, and
...
helper, and all-lanes preflight also reject reuse between the distinct
transaction hash, calldata, message id, payload, statement, commitment, and
finality block hash roles, so a canary cannot be replayed across
proof tuple versions, EVM-family lanes, drifted Groth16 public inputs, or
synthetic transcript-role aliases. A
...
the canonical EVM production profile and recomputes the EVM route canary hash
from the transaction calldata, public inputs, proof header, event metadata, and
consumed-message state before ETH/BSC destination rollout records can pass
launch readiness.
...
matching code-BoC root evidence are present:
...
`specVersion`, `transactionVersion`, and finalized runtime bytes:
...
validated route allowlist hash, then fetches the raw `TriggerSmartContract`
transaction, verifies that the visible and raw-data owner addresses match,
requires the canonical transaction signature to recover to that same owner, and
verifies the
...
`sccp:tron-route-canary-evidence:v2` hash commits the exact
...
accepted event tuple and governed verifier/backend/network pins. If
...
`usedMessageProofs` state is not carried. The offline direct TRON renderer
...
owner.
Live TOML also carries the route-canary signature hash, recovered address, and
`sccp_tron_route_canary_signature_recovers_to_owner` audit comments; all-lanes
preflight requires those comments before accepting TRON route-canary evidence.
...
carried route-canary selector, Groth16 tuple length/version/source-domain
header, public-input message id, target domain, commitment root, statement
hash, owner/signature fields, event source domain, destination binding, backend,
proof family, network id, and recomputed canary hash instead of trusting the
`call_data_matches_event` flag by itself.
...
`raw_data_hex` to hash to the `txID`, requires exactly one canonical 65-byte
low-S TRON recoverable secp256k1 signature that recovers to the source bridge
owner, requires saved replay JSON to parse the source-proof transaction
...
rebuilds java-tron's transaction Merkle root against `txTrieRoot`; when
...
material and rejects any staged source record whose decoded bytecode does not
hash back to the live source bridge runtime code hash and governed
`source_bridge_emitter_code_hash`.
...
the Solana source lane, and TON audit fields are only valid on the TON source
lane; lane-foreign audit fields are rejected before governance staging to match
...
recomputed. It also preserves the destination
...
recomputed from that same finalized runtime evidence. Missing or drifting
...
phase marker and completion sentinel expected by the release report and bundle
verifier. The Java Android phase runs the main-method
SCCP classes through
...
and the non-dry-run `SCCP production corridor completed.` sentinel. This keeps
`--phase-result <phase>=passed` from turning an arbitrary hashed file or dry-run
plan into release evidence.
...
canary evidence source used for the readiness decision. Reviewers can therefore
compare the release note against the governed hashes and the post-deploy canary
transcript without digging through the raw TOML first. It also records the user-prover SDK submission
...
corridor phases, plus the EVM/TRON contract-smoke phase where contract verifier
calldata is produced, so release reviewers can see whether a user-facing proof
path is validated or still blocked.
...
it is copied into the bundle. Before publishing or reviewing attached files, run
`python3 scripts/sccp_verify_release_bundle.py <bundle-dir>` to recompute every
attachment hash and confirm that the readiness report, all-lanes summary, and
manifest all agree on `production_ready`. The manifest and readiness-report
JSON roots reject missing or unknown top-level fields, and manifest/report
artifact entries reject unknown fields and require artifact `bytes`/`sha256`
claims to keep canonical JSON
...
artifact set must
exactly match the required reports, copied evidence inputs, and copied corridor
logs referenced by known passed phases in the readiness report, so a hash-bound
but unreviewed appendix or unknown phase log cannot be smuggled into an
...
`evm-groth16-bn254-v1`, and `tron-groth16-bn254-v1`), so public release notes
cannot claim a portal or mobile proof path is validated unless its required SDK
and contract-smoke phases actually passed. Those portal/mobile submission rows
...
copied evidence artifacts, non-canonical readiness-report artifact paths,
...
cryptographic evidence row domain/chain drift from embedded lane rows,
...
table, and missing, malformed, unbound, lane-mismatched, or extra-field
per-lane cryptographic evidence rows.
That final check recomputes the public cryptographic table from the embedded
all-lanes lane evidence and requires exact JSON domain/chain types plus
canonical bytes32 hash text, so a release note cannot publish a different or
malformed governed verifier, destination, route, or canary hash than the one
that actually passed preflight. For passed phases, the verifier independently re-reads each copied
corridor log and requires the same phase marker and completion sentinel as the
report generator.
...
remains diagnostic and must not be staged as production-ready governance.
...
all-zero placeholders before producing a submit-ready wallet payload. Python
...
empty event data, and the governed source bridge emitter. Placeholder
...
- `GET /v1/sccp/artifacts/message/{message_id}` returns the typed SCCP transparent proof artifact for the same canonical message id. Each artifact now bundles:
  ...
  - client helpers now exist for that route directly:
    ...
    destination binding also triggers the configured all-lanes launch gate, so
    one configured EVM/TRON rollout cannot expose production packaging while
    another advertised SCCP remote lane is still missing governed source,
    deployment, destination, or route evidence.
