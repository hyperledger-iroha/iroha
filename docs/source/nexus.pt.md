...
- Introduzir uma nova versao de ABI; mudancas miram ABI v1 com extensoes explicitas de syscalls e pointer-ABI conforme a politica de IVM.
...
- A validacao no recebimento descarta lotes cujas lanes/dataspaces nao concordam com o catalogo local ou cuja tag de plano nao corresponde a visibilidade do dataspace derivado.
...
#### Fluxo de trabalho CLI (`iroha space-directory manifest ...`)
...
3. **Expirar** manifestos que atingiram o fim de vida programado ou **revogar**
   UAIDs sob demanda. Ambos os comandos aceitam `--uaid uaid:<hex>` e o id numerico
   do dataspace:
...
- `GET /v1/space-directory/uaids/{uaid}` - resolver os bindings atuais de dataspace
  ...
  `address_format=compressed` para saida do Sora Name Service.
...
- Atualizacoes necessarias:
  - Adicionar a `ivm::syscalls::abi_syscall_list()` (manter ordem), gatear por politica.
  - Mapear numeros desconhecidos para `VMError::UnknownSyscall` nos hosts.
  - Atualizar testes: syscall list golden, ABI hash, golden de ids de pointer type e testes de politica.
...
- **Endpoint admin:** `POST /v1/nexus/lifecycle` (Torii) aceita um corpo Norito/JSON com `additions` (objetos completos `LaneMetadata`) e `retire` (ids de lane) para adicionar ou remover lanes sem reinicio. Requisicoes sao gateadas em `nexus.enabled=true` e reutilizam a mesma view de configuracao/estado Nexus que a fila.
...
3) Adicionar syscalls e tipos pointer para operacoes AMX (atomicas multi-DS); estender testes e docs; manter ABI v1.
...
- Testes IVM para novas syscalls e goldens ABI.
...
- **Grafana board:** `dashboards/grafana/nexus_lanes.json` agora exporta o dashboard "Nexus Lane Finality & Oracles" solicitado por NX-18. Paineis cobrem `histogram_quantile()` em `iroha_slot_duration_ms`, `iroha_da_quorum_ratio`, alertas de disponibilidade DA (`sumeragi_da_gate_block_total{reason="missing_availability_qc"}`), gauges de preco/staleness/TWAP/haircut de oraculos e o painel vivo `iroha_settlement_buffer_xor` para que operadores provem o slot de 1 s, DA e SLOs de tesouraria sem queries bespoke.
...
- ABI v1 apenas; sem toggles de runtime para politicas de ABI. Adicoes de syscalls e pointer-types seguem o processo de evolucao documentado com testes golden.
