...
- `torii_address_invalid_total{endpoint,reason}` increments whenever HTTP routes reject an account identifier (invalid IH58 (preferred)/sora (second-best) payloads, domain mismatches, etc.). Keep the `<0.1%` SLO by watching the dedicated Grafana board in `dashboards/grafana/address_ingest.json`.
...
- `torii_address_invalid_total{surface,reason}` — rejects grouped by Torii surface (e.g., `routing.source`, `iso_bridge.source`) and the stable address error code so SDK drift or malformed IH58 (preferred)/sora (second-best) literals are visible.
