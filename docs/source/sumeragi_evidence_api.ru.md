...
- GET `/v2/sumeragi/evidence/count`
  ...
  - Примечания:
    ...
    - Переживает рестарты и питает `/v2/sumeragi/evidence`; записи дедуплицируются по hash evidence.
...
- POST `/v2/sumeragi/evidence`
  ...
  - CLI helper: `iroha ops sumeragi evidence submit --evidence-hex <hex>` или `--evidence-hex-file <path>`.
