...
- `GET /v2/zk/proofs` һәм `GET /v2/zk/proofs/count` күпер-аңлы фильтрҙар ҡабул итә:
...
- `GET /v2/zk/proof/{backend}/{hash}` ҡайтарыу күпер метамағлүмәттәр (диапазон, асыҡ хеш, файҙалы йөк резюме) менән бер рәттән иҫбатлау id/статус/ВК бәйләүҙәр.
- Тулы Norito дәлил яҙмаһы (шул иҫәптән файҙалы йөк байттары) `GET /v2/proofs/{proof_id}` аша төйөндән тыш тикшерелеүселәр өсөн мөмкин.
