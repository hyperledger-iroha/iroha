...
- Профилдәр: `iroha3-dev` (сылбыр `iroha3-dev.local`, коллекционерҙар k=1 r=1, VRF орлоҡтан алынған сылбырлы id ҡасан NPoS һайланған), `iroha3-testus` (сылбыр `iroha3-testus`, коллекционерҙар k=3 р=3, талап итә `--vrf-seed-hex` NPoS һайланғанда), `iroha3-nexus` (сылбыр `iroha3-nexus`, коллекционерҙар k=5 р=3, NPoS һайланғанда `--vrf-seed-hex` талап итә).
...
- Өлгө өйөмдәр: алдан генерацияланған өйөмдәр `defaults/kagami/iroha3-{dev,testus,nexus}/` аҫтында йәшәй (genis.json, config.toml, docker.compose.yml, раҫлау.txt, README). Kagami менән регенерация.
