...
- `SYSCALL_MINT_ASSET(account: ptr AccountId, asset: ptr AssetDefinitionId, amount: ptr Amount)` → ISI `Mint<Numeric, Asset>`
- `SYSCALL_BURN_ASSET(account: ptr AccountId, asset: ptr AssetDefinitionId, amount: ptr Amount)` → ISI `Burn<Numeric, Asset>`
- `SYSCALL_TRANSFER_ASSET(from: ptr AccountId, to: ptr AccountId, asset: ptr AssetDefinitionId, amount: ptr Amount)` → ISI `Transfer<Asset>`
...
## TODO トラッキング（2026-01-30 更新）

| 項目 | 担当 | 現状 | 次ステップ / 期日 | 完了基準 |
|------|------|------|--------------------|-----------|
| `iroha_core::smartcontracts::ivm::host::CoreHost` 実装 | Core Host チーム (`@core-host`) | ✅ `CoreHost` 実装が本線に統合され、`host_roundtrip` と `ivm_host_shadow_execute` でネイティブ `Execute` 経路とのイベント/状態差分を検証済み。`apply_queued` 経由での ISI 適用とイベント整合性をテストで保証。 | — | `crates/ivm/tests/host_roundtrip.rs` と `crates/iroha_core/tests/ivm_host_shadow_execute.rs` が通過し、イベント/状態が一致。 |
...
| syscall ガステーブル／enforcement | IVM Core (`@ivm-core`) | ✅ `iroha_core/src/smartcontracts/limits.rs` に定数を移植し、`ivm/src/limits.rs` で取り込み。`limits_enforcement` を追加してスケジュール整合性を検証済み。 | — | `docs/source/gas_model.md` の表とスケジュールハッシュを更新済み。 |
