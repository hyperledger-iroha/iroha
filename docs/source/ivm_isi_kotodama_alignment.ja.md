...
| NFT 系 syscall の正式名称整備 | Streaming & NFT チーム (`@nft-streaming`) | ✅ `SYSCALL_NFT_MINT_ASSET` / `SYSCALL_NFT_SET_METADATA` などへリネーム済み。旧名称は `#[deprecated]` エイリアスとして残存。 | — | `cargo test -p ivm abi_syscall_list_golden` 更新済み。`docs/source/ivm_syscalls*.md` と本ドキュメントに新名称を反映。 |
