...
|---------|-------------|-------|
| Rust data-model | `crates/iroha_data_model/tests/account_address_vectors.rs` | Analiza el JSON, reconstruye payloads canonicos y comprueba conversiones IH58 (preferred)/snx1 (second-best)/canonical + errores estructurados. |
| Torii | `crates/iroha_torii/tests/account_address_vectors.rs` | Valida codecs del lado servidor para que Torii rechace payloads IH58 (preferred)/snx1 (second-best) malformados de forma determinista. |
| JavaScript SDK | `javascript/iroha_js/test/address.test.js` | Replica fixtures V1 (IH58 (preferred)/snx1 (second-best)/fullwidth) y afirma codigos de error estilo Norito para cada caso negativo. |
| Swift SDK | `IrohaSwift/Tests/IrohaSwiftTests/AccountAddressTests.swift` | Ejercita decodificacion IH58 (preferred)/snx1 (second-best), payloads multisig y exposicion de errores en plataformas Apple. |
