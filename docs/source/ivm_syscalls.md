...
| 0x22 | MINT_ASSET                 | `&AccountId`, `&AssetDefinitionId`, `&Amount`                           | `u64=0`     | `G_mint`                     | Mints `amount` of asset to account |
| 0x23 | BURN_ASSET                 | `&AccountId`, `&AssetDefinitionId`, `&Amount`                           | `u64=0`     | `G_burn`                     | Burns `amount` from account |
...
- `Amount` uses pointer type `0x0010` and the exact/rounded helper range
  `0x010040..=0x01004D`. Exact division rejects non-finite decimal results or
  canonical scale above 28; rounded division requires an explicit scale and
  rounding mode. Arithmetic and gas depend only on deterministic limb work.
- `JSON_BUILD` (`0x01004E`) consumes one compiler-emitted construction schema
  ...
  sorts object keys canonically, and encodes one `Json` payload. Amounts and
  large `u128` values remain exact decimal strings, bytes use lowercase `0x`
  hex, and no floating-point conversion is used.
- Every `JSON_GET_*` syscall returns a compiler-owned `Option<T>` sum handle;
  missing/wrongly typed fields are `none`. The Amount getter is
  `JSON_GET_AMOUNT`; the retired `JSON_GET_NUMERIC` name is not part of V1.
- `CORE_QUERY_GET` (`0x010001`) accepts stable entity tag `1..=5` for account,
  ...
  and returns `List<View, 64>` plus `Option<i64>` continuation handles. The
