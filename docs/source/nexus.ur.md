...
- UAID portfolio snapshots `GET /v2/accounts/{uaid}/portfolio` کے ذریعے؛ aggregator `iroha_core::nexus::portfolio` میں۔
...
UAID portfolio کو ایک ہی اثاثے تک محدود کرنے کے لیے `asset_id=<asset#definition::owner>` استعمال کریں۔
...
- `POST /v2/nexus/lifecycle` additions/retire بغیر restart؛ validation اور safety کیلئے shared state-view lock استعمال ہوتا ہے۔
