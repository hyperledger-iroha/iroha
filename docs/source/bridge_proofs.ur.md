...
- `GET /v2/zk/proofs` اور `GET /v2/zk/proofs/count` bridge-aware filters قبول کرتے ہیں:
...
- `GET /v2/zk/proof/{backend}/{hash}` bridge metadata (range, manifest hash, payload summary) کے ساتھ proof id/status/VK bindings واپس کرتا ہے۔
- مکمل Norito proof record (payload bytes سمیت) `GET /v2/proofs/{proof_id}` کے ذریعے آف نوڈ verifiers کیلئے دستیاب رہتا ہے۔
