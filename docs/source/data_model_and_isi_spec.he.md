...
- `AccountId` — הכתובת הקנונית נוצרת ע"י `AccountAddress` (קודק I105 / תצוגת סורא הדחוסה `i105` / hex). I105 הוא הפורמט המועדף, ו־`i105` הוא אפשרות שנייה ייעודית ל‑Sora. מחרוזות `alias` (rejected legacy form) נשמרות ככינויי ניתוב בלבד. Torii מנרמל קלטים דרך `AccountAddress::parse_encoded` כך שהמטא־דאטה נשמר בפורמט הקנוני. חשבון: `{ id, metadata }`. קוד: `crates/iroha_data_model/src/account.rs`.
