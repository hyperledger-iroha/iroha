...
- `AssetId`: מילולית מקודדת קנונית `<base58-asset-id>#<katakana-i105-account-id>` (טפסים טקסטואליים מדור קודם אינם נתמכים במהדורה הראשונה).- `NftId` — `nft$domain`. NFT: `{ id, content: Metadata, owned_by }`. קוד: `crates/iroha_data_model/src/nft.rs`.
...
- מזהי נכסים בגבולות הטבעה/שריפה/העברה נשארים קנוניים `<base58-asset-id>#<katakana-i105-account-id>`; השתמש ב-`iroha tools encode asset-id` עם `--definition <base58-asset-definition-id>` או `--alias ...` בתוספת `--account`.
