...
- `AssetId` — `asset#domain#account` או `asset##account` כאשר הדומיינים תואמים. נכס: `{ id, value: Numeric }`. קוד: `crates/iroha_data_model/src/asset/{id.rs,value.rs}`.
