...
## 实施脚手架- 数据模型现在带有用于平衡元数据的 `SetAssetKeyValue` / `RemoveAssetKeyValue` 支架
  编辑（`transparent.rs`）。
- 执行者访问者公开占位符，一旦主机接线登陆，这些占位符将控制权限
  （`default/mod.rs`）。
- 重新生成密钥原型类型 (`account::rekey`) 为滚动迁移提供着陆区。
- 世界状态包括由 `AccountAlias` 键控的 `account_rekey_records`，因此我们可以暂存别名 →
