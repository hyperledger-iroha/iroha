...
- `/v1/sumeragi/qc` (ברירת מחדל Norito): צילום Highest/Locked QC (`highest_qc`/`locked_qc`); כולל `subject_block_hash` עבור Highest QC אם ידוע. ניתן לבקש JSON דרך הכותרת `Accept`.
...
- `/v1/sumeragi/new-view` (JSON): צילום NEW_VIEW `{ ts_ms, items: [{ height, view, count }] }` עם `locked_qc { height, view }` (חלון זיכרון מוגבל; רשומות ישנות נזרקות).
...
| פרופיל     | `/status` | `/metrics` | מסלולי מפתחים (`/v1/sumeragi/*`, ‏SSE) | שימוש מיועד |
|------------|-----------|------------|----------------------------------------|-------------|
...
- לחץ תור טרנזקציות: `sumeragi_tx_queue_depth` / `sumeragi_tx_queue_capacity` מודדים ספירת פריטים, `sumeragi_tx_queue_retained_bytes` / `sumeragi_tx_queue_max_retained_bytes` מודדים זיכרון מוחזק, ו-`sumeragi_tx_queue_saturated_by_count`, ‏`sumeragi_tx_queue_saturated_by_bytes`, ‏`sumeragi_tx_queue_saturated_by_age`, ‏`sumeragi_tx_queue_oldest_queued_age_ms` מפרידים בין גורמי הלחץ. כאשר אחד מהם פעיל, `sumeragi_tx_queue_saturated` הוא `1`.
...
בדיקת תקלות:
1. לבדוק `/v1/sumeragi/leader` / `/v1/sumeragi/status` לראות מי היעד.
