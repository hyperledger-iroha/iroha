...
- `/v1/sumeragi/new_view` (JSON): מספרי קבלות NEW_VIEW אחרונות לכל `(height, view)`.
...
- `/v1/sumeragi/new_view/json` (JSON): צילום NEW_VIEW `{ ts_ms, items: [{ height, view, count }] }` עם `locked_qc { height, view }`.
