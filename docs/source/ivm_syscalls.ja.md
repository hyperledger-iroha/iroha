...
## バージョン管理
- 認識されるシステムコール集合はバイトコードヘッダーの `abi_version` フィールドに依存します。アクティブな `abi_version` に存在しない番号は決定論的に `E_SCALL_UNKNOWN` でトラップします。
...
- `0x20..=0x5F`: Iroha コア ISI ブリッジ（サポートされるすべての `abi_version` で安定）。
- `0x60..=0x7F`: プロトコル機能でゲートされる拡張 ISI。
- `0x80..=0xFF`: 実験的／ベンダー領域（対応する `abi_version` と機能ビットが必須）。
