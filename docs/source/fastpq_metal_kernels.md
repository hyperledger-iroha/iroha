...
| `poseidon_hash_columns` | Hash flattened trace columns and emit leaf digests | 256 threads | — | Consumes the `PoseidonColumnSlice` descriptors, absorbs every padded rate block in place, and writes the full Poseidon state (width = 3) per column. Launch sizing matches the FFT scheduler to keep ≥4 096 logical threads in flight, and the kernel stays available for the legacy “leaf-only” path used by CUDA and diagnostic captures.【crates/fastpq_prover/metal/kernels/poseidon2.metal:314】【crates/fastpq_prover/src/metal.rs:2319】
