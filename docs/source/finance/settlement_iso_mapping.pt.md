...
##### `pacs.009` / `camt.054` (financiamento e declarações PvP)- **Agentes (`InstgAgt`, `InstdAgt`, agentes devedores/credores)** → **BIC** com opcional
...
#### Regras de validação (aplicadas antes da emissão)

| Identificador | Regra de validação | Notas |
|------------|-----------------|-------|
...
- O auxiliar IVM agora ingere envelopes XML ISO 20022 reais (head.001 + `DataPDU` + `Document`)
  ...
  os blocos permanecem ignorados intencionalmente. Os testes de regressão consomem as amostras e os novosacessório de envelope de cabeçalho para proteger os mapeamentos.【crates/ivm/src/iso20022.rs:265】【crates/ivm/src/iso20022.rs:3301】【crates/ivm/src/iso20022.rs:3703】
...
eles financiam uma liquidação de títulos.| Campo de financiamento PvP | Caminho ISO 20022 | Notas |
|------------------------------------------------|-----------------------------------------------------|-------|
...
### Confirmação de pagamento versus pagamento → `sese.025`

| Campo PvP | Caminho ISO 20022 | Notas |
|----------------------------------------------------------|---------------------------|-------|
| `settlement_id` | `TxId` | Identificador de ciclo de vida estável |
...
Exemplo (visualização CLI ISO com ligações, retenção e MIC de mercado):
...
| Tamanho do corte de cabelo | `Substitution/Haircut` | Numérico; mapeia o desconto aplicado durante a substituição |
| IDs dos instrumentos originais/substitutos | `Substitution/OriginalFinInstrmId`, `Substitution/SubstituteFinInstrmId` | ISIN/CUSIP opcional para cada trecho |
...
### Notas de uso* Todos os valores são serializados usando os auxiliares numéricos Norito (`NumericSpec`)
...
* `SupplementaryData` deve ser JSON canônico (UTF-8, chaves classificadas, JSON nativo
...
### Ajudantes do SDK Builder

- O JavaScript SDK agora expõe `buildPacs008Message`/
...
- Exemplos de liquidação LuxCSD/Clearstream ISO 20022 mostrando `SttlmTpAndAddtlParams/SctiesMvmntTp` (`DELI`/`RECE`) e `Pmt` (`APMT`/`FREE`).[1](https://www.luxcsd.com/resource/blob/3434074/6f8add4708407a4701055be4dd04846b/c23005-eis-examples-cbf-data.pdf)
- Especificações Clearstream DCP que abrangem qualificadores de liquidação (`SttlmTxCond`, `PrtlSttlmInd`).[2](https://www.clearstream.com/clearstream-en/res-library/market-coverage/instruction-specifications-swift-iso-20022-dcp-mode-ceu-spain-2357008)
- Orientação SWIFT PMPG recomendando `pacs.009` com `CtgyPurp/Cd = SECU` para financiamento PvP relacionado a títulos.[3](https://www.swift.com/swift-resource/251897/download)
- Relatórios de definição de mensagens ISO 20022 para restrições de comprimento de identificador (BIC, Max35Text).[4](https://www.iso20022.org/sites/default/files/2020-12/ISO20022_MDRPart2_ChangeOrVerifyAccountIdentification_2020_2021_v1_ForSEGReview.pdf)
- Orientações da ANNA DSB sobre formato ISIN e regras de soma de verificação.[5](https://www.anna-dsb.com/isin/)

### Dicas de uso- Sempre cole o snippet Norito relevante ou o comando CLI para que o LLM possa inspecionar
...
- Ciclo de vida da liquidação: `sese.024` (status de processamento + códigos de motivo), `sese.025` (final).  
...
1. Transferência de Crédito FI-to-FI `pacs.009` por moeda com `CtgyPurp/Cd = SECU`; status via `pacs.002`; recuperar/cancelar via `camt.056`/`camt.029`; se já liquidado, `pacs.004` retorna.  
2. `sese.023` Instrução(ões) DvP com `HldInd=true` para que a perna dos títulos aguarde a confirmação do dinheiro.  
...
5. Se a janela FX for violada → cancelar/recuperar dinheiro (`camt.056/029` ou `pacs.004`) e cancelar títulos (`sese.020` + `sese.027`, ou reversão `sese.026` se já confirmado por regra de mercado).
