...
3. Сұрау арқылы орындалу уақыты сұралған ырғақты/шаш қиюды орындайтынын дәлелдеңіз.
   ...
   Torii `/v2/configuration` суреті жеткіліксіз.
...
- `GET /v2/repo/agreements` белсенді келісімдерді қосымша беттеу, сүзу арқылы қайтарады
...
- `POST /v2/repo/agreements/query` құрылымдық сұрау конвертін қабылдайды (беттеу, сұрыптау,
...
3. Өзгерістерді флот бойынша жылжытыңыз: `[settlement.repo]` жаңартыңыз, әрқайсысын қайта іске қосыңыз
   түйінін таңдаңыз, содан кейін `GET /v2/configuration` суретін түсіріңіз (немесе
...
1. Конфигурацияны әр теңдесінен алыңыз (`GET /v2/configuration` немесе
...
**Бекітуден кейінгі шығарылымдар**1. Бекітілген `[settlement.repo]` конфигурациясын қолданып, әрбір түйінді қайта іске қосыңыз (немесе айналдырыңыз)
   оны автоматтандыру арқылы). Дереу `GET /v2/configuration` нөміріне қоңырау шалып, мұрағаттаңыз
...
Осы мақсатқа арналған `/v2/configuration` маршруты және барлық SDK беттік көмекшілері, мысалы
...
1. Әр құрдасқа `GET /v2/configuration` (немесе SDK көмекшісі) кейін бірден қоңырау шалыңыз.
...
- `config/peers/*.json` әр теңдес үшін `/v2/configuration` суретін түсіреді,
...
3. **Бухгалтерлік кітапты/конфигурация суреттерін сақтаңыз.** `repo query list` шығысын бұрын/кейін экспорттаңыз
   ...
   өту, әр әріптеске `/v2/configuration` суретін түсіру (§2.9) және оларды сақтау
...
1. **Суретті баланстар.** Қуат беретін `FindAssets` сұрауын пайдаланыңыз.
   ...
   `i105...` үшін XOR баланстарын экспорттауға арналған `iroha_python` көмекшісі,
   `i105...` және шолуға қатысатын әрбір жұмыс үстелі тіркелгісі. Дүкен
   астында JSON
