...
- העוזר IVM מכניס כעת מעטפות XML אמיתיות של ISO 20022 (head.001 + `DataPDU` + `Document`)
  ...
  בלוקים נשארים דילוג בכוונה. מבחני רגרסיה צורכים את הדגימות ואת החדשיםמתקן מעטפת כותרת לשמירה על המיפויים.【crates/ivm/src/iso20022.rs:265】【crates/ivm/src/iso20022.rs:3301】【ארגזים/ivm/src/iso20022.rs:372.rs:

#### שיקולי רגולציה ומבנה שוק

- **הסדר T+1**: שוקי המניות בארה"ב/קנדה עברו ל-T+1 ב-2024; התאם את Norito
...
דוגמה (תצוגה מקדימה של CLI ISO עם קישורים, החזקה ו-MIC של שוק):
...
| גודל תספורת | `Substitution/Haircut` | מספרי; ממפה את התספורת שהוחלה במהלך ההחלפה |
| מזהי מכשירים מקוריים/חלופיים | `Substitution/OriginalFinInstrmId`, `Substitution/SubstituteFinInstrmId` | ISIN/CUSIP אופציונלי לכל רגל |
...
- שני העוזרים דורשים `creationDateTime` מפורש (ISO-8601 עם אזור זמן)
  ...
  של מתן ברירת המחדל ל-SDK לזמן שעון קיר.
...
- דוגמאות ליישוב LuxCSD / Clearstream ISO 20022 המציגות את `SttlmTpAndAddtlParams/SctiesMvmntTp` (`DELI`/`RECE`) ו-`Pmt` (`APMT`/`FREE`).[1](https://www.luxcsd.com/resource/blob/3434074/6f8add4708407a4701055be4dd04846b/c23005-eis-examples-cbf-data.pdf)
- מפרטי Clearstream DCP המכסים את מוקדי ההסדר (`SttlmTxCond`, `PrtlSttlmInd`).[2](https://www.clearstream.com/clearstream-en/res-library/market-coverage/instruction-specifications-swift-iso-20022-dcp-mode-ceu-spain-2357008)
- הנחיות SWIFT PMPG הממליצות על `pacs.009` עם `CtgyPurp/Cd = SECU` למימון PvP הקשור לניירות ערך.[3](https://www.swift.com/swift-resource/251897/download)
- דוחות הגדרות הודעות ISO 20022 עבור אילוצי אורך מזהה (BIC, Max35Text).[4](https://www.iso20022.org/sites/default/files/2020-12/ISO20022_MDRPart2_ChangeOrVerifyAccountIdentification_2020_2021_v1_ForSEGReview.pdf)
- הנחיות ANNA DSB לגבי פורמט ISIN וכללי סכום בדיקה.[5](https://www.anna-dsb.com/isin/)
...
7. דלתא מזומנים אופציונלית (עמלות/תספורת) → `pacs.009` העברת אשראי FI-to-FI עם `CtgyPurp/Cd = SECU`; סטטוס דרך `pacs.002`, מחזיר דרך `pacs.004`.
...
5. אם חלון המט"ח נפרץ ← בטל/חזר מזומן (`camt.056/029` או `pacs.004`) ובטל ניירות ערך (`sese.020` + `sese.027`, או `camt.056/029` או Norito אם כבר אושר לכל ביטול שוק).
...
- אביזרי החזרה מנרמלים כעת את `ChrgBr` (`DEBT`/`CRED`/`SHAR`/`SLEV`) וסיבות החזרה קנייניות חשופות כמו `CRED`/`SHAR`/`SLEV`) וסיבות החזרה קנייניות חשופות כ-Norito ו-Norito ייחוס מחדש קודי מפעיל מבלי לנתח מחדש את מעטפת ה-XML.
...
- השתמש ב-`SupplementaryData` כדי לתאם מזהים חיצוניים (לדוגמה, UETR עבור `pacs.009`) בעת הצורך.  
