...
##### `pacs.009` / `camt.054` (تمويل وبيانات PvP)- **الوكلاء (`InstgAgt`، `InstdAgt`، وكلاء المدين/الدائن)** → **BIC** مع اختياري
...
#### التزامات عبور المشاة وصيانة البيانات- الحفاظ على **ISIN ↔ Norito معرف الأصل** و **CUSIP ↔ ISIN** ممرات المشاة. تحديث ليلا من
...
- يقوم المساعد IVM الآن باستيعاب مظاريف ISO 20022 XML الحقيقية (head.001 + `DataPDU` + `Document`)
  ...
  تبقى الكتل تم تخطيها عمدا. اختبارات الانحدار تستهلك العينات والجديدةأداة تثبيت مغلف الرأس لحماية التعيينات. 【crates/ivm/src/iso20022.rs:265 】 【crates/ivm/src/iso20022.rs:3301 】 【crates/ivm/src/iso20022.rs:3703 】

#### الاعتبارات التنظيمية وهيكل السوق

- **تسوية T+1**: انتقلت أسواق الأسهم الأمريكية/الكندية إلى T+1 في عام 2024؛ ضبط Norito
...
### التسليم مقابل الدفع → `sese.023`| حقل DvP | مسار ISO 20022 | ملاحظات |
|--------------------------------------------------------|-------------------------------------------------------|-------|
...
يقومون بتمويل تسوية الأوراق المالية.| مجال تمويل حماية الأصناف النباتية | مسار ISO 20022 | ملاحظات |
|--------------------------------|-----------------------------------------------------|-------|
...
مثال (معاينة CLI ISO مع الروابط والتعليق وميكروفون السوق):
...
### استبدال ضمانات الريبو → `colr.007`| حقل الريبو / السياق | مسار ISO 20022 | ملاحظات |
|--------------------------------------------------|----------------------------------------||
...
| حجم قصة الشعر | `Substitution/Haircut` | رقمي؛ يرسم قصة الشعر المطبقة أثناء الاستبدال |
| معرفات الأداة الأصلية/البديلة | `Substitution/OriginalFinInstrmId`، `Substitution/SubstituteFinInstrmId` | رقم ISIN/CUSIP اختياري لكل ساق |
...
- يوضح `recipes/iso_bridge_builder.mjs` كيفية توصيل هؤلاء المساعدين
  ...
  تم إنشاء XML، وإرساله بشكل اختياري إلى Torii (`ISO_SUBMIT=1`)، وإعادة استخدامه
  نفس إيقاع الانتظار مثل وصفة جسر ISO.


### المراجع

- أمثلة تسوية LuxCSD / Clearstream ISO 20022 تظهر `SttlmTpAndAddtlParams/SctiesMvmntTp` (`DELI`/`RECE`) و`Pmt` (`APMT`/`FREE`).[1](https://www.luxcsd.com/resource/blob/3434074/6f8add4708407a4701055be4dd04846b/c23005-eis-examples-cbf-data.pdf)
- مواصفات Clearstream DCP التي تغطي مؤهلات التسوية (`SttlmTxCond`، `PrtlSttlmInd`).[2](https://www.clearstream.com/clearstream-en/res-library/market-coverage/instruction-specifications-swift-iso-20022-dcp-mode-ceu-spain-2357008)
- توصي إرشادات SWIFT PMPG بـ `pacs.009` مع `CtgyPurp/Cd = SECU` لتمويل PvP المتعلق بالأوراق المالية.[3](https://www.swift.com/swift-resource/251897/download)
- تقارير تعريف رسالة ISO 20022 لقيود طول المعرف (BIC، Max35Text).[4](https://www.iso20022.org/sites/default/files/2020-12/ISO20022_MDRPart2_ChangeOrVerifyAccountIdentification_2020_2021_v1_ForSEGReview.pdf)
- إرشادات ANNA DSB بشأن تنسيق ISIN وقواعد المجموع الاختباري.[5](https://www.anna-dsb.com/isin/)

### نصائح الاستخدام- الصق دائمًا مقتطف Norito أو أمر CLI ذي الصلة حتى تتمكن LLM من الفحص
...
**التوقيت:** نوافذ FX PvP (CLS/ثنائية) وقطع CSD؛ إبقاء أرجل الأوراق المالية معلقة في انتظار التأكيد النقدي.#### تصميم الرقصات للرسالة
