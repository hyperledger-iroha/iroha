...
1. **Snapshot balansları.** Gücləndirən `FindAssets` sorğusundan istifadə edin
   ...
   `<katakana-i105-account-id>` üçün XOR qalıqlarını ixrac etmək üçün `iroha_python` köməkçisi,
   `<katakana-i105-account-id>` və baxışda iştirak edən hər bir masa hesabı. Mağaza
   altındakı JSON
