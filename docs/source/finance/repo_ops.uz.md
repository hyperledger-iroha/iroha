...
1. **Snapshot balanslari.** quvvat beruvchi `FindAssets` so‘rovidan foydalaning
   ...
   `iroha_python` `<i105-account-id>` uchun XOR balanslarini eksport qilish uchun yordamchi,
   `<i105-account-id>` va ko'rib chiqishda ishtirok etgan har bir stol hisobi. Do'kon
   ostidagi JSON
