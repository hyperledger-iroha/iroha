...
3. የሩጫ ጊዜው አሁን የተጠየቀውን የፀጉር አቆራረጥ በመጠየቅ የሚያስፈጽም መሆኑን ያረጋግጡ
   ...
   የTorii's `/v2/configuration` ቅጽበታዊ ገጽ እይታ በቂ አይደለም።
...
- `GET /v2/repo/agreements` ገባሪ ስምምነቶችን ከአማራጭ ገፅ ጋር በማጣራት ይመልሳል
  (`filter={...}`)፣ መደርደር እና የአድራሻ-ቅርጸት መለኪያዎች። ይህንን ለፈጣን ኦዲት ይጠቀሙ ወይም
  ጥሬው የJSON ክፍያ ጭነቶች በቂ ሲሆኑ ዳሽቦርዶች።
- `POST /v2/repo/agreements/query` የተዋቀረውን የመጠይቅ ፖስታ ይቀበላል (ገጽታ፣ ደርድር፣
...
3. ለውጡን በመርከብ ላይ አዙረው፡ `[settlement.repo]`ን አዘምን፣ እያንዳንዱን እንደገና አስጀምር
   መስቀለኛ መንገድ፣ ከዚያ `GET /v2/configuration` ቅጽበተ ፎቶን ያንሱ (ወይም
...
1. አወቃቀሩን ከእያንዳንዱ አቻ (`GET /v2/configuration` ወይም
...
** የድህረ ማጽደቂያ ልቀቶች**1. የተፈቀደውን የ`[settlement.repo]` ውቅረት ይተግብሩ እና እያንዳንዱን መስቀለኛ መንገድ (ወይም ጥቅልል) እንደገና ያስጀምሩ
   በራስ-ሰርዎ በኩል ነው)። ወዲያውኑ `GET /v2/configuration` ይደውሉ እና በማህደር ያስቀምጡ
...
ለዚህ ዓላማ `/v2/configuration` መንገድ እና ሁሉም የኤስዲኬዎች ወለል ረዳቶች እንደ
...
- `config/peers/*.json` ለእያንዳንዱ እኩያ የ `/v2/configuration` ቅጽበታዊ ገጽ እይታዎችን ይይዛል ፣
...
3. **የደብዳቤ/የማዋቀር ቅጽበታዊ ገጽ እይታዎችን ያስቀምጡ።** `repo query list` ውፅዓት በፊት/በኋላ ይላኩ
   ...
   ያልፋል፣ `/v2/configuration` ቅጽበተ-ፎቶዎችን በአቻ ይቅረጹ (§2.9) እና ያከማቹ።
