...
備考: 高優先度キューはコンセンサス／制御トラフィックを、低優先度キューはゴシップと同期経路を扱います。

### 低優先度のレート制限（`[network]` 設定）

- `low_priority_rate_per_sec`（任意・msgs/sec）  
  - 低優先度トラフィック（ゴシップ／同期）にピア単位のトークンバケットを有効化。未設定なら無効。
- `low_priority_burst`（任意・msgs）  
  - バースト容量。未設定時は `low_priority_rate_per_sec` と同値。

有効化すると、低優先度の POST／ブロードキャストはピア単位でスロットリングされます。高優先度トラフィックは影響を受けません。
