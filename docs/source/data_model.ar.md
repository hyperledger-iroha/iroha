...
- بناة:
  - `NewAccount` عبر `Account::new_in_domain(id, domain)` يحقق تسجيلًا صريحًا مرتبطًا بالمجال وبالتالي يتطلب `ScopedAccountId`.
  - يقوم `NewAccount` عبر `Account::new(id)` بتسجيل موضوع الحساب العالمي فقط بدون مجال مرتبط.
