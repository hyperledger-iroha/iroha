...
- `local_amount` — exact dataspace gas-token debit encoded as a canonical
  non-negative decimal quantity.
- `xor_due` / `xor_after_haircut` / `xor_variance` — exact decimal XOR book
  entries and the per-receipt safety margin (`due - after haircut`).
...
- Exact decimal totals: `total_local_amount`, `total_xor_due`,
  `total_xor_after_haircut`, `total_xor_variance`.
