...
- POST `/v1/gov/finalize`
  ...
  - הערות: כאשר מספקים `authority`/`private_key`, Torii חותמת ומגישה; אחרת מוחזר שלד `FinalizeReferendum` לחתימה.
...
- `iroha gov vote-zk --election-id <id> --proof-b64 <b64> [--owner <account>@<domain> --salt-hex <32-byte-hex> --lock-amount <u128> --lock-duration-blocks <u64> --direction <Aye|Nay|Abstain>]`
  - בודק תוקף חשבונות, אוכף salt של 32 בייט שאינו אפס, ומשלב רמזים ב-`public_inputs_json`.
  - הסיכום מוסיף `fingerprint=<hex>` דטרמיניסטי מה-`CastZkBallot` והערכים (`owner`, ‏`amount`, ‏`duration_blocks`, ‏`direction`, ‏`salt_hex` בעת הצורך).
...
- `iroha gov vote-plain --referendum-id <id> --owner <account>@<domain> --amount <u128> --duration-blocks <u64> --direction <Aye|Nay|Abstain>`
  ...
  - הפלט מציג fingerprint ותוכן קריא בדומה ל-`vote-zk` (כולל `salt_hex` אם קיים).
