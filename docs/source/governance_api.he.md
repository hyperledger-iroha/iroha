...
- POST `/v2/contracts/deploy`
  ...
  - נקודות קשורות:
    - GET `/v2/contracts/code/{code_hash}` → מחזיר את המניפסט השמור.
...
- GET `/v2/gov/parliament/candidates`
...
- POST `/v2/gov/protected-namespaces`
...
- `iroha --output-format text app gov deploy audit --namespace apps [--contains calc --hash-prefix deadbeef]`
  - מאמת:
    ...
    - שהמניפסט ב-`/v2/contracts/code/{code_hash}` מדווח `code_hash` / `abi_hash` תואמים.
...
- POST `/v2/gov/ballots/zk-v1/ballot-proof` (פיצ'ר `zk-ballot`)
