...
- Introduire une nouvelle version ABI; les changements ciblent ABI v1 avec extensions explicites de syscalls et pointer-ABI selon la politique IVM.
...
- La validation cote reception drop les lots dont les lanes/dataspaces ne correspondent pas au catalogue local ou dont le tag de plan ne correspond pas a la visibilite dataspace derivee.
...
- `GET /v1/space-directory/uaids/{uaid}` - resoudre les liaisons de dataspace courantes
  ...
  `address_format=compressed` pour la sortie Sora Name Service.
...
- Determinisme: Toutes les nouvelles syscalls sont pures et deterministes compte tenu des entrees et des ensembles lecture/ecriture AMX declares. Aucun effet cache de temps ou d environnement.
...
- Mises a jour requises:
  - Ajouter a `ivm::syscalls::abi_syscall_list()` (garder l ordre), filtrer par politique.
  - Mapper les numeros inconnus vers `VMError::UnknownSyscall` dans les hosts.
  - Mettre a jour les tests: golden de la liste syscalls, hash ABI, golden des ids pointer type, et tests de politique.
...
- ivm: Ajouter syscalls et types pointer-ABI pour AMX (`amx_begin`, `amx_commit`, `amx_touch`) et preuves DA; mettre a jour tests/docs ABI selon la politique v1.
...
- **Endpoint admin:** `POST /v1/nexus/lifecycle` (Torii) accepte un corps Norito/JSON avec `additions` (objets `LaneMetadata` complets) et `retire` (ids de lane) pour ajouter ou retirer des lanes sans redemarrage. Les requetes sont gatees par `nexus.enabled=true` et reutilisent la meme vue configuration/etat Nexus que la queue.
...
- Tests IVM pour nouveaux syscalls et goldens ABI.
...
- **Dashboard Grafana:** `dashboards/grafana/nexus_lanes.json` exporte maintenant le dashboard "Nexus Lane Finality & Oracles" demande par NX-18. Les panels couvrent `histogram_quantile()` sur `iroha_slot_duration_ms`, `iroha_da_quorum_ratio`, avertissements de disponibilite DA (`sumeragi_da_gate_block_total{reason="missing_availability_qc"}`), gauges de prix/staleness/TWAP/haircut d oracles, et le panel live `iroha_settlement_buffer_xor` pour que les operateurs prouvent les SLO 1 s slot, DA et tresorerie sans requetes bespoke.
...
- ABI v1 seulement; pas de toggles runtime pour les politiques ABI. Les ajouts de syscalls et pointer-types suivent le processus d evolution documente avec tests golden.
