...
الحالة: تم تنفيذ نافذة اساس مشتقة من EMA، وارضية RTT، وحدود jitter/backoff قابلة للضبط. اصبح فاصل اقتراح pacemaker الان محصورا بين هدف block-time و propose timeout مع ارضية RTT، ومقيدا بـ `sumeragi.pacemaker_max_backoff_ms`. يتم تطبيق jitter بشكل حتمي لكل عقدة ولكل (height, view).
...
- السقف: `sumeragi.pacemaker_max_backoff_ms` (الافتراضي 60_000 ms). حد اعلى صارم للنافذة.
- بذرة فاصل الاقتراح: `max(block_time_ms, propose_timeout_ms * rtt_floor_multiplier)` ولا تتجاوز `pacemaker_max_backoff_ms`. هذا هو الفاصل في الحالة المستقرة حتى دون backoff.
