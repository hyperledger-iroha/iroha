...
1. `iroha sumeragi params --summary` を実行して、バイナリが
...
- **Mode selection mismatch** — `iroha sumeragi params --summary` が
...
- **Evidence ingestion stalls** — `sumeragi_evidence_records_total` が停滞し、chaos
  ...
  `iroha sumeragi evidence count` を実行し、`/v1/sumeragi/evidence/count` が CLI
