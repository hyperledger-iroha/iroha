...
- Writes use private, no-follow, bounded atomic replacement plus file and parent-directory sync. Startup rejects non-canonical, corrupt, truncated, trailing, unsafe-path, hard-linked, over-limit, unordered, duplicate, or forged snapshots; it never archives or silently reinitialises authoritative repair state.
...
- Cross-process locking and a persisted checkpoint digest reject stale writers. Sequence/nonce reservations, idempotency caches, dropped-event counts, PoR history, and governance votes are committed in the same snapshot; a pre-commit failure rolls the mutation back, while an ambiguous post-rename durability failure permanently fail-stops that store instance until a verified restart.
