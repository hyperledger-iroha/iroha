...
| Contracts & verifying keys | `POST /v1/contracts/{aliases,call,call/simulate,view,call/multisig/propose,call/multisig/approve}`; `POST /v1/zk/vk/{register,update}` | `SetContractAliasDto`, `ContractCallDto`, `ContractCallSimulateDto`, `ContractViewDto`, `MultisigContractCallProposeDto`, `MultisigContractCallApproveDto`, `ZkVkRegisterDto`, `ZkVkUpdateDto` | Structured Norito/JSON contract responses; strict `AppApiTransactionDraftDto` payloads for unsigned writes | Each DTO is decoded through `NoritoJson<T>` so callers may send Norito or Norito-backed JSON. Mutation schemas reject signing secrets. Torii either prepares a canonical unsigned payload for local signing or verifies an explicit detached signature; it never accepts a private key. Deployment likewise uses locally signed native transactions rather than a Torii deployment DTO. Public runtime calls build compact `Executable::ContractCall` transactions by reference, so Norito-RPC callers do not resend full contract bytecode/manifests on every invocation. |
| QueryEnvelope row/aggregate DSL reads | `POST /v1/{accounts,domains,assets/definitions,nfts,rwas,...}/query`; `POST /v1/accounts/{account_id}/{transactions,assets}/query`; `POST /v1/assets/{definition_id}/holders/query` | `QueryEnvelope` with `filter`, `select` or `aggregate`, `sort`, `pagination`, and `fetch_size` | Legacy JSON when `select`/`aggregate` are absent; otherwise dynamic rows with `items`, `total`, `indexed_height`, `indexed_block_hash`, and `query_source` | `select` switches any current QueryEnvelope route into row-mode and returns only requested flat fields. `aggregate` is mutually exclusive with `select`; `having` runs after grouping and before pagination. Aggregate support covers `accounts`, `account_transactions`, `account_assets`, `repo_agreements`, `domains`, `asset_definitions`, `nfts`, `rwas`, and `asset_holders`. DA projection export covers `accounts`, `account_assets`, `asset_holders`, `asset_definitions`, and `domains`; production holder reads serve published DA shards as `projection_da_cache` or hydrate from approved SoraFS providers as `projection_da_hydrated`. Incomplete production projections return `projection_archive_unavailable`; `live_debug` is only for explicit debug opt-in. |
...
- **Generic envelopes.** All other errors are serialized as Norito `ErrorEnvelope { code, message, details? }`[`crates/iroha_torii/src/lib.rs:8445`](/crates/iroha_torii/src/lib.rs#L8445). Clients should surface the machine-readable `code` first (`queue_full`, `already_enqueued`, etc.) and use `message` only as human diagnostics.
