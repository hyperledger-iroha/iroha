...
| QueryEnvelope row/aggregate DSL reads | `POST /v1/{accounts,domains,assets/definitions,nfts,rwas,...}/query`; `POST /v1/accounts/{account_id}/{transactions,assets}/query`; `POST /v1/assets/{definition_id}/holders/query` | `QueryEnvelope` with `filter`, `select` or `aggregate`, `sort`, `pagination`, and `fetch_size` | Legacy JSON when `select`/`aggregate` are absent; otherwise dynamic rows with `items`, `total`, `indexed_height`, `indexed_block_hash`, and `query_source` | `select` switches any current QueryEnvelope route into row-mode and returns only requested flat fields. `aggregate` is mutually exclusive with `select`; `having` runs after grouping and before pagination. Aggregate support covers `accounts`, `account_transactions`, `account_assets`, `repo_agreements`, `domains`, `asset_definitions`, `offline_allowances`, `offline_revocations`, `offline_transfers`, `nfts`, `rwas`, and `asset_holders`. DA projection export covers `accounts`, `account_assets`, `asset_holders`, `asset_definitions`, and `domains`; production holder reads serve published DA shards as `projection_da_cache` or hydrate from approved SoraFS providers as `projection_da_hydrated`. Incomplete production projections return `projection_archive_unavailable`; `live_debug` is only for explicit debug opt-in. |
