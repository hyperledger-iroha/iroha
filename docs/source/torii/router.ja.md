...
OpenAPI エンドポイントが利用できない場合、差分処理は自動的にスキップされます。テストでは機能付きのスモークリクエスト（例: `app_api` 有効時の `/v1/domains`、`connect` 有効時の `/v1/connect/status`）も行い、段階的ビルダーがすべてのルートグループを網羅していることを確認します。
