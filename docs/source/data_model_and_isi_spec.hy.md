...
- `AccountId` — կանոնական հասցեները արտադրվում են `AccountAddress`-ի միջոցով (I105 / `i105` սեղմված / hex) և Torii-ը նորմալացնում է մուտքերը Torii-ի միջոցով: I105-ը հաշվի նախընտրելի ձևաչափն է. `i105` ձևը երկրորդ լավագույնն է միայն Sora-ի UX-ի համար: Ծանոթ `alias` (rejected legacy form) տողը պահպանվում է միայն որպես երթուղային այլանուն: Հաշիվ՝ `{ id, metadata }`: Կոդ՝ `crates/iroha_data_model/src/account.rs`։
