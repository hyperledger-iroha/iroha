...
3. **Relay упаковка и DA-аттестации.**  
   ...
   статуса Sumeragi (`/v2/sumeragi/status`) для операторов и аудиторов. Broadcaster будет дальше
...
5. **Постоянное хранение и публикация.**  
   ...
   восстановить ту же редукцию. `/v2/sumeragi/status` публикует:
...
- **Torii поверхности:**  
  `/v2/sumeragi/status` включает `lane_commitments`, `lane_settlement_commitments` и снимки dataspace.
  `/v2/nexus/lane-config` (планируется) опубликует геометрию `LaneConfig`, чтобы клиенты могли
