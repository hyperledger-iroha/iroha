...
## TypeScript Connect Wrapper (WS qoşulma + nişanlar)
...
```
// Mühürleme SignResultOk (yalnız faydalı yük; Norito çərçivə göstərilmir):
// const signResult = new TextEncoder().encode(JSON.stringify({ SignResultOk: { imza: { alqoritm: 'ed25519', signature_hex: 'deadbeef' } } }));
// const { aead } = sealEnvelope gözləyir (kWallet, sidBytes, 'W2A', 1n, signResult);