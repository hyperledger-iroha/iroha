...
`/v2/domains`፣ `/v2/assets/definitions`፣ ኤንኤፍቲዎች፣ ሚዛኖች፣ የንብረት መያዣዎች እና
የመለያ ግብይት ታሪክ።
...
<24 ሰአት ይቀራል። ኤስዲኬ
በ `/v2/offline/allowances` የተጋለጡትን የ REST ማጣሪያዎች ያንጸባርቃል:
...
ኤክስፕሎረር ቴሌሜትሪ ለ`/v2/explorer/metrics` እና የተተየቡ ረዳቶችን ያቀርባል
ዳሽቦርዶች እንደገና መጫወት እንዲችሉ `/v2/explorer/accounts/{account_id}/qr` የመጨረሻ ነጥቦች
...
`/v2/connect/ws` URL (I18NI0000112X፣ `role`፣ እና የማስመሰያ መለኪያዎችን ጨምሮ)፣ ማሻሻያዎች
...
`ToriiClient.streamEvents()` `/v2/events/sse` እንደ ያልተመሳሰለ ድግግሞሽ አውቶማቲክ ያጋልጣል
...
- `ToriiClient.streamSumeragiStatus()` መስተዋቶች ተመሳሳይ በይነገጽ ለ
  `/v2/sumeragi/status/sse` ስለዚህ የጋራ መግባባት ቴሌሜትሪ ለብቻው ሊደረደር ይችላል ፣ እና
...
2. `ToriiClient.buildRbcSampleRequest(session, overrides)` መደበኛ `{blockHash,height,view}`
   ...
   Torii ይድረሱ።
3. `sampleRbcChunks()` ጥያቄውን ወደ `/v2/sumeragi/rbc/sample` ይለጠፋል፣ የቁርጥ ማስረጃዎችን ይመልሳል።
