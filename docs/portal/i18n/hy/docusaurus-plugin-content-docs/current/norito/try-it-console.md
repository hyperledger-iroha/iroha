...
4. Ընտրեք `application/x-norito` բովանդակության տեսակը «Փորձեք» դարակում, սեղմեք **Ընտրեք Ֆայլը** և ընտրեք ձեր օգտակար բեռը: Վստահված անձը վերագրում է հարցումը `/proxy/v2/pipeline/submit`-ին և այն նշում է `X-TryIt-Client=docs-portal-rapidoc`-ով:
...
Միայն JSON երթուղիների համար ներկառուցված «Փորձեք այն» ավազարկղը հաճախ ավելի արագ է. մուտքագրեք ուղին (օրինակ՝ `/v2/accounts/i105.../assets`), ընտրեք HTTP մեթոդը, տեղադրեք JSON մարմինը, երբ անհրաժեշտ է, և սեղմեք **Ուղարկել հարցումը**՝ վերնագրերը, տևողությունը և օգտակար բեռները ստուգելու համար:
