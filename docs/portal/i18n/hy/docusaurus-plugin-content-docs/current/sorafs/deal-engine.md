...
- `POST /v2/sorafs/deal/usage` ընդունում է `DealUsageReport` հեռաչափությունը և վերադառնում
  դետերմինիստական հաշվառման արդյունքներ (`UsageOutcome`):
- `POST /v2/sorafs/deal/settle`-ն ավարտում է ընթացիկ պատուհանը, հոսում է
  ստացվում է `DealSettlementRecord`՝ բազային64 կոդավորված `DealSettlementV1`-ի հետ մեկտեղ
  պատրաստ է կառավարման DAG հրապարակմանը.
- Torii-ի `/v2/events/sse` հոսքը այժմ հեռարձակում է `SorafsGatewayEvent::DealUsage`
