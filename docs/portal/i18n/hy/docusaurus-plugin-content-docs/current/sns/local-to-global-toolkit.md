...
- `scripts/address_local_toolkit.sh`-ը փաթաթում է `iroha` CLI-ն՝ արտադրելու համար.
  ...
  - `normalized.txt` — փոխարկված նախընտրելի IH58 / երկրորդ լավագույն սեղմված (`sora`) բառացիները տեղական տիրույթի յուրաքանչյուր ընտրողի համար:
...
- `--format compressed` `sora…` ելքի համար՝ IH58-ի փոխարեն:
- `--no-append-domain` բաց բառացի արտանետելու համար:
...
2. Արգելափակումը միաձուլվում է, երբ `audit.json` հաղորդում է Տեղական ընտրիչները (`domain.kind = local12`):
   ...
   `iroha tools address normalize --fail-on-warning --only-local` դեպի CI, ուստի ռեգրեսիա
