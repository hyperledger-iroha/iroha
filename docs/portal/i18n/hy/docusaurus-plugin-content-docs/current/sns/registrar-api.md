...
| Պահանջը | Մանրամասն |
|-------------|--------|
| Արձանագրություններ | ՀԱՆԳՍՏԵՔ `/v2/sns/*` և gRPC ծառայության `sns.v1.Registrar` տակ: Երկուսն էլ ընդունում են Norito-JSON (`application/json`) և Norito-RPC երկուական (`application/x-norito`): |
...
|----------|--------|---------|-------------|
| `/v2/sns/registrations` | ՓՈՍՏ | `RegisterNameRequestV1` | Գրանցվեք կամ նորից բացեք անունը: Լուծում է գնագոյացման մակարդակը, վավերացնում է վճարման/կառավարման ապացույցները, թողարկում է գրանցման իրադարձությունները: |
| `/v2/sns/registrations/{selector}/renew` | ՓՈՍՏ | `RenewNameRequestV1` | Երկարացնել ժամկետը. Պարտադրում է շնորհի/մարման պատուհանները քաղաքականությունից: |
| `/v2/sns/registrations/{selector}/transfer` | ՓՈՍՏ | `TransferNameRequestV1` | Փոխանցել սեփականության իրավունքը, երբ կցվեն կառավարման հաստատումները: |
| `/v2/sns/registrations/{selector}/controllers` | ԴՐԵԼ | `UpdateControllersRequestV1` | Փոխարինեք վերահսկիչի հավաքածու; վավերացնում է ստորագրված հաշվի հասցեները: |
| `/v2/sns/registrations/{selector}/freeze` | ՓՈՍՏ | `FreezeNameRequestV1` | Խնամակալի/խորհրդի սառեցում. Պահանջում է խնամակալի տոմս և հղում կառավարման փաստաթղթերին: |
| `/v2/sns/registrations/{selector}/freeze` | ՋՆՋԵԼ | `GovernanceHookV1` | Ապասառեցնել վերականգնումից հետո; ապահովում է խորհրդի անտեսումը գրանցված: |
| `/v2/sns/reserved/{selector}` | ՓՈՍՏ | `ReservedAssignmentRequestV1` | Ստյուարդ/խորհուրդ վերապահված անունների նշանակում: |
| `/v2/sns/policies/{suffix_id}` | ՍՏԱՆԱԼ | — | Ներբեռնեք ընթացիկ `SuffixPolicyV1` (cacheable): |
| `/v2/sns/registrations/{selector}` | ՍՏԱՆԱԼ | — | Վերադարձնում է ընթացիկ `NameRecordV1` + արդյունավետ վիճակ (Ակտիվ, շնորհ և այլն): |
...
`--governance-json`-ը պետք է պարունակի վավեր `GovernanceHookV1` գրառում (առաջարկի id, քվեարկության հեշեր, կառավարչի/խնամակալի ստորագրություններ): Յուրաքանչյուր հրաման պարզապես արտացոլում է համապատասխան `/v2/sns/registrations/{selector}/…` վերջնակետը, որպեսզի բետա օպերատորները կարողանան կրկնել ճշգրիտ Torii մակերեսները, որոնք կկանչեն SDK-ները:
...
1. Առաջարկի ID-ն գոյություն ունի կառավարման մատյանում (`/v2/governance/proposals/{id}`) և կարգավիճակը `Approved` է:
...
1. Հաճախորդը հարցումներ է կատարում `/v2/sns/policies/{suffix_id}`-ին՝ գները, շնորհը և մատչելի մակարդակները ստանալու համար:
...
3. Վերանորոգումից հետո ավագանու հարցերը գերակայում են. օպերատորն ուղարկում է DELETE `/v2/sns/registrations/{selector}/freeze` `GovernanceHookV1`-ով:
...
- Վճարման ապացույցների կրկնակի օգտագործում Norito մատյանային անդորրագրերը; գանձապետական ​​ծառայությունները տրամադրում են օգնական API (`/v2/finance/sns/payments`):
