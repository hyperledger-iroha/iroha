...
- עוזרי `docs/portal/scripts/sorafs-pin-release.sh` ו-`npm run probe:*`
...
| אות / כלי | الغرض |
| ------------- | ------- |
| `torii_sorafs_replication_sla_total` (נפגש/הוחמצה/בהמתנה) | يكشف توقف النسخ وخرق SLA. |
...
| `npm run check:links` | بوابة الروابط المكسورة؛ הפחתה. |
| `sorafs_cli manifest submit ... --alias-*` (مغلفة عبر `scripts/sorafs-pin-release.sh`) | آلية ترقية/اعادة alias. |
| לוח `Docs Portal Publishing` Grafana (`dashboards/grafana/docs_portal.json`) | סירובי טלמטריה/כינוי/TLS/שכפול. تنبيهات PagerDuty تشير الى هذه اللوحات كدليل. |
...
- تنبيهات Grafana على `torii_sorafs_gateway_refusals_total` او
  השקת `torii_sorafs_manifest_submit_total{status="error"}`.
...
2. **التقاط artefacts:** حمل `build/checksums.sha256`,
   `portal.manifest*.{json,to,bundle,sig}`, מחקרים ובדיקות לבניית בדיקה חוזרת
   الى digests الدقيقة.
...
1. פריט כניסות ומוצר: `DOCS_ANALYTICS_ENDPOINT` ו
...
- خدمة PagerDuty **Docs Portal Publishing** تملك التنبيهات المولدة من
  ...
  `DocsPortal/AliasCache`, ו-`DocsPortal/TLSExpiry` דף הפנייה ל-Docs/DevRel
