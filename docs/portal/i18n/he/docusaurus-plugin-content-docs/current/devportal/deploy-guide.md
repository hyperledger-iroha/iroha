...
- אספו את ראיות הניטור המתוארות ב-[Publishing & Monitoring](./publishing-monitoring.md)
  ...
  `expectHost` האופציונלית. הקריאה למטה כותבת גם תקציר JSON יחיד וגם את חבילת
...
`cargo xtask soradns-binding-template` (שהחליף את העוזר הישן ב-Node)
...
סקריפט השחרור עדיין כולל את סקריפט ה-Node לתאימות לאחור, אך עוזר ה-xtask הוא
המסלול הקנוני קדימה. סקריפט השחרור מריץ את עוזר האימות אוטומטית כדי שכרטיסי
DG-3 תמיד יכללו ראיות עדכניות. הריצו אותו ידנית בכל פעם שאתם משנים את JSON
ה-binding ביד:
...
- `scripts/monitor-publishing.mjs` טוען את קובץ הקונפיג (ראו
  ...
  gateway binding (`scripts/verify-sorafs-binding.mjs`) שכעת מאכף נוכחות
...
1. **הריצו את שער ה-probe.** `ci/check_sorafs_gateway_probe.sh` מפעיל
   ...
   לבדיקות נקודתיות קלות (למשל כאשר רק צמד alias/manifest השתנה), הריצו
   `node scripts/verify-sorafs-binding.mjs --alias "<alias>" --manifest "<digest>" --url "<gateway-url>"`.
   העוזר מושך את ה-endpoint של ה-gateway, מפענח את כותרת `Sora-Proof`
   המוצמדת, וכותב אופציונלית blob ראיות JSON דרך `--json-out`, שימושי לכרטיסי
   שחרור שדורשים רק אישור binding במקום תרגיל probe מלא.
