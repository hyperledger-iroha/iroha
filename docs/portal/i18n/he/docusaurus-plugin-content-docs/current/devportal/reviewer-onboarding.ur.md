...
DOCS-SORA ‏ בנייה מבוססת סכום בדיקה
...
| רול | عام اہداف | مطلوبہ artifacts | نوٹس |
| --- | --- | --- | --- |
| מתחזק ליבה | نئی گائیڈز ویری فائی کرنا، smoke tests چلانا۔ | GitHub handle، Matrix contact، signed CLA فائل پر۔ | عموما پہلے سے `docs-preview` GitHub ٹیم میں ہوتا ہے؛ پھر بھی درخواست درج کریں تاکہ رسائی auditable رہے۔ |
| סוקר שותף | پبلک ریلیز سے پہلے SDK snippets یا governance content ویری فائی کرنا۔ | דוא"ל תאגידי, POC חוקי, תנאי תצוגה מקדימה חתומים. | ٹیلی میٹری + data handling requirements کی منظوری لازم ہے۔ |
| מתנדב קהילתי | گائیڈز پر usability feedback دینا۔ | GitHub handle، preferred contact، timezone، CoC قبولیت۔ | کوہورٹس چھوٹے رکھیں؛ contributor agreement سائن کرنے والے ریویورز کو ترجیح دیں۔ |
...
| مرحلہ | اقدامات |
| --- | --- |
| בעיטה | רשימת תיוג צריכת צריכת מינונים [`preview-feedback-log`](./preview-feedback-log) דף סקירה של `invite-sent`. زیادہ ہو تو midpoint sync شیڈول کریں۔ |
| ניטור | preview telemetry مانیٹر کریں (غیر معمولی Try it ٹریفک، probe failures) اور اگر کوئی مشکوک چیز ہو تو incident runbook فالو کریں۔ findings آنے پر `feedback-submitted`/`issue-opened` events لاگ کریں تاکہ wave metrics درست رہیں۔ |
| יציאה למטוס | عارضی GitHub یا SoraFS رسائی واپس لیں، `access-revoked` درج کریں، درخواست archive کریں (feedback summary + outstanding actions شامل کریں)، اور reviewer registry اپ ڈیٹ کریں۔ ریویور سے مقامی builds صاف کرنے کو کہیں اور [`docs/examples/docs_preview_feedback_digest.md`](../../../examples/docs_preview_feedback_digest.md) سے بنا digest منسلک کریں۔ |
...
## הזמנת תבניות או מעקב- ہر outreach کی شروعات
