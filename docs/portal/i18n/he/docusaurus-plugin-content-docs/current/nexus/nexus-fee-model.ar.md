...
- تتضمن تهيئة اصل الغاز (`pipeline.gas.units_per_gas`) قيمة عشرية `twap_local_per_xor` و`liquidity_profile` (`tier1`, `tier2`, או `tier3`) ו-`volatility_class` (`stable`, `elevated`, `dislocated`). تغذي هذه العلامات موجه التسوية بحيث تطابق تسعيرة XOR الناتجة TWAP القانوني وطبقة haircut الخاصة بالـ lane.
...
يمكن للمستهلكين مراقبة `lane_settlement_commitments` الى جانب اللقطات الحالية لالتزامات lane وdataspace للتحقق من ان مخازن תספורת תספורת ותספורת ותספורת החלפה של תספורת קנה מידה Nexus אופנה.