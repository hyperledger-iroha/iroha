...
|----------------------------------------------------|
| `default_public` | عام | ללא רשות (הימור גלובלי) | نسخة حالة كاملة | SORA הפרלמנט | `xor_global` | دفتر عام اساسي |
| `public_custom` | عام | חסרי רשות או מגודרים על מוקד | نسخة حالة كاملة | وحدة مرجحة بال stake | `xor_lane_weighted` | تطبيقات عامة عالية السعة |
| `private_permissioned` | مقيد | مجموعة مدققين ثابتة (معتمدة من الحوكمة) | התחייבויות והוכחות | מועצה פדרציה | `xor_hosted_custody` | CBDC، اعمال كونسورتيوم |
...
- ניתוב טלפון קטלוג מידע ומידע על נתיב ונתונים. يوفر `LaneConfig` aliases مناسبة للتيليمتري في dashboards والlogs.
...
- קטלוג מסלולי מסלולים וקטלוגים. تحمل `LaneConfigEntry` alias وslug الاصليين لابقاء تيليمتري ومسارات التدقيق مقروءة.
...
## סטטוס טלמטריה- يكشف `/status` عن aliases لل lanes وربط dataspaces وhandles الحوكمة وملفات التسوية، مشتقة من catalog و`LaneConfig`.
...
- כלי עבודה של עמודים משפחות עמודים ונתיבי עמודים נתיבי עמודים ומרחב שמות של נתיבי קוד.
