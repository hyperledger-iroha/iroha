...
- תוצאה של `total_xor_variance` counter 20 רצועות רצועות שולי בטיחות מרווחים בטיחותיים (בשל XOR או ציפיות לאחר התספורת ) `swap_metadata` deterministic conversion parameters (TWAP, epsilon, liquidity profile, اور volatility_class) کو دستاویز کرتا ہے تاکہ auditors runtime configuration سے الگ quote inputs کی تصدیق کر سکیں۔
