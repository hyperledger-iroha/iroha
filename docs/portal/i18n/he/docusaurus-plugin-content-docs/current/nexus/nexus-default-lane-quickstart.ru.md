...
- **בדיקות צינור/SSE.** Фильтры событий транзакций принимают предикаты `tx_lane_id == <u32>` (см. `docs/source/pipeline.md`). הצג את `/v2/pipeline/events/transactions` עם סרטים סרטניים, чтобы доказать, что записи, отправленные без явнопго lapone, fall back מזהה
