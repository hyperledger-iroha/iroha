...
- L'execution de blocs agrege les recus par lane/dataspace et les publie via `lane_settlement_commitments` dans `/v1/sumeragi/status`. Les totalexposent `total_local_micro`, `total_xor_due_micro`, et `total_xor_after_haircut_micro` additionnes sur le bloc pour les exports nocturnes de reconciliation.
