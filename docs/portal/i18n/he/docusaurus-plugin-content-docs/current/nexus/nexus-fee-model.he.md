...
- ביצוע הבלוק מאגד קבלות לכל lane/dataspace ומפרסם אותן דרך `lane_settlement_commitments` ב-`/v1/sumeragi/status`. הסיכומים מציגים `total_local_micro`, `total_xor_due_micro`, ו-`total_xor_after_haircut_micro` מסוכמים על פני הבלוק ליצוא פיוס לילה.
