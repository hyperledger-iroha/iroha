...
- נתוני נתונים ספייס ספייס ספייס, דף מידע טלפון פרטי-DS.
...
- تعريف اقتصاديات التوكن او حوافز المدققين؛ לוח זמנים ותזמון הימורים.
- إدخال اصدار ABI جديد؛ التغييرات تستهدف ABI v1 مع امتدادات syscalls و pointer-ABI وفق سياسة IVM.المصطلحات
- Nexus Ledger: الدفتر المنطقي العالمي المتشكل من تركيب كتل Data Space (DS) في تاريخ مرتب واحد والتزام حالة.
- Data Space (DS): نطاق تنفيذ وتخزين محدود بمدققين خاصين وحوكمة وفئة خصوصية وسياسة DA وحصص وسياسة رسوم. דף: DS ציבורי ו-DS פרטי.
...
- IVM: Iroha Virtual Machine لتنفيذ العقود الذكية (Kotodama bytecode `.to`).
  - AIR: ייצוג ביניים אלגברי. تمثيل جبري للحساب من اجل ادلة نمط STARK يصف التنفيذ كسلاسل تعتمد على الحقول مع قيود انتقال وحدود.
...
- الحوكمة: العضوية والدوران في DS يحددان في قسم الحوكمة بالـ manifest (مقترحات على السلسلة، multisig، او حوكمة מידע על קשר ואישורים).Manifests القدرات و UAID
...
- Allowances: يحمل كل allow bucket حتمي `AllowanceWindow` (`PerSlot`, `PerMinute`, `PerDay`) مع `max_amount` اختياري. מארחים מארחים ו-SDK של Norito, שרתים מארחים ו-SDK.
...
لاجل ادلة تشغيل من طرف لطرف وملاحظات ترحيل SDK وقوائم نشر manifests، حافظ على هذا القسم متوافقا מדריך חשבון אוניברסלי (`docs/source/universal_accounts_guide.md`). ابق الوثيقتين متطابقتين عند تغير سياسة UAID او الادوات.

المعمارية عالية المستوى
1) طبقة التركيب العالمية (Nexus Chain)
- تحافظ على ترتيب قانوني واحد لكتل Nexus كل 1 ثانية لتثبيت معاملات ذرية تمتد عبر מרחבי נתונים (DS). كل معاملة ملتزمة تحدث world state الموحد (متجه roots لكل DS).
...
- الخصوصية: تصدر private DS فقط proofs/commitments مرتبطة بـ roots قبل/بعد. لا تخرج بيانات خاصة خام.4) توفر البيانات (DA) بترميز محو
...
מרווחי שמות
- DS-Qualified IDs: كل الكيانات (domains، accounts، assets، roles) مؤهلة بـ `dsid`. ערך: `ds::<domain>::account`, `ds::<domain>::asset#precision`.
- Global References: المرجع العالمي عبارة عن tuple `(dsid, object_id, version_hint)` ويمكن وضعه on-chain في طبقة nexus او في اوصاف AMX للاستخدام חוצה DS.
- Norito Serialization: جميع رسائل cross-DS (اوصاف AMX، proofs) تستخدم Norito codecs. لا استخدام لـ serde في مسارات الانتاج.العقود الذكية وامتدادات IVM
...
- الحتمية: كل syscalls الجديدة نقية وحتمية عند المدخلات ومجموعات القراءة/الكتابة AMX المعلنة. لا تأثيرات خفية للوقت او البيئة.اثباتات الصلاحية post-quantum (ISI معممة)
...
- נפילות:- ISI المعقدة/المخصصة يمكنها استخدام STARK عام (`zk.policy = "stark_fri_general"`) مع اثبات مؤجل ونهائية 1s عبر attestation QC + slashing على ادلة غير صحيحة.
...
نموذج الخصوصية
- מידע כללי: מערכות מידע ותקשורת ותקשורת WSV מערכות מידע פרטיות DS ‏ المدققين الخاصة.
...
- استدعاءات cross-DS غير متزامنة تتجنب انتظار متزامن طويل داخل تنفيذ private-DS.توفر البيانات وتصميم التخزين
1) ترميز المحو
...
- عقد DA (اختياري): تخزن/تنشر blobs عامة وتدعم sampling. في private DS، عقد DA تكون متجاورة مع المدققين او حراس موثوقين.تحسينات واعتبارات على مستوى النظام
...
- רקורסיה ואצוות מיקרו-אצט: מיקרו-אצטות. الاثبات وزمن التحقق مستقرا تحت الحمل العالي.
...
3 اعادة ارسالها للـ slot التالي. لا تغادر بيانات S الخاصة في اي خطوة.- اعتبارات الامان
