...
| المهمة | المالك | الملاحظات |
|------|--------|--------|
| بناء واجهة خلفية على القرص تغلف `sorafs_car::ChunkStore` مع فهرس manifest على القرص (`sled`/`sqlite`). | فريق التخزين | קוד מוצר: `<data_dir>/<manifest_cid>/chunk_{idx}.bin`. |
