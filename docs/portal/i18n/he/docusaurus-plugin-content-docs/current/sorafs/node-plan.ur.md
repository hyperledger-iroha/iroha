...
---
מזהה: תוכנית צמתים
כותרת: SoraFS תוכנית יישום צומת
sidebar_label: תוכנית יישום צומת
...
3. **תצורת אינסטלציה.** `SoraFsStorage` מבנה תצורה (דגל מופעל, קיבולת, ספריות, מגבלות בו-זמנית) ذریعے wire کریں۔
...
| משימה | בעלים | הערות |
|------|----------------|-------|
| `crates/sorafs_node` بنائیں جس میں `config`, `store`, `gateway`, `scheduler`, `telemetry` modules ہوں۔ | צוות אחסון | Torii integration کے لیے reusable types re-export کریں۔ |
| `StorageConfig` implement کریں جو `SoraFsStorage` سے mapped ہو (user → actual → defaults)۔ | צוות אחסון / Config WG | Norito/`iroha_config` layers کو deterministic رکھیں۔ |
| Torii کے pins/fetches کے لیے `NodeHandle` facade فراہم کریں۔ | צוות אחסון | storage internals اور async plumbing encapsulate کریں۔ |

### ב. חנות נתחים מתמשכת

| משימה | בעלים | הערות |
|------|----------------|-------|
| `sorafs_car::ChunkStore` کو on-disk manifest index (`sled`/`sqlite`) کے ساتھ disk backend میں wrap کریں۔ | צוות אחסון | פריסה דטרמיניסטית: `<data_dir>/<manifest_cid>/chunk_{idx}.bin`. |
| `ChunkStore::sample_leaves` کے ذریعے PoR metadata (64 KiB/4 KiB trees) maintain کریں۔ | צוות אחסון | Restart کے بعد replay support؛ corruption پر fail fast۔ |
| Startup پر integrity replay implement کریں (manifests rehash، incomplete pins prune)۔ | צוות אחסון | replay مکمل ہونے تک Torii start block کریں۔ |

### ג. נקודות קצה של שער| נקודת קצה | התנהגות | משימות |
|--------|------------|-------|
...
| `GET /sorafs/chunks/{cid}` + שאילתת טווח | `Content-Chunker` headers کے ساتھ chunk bytes serve کریں؛ range capability spec respect کریں۔ | scheduler + stream budgets استعمال کریں (SF-2d range capability کے ساتھ tie کریں)۔ |
...
| `GET /sorafs/telemetry` | סיכומים: קיבולת, הצלחה של PoR, ספירת שגיאות באחזור. | dashboards/operators کے لیے data فراہم کریں۔ |
...
### ד. מתזמן ואכיפת מכסות

| משימה | פרטים |
|------|--------|
| מכסת דיסקים | Disk bytes track کریں؛ `max_capacity_bytes` پر نئے pins reject کریں۔ مستقبل کی policies کے لیے eviction hooks فراہم کریں۔ |
| אחזר מקבילות | Global semaphore (`max_parallel_fetches`) کے ساتھ per-provider budgets جو SF-2d range caps سے آتے ہیں۔ |
| תור הצמדות | Outstanding ingestion jobs محدود کریں؛ queue depth کے لیے Norito status endpoints expose کریں۔ |
| קצב PoR | `por_sample_interval_secs` سے چلنے والا background worker۔ |
...
### ו. אסטרטגיית בדיקה1. **בדיקות יחידות.** התמדה בחנות נתחים, חישובי מכסות, משתנה מתזמן (ראה `crates/sorafs_node/src/scheduler.rs`).
2. **מבחני אינטגרציה** (`crates/sorafs_node/tests`). סיכה → אחזר הלוך ושוב, הפעלה מחדש של שחזור, דחיית מכסה, אימות הוכחת דגימת PoR.
