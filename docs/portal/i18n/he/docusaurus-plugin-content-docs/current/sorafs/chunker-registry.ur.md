...
כותרת: SoraFS chunker profile registry
sidebar_label: הרישום Chunker
...
## רישום פרופילי chunker SoraFS (SF-2a)
...
[רשימת בדיקה להשקת הרישום](./chunker-registry-rollout-checklist.md) אוור
[staging manifest playbook](./staging-manifest-playbook) کے مطابق fixtures کو staging اور production میں promote کریں۔

### פרופילים

| מרחב שמות | שם | SemVer | מזהה פרופיל | דקות (בתים) | יעד (בתים) | מקסימום (בתים) | לשבור מסכה | Multihash | כינויים | הערות |
|-----------|------|--------|---------------------------------------------------|
...
Manifest `ChunkingProfileV1` کے ذریعے profiles کو serialize کرتا ہے۔ 20 מטא נתונים של רישום מבנה
...
* **נתיב ראשי** – CARv2, תקציר מטען BLAKE3 (`0x1f` multihash),
