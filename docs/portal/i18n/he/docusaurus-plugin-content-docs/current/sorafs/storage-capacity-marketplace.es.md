...
|------|----------------|------|
| מטפלי אגרגר Torii `/v1/sorafs/capacity/declare`, `/v1/sorafs/capacity/telemetry`, `/v1/sorafs/capacity/orders` עם מטענים Norito JSON. | צוות Torii | Reflejar la logica del validador; reutilizar helpers Norito JSON. |
...
| ייצוא לוחות מחוונים/אזהרות למתן מדידה (פיגור של אינסטה, טלמטריה מיושן). | צפייה | Extender el pack de Grafana התייחסות ל-SF-6/SF-7. |- Torii ahora expone `/v1/sorafs/capacity/telemetry` y `/v1/sorafs/capacity/state` (JSON + Norito) para que operadores envien snapshots de telemetria por epoca y los inspectores can recuperen o audit el ledger recuperen evidencia.【crates/iroha_torii/src/sorafs/api.rs:268】【crates/iroha_torii/src/sorafs/api.rs:816】
