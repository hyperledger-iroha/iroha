...
| `CapacityLedger` | בדוק על-שרשרת/מחוץ לשרשרת. | وحدة smart contract أو stub لخدمة off-chain مع snapshot حتمي. |
...
| المهمة | בעלים | ملاحظات |
|------|----------------|-------|
| تعريف `CapacityDeclarationV1` و `ReplicationOrderV1` و `CapacityTelemetryV1`. | צוות אחסון / ממשל | استخدم Norito؛ وأدرج النسخ الدلالية ومراجع القدرات. |
| تنفيذ وحدات parser + validator في `sorafs_manifest`. | צוות אחסון | فرض IDs أحادية، حدود السعة، ومتطلبات stake. |
...
| صياغة وثيقة `MarketplacePolicy` التي تلتقط ضوابط القبول وجدول العقوبات. | מועצת ממשל | انشرها في docs بجانب policy defaults. |

#### تعريفات المخطط (منفذة)- يلتقط `CapacityDeclarationV1` التزامات سعة موقعة لكل مزود، بما في ذلك handles chunker القياسية، مراجع capabilities، حدود اختيارية لكل lane، تلميحات التسعير، نوافذ الصلاحية، وmetadata. تضمن عملية التحقق stake غير صفري، handles قياسية، aliases مزالة التكرار، حدود lane ضمن الإجمالي المعلن، ومحاسبة GiB أحادية.【crates/sorafs_manifest/src/capacity.rs:28】
...
| المهمة | בעל/ים | ملاحظات |
|------|----------------|-------|
| إضافة معالجات Torii لـ `/v1/sorafs/capacity/declare` و`/v1/sorafs/capacity/telemetry` و`/v1/sorafs/capacity/orders` بحمولات Norito JSON. | צוות Torii | محاكاة منطق التحقق؛ إعادة استخدام Norito JSON helpers. |
| تمرير snapshots الخاصة بـ `CapacityDeclarationV1` إلى metadata لوحة نقاط orchestrator وخطط الجلب في gateway. | צוות Tooling WG / תזמורת | تمديد `provider_metadata` بمراجع السعة حتى يحترم scoring متعدد المصادر حدود lane. |
| تغذية أوامر النسخ المتماثل إلى عملاء orchestrator/gateway لقيادة assignments وتلميحات failover. | Networking TL / Gateway צוות | يستهلك Scoreboard builder أوامر النسخ الموقعة من الحوكمة. |
...
### 3. سياسة السوق والحوكمة| المهمة | בעלים | ملاحظات |
|------|----------------|-------|
| إقرار `MarketplacePolicy` (الحد الأدنى لـ stake، مضاعفات العقوبة، وتواتر التدقيق). | מועצת ממשל | نشرها في docs وتسجيل سجل المراجعات. |
| إضافة خطافات الحوكمة حتى يستطيع Parliament اعتماد وتجديد وإلغاء declarations. | מועצת ממשל / צוות חוזה חכם | استخدام Norito events + manifest ingestion. |
| تنفيذ جدول العقوبات (تخفيض الرسوم، slashing للبوند) المرتبط بانتهاكات SLA المقاسة عن بعد. | מועצת ממשל / האוצר | مواءمة ذلك مع مخرجات التسوية في `DealEngine`. |
| توثيق عملية النزاع ومصفوفة التصعيد. | מסמכים / ממשל | الربط إلى dispute runbook + CLI helpers. |

### 4. الميترينغ وتوزيع الرسوم

| المهمة | בעלים | ملاحظات |
|------|----------------|-------|
| توسيع ingest الميترينغ في Torii لقبول `CapacityTelemetryV1`. | צוות Torii | מידע על GiB-hour ו-PoR וזמן פעולה. |
| تحديث خط أنابيب الميترينغ في `sorafs_node` للإبلاغ عن استخدام لكل أمر + إحصاءات SLA. | צוות אחסון | כלי עבודה וטיפול בצ'אנקר. |
| خط أنابيب التسوية: تحويل التليمترية + بيانات النسخ إلى payouts مقومة بـ XOR، وإنتاج ملخصات جاهزة פנקס חשבונות. | צוות אוצר / אחסנה | التوصيل إلى Deal Engine / Treasury exports. |
| تصدير dashboards/alerts لصحة الميترينغ (backlog ingestion، تليمترية قديمة). | צפייה | توسيع حزمة Grafana المشار إليها في SF-6/SF-7. |
...
- הצג תצלומי מצב של `CapacityTelemetrySnapshot` תצלום מצב `metering`, ותיצוא Prometheus Grafana الجاهزة للاستيراد في `docs/source/grafana_sorafs_metering.json` حتى تتمكن فرق الفوترة من مراقبة تراكم GiB-hour والرسوم nano-SORA المتوقعة والامتثال لـ SLA في الوقت الحقيقي.【crates/iroha_torii/src/routing.rs:5143】【docs/source/grafana_sorafs_metering.json:1】
- התקן את החלקת מדידה, הצג תצלום מצב של `smoothed_gib_hours` ו-`smoothed_por_success_bps`. بالعدادات الخام التي تعتمد عليها الحوكمة في payouts.【crates/sorafs_node/src/metering.rs:401】

### 5. معالجة النزاعات والإلغاء| المهمة | בעלים | ملاحظات |
|------|----------------|-------|
| تعريف حمولة `CapacityDisputeV1` (صاحب الشكوى، evidence، المزود المستهدف). | מועצת ממשל | مخطط Norito + مدقق. |
| دعم CLI لتقديم disputes والرد عليها (مع مرفقات evidence). | Tooling WG | ضمان hashing حتمي لحزمة evidence. |
| إضافة فحوص تلقائية لتكرار خروقات SLA (تصعيد تلقائي إلى dispute). | צפייה | عتبات تنبيه وخطافات حوكمة. |
| توثيق playbook الإلغاء (فترة سماح، إخلاء بيانات pinned). | צוות מסמכים / אחסון | الربط إلى وثيقة السياسة وoperator runbook. |
...
## الاعتماديات والتسلسل1. إكمال SF-2b (admission policy) - يعتمد marketplace على مزودين مدققين.
2. تنفيذ طبقة المخطط + السجل (هذا المستند) قبل تكامل Torii.
3. إكمال metering pipeline قبل تفعيل المدفوعات.
4. תקנות: בדיקת מדידה ומדידה.
