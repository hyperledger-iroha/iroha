...
SoraFS בינאריים (`sorafs_cli`, `sorafs_fetch`, עוזרים) ארגזי SDK
(`sorafs_car`, `sorafs_manifest`, `sorafs_chunker`) ایک ساتھ ship ہوتے ہیں۔ שחרר
...
- `cargo clippy --locked --all-targets` `sorafs_car` کے لیے (feature `cli` کے ساتھ)،
  `sorafs_manifest` אור `sorafs_chunker`
...
کے ساتھ continuous رہنا چاہیے؛ release branches میں fixes cherry-pick نہ کریں۔ שער
20 דגלי חתימה ללא מפתח (`--identity-token-issuer`, `--identity-token-audience`)
جہاں ضروری ہوں فراہم کیے گئے ہوں؛ missing arguments run کو fail کر دیتے ہیں۔

## 2. Versioning policy لاگو کریں

SoraFS ארגזי CLI/SDK ב-SemVer.
...
- `PATCH`: תיקוני באגים, מהדורות תיעוד בלבד, עדכוני תלות והתנהגות ניתנת לצפייה.

`sorafs_car`, `sorafs_manifest` אור `sorafs_chunker` גירסה 2000000035
SDK downstream consumers ایک aligned version string پر depend کر سکیں۔ Version bump کرتے وقت:1. ہر crate کے `Cargo.toml` میں `version =` fields اپڈیٹ کریں۔
...
גירסת שחרור ויומן שינויי סימון גיאוגרפיות ושינויים ב-CLI, SDK ושינויים המשפיעים על ממשל
کو highlight کرے۔ `docs/examples/sorafs_release_notes.md` תבנית קובץ גרסה
...
`scripts/release_sorafs_cli.sh` קובץ חתימה חבילת חתימה או סיכום אימות הפק גירסה של 2000
کے ساتھ ship ہوتے ہیں۔ Wrapper ضرورت کے مطابق CLI build کرتا ہے، `sorafs_cli manifest sign` چلاتا ہے، اور فوراً
`manifest verify-signature` replay کرتا ہے تاکہ tagging سے پہلے failures سامنے آ جائیں۔ מיתאר:
...
- `manifest.bundle.json`, `manifest.sig`, `manifest.sign.summary.json`, אוור `manifest.verify.summary.json` یہی packet governance notification میں refer ہوتا ہے۔
- שחרור אביזרי קנוניים, מניפסט מרענן, תוכנית נתחים, סיכומים של סיכומים ב-`fixtures/sorafs_manifest/ci_sample/` ב-2000000051 X کریں) tagging سے پہلے۔ Downstream operators committed fixtures پر depend کرتے ہیں تاکہ release bundle reproduce کر سکیں۔
- `sorafs_cli proof stream` אימות ערוץ מוגבל, הפעל לכידת יומן ריצה חבילת שחרור, צרף אמצעי הגנה להוכחת זרימה.
...
Checks پاس ہونے اور hooks مکمل ہونے کے بعد:1. `sorafs_cli --version` اور `sorafs_fetch --version` چلائیں تاکہ binaries نئی version report کریں۔
ב Ad-hoc environment variables پر انحصار نہ کریں؛ CLI کو `--config` (یا equivalent) کے ذریعے paths دیں تاکہ release inputs واضح اور reproducible رہیں۔
...
- גרסה חדשה תיעוד גרסה ותיעוד (התחלות מהירות, תבניות CI) ‏
