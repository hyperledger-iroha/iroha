...
- يقوم `OrchestratorConfig::with_telemetry_region("region")` بوسم مقاييس
  ...
  לוח CLI `--transport-policy`. تُشحن `TransportPolicy::SoranetPreferred` كافتراضي عبر
...
כתובית `Client::sorafs_fetch_via_gateway` דף הבית של מידע נוסף
...
يظل `ManifestBuilder` الطريقة المعتمدة لتجميع حمولات Norito برمجيًا:```rust
