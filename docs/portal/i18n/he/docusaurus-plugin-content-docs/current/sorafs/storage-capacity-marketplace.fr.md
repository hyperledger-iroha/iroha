...
- `PinProviderRegistry` לחשוף את תמונת המצב על השרשרת דרך `/v2/sorafs/capacity/state`, ב-`/v2/sorafs/capacity/state`, הצהרות משולבות של ספקים וכניסות ל-Fee Ledger Derrière un Norito JSON déterministe.【crates/iroha_torii/src/sorafs/registry.rs:17】【crates/iroha_torii/src/sorafs/api.rs:64】
...
|------|----------------|-------|
| Ajouter des handlers Torii `/v2/sorafs/capacity/declare`, `/v2/sorafs/capacity/telemetry`, `/v2/sorafs/capacity/orders` avec מטענים Norito JSON. | צוות Torii | Répliquer la logique de validation; réutiliser les helpers Norito JSON. |
