...
`docs/examples/sorafs_direct_mode_policy.json` אויר
...
```- `operator_jurisdictions` ISO‑3166 alpha‑2 תקליטורים 2
...
- `blinded_cid_opt_outs` manifest digests (blinded CIDs، uppercase hex) لسٹ کرتا
  ہے۔ התאמה של מטענים ישירים בלבד תזמון ישיר בלבד.
...
- `attestations` کمپلائنس کی سائنڈ پیکٹس محفوظ کرتا ہے۔ ہر entry ایک optional
  `jurisdiction` (ISO-3166 alpha-2), `document_uri`, 64 תווים `digest_hex`,
...
קטלוגים קנוניים לביטול הסכמה `governance/compliance/soranet_opt_outs.json`
...
[ספר תאימות GAR](../../../source/soranet/gar_compliance_playbook.md)
میں درج ہے۔

### 1.3 CLI اور SDK knobs| דגל / שדה | اثر |
|--------------|-----|
...
| `--deny-provider` / וו מדיניות ניקוד | adverts ڈیلیٹ کیے بغیر پرووائیڈرز کو deterministic طور پر exclude کرتا ہے۔ فوری بلیک لسٹنگ کے لیے مفید۔ |
| `--boost-provider=name:delta` | گورننس weights برقرار رکھتے ہوئے پرووائیڈر کے weighted round-robin credits ایڈجسٹ کرتا ہے۔ |
| `--telemetry-region` / `OrchestratorConfig::with_telemetry_region` | יומנים מובְנים של יומנים מובנים سکیں۔ |
...
מטענים של ספריית Guard סכימה קומפקטית סכימה קומפקטית גרסה:

`--guard-directory` flag اب Norito-encoded `GuardDirectorySnapshotV2` payload
expect کرتا ہے۔ binary snapshot میں شامل ہے:- `version` — schema ورژن (فی الحال `2`).
...
- `validation_phase` — שער מדיניות אישורים (`1` = חתימת Ed25519 יחידה
  לאפשר, `2` = מעדיף חתימות כפולות, `3` = נדרשות חתימות כפולות).
- `issuers` — מנפיקי ממשל: `fingerprint`, `ed25519_public`, `mldsa65_public`.
...
- `relays` — SRCv2 bundles کی فہرست (`RelayCertificateBundleV2::to_cbor()` output).
  1 מתאר ממסר צרור, דגלים של יכולת, מדיניות ML-KEM, או Ed25519/ML-DSA-65
  dual signatures رکھتا ہے۔

CLI ہر bundle کو declared issuer keys کے خلاف verify کرتی ہے اور پھر directory کو
snapshots لازمی ہیں۔

`--guard-directory` קובץ שמור של CLI קובץ מטמון
کے ساتھ merge کیا جا سکے۔ selector pinned guards کو برقرار رکھتا ہے جو ابھی
retention window کے اندر اور directory میں eligible ہیں؛ פג תוקף ממסרים
entries کو replace کرتے ہیں۔ کامیاب fetch کے بعد updated cache `--guard-cache`
کے path پر لکھ دی جاتی ہے، جس سے اگلی سیشنز deterministic رہتی ہیں۔ ערכות SDK 20.000
התנהגות `GuardSelector::select(&RelayDirectory, existing_guard_set, now_unix_secs)`
...
SNNet-5 rollout جاری ہو۔ החלפת שלב (`anon-guard-pq`, `anon-majority-pq`,
`anon-strict-pq`) או ממסרים קלאסיים.
...
### 1.5 מנהל מחזור חיים של מעגליםجب relay directory اور guard cache دونوں فراہم ہوں تو آرکسٹریٹر circuit lifecycle
...
- `relay_directory`: תמונת מצב של ספריית SNNet-3
...
עוזר `refresh_circuits` (`crates/sorafs_orchestrator/src/lib.rs:1346`) `CircuitEvent`
لاگز emit کرتا ہے تاکہ آپریٹرز lifecycle فیصلوں کو ٹریس کر سکیں۔ מבחן השרייה
...
הרחבות דפדפן QUIC SDK
adapters کو certificates یا guard cache keys manage نہ کرنا پڑیں۔ לולאה של פרוקסי
...
תקציר JSON גרסה `local_proxy` תוכנות הפעלה של פרוקסי:
...
- `kaigi_bridge` (اختیاری) Kaigi routes کو proxy پر expose کرتا ہے۔ `room_policy`
  ...
  תוויות gar
- `sorafs_cli fetch` `--local-proxy-mode=bridge|metadata-only` אוור
...
- `downgrade_remediation` (اختیاری) خودکار downgrade hook کو کنفیگر کرتا ہے۔
  ...
  `cooldown_secs` کے بعد `resume_mode` پر واپس آتا ہے۔ מערך `modes` ספציפי
...
- **`norito`** — יעד זרם `norito_bridge.spool_dir` פתרון זרם
...
جب enabled ہو تو proxy اب **manifest v2** ریکارڈز سرور کرتا ہے۔ תעודת מדיה
...
- רמזים לבחירת שומרי מעגל אוור (`circuit`, `guard_selection`, `route_hints`)
...
1. **Eligibility failures (pre-flight).** range capability نہ رکھنے والے، expired
   ...
   جاتا ہے اور scheduling سے نکال دیا جاتا ہے۔ סיכומי CLI `ineligible_providers`
...
2. **Runtime exhaustion.** ہر provider مسلسل failures کو track کرتا ہے۔ جب
   ...
   `disabled` کر دیا جاتا ہے۔ ספקי תקינות `disabled`
...
3. **Deterministic aborts.** سخت حدود structured errors کے طور پر ظاہر ہوتی ہیں:
   ...
   - `MultiSourceError::ExhaustedRetries` — תקציב ניסיון חוזר לכל נתח.
...
- מטא נתונים `provider` (מזהה, נקודות קצה, תקציב במקביל).

Scoreboard snapshots کو release artifacts کے ساتھ archive کریں تاکہ blacklist
اور rollout فیصلے audit-able رہیں۔

## 3. טלמטריה או ניפוי באגים

### מדדי 3.1 Prometheus

آرکسٹریٹر `iroha_telemetry` کے ذریعے درج ذیل میٹرکس emit کرتا ہے:| מדד | תוויות | תיאור |
|--------|--------|-------------|
...
פריסת SF-6 תצפית תוכנית מעקב בצע את התוכנית:
...
- `telemetry::sorafs.fetch.lifecycle` — `start` אור `complete` סמני מחזור חיים
  کے ساتھ chunk counts، retries، اور total duration۔
- `telemetry::sorafs.fetch.retry` - אירועי ניסיון חוזר (`provider`, `reason`, `attempts`).
...
کو ایک ہی source of truth ملے۔ אירועי מחזור חיים PQ/מיקס קלאסי
`anonymity_effective_policy`, `anonymity_pq_ratio`, `anonymity_classical_ratio`
סופרים מונים מונים מונים דלפקים וגרד מדדים
...
- מערכי `retry_stats` או `ineligible_providers`.مشکوک providers کو debug کرتے وقت summary file archive کریں — receipts اوپر
دیے گئے log metadata سے براہ راست جڑتے ہیں۔

## 4. רשימת בדיקה תפעולית
...
2. **Telemetry ویلیڈیٹ کریں۔** multi-source fetches enable کرنے سے پہلے یقینی
   פריסת `sorafs.fetch.*` מדדים או ייצוא יומנים מובנים
...
`policy_override.anonymity_policy` קובץ `orchestrator` JSON מוזיקה (
`sorafs_cli fetch` או `--transport-policy-override=` / `--anonymity-policy-override=`
