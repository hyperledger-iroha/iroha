...
- מטענים объявлений провайдеров должны включать `range_capability`, `stream_budget`
  ו `transport_hints`. Проверяйте через ответы `/v1/sorafs/providers` и מתקני כניסה;
...
## Интеграция с оркестратором и Torii- Torii `/v1/sorafs/providers` возвращает разобранные метаданные диапазонных возможностей
