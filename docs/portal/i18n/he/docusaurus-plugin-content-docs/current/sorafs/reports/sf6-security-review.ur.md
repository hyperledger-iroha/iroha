...
**חלון הערכה:** 2026-02-10 → 2026-02-18
**לידים לבדיקה:** גילדת הנדסת אבטחה (`@sec-eng`), קבוצת עבודה של כלי עבודה (`@tooling-wg`)
**היקף:** SoraFS CLI/SDK (`sorafs_cli`, `sorafs_car`, `sorafs_manifest`), ממשקי API להזרמת הוכחה, טיפול במניפסט Torii, שילוב Sigstore/OIDC, ווי שחרור CI.
**חפצים:**
- בדיקות מקור של CLI (`crates/sorafs_orchestrator/src/bin/sorafs_cli.rs`)
...
4. **בדיקת תצורה** ברירות מחדל של `iroha_config`, טיפול בדגל CLI או סקריפטים של שחרור לאמת ריצות דטרמיניסטיות, ניתנות לביקורת.
...
- **תיעוד:** מדריכים למפתחים ([אינדקס מפתחים](../developer-index.md), [הפניה ל-CLI](../developer-cli.md)) דגלים רגישים לאבטחה או זרימות עבודה להסלמה

## תוספות רשימת שחרורים

Release managers کو GA candidate promote کرتے وقت درج ذیل evidence **لازمی** attach کرنا ہوگا:1. تازہ ترین سیکیورٹی ریویو memo کا hash (یہ دستاویز)۔
...
4. parity harness کے captured logs (`cargo test -p sorafs_orchestrator --test sorafs_cli proof_stream_consumes_ndjson_and_reports_metrics -- --nocapture`)۔
...
**הפניות ל-hashs של חפץ (2026-02-20 sign-off):**
