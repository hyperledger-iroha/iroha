...
|-------|----------------|--------|
| רישום принимает канонические декларации емкости | Интеграционный тест выполняет `/v2/sorafs/capacity/declare` משתמש ב-API של אפליקציית האפליקציה, הורד שרת, הורד מטא נתונים ורישום רישום. | `crates/iroha_torii/src/routing.rs:7654` |
