...
| `sorafs_manifest_stub` | ⚠️ Legacy | builder של manifest מדור קודם; השתמשו ב-`iroha sorafs toolkit pack` לאריזת CAR/manifest והשאירו `--plan=-` לרה-ולידציה דטרמיניסטית. |
| `sorafs_provider_advert_stub` | ⚠️ Legacy | helper אימות offline בלבד; provider adverts צריכים להיווצר בצינור הפרסום ולהיבדק דרך `/v1/sorafs/providers`. |
...
| Bridge CARv1 (`sha2-256`) | ⚠️ מעבר | זמין ללקוחות legacy כאשר הבקשה מצהירה על הפרופיל הקנוני וגם `Accept-Digest: sha2-256`; התגובות כוללות `Content-Chunker: ...;legacy=true`. |
...
`sorafs.sf1@1.0.0` כ-deprecated ברישום והסרת Bridge CARv1 מ-gateways בפרודקשן.
...
ה-envelope הקנוני של manifest משתמש בשורשי CIDv1 עם `dag-cbor` (`0x71`). עבור תאימות legacy
...
* **Bridge legacy** – CARv1, SHA-256 payload digest (`0x12` multihash). שרתים
  רשאים לחשוף וריאנט זה כאשר הלקוח משמיט `Accept-Chunker` או מבקש
  `Accept-Digest: sha2-256`.

Manifests תמיד מפרסמים את ההתחייבות CARv2/BLAKE3. נתיבי legacy מספקים headers
...
* Manifests שמיוצרים על ידי `iroha sorafs toolkit pack` ו-`sorafs_manifest_stub` כוללים את מטאדאטת הרישום.
