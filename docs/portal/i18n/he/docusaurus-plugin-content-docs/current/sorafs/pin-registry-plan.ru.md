...
|--------|--------|----------------|
| Сервис Torii | Экспонировать `/v2/sorafs/pin` (שלח), `/v2/sorafs/pin/{cid}` (חיפוש), `/v2/sorafs/aliases` (רשימה/כריכה), `/v2/sorafs/replication` (הזמנות/קבלות). Обеспечить пагинацию + фильтрацию. | Networking TL / Core Infra |
...
- `GET /v2/sorafs/pin` ו-`GET /v2/sorafs/pin/{digest}` מציגים ביטויים с
  כריכות כינוי, הזמנות репликации объектом аттестации, производным от
  хэша последнего блока.
- `GET /v2/sorafs/aliases` ו-`GET /v2/sorafs/replication` публикуют активный
