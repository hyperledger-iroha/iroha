...
---
מזהה: מנוע עסקה
כותרת: מנוע עסקה SoraFS
sidebar_label: מנוע עסקאות
...
תנאי עסקה, נעילת אגרות חוב, תשלומים הסתברותיים ורישומי פשרה.

Embedded SoraFS worker (`sorafs_node::NodeHandle`) اب ہر node process کے لیے
ایک `DealEngine` instance بناتا ہے۔ 2 מנוע:
...
- deterministic BLAKE3-based sampling کے ذریعے probabilistic micropayment windows evaluate کرتا ہے؛ אוור
...
`deal_outstanding_nano`, `deal_bond_slash_nano`, `deal_publish_total`) או Torii לוחות מחוונים
SLO enforcement کے لیے update کرتی ہے۔ פריטי מעקב ביוזמת המבקר אוטומציה של חיתוך אוטומציה
...
`micropayment_outstanding_nano`, דלפקי כרטיסים אחרים
(`micropayment_tickets_processed_total`, `micropayment_tickets_won_total`,
`micropayment_tickets_duplicate_total`). 20 סכומי זרימת לוטו הסתברותית.
operators micropayment wins اور credit carry-over کو settlement outcomes کے ساتھ correlate کر سکیں۔

## אינטגרציה של Torii

Torii נקודות קצה ייעודיות חושפות את דוח השימוש של ספקי ספקים.
بدون bespoke wiring کے deal lifecycle drive کر سکیں:- `POST /v2/sorafs/deal/usage` `DealUsageReport` telemetry accept کرتا ہے اور
