...
| רשם פעולות ותפקוד | נקודות קצה פתוחות `/v2/sns/*`, תשלומים מתפשרים, טלמטריה ותחזוקה של צילומי SLI. | רשם API ([`registrar-api.md`](./registrar-api.md)), מדדים `sns_registrar_status_total`, preuves de paiement archivees sous `artifacts/sns/payments/*`. | מנהל תורנות דו רשם ותאגיד קשר. |
...
| `torii_request_duration_seconds{route="/v2/sns/*"}` | Metriques Torii | SLO de latence pour les handlers API; alimente des לוחות מחוונים issus de `torii_norito_rpc_observability.json`. |
