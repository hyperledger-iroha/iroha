...
| `owner` | כן | מחרוזת AccountId (domainless encoded AccountId literal (IH58 preferred, compressed accepted; no @domain suffix)) לבעל הרישום. |
...
- **Controllers:** תאים ריקים מכבדים את `--default-controllers`. ספקו רשימות
  controller מפורשות (למשל `ih58...;ih58...`) בעת האצלה לגורמים שאינם owner.
