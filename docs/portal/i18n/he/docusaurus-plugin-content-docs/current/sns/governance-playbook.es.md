...
| Operaciones de registrador y facturacion | נקודות קצה אופנתיות `/v1/sns/*`, פקודות מתפייסות, פולטות טלמטריה וצילומי מצב של CLI. | API del registrador ([`registrar-api.md`](./registrar-api.md)), מדדים `sns_registrar_status_total`, פרועbas de pago en `artifacts/sns/payments/*`. | מנהל תורנות של רישום y enlace de tesoreria. |
...
| `torii_request_duration_seconds{route="/v1/sns/*"}` | Metricas de Torii | SLOs de latencia para handlers API; לוחות מחוונים של alimenta basados ​​en `torii_norito_rpc_observability.json`. |
