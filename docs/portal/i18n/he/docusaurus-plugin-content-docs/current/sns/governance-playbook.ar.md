...
שירותים, פתרונות ושירותים של פותר וgateway. יפי
...
|------|----------------------|------------------------|---|
| مجلس الحوكمة | דיילים ודיילים. | `docs/source/sns/governance_addenda/`, `artifacts/sns/governance/*`, بطاقات تصويت المجلس المخزنة عبر `sns governance charter submit`. | رئيس المجلس + متعقب جدول اعمال الحوكمة. |
| مجلس guardian | يصدر تجميدات soft/hard، وقوانين طارئة، ومراجعات 72 h. | שומר אפוטרופוס על ידי `sns governance freeze`, ושירותי שמירה על תקנות `artifacts/sns/guardian/*`. | אפוטרופוס תורן (<=15 דקות ACK). |
...
| אפוטרופוס אופי | المجلس + guardians | `sns governance ballot cast --proposal <id>` ו-`sns governance guardian-ack --proposal <id>` | ارفاق محاضر مجزأة واثباتات النصاب. |
...
### 4.2 תקנות תקנות1. **الفحص المسبق:** يستعلم المسجل `SuffixPolicyV1` لتاكيد شريحة التسعير، الشروط
...
- **مراجعة KPI شهرية:** يجمع فحص اول ثلاثاء قائد المالية وsteward المناوب وPM
  ...
  תקשורת ותחזוקה, תקשורת וספה. فعّل حادثا
  اذا وجدت المراجعة خروقات SLA (نوافذ تجميد >72 h، ارتفاع اخطاء المسجل، انحراف ARPU).

### 4.4 מידע ותקשורת| المرحلة | المالك | الاجراء والدليل | SLA |
|--------|--------|----------------|-----|
| طلب تجميد soft | steward / الدعم | תקשורת `SNS-DF-<id>` מחזיקה בתשלום. | <=4 שעות לאחר המלחמה. |
| تذكرة guardian | مجلس guardian | `sns governance freeze --selector <I105> --reason <text> --until <ts>` יין `GuardianFreezeTicketV1`. خزّن JSON التذكرة تحت `artifacts/sns/guardian/<id>.json`. | <=30 דקות ACK, <=2 שעות זמן. |
...
| استئناف | guardian + المجلس | يضاعف الاستئناف bond ويعيد عملية المحلفين; سجل مانيفست Norito `DisputeAppealV1` واربط بالتذكرة الاصلية. | <=10 אימה. |
...
| `soradns_bundle_proof_age_seconds` ו-`soradns_bundle_cid_drift_total` | tailer شفافية resolver | تكشف ادلة قديمة او انحراف GAR؛ الحواجز معرفة في `dashboards/alerts/soradns_transparency_rules.yml`. |
...
| מד `guardian_freeze_active` | אפוטרופוס CLI | يتتبع نوافذ تجميد soft/hard لكل محدد؛ ناد SRE اذا بقيت القيمة `1` بعد SLA المعلن. |
| لوحات ملاحق KPI | المالية / الوثائق | ملخصات شهرية تنشر مع المذكرات التنظيمية؛ تضمها البوابة عبر [لوحة KPI الخاصة بـSNS](./kpi-dashboard.md) ليتمكن stewards والمنظمون من الوصول لنفس عرض Grafana. |## 6. متطلبات الادلة والتدقيق
...
| N2 - توسع | `.dao`, משווק תאגיד, נדל"ן, דייל נדל"ן. | لقطات شاشة للبوابة، مقاييس SLA للنزاعات، صادرات بطاقات تقييم steward، ميثاق حوكمة محدث يشير لسياسات reseller. |
...
|--------|-------|--------------|----------------------|
| انحراف resolver/GAR او ادلة قديمة | סב 1 | SRE resolver + مجلس guardian | استدعاء on-call للresolver، التقاط مخرجات tailer، تقرير ما اذا كان يجب تجميد الاسماء المتاثرة، نشر تحديث حالة كل 30 min. |
| تعطل المسجل، فشل الفوترة، او اخطاء API واسعة | סב 1 | مدير مناوبة المسجل | ايقاف المزادات الجديدة، التحول الى CLI يدوي، اخطار stewards/الخزينة، ارفاق سجلات Torii بوثيقة الحادث. |
| نزاع اسم واحد، عدم تطابق الدفع، او تصعيد عميل | סב' 2 | steward + قائد الدعم | جمع اثباتات الدفع، تحديد الحاجة الى تجميد soft، الرد على مقدم الطلب ضمن SLA، تسجيل النتيجة في متعقب النزاع. |
| ملاحظة تدقيق امتثال | סב' 2 | جهة اتصال الامتثال | صياغة خطة معالجة، حفظ مذكرة تحت `docs/source/sns/regulatory/`, جدولة جلسة مجلس متابعة. |
| تدريب او بروفة | סוו 3 | PM البرنامج | تنفيذ السيناريو المبرمج من `ops/drill-log.md`, ارشفة الاثار، ووسم الفجوات كمهام في roadmap. |
