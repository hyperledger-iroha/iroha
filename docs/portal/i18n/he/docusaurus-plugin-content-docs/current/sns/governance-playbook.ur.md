...
| רשם ואופציות חיוב | `/v1/sns/*` endpoints چلاتے ہیں، payments reconcile کرتے ہیں، telemetry emit کرتے ہیں، اور CLI snapshots برقرار رکھتے ہیں۔ | Registrar API ([`registrar-api.md`](./registrar-api.md)), `sns_registrar_status_total` metrics، payment proofs جو `artifacts/sns/payments/*` میں محفوظ ہیں۔ | מנהל תורן רשם או קשר אוצר. |
...
| כתובת UX מדריך | [`address-display-guidelines.md`](./address-display-guidelines.md) | کینونیکل IH58 (ترجیحی) / compressed (`sora`) (`sora`, second-best) renderings جو wallets/explorers میں جھلکتے ہیں۔ |
...
6. **Customer disclosure:** Customer-facing ledger (wallet/explorer) کو
   ...
   עיבודי IH58 דחוסים (`sora`) העתק/QR
...
| כרטיס שומר | לוח שומר | `sns governance freeze --selector <IH58> --reason <text> --until <ts>` `GuardianFreezeTicketV1` بناتا ہے۔ Ticket JSON `artifacts/sns/guardian/<id>.json` میں رکھیں۔ | <=30 דקות ACK, <=2 שעות ביצוע. |
...
| ביטול הקפאה ותיקון | רשם + פותר פעולות | `sns governance unfreeze --selector <IH58> --ticket <id>` چلائیں، registrar status اپڈیٹ کریں، اور GAR/resolver diffs propagate کریں۔ | Verdict کے فوراً بعد۔ |
...
| `torii_request_duration_seconds{route="/v1/sns/*"}` | מדדי Torii | API handlers کے لئے latency SLOs؛ `torii_norito_rpc_observability.json` سے بنے dashboards کو feed کرتا ہے۔ |
