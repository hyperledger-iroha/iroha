...
| `payment_asset_id` | `AsciiString` | מזהה נכס settlement ברירת מחדל (למשל `xor#sora`). |
...
2. יצירה מחדש של resolver templates (כתובות IH58 מועדפות + compressed (`sora`) כאופציה שנייה, text records).
