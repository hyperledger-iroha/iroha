...
| רכיב | מטרה | ראיה |
|------|------|------|
| `sns_settlementd` | מיישם מדיניות, חותם על bundles, ומציג `/v1/sns/settlements`. | JSON bundle + hash. |
| Settlement queue & writer | תור idempotent + שולח ledger מונע על ידי `iroha_cli sns settlement ledger`. | bundle hash <-> tx hash manifest. |
