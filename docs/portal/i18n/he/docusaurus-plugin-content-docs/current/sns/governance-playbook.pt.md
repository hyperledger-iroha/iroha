...
| Guia UX de enderecos | [`address-display-guidelines.md`](./address-display-guidelines.md) | Renderizacoes canonicas IH58 (preferido) e comprimidas (segunda melhor opcao) refletidas por ארנקים/חוקרים. |
...
6. **Divulgacao ao cliente:** Atualize o Ledger voltado ao cliente (ארנק/חוקר)
   ...
   garantindo que renderizacoes IH58 e comprimidas correspondam and orientacoes de copy/QR.
...
| אפוטרופוס כרטיס | אפוטרופוס קונסלו | `sns governance freeze --selector <IH58> --reason <text> --until <ts>` פרודוז `GuardianFreezeTicketV1`. Armazene o JSON לעשות כרטיס עם `artifacts/sns/guardian/<id>.json`. | <=30 דקות ACK, <=2 שעות ביצוע. |
...
| Descongelar e remediar | רשם + ops de resolver | מפעיל `sns governance unfreeze --selector <IH58> --ticket <id>`, התקן את סטטוס הרשם וההפצה של GAR/פותר. | Imediatamente apos o veredito. |
