...
| `payment_asset_id` | `AsciiString` | התנחלות Актив по умолчанию (например `xor#sora`). |
...
2. תבניות פותר Перегенерация (IH58 предпочтительно + דחוס (`sora`) как второй выбор, רשומות טקסט).
