...
| Registrar & Billing Ops | מפעילים נקודות קצה `/v2/sns/*`, מאזנים תשלומים, משדרים טלמטריה ושומרים snapshots של CLI. | API של registrar ([`registrar-api.md`](./registrar-api.md)), מדדים `sns_registrar_status_total`, הוכחות תשלום תחת `artifacts/sns/payments/*`. | duty manager של registrar וקישור אוצרות. |
...
| `torii_request_duration_seconds{route="/v2/sns/*"}` | מדדי Torii | SLOs של לטנטיות ל-handlers של API; מזין דשבורדים הבנויים מ-`torii_norito_rpc_observability.json`. |
