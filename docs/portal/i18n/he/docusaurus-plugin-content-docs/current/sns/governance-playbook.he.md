...
| מדריך UX לכתובות | [`address-display-guidelines.md`](./address-display-guidelines.md) | רינדורים קנוניים של IH58 (מועדף) ודחוסים (אפשרות שנייה) שמוחזרים על ידי ארנקים/אקספלוררים. |
...
6. **חשיפת לקוח:** לעדכן את ledger ללקוחות (wallet/explorer) דרך fixtures משותפים
   ...
   שרינדורים IH58 ודחוסים תואמים להנחיות copy/QR.
...
| טיקט guardian | Guardian board | `sns governance freeze --selector <IH58> --reason <text> --until <ts>` מפיק `GuardianFreezeTicketV1`. לשמור את JSON הטיקט תחת `artifacts/sns/guardian/<id>.json`. | <=30 min ACK, <=2 h ביצוע. |
...
| הפשרת הקפאה ושיקום | Registrar + resolver ops | לבצע `sns governance unfreeze --selector <IH58> --ticket <id>`, לעדכן סטטוס registrar ולהפיץ דיפי GAR/resolver. | מייד לאחר פסק הדין. |
