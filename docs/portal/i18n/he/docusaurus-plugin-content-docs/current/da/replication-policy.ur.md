...
template کے ساتھ per-class overrides کا array فراہم کرتی ہے۔ מזהי כיתה
חסרי רגישות לאותיות רישיות, מכשירי `taikai_segment`, `nexus_lane_sidecar`,
`governance_artifact`, یا `custom:<u16>` (governance-approved extensions) قبول
‏ Storage classes `hot`, `warm`, یا `cold` قبول کرتی ہیں۔
...
`default_retention` edit کریں۔Taikai availability classes کو الگ سے override کیا جا سکتا ہے via
...
1. **Drift پر نظر رکھیں۔** Torii
   `overriding DA retention policy to match configured network baseline` פולט
...
2. **Intent بمقابلہ live replicas کا diff۔** نیا audit helper استعمال کریں:
   ...
   - `replica_shortfall` - סדר שכפול חי `RetentionPolicy.required_replicas`
