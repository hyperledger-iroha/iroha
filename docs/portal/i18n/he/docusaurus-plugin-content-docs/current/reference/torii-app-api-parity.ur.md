...
תאריך: מרץ 2026-03-21  
קבוצה: Torii Platform, מוביל תוכנית SDK  
...
- `crates/iroha_torii/src/routing.rs` מטפל ב-`crates/iroha_torii/src/routing.rs`.
- `app_api` یا `connect` فیچر گروپس کے تحت روٹر رجسٹریشن۔
- موجودہ انٹیگریشن/یونٹ ٹیسٹس اور طویل مدتی کوریج کے ذمہ دار ٹیم۔

עיון/לחץ אחורי עמודים/לחץ אחורי. پری فلٹرنگ کے لئے اختیاری `asset_id` query پیرامیٹرز قبول کرتی ہیں۔

## تصدیق اور کینونیکل دستخط

- ایپ کے لئے GET/POST اینڈپوائنٹس اختیاری کینونیکل ریکوئسٹ ہیڈرز (`X-Iroha-Account`, `X-Iroha-Signature`) قبول کرتے ہیں جو `METHOD\n/path\nsorted_query\nsha256(body)` سے بنائے جاتے ہیں؛ Torii מוציא לפועל כ-18NI000000044X מספר 1800000X کی عکاسی کریں۔
- SDK ہیلپرز تمام بنیادی کلائنٹس میں دستیاب ہیں:
  - JS/TS: `canonicalRequest.js` ו-`buildCanonicalRequestHeaders({ accountId, method, path, query, body, privateKey })`.
...
- בדיקות: `crates/iroha_torii/tests/accounts_endpoints.rs:126` אור `crates/iroha_torii/tests/account_query_subrouter_smoke.rs:146`.
...
- בעלים: Torii Platform.
- Notes: جواب کی سطح deterministic hex اور backend identifiers نافذ کرتی ہے؛ SDKs اس DTO کو استعمال کرتے ہیں۔### Proof SSE ایونٹس (`GET /v1/events/sse`) — کورڈ
- Handler: `handle_v1_events_sse` فلٹر سپورٹ کے ساتھ (`crates/iroha_torii/src/routing.rs:14008-14133`).
- DTOs: `EventsSseParams` (`crates/iroha_torii/src/routing.rs:14000-14006`) חיווט מסנן הוכחה.
- כריכת נתב: `Torii::add_app_api_routes` (`crates/iroha_torii/src/lib.rs:6678-6797`).
- Tests: proof مخصوص SSE suites (`crates/iroha_torii/tests/sse_proof_envelope_hash.rs`,
  `sse_proof_callhash.rs`, `sse_proof_verified_fields.rs`, `sse_proof_rejected_fields.rs`) בדיקת עשן צינור SSE
...
- מטפלים: `handle_post_vk_register`, `handle_post_vk_update`, `handle_post_vk_deprecate`
  (`crates/iroha_torii/src/routing.rs:4282-4382`) אור `handle_get_vk` (`crates/iroha_torii/src/routing.rs:4384-4418`).
...
### Nexus Connect (`/v1/connect/*`) — צילום (תכונה `connect`)
...
- Tests: `crates/iroha_torii/tests/connect_gating.rs` (feature gating, سیشن لائف سائیکل، WS handshake) اور
  תכונת הנתב מטריצה (`crates/iroha_torii/tests/router_feature_matrix.rs:804-876`).
...
- Notes: SSE stream عالمی broadcast چینل کو دوبارہ استعمال کرتا ہے جبکہ ٹیلیمیٹری پروفائل gating نافذ کرتا 20 response schemas `docs/source/torii/kaigi_telemetry_api.md` میں دستاویزی ہیں۔

## ٹیسٹ کوریج خلاصہ- Router smoke tests (`crates/iroha_torii/tests/router_feature_matrix.rs`) یقینی بناتے ہیں کہ feature combinations ہر route رجسٹر کریں اور OpenAPI generation sync میں رہے۔
