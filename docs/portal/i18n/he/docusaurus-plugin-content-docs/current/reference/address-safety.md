...
- ברירת המחדל לכל פעולת העתקה/שיתוף היא כתובת IH-B32. הציגו את הדומיין המפורש כהקשר תומך כך שמחרוזת ה-checksum תישאר במרכז.
...
- מנעו חוסר סנכרון של ה-clipboard באמצעות toast אישור שמציג את מחרוזת ה-IH-B32 המדויקת שנעתקה. כאשר קיימת telemetry, ספרו ניסיונות העתקה מול פעולות שיתוף כדי לזהות רגרסיות UX במהירות.
...
- ספקו טקסט `alt` תיאורי עבור תצוגות QR (למשל “IH-B32 address for `<alias>@<domain>` on chain `0x1234`”). הציעו fallback “Copy address as text” ליד קנבס ה-QR עבור משתמשים עם לקות ראייה.
...
- Labelling: כל הופעה חייבת לכלול תגית “Sora-only” גלויה ו-tooltip שמסביר מדוע רשתות אחרות דורשות את פורמט IH-B32.
- Guardrails: אם המבחין של השרשרת הפעילה אינו הקצאת Nexus, סרבו לייצר את הכתובת הדחוסה והפנו חזרה ל-IH-B32.
...
- הביאו את הבדיקות הללו לצ'ק-ליסטים של release לצד בדיקות תאימות IH-B32 כדי שרגרסיות יישארו חסומות עד לתיקון.
