...
- `GET /v2/puzzle/config` - מחזיר את פרמטרי ה-PoW/puzzle האפקטיביים שנמשכים
  מ-JSON של ה-relay (`handshake.descriptor_commit_hex`, `pow.*`).
- `POST /v2/puzzle/mint` - מנפיק טיקט Argon2; גוף JSON אופציונלי
...
- `GET /v2/token/config` - כאשר `pow.token.enabled = true`, מחזיר את מדיניות
  admission-token הפעילה (issuer fingerprint, גבולות TTL/clock-skew, relay ID,
  וסט revocation ממוזג).
- `POST /v2/token/mint` - מנפיק ML-DSA admission token הקשור ל-resume hash שסופק;
...
קובץ ה-revocation שומר על `/v2/token/config` עדכני; תיאמו עדכונים עם הפקודה
...
הציבורי ML-DSA-44 שמאמת signed PoW tickets; `/v2/puzzle/config` מהדהד את המפתח
...
`POST /v2/puzzle/mint` מקבל `"signed": true` (ואופציונלית `"transcript_hash_hex"`) כדי
...
4. **אימות.** הנפיקו טיקט דרך `POST /v2/puzzle/mint` ואשרו ש-`difficulty` ו-`expires_at`
   תואמים למדיניות החדשה. דו"ח ה-soak (`docs/source/soranet/reports/pow_resilience.md`)
   מתעד גבולות latency צפויים לעיון. כאשר tokens מופעלים, משכו `/v2/token/config`
...
4. עקבו אחרי `soranet_handshake_pow_difficulty` כדי לוודא שהקושי יורד לערך hashcash
   הצפוי, ואמתו ש-`/v2/puzzle/config` מדווח `puzzle = null`.
...
- **Puzzle alignment:** `soranet_handshake_pow_difficulty` צריך להתאים לקושי שמוחזר
  מ-`/v2/puzzle/config`. סטיה מצביעה על תצורת relay מיושנת או restart כושל.
- **Token readiness:** התריעו אם `/v2/token/config` יורד ל-`enabled = false` באופן
...
- **Service health:** בדקו `/healthz` בקצב liveness הרגיל והתריעו אם `/v2/puzzle/mint`
  מחזיר תגובות HTTP 500 (מצביע על mismatch בפרמטרי Argon2 או כשלי RNG). שגיאות
  token minting מופיעות דרך תגובות HTTP 4xx/5xx ב-`/v2/token/mint`; יש להתייחס
