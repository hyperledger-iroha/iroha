...
- Binarios `koto build`, `ivm_run` ו-`iroha_cli`. Puedes construirlos desde el checkout del workspace como se muestra abajo או הורדת חפצי חפצים של כתב שחרור:
...
El runner imprime el log `debug::info("Hello from Kotodama")` y ejecuta el syscall `SET_ACCOUNT_DETAIL` contra el host simulado. זה אופציונלי בינארי `ivm_tool` esta disponible, `ivm_tool inspect target/quickstart/hello.to` muestra el encabezado ABI, los bits de features y los entrypoints exportados.
