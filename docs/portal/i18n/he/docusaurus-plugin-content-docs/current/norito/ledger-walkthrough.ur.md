...
یہ walkthrough [Norito quickstart](./quickstart.md) کی تکمیل کرتا ہے اور دکھاتا ہے کہ `iroha` CLI کے ساتھ لیجر اسٹیٹ کو کیسے بدلیں اور چیک کریں۔ הגדרה של נכסים רגילה. יחידות מנטה. עסקאות אחזקות או עסקאות אחזקות. ‏ התחלות מהירות של Rust/Python/JavaScript SDK. تصدیق کر سکیں۔
...
`wonderland` נמכר ב-`coffee` של נכסי מטבעות:
...
עומסי מטענים של Norito וערכות SDK של 200 اگر آپ اس فلو کو کوڈ کے ذریعے دہرائیں (نیچے SDK quickstarts دیکھیں)، تو hashes اور balances اس وقت تک align رہیں گے جب تک آپ اسی نیٹ ورک اور defaults کو ٹارگٹ کرتے ہیں۔

## זוגיות SDK- [Rust SDK quickstart](../sdks/rust) — Rust سے instructions رجسٹر کرنا، transactions submit کرنا، اور status poll کرنا دکھاتا ہے۔
- [Python SDK quickstart](../sdks/python) — Norito-backed JSON helpers کے ساتھ وہی register/mint operations دکھاتا ہے۔
- [התחלה מהירה של JavaScript SDK](../sdks/javascript) — בקשות Torii, עוזרי ממשל, או עטיפות שאילתות מוקלדות.
