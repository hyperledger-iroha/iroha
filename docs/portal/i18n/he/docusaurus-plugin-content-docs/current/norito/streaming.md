...
- **Guardrails לקודק.** CABAC נשאר כבוי אלא אם הבילד מאפשר אותו; rANS באריזות דורש `ENABLE_RANS_BUNDLES=1`. אכיפה דרך `streaming.codec.{entropy_mode,bundle_width,bundle_accel}` ואופציונלית `streaming.codec.rans_tables_path` כאשר מספקים טבלאות מותאמות. `bundle_width` באריזה חייב להיות בין 2 ל-3 (כולל); רוחב 1 הוא legacy בלבד.
