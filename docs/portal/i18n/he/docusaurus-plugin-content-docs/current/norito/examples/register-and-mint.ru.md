...
- ודאו שחשבון היעד (לדוגמה `soraカタカナ...`) קיים, בדומה לשלב ההכנה בכל quickstart של ה-SDK.
- הפעילו את נקודת הכניסה `register_and_mint` כדי ליצור את הגדרת הנכס ROSE ולהטביע 250 יחידות עבור Alice בעסקה אחת.
- אמתו יתרות דרך `client.request(FindAccountAssets)` או `iroha_cli ledger assets list --account soraカタカナ...` כדי לוודא שההטבעה הצליחה.
