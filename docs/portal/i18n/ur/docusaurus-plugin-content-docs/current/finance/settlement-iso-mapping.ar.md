...
lang: ur
direction: rtl
source: docs/portal/docs/finance/settlement-iso-mapping.ar.md
status: complete
...
---
ID: تصفیہ-آئسو میپنگ
عنوان: تصفیہ ↔ آئی ایس او 20022 فیلڈ میپنگ
سائڈبار_لیبل: تصفیہ ↔ آئی ایس او 20022
تفصیل: Iroha آبادکاری کے بہاؤ اور آئی ایس او 20022 پل کے درمیان کیننیکل میپنگ۔
---
...
یہ نوٹ Iroha تصفیہ ہدایات کے مابین کیننیکل میپنگ کو اپنی گرفت میں لے رہا ہے
.
...
ان ذمہ داریوں سے کہ Norito ↔ ISO 20022 پل کو پیغامات خارج کرنے سے پہلے نافذ کیا جانا چاہئے۔
...
- ** آلہ (`FinInstrmId`) **
  ...
  -فال بیکس:
    - ** CUSIP ** یا `…/OthrId/Id` کے تحت `Tp/Cd` کے ساتھ بیرونی ISO سے سیٹ کریں
...
- ابان اور آئی ایس او 4217 پابندیوں کو اسی پرت میں نافذ کیا گیا ہے: Pacs.008/Pacs.009 اب بہاؤ
  ...
  اقدار کو جلد ہی مسترد کردیا جاتا ہے۔
- سی ایل آئی بستی ایک ہی گارڈ ریلوں کے وارث ہونے میں مدد کرتا ہے: پاس
...
- `cargo xtask iso-bridge-lint` (اور CI ریپر `ci/check_iso_reference_data.sh`) لنٹ
  ...
  دلائل کے بغیر۔
- IVM مددگار اب اصلی ISO 20022 XML لفافے (ہیڈ .001 + `DataPDU` + `Document`) کو گھٹا دیتا ہے۔
  ...
  بلاکس جان بوجھ کر چھوڑ دیتے ہیں۔ 

#### ریگولیٹری اور مارکیٹ ڈھانچے کے تحفظات- ** ٹی+1 تصفیہ **: یو ایس/کینیڈا ایکویٹی مارکیٹ 2024 میں ٹی+1 میں منتقل ہوگئی۔ Norito کو ایڈجسٹ کریں
...
- ** اسی دن کے تصفیے کے پائلٹ **: ہندوستان کا ریگولیٹر T0/T+0 تصفیہ میں مرحلہ وار ہے۔ رکھیں۔ رکھیں
...
> ** تصفیہ کوالیفائر ** - برج آئینہ مارکیٹ پریکٹس تصفیہ کی حالت کوڈ (`SttlmTxCond`) ، جزوی تصفیے کے اشارے (`PrtlSttlmInd`) ، اور Norito میٹا ڈیٹا سے دوسرے اختیاری کوالیفائر جب `sese.023/025` میں آئینہ دار ہے۔ آئی ایس او بیرونی کوڈ کی فہرستوں میں شائع ہونے والے گنتیوں کو نافذ کریں تاکہ منزل کا سی ایس ڈی اقدار کو تسلیم کرے۔
...
| مؤثر تاریخ (سرکاری مارجن شیڈول) | `Substitution/EffectiveDt` | ISO تاریخ (YYYY-MM-DD) |
...
### استعمال نوٹس* Norito عددی مددگاروں (`NumericSpec`) کا استعمال کرتے ہوئے تمام مقدار میں سیریلائز کیا جاتا ہے
...
* `SupplementaryData` کیننیکل JSON (UTF-8 ، ترتیب شدہ چابیاں ، JSON-Native ہونا چاہئے
  فرار)۔ ایس ڈی کے اس پر دستخط ، ٹیلی میٹری ہیشوں اور آئی ایس او کو نافذ کرنے میں مدد کرتا ہے
...
- لکس سی ایس ڈی/کلیئر اسٹریم آئی ایس او 20022 تصفیے کی مثالوں میں `SttlmTpAndAddtlParams/SctiesMvmntTp` (`DELI`/`RECE`) اور `Pmt` (Norito/`FREE`)۔
- کلیئر اسٹریم ڈی سی پی کی وضاحتیں طے شدہ کوالیفائر (`SttlmTxCond` ، `PrtlSttlmInd`) کا احاطہ کرتی ہیں
...
5. `sese.024` اسٹیٹس ایڈوائز (قبول ، مماثل ، زیر التوا ، ناکام ، مسترد ، مسترد)۔  
6. `sese.025` تصدیق ایک بار بک کروائیں۔  
...
#### مطلوبہ اعتراف / حیثیت
- ٹرانسپورٹ کی سطح: گیٹ ویز `admi.007` خارج کر سکتے ہیں یا بزنس پروسیسنگ سے پہلے مسترد کرسکتے ہیں۔  
...
** شرکاء: ** ہم منصب اور نقد ایجنٹ ، کسٹوڈین سیکیورٹیز ، سی ایس ڈی/ٹی 2 ایس  
...
1. `pacs.009` FI-TO-FI کریڈٹ ٹرانسفر فی کرنسی `CtgyPurp/Cd = SECU` کے ساتھ ؛ `pacs.002` کے ذریعے حیثیت ؛ `camt.056`/`camt.029` کے ذریعے یاد کریں/منسوخ کریں ؛ اگر پہلے ہی طے شدہ ہے تو ، `pacs.004` واپسی۔  
2. `sese.023` DVP ہدایات (S) `HldInd=true` کے ساتھ لہذا سیکیورٹیز کی ٹانگ نقد تصدیق کا انتظار کرتی ہے۔  
3. لائف سائیکل `sese.024` نوٹسز (قبول/مماثل/زیر التواء)۔  
4. اگر دونوں `pacs.009` ٹانگیں `ACSC` تک پہنچ جاتی ہیں تو اس سے پہلے ونڈو کی میعاد ختم ہوجاتی ہے I `sese.030` → `sese.031` (MOD کی حیثیت) → `sese.025` (تصدیق) کے ساتھ رہائی۔  
...
- `PrtlSttlmInd` غیر اعلانیہ پارٹیلز کو روکنے کے لئے۔  
...
- ریٹرن فکسچر اب `ChrgBr` کو معمول پر لائیں (`DEBT`/`CRED`/`SHAR`/`SLEV`) اور پریپریٹری ریٹرن کی وجوہات کو `TxInf[*]/RtrdRsn/Prtry` کے ذریعہ بے نقاب کیا جاسکتا ہے۔
...
- `sese.024`/`sese.025` اسٹیٹس اور `pacs.002` نتائج کو بطور گیٹنگ سگنلز کا علاج کریں۔ `ACSC` ٹرگرز ریلیز ، `RJCT` فورسز کھولیں۔  
- `HldInd` ، `Lnkgs` ، `PrtlSttlmInd` ، `SttlmTxCond` ، اور اختیاری COSD قواعد کے ذریعے انکوڈ مشروط ترسیل۔  
- جب ضرورت ہو تو بیرونی IDs (جیسے `pacs.009` کے لئے UERT) کو جوڑنے کے لئے `SupplementaryData` استعمال کریں۔  
- مارکیٹ کیلنڈر/کٹ آفس کے ذریعہ پیرامیٹرائز ہولڈ/انوائنڈ ٹائمنگ ؛ `sese.030`/`camt.056` کو منسوخ کرنے کی آخری تاریخ سے پہلے جاری کریں ، جب ضروری ہو تو واپسی پر فال بیک۔
...
````SctiesMvmntTp=RECE` ، `Pmt=FREE` ، اور `WITH` لنکج `SUBST-2025-04-001-A` کی طرف اشارہ کرنے والے `SctiesMvmntTp=RECE` ، اور `WITH` لنکج کے ساتھ منسلک ہدایات `SUBST-2025-04-001-B` (FOP وصول کریں) جمع کروائیں۔ ایک بار متبادل کی منظوری کے بعد دونوں ٹانگوں کو مماثل `sese.030` کے ساتھ جاری کریں۔

#### سیکیورٹیز کی ٹانگوں پر ہولڈ زیر التواء FX تصدیق (`sese.023` + `sese.030`)
...
`pacs.002` ادائیگی کی حیثیت سے پتہ چلتا ہے (`ACSC` = تصدیق شدہ ، `RJCT` = مسترد)۔ اگر ونڈو کی خلاف ورزی کی گئی ہے تو ، `camt.056`/`camt.029` کے ذریعے یاد کریں یا آباد فنڈز واپس کرنے کیلئے `pacs.004` بھیجیں۔