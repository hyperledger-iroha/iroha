...
-** کرنسی ** → ** آئی ایس او 4217 ** 3-لیٹر کوڈ ، متعلقہ معمولی یونٹ گول۔ [^iso_4217]
۔ پل
  `Purp=SECU` کی ضرورت ہوتی ہے اور جب حوالہ ڈیٹا تشکیل دیا جاتا ہے تو اب BIC کراس واک کو نافذ کرتا ہے۔

#### توثیق کے قواعد (اجراء سے پہلے درخواست دیں)
...
| ** قانون ** | ریجیکس `^[A-Z0-9]{18}[0-9]{2}$` اور MOD-97 چیک ہندسے (ISO 17442) | قبولیت سے پہلے گلف ڈیلی ڈیلٹا فائلوں کے خلاف توثیق کریں۔ [^gleif] |
...
| ** مالک اکاؤنٹ/پارٹی آئی ڈی ** | `Max35Text` (UTF-8 ، ≤35 حروف) تراشے ہوئے وائٹ اسپیس کے ساتھ | `GenericAccountIdentification1.Id` اور `PartyIdentification135.Othr/Id` فیلڈز پر لاگو ہوتا ہے۔ 35 حروف سے زیادہ اندراجات کو مسترد کریں لہذا پل پے لوڈز آئی ایس او اسکیموں کے مطابق ہیں۔ |
...
- آڈٹ کے لئے برج میٹا ڈیٹا میں ڈیٹا پروویژن (ٹائم اسٹیمپ + ماخذ) ریکارڈ کریں۔ مستقل
...
- ابان اور آئی ایس او 4217 پابندیوں کو اسی پرت میں نافذ کیا گیا ہے: Pacs.008/Pacs.009 اب بہاؤ
  ...
  کرنسی کا تصفیہ `currency_assets` سے غائب ہے ، جس سے خراب شدہ پل کی روک تھام ہے
...
- `cargo xtask iso-bridge-lint` (اور CI ریپر `ci/check_iso_reference_data.sh`) لنٹ
  کراس واک سنیپ شاٹس اور فکسچر۔ قبول کمانڈز `--isin` ، `--bic-lei` ، `--mic` ، اور
...
| `delivery_leg.from` (بیچنے والے/ترسیل پارٹی) | `DlvrgSttlmPties/Pty/Bic` | شریک کی فراہمی کا BIC * (اکاؤنٹ کیننیکل ID فی الحال میٹا ڈیٹا میں برآمد کیا جاتا ہے) * |
...
`sese.025` جاری کرنا جاری رکھے ہوئے ہے ، لیکن اس کی تصدیق سیکیورٹیز کی ٹانگ کی عکاسی کرتی ہے
...
- جاوا اسکرپٹ SDK اب `buildPacs008Message` / کو بے نقاب کرتا ہے
  `buildPacs009Message` (`javascript/iroha_js/src/isoBridge.js` دیکھیں) صرف کلائنٹ
...
- کاغذی پگڈنڈی برقرار رکھنے کے لئے حوالہ جات (`provide clause references`) کی درخواست کریں
...
1. `pacs.009` FI-TO-FI کریڈٹ ٹرانسفر فی کرنسی `CtgyPurp/Cd = SECU` کے ساتھ ؛ `pacs.002` کے ذریعے حیثیت ؛ `camt.056`/`camt.029` کے ذریعے یاد کریں/منسوخ کریں ؛ اگر پہلے ہی قائم ہوا ہے تو ، `pacs.004` واپسی۔  
...
5. اگر ایف ایکس ونڈو کی خلاف ورزی کی گئی ہے → منسوخ/یاد کیش (`camt.056/029` یا `pacs.004`) اور سیکیورٹیز (`sese.020` + `sese.027` ، یا `sese.026` الٹ سے منسوخ کریں تو اگر پہلے ہی مارکیٹ رول کے ذریعہ تصدیق شدہ ہے)۔
...
````SctiesMvmntTp=RECE` ، `Pmt=FREE` ، اور `WITH` لنکج `SUBST-2025-04-001-A` کی طرف اشارہ کرنے والے `SctiesMvmntTp=RECE` ، اور `WITH` لنکج کے ساتھ منسلک ہدایات `SUBST-2025-04-001-B` (FOP وصول کریں) جمع کروائیں۔ ایک بار متبادل کی منظوری کے بعد دونوں پیروں کو مماثل `sese.030` کے ساتھ جاری کریں۔
