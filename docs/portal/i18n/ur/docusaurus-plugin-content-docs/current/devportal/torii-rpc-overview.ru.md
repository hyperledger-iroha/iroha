...
Norito -RPC - API Torii کے لئے بائنری ٹرانسپورٹ۔ یہ وہی HTTP راستوں کا استعمال کرتا ہے جیسے `/v2/pipeline` ، لیکن اسکیما اور چیکسم ہیشس کے ساتھ Norito فریمنگ کا تبادلہ کرتا ہے۔ جب اس کا استعمال کریں جب عین مطابق اور توثیق شدہ ردعمل کی ضرورت ہو یا جب JSON پائپ لائن کے ردعمل رکاوٹ بن جائیں۔
...
2. اس صفحے یا `/reference/torii-swagger` پینل پر ** کارڈ کو ** آزمائیں اور ایک اختتامی نقطہ منتخب کریں ، مثال کے طور پر `POST /v2/pipeline/submit`۔
3.
