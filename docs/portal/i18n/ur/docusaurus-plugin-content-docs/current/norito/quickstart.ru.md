...
- بائنریز `koto build` ، `ivm_run` اور `iroha_cli`۔ انہیں چیک آؤٹ ورک اسپیس سے جمع کیا جاسکتا ہے ، جیسا کہ ذیل میں دکھایا گیا ہے ، یا ریلیز کے متعلقہ نمونے ڈاؤن لوڈ کریں:
...
رنر لاگ ان `debug::info("Hello from Kotodama")` اور مصنوعی میزبان کے خلاف سیسکل `SET_ACCOUNT_DETAIL` پر عمل درآمد کرتا ہے۔ اگر اختیاری `ivm_tool` بائنری دستیاب ہے تو ، `ivm_tool inspect target/quickstart/hello.to` کمانڈ ABI ہیڈر ، فیچر بٹس ، اور برآمد شدہ انٹری پوائنٹس کو دکھاتا ہے۔
