...
- بائنریز `koto build` ، `ivm_run` اور `iroha_cli`۔ آپ انہیں نیچے کی طرح ورک اسپیس چیک آؤٹ سے تعمیر کرسکتے ہیں یا اسی ریلیز نمونے ڈاؤن لوڈ کرسکتے ہیں:
...
رنر لاگ `debug::info("Hello from Kotodama")` پرنٹ کرتا ہے اور مصنوعی میزبان کے خلاف سیسکل `SET_ACCOUNT_DETAIL` انجام دیتا ہے۔ اگر اختیاری بائنری `ivm_tool` دستیاب ہے تو ، `ivm_tool inspect target/quickstart/hello.to` ABI ہیڈر ، فیچر بٹس اور برآمد شدہ انٹری پوائنٹ کو دکھاتا ہے۔
