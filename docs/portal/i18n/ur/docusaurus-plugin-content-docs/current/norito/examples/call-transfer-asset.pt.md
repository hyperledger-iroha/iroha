...
- معاہدہ اتھارٹی (مثال کے طور پر `<katakana-i105-account-id>`) کو اس اثاثہ کے ساتھ فنڈ دیں جو اس نے منتقل کیا ہے اور اتھارٹی کو کاغذ `CanTransfer` یا مساوی اجازت دیں۔
- معاہدہ اکاؤنٹ سے 5 یونٹوں کو `<katakana-i105-account-id>` میں منتقل کرنے کے لئے انٹری پوائنٹ `call_transfer_asset` پر کال کریں ، اس بات کی عکاسی کرتے ہوئے کہ آن چین آٹومیشن میں میزبان کالوں کو کس طرح شامل کیا جاسکتا ہے۔
- `FindAccountAssets` یا `iroha_cli ledger assets list --account <katakana-i105-account-id>` کے ذریعے توازن چیک کریں اور اس بات کی تصدیق کے لئے واقعات کا معائنہ کریں کہ میٹا ڈیٹا گارڈ نے منتقلی کے سیاق و سباق کو ریکارڈ کیا۔
