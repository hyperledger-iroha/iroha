...
- بلاک پر عمل درآمد فی لین/ڈیٹا اسپیس کی رسیدیں شامل کرتا ہے اور `lane_settlement_commitments` کے ذریعے `/v2/sumeragi/status` میں شائع کرتا ہے۔ کل میں `total_local_micro` ، `total_xor_due_micro` ، اور `total_xor_after_haircut_micro` نے رات کے مفاہمت کی برآمدات کے لئے بلاک میں شامل کیا۔
