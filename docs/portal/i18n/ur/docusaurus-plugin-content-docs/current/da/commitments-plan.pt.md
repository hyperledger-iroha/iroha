...
- سوالات کو بے نقاب کریں Torii (`/v2/da/commitments/*`) اور ایسے ثبوت جو ریلے کی اجازت دیتے ہیں ،
...
| ------ | -------- | --------- | ------- |
| `/v2/da/commitments` | `POST` | `DaCommitmentQuery` (رینج فلٹر بذریعہ لین/ایپوچ/تسلسل ، صفحہ بندی) | کل ، وعدوں اور بلاک ہیش کے ساتھ `DaCommitmentPage` واپس کرتا ہے۔ |
| `/v2/da/commitments/prove` | `POST` | `DaCommitmentProofRequest` (لین + منشور ہیش یا ٹپل `(epoch, sequence)`)۔ | `DaCommitmentProof` (ریکارڈ + مرکل پاتھ + بلاک ہیش) کے ساتھ جواب دیتا ہے۔ |
| `/v2/da/commitments/verify` | `POST` | `DaCommitmentProof` | اسٹیٹ لیس مددگار جو بلاک ہیش کو دوبارہ گنتی کرتا ہے اور اس میں شامل ہونے کی توثیق کرتا ہے۔ SDKs کے ذریعہ استعمال کیا جاتا ہے جو براہ راست `iroha_crypto` سے لنک نہیں کرسکتے ہیں۔ |
