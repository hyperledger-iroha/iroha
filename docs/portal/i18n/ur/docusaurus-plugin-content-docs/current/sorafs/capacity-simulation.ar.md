...
یہ گائیڈ وضاحت کرتا ہے کہ SF-2C صلاحیت مارکیٹ تخروپن سویٹ کو کیسے چلائیں اور اس کے نتیجے میں میٹرکس دیکھیں۔ `docs/examples/sorafs_capacity_simulation/` میں لازمی فکسچر کا استعمال کرتے ہوئے کوٹہ مذاکرات ، فیل اوور پروسیسنگ ، اور اختتام سے آخر تک سلیشنگ پروسیسنگ کے لئے چیک۔ صلاحیت کے پے لوڈ اب بھی `sorafs_manifest_builder capacity` استعمال کرتے ہیں۔ ظاہر/کار پیکیجنگ فلوز کے لئے `iroha app sorafs toolkit pack` استعمال کریں۔
...
`run_cli.sh` `sorafs_manifest_builder capacity` کو آؤٹ پٹ Norito پے لوڈ ، بیس 64 بلبس ، Torii درخواست باڈیوں ، اور JSON ڈائجسٹ کے لئے
