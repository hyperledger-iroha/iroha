...
## ٹولنگ اور فکسچر- وینڈر اشتہار پے لوڈ میں میٹا ڈیٹا `range_capability` شامل ہونا ضروری ہے ،
  `stream_budget` اور `transport_hints`۔ `/v2/sorafs/providers` اور کے جوابات کے ذریعے توثیق کریں
...
- Torii `/v2/sorafs/providers` پارسڈ رینج کی صلاحیت میٹا ڈیٹا کے ساتھ ساتھ
