...
1. ** وجہ کا تعین کریں **
   ...
   اگر بیک بلاگ مستحکم پاسوں کے ساتھ بڑھ رہا ہے تو ، بورڈ کی منظوری کے منتظر ظاہر ہونے کی تصدیق کے لئے داخلہ (`/v2/sorafs/pin/*`) چیک کریں۔
...
2. ** اسٹیجنگ میں خشک رن **
   ...
   -اختتامی مقامات `/v2/sorafs/pin/{digest}` اور `/v2/sorafs/aliases` مصنوعی ثبوتوں کے ساتھ تازہ ، ریفریش ونڈو ، میعاد ختم ہونے اور مشکل سے متاثرہ معاملات کا احاطہ کرتے ہیں۔ اس رن بک کے خلاف HTTP کوڈز ، ہیڈر (`Sora-Proof-Status` ، `Retry-After` ، `Warning`) اور باڈی JSON فیلڈز چیک کریں۔
