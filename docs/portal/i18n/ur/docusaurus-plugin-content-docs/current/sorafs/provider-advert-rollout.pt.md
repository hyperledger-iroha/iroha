...
2. ** فراہم کنندہ ٹولنگ کے ساتھ دوبارہ تخلیق کریں۔ **
   ...
   - `/v2/sorafs/providers` اور `sorafs_fetch` کے ذریعے توثیق کریں ؛ صلاحیتوں کے بارے میں انتباہات
...
- ** واقعہ کا جواب۔ ** اگر `reject` فائر ، آن کال انجینئرز:
  1. Torii دریافت (`/v2/sorafs/providers`) کے ذریعے جارحانہ اشتہار کی تلاش کریں۔
