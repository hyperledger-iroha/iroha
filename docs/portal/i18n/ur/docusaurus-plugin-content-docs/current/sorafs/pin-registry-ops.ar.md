...
1. ** وجہ کی شناخت کریں **
   ...
   - اگر بیک بلاگ مستحکم ناکامیوں کے ساتھ بڑھتا ہے تو ، بورڈ کی منظوری کے منتظر ظاہر ہونے کی تصدیق کے لئے قبولیت (`/v2/sorafs/pin/*`) چیک کریں۔
...
2. ** اسٹیجنگ میں خشک رن **
   ...
   -ٹیسٹ کے اختتامی مقامات `/v2/sorafs/pin/{digest}` اور `/v2/sorafs/aliases` ایک مصنوعی سابقہ ​​کے ساتھ تازہ ، ریفریش ونڈو ، میعاد ختم ہونے والی ، اور سخت مجبوری ریاستوں کا احاطہ کرتا ہے۔ اس ڈائرکٹری کے خلاف HTTP ہیڈرز (`Sora-Proof-Status` ، `Retry-After` ، `Warning`) اور JSON باڈی فیلڈز چیک کریں۔
