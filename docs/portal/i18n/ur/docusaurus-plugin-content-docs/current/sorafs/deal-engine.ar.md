...
- `POST /v2/sorafs/deal/usage` `DealUsageReport` ٹیلی میٹری کو قبول کرتا ہے اور واپسی کرتا ہے
  ڈٹرمینسٹک اکاؤنٹنگ کے نتائج (`UsageOutcome`)۔
- `POST /v2/sorafs/deal/settle` موجودہ ونڈو ، اور نشریات کو ختم کرتا ہے
  `DealSettlementV1` کے ساتھ ساتھ `DealSettlementRecord` بیس 64 انکوڈڈ ہیں۔
  اور ڈی اے جی گورننس میں تعیناتی کے لئے تیار ہے۔
- `/v2/events/sse` Torii میں اب فیڈز میں `SorafsGatewayEvent::DealUsage` میں رجسٹر ہوتا ہے
