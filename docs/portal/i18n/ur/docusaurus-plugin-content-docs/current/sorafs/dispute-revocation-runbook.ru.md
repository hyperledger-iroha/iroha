...
- ** ٹیلی میٹری کی تصدیق کریں: ** سنیپ شاٹس `/v1/sorafs/capacity/state` اور `/v1/sorafs/capacity/telemetry` کو فراہم کنندہ سے وابستہ کریں۔
...
4. گورننس ٹرانزیکشن قطار کے ذریعے Torii `/v1/sorafs/capacity/dispute` پر JSON کی درخواست بھیجیں۔ ردعمل کی قیمت `dispute_id_hex` پر قبضہ کریں ؛ یہ اس کے بعد کے اعمال اور آڈٹ رپورٹس کو لنگر انداز کرتا ہے۔
...
2. ** `ProviderAdmissionRevocationV1` پیدا کریں: **
   - کسی منظور شدہ وجہ کے ساتھ `sorafs_manifest_builder provider-admission revoke` استعمال کریں۔
