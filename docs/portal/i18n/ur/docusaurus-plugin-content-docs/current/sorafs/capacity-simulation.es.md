...
اس رن بک میں بتایا گیا ہے کہ SF-2C صلاحیت مارکیٹ پلیس تخروپن کٹ کو کیسے چلائیں اور اس کے نتیجے میں میٹرکس دیکھیں۔ `docs/examples/sorafs_capacity_simulation/` میں ڈٹرمینسٹک فکسچر کا استعمال کرتے ہوئے کوٹہ مذاکرات ، فیل اوور مینجمنٹ ، اور اختتام سے آخر تک ختم ہونے والے تدارک کی توثیق کرتا ہے۔ صلاحیت کے پے لوڈ اب بھی `sorafs_manifest_builder capacity` استعمال کرتے ہیں۔ منشور/کار پیکیجنگ بہاؤ کے لئے `iroha app sorafs toolkit pack` استعمال کرتا ہے۔
...
`run_cli.sh` `sorafs_manifest_builder capacity` کو Norito پے لوڈز ، بیس 64 بلبس ، Torii کے لئے باڈیز کی درخواست کریں ، اور JSON ڈائجسٹس کے لئے درخواست کریں:
