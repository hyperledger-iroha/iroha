...
- تصدیق کریں کہ نوڈ ابھی تک `GET /v1/sorafs/capacity/state` کے ذریعہ متوقع صلاحیت کا اعلان کرتا ہے
  اجازت نامہ درج کریں۔
...
4. `GET /v1/sorafs/storage/state` چیک کریں تاکہ یہ یقینی بنایا جاسکے کہ `bytes_used` کی عکاسی ہوتی ہے
...
- مانیٹرنگ بورڈز کی پیروی کرنی چاہئے:
  ...
  - `/v1/sorafs/capacity/state` کے ذریعے دکھائے جانے والے پور کامیابی/ناکامی کاؤنٹرز
