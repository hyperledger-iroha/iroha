...
| ------------ | ------------- | ----------------- |
| خدمت Torii | `/v2/sorafs/pin` (جمع کروائیں) ، `/v2/sorafs/pin/{cid}` (تلاش) ، `/v2/sorafs/aliases` (فہرست/پابند) ، `/v2/sorafs/replication` (آرڈرز/رسیدیں) کو بے نقاب کریں۔ صفحہ بندی + فلٹرنگ فراہم کریں۔ | TL نیٹ ورکنگ / کور انفرا |
...
- `GET /v2/sorafs/pin` اور `GET /v2/sorafs/pin/{digest}` کے ساتھ ظاہر ہوتا ہے
  ...
  آخری بلاک کا ہیش۔
- `GET /v2/sorafs/aliases` اور `GET /v2/sorafs/replication` کی کیٹلاگ کو بے نقاب کریں
