...
1. ** وجہ کی شناخت کریں **
   ...
   اگر بیک بلاگ مستحکم مشنوں کے ساتھ بڑھتا ہے تو ، بورڈ کی منظوری کے منتظر ظاہر ہونے کی تصدیق کے لئے انٹیک (`/v2/sorafs/pin/*`) کا معائنہ کریں۔
...
2. ** اسٹیجنگ میں خشک رن **
   ...
   -`/v2/sorafs/pin/{digest}` اور `/v2/sorafs/aliases` اختتامی نکات کو مصنوعی ٹیسٹوں کے ساتھ تازہ ، تروتازہ ونڈو ، میعاد ختم ہونے اور سخت مبتلا معاملات کا احاطہ کرتا ہے۔ اس رن بک کے خلاف HTTP کوڈز ، ہیڈرز (`Sora-Proof-Status` ، `Retry-After` ، `Warning`) اور JSON باڈی فیلڈز کی توثیق کریں۔
