...
|------|----------|------------------|-------|
| 2026-02-12 | Tooling (LLM) | ❌ Failed | Step 1: `cargo test -p sorafs_chunker` `vectors` suite میں fail ہوتا ہے کیونکہ fixtures ابھی بھی legacy handle `sorafs.sf1@1.0.0` publish کرتے ہیں اور profile aliases/digests موجود نہیں (`fixtures/sorafs_chunker/sf1_profile_v1.*`). Step 2: `ci/check_sorafs_fixtures.sh` abort ہوتا ہے — repo state میں `manifest_signatures.json` missing ہے (working tree میں delete ہوا). Step 4: `export_vectors` signatures verify نہیں کر سکتا جب تک manifest فائل موجود نہ ہو۔ سفارش: signed fixtures restore کریں (یا council key فراہم کریں) اور bindings regenerate کریں تاکہ canonical handle + alias array tests کے مطابق embed ہو جائے۔ |
