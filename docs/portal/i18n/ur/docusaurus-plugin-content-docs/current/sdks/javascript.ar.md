...
`getExplorerAccountQr()` جب بھی آپ کو i105 (ترجیحی)/سورہ (دوسرا بہترین) لٹریلس پلس آن لائن کی ضرورت ہو
...
ان کلائنٹ مددگاروں کے ساتھ ساتھ پورٹل اور ماخذ دستاویزات ہم آہنگی میں رہیں۔
...
`fixtures/space_directory/` کے تحت فکسچر) یا کوئی شے جو سیریلائز کرتا ہے
...
1. `getSumeragiRbcSessions()` آئینہ `/v1/sumeragi/rbc/sessions` ، اور
   ...
   (انضمام سویٹ جب بھی اس میں واپس آتا ہے
...
4. `getSumeragiRbcDelivered(height, view)` نے آڈیٹرز کو میٹا ڈیٹا کی ہم آہنگی کی فراہمی کو اپنی گرفت میں لے لیا
...
3. سگریٹ نوشی کی نوکریوں کے لئے `npm test` (یا `node --test`) پر عمل کریں۔
...
- `javascript/iroha_js/index.d.ts` میں پیدا ہونے والی اقسام کا جائزہ لیں۔
