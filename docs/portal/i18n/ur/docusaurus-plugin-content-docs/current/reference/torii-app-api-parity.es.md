...
یہ صفحہ داخلی آڈٹ `TORII-APP-1` (`docs/source/torii/app_api_parity_audit.md`) کی عکاسی کرتا ہے تاکہ مونو-ریپو سے باہر کے قارئین دیکھ سکیں کہ کون سی `/v2/*` سطحیں وائرڈ ، ٹیسٹ اور دستاویزی دستاویزات ہیں۔ آڈٹ میں `Torii::add_app_api_routes` ، `add_contracts_and_vk_routes` اور `add_connect_routes` کے ذریعے دوبارہ برآمد شدہ راستوں کا سراغ لگایا گیا ہے۔

## دائرہ کار اور طریقہ

آڈٹ `crates/iroha_torii/src/lib.rs:256-522` میں عوامی دوبارہ برآمدات کا معائنہ کرتا ہے اور فیچر گیٹنگ کے ساتھ روٹ کنسٹرکٹرز۔ روڈ میپ کے ہر سطح `/v2/*` کے لئے ہم تصدیق کرتے ہیں:
...
- مالک: Torii پلیٹ فارم۔
۔ ایس ڈی کے ڈی ٹی او کا استعمال کرتے ہیں۔### ایس ایس ای پروف ایونٹس (`GET /v2/events/sse`) - احاطہ کرتا ہے
