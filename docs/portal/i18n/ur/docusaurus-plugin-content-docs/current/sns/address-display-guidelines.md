...
- **دو کاپی اہداف۔** دو واضح کاپی بٹنز دیں: IH58 (ڈیفالٹ) اور صرف Sora والا
  کمپریسڈ فارم (`snx1...`). IH58 ہمیشہ بیرونی شیئرنگ کے لئے محفوظ ہے اور QR
...
- کاپی بٹنز پر `data-copy-mode="ih58|compressed|qr"` لگائیں تاکہ فرنٹ اینڈز Torii
...
[Local -> Global toolkit](local-to-global-toolkit.md) استعمال کریں تاکہ legacy
...
|---------------|---------------|
| ضمنی ڈیفالٹ | `0x0200000120641297079357229f295938a4b5a333de35069bf47b9d0704e45805713d13c201` |
...
legacy Local encodings کو canonical IH58 یا کمپریسڈ strings میں بدلنے والے
...
3. canonical payload کو `ih58.value` یا `compressed` فیلڈز سے reuse کر کے تبدیل
...
5. بلک ڈیٹا سیٹس کے لئے
   ...
   اندراج کے لئے canonical/IH58/compressed summaries کے ساتھ JSON رپورٹ بناتی
   ہے، اور parse errors و Local-domain warnings گنتی ہے۔ legacy dumps میں junk
...
6. اگر newline-to-newline rewrite چاہیے تو
  `iroha address normalize --input legacy.txt --only-local --append-domain --network-prefix 753 --format ih58 --output normalized.txt` استعمال کریں۔
  ...
  non-Local rows چھوڑ دیتا ہے، باقی entries کو مطلوبہ encoding (IH58/compressed/hex/JSON)
...
`domain_kind="local12"` مطلوبہ 30 دن کی ونڈو تک صفر رہا، پھر mainnet legacy
...
> والٹ/ایکسپلورر pipelines legacy Local selectors کو canonical IH58/کمپریسڈ فارمز
