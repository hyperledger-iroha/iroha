...
- `GET /v2/puzzle/config` - نتیجے میں موثر POW/پہیلی پیرامیٹرز کو لوٹاتا ہے
...
- `POST /v2/token/mint` - ایک ML -DSA داخلہ کا اخراج کرتا ہے جو دوبارہ شروع ہیش سے منسلک ہوتا ہے
...
آؤٹ آف بینڈ پائپ لائن۔ منسوخی فائل دیکھنے والا `/v2/token/config` رکھتا ہے
...
`pow.signed_ticket_public_key_hex`۔ `POST /v2/puzzle/mint` `"signed": true` کو قبول کرتا ہے
...
4. ** توثیق کریں۔ ** `POST /v2/puzzle/mint` کے ذریعے ٹکٹ جاری کریں اور اس کی تصدیق کریں
   ...
   `/v2/token/config` اس بات کی تصدیق کرنے کے لئے کہ فنگر پرنٹ جاری کرنے والا اعلان کرتا ہے اور
...
4. اس مشکل کی تصدیق کرنے کے لئے `soranet_handshake_pow_difficulty` کی نگرانی کریں
   متوقع ہیشکاش ویلیو پر آتا ہے ، اور اس کی توثیق کرتا ہے کہ `/v2/puzzle/config`
...
- ** پہیلی سیدھ: ** `soranet_handshake_pow_difficulty` کے مطابق ہونا چاہئے
  `/v2/puzzle/config` کے ذریعہ لوٹ آیا۔ ایک تضاد ایک تشکیل کی نشاندہی کرتا ہے
...
۔
  انتباہ اگر `/v2/puzzle/mint` HTTP 500 جوابات واپس کرتا ہے (ایک مماثلت کی نشاندہی کرتا ہے
  ارگون 2 پیرامیٹرز یا آر این جی کی ناکامی)۔ ٹوکن کان کنی کی غلطیاں ہوتی ہیں
  `/v2/token/mint` پر HTTP 4XX/5XX جوابات کے ذریعے منشور ؛ ان کے ساتھ سلوک کرو
