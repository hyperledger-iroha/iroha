...
- レーン/データスペースと `lane_settlement_commitments` と `/v2/sumeragi/status` のレシートを確認します。 Итоги раскрывают `total_local_micro`, `total_xor_due_micro` および `total_xor_after_haircut_micro`, суммированные по блоку для ночных выгрузокね。
