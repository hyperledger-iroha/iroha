...
`/v1/pipeline/transactions`経由のtransacao。おお、献身的な報酬を認めてくれる助け人よ、
...
3. **パブリック ガイド パラ オペラドール:** レジストラ、`transition_id` の回覧、オン/オフランプのランブック。財布は assinam `/v1/confidential/assets/{id}/transitions` を購入し、さまざまな種類の財布を購入できます。
