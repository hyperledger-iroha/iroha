...
- Cada transacao que paga ガス登録 `LaneSettlementReceipt`。正確な情報、オリジェム フォルネシド ペロ シャマドールの識別子、マイクロ ヴァラー ローカル、XOR デビド イメディアタメンテ、XOR エスペラード アポス、ヘアカット、バリアント リアルリザダ (`xor_variance`)、ミリセグンドスのタイムスタンプ。
- `lane_settlement_commitments` または `/v1/sumeragi/status` を介して、レーン/データスペースの集合データを公開します。ほとんどの情報は `total_local_amount`、`total_xor_due`、`total_xor_after_haircut` であり、輸出に関する報告書は報告されません。
- 新たなコンタドール `total_xor_variance` ラストレイア クォント デ マージェム デ セグランカ フォイ コンスミダ (ディフェレンカ エントレ オ XOR デビド エ オ エスペラード pos-haircut)、`swap_metadata` ドキュメント オス パラメトロス デ コンバーサオ デターミニスティック (TWAP、イプシロン、流動性プロファイルvolatility_class) は、ランタイムの構成を独立して管理するための監査権限を持っています。
