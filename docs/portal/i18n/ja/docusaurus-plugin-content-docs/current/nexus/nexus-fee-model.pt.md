...
- `lane_settlement_commitments` または `/v2/sumeragi/status` を介して、レーン/データスペースの集合データを公開します。ほとんどの情報は `total_local_micro`、`total_xor_due_micro`、`total_xor_after_haircut_micro` であり、輸出に関する報告書は報告されません。
