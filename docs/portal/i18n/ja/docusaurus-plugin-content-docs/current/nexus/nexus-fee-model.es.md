...
- `lane_settlement_commitments` および `/v2/sumeragi/status` 経由で、レーン/データスペースの集合体が公開されます。損失の合計指数 `total_local_micro`、`total_xor_due_micro`、y `total_xor_after_haircut_micro` は、夜行性の輸出と和解のための非常に複雑なブロックです。
