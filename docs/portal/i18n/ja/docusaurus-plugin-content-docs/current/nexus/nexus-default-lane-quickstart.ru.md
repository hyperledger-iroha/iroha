...
- **パイプライン/SSE テスト。** Фильтры событий транзакций принимают предикаты `tx_lane_id == <u32>` (см. `docs/source/pipeline.md`)。 `/v2/pipeline/events/transactions` は、этим фильтром, чтобы доказать, что записи, отправленные без явного lane, приходятフォールバック レーン ID。
