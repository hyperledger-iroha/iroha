...
- Approvisionnez l'autorité du contrat (par example `<katakana-i105-account-id>`) avec l'actif qu'elle transfer et accordez-lui le rôle `CanTransfer` ou une permission équivalente.
- Appelez le point d'entrée `call_transfer_asset` pour transferer 5 Unités du compte du contrat vers `<katakana-i105-account-id>`、en reflétant la manière dont l'automatization on-chain peut encapsuler des appels hôte。
- `FindAccountAssets` または `iroha_cli ledger assets list --account <katakana-i105-account-id>` を介して、情報を確認し、ジャーナリゼ ル コンテキスト デュ トランスファーを検査します。
