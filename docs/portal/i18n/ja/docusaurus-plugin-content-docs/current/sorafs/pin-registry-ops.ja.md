...
3. **複製の再割り当て**
   - backlog の余裕 (`stat="avg"`) が 5 エポック未満になったら `sorafs_manifest_builder capacity replication-order` で新しいオーダーを発行 (manifest/CAR のパッケージングは `iroha app sorafs toolkit pack`)。
