...
- プロファイル・ハンドル: `sorafs.sf1@1.0.0`（legacy alias `sorafs-sf1`）
...
- `sf1_profile_v1.{json,rs,ts,go}` — Rust/TypeScript/Go の消費者向けの正規チャンク境界。
  各ファイルは `profile_aliases` の先頭に正規ハンドルを宣言し、続いて legacy alias
  （例: `sorafs.sf1@1.0.0` の後に `sorafs-sf1`）を記載します。順序は
