...
**アーティファクト:**
- ソース CLI とテスト (`crates/sorafs_orchestrator/src/bin/sorafs_cli.rs`)
...
- 決定的なパリティのハーネス (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`、[パリティの関係 GA SoraFS オーケストレーター](./orchestrator-ga-parity.md))
...
| SF6-SR-03 |モエンヌ |マニフェスト使節 | CLI は、検証者なしでマニフェストを受け入れ、チャンク プランは、`--plan` が存在しません。 | `sorafs_cli manifest submit` CAR のダイジェストを再計算して比較します。 `--expect-plan-digest` 4 つを調べ、不一致と修復のヒントを説明します。成功/診断テストのテスト (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`)。 |
...
4. ハーネスのキャプチャをログに記録します (`cargo test -p sorafs_orchestrator --test sorafs_cli proof_stream_consumes_ndjson_and_reports_metrics -- --nocapture`)。
