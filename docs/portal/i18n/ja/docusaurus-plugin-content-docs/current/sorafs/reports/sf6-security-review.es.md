...
**アーティファクト:**
- CLI テスト (`crates/sorafs_orchestrator/src/bin/sorafs_cli.rs`)
...
- ハーネス・デ・パリダード・デターミニスタ (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`、[Reporte de Paridad GA del Orchestrator SoraFS](./orchestrator-ga-parity.md))
...
| SF6-SR-03 |メディア |マニフェスト環境 | El CLI の aceptaba は、チャンクの検証面、`--plan` estaba ausente を明示します。 | `sorafs_cli manifest submit` CAR サルボの再計算と比較ダイジェストの証明 `--expect-plan-digest`、修復の不一致と修正のほとんどのピスタ。 Los テスト cubren casos de exito/falla (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`)。 |
