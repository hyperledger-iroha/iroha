...
**アーティファクト:**
- CLI ソースとテスト (`crates/sorafs_orchestrator/src/bin/sorafs_cli.rs`)
...
- Deterministic parity harness (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`, [SoraFS Orchestrator GA パリティレポート](./orchestrator-ga-parity.md))
...
| SF6-SR-03 | Medium | Manifest submission | `--plan` 不在時に CLI が embedded chunk plans を検証せずに manifests を受理。 | `sorafs_cli manifest submit` が `--expect-plan-digest` 未指定なら CAR digests を再計算して比較し、mismatch を拒否して remediation hints を表示。テストは成功/失敗ケースをカバー (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`)。 |
...
4. Parity harness のログ (`cargo test -p sorafs_orchestrator --test sorafs_cli proof_stream_consumes_ndjson_and_reports_metrics -- --nocapture`)。
