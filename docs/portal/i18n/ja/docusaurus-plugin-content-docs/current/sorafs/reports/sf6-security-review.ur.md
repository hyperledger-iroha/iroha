...
**アーティファクト:**
- CLI ソースのテスト (`crates/sorafs_orchestrator/src/bin/sorafs_cli.rs`)
...
- 確定的パリティ ハーネス (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`、[SoraFS Orchestrator GA パリティ レポート](./orchestrator-ga-parity.md))
...
| SF6-SR-03 |中 |マニフェストの提出 | CLI マニフェスト 埋め込みチャンク プランの検証 `--plan` موجود نہ تھا۔ | `sorafs_cli manifest submit` CAR ダイジェストの計算 比較の計算 `--expect-plan-digest` 不一致の拒否 修復のヒントدکھاتا ہے۔テストの成功/失敗のケースは、(`crates/sorafs_orchestrator/tests/sorafs_cli.rs`) をカバーしています。 |
...
4. パリティ ハーネスのキャプチャされたログ (`cargo test -p sorafs_orchestrator --test sorafs_cli proof_stream_consumes_ndjson_and_reports_metrics -- --nocapture`)
