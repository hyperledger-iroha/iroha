...
** セレクタ:** مقطع `{selector}` يقبل IH58 او مضغوط او 16 進数 قياسي حسب ADDR-5; Torii يطبعها عبر `NameSelectorV1`。
...
2. `RegisterNameRequestV1`:
   - `selector` مشتق من ラベル IH58 (المفضل) او المضغوط (الخيار الثاني)。
   - `term_years` 認証済み。
