...
| コンポーネント | 目的 | 証拠 |
|----------------|------|------|
| `sns_settlementd` | ポリシー適用、バンドル署名、`/v1/sns/settlements` の公開。 | JSON バンドル + hash. |
| Settlement queue & writer | idempotent キュー + `iroha_cli sns settlement ledger` による ledger 提出。 | bundle hash <-> tx hash manifest. |
