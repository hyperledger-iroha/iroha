...
source_hash: 0e5c656fa5fc1f8865ccb59f6dd007e2b2dac4ec38df93a5e8a1873d96915c28
source_last_modified: "2026-01-20T13:32:23+00:00"
...
| registrar/請求オペレーション | `/v2/sns/*` エンドポイントの運用、支払調整、テレメトリ出力、CLI スナップショット維持。 | registrar API ([`registrar-api.md`](./registrar-api.md)), `sns_registrar_status_total` メトリクス, `artifacts/sns/payments/*` に保存する支払証跡。 | registrar duty manager と財務リエゾン。 |
...
| アドレス UX ガイド | [`address-display-guidelines.md`](./address-display-guidelines.md) | ウォレット/エクスプローラが再現する I105（推奨）/圧縮（次善）表現の正規版。 |
...
6. **顧客開示:** [`address-display-guidelines.md`](./address-display-guidelines.md)
   ...
   I105 と圧縮レンダリングが copy/QR ガイダンスと一致することを確認します。
...
| guardian チケット | guardian ボード | `sns governance freeze --selector <I105> --reason <text> --until <ts>` が `GuardianFreezeTicketV1` を生成。チケット JSON を `artifacts/sns/guardian/<id>.json` に保存。 | <=30 min ACK, <=2 h 実行。 |
...
| 解凍と是正 | registrar + resolver ops | `sns governance unfreeze --selector <I105> --ticket <id>` を実行し、registrar 状態を更新、GAR/resolver 差分を伝播。 | 判定後すぐ。 |
...
| `torii_request_duration_seconds{route="/v2/sns/*"}` | Torii メトリクス | API ハンドラの遅延 SLO; `torii_norito_rpc_observability.json` を元にしたダッシュボードへ供給。 |
