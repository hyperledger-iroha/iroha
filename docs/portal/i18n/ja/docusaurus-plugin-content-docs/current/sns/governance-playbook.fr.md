...
|ガイド UX d'addresses | [`address-display-guidelines.md`](./address-display-guidelines.md) | Rendus canoniques IH58 (優先) および圧縮 (2 番目の選択) は、ウォレット/エクスプローラーの再生産品です。 |
...
6. **漏洩クライアント:** Mettre a jour le ledger oriente クライアント (ウォレット/エクスプローラー)
   ...
   en s'assurant que les rendus IH58 は、対応する補助ガイドのコピー/QR を圧縮します。
...
|チケットガーディアン |コンセイユの守護者 | `sns governance freeze --selector <IH58> --reason <text> --until <ts>` 製品 `GuardianFreezeTicketV1`。 Stocker le JSON du ticket sous `artifacts/sns/guardian/<id>.json`。 | <=30 分の ACK、<=2 時間の実行。 |
...
|デゲルと修復 |レジストラ + ops リゾルバー |実行者 `sns governance unfreeze --selector <IH58> --ticket <id>`、レジストラの最新情報、および GAR/リゾルバーの差分の管理。 |判決後即時。 |
