...
| `payment_asset_id` | Yes | settlement アセット (例: `61CtjvNd9T3THAR65GsMVHr82Bjc`). |
...
- ヘルパーは各リクエストに対して `POST /v1/sns/names` を送信し、
  最初の HTTP エラーで停止します。レスポンスは NDJSON としてログに追記。
- `--poll-status` は送信後に `/v1/sns/names/{namespace}/{literal}` を再照会
...
- **Controllers:** 空セルは `--default-controllers` に従います。非 owner へ委任
  する場合は `<katakana-i105-account-id>;<katakana-i105-account-id>` など明示的なリストを指定します。
