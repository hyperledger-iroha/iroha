...
- **Две цели копирования.** Предоставьте две явные кнопки копирования: I105
  (предпочтительно) и сжатая Sora のみのформа (`sora...`, второй по предпочтению)。 I105 のレビュー
...
Каждый SDK предоставляет удобный ヘルパー、возвращающий формы I105、сжатую、а
...
Обозреватели должны отражать телеметрию и доступность колелька:- Добавляйте `data-copy-mode="i105|i105_default|qr"` на кнопки копирования, чтобы
...
- Сопоставляйте каждому контролу отдельные `aria-label`/`aria-describedby`、
  объясняющие, безопасен ли リテラル для обмена (I105) или только для Sora
...
Операторы、конвертирующие устаревbolие ローカル кодировки в канонический I105 или
...
1. `iroha tools address inspect` теперь выдает структурированное JSON-резюме с I105,
   ...
   конвертированный エンコーディング как `<i105>@<domain>`。
2. SDK の警告/概要 JavaScript ヘルパー:
   ...
   ```
  ヘルパー сохраняет I105 префикс、извлеченный из リテラル、если только вы явно не
  указали `networkPrefix`, поэтому резюме для не-default сетей не перерисовываются
  тихо с дефолтным префиксом.3. Конвертируйте канонический ペイロード、повторно используя `i105.value` или
   `i105_default` は、`--format` です。 Эти
...
5. あなたのことを忘れないでください。
   ...
   каноническими/I105/сжатыми резюме для каждой записи и считает обки парсинга
...
6. Когда нужна построчная перепись, используйте
  ...
  кодировки、предупреждения и озибки парсинга за один проход。
   ...
   エンコーディング (I105/сжатый/hex/JSON) と сохраняет исходный домен の組み合わせ
...
> I105/ローカル-8/ローカル-12 メインネット。 Обновите любые
