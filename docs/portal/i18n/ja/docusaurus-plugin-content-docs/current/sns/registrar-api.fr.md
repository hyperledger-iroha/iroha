...
|エクシジェンス |詳細 |
|----------|----------|
|プロトコル | REST は `/v1/sns/*` であり、サービスは gRPC `sns.v1.Registrar` です。レドゥ受け入れ Norito-JSON (`application/json`) および Norito-RPC バイナリ (`application/x-norito`)。 |
...
1. 統治台帳上に存在する提案書の識別子 (`/v1/governance/proposals/{id}`) および法定規則 `Approved`。
2. 投票記録の特派員。
...
- Les preuves de paiement reutilisent les recus du ledger Norito; API ヘルパーの 4 つのアクセス サービス (`/v1/finance/sns/payments`)。
