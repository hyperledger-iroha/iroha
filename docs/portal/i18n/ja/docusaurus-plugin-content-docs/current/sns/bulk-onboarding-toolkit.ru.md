...
| `payment_asset_id` | Да | Актив 和解 (например `61CtjvNd9T3THAR65GsMVHr82Bjc`)。 |
...
- `--poll-status` は、`/v1/sns/names/{namespace}/{literal}` を表示します
...
- **コントローラー:** は `--default-controllers` です。 Указывайте
  явные списки コントローラー (например `<katakana-i105-account-id>;<katakana-i105-account-id>`) の所有者です。
