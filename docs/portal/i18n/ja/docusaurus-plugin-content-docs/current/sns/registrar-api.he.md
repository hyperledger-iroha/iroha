...
lang: he
direction: rtl
source: docs/portal/i18n/ja/docusaurus-plugin-content-docs/current/sns/registrar-api.md
status: complete
generator: scripts/sync_docs_i18n.py
source_hash: 0bacc88d8790e56999b9c81b0c9358dfa9bae388410f0fe5ef3fe38e768a9b02
source_last_modified: "2025-11-14T04:43:21.375765+00:00"
...
| 要件 | 詳細 |
|------|------|
| プロトコル | REST は `/v1/sns/*`、gRPC サービスは `sns.v1.Registrar`。どちらも Norito-JSON (`application/json`) と Norito-RPC バイナリ (`application/x-norito`) を受け付けます。 |
...
**セレクタのエンコード:** `{selector}` パスセグメントは ADDR-5 に従う IH58/圧縮/正規 hex を受け付け、Torii が `NameSelectorV1` で正規化します。
...
1. proposal id がガバナンス台帳 (`/v1/governance/proposals/{id}`) に存在し、状態が `Approved`。
...
- 支払い証明は Norito 台帳レシートを再利用; treasury サービスがヘルパー API (`/v1/finance/sns/payments`) を提供。
