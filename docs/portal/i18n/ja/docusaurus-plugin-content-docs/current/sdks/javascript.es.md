...
ページネーション ヘルパーは、`/v2/accounts` の Python SDK 人間工学を反映しています。
`/v2/domains`、`/v2/assets/definitions`、NFT、残高、資産保有者、および
アカウントの取引履歴。
...
残り 24 時間未満。 SDK
`/v2/offline/allowances` によって公開される REST フィルターをミラーリングします。
...
Explorer テレメトリは、`/v2/explorer/metrics` および
`/v2/explorer/accounts/{account_id}/qr` エンドポイント。ダッシュボードで
...
`/v2/connect/ws` URL (`sid`、`role`、およびトークンパラメータを含む)、アップグレード
...
`ToriiClient.streamEvents()` は、自動を備えた非同期イテレータとして `/v2/events/sse` を公開します。
...
- `ToriiClient.streamSumeragiStatus()` は、同じインターフェイスをミラーリングします。
  `/v2/sumeragi/status/sse` なので、コンセンサス テレメトリは個別に追跡できます。
...
1. `getSumeragiRbcSessions()` は `/v2/sumeragi/rbc/sessions` をミラーリングします。
...
2. `ToriiClient.buildRbcSampleRequest(session, overrides)` は `{blockHash,height,view}` を正規化します
   ...
   Torii に達します。
3. `sampleRbcChunks()` はリクエストを `/v2/sumeragi/rbc/sample` に POST し、チャンクプルーフを返します。
