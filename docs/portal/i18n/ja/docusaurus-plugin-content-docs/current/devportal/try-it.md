...
2. Swagger または RapiDoc で NRPC エンドポイント（例: `POST /v1/pipeline/submit`）を選び、**Content-Type** セレクタを `application/x-norito` に切り替えます。
...
`transaction_fixtures.manifest.json` にある任意の fixture に差し替えるか、`cargo xtask norito-rpc-fixtures` で独自 payload をエンコードしてください。Torii が canary モードのときは、`curl` を try-it proxy（`https://docs.sora.example/proxy/v1/pipeline/submit`）に向けて、ポータルウィジェットと同じインフラをテストできます。
