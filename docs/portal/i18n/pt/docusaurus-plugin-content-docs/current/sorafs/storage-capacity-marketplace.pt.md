...
- `PinProviderRegistry` agora expõe o snapshot on-chain via `/v2/sorafs/capacity/state`, combinando declarações de provedores e entradas do livro de taxas por meio de Norito JSON determinístico. [crates/iroha_torii/src/sorafs/registry.rs:17] [crates/iroha_torii/src/sorafs/api.rs:64]
...
|------|------------------|-------|
| Adicione os manipuladores Torii `/v2/sorafs/capacity/declare`, `/v2/sorafs/capacity/telemetry`, `/v2/sorafs/capacity/orders` com payloads Norito JSON. | Equipe Torii | Espelhar lógica do validador; reutilizar ajudantes Norito JSON. |
...
- Submeta via Torii (`/v2/sorafs/capacity/declare`) e captura `/v2/sorafs/capacity/state` mais
