...
|------|----------|-------|
| `/v1/sorafs/capacity/declare`, `/v1/sorafs/capacity/telemetry`, `/v1/sorafs/capacity/orders` Torii manipuladores e Norito cargas úteis JSON کے ساتھ شامل کریں۔ | Equipe Torii | lógica validadora کو espelho کریں؛ Ajudantes JSON Norito reutilizam کریں۔ |
...
| medição de integridade (backlog de ingestão, telemetria obsoleta) e exportação de painéis/alertas | Observabilidade | SF-6/SF-7 میں ریفرنس کیے گئے Pacote Grafana کو estender کریں۔ |- Torii اب `/v1/sorafs/capacity/telemetry` اور `/v1/sorafs/capacity/state` (JSON + Norito) expor کرتا ہے تاکہ operadores instantâneos de telemetria de época جمع کر سکیں اور auditoria de inspetores یا embalagem de evidências کے لیے razão canônica
