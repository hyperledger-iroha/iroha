...
**Artigos:**
- CLI e testes padrão (`crates/sorafs_orchestrator/src/bin/sorafs_cli.rs`)
...
- Chicote de paridade determinística (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`, [Отчет о паритете GA SoraFS Orchestrator](./orchestrator-ga-parity.md))
...
| SF6-SR-03 | Médio | Manifestos de manifestação | Os manifestos primários da CLI são baseados em planos de blocos padrão, criados por `--plan`. | `sorafs_cli manifest submit` теперь пересчитывает e сравнивает CAR digests, если не указан `--expect-plan-digest`, отклоняя incompatibilidades e выдавая dicas de remediação. Os testes foram feitos com sucesso/armazenamento (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`). |
...
4. Chicote de paridade Логи (`cargo test -p sorafs_orchestrator --test sorafs_cli proof_stream_consumes_ndjson_and_reports_metrics -- --nocapture`).
