...
- `PinProviderRegistry` expõe a distribuição do snapshot on-chain via `/v2/sorafs/capacity/state`, combinando declarações de provedores e entradas do razão de taxas atrás de um Norito JSON determinista.【crates/iroha_torii/src/sorafs/registry.rs:17】【crates/iroha_torii/src/sorafs/api.rs:64】
...
- Soumettre via Torii (`/v2/sorafs/capacity/declare`) depois do capturador `/v2/sorafs/capacity/state` e das capturas Grafana. Siga o fluxo de surtida em `docs/source/sorafs/capacity_onboarding_runbook.md`.
