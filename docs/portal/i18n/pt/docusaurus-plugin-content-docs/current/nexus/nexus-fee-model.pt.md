...
- A execução de blocos agregados recibos por lane/dataspace e os públicos via `lane_settlement_commitments` em `/v1/sumeragi/status`. Os totais mostram `total_local_micro`, `total_xor_due_micro` e `total_xor_after_haircut_micro` somados no bloco para exportações noturnas de conciliação.
