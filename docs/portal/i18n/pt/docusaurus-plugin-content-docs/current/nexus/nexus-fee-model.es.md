...
- A execução de blocos agrega recibos por lane/dataspace e os publica via `lane_settlement_commitments` e `/v2/sumeragi/status`. Os totais expostos `total_local_micro`, `total_xor_due_micro`, e `total_xor_after_haircut_micro` somados sobre o bloco para as exportações noturnas de conciliação.
