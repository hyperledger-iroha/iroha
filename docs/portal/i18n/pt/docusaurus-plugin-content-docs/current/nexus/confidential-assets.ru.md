...
3. **Instruções para operação:** Verifique o runbook `transition_id` e распространить runbook on/off-ramp. Кошельки и аудиторы подписываются на `/v2/confidential/assets/{id}/transitions`, чтобы узнать высоту открытия окна.
...
- CLI de expansão: `confidential create-keys`, `confidential send`, `confidential export-view-key`, аудит-инструменты para расшифровки memo e auxiliar `iroha app zk envelope` para создания/инспекции Norito envelopes de memorando офлайн. Torii é usado para derivação de fluxo como `POST /v2/confidential/derive-keyset`, formato hexadecimal e base64, formato de caixa grande программно получать иерархии ключей.## Limites, limites e controles DoS
