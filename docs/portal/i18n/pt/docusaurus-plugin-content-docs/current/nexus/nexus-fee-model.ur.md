...
- Bloquear recibos de execução کو pista/espaço de dados کے حساب سے agregado کرتا ہے اور انہیں `/v2/sumeragi/status` میں `lane_settlement_commitments` کے ذریعے شائع کرتا ہے۔ totais میں `total_local_micro`, `total_xor_due_micro`, اور `total_xor_after_haircut_micro` شامل ہوتے ہیں جو bloco پر جمع کر کے exportações de reconciliação noturna کے لئے فراہم ہوتے ہیں۔
