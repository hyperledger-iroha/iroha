...
Swift SDKs اب cola JSON sob medida کے بغیر instruções de escudo emitem کر سکتے ہیں: compromisso de nota de 32 bytes, carga útil criptografada e metadados de débito کے ساتھ `ShieldRequest` بنائیں, پھر `/v2/pipeline/transactions` پر sinal e relé کرنے کے لئے `IrohaSDK.submit(shield:keypair:)` (یا `submitAndWait`) کال کریں۔ Comprimentos de compromisso auxiliar validados کو espelho کرتا ہے تاکہ carteiras Rust کے ساتھ lock-step رہیں۔
...
3. **Publicar orientação do operador:** واپس ملنے والا `transition_id` registro کریں اور on/off-rampa runbook circular کریں۔ Carteiras اور auditores `/v2/confidential/assets/{id}/transitions` subscrever کر کے altura de abertura da janela جانتے ہیں۔
