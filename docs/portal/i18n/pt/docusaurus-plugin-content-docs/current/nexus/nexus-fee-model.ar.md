...
- Verifique se a faixa/espaço de dados está localizada em `lane_settlement_commitments` em `/v1/sumeragi/status`. Verifique os valores `total_local_micro`, `total_xor_due_micro` e `total_xor_after_haircut_micro` para obter mais informações sobre o produto. المطابقة الليلية.
