...
- A execução de blocos agrega as regras para lane/dataspace e publica via `lane_settlement_commitments` em `/v2/sumeragi/status`. Os totais expostos `total_local_micro`, `total_xor_due_micro` e `total_xor_after_haircut_micro` são adicionados ao bloco para as exportações noturnas de reconciliação.
