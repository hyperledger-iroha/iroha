...
O fluxo assume que você tem o binário `sorafs_cli` (built from the `sorafs_orchestrator` Cargo target), acesso a
...
- **Links de ticket.** Referência dos IDs do painel de Grafana ou exporta image adjuntos no ticket
