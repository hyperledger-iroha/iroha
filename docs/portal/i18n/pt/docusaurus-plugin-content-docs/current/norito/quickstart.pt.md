...
- Binários `koto build`, `ivm_run` e `iroha_cli`. Você pode compilar os itens a partir do checkout do workspace como mostrado abaixo ou baixar os artefatos de lançamento correspondentes:
...
O runner imprime o log `debug::info("Hello from Kotodama")` e executa o syscall `SET_ACCOUNT_DETAIL` contra o host mockado. Se o binário opcional `ivm_tool` estiver disponível, `ivm_tool inspect target/quickstart/hello.to` mostra o cabeçalho ABI, os feature bits e os pontos de entrada exportados.
