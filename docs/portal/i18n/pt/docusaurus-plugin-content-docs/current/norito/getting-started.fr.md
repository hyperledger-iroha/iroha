...
2. Construa ou baixe os binários de suporte:
   - `koto build` - compilador Kotodama que emite o bytecode IVM/Norito
...
- `ABI V1` alterou o contrato na versão ABI 1 (somente suportado no momento da redação).
- `--max-cycles 1000000` exige uma execução sem limite; fixe um nome positivo para suportar o preenchimento dos ciclos para as previsões de conhecimento zero.
...
- Se `koto build` recusar a versão ABI, verifique se o compilador e se ele está funcionando bem ABI v1 (execute `koto build --help` sem argumentos para listar o suporte).
