...
- Binários `koto build`, `ivm_run` e `iroha_cli`. Você pode construí-los a partir do checkout do espaço de trabalho, como mostrado abaixo ou baixar os artefatos da versão correspondente:
...
O executor imprime o log `debug::info("Hello from Kotodama")` e executa o syscall `SET_ACCOUNT_DETAIL` contra o host simulado. Se o binário opcional `ivm_tool` estiver disponível, `ivm_tool inspect target/quickstart/hello.to` mostra o ABI encabeçado, os bits de recursos e os pontos de entrada exportados.
