...
source_hash: 657b9dc1693197788af13c37c007c5001ef222b7c0d4c522f627d66a95ac08b7
...
- Financie a autoridade do contrato (por exemplo `<katakana-i105-account-id>`) com o ativo que ela transferira e conceda a autoridade o papel `CanTransfer` ou permissao equivalente.
- Chame o entrypoint `call_transfer_asset` para transferir 5 unidades da conta do contrato para `<katakana-i105-account-id>`, refletindo como a automacao on-chain pode envolver chamadas do host.
- Verifique saldos via `FindAccountAssets` ou `iroha_cli ledger assets list --account <katakana-i105-account-id>` e inspecione eventos para confirmar que o guard de metadados registrou o contexto da transferencia.
