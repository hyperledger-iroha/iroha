...
- Certifique-se de que a conta de destino (por exemplo, `i105...`) existe, refletindo a fase de mise no local em cada SDK de início rápido.
...
- Verifique as soldas via `client.request(FindAccountAssets)` ou `iroha_cli ledger assets list --account i105...` para confirmar se o frappe está reusado.
