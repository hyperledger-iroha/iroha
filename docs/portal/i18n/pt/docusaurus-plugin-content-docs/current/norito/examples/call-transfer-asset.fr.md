...
- Aprovar a autoridade do contrato (por exemplo `ih58...`) com o ato de transferência e conceder a função `CanTransfer` ou uma permissão equivalente.
- Ligue para o ponto de entrada `call_transfer_asset` para transferir 5 unidades da conta do contrato para `ih58...`, de modo que a maneira como a automatização na cadeia não pode encapsular as chamadas em casa.
- Verifique as vendas via `FindAccountAssets` ou `iroha_cli ledger assets list --account ih58...` e inspecione os eventos para confirmar que o acúmulo de metadonos foi publicado no contexto da transferência.
