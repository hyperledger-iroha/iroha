...
- Abra o contrato de política (por exemplo `soraカタカナ...`) ativo, feche o orçamento e выдайте use o papel `CanTransfer` ou a configuração atual.
- Вызовите точку входа `call_transfer_asset`, чтобы перевести 5 единиц аккаунта контракта на `soraカタカナ...`, отражая то, Como o aplicativo on-line pode ser usado para você.
- Verifique os saldos de `FindAccountAssets` ou `iroha_cli ledger assets list --account soraカタカナ...` e instale a proteção, чтобы подтвердить, что guarda O metadanных записал контекст перевода.
