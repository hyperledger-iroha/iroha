...
| شرط | تفصیل |
|-----|-------|
| پروٹوکولز | REST `/v2/sns/*` کے تحت اور serviço gRPC `sns.v1.Registrar`۔ O Norito-JSON (`application/json`) e o Norito-RPC são de alta qualidade (`application/x-norito`) |
...
|----------|-------|---------|-------|
| `/v2/sns/registrations` | POSTAR | `RegisterNameRequestV1` | Não há nenhum problema com você nível de preços حل کرتا ہے، provas de pagamento/governança کی توثیق کرتا ہے، eventos de registro emitem کرتا ہے۔ |
| `/v2/sns/registrations/{selector}/renew` | POSTAR | `RenewNameRequestV1` | مدت بڑھاتا ہے۔ پالیسی سے janelas de graça/redenção نافذ کرتا ہے۔ |
| `/v2/sns/registrations/{selector}/transfer` | POSTAR | `TransferNameRequestV1` | حکمرانی aprovações لگنے کے بعد propriedade منتقل کرتا ہے۔ |
| `/v2/sns/registrations/{selector}/controllers` | COLOCAR | `UpdateControllersRequestV1` | controladores endereços de contas assinadas کی توثیق کرتا ہے۔ |
| `/v2/sns/registrations/{selector}/freeze` | POSTAR | `FreezeNameRequestV1` | guardião/conselho congelar۔ bilhete de guardião ou súmula de governança کا حوالہ درکار۔ |
| `/v2/sns/registrations/{selector}/freeze` | EXCLUIR | `GovernanceHookV1` | remediação کے بعد descongelar؛ substituição do conselho ریکارڈ ہونے کو یقینی بناتا ہے۔ |
| `/v2/sns/reserved/{selector}` | POSTAR | `ReservedAssignmentRequestV1` | nomes reservados کی administrador/conselho کی طرف سے atribuição۔ |
| `/v2/sns/policies/{suffix_id}` | OBTER | -- | `SuffixPolicyV1` موجودہ حاصل کرتا ہے (armazenável em cache)۔ |
| `/v2/sns/registrations/{selector}` | OBTER | -- | موجودہ `NameRecordV1` + موثر حالت (Active, Grace وغیرہ) واپس کرتا ہے۔ |
...
`--governance-json` میں درست `GovernanceHookV1` ریکارڈ ہونا چاہیے (id da proposta, hashes de voto, assinaturas do administrador/guardião)۔ ہر کمانڈ متعلقہ `/v2/sns/registrations/{selector}/...` endpoint کی عکاسی کرتی ہے تاکہ beta operadores بالکل وہی Torii superfícies ensaiam کر O que há de melhor em SDKs
...
1. livro razão de governança de identificação de proposta (`/v2/governance/proposals/{id}`) موجود ہے اور status `Approved` ہے۔
...
### 6.1 Registro Padrão1. Cliente `/v2/sns/policies/{suffix_id}` کو consulta کرتا ہے تاکہ preços, graça e دستیاب níveis حاصل کرے۔
...
3. Remediação کے بعد substituição do conselho جاری کرتا ہے؛ operador DELETE `/v2/sns/registrations/{selector}/freeze` کو `GovernanceHookV1` کے ساتھ بھیجتا ہے۔
...
- Comprovantes de pagamento Norito recibos contábeis دوبارہ استعمال کرتے ہیں؛ APIs auxiliares de serviços de tesouraria (`/v2/finance/sns/payments`) فراہم کرتی ہیں۔
