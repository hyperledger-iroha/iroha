...
| Guia UX de endereços | [`address-display-guidelines.md`](./address-display-guidelines.md) | Rendus canonices IH58 (preferir) e compressas (segunda escolha) reproduzidos por carteiras/exploradores. |
...
6. **Cliente de divulgação:** Mettre a jour le ledger oriente client (wallet/explorer)
   ...
   com certeza que os resultados IH58 e as compactações correspondentes aos guias de cópia/QR.
...
| Guardião de ingressos | Conselho guardião | Produto `sns governance freeze --selector <IH58> --reason <text> --until <ts>` `GuardianFreezeTicketV1`. Armazene o JSON do ticket sob `artifacts/sns/guardian/<id>.json`. | <=30 min ACK, <=2 h de execução. |
...
| Degel e remediação | Registrador + resolvedor de operações | O executor `sns governance unfreeze --selector <IH58> --ticket <id>`, atualizou o status do registrador e propaga as diferenças GAR/resolver. | Imediatamente após o veredicto. |Les canons d'urgence (gels declenches par Guardian <=72 h) suivent le meme flux
