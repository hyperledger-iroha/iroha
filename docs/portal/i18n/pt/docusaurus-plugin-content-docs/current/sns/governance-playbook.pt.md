...
| Guia UX de enderecos | [`address-display-guidelines.md`](./address-display-guidelines.md) | Renderizações canônicas IH58 (preferidas) e comprimidas (segunda melhor opção) refletidas por wallets/explorers. |
...
| Guardião de ingressos | Conselho guardião | `sns governance freeze --selector <IH58> --reason <text> --until <ts>` produz `GuardianFreezeTicketV1`. Armazene o JSON do ticket em `artifacts/sns/guardian/<id>.json`. | <=30 min ACK, <=2 h execucao. |
...
| Descongelar e remediar | Registrador + operações de resolução | Executar `sns governance unfreeze --selector <IH58> --ticket <id>`, atualizar status do registrador e propagar diffs GAR/resolver. | Ocorreu após o veredito. |
