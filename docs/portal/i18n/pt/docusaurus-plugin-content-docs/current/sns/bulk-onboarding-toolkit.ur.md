...
| `payment_asset_id` | Sim | Ativo de liquidação (exemplo `61CtjvNd9T3THAR65GsMVHr82Bjc`). |
...
- Helper ہر request کے لئے `POST /v1/sns/names` بھیجتا ہے اور پہلی HTTP
  ...
  ہوتے ہیں۔
- `--poll-status` ہر submissão کے بعد `/v1/sns/names/{namespace}/{literal}` کو
...
- **Controladores:** células خالی `--default-controllers` کو honor کرتے ہیں۔ não proprietário
  atores کو delegado کرتے وقت listas de controladores explícitas دیں (مثال `<katakana-i105-account-id>;<katakana-i105-account-id>`)۔
