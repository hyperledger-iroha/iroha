...
| `payment_asset_id` | `AsciiString` | Liquidação ativa por умолчанию (por exemplo `xor#sora`). |
...
2. Modelos de resolução de configuração (IH58 предпочтительно + compactado (`sora`) как второй выбор, registros de texto).
