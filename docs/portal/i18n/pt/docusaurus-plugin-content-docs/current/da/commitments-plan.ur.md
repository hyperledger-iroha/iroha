...
- Consultas Torii (`/v2/da/commitments/*`) e provas فراہم کرنا تاکہ relés,
...
|-------|--------|---------|-------|
| `/v2/da/commitments` | `POST` | `DaCommitmentQuery` (filtro de faixa/época/intervalo de sequência, paginação) | `DaCommitmentPage` é um valor de contagem total, compromissos, hash de bloco e hash de bloco. |
| `/v2/da/commitments/prove` | `POST` | `DaCommitmentProofRequest` (pista + hash de manifesto یا `(epoch, sequence)` tupla)۔ | `DaCommitmentProof` واپس کرتا ہے (registro + caminho Merkle + hash de bloco)۔ |
| `/v2/da/commitments/verify` | `POST` | `DaCommitmentProof` | Ajudante sem estado جو cálculo de hash de bloco دوبارہ کرتا ہے اور validação de inclusão کرتا ہے؛ SDKs disponíveis para `iroha_crypto` com link de link para download |
