...
-POSTO `/v2/contracts/deploy`
  ...
  - Relacionado:
    - GET `/v2/contracts/code/{code_hash}` -> возвращает сохраненный manifesto
...
-POSTO `/v2/gov/finalize`
  ...
  - Notas:
    ...
    - A participação de Проверки используют только aprovar+rejeitar; abster-se de não participar.-POSTO `/v2/gov/enact`
...
-`iroha --output-format text app gov deploy audit --namespace apps [--contains calc --hash-prefix deadbeef]`
  - Use instâncias de contrato para namespace e prove, aqui:
    ...
    - O manifesto em `/v2/contracts/code/{code_hash}` é compatível com `code_hash` e `abi_hash`.
...
- GET `/v2/gov/instances/{ns}` - especifica as instâncias do contrato ativo para o namespace.
...
4. **Аудит результатов** через `/v2/sumeragi/evidence` e `/v2/sumeragi/status`, чтобы убедиться, что счетчик evidência вырос и governança выполнила удаление.
...
- Runtime e CLI показывают staged параметры через `/v2/sumeragi/params` e `iroha --output-format text ops sumeragi params`, чтобы операторы подтвердили alturas de ativação e escala de validação.
- A governação automática é necessária:
  ...
  3. Monitore `/v2/sumeragi/status` para configurar `effective_consensus_mode` em sua tela.
