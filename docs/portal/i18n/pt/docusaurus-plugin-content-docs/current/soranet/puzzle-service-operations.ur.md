...
- `GET /v1/puzzle/config` - relé JSON (`handshake.descriptor_commit_hex`, `pow.*`) Preto
  اٹھائے گئے موثر Parâmetros PoW/puzzle واپس کرتا ہے۔
- `POST /v1/puzzle/mint` - Bilhete Argon2 mint کرتا ہے؛ corpo JSON opcional
...
- `GET /v1/token/config` - جب `pow.token.enabled = true` ہو تو token de admissão ativo
...
- `POST /v1/token/mint` - Token de admissão ML-DSA mint کرتا ہے جو hash de currículo fornecido
...
Observador de arquivo de revogação `/v1/token/config` کو atual رکھتا ہے؛ atualizações
...
verificar کرنے کے لئے anúncio de chave pública ML-DSA-44 ہو؛ Chave `/v1/puzzle/config` یہ
...
3. **Estágio de reinicialização کریں۔** Anúncio de transição de rotação de governança کرے تو unidade systemd یا
   ...
   reiniciar
4. **Validar کریں۔** `POST /v1/puzzle/mint` کے ذریعے emissão de bilhete کریں اور تصدیق کریں کہ
   ...
   capturar کرتا ہے۔ Tokens permitem ہوں تو `/v1/token/config` buscar کریں تاکہ emissor anunciado
...
4. Monitor `soranet_handshake_pow_difficulty` کریں تاکہ dificuldade valor de hashcash esperado
   تک drop ہو، اور verificar کریں کہ `/v1/puzzle/config` `puzzle = null` رپورٹ کرے۔
...
- **Alinhamento do quebra-cabeça:** `soranet_handshake_pow_difficulty` کو `/v1/puzzle/config` سے واپس ہونے والی
...
- **Prontidão do token:** اگر `/v1/token/config` غیر متوقع طور پر `enabled = false` ہو جائے یا
...
- **Saúde do serviço:** `/healthz` کو معمول کی cadência de atividade پر sonda کریں اور alerta کریں اگر
  `/v1/puzzle/mint` HTTP 500 respostas دے (incompatibilidade de parâmetro Argon2 یا falhas RNG کی نشاندہی).
  Erros de cunhagem de token `/v1/token/mint` em respostas HTTP 4xx/5xx کے ذریعے نظر آتے ہیں؛ falhas repetidas
