...
3. **Репликацияны қайта тағайындау**
   - Кешіктіру (`stat="avg"`) 5 дәуірден төмен түскен кезде `sorafs_manifest_stub capacity replication-order` арқылы жаңа тапсырыстарды беріңіз (манифест/CAR қаптамасы `iroha app sorafs toolkit pack` пайдаланады).
