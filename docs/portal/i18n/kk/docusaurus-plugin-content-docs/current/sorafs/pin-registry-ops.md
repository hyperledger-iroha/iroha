...
Барлық өлшемдер әрбір суретті түсіру кезінде бастапқы қалпына келтіріледі, сондықтан бақылау тақталары `1m` каденсімен немесе одан да жылдамырақ үлгі алуы керек.
...
2. **Қаталог бүркеншік тенденциясы** – `torii_sorafs_registry_aliases_total`.
3. **Күйі бойынша тапсырыс кезегі** – `torii_sorafs_registry_orders_total` (`status` бойынша топтастырылған).
4. **Бақтық журнал және мерзімі өткен тапсырыстар** – беттік қанықтылыққа `torii_sorafs_replication_backlog_total` және `torii_sorafs_registry_orders_total{status="expired"}` біріктіреді.
...
- **SLA табысы < 0,95 15 минут**
  - Шекті: `sum(torii_sorafs_replication_sla_total{outcome="met"}) / clamp_min(sum(torii_sorafs_replication_sla_total{outcome=~"met|missed"}), 1) < 0.95`
  - Әрекет: Бет SRE; репликацияның артта қалған триажын бастау.
- **10-нан жоғары күтудегі артта қалу**
  - Шекті: `torii_sorafs_replication_backlog_total > 10` 10 минут бойы сақталады
  - Әрекет: провайдердің қолжетімділігін және Torii сыйымдылық жоспарлаушысын тексеріңіз.
- **Мерзімі өткен тапсырыстар > 0**
...
3. **Репликацияны қайта тағайындау**
   - Кешіктіру (`stat="avg"`) 5 дәуірден төмен түскен кезде `sorafs_manifest_builder capacity replication-order` арқылы жаңа тапсырыстарды беріңіз (манифест/CAR қаптамасы `iroha app sorafs toolkit pack` пайдаланады).
...
Өндірісте бүркеншік ат кэш саясатын қосу немесе қатайту кезінде осы кезеңдік процедураны орындаңыз:

1. **Конфигурацияны дайындаңыз**
   - `torii.sorafs_alias_cache` нұсқасын `iroha_config` (пайдаланушы → нақты) келісілген TTL және қосымша терезелермен жаңартыңыз: `positive_ttl`, `refresh_window`, `hard_expiry`, I108002, I10802 `revocation_ttl`, `rotation_max_age`, `successor_grace` және `governance_grace`. Әдепкі мәндер `docs/source/sorafs_alias_policy.md` саясатына сәйкес келеді.
   - SDK үшін бірдей мәндерді конфигурация деңгейлері арқылы таратыңыз (Rust / NAPI / Python байланыстыруларындағы `AliasCachePolicy::new(positive, refresh, hard, negative, revocation, rotation, successor, governance)`), клиентті орындау шлюзге сәйкес келеді.
2. **Сахнадағы құрғақ жүгіріс**
   - Өндіріс топологиясын көрсететін кезеңдік кластерге конфигурация өзгертуін қолданыңыз.
   - `cargo xtask sorafs-pin-fixtures` іске қосыңыз, канондық бүркеншік аттың декодтауы әлі де шифрленетінін растау және айналу; кез келген сәйкессіздік алдымен шешілуі керек жоғары ағынды манифест дрейфін білдіреді.
...
4. **Орналастырудан кейінгі тексеру**
   - 30 минут бойы `torii_sorafs_alias_cache_refresh_total` және `torii_sorafs_alias_cache_age_seconds` бақылаңыз. `error`/`expired` қисық сызықтарындағы төбелер саясатты жаңарту терезелерімен корреляциялануы керек; күтпеген өсу операторлар жалғастырмас бұрын бүркеншік аттың дәлелдері мен провайдердің денсаулығын тексеруі керек дегенді білдіреді.
...
5. **Қолданбау**
   - Егер бүркеншік ат беру артта қалса және жаңарту терезесі жиі өшірілсе, конфигурацияда `refresh_window` және `positive_ttl` арттыру арқылы саясатты уақытша босатыңыз, содан кейін қайта орналастырыңыз. `hard_expiry` толық сақтаңыз, сондықтан шынымен ескірген дәлелдер әлі қабылданбайды.
   - Телеметрия жоғары `error` сандарын көрсетуді жалғастырса, алдыңғы `iroha_config` суретін қалпына келтіру арқылы алдыңғы конфигурацияға оралыңыз, содан кейін бүркеншік аттың генерациясының кідірістерін қадағалау үшін оқиғаны ашыңыз.
