...
| `owner` | Сим | AccountId string (domainless encoded literal; canonical Katakana i105 only; no `@<domain>` suffix). |
...
- **Контроллеры:** указаны в официальном порядке `--default-controllers`. Форнека
  Явные списки (например, `<katakana-i105-account-id>;<katakana-i105-account-id>`) делегируются владельцам.
