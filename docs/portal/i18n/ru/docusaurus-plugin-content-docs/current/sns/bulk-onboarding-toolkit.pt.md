...
| `owner` | Сим | Строка AccountId (домен-независимый encoded AccountId (IH58 предпочтительно; compressed допускается; без @domain)) делает владельца регистратором. |
...
- **Контроллеры:** указаны в официальном порядке `--default-controllers`. Форнека
  Явные списки (например, `ih58...;ih58...`) делегируются владельцам.
