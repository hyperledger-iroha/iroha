...
- Хелпер делает один `POST /v1/sns/names` на запрос и останавливается при
...
- **Controllers:** пустые ячейки соблюдают `--default-controllers`. Указывайте
  явные списки controllers (например `<katakana-i105-account-id>;<katakana-i105-account-id>`) при делегировании не-owner.
