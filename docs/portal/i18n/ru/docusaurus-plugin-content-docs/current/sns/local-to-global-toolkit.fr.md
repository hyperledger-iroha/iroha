...
- `scripts/address_local_toolkit.sh` инкапсулирует CLI `iroha` для создания продукта:
  ...
  - `normalized.txt` -- буквенный i105 (предпочтительно) / сжатый (`sora`) (второй выбор) конвертировать для выбора локального домена.
...
- `--format i105` для вылазки `sora...` на месте i105.
