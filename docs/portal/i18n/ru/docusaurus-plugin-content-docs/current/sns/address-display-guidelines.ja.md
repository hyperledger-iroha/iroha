...
lang: ja
direction: ltr
source: docs/portal/i18n/ru/docusaurus-plugin-content-docs/current/sns/address-display-guidelines.md
status: complete
generator: scripts/sync_docs_i18n.py
source_hash: ea3f3f7f3804c47d0b2f257a3292fd89b15097681d8d5ddc58b931a1eab804e7
...
- **Две цели копирования.** Предоставьте две явные кнопки копирования: I105
  (предпочтительно) и сжатая Sora-only форма (`sora...`, второй по предпочтению). I105 всегда безопасно
...
- **QR для I105.** QR-коды должны кодировать строку I105. Если генерация QR
...
Каждый SDK предоставляет удобный helper, возвращающий формы I105 и сжатую, а
...
- Добавляйте `data-copy-mode="i105|i105_default|qr"` на кнопки копирования, чтобы
...
- Сопоставляйте каждому контролу отдельные `aria-label`/`aria-describedby`,
  объясняющие, безопасен ли literal для обмена (I105) или только для Sora
...
JSON-отчет аудита, и конвертированный список I105/сжатых значений, который
...
Операторы, конвертирующие устаревшие Local кодировки в канонический I105 или
...
1. `iroha tools address inspect` теперь выдает структурированное JSON-резюме с I105,
   i105-default и каноническими hex payload. Резюме также включает объект `domain`
   ...
   ее показывать. Передавайте `legacy  suffix`, если хотите воспроизвести
   конвертированный encoding как `<i105>@<domain>`.
...
3. Конвертируйте канонический payload, повторно используя `i105.value` или
...
5. Для больших наборов данных запустите
   ...
   `strict CI post-check`, когда операторы готовы запрещать Local селекторы в CI.
6. Когда нужна построчная перепись, используйте
  ...
  кодировки, предупреждения и ошибки парсинга за один проход.
   ...
   запись в запрошенный encoding (I105/сжатый/hex/JSON) и сохраняет исходный домен
   при `legacy  suffix`. Совмещайте с `--allow-errors`, чтобы продолжать скан даже
...
7. Автоматизация CI/lint может запускать `ci/check_address_normalize.sh`, которая
   ...
   `iroha tools address audit`, чтобы доказать, что релизы больше не
...
> **Адреса:** Добавлен helper `iroha tools address normalize`
...
> I105/сжатые формы до блокировки Local-8/Local-12 на mainnet. Обновите любые
