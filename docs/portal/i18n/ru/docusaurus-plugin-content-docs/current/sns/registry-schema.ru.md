...
| `payment_asset_id` | `AsciiString` | Активировать урегулирование по умолчанию (например `xor#sora`). |
...
2. Пегенерация шаблонов резольвера (IH58 считается + сжатый (`sora`) как второй выбор, текстовые записи).
