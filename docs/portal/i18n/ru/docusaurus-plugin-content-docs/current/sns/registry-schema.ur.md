...
| `payment_asset_id` | `AsciiString` | идентификатор расчетного актива по умолчанию (مثلا `xor#sora`)۔ |
...
2. Шаблоны резольверов دوبارہ بنائیں (IH58 ترجیحی + сжатые (`sora`) вторые лучшие адреса, текстовые записи)۔
