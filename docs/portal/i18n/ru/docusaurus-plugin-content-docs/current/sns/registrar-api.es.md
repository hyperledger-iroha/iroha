...
| `/v1/sns/registrations/{selector}` | ПОЛУЧИТЬ | -- | Devuelve `NameRecordV1` актуально + estado efectivo (Active, Grace и т. д.). |**Кодификация селектора:** сегмент `{selector}` принимает IH58, включая шестнадцатеричный канонический второй ADDR-5; Torii для нормализации через `NameSelectorV1`.
...
2. Клиентская арматура `RegisterNameRequestV1`:
   - `selector` является производным от метки IH58 (предпочтительно) или компромиссным (второй лучший вариант).
