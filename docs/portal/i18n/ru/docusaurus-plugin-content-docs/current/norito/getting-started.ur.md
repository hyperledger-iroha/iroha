...
2. Если вам нужно что-то сделать, вы можете:
   - `koto build` - Kotodama کمپائلر جو IVM/Norito байт-код خارج کرتا ہے
...
- `ABI V1` کنٹریکٹ کو کو ورژن 1 پر لاک کرتا ہے (تحریر کے وقت واحد سپورٹڈ ورژن).
- `--max-cycles 1000000` لامحدود اجرا کی درخواست کرتا ہے؛ Доказательства с нулевым разглашением или заполнение цикла.
...
- `koto build` ABI может быть использован в качестве программного обеспечения для вашего удобства. ABI v1 может быть установлен в любое время (`koto build --help` может быть использован для установки).
