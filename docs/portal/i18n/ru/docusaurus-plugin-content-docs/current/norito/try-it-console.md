...
Для маршрутов только с JSON встроенный Try it sandbox часто быстрее: введите путь (например, `/v1/accounts/soraカタカナ.../assets`), выберите HTTP-метод, вставьте JSON-тело при необходимости и нажмите **Send request**, чтобы сразу увидеть заголовки, длительность и пейлоады.
