...
- Получите доступ к быстрому запуску SDK (например, `<katakana-i105-account-id>`) и ознакомьтесь с кратким руководством по SDK. مرحلے کی عکاسی کرتا ہے۔
- `register_and_mint` پوائنٹ کال کریں تاکہ ROSE اثاثہ ڈیفینیشن بنے اور ایک ہی Имя Алисы: 250 лет назад
- `client.request(FindAccountAssets)` یا `iroha_cli ledger assets list --account <katakana-i105-account-id>` کے ذریعے بیلنس چیک کریں تاکہ کی کامیابی کی تصدیق ہو۔
