...
- Убедитесь, что назначение учетной записи (например, `soraカタカナ...`) существует, повторите этап подготовки в каждом SDK быстрого запуска.
...
- Проверьте балансы через `client.request(FindAccountAssets)` или `iroha_cli ledger assets list --account soraカタカナ...`, чтобы обеспечить успешный выпуск.
