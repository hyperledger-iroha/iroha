...
- Убедитесь, что назначение учетной записи (например, `<katakana-i105-account-id>`) существует, повторите этап подготовки в каждом SDK быстрого запуска.
...
- Проверьте балансы через `client.request(FindAccountAssets)` или `iroha_cli ledger assets list --account <katakana-i105-account-id>`, чтобы обеспечить успешный выпуск.
