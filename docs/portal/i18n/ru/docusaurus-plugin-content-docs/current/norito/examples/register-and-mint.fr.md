...
- Убедитесь, что адрес назначения (например, `soraカタカナ...`) существует, и отражается на этапе планирования на месте в каждом быстром запуске SDK.
...
- Проверьте продажи через `client.request(FindAccountAssets)` или `iroha_cli ledger assets list --account soraカタカナ...`, чтобы подтвердить, что фраппе по-русски.
