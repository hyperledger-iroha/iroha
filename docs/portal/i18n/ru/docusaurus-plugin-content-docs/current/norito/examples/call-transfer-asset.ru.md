...
- Пополните полномочия контракта (например, `ih58...`) активом, который он будет переводить, и выдайте полную роль `CanTransfer` или эквивалентное решение.
- Вызовите точку входа `call_transfer_asset`, чтобы перевести 5 единиц с контрактом аккаунта на `ih58...`, учитывая, что ончейн-автоматизация может оборачивать вызовы хоста.
- Проверьте балансы через `FindAccountAssets` или `iroha_cli ledger assets list --account ih58...` и просмотрите события, чтобы убедиться, что защита метаданных записала преобразование контекста.
