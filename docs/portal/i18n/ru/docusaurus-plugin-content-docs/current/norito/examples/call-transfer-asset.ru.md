...
- Пополните полномочия контракта (например, `i105...`) активом, который он будет переводить, и выдайте полную роль `CanTransfer` или эквивалентное решение.
- Вызовите точку входа `call_transfer_asset`, чтобы перевести 5 единиц с контрактом аккаунта на `i105...`, учитывая, что ончейн-автоматизация может оборачивать вызовы хоста.
- Проверьте балансы через `FindAccountAssets` или `iroha_cli ledger assets list --account i105...` и просмотрите события, чтобы убедиться, что защита метаданных записала преобразование контекста.
