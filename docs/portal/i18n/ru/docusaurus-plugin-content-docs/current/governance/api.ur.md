...
- ПОСТ `/v2/contracts/deploy`
  ...
  - Связано:
    ...
    - GET `/v2/contracts/code-bytes/{code_hash}` -> `{ code_b64 }` в исходном коде.
...
- Пользовательский параметр: `max_contract_code_bytes` (JSON u64).
  ...
  - Операторы `SetParameter(Custom)` کے ذریعے `id = "max_contract_code_bytes"` اور числовая полезная нагрузка دے کر ایڈجسٹ کر سکتے ہیں۔- ПОСТ `/v2/gov/ballots/zk`
...
- ПОСТ `/v2/gov/enact`
  ...
  - Примечания: جب `authority`/`private_key` فراہم ہوں تو Torii подписал транзакцию سبمٹ کرتا ہے؛ ورنہ وہ скелет и его скелет прообраз اختیاری اور فی الحال معلوماتی ہے۔- ПОЛУЧИТЬ `/v2/gov/proposals/{id}`
...
- `iroha --output-format text app gov deploy audit --namespace apps [--contains calc --hash-prefix deadbeef]`
  - Пространство имен и экземпляры контракта извлекают или перекрестно проверяют:
    ...
    - `/v2/contracts/code/{code_hash}` определяет соответствие манифеста `code_hash` и значения `abi_hash` رپورٹ کرتا ہے۔
...
Листинг экземпляров
- GET `/v2/gov/instances/{ns}` - пространство имен для экземпляров активных контрактов.
  - Параметры запроса:
...
- Torii: `GET /v2/sumeragi/evidence` или `GET /v2/sumeragi/evidence/count`.
...
4. **Проверка аудита** `/v2/sumeragi/evidence` или `/v2/sumeragi/status` - счетчик доказательств, контроль управления и удаление. کیا ہو۔
...
- Промежуточные параметры среды выполнения CLI, такие как `/v2/sumeragi/params` или `iroha --output-format text ops sumeragi params`, могут быть заданы значения высоты активации операторов и списки валидаторов. کی تصدیق کر سکیں۔
- Автоматизация управления:
  ...
  3. `/v2/sumeragi/status` для регулировки высоты и переключателя `effective_consensus_mode` для переключателя высоты.
