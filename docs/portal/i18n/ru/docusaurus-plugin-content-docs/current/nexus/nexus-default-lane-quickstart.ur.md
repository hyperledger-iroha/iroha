...
- **Тесты конвейера/SSE.** Фильтры событий транзакций `tx_lane_id == <u32>` предикаты قبول کرتے ہیں (دیکھیں `docs/source/pipeline.md`). `/v2/pipeline/events/transactions` Фильтр или подписка Если вы хотите использовать явную полосу, вы можете записать резервную полосу id کے تحت پہنچتی ہیں۔
