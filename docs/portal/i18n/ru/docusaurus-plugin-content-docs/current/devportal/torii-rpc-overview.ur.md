...
Norito-RPC Torii API-интерфейсы `/v2/pipeline` и HTTP-интерфейс, а также Norito и полезные нагрузки. Использование хешей схемы и контрольных сумм. Если вы хотите, чтобы это произошло, вы можете сделать это в любое время. Использование конвейера для JSON-файлов и файлов для чтения
...
2. Нажмите **Попробуйте** Установите `/reference/torii-swagger` и выберите `POST /v2/pipeline/submit` для конечной точки. کریں۔
