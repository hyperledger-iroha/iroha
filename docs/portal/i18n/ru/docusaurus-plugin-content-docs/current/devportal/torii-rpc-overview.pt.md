...
Norito-RPC и транспортировка двоичного файла для API Torii. Он повторно использует HTTP-запросы HTTP `/v2/pipeline`, большинство полезных данных которых задерживается для Norito, включая хэши схемы и контрольные суммы. Используйте, когда точные ответы детерминированы и валидированы или когда ответы JSON делают конвейер вирусным и гаргало.
...
2. Откройте карточку **Попробуйте**, откройте страницу или введите `/reference/torii-swagger` и выберите конечную точку в виде `POST /v2/pipeline/submit`.
