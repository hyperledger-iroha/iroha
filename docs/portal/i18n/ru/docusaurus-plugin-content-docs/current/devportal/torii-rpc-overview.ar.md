...
Norito-RPC в локальной сети Torii. Получение HTTP-запроса `/v2/pipeline` в режиме онлайн-загрузки. Norito хэширует данные и контрольные суммы. Он сказал, что в 1990-х годах он был рожден в 1980-х годах. Формат JSON для конвейера.
...
2. Нажмите **Попробуйте** в разделе `/reference/torii-swagger` и установите флажок `POST /v2/pipeline/submit`.
