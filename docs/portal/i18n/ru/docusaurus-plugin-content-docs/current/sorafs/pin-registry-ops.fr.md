...
1. **Идентификатор причины**
   ...
   - Если задержка увеличивается с помощью конюшен, инспектор по допуску (`/v2/sorafs/pin/*`) для подтверждения деклараций и внимания на одобрении совета.
...
2. **Прогон и постановка**
   ...
   - Используйте конечные точки `/v2/sorafs/pin/{digest}` и `/v2/sorafs/aliases` с доступными синтетическими продуктами, свежими, обновленными окнами, истекшими и окончательно истекшими. Проверяйте коды HTTP, заголовки (`Sora-Proof-Status`, `Retry-After`, `Warning`) и поля JSON для корпуса, содержащие Runbook.
