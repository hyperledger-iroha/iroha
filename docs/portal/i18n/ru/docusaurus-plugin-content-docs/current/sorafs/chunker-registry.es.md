...
| `sorafs_provider_advert_stub` | ⚠️ Ретирадо | Помощник по проверке подлинности в автономном режиме; Рекламные объявления поставщика должны производиться через конвейер публикации и проверки через `/v2/sorafs/providers`. |
...
- **Реклама поставщика** — полезные данные рекламы включают метаданные возможностей и псевдонимов; действительный выбор через `/v2/sorafs/providers` (стр. например, наличие емкости `range`).
