...
- **Подтвердить телеметрию:** зафиксируйте снимки `/v2/sorafs/capacity/state` и `/v2/sorafs/capacity/telemetry` для провайдера.
...
4. Отправьте JSON-запрос в Torii `/v2/sorafs/capacity/dispute` через очередь управления-транзакций. Зафиксируйте значение ответа `dispute_id_hex`; Он якорит действия по отзывам и аудиторским отчетам.
