...
storage, governance, DevRel и SDK могли согласованно перейти от legacy hosting
...
|------|------|---------------|-------------------------|-----------|
| **M0 - Bootstrap** | Недели 1-6 | Публикация детерминированных chunker fixtures и двойная публикация артефактов (legacy + SoraFS). | `sorafs_chunker` fixtures, интеграция CLI `sorafs_manifest_stub`, записи в migration ledger. | Docs, DevRel, Storage |
...
| **M2 - Registry First** | Недели 13-20 | Маршрутизировать pins через registry, заморозить legacy bundles и вывести parity телеметрию. | Контракт Pin Registry + CLI (`sorafs pin propose/approve`), наблюдательные dashboards, operator runbooks. | Governance, Ops, Observability |
| **M3 - Alias Only** | Неделя 21+ | Деактивировать legacy hosting и требовать alias proofs для получения. | Alias-only gateways, parity alerts, обновленные defaults SDK, уведомление о демонтаже legacy. | Ops, Networking, SDKs |
...
### 1. Перепаковка legacy данных

| Шаг | Веха | Описание | Owner(s) | Выход |
|-----|------|----------|----------|-------|
| Инвентаризация и тегирование | M0 | Экспортировать SHA3-256 digests legacy bundles и записать их в migration ledger (append-only). | Docs, DevRel | Записи ledger с `source_path`, `sha3_digest`, `owner`, `planned_manifest_cid`. |
| Детерминированная пересборка | M0-M1 | Запускать `sorafs_manifest_stub` для каждого release артефакта и сохранять CAR, manifest, signature envelope и fetch plan в `artifacts/<team>/<alias>/<timestamp>/`. | Docs, CI | Воспроизводимые CAR + manifest bundles на каждый релиз. |
| Валидационный цикл | M1 | Прогонять `sorafs_fetch` через staging gateways, чтобы подтвердить совпадение границ/digest chunk с fixtures. Фиксировать pass/fail в комментариях ledger. | Governance QA | Отчет верификации staging + GitHub issue по drift. |
| Registry cut-over | M2 | Перевести статус ledger в `Pinned`, когда digest manifest есть on-chain; legacy bundle становится read-only (служит, но не модифицируется). | Governance, Ops | Hash транзакции registry, read-only ticket для legacy storage. |
| Decommission | M3 | Удалить legacy CDN записи после 30-дневного grace периода, архивировать approvals DNS изменений и опубликовать post-mortem. | Ops | Decommission checklist, запись изменения DNS, закрытие инцидентного тикета. |
...
| Dual DNS + уведомление | M1-M2 | Эксплуатировать legacy DNS и Torii/SoraDNS параллельно; публиковать уведомления о миграции операторам и в SDK каналах. | Networking, DevRel | Анонс-пост + DNS change ticket. |
...
| Alias-only rollout | M3 | Удалить legacy DNS, обновить defaults SDK на Torii/SoraDNS + alias proofs, задокументировать окно rollback. | SDK Maintainers, Ops | Релиз-ноты SDK, обновление ops runbook, план rollback. |
...
| Gateway parity tooling | Нужно для enforcement alias proofs и parity chunks. | Выпустить gateway обновления в M1, держать legacy поведение за feature flag до критериев M2. |
...
| Веха | Критерии |
|------|----------|
| M0 | - Все целевые артефакты пересобраны через `sorafs_manifest_stub` с expectation flags. <br /> - Migration ledger заполнен для каждой семьи артефактов. <br /> - Двойная публикация (legacy + SoraFS) активна. |
...
| M2 | - 100% новых manifests проходят через Pin Registry. <br /> - Legacy storage отмечено как read-only; incident playbook утвержден. <br /> - Observability dashboards онлайн с порогами алертов. |
| M3 | - Alias-only gateways в production. <br /> - Legacy DNS удален и отражен в change tickets. <br /> - Defaults SDK обновлены и выпущены. <br /> - Финальный статус добавлен в migration ledger. |
