...
| الفحص | تحقق | دليل |
|-------|------------|----------|
| Реестр يقبل Для этого используется приложение `/v2/sorafs/capacity/declare`, которое позволяет использовать API приложения, а также использовать его в качестве приложения. Метаданные и доступ к реестру. | `crates/iroha_torii/src/routing.rs:7654` |
