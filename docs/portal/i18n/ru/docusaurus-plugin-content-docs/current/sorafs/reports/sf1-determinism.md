...
|------|----------|--------------------|-------|
| 2026-02-12 | Tooling (LLM) | ❌ Failed | Шаг 1: `cargo test -p sorafs_chunker` падает на `vectors` suite, потому что fixtures все еще публикуют legacy handle `sorafs.sf1@1.0.0` и не содержат profile aliases/digests (`fixtures/sorafs_chunker/sf1_profile_v1.*`). Шаг 2: `ci/check_sorafs_fixtures.sh` прерывается — `manifest_signatures.json` отсутствует в состоянии репозитория (удален в working tree). Шаг 4: `export_vectors` не может проверить подписи, пока отсутствует файл manifest. Рекомендуется восстановить подписанные fixtures (или предоставить ключ council) и регенерировать bindings, чтобы канонический handle + массив aliases были встроены согласно требованиям тестов. |
