...
- Обвязка детерминированной четности (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`, [Отчет о паритете GA SoraFS Orchestrator](./orchestrator-ga-parity.md))
...
| СФ6-СР-03 | Средний | Отправка манифестов | CLI принимал манифесты без проверки встроенных планов блоков, когда `--plan` отсутствовал. | `sorafs_cli manifest submit` теперь пересчитывает и сравнивает дайджесты CAR, если не указано `--expect-plan-digest`, отклоняя несоответствия и выдавая подсказки по исправлению. Тесты раскрывают/успехи (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`). |
...
4. Логи четности (`cargo test -p sorafs_orchestrator --test sorafs_cli proof_stream_consumes_ndjson_and_reports_metrics -- --nocapture`).
