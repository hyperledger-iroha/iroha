...
1. **Идентификатор причины**
   ...
   - Если в журнале пропущены задержки, проверьте допуск (`/v2/sorafs/pin/*`) для подтверждения декларации об утверждении согласия.
...
2. **Пробный прогон**
   ...
   - Конечные точки Exerca os `/v2/sorafs/pin/{digest}` и `/v2/sorafs/aliases` com provas sinteticas cobrindo casos свежие, с обновлением окна, с истекшим сроком действия и с жестким сроком действия. Действуют коды HTTP, заголовки (`Sora-Proof-Status`, `Retry-After`, `Warning`) и кампусы корпоративного JSON против этого Runbook.
