...
3. **Репликация и переназначение**
   - جب резерв невыполненной работы (`stat="avg"`) 5 эпох سے نیچے جائے تو `sorafs_manifest_builder capacity replication-order` کے ذریعے نئے Orders جاری کریں (манифест/упаковка автомобиля `iroha app sorafs toolkit pack` استعمال کرتا ہے)۔
