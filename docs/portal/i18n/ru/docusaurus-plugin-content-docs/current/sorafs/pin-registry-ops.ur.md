...
1. **Всё в порядке**
   ...
   - اگر отставание بڑھے اور пропускает مستحکم ہوں تو вход (`/v2/sorafs/pin/*`) چیک کریں تاکہ проявляет جو کونسل کی منظوری کے منتظر ہیں واضح ہوں۔
...
2. **Постановка пробного прогона**
   ...
   - `/v2/sorafs/pin/{digest}` или `/v2/sorafs/aliases` конечные точки для синтетических доказательств или для свежих, обновленных окон, истекших или с жестким истекшим сроком действия. کریں۔ Коды состояния HTTP, заголовки (`Sora-Proof-Status`, `Retry-After`, `Warning`) и поля тела JSON, а также Runbook и проверка проверки.
