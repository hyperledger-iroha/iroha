...
1. **Подтвердить входные данные governance.**
   - Все кандидаты-провайдеры должны публиковать конверты `ProviderAdvertV1` с payload'ами диапазонных возможностей и бюджетами потоков. Проверяйте через `/v2/sorafs/providers` и сверяйте с ожидаемыми полями возможностей.
