...
2. **Перегенерируйте с помощью инструментов поставщика.**
   ...
   - `/v1/sorafs/providers` или `sorafs_fetch` سے validate کریں؛ неизвестно
...
- **Реагирование на инциденты.** `reject` оповещает о вызове по вызову:
  1. Обнаружение Torii (`/v1/sorafs/providers`) для получения оскорбительной рекламы.
  2. Конвейер провайдера проверяет рекламу или проверяет `/v1/sorafs/providers`, сравнивает, отображает ошибку, воспроизводит ہو۔
