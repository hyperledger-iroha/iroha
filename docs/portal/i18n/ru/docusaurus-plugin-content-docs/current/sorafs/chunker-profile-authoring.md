...
- Канонический handle (`namespace.name@semver`) должен присутствовать в списке alias и
  **должен** быть первой записью. Далее идут legacy alias (например, `sorafs-sf1`).
...
| `profile_aliases` | Опциональные дополнительные handles (legacy имена, сокращения), доступные клиентам при переговорах. Всегда включайте канонический handle первым. | `["sorafs-sf1"]` |
