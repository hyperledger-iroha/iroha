...
Поверхностная консолидация `sorafs_cli` (provided by the `sorafs_orchestrator` crate) показывает каждый этап, необходимый для подготовки артефатов из SoraFS. Используйте эту кулинарную книгу непосредственно для общих рабочих процессов; Объединение конвейера манифеста и модулей Runbook ОС для выполнения контекстных операций.
