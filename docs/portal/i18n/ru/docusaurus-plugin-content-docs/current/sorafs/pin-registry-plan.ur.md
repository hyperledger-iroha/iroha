...
| интерфейс командной строки | `sorafs_manifest_builder` используется для `sorafs_pin` CLI, используется `pin submit`, `alias bind`, `order issue`, `registry export` ہو۔ | Инструментальная рабочая группа |
