...
| جزو | کام | مالک/مالکان |
|------|-----|-------------|
| Torii Сервис | `/v2/sorafs/pin` (отправка), `/v2/sorafs/pin/{cid}` (поиск), `/v2/sorafs/aliases` (список/привязка), `/v2/sorafs/replication` (заказы/квитанции). нумерация страниц + фильтрация | Сетевые TL/Core Infra |
...
- `GET /v2/sorafs/pin` или `GET /v2/sorafs/pin/{digest}` проявляет واپس کرتے ہیں جن میں
  привязки псевдонимов, заказы репликации, хеш-код блока, объект аттестации и т. д.
- `GET /v2/sorafs/aliases` или `GET /v2/sorafs/replication` в каталоге псевдонимов.
