...
- **Подтвердить телеметрию:** сделать снимки `/v2/sorafs/capacity/state` и `/v2/sorafs/capacity/telemetry` для поставщика.
...
4. Отправьте запрос в формате JSON на Torii `/v2/sorafs/capacity/dispute` и на страницы государственных транзакций. Каптура доблести спасения `dispute_id_hex`; а также действия по отзыву задних дверей и информированию аудитории.
