...
|---------|--------------------------------|----------------|-------------------|
| `sorafs_cli` выборка | `--anonymity-policy anon-guard-pq` или фазовая зависимость | `--anonymity-policy anon-majority-pq` | `--anonymity-policy anon-strict-pq` |
...
3. **Клиент/SDK canary (Т плюс 1 неделя)**
   ...
   - Инвертируйте `rollout_phase = "ramp"` в настройках клиента или передайте переопределения `anon-majority-pq` в указанные пакеты SDK.
...
2. Установите `rollout_phase = "canary"` (или переопределите с помощью `anonymity_policy anon-guard-pq`) для конфигураций оркестратора и клиента, затем перезапустите детализацию PQ Ratchet из [PQ Ratchet Runbook] (./pq-ratchet-runbook.md), чтобы подтвердить путь к переходу на более раннюю версию.
