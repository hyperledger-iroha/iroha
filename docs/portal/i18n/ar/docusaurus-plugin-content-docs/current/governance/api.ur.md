...
- المشاركة `/v2/contracts/deploy`
  ...
  - ذات صلة:
    - احصل على `/v2/contracts/code/{code_hash}` -> سجل بيانات البيان
    - احصل على `/v2/contracts/code-bytes/{code_hash}` -> `{ code_b64 }`
...
- احصل على `/v2/gov/council/current`
  ...
  - ملاحظات: المجلس موجود وموجود ومثبت أصول الحصة التي تم تكوينها والعتبات تستخدم لاشتقاق احتياطي حتمي (تعكس مواصفات VRF هذه القيمة وما زالت أدلة VRF المباشرة على السلسلة غير موجودة).- POST `/v2/gov/council/derive-vrf` (الميزة: gov_vrf)
...
نقطة نهاية الراحة
- POST `/v2/gov/protected-namespaces` - `gov_protected_namespaces` الذي يمكنك من خلاله تطبيق العقدة مرة أخرى.
...
-`iroha --output-format text app gov deploy audit --namespace apps [--contains calc --hash-prefix deadbeef]`
  - تقوم مثيلات العقد بمساحة الاسم بإحضار العقد والتحقق من العقد:
    ...
    - `/v2/contracts/code/{code_hash}` يوجد بيان موجود مطابق لقيم `code_hash` و`abi_hash`.
...
قائمة المثيلات
- الحصول على `/v2/gov/instances/{ns}` - مساحة الاسم لمثيلات العقد النشطة کی فہرست۔
  - معلمات الاستعلام:
...
4. ** نتائج التدقيق ** `/v2/sumeragi/evidence` و `/v2/sumeragi/status` لا تؤدي إلى إزالة الأدلة المضادة والحوكمة.
...
- تعمل المعلمات المرحلية لوقت التشغيل وCLI مثل `/v2/sumeragi/params` و`iroha --output-format text ops sumeragi params` على تحديد ارتفاعات تنشيط المشغلين وقوائم التحقق من الصحة.
- أتمتة الحكم:
  ...
  3. `/v2/sumeragi/status` لم يتم تحديد الارتفاع المتوقع قبل `effective_consensus_mode` للمفتاح.
