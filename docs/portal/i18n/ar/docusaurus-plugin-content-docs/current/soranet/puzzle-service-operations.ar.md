...
- `GET /v1/puzzle/config` - يعيد معلمات PoW/puzzle إلى المسح
  من JSON الخاص بالـ Relay (`handshake.descriptor_commit_hex`, `pow.*`).
- `POST /v1/puzzle/mint` - يصدر تذكرة Argon2; الجسم JSON اختياري
...
- `GET /v1/token/config` - عندما `pow.token.enabled = true` يعيد
  رمز الدخول المميز (بصمة المُصدر، نطاق TTL/انحراف الساعة، معرف التتابع،
  إلغاء NRF الصلبة).
- `POST /v1/token/mint` - يصدر رمز قبول ML-DSA مرتبطا بـ تجزئة السيرة الذاتية مقدم؛
...
مراقب لإلغاء الملف بالحفاظ على `/v1/token/config` محدثا؛ نسق التحديثات مع
...
العام ML-DSA-44 المستخدم من تذاكر PoW الموقعة; تعكس `/v1/puzzle/config`
...
4. **التحقق.** اصدار تذكرة عبر `POST /v1/puzzle/mint` وتأكد ان `difficulty` و`expires_at`
   ...
   `/v1/token/config` للتأكد من أن بصمة المُصدر لعدد وعدد الإلغاء
   حسبان القيم.
...
4. راقب `soranet_handshake_pow_difficulty` وكذلك ان يتم ضبطها الى قيمة hashcash
   لاحظ، وتحقق من ان `/v1/puzzle/config` لذلك `puzzle = null`.
...
- **محاذاة اللغز:** يجب ان يطابق `soranet_handshake_pow_difficulty`
  المعادة من `/v1/puzzle/config`. يشير الفرق الى التتابع المناسب قديم او فشل اعادة
  تشغيل.
- **جاهزية الرمز المميز:** تنبيه اذا متعدد `/v1/token/config` الى `enabled = false`
...
- **صحة الخدمة:** افحص `/healthz` حسب إيقاع الحياة المنتظم ونبه اذا
  عاد `/v1/puzzle/mint` استجابات HTTP 500 (يدل على عدم تطابق معلمات Argon2 او
  فشل RNG). حدوث أخطاء في سك الرموز عبر استجابات HTTP 4xx/5xx على `/v1/token/mint`؛
