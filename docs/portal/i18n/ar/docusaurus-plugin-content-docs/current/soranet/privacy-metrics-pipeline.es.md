...
يقوم هواة الجمع بإعادة تقديم الملاحظات بدون إضافة وسيلة نقل مخصصة:- `POST /v2/soranet/privacy/event` يقبل الحمولة
  ...
  طريقة التتابع.
- `POST /v2/soranet/privacy/share` يقبل الحمولة `RecordSoranetPrivacyShareDto`.
...
1. قم بتصدير NDJSON من مسؤول نقطة النهاية للتتابع والبث
   `/v2/soranet/privacy/event|share` منسق الموسيقى
