...
2. معونات البناء أو التنزيل:
   - `koto build` - Kotodama المحمول جو IVM/Norito رمز البايت خارج البطاقة
...
آمال فليغز:- `ABI V1` الجزء الأول من سلسلة ABI لشبكة ABI (الإصدار الأول من الإصدار الرياضي).
- `--max-cycles 1000000` لا حدود لاجرا كي درخواست كرتا ہے؛ براهين المعرفة الصفرية عبارة عن حشوة دورة تمتد إلى عدد إيجابي.
...
- إذا كان `koto build` ABI قد بدأ في تحديث برنامج الكمبيوتر المحمول وتحديث ABI v1 (`koto build --help`) فهذا هو السبب وراء ذلك. الرياضة دکھے).
