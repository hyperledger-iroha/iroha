...
أعلام شاف:- `ABI V1` تم إصلاح العقد على عكس ABI 1 (دعم فريد في الوقت الحالي).
- `--max-cycles 1000000` طلب التنفيذ بدون حدود؛ حدد رقمًا إيجابيًا للحد من حشوة الحلقات أو حشوتها لإثبات الإدراك صفرًا.
...
- إذا تم إعادة `koto build` إلى عكس ABI، تحقق من المترجم والعقدة miram ABI v1 (استخدم `koto build --help` بدون وسيطات لإدراج أو دعم).
