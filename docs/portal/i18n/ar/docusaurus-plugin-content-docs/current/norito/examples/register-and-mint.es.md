...
- التأكد من وجود حساب الوجهة (على سبيل المثال `<i105-account-id>`)، مما يعكس عملية التكوين في كل Quickstart من SDK.
- استدعاء نقطة الدخول `register_and_mint` لإنشاء تعريف نشط ROSE والحصول على 250 وحدة لـ Alice في معاملة واحدة فقط.
- تحقق من الأرصدة المتوسطة `client.request(FindAccountAssets)` أو `iroha_cli ledger assets list --account <i105-account-id>` لتأكيد نجاح الإحصاء.
