...
- عقد المساهمات الكاملة (على سبيل المثال `soraカタカナ...`) الذي يتم تجديده على الميزانية، ويحدد الدور الهام `CanTransfer` أو ما يعادلها.
- اختر هنا `call_transfer_asset` لتحويل 5 سنوات إلى عقد حساب `soraカタカナ...`، بالإضافة إلى ذلك، يمكن للأتمتة الأولية أن تدعم استخدام المضيف.
- التحقق من التوازن عبر `FindAccountAssets` أو `iroha_cli ledger assets list --account soraカタカナ...` وإظهار الاشتراكات للتأكد من حماية البيانات المسجلة إعادة صياغة السياق.
