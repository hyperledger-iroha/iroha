...
- تمويل التفويض التعاقدي (على سبيل المثال `<i105-account-id>`) من خلال القيام بنقل والتنازل عن التفويض أو الورق `CanTransfer` أو السماح بما يعادل ذلك.
- اسم نقطة الدخول `call_transfer_asset` لنقل 5 وحدات من عقد الحساب لـ `<i105-account-id>`، كما يمكن أن يشمل الاتصال التلقائي على السلسلة مكالمات المضيف.
- التحقق من البيانات عبر `FindAccountAssets` أو `iroha_cli ledger assets list --account <i105-account-id>` وفحص الأحداث للتأكد من أن حارس التعريفات يسجل سياق النقل.
