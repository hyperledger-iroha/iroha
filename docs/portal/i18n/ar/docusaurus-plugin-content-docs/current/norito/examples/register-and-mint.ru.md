...
- تأكد من أن اسم الحساب (على سبيل المثال `soraカタカナ...`) موجود، وهو ما يعكس التحسينات في Quickstart SDK.
- اختر هنا `register_and_mint` لإنشاء النشاط المقترح ROSE وشراء 250 وحدة لـ Alice في معاملة واحدة.
- تحقق من التوازن عبر `client.request(FindAccountAssets)` أو `iroha_cli ledger assets list --account soraカタカナ...` للتحقق من صحة البيانات بنجاح.
