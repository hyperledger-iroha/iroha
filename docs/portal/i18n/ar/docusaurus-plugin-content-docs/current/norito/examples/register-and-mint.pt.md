...
- ضمان وجود حساب الوجهة (على سبيل المثال `<katakana-i105-account-id>`)، وبدء عملية التكوين في كل Quickstart من SDK.
- قم باستدعاء نقطة الدخول `register_and_mint` لإنشاء تعريف لـ ROSE وجمع 250 وحدة لـ Alice في معاملة واحدة.
- تحقق من الرسائل عبر `client.request(FindAccountAssets)` أو `iroha_cli ledger assets list --account <katakana-i105-account-id>` لتأكيد نجاح العملية.
