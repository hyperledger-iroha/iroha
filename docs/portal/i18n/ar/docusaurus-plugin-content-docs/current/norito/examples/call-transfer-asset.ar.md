...
- المؤكد أن نقطة الدخول `call_transfer_asset` نقل 5 وحدات من حساب العقد إلى `soraカタカナ...`، بما في ذلك تأكيد دقة التسجيل على السجل لنداءات السجل المدني.
- التحقق من الرصدة عبر `FindAccountAssets` أو `iroha_cli ledger assets list --account soraカタカナ...` وفحص الأحداث لتأكيد بيانات تعريف البيانات الخاصة بسجل اتفاقية النقل.
