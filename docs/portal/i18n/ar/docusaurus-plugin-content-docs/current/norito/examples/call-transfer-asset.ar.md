...
- المؤكد أن نقطة الدخول `call_transfer_asset` نقل 5 وحدات من حساب العقد إلى `ih58...`، بما في ذلك تأكيد دقة التسجيل على السجل لنداءات السجل المدني.
- التحقق من الرصدة عبر `FindAccountAssets` أو `iroha_cli ledger assets list --account ih58...` وفحص الأحداث لتأكيد بيانات تعريف البيانات الخاصة بسجل اتفاقية النقل.
