...
- استعلامات Torii (`/v2/da/commitments/*`) ومرحلات البراهين،
...
|-------|--------|---------|-------|
| `/v2/da/commitments` | `POST` | `DaCommitmentQuery` (مرشح نطاق المسار/العصر/التسلسل، ترقيم الصفحات) | `DaCommitmentPage` عبارة عن رقم إجمالي وعدد الالتزامات وتجزئة الكتلة تتضمن ہے. |
| `/v2/da/commitments/prove` | `POST` | `DaCommitmentProofRequest` (الممر + تجزئة البيان أو `(epoch, sequence)` tuple)۔ | `DaCommitmentProof` واپس کرتا ہے (سجل + مسار Merkle + تجزئة الكتلة)۔ |
| `/v2/da/commitments/verify` | `POST` | `DaCommitmentProof` | مساعد عديم الجنسية جو كتلة حساب التجزئة تكرار والتحقق من صحة التضمين؛ لا يوجد رابط لمجموعات تطوير البرمجيات (SDK) `iroha_crypto`. |
