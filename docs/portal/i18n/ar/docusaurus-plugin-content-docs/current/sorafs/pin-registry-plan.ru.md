...
| المكون | زادا | إجابة |
|-----------|-------|---------------|
| خدمة Torii | Экспонировать `/v2/sorafs/pin` (إرسال)، `/v2/sorafs/pin/{cid}` (بحث)، `/v2/sorafs/aliases` (قائمة/ربط)، `/v2/sorafs/replication` (الطلبات/الإيصالات). قم بإلغاء ترقيم الصفحات + التصفية. | الشبكات TL / الأشعة تحت الحمراء الأساسية |
...
يتم وضع مرحلة REST مع قائمة نقاط النهاية المعتمدة:- `GET /v2/sorafs/pin` و `GET /v2/sorafs/pin/{digest}` يظهران مع
  ارتباطات الاسم المستعار، وتكرار الأوامر، والشهادات الموضوعية، المنتجة من
  هذه هي الكتلة التالية.
- `GET /v2/sorafs/aliases` و `GET /v2/sorafs/replication` نشاط النشر
