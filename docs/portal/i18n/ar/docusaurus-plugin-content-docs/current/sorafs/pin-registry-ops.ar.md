...
## سير عمل الفرز1. **تحديد السبب**
   ...
   - اذا زاد التراكم مع اخفاقات العمر، افحص القبول (`/v1/sorafs/pin/*`) لتاكيد البيانات التي تنتظر موافقة المجلس.
...
2. **التشغيل الجاف في التدريج**
   ...
   - اختبر endpoints `/v1/sorafs/pin/{digest}` و `/v1/sorafs/aliases` معادلة تركيبية تغطي حالات طازجة و Refresh-window و منتهية الصلاحية و Hard-Expired. تحقق من اكواد HTTP و الرؤوس (`Sora-Proof-Status`, `Retry-After`, `Warning`) وحقول جسم JSON تجاه هذا الدليل.
