...
سطح `sorafs_cli` الموحد (provided by the `sorafs_orchestrator` crate) SoraFS المصنوعات اليدوية المزيد من المعلومات عن فضح الكرتا. يستخدم كتاب الطبخ هذا سير العمل بشكل عام؛ السياق التشغيلي هو عبارة عن خطوط أنابيب واضحة وكتيبات تشغيل منسقة متزامنة.
