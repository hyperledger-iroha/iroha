...
يشير كتاب التشغيل هذا إلى مراقبة وتجربة Pin Registry SoraFS وتوافقات مستوى الخدمة (SLA) الخاصة بالنسخ المتماثل. يتم تصدير المقاييس من `iroha_torii` عبر Prometheus إلى مساحة الاسم `torii_sorafs_*`. Torii يقوم بتنشيط حالة التسجيل لمدة 30 ثانية في المخطط الأخير، مع بقاء لوحات المعلومات في نفس الوقت عندما يقوم المشغل باستجواب نقاط النهاية `/v2/sorafs/pin/*`. قم باستيراد لوحة المعلومات المنسقة (`docs/source/grafana_sorafs_pin_registry.json`) للتخطيط Grafana قبل العمل الذي يتوافق مباشرة مع الأقسام ci-dessous.
...
## سير العمل في الفرز1. **معرف السبب**
   ...
   - إذا تم زيادة التراكم باستخدام إسطبلات الفحص، قم بفحص القبول (`/v2/sorafs/pin/*`) لتأكيد البيانات في انتظار الموافقة على الاستشارة.
...
2. **التشغيل الجاف والتدريج**
   ...
   - قم بتمرين نقاط النهاية `/v2/sorafs/pin/{digest}` و`/v2/sorafs/aliases` باستخدام مواد صناعية أولية تغطي فترة جديدة ونافذة تحديث ومنتهية الصلاحية ومنتهية الصلاحية. التحقق من رموز HTTP والرؤوس (`Sora-Proof-Status`، `Retry-After`، `Warning`) وأبطال مجموعة JSON ضد دليل التشغيل هذا.
