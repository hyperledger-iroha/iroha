...
## تدفق الثلاثة1. **تحديد السبب**
   ...
   - إذا نشأ تراكم الأعمال مع مؤسسات مفقودة، فافحص القبول (`/v2/sorafs/pin/*`) لتأكيد البيانات على أمل الحصول على المشورة.
...
2. **التشغيل الجاف والتدريج**
   ...
   - استخدم نقاط النهاية `/v2/sorafs/pin/{digest}` و`/v2/sorafs/aliases` مع اختبارات عقلانية تحمي الحالات الجديدة ونافذة التحديث والمنتهية الصلاحية والمنتهية الصلاحية. التحقق من صحة رموز HTTP والرؤوس (`Sora-Proof-Status` و`Retry-After` و`Warning`) ومجالات الجسم JSON مقابل دليل التشغيل هذا.
3. **التمكن من الإنتاج**
