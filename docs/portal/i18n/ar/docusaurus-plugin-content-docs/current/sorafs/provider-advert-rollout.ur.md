...
2. **إعادة الإنشاء باستخدام أدوات الموفر.**
   ...
   - `/v2/sorafs/providers` و`sorafs_fetch` للتحقق من صحة البطاقة؛ غير معروف
...
- **الاستجابة للحوادث.** إذا `reject` تنبيهات فورية يمكنك الاتصال عند الاتصال:
  1. اكتشاف Torii (`/v2/sorafs/providers`) هو جلب إعلان مخالف.
  2. يمكن لخط أنابيب الموفر التحقق من صحة الإعلان مرة أخرى و`/v2/sorafs/providers` ومقارنة خطأ إعادة إنتاج الخطأ.
