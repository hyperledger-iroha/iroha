...
1. **الأمر سيان**
   ...
   - إذا كانت الأعمال المتراكمة والأخطاء تستحق القبول (`/v2/sorafs/pin/*` بشكل واضح) فإن هذا الإجراء يتجلى في رؤية المستشار المنتظر.
...
2. ** التدريج للتشغيل الجاف **
   ...
   - نقاط النهاية `/v2/sorafs/pin/{digest}` و`/v2/sorafs/aliases` هي البراهين الاصطناعية التي لا تزال جديدة ونافذة التحديث ومنتهية الصلاحية ومنتهية الصلاحية. رموز حالة HTTP، والرؤوس (`Sora-Proof-Status`، `Retry-After`، `Warning`) وحقول نص JSON الموجودة في دليل التشغيل تتوافق مع التحقق من الصحة.
3. **الإنتاج فعال**
