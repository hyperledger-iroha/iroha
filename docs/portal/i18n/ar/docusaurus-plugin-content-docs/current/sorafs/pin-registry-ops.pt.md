...
## فلوكسو دي ترياجيم1. **تحديد السبب**
   ...
   - إذا نشأ تراكم الأعمال المتراكمة مع المفقودين، فافحص القبول (`/v1/sorafs/pin/*`) لتأكيد بيانات الضمان التي تم تقديمها للمشورة.
...
2. ** التدريج الجاف **
   ...
   - ممارسة نقاط النهاية `/v1/sorafs/pin/{digest}` و`/v1/sorafs/aliases` مع اختبار cobrindo كاسوس جديدة، نافذة التحديث، منتهية الصلاحية ومنتهية الصلاحية. رموز HTTP صالحة، والرؤوس (`Sora-Proof-Status`، `Retry-After`، `Warning`) ومجالات جسم JSON مقابل دليل التشغيل هذا.
3. **التمكن من الإنتاج**
