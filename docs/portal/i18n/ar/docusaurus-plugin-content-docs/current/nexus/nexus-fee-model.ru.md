...
- كل معاملة غاز مملوء بالغاز، اكتب `LaneSettlementReceipt`. في حالة وجود إيصال استلام، مُعرِّف تعريفي مُسبق، مجموع صغير محلي، XOR إلى لوحة غير منتظمة، بجانب XOR بعد قص الشعر، والتباين الفعلي (`xor_variance`) وكتلة زمنية صغيرة بالمللي ثانية.
- قم بتجميع الإيصالات في الممر/مساحة البيانات ونشرها عبر `lane_settlement_commitments` في `/v1/sumeragi/status`. توضح هذه العناصر `total_local_amount` و`total_xor_due` و`total_xor_after_haircut`، وهي مغمورة في كتلة للتدفئة الليلية.
- تم حذف الكمبيوتر الجديد `total_xor_variance`، حيث تم إلغاء الحماية الآمنة (الفرق بين XOR الجديد و المراقبة بعد الحلاقة)، `swap_metadata` توثق محادثات تحديد معلمات (TWAP، epsilon، ملف تعريف السيولة وvolatility_class)، لمدققي الحسابات يمكنك التحقق من المعلمات غير المرغوب فيها لتكوين النطاق.
