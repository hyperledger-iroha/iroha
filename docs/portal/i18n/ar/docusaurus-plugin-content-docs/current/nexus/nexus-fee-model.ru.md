...
- قم بتجميع الإيصالات في الممر/مساحة البيانات ونشرها عبر `lane_settlement_commitments` في `/v1/sumeragi/status`. توضح هذه العناصر `total_local_micro` و`total_xor_due_micro` و`total_xor_after_haircut_micro`، وهي مغمورة في كتلة للتدفئة الليلية.
