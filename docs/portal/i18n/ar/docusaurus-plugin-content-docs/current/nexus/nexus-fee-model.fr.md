...
- يؤدي تنفيذ الكتل إلى تجميع العناصر المخصصة للمسار/مساحة البيانات ونشرها عبر `lane_settlement_commitments` في `/v2/sumeragi/status`. تعرض جميع العناصر `total_local_micro` و`total_xor_due_micro` و`total_xor_after_haircut_micro` بالإضافة إلى الكتلة من أجل صادرات المصالحة الليلية.
