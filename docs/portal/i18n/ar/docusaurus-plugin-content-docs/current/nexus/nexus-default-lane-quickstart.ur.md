...
- **اختبارات الأنابيب/SSE.** عوامل تصفية حدث المعاملة `tx_lane_id == <u32>` مسندات قبول کرتے ہیں (دیکھیں `docs/source/pipeline.md`). `/v2/pipeline/events/transactions` الذي يقوم بالتصفية للاشتراك في ميزة التسجيل أو وجود ممر واضح يتم كتابة معرف المسار الاحتياطي فيه تحت القائمة.
