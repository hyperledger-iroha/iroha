...
- **اختبارات الأنابيب/SSE.** تقبل مرشحات أحداث المعاملات المسندات `tx_lane_id == <u32>` (تظهر `docs/source/pipeline.md`). أدخل `/v2/pipeline/events/transactions` مع هذا الفلتر للتأكد من أن الكتب المرسلة بدون مسار صريح تصل إلى معرف المسار الاحتياطي.
