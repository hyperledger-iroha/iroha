...
- تنفيذ الكتل التي يتم جمعها من خلال المسار/مساحة البيانات ونشرها عبر `lane_settlement_commitments` في `/v2/sumeragi/status`. كل ما في الأمر هو `total_local_micro`، `total_xor_due_micro` و`total_xor_after_haircut_micro`، لا يوجد كتلة للتصدير في أيام التوفيق.
