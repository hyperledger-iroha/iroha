...
- التكامل الشامل لليصالات لكل حارة/مساحة بيانات وينشرها عبر `lane_settlement_commitments` في `/v1/sumeragi/status`. تم عرض الاجماليات `total_local_micro` و`total_xor_due_micro` و`total_xor_after_haircut_micro` مجمعة على مستوى الشفافية من اجل مستندات المطابقة الذهبية.
