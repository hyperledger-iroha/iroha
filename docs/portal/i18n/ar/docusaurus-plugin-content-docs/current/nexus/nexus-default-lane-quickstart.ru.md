...
- **اختبارات خطوط الأنابيب/SSE.** مرشحات مبدأ المعاملات الخاصة `tx_lane_id == <u32>` (برمز `docs/source/pipeline.md`). قم بالإشارة إلى `/v2/pipeline/events/transactions` باستخدام هذا الفلتر للإشارة إلى أنه يتم التحكم فيه بدون حارة جديدة، ويتم الحصول على معرف المسار الاحتياطي.
