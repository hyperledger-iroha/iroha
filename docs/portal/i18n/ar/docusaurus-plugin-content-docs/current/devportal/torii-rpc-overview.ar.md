...
Norito-RPC هو الإصدار الجديد لواجهات Torii. أعاد استخدام مسارات HTTP لنفسه مثل `/v1/pipeline` لكنه يتبادل حمولات مؤطرة عبر Norito تتضمن hashes للمخطط و المجموع الاختباري. استخدمه عندما تحتاج الى ردود حتمية ومتحقق منها او عندما تصبح استجابات JSON الخاصة بالـ Pipeline رقيقة زجاجة.
...
2. قم بتسجيل الدخول إلى البطاقة **جربها** في هذه الصفحة او لوحة `/reference/torii-swagger` والعالم بطريقا مثل `POST /v1/pipeline/submit`.
