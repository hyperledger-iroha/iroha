...
التدفق يفترض أنك تتحدث عن الثنائي `sorafs_cli` (built from the `sorafs_orchestrator` Cargo target)، الوصول إلى
...
- **روابط التذكرة.** راجع معرفات اللوحة Grafana أو تصدير image الملحقة للتذكرة
