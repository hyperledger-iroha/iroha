...
|-----------|--------|
| البروتوكولات | REST bajo `/v2/sns/*` وخدمة gRPC `sns.v1.Registrar`. Ambos aceptan Norito-JSON (`application/json`) وNorito-RPC ثنائي (`application/x-norito`). |
...
|----------|-------|--------|-------------|
| `/v2/sns/registrations` | مشاركة | `RegisterNameRequestV1` | قم بالتسجيل أو إعادة إنشاء اسم. قم بإعادة تحديد مستوى السعر، وصلاحية الدفع/الإشراف، وإصدار أحداث التسجيل. |
| `/v2/sns/registrations/{selector}/renew` | مشاركة | `RenewNameRequestV1` | تمديد النهاية. Aplica ventanas de gracia/redencion segun la politica. |
| `/v2/sns/registrations/{selector}/transfer` | مشاركة | `TransferNameRequestV1` | قم بنقل بعض الملحقات الإضافية لمتطلبات الإدارة. |
| `/v2/sns/registrations/{selector}/controllers` | ضع | `UpdateControllersRequestV1` | استبدال مجموعة وحدات التحكم؛ اتجاهات صالحة للحسابات التجارية. |
| `/v2/sns/registrations/{selector}/freeze` | مشاركة | `FreezeNameRequestV1` | تجميد الوصي/المجلس. يتطلب تذكرة الوصي والرجوع إلى جدول أعمال الحكومة. |
| `/v2/sns/registrations/{selector}/freeze` | حذف | `GovernanceHookV1` | قم بإلغاء تجميد العلاج؛ asegura تجاوز مجلس التسجيل. |
| `/v2/sns/reserved/{selector}` | مشاركة | `ReservedAssignmentRequestV1` | تعيين الأسماء المحجوزة من قبل المضيف/المجلس. |
| `/v2/sns/policies/{suffix_id}` | احصل على | -- | احصل على `SuffixPolicyV1` الفعلي (قابل للتخزين المؤقت). |
| `/v2/sns/registrations/{selector}` | احصل على | -- | Devuelve `NameRecordV1` الفعلي + الحالة الفعالة (نشط، نعمة، وما إلى ذلك). |
...
````--governance-json` يجب أن يحتوي على سجل `GovernanceHookV1` صالح (معرف الاقتراح، تجزئات التصويت، شركة المضيف/الوصي). كل أمر يشير ببساطة إلى نقطة النهاية `/v2/sns/registrations/{selector}/...` التي تتوافق مع مشغلي الإصدار التجريبي من الأسطح Torii التي تتعرف على SDKs تمامًا.
...
Torii التحقق من اختبار المنتج:1. معرف الاقتراح موجود في دفتر الأستاذ الحكومي (`/v2/governance/proposals/{id}`) والحالة هي `Approved`.
...
1. يستشير العميل `/v2/sns/policies/{suffix_id}` للحصول على أسعار معقولة ومستويات متاحة.
...
3. من خلال العلاج، تجاوز انبعاث المجلس؛ يقوم المشغل بإرسال DELETE `/v2/sns/registrations/{selector}/freeze` مع `GovernanceHookV1`.
...
- تقوم عمليات الدفع بإعادة استخدام إيصالات دفتر الأستاذ Norito؛ خدمات البحث المثبتة لمساعد واجهات برمجة التطبيقات (`/v2/finance/sns/payments`).
