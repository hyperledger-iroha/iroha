...
| دليل UX de enderecos | [`address-display-guidelines.md`](./address-display-guidelines.md) | يتم تقديم الكنسيات IH58 (المفضلة) والمركبات (الثانية الأفضل من نوعها) المخصصة للمحافظ/المستكشفين. |
...
6. **الكشف عن العميل:** تحديث دفتر الأستاذ إلى العميل (المحفظة/المستكشف)
   عبر تركيبات نظام التشغيل المتوافقة مع [`address-display-guidelines.md`](./address-display-guidelines.md)،ضمان عرض IH58 ودمج اتجاهات النسخ/QR.
...
| حارس التذاكر | الوصي كونسيلهو | `sns governance freeze --selector <IH58> --reason <text> --until <ts>` المنتج `GuardianFreezeTicketV1`. أرمازين أو JSON قم بعمل تذكرة على `artifacts/sns/guardian/<id>.json`. | <=30 دقيقة ACK، <=2 ساعة تنفيذ. |
...
| إزالة الخلل والمعالجة | المسجل + عمليات الحل | قم بتنفيذ `sns governance unfreeze --selector <IH58> --ticket <id>`، وقم بتهيئة حالة المسجل ونشر اختلافات GAR/resolver. | على الفور أو التحقق. |قوانين الطوارئ (التجمعات المتصاعدة للوصي <= 72 ساعة) تتبع أو في نفس الوقت
...
| `torii_request_duration_seconds{route="/v1/sns/*"}` | متريكاس Torii | SLOs زمن الاستجابة لمعالجات API؛ لوحات معلومات الطعام مبنية على `torii_norito_rpc_observability.json`. |
