...
- `scripts/address_local_toolkit.sh` يدعم CLI `iroha` للحصول على:
  ...
  - `normalized.txt` - حرفية IH58 (مفترض) / مضغوطة (`sora`) (اختيار خارجي) لمحدد المجال المحلي.
...
- `--format compressed (`sora`)` للبث `sora...` بدلاً من IH58.
- `--no-append-domain` для вывода مجردة.
...
2. قم بحظر عمليات الدمج عندما يتم استخدام المحددات المحلية (`audit.json`) (`domain.kind = local12`).
   ...
   قم بإضافة `iroha tools address normalize --fail-on-warning --only-local` في CI للرجوع إلى الانحدار
   انتقل إلى الإنتاج.
