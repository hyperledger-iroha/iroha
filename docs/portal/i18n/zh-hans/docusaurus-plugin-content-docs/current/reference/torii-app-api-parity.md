...
因此 mono-repo 之外的读者可以看到哪些 `/v2/*` 表面已接线、经过测试、
...
功能门控路线构建器。对于路线图中的每个 `/v2/*` 表面，我们验证了：
...
### 帐户权限 (`/v2/accounts/{id}/permissions`) — 涵盖
...
### 别名 OPRF 评估 (`POST /v2/aliases/voprf/evaluate`) — 已覆盖
...
### Nexus 连接 (`/v2/connect/*`) — 涵盖（功能 `connect`）
