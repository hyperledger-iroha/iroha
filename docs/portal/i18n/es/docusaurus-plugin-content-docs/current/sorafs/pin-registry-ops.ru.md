...
## Clasificación del flujo de trabajo1. **Определить причину**
   ...
   - Если trabajo pendiente растет при стабильных пропусках, проверьте admisión (`/v2/sorafs/pin/*`), чтобы подтвердить manifiestos, ожидающие утверждения совета.
...
2. **Ejecución en seco y puesta en escena**
   ...
   - Progonia los puntos finales `/v2/sorafs/pin/{digest}` e `/v2/sorafs/aliases` con pruebas sintéticas, que incluyen ventanas frescas, de actualización, caducadas y duramente caducadas. Proveer códigos HTTP, encabezados (`Sora-Proof-Status`, `Retry-After`, `Warning`) y archivos JSON de este runbook exclusivo.
