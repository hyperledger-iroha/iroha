...
- `blinded_cid_opt_outs` يسرد digests المانيفست (blinded CIDs بصيغة hex
  بأحرف كبيرة). التطابقات تفرض أيضاً الجدولة direct-only وتُظهر
...
`upload-pq-only`، فإن استثناءات الولايات أو المانيفست تفرض النقل direct-only
وتفشل سريعاً عندما لا يوجد مزوّدون متوافقون.

توجد كتالوجات opt-out المعتمدة في
...
| `--transport-policy` / `OrchestratorConfig::with_transport_policy` | الافتراضي `soranet-first` الآن بعد أن أصبح المُنسِّق متعدد المصادر أساسياً. استخدم `direct-only` عند التحضير لخفض أو امتثال، واحجز `soranet-strict` لتجارب PQ-only؛ تظل تجاوزات الامتثال سقفاً صارماً. |
...
تحتوي اللقطة الثنائية على:

- `version` — نسخة المخطط (حالياً `2`).
...
- `relays` — قائمة بحزم SRCv2 (خرج `RelayCertificateBundleV2::to_cbor()`). تحمل
  كل حزمة واصف relay وأعلام القدرات وسياسة ML-KEM وتوقيعات Ed25519/ML-DSA-65
  المزدوجة.
...
يجدد المدير الدوائر كلما تغيرت بيانات الحارس (endpoint أو مفتاح PQ أو الطابع
...
- `prewarm_circuits`, `max_streams_per_circuit`, `circuit_ttl_hint_secs` تعطي
...
في كلا الحالتين يزوّد الوكيل HMAC الخاص بـ cache-tag (عند وجود مفتاح cache أثناء
...
يصدر المُنسِّق المقاييس التالية عبر `iroha_telemetry`:
...
`--anonymity-policy-override=` إلى `sorafs_cli fetch`). عند وجود أي override،
