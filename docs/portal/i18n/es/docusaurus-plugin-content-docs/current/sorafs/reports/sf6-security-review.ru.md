...
**Artículos:**
- CLI y pruebas de dispositivos (`crates/sorafs_orchestrator/src/bin/sorafs_cli.rs`)
...
- Arnés de paridad determinista (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`, [Отчет о паритете GA SoraFS Orchestrator](./orchestrator-ga-parity.md))
...
4. Arnés de paridad lógica (`cargo test -p sorafs_orchestrator --test sorafs_cli proof_stream_consumes_ndjson_and_reports_metrics -- --nocapture`).
