...
- PUBLICACIÓN `/v2/contracts/deploy`
  ...
  - Mentira:
    - GET `/v2/contracts/code/{code_hash}` -> enviar el manifiesto en stock
    - OBTENER `/v2/contracts/code-bytes/{code_hash}` -> enviar `{ code_b64 }`
...
- Parámetro personalizado: `max_contract_code_bytes` (JSON u64)
  ...
  - Los operadores pueden ajustar mediante `SetParameter(Custom)` con `id = "max_contract_code_bytes"` y una carga útil numérica.- PUBLICACIÓN `/v2/gov/ballots/zk`
  ...
  - Notas:
    ...
    - La ejecución del contrato debe apelar `ZK_VOTE_VERIFY_BALLOT` antes de presentar `SubmitBallot`; Los hosts imponen un pestillo a una sola vez.- PUBLICACIÓN `/v2/gov/ballots/plain`
  ...
  - Respuesta: { "ok": verdadero, "aceptado": verdadero, "tx_instructions": [{...}] }
  - Notas: les re-votes sont en extension seule - una nueva votación no puede reducir el monto o el vencimiento del verrou existente. El `owner` tiene la misma autoridad de transacción. La duración mínima es `conviction_step_blocks`.- PUBLICACIÓN `/v2/gov/finalize`
  ...
  - Notas:
    ...
    - Las verificaciones de participación se utilizan solo para aprobar+rechazar; abstenerse ne compte pas pour le participación.- PUBLICACIÓN `/v2/gov/enact`
...
- OBTENER `/v2/gov/council/current`
  ...
  - Notas: renvoie le Council persiste si presente; Sinon deriva un respaldo determinista con el activo de participación configurado y los seguros (miroir de la spec VRF justqu'a ce que des preuves VRF en direct soient persistees on-chain).- PUBLICACIÓN `/v2/gov/council/derive-vrf` (característica: gov_vrf)
...
- POST `/v2/gov/protected-namespaces` - aplique `gov_protected_namespaces` directement sur le noeud.
...
- `iroha --output-format text app gov deploy audit --namespace apps [--contains calc --hash-prefix deadbeef]`
  - Recupere las instancias de contrato para el espacio de nombres y verifique que:
    ...
    - Le manifeste stocke sous `/v2/contracts/code/{code_hash}` rapporte des valeurs `code_hash` et `abi_hash` corresponsales.
...
- GET `/v2/gov/instances/{ns}`: lista las instancias de contrato activas para un espacio de nombres.
...
Los operadores y servicios pueden inspeccionar y retransmitir las cargas útiles a través de:- Torii: `GET /v2/sumeragi/evidence` y `GET /v2/sumeragi/evidence/count`.
...
4. **Audite los resultados** vía `/v2/sumeragi/evidence` e `/v2/sumeragi/status` para confirmar que le compteur d'evidence a avance et que la gouvernance a applique le retrait.
...
- El tiempo de ejecución y la CLI exponen los parámetros organizados a través de `/v2/sumeragi/params` e `iroha --output-format text ops sumeragi params`, para que los operadores confirmen los botones de activación y las listas de validadores.
- La automatización de la gobernanza de siempre:
  ...
  3. Surveiller `/v2/sumeragi/status` jusqu'a ce que `effective_consensus_mode` bascule a la hauteur escort.
