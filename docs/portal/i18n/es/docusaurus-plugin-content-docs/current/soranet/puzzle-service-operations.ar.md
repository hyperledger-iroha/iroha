...
- `POST /v1/puzzle/mint` - يصدر تذكرة Argon2; cuerpo JSON اختياري
...
- `POST /v1/token/mint` - يصدر ML-DSA token de admisión مرتبطا بـ resume hash المقدم؛
...
observador لملف revocación بالحفاظ على `/v1/token/config` محدثا؛ نسق التحديثات مع
...
`POST /v1/puzzle/mint` y `"signed": true` (y `"transcript_hash_hex"`) para
...
4. **التحقق.** Haga clic en `POST /v1/puzzle/mint` y en `difficulty` y `expires_at`.
   ...
   `/v1/token/config` Revocación de la huella digital del emisor
...
4. راقب `soranet_handshake_pow_difficulty` para obtener hashcash
   Utilice el `/v1/puzzle/config` y el `puzzle = null`.
...
- **Alineación del rompecabezas:** يجب ان يطابق `soranet_handshake_pow_difficulty` الصعوبة
  المعادة من `/v1/puzzle/config`. يشير الاختلاف الى تكوين relé قديم او فشل اعادة
  التشغيل.
- **Preparación del token:** نبه اذا انخفض `/v1/token/config` الى `enabled = false`
...
- **Salud del servicio:** افحص `/healthz` وفق cadence liveness المعتادة ونبه اذا
  Utilice `/v1/puzzle/mint` para HTTP 500 (para obtener información sobre Argon2 y
  فشل RNG). تظهر اخطاء token acuñación عبر استجابات HTTP 4xx/5xx على `/v1/token/mint`؛
