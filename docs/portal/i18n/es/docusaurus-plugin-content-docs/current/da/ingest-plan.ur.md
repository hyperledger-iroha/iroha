...
- `iroha app da get` Alias centrado en DA فراہم کرتا ہے جو multi-source Orchestrator کو use کرتا ہے جو پہلے ہی
  ...
  **یا** Ticket de almacenamiento Torii a través de `--storage-ticket` دے سکتے ہیں۔ Ruta del billete پر CLI `/v2/da/manifests/<ticket>` سےdescarga de manifiesto کرتی ہے، paquete کو `artifacts/da/fetch_<timestamp>/` میں محفوظ کرتی ہے (anulación mediante
...
- `iroha app da get-blob` Torii سے `GET /v2/da/manifests/{storage_ticket}` کے ذریعے manifiestos canónicos کھینچتا ہے۔
