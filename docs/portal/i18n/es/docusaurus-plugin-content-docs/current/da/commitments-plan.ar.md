...
- كشف استعلامات Torii (`/v1/da/commitments/*`) y براهين تسمح للـ relés y SDK
...
4. **Use RPC en Torii** para listar/consultar/probar datos `/v1/da/commitments`.
...
|--------|---------|---------|---------|
| `/v1/da/commitments` | `POST` | `DaCommitmentQuery` (carril/época/secuencia, sin paginación) | El `DaCommitmentPage` contiene datos y hash. |
| `/v1/da/commitments/prove` | `POST` | `DaCommitmentProofRequest` (carril + hash de manifiesto y tupla `(epoch, sequence)`). | Nombre `DaCommitmentProof` (registro + archivo Merkle + hash الكتلة). |
| `/v1/da/commitments/verify` | `POST` | `DaCommitmentProof` | مساعد apátrida يعيد حساب hash الكتلة ويتحقق من الاشتمال؛ Los SDK están disponibles en `iroha_crypto`. |
