...
- `koto build`, `ivm_run`, y binarios `iroha_cli`. آپ انہیں checkout del espacio de trabajo سے نیچے دکھائے گئے طریقے کے مطابق بنا سکتے ہیں یا artefactos de lanzamiento coincidentes ڈاؤن لوڈ کر سکتے ہیں:
...
Runner `debug::info("Hello from Kotodama")` لاگ پرنٹ کرتا ہے اور burlado host کے خلاف `SET_ACCOUNT_DETAIL` syscall انجام دیتا ہے۔ Configuración binaria `ivm_tool` y encabezado ABI `ivm_tool inspect target/quickstart/hello.to`, bits de características y puntos de entrada exportados
