...
- **Rutas SoraNet.** `streaming.soranet.*` controla el transporte anonimo: `exit_multiaddr` (por defecto `/dns/torii/udp/9443/quic`), `padding_budget_ms` (por defecto 25 ms), `access_kind` (`authenticated` vs `read-only`), `channel_salt` opcional, `provision_spool_dir` (por defecto `./storage/streaming/soranet_routes`), `provision_spool_max_bytes` (por defecto 0, ilimitado), `provision_window_segments` (por defecto 4) y `provision_queue_capacity` (por defecto 256).
