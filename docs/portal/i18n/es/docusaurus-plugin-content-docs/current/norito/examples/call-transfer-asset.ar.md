...
- Utilice el cable de alimentación (`<katakana-i105-account-id>`) para conectar el cable y el conector de `CanTransfer` y el cable de alimentación.
- Presione el botón `call_transfer_asset` para 5 veces el conector `<katakana-i105-account-id>`, y luego presione el botón `<katakana-i105-account-id>`. الأتمتة على السلسلة لنداءات المضيف.
- Para cambiar la configuración de `FindAccountAssets` o `iroha_cli ledger assets list --account <katakana-i105-account-id>` y de las opciones de configuración سياق النقل.
