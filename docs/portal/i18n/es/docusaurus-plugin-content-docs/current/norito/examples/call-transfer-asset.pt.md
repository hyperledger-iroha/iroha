...
- Financie a autoridade do contrato (por ejemplo `<i105-account-id>`) con o activo que ela transferira e conceda a autoridade o papel `CanTransfer` o permiso equivalente.
- Chame o Entrypoint `call_transfer_asset` para transferir 5 unidades del contacto del contrato para `<i105-account-id>`, reflejando como un automático en cadena que puede involucrar chamadas do host.
- Verifique saldos vía `FindAccountAssets` o `iroha_cli ledger assets list --account <i105-account-id>` e inspeccione eventos para confirmar que o guard de metadados registrou o contexto da transferencia.
