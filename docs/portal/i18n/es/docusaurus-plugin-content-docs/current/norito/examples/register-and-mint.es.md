...
- Asegúrese de que exista la cuenta de destino (por ejemplo `<katakana-i105-account-id>`), reflejando la fase de configuración en cada inicio rápido del SDK.
...
- Verifica los saldos mediante `client.request(FindAccountAssets)` o `iroha_cli ledger assets list --account <katakana-i105-account-id>` para confirmar que la acuñación tuvo éxito.
