...
- Asegúrese de que la cuenta de destino (por ejemplo, `<i105-account-id>`) exista, reflejando la fase de instalación en cada SDK de inicio rápido.
...
- Verifique las ventas a través de `client.request(FindAccountAssets)` o `iroha_cli ledger assets list --account <i105-account-id>` para confirmar que el frappe a réussi.
