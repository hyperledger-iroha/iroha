...
Banderas chave:- `ABI V1` arregla el contrato na versao ABI 1 (un unica soportada no momento).
- `--max-cycles 1000000` solicita ejecución sin límite; Defina un número positivo para limitar el relleno de ciclos para probar el resultado cero.
...
- Si `koto build` rejeitar a versao ABI, verifique si o compilador y o nodo miram ABI v1 (rode `koto build --help` sem argumentos para listar o soporte).
