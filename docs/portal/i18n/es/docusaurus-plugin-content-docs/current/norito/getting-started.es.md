...
Claves de opciones:- `ABI V1` fija el contrato a la versión ABI 1 (la única soportada al momento de escribir).
- `--max-cycles 1000000` solicita ejecucion sin limites; establece un número positivo para acotar el padding de ciclos para pruebas de conocimiento cero.
...
- Si `koto build` rechaza la versión ABI, verifique que el compilador y el nodo apunten a ABI v1 (ejecuta `koto build --help` sin argumentos para listar soporte).
