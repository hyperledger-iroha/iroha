...
|------|---------|---------|---------------------|
| **P0 - Paridad de laboratorio** | Q2 2025 | Las suites smoke de Rust CLI + Python ejecutan `/v2/norito-rpc` en CI, el helper de JS pasa pruebas unitarias, el harness mock de Android ejercita ambos transportes. | `python/iroha_python/scripts/run_norito_rpc_smoke.sh` y `javascript/iroha_js/test/noritoRpcClient.test.js` en verde en CI; el harness de Android conectado a `./gradlew test`. |
