...
- **Pruebas de tubería/SSE.** Los filtros de eventos de transacao aceitam predicados `tx_lane_id == <u32>` (veja `docs/source/pipeline.md`). Assine `/v2/pipeline/events/transactions` com esse filtro para provar que escritas enviadas sem lane explícito chegam sollozo o id de lane de fallback.
