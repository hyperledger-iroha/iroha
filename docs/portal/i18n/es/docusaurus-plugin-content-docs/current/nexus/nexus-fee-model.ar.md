...
- تنفيذ الكتلة يجمع الايصالات لكل lane/dataspace y عبر `lane_settlement_commitments` في `/v2/sumeragi/status`. Los dispositivos `total_local_micro`, `total_xor_due_micro` y `total_xor_after_haircut_micro` están conectados a una fuente de alimentación.
