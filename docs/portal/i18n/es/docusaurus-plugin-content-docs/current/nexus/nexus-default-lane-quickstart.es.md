...
- **Pruebas de tubería/SSE.** Los filtros de eventos de transacciones aceptan predicados `tx_lane_id == <u32>` (ver `docs/source/pipeline.md`). Suscríbase a `/v1/pipeline/events/transactions` con ese filtro para demostrar que las escrituras enviadas sin un carril explícito llegan bajo el id de carril de fallback.
