...
- **Pruebas de canalización/SSE.** filtros de eventos de transacción Predicados `tx_lane_id == <u32>` قبول کرتے ہیں (دیکھیں `docs/source/pipeline.md`). `/v2/pipeline/events/transactions` کو اس filtro کے ساتھ suscribirse کریں تاکہ یہ ثابت ہو کہ carril explícito کے بغیر بھیجی گئی escribe ID de carril alternativo کے تحت پہنچتی ہیں۔
