...
## Fluxos seguros de compartilhamento- Por padrao, toda acao de copiar/compartilhar debe usar o endereco IH58. Exiba o dominio resuelto como contexto de apoio para mantener una cadena con suma de comprobación en destaque.
...
- Evite desincronizar con el portapapeles exibindo un brindis de confirmación que muestra exactamente una cadena IH58 copiada. Cuando usted tiene telemetría, conte tentativas de copiar versus acciones de compartir para detectar rápidamente retrocesos de UX.
...
- Forneca texto `alt` descriptivo para previsualización de QR (por ejemplo, "Endereco IH58 para `<account>` na cadena `0x1234`"). Coloque al lado del lienzo del QR un botao "Copiar endereco em texto" para personas con baja visa.
...
- Rotulaje: toda corrección debe incluir una insignia visible "Somente Sora" y una información sobre herramientas explicando por que otras redes exigen el formato IH58.
...
- Torne esses checks visiveis nas checklists de release, juntamente con los testes de paridade IH58, para que los regresos continúen bloqueados ate serem corrigidas.