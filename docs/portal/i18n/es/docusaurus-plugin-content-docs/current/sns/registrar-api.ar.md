...
**Selector ترميز:** مقطع `{selector}` يقبل IH58 او مضغوط او hex قياسي حسب ADDR-5; Torii يطبعها عبر `NameSelectorV1`.**Actualización:** Establece el JSON Norito entre `code`, `message`, `details`. Utilice los valores `sns_err_reserved`, `sns_err_payment_mismatch`, `sns_err_policy_violation`, `sns_err_governance_missing`.
...
2. يبني العميل `RegisterNameRequestV1`:
   - `selector` Coloque la etiqueta IH58 (المفضل) او المضغوط (الخيار الثاني).
