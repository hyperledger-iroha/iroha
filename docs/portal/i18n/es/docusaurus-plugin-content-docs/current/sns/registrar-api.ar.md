...
| المتطلب | التفاصيل |
|---------|----------|
| Artículos | REST utiliza `/v2/sns/*` y gRPC `sns.v1.Registrar`. Utilice Norito-JSON (`application/json`) y Norito-RPC (`application/x-norito`). |
...
|-------------|---------|---------|-------|
| `/v2/sns/registrations` | PUBLICAR | `RegisterNameRequestV1` | تسجيل او اعادة فتح اسم. يحل شريحة التسعير، يتحقق من اثباتات الدفع/الحوكمة, ويصدر احداث السجل. |
| `/v2/sns/registrations/{selector}/renew` | PUBLICAR | `RenewNameRequestV1` | يمدد المدة. يفرض نوافذ gracia/redención من السياسة. |
| `/v2/sns/registrations/{selector}/transfer` | PUBLICAR | `TransferNameRequestV1` | ينقل الملكية بعد ارفاق موافقات الحوكمة. |
| `/v2/sns/registrations/{selector}/controllers` | PONER | `UpdateControllersRequestV1` | يستبدل مجموعة controladores؛ يتحقق من عناوين الحساب الموقعة. |
| `/v2/sns/registrations/{selector}/freeze` | PUBLICAR | `FreezeNameRequestV1` | تجميد tutor/consejo. يتطلب تذكرة guardian ومرجع دفتر حوكمة. |
| `/v2/sns/registrations/{selector}/freeze` | BORRAR | `GovernanceHookV1` | فك التجميد بعد المعالجة؛ Esta es una anulación de la función. |
| `/v2/sns/reserved/{selector}` | PUBLICAR | `ReservedAssignmentRequestV1` | تعيين اسماء محجوزة بواسطة administrador/consejo. |
| `/v2/sns/policies/{suffix_id}` | OBTENER | -- | يجلب `SuffixPolicyV1` الحالي (قابل للكاش). |
| `/v2/sns/registrations/{selector}` | OBTENER | -- | يعيد `NameRecordV1` الحالي + الحالة الفعلية (Activo, Gracia, الخ). |
...
`--governance-json` es el nombre de `GovernanceHookV1` (identificación de propuesta, hashes de voto, administrador/tutor). كل امر يعكس ببساطة نقطة النهاية `/v2/sns/registrations/{selector}/...` المقابلة حتى يتمكن مشغلو البيتا من تمرين اسطح Torii Estos son los SDK.
...
1. ID de propuesta موجود في دفتر الحوكمة (`/v2/governance/proposals/{id}`) وحالته `Approved`.
...
### 6.1 Actualizar1. يستعلم العميل `/v2/sns/policies/{suffix_id}` للحصول على الاسعار وفترة Grace والشرائح المتاحة.
...
3. Anulación de بعد المعالجة، يصدر المجلس; Haga clic en DELETE `/v2/sns/registrations/{selector}/freeze` o `GovernanceHookV1`.
...
- Otros productos relacionados con el producto Norito؛ خدمات الخزينة توفر API مساعدة (`/v2/finance/sns/payments`).
