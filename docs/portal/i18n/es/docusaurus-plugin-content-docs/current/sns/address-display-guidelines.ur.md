...
`examples/android/retail-wallet` میں مطلوبہ UX پیٹرن دکھایا گیا ہے:- **دو کاپی اہداف۔** دو واضح کاپی بٹنز دیں: I105 (ترجیحی) اور صرف Sora والا
  کمپریسڈ فارم (`sora...`، segundo mejor). I105 ہمیشہ بیرونی شیئرنگ کے لئے محفوظ ہے اور QR
...
- **Cargas útiles QR I105 ۔** Código QR کوڈز کو Codificación de cadena I105 کرنا چاہیے۔ QR
...
ایکسپلوررز کو والٹ کی telemetría اور accesibilidad کے کام کو espejo کرنا چاہیے:- کاپی بٹنز پر `data-copy-mode="i105|I105|qr"` لگائیں تاکہ فرنٹ اینڈز Torii
...
I105/کمپریسڈ لسٹ دونوں بناتا ہے جنہیں آپریٹرز tickets de preparación کے ساتھ منسلک
...
1. `iroha tools address inspect` اب I105, کمپریسڈ، اور cargas útiles hexadecimales canónicas کے ساتھ
   ...
   جب بھی آپ convertir شدہ codificación کو `<i105>@<domain>` کی صورت میں reproducir کرنا
...
2. SDK اسی وارننگ/resumen کو Ayudante de JavaScript کے ذریعے دکھا سکتے ہیں:
   ...
   ```
  literal auxiliar سے detectar کیا گیا prefijo I105 محفوظ رکھتا ہے جب تک آپ
  `networkPrefix` واضح طور پر فراہم نہ کریں؛ اس لئے redes no predeterminadas کے
  resúmenes خاموشی سے prefijo predeterminado کے ساتھ دوبارہ render نہیں ہوتے۔3. carga útil canónica کو `i105.value` یا `I105` فیلڈز سے reutilización کر کے تبدیل
...
5. بلک ڈیٹا سیٹس کے لئے
   ...
   اندراج کے لئے canonical/I105 (ترجیحی)/I105 (`sora`, segundo mejor) resúmenes کے ساتھ JSON رپورٹ بناتی
...
6. Reescritura de nueva línea a nueva línea چاہیے تو
  ...
  filas no locales چھوڑ دیتا ہے، باقی entradas کو مطلوبہ codificación (I105 ترجیحی/comprimido (`sora`) segundo mejor/hexadecimal/JSON)
