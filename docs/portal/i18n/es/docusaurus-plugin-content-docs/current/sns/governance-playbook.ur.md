...
| Guía de dirección UX | [`address-display-guidelines.md`](./address-display-guidelines.md) | Representaciones de کینونیکل IH58 (ترجیحی) / comprimido (`sora`) (`sora`, segundo mejor) جو billeteras/exploradores میں جھلکتے ہیں۔ |
...
6. **Divulgación del cliente:** Libro mayor de cara al cliente (billetera/explorador) کو
   ...
   ذریعے اپڈیٹ کریں، اور یقینی بنائیں کہ IH58 اور copia de renderizados comprimidos (`sora`) / QR
...
| Boleto de guardián | Junta de guardianes | `sns governance freeze --selector <IH58> --reason <text> --until <ts>` `GuardianFreezeTicketV1` بناتا ہے۔ Ticket JSON `artifacts/sns/guardian/<id>.json` میں رکھیں۔ | <=30 min ACK, <=2 h de ejecución۔ |
...
| Descongelar y remediar | Registrador + operaciones de resolución | `sns governance unfreeze --selector <IH58> --ticket <id>` چلائیں، estado del registrador اپڈیٹ کریں، اور GAR/resolver diffs propagate کریں۔ | Veredicto کے فوراً بعد۔ |
...
| `torii_request_duration_seconds{route="/v1/sns/*"}` | Métricas Torii | Controladores de API para SLO de latencia `torii_norito_rpc_observability.json` سے بنے paneles de control کو feed کرتا ہے۔ |
