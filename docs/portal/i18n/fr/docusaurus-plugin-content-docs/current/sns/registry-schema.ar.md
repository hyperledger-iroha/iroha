...
| `payment_asset_id` | `AsciiString` | معرف اصل règlement الافتراضي (مثلا `xor#sora`). |
...
2. Utilisez des modèles de résolveur (modèles IH58 + compressés (`sora`) pour les enregistrements de texte).
