...
| تذكرة gardien | مجلس gardien | `sns governance freeze --selector <IH58> --reason <text> --until <ts>` à `GuardianFreezeTicketV1`. La version JSON est `artifacts/sns/guardian/<id>.json`. | <=30 min ACK, <=2 h تنفيذ. |
...
| فك التجميد والمعالجة | المسجل + عمليات résolveur | Utilisez `sns governance unfreeze --selector <IH58> --ticket <id>` pour utiliser GAR/résolveur. | مباشرة بعد الحكم. |
