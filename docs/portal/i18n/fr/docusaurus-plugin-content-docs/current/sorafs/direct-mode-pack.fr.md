...
- `fetch_command_respects_direct_transports` exerce `sorafs_cli fetch --transport-policy=direct-only` contre une passerelle Torii simulé, fournissant un test de fumée pour les environnements régulés qui épinglent les transports directs.【crates/sorafs_orchestrator/tests/sorafs_cli.rs:2733】
