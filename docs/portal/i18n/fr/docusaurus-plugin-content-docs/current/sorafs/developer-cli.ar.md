...
سطح `sorafs_cli` الموحّد (provided by the `sorafs_orchestrator` crate) يعرض كل خطوة مطلوبة Utilisez le code SoraFS. استخدم هذا الكتيب للانتقال مباشرةً إلى مسارات العمل الشائعة؛ واقرنه بخط أنابيب المانيفست ودلائل تشغيل المُنسِّق للحصول على السياق التشغيلي.
