...
- `PinProviderRegistry` permet de publier un instantané en chaîne à partir de `/v2/sorafs/capacity/state`, de consulter les fournisseurs et de consulter le registre des frais pour déterminer Norito. JSON.【crates/iroha_torii/src/sorafs/registry.rs:17】【crates/iroha_torii/src/sorafs/api.rs:64】
...
- Outils d'opérateur : `sorafs_manifest_stub capacity {declaration, telemetry, replication-order}` convertit les spécifications techniques en charges utiles canoniques Norito, les blobs base64 et les résumés JSON, les opérateurs peuvent créer des appareils. `/v2/sorafs/capacity/declare`, `/v2/sorafs/capacity/telemetry` et les appareils d'ordre de réplication avec validation locale.【crates/sorafs_car/src/bin/sorafs_manifest_stub/capacity.rs:1】 Appareils de référence trouvés dans `fixtures/sorafs_manifest/replication_order/` (`order_v1.json`, `order_v1.to`) et généré à partir de `cargo run -p sorafs_car --bin sorafs_manifest_stub -- capacity replication-order`.### 2. Plan de contrôle d'intégration

| Задача | Propriétaire(s) | Première |
|------|----------|-------|
| Ajoutez les charges utiles Torii `/v2/sorafs/capacity/declare`, `/v2/sorafs/capacity/telemetry`, `/v2/sorafs/capacity/orders` avec les charges utiles JSON Norito. | Équipe Torii | Validation de la logique ; Utilisez les assistants JSON Norito. |
...
| Exportation de tableaux de bord/alertes pour la mesure des retards (ingestion du backlog, utilisation de la télémétrie). | Observabilité | Téléchargez le paquet Grafana pour votre SF-6/SF-7. |- Torii doit publier `/v2/sorafs/capacity/telemetry` et `/v2/sorafs/capacity/state` (JSON + Norito), les opérateurs peuvent utiliser les instantanés de télémétrie pour эпохам, а инспекторы - получать канонический ledger для аудита или упаковки доказательств.【crates/iroha_torii/src/sorafs/api.rs:268】【crates/iroha_torii/src/sorafs/api.rs:816】
...
- Sélectionnez Torii (`/v2/sorafs/capacity/declare`), puis fixez-le
  `/v2/sorafs/capacity/state` et étiquettes Grafana. Следуйте flow выхода в
