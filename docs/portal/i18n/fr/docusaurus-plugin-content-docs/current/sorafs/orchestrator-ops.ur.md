...
titre : SoraFS آرکسٹریٹر آپریشنز رن بک
...
:::note مستند ماخذ
یہ صفحہ `docs/source/sorafs/runbooks/sorafs_orchestrator_ops.md` کی عکاسی کرتا ہے۔ Il s'agit d'un Sphinx qui est en train de s'en sortir avec un ami. رکھیں۔
:::

یہ رن بک SRE ٹیموں کو ملٹی سورس fetch آرکسٹریٹر کی تیاری، رول آؤٹ اور آپریشنز میں رہنمائی کرتی ہے۔ یہ ڈیولپر گائیڈ کو پروڈکشن رول آؤٹس کے مطابق طریقۂ کار سے مکمل کرتی ہے، جن میں مرحلہ وار فعال سازی اور pairs کی بلیک لسٹنگ شامل ہے۔
...
1. **پرووائیڈر ان پٹس جمع کریں**
   ...
   - Il s'agit d'une charge utile utile (`plan.json`)۔
2. **متعین (déterministe) اسکور بورڈ تیار کریں**
   ...
   - Les fichiers JSON sont compatibles avec les fichiers JSON. Il s'agit d'un morceau de morceau qui est en train de devenir un morceau de papier.
3. **fixations et fonctionnement à sec** — `docs/examples/sorafs_ci_sample/` pour les charges utiles et les charges utiles چھونے سے پہلے آرکسٹریٹر بائنری متوقع ورژن سے میچ ہو جائے۔
...
2. **رَیمپ مرحلہ (50% پرووائیڈرز)**
   - `--max-peers` شاٹ کے ساتھ دوبارہ چلائیں۔
...
3. **مکمل رول آؤٹ**
   ...
   - کلائنٹ ڈیپلائمنٹس میں آرکسٹریٹر موڈ فعال کریں: محفوظ شدہ اسکور بورڈ اور Le JSON est un outil de conversion de fichiers JSON.
...
## 3. pairs کی بلیک لسٹنگ اور بوسٹنگ
...
- اوور رائیڈز کو انسیڈنٹ میں درج کریں اور JSON آؤٹ پٹس منسلک کریں تاکہ بنیادی مسئلہ حل ہونے پر ذمہ دار ٹیم اسٹیٹ کو ہم آہنگ کر سکے۔

مستقل تبدیلیوں کے لیے سورس ٹیلیمیٹری میں ترمیم کریں (خلاف ورزی کرنے والے کو pénalisé کے طور پر نشان زد کریں) یا CLI اوور رائیڈز صاف کرنے سے پہلے اپ ڈیٹڈ اسٹریم بجٹس کے ساتھ اشتہار ریفریش کریں۔

## 4. خرابیوں کی جانچ

Je vais chercher ناکام ہو:
...
2. `session.summary.json` میں قابل فہم ایرر سٹرنگ دیکھیں :
   ...
   - `retry budget exhausted ...` → `--retry-budget` pour les pairs
...
2. Le produit `--boost-provider` est un produit de haute qualité qui est en vente libre. جائے۔
