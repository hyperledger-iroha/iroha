...
titre : Runbook des litiges et révocations SoraFS
sidebar_label : Litiges et révocations du Runbook
...
Cette page reflète `docs/source/sorafs/dispute_revocation_runbook.md`. Conservez les deux copies synchronisées jusqu’à ce que la documentation Sphinx subsiste soit retirée.
:::
...
2. Normaliser dans une archive déterministe (par exemple, un tarball) ; consignateur :
   - résumé BLAKE3-256 (`evidence_digest`)
   - type de média (`application/zip`, `application/jsonl`, etc.)
   - URI d’hébergement (stockage objet, pin SoraFS ou endpoint accessible via Torii)
3. Stockez le bundle dans le bucket de collecte des preuves de gouvernance avec un accès en écriture unique.
...
2. **Générez `ProviderAdmissionRevocationV1` :**
   - Utilisez `sorafs_manifest_builder provider-admission revoke` avec la raison certifiée.
   - Vérifiez les signatures et le digest de révocation.
3. **Publiez la révocation :**
   - Soumettez la requête de révocation à Torii.
   - Assurez-vous que les publicités du fournisseur sont bloquées (attendez une hausse de `torii_sorafs_admission_total{result="rejected",reason="admission_missing"}`).
4. **Mettez à jour les tableaux de bord :** signalez le fournisseur comme révoqué, référencez l’ID du litige et liez le bundle de preuves.

## 5. Post-mortem et suivi

- enregistrer la chronologie, la cause racine et les actions de remédiation dans le tracker d'incidents de gouvernance.
- Déterminez la restitution (slashing du mise, clawbacks de frais, remboursements clients).
- Documentez les leçons ; Mettez à jour les seuils SLA ou les alertes de surveillance si nécessaire.

## 6. Documents de référence

-`sorafs_manifest_builder capacity dispute --help`
- `docs/source/sorafs/storage_capacity_marketplace.md` (rubrique litiges)
