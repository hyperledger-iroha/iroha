...
- `sorafs_manifest_builder --chunker-profile=<handle>` — подтверждает, что manifestes и CAR
  планы встраивают канонический handle и alias.
- `sorafs_manifest_builder --plan=-` — avant `chunk_fetch_specs` pour les vérifications
