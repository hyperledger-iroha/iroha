...
titre : Conception du stockage du nœud SoraFS
...
disponibilité des données SoraFS et réserver une partie du disque local pour
stocker et servir des morceaux. Elle complète la spécification de découverte
`sorafs_node_client_protocol.md` et le travail de luminaires SF-1b en détaillant
l'architecture côté stockage, les contrôles de ressources et la plomberie de
configuration qui doit arriver dans les chemins de code du nœud et du gateway.
Les opérateurs de forage se trouvent dans le
[Runbook d'opérations du nœud](./node-operations).

### Objectifs- Permettre à tout validateur ou processus Iroha auxiliaire d'exposer du disque
  disponible en tant que fournisseur SoraFS sans affecter les responsabilités du grand livre.
- Garder le module de stockage déterministe et piloté par Norito : manifestes,
  plans de chunk, racines Proof-of-Retrievability (PoR) et annonces de fournisseur
...
Clés des modules :- **Gateway** : expose des endpoints HTTP Norito pour les propositions de pin,
  les requêtes de fetch de chunks, l'échantillonnage PoR et la télémétrie. Il
  validez les payloads Norito et acheminez les requêtes vers le chunk store. Réutiliser
  la pile HTTP Torii existe pour éviter un nouveau démon.
- **Pin Registry** : l'état des pins de manifeste suivi dans `iroha_data_model::sorafs`
  et `iroha_core`. Lorsqu'un manifeste est accepté, le registre enregistre le digest
  du manifest, le digest du plan de chunk, la racine PoR et les flags de capacité du
  fournisseur.
- **Chunk Storage** : implémentation `ChunkStore` sur disque qui ingère des manifestes
  signés, matérialiser les plans de chunk via `ChunkProfile::DEFAULT`, et persiste
  les chunks selon un layout déterministe. Chaque chunk est associé à une empreinte digitale
...
- **Quota/Scheduler** : impose des limites configurées par l'opérateur (bytes disque max,
  ...
  les tâches ledger ne soient pas affamées. Le planificateur est également responsable du
  service des preuves PoR et des requêtes d'échantillonnage avec CPU borné.

###Configuration

Ajouter une nouvelle section à `iroha_config` :
...
```- `enabled` : bascule de participation. Quand false, le gateway 503 pour les
  endpoints de storage et le nœud ne s'annonce pas en découverte.
- `data_dir` : répertoire racine des données de chunk, arbres PoR et télémétrie de
  aller chercher. Défaut `<iroha.data_dir>/sorafs`.
- `max_capacity_bytes` : limite dure pour les morceaux pinés. Une tâche de fond
  rejette les nouveaux pins quand la limite est atteinte.
- `max_parallel_fetches` : plafond de concurrence imposé par le planning pour
  équilibrer bande passante/IO disque avec la charge du validateur.
- `max_pins` : nombre maximum de broches de manifeste que le nœud accepte avant d'appliquer
  expulsion/contre-pression.
- `por_sample_interval_secs` : cadence des jobs d'échantillonnage PoR automatiques. Chaque emploi
  échantillonne `N` feuilles (configurable par manifeste) et émet des événements de télémétrie.
...
- `adverts` : structure utilisée par le générateur d'adverts pour remplir les champs
  `ProviderAdvertV1` (pointeur d'enjeu, conseils QoS, sujets). Si omis, le nœud utilise les
  défauts du registre de gouvernance.

Plomberie de configuration :- `[sorafs.storage]` est défini dans `iroha_config` comme `SorafsStorage` et chargé
  depuis le fichier de configuration du noeud.
- `iroha_core` et `iroha_torii` transmettent la configuration de stockage au builder gateway
  et au chunk store au démarrage.
- Des remplacements dev/test existants (`SORAFS_STORAGE_*`, `SORAFS_STORAGE_PIN_*`), mais
  les déploiements de production doivent s’appuyer sur le fichier de config.

### Utilitaires CLI

Alors que la surface HTTP Torii est encore en cours de connexion, le crate
...
- `ingest` attend un manifest `.to` encodé Norito et les octets de payload associés.
  ...
  JSON `chunk_fetch_specs` pour que les outils en aval puissent valider la mise en page.
- `export` accepte un ID de manifest et écrit le manifest/payload stocké sur disque
  (avec plan JSON optionnel) pour que les luminaires restent reproductibles.Les deux commandes impriment un résumé Norito JSON sur stdout, ce qui facilite le
piping dans des scripts. La CLI est couverte par un test d'intégration pour garantir
que les manifestes et les charges utiles s'effectuent correctement avant l'arrivée des API Torii.【crates/sorafs_node/tests/cli.rs:1】
...
> des scripts locaux aux sondes HTTP sans changer de parseurs.【crates/iroha_torii/src/sorafs/api.rs:1207】【crates/iroha_torii/src/sorafs/api.rs:1259】

### Cycle de vie du nœud1. **Démarrage** :
   - Si le stockage est activé, le nœud initialise le chunk store avec le répertoire
     et la capacité configurée. Cela inclut la vérification ou la création de la base
     de données de manifest PoR et le replay des manifestes pinés pour chauffer les caches.
   - Enregistrer les routes du gateway SoraFS (endpoints Norito JSON POST/GET pour pin,
     fetch, échantillonnage PoR, télémétrie).
   - Lancer le travailleur d'échantillonnage PoR et le moniteur de quotas.
2. **Découverte / Annonces** :
   - Générer des documents `ProviderAdvertV1` avec la capacité/santé courante, les signataires
     avec la clé enregistrée par le conseil, et publier via le canal de découverte.
     Utiliser la nouvelle liste `profile_aliases` pour garder les poignées canoniques
3. **Workflow de pin** :
   - Le gateway reçoit un manifeste signé (incluant plan de chunk, racine PoR, signatures
     du conseil). Valider la liste d'alias (`sorafs.sf1@1.0.0` requis) et s'assurer que
     le plan de chunk correspond aux métadonnées du manifeste.
   - Vérifier les quotas. Si les limites de capacité/pins seraient dépassées, répondez
   ...
   - Streamer les données de chunk dans `ChunkStore` en vérifiant les digests à l'ingestion.
     Mettre à jour les arbres PoR et stocker les métadonnées du manifeste dans le registre.
4. **Workflow de récupération** :- Servir les requêtes de range de chunk depuis le disque. Le planificateur impose
     `max_parallel_fetches` et renvoyer `429` en cas de saturation.
   - Émettre une télémétrie structurée (Norito JSON) avec latence, bytes servis et
     compteurs d'erreurs pour le monitoring en aval.
5. **Échantillonnage PoR** :
   - Le travailleur sélectionne les manifestes proportionnellement au poids (ex. bytes stockés)
     et exécute un échantillonnage déterministe via l'arbre PoR du chunk store.
   - Persister les résultats pour les audits de gouvernance et inclure des curriculum vitae dans
     les adverts supplier / endpoints de télémétrie.
6. **Éviction / application des quotas** :
   - Lorsque la capacité est atteinte, le nœud rejette les nouvelles broches par défaut. Fr
...
- Le `TelemetryAccumulator` embarqué peut être muté via `NodeHandle::update_telemetry`, permettant aux
  ...
  `CapacityTelemetryV1` sans toucher aux internes du planificateur.【crates/sorafs_node/src/lib.rs:142】【crates/sorafs_node/src/telemetry.rs:1】
...
- **Gouvernance** : étendre `sorafs_pin_registry_tracker.md` avec la télémétrie de stockage
  ...
  minimum ou un taux de succès PoR minimal avant d'accepter des publicités.
- **SDKs clients** : exposer la nouvelle configuration de stockage (limites disque, alias) pour que
  les outils de gestion peuvent amorcer les nœuds par programme.
- **Télémétrie** : intégrer avec la stack de métriques existante (Prometheus /
  OpenTelemetry) afin que les métriques de stockage apparaissent dans les tableaux de bord d'observabilité.
- **Sécurité** : exécuter le module de stockage dans un pool de tâches async dédié avec back-pression
  et envisagez le sandboxing des lectures de chunks via io_uring ou des pools tokio bornés pour empêcher
  des clients malveillants d'épuiser les ressources.Ce design maintient le module de stockage optionnel et déterministe tout en donnant aux opérateurs les
boutons nécessaires pour participer à la couche de disponibilité des données SoraFS. Son implémentation
impliquera des changements dans `iroha_config`, `iroha_core`, `iroha_torii` et la passerelle Norito, ainsi
que les outils d'annonce du fournisseur.