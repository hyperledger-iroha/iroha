...
**Articles :**
- CLI et tests (`crates/sorafs_orchestrator/src/bin/sorafs_cli.rs`)
...
- Harnais de parité déterministe (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`, [Отчет о паритете GA SoraFS Orchestrator](./orchestrator-ga-parity.md))
...
4. Faisceau de parité Log (`cargo test -p sorafs_orchestrator --test sorafs_cli proof_stream_consumes_ndjson_and_reports_metrics -- --nocapture`).
