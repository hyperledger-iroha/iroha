...
|------|----------|-----------------------|-------|
| 2026-02-12 | Tooling (LLM) | ❌ Échec | Étape 1 : `cargo test -p sorafs_chunker` échoue la suite `vectors` car les fixtures publient encore le handle legacy `sorafs.sf1@1.0.0` et n'incluent pas les profile aliases/digests (`fixtures/sorafs_chunker/sf1_profile_v1.*`). Étape 2 : `ci/check_sorafs_fixtures.sh` s'interrompt - `manifest_signatures.json` manque dans l'état du repo (supprimé dans le working tree). Étape 4 : `export_vectors` ne peut pas vérifier les signatures tant que le fichier manifest est absent. Recommandation : restaurer les fixtures signées (ou fournir la clé du council) et régénérer les bindings afin d'intégrer le handle canonique + le tableau d'aliases, comme l'exigent les tests. |
