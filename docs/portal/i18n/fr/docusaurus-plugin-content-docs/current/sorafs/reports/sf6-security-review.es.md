...
**Artefacts :**
- Source de CLI et tests (`crates/sorafs_orchestrator/src/bin/sorafs_cli.rs`)
...
- Harness de paridad determinista (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`, [Reporte de Paridad GA del Orchestrator SoraFS](./orchestrator-ga-parity.md))
...
| SF6-SR-03 | Médias | Envio de manifestes | La CLI acceptée se manifeste sans vérifier les plans des morceaux intégrés lorsque `--plan` est activé. | `sorafs_cli manifest submit` ahora recalcula et comparera les résumés de la salve CAR qui se prouvent `--expect-plan-digest`, rechazando incohérences et montre les pistes de remédiation. Les tests contiennent des cas de sortie/d'erreur (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`). |
...
4. Journaux capturés du harnais de parité (`cargo test -p sorafs_orchestrator --test sorafs_cli proof_stream_consumes_ndjson_and_reports_metrics -- --nocapture`).
