...
Cette page reflete l'audit interne `TORII-APP-1` (`docs/source/torii/app_api_parity_audit.md`) afin que les lecteurs en dehors du mono-repo puissent voir quelles surfaces `/v1/*` sont cablees, testees et documentees. L'audit suit les routes reexportees via `Torii::add_app_api_routes`, `add_contracts_and_vk_routes` et `add_connect_routes`.

## Portee et methode

L'audit inspecte les reexportations publiques dans `crates/iroha_torii/src/lib.rs:256-522` et les constructeurs de routes soumis au feature gating. Pour chaque surface `/v1/*` du roadmap, nous avons verifie:
...
### Evaluation OPRF d'alias (`POST /v1/aliases/voprf/evaluate`) - Couvert
...
### Evenements de proof SSE (`GET /v1/events/sse`) - Couvert
...
### Nexus Connect (`/v1/connect/*`) - Couvert (feature `connect`)
