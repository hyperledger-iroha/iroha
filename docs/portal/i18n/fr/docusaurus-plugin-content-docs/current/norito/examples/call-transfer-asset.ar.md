...
- موّل سلطة العقد (مثلا `soraカタカナ...`) pour أصل الذي ستنقله وامنح السلطة دور `CanTransfer` أو إذنا مكافئا.
...
- تحقق من أرصدة عبر `FindAccountAssets` أو `iroha_cli ledger assets list --account soraカタカナ...` وافحص الأحداث لتأكيد أن حارس بيانات التعريف سجل سياق النقل.
