...
- Assurez-vous que le compte de destination existe (par exemple `<i105-account-id>`), en réfléchissant à la phase de configuration dans chaque démarrage rapide du SDK.
...
- Vérifiez les soldes intermédiaires `client.request(FindAccountAssets)` ou `iroha_cli ledger assets list --account <i105-account-id>` pour confirmer que la détection est réussie.
