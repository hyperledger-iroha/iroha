...
- Utiliser le bloc pour agréger les reçus sur la voie/l'espace de données et publier sur `lane_settlement_commitments` dans `/v1/sumeragi/status`. Il s'agit d'un `total_local_micro`, d'un `total_xor_due_micro` et d'un `total_xor_after_haircut_micro`, résumés dans le bloc pour les petits utilisateurs.- Le nouveau schéma `total_xor_variance_micro` s'applique, le cas échéant étant donné que le XOR et l'opération sont effectués après la coupe de cheveux), le `swap_metadata` documente les paramètres de conversion (TWAP, epsilon, profil de liquidité et volatilité_class), que les auditeurs peuvent vérifier. Les paramètres de configuration ne correspondent pas à la configuration.
