...
- La gestion des voies/espaces de données est basée sur `lane_settlement_commitments` et `/v2/sumeragi/status`. تعرض الاجماليات `total_local_micro` و`total_xor_due_micro` و`total_xor_after_haircut_micro` مجمعة مستوى الكتلة من اجل صادرات المطابقة الليلية.
