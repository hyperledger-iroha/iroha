...
- **Tests Pipeline/SSE.** Les filtres d'événements de transactions acceptent les prédicats `tx_lane_id == <u32>` (ver `docs/source/pipeline.md`). Abonnez-vous à `/v2/pipeline/events/transactions` avec ce filtre pour démontrer que les écritures envoyées sans une voie explicite utilisent l'identifiant de la voie de repli.
