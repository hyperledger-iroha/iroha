...
- **Tests Pipeline/SSE.** Les filtres d'événements de transactions acceptent les prédicats `tx_lane_id == <u32>` (voir `docs/source/pipeline.md`). Abonnez-vous à `/v2/pipeline/events/transactions` avec ce filtre pour prouver que les écritures envoyées sans voie explicite arrivent sous l'id de voie de repli.
