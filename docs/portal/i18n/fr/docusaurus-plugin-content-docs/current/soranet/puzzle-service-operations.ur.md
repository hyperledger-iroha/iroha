...
- `GET /v2/puzzle/config` - relais JSON (`handshake.descriptor_commit_hex`, `pow.*`) سے
  Voici les paramètres PoW/puzzle et les détails
- `POST /v2/puzzle/mint` - Billet Argon2 neuf corps JSON facultatif
...
- `GET /v2/token/config` - pour `pow.token.enabled = true` et un jeton d'admission actif
...
- `POST /v2/token/mint` - Jeton d'admission ML-DSA neuf et hachage de CV fourni
...
Observateur de fichiers de révocation `/v2/token/config` et version actuelle mises à jour
...
vérifier la clé publique ML-DSA-44 et la publicité `/v2/puzzle/config` clé et
...
`pow.signed_ticket_public_key_hex` pour valider ici `POST /v2/puzzle/mint`
...
3. **Étape de redémarrage ici** Annonce du basculement de la rotation de la gouvernance dans l'unité systemd ici
   ...
   redémarrer ضروری ہے۔
4. **Valider le ticket** `POST /v2/puzzle/mint` pour l'émission du ticket et le ticket de caisse.
   ...
   capturer کرتا ہے۔ Les jetons permettent à `/v2/token/config` de récupérer un émetteur annoncé
...
4. `soranet_handshake_pow_difficulty` surveille la difficulté de la valeur de hashcash attendue
   تک drop ہو، اور verify کریں کہ `/v2/puzzle/config` `puzzle = null` رپورٹ کرے۔
...
- **Alignement du puzzle :** `soranet_handshake_pow_difficulty` et `/v2/puzzle/config` et `soranet_handshake_pow_difficulty` et `/v2/puzzle/config`.
  difficulté کے ساتھ match ہونا چاہئے۔ Divergence de configuration de relais obsolète et échec du redémarrage
- **Préparation au jeton :** اگر `/v2/token/config` غیر متوقع طور پر `enabled = false` ہو جائے یا
...
- **Santé du service :** `/healthz` pour la cadence de vivacité et la sonde et l'alerte.
  `/v2/puzzle/mint` Réponses HTTP 500 pour (incompatibilité des paramètres Argon2 et échecs RNG par exemple).
  Erreurs de création de jetons `/v2/token/mint` dans les réponses HTTP 4xx/5xx échecs répétés
