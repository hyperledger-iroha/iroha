...
- `GET /v2/puzzle/config` - développer les paramètres efficaces de PoW/puzzle
  tomes du relais JSON (`handshake.descriptor_commit_hex`, `pow.*`).
- `POST /v2/puzzle/mint` - détecte un ticket Argon2 ; un corps JSON facultatif
...
- `GET /v2/token/config` - quand `pow.token.enabled = true`, devuelve la
...
- `POST /v2/token/mint` - détecte un jeton d'admission ML-DSA lié au CV
...
maintenir `/v2/token/config` actualisé; coordonnées des mises à jour avec le commandant
...
la clé publique ML-DSA-44 utilisée pour vérifier les billets PoW firmados ; `/v2/puzzle/config`
...
`POST /v2/puzzle/mint` accepte `"signed": true` (et facultatif `"transcript_hash_hex"`) pour
...
4. **Valider.** Émettez un ticket via `POST /v2/puzzle/mint` et confirmez que les
   ...
   Consultez `/v2/token/config` pour vous assurer que l'empreinte digitale de l'émetteur est annoncée
...
4. Monitorea `soranet_handshake_pow_difficulty` pour garantir la difficulté
   cae al valor hashcash espéré, et vérifier que `/v2/puzzle/config` rapporte
   `puzzle = null`.
...
- **Alignement du puzzle :** `soranet_handshake_pow_difficulty` doit coïncider avec la
  difficulté à développer par `/v2/puzzle/config`. Configuration de la divergence indica
  le relais est périmé ou un roi est tombé en panne.
- **Préparation au jeton :** Alerte si `/v2/token/config` cae a `enabled = false`
...
- **Service santé :** Sondea `/healthz` avec la cadence habituelle de vivacité et d'alerte
  si `/v2/puzzle/mint` donne des réponses HTTP 500 (indique une incompatibilité des paramètres
  Argon2 ou erreurs RNG). Les erreurs de frappe de jetons apparaissent avec des réponses
  HTTP 4xx/5xx et `/v2/token/mint` ; trata fallas repetidas como condition de paging.
