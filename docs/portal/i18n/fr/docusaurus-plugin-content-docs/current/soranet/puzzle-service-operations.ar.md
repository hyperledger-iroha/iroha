...
- `POST /v2/puzzle/mint` - pour Argon2 ; corps JSON version
...
- `GET /v2/token/config` - `pow.token.enabled = true` est disponible
  jeton d'admission (empreinte digitale de l'émetteur, TTL/clock-skew, ID de relais,
  ومجموعة révocation المدمجة).
- `POST /v2/token/mint` - Jeton d'admission ML-DSA utilisé pour le hachage de reprise
...
watcher لملف révocation بالحفاظ على `/v2/token/config` محدثا؛ نسق التحديثات مع
...
العام ML-DSA-44 المستخدم للتحقق من PoW tickets الموقعة ; عكس `/v2/puzzle/config`
...
`POST /v2/puzzle/mint` pour `"signed": true` (et `"transcript_hash_hex"`) pour
...
4. **التحقق.** اصدر تذكرة عبر `POST /v2/puzzle/mint` et `difficulty` و`expires_at`
   ...
   `/v2/token/config` pour la révocation des empreintes digitales de l'émetteur
...
4. Utilisez `soranet_handshake_pow_difficulty` pour utiliser le hashcash
   Il s'agit d'un `/v2/puzzle/config` ou d'un `puzzle = null`.
...
- **Alignement du puzzle :** يجب ان يطابق `soranet_handshake_pow_difficulty` الصعوبة
  المعادة من `/v2/puzzle/config`. يشير الاختلاف الى تكوين relais قديم او فشل اعادة
  التشغيل.
- **Préparation au jeton :** نبه اذا انخفض `/v2/token/config` et `enabled = false`
...
- **Santé du service :** افحص `/healthz` et vivacité de la cadence المعتادة ونبه اذا
  Utilisez `/v2/puzzle/mint` pour HTTP 500 (pour utiliser Argon2 et Argon2).
  (RNG). Comment créer des tokens avec HTTP 4xx/5xx selon `/v2/token/mint`
