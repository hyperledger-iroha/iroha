...
| Tələb | Ətraflı |
|-------------|--------|
| Protokollar | `/v2/sns/*` və gRPC xidməti `sns.v1.Registrar` altında REST. Hər ikisi Norito-JSON (`application/json`) və Norito-RPC ikili (`application/x-norito`) qəbul edir. |
...
|----------|--------|---------|-------------|
| `/v2/sns/registrations` | POST | `RegisterNameRequestV1` | Qeydiyyatdan keçin və ya adı yenidən açın. Qiymət səviyyəsini həll edir, ödəniş/idarəetmə sübutlarını təsdiqləyir, qeyd hadisələrini yayır. |
| `/v2/sns/registrations/{selector}/renew` | POST | `RenewNameRequestV1` | Müddəti uzatmaq. Siyasətdən lütf/ödəniş pəncərələrini tətbiq edir. |
| `/v2/sns/registrations/{selector}/transfer` | POST | `TransferNameRequestV1` | İdarəetmə təsdiqləri əlavə edildikdən sonra mülkiyyəti köçürün. |
| `/v2/sns/registrations/{selector}/controllers` | PUT | `UpdateControllersRequestV1` | Nəzarətçi dəstini dəyişdirin; imzalanmış hesab ünvanlarını doğrulayır. |
| `/v2/sns/registrations/{selector}/freeze` | POST | `FreezeNameRequestV1` | Qəyyum/məclis dondurulur. Qəyyum bileti və idarəetmə sənədinə istinad tələb olunur. |
| `/v2/sns/registrations/{selector}/freeze` | SİL | `GovernanceHookV1` | Təmirdən sonra dondurun; şuranın ləğv edilməsinin qeydə alınmasını təmin edir. |
| `/v2/sns/reserved/{selector}` | POST | `ReservedAssignmentRequestV1` | Qorunan adların stüard/məclis təyinatı. |
| `/v2/sns/policies/{suffix_id}` | GET | — | Cari əldə edin `SuffixPolicyV1` (keşlənə bilər). |
| `/v2/sns/registrations/{selector}` | GET | — | Cari `NameRecordV1` + effektiv vəziyyəti qaytarır (Aktiv, Grace və s.). |
...
`--governance-json` etibarlı `GovernanceHookV1` qeydini (təklif identifikatoru, səs heşləri, stüard/qəyyum imzaları) ehtiva etməlidir. Hər bir əmr sadəcə olaraq müvafiq `/v2/sns/registrations/{selector}/…` son nöqtəsini əks etdirir ki, beta operatorları SDK-ların çağıracağı dəqiq Torii səthlərini məşq edə bilsinlər.
...
1. Təklif id-si idarəetmə kitabçasında (`/v2/governance/proposals/{id}`) mövcuddur və status `Approved`-dir.
...
1. Müştəri qiymətləri, lütfləri və mövcud səviyyələri əldə etmək üçün `/v2/sns/policies/{suffix_id}` sorğusu göndərir.
...
3. Təmirdən sonra şuranın məsələləri ləğv edilir; operator DELETE `/v2/sns/registrations/{selector}/freeze`-i `GovernanceHookV1` ilə göndərir.
...
- Ödəniş sübutları Norito kitab qəbzlərindən təkrar istifadə edir; xəzinə xidmətləri köməkçi API təmin edir (`/v2/finance/sns/payments`).
