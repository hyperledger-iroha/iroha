...
コントラクトのデプロイ後は、`iroha contract call --contract-address <contract-address> --entrypoint main --wait` またはアプリケーションクライアント経由で呼び出せます。アカウントの権限が必要な syscall (`set_account_detail`, `transfer_asset` など) を許可していることを確認してください。
