...
1. **זהה סיבה**
   ...
   - אם העומס גדל עם החמצות יציבות, בדוק את הקבלה (`/v2/sorafs/pin/*`) כדי לאשר מניפסטים הממתינים לאישור המועצה.
2. **אמת סטטוס ספק**
...
2. **ריצה יבשה בהיערכות**
   ...
   - הפעל את נקודות הקצה `/v2/sorafs/pin/{digest}` ו-`/v2/sorafs/aliases` עם הוכחות סינתטיות המכסות מקרים טריים, חלון רענון, פג תוקפם ומקרים שפג תוקפם קשה. אמת את קודי מצב ה-HTTP, הכותרות (`Sora-Proof-Status`, `Retry-After`, `Warning`), ושדות הגוף של JSON מול ספר ההפעלה הזה.
3. **אפשר בייצור**
