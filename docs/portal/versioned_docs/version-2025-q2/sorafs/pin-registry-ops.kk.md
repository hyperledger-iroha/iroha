...
1. **Себепті анықтаңыз**
   ...
   - Егер артта қалу тұрақты жіберіп алумен өссе, кеңестің мақұлдауын күтіп тұрған манифесттерді растау үшін қабылдауды (`/v1/sorafs/pin/*`) тексеріңіз.
...
2. **Сахнадағы құрғақ жүгіріс**
   ...
   - `/v1/sorafs/pin/{digest}` және `/v1/sorafs/aliases` соңғы нүктелерін жаңа, жаңарту терезесі, мерзімі өткен және мерзімі өтіп кеткен жағдайларды қамтитын синтетикалық дәлелдермен орындаңыз. HTTP күй кодтарын, тақырыптарды (`Sora-Proof-Status`, `Retry-After`, `Warning`) және JSON негізгі өрістерін осы жұмыс кітабына қарсы тексеріңіз.
3. **Өндірісте қосу**
