...
1. **原因の特定**
   ...
   - 安定したミスでバックログが増加する場合は、入場 (`/v2/sorafs/pin/*`) を検査して、議会の承認を待っているマニフェストを確認します。
...
2. **ステージングでの予行演習**
   ...
   - `/v2/sorafs/pin/{digest}` および `/v2/sorafs/aliases` エンドポイントを、新しいケース、リフレッシュ ウィンドウのケース、期限切れのケース、および完全期限切れのケースをカバーする合成証明で実行します。この Runbook に対して HTTP ステータス コード、ヘッダー (`Sora-Proof-Status`、`Retry-After`、`Warning`)、および JSON 本体フィールドを検証します。
3. **運用環境で有効にする**
