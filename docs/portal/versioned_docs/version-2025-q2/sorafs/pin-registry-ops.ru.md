...
3. **Переназначить репликацию**
   - Выдавайте новые заказы через `sorafs_manifest_builder capacity replication-order`, когда резерв резерва (`stat="avg"`) падает ниже 5 эпох (упаковка манифеста/CAR использует `iroha app sorafs toolkit pack`).
