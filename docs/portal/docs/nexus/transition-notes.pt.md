...
- A evidencia de CI/tests multi-lane agora vive em `integration_tests/tests/nexus/multilane_pipeline.rs` e roda sob o workflow `Nexus Multilane Pipeline` (`.github/workflows/integration_tests_multilane.yml`), substituindo a referencia aposentada `pytests/nexus/test_multilane_pipeline.py`; mantenha o hash de `defaults/nexus/config.toml` (`nexus.enabled = true`, blake2b `7adf4b54cfbe0e6a3d3fb407f020d78248654d4009e9458c1665f5a2f30ec50e`) em sync com o tracker ao atualizar bundles de ensaio.
