...
- تنفيذ الكتلة يجمع الايصالات لكل lane/dataspace وينشرها عبر `lane_settlement_commitments` في `/v2/sumeragi/status`. تعرض الاجماليات `total_local_micro` و`total_xor_due_micro` و`total_xor_after_haircut_micro` مجمعة على مستوى الكتلة من اجل صادرات المطابقة الليلية.
