...
- Registryのライフサイクルは `Proposed → Active → Deprecated → Withdrawn`。`Active` のみがproof検証可能で、`Deprecated` は `deprecation_height` まで有効、`Withdrawn` はproofを確定的に拒否する。
...
- Commitmentsとハッシュは `params_id` をドメイン分離し、deprecatedセットのビットパターンを再利用しない。IDはnote commitmentsとnullifierドメインタグに埋め込まれる。
- Circuitsは検証時に複数パラメータ選択をサポートする。Deprecatedセットは `deprecation_height` まで消費可能で、withdrawnセットは `withdraw_height` で確定的に拒否される。
...
|----------------------|-----------------------------|----------------|
| `enabled=true`, `assume_valid=false`, backend matches, digest matches | Accepted | Peerは `Ready` 状態に到達し、proposal/vote/RBC fan-outに参加する。手動対応は不要。 |
| `enabled=true`, `assume_valid=false`, backend matches, digest stale or missing | Rejected (`HandshakeConfidentialMismatch`) | Remoteは予定されたregistry/parameter activationを適用するか `activation_height` まで待つ必要がある。修正されるまでnodeは検出可能だがconsensus rotationに入らない。 |
| `enabled=true`, `assume_valid=true` | Rejected (`HandshakeConfidentialMismatch`) | バリデータはproof検証が必須。remoteをToriiのみのobserverにするか、完全検証を有効化後に `assume_valid=false` を設定する。 |
| `enabled=false`, handshake fields omitted (out-of-date build), or verifier backend differs | Rejected (`HandshakeConfidentialMismatch`) | 古い/部分アップグレードpeerはconsensusネットワークに参加できない。現行リリースへ更新し、backend+digestが一致してから再接続する。 |

Proof検証を意図的にスキップするobserverノードは、capability gate付きバリデータとのコンセンサス接続を開いてはならない。ToriiやアーカイブAPI経由でブロックを取得できるが、互換能力を広告するまでconsensusネットワークは拒否する。
...
- **Stale digest remediation:** `HandshakeConfidentialMismatch` がdigest driftで発生した場合、(1)クラスタのnullifier retention windowが一致しているか確認し、(2)`iroha_cli confidential verify-ledger` で保持済みnullifier集合からdigestを再生成し、(3)更新済みmanifestを再展開する。早期に削除されたnullifierは再参加前にcold storageから復元する必要がある。
...
- CLI追加: `confidential create-keys`, `confidential send`, `confidential export-view-key`, メモ復号のauditor tooling, オフラインでNorito memo envelopeを生成/検査する `iroha zk envelope` helper。Toriiは `POST /v1/confidential/derive-keyset` で同じ導出フローを提供し、hexとbase64形式を返すのでwalletがプログラムでキー階層を取得できる。
...
1. **Phase M0 — Stop-Ship Hardening**
   ...
   - Registry FSM（Proposed/Active/Deprecated/Withdrawn）とactivation/deprecation/withdraw高さ、emergency withdrawal処理を実装。
