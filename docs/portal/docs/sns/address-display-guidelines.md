...
description: UX and CLI requirements for IH58 vs compressed Sora address presentation (ADDR-6).
---
...
Each SDK exposes a convenience helper that returns the IH58 and compressed
...
- Pair every control with distinct `aria-label`/`aria-describedby` hints that explain whether a
  literal is safe to share (`IH58`) or Sora-only (compressed). Include the implicit-domain caption in
...
`torii.strict_addresses=true`; only override it to `false` on dev/test clusters
...
- `AddressLocal8Resurgence` pages whenever a context reports a fresh Local-8
  ...
  dashboard, and, if needed, temporarily set `torii.strict_addresses=false`
