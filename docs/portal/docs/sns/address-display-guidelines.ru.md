...
- **Две цели копирования.** Предоставьте две явные кнопки копирования: IH58
  (по умолчанию) и сжатая Sora-only форма (`snx1...`). IH58 всегда безопасно
...
5. Для больших наборов данных запустите
   ...
   legacy dumps с мусорными строками и блокируйте автоматизацию
...
необходимых 30 дней до отключения legacy селекторов на mainnet. Пакет
