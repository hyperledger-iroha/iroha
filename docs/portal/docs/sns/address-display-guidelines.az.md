...
- **İkili nüsxə hədəfləri.** İki açıq surətdə nüsxə düyməsini göndərin—I105 (üstünlük verilir) və
  sıxılmış Sora forması (`i105`, ikinci ən yaxşı). I105 xaricdən paylaşmaq üçün həmişə təhlükəsizdir
...
- **Düzgün defolt domen göstərişləri.** Seçici gizli olanı göstərdikdə
  ...
  həzmi kodlayır.
- **I105 QR faydalı yükləri.** QR kodları I105 sətirini kodlamalıdır. QR nəsil varsa
...
Hər bir SDK I105 (üstünlük verilir) və sıxılmış (`sora`, ikinci ən yaxşı) qaytaran bir rahatlıq köməkçisini təqdim edir.
...
- JavaScript müfəttişi: `inspectAccountId(...)` sıxılmış xəbərdarlığı qaytarır
  string və zəng edənlər `i105` təqdim etdikdə onu `warnings`-ə əlavə edir
...
- Hər bir nəzarəti fərqli `aria-label`/`aria-describedby` göstərişləri ilə birləşdirin.
  hərfi paylaşmaq təhlükəsizdir (`I105`) və ya yalnız Sora (sıxılmış `sora`). Gizli domen başlığını daxil edin
...
JSON audit hesabatı və operatorların əlavə etdiyi çevrilmiş üstünlük verilən I105 / ikinci ən yaxşı sıxılmış (`sora`) siyahısı
...
1. `iroha tools address inspect` indi I105 ilə strukturlaşdırılmış JSON xülasəsi verir,
   ...
   kodlaşdırma `<i105>@<domain>` kimi təkrarlanır.
2. SDK-lar JavaScript köməkçisi vasitəsilə eyni xəbərdarlıq/xülasə təqdim edə bilər:
   ...
   ```
  Köməkçi hərfidən aşkarlanan I105 prefiksini qoruyur
...
5. Kütləvi məlumat dəstləri üçün işə salın
   ...
   kanonik/tercih edilən I105/ikinci ən yaxşı sıxılmış (`sora`) hər giriş üçün xülasələr və hər iki təhlili hesablayır
...
6. Yeni sətirdən yeni sətirə yenidən yazmağa ehtiyacınız olduqda, istifadə edin
  ...
  bir keçiddə kanonik kodlaşdırmaları, xəbərdarlıqları və təhlil uğursuzluqlarını vurğulayan `input,status,format,…` CSV-ni ixrac etmək.
   ...
   tələb olunan kodlaşdırmaya (I105 üstünlük verilir/sıxılmış (`sora`) ikinci ən yaxşı/hex/JSON) daxil edir və
