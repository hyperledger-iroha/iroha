...
**Selector encoding:** the `{selector}` path segment accepts IH58, compressed, or canonical hex per ADDR-5; Torii normalises it via `NameSelectorV1`.
...
2. Client builds `RegisterNameRequestV1`:
   - `selector` derived from IH58/compressed label.
