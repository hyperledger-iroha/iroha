...
- **雙複製目標。 ** 提供兩個顯式複制按鈕 - I105（首選）和
  僅壓縮的 Sora 形式（`i105`，第二好）。 I105 始終安全地對外共享
...
- **隱式默認域提示。 **當選擇器指向隱式
  ...
  編碼摘要。
- **I105 QR 有效負載。 ** QR 代碼必須對 I105 字符串進行編碼。如果二維碼生成
  失敗，顯示明確的錯誤而不是空白圖像。
...
- 應用 `data-copy-mode="i105|i105_default|qr"` 複製按鈕，以便前端可以發出使用計數器
...
- 將每個控件與不同的 `aria-label`/`aria-describedby` 提示配對，以解釋是否
  文字可以安全地共享（`I105`）或僅限 Sora（壓縮的 `sora`）。將隱式域標題包含在
...
JSON 審核報告和操作員附加的轉換後的首選 I105/第二最佳壓縮 (`sora`) 列表
...
1. `iroha tools address inspect` 現在使用 I105 發出結構化 JSON 摘要，
...
2. SDK 可以通過 JavaScript 幫助程序顯示相同的警告/摘要：
   ...
   ```
  幫助器保留從文字中檢測到的 I105 前綴，除非您
  明確提供 `networkPrefix`，因此非默認網絡的摘要不會
  不會使用默認前綴默默地重新渲染。

3. 通過重用 `i105.value` 或 `i105_default` 轉換規範負載
...
5. 對於批量數據集，運行
   ...
   每個條目的規範/首選 I105/第二最佳壓縮 (`sora`) 摘要，並對兩個解析進行計數
...
6. 當需要換行符到換行符重寫時，使用
  ...
  導出 `input,status,format,…` CSV，一次性突出顯示規範編碼、警告和解析失敗。
   ...
   到請求的編碼（I105首選/壓縮（`sora`）第二好/十六進制/JSON），並保留
