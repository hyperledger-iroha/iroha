...
- ထုတ်လုပ်ရန် `scripts/address_local_toolkit.sh` သည် `iroha` CLI ကို ထုပ်ပိုးထားသည်-
  ...
  - `normalized.txt` — Local-domain ရွေးပေးသူတိုင်းအတွက် နှစ်သက်ရာ IH58 / ဒုတိယအကောင်းဆုံးချုံ့ထားသော (`sora`) literals များကို ပြောင်းထားသည်။
...
- IH58 အစား `sora…` အထွက်အတွက် `--format compressed`။
- စာလုံးဗလာများကိုထုတ်လွှတ်ရန် `--no-append-domain`။
...
2. `audit.json` သည် Local selectors (`domain.kind = local12`) ကို အစီရင်ခံသောအခါ ပေါင်းစပ်မှုများကို ပိတ်ဆို့ပါ။
   ...
   `iroha tools address normalize --fail-on-warning --only-local` သည် CI ဖြစ်သောကြောင့် ဆုတ်ယုတ်သွားသည်။
