...
| `owner` | አዎ | AccountId string (domainless encoded literal; canonical Katakana i105 only; no `@<domain>` suffix). |
...
- ** ተቆጣጣሪዎች: ** ባዶ ሕዋሳት `--default-controllers` ያከብራሉ. በግልፅ ያቅርቡ
  የመቆጣጠሪያ ዝርዝሮች (ለምሳሌ `<katakana-i105-account-id>;<katakana-i105-account-id>`) ባለቤት ላልሆኑ ውክልና ሲሰጡ
  ተዋናዮች.
