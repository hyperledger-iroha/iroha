...
- `iroha app da get` artıq səlahiyyət verən çoxmənbəli orkestrator üçün DA-yönümlü ləqəb əlavə edir
  ...
  yol istifadə olunur, CLI manifestini `/v2/da/manifests/<ticket>`-dən çıxarır, altındakı paketi saxlayır
...
- `iroha app da get-blob` `GET /v2/da/manifests/{storage_ticket}` vasitəsilə birbaşa Torii-dən kanonik manifestləri çıxarır.
...
- **PDP/PoTR qarmaqları** — PDP öhdəlikləri birbaşa yığın mağazasından alınır və davamlıdır
  ...
  `/v2/da/ingest` plus `/v2/da/manifests/{ticket}` indi `Sora-PDP-Commitment` başlığını ehtiva edir
...
- Torii-in `/v2/da/ingest` son nöqtəsi indi faydalı yükün sıxılmasını normallaşdırır, təkrar oynatma keşini tətbiq edir,
