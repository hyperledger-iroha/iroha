...
3. Right before sealing, the block builder sorts commitments by `(lane_id,
   epoch, sequence)`, constructs the versioned
   `(version, leaf_count, merkle_root)` descriptor, and writes that commitment
   to `da_commitments_hash`.
...
Because the V1 data model has no KZG variant or commitment field, neither Torii
nor a lifecycle transition can construct or sign a KZG policy/record. Unknown
future wire discriminants fail decoding before block construction or admission.
