...
- `iroha app da get` 為已經支持的多源協調器添加了一個以 DA 為中心的別名
  ...
  使用 CLI 從 `/v2/da/manifests/<ticket>` 中提取清單的路徑，將捆綁包保留在
...
- **PDP/PoTR 掛鉤** — PDP 承諾直接源自塊存儲並被持久化
  ...
  `/v2/da/ingest` 和 `/v2/da/manifests/{ticket}` 現在包含 `Sora-PDP-Commitment` 標頭
...
- Torii 的 `/v2/da/ingest` 端點現在標準化有效負載壓縮，強制重播緩存，
