...
- `iroha app da get` 为已经支持的多源协调器添加了一个以 DA 为中心的别名
  ...
  使用 CLI 从 `/v1/da/manifests/<ticket>` 中提取清单的路径，将捆绑包保留在
...
- **PDP/PoTR 挂钩** — PDP 承诺直接源自块存储并被持久化
  ...
  `/v1/da/ingest` 和 `/v1/da/manifests/{ticket}` 现在包含 `Sora-PDP-Commitment` 标头
...
- Torii 的 `/v1/da/ingest` 端点现在标准化有效负载压缩，强制重播缓存，
