...
- `iroha app da get` әлдеқашан қуат беретін көп көзді оркестр үшін DA-ға бағытталған бүркеншік атын қосады
  ...
  жол пайдаланылады, CLI манифестті `/v2/da/manifests/<ticket>` ішінен шығарады, астындағы топтаманы сақтайды
...
- **PDP/PoTR ілмектері** — PDP міндеттемелері тікелей бөлшектер қоймасынан алынады және сақталады
  ...
  `/v2/da/ingest` плюс `/v2/da/manifests/{ticket}` енді `Sora-PDP-Commitment` тақырыбын қамтиды
...
- Torii `/v2/da/ingest` соңғы нүктесі енді пайдалы жүктің қысылуын қалыпқа келтіреді, қайта ойнату кэшін күшейтеді,
...
- Torii сонымен қатар `GET /v2/da/manifests/{storage_ticket}` көрсетеді, осылайша SDK және операторлар манифесттерді ала алады.
