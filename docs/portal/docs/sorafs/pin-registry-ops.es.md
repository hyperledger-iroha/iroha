...
2. **Validar estado de providers**
   - Ejecuta `iroha sorafs providers list` y verifica que las capacidades anunciadas cumplan los requisitos de replicación.
   - Revisa los gauges `torii_sorafs_capacity_*` para confirmar GiB provisionados y éxito de PoR.
3. **Reasignar replicación**
   - Emite nuevas órdenes vía `sorafs_manifest_stub capacity replication-order` cuando la holgura del backlog (`stat="avg"`) cae por debajo de 5 épocas (el empaquetado de manifest/CAR usa `iroha sorafs toolkit pack`).
