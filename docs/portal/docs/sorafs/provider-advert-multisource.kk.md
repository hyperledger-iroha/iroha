...
- Провайдер жарнамасының пайдалы жүктемелері `range_capability`, `stream_budget` және
  `transport_hints` метадеректері. `/v2/sorafs/providers` жауаптары арқылы растаңыз және
...
- Torii `/v2/sorafs/providers` талданған ауқым мүмкіндігінің метадеректерін қайтарады
