...
title: SoraFS オーケストレーター設定
sidebar_label: オーケストレーター設定
description: マルチソース fetch オーケストレーターの設定、失敗の読み取り、テレメトリ出力のデバッグ方法を解説します。
---
...
セレクターは legacy descriptor フィールドに戻ります。証明書は circuit のライフサイクル
