...
source_hash: 32836e1dd2d4c5b1ce7a203a8a9bcb35bbe1da25e14093e7bfd08982133c4038
source_last_modified: "2025-11-13T07:55:08.200044+00:00"
...
**Артефакты:**
- Исходники CLI и тесты (`crates/sorafs_orchestrator/src/bin/sorafs_cli.rs`)
...
- Deterministic parity harness (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`, [Отчет о паритете GA SoraFS Orchestrator](./orchestrator-ga-parity.md))
...
| SF6-SR-03 | Medium | Отправка manifests | CLI принимал manifests без проверки встроенных chunk plans, когда `--plan` отсутствовал. | `sorafs_cli manifest submit` теперь пересчитывает и сравнивает CAR digests, если не указан `--expect-plan-digest`, отклоняя mismatches и выдавая remediation hints. Тесты покрывают успех/ошибки (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`). |
...
4. Логи parity harness (`cargo test -p sorafs_orchestrator --test sorafs_cli proof_stream_consumes_ndjson_and_reports_metrics -- --nocapture`).
