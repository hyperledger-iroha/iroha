...
|------|----------|-------------------------|-------|
| 2026-02-12 | Tooling (LLM) | FAIL | Paso 1: `cargo test -p sorafs_chunker` falla el suite `vectors` porque los fixtures aun publican el handle legacy `sorafs-sf1` y carecen de profile aliases/digests (`fixtures/sorafs_chunker/sf1_profile_v1.*`). Paso 2: `ci/check_sorafs_fixtures.sh` aborta - falta `manifest_signatures.json` en el estado del repo (borrado en el working tree). Paso 4: `export_vectors` no puede verificar firmas mientras falta el archivo de manifest. Se recomienda restaurar los fixtures firmados (o proporcionar la clave del council) y regenerar bindings para que el handle canonico + arreglo de aliases se incrusten como exigen los tests. |
