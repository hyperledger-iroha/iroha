...
**Artefactos:**
- Fuente del CLI y tests (`crates/sorafs_orchestrator/src/bin/sorafs_cli.rs`)
...
4. Logs capturados del harness de paridad (`cargo test -p sorafs_orchestrator --test sorafs_cli proof_stream_consumes_ndjson_and_reports_metrics -- --nocapture`).
