...
**ቅርሶች:**
- የ CLI ምንጭ እና ሙከራዎች (`crates/sorafs_orchestrator/src/bin/sorafs_cli.rs`)
...
- ቆራጥ እኩልነት መታጠቂያ (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`፣ [SoraFS ኦርኬስትራ GA የፓሪቲ ሪፖርት](./orchestrator-ga-parity.md))
...
| SF6-SR-03 | መካከለኛ | ማስረከቢያ | `--plan` በሌለበት ጊዜ CLI የተከተቱ ቁርጥራጭ ዕቅዶችን ሳያረጋግጥ ተቀብሏል። | `sorafs_cli manifest submit` አሁን `--expect-plan-digest` እስካልቀረበ ድረስ የ CAR መፈጨትን ያሰላል እና ያወዳድራል፣ አለመዛመጃዎችን ውድቅ ያደርጋል እና የማሻሻያ ፍንጮችን ያሳያል። ሙከራዎች የስኬት/የሽንፈት ጉዳዮችን ይሸፍናሉ (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`)። |
...
4. የተያዙ ምዝግብ ማስታወሻዎች ከፓሪቲ ታጥቆ (`cargo test -p sorafs_orchestrator --test sorafs_cli proof_stream_consumes_ndjson_and_reports_metrics -- --nocapture`).
