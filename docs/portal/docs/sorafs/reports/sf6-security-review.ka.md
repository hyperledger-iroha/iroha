...
**არტეფაქტები:**
- CLI წყარო და ტესტები (`crates/sorafs_orchestrator/src/bin/sorafs_cli.rs`)
...
- დეტერმინისტული პარიტეტის აღკაზმულობა (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`, [SoraFS Orchestrator GA პარიტეტის ანგარიში] (./orchestrator-ga-parity.md))
...
| SF6-SR-03 | საშუალო | მანიფესტი წარდგენა | CLI-მ მიიღო მანიფესტები ჩაშენებული ნაწილის გეგმების გადამოწმების გარეშე, როდესაც `--plan` არ იყო. | `sorafs_cli manifest submit` ახლა ხელახლა ითვლის და ადარებს CAR-ის შეგროვებას, თუ არ არის მოწოდებული `--expect-plan-digest`, უარყოფს შეუსაბამობებს და ასახავს ზედაპირული გამოსწორების მინიშნებებს. ტესტები მოიცავს წარმატების/მარცხის შემთხვევებს (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`). |
...
4. აღებული მორები პარიტეტული აღკაზმულობიდან (`cargo test -p sorafs_orchestrator --test sorafs_cli proof_stream_consumes_ndjson_and_reports_metrics -- --nocapture`).
