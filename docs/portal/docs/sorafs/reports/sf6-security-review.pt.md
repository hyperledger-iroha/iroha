...
**Artefactos:**
- Fonte do CLI e tests (`crates/sorafs_orchestrator/src/bin/sorafs_cli.rs`)
...
- Deterministic parity harness (`crates/sorafs_orchestrator/tests/sorafs_cli.rs`, [SoraFS Orchestrator GA Parity Report](./orchestrator-ga-parity.md))
...
4. Logs capturados do parity harness (`cargo test -p sorafs_orchestrator --test sorafs_cli proof_stream_consumes_ndjson_and_reports_metrics -- --nocapture`).
