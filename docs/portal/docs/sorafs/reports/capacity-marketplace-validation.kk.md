...
|-------|------------|----------|
| Реестр канондық сыйымдылық туралы мәлімдемелерді қабылдайды | Қолтаңбаны өңдеуді, метадеректерді түсіруді және түйін тізіліміне жіберуді тексеретін API қолданбасы арқылы `/v2/sorafs/capacity/declare` интеграциялық сынақ жаттығулары. | `crates/iroha_torii/src/routing.rs:7654` |
