...
посылать legacy handles в `Accept-Chunker` без догадок. Правила charter реестра требуют, чтобы
канонический handle (`namespace.name@semver`) был первой записью в `profile_aliases`, за которой
следуют любые legacy-алиасы.
...
| `sorafs_manifest_stub` | ⚠️ Legacy | Legacy builder манифеста; используйте `iroha sorafs toolkit pack` для упаковки CAR/manifest и держите `--plan=-` для детерминированной ревалидации. |
| `sorafs_provider_advert_stub` | ⚠️ Legacy | Только offline helper для валидации; provider adverts должны выпускаться publishing pipeline и валидироваться через `/v1/sorafs/providers`. |
...
| CARv1 bridge (`sha2-256`) | ⚠️ Переходный | Доступен для legacy клиентов, когда запрос объявляет и канонический профиль, и `Accept-Digest: sha2-256`; ответы включают `Content-Chunker: ...;legacy=true`. |
...
(описанное в предложении) до того, как отметить `sorafs.sf1@1.0.0` как deprecated в реестре и удалить
...
Канонический manifest envelope использует CIDv1 корни с `dag-cbor` (`0x71`). Для legacy совместимости
...
* **Legacy bridge** – CARv1, SHA-256 payload digest (`0x12` multihash). Серверы
  МОГУТ выдавать этот вариант, когда клиент опускает `Accept-Chunker` или запрашивает
  `Accept-Digest: sha2-256`.

Manifests всегда объявляют CARv2/BLAKE3 commitment. Legacy каналы предоставляют дополнительные
