...
|-----------|------|----------|
| Torii სერვისი | გამოაქვეყნეთ `/v2/sorafs/pin` (გაგზავნა), `/v2/sorafs/pin/{cid}` (მოძიება), `/v2/sorafs/aliases` (სია/შეკვრა), `/v2/sorafs/replication` (შეკვეთები/შეკვეთები). უზრუნველყოს პაგინაცია + ფილტრაცია. | ქსელის TL / Core Infra |
...
- `GET /v2/sorafs/pin` და `GET /v2/sorafs/pin/{digest}` დაბრუნება გამოიხატება
  ფსევდონიმების შეკვრა, რეპლიკაციის ბრძანებები და ატესტაციის ობიექტი, რომელიც მიღებულია
  უახლესი ბლოკის ჰეში.
- `GET /v2/sorafs/aliases` და `GET /v2/sorafs/replication` ამჟღავნებს აქტიურს
