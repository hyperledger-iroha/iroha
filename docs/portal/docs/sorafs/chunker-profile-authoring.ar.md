...
- يجب أن يظهر المقبض المعتمد (`namespace.name@semver`) في قائمة البدائل
  وأن يكون **الأول**. تليه البدائل القديمة (مثل `sorafs-sf1`).
...
| `profile_aliases` | مقابض إضافية اختيارية (أسماء قديمة، اختصارات) تُعرض للعملاء أثناء التفاوض. يجب تضمين المقبض المعتمد أولاً. | `["sorafs-sf1"]` |
