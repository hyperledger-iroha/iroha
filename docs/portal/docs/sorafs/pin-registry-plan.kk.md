...
|----------|------|----------|
| Torii қызметі | `/v2/sorafs/pin` (жіберу), `/v2/sorafs/pin/{cid}` (іздеу), `/v2/sorafs/aliases` (тізім/байлау), `/v2/sorafs/replication` (тапсырыстар/түбіртектерді) көрсетіңіз. Беттеу + сүзуді қамтамасыз етіңіз. | Networking TL / Core Infra |
...
- `GET /v2/sorafs/pin` және `GET /v2/sorafs/pin/{digest}` қайтару манифестерімен
  бүркеншік ат байланыстырулары, репликация тапсырыстары және алынған аттестаттау нысаны
  соңғы блок хэші.
- `GET /v2/sorafs/aliases` және `GET /v2/sorafs/replication` белсенділікті көрсетеді
