...
This page mirrors the plan maintained under `docs/source/sorafs_observability_plan.md`. Keep both copies in sync until the legacy Sphinx set is fully migrated.
:::
...
| `torii_sorafs_chunk_range_requests_total`, `torii_sorafs_gateway_refusals_total` | Counter | Legacy label sets | Prometheus metrics retained for historical dashboards; emitted alongside the new OTLP series. |
...
- Collectors: run OTEL collectors alongside Prometheus/Loki/Tempo (Tempo preferred). Jaeger-compatible exporters remain optional.
