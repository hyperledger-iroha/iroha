...
| `max_penalty` | 1 000 000 000 | Максимальный штраф за косую черту, разрешенный для эскалации ремонта (nano-XOR). |

- Предложения, созданные планировщиком, ограничены `max_penalty`; Представления аудитора, превышающие этот предел, отклоняются.
