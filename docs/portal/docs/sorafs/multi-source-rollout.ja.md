...
title: マルチソース・クライアントのロールアウトとブラックリスト運用ランブック
sidebar_label: マルチソース・ロールアウト・ランブック
description: 段階的なマルチソース・ロールアウトと緊急時のプロバイダー拒否のための運用チェックリスト。
---
...
1. **ガバナンス入力を確認する。**
   - すべての候補プロバイダーは、レンジ能力 payload とストリーム予算を含む `ProviderAdvertV1` エンベロープを公開する必要があります。`/v1/sorafs/providers` で検証し、期待される capability フィールドと照合してください。
