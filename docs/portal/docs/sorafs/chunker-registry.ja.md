...
---
id: chunker-registry
title: SoraFS チャンカー・プロファイル・レジストリ
sidebar_label: チャンカー・レジストリ
description: SoraFS チャンカー・レジストリのプロファイル ID、パラメータ、交渉計画。
---
...
| CARv1 bridge (`sha2-256`) | ⚠️ 移行中 | リクエストが正規プロファイルと `Accept-Digest: sha2-256` の両方を宣言した場合にレガシークライアント向けに利用可能。レスポンスには `Content-Chunker: ...;legacy=true` が含まれます。 |
...
`sorafs.sf1@1.0.0` をレジストリで deprecated としてマークし、本番ゲートウェイから CARv1 bridge を削除します。
