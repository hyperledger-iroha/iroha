...
| `sorafs_manifest_builder` | ⚠️ レガシー | レガシー manifest builder。CAR/manifest のパッケージングには `iroha app sorafs toolkit pack` を使い、決定的な再検証のために `--plan=-` を維持します。 |
...
manifest builder は同じデータを反映しており、パイプラインで `--chunker-profile-id` の選択を
...
* `iroha app sorafs toolkit pack` と `sorafs_manifest_builder` が生成する manifest にはレジストリメタデータが含まれます。
