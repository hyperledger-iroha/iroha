...
---
title: "תוכנית rollout ותאימות של adverts לספקי SoraFS"
---
...
| **R3 – כיבוי legacy** | **מ-2025-07-01 ואילך** | Discovery מפסיק לתמוך ב-adverts בינאריים שאינם מציבים `signature_strict=true` או שחסרים `profile_aliases`. Torii discovery cache מנקה ערכים מיושנים שהחמצו את refresh deadline ללא חידוש. | - לתזמן חלון decommission סופי לסטאקים legacy של ספקים. <br />- לוודא ש-`--allow-unknown` GREASE רץ רק בתרגילים מבוקרים ונרשם בלוגים. <br />- לעדכן playbooks של אירועים כך שפלט האזהרה של `sorafs_fetch` ייחשב blocker לפני releases. | להחמיר alerts: כל `warn` מציף on-call. להוסיף checks סינתטיים שמושכים discovery JSON ומוודאים רשימות capabilities של ספקים. |
