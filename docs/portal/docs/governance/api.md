...
- Governance endpoints never server-sign. Clients assemble a `SignedTransaction`
  using their authority and chain id, then sign locally and POST to
...
- POST `/v1/gov/enact`
  - Request: `{ "proposal_id": "…64hex" }`
  - Response: `{ "ok": true, "tx_instructions": [{ "wire_id": "…EnactReferendum", "payload_hex": "…" }] }`
  - Notes: Torii returns a skeleton for clients to sign and submit locally.
...
- POST `/v1/gov/ballots/zk-v1/ballot-proof`
  ...
  - Response:
    ...
    ```
  - Notes:
    - The strict request has no private-key field; Torii returns only an unsigned instruction skeleton for local signing.
