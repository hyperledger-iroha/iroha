...
- JavaScript (`@iroha/iroha-js`)：`ToriiClient` 表面鍵入了提案、公投、統計、鎖定、解鎖統計數據的幫助程序，現在 `listGovernanceInstances(namespace, options)` 加上理事會端點（`getGovernanceCouncilCurrent`、`governanceDeriveCouncilVrf`、`governancePersistCouncil`、 `getGovernanceCouncilAudit`），因此 Node.js 客戶端可以對 `/v2/gov/instances/{ns}` 進行分頁，並在現有合約實例列表旁邊驅動 VRF 支持的工作流程。

端點

- 後 `/v2/gov/proposals/deploy-contract`
  - 請求（JSON）：
    {
      “命名空間”：“應用程序”，
...
- 獲取 `/v2/gov/council/current`
  ...
  - 註釋：返回存在的持久理事會；否則使用配置的權益資產和閾值得出確定性後備（鏡像 VRF 規範，直到實時 VRF 證明保留在鏈上）。- POST `/v2/gov/council/derive-vrf`（功能：gov_vrf）
...
便利端點
- POST `/v2/gov/protected-namespaces` — 直接在節點上應用 `gov_protected_namespaces`。
  - 請求：{“命名空間”：[“應用程序”，“系統”]}
...
5. 通過 `/v2/sumeragi/evidence` 和 `/v2/sumeragi/status` **審核結果**，以確保證據反駁取得進展並由治理部門實施刪除。
...
- 運行時和 CLI 通過 `/v2/sumeragi/params` 和 `iroha --output-format text ops sumeragi params` 公開分階段參數，以便操作員可以確認激活高度和驗證器名冊。
- 治理自動化應始終：
