...
- Agar `authority` va `private_key` (yoki saylov byulletenlarida `private_key`) taqdim etilgan bo'lsa, Torii imzo qo'yadi va bitimni topshiradi va baribir `tx_instructions` ni qaytaradi.
...
- POST `/v1/gov/finalize`
  ...
  - Eslatmalar:
    - ZK saylovlari uchun kontrakt yo'llari `FinalizeElection` ni bajarishdan oldin `ZK_VOTE_VERIFY_TALLY` raqamiga qo'ng'iroq qilishlari kerak; xostlar bir martalik mandalni majbur qiladi. `FinalizeReferendum` saylovlar yakuni aniqlanmaguncha ZK referendumini rad etadi.
...
Doimiy ro'yxat mavjud bo'lmaganda Torii tomonidan qo'llaniladigan kengash zaxirasi `iroha_config` orqali parametrlanadi:
...
`parliament_committee_size` hech qanday kengash davom etmaganda qaytarilgan a'zolar sonini cheklaydi, `parliament_term_blocks` urug' olish uchun ishlatiladigan davr uzunligini belgilaydi (`epoch = floor(height / term_blocks)`), `parliament_min_stake` (eng kichik va eng kichik birliklarni talab qiladi) `parliament_eligibility_asset_id` nomzodlar to'plamini yaratishda qaysi aktiv balansi skanerlanishini tanlaydi.

Boshqaruv VK tekshiruvini chetlab o‘tish imkoniyati yo‘q: byulletenni tekshirish har doim ichki baytlarga ega `Active` tasdiqlash kalitini talab qiladi va muhitlar tekshirishni o‘tkazib yuborish uchun faqat test o‘tish tugmalariga tayanmasligi kerak.
...
Himoyalangan nom maydonlari
- `gov_protected_namespaces` maxsus parametri (JSON qatorlar qatori) roʻyxatdagi nomlar boʻshliqlariga joylashtirish uchun ruxsat berish imkoniyatini beradi.
...
- `gov_manifest_approvers`: validator hisob identifikatorlarining ixtiyoriy JSON massivi. Agar chiziqli manifest birdan ortiq kvorumni e'lon qilsa, qabul qilish manifest kvorumini qondirish uchun tranzaksiya organi va sanab o'tilgan hisoblarni talab qiladi.
- Telemetriya `governance_manifest_admission_total{result}` orqali yaxlit qabul qilish hisoblagichlarini ochib beradi, shuning uchun operatorlar muvaffaqiyatli qabullarni `missing_manifest`, `non_validator_authority`, `quorum_rejected`, `protected_namespace_rejected`, `protected_namespace_rejected`, `protected_namespace_rejected`, I18100 va I18NI00000100 yoʻllaridan ajrata oladilar.
- Telemetriya ijro yo'lini `governance_manifest_quorum_total{outcome}` (qiymatlari `satisfied` / `rejected`) orqali yopadi, shuning uchun operatorlar etishmayotgan tasdiqlarni tekshirishlari mumkin.
- Yo'llar o'zlarining manifestlarida chop etilgan nomlar maydoni ruxsat etilgan ro'yxatini ta'minlaydi. `gov_namespace` o'rnatuvchi har qanday tranzaksiya `gov_contract_id` ni ta'minlashi kerak va nom maydoni manifestning `protected_namespaces` to'plamida paydo bo'lishi kerak. Himoya yoqilgan bo'lsa, ushbu metama'lumotlarsiz `RegisterSmartContractCode` taqdimnomalari rad etiladi.
- `(namespace, contract_id, code_hash, abi_hash)` korteji uchun kuchga kirgan boshqaruv taklifi mavjudligini qabul qilish; aks holda tekshirish Ruxsat berilmagan xato bilan bajarilmaydi.
...
- Kanca maydonlari:
  ...
  - `allowed_ids` (satrlar massivi): metama'lumotlar qiymatlarining ixtiyoriy ruxsat etilgan ro'yxati (qirqishdan keyin). Taqdim etilgan qiymat ro'yxatda bo'lmaganda rad etadi.
- Ilgak mavjud bo'lganda, navbatga kirish tranzaksiya navbatga kirishidan oldin metadata siyosatini amalga oshiradi. Yo'qolgan metama'lumotlar, bo'sh qiymatlar yoki ruxsat berilganlar ro'yxatidan tashqaridagi qiymatlar deterministik `NotPermitted` xatosini keltirib chiqaradi.
...
- POST `/v1/gov/ballots/zk-v1`
  ...
  - Eslatmalar:
    - Server ixtiyoriy `root_hint`/`owner`/`amount`/`duration_blocks`/`direction`/`nullifier` dan I018 uchun byulletenni I020 dan I020 ga xaritalaydi. `CastZkBallot`.
    - Zarf baytlari ko'rsatmalarning foydali yuki uchun base64 sifatida qayta kodlangan.
    - `reason` javobi Torii ovoz berish byulletenini topshirganda `submitted transaction` ga o'zgaradi.
...
- Agar `public_inputs_json` taʼminlangan boʻlsa, u JSON obyekti boʻlishi kerak; ob'ekt bo'lmagan foydali yuklar rad etiladi.
...
- Saqlangan tasdiqlovchi kalit baytlar `hash_vk` bilan qayta xeshlanadi; har qanday majburiyatning nomuvofiqligi ro'yxatga olish kitobi o'zgartirilgan yozuvlaridan (`BallotRejected` bilan `verifying key commitment mismatch`) himoya qilish uchun tekshirishdan oldin bajarilishini to'xtatadi.
- Tasdiqlash baytlari `zk::verify_backend` orqali ro'yxatdan o'tgan backendga yuboriladi; yaroqsiz transkriptlar `invalid proof` bilan `BallotRejected` ko'rinishida paydo bo'ladi va ko'rsatma aniq bajarilmaydi.
...
### Slashing va Jailing Ish jarayoni

Har safar validator protokolni buzsa, konsensus Norito kodli `Evidence` chiqaradi. Har bir foydali yuk `EvidenceStore` xotirasiga tushadi va agar ko'rinmasa, WSV tomonidan qo'llab-quvvatlanadigan `consensus_evidence` xaritasida amalga oshiriladi. `sumeragi.npos.reconfig.evidence_horizon_blocks` dan eskiroq yozuvlar (standart `7 200` bloklari) rad etiladi, shuning uchun arxiv cheklangan bo'lib qoladi, lekin rad etish operatorlar uchun qayd etiladi. Ufqdagi dalillar qo'shma konsensus bosqichma-bosqich qoidasiga (`mode_activation_height requires next_mode to be set in the same block`), faollashtirish kechikishiga (`sumeragi.npos.reconfig.activation_lag_blocks`, sukut bo'yicha `1`) va kesish kechikishiga (`sumeragi.npos.reconfig.slashing_delay_blocks`, sukut bo'yicha I04NIC) bo'ysunadi. qo'llashdan oldin jarimalar.
...
- **DoublePrepare/DoubleCommit** — validator bir xil `(phase,height,view,epoch)` korteji uchun ziddiyatli xeshlarni imzoladi.
- **InvalidQc** — agregator shakli deterministik tekshiruvlardan o'tmagan majburiyat sertifikatini g'iybat qildi (masalan, imzo qo'yuvchining bo'sh xaritasi).
...
Qo'shma konsensus, chiquvchi validator to'plami yangi to'plam taklif qilishni boshlashdan oldin chegara blokini yakunlashini kafolatlaydi. Ish vaqti qoidani juftlashtirilgan parametrlar orqali amalga oshiradi:

- `SumeragiParameter::NextMode` va `SumeragiParameter::ModeActivationHeight` **bir xil blokda** amalga oshirilishi kerak. `mode_activation_height` kamida bir blokli kechikishni ta'minlab, yangilanishni amalga oshirgan blok balandligidan qat'iy kattaroq bo'lishi kerak.
- `sumeragi.npos.reconfig.activation_lag_blocks` (standart `1`) nol kechikishlarni oldini oluvchi konfiguratsiya himoyachisi:
...
- `/status` takliflar sonini aks ettiruvchi `governance` ob'ektini o'z ichiga oladi, himoyalangan nomlar maydoni jami to'g'risida hisobot beradi va so'nggi manifest faollashuvlari (nomlar maydoni, shartnoma identifikatori, kod/ABI xeshi, blok balandligi, faollashtirish vaqt tamg'asi) ro'yxatini beradi. Operatorlar ushbu maydonda yangilangan qonunlar manifestligini va himoyalangan nom maydoni eshiklari kuchga kirganligini tasdiqlash uchun so'rov o'tkazishlari mumkin.
