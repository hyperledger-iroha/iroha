...
- Default every copy/share action to the IH-B32 address. Display the resolved
...
- Prevent clipboard desync by emitting a confirmation toast that previews the
  exact IH-B32 string that was copied. Where telemetry is available, count copy
...
- Supply descriptive `alt` text for QR previews (e.g., “IH-B32 address for
  `<alias>@<domain>` on chain `0x1234`”). Provide a “Copy address as text”
...
- Labelling: every occurrence must include a visible “Sora-only” badge and a
  tooltip describing why other networks require the IH-B32 form.
...
- Surface these checks in release checklists alongside the IH-B32 parity tests
