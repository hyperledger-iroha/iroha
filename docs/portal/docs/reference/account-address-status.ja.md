...
各ハーネスはカノニカルバイト + IH58 + 圧縮エンコードを往復し、ネガティブケースの Norito スタイルのエラーコードが fixture と一致することを検証します。
