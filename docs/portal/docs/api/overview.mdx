...
automatically. The generator builds the canonical OpenAPI projection from the
Torii route catalog and fails if that live projection cannot be produced. The
release workflow has no stub escape hatch: a missing, empty, or structurally
incomplete document fails before manifest creation, signing, synchronization,
or version publication.

## Current status

- **Spec source:** `cargo run -p xtask --bin xtask -- openapi` queries the Torii
  router directly and fails closed if the complete specification is
  unavailable.
...
Torii also exposes a native Model Context Protocol bridge at `/v1/mcp`.
Unlike REST endpoints, MCP calls use JSON-RPC 2.0 (`initialize`, `tools/list`,
`tools/call`, `tools/call_batch`, `tools/call_async`, `tools/jobs/get`) and
are controlled by `torii.mcp` configuration.

Use the dedicated MCP reference for agent integrations:

- [`/reference/torii-mcp`](/reference/torii-mcp)
