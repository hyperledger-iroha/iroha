...
explicit `--allow-stub` escape hatch is diagnostic-only and must never be used
for a tracked canonical or release artifact.

## Current status

- **Spec source:** `cargo run -p xtask --bin xtask -- openapi` queries the Torii
  router directly; stub output requires an explicit diagnostic opt-in and is
  rejected by the canonical artifact workflow.
