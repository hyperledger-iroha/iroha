...
- **Құрал (`FinInstrmId`)**
  ...
  - Қайтармалар:
    - **CUSIP** немесе сыртқы ISO стандартынан `Tp/Cd` орнатылған `…/OthrId/Id` астында басқа NSIN
      код тізімі (мысалы, `CUSP`); рұқсат етілген кезде эмитентті `Issr` ішіне қосыңыз.[^iso_mdr]
    - **Norito актив идентификаторы** меншік ретінде: `…/OthrId/Id`, `Tp/Prtry="NORITO_ASSET_ID"` және
...
- **Тараптар (`DlvrgSttlmPties`, `RcvgSttlmPties`)**
  ...
  - Қалпына келтіру: **LEI** мұнда хабардың нұсқасы арнайы LEI өрісін көрсетеді; егер
    жоқ болса, анық `Prtry` белгілері бар меншікті идентификаторларды алып жүріңіз және метадеректерге BIC қосыңыз.[^iso_cr]
...
##### `pacs.009` / `camt.054` (PvP қаржыландыру және мәлімдемелер)

- **Агенттер (`InstgAgt`, `InstdAgt`, борышкер/кредитор агенттері)** → **BIC** міндетті емес
...
- **Torii қабылдау** → `POST /v2/iso20022/pacs009` арқылы PvP қаржыландыру аяқтарын жіберу; көпір
  `Purp=SECU` талап етеді және енді анықтамалық деректер конфигурацияланған кезде BIC жаяу жүргіншілер өткелдерін мәжбүрлейді.
...
#### Жаяу жүргіншілер жолы және деректерге техникалық қызмет көрсету бойынша міндеттемелер

- **ISIN ↔ Norito актив идентификаторын** және **CUSIP ↔ ISIN** жаяу жүргіншілер өткелдерін ұстаңыз. Түнде жаңартыңыз
...
- ISO жаяу жүргіншілер жолының суреттері `iroha_core::iso_bridge::reference_data` арқылы қабылданады.
  ...
  көпір `pacs.008` жіберулерді қабылдамайды, олардың агенті BIC конфигурацияланғанда жоқ
...
- IBAN және ISO 4217 байланыстырулары бір деңгейде орындалады: pacs.008/pacs.009 ағындары қазір
  ...
  `currency_assets` есеп айырысу валютасы жоқ, бұл дұрыс емес көпірдің алдын алады
...
- CLI есеп айырысу көмекшілері бірдей қорғаныс рельстерін мұраға алады: өту
  ...
  `sese.023` XML суретін шығармас бұрын құрал идентификаторларын тексеру.【crates/iroha_cli/src/main.rs#L3752】
...
- IVM көмекшісі енді нақты ISO 20022 XML конверттерін қабылдайды (бас.001 + `DataPDU` + `Document`)
  ...
  блоктар әдейі өткізілмей қалады. 

#### Нормативтік және нарықтық құрылымдық ойлар- **T+1 есеп айырысуы**: АҚШ/Канада үлестік нарықтары 2024 жылы T+1 деңгейіне көшті; Norito реттеңіз
  сәйкесінше жоспарлау және SLA ескертулері.[^sec_t1][^csa_t1]
- **CSDR айыппұлдары**: Есеп айырысу тәртібінің ережелері қолма-қол айыппұлдарды жүзеге асырады; Norito қамтамасыз етіңіз
...
### Жеткізу төлемге қарсы → `sese.023`

| DvP өрісі | ISO 20022 жолы | Ескертпелер |
|------------------------------------------------------|---------------------------------------|-------|
...
олар бағалы қағаздармен есеп айырысуды қаржыландырады.

| PvP қаржыландыру өрісі | ISO 20022 жолы | Ескертпелер |
|------------------------------------------------|----------------------------------------------------|-------|
...
| Есеп айырысу мақсаты | `CdtTrfTxInf/PmtTpInf/CtgyPurp/Cd` | Бағалы қағаздарға қатысты PvP қаржыландыруы үшін `SECU` мәніне орнатыңыз. |
...
`sese.025` шығаруды жалғастырады, бірақ бұл растау бағалы қағаздардың аяғын көрсетеді
PvP «мақсаты» емес, күй (мысалы, `ConfSts = ACCP`).

### Төлем мен төлемді растау → `sese.025`
...
### РЕПО кепілін ауыстыру → `colr.007`

| Репо өрісі / контекст | ISO 20022 жолы | Ескертпелер |
|------------------------------------------------|----------------------------------|-------|
...
* `TxId` мәндері `Max35Text` — UTF‑8 ұзындығы ≤35 таңбадан бұрын күшіне енеді
...
* `SupplementaryData` канондық JSON болуы керек (UTF‑8, сұрыпталған кілттер, JSON түпнұсқасы
...
- `SttlmTpAndAddtlParams/SctiesMvmntTp` (`DELI`/`RECE`) және `Pmt` көрсететін LuxCSD / Clearstream ISO 20022 есеп айырысу мысалдары (`APMT`/`FREE`).<sup>[1](https://www.luxcsd.com/resource/blob/3434074/6f8add4708407a4701055be4dd04846b/c23005-eis-examples-cbf-data.pdf)</sup>
- Есеп айырысу квалификацияларын қамтитын Clearstream DCP спецификациялары (`SttlmTxCond`, `PrtlSttlmInd`).<sup>[2](https://www.clearstream.com/clearstream-en/res-library/market-coverage/instruction-specifications-swift-iso-20022-dcp-mode-ceu-spain-2357008)</sup>
- SWIFT PMPG нұсқаулығы бағалы қағаздарға қатысты PvP қаржыландыру үшін `pacs.009` және `CtgyPurp/Cd = SECU` ұсынатын.<sup>[3](https://www.swift.com/swift-resource/251897/download)</sup>
- Идентификатор ұзындығы шектеулері үшін ISO 20022 хабар анықтамасының есептері (BIC, Max35Text).<sup>[4](https://www.iso20022.org/sites/default/files/2020-12/ISO20022_MDRPart2_ChangeOrVerifyAccountIdentification_2020_2021_v1_ForSEGReview.pdf)</sup>
- ISIN пішімі және бақылау сомасы ережелері бойынша ANNA DSB нұсқаулығы.<sup>[5](https://www.anna-dsb.com/isin/)</sup>

### Пайдалану бойынша кеңестер

- LLM тексере алуы үшін әрқашан сәйкес Norito үзіндісін немесе CLI пәрменін қойыңыз.
...
5. `sese.024` күй кеңестері (қабылданды, сәйкес келді, күтуде, орындалмады, қабылданбады).  
...
- `SttlmParams/SttlmTxCond/Cd` нарықтық жағдайларға арналған (`NOMC`, т.б.).  
...
**Уақыт:** FX PvP терезелері (CLS/екі жақты) және CSD үзінділері; бағалы қағаздарды қолма-қол ақшаны растауды күтуде ұстау.

#### Хабарлама хореографиясы
1. `CtgyPurp/Cd = SECU` валютасына `pacs.009` FI-to-FI Credit Transfer; `pacs.002` арқылы күй; `camt.056`/`camt.029` арқылы қайтарып алу/бас тарту; егер шешілсе, `pacs.004` қайтарылады.  
...
4. Терезенің мерзімі біткенге дейін `pacs.009` екі аяғы да `ACSC` жетсе → `sese.030` → `sese.031` (мод күйі) → `sese.025` (растау) арқылы шығарыңыз.  
5. FX терезесі бұзылса → қолма-қол ақшаны жою/қайтарып алу (`camt.056/029` немесе `pacs.004`) және бағалы қағаздардан бас тарту (`sese.020` + `sese.027` немесе I18NI00000 ережелері расталған болса).
...
- қайтару құрылғылары енді `ChrgBr` (`DEBT`/`CRED`/`SHAR`/`SLEV`) қалыпқа келтіреді және меншікті қайтару себептері I18NI0100, сондықтан тұтынушыларға қайта әсер етеді. XML конвертін қайта талдаусыз атрибуция және оператор кодтары.
...
- нарықтық күнтізбе/кесінділер бойынша ұстау/босату уақытын параметрлеу; `sese.030`/`camt.056` шығару мерзімін тоқтату мерзіміне дейін, қажет болған жағдайда қайтару үшін қайтару.
...
Байланыстырылған нұсқауды `SUBST-2025-04-001-B` (ауыстырмалы кепілді алу) `SctiesMvmntTp=RECE`, `Pmt=FREE` және `SUBST-2025-04-001-A` сілтемесін көрсететін `WITH` сілтемесін жіберіңіз. Ауыстыру мақұлданғаннан кейін екі аяқты сәйкес `sese.030` арқылы босатыңыз.
...
````pacs.009` екі аяғы да `ACSC` жеткенде босатыңыз:
...
`pacs.002` төлем күйін қадағалайды (`ACSC` = расталды, `RJCT` = қабылдамау). Терезе бұзылса, `camt.056`/`camt.029` арқылы қайтарып алыңыз немесе есептелген қаражатты қайтару үшін `pacs.004` жіберіңіз.