...
zonefile skeleton はゾーンの authoritative レコードのみを定義します。通常のインターネットがネームサーバーを見つけられるよう、親ゾーンの NS/DS 委任はレジストラまたは DNS プロバイダで別途設定してください。

- apex/TLD の cutover では ALIAS/ANAME（プロバイダ依存）または gateway の anycast IP を指す A/AAAA を公開します。
- サブドメインは derived pretty host (`<fqdn>.gw.sora.name`) への CNAME を公開します。
- canonical host (`<hash>.gw.sora.id`) は gateway ドメイン配下に残り、公開ゾーンには掲載しません。
