...
O fluxo assume que voce tem o binario `sorafs_cli` (built from the `sorafs_orchestrator` Cargo target), acesso a
...
- `sorafs_cli` a partir de `cargo run -p sorafs_orchestrator --bin sorafs_cli`.
