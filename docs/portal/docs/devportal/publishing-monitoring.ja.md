...
title: SoraFS 公開と監視
sidebar_label: 公開と監視
description: SoraFS ポータルのリリースに対する end-to-end の監視フローを記録し、DOCS-3c が決定的な probes とテレメトリー、証跡バンドルを持てるようにする。
---
...
2. **ピン留めと検証** - `sorafs_cli manifest submit`、`verify-sorafs-binding.mjs`、
...
各 binding エントリは `Sora-Name`, `Sora-Proof`, `Sora-Content-CID`（header と payload）と
`expectHost` ガードを強制し、DNS 昇格（`docs.sora` と `docs-preview.sora.link`）が pin registry に記録した
alias から逸脱しないようにします。gateway が `Sora-Content-CID`/`Sora-Proof` ヘッダを付与しなくなったり、
無効な base64 が混入したり、advertise された manifest/CID が pin した payload（サイト、OpenAPI、SBOM）と
ずれた場合、チェックは即時に失敗します。
...
- **Gateway SLO** - `DocsPortal/GatewayRefusals` は alias binding が pin された manifest digest を広告し続けることを保証し、
  エスカレーションは公開時に取得した `verify-sorafs-binding.mjs` の CLI transcript にリンクします。
...
- `scripts/verify-sorafs-binding.mjs` による alias 検証出力。
