...
- `FindAssetById` / `iroha ledger asset list all --verbose` کے ساتھ بیلنس کا معائنہ کریں اور `GET /v1/contracts/state?paths=payer_account,recipient_account,escrow_account_id,escrow_asset_definition,target_amount_value,funded_amount_value,is_open,is_released,is_refunded&decode=json` کے ساتھ معاہدے کی حالت کا معائنہ کریں۔
