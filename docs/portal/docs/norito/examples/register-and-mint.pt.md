...
- Garanta que a conta de destino (por exemplo `alice@wonderland`) exista, espelhando a fase de configuracao em cada quickstart do SDK.
...
- Verifique os saldos via `client.request(FindAccountAssets)` ou `iroha_cli ledger assets list --account alice@wonderland` para confirmar que a cunhagem foi bem-sucedida.
