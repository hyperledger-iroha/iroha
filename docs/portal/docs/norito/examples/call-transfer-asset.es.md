...
- Fondea la autoridad del contrato (por ejemplo `soraカタカナ...`) con el activo que transferirá y otórgale el rol `CanTransfer` o un permiso equivalente.
- Llama al entrypoint `call_transfer_asset` para transferir 5 unidades desde la cuenta del contrato a `soraカタカナ...`, reflejando la forma en que la automatización on-chain puede envolver llamadas del host.
- Verifica los balances mediante `FindAccountAssets` o `iroha_cli ledger assets list --account soraカタカナ...` e inspecciona los eventos para confirmar que la guardia de metadatos registró el contexto de la transferencia.
