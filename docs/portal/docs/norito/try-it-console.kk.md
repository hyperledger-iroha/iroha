...
4. “Байқап көру” тартпасында `application/x-norito` мазмұн түрін таңдап, **Файлды таңдау** түймесін басып, пайдалы жүктемені таңдаңыз. Прокси сұрауды `/proxy/v2/pipeline/submit` түріне қайта жазады және оны `X-TryIt-Client=docs-portal-rapidoc` белгісімен белгілейді.
