...
| Torii のログに `ERR_STRICT_ADDRESS_REQUIRED` を伴う `502/504` が出る。 | Norito IH58/圧縮解析のサポートなしでリクエストが転送された。 | 対象の Torii ビルドに ADDR-5 の変更が含まれているか（`crates/iroha_torii/tests/address_parsing.rs` を参照）と、正しい環境を指しているかを確認します。 |
