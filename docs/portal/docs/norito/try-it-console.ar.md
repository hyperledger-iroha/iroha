...
بالنسبة للمسارات التي تستخدم JSON فقط، يكون sandbox Try it المضمّن غالبا أسرع: أدخل المسار (على سبيل المثال `/v1/accounts/<katakana-i105-account-id>/assets`)، اختر طريقة HTTP، الصق جسم JSON عند الحاجة، واضغط **Send request** لفحص الرؤوس والمدة والحمولات مباشرة.
