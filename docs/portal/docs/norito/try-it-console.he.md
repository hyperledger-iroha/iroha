...
לנתיבים של JSON בלבד ה-sandbox המובנה של Try it לרוב מהיר יותר: הזינו את הנתיב (לדוגמה `/v1/accounts/soraカタカナ.../assets`), בחרו את שיטת ה-HTTP, הדביקו גוף JSON כשצריך, ולחצו **Send request** כדי לבדוק כותרות, משך זמן ומטענים במקום.
