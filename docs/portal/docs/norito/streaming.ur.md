...
- **Codec guardrails.** CABAC غیر فعال رہتا ہے جب تک build اسے enable نہ کرے؛ bundled rANS کے لئے `ENABLE_RANS_BUNDLES=1` لازمی ہے۔ `streaming.codec.{entropy_mode,bundle_width,bundle_accel}` اور optional `streaming.codec.rans_tables_path` کے ذریعے enforce کریں جب custom tables فراہم ہوں۔ bundled `bundle_width` 2 اور 3 کے درمیان (inclusive) ہونا چاہیے؛ width 1 صرف legacy ہے۔
