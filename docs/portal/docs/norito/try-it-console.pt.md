...
Para rotas somente JSON, o sandbox Try it embutido costuma ser mais rapido: informe o caminho (por exemplo, `/v1/accounts/<katakana-i105-account-id>/assets`), selecione o metodo HTTP, cole um corpo JSON quando necessario e clique em **Send request** para inspecionar cabecalhos, duracao e payloads no proprio painel.
