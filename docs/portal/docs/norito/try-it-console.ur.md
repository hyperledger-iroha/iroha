...
صرف JSON روٹس کے لئے ایمبیڈڈ Try it sandbox عموما تیز ہوتا ہے: پاتھ درج کریں (مثلا `/v1/accounts/<katakana-i105-account-id>/assets`)، HTTP طریقہ منتخب کریں، ضرورت ہو تو JSON باڈی پیسٹ کریں، اور **Send request** پر کلک کریں تاکہ ہیڈرز، دورانیہ اور پے لوڈز فوری طور پر دیکھ سکیں۔
