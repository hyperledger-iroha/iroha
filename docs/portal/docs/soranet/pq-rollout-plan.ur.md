...
`iroha::Client` اب parsed phase record کرتا ہے (`crates/iroha/src/client.rs:2315`) تاکہ helper commands (مثال کے طور پر `iroha_cli sorafs fetch`) موجودہ phase کو default anonymity policy کے ساتھ report کر سکیں۔
