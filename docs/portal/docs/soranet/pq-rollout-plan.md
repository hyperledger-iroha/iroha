...
`iroha::Client` now records the parsed phase (`crates/iroha/src/client.rs:2315`) so helper commands (for example `iroha_cli sorafs fetch`) can report the current phase alongside the default anonymity policy.
