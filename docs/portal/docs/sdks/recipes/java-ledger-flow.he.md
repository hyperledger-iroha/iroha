...
---
title: מתכון זרימת לדג'ר ב-Java
description: הניעו את הדמו של רישום → הטבעה → העברה באמצעות ספריית JVM של IrohaAndroid.
slug: /sdks/recipes/java-ledger-flow
---

import SampleDownload from '@site/src/components/SampleDownload';

המתכון הזה מיועד לגרסת desktop/JVM של המודול `java/iroha_android`. הוא משקף את ה-walkthrough של ה-CLI באמצעות רישום הגדרת נכס, הטבעה לחשבון admin, העברה לחשבון שני והדפסת היתרה שנוצרה.

<SampleDownload
  href="/sdk-recipes/java/src/main/java/ledger/LedgerFlow.java"
  filename="src/main/java/ledger/LedgerFlow.java"
  description="הורידו את דוגמת ה-Java המלאה כדי לייבא אותה ל-IDE או לתבנית הפרויקט שלכם."
/>

## 1. רשמו את הגדרת הנכס (CLI)
...
> השתמשו ב-`iroha_cli tools crypto private-key export --raw --private-key <multihash>` אם צריך להסיר את קידומת ה-multihash מהקונפיגורציה ברירת המחדל.
...
בנו עם כלי הבילד המועדף עליכם (`./gradlew :java:iroha_android:assemble && ./gradlew run`).
