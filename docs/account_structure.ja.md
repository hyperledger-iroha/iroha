...
- チェックサム付き、人間向けの **Iroha Base58 アドレス (IH58)** によって生成されます。
  `AccountAddress::to_ih58` チェーン判別式をアカウントにバインドします
...
現在、ウォレットとオフチェーン ツールは生の `alias@domain` ルーティング エイリアスに依存しています。これ
には 2 つの大きな欠点があります。
...
- データ モデルに実装された IH58 Base58 エンベロープと
...
(`AccountAddress`) は、人間向けのいくつかの形式として出力できます。 IH58は
...
`AccountAddress::parse_any` は、IH58 (推奨)、圧縮 (`sora`、2 番目に優れた)、または正規の 16 進数を自動検出します。
(`0x...` のみ。裸の 16 進数は拒否されます) デコードされたペイロードと検出されたペイロードの両方を入力して返します。
`AccountAddressFormat`。鳥居は ISO 20022 補足のために `parse_any` を呼び出します
メタデータが決定性を維持できるように、正規の 16 進形式をアドレス指定して保存します。
元の表現に関係なく。

#### 2.1 ヘッダーバイトレイアウト (ADDR-1a)

すべての正規ペイロードは `header · domain selector · controller` としてレイアウトされます。の
...
#### 2.2 ドメイン セレクター エンコーディング (ADDR-1a)

ドメイン セレクターはヘッダーの直後にあり、タグ付きユニオンです。

|タグ |意味 |ペイロード |メモ |
|-----|----------|----------|----------|
| `0x00` |暗黙的なデフォルト ドメイン |なし |設定された `default_domain_name()` と一致します。 |
| `0x01` |ローカルドメインダイジェスト | 12バイト |ダイジェスト = `blake2s_mac(key = "SORA-LOCAL-K:v1", canonical_label)[0..12]`。 |
| `0x02` |グローバル レジストリ エントリ | 4バイト |ビッグエンディアン `registry_id`;グローバル レジストリが出荷されるまで予約されます。 |

ドメイン ラベルはハッシュ化される前に正規化されます (UTS-46 + STD3 + NFC)。不明なタグでは `AccountAddressError::UnknownDomainTag` が発生します。ドメインに対してアドレスを検証するときに、セレクターが一致しないと `AccountAddressError::DomainMismatch` が発生します。
...
セレクターはコントローラーのペイロードのすぐ隣にあるため、デコーダーはウォークできる
ワイヤ形式を順番に指定します。タグバイトを読み取り、タグ固有のペイロードを読み取り、次に進みます。
コントローラバイトに転送します。

**セレクターの例**

- *暗黙的なデフォルト* (`tag = 0x00`)。ペイロードがありません。デフォルトの標準 16 進数の例
  決定論的テストキーを使用したドメイン:
  `0x02000001203b6a27bcceb6a42d62a3a8d02a6f0d73653215771de243a63ac048a18b59da29`。
- *ローカル ダイジェスト* (`tag = 0x01`)。ペイロードは 12 バイトのダイジェストです。例 (`treasury` シード)
  `0x01`): `0x0201b18fe9c1abbac45b3e38fc5d0001208a88e3dd7409f195fd52db2d3cba5d72ca6709bf1d94121bf3748801b40f6f5c`。
- *グローバル レジストリ* (`tag = 0x02`)。ペイロードはビッグエンディアン `registry_id:u32` です。バイト
  ペイロードに続くものは、暗黙的なデフォルトの場合と同じです。セレクターは単純に
  正規化されたドメイン文字列をレジストリ ポインタに置き換えます。使用例
  `registry_id = 0x0000_002A` (10 進数 42) と決定的なデフォルト コントローラー:
  `0x02020000002a000120641297079357229f295938a4b5a333de35069bf47b9d0704e45805713d13c201`。  
  内訳: `0x02` ヘッダー、`0x02` セレクター タグ、`00 00 00 2A` レジストリ ID、`0x00`
  コントローラー タグ、`0x01` カーブ ID、`0x20` キーの長さ、32 バイトの Ed25519 キー ペイロード。
...
- バイナリ コントローラ ペイロード (`ControllerPayload::Multisig`) はエンコードされます。
  ...
  `AccountAddress::canonical_bytes()` は、IH58 (推奨)/sora (2 番目に優れた) ペイロードに書き込みます。
- ハッシュ (`MultisigPolicy::digest_blake2b256()`) は、Blake2b-256 を使用します。
  `iroha-ms-policy` パーソナライゼーション文字列を使用して、ガバナンス マニフェストを
  IH58 に埋め込まれたコントローラ バイトと一致する決定的なポリシー ID。
- フィクスチャ カバレッジは `fixtures/account/address_vectors.json` にあります (ケース
  `addr-multisig-*`)。ウォレットと SDK は正規の IH58 文字列をアサートする必要があります
  以下で、エンコーダが Rust 実装と一致していることを確認します。

|ケースID |しきい値 / メンバー | IH58 リテラル (接頭辞 `0x02F1`) | Sora 圧縮 (`sora`) リテラル |メモ |
|----------|---------------------|----------------------------|----------------------|----------|
...
- IME/NFKC 変換: 半角のそらかなは、デコードを中断することなく全角形式に正規化できますが、ASCII `sora` センチネルと IH58 の数字/文字は ASCII のままでなければなりません。全角または大文字小文字を折り畳んだセンチネルは `ERR_MISSING_COMPRESSED_SENTINEL` を表示し、全角 ASCII ペイロードは `ERR_INVALID_COMPRESSED_CHAR` を発生させ、チェックサムの不一致は `ERR_CHECKSUM_MISMATCH` として発生します。 `crates/iroha_data_model/src/account/address.rs` のプロパティ テストはこれらのパスをカバーするため、SDK とウォレットは決定的な障害に依存できます。
- `address@domain` エイリアスの Torii および SDK 解析では、IH58 (優先)/sora (2 番目に良い) 入力がエイリアスのフォールバック前に失敗した場合 (例: チェックサムの不一致、ドメイン ダイジェストの不一致)、同じ `ERR_*` コードが出力されるようになりました。そのため、クライアントは散文文字列から推測することなく、構造化された理由を伝えることができます。
- 12 バイト未満のローカル セレクター ペイロードは `ERR_LOCAL8_DEPRECATED` を表示し、従来の Local‑8 ダイジェストからのハード カットオーバーを維持します。
- ドメインレス IH58 (推奨)/sora (2 番目に優れた) リテラルは、ドメイン セレクター リゾルバーを介して埋め込みセレクターを解決します。何もインストールされていない場合 (またはセレクターを解決できない場合)、解析は `ERR_DOMAIN_SELECTOR_UNRESOLVED` で失敗します。暗黙的なデフォルト セレクターは、リゾルバーを必要とせずに、構成されたデフォルトのドメイン ラベルに解決されます。

#### 2.5 規範的なバイナリ ベクトル

- **暗黙的なデフォルト ドメイン (`default`、シード バイト `0x00`)**  
  正規の 16 進数: `0x02000001203b6a27bcceb6a42d62a3a8d02a6f0d73653215771de243a63ac048a18b59da29`。  
...
単体テスト (`account::address::tests::parse_any_accepts_all_formats`) は、`AccountAddress::parse_any` を介して以下の V1 ベクトルをアサートし、ツールが 16 進数、IH58 (推奨)、および圧縮 (`sora`、次善の) 形式にわたる正規のペイロードに依存できることを保証します。 `cargo run -p iroha_data_model --example address_vectors` を使用して拡張フィクスチャ セットを再生成します。

|ドメイン |シードバイト |正規の 16 進数 |圧縮 (`sora`) |
|-----------|-----------|----------------------------------------------------------------------|------------|
|デフォルト | `0x00` | `0x02000001203b6a27bcceb6a42d62a3a8d02a6f0d73653215771de243a63ac048a18b59da29` | `sora2QGﾈkﾀﾍrNﾒBﾎwﾍwﾙwﾗXHwﾜCﾘﾂY8ryGUﾈﾎyQｲHyヰD8ｲﾁYVY9VF8` |
...
したがって、`AccountAddress::to_ih58` ヘルパーと `to_compressed_sora` ヘルパーは次のように出力します。
...
|アカウント/セレクター | IH58 リテラル (接頭辞 `0x02F1`) | Sora 圧縮 (`sora`) リテラル |
|---------------------|--------------------------------|--------------------------|
| `default` ドメイン (暗黙的なセレクター、シード `0x00`) | `RnuaJGGDL8HNkN8bwHwBTU32fTWQmbRoM3QZBJintx5RqTU7GgPJmNiA` | `sora2QGﾈkﾀﾍrNﾒBﾎwﾍwﾙwﾗXHwﾜCﾘﾂY8ryGUﾈﾎyQｲHyヰD8ｲﾁYVY9VF8` (明示的なルーティング ヒントを提供する場合は、オプションの `@default` サフィックス) |
...
応答 (`address_format=ih58|compressed`) と SDK ヘルパー、つまり UX のコピー/ペースト
フローはそれらをそのまま信頼できます。 `<address>@<domain>` は、明示的なルーティング ヒントが必要な場合にのみ追加します。サフィックスは正規の出力の一部ではありません。
...
- **チェーンエイリアスのスタイル:** `ih:<chain-alias>:<alias@domain>` (ログと人間の場合)
  ...
  不一致。
- **CAIP-10 フォーム:** `iroha:<caip-2-id>:<ih58-addr>` (チェーンに依存しない)
  統合。このマッピングは出荷された製品では**まだ実装されていません**
  ツールチェーン。
- **マシン ヘルパー:** Rust、TypeScript/JavaScript、Python、
  IH58 と圧縮形式をカバーする Kotlin (`AccountAddress::to_ih58`、
  `AccountAddress::parse_any`、および同等の SDK)。 CAIP-10 ヘルパーは、
  今後の仕事。

#### 2.7 決定的な IH58 エイリアス

- **プレフィックス マッピング:** `chain_discriminant` を IH58 ネットワーク プレフィックスとして再利用します。
  `encode_ih58_prefix()` (`crates/iroha_data_model/src/account/address.rs` を参照)
...
- **アカウント資料:** IH58 は、によって構築された正規ペイロードをエンコードします。
  `AccountAddress::canonical_bytes()` - ヘッダー バイト、ドメイン セレクター、および
  コントローラーのペイロード。追加のハッシュ手順はありません。 IH58 には、
  Rust によって生成されたバイナリ コントローラ ペイロード (単一キーまたはマルチシグ)
  マルチシグ ポリシー ダイジェストに使用される CTAP2 マップではなく、エンコーダです。
- **エンコーディング:** `encode_ih58()` はプレフィックス バイトを正規文字列と連結します。
  ペイロードを作成し、Blake2b-512 から派生した 16 ビット チェックサムを追加します。
  接頭辞 `IH58PRE` (`b"IH58PRE" || prefix || payload`)。結果は、`bs58` によって Base58 でエンコードされます。
  CLI/SDK ヘルパーは同じプロシージャを公開しており、`AccountAddress::parse_any`
  `decode_ih58` を介してそれを反転します。

#### 2.8 規範的なテキストテストベクトル

`fixtures/account/address_vectors.json` には完全な IH58 (推奨) と圧縮 (`sora`、2 番目に優れた) が含まれています
すべての正規ペイロードのリテラル。ハイライト:

- **`addr-single-default-ed25519` (Sora Nexus、プレフィックス `0x02F1`)。**  
  IH58 `RnuaJGGDL8HNkN8bwHwBTU32fTWQmbRoM3QZBJintx5RqTU7GgPJmNiA`、圧縮 (`sora`)
  `sora2QG…U4N5E5`。鳥居は、`AccountId` からこれらの正確な文字列を出力します。
  `Display` 実装 (標準 IH58) および `AccountAddress::to_compressed_sora`。
- **`addr-global-registry-002a` (レジストリ セレクタ → 財務)。**  
  IH58 `3oE9sLeRGP49Cu7mQ1nF4wtKAm29BG4TGLiRsaXe7mhbMP5WZ113nNW1N6RbqF`、圧縮 (`sora`)
  `sorakX…CM6AEP`。レジストリ セレクターが引き続きデコードされることを示します。
  対応するローカル ダイジェストと同じ正規ペイロード。
- **失敗ケース (`ih58-prefix-mismatch`)。**  
  ノード上でプレフィックス `NETWORK_PREFIX + 1` でエンコードされた IH58 リテラルを解析する
  デフォルトのプレフィックスの結果を期待すると、
  `AccountAddressError::UnexpectedNetworkPrefix { expected: 753, found: 754 }`
  ドメインルーティングが試行される前に。 `ih58-checksum-mismatch` フィクスチャ
  Blake2b チェックサムに対して改ざん検出を実行します。

#### 2.9 コンプライアンス対策

ADDR‑2には、ポジティブとネガティブをカバーする再生可能なフィクスチャバンドルが同梱されています
標準 16 進数、IH58 (推奨)、圧縮 (`sora`、半角/全角)、暗黙的なシナリオ
...
|タイプ |目的 |必須フィールド |
|------|------|------|
| `global_domain` |ドメインがグローバルに登録され、チェーン識別子と IH58 プレフィックスにマップされる必要があることを宣言します。 | `{ "domain": "<label>", "chain": "sora:nexus:global", "ih58_prefix": 753, "selector": "global" }` |
...
`{domain, chain, discriminant/ih58_prefix}`。 `tombstone` レコードは**引用する必要があります**
...
2. **正規ダイジェストを取得します。** 実行します。
   ...
   CLI は、IH58、`sora…`、および正規の `0x…` リテラルを受け入れます。追加する
   `@<domain>` は、マニフェストのラベルを保持する必要がある場合のみ。
   JSON 概要では、`input_domain` フィールドを介してそのドメインが表示されます。
   `--append-domain` は、変換されたエンコーディングを `<address>@<domain>` として再生します。
   マニフェストの差分 (このサフィックスはメタデータであり、正規のアカウント ID ではありません)。
   改行指向のエクスポートの場合は、次を使用します
   `iroha tools address normalize --input <file> --only-local` でローカルを一括変換します
   スキップ中の正規 IH58 (推奨)、圧縮 (`sora`、2 番目に良い)、16 進数、または JSON 形式へのセレクター
...
4. **検証して公開します。** ランブックのチェックリストに従います (ハッシュ、Sigstore、
   ...
   バンドルが到着した直後に、IH58 (推奨)/sora (2 番目に優れた) リテラルを正規化します。
...
- **デフォルトの表示:** ウォレットには IH58 アドレス (短い、チェックサム付き) が表示されます。
  さらに、レジストリからフェッチされたラベルとして解決されたドメインが追加されます。ドメインは
  変更される可能性がある説明的なメタデータとして明確にマークされていますが、IH58 は
  安定したアドレス。
- **入力の正規化:** Torii と SDK は IH58 (推奨)/sora (2 番目に優れた)/0x を受け入れます。
  アドレスに加えて、`alias@domain`、`public_key@domain`、`uaid:…`、および
  `opaque:…` フォームを作成し、出力用に IH58 に正規化します。ありません
  厳密モードの切り替え。生の電話/電子メール識別子は台帳から外しておく必要があります
  UAID/不透明マッピング経由。
- **エラー防止:** ウォレットは IH58 プレフィックスを解析し、チェーン判別を強制します。
  期待。チェーンの不一致によりハード障害が引き起こされ、実用的な診断が行われます。
- **コーデック ライブラリ:** 公式 Rust、TypeScript/JavaScript、Python、Kotlin
  ライブラリは、IH58 エンコーディング/デコーディングと圧縮 (`sora`) のサポートを提供します。
...
- **安全な共有フロー:** アドレスをコピーまたは表示するサーフェスは、デフォルトで IH58 形式になり、完全な文字列と同じペイロードから派生した QR コードの両方を表示する隣接する「共有」アクションを公開するため、ユーザーは目視またはスキャンによってチェックサムを確認できます。切り捨てが避けられない場合 (小さい画面など)、文字列の先頭と末尾を保持し、明確な省略記号を追加し、クリップボードへのコピーを介して完全なアドレスにアクセスできるようにして、誤って切り取られるのを防ぎます。
...
- **スクリーン リーダーのサポート:** 先頭の Base58 プレフィックス数字を説明する視覚的に隠されたラベル (`aria-label`/`aria-describedby`) を提供し、IH58 ペイロードを 4 文字または 8 文字のグループに分割するため、支援技術はランオン文字列の代わりにグループ化された文字を読み取ります。礼儀正しいライブ リージョンを通じてコピー/共有の成功を発表し、QR プレビューに説明的な代替テキスト (「チェーン 0x02F1 の <エイリアス> の IH58 アドレス」) が含まれていることを確認します。
- **Sora のみの圧縮の使用:** `sora…` 圧縮ビューには必ず「Sora のみ」というラベルを付け、コピーする前に明示的な確認を行ってゲートします。 SDKとウォレットは、チェーン判別式がSora Nexus値ではない場合、圧縮された出力の表示を拒否しなければならず、資金の誤ったルーティングを避けるために、ユーザーをネットワーク間送金のためにIH58に戻す必要があります。

## 実装チェックリスト

- **IH58 エンベロープ:** プレフィックスは、コンパクト形式を使用して `chain_discriminant` をエンコードします。
  `encode_ih58_prefix()` の 6/14 ビット スキーム、本文は正規のバイトです
  (`AccountAddress::canonical_bytes()`)、チェックサムは最初の 2 バイトです
  Blake2b-512 (`b"IH58PRE"` || 接頭辞 || 本文) の。完全なペイロードは Base58 です。
  `bs58` 経由でエンコードされます。
- **レジストリ契約:** 署名付き JSON (およびオプションの Merkle ルート) の公開
  `{discriminant, ih58_prefix, chain_alias, endpoints}` 24 時間 TTL と
  回転キー。
- **ドメイン ポリシー:** ASCII `Name` 今日; i18n を有効にする場合は、UTS-46 を適用します。
  正規化と、紛らわしいチェックのための UTS-39。最大ラベル (63) を強制し、
  合計 (255) の長さ。
- **テキストヘルパー:** Rust で IH58 ↔ 圧縮 (`sora…`) コーデックを出荷、
...
- **CLI ツール:** `iroha tools address convert` を介して決定論的なオペレーター ワークフローを提供します。
  (`crates/iroha_cli/src/address.rs` を参照)。IH58/`sora…`/`0x…` リテラルを受け入れます。
  オプションの `<address>@<domain>` ラベル。デフォルトは Sora Nexus プレフィックス (`753`) を使用した IH58 出力です。
  オペレーターが明示的に要求した場合にのみ、Sora のみの圧縮アルファベットを出力します。
  `--format compressed` または JSON サマリー モード。このコマンドは、プレフィックスの期待を強制します。
  解析し、指定されたドメイン (JSON の `input_domain`) と `--append-domain` フラグを記録します
  変換されたエンコーディングを `<address>@<domain>` として再生するため、マニフェストの差分は人間工学に基づいたままになります。
- **ウォレット/エクスプローラー UX:** [アドレス表示ガイドライン](source/sns/address_display_guidelines.md) に従ってください。
  ADDR-6 に同梱 - デュアル コピー ボタンを提供し、IH58 を QR ペイロードとして保持し、警告を表示します
  ユーザーは、圧縮された `sora…` フォームは Sora 専用であり、IME の書き換えの影響を受けやすいことを認識しています。
- **Torii の統合:** TTL を考慮したキャッシュ Nexus マニフェスト、エミット
  `ForeignDomain`/`UnknownDomain`/`RegistryUnavailable` を決定的に、そして
  `alias@domain` を正規化するために `POST /v1/accounts/resolve` を公開し、
  `public_key@domain`、`uaid:`/`opaque:` リテラル、またはエンコードされたアドレス
  IH58 で、解決されたドメインとソースを返します。

### 鳥居応答フォーマット

- `GET /v1/accounts` は、オプションの `address_format` クエリ パラメータを受け入れ、
  `POST /v1/accounts/query` は、JSON エンベロープ内の同じフィールドを受け入れます。
  サポートされている値は次のとおりです。
  - `ih58` (デフォルト) — 応答は正規の IH58 Base58 ペイロードを出力します (例:
    `RnuaJGGDL8HNkN8bwHwBTU32fTWQmbRoM3QZBJintx5RqTU7GgPJmNiA`)。
  - `compressed` — 応答は Sora のみの `sora…` 圧縮ビューを生成します。
    フィルタ/パスパラメータを正規に保ちます。
- 無効な値は `400` (`QueryExecutionFail::Conversion`) を返します。これにより、
  ウォレットとエクスプローラーは、Sora のみの UX の圧縮文字列をリクエストします。
  IH58 を相互運用可能なデフォルトとして維持します。
- アセットホルダーのリスト (`GET /v1/assets/{definition_id}/holders`) とその JSON
  対応する封筒 (`POST …/holders/query`) も `address_format` を尊重します。
  `items[*].account_id` フィールドは、
  パラメータ/エンベロープ フィールドが `compressed` に設定され、アカウントがミラーリングされます
...
| `ERR_INVALID_IH58_PREFIX` |要求された IH58 ネットワーク プレフィックスをエンコードできません。 |チェーン レジストリで公開されている `0..=16383` の範囲内でプレフィックスを選択してください。 |
...
|コード |失敗 |推奨される修復 |
|------|------|----------------------|
| `ERR_INVALID_IH58_ENCODING` | IH58 文字列にアルファベット以外の文字が含まれています。 |アドレスが公開されている IH58 アルファベットを使用しており、コピー/ペースト中に切り捨てられていないことを確認してください。 |
| `ERR_INVALID_LENGTH` |ペイロードの長さが、セレクター/コントローラーの予想される標準サイズと一致しません。 |選択したドメイン セレクターとコントローラー レイアウトの完全な正規ペイロードを指定します。 |
| `ERR_CHECKSUM_MISMATCH` | IH58 (推奨) または圧縮 (`sora`、2 番目に良い) チェックサム検証が失敗しました。 |信頼できるソースからアドレスを再生成します。これは通常、コピー/貼り付けエラーを示します。 |
| `ERR_INVALID_IH58_PREFIX_ENCODING` | IH58 プレフィックス バイトの形式が不正です。 |準拠したエンコーダを使用してアドレスを再エンコードします。先頭の Base58 バイトを手動で変更しないでください。 |
...
| `ERR_UNSUPPORTED_ADDRESS_FORMAT` |自動検出で入力形式を認識できませんでした。 |パーサーを呼び出すときは、IH58 (推奨)、圧縮 (`sora`)、または正規の `0x` 16 進文字列を指定します。 |
...
| `ERR_UNEXPECTED_NETWORK_PREFIX` |デコードされた IH58 ネットワーク プレフィックスが設定値と異なります。 |ターゲット チェーンのアドレスに切り替えるか、予期される識別子/プレフィックスを調整します。 |
...
- **純粋な Base58Check (ビットコイン スタイル)。** チェックサムは単純ですが、エラー検出は弱い
  Blake2b 由来の IH58 チェックサムよりも優れています (`encode_ih58` は 512 ビット ハッシュを切り捨てます)
...
- **Bech32m エンベロープ。** QR フレンドリーで、人間が判読できるプレフィックスを提供しますが、
  出荷時の IH58 実装とは異なる可能性があります (`AccountAddress::to_ih58`)
  すべてのフィクスチャ/SDK を再作成する必要があります。現在のロードマップでは IH58 + が維持されます。
...
- 言霊/IVM コントラクトが IH58 ヘルパーにアクセスする方法を指定します (`to_address()`、
  `parse_address()`)、およびオンチェーン ストレージが CAIP-10 を公開する必要があるかどうか
  マッピング (現在では IH58 が正規です)。
- 外部レジストリ (IH58 レジストリ、
  CAIP 名前空間ディレクトリ）を使用して、より広範なエコシステムの連携を実現します。

## 次のステップ

1. IH58 エンコーディングは `iroha_data_model` (`AccountAddress::to_ih58`,
   `parse_any`);フィクスチャ/テストのすべての SDK への移植を続行し、すべての SDK をパージします。
   Bech32m プレースホルダー。
