...
- Барлық Қосылу бағыттарын (`/v2/connect/*`) өшіру үшін `connect.enabled=false` орнатыңыз.
- WS сеансының соңғы нүктелерін және `/v2/connect/status` қосу үшін оны `true` (әдепкі) қалдырыңыз.
...
- Орындау уақытында өшірілген кезде (`connect.enabled=false`), WS қосылымы және күй бағыттары қосылмайды.
  тіркелген; `/v2/connect/ws` және `/v2/connect/status` сұраулары 404 қайтарады.
- Сервер `/v2/connect/session` (base64url немесе hex, 32 байт) үшін клиент ұсынған `sid` қажет етеді.
