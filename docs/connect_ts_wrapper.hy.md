...
## TypeScript Connect Wrapper (WS միացում + նշաններ)
...
```
// Sealing SignResultOk (միայն ծանրաբեռնվածություն; Norito շրջանակը ցուցադրված չէ):
// const signResult = new TextEncoder().encode(JSON.stringify({ SignResultOk: { signature: {algorithm: 'ed25519', signature_hex: 'deadbeef' } }));
// const {aead } = սպասել sealEnvelope (kWallet, sidBytes, 'W2A', 1n, signResult);