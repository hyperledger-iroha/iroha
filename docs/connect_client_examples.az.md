...
Qeydlər:
- Müştəri `sid` (32 bayt; base64url/hex) hesablayır və birdəfəlik tokenlər əldə etmək üçün onu `/v2/connect/session`-ə POST edir; server `sid` səslənir. `Authorization: Bearer <token>` və ya `Sec-WebSocket-Protocol: iroha-connect.token.v1.<base64url(token)>` ilə WS-ə qoşulun.
