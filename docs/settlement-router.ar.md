...
1. **طبقة موجّه واحدة:** جميع الـ lanes — سواء الـ legacy ذات lane واحد أو الـ lanes
...
1. **وضع Shadow:** تفعيل حسابات الموجّه + الـ telemetry مع إبقاء
   `settlement_debit=false`؛ ثم مقارنة النتائج مع المسار legacy.
...
5. **الوصول إلى GA:** إزالة المسار legacy، تحديث `status.md` وملاحظات الإصدار، وربط
