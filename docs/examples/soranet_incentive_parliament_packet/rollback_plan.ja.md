...
3. **ガバナンス承認の取り消し。** `reward_config.json` を編集し、
   ...
   `iroha sorafs incentives service init` で再デプロイ (長期稼働の daemon の場合は `update-config`)。payout エンジンは `MissingBudgetApprovalId` で fail-closed となり、新しい承認 hash が復元されるまで mint を拒否します。git commit と変更後 config の SHA-256 をインシデントログに記録。
