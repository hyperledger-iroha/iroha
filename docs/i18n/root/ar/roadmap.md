...
- [x] Investigate per-peer commit persistence stalls (Izanami `irohad_test_network_o6jN6V`: swaying_deer never logged `stored committed block to kura` for height 40 while other peers did); added commit->Kura stage breakdown metrics (kura_store/state_apply/state_commit) plus state commit write-lock wait/hold histograms with legacy view-lock aliases to correlate persistence stalls with lock contention.
...
1. **SDK-SWIFT-DEVEX — Clarify bridge dependency and make signing deterministic** (SDK/Swift, Line: Shared, Owner: Swift SDK WG, Priority: Medium, Status: 🈴 Completed, target TBD)
 - [x] NoritoBridge policy: `dist/NoritoBridge.xcframework` is mandatory. `Package.swift` rejects a missing or malformed artifact, and unavailable runtime symbols fail closed with the expected bridge-path hint; there is no Swift-only signing or canonical-encoding fallback.
 - [x] Bridge coverage: Connect codec and transaction-encoder regressions use runtime overrides to assert that `bridgeUnavailable` errors retain the bridge-path hint without enabling a fallback encoder.
 - [x] SwiftPM guardrails: `Package.swift` validates the XCFramework metadata and referenced slice binaries, always registers the binary target, and defines the bridge-required build settings.
 - [x] SPM validation: `scripts/check_swift_spm_validation.sh`, wrapped by `ci/check_swift_spm_validation.sh`, builds with the bridge present and requires package resolution to reject a deliberately missing bridge. No separate workflow is claimed here.【scripts/check_swift_spm_validation.sh:1】【ci/check_swift_spm_validation.sh:1】
 ...
 - [x] CocoaPods validation: the podspec metadata and local lint entrypoints are present; `scripts/check_swift_pod_bridge.sh` rejects a missing XCFramework before running `pod lib lint` when CocoaPods is installed. No nonexistent packaging workflow is cited.【IrohaSwift/IrohaSwift.podspec:1】【scripts/check_swift_pod_bridge.sh:1】【ci/check_swift_pod_bridge.sh:1】
 - [x] Install docs: the README and Swift quickstart document the required bridge, artifact build/materialization flow, and fail-closed bridge-path troubleshooting.
...
### AGENTS Task Breakdown
 ...
 - [x] Wire `scripts/check_no_legacy_codec.sh` into `make agents-preflight`/CI so retired codec dependencies stay out of workspace manifests.
