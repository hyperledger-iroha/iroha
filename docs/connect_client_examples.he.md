<!-- Hebrew translation of docs/connect_client_examples.md -->
...
## דוגמאות לקוח Iroha Connect (TypeScript ו-Kotlin)

המסמך מציג מקטעי קוד מינימליים העומדים בכללי v0:
- ה-AEAD AAD קושר את כותרת המעטפת (גרסה, sid, ‏dir, ‏seq, ‏kind=Ciphertext).
- ה-nonce נגזר מ-`seq` (nonce בפורמט IETF בן 12 בתים: ‎0x00000000 || seq_le).
- מסגרות שליטה לאחר Approve (Close/Reject) נשלחות כשהן מוצפנות.

הקוד מיועד להדגמה; הקשחות ובדיקות ייצור מושמטות.

### TypeScript (libsodium + WebCrypto)

תלויות: ‏`libsodium-wrappers` (X25519, ‏BLAKE2b, ‏ChaCha20-Poly1305), ‏WebCrypto (HKDF-SHA-256).
...
הערות:
- הלקוח מחשב `sid` (‎32 בתים; base64url/הקס) ושולח POST אל `/v2/connect/session` כדי לקבל אסימונים חד-פעמיים. השרת מחזיר את ה-sid, ואל ה-WS מתחברים עם `Authorization: Bearer <token>` או `Sec-WebSocket-Protocol: iroha-connect.token.v1.<base64url(token)>`.
