...
注意事项：
- 客户端计算 `sid`（32 字节；base64url/hex）并将其 POST 到 `/v2/connect/session` 以获取一次性令牌；服务器回显 `sid`。使用 `Authorization: Bearer <token>` 或 `Sec-WebSocket-Protocol: iroha-connect.token.v1.<base64url(token)>` 加入 WS。
