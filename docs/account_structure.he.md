...
`AccountAddress::parse_encoded` מזהה אוטומטית IH58 (מועדף), דחוס (`sora`, השני הכי טוב) או hex קנוני
(`0x...` בלבד; hex חשוף נדחה) מכניס ומחזיר גם את המטען שפוענח וגם את המזוהה
`AccountAddressFormat`. Torii קורא כעת ל-`parse_encoded` עבור המשלים של ISO 20022
...
- **כלי CLI:** ספק זרימת עבודה דטרמיניסטית של מפעיל באמצעות `iroha tools address convert`
  ...
  תוויות `<address>@<domain>` (rejected legacy form) אופציונליות, ברירת המחדל היא פלט IH58 באמצעות הקידומת Sora Nexus (`753`),
