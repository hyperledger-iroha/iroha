<!-- Japanese translation of docs/connect_client_examples.md -->
...
## Iroha Connect クライアント例（TypeScript と Kotlin）

本ドキュメントでは v0 ルールを実装する最小限のクライアント側スニペットを示します。
- AEAD の AAD には外側ヘッダー（バージョン、sid、dir、seq、kind=Ciphertext）をバインドする。
- ノンスは `seq` から導出（12 バイト IETF ノンス: 0x00000000 || seq_le）。
- Approve 後の制御フレーム（Close/Reject）は暗号化して送信する。

以下は参考実装であり、ハードニング／本番向けの検証は省略しています。

### TypeScript（libsodium + WebCrypto）

依存関係: `libsodium-wrappers`（X25519、BLAKE2b、ChaCha20-Poly1305）、WebCrypto（HKDF-SHA-256）
...
備考:
- クライアントが `sid`（32 バイト; base64url/16 進）を計算し、`/v2/connect/session` に POST してワンタイムトークンを取得します。サーバーは `sid` をエコーし、WS は `Authorization: Bearer <token>` または `Sec-WebSocket-Protocol: iroha-connect.token.v1.<base64url(token)>` で接続します。
