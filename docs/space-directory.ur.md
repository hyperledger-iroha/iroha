...
| Publish | Lane ops | CLI (`iroha app space-directory manifest publish`) کے ذریعے publish کریں، Norito `.to` payload یا raw JSON استعمال کرتے ہوئے **یا** Torii پر `/v2/space-directory/manifests` POST کریں، Torii رسپانس verify کریں، اور `SpaceDirectoryEvent` capture کریں۔ | CLI/Torii receipt، event log۔ |
...
| Revoke | گورننس + Lane ops | `iroha app space-directory manifest revoke` (UAID، dataspace، epoch، reason) چلائیں **یا** Torii پر `/v2/space-directory/manifests/revoke` POST کریں، `SpaceDirectoryEvent::ManifestRevoked` verify کریں، اور evidence bundle اپ ڈیٹ کریں۔ | CLI/Torii receipt، event log، ٹکٹ نوٹ۔ |
