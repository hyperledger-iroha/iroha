...
|---------|---------------|---------------------|
| `iroha_slot_duration_ms` p95 עולה מעל 1 000 ms | יצוא Prometheus מ-`/telemetry/metrics` וסנאפשוט `/v1/sumeragi/rbc` כדי לאשר דחיות DA; להשוות מול הארטיפקט האחרון של `ci/acceptance/slot_1s.yml`. | להקטין batch של AMX או להפעיל אגרגטורים נוספים (`sumeragi.npos.k_aggregators`), ואז להריץ מחדש את acceptance harness. |
