...
<!-- docs/connect_ts_wrapper.md (TypeScript Connect Wrapper) کا اردو ترجمہ -->
...
// // keys derive کرنے اور ConnectFrameV1 (Norito‑encoded) بنانے کے بعد:
// ws.send(noritoEncodedBinaryFrame);
```

```ts
// SignResultOk کو seal کرنا (صرف payload؛ Norito framing یہاں دکھایا نہیں گیا):
