...
1. יצירת תבנית:
   ...
   ```
   הפרמטר `--consensus-mode` קובע אילו פרמטרי קונצנזוס Kagami זורע בתוך בלוק ה-`parameters`. השתמשו ב-`permissioned` (ברירת מחדל) או ב-`npos` כדי לזרוע את `Sumeragi::NextMode` ואת המטען `sumeragi_npos_parameters`; במהלך נירמול/חתימה הם מתורגמים להוראות `SetParameter` בבלוק.
...
- **טופולוגיה**: הגדרת כלי-peer ראשוניים דרך `topology`.
...
- התחילו מתבנית עם Kagami:
  - ISI בלבד: `kagami genesis generate --ivm-dir <dir> --genesis-public-key <PK> [--consensus-mode npos] > genesis.json`
