...
<!-- Tradução em português de docs/connect_client_examples.md (Iroha Connect Client Examples) -->

## Exemplos de Cliente Iroha Connect (TypeScript e Kotlin)

Este documento mostra trechos mínimos do lado cliente que implementam as regras
v0:
- O AAD do AEAD vincula o cabeçalho externo (version, `sid`, `dir`, `seq`,
  `kind=Ciphertext`).
- O nonce é derivado de `seq` (nonce IETF de 12 bytes:
  `0x00000000 || seq_le`).
- Frames de controle pós‑Approve (Close/Reject) são enviados cifrados.

São exemplos ilustrativos; validações e endurecimentos de produção foram
omitidos.

### TypeScript (libsodium + WebCrypto)

Dependências: `libsodium-wrappers` (X25519, BLAKE2b, ChaCha20‑Poly1305),
...
(async () => { await sodium.ready;
  ...
  // Criar sessão: POST /v2/connect/session com sid calculado no cliente
