...
Ескертулер:
- Клиент `sid` (32 байт; base64url/hex) есептейді және бір реттік таңбалауыштарды алу үшін оны `/v1/connect/session` мекенжайына POST жібереді; сервер `sid` жаңғырығы. WS жүйесіне `Authorization: Bearer <token>` немесе `Sec-WebSocket-Protocol: iroha-connect.token.v1.<base64url(token)>` арқылы қосылыңыз.
