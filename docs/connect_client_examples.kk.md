...
## Iroha Клиентті қосу мысалдары (TypeScript және Kotlin)

Бұл құжат v0 ережелерін орындайтын ең аз клиенттік үзінділерді көрсетеді:
- AEAD AAD сыртқы тақырыпты байланыстырады (нұсқа, sid, dir, seq, kind=Шифр мәтіні).
- `seq` (12 байт IETF емес: 0x00000000 || seq_le) ішінен алынған емес.
- Бекітуден кейінгі басқару жақтаулары (жабу/қабылдамау) шифрланған түрде жіберілді.

Бұл иллюстрациялық; қатайту/өндірістік тексерулер өткізілмеді.

### TypeScript (libsodium + WebCrypto)

Тәуелділіктер: `libsodium-wrappers` (X25519, BLAKE2b, ChaCha20‑Poly1305), WebCrypto (HKDF‑SHA‑256).
...
Ескертулер:
- Клиент `sid` (32 байт; base64url/hex) есептейді және бір реттік таңбалауыштарды алу үшін оны `/v2/connect/session` мекенжайына POST жібереді; сервер `sid` жаңғырығы. WS жүйесіне `Authorization: Bearer <token>` немесе `Sec-WebSocket-Protocol: iroha-connect.token.v1.<base64url(token)>` арқылы қосылыңыз.
- Кілттер болғаннан кейін (Бекіту), шифрланған пайдалы жүктемелерде Жабу/Қабылдамау жіберіңіз.
- Dedupe кілттері және `seq` қолданба/әмиян жақтаулары үшін бағыт бойынша монотонды болуы керек; `Envelope.seq == frame.seq`. Сервер оқиғалары сервер жағындағы бөлек тізбекті пайдаланады және AEAD/dedupe ішінен шығарылады.