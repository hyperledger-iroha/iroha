...
1. Сгенерировать шаблон:
   ...
   - При выборе `npos` Kagami добавляет payload `sumeragi_npos_parameters`, который задаёт fan-out коллекторов, политику выборов и окна реконфигурации; нормализация/подпись превращает это в инструкции `SetParameter` в подписанном блоке.
...
- **Параметры**: установка начальных значений для Sumeragi (время блока/
  ...
  `Sumeragi::NextMode` и payload `sumeragi_npos_parameters` (выборы, реконфигурация) в блоке
