...
Әрбір канондық пайдалы жүктеме `header · domain selector · controller` ретінде көрсетілген. The
...
#### 2.2 Домен селекторының кодтаулары (ADDR-1a)

Домен селекторы бірден тақырыпты бақылайды және тегтелген одақ болып табылады:

| Тег | Мағынасы | Жүктеме | Ескертпелер |
|-----|---------|---------|-------|
| `0x00` | Жасырын әдепкі домен | ешқайсысы | Конфигурацияланған `default_domain_name()` сәйкес келеді. |
| `0x01` | Жергілікті домен дайджесті | 12 байт | Дайджест = `blake2s_mac(key = "SORA-LOCAL-K:v1", canonical_label)[0..12]`. |
| `0x02` | Ғаламдық тізілім жазбасы | 4 байт | Үлкен-эндиан `registry_id`; жаһандық тізілім жіберілгенге дейін сақталады. |

Домен белгілері хэштеу алдында канонизацияланады (UTS-46 + STD3 + NFC). Белгісіз тегтер `AccountAddressError::UnknownDomainTag` көтереді. Доменге қатысты мекенжайды тексеру кезінде сәйкес келмейтін селекторлар `AccountAddressError::DomainMismatch` деңгейін көтереді.
...
Селектор контроллердің пайдалы жүктемесіне бірден жақын орналасқан, сондықтан декодер жүре алады
сым пішімін ретімен: тег байтты оқыңыз, тегке арналған пайдалы жүктемені оқыңыз, содан кейін жалғастырыңыз
контроллер байттарына.

**Таңдаушы мысалдар**

- *Жасырын әдепкі* (`tag = 0x00`). Пайдалы жүктеме жоқ. Әдепкі үшін канондық он алтылық мысалы
  детерминирленген сынақ кілті арқылы домен:
  `0x02000001203b6a27bcceb6a42d62a3a8d02a6f0d73653215771de243a63ac048a18b59da29`.
- *Жергілікті дайджест* (`tag = 0x01`). Пайдалы жүктеме - 12 байт дайджест. Мысал (`treasury` тұқымы
  `0x01`): `0x0201b18fe9c1abbac45b3e38fc5d0001208a88e3dd7409f195fd52db2d3cba5d72ca6709bf1d94121bf3748801b40f6f5c`.
- *Жаһандық тізілім* (`tag = 0x02`). Пайдалы жүк - бұл үлкен-енді `registry_id:u32`. Байттар
  пайдалы жүктемеден кейінгілер жасырын-әдепкі жағдаймен бірдей; селектор жай
  нормаланған домен жолын тізбе көрсеткішімен ауыстырады. Мысал пайдалану
  `registry_id = 0x0000_002A` (ондық42) және детерминирленген әдепкі контроллер:
  `0x02020000002a000120641297079357229f295938a4b5a333de35069bf47b9d0704e45805713d13c201`.  
  Бөлім: `0x02` тақырыбы, `0x02` селектор тегі, `00 00 00 2A` тізілім идентификаторы, `0x00`
  контроллер тегі, `0x01` қисық идентификаторы, `0x20` кілт ұзындығы, 32 байт Ed25519 кілтінің пайдалы жүктемесі.
...
- Доменсіз IH58 (қалаулы)/sora (екінші ең жақсы) литералдар ендірілген селекторды домен таңдаушы шешушісі арқылы шешеді; ешқайсысы орнатылмаса (немесе селекторды шешу мүмкін болмаса) `ERR_DOMAIN_SELECTOR_UNRESOLVED` көмегімен талдау сәтсіз аяқталады. Жасырын әдепкі таңдағыш шешушіні қажет етпей конфигурацияланған әдепкі домен белгісін шешеді.

#### 2.5 Нормативтік екілік векторлар

- **Жасырын әдепкі домен (`default`, бастапқы байты `0x00`)**  
  Канондық алтылық: `0x02000001203b6a27bcceb6a42d62a3a8d02a6f0d73653215771de243a63ac048a18b59da29`.  
...
|-------------|-----------|-----------------------------------------------------------------------------------------|------------|
| әдепкі | `0x00` | `0x02000001203b6a27bcceb6a42d62a3a8d02a6f0d73653215771de243a63ac048a18b59da29` | `sora2QGﾈkﾀﾍrNﾒBﾎwﾍwﾙwﾗXHwﾜCﾘﾂY8ryGUﾈﾎyQｲHyヰD8ｲﾁYVY9VF8` |
...
|--------------------------------|--------------------------------|-------------------------|
| `default` домені (жасырын селектор, `0x00` тұқымы) | `RnuaJGGDL8HNkN8bwHwBTU32fTWQmbRoM3QZBJintx5RqTU7GgPJmNiA` | `sora2QGﾈkﾀﾍrNﾒBﾎwﾍwﾙwﾗXHwﾜCﾘﾂY8ryGUﾈﾎyQｲHyヰD8ｲﾁYVY9VF8` (айқын бағыттау туралы кеңестер берген кезде қосымша `@default` жұрнағы) |
...
- **`addr-single-default-ed25519` (Sora Nexus, `0x02F1` префиксі).**  
  IH58 `RnuaJGGDL8HNkN8bwHwBTU32fTWQmbRoM3QZBJintx5RqTU7GgPJmNiA`, қысылған (`sora`)
...
2. **Канондық дайджесттерді шығарыңыз.** Жүгіру
   ...
   `--append-domain` түрлендірілген кодтауды `<address>@<domain>` ретінде қайталайды.
   ...
   `iroha tools address normalize --input <file> --only-local` Жергілікті жаппай түрлендіру үшін
...
- **CLI құралдары:** `iroha tools address convert` арқылы детерминирленген оператор жұмыс процесін қамтамасыз етіңіз
  ...
  талдау, берілген доменді (JSON ішіндегі `input_domain`) және `--append-domain` жалауын жазады
...
- **Torii интеграциясы:** Nexus кэші TTL-ге қатысты көрінеді, шығарылады
  ...
  `alias@domain` канонизациясы үшін `POST /v1/accounts/resolve` көрсетіңіз,
  `public_key@domain`, `uaid:`/`opaque:` литералдар немесе кодталған мекенжайлар
  IH58 шешілген домен мен көзді қайтару кезінде.
