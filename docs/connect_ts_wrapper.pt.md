...
<!-- Tradução em português de docs/connect_ts_wrapper.md (TypeScript Connect Wrapper) -->
...
// // Após derivar as chaves e construir um ConnectFrameV1 (codificado em Norito), envie:
// ws.send(noritoEncodedBinaryFrame);
```

```ts
// Selando SignResultOk (apenas o payload; o framing Norito não é mostrado):
