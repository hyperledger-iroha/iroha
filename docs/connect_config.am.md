...
- ሁሉንም የግንኙነት መንገዶችን (`/v2/connect/*`) ለማሰናከል `connect.enabled=false` ያዘጋጁ።
- የ WS ክፍለ ጊዜ የመጨረሻ ነጥቦችን እና `/v2/connect/status` ለማንቃት `true` (ነባሪ) ይተዉት።
...
- በሚሠራበት ጊዜ (`connect.enabled=false`) ሲሰናከል WS አገናኝ እና የሁኔታ መስመሮች አይደሉም
  የተመዘገበ; ወደ `/v2/connect/ws` እና I18NI0000045X መመለሻ 404 ጥያቄዎች.
