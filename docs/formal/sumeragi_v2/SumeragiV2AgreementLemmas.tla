  ...
  <1>1. ASSUME QuorumConfiguration,
    ...
    <2>1. CASE left.view = right.view
      <3>1. SameCertificateSlot(left, right)
        BY <1>1, <2>1 DEF CertificatePhasesCorrect,
                          SameCertificateSlot
      <3>2. /\ left.context.epoch \in Epochs
    ...
    <2>3. CASE right.view < left.view
      ...
      <3> QED BY <3>1, <3>2
    <2>4. \/ left.view = right.view
          \/ left.view < right.view
          \/ right.view < left.view
      BY <1>1, SMT DEF CertificatesBackedByIntents,
                         HistoricalQcValid, Views
    <2> QED BY <2>1, <2>2, <2>3, <2>4, Zenon
  <1> QED BY <1>1
