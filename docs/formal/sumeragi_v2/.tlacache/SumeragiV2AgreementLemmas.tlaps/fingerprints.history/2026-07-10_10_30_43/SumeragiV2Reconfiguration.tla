...
AdvanceContext(subject) ==
  LET nextHeight == height + 1
      nextContext == ContextRecord(nextHeight, subject)
  IN /\ height < MaxHeight
     /\ CommonAppliedSubject(subject)
     /\ height' = nextHeight
     /\ context' = nextContext
...
LivenessSpec ==
  ...
  /\ WF_vars(ReliableNextV2)

ContextIdentityBindsParent ==
  \A blockHeight \in Heights, leftParent, rightParent \in SubjectOrNone:
    ContextRecord(blockHeight, leftParent)
      = ContextRecord(blockHeight, rightParent)
...
THEOREM ContextRecordCarriesFrozenEpoch ==
  \A blockHeight \in Heights, parent \in SubjectOrNone:
    ContextRecord(blockHeight, parent).epoch = ExpectedEpoch(blockHeight)
BY DEF ContextRecord

THEOREM ContextRecordCarriesParent ==
  \A blockHeight \in Heights, parent \in SubjectOrNone:
    ContextRecord(blockHeight, parent).parent = parent
BY DEF ContextRecord
