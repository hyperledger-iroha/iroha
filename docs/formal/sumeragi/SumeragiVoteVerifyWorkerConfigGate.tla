...
(***************************************************************************
A bounded abstract model for vote-signature verification worker configuration.

`resolve_worker_config(...)` turns zero-valued configuration knobs into
runtime-derived defaults before `spawn_vote_verify_workers(...)` allocates
worker channels. This slice pins the deterministic parts of that mapping:
explicit thread and queue-cap values are preserved, auto thread count uses the
observed parallelism fallback, zero work queue caps become `threads * 4` with a
floor of four, and zero result queue caps become `threads * 8` with a floor of
eight.
***************************************************************************)
