...
<!-- Traduction française de docs/connect_ts_wrapper.md (TypeScript Connect Wrapper) -->

## Wrapper TypeScript pour Connect (WS join + tokens)
...
// // Après dérivation des clés et construction d’un ConnectFrameV1 (encodé en Norito), envoyer :
// ws.send(noritoEncodedBinaryFrame);
```

```ts
// Scellage de SignResultOk (payload seule ; framing Norito non montré) :
