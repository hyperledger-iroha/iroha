...
`AccountAddress::parse_any` IH58 (የተመረጠ)፣ የታመቀ (`sora`፣ ሁለተኛ-ምርጥ) ወይም ቀኖናዊ ሄክስን በራስ ሰር ያገኛል
(`0x...` ብቻ፤ ባዶ ሄክስ ውድቅ ነው) ግብአት እና የተፈታውን ክፍያ እና የተገኘውን ሁለቱንም ይመልሳል።
`AccountAddressFormat`. Torii አሁን `parse_any` ለ ISO 20022 ማሟያ ይደውላል
ቀኖናዊውን ሄክስ ቅጽ አድራሻዎችን እና ማከማቻዎችን ያከማቻል ስለዚህ ሜታዳታ የሚወሰን ሆኖ ይቆያል
ዋናው ውክልና ምንም ይሁን ምን.

#### 2.1 የራስጌ ባይት አቀማመጥ (ADDR-1a)

እያንዳንዱ ቀኖናዊ ክፍያ እንደ `header · domain selector · controller` ተቀምጧል። የ
...
#### 2.2 የጎራ መራጭ ኢንኮዲንግ (ADDR-1a)

የጎራ መምረጫው ወዲያውኑ ራስጌውን ይከተላል እና መለያ የተሰጠው ማህበር ነው፡-

| መለያ | ትርጉም | ጭነት | ማስታወሻ |
|-------------|----|------|
| `0x00` | ስውር ነባሪ ጎራ | የለም | ከተዋቀረው `default_domain_name()` ጋር ይዛመዳል። |
| `0x01` | የአካባቢ ጎራ መፍጨት | 12 ባይት | ዳይጀስት = `blake2s_mac(key = "SORA-LOCAL-K:v1", canonical_label)[0..12]`. |
| `0x02` | ዓለም አቀፍ ምዝገባ | 4 ባይት | ቢግ-ኤንዲያን `registry_id`; ዓለም አቀፋዊ የመመዝገቢያ መርከቦች እስኪመጡ ድረስ የተጠበቀ ነው. |

የጎራ መለያዎች ከመጥለፉ በፊት ቀኖናዊ ናቸው (UTS-46 + STD3 + NFC)። ያልታወቁ መለያዎች `AccountAddressError::UnknownDomainTag` ያሳድጋሉ። አድራሻን ከጎራ ጋር ሲያጸድቅ፣ የማይዛመዱ መራጮች `AccountAddressError::DomainMismatch` ያሳድጋሉ።
...
መራጩ ወዲያውኑ ከመቆጣጠሪያው ክፍያ አጠገብ ነው, ስለዚህ ዲኮደር መራመድ ይችላል
የሽቦውን ቅርጸት በቅደም ተከተል: የመለያ ባይት ያንብቡ, መለያ-ተኮር ክፍያን ያንብቡ እና ከዚያ ይቀጥሉ
ወደ መቆጣጠሪያው ባይት.

** የመራጮች ምሳሌዎች ***

- * ስውር ነባሪ * (`tag = 0x00`)። ምንም አይነት ጭነት የለም። ለነባሪ ቀኖናዊ ሄክስ ምሳሌ
  የመወሰኛ ሙከራ ቁልፍን በመጠቀም ጎራ፡-
  `0x02000001203b6a27bcceb6a42d62a3a8d02a6f0d73653215771de243a63ac048a18b59da29`.
- * የአካባቢ መፍጨት * (`tag = 0x01`)። ክፍያ 12-ባይት መፍጨት ነው። ምሳሌ (`treasury` ዘር
  `0x01`): `0x0201b18fe9c1abbac45b3e38fc5d0001208a88e3dd7409f195fd52db2d3cba5d72ca6709bf1d94121bf3748801b40f6f5c`.
- * ዓለም አቀፍ መዝገብ * (`tag = 0x02`)። ክፍያ ትልቅ ኢንዲያን `registry_id:u32` ነው። ባይት
  ክፍያውን የሚከተሉ ከስውር-ነባሪ ጉዳይ ጋር ተመሳሳይ ናቸው; መራጩ በቀላሉ
  የመደበኛውን የጎራ ሕብረቁምፊ በመዝገብ ጠቋሚ ይተካል። ምሳሌ በመጠቀም
  `registry_id = 0x0000_002A` (አስርዮሽ42) እና የሚወስነው ነባሪ መቆጣጠሪያ፡-
  `0x02020000002a000120641297079357229f295938a4b5a333de35069bf47b9d0704e45805713d13c201`.  
  መለያየት፡ `0x02` ራስጌ፣ `0x02` መራጭ መለያ፣ `00 00 00 2A` የመመዝገቢያ መታወቂያ፣ `0x00`
  የመቆጣጠሪያ መለያ፣ `0x01` ከርቭ መታወቂያ፣ `0x20` የቁልፍ ርዝመት፣ 32-ባይት Ed25519 ቁልፍ ጭነት።
...
- Domainless IH58 (ተመራጭ)/ሶራ (ሁለተኛ-ምርጥ) በጥሬው የተከተተውን መራጭ በጎራ መራጭ ፈላጊ በኩል ይፈታል፤ ምንም ካልተጫነ (ወይም መራጩ ሊፈታ ካልቻለ) በ`ERR_DOMAIN_SELECTOR_UNRESOLVED` መተንተን አልተሳካም። ስውር ነባሪ መራጭ ፈላጊ ሳያስፈልገው ወደ የተዋቀረው ነባሪ የጎራ መለያ ይፈታል።

#### 2.5 መደበኛ ሁለትዮሽ ቬክተሮች

- ** ስውር ነባሪ ጎራ (`default`፣ ዘር ባይት `0x00`)**  
  ቀኖናዊ ሄክስ፡ `0x02000001203b6a27bcceb6a42d62a3a8d02a6f0d73653215771de243a63ac048a18b59da29`።  
...
- ** የአካባቢ ጎራ መፍጨት (`treasury`፣ የዘር ባይት `0x01`)**  
  ...
  መለያየት፡ I18NI0000282X ራስጌ፣ መራጭ መለያ I18NI0000283X ሲደመር መፈጨት `b1 8f e9 c1 ab ba c4 5b 3e 38 fc 5d`፣ በመቀጠል ነጠላ-ቁልፍ ክፍያ (`0x00` መለያ፣ `0x01` ከርቭ መታወቂያ፣00102by Ed25519 ቁልፍ)የዩኒት ፈተናዎች (`account::address::tests::parse_any_accepts_all_formats`) ከዚህ በታች ያሉትን የV1 ቬክተሮች በ`AccountAddress::parse_any` በኩል ያረጋግጣሉ፣ ይህም የመሳሪያ ስራ በሄክስ፣ IH58 (ተመራጭ) እና በተጨመቀ (`sora`፣ ሁለተኛ-ምርጥ) ቅጾች ላይ ባለው ቀኖናዊ ጭነት ላይ እንደሚተማመን ዋስትና ይሰጣል። በ`cargo run -p iroha_data_model --example address_vectors` የተራዘመውን የመገጣጠሚያ ስብስብ እንደገና ያድሱ።

| ጎራ | ዘር ባይት | ቀኖናዊ ሄክስ | የታመቀ (`sora`) |
|------------|-------|
| ነባሪ | `0x00` | `0x02000001203b6a27bcceb6a42d62a3a8d02a6f0d73653215771de243a63ac048a18b59da29` | `sora2QGﾈkﾀﾍrNﾒBﾎwﾍwﾙwﾗXHwﾜCﾘﾂY8ryGUﾈﾎyQｲHyヰD8ｲﾁYVY9VF8` |
...
|-------------------|
| `default` ጎራ (ስውር መራጭ፣ ዘር `0x00`) | `RnuaJGGDL8HNkN8bwHwBTU32fTWQmbRoM3QZBJintx5RqTU7GgPJmNiA` | `sora2QGﾈkﾀﾍrNﾒBﾎwﾍwﾙwﾗXHwﾜCﾘﾂY8ryGUﾈﾎyQｲHyヰD8ｲﾁYVY9VF8` (ግልጽ የማዞሪያ ፍንጮች ሲያቀርቡ አማራጭ `@default` ቅጥያ) |
...
- ** የማሽን ረዳቶች፡** ለ Rust፣ TypeScript/JavaScript፣ Python፣ ኮዴኮችን ያትሙ
  ...
  `AccountAddress::parse_any`፣ እና የ SDK አቻዎቻቸው)። CAIP-10 ረዳቶች ናቸው
  ወደፊት ሥራ.
...
** ኢንኮዲንግ:** I18NI0000363X ቅድመ ቅጥያ ባይት ከቀኖናዊው ጋር ያገናኛል
  ...
  የ CLI/SDK ረዳቶች ተመሳሳይ አሰራርን እና `AccountAddress::parse_any` ያጋልጣሉ
...
- **`addr-single-default-ed25519` (ሶራ I18NT0000016X፣ ቅድመ ቅጥያ `0x02F1`)**  
  IH58 `RnuaJGGDL8HNkN8bwHwBTU32fTWQmbRoM3QZBJintx5RqTU7GgPJmNiA`፣ የታመቀ (`sora`)
...
2. ** ቀኖናዊ የምግብ መፈጨትን ያውጡ።** ሩጫ
   ...
   `--append-domain` የተለወጠውን ኮድ እንደ `<address>@<domain>` ለ
   አንጸባራቂ ልዩነቶች (ይህ ቅጥያ ሜታዳታ እንጂ ቀኖናዊ መለያ መታወቂያ አይደለም)።
   ለአዲስ መስመር ተኮር ወደ ውጭ መላክ ይጠቀሙ
   `iroha tools address normalize --input <file> --only-local` ወደ ጅምላ-አከባቢ ለመቀየር
...
- ** የግቤት ቀኖናዊነት፡** Torii እና ኤስዲኬዎች IH58 (የተመረጡ)/ሶራ (ሁለተኛ-ምርጥ)/0x ይቀበላሉ
  አድራሻዎች ሲደመር `alias@domain`፣ `public_key@domain`፣ `uaid:…`፣ እና
...
- ** CLI መሳሪያ:** በ`iroha tools address convert` በኩል የሚወስን ኦፕሬተር የስራ ፍሰት ያቅርቡ
  ...
  መተንተን፣ የቀረበውን ጎራ (`input_domain` በJSON) እና የ`--append-domain` ባንዲራ ይመዘግባል።
...
- ** Torii ውህደት:** መሸጎጫ Nexus TTLን በማክበር ያሳያል
  ...
  `POST /v1/accounts/resolve`ን ወደ `alias@domain` ቀኖናዊ ለማድረግ፣
  `public_key@domain`፣ `uaid:`/`opaque:` ቃል በቃል፣ ወይም በኮድ የተደረጉ አድራሻዎች
  IH58 የተፈታውን ጎራ እና ምንጭ ሲመልስ።
...
1. IH58 ኢንኮዲንግ በ`iroha_data_model` (`AccountAddress::to_ih58`፣
   `parse_any`); እቃዎችን/ሙከራዎችን ወደ እያንዳንዱ ኤስዲኬ ማጓጓዝዎን ይቀጥሉ እና ማንኛውንም ያፅዱ
   Bech32m ቦታ ያዥ።
