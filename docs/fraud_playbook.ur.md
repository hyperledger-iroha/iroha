...
- **Risk intake** – `POST /v2/fraud/query` اسکیمہ `RiskQuery` کو accept
  کرتا ہے:
...
- **Risk decision** – `POST /v2/fraud/assessment`، `FraudAssessment`
...
- **Governance export** – جب `governance` feature enabled ہو، تو
  `GET /v2/fraud/governance/export`، `GovernanceExport` structure واپس
