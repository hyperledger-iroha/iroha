...
## TypeScript ချိတ်ဆက် Wrapper (WS ချိတ်ဆက် + တိုကင်များ)
...
```
// Sealing SignResultOk ( payload only; Norito ဘောင်မပြပါ)
// const signResult = new TextEncoder().encode(JSON.stringify({ SignResultOk: { signature: { algorithm: 'ed25519', signature_hex: 'deadbeef' } } }));
// const { aead } = စောင့်ဆိုင်းနေသော sealEnvelope(kWallet၊ sidBytes၊ 'W2A'၊ 1n၊ signResult);