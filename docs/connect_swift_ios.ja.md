...
- `ConnectClient` が `URLSessionWebSocketTask` 上で `/v2/connect/ws?...` の WebSocket を維持します。
...
// Connect セッションを作成: クライアントが sid を計算し /v2/connect/session に POST
...
備考:
- クライアント側で `sid` を計算し、その `sid` を添えて `/v2/connect/session` に POST するとロールごとのトークンが取得できます。WS 参加時はトークンを使用してください。
