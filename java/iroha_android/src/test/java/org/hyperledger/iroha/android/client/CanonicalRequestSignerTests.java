...
public final class CanonicalRequestSignerTests {
  ...
  private static void headersCarryVerifiableSignature() throws Exception {
    ...
    final URI uri =
        new URI("http://localhost:8080/v1/accounts/soraゴヂアヌャェボヰセキュホュヨモチゥカッパダォレジゴシホセギツキゴヒョヲヌタシャッヱロゥテニョヒシホイヌヘ/assets?limit=5");
    ...
    final Map<String, String> headers =
        CanonicalRequestSigner.buildHeaders(
            "get", uri, body, "soraゴヂアヌャェボヰセキュホュヨモチゥカッパダォレジゴシホセギツキゴヒョヲヌタシャッヱロゥテニョヒシホイヌヘ", keyPair.getPrivate(), timestampMs, nonce);
    final byte[] message =
