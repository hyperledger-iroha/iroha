...
public final class TransferWirePayloadEncoder {
  ...
  private static final class AccountControllerAdapter implements TypeAdapter<AccountController> {
    ...
    private static byte[] canonicalSortKey(AccountAddress.MultisigMemberPayload member) {
      String algo = PublicKeyCodec.algorithmForCurveId(member.curveId());
      byte[] algoBytes = algo != null ? algo.getBytes(java.nio.charset.StandardCharsets.UTF_8)
          : new byte[0];
      byte[] keyBytes = member.publicKey();
      byte[] sortKey = new byte[algoBytes.length + 1 + keyBytes.length];
      System.arraycopy(algoBytes, 0, sortKey, 0, algoBytes.length);
      sortKey[algoBytes.length] = 0;
      System.arraycopy(keyBytes, 0, sortKey, algoBytes.length + 1, keyBytes.length);
      return sortKey;
    }
