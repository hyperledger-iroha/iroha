...
final class NoritoArchiveValidation {
  static final int NATIVE_ARCHIVE_MAX_BYTES = 256 * 1024 * 1024;
  ...
  private NoritoArchiveValidation() {}
  ...
  static void requireNativeInput(final byte[] archive, final String archiveName) {
    ...
    if (!hasNonEmptyNoritoPayload(archive)) {
      throw new IllegalArgumentException(archiveName + " must contain a non-empty Norito payload");
    }
  }
  ...
  static boolean detectNativeAvailability(
    ...
    try {
      loadLibrary.run();
    } catch (final IllegalArgumentException | UnsatisfiedLinkError | SecurityException error) {
      return false;
