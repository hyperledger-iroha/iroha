...
public final class OfflineNoteV2NfcApduProtocol {
  ...
  private OfflineNoteV2NfcApduProtocol() {}
  ...
  public static PayloadInfo decodeInfo(final byte[] data) {
    ...
    if ((data[0] & 0xFF) != PROTOCOL_VERSION) {
      return null;
    }
    ...
    return new PayloadInfo(kind, payloadLength, maxChunkLength, Arrays.copyOfRange(data, 8, 40));
  }
  ...
  private static Command parseWriteMeta(final byte[] data) {
    if (data.length != 38 || (data[0] & 0xFF) != PROTOCOL_VERSION) {
      return Command.invalid();
    }
    final PayloadKind kind = PayloadKind.fromCode(data[1] & 0xFF);
    if (kind == null) {
