...
public final class NoritoBridgeKit {
  ...
  // MARK: - Permissions/Proof (optional JSON helpers via dlsym)
  public func decodeControlOpenPermissionsJson(_ frame: Data) throws -> String {
    ...
    let rc = frame.withUnsafeBytes { fp in fn(fp.bindMemory(to: UInt8.self).baseAddress!, CUnsignedLong(frame.count), &ptr, &len) }
  ...
  public func decodeControlApproveProofJson(_ frame: Data) throws -> String {
    guard let sym = dlsym(RTLD_DEFAULT, "connect_norito_decode_control_approve_proof_json") else { throw NoritoError.ffi(-1) }
    typealias Fn = @convention(c) (UnsafePointer<UInt8>, CUnsignedLong, UnsafeMutablePointer<UnsafeMutablePointer<UInt8>?>, UnsafeMutablePointer<CUnsignedLong>) -> Int32
    let fn = unsafeBitCast(sym, to: Fn.self)
    var ptr: UnsafeMutablePointer<UInt8>? = nil; var len: CUnsignedLong = 0
    let rc = frame.withUnsafeBytes { fp in fn(fp.bindMemory(to: UInt8.self).baseAddress!, CUnsignedLong(frame.count), &ptr, &len) }
