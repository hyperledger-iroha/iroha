...
| --- | --- | --- |
| Taira reset and startup | Release65 built and transferred all four Linux binaries and passed private-descriptor, populated-inventory and frozen-stage process checks, four native validator configuration checks, inventory assembly, seed custody capture and authorization. Release66 fixes OpenSSH startup closing inherited descriptors by keeping pinned inputs in the parent process; real Linux authentication and strict host-key rejection pass. The new early release gate passes all 13 native regressions and the complete CLI suite passes 1,465 tests with zero failures (two ignored). | Release65 rolled back before host mutation at SSH preflight; ingress originals and retained prep62 genesis/keys are preserved. Native preflight now checks all four validators and the edge before runtime custody or journal creation. Release66 Core/prover/model/schema validation passes all 677 selected tests; KAGEMUSHA validation passes 89 regressions, two explicit row benchmarks and the codec guard. The signed Linux build and live rollout remain pending. Public signed canaries, application connectivity, epoch-boundary roster publication, storage growth and reboot qualification remain open. |
