...
| --- | --- | --- |
| Kotodama V1 redesign | Implementation and all 35 redesigned-behavior acceptance rows are complete with scoped passes. Apple6 qualifies current 65021 native provenance with unchanged outputs/pins; workspace build and SoraFS9 pass. SDK, highlighted-docs, editor, onboarding and four-validator results retain their recorded sources. | Broader workspace health remains imperfect: test reports 1,512 passes, three storage-startup failures and 27 ignored cases; Clippy stops at a retained unrelated primitives lint. Later targets are unqualified. Prior failures, index incidents and the unexplained cleanup exit remain recorded; no full-workspace or release-readiness claim. See [evidence and limits](specs/kotodama_v1_redesign.md). |
