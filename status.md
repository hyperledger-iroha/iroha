...
- Sakura Storm v27: added `v26_preview` palette (extracted from the v26 fullwidth katakana preview) and made katabox default to it with a denser v26-like grid, full data disk with a smaller logo hole, and stronger katakana box/text contrast; symbol replay builds the logo mask for `SS_KATAKANA_OUTSIDE_LOGO` even when overlay is off, symbol extraction now respects `SS_GRID_OFFSET` for centered dense grids, and katabox can now pull the exact v26 logo mask from a reference GIF (`SS_LOGO_MASK_REF`).
