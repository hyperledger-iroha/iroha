...
| --- | --- | --- |
| Taira reset and startup | Corrected numeric systemd manager exit classification and validator running-state detection. Start and restart now wait within the original deadline for the exact signed Python launcher to execute the selected daemon, then require full process attestation. All 33 focused native regressions pass in 177 seconds. Obsolete deployment copies were removed; nginx rotation runs every five minutes with four compressed archives. | Public Taira remains unavailable. Release69 never reached validator installation/start and its incomplete journal is retained. Release70 compiled successfully but was rejected after a concurrent source merge; nothing was transferred or deployed. Release71 uses the merged optimizations source and corrected startup handling. Public finality, signed canaries and application connectivity remain unverified. |
