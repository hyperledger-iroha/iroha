...
| Atomic private settlement | Canonical PublicKey JSON accepts bare multihash only; five focused codec tests and one compile-fail doctest pass. The development Core/model/configuration run passes 284 tests; three omitted AccountId cases pass separately on the same model executable after correcting their module filters, and nine further signature/privacy codec cases pass. Pinned TLC output ordering is corrected with 11 report-tool tests and authentic positive/mutant log validation. | Shared-checkout source changed during the combined build; its original incomplete test-inventory assessment remains retained. These development outcomes do not qualify unchanged current-source native/IVM/PQ full proofs, ten fresh 16-process N=3 successes, faults, leakage, benchmarks, independent audit or release evidence. See [the protocol](specs/private_settlement.md). |
...
| Nested encoding work | Destination-owned counting propagates through packed fields, collections and embedded instruction frames without repeated child serialization. All 1,642 codec/derive/primitives tests pass (three ignored cases), and strict library Clippy passes for those three packages. Exact wire bytes, CRC/layout drift, output bounds and one-visit counting regressions pass. The unchanged SDK Musubi boundary test passes in 98 seconds on the default stack. | All 706 SDK tests passed before scratch removal; the model run had 3,463 passes and one stale roster-error expectation. That test is corrected and all unused generated scratch buffers are removed; complete model/SDK reruns and Core/Torii caller checks remain ongoing. These local timings do not qualify pinned-runner memory or release performance budgets. |
