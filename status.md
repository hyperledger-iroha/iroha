...
| --- | --- | --- |
| Taira reset and startup | Startup reconciliation passes both full regressions; earlier CLI reset/Connect evidence covers 178 tests. The generator removes public fixture privileges, uses non-signing gas custody and independent private mint seeds. Four native Linux62 validator configuration checks now pass with one signed genesis. Storage admission uses separate temporary/shared limits, and the generated signed NPoS election ceiling matches the generated committee. All 31 focused Taira storage tests and both committee generation tests pass. | Inventory assembly exposed descriptor pseudo-paths being rejected by strict file loading. Explicit retained configuration/token descriptors replace these paths; 187 focused CLI tests pass, including native descriptor custody, secret-safe diagnostics and identical-argument recovery. The rebuilt release remains pending. The four-validator reset, public signed canary and application connectivity are not complete. Next-epoch roster publication must be verified at a real boundary; full storage-growth and host reboot qualification remain open. |
