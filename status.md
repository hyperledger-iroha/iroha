...
- Sumeragi: `block_created_rebuilds_qc_with_snapshot_roster` now treats a BlockCreated as accepted if it either caches the pending block or immediately forms/caches a commit QC (with a fallback to the commit-QC history), keeping the snapshot-roster assertion intact. Tests: `CARGO_TARGET_DIR=target_tmp cargo test -p iroha_core sumeragi::main_loop::tests::block_created_rebuilds_qc_with_snapshot_roster -- --nocapture` (ok; command timed out after the filtered binaries continued running).
