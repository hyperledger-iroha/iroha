...
| Taira reset and startup | Release73 separates preseed and verification deadlines from state reset, batches Linux SoraFS persistence, and keeps validator config rebasing inside native inherited-FD handling with 0600 artifacts. Release preparation resumes completed work and reuses the warm build cache. Focused checks pass: 51 native, 17 SoraFS and 35 Python tests; coordinated Core, recovery and API gates also pass. | Linux release73 is ready to build and deploy. Release72 exhausted its preseed deadline and rolled back before validator startup or consensus. Public Taira remains unavailable; public finality, signed canaries and application connectivity remain unverified. |
| KAGEMUSHA release work | R5 passed 1,490 full JVM tests, 12 wallet and 108 client managed Android tests; separate host JNI, 13 Kotlin and 24 JS/TypeScript tests also pass. Vendor memory checks pass 138 executions; Base passes 14 tests and two map benchmarks. Core passes 92 tests plus two row benchmarks; journal/recovery passes 54 tests. The real-proof API check passes after a profiling-helper cfg fix. Row-generation RSS falls from 189.6 to 28.3 MB with identical output. | Full aggregate proofs/resource limits, canonical Swift/native release artifacts, governed physical profiles and independent review remain unqualified. Focused host benchmarks do not qualify full-proof/device RSS or strict Clippy. Swift parity edits are preserved for reapplication after the clean native checkpoint. See [current readiness](specs/kagemusha_v1_production_readiness.md). |
...
| SoraFS owner candidate (Y) | All 284 concrete owners declare captured identities; 17 borrowed wire records and 12 field helpers have payload-only boundaries. The final candidate passes 1,271 test executions across the full SoraFS library, fixture/CLI integration, cryptography and minimal-feature controls on 19,699 identical inputs. Governance is below the production/test limits and its exception is removed; formatting and codec checks pass. | Candidate scope only. Source-size checking retains 239 findings/172 exceptions. Full strict Clippy stops on eight unchanged crypto errors; package-only strict Clippy retains six SoraFS lint sites. Remaining manual identities, atomic codec selection, physical extraction and release qualification remain open. |
| Crypto lint closure | Eight crypto lint findings are repaired without changing signature or V1 digest semantics. The application/BLS library passes strict scoped Clippy; all 32 selected cryptography/parser tests pass on 2,219 unchanged inputs without a stack override. The multihash regression rejects every truncated prefix and odd/even trailing bytes. | This is a separate follow-up to candidate Y; its full feature/workspace and native qualification remain open. SoraFS retains its six package-only lint sites. |
