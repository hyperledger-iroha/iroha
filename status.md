...
- Tiered state: added diff payload snapshots and a background snapshot worker to avoid synchronous JSON hashing/IO in `StateBlock::commit`, gated payload-based spilling for entries without reusable cold data, and updated tiered snapshot docs across locales. Tests: `cargo fmt --all` (warns about nightly-only rustfmt options). `cargo test -p iroha_core record_world_snapshot_with_payload_updates_touched_entries -- --nocapture` (timed out twice waiting on the build directory lock).
