...
| --- | --- | --- |
| Taira reset and startup | Both full startup reconciliation regressions pass on the retained Core60 harness, including repeated startup and rejection before partial mutation. The CLI passes 178 reset/Connect regressions. A Linux-only namespace move error is corrected; namespace deduplication now occurs only after successful evidence validation, with six focused helper regressions passing. The canonical Taira storage profile reserves 4 GiB per validator so the actual guest image fits SoraFS; three configuration regressions pass. | The previous Linux release build failed before producing all four artifacts. A new signed Linux release build, live four-validator reset, public canary and application connectivity remain pending. Full storage-growth and host reboot qualification remain open. |
