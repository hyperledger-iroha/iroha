...
| --- | --- | --- |
| Taira reset and startup | Release66 built and transferred all four signed Linux binaries, passed private-descriptor, populated-inventory and frozen-stage process checks, four native validator configuration checks, inventory assembly and seed-custody capture. SSH authentication now reaches the fixed remote dispatcher. The CLI suite passed 1,465 tests, and Core/prover/model/schema passed all 677 selected tests; KAGEMUSHA passed 89 regressions, two explicit row benchmarks and the codec guard. Release67 corrects the KVM query to pass a null ioctl operand while preserving its integer result and errno; all 14 early CLI regressions pass. | Release66 stopped before ingress or apply: the previous variadic KVM query omitted its required zero argument. The real guest returns API12 for zero and EINVAL for nonzero. No release66 journal or validator startup occurred; retained prep62 genesis/keys and ingress originals remain intact. Release67 signed build and live rollout remain pending. Public signed canaries, application connectivity, epoch-boundary roster publication, storage growth and reboot qualification remain open. |
