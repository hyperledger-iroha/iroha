...
- The codecs roundtrip through the public Norito audit decoder, reject token
  ids that do not match the embedded audit bundle, support the Bearer Cash
  payment text prefix, and produce Fountain QR frames tagged as
...
- Torii MCP no longer advertises legacy offline transfer/revocation tool names;
  first-release clients use the mounted Offline Note issuer routes and Bearer
  Cash SDK text payloads directly.
