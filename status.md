...
- Norito: added gpuzstd_metal helper cdylib (Metal prepass + CPU zstd fallback) and documented build steps; TODO recorded for full GPU match-finding/entropy coding.
...
- Sumeragi: roster-validation cached path now defers empty-signature (genesis-stub) commit QCs/checkpoints to full validation so structural checks are not bypassed.
- Tests: `cargo test -p iroha_core validate_commit_qc_roster_cached_memoizes -- --nocapture` (failed: unclosed delimiter/merge conflict markers in `crates/iroha_core/src/smartcontracts/isi/staking.rs`).
- Tests: `cargo test -p iroha_core validate_checkpoint_roster_cached_memoizes -- --nocapture` (not run; blocked by the same merge conflict).
- Izanami 1 TPS rerun after roster-validation memoization: not run (build blocked by the same merge conflict).
- Formatting: `cargo fmt --all` (stable rustfmt warns about nightly-only options).
