...
| --- | --- | --- |
| Taira reset and startup | Release63 built all four Linux binaries from signed source; source/toolchain pins remained unchanged. All 187 focused CLI tests and four actual Linux descriptor checks pass. The retained private genesis passes all four native63 validator checks; inventory assembly and metadata-only seed custody capture succeed. | Authorization exposed typed Inrou placement accounts being decoded before entering Taira's fixed network context. One fixed-context inventory decoder now serves assembly, authorization, signed admission, host requests and all receipt readers; canonical assembly verifies a roundtrip before publishing. All 180 focused reset tests pass, including populated-placement decoding from the default network context, strict identity rejection and retained-byte file admission. Release64 remains pending. No authorization has been issued or validator started for the replacement network. Public signed canary, application connectivity, next-epoch roster publication at a real boundary, storage-growth and host reboot qualification remain open. |
