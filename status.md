...
| Atomic private settlement | Combined correction selection passes 281 tests (Core224/model56/configuration1); scoped shared API passes 14. All80 named authority/native/PQ/IVM prerequisites execute. Raw SHA256 preservation and successive-settlement regressions pass; an earlier captured binary passes its native-owner full proof. | Concurrent model/schema and SDK changes prevent unchanged-source qualification. Three current full proofs, fresh 16-process N=3 settlements, faults, leakage, benchmarks, independent audit and release evidence remain open. See [the protocol](specs/private_settlement.md). |
...
| Norito migration capture | Eight probe-generator, 17 driver and 32 syntax/context-graph tests pass. Controlled crypto/model captures pass 206/6,112 probes. Another 108 crypto declarations pass nine module suites and seven existing wire/identity golden tests. The reviewed 1,771-declaration model batch preserves every pre-capture identity and its 110 owner-scoped fixture tests pass on the sealed post-declaration harness. Both `RegistrableBuilder` callers now declare distinct child identities; eight generator tests, three compile-fail cases and 23 owner tests pass. | Generic, local, remaining generated families and other feature selections remain incomplete. A wider sealed model run excluding the recursive-wire test completed 3,045 passes with 37 existing failures, so the complete model suite is not qualified. Shipping codecs are unchanged. |
