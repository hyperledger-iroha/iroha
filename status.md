...
- IVM/Kotodama: align CoreHost/WsvHost numeric syscall behavior (signed + fractional), return NoritoBytes balances in WsvHost, allow negative NumericFromInt literals for access-hint encoding, and refresh numeric syscall + Kotodama literal tests. Tests: not run (not requested).
- Kotodama: manifest trigger descriptors are now registered on contract instance activation and removed on deactivation, runtime injects contract context metadata into triggers, and deactivation emits trigger deletion events; added manifest-trigger regression coverage. Tests: `cargo test -p iroha_core --test contract_manifest_triggers -- --nocapture` (failed: `ivm` compile error, unexpected closing delimiter in `crates/ivm/src/mock_wsv.rs:5006`).
- IVM/Kotodama: GET_ACCOUNT_BALANCE now returns `&NoritoBytes(Numeric)` (syscall docs/tests updated), numeric syscalls accept signed mantissas, Kotodama supports decimal literals, and trigger args parse fractional JSON numbers. Tests: not run (not requested).
- IVM/Kotodama: fix numeric negation to use `BigInt::neg`, wire kotoba entries into contract manifests, and handle decimal literals in codegen support checks. Tests: `cargo test -p ivm --test kotodama manifest_emits_wildcard_hints_when_isi_targets_are_opaque -- --nocapture` (pass; norito dead_code warnings).
- Kotodama access hints: replace skip-reason flow with wildcard hints for dynamic state paths and opaque ISI/host calls, wire diagnostics counters, update tests/docs (including Android instructions). Format: `cargo fmt --all` (stable rustfmt warns about nightly-only config). Tests: `cargo test -p ivm manifest_emits_wildcard_hints_when_isi_targets_are_opaque -- --nocapture` (failed: existing `ivm` compile errors in `crates/ivm/src/core_host.rs`, `crates/ivm/src/host.rs`, and `crates/ivm/src/mock_wsv.rs` around `BigInt` negation + `decode_numeric_u128_ptr`).
- Kotodama access hints: revert skip-reason flow back to wildcard hints for opaque ISI/dynamic state paths; compiler/tests updated. Tests: `cargo test -p ivm --no-run` (failed: borrow error in `crates/ivm/tests/wsv_host_register_account_asset_tlv.rs`).
- IVM/Kotodama: opaque host access now emits conservative wildcard access hints (`*`) instead of skipping; lint/CLI diagnostics updated; docs refreshed; added compiler coverage for fallback hints. Tests: `CARGO_TARGET_DIR=target/codex-iroha cargo test -p ivm --lib manifest_access_set_hints_fallback_for_opaque_host_calls -- --nocapture` (pass); `CARGO_TARGET_DIR=target/codex-iroha cargo test -p ivm manifest_access_set_hints_fallback_for_opaque_host_calls -- --nocapture` (failed: existing borrow errors in `crates/ivm/tests/wsv_host_role_admin_tlv.rs`).
- IVM/Kotodama: numeric syscalls now operate on full Norito `Numeric` (mantissa+scale) with signed mantissas; asset amounts are passed as `NoritoBytes(Numeric)` pointers; ABI goldens/tests and syscalls/Kotodama docs updated; added numeric syscall unit coverage. Tests: not run (per request).
- Sumeragi NEW_VIEW recovery: defer proposals when the highest QC block is missing locally, seed missing-block fetches from precommit/commit QC history (fallback to latest commit QC roster), and update proposal assembly/new-view tests plus logging terminology. Tests: not run (not requested).
- Kotodama/IVM: access-hint skipping now drops wildcard keys and records skip reasons, ISI hint fallback tracks per-function completeness, `get_or_default` coercion avoids borrow conflicts, mock WSV numeric pointer decoding is typed, and Torii evidence validation tests pin mode tags for valid BLS signatures. Tests: `CARGO_TARGET_DIR=target/codex-iroha-torii cargo test -p iroha_torii routing::evidence_submit_tests::decode_and_validate_evidence_accepts_valid_payload -- --nocapture` (pass).
- Kotodama: numeric aliases now use full `Numeric` semantics (signed mantissa + scale) via Numeric syscalls, map-key comparisons lower to numeric compare, and access hints now include literal numeric map keys; added IR/runtime tests plus cast coverage. Tests: not run (not requested).
- NPoS penalties: re-enable deterministic VRF jailing and delayed consensus slashing (`slashing_delay_blocks`, default 259200), add `CancelConsensusEvidencePenalty` governance cancellation with `penalty_cancelled_at_height`, validate NPoS reconfig fields on-chain, and update docs/runbooks/default genesis configs. Format: `cargo fmt --all` (stable rustfmt warns about unstable options). Tests: `cargo test --workspace` (failed: `crates/ivm/src/core_host.rs` and `crates/ivm/src/host.rs` apply unary `-` to `BigInt` without `Neg`).
- Kotodama access hints: skip hints when dynamic state paths or opaque ISI prevent concrete access inference, avoid wildcard emission, and report skip reasons via entrypoint descriptors; updated related tests. Tests: `cargo test -p ivm --no-run` (failed: existing ivm mock_wsv compile errors around `Numeric`/`BigInt` trait bounds).
- Sumeragi: permissioned leader/collector selection now uses PRF-based ordering (no previous-block-hash rotation); fallback collectors use the contiguous tail slice when PRF seed is missing; rotation/collector integration tests and permissioned topology PRF unit coverage updated. Tests: `cargo test -p iroha_core permissioned_topology_for_view_applies_prf_shuffle_and_rotation -- --nocapture` (failed: `ivm` compile errors in `crates/ivm/src/kotodama/compiler.rs`, E0308).
...
- Sumeragi NEW_VIEW: require NEW_VIEW height == highest_qc.height + 1 for prepare/commit, advance active round height from the highest QC, add prepare-highest NEW_VIEW vote/QC coverage, and update `docs/source/sumeragi.md`, `docs/source/global_feature_matrix.md`, `docs/source/global_feature_matrix.ja.md`, `docs/source/global_feature_matrix.he.md`, and `docs/source/sumeragi_npos_task_breakdown.md` wording. Tests: `cargo test -p iroha_core prepare_highest -- --nocapture`, `cargo test -p iroha_core handle_qc_accepts_new_view_prepare_highest_next_height -- --nocapture` (pass).
- Sumeragi NEW_VIEW: accept Prepare QCs for highest certificate selection/validation so view changes preserve locks and avoid liveness deadlocks; updated NEW_VIEW QC aggregation helper test and refreshed Sumeragi/telemetry/evidence/operator docs to describe highest/locked QC semantics. Tests: `cargo test -p iroha_core select_new_view_highest_qc_accepts_prepare -- --nocapture` (pass; workspace tests not run per request).
- Roadmap: add DA audit tasks for Torii DA refactor, rs16 dedupe, SIMD acceleration, and chunking latency metric. Tests: not run (docs-only change).
- Sumeragi/RBC: respond to missing-block fetches with `RbcInit` rebuilt from the block payload when DA is enabled, so peers missing INIT can process READY/DELIVER; unit coverage added in `fetch_pending_block_sends_rbc_init_when_da_enabled`. Tests: not run (not requested).
- Sumeragi/RBC: accept stale-view RBC messages when the only local payload is from an aborted pending block, add unit coverage, and update Sumeragi DA/RBC docs. Tests: not run (cargo tests already running).
- Test network: default NPoS genesis now bootstraps stake (clamped to `min_self_bond`), enables Nexus in the bootstrap config layer, and writes `telemetry_enabled=true`; added unit coverage for default bootstrap + min-self-bond clamp. Sumeragi: expose RBC allocation helper to tests with doc. Tests: `cargo test -p iroha_core canonicalize_order_sorts_without_resetting_view -- --nocapture` timed out waiting on the build directory lock; other tests not run.
...
- Integration tests: `CARGO_TARGET_DIR=/tmp/iroha-codex-roadmap-target IROHA_TEST_NETWORK_PARALLELISM=1 IROHA_TEST_NETWORK_PERMIT_DIR=$(mktemp -d) IROHA_TEST_NETWORK_KEEP_DIRS=1 cargo test -p integration_tests --test sumeragi_localnet_smoke -- --nocapture --test-threads=1` (failed: `permissioned_localnet_reaches_100_blocks` stalled at height 5/3/target 101 within 230s; logs in `/var/folders/7l/w31n0ppj4zg874c4szhllss00000gn/T/irohad_test_network_bpZ07y`).
- Sumeragi: exempt NEW_VIEW vote/cert messages from future view-window gating to improve view-change convergence; unit coverage added. Tests: `cargo test -p iroha_core future_consensus_view_window_allows_new_view_messages -- --nocapture` (timed out waiting on build directory lock; other cargo tests running).
...
- Format: `cargo fmt --all` (stable toolchain warns about unstable fmt options).
- Tests: `cargo test --workspace` (failed: `crates/iroha_torii/src/da/ingest.rs` duplicate `LaneConfig` import and config/model `LaneConfig` mismatches, plus `crates/iroha_torii/src/da/commitments.rs` missing `LaneConfig::from_catalog` and type mismatches in DA proof verification).
