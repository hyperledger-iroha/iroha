...
| --- | --- | --- |
| Taira reset and startup | Startup completion has an immutable observation separate from one-use reconciliation authority. The CLI admits the signed genesis directly and rejects conflicting manifest/file sources; all 178 reset and Connect regressions pass. The GNU/Linux Zig adapter passes 12 tests and the public Host routing correction passes 19. Actual Linux validator units and both guest and Mac nginx candidates pass configuration checks. | The full restart fixture reached and passed repeated reconciliation and terminal replay, then exposed a stale-owner fixture ordering error. Its corrected final run, Linux release binaries, live four-validator consensus, public transactions and application rollout remain pending. |
