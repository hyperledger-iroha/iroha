...
Norito and source-closure checks pass; the complete Rust workspace and runtime Core/network
matrix remain unqualified. Swift execution still requires the ABI-23 bridge.
...
| Taira reset and startup | Release73 built and transferred, then rolled back on preseed OS I/O error 5. A four-store reproduction passed in 1,725.87 seconds and exposed repeated full metadata/index writes. The corrected verifier uses one lifecycle-leased stream; Linux/Android file guards use platform flags. Public onboarding binds signed intent and deadline without reset custody. Sixty-seven distinct native regressions, 51 JVM contract tests with the rebuilt ABI-23 bridge and five descriptor-policy tests pass; the actual retained genesis passes current native signature/wire/roster validation. | The original I/O error remains unreproduced. Linux compilation exposed Cargo reusing a host build script from an older frozen source; source-bound fingerprint admission now reproduces and fixes that cache failure. The runner passes 50 tests and 15 subtests, including a real Cargo stale-build reproduction and interrupted-checkpoint recovery. Corrected Linux build/rollout, four-validator finality, signed canaries and test.inori.co.il connectivity remain pending. Public Taira is unavailable; no release GO is claimed. |
