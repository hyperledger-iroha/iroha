...
"#;
    ...
    fn explorer_account_record_decodes_payload() {
        let value = norito::json!({
            "id": "alice@test",
            "ih58_address": "ih58:sora:alice@test",
            "compressed_address": "sora:alice@test",
            "network_prefix": 42,
            "metadata": { "role": "admin" },
            "owned_domains": 2,
            "owned_assets": 5,
        ...
        assert_eq!(record.id, "alice@test");
        assert_eq!(record.ih58_address, "ih58:sora:alice@test");
        assert_eq!(record.compressed_address, "sora:alice@test");
    ...
    fn explorer_accounts_page_decodes_entries() {
        ...
        assert_eq!(page.items[0].id, "alice@test");
    }
    ...
    fn explorer_domain_record_decodes_payload() {
        ...
        assert_eq!(record.owned_by, "alice@test");
    ...
    fn explorer_domains_page_validates_entries() {
        let value = norito::json!({
            ...
            "items":[{ "id":"sora","owned_by":"alice@test","accounts":1,"assets":0,"nfts":0 }]
        });
    ...
    fn explorer_asset_definition_page_validates_entries() {
        let value = norito::json!({
            ...
            "items":[
                {"id":"usd#sora","mintable":"Infinitely","owned_by":"alice@test","assets":2}
            ]
        });
    ...
    fn explorer_assets_page_validates_entries() {
        let value = norito::json!({
            "pagination":{"page":1,"per_page":10,"total_pages":1,"total_items":1},
            "items":[
                {"id":"usd#sora@alice@test","definition_id":"usd#sora","account_id":"alice@test","value":"10"}
            ]
        });
    ...
    fn explorer_nfts_page_validates_entries() {
        let value = norito::json!({
            ...
            "items":[{"id":"art#gallery","owned_by":"alice@test","metadata":{}}]
        });
        let page = ExplorerNftsPage::from_json(&value).expect("page");
        assert_eq!(page.items.len(), 1);
        assert_eq!(page.items[0].owned_by, "alice@test");
    }
    ...
    async fn fetch_explorer_domains_page_applies_filters() {
        ...
        let mock = server.mock(|when, then| {
            when.method(GET)
                ...
                .query_param("owned_by", "alice@test");
            then.status(200)
    ...
    async fn fetch_explorer_asset_definitions_page_applies_filters() {
        ...
        let body = norito::json!({
            ...
            "items":[{"id":"usd#sora","mintable":"Infinitely","owned_by":"alice@test","assets":7}]
        });
        let mock = server.mock(|when, then| {
            when.method(GET)
    ...
    async fn fetch_explorer_assets_page_applies_filters() {
        ...
        let body = norito::json!({
            ...
            "items":[{"id":"usd#sora@alice@test","definition_id":"usd#sora","account_id":"alice@test","value":"10"}]
        });
        let mock = server.mock(|when, then| {
            when.method(GET)
        ...
        mock.assert();
        assert_eq!(page.items.len(), 1);
        assert_eq!(page.items[0].account_id, "alice@test");
    }
    ...
    async fn list_triggers_parses_results() {
        ...
        let payload = norito::json!({
            "items": [
                {
                    "id": "daily-airdrop",
                    "action": { "Mint": { "asset_id": "usd#sora", "account_id": "alice@sora", "value": "5" } },
                    "metadata": { "cron": "0 0 * * *" }
                }
            ],
            "total": 7
        ...
        let mock = server.mock(|when, then| {
            when.method(GET)
                ...
                .query_param("authority", "alice@test")
                .query_param("limit", "5")
        ...
        let page = client
            .list_triggers(TriggerListQuery {
                ...
                authority: Some("alice@test".into()),
                limit: Some(5),
                offset: Some(10),
            })
            .await
    ...
    async fn get_trigger_supports_missing() {
        ...
        let body = norito::json!({
            "id": "mint-hook",
            "action": { "Register": { "Account": "alice@test" } },
            "metadata": {}
        });
    ...
    async fn register_trigger_posts_json() {
        ...
        let request = norito::json!({
            "id": "hook",
            "action": { "Mint": { "asset_id": "usd#sora", "account_id": "alice@sora", "value": "42" } },
            "metadata": { "note": "demo" }
        });
