...
_RECOVERED_SUCCESSOR_STATUS_MUTATIONS = (
    ('frame_genesis_context', 'self.artifact.height == 1', 'true'),
    ('frame_genesis_context', 'self.artifact.height_context == *verified', 'true'),
    ('frame_genesis_context', 'verified.height == 1', 'true'),
    ('frame_genesis_context', 'verified.parent_commit_qc.is_none()', 'true'),
    ('frame_genesis_context', 'verified.snapshot_bootstrap.is_none()', 'true'),
    ('frame_genesis_context', 'BlockSignaturePolicy::GenesisAuthority(_)', 'BlockSignaturePolicy::RotatingLeader'),
    ('frame_genesis_context', 'context.height() == 1', 'true'),
    ('frame_genesis_context', 'context.id().as_bytes() == self.artifact.context_id().0.as_ref()', 'true'),
    ('frame_policy_receipt', 'self.authenticates_genesis_lifecycle_context(context)', 'true'),
    ('frame_policy_receipt', 'self.artifact.height > 1', 'self.artifact.height >= 1'),
    ('frame_policy_receipt', '&self.artifact, &self.receipt', '&self.artifact, &foreign_receipt'),
    ('frame_policy_receipt', 'predecessor == self.activation.predecessor()', 'true'),
    ('frame_policy_receipt', 'verified.height == self.artifact.height', 'true'),
    ('frame_policy_receipt', 'context.height() == self.artifact.height', 'true'),
    ('frame_policy_receipt', 'context.id().as_bytes() == self.artifact.context_id().0.as_ref()', 'true'),
    ('frame_mint', 'opened != *expected', 'false'),
    ('frame_mint', 'if !present', 'if false'),
    ('frame_mint', 'store_path: self.path.clone(),', 'store_path: foreign_path,'),
    ('frame_mint', 'store_directory_identity: self.directory.identity,', 'store_directory_identity: foreign_directory,'),
    ('frame_mint', 'Hash::new(frame)', 'Hash::new(foreign_frame)'),
    ('frame_digest', 'ledger.context() == self.context', 'true'),
    ('frame_digest', 'self.max_frame_bytes', 'u64::MAX'),
    ('frame_digest', 'LifecycleDigest::new(Hash::new(frame).into()) == self.ledger_frame_hash', 'true'),
    ('frame_target', 'complete_tip.authorizes_predecessor_lifecycle_root(root)', 'true'),
    ('frame_target', 'self.store_path == root.join(LEDGER_FILE)', 'true'),
    ('frame_target', 'self.directory_identity_still_exact(root)', 'true'),
    ('frame_target', 'self.binds_ledger(ledger)', 'true'),
    ('frame_target', 'complete_tip.authorizes_retired_lifecycle(ledger.context())', 'true'),
    ('frame_directory', 'bind_lifecycle_directory_path(root, false)', 'bind_lifecycle_directory_path(root, true)'),
    ('frame_directory', '== self.store_directory_identity', '== foreign_directory'),
    ('frame_directory', 'let _ = root;\n            false', 'let _ = root;\n            true'),
    ('frame_rejoin', 'self.store_path == store.path', 'true'),
    ('frame_rejoin', 'self.store_directory_identity == store.directory.identity', 'true'),
    ('frame_rejoin', 'self.context != store.context', 'false'),
    ('frame_rejoin', 'self.max_records != store.max_records', 'false'),
    ('frame_rejoin', 'self.max_frame_bytes != store.max_frame_bytes', 'false'),
    ('frame_rejoin', '!self.binds_ledger(ledger)', 'false'),
    ('frame_rejoin', 'present && opened == *ledger', 'opened == *ledger'),
    ('frame_rejoin', 'present && opened == *ledger', 'present'),
    ('frame_stage', 'self.high_water() == 0', 'true'),
    ('frame_stage', 'self.records.is_empty()', 'true'),
    ('frame_stage', 'self.producer_debts.is_empty()', 'true'),
    ('frame_stage', 'complete_tip.authenticates_genesis_lifecycle_context(self.context())', 'true'),
    ('frame_stage', 'self.validate(MAX_LIFECYCLE_RECORDS_PER_HEIGHT)?;', ''),
    ('frame_stage', 'present_frame.authorizes_canonical_retired_predecessor(self, complete_tip)', 'true'),
    ('frame_stage', 'CompleteTipPredecessorLifecycleEvidenceV1::CanonicalFrame(present_frame)', 'CompleteTipPredecessorLifecycleEvidenceV1::EmptyGenesis'),
    ('frame_stage', 'candidates.next().is_some()', 'false'),
    ('frame_stage', 'staged.authenticate_complete_tip_terminal_apply(complete_tip)?', 'foreign_ordinal'),
    ('frame_evidence', 'ledger.high_water() == 0', 'true'),
    ('frame_evidence', 'ledger.records().is_empty()', 'true'),
    ('frame_evidence', 'ledger.producer_debts.is_empty()', 'true'),
    ('frame_evidence', 'present.authorizes_canonical_retired_predecessor(ledger, complete_tip)', 'true'),
    ('frame_evidence', 'present.exactly_matches(store, ledger)', 'true'),
    ('frame_retirement_census', '!self.exactly_matches(store, current, complete_tip)', 'false'),
    ('frame_retirement_census', 'current.context() != retired.context()', 'false'),
    ('frame_retirement_census', 'current.high_water() != retired.high_water()', 'false'),
    ('frame_retirement_census', 'current.records().len() != retired.records().len()', 'false'),
    ('frame_retirement_census', '!retired.producer_debts.is_empty()', 'false'),
    ('frame_retirement_census', 'record.terminal() == Some(None)', 'false'),
    ('frame_retirement_publish', 'if !self.is_exact()?', 'if false'),
    ('frame_retirement_publish', 'retire_authenticated_cut(serve_payloads, &retained_serve_payloads)', 'retire_unchecked_cut(serve_payloads, &retained_serve_payloads)'),
    ('frame_retirement_publish', 'stage_finalized_height_all_row_retirement(serve_reconciliation)', 'stage_partial_retirement(serve_reconciliation)'),
    ('frame_retirement_publish', 'if !retained_predecessor_is_exact', 'if false'),
    ('frame_retirement_publish', 'persist_exact_successor(&terminal.ledger, &retired)', 'persist_exact_successor(&terminal.ledger, &terminal.ledger)'),
    ('frame_retirement_publish', 'terminal.ledger_store.load()? != retired', 'false'),
    ('frame_retirement_publish', 'open_initialized_or_descendant(retired.high_water())', 'open_initialized_or_descendant(0)'),
