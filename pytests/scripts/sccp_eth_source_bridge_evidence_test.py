...
class HostileEthSourceString(str):
    def __new__(cls, value):
        return str.__new__(cls, value)

    def strip(self, *args, **kwargs):
        raise AssertionError("secret-token ETH source string was stripped")

    def startswith(self, *args, **kwargs):
        raise AssertionError("secret-token ETH source string prefix was checked")

    def isascii(self):
        raise AssertionError("secret-token ETH source string ascii was checked")

    def isdecimal(self):
        raise AssertionError("secret-token ETH source string decimal was checked")

    def __len__(self):
        raise AssertionError("secret-token ETH source string length was read")

    def __iter__(self):
        raise AssertionError("secret-token ETH source string was iterated")

    def __str__(self):
        raise AssertionError("secret-token ETH source string was stringified")

    def __repr__(self):
        raise AssertionError("secret-token ETH source string was repr'd")

    def __eq__(self, _other):
        raise AssertionError("secret-token ETH source string was compared")
...
def test_eth_source_exact_string_parsers_reject_subclasses_without_hooks():
    module = load_evidence_module()
    hostile_hex = HostileEthSourceString("0x" + "11" * 32)
    hostile_address = HostileEthSourceString("0x" + "22" * 20)
    hostile_runtime = HostileEthSourceString("0x6080604052")
    hostile_decimal = HostileEthSourceString("4660")
