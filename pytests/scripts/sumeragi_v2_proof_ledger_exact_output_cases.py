...
def _worker_ownership_mutations_survive_digest_refresh(tmp_path: Path) -> None:
    ...
    mutations = (
        ...
        ("durable_history_source_covers", ".read_certified_lane_block_artifact_read_only(*lane_id, *lane_block_height)\n                .map_err(|error| format!(\"durable lane certificate source is unreadable: {error}\"))?", ".read_certified_lane_block_artifact(*lane_id, *lane_block_height)", "error-preserving read-only lookup"),
        ("durable_history_source_covers", ".map_err(|error| format!(\"durable lane certificate source is unreadable: {error}\"))?", ".ok().flatten()", "error-preserving read-only lookup"),
        ("durable_history_source_covers", "read_certified_lane_block_artifact_read_only(*lane_id, *lane_block_height)", "read_certified_lane_block_artifact_read_only(*lane_id, *proposal_height)", "exact certified Kura source through"),
        ("durable_history_source_covers", "if *proposal_height > maximum_source_height {", "if false {", "exact certified Kura source through"),
        ("durable_history_source_covers", "|| certificate.prepare_qc != source.prepare_qc", "|| false", "exact certified Kura source through"),
        ("durable_history_source_covers", "|| certificate.commit_qc != source.commit_qc", "|| false", "exact certified Kura source through"),
...
def _ingress_effects_mutations_survive_digest_refresh(tmp_path: Path) -> None:
    ...
    mutations = (
        ...
        ("predecessor_remains_exact", "if !attestation.mode().retains_apply_suffix()", "if false", "restricted to the attested same-batch or delayed periodic Apply suffix"),
...
def _ordinary_ingress_consumer_mutations_survive_digest_refresh(tmp_path: Path) -> None:
    ...
    mutations = (
        ...
        ("first lane dispatch precedes handoff completion", "dispatch_lane_work_effects(lane_work, services, lane_output_limit)?;",
         "", "lane ingress failure and shared recovery"),
        ("lane dispatch propagates failure", "dispatch_lane_work_effects(lane_work, services, lane_output_limit)?;",
         "let _ = dispatch_lane_work_effects(lane_work, services, lane_output_limit);", "lane ingress failure and shared recovery"),
        ("lane dispatch uses the runner budget", "dispatch_lane_work_effects(lane_work, services, lane_output_limit)?;",
         "dispatch_lane_work_effects(lane_work, services, usize::MAX)?;", "lane ingress failure and shared recovery"),
        ("lane dispatch follows ingress authentication", "            let _ = lane_work.accept_lane_message_with_ingress_ownership(",
         "            dispatch_lane_work_effects(lane_work, services, lane_output_limit)?;\n            let _ = lane_work.accept_lane_message_with_ingress_ownership(", "lane ingress failure and shared recovery"),
...
def test_merge_execution_validation_cache_semantics_survive_digest_refresh(tmp_path: Path) -> None:
    ...
    mutations = (
        (
            ...
            "*view == active_view && !pending.is_ready(&wake)",
            "false",
...
def test_merge_frontier_deferral_semantics_survive_digest_refresh(tmp_path: Path) -> None:
    ...
    mutations = (
        ...
        ('validate_merge_candidate_for_active_round', 'if self.classify_merge_state_validation(active_view, validation)?', 'if Ok::<_, MergeCandidateValidationError>(MergeCandidateValidation::Ready)?', 'stable live validation error'),
        ...
        ('defer_candidate_snapshot', 'CANDIDATE_WORK_RECHECK', 'NON_EMPTY_CANDIDATE_WORK_RECHECK', 'producer uses bounded recheck'),
        ('schedule_local_proposal', 'CandidateAssemblyOutcome::WorkDeferred { report, reason } => {\n                proposal_state.defer_candidate_snapshot(owner, Instant::now());', 'CandidateAssemblyOutcome::WorkDeferred { report, reason } => {\n                proposal_state.defer_candidate_work(owner, Instant::now());', 'producer returns before admitting'),
    ...
    for index, (name, old, new, diagnostic) in enumerate(mutations):
        ...
        try:
            ...
            assert any(diagnostic in error and "exact reviewed token digest" not in error
                       for error in errors), (index, name, old, errors)
            assert not any("exact reviewed token digest" in error for error in errors), errors
        finally:
