...
import resource_publisher_fixture as integration
...
def actual(tmp_path_factory):
    published = integration.RawPublishedRuns(tmp_path_factory.mktemp('reconcile') / 'bundle')
    results = tuple(published.replay_run(index) for index in range(10))
...
def test_reports_cannot_relabel_weaken_or_replace_the_actual_replay(actual, case):
    ...
    elif case == 'wrong_replay_count': result = replace(result, resources=result.resources._replace(samples=result.resources.samples[:-1]))
    elif case == 'wrong_replay_schedule':
        rows = (result.resources.samples[0]._replace(scheduled_offset_ns=1), *result.resources.samples[1:])
        result = replace(result, resources=result.resources._replace(samples=rows))
