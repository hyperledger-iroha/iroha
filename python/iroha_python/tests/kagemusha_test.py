...
RECURSIVE_SPEND_TOPUP_METHOD = "kagemusha_recursive_spend_topup"
...
def test_kagemusha_recursive_redeem_transaction_helper_derives_instruction_before_signing(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    def fake_redeem_instruction(request: object) -> object:
        request_bytes = bytes(request)  # type: ignore[arg-type]
        calls.append(request_bytes)
        return kagemusha.kagemusha_instruction_archive_instruction(
            kagemusha.KAGEMUSHA_INSTRUCTION_ARCHIVE_TYPE_REDEEM_RECURSIVE,
    ...
    assert len(draft) == 1
    assert calls == [request_archive, request_archive, request_archive]


def test_kagemusha_recursive_redeem_rejects_synthetic_shared_abi7_fixture() -> None:
    request_archive = _shared_recursive_spend_abi7_archive("redeem_request")
    with pytest.raises(
        ValueError,
        match=(
            r"invalid Kagemusha recursive spend redeem request: record-backed recursive "
            r"Kagemusha lineage accumulator does not match redeem bundle"
        ),
    ):
        kagemusha.kagemusha_recursive_redeem_instruction(request_archive)
...
def test_kagemusha_recursive_topup_transaction_helper_derives_topup_before_signing(
    ...
    keypair = _kagemusha_test_keypair()
    authority = keypair.default_account_id("wonderland")
    envelope = kagemusha.build_kagemusha_recursive_topup_transaction(
        "chain",
        authority,
        keypair.private_key,
        ...
        ttl_ms=10_000,
        nonce=5,
        metadata={"kagemusha": "recursive-topup"},
    )
    assert envelope.chain_id == "chain"
    assert envelope.authority == authority
    assert bytes(envelope.signed_transaction)
...
def test_kagemusha_instruction_transaction_helpers_copy_mutable_archives_before_building(
    ...
    top_up_calls: list[bytes] = []
    recursive_redeem_calls: list[bytes] = []

    def fake_topup(request_archive: object) -> bytes:
        top_up_calls.append(bytes(request_archive))  # type: ignore[arg-type]
        return bytes(top_up_instruction_archive)

    def fake_redeem_instruction(request_archive: object) -> object:
        recursive_redeem_calls.append(bytes(request_archive))  # type: ignore[arg-type]
        return kagemusha.kagemusha_instruction_archive_instruction(
            kagemusha.KAGEMUSHA_INSTRUCTION_ARCHIVE_TYPE_REDEEM_RECURSIVE,
    ...
    top_up_envelope = kagemusha.build_kagemusha_recursive_topup_transaction(
        "chain",
        authority,
        memoryview(private_key),
        memoryview(topup_request_archive),
    ...
    expected_top_up_envelope = kagemusha.build_kagemusha_instruction_transaction(
        "chain",
        authority,
        expected_private_key,
        kagemusha.KAGEMUSHA_INSTRUCTION_ARCHIVE_TYPE_TOPUP_RECURSIVE,
    ...
    assert _instruction_archive_bytes(top_up_instruction) == _instruction_archive_bytes(
        expected_top_up_instruction
    )
    ...
    assert recursive_redeem_calls == [expected_request_archive] * 4
...
def test_kagemusha_instruction_transaction_helpers_reject_padded_chain_and_authority_before_signing() -> None:
    ...
    with pytest.raises(ValueError, match="chain_id must not contain surrounding whitespace"):
        kagemusha.build_kagemusha_recursive_topup_transaction(
            " chain",
            authority,
            keypair.private_key,
...
def test_kagemusha_instruction_archive_transaction_helpers_reject_adversarial_inputs() -> None:
    ...
    with pytest.raises(ValueError, match="request_archive must be a valid Norito archive"):
        kagemusha.kagemusha_recursive_spend_topup(b"\x00")
    ...
    bad_init_flags = bytearray(_shared_recursive_spend_archive("init_request"))
    bad_init_flags[39] = 0x20
    with pytest.raises(ValueError, match="request_archive must be a valid Norito archive"):
        kagemusha.kagemusha_recursive_spend_topup(bad_init_flags)
...
class _Native:
    ...
    def kagemusha_recursive_spend_topup(self, request: bytes) -> bytes:
        self._reject_probe("topup", request)
        self.calls.append(("topup", request))
        return _kagemusha_norito_frame_with_payload(0x3C)
...
def test_recursive_kagemusha_helpers_reject_empty_requests(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for helper in (
        ...
        kagemusha.kagemusha_recursive_spend_topup,
...
def test_recursive_kagemusha_helpers_probe_and_delegate(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert not hasattr(kagemusha, "KAGEMUSHA_OFFLINE_SPEND_MODE_RECURSIVE_COMPACT_V1")
    assert not hasattr(kagemusha, "KAGEMUSHA_OFFLINE_SPEND_MODE_RECURSIVE_V2")
    ...
    assert kagemusha.is_kagemusha_spend_again_mode("RECURSIVE_SPEND_V1") is False
    assert kagemusha.is_kagemusha_spend_again_mode(None) is False
    assert (
        kagemusha.preferred_kagemusha_offline_spend_mode(
            pasta_cycle_v3_backend_available=True,
        )
...
def test_recursive_kagemusha_shared_abi6_fixture_matches_sdk_surface() -> None:
    ...
    assert "reserved_lineage" not in circuit_ids
    assert (
    ...
    assert not kagemusha.can_append_kagemusha_recursive_spend_witnessless_lineage(63)
...
def test_recursive_kagemusha_typed_request_codecs_round_trip_shared_fixtures() -> None:
    ...
    assert semantic_init_request.lineage_proving_key_archive is None
    _assert_kagemusha_archive_schema(
        kagemusha.encode_kagemusha_recursive_spend_init_request(semantic_init_request),
        kagemusha.KAGEMUSHA_RECURSIVE_SPEND_INIT_REQUEST_WIRE_NAME,
    )
    ...
    exact_redeem_payload = _kagemusha_archive_payload(
        kagemusha.encode_kagemusha_recursive_spend_redeem_request(
            kagemusha.KagemushaRecursiveSpendRedeemRequest(
                ...
                lineage_witness=_shared_recursive_spend_archive(
                    "lineage_witness_append_result"
                ),
                lineage_verifier_record=verifier_record,
            )
    ...
    assert exact_redeem_fields[4][0] == 1
...
def test_recursive_kagemusha_typed_request_codecs_reject_malformed_inputs() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="lineage_proving_key_archive is required when lineage_verifier_key is present",
    ):
        kagemusha.KagemushaRecursiveSpendInitRequest(
            record_bundle=record_bundle,
            pallas_open_envelopes=pallas,
    ...
    for _metadata_field, metadata_kwargs, _expected in malformed_pallas_metadata_payloads:
        with pytest.raises(
            ValueError,
            match="output_proof_circuit_id is not valid for the previous bundle",
        ):
    ...
    for sequence_kwargs, _expected, _previous_expected in malformed_pallas_sequence_payloads:
        with pytest.raises(
            ValueError,
            match="output_proof_circuit_id is not valid for the previous bundle",
        ):
...
def test_recursive_kagemusha_key_artifact_helpers_are_package_root_exports() -> None:
    ...
    from iroha_python import (
        ...
        as root_is_recursive_spend_topup_available,
        is_kagemusha_pallas_open_envelope_builder_available
        as root_is_pallas_open_envelope_builder_available,
        kagemusha_recursive_spend_topup as root_recursive_spend_topup,
        kagemusha_recursive_spend_topup_typed as root_recursive_spend_topup_typed,
        kagemusha_recursive_topup_instruction as root_recursive_topup_instruction,
        build_kagemusha_recursive_topup_transaction
        as root_build_recursive_topup_transaction,
    ...
    assert "is_kagemusha_recursive_spend_topup_available" in iroha_python.__all__
    assert "kagemusha_recursive_spend_topup" in iroha_python.__all__
    assert "kagemusha_recursive_spend_topup_typed" in iroha_python.__all__
    assert "kagemusha_recursive_topup_instruction" in iroha_python.__all__
    assert "build_kagemusha_recursive_topup_transaction" in iroha_python.__all__
    assert (
    ...
    assert (
        root_is_recursive_spend_topup_available
        is kagemusha.is_kagemusha_recursive_spend_topup_available
    )
    assert root_recursive_spend_topup is kagemusha.kagemusha_recursive_spend_topup
    assert (
        root_recursive_spend_topup_typed
        is kagemusha.kagemusha_recursive_spend_topup_typed
    )
    assert (
        root_recursive_topup_instruction
        is kagemusha.kagemusha_recursive_topup_instruction
    )
    assert (
        root_build_recursive_topup_transaction
        is kagemusha.build_kagemusha_recursive_topup_transaction
    )
    assert (
...
def test_recursive_kagemusha_exports_stable_circuit_ids() -> None:
    ...
    assert not kagemusha.KAGEMUSHA_RECURSIVE_SPEND_LINEAGE_TRANSITION_CIRCUIT_WIRED_V1
    assert (
        kagemusha.KAGEMUSHA_RECURSIVE_PREVIOUS_PROOF_OPEN_ENVELOPES_REQUIRED_COUNT_V1
        == 1
    )
    ...
    assert kagemusha.KAGEMUSHA_NATIVE_ARCHIVE_MAX_BYTES == 256 * 1024 * 1024
    ...
    assert not kagemusha.is_kagemusha_recursive_spend_lineage_proof_circuit_id(
        "kagemusha-recursive-spend-lineage-v1",
    )
    ...
    assert not kagemusha.is_supported_kagemusha_recursive_spend_previous_proof_circuit_id(
        "kagemusha-recursive-spend-lineage-v1",
    )
    assert kagemusha.is_supported_kagemusha_recursive_spend_previous_proof_circuit_id(
        kagemusha.KAGEMUSHA_RECURSIVE_SPEND_LINEAGE_ONE_HOP_PROOF_CIRCUIT_ID_V1,
    ...
    for circuit_id, hop_count in (
        ...
        ("kagemusha-recursive-spend-lineage-v1", 0),
        ("kagemusha-recursive-spend-lineage-v1", -1),
        ("kagemusha-recursive-spend-lineage-v1", 65),
        ("kagemusha-recursive-spend-lineage-v1", 2**63),
        ("kagemusha-recursive-spend-lineage-v1", 1.5),
        ("kagemusha-recursive-spend-lineage-v1", float("nan")),
        ("kagemusha-recursive-spend-lineage-v1", float("inf")),
        ("kagemusha-recursive-spend-lineage-v1", float("-inf")),
        ("kagemusha-recursive-spend-lineage-v1", True),
        ("kagemusha-recursive-spend-lineage-v1", "1"),
        (None, 1),
        ("", 1),
    ...
    for circuit_id in (
        ...
        for hop_count in (1, 2, 63, 64):
            ...
            assert kagemusha.requires_kagemusha_recursive_spend_lineage_witness_for_redeem(
                circuit_id,
                hop_count,
            ), f"{circuit_id} hop {hop_count} must require a record-backed lineage witness"
    ...
    for circuit_id, previous_hop_count in (
        ...
        ("kagemusha-recursive-spend-lineage-v1", 64),
        ("unknown-kagemusha-recursive-spend-circuit", 1),
...
def test_recursive_kagemusha_topup_availability_requires_abi_15_and_symbol(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    native = _Native()
    ...
    missing = _Native()
    setattr(missing, RECURSIVE_SPEND_TOPUP_METHOD, None)
    monkeypatch.setattr(kagemusha, "load_crypto_extension", lambda: missing)
    assert kagemusha.is_kagemusha_recursive_spend_available() is True
    assert kagemusha.is_kagemusha_recursive_spend_topup_available() is False
    with pytest.raises(RuntimeError, match=RECURSIVE_SPEND_TOPUP_METHOD):
        kagemusha.kagemusha_recursive_spend_topup(_kagemusha_input_archive(0x75))

    native = _Native()
    monkeypatch.setattr(kagemusha, "load_crypto_extension", lambda: native)
    request = _kagemusha_input_archive(0x76)
    assert kagemusha.is_kagemusha_recursive_spend_topup_available() is True
    assert kagemusha.kagemusha_recursive_spend_topup(request).startswith(b"NRT0")
    assert native.calls[-1] == ("topup", request)
...
def test_recursive_kagemusha_helpers_reject_empty_native_outputs(
    ...
    native.kagemusha_recursive_spend_topup = empty_one
