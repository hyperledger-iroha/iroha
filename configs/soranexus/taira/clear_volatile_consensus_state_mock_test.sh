...
run_apply_quarantines_only_volatile_state_case() {
  ...
  grep -q "IROHAD_BIN=${root}/bin/iroha3d LOG_LEVEL=warn" "${dist}/start.invocation"
}
...
ps() {
  if [[ "${1:-}" == "-p" && "${2:-}" == "${TAIRA_TEST_PID:-}" && "${3:-}" == "-o" && "${4:-}" == "command=" ]]; then
    printf '/tmp/iroha3d --config %s/peer0.toml.bak\n' "${TAIRA_TEST_DIST:?}"
    return 0
  fi
  command ps "$@"
}
...
ps() {
  if [[ "${1:-}" == "-axo" ]]; then
    printf '%s /tmp/iroha3d --config %s/peer0.toml.bak\n' "${TAIRA_TEST_FAKE_PID:?}" "${TAIRA_TEST_DIST:?}"
    return 0
  fi
  command ps "$@"
}
...
run_apply_ignores_retired_irohad_alias_ps_scan_case() {
  local root dist output bash_env
  root="$(make_case_root)"
  cleanup_paths+=("$root")
  dist="${root}/dist"
  output="${root}/retired-irohad-alias-ps-scan.log"
  bash_env="${root}/bash-env.sh"
  rm -f "${dist}/peer0.pid"
  cat >"$bash_env" <<'SH'
kill() {
  if [[ "${1:-}" == "${TAIRA_TEST_FAKE_PID:-}" || "${2:-}" == "${TAIRA_TEST_FAKE_PID:-}" ]]; then
    echo "unexpected kill of retired irohad alias ps row" >&2
    return 42
  fi
  command kill "$@"
}
ps() {
  if [[ "${1:-}" == "-axo" ]]; then
    printf '%s /tmp/irohad --config=%s/peer0.toml\n' "${TAIRA_TEST_FAKE_PID:?}" "${TAIRA_TEST_DIST:?}"
    return 0
  fi
  command ps "$@"
}
SH
  ...
  ! grep -q 'unexpected kill of retired irohad alias ps row' "$output"
  grep -q 'no running peer processes matched' "$output"
  grep -q 'volatile consensus quarantine completed.' "$output"
  test ! -e "${dist}/storage/peer0/queue_plan_journal"
  test ! -d "${dist}/storage/peer0/rbc_sessions"
...
ps() {
  if [[ "${1:-}" == "-axo" ]]; then
    printf '%s /tmp/iroha3d --config=%s/peer2.toml\n' "${TAIRA_TEST_FAKE_PID:?}" "${TAIRA_TEST_DIST:?}"
    return 0
  fi
...
ps() {
  if [[ "${1:-}" == "-p" && "${2:-}" == "${TAIRA_TEST_PID:-}" && "${3:-}" == "-o" && "${4:-}" == "command=" ]]; then
    printf '/tmp/iroha3d --config %s/peer0.toml\n' "${TAIRA_TEST_DIST:?}"
    return 0
  fi
